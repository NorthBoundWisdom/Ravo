...
| S9 | common/exif*, metadata*, metadata_export*, tags* | CORE/DATA | Bounded rendered-export packets are accepted under ADR-0038. Remaining: capture timezone/GPS schema, general sidecar/history interchange, shared old consumers, and deletion gates. Originals stay read-only; Exiv2 types remain adapter-private. Do not claim this row complete. |
...
| I11 | imageio/format/jpeg.c | Adapter/export | Core accepted: typed quality/subsampling, ICC APP2, and Catalog-owned Exif APP1 / XMP APP1 / optional IPTC APP13 embed before ADR-0032 publication. CLI and Studio now expose quality plus `auto|444|440|422|420` subsampling. Remaining: capture timezone/GPS; sidecar/history policy; shared consumers; then zero-consumer plugin/registration retirement. Keep the old owner. |
| I12 | imageio/format/png.c | Adapter/export | Core accepted: typed 8/16-bit depth and compression 0–9/default 5 propagate to a bounded private libpng RGB8/RGB16/opaque/non-interlaced ICC + known-built-in-cICP encoder and shared atomic publication. The RGB16 encoder path consumes a real host-endian 16-bit source. Catalog/Qt product PNG16 now renders engine-owned RGB16; an RGB8 source still returns `unsupported_png_16bit_source` without 8-to-16 expansion. PNG-qualified CLI `--png-bit-depth`/`--png-compression` preserve defaults, canonical values, last-value-wins, structured errors, and PNG-only scope. Rendered PNG now also embeds one eXIf TIFF profile and one uncompressed XMP iTXt from the Catalog snapshot. CLI and Studio now expose the typed PNG bit-depth/compression controls. Remaining: capture timezone/GPS and sidecar/history policy (the frozen owner still writes no pHYs); shared imageio/storage/job consumers; then zero-consumer plugin/registration retirement. S9/J6 are not complete. Keep the old owner; this row does not reopen paused algorithm migration. |
| I13 | imageio/format/tiff.c | Adapter/export | Core accepted: typed uint8/uint16/float16/float32 and none/Deflate/predictor with level 1–9/default 6 plus conditional grayscale propagate to a bounded pinned private LibTIFF classic-LE opaque RGB8/RGB16/float contiguous-planar strip/exact-ICC encoder and ADR-0032 atomic publication. Baseline directory metadata now owns 72–9600 inch resolution, bounded normalized-destination DocumentName, and current description/creator/copyright values before publication; EXIFIFD, XMP 700, and optional IPTC 33723 are written through public LibTIFF fields from the same Catalog snapshot. Title stays out of Exif. There is no sidecar change or post-publish Exiv2 rewrite. TIFF-qualified CLI sample/compression/level/grayscale/resolution options preserve their defaults, canonical values, and structured errors; Studio exposes the same typed TIFF options; product uint16/float16/float32 now consume engine-owned RGB16 or finite RGB float. Remaining: capture timezone/GPS and XMP attach/history/sidecar policy; multipage masks; shared imageio/storage/job consumers; then zero-consumer plugin/registration retirement. S9/J6 are not complete. Keep QTiffPlugin for separate I7 input and do not delete the old output owner yet; this row does not reopen paused algorithm migration. |
