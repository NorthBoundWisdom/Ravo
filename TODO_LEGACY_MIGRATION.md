...
| I12 | imageio/format/png.c | Adapter/export | Core accepted: typed 8/16-bit depth and compression 0–9/default 5 propagate to a bounded private libpng RGB8/RGB16/opaque/non-interlaced ICC + known-built-in-cICP encoder and shared atomic publication. The RGB16 encoder path consumes a real host-endian 16-bit source; Catalog/Qt product export still has only RGB8 and returns `unsupported_png_16bit_source` without 8-to-16 expansion. PNG-qualified CLI `--png-bit-depth`/`--png-compression` preserve defaults, canonical values, last-value-wins, structured errors, and PNG-only scope. Remaining: engine-owned RGB16 rendered source for product export; S9/J6 EXIF/XMP packets (the frozen owner does not write pHYs, so resolution is not invented here); shared imageio/storage/job consumers; then zero-consumer plugin/registration retirement. Keep the old owner; this row does not reopen paused algorithm migration. |
| I13 | imageio/format/tiff.c | Adapter/export | Core accepted: typed uint8/uint16/float16/float32 and none/Deflate/predictor with level 1–9/default 6 plus conditional grayscale propagate to a bounded pinned private LibTIFF classic-LE RGB8/opaque/strip/exact-ICC encoder and ADR-0032 atomic publication. Baseline directory metadata now owns 72–9600 inch resolution, bounded normalized-destination DocumentName, and current description/creator/copyright values before publication; it deliberately omits title, EXIFIFD/IPTC/XMP, sidecar changes, and any post-publish Exiv2 rewrite. TIFF-qualified CLI sample/compression/level/grayscale options preserve their defaults, canonical values, structured errors, and high-precision unsupported state. Remaining: real uint16/float16/float32 rendered sources; S9/J6 complete EXIF/IPTC/XMP packets, capture/timezone/GPS, XMP attach/history/sidecar policy; multipage masks; explicit TIFF Studio options; shared imageio/storage/job consumers; then zero-consumer plugin/registration retirement. Keep QTiffPlugin for separate I7 input and do not delete the old output owner yet; this row does not reopen paused algorithm migration. |
