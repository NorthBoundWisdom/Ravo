...
| `ravo.color.colorharmonizer` | `colorharmonizer` | exact 17-field profile-aware dt-UCS/RYB CPU core with canonical-ROI private recursive smoothing; frozen 0176 records 12/13 are zero-smoothing import evidence, while masks/presentation and retirement remain later |
...
| `colorharmonizer` | yes | bounded canonical smoothing accepted by ADR-0042; strict zero-evidence import and product consumers are accepted, while masks/presentation and old-owner retirement remain later |
...
| Masks and blending | S3.1 provides canonical all/linear-gradient/circle/ellipse/parametric/group graphs and normal attachments for Color Harmonizer and Graduated ND; ADR-0044 adds bounded Studio-owned all/spatial/parametric leaf authoring with strict recipe fields and read-only external/shared/group state. Exposure, Color Checker, Color Correction, and Color Contrast retain exact default-unmasked legacy boundaries | Alpha-overlay/group-child presentation, path/brush, further blend modes, M1 acceptance, and strict legacy mask/custom-blend import remain deferred |
