...
| Metadata and GPS | XMP/EXIF plus the frozen product scope. | Read/write fields, orientation/ICC policy, GPS read-only boundary, and unsupported fields. | ADR-0040 accepts typed Catalog capture datetime/offset/GPS from embedded Exif only, persisted in schema v5 and re-embedded on rendered JPEG/PNG/TIFF. Map editing, privacy stripping, sidecar interchange, and metadata refresh remain unresolved. |
...
| Local export | JPEG, PNG, TIFF, and original-file copy. | Format, bit depth, alpha, metadata, ICC, overwrite, and disk-full policy. | Ravo CatalogService/CLI/Studio accept JPEG/PNG/original copy and TIFF. Rendered JPEG/PNG/TIFF embed the Catalog-owned public snapshot including validated capture time/GPS; original-copy stays exact bytes. Sidecar/history policy remains later. |
...
| HSL zoning | `colorequal`, `colorzones`. | One default hue-partition capability and explicit optional-operation overlap policy. | **Keep `colorequal` as default.** Decision/review owner: WisdomToNorth, 2026-08-25. `ravo.color.colorequal` v1 is the frozen dt-UCS 22 eight-node periodic RBF equalizer. ADR-0073 accepts `ravo.color.colorzones` as a separate optional D50 Lab three-curve operation, never a fallback or alias. |
