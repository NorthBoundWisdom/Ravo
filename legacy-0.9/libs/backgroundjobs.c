...
/* proxy functions */
static void *_lib_backgroundjobs_added(dt_lib_module_t *self, gboolean has_progress_bar,
                                       const gchar *message);
...
static void _lib_backgroundjobs_updated(dt_lib_module_t *self,
                                        dt_lib_backgroundjob_element_t *instance, double value);
...
void gui_init(dt_lib_module_t *self)
{
    ...
    // its gui_data!
    for (const GList *iter = darktable.control->progress_system.list; iter;
...
static gboolean _update_gui_thread(gpointer user_data)
{
    ...
    gtk_progress_bar_set_fraction(GTK_PROGRESS_BAR(params->instance->progressbar),
                                  CLAMP(params->value, 0, 1.0));
