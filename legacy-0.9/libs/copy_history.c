...
void gui_update(dt_lib_module_t *self)
{
    ...
    const gboolean can_paste =
        ...
        (act_on_mult ||
         (act_on_one && (darktable.view_manager->copy_paste.copied_imageid != act_on_img)));
...
static void load_button_clicked(GtkWidget *widget, dt_lib_module_t *self)
{
    ...
    GtkFileChooserNative *filechooser =
        gtk_file_chooser_native_new(_("open sidecar file"), GTK_WINDOW(win),
    ...
    if (gtk_native_dialog_run(GTK_NATIVE_DIALOG(filechooser)) == GTK_RESPONSE_ACCEPT)
    {
        ...
        if (dt_history_load_and_apply_on_list(dtfilename, imgs) != 0)
        {
            GtkWidget *dialog = gtk_message_dialog_new(
                GTK_WINDOW(win), GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_ERROR,
...
static void _collection_updated_callback(gpointer instance, dt_collection_change_t query_change,
                                         ...
                                         const gpointer imgs, const int next, dt_lib_module_t *self)
{
    dt_lib_gui_queue_update(self);
}
...
void gui_init(dt_lib_module_t *self)
{
    ...
    d->copy_parts_button =
        dt_action_button_new(self, N_("selective copy..."), copy_parts_button_clicked, self,
                             _("choose which modules to copy from the source image"), GDK_KEY_c,
    ...
    d->paste_parts =
        dt_action_button_new(self, N_("selective paste..."), paste_parts_button_clicked, self,
                             _("choose which modules to paste to the target image(s)"), GDK_KEY_v,
                             GDK_CONTROL_MASK | GDK_SHIFT_MASK);
