...
typedef struct dt_scopes_functions_t
{
    ...
    void (*process)(struct dt_scopes_mode_t *const self, const float *const input,
                    // FIXME: should ROI by dt_histogram_roi_t or another type?
                    dt_histogram_roi_t *const roi, const dt_iop_order_iccprofile_info_t *vs_prof);
    // FIXME: do want a proper clear function or just tag as not up to date?
    void (*clear)(struct dt_scopes_mode_t *const self);
    void (*draw_bkgd)(const struct dt_scopes_mode_t *const self, cairo_t *cr, const int width,
                      const int height);
    void (*draw_grid)(const struct dt_scopes_mode_t *const self, cairo_t *cr, const int width,
                      const int height);
    void (*draw_highlight)(const struct dt_scopes_mode_t *const self, cairo_t *cr,
                           dt_scopes_highlight_t highlight, const int width, const int height);
    void (*draw_scope)(const struct dt_scopes_mode_t *const self, cairo_t *cr, const int width,
                       const int height);
    void (*draw_scope_channels)(const struct dt_scopes_mode_t *const self, cairo_t *cr,
    ...
    double (*get_exposure_delta)(const struct dt_scopes_mode_t *const self, const double offset_x,
                                 const double offset_y);
    void (*append_to_tooltip)(const struct dt_scopes_mode_t *const self, gchar **tip);
    void (*eventbox_scroll)(struct dt_scopes_mode_t *const self, gdouble x, gdouble y,
...
extern void lib_histogram_draw_bkgd(const dt_scopes_mode_t *const self, cairo_t *cr,
                                    const int width, const int height);
