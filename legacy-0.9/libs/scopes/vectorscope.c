...
const gchar *dt_scopes_vec_scale_names[DT_SCOPES_VEC_SCALE_N] = {"logarithmic", "linear"};

const gchar *dt_scopes_vec_vectorscope_type_names[DT_SCOPES_VEC_VECTORSCOPE_N] = {"u*v*", "AzBz",
                                                                                  "RYB"};

const gchar *dt_scopes_vec_color_harmony_width_names[DT_COLOR_HARMONY_WIDTH_N] = {"normal", "large",
                                                                                  "narrow", "line"};
...
typedef struct dt_scopes_vec_t
{
    ...
    GtkWidget
        *color_harmony_button[DT_COLOR_HARMONY_N]; // GtkButton -- RYB vectorscope color harmonies
...
static void _ryb2rgb(const dt_aligned_pixel_t ryb, dt_aligned_pixel_t rgb, const float *ryb2rgb_ypp)
{
    ...
    HSV[0] =
        interpolate_val(sizeof(dt_color_ryb_x_vtx) / sizeof(float), (float *)dt_color_ryb_x_vtx,
                        HSV[0], (float *)rgb_y_vtx, (float *)ryb2rgb_ypp, CUBIC_SPLINE);
...
static void _rgb2ryb(const dt_aligned_pixel_t rgb, dt_aligned_pixel_t ryb, const float *rgb2ryb_ypp)
{
    ...
    HSV[0] =
        interpolate_val(sizeof(dt_color_ryb_x_vtx) / sizeof(float), (float *)dt_color_ryb_x_vtx,
                        HSV[0], (float *)dt_color_ryb_y_vtx, (float *)rgb2ryb_ypp, CUBIC_SPLINE);
                                            ...
                                            const dt_iop_order_iccprofile_info_t *const vs_prof)
{
    ...
    if (vs_prof == d->hue_ring_prof && d->vectorscope_scale == d->hue_ring_scale &&
        d->vectorscope_type == d->hue_ring_colorspace && d->vectorscope_radius != 0.f)
        return;
    ...
    for (int k = 0; k < 6; k++)
    {
        ...
        for_each_channel(ch, aligned(vertex_rgb, delta : 16)) delta[ch] =
            (vertex_rgb[(k + 1) % 6][ch] - vertex_rgb[k][ch]) / VECTORSCOPE_HUES;
        for (int i = 0; i < VECTORSCOPE_HUES; i++)
        {
            dt_aligned_pixel_t rgb_scope, XYZ_D50 = {0}, chromaticity = {0};
            for_each_channel(ch, aligned(vertex_rgb, delta, rgb_scope : 16)) rgb_scope[ch] =
                vertex_rgb[k][ch] + delta[ch] * i;
            switch (vs_type)
            {
            ...
            for_each_channel(ch, aligned(rgb_display : 16)) rgb_display[ch] =
                rgb_display[ch] / max_RGB;
            if (k == 0 && i == 0)
            {
            ...
            else
            {
                ...
                cairo_mesh_pattern_set_corner_color_rgb(p, 0, prev_rgb_display[0],
                                                        prev_rgb_display[1], prev_rgb_display[2]);
                cairo_mesh_pattern_set_corner_color_rgb(p, 1, prev_rgb_display[0],
                                                        prev_rgb_display[1], prev_rgb_display[2]);
                cairo_mesh_pattern_set_corner_color_rgb(p, 2, rgb_display[0], rgb_display[1],
                                                        rgb_display[2]);
                cairo_mesh_pattern_set_corner_color_rgb(p, 3, rgb_display[0], rgb_display[1],
                                                        rgb_display[2]);
            ...
            px = chromaticity[1];
            py = chromaticity[2];
            for_each_channel(ch, aligned(prev_rgb_display, rgb_display : 16)) prev_rgb_display[ch] =
                rgb_display[ch];
        }
    }
    // last patch
    ...
    cairo_mesh_pattern_set_corner_color_rgb(p, 0, prev_rgb_display[0], prev_rgb_display[1],
                                            prev_rgb_display[2]);
    cairo_mesh_pattern_set_corner_color_rgb(p, 1, prev_rgb_display[0], prev_rgb_display[1],
                                            prev_rgb_display[2]);
    cairo_mesh_pattern_set_corner_color_rgb(p, 2, first_rgb_display[0], first_rgb_display[1],
                                            first_rgb_display[2]);
    cairo_mesh_pattern_set_corner_color_rgb(p, 3, first_rgb_display[0], first_rgb_display[1],
                                            first_rgb_display[2]);
    cairo_mesh_pattern_end_patch(p);
    ...
    cairo_surface_t *bkgd_surface = cairo_image_surface_create_for_data(
        d->vectorscope_bkgd, CAIRO_FORMAT_RGB24, diam_px, diam_px,
...
static void _vec_process(dt_scopes_mode_t *const self, const float *const input,
        ...
        for (size_t x = 0; x < sample_max_x; x += 2)
        {
            ...
            for (size_t xx = 0; xx < 2; xx++)
                for (size_t yy = 0; yy < 2; yy++)
                    for_each_channel(ch, aligned(px, RGB : 16)) RGB[ch] +=
                        px[4U * (yy * roi->width + xx) + ch] * 0.25f;
...
static void _vec_draw(const dt_scopes_mode_t *const self, cairo_t *cr, const int width,
                      const int height)
{
    dt_scopes_vec_t *const d = self->data;
    ...
    // background
    cairo_pattern_t *p =
        cairo_pattern_create_radial(0.5 * width, 0.5 * height, 0.5 * min_size, 0.5 * width,
                                    0.5 * height, 0.5 * hypot(min_size, min_size));
    cairo_pattern_add_color_stop_rgb(p, 0., darktable.bauhaus->graph_bg.red,
                                     darktable.bauhaus->graph_bg.green,
                                     darktable.bauhaus->graph_bg.blue);
    cairo_pattern_add_color_stop_rgb(p, 1., darktable.bauhaus->graph_exterior.red,
    ...
    const float grid_radius =
        d->hue_ring_colorspace == DT_SCOPES_VEC_VECTORSCOPE_CIELUV ? 100. : 0.01;
    ...
    // chromaticities for drawing both hue ring and graph
    cairo_surface_t *bkgd_surface = dt_cairo_image_surface_create_for_data(
        d->vectorscope_bkgd, CAIRO_FORMAT_RGB24, diam_px, diam_px,
        cairo_format_stride_for_width(CAIRO_FORMAT_RGB24, diam_px));
    ...
    cairo_matrix_t matrix;
    cairo_matrix_init_translate(&matrix, 0.5 * diam_px / darktable.gui->ppd,
    ...
    cairo_surface_t *graph_surface = dt_cairo_image_surface_create_for_data(
        d->vectorscope_graph, CAIRO_FORMAT_A8, diam_px, diam_px,
    ...
    dt_color_harmony_type_t cur_harmony =
        (is_ryb ? (d->harmony_prelight != DT_COLOR_HARMONY_NONE ? d->harmony_prelight :
    ...
    if (cur_harmony || is_custom_harmony)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < hm.sectors; i++)
            {
                ...
                if (d->vectorscope_scale == DT_SCOPES_VEC_SCALE_LOGARITHMIC)
                    hr = baselog(hr, vs_radius);
                const float span1 = (i > 0 ? MIN(hw, (hm.angle[i] - hm.angle[i - 1]) / 2.f) :
                                             hw); // avoid sectors overlap
                const float span2 =
                    (i < hm.sectors - 1 ? MIN(hw, (hm.angle[i + 1] - hm.angle[i]) / 2.f) : hw);
                const float angle1 =
        ...
        else
        {
            ...
            cairo_paint_with_alpha(
                cr, dt_conf_get_float("plugins/darkroom/histogram/vectorscope/harmony/dim"));
        ...
        if (!is_custom_harmony && gtk_widget_get_visible(self->scopes->button_box_right))
        {
            ...
            cairo_scale(cr, 1., -1.);
            cairo_rotate(cr, -d->vectorscope_angle);
            cairo_move_to(cr, -0.48 * width - ink.x, 0.48 * height - ink.height - ink.y);
...
static void _vec_update_buttons(const dt_scopes_mode_t *const self)
{
    ...
    case DT_SCOPES_VEC_SCALE_LINEAR:
        ...
        dtgtk_button_set_paint(DTGTK_BUTTON(d->vec_scale_button), dtgtk_cairo_paint_linear_scale,
                               CPF_NONE, NULL);
        break;
    ...
    case DT_SCOPES_VEC_VECTORSCOPE_CIELUV:
        ...
        dtgtk_button_set_paint(DTGTK_BUTTON(d->colorspace_button), dtgtk_cairo_paint_luv, CPF_NONE,
                               NULL);
        gtk_widget_hide(d->color_harmony_box);
        break;
    case DT_SCOPES_VEC_VECTORSCOPE_JZAZBZ:
        gtk_widget_set_tooltip_text(d->colorspace_button, _("set view to RYB"));
        dtgtk_button_set_paint(DTGTK_BUTTON(d->colorspace_button), dtgtk_cairo_paint_jzazbz,
                               CPF_NONE, NULL);
        gtk_widget_hide(d->color_harmony_box);
        break;
    case DT_SCOPES_VEC_VECTORSCOPE_RYB:
        gtk_widget_set_tooltip_text(d->colorspace_button, _("set view to u*v*"));
        dtgtk_button_set_paint(DTGTK_BUTTON(d->colorspace_button), dtgtk_cairo_paint_ryb, CPF_NONE,
                               NULL);
...
static void _color_harmony_changed_record(dt_scopes_mode_t *const self)
{
    ...
    if (img)
    {
        memcpy(&img->color_harmony_guide, &d->harmony_guide, sizeof(dt_color_harmony_guide_t));
...
static void _color_harmony_state_changed(GtkWidget *widget, GtkStateFlags flags,
...
static void _color_harmony_toggled(GtkButton *button, dt_scopes_mode_t *const self)
{
    if (!self || !self->data)
        return;
    ...
    for (dt_color_harmony_type_t i = DT_COLOR_HARMONY_NONE + 1; i < DT_COLOR_HARMONY_N; i++)
    {
        if (d->color_harmony_button[i] == GTK_WIDGET(button))
        {
            ...
            else
            {
                ...
                {
                    g_signal_handler_block(d->color_harmony_button[prior],
                                           d->toggle_signal_handler[prior]);
                    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(d->color_harmony_button[prior]),
                                                 FALSE);
                    g_signal_handler_unblock(d->color_harmony_button[prior],
                                             d->toggle_signal_handler[prior]);
                }
...
static void _vec_append_to_tooltip(const dt_scopes_mode_t *const self, gchar **tip)
{
    ...
    if (d->vectorscope_type == DT_SCOPES_VEC_VECTORSCOPE_RYB &&
        ...
        dt_util_str_cat(tip, "\n%s\n%s\n%s\n%s", _("scroll to coarse-rotate"),
                        _("ctrl+scroll to fine rotate"), _("shift+scroll to change width"),
                        _("alt+scroll to cycle"));
}

static void _vec_eventbox_scroll(dt_scopes_mode_t *const self, gdouble x, gdouble y,
                                 gdouble delta_x, gdouble delta_y, GdkModifierType state)
{
    dt_scopes_vec_t *const d = self->data;
    ...
    {
        d->harmony_guide.width =
            (d->harmony_guide.width + delta + DT_COLOR_HARMONY_WIDTH_N) % DT_COLOR_HARMONY_WIDTH_N;
    }
    else if (dt_modifier_is(state, GDK_MOD1_MASK)) // ALT+SCROLL
    {
        ...
        if (new_type == DT_COLOR_HARMONY_NONE)
            ...
            gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(d->color_harmony_button[new_type]),
                                         TRUE);
    }
...
static void _harmony_adjust_page(GtkWidget *widget, GtkAllocation *alloc, dt_scopes_mode_t *self)
{
    dt_scopes_vec_t *const d = self->data;
...
void _vec_signal_image_changed(gpointer instance, dt_scopes_mode_t *const self)
{
    ...
    {
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(d->color_harmony_button[new_guide.type]),
                                     TRUE);
...
static void _vec_gui_init(dt_scopes_mode_t *const self, dt_scopes_t *const scopes)
{
    ...
    d->vectorscope_graph = dt_alloc_align_uint8(
        d->vectorscope_diameter_px *
        cairo_format_stride_for_width(CAIRO_FORMAT_A8, d->vectorscope_diameter_px));
    d->vectorscope_bkgd = dt_alloc_align_uint8(
        4U * d->vectorscope_diameter_px *
...
static void _vec_add_options(dt_scopes_mode_t *const self, dt_action_t *dark)
{
    ...
    dt_action_define(dark, NULL, N_("cycle vectorscope types"), d->colorspace_button,
    ...
    dt_action_define(dark, NULL, N_("switch vectorscope scale"), d->vec_scale_button,
                     &dt_action_def_button);
    ...
    for (dt_color_harmony_type_t i = DT_COLOR_HARMONY_MONOCHROMATIC; i < DT_COLOR_HARMONY_N; i++)
    {
        GtkWidget *rb = dtgtk_togglebutton_new(dtgtk_cairo_paint_color_harmony, CPF_NONE,
                                               &(_vec_color_harmonies[i]));
        dt_action_define(dark, N_("color harmonies"), _vec_color_harmonies[i].name, rb,
                         &dt_action_def_toggle);
        d->toggle_signal_handler[i] = g_signal_connect_data(
            G_OBJECT(rb), "toggled", G_CALLBACK(_color_harmony_toggled), self, NULL, 0);
    ...
    dt_action_register(dark, N_("cycle color harmonies"), _lib_histogram_cycle_harmony_callback, 0,
                       0);

    /* connect callbacks */
    g_signal_connect(G_OBJECT(d->vec_scale_button), "clicked", G_CALLBACK(_vec_scale_clicked),
                     self);
    g_signal_connect(G_OBJECT(d->colorspace_button), "clicked", G_CALLBACK(_vec_colorspace_clicked),
                     self);
    dt_gui_connect_motion(d->harmony_viewport, _harmony_motion, NULL, _harmony_leave, self);
    g_signal_connect(G_OBJECT(colorspace_box), "size-allocate", G_CALLBACK(_harmony_adjust_page),
                     self);
...
void dt_vec_set_harmony(dt_scopes_mode_t *mode, const dt_color_harmony_guide_t *guide)
{
    ...
    {
        g_signal_handler_block(d->color_harmony_button[old_type],
                               d->toggle_signal_handler[old_type]);
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(d->color_harmony_button[old_type]), FALSE);
        g_signal_handler_unblock(d->color_harmony_button[old_type],
                                 d->toggle_signal_handler[old_type]);
    }
    // Activate new button
    if (guide->type != DT_COLOR_HARMONY_NONE)
    {
        g_signal_handler_block(d->color_harmony_button[guide->type],
                               d->toggle_signal_handler[guide->type]);
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(d->color_harmony_button[guide->type]), TRUE);
        g_signal_handler_unblock(d->color_harmony_button[guide->type],
                                 d->toggle_signal_handler[guide->type]);
    }
...
{
    dt_scopes_vec_t *const d = mode->data;
    if (d->vectorscope_type == (dt_scopes_vec_vectorscope_type_t)type)
        return;
