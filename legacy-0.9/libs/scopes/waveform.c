...
static const gchar *dt_wave_orient_names[DT_WAVE_ORIENT_N] = {"horizontal", "vertical"};
...
static void _wave_process(dt_scopes_mode_t *const self, const float *const input,
                          ...
                          const dt_iop_order_iccprofile_info_t *vs_prof)
{
    ...
    const size_t wf_img_stride = cairo_format_stride_for_width(
        CAIRO_FORMAT_A8, orient == DT_WAVE_ORIENT_HORI ? num_bins : num_tones);
    ...
    const float scale =
        brightness /
        ((orient == DT_WAVE_ORIENT_HORI ? sample_height : sample_width) * samples_per_bin);
    ...
    self->scopes->modes[DT_SCOPES_MODE_WAVEFORM].update_counter =
        self->scopes->modes[DT_SCOPES_MODE_PARADE].update_counter = self->scopes->update_counter;
}

static void _wave_draw_grid(const dt_scopes_mode_t *const self, cairo_t *cr, const int width,
                            const int height)
{
    const dt_scopes_wave_t *const d = self->data;
    dt_draw_waveform_lines(cr, 0, 0, width, height, d->orient == DT_WAVE_ORIENT_HORI);
}

static void _wave_draw_highlight(const dt_scopes_mode_t *const self, cairo_t *cr,
                                 dt_scopes_highlight_t highlight, const int width, const int height)
{
    dt_scopes_wave_t *const d = self->data;
...
static void _wave_draw(const dt_scopes_mode_t *const self, cairo_t *cr, const int width,
                       const int height, const scopes_channels_t channels)
{
    const dt_scopes_wave_t *const d = self->data;
    ...
    const int img_width = d->orient == DT_WAVE_ORIENT_HORI ? d->waveform_bins : d->waveform_tones;
    const int img_height = d->orient == DT_WAVE_ORIENT_HORI ? d->waveform_tones : d->waveform_bins;
    ...
    for (int ch = 0; ch < 3; ch++)
        if (channels[ch])
        {
            ...
            cairo_set_source_rgba(crt, ch == 0 ? 1. : 0., ch == 1 ? 1. : 0., ch == 2 ? 1. : 0.,
                                  alpha_chroma);
    ...
    for (int ch = 0; ch < 3; ch++)
        if (cs[ch])
        {
            cairo_set_source_rgba(crt, ch == 0 ? 1. : desat_over, ch == 1 ? 1. : desat_over,
...
static double _wave_get_exposure_delta(const dt_scopes_mode_t *const self, const double offset_x,
...
static void _wave_update_buttons(const dt_scopes_mode_t *const self)
{
    ...
    case DT_WAVE_ORIENT_HORI:
        ...
        dtgtk_button_set_paint(DTGTK_BUTTON(d->orient_button), dtgtk_cairo_paint_arrow,
...
static void _wave_gui_init(dt_scopes_mode_t *const self, dt_scopes_t *const scopes)
{
    ...
    const size_t bytes_hori =
        d->waveform_tones * cairo_format_stride_for_width(CAIRO_FORMAT_A8, d->waveform_max_bins);
    const size_t bytes_vert =
        d->waveform_max_bins * cairo_format_stride_for_width(CAIRO_FORMAT_A8, d->waveform_tones);
    for (int ch = 0; ch < 3; ch++)
...
static void _wave_orient_clicked(GtkWidget *button, dt_scopes_mode_t *const self)
{
    ...
    dt_conf_set_string("plugins/darkroom/histogram/orient", dt_wave_orient_names[d->orient]);
    d->waveform_bins = 0;
...
static void _wave_add_options(dt_scopes_mode_t *const self, dt_action_t *dark)
{
    ...
    dt_action_define(dark, NULL, N_("switch scope orientation"), d->orient_button,
                     &dt_action_def_button);
    dt_gui_box_add(self->options_box, d->orient_button);
    g_signal_connect(G_OBJECT(d->orient_button), "clicked", G_CALLBACK(_wave_orient_clicked), self);
}
...
const dt_scopes_functions_t dt_scopes_functions_waveform = {.name = _wave_name,
...
static void _parade_draw(const dt_scopes_mode_t *const self, cairo_t *cr, const int width,
                         const int height)
{
    const dt_scopes_wave_t *const d = self->data;
    ...
    for (int ch = 0; ch < 3; ch++)
    {
        cairo_surface_t *cs = cairo_image_surface_create_for_data(
            d->waveform_img[ch], CAIRO_FORMAT_A8, img_width, img_height, img_stride);
        cairo_set_source_rgba(crt, ch == 0 ? 1. : 0., ch == 1 ? 1. : 0., ch == 2 ? 1. : 0.,
                              alpha_chroma);
        cairo_set_operator(crt, CAIRO_OPERATOR_ADD);
...
static void _parade_gui_init(dt_scopes_mode_t *const self, dt_scopes_t *const scopes)
{
...
const dt_scopes_functions_t dt_scopes_functions_parade = {.name = _parade_name,
                                                          .process = _wave_process,
                                                          .clear = _wave_clear,
                                                          .draw_bkgd = lib_histogram_draw_bkgd,
                                                          .draw_grid = _wave_draw_grid,
