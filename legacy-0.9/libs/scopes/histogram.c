...
static const gchar *dt_hist_scale_names[DT_HIST_SCALE_N] = {"logarithmic", "linear"};
...
static void _hist_process(dt_scopes_mode_t *const self, const float *const input,
                          ...
                          const dt_iop_order_iccprofile_info_t *vs_prof)
{
    ...
    dt_dev_histogram_stats_t histogram_stats = {.bins_count = HISTOGRAM_BINS,
                                                .ch = 4,
                                                .pixels = 0,
    ...
    dt_histogram_helper(&histogram_params, &histogram_stats, cst, IOP_CS_NONE, input, &d->histogram,
                        histogram_max, FALSE, NULL);
    d->histogram_max = MAX(MAX(histogram_max[0], histogram_max[1]), histogram_max[2]);
    self->update_counter = self->scopes->update_counter;
}

static void _hist_draw_grid(const dt_scopes_mode_t *const self, cairo_t *cr, const int width,
                            const int height)
{
...
static void _hist_draw_highlight(const dt_scopes_mode_t *const self, cairo_t *cr,
...
{
    return posx < BLACK_POINT_REGION ? DT_SCOPES_HIGHLIGHT_BLACK_POINT :
                                       DT_SCOPES_HIGHLIGHT_EXPOSURE;
}

static double _hist_get_exposure_delta(const dt_scopes_mode_t *const self, const double offset_x,
                                       const double offset_y)
{
    return offset_x / gtk_widget_get_allocated_width(self->scopes->scope_draw);
}

static void _hist_draw(const dt_scopes_mode_t *const self, cairo_t *cr, const int width,
                       const int height, const scopes_channels_t channels)
{
    ...
    const float hist_max =
        d->scale == DT_HIST_SCALE_LINEAR ? d->histogram_max : logf(1.0 + d->histogram_max);
    ...
    for (int k = 0; k < DT_SCOPES_RGB_N; k++)
        if (channels[k])
        {
            ...
            dt_draw_histogram_8(cr, d->histogram, 4, k, d->scale == DT_HIST_SCALE_LINEAR);
        }
...
static void _hist_gui_init(dt_scopes_mode_t *const self, dt_scopes_t *const scopes)
{
...
static void _hist_update_buttons(const dt_scopes_mode_t *const self)
{
    ...
    case DT_HIST_SCALE_LOGARITHMIC:
        ...
        dtgtk_button_set_paint(DTGTK_BUTTON(d->scale_button), dtgtk_cairo_paint_logarithmic_scale,
                               CPF_NONE, NULL);
        break;
    case DT_HIST_SCALE_LINEAR:
        gtk_widget_set_tooltip_text(d->scale_button, _("set scale to logarithmic"));
        dtgtk_button_set_paint(DTGTK_BUTTON(d->scale_button), dtgtk_cairo_paint_linear_scale,
                               CPF_NONE, NULL);
        break;
...
static void _hist_scale_clicked(GtkWidget *button, dt_scopes_mode_t *self)
{
    ...
    dt_conf_set_string("plugins/darkroom/histogram/histogram", dt_hist_scale_names[d->scale]);
...
static void _hist_add_options(dt_scopes_mode_t *const self, dt_action_t *dark)
{
    ...
    dt_action_define(dark, NULL, N_("switch histogram scale"), d->scale_button,
                     &dt_action_def_button);
    dt_gui_box_add(self->options_box, d->scale_button);
    g_signal_connect(G_OBJECT(d->scale_button), "clicked", G_CALLBACK(_hist_scale_clicked), self);
}
...
const dt_scopes_functions_t dt_scopes_functions_histogram = {.name = _hist_name,
