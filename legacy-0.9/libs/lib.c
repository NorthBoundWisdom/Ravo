...
static gchar *_get_lib_view_path(const dt_lib_module_t *module, const dt_view_t *cv, char *suffix);

gboolean dt_lib_is_visible_in_view(dt_lib_module_t *module, const dt_view_t *view)
{
    if (!module->views)
    {
        dt_print(DT_DEBUG_ALWAYS, "module %s doesn't have views flags", module->name(module));
        return FALSE;
    }
...
static void _edit_preset(const char *name_in, dt_lib_module_info_t *minfo)
{
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
    ...
    dt_gui_presets_show_edit_dialog(name, rowid, NULL, NULL, TRUE, TRUE, FALSE, GTK_WINDOW(window));
}

static void _menuitem_update_preset(GtkMenuItem *menuitem, dt_lib_module_info_t *minfo)
{
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
        ...
        sqlite3_step(stmt);
        sqlite3_finalize(stmt);
        DT_CONTROL_SIGNAL_RAISE(DT_SIGNAL_PRESETS_CHANGED, g_strdup(minfo->plugin_name));
    }
}

static void _menuitem_new_preset(GtkMenuItem *menuitem, dt_lib_module_info_t *minfo)
{
...
static void _menuitem_edit_preset(GtkMenuItem *menuitem, dt_lib_module_info_t *minfo)
{
    _edit_preset(NULL, minfo);
}

static void _menuitem_manage_presets(GtkMenuItem *menuitem, dt_lib_module_info_t *minfo)
{
    if (minfo->module->manage_presets)
        minfo->module->manage_presets(minfo->module);
}

static void _menuitem_delete_preset(GtkMenuItem *menuitem, dt_lib_module_info_t *minfo)
{
                              ...
                              const int module_version)
{
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2(
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        if (!writeprotect)
            dt_gui_store_last_preset(preset);
    }
    else
        ret = FALSE;
    sqlite3_finalize(stmt);
...
static void _dt_lib_presets_popup_menu_show(dt_lib_module_info_t *minfo, GtkWidget *w)
{
  ...
  gchar *query = g_strdup_printf("SELECT name, op_params, writeprotect, description"
    ...
    else
    {
        ...
        if (darktable.gui->last_preset && found)
        {
            ...
            g_signal_connect(G_OBJECT(mi), "activate", G_CALLBACK(_menuitem_update_preset), minfo);
            gtk_menu_shell_append(GTK_MENU_SHELL(menu), mi);
            g_free(markup);
        }
        cnt++;
...
void dt_lib_init_presets(dt_lib_module_t *module)
{
    ...
    DT_CONTROL_SIGNAL_RAISE(DT_SIGNAL_PRESETS_CHANGED, g_strdup(module->plugin_name));
...
static void dt_lib_init_module(void *m)
{
    ...
    if (darktable.gui)
    {
        ...
        if (module->widget)
        {
            ...
            g_signal_connect(G_OBJECT(module->widget), "draw", G_CALLBACK(_lib_draw_callback),
                             module);
        }
    }
}
...
{
    if (!module->expander || !module->arrow)
        return;
    ...
    snprintf(var, sizeof(var), "plugins/%s/%s/expanded", current_view->module_name,
             module->plugin_name);
    ...
    if (e->button == GDK_BUTTON_PRIMARY)
    {
                 ...
                 !dt_modifier_is(e->state, GDK_SHIFT_MASK))
        {
            ...
            for (const GList *it = darktable.lib->plugins; it; it = g_list_next(it))
            {
                ...
                if (m != module && dt_lib_get_container(m) == side && m->expandable(m) &&
        ...
        {
            /* else just toggle */
            dt_lib_gui_set_expanded(module,
...
static void show_module_callback(dt_lib_module_t *module)
{
    ...
    if (dt_conf_get_bool("lighttable/ui/single_module"))
    {
        ...
        for (const GList *it = darktable.lib->plugins; it; it = g_list_next(it))
        {
            ...
            {
                all_other_closed =
                    all_other_closed && !dtgtk_expander_get_expanded(DTGTK_EXPANDER(m->expander));
                dt_lib_gui_set_expanded(m, FALSE);
            }
        }
        if (all_other_closed)
                                ...
                                dt_lib_module_t *dest)
{
    ...
    else
    {
        ...
        below = y > (ABS(dest_pos - src_pos) == 1 && !dtgtk_expander_get_expanded(exp) ?
                         header_height / 2 :
                     dest_pos < src_pos ? gtk_widget_get_allocated_height(widget) - header_height :
                                          header_height);
...
static gboolean _on_drag_drop(GtkWidget *widget, GdkDragContext *dc, gint x, gint y, guint time,
                              dt_lib_module_t *dest)
{
...
GtkWidget *dt_lib_gui_get_expander(dt_lib_module_t *module)
{
        ...
        container == DT_UI_CONTAINER_PANEL_RIGHT_CENTER)
    {
        ...
        gtk_drag_dest_set(expander, GTK_DEST_DEFAULT_DROP | GTK_DEST_DEFAULT_HIGHLIGHT, target_list,
                          1, GDK_ACTION_COPY);
...
void dt_lib_init(dt_lib_t *lib)
{
    ...
    darktable.lib->plugins =
        dt_module_load_modules("/plugins/lighttable", sizeof(dt_lib_module_t), dt_lib_load_module,
                               dt_lib_init_module, dt_lib_sort_plugins);
...
{
    return dt_lib_is_visible_in_view(module,
...
{
    if (!lib || !lib->proxy.histogram.module || !lib->proxy.histogram.get_harmony)
        return;
...
{
    if (!lib || !lib->proxy.histogram.module || !lib->proxy.histogram.set_harmony)
        return;
...
void dt_lib_histogram_set_harmony_callback(dt_lib_t *lib,
                                           ...
                                           void *user_data)
{
    if (!lib || !lib->proxy.histogram.module || !lib->proxy.histogram.set_harmony_callback)
        return;
...
{
    if (!lib || !lib->proxy.histogram.module || !lib->proxy.histogram.set_scope)
        return;
...
{
    if (!lib || !lib->proxy.histogram.module || !lib->proxy.histogram.set_type)
        return;
                                        ...
                                        const int rotation, float *angles, int *n)
{
    if (n)
        *n = 0;
    if (!lib || !lib->proxy.histogram.module || !lib->proxy.histogram.get_sector_angles)
        return;
