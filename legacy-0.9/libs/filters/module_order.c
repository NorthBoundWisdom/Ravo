...
static void _module_order_changed(GtkWidget *widget, gpointer user_data)
{
    ...
    if (module_order->rule->manual_widget_set)
        return;
...
static gboolean _module_order_update(dt_lib_filtering_rule_t *rule)
{
  ...
  g_snprintf(query, sizeof(query),
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        const int v =
            (sqlite3_column_bytes(stmt, 0) == 0) ? DT_IOP_ORDER_LAST : sqlite3_column_int(stmt, 0);
        counts[v] = count;
    }
                                      ...
                                      dt_lib_module_t *self)
{
    ...
    if (!_module_order_names)
    {
        ...
        for (int i = 0; i < DT_IOP_ORDER_LAST; i++)
            _module_order_names[i + 1] = g_strdup(N_(dt_iop_order_string(i)));
