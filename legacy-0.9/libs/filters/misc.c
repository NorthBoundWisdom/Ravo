...
static void _misc_changed(GtkWidget *widget, gpointer user_data)
{
    ...
    if (misc->rule->manual_widget_set)
        return;
...
void _misc_tree_update(_widgets_misc_t *misc)
{
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        else
        {
            ...
            gtk_list_store_insert_with_values(GTK_LIST_STORE(name_model), NULL, -1, TREE_COL_TEXT,
                                              name, TREE_COL_TOOLTIP, name, TREE_COL_PATH,
                                              value_path, TREE_COL_COUNT, count, -1);
            g_free(value_path);
        }
    }
    ...
    {
        gtk_list_store_insert_with_values(GTK_LIST_STORE(name_model), NULL, -1, TREE_COL_TEXT,
                                          _("unnamed"), TREE_COL_TOOLTIP, tooltip, TREE_COL_PATH,
                                          _("unnamed"), TREE_COL_COUNT, unset, -1);
    }
    g_free(tooltip);
    misc->tree_ok = TRUE;
}

void _misc_tree_update_visibility(GtkWidget *w, _widgets_misc_t *misc)
{
    if (!misc->tree_ok)
        _misc_tree_update(misc);
}

static gboolean _misc_select_func(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter,
                                  gpointer data)
{
...
static void _misc_update_selection(_widgets_misc_t *misc)
{
    ...
    if (g_strcmp0(txt, ""))
    {
        ...
        gtk_tree_model_foreach(gtk_tree_view_get_model(GTK_TREE_VIEW(misc->name_tree)),
                               _misc_select_func, sel);
        g_strfreev(elems);
    }
    misc->internal_change--;
}
...
static void _misc_popup_closed(GtkWidget *w, _widgets_misc_t *misc)
{
    // we validate the corresponding entry
    gtk_widget_activate(gtk_popover_get_default_widget(GTK_POPOVER(w)));
}

static void _misc_tree_row_activated(GtkTreeView *self, GtkTreePath *path,
                                     GtkTreeViewColumn *column, _widgets_misc_t *misc)
{
    // as the selection as already been updated on first click, we just close the popup
    gtk_widget_hide(misc->pop);
}

static void _misc_tree_selection_changed(GtkTreeSelection *sel, _widgets_misc_t *misc)
{
    if (!misc || misc->internal_change)
        return;
...
static void _misc_ok_clicked(GtkWidget *w, _widgets_misc_t *misc)
{
    gtk_widget_hide(misc->pop);
}

void _misc_tree_count_func(GtkTreeViewColumn *col, GtkCellRenderer *renderer, GtkTreeModel *model,
                              ...
                              const gchar *text, dt_lib_module_t *self)
{
    ...
    g_signal_connect(G_OBJECT(misc->name_tree), "row-activated",
                     G_CALLBACK(_misc_tree_row_activated), misc);
