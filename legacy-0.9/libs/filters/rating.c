...
static void _rating_legacy_changed(GtkWidget *widget, gpointer user_data)
{
    ...
    if (rating_legacy->rule->manual_widget_set)
        return;
                                ...
                                dt_lib_module_t *self)
{
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(rating_legacy->comparator, self, N_("rules"), N_("comparator"),
                                 _("filter by images rating"), 3, _rating_legacy_changed,
                                 rating_legacy, "<", "≤", "=", "≥", ">", "≠");
    ...
    gtk_overlay_set_overlay_pass_through(GTK_OVERLAY(rating_legacy->overlay),
                                         rating_legacy->comparator, TRUE);

    /* create the filter combobox */
    DT_BAUHAUS_COMBOBOX_NEW_FULL(
        rating_legacy->stars, self, N_("rules"), N_("ratings"), _("filter by images rating"), 0,
