...
static gboolean _colors_update(dt_lib_filtering_rule_t *rule)
{
    ...
    {
        const int i_mask = mask & mask_excluded ? CPF_USER_DATA_EXCLUDE :
                           mask & mask_included ? CPF_USER_DATA_INCLUDE :
                                                  0;
...
{
    if (!target || element >= DT_COLORLABELS_LAST + 2)
...
const dt_action_element_def_t _action_elements_colors[] = {
    {N_("operator"), dt_action_effect_toggle}, {N_("red"), dt_action_effect_toggle},
    {N_("yellow"), dt_action_effect_toggle},   {N_("green"), dt_action_effect_toggle},
    {N_("blue"), dt_action_effect_toggle},     {N_("purple"), dt_action_effect_toggle},
    {N_("all"), dt_action_effect_toggle},      {NULL}};

const dt_action_def_t dt_action_def_colors_rule = {N_("color filter"), _action_process_colors,
                                ...
                                dt_lib_module_t *self)
{
    ...
    for (int k = 0; k < DT_COLORLABELS_LAST + 1; k++)
    {
        ...
        gtk_widget_set_tooltip_text(colors->colors[k],
    ...
    gtk_widget_set_tooltip_text(
        colors->operator, _("filter by images color label"
    ...
    dt_shortcut_register(ac, DT_COLORLABELS_RED + 1, DT_ACTION_EFFECT_TOGGLE, GDK_KEY_F1,
                         GDK_SHIFT_MASK);
    dt_shortcut_register(ac, DT_COLORLABELS_YELLOW + 1, DT_ACTION_EFFECT_TOGGLE, GDK_KEY_F2,
                         GDK_SHIFT_MASK);
    dt_shortcut_register(ac, DT_COLORLABELS_GREEN + 1, DT_ACTION_EFFECT_TOGGLE, GDK_KEY_F3,
                         GDK_SHIFT_MASK);
    dt_shortcut_register(ac, DT_COLORLABELS_BLUE + 1, DT_ACTION_EFFECT_TOGGLE, GDK_KEY_F4,
                         GDK_SHIFT_MASK);
