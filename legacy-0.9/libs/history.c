...
static gboolean _lib_history_compress_pressed_callback(GtkWidget *widget, GdkEventButton *e,
                                                       gpointer user_data);
...
static void _lib_history_module_remove_callback(gpointer instance, dt_iop_module_t *module,
                                                gpointer user_data);
...
void gui_init(dt_lib_module_t *self)
{
    ...
    dt_action_define(DT_ACTION(self), NULL, N_("create style from history"), d->create_button,
                     &dt_action_def_button);

    self->widget =
        dt_gui_vbox(dt_ui_resize_wrap(d->history_box, 1, "plugins/darkroom/history/windowheight"),
    ...
    DT_CONTROL_SIGNAL_HANDLE(DT_SIGNAL_DEVELOP_HISTORY_WILL_CHANGE,
...
static void _reset_module_instance(GList *hist, dt_iop_module_t *module, const int multi_priority)
{
    for (; hist; hist = g_list_next(hist))
    {
        dt_dev_history_item_t *hit = hist->data;

        if (!hit->module && strcmp(hit->op_name, module->op) == 0 &&
...
static void _undo_items_cb(gpointer user_data, const dt_undo_type_t type, dt_undo_data_t data)
{
...
static void _history_invalidate_cb(gpointer user_data, const dt_undo_type_t type,
                                   dt_undo_data_t data)
{
...
static gboolean _check_deleted_instances(dt_develop_t *dev, GList **_iop_list, GList *history_list)
{
    ...
    while (modules)
    {
        ...
        if (mod->multi_priority == 0)
        {
            ...
            if (modules_next)
            {
                ...
                if (strcmp(mod_next->op, mod->op) == 0 && mod_next->multi_priority == 0)
                {
                    ...
                    const gboolean mod_in_history =
                        _search_history_by_module(history_list, mod) != NULL;
                    ...
                    else
                    {
                        if (mod_in_history && mod_next_in_history)
                            dt_print(DT_DEBUG_ALWAYS,
                                     ...
                                     mod->op, mod->multi_name, mod->multi_priority, mod_next->op,
                                     mod_next->multi_name, mod_next->multi_priority);
                        else
                            dt_print(DT_DEBUG_ALWAYS,
                                     "[_check_deleted_instances] found duplicate module"
        ...
        if (delete_module)
        {
            ...
            dt_undo_iterate(darktable.undo, DT_UNDO_HISTORY, mod, &_history_invalidate_cb);
...
static gboolean _create_deleted_modules(GList **_iop_list, GList *history_list)
{
    ...
    while (l)
    {
        ...
        if (hitem->module == NULL)
        {
            ...
            if (base_module == NULL)
            {
                dt_print(DT_DEBUG_ALWAYS, "[_create_deleted_modules] can't find base module for %s",
                         hitem->op_name);
                return changed;
            }
...
static void _pop_undo(gpointer user_data, dt_undo_type_t type, dt_undo_data_t data,
                      dt_undo_action_t action, GList **imgs)
{
...
static void _lib_history_will_change_callback(gpointer instance, dt_lib_module_t *self)
{
    ...
    gtk_container_foreach(GTK_CONTAINER(lib->history_box), (GtkCallback)gtk_widget_set_has_tooltip,
                          NULL);
    ...
    if (lib->record_history_level++ == 0 && lib->record_undo)
    {
        ...
        hist->iop_order_list = dt_ioppr_iop_order_copy_deep(darktable.develop->iop_order_list);
...
static gchar *_lib_history_bauhaus_text(gpointer bhfield, const char *d, dt_iop_module_t *module,
                                        float oval, float pval)
{
    for (GSList *w = module->widget_list_bh; w; w = g_slist_next(w))
    {
        ...
        if (dt_bauhaus_widget_get_field(widget) == bhfield)
        {
            ...
            gchar *s = as && as->type == DT_ACTION_TYPE_SECTION ?
                           g_strdup_printf("%s/%s", as->label, d) :
                           NULL;
...
#define CHANGE_TEXT_BAUHAUS(format, fieldtype)                                                       \
    ...
    case DT_INTROSPECTION_TYPE_UNION:
    {
        ...
        for (int i = 0; i < field->Struct.entries; i++)
        {
            ...
            gchar *description = _(*entry->header.description ? entry->header.description :
                                                                entry->header.field_name);

            if (d)
                description = g_strdup_printf("%s.%s", d, description);
    ...
    case DT_INTROSPECTION_TYPE_ARRAY:
        if (field->Array.type == DT_INTROSPECTION_TYPE_CHAR)
        {
            const gboolean is_valid =
                g_utf8_validate((char *)o, -1, NULL) && g_utf8_validate((char *)p, -1, NULL);
        ...
        else
        {
                 ...
                 i++, item_offset += field->Array.field->header.size)
            {
                ...
                char *element_text = _lib_history_change_text(
                    field->Array.field, description, module, (uint8_t *)params + item_offset,
    ...
    case DT_INTROSPECTION_TYPE_FLOAT:
        if (!feqf(*(float *)o, *(float *)p, 1e-6) &&
            (dt_isfinite(*(float *)o) || dt_isfinite(*(float *)p)))
            // clang-format off
...
static gboolean _changes_tooltip_callback(GtkWidget *widget, const gint x, const gint y,
                                          ...
                                          const dt_dev_history_item_t *hitem)
{
    ...
    for (const GList *find_old = darktable.develop->history; find_old && find_old->data != hitem;
...
#define add_blend_history_change_enum(field, label, list)                                          \
    ...
    {                                                                                              \
        ...
        {                                                                                          \
            if (i->value == (old_blend->field))                                                    \
                old_str = i->name;                                                                 \
            if (i->value == (hitem->blend_params->field))                                          \
                new_str = i->name;                                                                 \
        }                                                                                          \
                                                                                                   \
        change_parts[num_parts++] =                                                                \
        ...
        add_blend_history_change_enum(mask_combine & (DEVELOP_COMBINE_INV | DEVELOP_COMBINE_INCL),
                                      _("combine masks"), dt_develop_combine_masks_names);
        ...
        const char new_combine =
            hitem->blend_params->mask_combine & DEVELOP_COMBINE_MASKS_POS ? '+' : '-';
        ...
        for (int in_out = 1; in_out >= 0; in_out--)
        {
                 ...
                 b && b->label != NULL; b++)
            {
                    ...
                    (memcmp(of, nf, sizeof(float) * 4) || opolarity != npolarity))
                {
                    if (first)
                    {
                        change_parts[num_parts++] = g_strdup(in_out ? _("parametric output mask:") :
    ...
    if (show_tooltip)
    {
        ...
        PangoTabArray *tabs = pango_tab_array_new_with_positions(
            5, FALSE, PANGO_TAB_LEFT, char_width * (t[0]), PANGO_TAB_DECIMAL,
            char_width * (t[0] + t[1] + 1), PANGO_TAB_LEFT, char_width * (t[0] + t[3] + 1),
            PANGO_TAB_LEFT, char_width * (t[0] + t[3] + 3), PANGO_TAB_DECIMAL,
            char_width * (t[0] + t[3] + 4 + t[4]));
...
static gchar *_lib_history_button_label(const dt_dev_history_item_t *item)
{
    ...
    if (!item)
        ...
        label = dt_history_get_name_label(item->module->name(), item->multi_name, TRUE,
...
void gui_update(dt_lib_module_t *self)
{
    ...
    for (const GList *history = darktable.develop->history; history; history = g_list_next(history))
    {
        ...
        g_signal_connect(G_OBJECT(widget), "query-tooltip", G_CALLBACK(_changes_tooltip_callback),
                         (void *)hitem);
