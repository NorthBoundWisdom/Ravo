...
/** Updates the combo box and shows only the supported formats of
 * current selected storage module */
...
static inline uint32_t print2pixels(dt_lib_export_t *self, const float value)
{
    const dt_dimensions_type_t d_type =
        (dt_dimensions_type_t)dt_bauhaus_combobox_get(self->dimensions_type);
    switch (d_type)
    {
...
void gui_update(dt_lib_module_t *self)
{
    ...
    const int format_index =
        dt_imageio_get_index_of_format(dt_imageio_get_format_by_name(format_name));
    ...
    gtk_widget_set_sensitive(GTK_WIDGET(d->export_button), has_act_on && format_index != -1 &&
                                                               storage_index != -1 &&
                                                               export_enabled);
}
...
static void _export_with_current_settings(dt_lib_module_t *self)
{
    ...
    // get the format_name and storage_name settings which are plug-ins
    // name and not necessary what is displayed on the combobox.  note
    // that we cannot take directly the combobox entry index as
    // depending on the storage some format are not listed.
    const char *format_name = dt_conf_get_string_const(CONFIG_PREFIX "format_name");
    const char *storage_name = dt_conf_get_string_const(CONFIG_PREFIX "storage_name");
    const int format_index =
        dt_imageio_get_index_of_format(dt_imageio_get_format_by_name(format_name));
    ...
    const gboolean scaledimension =
        dt_conf_get_int(CONFIG_PREFIX "dimensions_type") == DT_DIMENSIONS_SCALE;
    ...
    // scaling
    const gboolean is_scaling = dt_conf_is_equal("plugins/lighttable/export/resizing", "scaling");
    ...
    dt_control_export(list, max_width, max_height, format_index, storage_index, high_quality,
                      upscale, scaledimension, is_scaling, scale_factor, export_masks, style,
                      style_append, icc_type, icc_filename, icc_intent, d->metadata_export);
...
static void _export_with_preset(const gchar *preset_name, dt_lib_module_t *self)
{
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        if (login)
            ...
            dt_control_log(_("could not login to storage `%s'!"),
                           d->storage_module->name(d->storage_module));
    }
...
static void _batch_export_button_clicked(GtkWidget *widget, dt_lib_module_t *self)
{
    ...
    while (valid)
    {
        ...
        gtk_tree_model_get(model, &iter, DT_EXPORT_BATCH_COL_ACTIVE, &active,
                           DT_EXPORT_BATCH_COL_NAME, &preset_name, -1);
...
{
    const dt_dimensions_type_t d_type =
        (dt_dimensions_type_t)dt_bauhaus_combobox_get(d->dimensions_type);
    ...
    else
    {
        ...
        snprintf(size_in_px_txt, sizeof(size_in_px_txt) / sizeof(size_in_px_txt[0]),
                 _("which is equal to %s × %s px"), gtk_entry_get_text(GTK_ENTRY(d->width)),
                 gtk_entry_get_text(GTK_ENTRY(d->height)));
        gtk_label_set_text(GTK_LABEL(d->size_in_px), size_in_px_txt);
    }
}

void _set_dimensions(dt_lib_export_t *d, const uint32_t max_width, const uint32_t max_height,
                     const uint32_t print_dpi, const gchar *scale)
{
...
void _size_update_display(dt_lib_export_t *self)
{
    ...
    gtk_widget_set_visible(self->print_size,
                           d_type == DT_DIMENSIONS_CM || d_type == DT_DIMENSIONS_INCH);
...
void gui_reset(dt_lib_module_t *self)
{
    ...
    dt_bauhaus_combobox_set(d->dimensions_type,
    ...
    // Set default storage
    const int storage_index = dt_imageio_get_index_of_storage(
        dt_imageio_get_storage_by_name(dt_confgen_get(CONFIG_PREFIX "storage_name", DT_DEFAULT)));
    dt_bauhaus_combobox_set(d->storage, storage_index);

    // Set default format
    const int format_index = dt_imageio_get_index_of_format(
        dt_imageio_get_format_by_name(dt_confgen_get(CONFIG_PREFIX "format_name", DT_DEFAULT)));
    dt_bauhaus_combobox_set(d->format, format_index);

    dt_bauhaus_combobox_set(d->upscale,
                            dt_confgen_get_bool(CONFIG_PREFIX "upscale", DT_DEFAULT) ? 1 : 0);
    dt_bauhaus_combobox_set(
        d->high_quality,
        dt_confgen_get_bool(CONFIG_PREFIX "high_quality_processing", DT_DEFAULT) ? 1 : 0);
    dt_bauhaus_combobox_set(d->export_masks,
                            dt_confgen_get_bool(CONFIG_PREFIX "export_masks", DT_DEFAULT) ? 1 : 0);

    dt_bauhaus_combobox_set(d->intent,
                            dt_confgen_get_int(CONFIG_PREFIX "iccintent", DT_DEFAULT) + 1);
...
static void set_format_by_name(dt_lib_export_t *d, const char *name)
{
    ...
    // only some modules support export of masks set it to 0 when
    // insensitive and restore when making it sensitive again. this
    // doesn't survive dt restarts.
    const gboolean support_layers =
    ...
    {
        // combobox was disabled and shall be enabled. we want to restore
        // the old setting.
        const gboolean export_masks = dt_conf_get_bool(CONFIG_PREFIX "export_masks");
    ...
    {
        // combobox was enabled but shall be disabled. we want to save the
        // current setting.
...
static void _dimensions_type_changed(GtkWidget *widget, dt_lib_export_t *d)
{
    ...
    else
    {
        ...
        dt_conf_set_int("metadata/resolution",
                        dt_confgen_get_int("metadata/resolution", DT_DEFAULT));
    }
    _size_update_display(d);
}
...
static void _update_style_label(dt_lib_export_t *d, const char *name)
{
    ...
    char *localized_style =
        name[0] ? dt_util_localize_segmented_name(name, TRUE) : g_strdup(_("none"));
    ...
    char *tooltip =
        g_strdup_printf(_("style to be applied on export:\n<b>%s</b>"), localized_style);
...
static gboolean _apply_style_button_callback(GtkMenuItem *menuitem, GdkEventButton *event,
                                             const dt_stylemenu_data_t *menu_data)
{
...
static void _style_popupmenu_callback(GtkWidget *w, gpointer user_data)
{
    ...
    GtkMenuShell *menu = dtgtk_build_style_menu_hierarchy(TRUE, _apply_style_activate_callback,
                                                          _apply_style_button_callback, user_data);
    if (menu)
    {
...
static gboolean _batch_preset_active(dt_lib_module_t *self)
{
    ...
    {
        gboolean active;
        gtk_tree_model_get(model, &iter, DT_EXPORT_BATCH_COL_ACTIVE, &active, -1);
        if (active)
        {
...
static void _batch_export_toggled_callback(GtkCellRendererToggle *cell_renderer, gchar *path_str,
                                           gpointer user_data)
{
    ...
    gtk_tree_model_get(model, &iter, DT_EXPORT_BATCH_COL_ACTIVE, &toggle, DT_EXPORT_BATCH_COL_NAME,
                       &name, -1);
    toggle = !toggle;
...
static void _fill_batch_export_list(dt_lib_module_t *self)
{
    ...
    while (valid)
    {
        ...
        gtk_tree_model_get(model, &iter, DT_EXPORT_BATCH_COL_NAME, &preset_name, -1);
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        gtk_list_store_insert_with_values(GTK_LIST_STORE(model), NULL, -1,
                                          ...
                                          DT_EXPORT_BATCH_COL_NAME, name, -1);
        has_active |= active;

        // remove unchanged preset names from the list so the list will
        // contain only preset names which no longer exist
        for (GList *preset_iter = nonexistent_presets; preset_iter;
             preset_iter = g_list_next(preset_iter))
        {
...
static void _export_presets_changed_callback(gpointer instance, gpointer module,
...
void gui_init(dt_lib_module_t *self)
{
    ...
    g_signal_connect(G_OBJECT(d->storage), "value-changed", G_CALLBACK(_storage_changed), self);
    ...
    g_signal_connect(G_OBJECT(d->format), "value-changed", G_CALLBACK(_format_changed),
                     (gpointer)d);
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(d->dimensions_type, self, NULL, N_("set size"),
                                 ...
                                 _dimensions_type_changed, d, N_("in pixels (for file)"),
                                 N_("in cm (for print)"), N_("in inch (for print)"),
                                 N_("by scale (for file)"));

    d->print_width =
        dt_action_entry_new(DT_ACTION(self), N_("print width"), G_CALLBACK(_print_width_changed), d,
                            _("maximum output width limit.\n"
                              "click middle mouse button to reset to 0."),
                            NULL);

    d->print_height = dt_action_entry_new(DT_ACTION(self), N_("print height"),
                                          G_CALLBACK(_print_height_changed), d,
                                          _("maximum output height limit.\n"
                                            "click middle mouse button to reset to 0."),
                                          NULL);

    d->print_dpi = dt_action_entry_new(DT_ACTION(self), N_("dpi"), G_CALLBACK(_print_dpi_changed),
                                       d, _("resolution in dot per inch"),
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(d->upscale, self, NULL, N_("allow upscaling"), NULL,
                                 ...
                                 (gpointer)CONFIG_PREFIX "upscale", N_("no"), N_("yes"));
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(d->high_quality, self, NULL, N_("high quality resampling"),
                                 ...
                                 _callback_bool, (gpointer)CONFIG_PREFIX "high_quality_processing",
                                 N_("no"), N_("yes"));

    DT_BAUHAUS_COMBOBOX_NEW_FULL(
        d->export_masks, self, NULL, N_("store masks"),
        ...
        (gpointer)CONFIG_PREFIX "export_masks", N_("no"), N_("yes"));
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(
        ...
        0, _intent_changed, self, N_("image settings"), N_("perceptual"),
        N_("relative colorimetric"), NC_("rendering intent", "saturation"),
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(
        ...
        (gpointer)CONFIG_PREFIX "style_append", N_("replace history"), N_("append history"));
    ...
    g_signal_connect(G_OBJECT(styles_button), "clicked", G_CALLBACK(_style_popupmenu_callback),
                     (gpointer)d);
    ...
    g_signal_connect(G_OBJECT(d->profile), "value-changed", G_CALLBACK(_profile_changed),
                     (gpointer)d);

    // Export button
    d->export_button = GTK_BUTTON(dt_action_button_new(self, NC_("actionbutton", "start export"),
                                                       _export_button_clicked, self, NULL,
                                                       GDK_KEY_e, GDK_CONTROL_MASK));

    self->widget = dt_gui_vbox(
        dt_ui_section_label_new(C_("section", "storage options")), d->storage,
        d->storage_extra_container, dt_ui_section_label_new(C_("section", "format options")),
        d->format, d->format_extra_container,
        dt_ui_section_label_new(C_("section", "global options")), d->dimensions_type, d->px_size,
        d->print_size, d->scale, d->size_in_px, d->upscale, d->high_quality, d->export_masks,
        d->profile, d->intent, style_box, d->style_mode, d->export_button);

    // multi-preset export
    dt_gui_new_collapsible_section(&d->cs, "plugins/lighttable/export/batch_export_expanded",
                                   _("multi-preset export"), GTK_BOX(self->widget),
                                   DT_ACTION(self));
    gtk_widget_set_tooltip_text(d->cs.expander,
                                _("export the selected images with multiple presets"));
    GtkListStore *store =
        gtk_list_store_new(DT_EXPORT_BATCH_NUM_COLS, G_TYPE_BOOLEAN, G_TYPE_STRING);
    ...
    g_signal_connect(renderer, "toggled", G_CALLBACK(_batch_export_toggled_callback), self);
    ...
    d->batch_export_button =
        GTK_BUTTON(dt_action_button_new(self, NC_("actionbutton", "start batch export"),
                                        _batch_export_button_clicked, self, NULL, 0, 0));
    ...
    const int storage_index =
        dt_imageio_get_index_of_storage(dt_imageio_get_storage_by_name(setting));
    dt_bauhaus_combobox_set(d->storage, storage_index);
...
void *get_params(dt_lib_module_t *self, int *size)
{
    ...
    // size will be only as large as needed to remove random pointers
    // from params (stored at the end).
    ...
    // we allow null pointers (plugin not ready for export in current
    // state), and just don't copy back the settings later:
    if (!sdata)
        ssize = 0;
    if (!fdata)
        fsize = 0;
    if (fdata)
    {
        // clean up format global params (need to set all bytes to
        // reliably detect which preset is active).  we happen to want to
        // set it all to 0
        memset(fdata, 0, sizeof(dt_imageio_module_data_t));
    }

    // FIXME: also the web preset has to be applied twice to be known as
    // preset! (other dimension magic going on here?)  TODO: get this
    // stuff from gui and not from conf, so it will be sanity-checked
    // (you can never delete an insane preset)?  also store icc
    // profile/intent here.
    ...
    *size = fname_len + 1 + sname_len + 1 + 4 * sizeof(int32_t) + fsize + ssize +
            9 * sizeof(int32_t) + strlen(scale) + 1 + strlen(iccfilename) + 1 +
...
int set_params(dt_lib_module_t *self, const void *params, const int size)
{
    ...
    if (size != strlen(fname) + 1 + strlen(sname) + 1 + 4 * sizeof(int32_t) + fsize + ssize +
                    9 * sizeof(int32_t) + strlen(scale) + 1 + strlen(metadata_export) + 1 +
