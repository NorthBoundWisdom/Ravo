...
static void _add_selected_metadata(gchar *tagname, dt_lib_export_metadata_t *d)
{
    ...
    {
        gtk_list_store_insert_with_values(d->liststore, &iter, -1, DT_LIB_EXPORT_METADATA_COL_XMP,
                                          tagname, DT_LIB_EXPORT_METADATA_COL_FORMULA, "", -1);
...
static void _metadata_activated(GtkTreeView *tree_view, GtkTreePath *path,
                                GtkTreeViewColumn *column, dt_lib_export_metadata_t *d)
{
...
static gboolean _key_press_on_list(GtkWidget *widget, GdkEventKey *event,
...
static void _formula_edited(GtkCellRenderer *renderer, gchar *path, gchar *new_text,
                            dt_lib_export_metadata_t *d)
{
    GtkTreeIter iter;
...
char *dt_lib_export_metadata_configuration_dialog(char *metadata_presets, const gboolean ondisk)
{
    ...
    GtkWidget *dialog = gtk_dialog_new_with_buttons(
        _("edit metadata exportation"), GTK_WINDOW(win), GTK_DIALOG_DESTROY_WITH_PARENT,
    ...
    gtk_widget_set_tooltip_text(dtmetadata,
                                _("export darktable XMP metadata (from metadata editor module)"));
    ...
    if (!ondisk)
    {
        ...
        gtk_widget_set_tooltip_text(
            calculated,
            _("per default the interface sends some (limited) metadata beside the image to remote storage.\n"
    ...
    gtk_widget_set_tooltip_text(
        d->omithierarchy,
        _("only the last part of the hierarchical tags is included. can be useful if categories are not used"));
    ...
    gtk_widget_set_tooltip_text(hierarchical,
                                _("export hierarchical tags (to Xmp.lr.Hierarchical Subject)"));
    GtkWidget *dthistory = gtk_check_button_new_with_label(_("develop history"));
    gtk_widget_set_tooltip_text(
        dthistory,
        _("export darktable development data (recovery purpose in case of loss of database or XMP file)"));
    ...
    GtkTreeViewColumn *col =
        gtk_tree_view_column_new_with_attributes(_("redefined tag"), renderer, "text", 0, NULL);
    ...
    g_signal_connect(G_OBJECT(view), "key_press_event", G_CALLBACK(_key_press_on_list),
                     (gpointer)d);

    GtkListStore *liststore = gtk_list_store_new(3, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING);
    d->liststore = liststore;
    gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(liststore),
                                         DT_LIB_EXPORT_METADATA_COL_XMP, GTK_SORT_ASCENDING);
    ...
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(d->omithierarchy),
                                 flags & DT_META_OMIT_HIERARCHY);
    ...
    if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT)
    {
        ...
        const gint newflags =
            ((gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(exiftag)) ? DT_META_EXIF : 0) |
             ...
             (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(d->private)) ? DT_META_PRIVATE_TAG :
                                                                            0) |
             (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(d->synonyms)) ? DT_META_SYNONYMS_TAG :
                                                                             0) |
             (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(d->omithierarchy)) ?
                  DT_META_OMIT_HIERARCHY :
                  0) |
             (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(hierarchical)) ?
                  DT_META_HIERARCHICAL_TAG :
                  0) |
             (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(dthistory)) ? DT_META_DT_HISTORY : 0) |
             (!ondisk ? (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(calculated)) ?
                             DT_META_CALCULATED :
                             0) :
                        0));
        ...
        while (valid)
        {
            ...
            gtk_tree_model_get(GTK_TREE_MODEL(d->liststore), &iter, DT_LIB_EXPORT_METADATA_COL_XMP,
                               &tagname, DT_LIB_EXPORT_METADATA_COL_FORMULA, &formula, -1);
