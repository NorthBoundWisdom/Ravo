...
static dt_colorspaces_color_profile_type_t get_icc_type(const char *option)
{
    ...
    ICC_FROM_STR(BRG);
    ICC_FROM_STR(
        EXPORT); // export and softproof are categories and will return NULL with dt_colorspaces_get_profile()
...
int main(int argc, char *arg[])
{
    ...
    gboolean verbose = FALSE, high_quality = TRUE, upscale = FALSE, style_overwrite = FALSE,
             custom_presets = TRUE, export_masks = FALSE, output_to_dir = FALSE;
    ...
    for (k = 1; k < argc; k++)
    {
        if (arg[k][0] == '-')
        {
            ...
            else if (!strcmp(arg[k], "--apply-custom-presets") && argc > k + 1)
            {
                ...
                {
                    fprintf(stderr, "%s: %s\n", _("unknown option for --apply-custom-presets"),
                            arg[k]);
                    usage(arg[0]);
                    exit(1);
                }
            ...
            else if (!strcmp(arg[k], "--icc-file") && argc > k + 1)
            {
                k++;
                if (g_file_test(arg[k], G_FILE_TEST_EXISTS) &&
                    !g_file_test(arg[k], G_FILE_TEST_IS_DIR))
                {
                    if (icc_filename)
                        g_free(icc_filename);
    ...
    {
        if (!output_ext || (output_ext && g_str_has_suffix(output_filename, output_ext) &&
                            !g_strcmp0(output_ext, strrchr(output_filename, '.') + 1)))
        {
    ...
    for (GList *iter = id_list; iter; iter = g_list_next(iter), num++)
    {
        ...
        if (storage->store(storage, sdata, id, format, fdata, num, total, high_quality, upscale,
                           FALSE, 1.0, export_masks, icc_type, icc_filename, icc_intent,
                           &metadata) != 0)
            res = 1;
    }

    // cleanup time
    if (storage->finalize_store)
        storage->finalize_store(storage, sdata);
