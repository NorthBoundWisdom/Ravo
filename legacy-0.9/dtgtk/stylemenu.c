...
static gboolean _styles_tooltip_callback(GtkWidget *self, const gint x, const gint y,
...
static void _build_style_submenus(GtkMenuShell *menu, const gchar *style_name, gchar **splits,
                                  const int index,
                                  ...
                                  gpointer user_data)
{
    ...
    if (!splits[index + 1])
    {
        ...
        if (style_name && style_name[0]) // don't add tooltip for "none" style
        {
            ...
            g_signal_connect_data(mi, "query-tooltip", G_CALLBACK(_styles_tooltip_callback),
                                  g_strdup(style_name), (GClosureNotify)g_free, 0);
            dt_action_define(&darktable.control->actions_global, "styles", style_name,
                             GTK_WIDGET(mi), NULL);
        }
        else
    ...
    else
    {
        ...
        _build_style_submenus(GTK_MENU_SHELL(sm), style_name, splits, index + 1, activate_callback,
                              button_callback, user_data);
    }
    ...
    if (activate_callback)
    {
        ...
        if (menu_data)
        {
            ...
            g_signal_connect_data(G_OBJECT(mi), "activate", G_CALLBACK(activate_callback),
    ...
    if (button_callback)
    {
        ...
        if (menu_data)
        {
            menu_data->name = g_strdup(style_name);
            menu_data->user_data = user_data;
            g_signal_connect_data(G_OBJECT(mi), "button-press-event", G_CALLBACK(button_callback),
                                  menu_data, (GClosureNotify)_free_menu_data, 0);
        }
    }
