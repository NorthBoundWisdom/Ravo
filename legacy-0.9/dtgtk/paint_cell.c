...
static int _paint_cell_compute_size(GtkWidget *widget)
{
    ...
    if (widget)
    {
        ...
        const int line_h =
            (pango_font_metrics_get_ascent(m) + pango_font_metrics_get_descent(m)) / PANGO_SCALE;
...
static void _paint_cell_get_preferred_width(GtkCellRenderer *r, GtkWidget *widget,
...
static void _paint_cell_get_preferred_height(GtkCellRenderer *r, GtkWidget *widget,
                                             gint *minimum_size, gint *natural_size)
{
    (void)r;
    const int s = _paint_cell_compute_size(widget);
...
static void _paint_cell_render(GtkCellRenderer *r, cairo_t *cr, GtkWidget *widget,
                               const GdkRectangle *bg_area, const GdkRectangle *cell_area,
                               GtkCellRendererState flags)
{
    ...
    self->paint(cr, cell_area->x + mx, cell_area->y + my, cell_area->width - 2 * mx,
                cell_area->height - 2 * my, self->paint_flags, self->paint_data);
    cairo_restore(cr);
}
