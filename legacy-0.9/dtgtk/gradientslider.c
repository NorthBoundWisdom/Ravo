...
// Class overrides
static void _gradient_slider_get_preferred_height(GtkWidget *widget, gint *min_height,
                                                  gint *nat_height);
static void _gradient_slider_get_preferred_width(GtkWidget *widget, gint *min_width,
                                                 gint *nat_width);
...
static gboolean _gradient_slider_postponed_value_change(gpointer data)
{
    ...
    if (!DTGTK_GRADIENT_SLIDER(data)->is_dragging)
    ...
    else
    {
        ...
        DTGTK_GRADIENT_SLIDER(data)->timeout_handle =
            g_timeout_add(delay, _gradient_slider_postponed_value_change, data);
    }
...
static inline gdouble _screen_to_scale(GtkWidget *widget, const gint screen)
{
    GtkDarktableGradientSlider *gslider = DTGTK_GRADIENT_SLIDER(widget);

    GtkAllocation allocation;
    gtk_widget_get_allocation(widget, &allocation);
    return ((gdouble)screen - gslider->margin_left) /
           ((gdouble)allocation.width - gslider->margin_left - gslider->margin_right);
}

static inline gint _scale_to_screen(GtkWidget *widget, const gdouble scale)
{
    GtkDarktableGradientSlider *gslider = DTGTK_GRADIENT_SLIDER(widget);

    GtkAllocation allocation;
    gtk_widget_get_allocation(widget, &allocation);
    return (gint)(scale * (allocation.width - gslider->margin_left - gslider->margin_right) +
                  gslider->margin_left);
}
...
static inline gdouble _get_position_from_screen(GtkWidget *widget, const gdouble x)
{
    GtkDarktableGradientSlider *gslider = DTGTK_GRADIENT_SLIDER(widget);
    gdouble position =
        roundf(_screen_to_scale(widget, x) / gslider->increment) * gslider->increment;
...
static inline void _clamp_marker(GtkDarktableGradientSlider *gslider, const gint selected)
{
    ...
    const gdouble max =
        (selected == gslider->positions - 1) ? 1.0f : gslider->position[selected + 1];
...
static gint _get_active_marker_internal(GtkWidget *widget, const gdouble x, const gboolean up)
{
    ...
    for (int k = 0; k < gslider->positions; k++)
        if (_test_if_marker_is_upper_or_down(gslider->marker[k], up))
        {
            if (lselected < 0)
                lselected = k;
            if (fabs(newposition - gslider->position[k]) <
                fabs(newposition - gslider->position[lselected]))
                lselected = k;
        }
...
static gint _get_active_marker_from_screen(GtkWidget *widget, const gdouble x, const gdouble y)
{
    ...
    if (lselected < 0)
        lselected = _get_active_marker_internal(widget, x, !up);
...
static gdouble _slider_move(GtkWidget *widget, const gint k, const gdouble value,
                            const gint direction)
{
    ...
    case FREE_MARKERS:
    {
        ...
        case MOVE_LEFT:
            if (value < leftnext + ms)
                newvalue = (k == 0) ? fmax(value, 0.0f) :
                                      _slider_move(widget, k - 1, value - ms, direction) + ms;
            break;
        case MOVE_RIGHT:
            if (value > rightnext - ms)
                newvalue = (k == gslider->positions - 1) ?
                               fmin(value, 1.0f) :
                               _slider_move(widget, k + 1, value + ms, direction) - ms;
            break;
        }
    ...
    case PROPORTIONAL_MARKERS:
    {
        ...
        const double vmax =
            ((k == gslider->positions - 1) ? 1.0f : gslider->position[gslider->positions - 1]);

        newvalue = CLAMP(value, vmin + ms * k, vmax - ms * (gslider->positions - 1 - k));
        const double rl =
            (newvalue - gslider->position[0]) / (gslider->position[k] - gslider->position[0]);
        const double rh = (gslider->position[gslider->positions - 1] - newvalue) /
                          (gslider->position[gslider->positions - 1] - gslider->position[k]);

        for (int i = 1; i < k; i++)
            gslider->position[i] =
                rl * (gslider->position[i] - gslider->position[0]) + gslider->position[0];

        for (int i = k + 1; i < gslider->positions; i++)
            gslider->position[i] =
                gslider->position[gslider->positions - 1] -
...
static gboolean _gradient_slider_add_delta_internal(GtkWidget *widget, gdouble delta,
                                                    const guint state, const gint selected)
{
...
static float _default_linear_scale_callback(GtkWidget *self, const float value, const int dir)
{
...
static gboolean _gradient_slider_button_press(GtkWidget *widget, GdkEventButton *event)
{
    ...
    if (event->button == GDK_BUTTON_PRIMARY && event->type == GDK_2BUTTON_PRESS &&
        gslider->is_resettable)
    {
        gslider->is_dragging = FALSE;
        gslider->do_reset = TRUE;
        gslider->selected = -1;
        for (int k = 0; k < gslider->positions; k++)
            gslider->position[k] = gslider->resetvalue[k];
        gtk_widget_queue_draw(widget);
        g_signal_emit_by_name(G_OBJECT(widget), "value-changed");
             ...
             event->type == GDK_BUTTON_PRESS)
    {
        ...
        if (event->button == GDK_BUTTON_PRIMARY) // left mouse button : select and start dragging
        {
            ...
            const gint direction =
                gslider->position[gslider->selected] <= newposition ? MOVE_RIGHT : MOVE_LEFT;
...
static void _gradient_slider_init(GtkDarktableGradientSlider *gslider)
{
    ...
    gtk_widget_add_events(
        widget, GDK_EXPOSURE_MASK | GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK |
                    GDK_ENTER_NOTIFY_MASK | GDK_LEAVE_NOTIFY_MASK | GDK_KEY_PRESS_MASK |
                    GDK_KEY_RELEASE_MASK | GDK_POINTER_MOTION_MASK | darktable.gui->scroll_mask);
                                                  ...
                                                  gint *nat_height)
{
    ...
    *min_height = *nat_height = css_min_height + padding.top + padding.bottom + border.top +
                                                 ...
                                                 gint *nat_width)
{
    ...
    gtk_style_context_get_border(context, state, &border);
    gtk_style_context_get_padding(context, state, &padding);
    *min_width = *nat_width = css_min_width + padding.left + padding.right + border.left +
...
static gboolean _gradient_slider_draw(GtkWidget *widget, cairo_t *cr)
{
    ...
    if (gslider->colors)
    {
        ...
        for (GList *current = gslider->colors; current; current = g_list_next(current))
        {
            ...
            cairo_pattern_add_color_stop_rgba(gradient, stop->position, stop->color.red,
    ...
    for (int k = 0; k < gslider->positions; k++)
    {
        ...
        {
            if (mk & 0x01) /* filled */
                dtgtk_cairo_paint_solid_triangle(cr, round(vx - 0.5f * sz),
                                                 round((float)y1 - 0.55f * sz), sz, sz,
                                                 CPF_DIRECTION_DOWN, NULL);
            else
                dtgtk_cairo_paint_triangle(cr, round(vx - 0.5f * sz), round((float)y1 - 0.55f * sz),
                                           sz, sz, CPF_DIRECTION_DOWN, NULL);
        }
        ...
        if (mk & 0x02) /* lower arrow */
        {
            if (mk & 0x01) /* filled */
                dtgtk_cairo_paint_solid_triangle(cr, round(vx - 0.5f * sz),
                                                 round((float)cheight - y1 - 0.45f * sz), sz, sz,
                                                 CPF_DIRECTION_UP, NULL);
            else
                dtgtk_cairo_paint_triangle(cr, round(vx - 0.5f * sz),
                                           round((float)cheight - y1 - 0.45f * sz), sz, sz,
                                           CPF_DIRECTION_UP, NULL);
        }
    }
...
GtkWidget *dtgtk_gradient_slider_multivalue_new_with_name(const gint positions, gchar *name)
{
...
GtkWidget *dtgtk_gradient_slider_multivalue_new_with_color(const GdkRGBA start, const GdkRGBA end,
...
GtkWidget *dtgtk_gradient_slider_multivalue_new_with_color_and_name(const GdkRGBA start,
                                                                    const GdkRGBA end,
                                                                    const gint positions,
                                                                    gchar *name)
{
    GtkWidget *widget =
        GTK_WIDGET(dtgtk_gradient_slider_multivalue_new_with_color(start, end, positions));
    if (name)
        gtk_widget_set_name(widget, name);
...
void dtgtk_gradient_slider_multivalue_set_stop(GtkDarktableGradientSlider *gslider,
                                               const gfloat position, const GdkRGBA color)
{
    g_return_if_fail(gslider != NULL);
    const gfloat rawposition =
        gslider->scale_callback((GtkWidget *)gslider, position, GRADIENT_SLIDER_SET);
    // First find color at position, if exists update color, otherwise
    // create a new stop at position.
    GList *current =
        g_list_find_custom(gslider->colors, (gpointer)&rawposition, _list_find_by_position);
    if (current != NULL)
    {
                                                   ...
                                                   const gint pos)
{
    ...
    return gslider->scale_callback((GtkWidget *)gslider, gslider->position[pos],
                                   GRADIENT_SLIDER_GET);
}
                                                 ...
                                                 gdouble *values)
{
    g_return_if_fail(gslider != NULL);
    for (int k = 0; k < gslider->positions; k++)
        values[k] = gslider->scale_callback((GtkWidget *)gslider, gslider->position[k],
                                            GRADIENT_SLIDER_GET);
}

void dtgtk_gradient_slider_multivalue_set_value(GtkDarktableGradientSlider *gslider,
                                                const gdouble value, const gint pos)
{
    g_return_if_fail(gslider != NULL);
    assert(pos <= gslider->positions);

    gslider->position[pos] =
        CLAMP(gslider->scale_callback((GtkWidget *)gslider, value, GRADIENT_SLIDER_SET), 0.0, 1.0);
    gslider->selected = gslider->positions == 1 ? 0 : -1;
    if (!DT_IN_GUI_UPDATE())
                                                 ...
                                                 gdouble *values)
{
    ...
    for (int k = 0; k < gslider->positions; k++)
        gslider->position[k] =
            CLAMP(gslider->scale_callback((GtkWidget *)gslider, values[k], GRADIENT_SLIDER_SET),
                  0.0, 1.0);
    gslider->selected = gslider->positions == 1 ? 0 : -1;
    if (!DT_IN_GUI_UPDATE())
        g_signal_emit_by_name(G_OBJECT(gslider), "value-changed");
...
void dtgtk_gradient_slider_multivalue_set_marker(GtkDarktableGradientSlider *gslider,
                                                 const gint mark, const gint pos)
{
    g_return_if_fail(gslider != NULL);
    assert(pos <= gslider->positions);
                                                        ...
                                                        const gint pos)
{
    ...
    return gslider->scale_callback((GtkWidget *)gslider, gslider->resetvalue[pos],
                                   GRADIENT_SLIDER_GET);
}
                                                      ...
                                                      gdouble *values)
{
    g_return_if_fail(gslider != NULL);
    for (int k = 0; k < gslider->positions; k++)
        gslider->resetvalue[k] =
            gslider->scale_callback((GtkWidget *)gslider, values[k], GRADIENT_SLIDER_SET);
    gslider->is_resettable = TRUE;
}
...
void dtgtk_gradient_slider_multivalue_set_picker_meanminmax(GtkDarktableGradientSlider *gslider,
                                                            const gdouble mean, const gdouble min,
                                                            const gdouble max)
{
    g_return_if_fail(gslider != NULL);
...
void dtgtk_gradient_slider_multivalue_set_scale_callback(GtkDarktableGradientSlider *gslider,
                                                         float (*callback)(GtkWidget *self,
                                                                           float value, int dir))
{
    float (*old_callback)(GtkWidget *, float, int) = gslider->scale_callback;
    float (*new_callback)(GtkWidget *, float, int) =
        (callback == NULL ? _default_linear_scale_callback : callback);
    ...
    {
        gslider->position[k] =
            new_callback(self, old_callback(self, gslider->position[k], GRADIENT_SLIDER_GET),
                         GRADIENT_SLIDER_SET);
        gslider->resetvalue[k] =
            new_callback(self, old_callback(self, gslider->resetvalue[k], GRADIENT_SLIDER_GET),
                         GRADIENT_SLIDER_SET);
    }

    for (int k = 0; k < 3; k++)
    {
        gslider->picker[k] = new_callback(
            self, old_callback(self, gslider->picker[k], GRADIENT_SLIDER_GET), GRADIENT_SLIDER_SET);
    }

    for (GList *current = gslider->colors; current; current = g_list_next(current))
    {
        _gradient_slider_stop_t *stop = (_gradient_slider_stop_t *)current->data;
        stop->position = new_callback(self, old_callback(self, stop->position, GRADIENT_SLIDER_GET),
                                      GRADIENT_SLIDER_SET);
    }
...
GtkWidget *dtgtk_gradient_slider_new_with_color(const GdkRGBA start, const GdkRGBA end)
{
...
GtkWidget *dtgtk_gradient_slider_new_with_color_and_name(const GdkRGBA start, const GdkRGBA end,
                                                         gchar *name)
{
...
void dtgtk_gradient_slider_set_stop(GtkDarktableGradientSlider *gslider, const gfloat position,
                                    const GdkRGBA color)
{
...
void dtgtk_gradient_slider_set_value(GtkDarktableGradientSlider *gslider, const gdouble value)
{
    dtgtk_gradient_slider_multivalue_set_value(gslider, value, 0);
}

void dtgtk_gradient_slider_set_marker(GtkDarktableGradientSlider *gslider, const gint mark)
{
    dtgtk_gradient_slider_multivalue_set_marker(gslider, mark, 0);
}

void dtgtk_gradient_slider_set_resetvalue(GtkDarktableGradientSlider *gslider, const gdouble value)
{
...
void dtgtk_gradient_slider_set_picker(GtkDarktableGradientSlider *gslider, const gdouble value)
{
    g_return_if_fail(gslider != NULL);
    gslider->picker[0] = gslider->picker[1] = gslider->picker[2] =
        gslider->scale_callback((GtkWidget *)gslider, value, GRADIENT_SLIDER_SET);
...
void dtgtk_gradient_slider_set_increment(GtkDarktableGradientSlider *gslider, const gdouble value)
{
    g_return_if_fail(gslider != NULL);
    gslider->increment = value;
}
