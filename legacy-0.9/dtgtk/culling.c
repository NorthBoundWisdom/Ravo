...
static gboolean _compute_sizes(dt_culling_t *table, const gboolean force)
{
    ...
    if (table->mode == DT_CULLING_MODE_CULLING)
    {
        ...
        if (force || allocation.width != table->view_width ||
            allocation.height != table->view_height || npr != table->thumbs_count)
        {
...
static gboolean _zoom_and_shift(dt_thumbnail_t *th, const int x_offset, const int y_offset,
                                const float zoom_delta, const gboolean deferred)
{
    ...
    dt_print(DT_DEBUG_INPUT, "[culling _zoom_and_shift] new_zoomx=%.1f new_zoomy=%.1f", th->zoomx,
             th->zoomy);
...
static gboolean _zoom_to_x_root(dt_thumbnail_t *th, const float x_culling, const float y_culling,
                                const float zoom_delta, const gboolean deferred)
{
...
static gboolean _zoom_to_center(dt_thumbnail_t *th, const float zoom_delta, const gboolean deferred)
{
    const float zd = CLAMP(th->zoom + zoom_delta, 1.0f, th->zoom_100);
    if (zd == th->zoom)
        return FALSE; // delta_zoom did not change this thumbnail's zoom factor
    ...
    th->zoomx =
        fmaxf((float)iw - effective_w, fminf(0.0f, iw / 2.0 - (iw / 2.0 - th->zoomx) * z_ratio));
    th->zoomy =
        fmaxf((float)ih - effective_h, fminf(0.0f, ih / 2.0 - (ih / 2.0 - th->zoomy) * z_ratio));
...
static gboolean _thumbs_zoom_add(dt_culling_t *table, const float zoom_delta, const float x_culling,
                                 const float y_culling, const int state, const gboolean deferred)
{
...
static gboolean _zoom_thumb_max(dt_thumbnail_t *th, const float x_culling, const float y_culling)
{
    dt_thumbnail_get_zoom100(th);
...
static void _update_selected_thumbnail(dt_culling_t *table, const dt_imgid_t old_sel)
{
    for (GList *l = table->list; l; l = g_list_next(l))
    {
        const dt_thumbnail_t *th = l->data;
...
static gboolean _event_gesture(GtkWidget *widget, GdkEvent *event, gpointer user_data)
{
    ...
    dt_view_manager_gesture_pinch(darktable.view_manager, pinch->x_root, pinch->y_root, pinch->dx,
                                  pinch->dy, pinch->phase, pinch->scale, pinch->state & 0xf);
...
static gboolean _event_scroll(GtkWidget *widget, GdkEvent *event, gpointer user_data)
{
    ...
    dt_print(DT_DEBUG_INPUT,
             ...
             e->direction, e->direction == GDK_SCROLL_SMOOTH ? "yes" : "no",
             e->is_stop ? "yes" : "no", dt_modifier_is(e->state, GDK_CONTROL_MASK) ? "yes" : "no",
             device ? gdk_device_get_name(device) : "<none>",
             device ? (int)gdk_device_get_source(device) : -1, e->x_root, e->y_root, e->delta_x,
             e->delta_y, e->state);
        ...
        !dt_modifiers_include(e->state, GDK_CONTROL_MASK))
    {
        ...
        dt_print(DT_DEBUG_INPUT, "[culling scroll] smooth: max_zoom=%.3f -> %s", fz,
                 fz > 1.0f ? "pan path" : "navigate path");
        if (fz > 1.0f)
        {
            gdouble dx = 0.0, dy = 0.0;
            if (dt_gui_get_scroll_deltas(e, &dx, &dy) && (dx != 0.0 || dy != 0.0))
            {
                ...
                dt_print(DT_DEBUG_INPUT,
                         "[culling scroll] panning dx=%.3f dy=%.3f (scaled: dx=%.1f dy=%.1f)", dx,
                         dy, dx * 50.0, dy * 50.0);
            ...
            {
                dt_print(DT_DEBUG_INPUT,
                         "[culling scroll] smooth pan: no delta from dt_gui_get_scroll_deltas");
            }
            return TRUE;
        }
    }
...
static gboolean _event_button_release(GtkWidget *widget, GdkEventButton *event, gpointer user_data)
{
    ...
    if (dt_act_on_use_culling_selection() && dt_is_valid_imgid(overid) &&
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2
      (dt_database_get(darktable.db),
       ...
       -1, &stmt, NULL);
        ...
        const int nz =
            (sel_count <= 1) ? dt_conf_get_int("plugins/lighttable/culling_num_images") : sel_count;
...
static void _dt_profile_change_callback(gpointer instance, const int type, gpointer user_data)
{
    if (!user_data)
        return;
    dt_culling_t *table = (dt_culling_t *)user_data;
...
static void _dt_filmstrip_change(gpointer instance, const dt_imgid_t imgid, gpointer user_data)
{
    if (!user_data || !dt_is_valid_imgid(imgid))
        return;
    dt_culling_t *table = (dt_culling_t *)user_data;
    if (!gtk_widget_get_visible(table->widget))
        return;
...
dt_culling_t *dt_culling_new(const dt_culling_mode_t mode)
{
    ...
    g_signal_connect(G_OBJECT(table->widget), "event", G_CALLBACK(_event_gesture), table);
    ...
    DT_CONTROL_SIGNAL_CONNECT(DT_SIGNAL_VIEWMANAGER_THUMBTABLE_ACTIVATE, _dt_filmstrip_change,
                              table);
                     ...
                     const dt_lighttable_culling_restriction_t restriction)
{
   ...
   */
    ...
    const gboolean culling_dynamic = (table->mode == DT_CULLING_MODE_CULLING &&
                                      dt_view_lighttable_get_layout(darktable.view_manager) ==
                                          DT_LIGHTTABLE_LAYOUT_CULLING_DYNAMIC);
...
static gboolean _thumbs_recreate_list_at(dt_culling_t *table, const int offset)
{
    ...
    if (table->list)
    {
        const dt_thumbnail_t *th_model = table->list->data;
        nw = th_model->width;
        nh = th_model->height;
    }
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        else
        {
            ...
            thumb = dt_thumbnail_new(nw, nh, table->zoom_ratio, nid, nrow, table->overlays,
                                     container, table->show_tooltips, selected);
    ...
    if (table->list)
    {
        const dt_thumbnail_t *thumb = table->list->data;
...
static gboolean _thumbs_compute_positions(dt_culling_t *table)
{
    ...
    // Note: Stacking only make sense for images in the same row as the
    //       portrait image.  The algorithm does not check for this so
    //       unnecessary stacking can occur.
    ...
    for (GList *l = table->list; l; l = g_list_next(l))
    {
        ...
        for (; slot_iter; slot_iter = slot_iter->next)
        {
            ...
            for (GList *slot_thumb_iter = slot; slot_thumb_iter;
            ...
            if (slot_heigth + spacing + thumb->height < max_thumb_height)
            {
                ...
                max_slot_heigth = MAX(max_slot_heigth, slot_heigth + spacing + thumb->height);
                break;
            }
        }
    ...
    // finished assigning thumbnails to slots we also know max slot
    // height, so we can now scale all slots to this height and then
    // calculate average slot height and width
    int slot_counter = 0;
    ...
    for (GList *slot_iter = slots; slot_iter; slot_iter = g_list_next(slot_iter))
    {
             ...
             slot_thumb_iter = g_list_next(slot_thumb_iter))
        {
            ...
            float stack_heigth_factor = (max_slot_heigth) / (float)slot_heigth;

            if (number_of_slots == 2)
            {
                // limit scaling factor to 20% if only two images are
                // displayed so that slight differences are corrected but
                // portrait and landscape orientation are displayed at similar
                // sizes
                stack_heigth_factor = MIN(stack_heigth_factor, 1.2);
            }
            else
            {
                // limit scaling so that width does not increase to more than
                // twice the average thumbnail width
                stack_heigth_factor =
                    MIN(stack_heigth_factor, 2 * avg_thumb_width / (float)thumb->width);
            }
        ...
        avg_slot_aspect_r += (scaled_slot_width / (float)scaled_slot_height - avg_slot_aspect_r) /
                             (float)slot_counter;
        avg_slot_width += (scaled_slot_width - avg_slot_width) / (float)thumb_counter;
    }
    total_slot_width -= spacing;

    // variables to hold vertical and horizontal width of all thumbnails
    // after their final placement
    ...
    do
    {
        ...
        deviation_tmp =
            _absmul(planned_total_width_tmp / (float)planned_total_height_tmp, screen_aspect_r);
    ...
    GList *rows = g_list_append(NULL, NULL);
    {
        ...
        for (GList *slot_iter = slots; slot_iter; slot_iter = g_list_next(slot_iter))
        {
            ...
            // Calculate max width and total height of thumbs in the slot so
            // that all thumbs can be centered within the slot.
            ...
            //  if slot is about to be placed outside of allocated horizontal
            //  space, place the slot in a new row we allow for 20%
            //  thumbnail width tolerance to account for the influence of
            //  images with mixed aspect ratios in the math
            gboolean create_new_row = FALSE;

            //  if the row limit is exceeded by more than 60% of a slot place
            //  it in the next row unless this is the last thumbnail and
            //  squeezing it into the current row results in a better
            //  placement ratio than opening a new row.
            if (thumb_x + 0.4 * slot_max_thumb_width > row_width_limit)
            {
                ...
                if (!slot_iter->next)
                {
                    const float ratio_same_row =
                        _absmul(MAX(total_width, (thumb_x + slot_max_thumb_width)) /
                                    (float)MAX(total_height, row_y + slot_total_heigth),
                                table->view_width / (float)table->view_height);
                    const float ratio_new_row =
                        _absmul(MAX(total_width, slot_max_thumb_width) /
                                    (float)(total_height + slot_total_heigth),
                                table->view_width / (float)table->view_height);
                 ...
                 slot_thumb_iter = g_list_next(slot_thumb_iter))
            {
                dt_thumbnail_t *thumb = slot_thumb_iter->data;
                thumb->x =
                    thumb_x + (slot_max_thumb_width - thumb->width) /
                                  2; // x position should be horizontally centered within the slot
                thumb->y = thumb_y;  // y position starts at 0
                thumb_y +=
                    thumb->height +
                    spacing; // and is increased by the height of the thumb + spacing of spacing for placing the next image of the slot
            }
            rows->data = g_list_append(rows->data, slot); // append slot to row
            row_heigth = MAX(row_heigth, thumb_y - row_y);
            total_height = MAX(
                total_height,
                thumb_y); // update total height of all thumbs combined as we fill column by column with thumbnails
            thumb_x += slot_max_thumb_width + spacing;
            total_width = MAX(
                total_width,
                thumb_x); // update total width of all thumbs combined as we fill column by column with thumbnails
        }
        total_width -= spacing;
        g_list_free(slots);
        slots = NULL;
    ...
    for (const GList *row_iter = rows; row_iter; row_iter = g_list_next(row_iter))
    {
        ...
        for (GList *slot_iter = row; slot_iter; slot_iter = g_list_next(slot_iter))
        {
            GList *slot = (GList *)slot_iter->data;
            int slot_heigth = 0;
    ...
    for (GList *l = table->list; l; l = g_list_next(l))
    {
        ...
        dt_print(DT_DEBUG_LIGHTTABLE,
                 ...
                 thumb->imgid, thumb->x, thumb->y, thumb->width, thumb->height, table->view_width,
                 table->view_height);
    }
...
void dt_culling_update_active_images_list(dt_culling_t *table)
{
    ...
    for (GList *l = table->list; l; l = g_list_next(l))
    {
        const dt_thumbnail_t *thumb = l->data;
...
{
    if (!gtk_widget_get_visible(table->widget) && !force)
        return;
    ...
    for (GList *l = table->list; l; l = g_list_next(l))
    {
        ...
        else
        {
            ...
            const float zoom_ratio =
                thumb->zoom_100 > 1 ? thumb->zoom / thumb->zoom_100 : IMG_TO_FIT;
    ...
    if (table->selection_sync)
    {
        ...
        // deselect all
        DT_DEBUG_SQLITE3_EXEC(dt_database_get(darktable.db), "DELETE FROM main.selected_images",
                              NULL, NULL, NULL);
...
gboolean dt_culling_zoom_add(dt_culling_t *table, const float zoom_delta, const float x_root,
                             const float y_root, const int state)
{
    if (!table)
        return FALSE;
...
gboolean dt_culling_pan_move(dt_culling_t *table, const float dx, const float dy, const int state)
{
    if (!table)
        return FALSE;
