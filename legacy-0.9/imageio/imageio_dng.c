...
static void _dt_dng_tiff_warning(const char *module, const char *fmt, va_list ap)
{
    if (darktable.unmuted & DT_DEBUG_IMAGEIO)
    {
        fprintf(stderr,
                "%11.4f [imageio_dng] warning: %s: ", dt_get_wtime() - darktable.start_wtime,
                module ? module : "(none)");
...
static void _dt_dng_tiff_error(const char *module, const char *fmt, va_list ap)
{
    fprintf(stderr, "%11.4f [imageio_dng] error: %s: ", dt_get_wtime() - darktable.start_wtime,
            module ? module : "(none)");
    vfprintf(stderr, fmt, ap);
    fprintf(stderr, "\n");
}
...
static void _register_extra_dng_fields(TIFF *tif)
{
    static const TIFFFieldInfo extra[] = {
        {TIFFTAG_PREVIEWCOLORSPACE, 1, 1, TIFF_LONG, FIELD_CUSTOM, TRUE, FALSE,
         (char *)"PreviewColorSpace"},
        {TIFFTAG_CFAREPEATPATTERNDIM, 2, 2, TIFF_SHORT, FIELD_CUSTOM, TRUE, FALSE,
         (char *)"CFARepeatPatternDim"},
        {TIFFTAG_CFAPATTERN, -1, -1, TIFF_BYTE, FIELD_CUSTOM, TRUE, TRUE, (char *)"CFAPattern"},
    };
...
static void _set_dng_shared_metadata(TIFF *tif, const dt_image_t *img)
{
    ...
    if (img->filename[0])
    {
        ...
        TIFFSetField(tif, TIFFTAG_ORIGINALRAWFILENAME, (uint32_t)fn_len, img->filename);
    }
    ...
    if (img->wb_coeffs[0] > 0.0f && img->wb_coeffs[1] > 0.0f && img->wb_coeffs[2] > 0.0f)
    {
        float neutral[3];
        for (int i = 0; i < 3; i++)
            neutral[i] = 1.0f / img->wb_coeffs[i];
...
static uint8_t *_make_thumb_jpeg(const uint8_t *src_jpeg, const int src_len, int *out_w, int *out_h,
                                 int *out_len)
{
    ...
    const int dw = (long_edge <= DNG_THUMB_MAX_DIM) ?
                       sw :
                       (sw * DNG_THUMB_MAX_DIM + long_edge / 2) / long_edge;
...
static int _write_thumb_ifd0(TIFF *tif, const dt_image_t *img, const dt_imageio_dng_preview_t *p,
                             const int n_subifds)
{
...
static int _write_full_preview_subifd(TIFF *tif, const dt_imageio_dng_preview_t *p)
{
    TIFFSetField(tif, TIFFTAG_SUBFILETYPE, FILETYPE_REDUCEDIMAGE);
    TIFFSetField(tif, TIFFTAG_IMAGEWIDTH, (uint32_t)p->width);
    TIFFSetField(tif, TIFFTAG_IMAGELENGTH, (uint32_t)p->height);
...
int dt_imageio_dng_write_cfa_bayer(const char *filename, const uint16_t *cfa, int width, int height,
                                   const dt_image_t *img, const void *exif_blob, int exif_len,
                                   const dt_imageio_dng_preview_t *preview)
{
    ...
    const gboolean canonical =
        (preview && preview->data && preview->len > 0 && preview->width > 0 && preview->height > 0);
    uint8_t *thumb_jpeg = NULL;
    int thumb_w = 0, thumb_h = 0, thumb_len = 0;
    if (canonical)
        thumb_jpeg = _make_thumb_jpeg(preview->data, preview->len, &thumb_w, &thumb_h, &thumb_len);
    ...
    if (canonical)
    {
        ...
        if (have_thumb)
            ifd0 = (dt_imageio_dng_preview_t){
                .data = thumb_jpeg, .len = thumb_len, .width = thumb_w, .height = thumb_h};
        else
            ifd0 = *preview;
    ...
    const int vis_w =
        (img->p_width > 0 && img->p_width <= width - crop_x) ? img->p_width : (width - crop_x);
    const int vis_h =
        (img->p_height > 0 && img->p_height <= height - crop_y) ? img->p_height : (height - crop_y);
    ...
    const uint32_t white = img->raw_white_point ? img->raw_white_point : 65535u;
    ...
    const uint32_t active_area[4] = {
        (uint32_t)crop_y,
        (uint32_t)crop_x,
        (uint32_t)(crop_y + vis_h),
        (uint32_t)(crop_x + vis_w),
    };
    ...
    if (res == 0 && have_thumb)
    {
        ...
        else
        {
            TIFFCreateDirectory(tif);
            _register_extra_dng_fields(tif);
            if (_write_full_preview_subifd(tif, preview) != 0)
                res = 1;
        }
    }
    ...
    if (res == 0 && exif_blob && exif_len > 0)
        dt_exif_write_blob((uint8_t *)exif_blob, (uint32_t)exif_len, filename, 0);
...
static inline int _imageio_dng_make_tag(const uint16_t tag, const uint16_t type, const uint32_t lng,
                                        const uint32_t fld, uint8_t *buf, const uint32_t b,
                                        uint8_t *cnt)
{
...
static inline void _imageio_dng_write_tiff_header(FILE *fp, uint32_t xs, uint32_t ys, float Tv,
                                                  float Av, float f, float iso, uint32_t filter,
                                                  ...
                                                  const float adobe_XYZ_to_CAM[4][3])
{
    ...
    enum write_tags
    {
        ...
        EXIF_TAG_SHOT_NEUTRAL =
            50728, /* AsShotNeutral for rawspeed Dngdecoder camera white balance */
    ...
    b = _imageio_dng_make_tag(EXIF_TAG_STRIP_BCOUNT, LONG, 1, (ys * xs * channels * 4), buf, b,
                              &cnt);
    ...
    if (filter == 9u) // xtrans
    {
        b = _imageio_dng_make_tag(EXIF_TAG_SENS_PATTERN, BYTE, 36, data, buf, b,
                                  &cnt); /* xtrans PATTERN */
        ...
        b = _imageio_dng_make_tag(EXIF_TAG_SENS_PATTERN, BYTE, 4, cfapattern, buf, b,
                                  &cnt); /* bayer PATTERN */

    b = _imageio_dng_make_tag(EXIF_TAG_VERSION, BYTE, 4, 1 | (4 << 8), buf, b, &cnt);

    // WhiteLevel is straight integer even for float DNGs
    b = _imageio_dng_make_tag(EXIF_TAG_WHITE_LEVEL, LONG, 1, (uint32_t)roundf(whitelevel), buf, b,
                              &cnt);
    ...
    b = _imageio_dng_make_tag(EXIF_TAG_COLOR_MATRIX1, SRATIONAL, 9, data, buf, b,
                              &cnt); /* ColorMatrix1 (XYZ->native cam) */
    for (int k = 0; k < 9; k++)
    {
    ...
    {
        dt_print(DT_DEBUG_ALWAYS,
                 "[dng_write_header] can't write valid header, unexpected number of entries!");
        return;
    }
    ...
    {
        dt_print(DT_DEBUG_ALWAYS,
                 "[dng_write_header] can't write valid header as it exceeds buffer size!");
        return;
    }
    ...
    if (written != data)
        dt_print(DT_DEBUG_ALWAYS, "[dng_write_header] failed to write image header!");
}

void dt_imageio_dng_write_float(const char *filename, const float *const pixel, const int wd,
                                ...
                                const float adobe_XYZ_to_CAM[4][3])
{
    ...
    if (f)
    {
        _imageio_dng_write_tiff_header(f, wd, ht, 1.0f / 100.0f, 1.0f / 4.0f, 50.0f, 100.0f, filter,
                                       xtrans, whitelevel, wb_coeffs, adobe_XYZ_to_CAM);
        const int k = fwrite(pixel, sizeof(float), (size_t)wd * ht, f);
        if (k != wd * ht)
            dt_print(DT_DEBUG_ALWAYS, "[dng_write] Error writing image data to %s", filename);
        fclose(f);
        if (exif)
