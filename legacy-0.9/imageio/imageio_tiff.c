...
static inline int _read_chunky_8(tiff_t *t, uint16_t photometric)
{
    ...
    for (uint32_t row = 0; row < t->height; row++)
    {
        ...
        for (uint32_t i = 0; i < t->width; i++, in += t->spp, out += 4)
        {
            ...
            out[0] = need_invert ? 1.0f - ((float)in[0]) * (1.0f / 255.0f) :
...
static void _warning_error_handler(const char *type, const char *module, const char *fmt,
                                   va_list ap)
{
    // We prepend the wall time from the program start to output line
    // to be consistent with dt_print
    fprintf(stderr, "%11.4f [tiff_open] %s: %s: ", dt_get_wtime() - darktable.start_wtime, type,
            module);
                                         ...
                                         dt_mipmap_buffer_t *mbuf)
{
    ...
    dt_print(DT_DEBUG_IMAGEIO, "[tiff_open] %dx%d %dbpp, %d samples per pixel", t.width, t.height,
             t.bpp, t.spp);
    ...
    if (!t.mipbuf)
    {
        dt_print(DT_DEBUG_ALWAYS, "[tiff_open] error: could not alloc full buffer for '%s'",
                 t.image->filename);
        TIFFClose(t.tiff);
        ...
        t.sampleformat == SAMPLEFORMAT_UINT)
    {
        ok = _read_chunky_8_Lab(&t, photometric);
             ...
             t.bpp == 16 && t.sampleformat == SAMPLEFORMAT_UINT)
    {
...
int dt_imageio_tiff_read_profile(const char *filename, uint8_t **out)
{
    ...
    {
        profile = dt_colorspaces_get_profile(LAB_CONVERSION_PROFILE, "", DT_PROFILE_DIRECTION_ANY)
                      ->profile;
