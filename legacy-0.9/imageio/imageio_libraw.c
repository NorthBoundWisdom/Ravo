...
static gboolean _supported_image(const gchar *filename)
{
    ...
    if (dt_conf_key_not_empty("libraw_extensions"))
        extensions_whitelist =
            g_strjoin(" ", always_by_libraw, dt_conf_get_string_const("libraw_extensions"), NULL);
    else
        extensions_whitelist = g_strdup(always_by_libraw);

    dt_print(DT_DEBUG_IMAGEIO, "[libraw_open] extensions whitelist: '%s'", extensions_whitelist);
...
void _check_libraw_missing_support(const struct dt_image_t *img)
{
    ...
    if (!is_in_glist(warning_missing_support_seen, img->exif_model) &&
        !dt_libraw_lookup_makermodel(img->exif_maker, img->exif_model, lr_mk, sizeof(lr_mk), lr_md,
                                     sizeof(lr_md), lr_al, sizeof(lr_al)))
    {
...
gboolean dt_libraw_lookup_makermodel(const char *maker, const char *model, char *mk, int mk_len,
...
dt_imageio_retval_t dt_imageio_open_libraw(dt_image_t *img, const char *filename,
                                           dt_mipmap_buffer_t *mbuf)
{
    ...
    if (raw->rawdata.color.cam_mul[0] == 0.0f || dt_isnan(raw->rawdata.color.cam_mul[0]) ||
        !raw->rawdata.raw_image)
    {
        dt_print(DT_DEBUG_ALWAYS, "[libraw_open] detected unsupported image `%s'", img->filename);
    ...
    img->raw_white_point = raw->rawdata.color.linear_max[0] ? raw->rawdata.color.linear_max[0] :
    ...
    img->crop_right =
        raw->rawdata.sizes.raw_width - raw->rawdata.sizes.width - raw->rawdata.sizes.left_margin;
    img->crop_bottom =
        raw->rawdata.sizes.raw_height - raw->rawdata.sizes.height - raw->rawdata.sizes.top_margin;
    ...
    if (raw->rawdata.iparams.colors == 3)
    {
        ...
        img->buf_dsc.filters =
            raw->rawdata.iparams.filters & ~((raw->rawdata.iparams.filters & 0x55555555U) << 1);
    }
    else
    {
    ...
    const size_t bufSize_libraw =
        (size_t)raw->rawdata.sizes.raw_pitch * raw->rawdata.sizes.raw_height;
...
error:
    if (libraw_err != LIBRAW_SUCCESS)
    {
        dt_print(DT_DEBUG_ALWAYS, "[libraw_open] `%s': %s", img->filename,
