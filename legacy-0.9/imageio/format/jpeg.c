                ...
                struct dt_dev_pixelpipe_t *pipe, const gboolean export_masks)
{
    ...
    if (jpg->quality > 95)
        jpg->cinfo.dct_method = JDCT_FLOAT;
    if (jpg->quality < 50)
        jpg->cinfo.dct_method = JDCT_IFAST;
    if (jpg->quality < 80)
        jpg->cinfo.smoothing_factor = 20;
    if (jpg->quality < 60)
        jpg->cinfo.smoothing_factor = 40;
    if (jpg->quality < 40)
        jpg->cinfo.smoothing_factor = 60;
...
int read_image(dt_imageio_module_data_t *jpg_tmp, uint8_t *out)
{
    ...
    row_pointer[0] =
        (uint8_t *)dt_alloc_aligned((size_t)jpg->dinfo.output_width * jpg->dinfo.num_components);
    uint8_t *tmp = out;
...
{
    return sizeof(dt_imageio_module_data_t) + sizeof(int) + sizeof(dt_imageio_jpeg_subsample_t);
}
...
int dimension(struct dt_imageio_module_format_t *self, struct dt_imageio_module_data_t *data,
...
void gui_init(dt_imageio_module_format_t *self)
{
    ...
    g->quality = dt_bauhaus_slider_new_with_range(
        (dt_iop_module_t *)self, dt_confgen_get_int("plugins/imageio/format/jpeg/quality", DT_MIN),
        dt_confgen_get_int("plugins/imageio/format/jpeg/quality", DT_MAX), 1,
        dt_confgen_get_int("plugins/imageio/format/jpeg/quality", DT_DEFAULT), 0);
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(
        g->subsample, self, NULL, N_("chroma subsampling"),
        ...
        subsample, subsample_combobox_changed, self, N_("auto"), N_("4:4:4"), N_("4:4:0"),
        N_("4:2:2"), N_("4:2:0"));
