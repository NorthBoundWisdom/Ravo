...
int dt_imageio_jpeg_decompress(dt_imageio_jpeg_t *jpg, uint8_t *out)
{
   ...
   */
    ...
    if (jcs_alpha_valid)
    {
        if (decompress_jsc(jpg, out))
            return 1;
    }
    else
    {
        if (decompress_plain(jpg, out))
            return 1;
    }
#else
    if (decompress_plain(jpg, out))
...
static boolean read_icc_profile(const j_decompress_ptr dinfo, JOCTET **icc_data_ptr,
                                unsigned int *icc_data_len)
{
...
int dt_imageio_jpeg_write_with_icc_profile(const char *filename, const uint8_t *in, const int width,
                                           ...
                                           const int exif_len, const dt_imgid_t imgid)
{
    struct dt_imageio_jpeg_error_mgr jerr;
    dt_imageio_jpeg_t jpg;
    ...
    if (dt_is_valid_imgid(imgid))
    {
        ...
        cmsHPROFILE out_profile =
            dt_colorspaces_get_output_profile(imgid, DT_COLORSPACE_NONE, "")->profile;
        uint32_t len = 0;
...
int dt_imageio_jpeg_write(const char *filename, const uint8_t *in, const int width,
                          const int height, const int quality, const void *exif, const int exif_len)
{
    return dt_imageio_jpeg_write_with_icc_profile(filename, in, width, height, quality, exif,
                                                  exif_len, -1);
}
...
{
    for (jpeg_saved_marker_ptr marker = jpg->dinfo.marker_list; marker != NULL;
         marker = marker->next)
    {
