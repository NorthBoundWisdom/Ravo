...
typedef void (*dt_bauhaus_combobox_populate_fct)(GtkWidget *widget,
                                                 struct dt_iop_module_t **module);

typedef int (*dt_bauhaus_combobox_entry_select_fct)(GtkWidget *widget, const char *entry,
                                                    const int delta,
                                                    struct dt_iop_module_t **module);
...
typedef struct dt_bauhaus_t
{
    ...
    PangoFontDescription
        *pango_font_desc; // no need to recreate this for every string we want to print
    ...
    GdkRGBA color_fg, color_fg_hover, color_fg_insensitive, color_bg, color_border,
        indicator_border, color_fill;

    // colors for graphs
    GdkRGBA graph_bg, graph_exterior, graph_border, graph_fg, graph_grid, graph_fg_active,
        graph_overlay, inset_histogram;
...
void dt_bauhaus_widget_set_quad_toggle(GtkWidget *widget, gboolean toggle);
// set active status for the quad toggle button:
void dt_bauhaus_widget_set_quad_active(GtkWidget *widget, gboolean active);
...
static inline void dt_bauhaus_widget_set_quad(GtkWidget *widget, dt_iop_module_t *self,
                                              dt_bauhaus_quad_paint_f paint, gboolean toggle,
                                              ...
                                              const gchar *tooltip)
{
    ...
    if (tooltip)
        dt_bauhaus_widget_set_quad_tooltip(widget, tooltip);
}
...
GtkWidget *dt_bauhaus_slider_new_with_range_and_feedback(dt_iop_module_t *self, float min,
                                                         float max, float step, float defval,
                                                         int digits, int feedback);
GtkWidget *dt_bauhaus_slider_from_widget(dt_bauhaus_widget_t *widget, dt_iop_module_t *self,
...
char *dt_bauhaus_slider_get_text(GtkWidget *widget, float val);
...
void dt_bauhaus_slider_set_soft_range(GtkWidget *widget, const float soft_min,
...
// combobox:
GtkWidget *dt_bauhaus_combobox_from_widget(dt_bauhaus_widget_t *widget, dt_iop_module_t *self);
...
void dt_bauhaus_combobox_insert_full(GtkWidget *widget, const char *text,
                                     dt_bauhaus_combobox_alignment_t align, gpointer data,
                                     void (*free_func)(void *data), const int pos);
...
void dt_bauhaus_combobox_entry_set_sensitive(GtkWidget *widget, int pos, const gboolean sensitive);
...
#define DT_IOP_SECTION_FOR_PARAMS_UNWIND(self)                                                     \
    while (self && self->actions == DT_ACTION_TYPE_IOP_SECTION)                                    \
    self = (dt_iop_module_t *)self->module
