...
typedef void(_blend_row_func)(const float *const a, const float *const b, float *const out,
...
static inline void _CLAMP_XYZ(dt_aligned_pixel_t XYZ, const dt_aligned_pixel_t min,
                                ...
                                const unsigned int *const restrict invert_mask)
{
    ...
    for (size_t x = 0, j = 0; x < stride; x++, j += DT_BLENDIF_LAB_CH)
    {
        ...
        factor *= _blendif_compute_factor(LCH[2], invert_mask[1],
                                          parameters + DEVELOP_BLENDIF_PARAMETER_ITEMS);
        mask[x] *= factor;
    }
}
...
static void _blendif_combine_channels(const float *const restrict pixels,
                                      float *const restrict mask, const size_t stride,
                                      ...
                                      const float *const restrict parameters)
{
    if (blendif & (1 << DEVELOP_BLENDIF_L_in))
    {
        ...
        _blendif_lab_l(pixels, mask, stride,
                       parameters + DEVELOP_BLENDIF_PARAMETER_ITEMS * DEVELOP_BLENDIF_L_in,
                       invert_mask);
    }
    ...
    if (blendif & (1 << DEVELOP_BLENDIF_A_in))
    {
        ...
        _blendif_lab_a(pixels, mask, stride,
                       parameters + DEVELOP_BLENDIF_PARAMETER_ITEMS * DEVELOP_BLENDIF_A_in,
                       invert_mask);
    }
    ...
    if (blendif & (1 << DEVELOP_BLENDIF_B_in))
    {
        ...
        _blendif_lab_b(pixels, mask, stride,
                       parameters + DEVELOP_BLENDIF_PARAMETER_ITEMS * DEVELOP_BLENDIF_B_in,
                       invert_mask);
    }
    ...
    if (blendif & ((1 << DEVELOP_BLENDIF_C_in) | (1 << DEVELOP_BLENDIF_h_in)))
    {
        ...
        _blendif_lch(pixels, mask, stride,
                     parameters + DEVELOP_BLENDIF_PARAMETER_ITEMS * DEVELOP_BLENDIF_C_in,
                     invert_mask);
    }
}

void dt_develop_blendif_lab_make_mask(dt_dev_pixelpipe_iop_t *piece, const float *const restrict a,
                                      ...
                                      float *const restrict mask)
{
    ...
    if (piece->colors != DT_BLENDIF_LAB_CH)
        return;
    ...
    if (!(d->mask_mode & DEVELOP_MASK_CONDITIONAL) || (!canceling_channel && !any_channel_active))
    {
        ...
        {
            dt_iop_image_mul_const(mask, global_opacity, owidth, oheight,
                                   1); //mask[k] *= global_opacity;
        }
    }
    ...
    else
    {
        ...
        DT_OMP_PRAGMA(parallel default(none) dt_omp_firstprivate(
            temp_mask, mask, a, b, oheight, owidth, iwidth, yoffs, xoffs, buffsize, blendif,
            parameters, mask_inclusive, mask_inversed, global_opacity))
        {
      ...
      for (size_t y = 0; y < oheight; y++)
      {
          ...
          _blendif_combine_channels(a + start, temp_mask + (y * owidth), owidth, blendif,
                                    parameters);
      }
      DT_OMP_PRAGMA(for schedule(static))
      for (size_t y = 0; y < oheight; y++)
      {
          ...
          _blendif_combine_channels(
              b + start, temp_mask + (y * owidth), owidth, blendif >> DEVELOP_BLENDIF_L_out,
      ...
      if (mask_inclusive)
      {
          ...
          for (size_t x = 0; x < buffsize; x++)
              mask[x] = global_opacity * (1.0f - mask[x]) * temp_mask[x];
          }
          else
          {
          DT_OMP_PRAGMA(for simd schedule(static) aligned(mask, temp_mask:64))
          for (size_t x = 0; x < buffsize; x++)
              mask[x] = global_opacity * (1.0f - (1.0f - mask[x]) * temp_mask[x]);
          }
      }
      else
      {
          ...
          DT_OMP_PRAGMA(for simd schedule(static) aligned(mask, temp_mask:64))
          for (size_t x = 0; x < buffsize; x++)
              mask[x] = global_opacity * mask[x] * temp_mask[x];
          }
      }
...
static inline void _blend_Lab_scale(const float *i, float *o)
{
    ...
    for_each_channel(c) o[c] = i[c] * scale[c];
}
...
_BLEND_FUNC _blend_normal_bounded(const float *const a, const float *const b, float *const out,
                                  const float *const restrict mask, const size_t stride,
                                  const dt_aligned_pixel_t min, const dt_aligned_pixel_t max)
{
    for (size_t i = 0; i < stride; i++)
    {
        ...
        for_each_channel(x) tb[x] =
            _CLAMP(ta[x] * (1.0f - local_opacity) + tb[x] * local_opacity, min[x], max[x]);
...
_BLEND_FUNC _blend_normal_unbounded(const float *const a, const float *const b, float *const out,
                                    const float *const restrict mask, const size_t stride,
                                    const dt_aligned_pixel_t min, const dt_aligned_pixel_t max)
{
    for (size_t i = 0; i < stride; i++)
    {
        ...
        for_each_channel(x) tb[x] = ta[x] * (1.0f - local_opacity) + tb[x] * local_opacity;
...
/* lighten */
_BLEND_FUNC _blend_lighten(const float *const a, const float *const b, float *const out,
                           const float *const restrict mask, const size_t stride,
                           const dt_aligned_pixel_t min, const dt_aligned_pixel_t max)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_LAB_CH)
    {
        ...
        tb[0] =
            _CLAMP(ta[0] * (1.0f - local_opacity) + (ta[0] > tb[0] ? ta[0] : tb[0]) * local_opacity,
                   min[0], max[0]);
        tb[1] = _CLAMP(ta[1] * (1.0f - fabsf(tb[0] - ta[0])) +
                           0.5f * (ta[1] + tb[1]) * fabsf(tb[0] - ta[0]),
                       min[1], max[1]);
        tb[2] = _CLAMP(ta[2] * (1.0f - fabsf(tb[0] - ta[0])) +
                           0.5f * (ta[2] + tb[2]) * fabsf(tb[0] - ta[0]),
                       min[2], max[2]);
...
/* darken */
_BLEND_FUNC _blend_darken(const float *const a, const float *const b, float *const out,
                          const float *const restrict mask, const size_t stride,
                          const dt_aligned_pixel_t min, const dt_aligned_pixel_t max)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_LAB_CH)
...
/* multiply */
_BLEND_FUNC _blend_multiply(const float *const a, const float *const b, float *const out,
                            const float *const restrict mask, const size_t stride,
                            const dt_aligned_pixel_t min, const dt_aligned_pixel_t max)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_LAB_CH)
    {
        ...
        tb[0] = _CLAMP(ta[0] * (1.0f - local_opacity) + (ta[0] * tb[0]) * local_opacity, min[0],
                       max[0]);

        const float f = fmaxf(ta[0], 0.01f);
        tb[1] = _CLAMP(ta[1] * (1.0f - local_opacity) + (ta[1] + tb[1]) * tb[0] / f * local_opacity,
                       min[1], max[1]);
        tb[2] = _CLAMP(ta[2] * (1.0f - local_opacity) + (ta[2] + tb[2]) * tb[0] / f * local_opacity,
                       min[2], max[2]);
...
/* average */
_BLEND_FUNC _blend_average(const float *const a, const float *const b, float *const out,
                           const float *const restrict mask, const size_t stride,
                           const dt_aligned_pixel_t min, const dt_aligned_pixel_t max)
{
    for (size_t i = 0; i < stride; i++)
    {
        ...
        for_each_channel(x) tb[x] =
            _CLAMP(ta[x] * (1.0f - local_opacity) + (ta[x] + tb[x]) / 2.0f * local_opacity, min[x],
                   max[x]);
...
/* add */
_BLEND_FUNC _blend_add(const float *const a, const float *const b, float *const out,
                       const float *const restrict mask, const size_t stride,
                       const dt_aligned_pixel_t min, const dt_aligned_pixel_t max)
{
    for (size_t i = 0; i < stride; i++)
    {
        ...
        for_each_channel(x) tb[x] = _CLAMP(
            ta[x] * (1.0f - local_opacity) + (ta[x] + tb[x]) * local_opacity, min[x], max[x]);
...
/* subtract */
_BLEND_FUNC _blend_subtract(const float *const a, const float *const b, float *const out,
                            const float *const restrict mask, const size_t stride,
                            const dt_aligned_pixel_t min, const dt_aligned_pixel_t max)
{
    for (size_t i = 0; i < stride; i++)
    {
        ...
        for_each_channel(x) tb[x] =
            _CLAMP(ta[x] * (1.0f - local_opacity) +
                       ((tb[x] + ta[x]) - (fabsf(min[x] + max[x]))) * local_opacity,
                   min[x], max[x]);
                               ...
                               const dt_aligned_pixel_t min, const dt_aligned_pixel_t max)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_LAB_CH)
    {
        ...
        for_each_channel(x) tb[x] = fabsf(ta[x] - tb[x]) / fabsf(max[x] - min[x]);
        tb[0] = max3f(tb);
...
/* screen */
_BLEND_FUNC _blend_screen(const float *const a, const float *const b, float *const out,
                          const float *const restrict mask, const size_t stride,
                          const dt_aligned_pixel_t min, const dt_aligned_pixel_t max)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_LAB_CH)
    {
        ...
        tb[0] = _CLAMP(la * (1.0f - local_opacity) +
                           ((lmax - (lmax - la) * (lmax - lb))) * local_opacity,
                       lmin, lmax) -
                fabsf(min[0]);

        const float f = fmaxf(ta[0], 0.01f);
        tb[1] = _CLAMP(ta[1] * (1.0f - local_opacity) +
                           0.5f * (ta[1] + tb[1]) * tb[0] / f * local_opacity,
                       min[1], max[1]);
        tb[2] = _CLAMP(ta[2] * (1.0f - local_opacity) +
                           0.5f * (ta[2] + tb[2]) * tb[0] / f * local_opacity,
                       min[2], max[2]);
...
/* overlay */
_BLEND_FUNC _blend_overlay(const float *const a, const float *const b, float *const out,
                           const float *const restrict mask, const size_t stride,
                           const dt_aligned_pixel_t min, const dt_aligned_pixel_t max)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_LAB_CH)
    {
        ...
        const float f = fmaxf(ta[0], 0.01f);
        tb[1] =
            _CLAMP(ta[1] * (1.0f - local_opacity2) + (ta[1] + tb[1]) * tb[0] / f * local_opacity2,
                   min[1], max[1]);
        tb[2] =
            _CLAMP(ta[2] * (1.0f - local_opacity2) + (ta[2] + tb[2]) * tb[0] / f * local_opacity2,
                   min[2], max[2]);
...
/* softlight */
_BLEND_FUNC _blend_softlight(const float *const a, const float *const b, float *const out,
                             const float *const restrict mask, const size_t stride,
                             const dt_aligned_pixel_t min, const dt_aligned_pixel_t max)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_LAB_CH)
...
/* hardlight */
_BLEND_FUNC _blend_hardlight(const float *const a, const float *const b, float *const out,
                             const float *const restrict mask, const size_t stride,
                             const dt_aligned_pixel_t min, const dt_aligned_pixel_t max)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_LAB_CH)
...
/* vividlight */
_BLEND_FUNC _blend_vividlight(const float *const a, const float *const b, float *const out,
                              const float *const restrict mask, const size_t stride,
                              const dt_aligned_pixel_t min, const dt_aligned_pixel_t max)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_LAB_CH)
    {
        ...
        tb[0] = _CLAMP(la * (1.0f - local_opacity2) +
                           (lb > halfmax ?
                                (lb >= lmax ? lmax : la / (doublemax * (lmax - lb))) :
...
/* linearlight */
_BLEND_FUNC _blend_linearlight(const float *const a, const float *const b, float *const out,
                               const float *const restrict mask, const size_t stride,
                               const dt_aligned_pixel_t min, const dt_aligned_pixel_t max)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_LAB_CH)
    {
        ...
        tb[0] = _CLAMP(la * (1.0f - local_opacity2) + (la + doublemax * lb - lmax) * local_opacity2,
                       lmin, lmax) -
                fabsf(min[0]);
...
/* pinlight */
_BLEND_FUNC _blend_pinlight(const float *const a, const float *const b, float *const out,
                            const float *const restrict mask, const size_t stride,
                            const dt_aligned_pixel_t min, const dt_aligned_pixel_t max)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_LAB_CH)
    {
        ...
        tb[0] = _CLAMP(la * (1.0f - local_opacity2) + (lb > halfmax ?
                                                           fmaxf(la, doublemax * (lb - halfmax)) :
...
/* lightness blend */
_BLEND_FUNC _blend_lightness(const float *const a, const float *const b, float *const out,
                             const float *const restrict mask, const size_t stride,
                             const dt_aligned_pixel_t min, const dt_aligned_pixel_t max)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_LAB_CH)
...
/* chroma blend */
_BLEND_FUNC _blend_chromaticity(const float *const a, const float *const b, float *const out,
                                const float *const restrict mask, const size_t stride,
                                const dt_aligned_pixel_t min, const dt_aligned_pixel_t max)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_LAB_CH)
...
/* hue blend */
_BLEND_FUNC _blend_hue(const float *const a, const float *const b, float *const out,
                       const float *const restrict mask, const size_t stride,
                       const dt_aligned_pixel_t min, const dt_aligned_pixel_t max)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_LAB_CH)
...
_BLEND_FUNC _blend_color(const float *const a, const float *const b, float *const out,
                         const float *const restrict mask, const size_t stride,
                         const dt_aligned_pixel_t min, const dt_aligned_pixel_t max)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_LAB_CH)
...
_BLEND_FUNC _blend_coloradjust(const float *const a, const float *const b, float *const out,
                               const float *const restrict mask, const size_t stride,
                               const dt_aligned_pixel_t min, const dt_aligned_pixel_t max)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_LAB_CH)
...
_BLEND_FUNC _blend_Lab_lightness(const float *const a, const float *const b, float *const out,
                                 const float *const restrict mask, const size_t stride,
                                 const dt_aligned_pixel_t min, const dt_aligned_pixel_t max)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_LAB_CH)
...
_BLEND_FUNC _blend_Lab_a(const float *const a, const float *const b, float *const out,
                         const float *const restrict mask, const size_t stride,
                         const dt_aligned_pixel_t min, const dt_aligned_pixel_t max)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_LAB_CH)
...
_BLEND_FUNC _blend_Lab_b(const float *const a, const float *const b, float *const out,
                         const float *const restrict mask, const size_t stride,
                         const dt_aligned_pixel_t min, const dt_aligned_pixel_t max)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_LAB_CH)
...
_BLEND_FUNC _blend_Lab_color(const float *const a, const float *const b, float *const out,
                             const float *const restrict mask, const size_t stride,
                             const dt_aligned_pixel_t min, const dt_aligned_pixel_t max)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_LAB_CH)
...
static inline void _display_channel_value(dt_aligned_pixel_t out, const float value,
                                          const float mask)
{
    out[0] = value;
    out[1] = value;
...
static void _display_channel(const float *const restrict a, float *const restrict b,
                             const float *const restrict mask, const size_t stride,
                             const int channel, const float *const restrict boost_factors)
{
    switch (channel)
    {
    ...
    {
        const float factor =
            1.0f / (128.0f * M_SQRT2_F * exp2f(boost_factors[DEVELOP_BLENDIF_C_in]));
        for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_LAB_CH)
        {
            dt_aligned_pixel_t LCH;
    ...
    {
        const float factor =
            1.0f / (128.0f * M_SQRT2_F * exp2f(boost_factors[DEVELOP_BLENDIF_C_out]));
        for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_LAB_CH)
        {
            dt_aligned_pixel_t LCH;
...
static inline void _copy_mask(const float *const restrict a, float *const restrict b,
                              const size_t stride)
{
    DT_OMP_SIMD(aligned(a, b : 16))
    for (size_t x = DT_BLENDIF_LAB_BCH; x < stride; x += DT_BLENDIF_LAB_CH)
        b[x] = a[x];
}

void dt_develop_blendif_lab_blend(dt_dev_pixelpipe_iop_t *piece, const float *const a,
                                  float *const b, const dt_iop_roi_t *const roi_in,
    ...
    if (request_mask_display & DT_DEV_PIXELPIPE_DISPLAY_ANY)
    {
        ...
        if (profile)
        {
            ...
            for (size_t j = 0; j < buffsize; j += DT_BLENDIF_LAB_CH)
            {
                ...
                for_each_channel(c, aligned(b)) pixel[c] = b[j + c];
                const float yellow_mask =
                    b[j + 3]; // preserve alpha for code which does in-place conversion
                dt_ioppr_rgb_matrix_to_lab(pixel, b + j, profile->matrix_in_transposed,
