...
static void _iop_modify_roi_in(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                               const dt_iop_roi_t *roi_out, dt_iop_roi_t *roi_in)
{
    *roi_in = *roi_out;
}

static void _iop_modify_roi_out(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
...
static void default_init_pipe(dt_iop_module_t *self, dt_dev_pixelpipe_t *pipe,
                              dt_dev_pixelpipe_iop_t *piece)
{
    piece->data = calloc(1, self->params_size);
}

static void default_cleanup_pipe(dt_iop_module_t *self, dt_dev_pixelpipe_t *pipe,
                                 dt_dev_pixelpipe_iop_t *piece)
{
    free(piece->data);
}
...
static void default_process(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
...
void dt_iop_default_init(dt_iop_module_t *module)
{
    ...
    while (i->header.type != DT_INTROSPECTION_TYPE_NONE)
    {
        ...
        {
            if (i->Array.type == DT_INTROSPECTION_TYPE_CHAR)
                break;
...
#include "iop/iop_api.h"
    ...
    if (module->introspection_init)
    {
        if (!module->introspection_init(module, DT_INTROSPECTION_VERSION))
        {
            ...
            if (module->get_p == default_get_p || module->get_f == default_get_f ||
...
gboolean dt_iop_load_module_by_so(dt_iop_module_t *module, dt_iop_module_so_t *so,
                                  dt_develop_t *dev)
{
...
void dt_iop_init_pipe(dt_iop_module_t *module, dt_dev_pixelpipe_t *pipe,
                      dt_dev_pixelpipe_iop_t *piece)
{
...
static gboolean _header_motion_notify_show_callback(GtkWidget *eventbox, GdkEventCrossing *event,
...
static gboolean _header_motion_notify_hide_callback(GtkWidget *eventbox, GdkEventCrossing *event,
                                                    dt_iop_module_t *module)
{
    return dt_iop_show_hide_header_buttons(module, event, FALSE, FALSE);
}

static void _header_menu_deactivate_callback(GtkMenuShell *menushell, dt_iop_module_t *module)
{
...
dt_iop_module_t *dt_iop_gui_get_next_visible_module(const dt_iop_module_t *module)
{
    ...
    for (const GList *modules = g_list_last(module->dev->iop); modules;
...
static gboolean _rename_module_key_press(GtkWidget *entry, GdkEventKey *event,
                                         dt_iop_module_t *module)
{
    gboolean ended = FALSE;

    if (event->type == GDK_FOCUS_CHANGE || event->keyval == GDK_KEY_Return ||
...
static void _rename_module_resize(GtkWidget *entry, gpointer user)
{
    int width = 0;
    GtkBorder padding;
...
void dt_iop_gui_rename_module(dt_iop_module_t *module)
{
    ...
    gtk_entry_set_text(GTK_ENTRY(entry), strcmp(module->multi_name, "0") ||
                                                 module->multi_priority > 0 ||
...
void _get_multi_show(dt_iop_module_t *module, dt_iop_gui_multi_show_t *multi_show)
{
...
static gboolean _gui_multiinstance_callback(GtkButton *button, GdkEventButton *event,
                                            dt_iop_module_t *module)
{
    ...
    g_signal_connect(G_OBJECT(menu), "deactivate", G_CALLBACK(_header_menu_deactivate_callback),
                     module);

    dt_gui_menu_popup(GTK_MENU(menu), GTK_WIDGET(button), GDK_GRAVITY_SOUTH_EAST,
...
static void _update_module_active_class(dt_iop_module_t *module)
{
    ...
    GtkWidget *header_evb = dtgtk_expander_get_header_event_box(DTGTK_EXPANDER(module->expander));
...
static void _gui_off_callback(GtkToggleButton *togglebutton, dt_iop_module_t *module)
{
    ...
    */
    ...
    snprintf(tooltip, sizeof(tooltip),
             module->enabled ? _("'%s' is switched on") : _("'%s' is switched off"), module_label);
    g_free(module_label);
...
void dt_iop_gui_set_enable_button_icon(GtkWidget *w, dt_iop_module_t *module)
{
    ...
    {
        dtgtk_togglebutton_set_paint(DTGTK_TOGGLEBUTTON(w), dtgtk_cairo_paint_switch_off, 0,
                                     module);
    }
    else
    {
...
void dt_iop_set_module_trouble_message(dt_iop_module_t *const module, const char *const trouble_msg,
                                       ...
                                       const char *const stderr_message)
{
    ...
    if (stderr_message)
    {
        ...
        dt_print(DT_DEBUG_ALWAYS, "Trouble: [%s] %s (%s %d)", name, stderr_message,
                 img ? img->filename : "?", img ? img->id : -1);
    }

    if (!dt_iop_is_hidden(module) && module->gui_data &&
        dt_conf_get_bool("plugins/darkroom/show_warnings"))
        DT_CONTROL_SIGNAL_RAISE(DT_SIGNAL_TROUBLE_MESSAGE, module, trouble_msg, trouble_tooltip);
}
...
static void _init_module_so(void *m)
{
    ...
    if (darktable.gui)
    {
        ...
        {
            dt_print(DT_DEBUG_VERBOSE, "loading processing module : %s", module_instance->op);
            ...
            if (!blending_accels_initialized)
            {
                dt_iop_colorspace_type_t cst =
                    module->blend_colorspace(module_instance, NULL, NULL);
...
void _iop_set_darktable_iop_table()
{
    ...
    for (GList *iop = darktable.iop; iop; iop = g_list_next(iop))
    {
        ...
        dt_util_str_cat(&module_list, "(\"%s\",\"%s\"),", module->op, module->name());
    }

    if (module_list)
    {
        module_list[strlen(module_list) - 1] = '\0';
        gchar *query = g_strdup_printf("INSERT INTO memory.darktable_iop_names (operation, name)"
                                       " VALUES %s",
                                       module_list);
...
{
    darktable.iop = dt_module_load_modules("/plugins", sizeof(dt_iop_module_so_t),
                                           dt_iop_load_module_so, _init_module_so, NULL);

    DT_CONTROL_SIGNAL_CONNECT(DT_SIGNAL_PREFERENCES_CHANGE, _iop_preferences_changed,
                              darktable.iop);
...
gboolean dt_iop_load_module(dt_iop_module_t *module, dt_iop_module_so_t *module_so,
                            dt_develop_t *dev)
{
...
void dt_iop_cleanup_module(dt_iop_module_t *module)
{
    ...
    if (darktable.lib && darktable.lib->proxy.colorpicker.picker_proxy &&
                                            ...
                                            const dt_develop_blend_params_t *blendop_params)
{
    ...
    if (blendop_params == module->default_blendop_params || module->dev == NULL)
    {
        module->raster_mask.sink.source = NULL;
        module->raster_mask.sink.id = INVALID_MASKID;
        return NULL;
    ...
    for (GList *iter = module->dev->iop; iter; iter = g_list_next(iter))
    {
        ...
        if (dt_iop_module_is(candidate, blendop_params->raster_mask_source))
        {
            ...
            {
                const gboolean new =
                    g_hash_table_insert(candidate->raster_mask.source.users, module,
                ...
                dt_print_pipe(DT_DEBUG_PIPE | DT_DEBUG_MASKS | DT_DEBUG_VERBOSE,
                              "request raster mask", NULL, module, DT_DEVICE_NONE, NULL, NULL,
                              "from '%s%s' %s", candidate->op, dt_iop_get_instance_id(candidate),
                              new ? "new" : "replaced");
  ...
  */
    ...
    if (sink_source)
    {
        if (g_hash_table_remove(module->raster_mask.sink.source->raster_mask.source.users, module))
            dt_print_pipe(DT_DEBUG_PIPE | DT_DEBUG_MASKS | DT_DEBUG_VERBOSE, "clear as raster user",
                          NULL, module, DT_DEVICE_NONE, NULL, NULL, "from '%s%s'", sink_source->op,
                          dt_iop_get_instance_id(sink_source));
    }
    module->raster_mask.sink.source = NULL;
    module->raster_mask.sink.id = INVALID_MASKID;
    return NULL;
...
gboolean _iop_validate_params(dt_introspection_field_t *field, gpointer params,
                              const gboolean report, const char *name)
{
    ...
    case DT_INTROSPECTION_TYPE_ARRAY:
        ...
        {
            for (int i = 0, item_offset = 0; i < field->Array.count;
                 i++, item_offset += field->Array.field->header.size)
            {
                if (!_iop_validate_params(field->Array.field, (uint8_t *)params + item_offset,
                                          report, name))
                {
                    if (report)
                        dt_print(DT_DEBUG_ALWAYS,
                                 "[iop_validate_params] `%s' failed"
        ...
        break;
    case DT_INTROSPECTION_TYPE_FLOAT:
        all_ok = dt_isnan(*(float *)p) || dt_isinf(*(float *)p) ||
        ...
        values = g_strdup_printf(" (%.8f - [%f..%f] : default %f)%s", *(float *)p, field->Float.Min,
                                 field->Float.Max, field->Float.Default,
                                 relaxed ? " relaxed range" : "");
        break;
    case DT_INTROSPECTION_TYPE_INT:
        all_ok = (*(int *)p >= field->Int.Min && *(int *)p <= field->Int.Max);
        values = g_strdup_printf(" (%d - [%d..%d] : default %d)", *(int *)p, field->Int.Min,
                                 field->Int.Max, field->Int.Default);
        break;
    case DT_INTROSPECTION_TYPE_UINT:
        all_ok = (*(unsigned int *)p >= field->UInt.Min && *(unsigned int *)p <= field->UInt.Max);
        values = g_strdup_printf(" (%ud - [%ud..%ud] : default %ud)", *(unsigned int *)p,
                                 field->UInt.Min, field->UInt.Max, field->UInt.Default);
        break;
    case DT_INTROSPECTION_TYPE_USHORT:
        all_ok = (*(unsigned short int *)p >= field->UShort.Min &&
                  *(unsigned short int *)p <= field->UShort.Max);
        values = g_strdup_printf(" (%ud - [%ud..%ud] : default %ud)", *(unsigned short int *)p,
                                 field->UShort.Min, field->UShort.Max, field->UShort.Default);
        break;
    case DT_INTROSPECTION_TYPE_INT8:
        all_ok = (*(uint8_t *)p >= field->Int8.Min && *(uint8_t *)p <= field->Int8.Max);
        values = g_strdup_printf(" (%ud - [%ud..%ud] : default %ud)", *(uint8_t *)p,
                                 field->Int8.Min, field->Int8.Max, field->Int8.Default);
        break;
    case DT_INTROSPECTION_TYPE_CHAR:
        all_ok = (*(char *)p >= field->Char.Min && *(char *)p <= field->Char.Max);
        values = g_strdup_printf(" (%c - [%c..%c] : default %c)", *(char *)p, field->Char.Min,
    ...
    case DT_INTROSPECTION_TYPE_ENUM:
        ...
        values = g_strdup_printf(" (%d)", *(int *)p);
        break;
    case DT_INTROSPECTION_TYPE_BOOL:
        // *(gboolean*)p
        values = g_strdup_printf(" (%d)", *(gboolean *)p);
        break;
    case DT_INTROSPECTION_TYPE_OPAQUE:
    ...
    if (all_ok)
    {
        dt_print(DT_DEBUG_VERBOSE,
                 "[iop_validate_params] `%s' validated data for type \"%s\"%s%s%s", name,
                 field->header.type_name, *field->header.name ? ", field: " : "",
                 field->header.name, values ? values : "");
    }
    else if (report)
    {
        dt_print(DT_DEBUG_ALWAYS, "[iop_validate_params] `%s' failed for type \"%s\"%s%s%s", name,
                 field->header.type_name, *field->header.name ? ", field: " : "",
                 field->header.name, values ? values : "");
    }
    g_free(values);
                          ...
                          dt_dev_pixelpipe_iop_t *piece)
{
  ...
  */
    ...
    if ((piece->enabled || module->enabled) // better to check for both
        && module->so->get_introspection() && darktable.unmuted & DT_DEBUG_PARAMS)
    {
        if (!_iop_validate_params(module->so->get_introspection()->field, params, TRUE,
                                  module->so->op))
        {
            ...
            dt_control_log(_("'%s' has an introspection error"), module->op);
        }
    }
...
void dt_iop_gui_update(dt_iop_module_t *module)
{
    ...
    if (!dt_iop_is_hidden(module))
    {
        ...
        // if available.
        if (!darktable.develop->history_updating && !darktable.develop->gui_synch &&
...
static gboolean _gui_reset_callback(GtkButton *button, GdkEventButton *event,
                                    dt_iop_module_t *module)
{
...
static gboolean _presets_popup_callback(GtkButton *button, GdkEventButton *event,
                                        dt_iop_module_t *module)
{
    const gboolean disabled = !module->default_enabled && module->hide_enable_button;
    if (disabled)
...
static gboolean _presets_scroll_callback(GtkWidget *widget, GdkEventScroll *event,
                                         dt_iop_module_t *module)
{
...
{
    return module && module->dev && module->dev->gui_attached &&
...
void dt_iop_request_focus(dt_iop_module_t *module)
{
    ...
    if (out_focus_module)
    {
        ...
        gtk_widget_set_state_flags(dt_iop_gui_get_pluginui(out_focus_module), GTK_STATE_FLAG_NORMAL,
                                   TRUE);
        ...
        GtkWidget *hevb =
            dtgtk_expander_get_header_event_box(DTGTK_EXPANDER(out_focus_module->expander));
    ...
    if (module)
    {
        ...
        GtkWidget *hevb =
            dtgtk_expander_get_header_event_box(DTGTK_EXPANDER(dev->gui_module->expander));
...
void dt_iop_gui_set_expanded(dt_iop_module_t *module, const gboolean expanded,
...
static gboolean _iop_plugin_header_button_release(GtkWidget *w, GdkEventButton *e,
                                                  gpointer user_data)
{
...
static void _header_size_callback(GtkWidget *widget, GdkRectangle *allocation, GtkWidget *header)
{
    ...
    int num_buttons = 0;
    for (button = g_list_last(children); button && GTK_IS_BUTTON(button->data);
         button = g_list_previous(button))
        num_buttons++;

    gboolean hide_all = (allocation->width == 1);
    int num_to_unhide = (allocation->width - 2) / button_size.width;
    double opacity_leftmost =
        num_to_unhide > 0 ? 1.0 : (double)allocation->width / button_size.width;
    ...
    if (g_strcmp0(config, "glide")) // glide uses all defaults above
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    opacity_leftmost = 1.0;
                    if (total_alloc.width < panel_trigger_width)
                        hide_all = TRUE;
                }
                                         ...
                                         gboolean show_buttons, const gboolean always_hide)
{
    ...
    else
    {
        if (dynamic)
        {
            ...
            g_signal_connect(G_OBJECT(space), "size-allocate", G_CALLBACK(_header_size_callback),
                             header);
        }
    }
...
static void _display_mask_indicator_callback(GtkToggleButton *bt, dt_iop_module_t *module)
{
    ...
    module->request_mask_display |=
        (is_active ? DT_DEV_PIXELPIPE_DISPLAY_MASK : DT_DEV_PIXELPIPE_DISPLAY_NONE);
...
void dt_iop_add_remove_mask_indicator(dt_iop_module_t *module, gboolean add)
{
    ...
    else if (show)
    {
        ...
        for (child = g_list_last(children); child && GTK_IS_BUTTON(child->data);
        ...
        if (GTK_IS_DRAWING_AREA(child->data))
        {
            ...
            gtk_container_child_get_property(GTK_CONTAINER(module->header), child->data, "position",
                                             &position);
...
static void _iop_tooltip_reposition(GtkWidget *widget, GdkRectangle *allocation, gpointer user_data)
{
    ...
    gtk_widget_translate_coordinates(user_data, gtk_widget_get_toplevel(user_data), 0, 0,
...
gboolean _iop_tooltip_callback(GtkWidget *widget, const gint x, const gint y,
                               const gboolean keyboard_mode, GtkTooltip *tooltip,
                               gpointer user_data)
{
    dt_iop_module_t *module = (dt_iop_module_t *)user_data;
    ...
    if (des[1])
        dt_gui_add_class(label, "dt_section_label");
    ...
    g_signal_connect(G_OBJECT(vbox), "size-allocate", G_CALLBACK(_iop_tooltip_reposition),
                     module->header);

    return dt_shortcut_tooltip_callback(widget, x, y, keyboard_mode, tooltip, vbox);
}

GtkWidget *dt_iop_gui_header_button(dt_iop_module_t *module, DTGTKCairoPaintIconFunc paint,
                                    dt_action_element_t element, GtkWidget *header)
{
    GtkWidget *button;
    ...
    else
    {
        ...
        else if (element == DT_ACTION_ELEMENT_PRESETS)
        {
            ...
            g_signal_connect(button, "scroll-event", G_CALLBACK(_presets_scroll_callback), module);
            gtk_widget_add_events(button, darktable.gui->scroll_mask);
        }
        gtk_box_pack_end(GTK_BOX(header), button, FALSE, FALSE, 0);
    }
    g_signal_connect(button, "enter-notify-event", G_CALLBACK(_header_enter_notify_callback),
...
static gboolean _on_drag_motion(GtkWidget *widget, GdkDragContext *dc, const gint x, const gint y,
                                const guint time, dt_iop_module_t *dest)
{
    ...
    else
    {
        ...
        gtk_container_child_get(GTK_CONTAINER(panel), dest->expander, "position", &position, NULL);
...
static gboolean _on_drag_drop(GtkWidget *widget, GdkDragContext *dc, const gint x, const gint y,
                              const guint time, dt_iop_module_t *module)
{
...
void dt_iop_gui_set_expander(dt_iop_module_t *module)
{
   ...
   */
    ...
    module->presets_button = dt_iop_gui_header_button(module, dtgtk_cairo_paint_presets,
                                                      DT_ACTION_ELEMENT_PRESETS, header);
    module->reset_button =
        dt_iop_gui_header_button(module, dtgtk_cairo_paint_reset, DT_ACTION_ELEMENT_RESET, header);
    module->multimenu_button = dt_iop_gui_header_button(module, dtgtk_cairo_paint_multiinstance,
                                                        DT_ACTION_ELEMENT_INSTANCE, header);
    if (!(module->flags() & IOP_FLAGS_ONE_INSTANCE))
        gtk_widget_set_tooltip_text(
    ...
    /* add enabled button */
    module->off = dt_iop_gui_header_button(module, dtgtk_cairo_paint_switch,
                                           DT_ACTION_ELEMENT_ENABLE, header);
    ...
    if (state == IOP_STATE_HIDDEN)
    {
        for (mods = darktable.develop->iop; mods; mods = g_list_next(mods))
        {
            ...
            if (mod->so == module && mod->expander)
                gtk_widget_hide(GTK_WIDGET(mod->expander));
        }
...
void dt_iop_update_multi_name(dt_iop_module_t *module, const char *name, const gboolean hand_edited,
                              const gboolean enable, const gboolean force)
{
    const gboolean auto_module = dt_conf_get_bool("darkroom/ui/auto_module_name_update");

    char *l_name = g_strstrip(g_strdup(name));

    if (force || (auto_module && !module->multi_name_hand_edited))
    {
...
gboolean dt_iop_piece_is_raster_mask_used(const dt_dev_pixelpipe_iop_t *piece,
...
void dt_iop_piece_set_raster(dt_dev_pixelpipe_iop_t *piece, float *mask,
                             const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    dt_print_pipe(DT_DEBUG_PIPE | DT_DEBUG_MASKS, "write raster mask", piece->pipe, piece->module,
                  DT_DEVICE_NONE, roi_in, roi_out, "%s (%ix%i)", new ? "new" : "replaced",
...
void dt_iop_piece_clear_raster(dt_dev_pixelpipe_iop_t *piece, float *mask)
{
    ...
    {
        dt_print_pipe(DT_DEBUG_PIPE | DT_DEBUG_MASKS, "delete raster mask", piece->pipe,
                      piece->module, piece->pipe->devid, NULL, NULL);
        dt_dev_pixelpipe_cache_invalidate_later(piece->pipe, piece->module->iop_order,
                                                "clear raster: ");
    }
    dt_free_align(mask);
...
dt_iop_module_t *dt_iop_get_module_by_op_priority(GList *modules, const char *operation,
                                          ...
                                          const gboolean prefer_first)
{
    ...
    for (GList *iop_mods = g_list_last(darktable.develop->iop); iop_mods;
         iop_mods = g_list_previous(iop_mods))
    {
        ...
        {
            if (enabled_mode == FILTER_REQUIRE && !mod->enabled)
                continue;
...
dt_iop_module_t *dt_iop_get_module_preferred_instance(const dt_iop_module_so_t *module)
{
  ...
  */
    ...
    if (DT_ACTION(module) == &darktable.control->actions_focus ||
        (gui_module && gui_module->so == module && dt_conf_get_bool("accel/prefer_focused")))
        return gui_module;
    else
        return _find_preferred_instance(module, dt_conf_get_bool("accel/prefer_expanded"),
                                        dt_conf_get_bool("accel/prefer_enabled") ? FILTER_PREFER :
                                                                                   FILTER_NEUTRAL,
...
dt_iop_module_t *
dt_iop_get_module_enabled_preferring_unmasked_first_instance(const dt_iop_module_so_t *module)
{
    return _find_preferred_instance(module, FALSE, FILTER_REQUIRE, TRUE, TRUE);
}
...
{
    for (const GList *iop_mods = g_list_last(darktable.develop->iop); iop_mods;
         iop_mods = g_list_previous(iop_mods))
    {
        dt_iop_module_t *mod = iop_mods->data;
...
dt_iop_module_t *dt_iop_get_module_by_instance_name(GList *modules, const char *operation,
...
{
    return (module->multi_priority > 0 || module->multi_name_hand_edited) ? module->multi_name : "";
}
...
const char **dt_iop_set_description(dt_iop_module_t *module, const char *main_text,
                                    const char *purpose, const char *input, const char *process,
                                    const char *output)
{
...
gboolean dt_iop_have_required_input_format(const int req_ch, dt_iop_module_t *const module,
                                           const int ch, const void *const restrict ivoid,
        ...
        if (module)
        {
            dt_iop_set_module_trouble_message(module, _("unsupported input"),
                                              _("you have placed this module at\n"
                                                ...
                                                "its requirements."),
                                              NULL);
            dt_print_pipe(DT_DEBUG_ALWAYS, "unsupported data format", NULL, module, DT_DEVICE_NONE,
                          roi_in, roi_out);
        }
        else
        {
            dt_print_pipe(DT_DEBUG_ALWAYS, "unsupported data format", NULL, NULL, DT_DEVICE_NONE,
                          roi_in, roi_out, "no module given");
        }
        return FALSE;
    }
}
...
{
    return dt_iop_color_picker_is_visible(dev) || darktable.lib->proxy.snapshots.enabled;
}

void dt_iop_gui_changed(dt_action_t *action, GtkWidget *widget, gpointer data)
{
    if (!action || action->type != DT_ACTION_TYPE_IOP_INSTANCE)
        return;
    dt_iop_module_t *module = (dt_iop_module_t *)action;

    if (module->gui_changed)
...
gboolean dt_iop_module_is_skipped(const dt_develop_t *dev, const dt_iop_module_t *module)
{
    return dev->gui_module && dev->gui_module != module &&
                             ...
                             const dt_action_effect_t effect, float move_size)
{
    ...
    if (DT_PERFORM_ACTION(move_size))
    {
        ...
        case DT_ACTION_ELEMENT_FOCUS:
            if (DT_ACTION_TOGGLE_NEEDED(effect, move_size, dt_dev_gui_module() == module))
                _request_module_focus_callback(module);
            break;
        case DT_ACTION_ELEMENT_ENABLE:
            if (DT_ACTION_TOGGLE_NEEDED(
                    effect, move_size,
        ...
        case DT_ACTION_ELEMENT_SHOW:
            if (DT_ACTION_TOGGLE_NEEDED(effect, move_size, module->expanded))
        ...
        case DT_ACTION_ELEMENT_RESET:
        {
            GdkEventButton event = {
                .state = (effect == DT_ACTION_EFFECT_ACTIVATE_CTRL ? GDK_CONTROL_MASK : 0)};
        ...
        dt_action_widget_toast(target, NULL, "%s, %s", dt_action_def_iop.elements[element].name,
...
const gchar *dt_action_effect_instance[] = {N_("show"),   N_("move up"), N_("move down"), N_("new"),
                                            N_("delete"), N_("rename"),  N_("duplicate"), NULL};
...
const dt_action_def_t dt_action_def_iop = {N_("processing module"), _action_process,
                                           _action_elements, _action_fallbacks};
