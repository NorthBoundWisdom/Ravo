...
_blend_default_module_blend_colorspace(dt_iop_module_t *module, const gboolean is_scene_referred)
{
    ...
    for (size_t i = 0, j = 0; i < DEVELOP_BLENDIF_SIZE; i++, j += DEVELOP_BLENDIF_PARAMETER_ITEMS)
    {
        if (blendif & (1 << i))
        {
            ...
            if (blend_csp == DEVELOP_BLEND_CS_LAB &&
                (i == DEVELOP_BLENDIF_A_in || i == DEVELOP_BLENDIF_A_out ||
                                                 ...
                                                 const dt_develop_blend_colorspace_t cst)
{
    ...
    const dt_iop_order_iccprofile_info_t *const profile =
        (cst == DEVELOP_BLEND_CS_RGB_SCENE) ?
...
static void _refine_with_detail_mask(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                                     float *mask, const dt_iop_roi_t *const roi_in,
                                     const dt_iop_roi_t *const roi_out, const float level)
{
    if (feqf(level, 0.0f, 1e-6f))
        return;
    ...
    dt_print_pipe(DT_DEBUG_PIPE, "refine with detail mask", piece->pipe, self, DT_DEVICE_CPU,
                  roi_in, roi_out);
...
error:
    dt_print_pipe(DT_DEBUG_PIPE | DT_DEBUG_MASKS, "refine with detail mask", piece->pipe, self,
                  DT_DEVICE_CPU, roi_in, roi_out, "no mask data available");
                                   ...
                                   _develop_mask_post_processing operations[3])
{
    ...
    const gboolean mask_feather_out = bp->feathering_guide == DEVELOP_MASK_GUIDE_OUT_BEFORE_BLUR ||
    ...
    if (mask_feather)
    {
        ...
        {
            operations[index++] =
                mask_feather_out ? DEVELOP_MASK_POST_FEATHER_OUT : DEVELOP_MASK_POST_FEATHER_IN;
            if (mask_blur)
...
static void _develop_blend_process_feather(const float *const guide, float *const mask,
                                           const size_t width, const size_t height, const int ch,
                                           const float guide_weight, const float feathering_radius,
                                           const float scale, const float sqrt_eps)
{
    ...
    if (mask_bak)
    {
        ...
        guided_filter(guide, mask_bak, mask, width, height, ch, w, sqrt_eps, guide_weight, 0.f,
                      1.f);
...
static void _develop_blend_process_mask_tone_curve(float *const restrict mask,
                                                   const size_t buffsize, const float contrast,
                                                   const float brightness, const float opacity)
{
...
static const char *_develop_blend_colorspace_to_str(const dt_develop_blend_colorspace_t type)
{
    ...
    case DEVELOP_BLEND_CS_NONE:
        return "BLEND_CS_NONE";
    case DEVELOP_BLEND_CS_RAW:
        return "BLEND_CS_RAW";
    case DEVELOP_BLEND_CS_LAB:
        return "BLEND_CS_LAB";
    case DEVELOP_BLEND_CS_RGB_DISPLAY:
        return "BLEND_CS_RGB_DISPLAY";
    case DEVELOP_BLEND_CS_RGB_SCENE:
        return "BLEND_CS_RGB_SCENE";
    default:
        return "invalid BLEND_CS";
    }
}

/* we test in pixelpipe processing if this required */
void dt_develop_blend_process(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                              ...
                              const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    const gboolean inside_roi = (roi_in->width - dx >= owidth) && (roi_in->height - dy >= oheight);
  ...
  */
    if (!inside_roi)
    {
        dt_print_pipe(DT_DEBUG_ALWAYS, "dt_develop_blend", piece->pipe, self, DT_DEVICE_CPU, roi_in,
                      roi_out, "skip blending, work area mismatch");
        return;
    }
    ...
    const dt_dev_pixelpipe_display_mask_t request_mask_display =
        valid_request && (mode_parametric || mode_drawn) ? self->request_mask_display :
                                                           DT_DEV_PIXELPIPE_DISPLAY_NONE;

    const dt_dev_pixelpipe_display_mask_t request_raster_display =
        valid_request && raster ? self->request_mask_display : DT_DEV_PIXELPIPE_DISPLAY_NONE;
    ...
    const gboolean suppress_mask =
        self->suppress_mask && valid_request && (mask_mode & ~DEVELOP_MASK_ENABLED);
    ...
    if (!_mask)
    {
        dt_print_pipe(DT_DEBUG_PIPE, "dt_develop_blend", piece->pipe, self, DT_DEVICE_CPU, roi_in,
                      roi_out, "could not allocate buffer for blending");
        return;
    }
    ...
    */
        gboolean free_mask;
        float *raster_mask = dt_dev_get_raster_mask(piece, self->raster_mask.sink.source,
                                                    self->raster_mask.sink.id, self, &free_mask);
        if (raster_mask)
        {
            dt_print_pipe(DT_DEBUG_PIPE, "blend raster", piece->pipe, self, DT_DEVICE_CPU, roi_in,
                          roi_out, "%s%s%s", dt_iop_colorspace_to_name(cst),
    ...
    else
    {
        ...
        dt_print_pipe(DT_DEBUG_PIPE, form && form_ok ? "blend with form" : "blend without form",
                      piece->pipe, self, DT_DEVICE_CPU, roi_in, roi_out, "%s, %s%s%s",
                      dt_iop_colorspace_to_name(cst), _develop_blend_colorspace_to_str(blend_csp),
                      inverted ? ", inverted" : "", rois_equal ? "" : ", roi differ");
        ...
        case DEVELOP_BLEND_CS_LAB:
            dt_develop_blendif_lab_make_mask(piece, (const float *const restrict)ivoid,
                                             (const float *const restrict)ovoid, roi_in, roi_out,
                                             mask);
            break;
        case DEVELOP_BLEND_CS_RGB_DISPLAY:
            dt_develop_blendif_rgb_hsl_make_mask(piece, (const float *const restrict)ivoid,
                                                 (const float *const restrict)ovoid, roi_in,
                                                 roi_out, mask);
            break;
        case DEVELOP_BLEND_CS_RGB_SCENE:
    ...
    if (!uniform)
    {
        ...
        for (size_t index = 0; index < post_operations_size; ++index)
        {
            ...
            {
                if (rois_equal)
                    _develop_blend_process_feather((float *restrict)ivoid, mask, owidth, oheight,
                                                   ch, guide_weight, d->feathering_radius,
                                                   roi_out->scale / piece->iscale, sqrt_eps);
                else
                {
                    ...
                    if (guide)
                    {
                        ...
                        _develop_blend_process_feather(guide, mask, owidth, oheight, ch,
                                                       guide_weight, d->feathering_radius,
                                                       roi_out->scale / piece->iscale, sqrt_eps);
                        dt_free_align(guide);
                    }
                }
            ...
            {
                _develop_blend_process_feather((const float *const restrict)ovoid, mask, owidth,
                                               oheight, ch, guide_weight, d->feathering_radius,
                                               roi_out->scale / piece->iscale, sqrt_eps);
            }
            ...
            {
                _develop_blend_process_mask_tone_curve(mask, obuffsize, d->contrast, d->brightness,
                                                       opacity);
            }
            else
            {
    ...
    case DEVELOP_BLEND_CS_LAB:
        dt_develop_blendif_lab_blend(piece, (const float *const restrict)ivoid,
                                     (float *const restrict)ovoid, roi_in, roi_out, mask,
...
#ifdef HAVE_OPENCL
static void _refine_with_detail_mask_cl(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                                        float *mask, const dt_iop_roi_t *roi_in,
                                        const dt_iop_roi_t *roi_out, const float level,
                                        const int devid)
{
    if (feqf(level, 0.0f, 1e-6f))
        return;
    ...
    if (p->scharr.data == NULL)
    {
        dt_print_pipe(DT_DEBUG_PIPE | DT_DEBUG_OPENCL, "no detail data available", piece->pipe,
                      self, devid, roi_in, roi_out);
        return;
    }
    ...
    err = dt_opencl_write_buffer_to_device(devid, p->scharr.data, out, 0,
                                           sizeof(float) * iwidth * iheight, TRUE);
    if (err != CL_SUCCESS)
        goto error;

    err = dt_opencl_enqueue_kernel_2d_args(devid, darktable.opencl->blendop->kernel_calc_blend,
                                           iwidth, iheight, CLARG(out), CLARG(blur), CLARG(iwidth),
                                           CLARG(iheight), CLARG(threshold), CLARG(detail));
    if (err != CL_SUCCESS)
        goto error;
    ...
    err = dt_opencl_read_buffer_from_device(devid, lum, out, 0, sizeof(float) * iwidth * iheight,
                                            TRUE);
    if (err != CL_SUCCESS)
        goto error;
    ...
    dt_print_pipe(DT_DEBUG_PIPE | DT_DEBUG_VERBOSE, "refine with detail mask", piece->pipe, self,
                  devid, roi_in, roi_out);
...
error:
    ...
    dt_print_pipe(DT_DEBUG_PIPE | DT_DEBUG_OPENCL, "refine with detail_mask", piece->pipe, self,
                  piece->pipe->devid, roi_in, roi_out, "OpenCL error: %s", cl_errstr(err));
...
/* we test in pixelpipe processing if this required */
gboolean dt_develop_blend_process_cl(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                                     cl_mem dev_in, cl_mem dev_out, const dt_iop_roi_t *roi_in,
                                     const dt_iop_roi_t *roi_out)
{
    ...
    if (!inside_roi)
    {
        dt_print_pipe(DT_DEBUG_PIPE, "dt_develop_blend", piece->pipe, self, piece->pipe->devid,
                      roi_in, roi_out, "skip OpenCL blending, work area mismatch");
        return TRUE;
    }
    ...
    if (!_mask)
    {
        dt_print_pipe(DT_DEBUG_ALWAYS, "dt_develop_blend", piece->pipe, self, piece->pipe->devid,
                      roi_in, roi_out, "could not allocate buffer for blending");
        return FALSE;
    }
    ...
    err =
        dt_ioppr_build_iccprofile_params_cl(use_profile ? &profile : NULL, devid, &profile_info_cl,
                                            &profile_lut_cl, &dev_profile_info, &dev_profile_lut);
    if (err != CL_SUCCESS)
    {
        dt_print(DT_DEBUG_OPENCL, "[opencl_blendop] profile_info_cl: %s", cl_errstr(err));
        goto error;
    }
    ...
    if (uniform)
    {
        ...
        err = dt_opencl_enqueue_kernel_2d_args(devid, kernel_set_mask, owidth, oheight,
                                               CLARG(dev_mask), CLARG(owidth), CLARG(oheight),
                                               CLARG(opacity));
        if (err != CL_SUCCESS)
        {
            dt_print(DT_DEBUG_OPENCL, "[opencl_blendop] kernel_set_mask: %s", cl_errstr(err));
            goto error;
        }
    }
    else if (raster)
    ...
    */
        ...
        if (raster_mask)
        {
            dt_print_pipe(DT_DEBUG_PIPE, "blend raster", piece->pipe, self, piece->pipe->devid,
                          roi_in, roi_out, "%s%s%s", dt_iop_colorspace_to_name(cst),
                          d->raster_mask_invert ? " inverted" : "",
                          free_mask ? " temp" : " permanent");
            // invert if required
            if (d->raster_mask_invert)
    ...
    else
    {
        ...
        err =
            dt_opencl_write_host_to_image(devid, mask, dev_mask_2, owidth, oheight, sizeof(float));
        if (err != CL_SUCCESS)
            goto error;
        ...
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, kernel_mask, owidth, oheight, CLARG(dev_in), CLARG(dev_out), CLARG(dev_mask_2),
            CLARG(dev_mask), CLARG(owidth), CLARG(oheight), CLARG(opacity), CLARG(blendif),
            CLARG(dev_blendif_params), CLARG(mask_mode), CLARG(d->mask_combine), CLARRAY(2, offs),
            CLARG(dev_profile_info), CLARG(dev_profile_lut), CLARG(use_profile));
        if (err != CL_SUCCESS)
        {
            dt_print(DT_DEBUG_OPENCL, "[opencl_blendop] apply global opacity: %s", cl_errstr(err));
            goto error;
        }
    }
    ...
    if (!uniform && post_operations_size)
    {
        ...
        for (size_t index = 0; index < post_operations_size; index++)
        {
            ...
            if (operation == DEVELOP_MASK_POST_FEATHER_IN)
            {
                ...
                {
                    cl_mem dev_guide =
                        dt_opencl_alloc_device(devid, owidth, oheight, sizeof(float) * ch);
                    if (dev_guide == NULL)
                        goto error;

                    const size_t origin_1[2] = {dx, dy};
                    err = dt_opencl_enqueue_copy_image(devid, dev_in, dev_guide, CLIMG_ORIGIN,
                                                       origin_1, region);
                    if (err != CL_SUCCESS)
                    {
                    ...
                    err = guided_filter_cl(devid, dev_guide, dev_mask, dev_mask_2, owidth, oheight,
                                           ch, featherw, sqrt_eps, guide_weight, 0.0f, 1.0f);
                    dt_opencl_release_mem_object(dev_guide);
                    if (err != CL_SUCCESS)
                        goto error;
                }
            ...
            else if (operation == DEVELOP_MASK_POST_BLUR)
            {
                ...
                dt_gaussian_cl_t *g =
                    dt_gaussian_init_cl(devid, owidth, oheight, 1, mmax, mmin, sigma, 0);
                err = CL_MEM_OBJECT_ALLOCATION_FAILURE;
                if (!g)
                    goto error;
            ...
            else if (operation == DEVELOP_MASK_POST_TONE_CURVE)
            {
                ...
                err = dt_opencl_enqueue_kernel_2d_args(devid, kernel_mask_tone_curve, owidth,
                                                       oheight, CLARG(dev_mask), CLARG(dev_mask_2),
                                                       CLARG(owidth), CLARG(oheight), CLARG(e),
                                                       CLARG(d->brightness), CLARG(opacity));
                if (err != CL_SUCCESS)
                {
    ...
    if (request_mask_display & DT_DEV_PIXELPIPE_DISPLAY_ANY)
    {
        ...
        err = CL_MEM_OBJECT_ALLOCATION_FAILURE;
        dev_boost_factors = dt_opencl_copy_host_to_device_constant(
            devid, sizeof(d->blendif_boost_factors), d->blendif_boost_factors);
        if (dev_boost_factors == NULL)
            goto error;
        ...
        err = dt_ioppr_build_iccprofile_params_cl(work_profile, devid, &work_profile_info_cl,
                                                  &work_profile_lut_cl, &dev_work_profile_info,
                                                  &dev_work_profile_lut);
        if (err != CL_SUCCESS)
        {
            dt_print(DT_DEBUG_OPENCL, "[opencl_blendop] work_profile_info_cl: %s", cl_errstr(err));
            goto error;
        }
        // let us display a specific channel
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, kernel_display_channel, owidth, oheight, CLARG(dev_in), CLARG(dev_tmp),
            CLARG(dev_mask), CLARG(dev_out), CLARG(owidth), CLARG(oheight), CLARRAY(2, offs),
            CLARG(request_mask_display), CLARG(dev_boost_factors), CLARG(dev_profile_info),
            CLARG(dev_profile_lut), CLARG(use_profile), CLARG(dev_work_profile_info),
    ...
    else
    {
        ...
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, kernel, owidth, oheight, CLARG(dev_in), CLARG(dev_tmp), CLARG(dev_mask),
            CLARG(dev_out), CLARG(owidth), CLARG(oheight), CLARG(d->blend_mode),
            CLARG(blend_parameter), CLARRAY(2, offs), CLARG(mask_display));
        if (err != CL_SUCCESS)
        {
            dt_print(DT_DEBUG_OPENCL, "[opencl_blendop] blend_parameter: %s", cl_errstr(err));
            goto error;
        }
    }
    ...
    if (dt_iop_piece_is_raster_mask_used(piece, BLEND_RASTER_ID))
    {
        ...
        {
            err =
                dt_opencl_copy_image_to_host(devid, mask, dev_mask, owidth, oheight, sizeof(float));
            if (err != CL_SUCCESS)
            {
                dt_iop_piece_clear_raster(piece, _mask);
                goto error;
...
dt_blendop_cl_global_t *dt_develop_blend_init_cl_global(void)
{
    ...
    b->kernel_blendop_RAW4 = dt_opencl_create_kernel(program, "blendop_RAW4");
    ...
    b->kernel_calc_Y0_mask = dt_opencl_create_kernel(program, "calc_Y0_mask");
    b->kernel_calc_scharr_mask = dt_opencl_create_kernel(program, "calc_scharr_mask");
    b->kernel_calc_blend = dt_opencl_create_kernel(program, "calc_detail_blend");
...
/** report back specific memory requirements for blend step (only relevant for OpenCL path).
    ...
    are not met.
*/
void tiling_callback_blendop(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                             ...
                             dt_develop_tiling_t *tiling)
{
    ...
    if (bldata->details != 0.0f)
    {
        ...
        if (details->data)
        {
            tiling->factor = 0.5f * (float)(details->roi.width * details->roi.height) /
    ...
    if (bldata->feathering_radius > 0.1f) // we don't feather below that
    {
      ...
      */
        ...
        tiling->factor =
            MAX(tiling->factor,
                18.0f * 0.25f); // we need all 18 intermediate guided filter mask buffers
    ...
    const float outnorm =
        (float)(roi_out->width * roi_out->height) / (roi_in->width * roi_in->height);
