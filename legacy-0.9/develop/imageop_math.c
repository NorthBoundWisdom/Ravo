...
void dt_iop_flip_and_zoom_8(const uint8_t *in, const int32_t iw, const int32_t ih, uint8_t *out,
                            const int32_t ow, const int32_t oh,
                            const dt_image_orientation_t orientation, uint32_t *width,
                            uint32_t *height)
{
    // init strides:
...
void dt_iop_clip_and_zoom_8(const uint8_t *i, const int32_t ix, const int32_t iy, const int32_t iw,
                            const int32_t ih, const int32_t ibw, const int32_t ibh, uint8_t *o,
                            const int32_t ox, const int32_t oy, const int32_t ow, const int32_t oh,
                            const int32_t obw, const int32_t obh)
{
        ...
        for (int t = 0; t < ow2; t++)
        {
            for (int k = 0; k < 3; k++)
                o[4 * idx + k] = // i[3*(ibw* (int)y +             (int)x             ) + k)];
                    CLAMP(
                        ((int32_t)i[(4 * (ibw * (int32_t)y + (int32_t)(x + .5f * scalex)) + k)] +
                         (int32_t)i[(
                             4 * (ibw * (int32_t)(y + .5f * scaley) + (int32_t)(x + .5f * scalex)) +
                             k)] +
...
void dt_iop_clip_and_zoom(float *out, const float *const in, const dt_iop_roi_t *const roi_out,
                          const dt_iop_roi_t *const roi_in, const gboolean gamma)
{
                                ...
                                const dt_iop_roi_t *const roi_out, const dt_iop_roi_t *const roi_in)
{
    ...
    if (err == CL_INVALID_WORK_GROUP_SIZE)
    {
        ...
        if (out && in)
        {
            err = dt_opencl_copy_image_to_host(devid, in, dev_in, roi_in->width, roi_in->height,
                                               4 * sizeof(float));
            if (err == CL_SUCCESS)
            {
                dt_iop_clip_and_zoom_roi(out, in, roi_out, roi_in);
                err = dt_opencl_write_host_to_image(devid, out, dev_out, roi_out->width,
                                                    roi_out->height, 4 * sizeof(float));
            }
        }
        if (err == CL_SUCCESS)
                                           ...
                                           const uint32_t filters)
{
    ...
    for (int y = 0; y < roi_out->height; y++)
    {
        ...
        const int miny =
            (CLAMPS((int)floorf(fy - px_footprint), 0, roi_in->height - 3) & ~1u) + rggby;
        ...
        {
            const int minx =
                (CLAMPS((int)floorf(fx - px_footprint), 0, roi_in->width - 3) & ~1u) + rggbx;
    ...
    for (int y = 0; y < roi_out->height; y++)
    {
        ...
        for (int x = 0; x < roi_out->width; x++)
        {
            ...
            if (maxi == px + 2 * samples && maxj == py + 2 * samples)
            {
                ...
                p[0] = in[maxi + 2 + in_stride * (maxj + 2)];
                p[1] =
                    in[maxi + 3 + in_stride * (maxj + 2)] + in[maxi + 2 + in_stride * (maxj + 3)];
                                                       ...
                                                       const uint8_t (*const xtrans)[6])
{
    ...
    {
        float *outc = out + 4 * (out_stride * y);
        const int py = CLAMPS((int)round((y - 0.5f) * px_footprint), 0, roi_in->height - 3);
        ...
        for (int x = 0; x < roi_out->width; x++, outc += 4)
        {
            ...
            int num = 0;
            const int px = CLAMPS((int)round((x - 0.5f) * px_footprint), 0, roi_in->width - 3);
