...
void dt_dev_init(dt_develop_t *dev, const gboolean gui_attached)
{
...
void dt_dev_cleanup(dt_develop_t *dev)
{
    ...
    if (dev->histogram_pre_tonecurve)
        free(dev->histogram_pre_tonecurve);
    if (dev->histogram_pre_levels)
...
{
    if (!dev->gui_attached || dev->full.pipe->processing)
        return;
    const gboolean err =
        dt_control_add_job_res(dt_dev_process_image_job_create(dev), DT_CTL_WORKER_ZOOM_1);
    if (err)
        dt_print(DT_DEBUG_ALWAYS, "[dev_process_image] job queue exceeded!");
}
...
void dt_dev_process_preview(dt_develop_t *dev)
{
    if (!dev->gui_attached)
        return;
    const gboolean err =
        dt_control_add_job_res(dt_dev_process_preview_job_create(dev), DT_CTL_WORKER_ZOOM_FILL);
    if (err)
        dt_print(DT_DEBUG_ALWAYS, "[dev_process_preview] job queue exceeded!");
}

void dt_dev_process_preview2(dt_develop_t *dev)
{
    if (!dev->gui_attached && !dev->preview2.widget)
        return;
    const gboolean err =
        dt_control_add_job_res(dt_dev_process_preview2_job_create(dev), DT_CTL_WORKER_ZOOM_2);
    if (err)
        dt_print(DT_DEBUG_ALWAYS, "[dev_process_preview2] job queue exceeded!");
}
...
static void _activate_pinned_dev(dt_develop_t *dev, dt_develop_t *new_pinned_dev)
{
    ...
    {
        g_signal_handlers_block_matched(G_OBJECT(dev->preview2.pin_button), G_SIGNAL_MATCH_DATA, 0,
                                        0, NULL, NULL, dev);
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(dev->preview2.pin_button), TRUE);
        g_signal_handlers_unblock_matched(G_OBJECT(dev->preview2.pin_button), G_SIGNAL_MATCH_DATA,
                                          0, 0, NULL, NULL, dev);
...
{
    if (!dev || !dt_is_valid_imgid(imgid))
        return;
...
static void _dev_average_delay_update(const dt_times_t *start, uint32_t *average_delay)
{
    dt_times_t end;
    dt_get_times(&end);
...
void dt_dev_process_image_job(dt_develop_t *dev, dt_dev_viewport_t *port, dt_dev_pixelpipe_t *pipe,
                              dt_signal_t signal, const int devid)
{
    ...
    dt_mipmap_cache_get(&buf, dev->image_storage.id, port ? DT_MIPMAP_FULL : DT_MIPMAP_F,
                        port ? DT_MIPMAP_BLOCKING : DT_MIPMAP_BEST_EFFORT, 'r');
    ...
    if (pipe->loading)
    {
        ...
        if (pipe == dev->full.pipe)
        {
            ...
            else
            {
                ...
                pipe->loading = FALSE;
                if (dev->preview_pipe)
                    dev->preview_pipe->loading = FALSE;
                if (dev->preview2.pipe)
                    dev->preview2.pipe->loading = FALSE;
            }
...
restart:
  ...
  */
    ...
    */
    ...
    const dt_dev_pixelpipe_stopper_t shutdown =
        dt_atomic_exch_int(&pipe->shutdown, DT_DEV_PIXELPIPE_STOP_NO);
    const gboolean stopped = early || shutdown != DT_DEV_PIXELPIPE_STOP_NO;

    const dt_iop_roi_t proi =
        (dt_iop_roi_t){.x = x, .y = y, .width = wd, .height = ht, .scale = scale};
    dt_print_pipe(DT_DEBUG_PIPE, stopped ? "pixelpipe_process stopped" : "pixelpipe_process good",
                  ...
                  restarts > 5 ? "LOOPING " : "", dt_dev_pixelpipe_shutdown_to_str(shutdown),
    ...
    if (stopped)
    {
        ...
        const gboolean img_changed =
            dev->image_force_reload || pipe->loading || pipe->input_changed;
        if (img_changed)
        {
            if (dev->image_force_reload)
            {
    ...
    */
        ...
        {
            dt_print_pipe(DT_DEBUG_PIPE | DT_DEBUG_VERBOSE, "image_job restart", pipe, NULL,
                          DT_DEVICE_NONE, &proi, NULL);
            goto restart;
        }
    }
  ...
  */
    ...
    */
    ...
    dt_print_pipe(DT_DEBUG_PIPE, "process_image_job done", pipe, NULL, DT_DEVICE_NONE, &proi, NULL,
                  "\n");
    dt_show_times_f(&start, "[dev_process_image] pixel pipeline", "processing `%s'",
...
float dt_dev_get_zoom_scale(dt_dev_viewport_t *port, const dt_dev_zoom_t zoom,
                            const int closeup_factor, const gboolean preview)
{
    float zoom_scale = .0f;
...
float dt_dev_get_zoomed_in(void)
{
    ...
    const float min_scale =
        dt_dev_get_zoom_scale(&darktable.develop->full, DT_ZOOM_FIT, 1 << closeup, FALSE);
    const float cur_scale =
        dt_dev_get_zoom_scale(&darktable.develop->full, zoom, 1 << closeup, FALSE);
...
void dt_dev_load_image(dt_develop_t *dev, const dt_imgid_t imgid)
{
    ...
    if (dev->full.pipe)
    {
        ...
        dev->full.pipe->loading = dev->preview_pipe->loading = dev->preview2.pipe->loading = TRUE;
        dev->full.pipe->status = dev->preview_pipe->status = dev->preview2.pipe->status =
...
static void _dev_write_history_item(const dt_imgid_t imgid, dt_dev_history_item_t *h,
                                    const int32_t num)
{
    sqlite3_stmt *stmt;
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2
    ...
    DT_DEBUG_SQLITE3_BIND_BLOB(stmt, 7, h->blend_params, sizeof(dt_develop_blend_params_t),
                               SQLITE_TRANSIENT);
...
static void _dev_auto_save(dt_develop_t *dev)
{
  ...
  */
    ...
    const gboolean saving = (user_delay >= 1.0) && ((start - dev->autosave_time) > user_delay) &&
                            !dev->full.pipe->loading && dev->requested_id == imgid &&
...
static void _dev_auto_module_label(dt_develop_t *dev, dt_iop_module_t *module)
{
    // adjust the label to match presets if possible or otherwise the default
    // multi_name for this module.
    if (!dt_iop_is_hidden(module) && !module->multi_name_hand_edited &&
        dt_conf_get_bool("darkroom/ui/auto_module_name_update"))
    {
        ...
        char *preset_name = dt_presets_get_module_label(
            module->op, module->params, module->params_size, is_default_params,
...
static void _dev_add_history_item_ext(dt_develop_t *dev, dt_iop_module_t *module,
                                      const gboolean enable, const gboolean new_item,
                                      const gboolean no_image, const gboolean include_masks,
                                      const gboolean auto_name_module)
{
    ...
    while (history)
    {
        ...
        if ((!hist->module->hide_enable_button && !hist->module->default_enabled) || earlier_entry)
        {
    ...
    if (!history    // no history yet, push new item
        ...
        || ((dev->focus_hash != hist->focus_hash)                 // or if focused out and in
            ...
            && ((module->params_size != hist->module->params_size) || include_masks ||
...
void dt_dev_add_history_item_ext(dt_develop_t *dev, dt_iop_module_t *module, const gboolean enable,
                                 const int no_image)
{
...
static gboolean _dev_undo_start_record_target(dt_develop_t *dev, gpointer target)
{
    ...
    if (target && target == dev->gui_previous_target && this_time < MIN(merge_time, review_time))
    {
        return FALSE;
    }
...
static void _dev_add_history_item(dt_develop_t *dev, dt_iop_module_t *module, const gboolean enable,
                                  const gboolean new_item, const gpointer target)
{
    if (!darktable.gui || DT_IN_GUI_UPDATE())
        return;
    ...
    if (!dev->history_postpone_invalidate || module != dev->gui_module)
...
void dt_dev_add_history_item(dt_develop_t *dev, dt_iop_module_t *module, const gboolean enable)
{
    _dev_add_history_item(dev, module, enable, FALSE, NULL);
}

void dt_dev_add_history_item_target(dt_develop_t *dev, dt_iop_module_t *module,
...
void dt_dev_add_masks_history_item_ext(dt_develop_t *dev, dt_iop_module_t *_module,
                                       const gboolean _enable, const gboolean no_image)
{
...
void dt_dev_add_masks_history_item(dt_develop_t *dev, dt_iop_module_t *module,
                                   const gboolean enable)
{
    ...
    if (form && gui)
    {
        ...
        if (fpt)
            target = GINT_TO_POINTER(fpt->formid);
    }

    dt_pthread_mutex_lock(&dev->history_mutex);

    const gboolean need_end_record = _dev_undo_start_record_target(dev, target);
...
static void _cleanup_history(const dt_imgid_t imgid)
{
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "DELETE FROM main.masks_history WHERE imgid = ?1", -1, &stmt, NULL);
    DT_DEBUG_SQLITE3_BIND_INT(stmt, 1, imgid);
    sqlite3_step(stmt);
    sqlite3_finalize(stmt);
}
...
void dt_dev_write_history_ext(dt_develop_t *dev, const dt_imgid_t imgid)
{
    ...
    dt_print(DT_DEBUG_IOPORDER,
             "[dt_dev_write_history_ext] Writing history image id=%d `%s', iop version: %i", imgid,
             dev->image_storage.filename, dev->iop_order_version);
    for (int i = 0; history; i++)
    {
        ...
        dt_print(DT_DEBUG_IOPORDER, "%20s, num %2i, order %2d, v(%i), multiprio %i%s",
                 hist->module->op, i, hist->iop_order, hist->module->version(),
                 hist->multi_priority, (hist->enabled) ? ", enabled" : "");
...
static int _dev_get_module_nb_records(void)
{
    sqlite3_stmt *stmt;
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "SELECT count (*) FROM  memory.history", -1, &stmt, NULL);
    sqlite3_step(stmt);
...
static void _dev_insert_module(dt_develop_t *dev, dt_iop_module_t *module, const dt_imgid_t imgid)
{
    sqlite3_stmt *stmt;
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(
        dt_database_get(darktable.db),
        "INSERT INTO memory.history VALUES (?1, 0, ?2, ?3, ?4, 1, NULL, 0, 0, ?5, 0)", -1, &stmt,
        NULL);
    DT_DEBUG_SQLITE3_BIND_INT(stmt, 1, imgid);
...
static gboolean _dev_auto_apply_presets(dt_develop_t *dev)
{
    ...
    if (!(image->flags & DT_IMAGE_AUTO_PRESETS_APPLIED))
        run = TRUE;
  ...
  snprintf(query, sizeof(query),
    ...
    //       the darktable internal ones.
        ...
        DT_DEBUG_SQLITE3_BIND_DOUBLE(stmt, 10,
                                     fmaxf(0.0f, fminf(1000000, image->exif_focal_length)));
        // 0: dontcare, 1: ldr, 2: raw plus monochrome & color
        DT_DEBUG_SQLITE3_BIND_INT(stmt, 11, iformat);
        DT_DEBUG_SQLITE3_BIND_INT(stmt, 12, excluded);
...
static void _dev_add_default_modules(dt_develop_t *dev, const dt_imgid_t imgid)
{
    ...
    for (GList *modules = dev->iop; modules; modules = g_list_next(modules))
    {
        ...
        if (!dt_history_check_module_exists(imgid, module->op, FALSE) && module->default_enabled &&
            module->hide_enable_button && !(module->flags() & IOP_FLAGS_NO_HISTORY_STACK))
        {
...
static void _dev_merge_history(dt_develop_t *dev, const dt_imgid_t imgid)
{
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        if (cnt > 0)
        {
            ...
            DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                        "SELECT rowid FROM memory.history ORDER BY rowid DESC", -1,
                                        &stmt, NULL);
            ...
            DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                        "UPDATE memory.history SET num=?1 WHERE rowid=?2", -1,
                                        &stmt, NULL);
            ...
            // history.
            DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                        "UPDATE main.history SET num=num+?1 WHERE imgid=?2", -1,
                                        &stmt, NULL);
...
static void _dev_write_history(dt_develop_t *dev, const dt_imgid_t imgid)
{
    _cleanup_history(imgid);
    // write history entries
    GList *history = dev->history;
...
void dt_dev_read_history_ext(dt_develop_t *dev, const dt_imgid_t imgid, const gboolean no_image)
{
    if (!dt_is_valid_imgid(imgid))
        return;
    if (!dev->iop)
    ...
    {
        // cleanup
        DT_DEBUG_SQLITE3_EXEC(dt_database_get(darktable.db), "DELETE FROM memory.history", NULL,
                              NULL, NULL);
        ...
        dt_print(DT_DEBUG_PARAMS, "[dt_dev_read_history_ext] temporary history initialised with"
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "SELECT history_end FROM main.images WHERE id = ?1", -1, &stmt,
                                NULL);
    DT_DEBUG_SQLITE3_BIND_INT(stmt, 1, imgid);
  ...
  else
  {
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        dt_print(DT_DEBUG_PARAMS,
                 ...
                 module_name, blendop_version, _print_validity(is_valid_blendop_version),
                 _print_validity(is_valid_blendop_size), modversion,
                 _print_validity(is_valid_module_version), _print_validity(is_valid_params_size));
        ...
        hist->multi_priority =
            (hist->module->flags() & IOP_FLAGS_ONE_INSTANCE) ? 0 : multi_priority;
    ...
    {
        dt_print(DT_DEBUG_PARAMS, "[dt_dev_read_history_ext] reset defaults for workflow none");
    ...
    if (first_run)
    {
        ...
        {
            flags =
                flags | (auto_apply_modules_count ? DT_HISTORY_HASH_AUTO : DT_HISTORY_HASH_BASIC);
        }
...
gboolean dt_dev_get_zoom_bounds(dt_dev_viewport_t *port, float *zoom_x, float *zoom_y, float *boxw,
                                float *boxh)
{
...
gboolean dt_dev_get_preview_size(const dt_develop_t *dev, float *wd, float *ht)
{
...
gboolean dt_dev_get_processed_size(dt_dev_viewport_t *port, int *procw, int *proch)
{
...
// escape hatch.
static void _zoom_constraint_bounds(const gboolean constrained, const float tscaleold,
                                    const float tscalefit, float *tscalemin, float *tscalemax)
{
    ...
    case SIZE_LARGE:
        *tscalemax = constrained ?
                         (tscaleold > 2.0f ? tscaletop : (tscaleold > 1.0f ? 2.0f : 1.0f)) :
                         tscaletop;
                                               ...
                                               const float tscaleold, const float tscalefit)
{
    ...
    return up ? MIN(tscalenew, tscalemax) : MAX(tscalenew, tscalemin);
}
...
static char *_transform_type(const dt_dev_transform_direction_t transf_direction)
{
    ...
    case DT_DEV_TRANSFORM_DIR_ALL:
        return "all included";
    case DT_DEV_TRANSFORM_DIR_FORW_INCL:
        return "forward inclusive";
    case DT_DEV_TRANSFORM_DIR_FORW_EXCL:
        return "forward exclusive";
    case DT_DEV_TRANSFORM_DIR_BACK_INCL:
        return "backward inclusive";
    case DT_DEV_TRANSFORM_DIR_BACK_EXCL:
        return "backward exclusive";
    case DT_DEV_TRANSFORM_DIR_ALL_GEOMETRY:
...
static gboolean _dev_distort_transform_locked(dt_develop_t *dev, dt_dev_pixelpipe_t *pipe,
                                              const gboolean back, const double iop_order,
                                              ...
                                              float *points, const size_t points_count)
{
    ...
    const gboolean log = (darktable.unmuted & (DT_DEBUG_CONTROL | DT_DEBUG_VERBOSE)) ==
                         (DT_DEBUG_CONTROL | DT_DEBUG_VERBOSE);
    ...
    while (modules)
    {
        ...
        if (piece->enabled && transform && piece->data &&
            ((transf_direction == DT_DEV_TRANSFORM_DIR_ALL) ||
             ...
             (transf_direction == DT_DEV_TRANSFORM_DIR_BACK_EXCL &&
              module->iop_order < iop_order)) &&
            !(dt_iop_module_is_skipped(dev, module) && (pipe->type & DT_DEV_PIXELPIPE_BASIC)))
        {
            if (log)
            {
                char vbuf[1024] = {0};
                ...
                for (int p = 0; p < tested; p++)
                {
                    ...
                    snprintf(vbuf + done, sizeof(vbuf) - done, "  [P%d] %.1f %.1f -> %.1f %.1f", p,
                             old[2 * p], old[2 * p + 1], points[2 * p], points[2 * p + 1]);
                }
                dt_print_pipe(DT_DEBUG_ALWAYS, "distort_transform", pipe, module, DT_DEVICE_NONE,
                              NULL, NULL, "%s %s, order=%d, %d points:%s",
                              (back ? "back" : "forward"), _transform_type(transf_direction),
...
void dt_dev_zoom_move(dt_dev_viewport_t *port, dt_dev_zoom_t zoom, float scale, int closeup,
                      const float x, const float y, const gboolean constrain)
{
    ...
    _dev_distort_transform_locked(dev, port->pipe, FALSE, 0.0f, DT_DEV_TRANSFORM_DIR_ALL_GEOMETRY,
                                  pts, 1);
    ...
    else
    {
        ...
        else if (zoom == DT_ZOOM_SCROLL)
        {
            ...
            const float tscale =
                _calculate_new_scroll_zoom_tscale(closeup, constrain, tscaleold, fitscale * ppd);
            scale = tscale / ppd;
        ...
        else if (zoom == DT_ZOOM_FREE && constrain)
        {
            ...
            const float tscaleold = cur_scale * ppd;
            float tscalemin, tscalemax;
            _zoom_constraint_bounds(TRUE, tscaleold, fitscale * ppd, &tscalemin, &tscalemax);
    ...
    const gboolean has_moved =
        fabsf(pts[0] - old_pts0) + fabsf(pts[1] - old_pts1) > moved_threshold ||
        (zoom == DT_ZOOM_MOVE && (x || y));
    if (has_moved)
    {
        _dev_distort_transform_locked(dev, port->pipe, TRUE, 0.0f,
                                      DT_DEV_TRANSFORM_DIR_ALL_GEOMETRY, pts, 1);
        port->zoom_x = pts[0];
        port->zoom_y = pts[1];
    }
    ...
    if (!has_moved && fabsf(old_zoom_scale - port->zoom_scale) < 0.01f && old_zoom == port->zoom &&
        old_closeup == port->closeup)
        return;

    dt_print_pipe(DT_DEBUG_PIPE | DT_DEBUG_CONTROL | DT_DEBUG_VERBOSE,
                  "dt_dev_zoom_move sets DT_DEV_PIPE_ZOOMED", port->pipe, NULL, DT_DEVICE_NONE,
...
void dt_dev_get_pointer_zoom_pos(dt_dev_viewport_t *port, const float px, const float py,
                                 float *zoom_x, float *zoom_y, float *zoom_scale)
{
    dt_dev_zoom_t zoom;
...
void dt_dev_get_pointer_zoom_pos_from_bounds(dt_dev_viewport_t *port, const float px,
                                             const float py, const float zbound_x,
                                             const float zbound_y, float *zoom_x, float *zoom_y,
                                             float *zoom_scale)
{
    dt_dev_zoom_t zoom;
    int closeup = 0, procw = 0, proch = 0;
...
void dt_dev_get_viewport_params(dt_dev_viewport_t *port, dt_dev_zoom_t *zoom, int *closeup,
                                float *x, float *y)
{
    dt_pthread_mutex_lock(&(darktable.control->global_mutex));
    ...
    if (zoom)
        *zoom = port->zoom;
    if (closeup)
        *closeup = port->closeup;

    if (x && y && port->pipe)
    {
        float pts[2] = {port->zoom_x, port->zoom_y};
        dt_dev_distort_transform_plus(port->dev ? port->dev : darktable.develop, port->pipe, 0.0f,
                                      DT_DEV_TRANSFORM_DIR_ALL_GEOMETRY, pts, 1);
...
gboolean dt_dev_is_current_image(const dt_develop_t *dev, const dt_imgid_t imgid)
{
...
{
    if (!dev->proxy.exposure.module || dt_view_get_current() != DT_VIEW_DARKROOM)
        return NULL;
...
float dt_dev_exposure_get_exposure(dt_develop_t *dev)
{
    ...
    return instance && instance->get_exposure && instance->module->enabled ?
               instance->get_exposure(instance->module) :
               0.0f;
}
...
float dt_dev_exposure_get_effective_exposure(dt_develop_t *dev)
{
    ...
    if (exposure_so)
    {
        dt_iop_module_t *preferred_exposure_instance =
            dt_iop_get_module_enabled_preferring_unmasked_first_instance(exposure_so);
...
{
    const dt_dev_proxy_exposure_t *instance = _dev_exposure_proxy_available(dev);
    return instance && instance->get_black && instance->module->enabled ?
               instance->get_black(instance->module) :
               0.0f;
}

void dt_dev_exposure_handle_event(int n_press, gdouble delta, GdkModifierType state,
...
gboolean dt_dev_modulegroups_test(dt_develop_t *dev, const uint32_t group, dt_iop_module_t *module)
{
...
int dt_dev_modulegroups_basics_module_toggle(dt_develop_t *dev, GtkWidget *widget,
                                             const gboolean doit)
{
    if (dev->proxy.modulegroups.module && dev->proxy.modulegroups.basics_module_toggle)
        return dev->proxy.modulegroups.basics_module_toggle(dev->proxy.modulegroups.module, widget,
                                                            doit);
    return 0;
}
...
void dt_dev_masks_list_remove(dt_develop_t *dev, const dt_mask_id_t formid,
...
void dt_dev_masks_selection_change(dt_develop_t *dev, dt_iop_module_t *module,
...
dt_iop_module_t *dt_dev_module_duplicate_ext(dt_develop_t *dev, dt_iop_module_t *base,
...
{
    return dt_history_get_name_label(module->name(), module->multi_name, FALSE,
                                     module->multi_name_hand_edited);
}

gboolean dt_dev_distort_transform(dt_develop_t *dev, float *points, const size_t points_count)
{
    return dt_dev_distort_transform_plus(dev, dev->preview_pipe, 0.0f, DT_DEV_TRANSFORM_DIR_ALL,
                                         points, points_count);
}

gboolean dt_dev_distort_backtransform(dt_develop_t *dev, float *points, const size_t points_count)
{
    return dt_dev_distort_backtransform_plus(dev, dev->preview_pipe, 0.0f, DT_DEV_TRANSFORM_DIR_ALL,
                                             points, points_count);
}

gboolean dt_dev_distort_transform_plus(dt_develop_t *dev, dt_dev_pixelpipe_t *pipe,
...
gboolean dt_dev_distort_backtransform_plus(dt_develop_t *dev, dt_dev_pixelpipe_t *pipe,
                                           const double iop_order,
                                           const dt_dev_transform_direction_t transf_direction,
                                           float *points, const size_t points_count)
{
...
dt_dev_pixelpipe_iop_t *dt_dev_distort_get_iop_pipe(dt_develop_t *dev, dt_dev_pixelpipe_t *pipe,
...
dt_hash_t dt_dev_hash_plus(dt_develop_t *dev, dt_dev_pixelpipe_t *pipe, const double iop_order,
                           const dt_dev_transform_direction_t transf_direction)
{
...
static gboolean _dev_wait_hash(dt_develop_t *dev, dt_dev_pixelpipe_t *pipe, const double iop_order,
                               const dt_dev_transform_direction_t transf_direction,
                               dt_pthread_mutex_t *lock, const volatile dt_hash_t *const hash)
{
    const int usec = 5000;
    int nloop;
...
gboolean dt_dev_sync_pixelpipe_hash(dt_develop_t *dev, dt_dev_pixelpipe_t *pipe,
                                    const double iop_order,
                                    const dt_dev_transform_direction_t transf_direction,
                                    dt_pthread_mutex_t *lock, const volatile dt_hash_t *const hash)
{
...
dt_hash_t dt_dev_hash_distort_plus(dt_develop_t *dev, dt_dev_pixelpipe_t *pipe,
                                   const double iop_order,
                                   const dt_dev_transform_direction_t transf_direction)
{
    dt_hash_t hash = DT_INITHASH;
...
void dt_dev_image(const dt_imgid_t imgid, const size_t width, const size_t height,
                  const int history_end, uint8_t **buf, float *scale, size_t *buf_width,
                  size_t *buf_height, dt_dev_zoom_pos_t zoom_pos, const int snapshot_id,
                  GList *module_filter_out, const int devid, const gboolean finalscale)
{
    dt_develop_t dev;
    ...
    pipe->type |= DT_DEV_PIXELPIPE_IMAGE |
                  (finalscale ? DT_DEV_PIXELPIPE_IMAGE_FINAL : DT_DEV_PIXELPIPE_NONE);
    ...
    const uint32_t bufsize = sizeof(uint32_t) * pipe->backbuf_width * pipe->backbuf_height;
    *buf = dt_alloc_aligned(bufsize);
    ...
    if (zoom_pos)
        memcpy(zoom_pos, pipe->backbuf_zoom_pos, sizeof(dt_dev_zoom_pos_t));
...
{
    return feqf(f[0], (float)d[0], 0.00001f) && feqf(f[1], (float)d[1], 0.00001f) &&
...
void dt_dev_reset_chroma(dt_develop_t *dev)
{
    ...
    for_four_channels(c) chr->wb_coeffs[c] = 1.0f;
}
