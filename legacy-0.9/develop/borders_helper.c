...
void dt_iop_copy_image_with_border(float *out, const float *const in,
...
void dt_iop_setup_binfo(const dt_dev_pixelpipe_iop_t *piece, const dt_iop_roi_t *const roi_in,
                        const dt_iop_roi_t *const roi_out, const float pos_v, const float pos_h,
                        const float *bcolor, const float *fcolor, const float f_size,
                        const float f_offset, dt_iop_border_positions_t *binfo)
{
    ...
    const int border_min_size = MIN(MIN(binfo->border_size_t, binfo->border_size_b),
                                    MIN(binfo->border_size_l, binfo->border_size_r));
        ...
        binfo->frame_br_out_x =
            (f_offset == 1.0f &&
             (MIN(binfo->border_size_l, binfo->border_size_r) - border_min_size < 2)) ?
                (roi_out->width) :
                CLAMP(image_lx - frame_offset - binfo->frame_size + frame_out_width - 1, 0,
                      roi_out->width - 1);
        binfo->frame_br_out_y =
            (f_offset == 1.0f &&
             (MIN(binfo->border_size_t, binfo->border_size_b) - border_min_size < 2)) ?
                (roi_out->height) :
                CLAMP(image_ty - frame_offset - binfo->frame_size + frame_out_height - 1, 0,
                      roi_out->height - 1);
