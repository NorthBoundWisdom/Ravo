...
typedef struct dt_dev_pixelpipe_iop_t
{
    ...
    uint32_t
        *histogram; // pointer to histogram data; histogram_bins_count bins with 4 channels each
    ...
    dt_iop_roi_t
        buf_in; // theoretical full buffer regions of interest, as passed through modify_roi_out
    ...
    gboolean
        process_cl_ready; // set this to FALSE in commit_params to temporarily disable the use of process_cl
    gboolean
        process_tiling_ready; // set this to FALSE in commit_params to temporarily disable tiling
...
void dt_dev_pixelpipe_set_input(dt_dev_pixelpipe_t *pipe, struct dt_develop_t *dev, float *input,
...
void dt_dev_pixelpipe_get_dimensions(dt_dev_pixelpipe_t *pipe, struct dt_develop_t *dev,
...
gboolean dt_dev_pixelpipe_process(dt_dev_pixelpipe_t *pipe, struct dt_develop_t *dev, const int x,
                                  const int y, const int width, const int height, const float scale,
                                  const int devid);
// convenience method that does not gamma-compress the image.
gboolean dt_dev_pixelpipe_process_no_gamma(dt_dev_pixelpipe_t *pipe, struct dt_develop_t *dev,
...
float *dt_dev_get_raster_mask(dt_dev_pixelpipe_iop_t *piece,
                              ...
                              const struct dt_iop_module_t *target_module, gboolean *free_mask);
// some helper functions related to the details mask interface
void dt_dev_clear_scharr_mask(dt_dev_pixelpipe_t *pipe);

gboolean dt_dev_write_scharr_mask(dt_dev_pixelpipe_iop_t *piece, float *const rgb,
                                  const dt_iop_roi_t *const roi_in, const gboolean mode);
#ifdef HAVE_OPENCL
...
void dt_print_pipe_ext(const char *title, const dt_dev_pixelpipe_t *pipe,
                       const struct dt_iop_module_t *mod, const int device,
