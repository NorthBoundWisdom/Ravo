...
GtkWidget *dt_bauhaus_slider_from_params(dt_iop_module_t *self, const char *param)
{
    ...
    if (f)
    {
        if (f->header.type == DT_INTROSPECTION_TYPE_FLOAT)
        {
            ...
            slider =
                dt_bauhaus_slider_new_with_range_and_feedback(self, min, max, 0, defval, digits, 1);
        }
    ...
    {
        gchar *str =
            g_strdup_printf("'%s' is not a float/int/unsigned short/slider parameter", param_name);
...
GtkWidget *dt_bauhaus_combobox_from_params(dt_iop_module_t *self, const char *param)
{
              ...
              f->header.type == DT_INTROSPECTION_TYPE_BOOL))
    {
        ...
        if (f->header.type == DT_INTROSPECTION_TYPE_BOOL)
        {
            ...
            dt_bauhaus_combobox_set_default(combobox,
...
GtkWidget *dt_bauhaus_toggle_from_params(dt_iop_module_t *self, const char *param)
{
    ...
    if (f && f->header.type == DT_INTROSPECTION_TYPE_BOOL)
    {
        ...
        g_signal_connect_data(G_OBJECT(button), "toggled", G_CALLBACK(_iop_toggle_callback),
                              module_param, (GClosureNotify)g_free, 0);
...
GtkWidget *dt_iop_button_new(dt_iop_module_t *self, const gchar *label, GCallback callback,
