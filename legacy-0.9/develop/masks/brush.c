...
static void _brush_bounding_box(const float *const points, const float *const border,
                                const int nb_corner, const int num_points, int *width, int *height,
                                int *posx, int *posy);
...
static float _brush_point_line_distance2(const int index, const int pointscount,
                                         const float *points, const float *payload)
{
...
static GList *_brush_ramer_douglas_peucker(const float *points, const int points_count,
                                           const float *payload, const float epsilon2)
{
...
static void _brush_get_XY(const float p0x, const float p0y, const float p1x, const float p1y,
 ...
 * border point */
static void _brush_border_get_XY(const float p0x, const float p0y, const float p1x, const float p1y,
                                 const float p2x, const float p2y, const float p3x, const float p3y,
...
static void _brush_ctrl2_to_feather(const float ptx, const float pty, const float ctrlx,
                                    const float ctrly, float *fx, float *fy,
...
/** the values should be in orthonormal space */
static void _brush_feather_to_ctrl(const float ptx, const float pty, const float fx, const float fy,
...
static void _brush_catmull_to_bezier(const float x1, const float y1, const float x2, const float y2,
...
static void _brush_init_ctrl_points(dt_masks_form_t *form)
{
    ...
    for (GList *form_points = form->points; form_points; form_points = g_list_next(form_points))
    {
        ...
        if (point3->state & DT_MASKS_POINT_STATE_NORMAL)
        {
            ...
            {
                start_point[0].corner[0] = start_point[1].corner[0] =
                    2 * point3->corner[0] - point4->corner[0];
                start_point[0].corner[1] = start_point[1].corner[1] =
                    2 * point3->corner[1] - point4->corner[1];
            ...
            {
                end_point[0].corner[0] = end_point[1].corner[0] =
                    2 * point3->corner[0] - point2->corner[0];
                end_point[0].corner[1] = end_point[1].corner[1] =
                    2 * point3->corner[1] - point2->corner[1];
                point4 = &(end_point[0]);
                point5 = &(end_point[1]);
            }
            else if (point5 == NULL)
            ...
            _brush_catmull_to_bezier(point1->corner[0], point1->corner[1], point2->corner[0],
            ...
            point3->ctrl1[0] = bx2;
            point3->ctrl1[1] = by2;
            _brush_catmull_to_bezier(point2->corner[0], point2->corner[1], point3->corner[0],
...
static void _brush_points_recurs_border_small_gaps(float *cmax, float *bmin, float *bmin2,
                                                   float *bmax, dt_masks_dynbuf_t *dpoints,
                                                   dt_masks_dynbuf_t *dborder)
{
    // we want to find the start and end angles
    const float a1 = fmodf(atan2f(bmin[1] - cmax[1], bmin[0] - cmax[0]) + DT_2PI_F, DT_2PI_F);
    const float a2 = fmodf(atan2f(bmax[1] - cmax[1], bmax[0] - cmax[0]) + DT_2PI_F, DT_2PI_F);
    ...
    float delta = a2 - a1;
    if (fabsf(delta) > M_PI_F)
        delta = delta - copysignf(DT_2PI_F, delta);
...
static void _brush_points_stamp(float *cmax, float *bmin, dt_masks_dynbuf_t *dpoints,
                                dt_masks_dynbuf_t *dborder, const gboolean clockwise)
{
 ...
 * points */
static void _brush_points_recurs(float *p1, float *p2, const double tmin, const double tmax,
                                 float *points_min, float *points_max, float *border_min,
                                 float *border_max, float *rpoints, float *rborder, float *rpayload,
                                 dt_masks_dynbuf_t *dpoints, dt_masks_dynbuf_t *dborder,
                                 dt_masks_dynbuf_t *dpayload)
{
                          ...
                          (int)border_min[1] - (int)border_max[1] > -1))))
    {
        ...
        if (withborder)
        {
            ...
            {
                _brush_points_recurs_border_small_gaps(points_max, border_min, NULL, border_max,
                                                       dpoints, dborder);
            }
    ...
    _brush_points_recurs(p1, p2, tmin, tx, points_min, c, border_min, b, rc, rb, rp, dpoints,
                         dborder, dpayload);
    _brush_points_recurs(p1, p2, tx, tmax, rc, points_max, rb, border_max, rpoints, rborder,
                         rpayload, dpoints, dborder, dpayload);
}
...
static int _brush_get_pts_border(dt_develop_t *dev, dt_masks_form_t *form, const double iop_order,
                                 const int transf_direction, dt_dev_pixelpipe_t *pipe,
                                 float **points, int *points_count, float **border,
                                 int *border_count, float **payload, int *payload_count,
                                 const int source)
{
    ...
    *points_count = 0;
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] brush_points init took %0.04f sec",
    ...
    for (int n = 0; n < 2 * nb; n++)
    {
        ...
        _brush_points_recurs(p1, p2, 0.0, 1.0, cmin, cmax, bmin, bmax, rc, rb, rp, dpoints, dborder,
                             dpayload);
        ...
        if (dborder && nb >= 3)
        {
            ...
            _brush_border_get_XY(p3[0], p3[1], p3[2], p3[3], p4[2], p4[3], p4[0], p4[1], 0, p3[4],
                                 cmin, cmin + 1, bmax, bmax + 1);
            if (bmax[0] == DT_INVALID_COORDINATE)
            {
                _brush_border_get_XY(p3[0], p3[1], p3[2], p3[3], p4[2], p4[3], p4[0], p4[1], 0.0001,
    ...
    *points = dt_masks_dynbuf_harvest(dpoints);
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] brush_points point recurs %0.04f sec",
             form->name, dt_get_lap_time(&start2));
                                    ...
                                    const int source, const dt_iop_module_t *module)
{
    ...
    return _brush_get_pts_border(dev, form, ioporder, DT_DEV_TRANSFORM_DIR_ALL, dev->preview_pipe,
                                 points, points_count, border, border_count, NULL, NULL, source);
}
...
static float _brush_get_position_in_segment(const float x, const float y, dt_masks_form_t *form,
                                            const int segment)
{
    ...
    for (int i = 0; i <= 100; i++)
    {
        ...
        _brush_get_XY(point0->corner[0], point0->corner[1], point1->corner[0], point1->corner[1],
                      point2->corner[0], point2->corner[1], point3->corner[0], point3->corner[1], t,
                      &sx, &sy);
...
static int _brush_events_mouse_scrolled(dt_iop_module_t *module, const float pzx, const float pzy,
    ...
    if (gui->creation)
    {
        if (dt_modifier_is(state, GDK_SHIFT_MASK))
        {
            const float masks_hardness = dt_masks_change_size(
                up, dt_conf_get_float(DT_MASKS_CONF(form->type, brush, hardness)), HARDNESS_MIN,
                HARDNESS_MAX);
        ...
        else if (dt_modifier_is(state, 0))
        {
            const float masks_border = dt_masks_change_size(
                up, dt_conf_get_float(DT_MASKS_CONF(form->type, brush, border)), BORDER_MIN,
                BORDER_MAX);
            ...
            if (dt_modifier_is(state, GDK_SHIFT_MASK))
            {
                ...
                for (GList *l = form->points; l; l = g_list_next(l))
                {
                    if (gui->point_selected == -1 || gui->point_selected == pts_number)
                    {
                        ...
                        point->hardness =
                            dt_masks_change_size(up, point->hardness, HARDNESS_MIN, HARDNESS_MAX);
            ...
            else
            {
                ...
                for (GList *l = form->points; l; l = g_list_next(l))
                {
                    if (gui->point_selected == -1 || gui->point_selected == pts_number)
                    {
                        ...
                        point->border[0] =
                            dt_masks_change_size(up, point->border[0], BORDER_MIN, BORDER_MAX);

                        point->border[1] =
                            dt_masks_change_size(up, point->border[1], BORDER_MIN, BORDER_MAX);
                    }
                    pts_number++;
                }
...
static int _brush_events_button_pressed(dt_iop_module_t *module, const float pzx, const float pzy,
                                        ...
                                        const int index)
{
    ...
    if (gui->creation && which == GDK_BUTTON_PRIMARY &&
    ...
    else if (which == GDK_BUTTON_PRIMARY)
    {
        ...
        else if (gui->seg_selected >= 0)
        {
            ...
            if (dt_modifier_is(state, GDK_CONTROL_MASK) && gui->seg_selected < nb - 1)
            {
                ...
                const float t = _brush_get_position_in_segment(bzpt->corner[0], bzpt->corner[1],
...
static int _brush_events_button_released(dt_iop_module_t *module, const float pzx, const float pzy,
                                         ...
                                         dt_masks_form_gui_t *gui, const int index)
{
    ...
    else if (gui->creation && which == GDK_BUTTON_PRIMARY)
    {
        if (gui->guipoints && gui->guipoints_count > 0)
        {
            ...
            const float epsilon2 =
                factor * MAX(BORDER_MIN, masks_border) * MAX(BORDER_MIN, masks_border);
    ...
    else if (gui->feather_dragging >= 0)
    {
        ...
        _brush_feather_to_ctrl(point->corner[0] * iwidth, point->corner[1] * iheight, pts[0],
                               pts[1], &p1x, &p1y, &p2x, &p2y, TRUE);
...
static int _brush_events_mouse_moved(struct dt_iop_module_t *module, float pzx, float pzy,
                                     ...
                                     dt_masks_form_gui_t *gui, const int index)
{
    ...
    if ((gui->group_selected == index) && gui->point_edited >= 0)
    {
        ...
        {
            float ffx, ffy;
            _brush_ctrl2_to_feather(gpt->points[k * 6 + 2], gpt->points[k * 6 + 3],
                                    gpt->points[k * 6 + 4], gpt->points[k * 6 + 5], &ffx, &ffy,
                                    TRUE);
...
static void _brush_events_post_expose(cairo_t *cr, const float zoom_scale, dt_masks_form_gui_t *gui,
                                      const int index, const int nb)
{
    ...
    if (gui->creation)
    {
        ...
        if (gui->guipoints_count == 0)
        {
            ...
            {
                float x = 0.0f, y = 0.0f;
                dt_masks_calculate_source_pos_value(gui, DT_MASKS_BRUSH, gui->posx, gui->posy,
                                                    gui->posx, gui->posy, &x, &y, FALSE);
        ...
        else
        {
                ...
                (darktable.develop->form_visible->type & DT_MASKS_CLONE))
            {
                ...
                float x = 0.0f, y = 0.0f;
                dt_masks_calculate_source_pos_value(gui, DT_MASKS_BRUSH, guipoints[0], guipoints[1],
                                                    guipoints[i * 2], guipoints[i * 2 + 1], &x, &y,
                                                    TRUE);
                dt_masks_draw_clone_source_pos(cr, zoom_scale, x, y);
            }
    ...
    cairo_stroke(cr);*/
        float ffx, ffy;
        _brush_ctrl2_to_feather(gpt->points[k * 6 + 2], gpt->points[k * 6 + 3],
                                gpt->points[k * 6 + 4], gpt->points[k * 6 + 5], &ffx, &ffy, TRUE);
    ...
    if (!gui->creation && gpt->source_count > _nb_ctrl_point(nb) + 2)
    {
        ...
        _brush_bounding_box(gpt->source, NULL, nb, gpt->source_count, &width, &height, &posx,
                            &posy);
        ...
        dt_masks_closest_point(gpt->points_count, _nb_ctrl_point(nb), gpt->points, center_x,
                               center_y, &to_x, &to_y);

        // 4. source border, closest to point border
        dt_masks_closest_point(gpt->source_count, _nb_ctrl_point(nb), gpt->source, to_x, to_y,
                               &from_x, &from_y);
...
static void _brush_bounding_box_raw(const float *const points, const float *const border,
                                    const int nb_corner, const int num_points, float *x_min,
                     ...
                     const int get_source)
{
    ...
    if (!_brush_get_pts_border(module->dev, form, module->iop_order, DT_DEV_TRANSFORM_DIR_BACK_INCL,
                               piece->pipe, &points, &points_count, &border, &border_count, NULL,
                               NULL, get_source))
    {
        dt_free_align(points);
        dt_free_align(border);
...
static void _brush_falloff(float *const restrict buffer, int p0[2], int p1[2], const int posx,
                           const int posy, const int bw, const float hardness, const float density)
{
    // segment length
    ...
    for (int i = 0; i < l; i++)
    {
        ...
        if (x > 0)
            buffer[y * bw + x - 1] = MAX(buffer[y * bw + x - 1], op); // this one is to avoid
                                                                      // gap due to int
                                                                      // rounding
        if (y > 0)
            buffer[(y - 1) * bw + x] = MAX(buffer[(y - 1) * bw + x], op); // this one is to avoid
                                                                          // gap due to int
                                                                          // rounding
    }
}
                           ...
                           float **buffer, int *width, int *height, int *posx, int *posy)
{
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] brush points took %0.04f sec", form->name,
             dt_get_lap_time(&start2));

    const guint nb_corner = g_list_length(form->points);
    _brush_bounding_box(points, border, nb_corner, points_count, width, height, posx, posy);

    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] brush_fill min max took %0.04f sec",
             form->name, dt_get_lap_time(&start2));
    ...
    *buffer = dt_calloc_align_float(bufsize);
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] brush fill buffer took %0.04f sec",
...
static inline void _brush_falloff_roi(float *buffer, const int *p0, const int *p1, const int bw,
                                      const int bh, const float hardness, const float density)
{
                               ...
                               dt_masks_form_t *const form, const dt_iop_roi_t *roi, float *buffer)
{
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] brush set falloff took %0.04f sec",
             form->name, dt_get_lap_time(&start2));
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] brush fill buffer took %0.04f sec",
             form->name, dt_get_lap_time(&start));
...
static void _brush_duplicate_points(dt_develop_t *const dev, dt_masks_form_t *const base,
...
static void _brush_modify_property(dt_masks_form_t *const form, dt_masks_property_t prop,
        ...
        else
        {
            for (GList *l = form->points; l; l = g_list_next(l))
            {
                if (gui->point_selected == -1 || gui->point_selected == pts_number)
                {
                    ...
                    *max = fminf(
                        *max, fminf(BORDER_MAX / point->border[0], BORDER_MAX / point->border[1]));
                    *min = fmaxf(
                        *min, fmaxf(BORDER_MIN / point->border[0], BORDER_MIN / point->border[1]));
                    ++*count;
                }
                pts_number++;
            }
