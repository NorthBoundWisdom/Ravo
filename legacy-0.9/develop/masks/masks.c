...
static void _set_hinter_message(const dt_masks_form_gui_t *gui, const dt_masks_form_t *form)
{
    char msg[512] = "";
                              ...
                              const dt_iop_module_t *module)
{
    ...
    if (dt_masks_get_points_border(darktable.develop, form, &gpt->points, &gpt->points_count,
                                   &gpt->border, &gpt->border_count, 0, NULL))
    {
        if (form->type & DT_MASKS_CLONE)
            dt_masks_get_points_border(darktable.develop, form, &gpt->source, &gpt->source_count,
                                       NULL, NULL, 1, module);
...
static void _set_group_name_from_module(const dt_iop_module_t *module, dt_masks_form_t *grp)
{
...
static dt_masks_form_t *_group_create(dt_develop_t *dev, const dt_iop_module_t *module,
...
static dt_masks_form_t *_group_from_module(const dt_develop_t *dev, const dt_iop_module_t *module)
{
...
void dt_masks_gui_form_save_creation(dt_develop_t *dev, dt_iop_module_t *module,
                               ...
                               const int source, const dt_iop_module_t *module)
{
    ...
    {
        return form->functions->get_points_border(dev, form, points, points_count, border,
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        if (form->version != dt_masks_version())
        {
            dt_print(DT_DEBUG_ALWAYS,
                     "[_dev_read_masks_history] mask version mismatch: history is %d, current is %d",
                     form->version, dt_masks_version());
            continue;
        }
        ...
        if (num < dev->history_end)
            hist_item_last = hist_item;
    }
...
gboolean dt_masks_events_mouse_moved(dt_iop_module_t *module, const float pzx, const float pzy,
...
*/
    const gboolean skipped =
        (module && !module->enabled) && !dt_lib_gui_get_expanded(dt_lib_get_module("masks"));

    dt_print(DT_DEBUG_VERBOSE, "[dt_masks_events_mouse_moved] %s %s",
             module ? module->so->op : "mask manager", skipped ? "skipped" : "");
    ...
    int rep = 0;
    if (form->functions)
        rep = form->functions->mouse_moved(module, pzx, pzy, pressure, which, zoom_scale, form, 0,
                                           gui, 0);
...
gboolean dt_masks_events_button_released(dt_iop_module_t *module, const float pzx, const float pzy,
...
gboolean dt_masks_events_button_pressed(dt_iop_module_t *module, const float pzx, const float pzy,
                                        ...
                                        const uint32_t state)
{
    ...
    if (gui && which == GDK_BUTTON_PRIMARY)
    {
        ...
        if ((gui->form_selected || gui->source_selected || gui->point_selected ||
             gui->seg_selected || gui->feather_selected) &&
    ...
    if (form->functions)
        return form->functions->button_pressed(module, pzx, pzy, pressure, which, type, state, form,
                                               0, gui, 0) ||
...
gboolean dt_masks_events_mouse_scrolled(dt_iop_module_t *module, const float pzx, const float pzy,
                                        const gboolean up, const uint32_t state)
{
    dt_masks_form_t *form = darktable.develop->form_visible;
    dt_masks_form_gui_t *gui = darktable.develop->form_gui;
    ...
    if (form->functions)
        ret = (form->functions->mouse_scrolled(module, pzx, pzy, incr ? 1 : 0, state, form, 0, gui,
                                               0)) != 0;
...
void dt_masks_events_post_expose(const dt_iop_module_t *module, cairo_t *cr, const int32_t width,
                                 const int32_t height, const float pzx, const float pzy,
                                 const float zoom_scale)
{
    ...
    if (!(((form->type & DT_MASKS_CIRCLE) || (form->type & DT_MASKS_ELLIPSE) ||
...
void dt_masks_clear_form_gui(const dt_develop_t *dev)
{
    ...
    dev->form_gui->form_selected = dev->form_gui->border_selected = dev->form_gui->form_dragging =
        dev->form_gui->form_rotating = dev->form_gui->border_toggling =
            dev->form_gui->gradient_toggling = FALSE;
...
void dt_masks_change_form_gui(dt_masks_form_t *newform)
{
    ...
    if (newform != old && darktable.view_manager->accels_window.window &&
...
{
    if (darktable.develop->first_load)
        return;
    for (GList *modules = darktable.develop->iop; modules; modules = g_list_next(modules))
    {
        ...
        if (m && (m->flags() & IOP_FLAGS_SUPPORTS_BLENDING) && !(m->flags() & IOP_FLAGS_NO_MASKS))
        {
            dt_iop_gui_blend_data_t *bd = m->blend_data;
            if (!bd)
                break; // TODO: this doesn't look right. Why do we
...
{
    return darktable.develop->form_gui ? darktable.develop->form_gui->edit_mode : DT_MASKS_EDIT_OFF;
}
...
void dt_masks_set_edit_mode(dt_iop_module_t *module, const dt_masks_edit_mode_t value)
{
    if (!module)
        return;
...
void dt_masks_set_edit_mode_single_form(dt_iop_module_t *module, const dt_mask_id_t formid,
...
void dt_masks_iop_edit_toggle_callback(GtkToggleButton *togglebutton, dt_iop_module_t *module)
{
    ...
    dt_masks_set_edit_mode(
        module, (bd->masks_shown == DT_MASKS_EDIT_OFF ? DT_MASKS_EDIT_FULL : DT_MASKS_EDIT_OFF));
}
...
static void _menu_add_shape(dt_iop_module_t *module, const dt_masks_type_t type)
{
...
static void _menu_add_exist(dt_iop_module_t *module, const dt_mask_id_t formid)
{
    if (!module)
        return;
...
void dt_masks_iop_use_same_as(dt_iop_module_t *module, dt_iop_module_t *src)
{
    ...
    if (!src_grp || src_grp->type != DT_MASKS_GROUP)
        return;
...
void dt_masks_iop_combo_populate(GtkWidget *w, dt_iop_module_t **m)
{
    ...
    const guint nbe =
        5 + g_list_length(darktable.develop->forms) + g_list_length(darktable.develop->iop);
    ...
    for (GList *modules = darktable.develop->iop; modules; modules = g_list_next(modules))
    {
        ...
        if ((other_mod != module) && (other_mod->flags() & IOP_FLAGS_SUPPORTS_BLENDING) &&
...
void dt_masks_iop_value_changed_callback(GtkWidget *widget, dt_iop_module_t *module)
{
...
void dt_masks_form_remove(dt_iop_module_t *module, dt_masks_form_t *grp, dt_masks_form_t *form)
{
    if (!form)
        return;
...
float dt_masks_form_change_opacity(dt_masks_form_t *form, const dt_mask_id_t parentid,
                                   const float amount)
{
    if (!form)
        return 0;
...
void dt_masks_form_move(dt_masks_form_t *grp, const dt_mask_id_t formid, const gboolean up)
{
    ...
    if (grpt)
    {
        if (!up && pos == 0)
            return;
        if (up && pos == g_list_length(grp->points) - 1)
            return;
...
static int _find_in_group(const dt_masks_form_t *grp, const dt_mask_id_t formid)
{
    if (!(grp->type & DT_MASKS_GROUP))
        return 0;
    if (grp->formid == formid)
        return 1;
...
void dt_masks_group_ungroup(dt_masks_form_t *dest_grp, dt_masks_form_t *grp)
{
    if (!grp || !dest_grp)
        return;
    if (!(grp->type & DT_MASKS_GROUP) || !(dest_grp->type & DT_MASKS_GROUP))
        return;
...
static void _cleanup_unused_recurs(GList *forms, const dt_mask_id_t formid, int *used, const int nb)
{
...
static int _masks_cleanup_unused(GList **_forms, GList *history_list, const int history_end)
{
    ...
    // array
    int num = 0;
    for (GList *history = history_list; history && num < history_end && used;
...
void dt_masks_cleanup_unused_from_list(GList *history_list)
{
    ...
    for (const GList *history = g_list_last(history_list); history;
...
void dt_masks_cleanup_unused(dt_develop_t *dev)
{
    ...
    int num = 0;
    for (const GList *history = dev->history; history && num < dev->history_end;
         history = g_list_next(history))
    {
        const dt_dev_history_item_t *hist = history->data;
        ...
        if (hist->forms)
            forms = hist->forms;
        if (hist->module && strcmp(hist->op_name, "mask_manager") != 0)
            module = hist->module;
                                      ...
                                      const int points_start, const int points_count)
{
    ...
    if (points_count > 2 + points_start)
    {
        ...
        for (int i = start, next = start + 1; i < points_count;)
        {
            ...
            if (((y <= y2 && y > y1) || (y >= y2 && y < y1)) && (points[i * 2] > x))
                nb++;
...
{
    const float v = up ? value / 0.97f : value * 0.97f;
...
float dt_masks_change_rotation(const gboolean up, const float value, const gboolean is_degree)
{
    ...
    const float v = up ? value + incr : value - incr;
...
// allow to select a shape inside an iop
void dt_masks_select_form(dt_iop_module_t *module, const dt_masks_form_t *sel)
{
                                           ...
                                           dt_masks_form_t *form, const float pzx, const float pzy)
{
    ...
    if (gui->source_pos_type == DT_MASKS_SOURCE_POS_RELATIVE_TEMP)
    {
        ...
        if (gui->posx_source == -1.0f && gui->posy_source == -1.0f)
        {
            ...
            {
                form->functions->initial_source_pos(iwidth, iheight, &gui->posx_source,
                                                    &gui->posy_source);
            }
            else
                dt_print(
                          ...
                          const float x, const float y)
{
    ...
    cairo_rectangle(cr, x - (anchor_size * 0.5f), y - (anchor_size * 0.5f), anchor_size,
                    anchor_size);
                         ...
                         const float to_y, const float zoom_scale, const gboolean touch_dest)
{
    ...
    if (draw_arrow)
    {
        ...
        cairo_move_to(cr, x + arrow_scale * cosf(cangle + (0.4)),
                      y + arrow_scale * sinf(cangle + (0.4)));

        cairo_line_to(cr, x, y);

        cairo_line_to(cr, x + arrow_scale * cosf(cangle - (0.4)),
                      y + arrow_scale * sinf(cangle - (0.4)));
    }
}
                          ...
                          const gboolean selected, const float zoom_scale)
{
    ...
    const double lwidth = (dt_iop_canvas_not_sensitive(darktable.develop) ? 0.5 : 1.0) / zoom_scale;
    const double line_width = ((border ? size_border : (source ? size_source : size_mask)) *
