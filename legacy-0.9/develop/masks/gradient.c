...
static int _gradient_events_mouse_scrolled(dt_iop_module_t *module, const float pzx,
                                           const float pzy, const int up, const uint32_t state,
...
static int _gradient_events_button_pressed(dt_iop_module_t *module, const float pzx,
                                           const float pzy, const double pressure, const int which,
...
static void _gradient_init_values(const float zoom_scale, const dt_masks_form_gui_t *gui,
                                  const float xpos, const float ypos, const float pzx,
                                  const float pzy, float *anchorx, float *anchory, float *rotation,
                                  float *compression, float *curvature)
{
    ...
    if (!gui->form_dragging || (gui->posx_source - xpos > -diff && gui->posx_source - xpos < diff &&
    ...
    *anchory = pts[1] / iheight;
    ...
    float check_angle =
        atan2f(pts[7] - pts[1], pts[6] - pts[0]) - atan2f(pts[5] - pts[1], pts[4] - pts[0]);
...
static int _gradient_events_button_released(dt_iop_module_t *module, const float pzx,
                                            ...
                                            dt_masks_form_gui_t *gui, const int index)
{
    ...
    if (gui->form_dragging && form->points && gui->edit_mode == DT_MASKS_EDIT_FULL)
    {
        // we get the gradient
        dt_masks_point_gradient_t *gradient = form->points->data;
    ...
    else if (gui->form_rotating && form->points && gui->edit_mode == DT_MASKS_EDIT_FULL)
    {
        // we get the gradient
        dt_masks_point_gradient_t *gradient = form->points->data;
    ...
    else if (gui->creation)
    {
        ...
        _gradient_init_values(zoom_scale, gui, gui->posx, gui->posy, pzx * wd, pzy * ht,
                              &gradient->anchor[0], &gradient->anchor[1], &gradient->rotation,
...
static int _gradient_events_mouse_moved(dt_iop_module_t *module, const float pzx, const float pzy,
...
static inline gboolean _gradient_is_canonical(const float x, const float y, const float wd,
                                              const float ht)
{
    return (dt_isnormal(x) && dt_isnormal(y) && x >= -wd && x <= 2 * wd && y >= -ht && y <= 2 * ht);
}

static int _gradient_get_points(dt_develop_t *dev, const float x, const float y,
                                const float rotation, const float curvature, float **points,
                                int *points_count)
{
    *points = NULL;
    *points_count = 0;
...
static int _gradient_get_pts_border(dt_develop_t *dev, const float x, const float y,
                                    const float rotation, const float distance,
                                    const float curvature, float **points, int *points_count)
{
    *points = NULL;
    *points_count = 0;
...
static void _gradient_draw_lines(const gboolean borders, cairo_t *cr, const gboolean selected,
                                 const float zoom_scale, const float *pts_line,
                                 const int pts_line_count, const float xref, const float yref)
{
    // safeguard in case of malformed arrays of points
    if (borders && pts_line_count <= 3)
        return;
    if (!borders && pts_line_count <= 4)
        return;
...
static void _gradient_draw_arrow(cairo_t *cr, const gboolean selected,
                                 const gboolean border_selected, const float zoom_scale,
                                 const float *pts, const int pts_count)
{
    if (pts_count < 3)
        return;
    ...
    dt_masks_draw_arrow(cr, pivot_start_x, pivot_start_y, pivot_end_x, pivot_end_y, zoom_scale,
                        TRUE);
...
static void _gradient_events_post_expose(cairo_t *cr, const float zoom_scale,
                                         dt_masks_form_gui_t *gui, const int index, const int nb)
{
    ...
    if (gui->creation)
    {
        ...
        float *border = NULL;
        int border_count = 0;
        int draw = _gradient_get_points(darktable.develop, xx, yy, rotation, curvature, &points,
                                        &points_count);
        if (draw && compression > 0.0)
        {
            draw = _gradient_get_pts_border(darktable.develop, xx, yy, rotation, compression,
                                            curvature, &border, &border_count);
        }

        cairo_save(cr);
        // draw main line
        _gradient_draw_lines(FALSE, cr, FALSE, zoom_scale, points, points_count, points[0],
                             points[1]);
        // draw borders
        _gradient_draw_lines(TRUE, cr, FALSE, zoom_scale, border, border_count, points[0],
                             points[1]);
        // draw arrow
        _gradient_draw_arrow(cr, FALSE, FALSE, zoom_scale, points, points_count);
        cairo_restore(cr);
    ...
    // draw main line
    _gradient_draw_lines(FALSE, cr, selected, zoom_scale, gpt->points, gpt->points_count, xref,
                         yref);
    // draw borders
    if (gui->show_all_feathers || gui->group_selected == index)
        _gradient_draw_lines(TRUE, cr, gui->border_selected, zoom_scale, gpt->border,
                             gpt->border_count, xref, yref);

    _gradient_draw_arrow(cr, selected, ((gui->group_selected == index) && (gui->border_selected)),
                         zoom_scale, gpt->points, gpt->points_count);
}

static int _gradient_get_points_border(dt_develop_t *dev, dt_masks_form_t *form, float **points,
                                       ...
                                       const int source, const dt_iop_module_t *module)
{
    ...
    if (_gradient_get_points(dev, gradient->anchor[0], gradient->anchor[1], gradient->rotation,
                             gradient->curvature, points, points_count))
    {
        if (border)
            return _gradient_get_pts_border(dev, gradient->anchor[0], gradient->anchor[1],
                                            gradient->rotation, gradient->compression,
                                            gradient->curvature, border, border_count);
        else
            return 1;
    }
    return 0;
...
static int _gradient_get_area(const dt_iop_module_t *const module,
                              ...
                              dt_masks_form_t *const form, int *width, int *height, int *posx,
                              int *posy)
{
...
static int _gradient_get_mask(const dt_iop_module_t *const module,
                              const dt_dev_pixelpipe_iop_t *const piece,
                              dt_masks_form_t *const form, float **buffer, int *width, int *height,
                              int *posx, int *posy)
{
    double start2 = dt_get_debug_wtime();
    // we get the area
    if (!_gradient_get_area(module, piece, form, width, height, posx, posy))
        return 0;

    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] gradient area took %0.04f sec", form->name,
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] gradient draw took %0.04f sec", form->name,
             dt_get_lap_time(&start2));
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] gradient transform took %0.04f sec",
    ...
    for (int n = 0; n < lutsize; n++)
    {
        ...
        const float value = 0.5f + 0.5f * ((state == DT_MASKS_GRADIENT_STATE_LINEAR) ?
                                               normf * distance :
    ...
    for (int j = 0; j < gh; j++)
    {
        for (int i = 0; i < gw; i++)
        {
            ...
            points[(j * gw + i) * 2] = (distance <= -4.0f * compression) ?
                                           0.0f :
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] gradient fill took %0.04f sec", form->name,
             dt_get_lap_time(&start2));
                                  ...
                                  float *buffer)
{
    ...
    for (int j = 0; j < h; j++)
    {
        ...
        for (int i = 0; i < w; i++)
        {
            ...
            buffer[(size_t)j * w + i] =
                (points[mindex * 2] * grid_ii * grid_jj + points[(mindex + 1) * 2] * ii * grid_jj +
...
static void _gradient_set_form_name(dt_masks_form_t *const form, const size_t nb)
{
...
static void _gradient_duplicate_points(dt_develop_t *dev, dt_masks_form_t *const base,
...
static void _gradient_modify_property(dt_masks_form_t *const form, const dt_masks_property_t prop,
