...
static void _path_bounding_box(const float *const points, const float *border, const int nb_corner,
                               const int num_points, const int num_borders, int *width, int *height,
                               int *posx, int *posy);

/** get the point of the path at pos t [0,1]  */
static void _path_get_XY(const float p0x, const float p0y, const float p1x, const float p1y,
                         const float p2x, const float p2y, const float p3x, const float p3y,
                         const float t, float *x, float *y)
{
 ...
 */
static void _path_border_get_XY(const float p0x, const float p0y, const float p1x, const float p1y,
                                const float p2x, const float p2y, const float p3x, const float p3y,
                                const float t, const float rad, float *xc, float *yc, float *xb,
                                float *yb)
{
 ...
 */
static inline float angle_2d(const float x1, const float y1, const float x_ref, const float y_ref)
{
 ...
 */
static inline float _get_ctrl_angle(const float x_ref, const float y_ref, const float x1,
 ...
 * @param[in] aspect_ratio aspect ratio of the image
 */
static void _set_ctrl_angle(const float x_ref, const float y_ref, const float angle,
                            const gboolean move_p2, float *x1, float *y1, float *x2, float *y2,
                            const float aspect_ratio)
{
    const float x1a = *x1 * aspect_ratio;
    const float x2a = *x2 * aspect_ratio;
 ...
 */
static inline float _get_ctrl_scale(const float x_ref, const float y_ref, const float x1,
                                    const float y1, const float x2, const float y2,
                                    const float aspect_ratio)
{
    const float x1a = x1 * aspect_ratio;
 ...
 * @param[in] aspect_ratio aspect ratio of the image
 */
static void _set_ctrl_scale(const float x_ref, const float y_ref, const float scale,
                            const gboolean move_p2, float *x1, float *y1, float *x2, float *y2,
                            const float aspect_ratio)
{
 ...
 * @param[in,out] y2 y coordinate of the second point
 */
static void _set_ctrl_symmetric(const float x_ref, const float y_ref, const gboolean move_p2,
...
void _update_bezier_ctrl_points(dt_masks_point_path_t *point, const float new_x, const float new_y,
                                ...
                                const dt_masks_path_edit_mode_t ctrl_mode, const float ctrl_angle,
                                const float ctrl_scale, const float aspect_ratio)
{
    ...
    case DT_MASKS_BEZIER_NONE:
        _set_ctrl_scale(point->corner[0], point->corner[1], ctrl_scale, move_p2, &point->ctrl1[0],
                        &point->ctrl1[1], &point->ctrl2[0], &point->ctrl2[1], aspect_ratio);
        _set_ctrl_angle(point->corner[0], point->corner[1], ctrl_angle, move_p2, &point->ctrl1[0],
                        &point->ctrl1[1], &point->ctrl2[0], &point->ctrl2[1], aspect_ratio);
        break;
    case DT_MASKS_BEZIER_SINGLE:
        break;
    case DT_MASKS_BEZIER_SYMMETRIC:
        _set_ctrl_symmetric(point->corner[0], point->corner[1], move_p2, &point->ctrl1[0],
                            &point->ctrl1[1], &point->ctrl2[0], &point->ctrl2[1]);
        break;
 ...
 * spline */
static void _path_catmull_to_bezier(const float x1, const float y1, const float x2, const float y2,
...
static void _path_init_ctrl_points(dt_masks_form_t *form)
{
    ...
    for (int k = 0; k < nb; k++)
    {
        ...
        if (point3->state & DT_MASKS_POINT_STATE_NORMAL)
        {
            ...
            _path_catmull_to_bezier(point1->corner[0], point1->corner[1], point2->corner[0],
                                    point2->corner[1], point3->corner[0], point3->corner[1],
                                    point4->corner[0], point4->corner[1], &bx1, &by1, &bx2, &by2);
            ...
            point3->ctrl1[0] = bx2;
            point3->ctrl1[1] = by2;
            _path_catmull_to_bezier(point2->corner[0], point2->corner[1], point3->corner[0],
                                    point3->corner[1], point4->corner[0], point4->corner[1],
                                    point5->corner[0], point5->corner[1], &bx1, &by1, &bx2, &by2);
...
static gboolean _path_is_clockwise(dt_masks_form_t *form)
{
    ...
    {
        float sum = 0.0f;
        for (const GList *form_points = form->points; form_points;
             form_points = g_list_next(form_points))
        {
            ...
            sum +=
                (point2->corner[0] - point1->corner[0]) * (point2->corner[1] + point1->corner[1]);
        }
        return (sum < 0);
    }
    // return dummy answer
...
static int _path_fill_gaps(const int lastx, const int lasty, const int x, const int y,
 ...
 */
static void _path_points_fill_border_gaps(float *cmax, float *bmin, float *bmax,
...
static inline float _smoothstep(const float p1, const float p2, const float t)
{
...
static void _path_points_recurs(float *p1, float *p2, const double tmin, const double tmax,
                                float *path_min, float *path_max, float *border_min,
                                float *border_max, float *rpath, float *rborder,
                                dt_masks_dynbuf_t *dpoints, dt_masks_dynbuf_t *dborder,
                                const int withborder)
{
    ...
    if (path_min[0] == DT_INVALID_COORDINATE)
    {
        _path_border_get_XY(p1[0], p1[1], p1[2], p1[3], p2[2], p2[3], p2[0], p2[1], tmin,
                            _smoothstep(p1[4], p2[4], tmin), path_min, path_min + 1, border_min,
                            border_min + 1);
    }
    if (path_max[0] == DT_INVALID_COORDINATE)
    {
        _path_border_get_XY(p1[0], p1[1], p1[2], p1[3], p2[2], p2[3], p2[0], p2[1], tmax,
                            _smoothstep(p1[4], p2[4], tmax), path_max, path_max + 1, border_max,
                            border_max + 1);
    }
    // are the points near ?
    if ((tmax - tmin < 0.0001) ||
        ((int)path_min[0] - (int)path_max[0] < 1 && (int)path_min[0] - (int)path_max[0] > -1 &&
         ...
         (!withborder || ((int)border_min[0] - (int)border_max[0] < 1 &&
    ...
    float rc[2] = {0}, rb[2] = {0};
    _path_points_recurs(p1, p2, tmin, tx, path_min, c, border_min, b, rc, rb, dpoints, dborder,
                        withborder);
    _path_points_recurs(p1, p2, tx, tmax, rc, path_max, rb, border_max, rpath, rborder, dpoints,
                        dborder, withborder);
}

static inline float _dist_squared_2d(const float x1, const float y1, const float x2, const float y2)
{
...
/* Border point indexes are in range [min, max), indexes >= max are
   wrapped around */
static inline int _border_index_wrap_around(const int min, const int max, const int idx)
{
...
static inline int _border_index_order(const int index, const int wrap_index, const int offset)
{
 ...
 */
static inline int _find_closer_point(const float *const border, const int border_first,
                                     const int border_last, const int idx_fixed,
 ...
 */
static void _optimize_intersection_points(const float *const border, const int border_first,
                                          const int border_wrap, const int border_last, int *idx1,
                                          int *idx2, const int idx1_min_ord, const int idx1_max_ord,
                                          const int idx2_min_ord, const int idx2_max_ord)
{
    ...
    if (idx1_ord + 2 * (MAX_ITER + 1) > idx2_ord)
        return;
    ...
    while (iter < MAX_ITER)
    {
        ...
        {
            if (!idx1_updated)
                break; // Optimization tried both sides and did not find an improvement.
        };
 ...
 */
static gboolean _check_cutable(const int seg_start, const int seg_end, const int *const extrema_ord,
...
static int _path_find_self_intersection(dt_masks_dynbuf_t *inter,
                                        const dt_masks_intbuf_t *const gap_fill_segments,
                                        const int nb_corners, float *const border,
                                        const int border_len)
{
    if (nb_corners == 0 || border_len == 0)
        return 0;
    ...
    for (int i = border_first; i < border_len; i++)
    {
        if ((border[i * 2] == DT_INVALID_COORDINATE) ||
            (border[i * 2 + 1] == DT_INVALID_COORDINATE))
        {
            ...
            for (int k = 0; k < 3; k++)
            {
                ...
                if (_check_cutable(curr_start_ord, curr_end_ord, posextr_ord, gap_fill_segments))
                {
                    ...
                    if (inter_count == 0)
                    {
                        ...
                        _optimize_intersection_points(border, border_first, posextr[1], border_len,
                                                      &opt_start, &opt_end, posextr[1],
                                                      border_last_ord, posextr[1], curr_end_ord);
                    ...
                    for (n = 0; n < inter_count; n++)
                    {
                        ...
                        if (curr_start_ord < prev_start_ord && prev_end_ord < curr_end_ord)
                        {
                            ...
                            // idx2_max_ord: curr_end_ord - don't go beyond the current loop index
                            _optimize_intersection_points(
                                border, border_first, posextr[1], border_len, &opt_start, &opt_end,
                        ...
                        && prev_end_ord < curr_start_ord)
                    {
                        ...
                        // idx2_max_ord: curr_end_ord - don't go beyond the current loop index
                        _optimize_intersection_points(border, border_first, posextr[1], border_len,
                                                      &opt_start, &opt_end, prev_end_ord,
                                                      border_last_ord, posextr[1], curr_end_ord);
...
static int _path_get_pts_border(dt_develop_t *dev, dt_masks_form_t *form, const double iop_order,
                                const int transf_direction, dt_dev_pixelpipe_t *pipe,
                                float **points, int *points_count, float **border,
                                int *border_count, const gboolean source)
{
    ...
    *points_count = 0;
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] path_points init took %0.04f sec",
    ...
    for (int k = 0; k < nb; k++)
    {
        ...
        float p1[5] = {point1->corner[0] * wd - dx, point1->corner[1] * ht - dy,
                       point1->ctrl2[0] * wd - dx, point1->ctrl2[1] * ht - dy,
                       cw * point1->border[1] * MIN(wd, ht)};
        float p2[5] = {point2->corner[0] * wd - dx, point2->corner[1] * ht - dy,
                       point2->ctrl1[0] * wd - dx, point2->ctrl1[1] * ht - dy,
                       cw * point2->border[0] * MIN(wd, ht)};
        float p3[5] = {point2->corner[0] * wd - dx, point2->corner[1] * ht - dy,
                       point2->ctrl2[0] * wd - dx, point2->ctrl2[1] * ht - dy,
                       cw * point2->border[1] * MIN(wd, ht)};
        float p4[5] = {point3->corner[0] * wd - dx, point3->corner[1] * ht - dy,
                       point3->ctrl1[0] * wd - dx, point3->ctrl1[1] * ht - dy,
        ...
        _path_points_recurs(p1, p2, 0.0, 1.0, cmin, cmax, bmin, bmax, rc, rb, dpoints, dborder,
                            border && (nb >= 3));
        ...
        if (dborder)
        {
            ...
            (dt_masks_dynbuf_buffer(dborder))[k * 6] = border_init[k * 6] =
                (dt_masks_dynbuf_buffer(dborder))[pb];
            (dt_masks_dynbuf_buffer(dborder))[k * 6 + 1] = border_init[k * 6 + 1] =
        ...
        if (dborder && nb >= 3)
        {
            ...
            _path_border_get_XY(p3[0], p3[1], p3[2], p3[3], p4[2], p4[3], p4[0], p4[1], 0.00001f,
                                p3[4], cmin, cmin + 1, bmax, bmax + 1);
            ...
            if (bmax[0] - rb[0] > 1 || bmax[0] - rb[0] < -1 || bmax[1] - rb[1] > 1 ||
                bmax[1] - rb[1] < -1)
            {
                _path_points_fill_border_gaps(rc, rb, bmax, dpoints, dborder, gap_fill_segs,
    ...
    *points = dt_masks_dynbuf_harvest(dpoints);
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] path_points point recurs %0.04f sec",
             form->name, dt_get_lap_time(&start2));
    ...
    {
        inter_count =
            _path_find_self_intersection(intersections, gap_fill_segs, nb, *border, *border_count);
    ...
    if (source && transf_direction == DT_DEV_TRANSFORM_DIR_ALL)
    {
        ...
        if (dt_dev_distort_transform_plus(dev, pipe, iop_order, DT_DEV_TRANSFORM_DIR_BACK_EXCL,
                                          *points, *points_count))
        {
            ...
            if (!dt_dev_distort_transform_plus(dev, pipe, iop_order, DT_DEV_TRANSFORM_DIR_FORW_INCL,
                                               *points, *points_count))
                goto fail;
        }

        dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] path_points end took %0.04f sec",
                 form->name, dt_get_lap_time(&start2));
    ...
    if (dt_dev_distort_transform_plus(dev, pipe, iop_order, transf_direction, *points,
                                      *points_count))
    {
        if (!border || dt_dev_distort_transform_plus(dev, pipe, iop_order, transf_direction,
                                                     *border, *border_count))
        {
            dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF,
            ...
            if (border)
            {
                ...
                for (int k = 0; k < nb; k++)
                    for (int i = 2; i < 6; i++)
                        (*border)[k * 6 + i] = border_init[k * 6 + i];
                ...
                for (int i = 0; i < inter_count; i++)
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (((*border)[nb * 6] == DT_INVALID_COORDINATE) &&
                                ((*border)[nb * 6 + 1] == DT_INVALID_COORDINATE))
                                (*border)[nb * 6 + 1] = w;
...
static int _path_get_points_border(dt_develop_t *dev, dt_masks_form_t *form, float **points,
                                   ...
                                   const int source, const dt_iop_module_t *module)
{
    if (source && !module)
        return 0;
    const double ioporder = (module) ? module->iop_order : 0.0f;
    return _path_get_pts_border(dev, form, ioporder, DT_DEV_TRANSFORM_DIR_ALL, dev->preview_pipe,
                                points, points_count, border, border_count, source);
}

static int _path_events_mouse_scrolled(dt_iop_module_t *module, const float pzx, const float pzy,
                                       const int up, const uint32_t state, dt_masks_form_t *form,
                                       const dt_mask_id_t parentid, dt_masks_form_gui_t *gui,
                                       const int index)
{
    // resize a shape even if on a node or segment
    if (gui->form_selected || gui->point_selected >= 0 ||
        gui->feather_selected >= 0 // bezier control points
        || gui->seg_selected >= 0 || gui->point_border_selected >= 0)
    {
        ...
        else
        {
            ...
            if (dt_modifier_is(state, GDK_SHIFT_MASK))
            {
                ...
                for (const GList *l = form->points; l; l = g_list_next(l))
                {
                    ...
                    point->border[0] = dt_masks_change_size(up, point->border[0], 0.0005f, 0.5f);
                    point->border[1] = dt_masks_change_size(up, point->border[1], 0.0005f, 0.5f);

                    feather_size += point->border[0] + point->border[1];
                }

                const float masks_border = dt_masks_change_size(
                    up, dt_conf_get_float(DT_MASKS_CONF(form->type, path, border)), 0.0005f, 0.5f);
            ...
            else if (gui->edit_mode == DT_MASKS_EDIT_FULL)
            {
                     ...
                     form_points = g_list_next(form_points))
                {
                    ...
                    dt_masks_point_path_t *point1 =
                        form_points->data; // kth element of form->points
                    dt_masks_point_path_t *point2 = next->data;
                ...
                for (GList *l = form->points; l; l = g_list_next(l))
                {
                    dt_masks_point_path_t *point = l->data;
                    const float x =
                        dt_masks_change_size(up, point->corner[0] - bx, -FLT_MAX, FLT_MAX);
                    const float y =
                        dt_masks_change_size(up, point->corner[1] - by, -FLT_MAX, FLT_MAX);

                    // we stretch ctrl points
                    const float ct1x = dt_masks_change_size(up, point->ctrl1[0] - point->corner[0],
                                                            -FLT_MAX, FLT_MAX);
                    const float ct1y = dt_masks_change_size(up, point->ctrl1[1] - point->corner[1],
                                                            -FLT_MAX, FLT_MAX);
                    const float ct2x = dt_masks_change_size(up, point->ctrl2[0] - point->corner[0],
                                                            -FLT_MAX, FLT_MAX);
                    const float ct2y = dt_masks_change_size(up, point->ctrl2[1] - point->corner[1],
                                                            -FLT_MAX, FLT_MAX);
...
static int _path_events_button_pressed(dt_iop_module_t *module, const float pzx, const float pzy,
                                       const double pressure, const int which, const int type,
                                       const uint32_t state, dt_masks_form_t *form,
                                       const dt_mask_id_t parentid, dt_masks_form_gui_t *gui,
                                       const int index)
{
    ...
    if (gui->creation && which == GDK_BUTTON_PRIMARY && form->points == NULL &&
    ...
    else if (gui->creation && (which == GDK_BUTTON_SECONDARY || gui->creation_closing_form))
    {
        ...
        else
        {
            ...
            if (crea_module)
            {
                ...
                if (gui->creation_continuous && dt_iop_module_is(crea_module, "retouch"))
                    dt_masks_set_edit_mode_single_form(crea_module, form->formid,
                                                       DT_MASKS_EDIT_FULL);
            ...
            {
                if (crea_module && !dt_iop_module_is(crea_module, "retouch"))
                {
    ...
    else if (which == GDK_BUTTON_PRIMARY)
    {
        ...
        else if (gui->feather_selected >= 0) // Bézier control point
        {
            ...
            gui->bezier_ctrl_angle = _get_ctrl_angle(
                point->corner[0], point->corner[1], point->ctrl1[0], point->ctrl1[1],
                point->ctrl2[0], point->ctrl2[1], iwidth / iheight);
            gui->bezier_ctrl_scale = _get_ctrl_scale(
                point->corner[0], point->corner[1], point->ctrl1[0], point->ctrl1[1],
                point->ctrl2[0], point->ctrl2[1], iwidth / iheight);
    ...
    else if (which == GDK_BUTTON_SECONDARY && gui->point_selected >= 0)
    {
        ...
        if (g_list_shorter_than(form->points, 4))
        {
            ...
            if (!dt_is_valid_maskid(parentid))
                return 1;
            ...
            else
            {
                ...
                for (GList *forms = darktable.develop->form_visible->points; forms;
        ...
        return 1;
             ...
             gui->feather_selected >= 0) // right-click to reset Bézier controls
    {
        dt_masks_point_path_t *point =
            (dt_masks_point_path_t *)g_list_nth_data(form->points, gui->feather_selected);
    ...
    else if (which == GDK_BUTTON_SECONDARY && dt_is_valid_maskid(parentid) &&
...
static int _path_events_button_released(dt_iop_module_t *module, const float pzx, const float pzy,
                                        const int which, const uint32_t state,
                                        dt_masks_form_t *form, const dt_mask_id_t parentid,
                                        dt_masks_form_gui_t *gui, const int index)
{
    if (!gui)
        return 0;
    if (gui->creation)
    ...
    else if (gui->feather_dragging >= 0) // Bézier control point
    {
        ...
        _update_bezier_ctrl_points(point, pts[0] / iwidth, pts[1] / iheight, gui->bezier_ctrl,
                                   gui->bezier_mode, gui->bezier_ctrl_angle, gui->bezier_ctrl_scale,
                                   iwidth / iheight);
        gui->bezier_mode = DT_MASKS_BEZIER_NONE;
        point->state = DT_MASKS_POINT_STATE_USER;
...
static int _path_events_mouse_moved(dt_iop_module_t *module, float pzx, float pzy,
                                    const double pressure, const int which, const float zoom_scale,
                                    dt_masks_form_t *form, const dt_mask_id_t parentid,
                                    dt_masks_form_gui_t *gui, const int index)
{
    ...
    if (gui->point_dragging >= 0)
    {
        ...
        if ((form->type & DT_MASKS_CLONE) && gui->point_dragging == 0)
        {
    ...
    else if (gui->seg_dragging >= 0)
    {
        ...
        if ((form->type & DT_MASKS_CLONE) &&
            (gui->seg_dragging == 0 || gui->seg_dragging == (g_list_length(form->points) - 1)))
        {
            form->source[0] += dx;
            form->source[1] += dy;
        }
    ...
    if ((gui->group_selected == index) && gui->point_edited >= 0)
    {
        ...
        {
            if (pzx - gpt->points[k * 6] > -as && pzx - gpt->points[k * 6] < as &&
                pzy - gpt->points[k * 6 + 1] > -as && pzy - gpt->points[k * 6 + 1] < as)
            {
            ...
            if (pzx - gpt->points[k * 6 + 4] > -as && pzx - gpt->points[k * 6 + 4] < as &&
                pzy - gpt->points[k * 6 + 5] > -as && pzy - gpt->points[k * 6 + 5] < as)
            {
                gui->feather_selected = k;
    ...
    {
        // corner ??
        if (!gui->select_only_border && pzx - gpt->points[k * 6 + 2] > -as &&
            pzx - gpt->points[k * 6 + 2] < as && pzy - gpt->points[k * 6 + 3] > -as &&
...
static void _path_events_post_expose(cairo_t *cr, const float zoom_scale, dt_masks_form_gui_t *gui,
                                     const int index, const int nb)
{
    if (!gui)
        return;
    dt_masks_form_gui_points_t *gpt = g_list_nth_data(gui->points, index);
    ...
    {
        for (int k = 0; k < nb; k++)
            dt_masks_draw_anchor(cr, k == gui->point_dragging || k == gui->point_selected,
                                 zoom_scale, gpt->points[k * 6 + 2], gpt->points[k * 6 + 3]);
    }
        ...
        gpt->border_count > _nb_wctrl_points(nb) + 6)
    {
        ...
        for (int i = _nb_wctrl_points(nb); i < gpt->border_count; i++)
        {
            ...
            {
                if (gpt->border[i * 2 + 1] == DT_INVALID_COORDINATE)
                    break;
        ...
        for (int k = 0; k < nb; k++)
        {
            ...
            dt_masks_draw_anchor(cr, gui->point_border_selected == k, zoom_scale,
                                 gpt->border[k * 6], gpt->border[k * 6 + 1]);
        }
    }

    // draw a cross where the source will be created
    if (gui->creation && darktable.develop->form_visible &&
        (darktable.develop->form_visible->type & DT_MASKS_CLONE))
    {
        ...
        if ((k * 6 + 2) >= 0)
        {
            ...
            dt_masks_calculate_source_pos_value(gui, DT_MASKS_PATH, gpt->points[2], gpt->points[3],
                                                gpt->points[k * 6 + 2], gpt->points[k * 6 + 3], &x,
                                                &y, TRUE);
        ...
        {
            float x = 0.0f, y = 0.0f;
            dt_masks_calculate_source_pos_value(gui, DT_MASKS_PATH, gui->posx, gui->posy, gui->posx,
                                                gui->posy, &x, &y, FALSE);
            dt_masks_draw_clone_source_pos(cr, zoom_scale, x, y);
        }
    }
    ...
    if (!gui->creation && gpt->source_count > _nb_wctrl_points(nb) + 6)
    {
        ...
        _path_bounding_box(gpt->source, NULL, nb, gpt->source_count, 0, &width, &height, &posx,
                           &posy);
        ...
        dt_masks_closest_point(gpt->points_count, _nb_wctrl_points(nb), gpt->points, center_x,
                               center_y, &to_x, &to_y);

        // 4. source border, closest to point border
        dt_masks_closest_point(gpt->source_count, _nb_wctrl_points(nb), gpt->source, to_x, to_y,
                               &from_x, &from_y);

        // 5. we draw the line between source and dest
        dt_masks_draw_arrow(cr, from_x, from_y, to_x, to_y, zoom_scale, FALSE);
        ...
        dt_masks_line_stroke(cr, FALSE, TRUE,
                             (gui->group_selected == index) &&
                                   ...
                                   float *x_min, float *x_max, float *y_min, float *y_max)
{
    ...
    for (int i = _nb_wctrl_points(nb_corner); i < num_borders; i++)
    {
        ...
        {
            if (yy == DT_INVALID_COORDINATE)
                break; // that means we have to skip the end of the border path
            i = yy - 1;
            continue;
        }
        xmin = MIN(xx, xmin);
                               ...
                               int *posx, int *posy)
{
    ...
    float xmin, xmax, ymin, ymax;
    _path_bounding_box_raw(points, border, nb_corner, num_points, num_borders, &xmin, &xmax, &ymin,
                           &ymax);
    *height = ymax - ymin + 4;
    *width = xmax - xmin + 4;
    *posx = xmin - 2;
                     ...
                     const gboolean get_source)
{
    ...
    if (!_path_get_pts_border(module->dev, form, module->iop_order, DT_DEV_TRANSFORM_DIR_BACK_INCL,
                              piece->pipe, &points, &points_count, &border, &border_count,
                              get_source))
    {
        dt_free_align(points);
        dt_free_align(border);
    ...
    _path_bounding_box(points, border, nb_corner, points_count, border_count, width, height, posx,
                       posy);
...
static int _path_get_source_area(dt_iop_module_t *module, dt_dev_pixelpipe_iop_t *piece,
                                 dt_masks_form_t *form, int *width, int *height, int *posx,
                                 int *posy)
{
...
static int _path_get_area(const dt_iop_module_t *const module,
                          const dt_dev_pixelpipe_iop_t *const piece, dt_masks_form_t *const form,
...
static void _path_falloff(float *const restrict buffer, int *p0, int *p1, const int posx,
                          const int posy, const int bw)
{
    // segment length
                          ...
                          float **buffer, int *width, int *height, int *posx, int *posy)
{
    ...
    if (!_path_get_pts_border(module->dev, form, module->iop_order, DT_DEV_TRANSFORM_DIR_BACK_INCL,
                              piece->pipe, &points, &points_count, &border, &border_count, FALSE))
    {
        dt_free_align(points);
        dt_free_align(border);
        return 0;
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] path points took %0.04f sec", form->name,
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] path_fill min max took %0.04f sec",
             form->name, dt_get_lap_time(&start2));
    ...
    if (nbp > 2)
    {
        ...
        for (int ii = _nb_wctrl_points(nb_corner); ii < 2 * nbp - _nb_wctrl_points(nb_corner); ii++)
        {
            // we are writing more than 1 loop in the case the dir in y change
            // exactly at start/end point
            int i = ii;
            if (ii >= nbp)
                i = (ii - _nb_wctrl_points(nb_corner)) % (nbp - _nb_wctrl_points(nb_corner)) +
                    _nb_wctrl_points(nb_corner);
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] path_fill draw path took %0.04f sec",
             form->name, dt_get_lap_time(&start2));
    ...
    for (int yy = 0; yy < hb; yy++)
    {
        ...
        for (int xx = 0; xx < wb; xx++)
        {
            ...
            if (v == 1.0f)
                state = !state;
            if (state)
                bufptr[yy * wb + xx] = 1.0f;
        }
    }

    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] path_fill fill plain took %0.04f sec",
             form->name, dt_get_lap_time(&start2));
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] path_fill fill falloff took %0.04f sec",
             form->name, dt_get_lap_time(&start2));

    dt_free_align(points);
    dt_free_align(border);

    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] path fill buffer took %0.04f sec",
...
static int _path_crop_to_roi(float *path, const int point_count, const float xmin, const float xmax,
                             const float ymin, const float ymax)
{
    ...
    for (int k = 0; k < point_count; k++)
    {
        ...
        if (l < 0 && path[2 * kk] < xmin)
            l = k; // where we leave roi
        if (l >= 0 && path[2 * kk] >= xmin)
            r = k - 1; // where we re-enter roi
        ...
        if (l >= 0 && r >= 0)
        {
            ...
            const float delta_y =
                (count == 1) ? 0 : (path[2 * rr + 1] - path[2 * ll + 1]) / (count - 1);
...
static void _path_falloff_roi(float *buffer, int *p0, int *p1, const int bw, const int bh)
{
    // segment length
    const int l = sqrtf((p1[0] - p0[0]) * (p1[0] - p0[0]) + (p1[1] - p0[1]) * (p1[1] - p0[1])) + 1;
                              ...
                              dt_masks_form_t *const form, const dt_iop_roi_t *roi, float *buffer)
{
    ...
    float xmin, xmax, ymin, ymax;
    _path_bounding_box_raw(points, border, nb_corner, points_count, border_count, &xmin, &xmax,
                           &ymin, &ymax);

    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] path_fill min max took %0.04f sec",
             form->name, dt_get_lap_time(&start2));

    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] path_fill clear mask took %0.04f sec",
             form->name, dt_get_lap_time(&start2));
    ...
    if (path_in_roi)
    {
        ...
        // algorithm.
        const int crop_success =
            _path_crop_to_roi(cpoints + 2 * _nb_wctrl_points(nb_corner),
                              points_count - _nb_wctrl_points(nb_corner), 0, width - 1, 0, height);
        path_encircles_roi = path_encircles_roi || !crop_success;

        dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] path_fill crop to roi took %0.04f sec",
                 form->name, dt_get_lap_time(&start2));
...
static void _path_set_form_name(dt_masks_form_t *const form, const size_t nb)
{
...
static void _path_set_hint_message(const dt_masks_form_gui_t *const gui,
                                   const dt_masks_form_t *const form, const int opacity,
...
static void _path_duplicate_points(dt_develop_t *const dev, dt_masks_form_t *const base,
...
static void _path_modify_property(dt_masks_form_t *const form, dt_masks_property_t prop,
                                  const float old_val, const float new_val, float *sum, int *count,
                                  float *min, float *max)
{
