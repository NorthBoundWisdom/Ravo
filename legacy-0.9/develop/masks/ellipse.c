...
static inline void _ellipse_point_transform(const float xref, const float yref, const float x,
                                            const float y, const float sinr, const float cosr,
                                            float *xnew, float *ynew)
{
    const float xtmp =
        (sinr * sinr + cosr * cosr) * (x - xref) + (cosr * sinr - cosr * sinr) * (y - yref);
    const float ytmp =
        (cosr * sinr - cosr * sinr) * (x - xref) + (sinr * sinr + cosr * cosr) * (y - yref);
...
static int _ellipse_cross_test(const float x, const float y, const float *point_1,
                               const float *point_2)
{
    const float x_a = x;
    const float y_a = y;
...
static int _ellipse_point_in_polygon(const float x, const float y, const float *points,
...
static void _ellipse_draw_shape(const gboolean borders, const gboolean source, cairo_t *cr,
                                const int selected, const float zoom_scale, const float xref,
                                const float yref, const float *points, const int points_count)
{
    if (points_count <= 10)
        return;
...
static float *_points_to_transform(const float xx, const float yy, const float radius_a,
                                   const float radius_b, const float rotation, const float wd,
                                   const float ht, int *points_count)
{
    ...
    const int l = MAX(100, (int)((M_PI_F * (a + b) *
...
static int _ellipse_get_points_source(dt_develop_t *dev, const float xx, const float yy,
                                      const float xs, const float ys, const float radius_a,
                                      const float radius_b, const float rotation, float **points,
                                      int *points_count, const dt_iop_module_t *module)
{
    float wd, ht;
    ...
    *points = _points_to_transform(xx, yy, radius_a, radius_b, rotation, wd, ht, points_count);
    ...
    if (dt_dev_distort_transform_plus(dev, dev->preview_pipe, module->iop_order,
                                      DT_DEV_TRANSFORM_DIR_BACK_EXCL, *points, *points_count))
    {
...
static int _ellipse_get_points(dt_develop_t *dev, const float xx, const float yy,
                               const float radius_a, const float radius_b, const float rotation,
...
static int _ellipse_get_points_border(dt_develop_t *dev, dt_masks_form_t *form, float **points,
                                      ...
                                      const int source, const dt_iop_module_t *module)
{
    ...
    if (source)
    {
        ...
        return _ellipse_get_points_source(dev, x, y, xs, ys, a, b, ellipse->rotation, points,
                                          points_count, module);
    }
    else
    {
        if (_ellipse_get_points(dev, x, y, a, b, ellipse->rotation, points, points_count))
        {
            if (border)
            {
                ...
                return _ellipse_get_points(
                    dev, x, y, (prop ? a * (1.0f + ellipse->border) : a + ellipse->border),
...
static int _ellipse_events_mouse_scrolled(dt_iop_module_t *module, const float pzx, const float pzy,
                                          ...
                                          const int index)
{
    ...
    if (gui->creation)
    {
        ...
        {
            const float rotation = dt_masks_change_rotation(
                up, dt_conf_get_float(DT_MASKS_CONF(form->type, ellipse, rotation)), TRUE);
    ...
    if (gui->form_selected)
    {
        ...
        else
        {
            ...
            {
                const float reference = (ellipse->flags & DT_MASKS_ELLIPSE_PROPORTIONAL ?
                                             1.0f / fmin(ellipse->radius[0], ellipse->radius[1]) :
                                             1.0f);

                ellipse->border = dt_masks_change_size(up, ellipse->border, 0.001f * reference,
            ...
            else if (gui->edit_mode == DT_MASKS_EDIT_FULL && dt_modifier_is(state, 0))
            {
                ...
                ellipse->radius[0] =
                    dt_masks_change_size(up, ellipse->radius[0], 0.001f, radius_limit);
                ...
                dt_toast_log(_("size: %3.2f%%"),
                             fmaxf(ellipse->radius[0], ellipse->radius[1]) * 100);
            }
            else if (!dt_modifier_is(state, 0))
            {
...
static int _ellipse_events_button_pressed(dt_iop_module_t *module, const float pzx, const float pzy,
...
static int _ellipse_events_button_released(dt_iop_module_t *module, const float pzx,
                                           const float pzy, const int which, const uint32_t state,
                                           ...
                                           dt_masks_form_gui_t *gui, const int index)
{
    ...
    if (which == 3 && dt_is_valid_maskid(parentid) && gui->edit_mode == DT_MASKS_EDIT_FULL)
    {
        // we hide the form
    ...
    else if (gui->form_rotating && gui->edit_mode == DT_MASKS_EDIT_FULL)
    {
        ...
        float check_angle = atan2f(pts2[7] - pts2[1], pts2[6] - pts2[0]) -
                            atan2f(pts2[5] - pts2[1], pts2[4] - pts2[0]);
...
static int _ellipse_events_mouse_moved(dt_iop_module_t *module, const float pzx, const float pzy,
                                       const double pressure, const int which,
                                       const float zoom_scale, dt_masks_form_t *form,
                                       const dt_mask_id_t parentid, dt_masks_form_gui_t *gui,
                                       const int index)
{
    ...
    else if (!gui->creation)
    {
        ...
        dt_control_queue_redraw_center();
        if (!gui->form_selected && !gui->border_selected)
            return 0;
...
static void _ellipse_events_post_expose(cairo_t *cr, const float zoom_scale,
                                        dt_masks_form_gui_t *gui, const int index,
                                        const int num_points)
{
    ...
    if (gui->creation)
    {
        if (gui->guipoints_count == 0)
        {
            ...
            const int flags = dt_conf_get_int(DT_MASKS_CONF(form->type, ellipse, flags));
            const float radius_a = dt_conf_get_float(DT_MASKS_CONF(form->type, ellipse, radius_a));
            const float radius_b = dt_conf_get_float(DT_MASKS_CONF(form->type, ellipse, radius_b));
            const float rotation = dt_conf_get_float(DT_MASKS_CONF(form->type, ellipse, rotation));
            ...
            draw = _ellipse_get_points(darktable.develop, x, y, radius_a, radius_b, rotation,
                                       &points, &points_count);
            if (draw && masks_border > 0.f)
            {
                draw = _ellipse_get_points(
                    darktable.develop, x, y,
                    (flags & DT_MASKS_ELLIPSE_PROPORTIONAL ? radius_a * (1.0f + masks_border) :
                                                             radius_a + masks_border),
                    (flags & DT_MASKS_ELLIPSE_PROPORTIONAL ? radius_b * (1.0f + masks_border) :
            ...
            if (draw && points_count >= 2)
            {
                ...
                _ellipse_draw_shape(FALSE, FALSE, cr, FALSE, zoom_scale, xref, yref, points,
                                    points_count);
            }
            if (draw && border_count >= 2)
            {
                ...
                _ellipse_draw_shape(TRUE, FALSE, cr, FALSE, zoom_scale, xref, yref, border,
                                    border_count);
            }
            ...
            if (form->type & DT_MASKS_CLONE)
            {
                x = 0.0f;
                y = 0.0f;
                dt_masks_calculate_source_pos_value(gui, DT_MASKS_ELLIPSE, gui->posx, gui->posy,
    ...
    _ellipse_draw_shape(FALSE, FALSE, cr, selected, zoom_scale, xref, yref, gpt->points,
                        gpt->points_count);

    // draw border
    if (gui->show_all_feathers || gui->group_selected == index)
    {
        _ellipse_draw_shape(TRUE, FALSE, cr, gui->border_selected, zoom_scale, xref, yref,
        ...
        {
            float x, y;
            _ellipse_point_transform(xref, yref, gpt->points[i * 2], gpt->points[i * 2 + 1], sinr,
                                     cosr, &x, &y);
            dt_masks_draw_anchor(cr, i == gui->point_dragging || i == gui->point_selected,
                                 zoom_scale, x, y);
            _ellipse_point_transform(xref, yref, gpt->border[i * 2], gpt->border[i * 2 + 1], sinr,
                                     cosr, &x, &y);
            dt_masks_draw_anchor(cr,
                                 i == gui->point_border_dragging || i == gui->point_border_selected,
                                 zoom_scale, x, y);
        }
    }
    ...
    if (gpt->source_count > 10)
    {
        ...
        if (cdx != 0.0 && cdy != 0.0)
        {
            ...
            dt_masks_closest_point(gpt->points_count, _nb_ctrl_point(), gpt->points, gpt->source[0],
                                   gpt->source[1], &to_x, &to_y);

            dt_masks_closest_point(gpt->source_count, _nb_ctrl_point(), gpt->source, to_x, to_y,
                                   &from_x, &from_y);
        ...
        // we draw the source
        _ellipse_draw_shape(FALSE, TRUE, cr, selected, zoom_scale, xrefs, yrefs, gpt->source,
                            gpt->source_count);
    }
}

static void _bounding_box(const float *const points, const int num_points, int *width, int *height,
                          int *posx, int *posy)
{
...
static void _fill_mask(const size_t numpoints, float *const bufptr, const float *const points,
                       const float *const center, const float a, const float b, const float ta,
                       const float tb, const float alpha, const size_t out_scale)
{
    const float a2 = a * a;
    const float b2 = b * b;
                                    ...
                                    int *posy)
{
    ...
    const int prop = ellipse->flags & DT_MASKS_ELLIPSE_PROPORTIONAL;
    const float total[2] = {(prop ? ellipse->radius[0] * (1.0f + ellipse->border) :
                                    ellipse->radius[0] + ellipse->border) *
                                MIN(wd, ht),
                            (prop ? ellipse->radius[1] * (1.0f + ellipse->border) :
    ...
    float *const restrict points =
        _ellipse_points_to_transform(form->source[0], form->source[1], total[0], total[1],
                             ...
                             int *width, int *height, int *posx, int *posy)
{
    ...
    size_t point_count = 0;
    float *const restrict points =
        _ellipse_points_to_transform(ellipse->center[0], ellipse->center[1], total[0], total[1],
                                     ellipse->rotation, wd, ht, &point_count);
    if (!points)
        return 0;
                             ...
                             float **buffer, int *width, int *height, int *posx, int *posy)
{
    ...
    // we get the area
    if (!_ellipse_get_area(module, piece, form, width, height, posx, posy))
        return 0;

    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] ellipse area took %0.04f sec", form->name,
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] ellipse draw took %0.04f sec", form->name,
             dt_get_lap_time(&start2));
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] ellipse transform took %0.04f sec",
    ...
    *buffer = dt_alloc_align_float((size_t)w * h);
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] ellipse fill took %0.04f sec", form->name,
             dt_get_lap_time(&start2));
                                 ...
                                 float *buffer)
{
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] ellipse init took %0.04f sec", form->name,
             dt_get_lap_time(&start2));
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] ellipse outline took %0.04f sec",
             form->name, dt_get_lap_time(&start2));

    // we transform the outline from input image coordinates to current position in pixelpipe
    if (!dt_dev_distort_transform_plus(module->dev, piece->pipe, module->iop_order,
                                       DT_DEV_TRANSFORM_DIR_BACK_INCL, ell, ellpts))
    {
        dt_free_align(ell);
        return 0;
    }

    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] ellipse outline transform took %0.04f sec",
             form->name, dt_get_lap_time(&start2));
    ...
    for (int n = 0; n < ellpts; n++)
    {
        ...
        if (!(dt_isnormal(ell[2 * n]) && dt_isnormal(ell[2 * n + 1])))
            continue;
  ...
  printf("gw %d, gh %d, bbw %d, bbh %d\n", gw, gh, bbw, bbh);
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] ellipse bounding box took %0.04f sec",
             form->name, dt_get_lap_time(&start2));
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] ellipse grid took %0.04f sec", form->name,
             dt_get_lap_time(&start2));
    ...
    for (int j = bbym * grid; j < endy; j++)
    {
        ...
        for (int i = bbxm * grid; i < endx; i++)
        {
            ...
            buffer[(size_t)j * w + i] = (points[mindex * 2] * (grid - ii) * (grid - jj) +
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] ellipse fill took %0.04f sec", form->name,
             dt_get_lap_time(&start2));
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] ellipse total render took %0.04f sec",
...
static GSList *_ellipse_setup_mouse_actions(const struct dt_masks_form_t *const form)
{
    ...
    lm = dt_mouse_action_create_simple(
        lm, DT_MOUSE_ACTION_SCROLL, GDK_SHIFT_MASK | GDK_CONTROL_MASK, _("[ELLIPSE] rotate shape"));
    ...
    lm = dt_mouse_action_create_simple(lm, DT_MOUSE_ACTION_LEFT_DRAG, GDK_CONTROL_MASK,
                                       _("[ELLIPSE] rotate shape"));
    return lm;
}

static void _ellipse_set_form_name(dt_masks_form_t *const form, const size_t nb)
{
    snprintf(form->name, sizeof(form->name), _("ellipse #%d"), (int)nb);
}

static void _ellipse_duplicate_points(dt_develop_t *const dev, dt_masks_form_t *const base,
...
static void _ellipse_sanitize_config(const dt_masks_type_t type)
{
    ...
    const int flags =
        dt_conf_get_and_sanitize_int(DT_MASKS_CONF(type, ellipse, flags),
...
static void _ellipse_modify_property(dt_masks_form_t *const form, const dt_masks_property_t prop,
                                     const float old_val, const float new_val, float *sum,
                                     int *count, float *min, float *max)
{
    const float ratio = (!old_val || !new_val) ? 1.0f : new_val / old_val;

    dt_masks_point_ellipse_t *ellipse = form->points ? form->points->data : NULL;

    float radius_a = ellipse ? ellipse->radius[0] :
                               dt_conf_get_float(DT_MASKS_CONF(form->type, ellipse, radius_a));
    float radius_b = ellipse ? ellipse->radius[1] :
    ...
    case DT_MASKS_PROPERTY_SIZE:;
        ...
        if (ellipse)
            ellipse->radius[0] = radius_a;
        if (ellipse)
            ellipse->radius[1] = radius_b;
        dt_conf_set_float(DT_MASKS_CONF(form->type, ellipse, radius_a), radius_a);
        dt_conf_set_float(DT_MASKS_CONF(form->type, ellipse, radius_b), radius_b);
    ...
    case DT_MASKS_PROPERTY_FEATHER:;
        const dt_masks_ellipse_flags_t flags =
            ellipse ? ellipse->flags : dt_conf_get_int(DT_MASKS_CONF(form->type, ellipse, flags));
        const float reference =
            flags & DT_MASKS_ELLIPSE_PROPORTIONAL ? 1.0f / fmin(radius_a, radius_b) : 1.0f;
    ...
    case DT_MASKS_PROPERTY_ROTATION:;
        float rotation = ellipse ? ellipse->rotation :
