...
static dt_gtkentry_completion_spec _default_path_compl_list[] = {
    ...
    {"VERSION.IF_MULTI",
     N_("$(VERSION.IF_MULTI) - same as $(VERSION) but null string if only one version exists")},
    {"VERSION.NAME", N_("$(VERSION.NAME) - version name from metadata")},
    {"JOBCODE", N_("$(JOBCODE) - job code for import")},
    {"SEQUENCE[4,1]",
     N_("$(SEQUENCE[n,m]) - sequence number, n: number of digits, m: start number")},
    ...
    {"EXIF.MONTH.SHORT",
     N_("$(EXIF.MONTH.SHORT) - abbreviated EXIF month name according to the current locale")},
    {"EXIF.MONTH.LONG",
     N_("$(EXIF.MONTH.LONG) - full EXIF month name according to the current locale")},
    ...
    {"EXIF.FOCAL.LENGTH.EQUIV",
     N_("$(EXIF.FOCAL.LENGTH.EQUIV) - EXIF 35 mm equivalent focal length")},
    ...
    {"GPS.LOCATION.ICON",
     N_("$(GPS.LOCATION.ICON) - icon indicating whether GPS location is known")},
    ...
    {"IMAGE.TAGS.HIERARCHY",
     N_("$(IMAGE.TAGS.HIERARCHY) - tags as set in metadata settings, preserving hierarchy")},
    ...
    if (var_present)
    {
        ...
        if (item != NULL)
        {
            ...
            if (normalized_string != NULL)
            {
                ...
                if (!g_ascii_strncasecmp(varname, case_normalized_string, strlen(varname)))
                    ret = TRUE;
...
{
    _completion_model = gtk_list_store_new(3, G_TYPE_INT, G_TYPE_STRING, G_TYPE_STRING);
    /* Populate the completion database. */
    for (const dt_gtkentry_completion_spec *l = _default_path_compl_list; l && l->varname; l++)
    {
        gtk_list_store_insert_with_values(
            _completion_model, NULL, -1, COMPL_ID, -1, // -1 = internal
            COMPL_VARNAME, l->varname, COMPL_DESCRIPTION, _(l->description), -1);
    }
...
void dt_gtkentry_variables_add_metadata(dt_metadata_t *metadata)
{
    ...
    gtk_list_store_insert_with_values(_completion_model, NULL, -1, COMPL_ID, metadata->key,
                                      COMPL_VARNAME, varname, COMPL_DESCRIPTION, description, -1);
    g_free(varname);
    g_free(description);
}
...
void dt_gtkentry_variables_remove_metadata(dt_metadata_t *metadata)
{
    ...
    {
        int32_t id;
        gtk_tree_model_get(GTK_TREE_MODEL(_completion_model), &iter, COMPL_ID, &id, -1);
