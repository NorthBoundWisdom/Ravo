...
static gboolean _gui_styles_dialog_run(const gboolean edit, const char *name,
                                       const dt_imgid_t imgid, char **new_name);
...
static gboolean _gui_styles_is_update_module_order_set(dt_gui_styles_dialog_t *d)
{
    ...
    gint num = 0;
    if (gtk_tree_model_get_iter_first(model, &iter))
        gtk_tree_model_get(model, &iter, DT_STYLE_ITEMS_COL_UPDATE, &active, DT_STYLE_ITEMS_COL_NUM,
                           &num, -1);
    return active && (num == -1);
}
...
void _gui_styles_get_active_items(dt_gui_styles_dialog_t *sd, GList **enabled, GList **update)
{
    ...
    if (gtk_tree_model_get_iter_first(model, &iter))
    {
        ...
        {
            gtk_tree_model_get(model, &iter, DT_STYLE_ITEMS_COL_ENABLED, &active,
                               DT_STYLE_ITEMS_COL_UPDATE, &uactive, DT_STYLE_ITEMS_COL_NUM, &num,
                               DT_STYLE_ITEMS_COL_UPDATE_NUM, &update_num,
                               DT_STYLE_ITEMS_COL_AUTOINIT, &autoinit, -1);
    ...
    if (gtk_tree_model_get_iter_first(model, &iter))
    {
        ...
        {
            gtk_tree_model_get(model, &iter, DT_STYLE_ITEMS_COL_ENABLED, &active,
                               DT_STYLE_ITEMS_COL_NUM, &num, DT_STYLE_ITEMS_COL_UPDATE_NUM,
                               &update_num, DT_STYLE_ITEMS_COL_AUTOINIT, &autoinit, -1);
            if (active)
            {
...
static void _gui_styles_select_all_items(dt_gui_styles_dialog_t *d, const gboolean active)
{
    ...
    if (gtk_tree_model_get_iter_first(model, &iter))
    {
        do
        {
            gtk_list_store_set(GTK_LIST_STORE(model), &iter, DT_STYLE_ITEMS_COL_ENABLED, active,
                               -1);
        } while (gtk_tree_model_iter_next(model, &iter));
    }
}

static void _gui_styles_new_style_response(GtkDialog *dialog, const gint response_id,
                                           dt_gui_styles_dialog_t *g)
{
    g->cancelled =
        (response_id == GTK_RESPONSE_DELETE_EVENT) || (response_id == GTK_RESPONSE_REJECT);
    ...
    else if (response_id == GTK_RESPONSE_ACCEPT)
    {
        ...
        if (newname && *newname)
        {
            ...
            if (dt_styles_create_from_image(newname, gtk_entry_get_text(GTK_ENTRY(g->description)),
                                            g->imgid, result,
        ...
        else
        {
            ...
            GtkWidget *dlg_changename = gtk_message_dialog_new(
                GTK_WINDOW(window), GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_WARNING,
...
static void _gui_styles_edit_style_response(GtkDialog *dialog, const gint response_id,
                                            dt_gui_styles_dialog_t *g)
{
    g->cancelled =
        (response_id == GTK_RESPONSE_DELETE_EVENT) || (response_id == GTK_RESPONSE_REJECT);
        ...
        if (newname && *newname)
        {
            if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(g->duplicate)))
            {
                dt_styles_create_from_style(
                    g->nameorig, newname, gtk_entry_get_text(GTK_ENTRY(g->description)), result,
                    g->imgid, update, _gui_styles_is_copy_module_order_set(g),
                    _gui_styles_is_update_module_order_set(g));
            }
            else
            {
                dt_styles_update(g->nameorig, newname,
                                 gtk_entry_get_text(GTK_ENTRY(g->description)), result, g->imgid,
...
static void _gui_styles_item_toggled(GtkCellRendererToggle *cell, gchar *path_str, gpointer data)
{
    ...
    gtk_tree_model_get(model, &iter, DT_STYLE_ITEMS_COL_ENABLED, &toggle_item,
                       DT_STYLE_ITEMS_COL_NUM, &num, DT_STYLE_ITEMS_COL_UPDATE_NUM, &update_num,
                       -1);
...
static void _gui_styles_item_autoinit_toggled(GtkCellRendererToggle *cell, gchar *path_str,
                                              gpointer data)
{
    dt_gui_styles_dialog_t *sd = (dt_gui_styles_dialog_t *)data;
    ...
    gtk_tree_model_get_iter(model, &iter, path);
    gtk_tree_model_get(model, &iter, DT_STYLE_ITEMS_COL_AUTOINIT, &toggle_item, -1);

    toggle_item = (toggle_item == TRUE) ? FALSE : TRUE;

    gtk_list_store_set(GTK_LIST_STORE(model), &iter, DT_STYLE_ITEMS_COL_AUTOINIT, toggle_item, -1);

    // auto-init (reset) is only meaningful if the module is also updated
    if (toggle_item)
        gtk_list_store_set(GTK_LIST_STORE(model), &iter, DT_STYLE_ITEMS_COL_ENABLED, !toggle_item,
                           DT_STYLE_ITEMS_COL_UPDATE, toggle_item, -1);

    gtk_tree_path_free(path);
}

static void _gui_styles_item_new_autoinit_toggled(GtkCellRendererToggle *cell, gchar *path_str,
                                                  gpointer data)
{
    dt_gui_styles_dialog_t *sd = (dt_gui_styles_dialog_t *)data;
...
static void _gui_styles_item_new_toggled(GtkCellRendererToggle *cell, gchar *path_str,
                                         gpointer data)
{
    dt_gui_styles_dialog_t *sd = (dt_gui_styles_dialog_t *)data;
    ...
    // auto-init (reset) is only meaningful if the module is also included
    if (!toggle_item)
        gtk_list_store_set(GTK_LIST_STORE(model), &iter, DT_STYLE_ITEMS_COL_AUTOINIT, toggle_item,
                           -1);
                                       ...
                                       const dt_imgid_t imgid, char **new_name)
{
    ...
    if (edit)
    {
        ...
        gtk_widget_set_tooltip_text(sd->duplicate,
    ...
    GtkDialog *dialog = GTK_DIALOG(gtk_dialog_new_with_buttons(
        title, GTK_WINDOW(window), GTK_DIALOG_DESTROY_WITH_PARENT, _("select _all"),
        GTK_RESPONSE_YES, _("select _none"), GTK_RESPONSE_NONE, _("_cancel"), GTK_RESPONSE_REJECT,
    ...
    GtkListStore *liststore =
        gtk_list_store_new(DT_STYLE_ITEMS_NUM_COLS, G_TYPE_BOOLEAN, G_TYPE_BOOLEAN, GDK_TYPE_PIXBUF,
                           G_TYPE_BOOLEAN, G_TYPE_STRING, GDK_TYPE_PIXBUF, G_TYPE_INT, G_TYPE_INT);

    sd->items_new = GTK_TREE_VIEW(gtk_tree_view_new());
    GtkListStore *liststore_new =
        gtk_list_store_new(DT_STYLE_ITEMS_NUM_COLS, G_TYPE_BOOLEAN, G_TYPE_STRING, GDK_TYPE_PIXBUF,
                           G_TYPE_BOOLEAN, G_TYPE_STRING, GDK_TYPE_PIXBUF, G_TYPE_INT, G_TYPE_INT);
    ...
    gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(sd->items), -1,
                                                edit ? _("keep") : _("include"), renderer, "active",
    ...
    g_signal_connect(renderer, "toggled", G_CALLBACK(_gui_styles_item_autoinit_toggled), sd);
    gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(sd->items), -1, _("reset"), renderer,
                                                "active", DT_STYLE_ITEMS_COL_AUTOINIT, NULL);
    ...
    if (edit)
    {
        ...
        gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(sd->items_new), -1, _("include"),
                                                    renderer, "active", DT_STYLE_ITEMS_COL_ENABLED,
                                                    NULL);
        ...
        gtk_cell_renderer_toggle_set_activatable(GTK_CELL_RENDERER_TOGGLE(renderer), TRUE);
        g_object_set_data(G_OBJECT(renderer), "column", (gint *)DT_STYLE_ITEMS_COL_AUTOINIT);
        g_signal_connect(renderer, "toggled", G_CALLBACK(_gui_styles_item_new_autoinit_toggled),
                         sd);
        gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(sd->items_new), -1, _("reset"),
                                                    renderer, "active", DT_STYLE_ITEMS_COL_AUTOINIT,
                                                    NULL);
    }
    ...
    if (edit && dt_is_valid_imgid(imgid))
    {
        ...
        gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(sd->items), -1, _("update"),
                                                    renderer, "active", DT_STYLE_ITEMS_COL_UPDATE,
                                                    NULL);
    }
    ...
    GtkTreeViewColumn *column = gtk_tree_view_column_new_with_attributes(
        "", renderer, "pixbuf", DT_STYLE_ITEMS_COL_ISACTIVE, NULL);
    ...
    gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(sd->items), -1, _("item"), renderer,
                                                "markup", DT_STYLE_ITEMS_COL_NAME, NULL);

    if (edit)
    {
        gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(sd->items_new), -1, _("item"),
                                                    renderer, "markup", DT_STYLE_ITEMS_COL_NAME,
                                                    NULL);
    }
    ...
    gtk_tree_selection_set_mode(gtk_tree_view_get_selection(GTK_TREE_VIEW(sd->items)),
                                GTK_SELECTION_SINGLE);
    gtk_tree_view_set_model(GTK_TREE_VIEW(sd->items), GTK_TREE_MODEL(liststore));

    gtk_tree_selection_set_mode(gtk_tree_view_get_selection(GTK_TREE_VIEW(sd->items_new)),
                                GTK_SELECTION_SINGLE);
    ...
    if (edit)
    {
        gtk_list_store_insert_with_values(
            liststore, NULL, -1, DT_STYLE_ITEMS_COL_ENABLED, dt_styles_has_module_order(name),
            DT_STYLE_ITEMS_COL_ISACTIVE, is_active_pb, DT_STYLE_ITEMS_COL_NAME, _("module order"),
        ...
        if (items)
        {
            for (const GList *items_iter = items; items_iter; items_iter = g_list_next(items_iter))
            {
                ...
                {
                    gtk_list_store_insert_with_values(
                        liststore, NULL, -1, DT_STYLE_ITEMS_COL_ENABLED, TRUE,
                        DT_STYLE_ITEMS_COL_AUTOINIT, FALSE, DT_STYLE_ITEMS_COL_UPDATE, FALSE,
                        DT_STYLE_ITEMS_COL_ISACTIVE, item->enabled ? is_active_pb : is_inactive_pb,
                        DT_STYLE_ITEMS_COL_NAME, item->name, DT_STYLE_ITEMS_COL_MASK,
                        mask_mode > 0 ? mask : NULL, DT_STYLE_ITEMS_COL_NUM, item->num,
                        DT_STYLE_ITEMS_COL_UPDATE_NUM, item->selimg_num, -1);
                    has_item = TRUE;
                }
                else if (item->num != -1 ||
                         item->selimg_num !=
                             -1) // defined in one or the other, let a way to select it or not
                {
                    gtk_list_store_insert_with_values(
                        liststore_new, NULL, -1, DT_STYLE_ITEMS_COL_ENABLED,
                        item->num != -1 ? TRUE : FALSE, DT_STYLE_ITEMS_COL_AUTOINIT, FALSE,
                        DT_STYLE_ITEMS_COL_ISACTIVE, item->enabled ? is_active_pb : is_inactive_pb,
                        DT_STYLE_ITEMS_COL_NAME, item->name, DT_STYLE_ITEMS_COL_MASK,
                        mask_mode > 0 ? mask : NULL, DT_STYLE_ITEMS_COL_NUM, item->num,
                        DT_STYLE_ITEMS_COL_UPDATE_NUM, item->selimg_num, -1);
    ...
    else
    {
        ...
        gtk_list_store_insert_with_values(
            liststore, NULL, -1, DT_STYLE_ITEMS_COL_ENABLED, TRUE, DT_STYLE_ITEMS_COL_ISACTIVE,
            is_active_pb, DT_STYLE_ITEMS_COL_NAME, label, DT_STYLE_ITEMS_COL_NUM, -1, -1);
        g_free(label);
        ...
        if (items)
        {
            for (const GList *items_iter = items; items_iter; items_iter = g_list_next(items_iter))
            {
                ...
                {
                    GList *result =
                        g_list_find_custom(modules, item->op, _g_list_find_module_by_name);
                    if (result)
                    {
                ...
                gtk_list_store_insert_with_values(
                    liststore, NULL, -1, DT_STYLE_ITEMS_COL_ENABLED, enabled,
                    DT_STYLE_ITEMS_COL_AUTOINIT, FALSE, DT_STYLE_ITEMS_COL_ISACTIVE,
                    item->enabled ? is_active_pb : is_inactive_pb, DT_STYLE_ITEMS_COL_NAME,
                    item->name, DT_STYLE_ITEMS_COL_MASK, item->mask_mode > 0 ? mask : NULL,
...
static gboolean _preview_draw(GtkWidget *widget, cairo_t *cr, gpointer user_data)
{
    ...
    if (data->surface)
    {
        ...
        cairo_set_source_surface(cr, data->surface, .5f * (psize - swidth),
                                 .5f * (psize - sheight));
        cairo_paint(cr);
    }
    else
...
GtkWidget *dt_gui_style_content_dialog(char *name, const dt_imgid_t imgid)
{
    ...
    if (imgid != data.imgid || g_strcmp0(data.style_name, name) ||
        data.hash_len != hash.current_len || memcmp(data.hash, hash.current, data.hash_len))
    {
        if (data.surface)
        {
...
#define STYLE_TOOLTIP_MAX_WIDTH 30
    ...
    if (des && strlen(des) > 0)
    {
        ...
        gtk_box_pack_start(GTK_BOX(ht), gtk_separator_new(GTK_ORIENTATION_HORIZONTAL), TRUE, TRUE,
                           0);
    ...
    while (l)
    {
        ...
        char buf[1024];
        snprintf(buf, sizeof(buf), "  %s %s %s", i->enabled ? "●" : "○", gettext(i->name), mn);
