...
void dt_splash_screen_create(const gboolean force)
{
    ...
    if (darktable.splash.start_screen || dt_check_gimpmode("file") || dt_check_gimpmode("thumb") ||
    ...
    gchar *image_file =
        g_strdup_printf("%s/pixmaps/splashscreen-%02d.jpg", darktable.datadir, imgnum);
    ...
    dt_gui_box_add(content,
                   dt_gui_hbox(dt_gui_vbox(logo, version, program_name, program_desc), image));
#else
    ...
    dt_gui_box_add(content, dt_gui_vbox(dt_gui_hbox(logo_col, title_col), program_desc, sep,
                                        darktable.splash.progress_text));
#endif

    gtk_widget_set_halign(darktable.splash.progress_text, GTK_ALIGN_START);

    darktable.splash.remaining_box = dt_gui_hbox(dtgtk_button_new(dtgtk_cairo_paint_clock, 0, 0),
                                           ...
                                           const double elapsed)
{
    ...
    if (darktable.splash.start_screen)
    {
        ...
        if (elapsed >= 2.0 || fraction > 0.01)
        {
            ...
            const double remain =
                (total - elapsed) + 0.5; // round to full seconds rather than truncating
