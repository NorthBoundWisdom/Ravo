...
{
    const char *name =
        dt_metadata_get_tag_subkey((char *)g_object_get_data(G_OBJECT(widget), "tagname"));
...
static void _metadata_reset_all(dt_import_metadata_t *metadata, const gboolean hard)
{
    ...
    {
        // import module reset
        for (unsigned int i = DT_META_META_VALUE; i < metadata->num_grid_rows + DT_META_TAGS_VALUE;
             i++)
        {
...
static void _import_metadata_toggled(GtkWidget *widget, dt_import_metadata_t *metadata)
{
    ...
    if (g_strcmp0(name, "tags"))
    {
        ...
        dt_conf_set_int(setting, imported ? flag | DT_METADATA_FLAG_IMPORTED :
...
{
    GtkWidget *w = gtk_grid_get_child_at(GTK_GRID(metadata->grid), 1,
                                         metadata->num_grid_rows + DT_META_META_VALUE);
    gtk_combo_box_set_active(GTK_COMBO_BOX(w), -1);
    w = gtk_grid_get_child_at(GTK_GRID(metadata->grid), 1,
                              metadata->num_grid_rows + DT_META_TAGS_HEADER);
...
static void _update_layout(dt_import_metadata_t *metadata)
{
    ...
    w = gtk_grid_get_child_at(GTK_GRID(metadata->grid), 2,
                              metadata->num_grid_rows + DT_META_TAGS_HEADER);
    gtk_widget_set_visible(w, !write_xmp);
}
...
static void _metadata_list_changed(gpointer instance, const int type,
                                   dt_import_metadata_t *metadata)
{
...
static void _import_metadata_presets_update(dt_import_metadata_t *metadata)
{
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        {
            gtk_list_store_insert_with_values(metadata->m_model, NULL, -1, 0,
                                              (char *)sqlite3_column_text(stmt, 0), 1, metadata_kv,
                                              -1);
        }
    }
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        if (op_params_size)
        {
            ...
            if (tokens)
            {
                ...
                g_strfreev(tokens);
                gtk_list_store_insert_with_values(metadata->t_model, NULL, -1, 0,
                                                  (char *)sqlite3_column_text(stmt, 0), 1, tags,
                                                  -1);
                g_free(tags);
            }
        }
...
static void _metadata_presets_changed(gpointer instance, gpointer module,
                                      dt_import_metadata_t *metadata)
{
...
static GtkWidget *_set_up_combobox(GtkListStore *model, const int line,
                                   dt_import_metadata_t *metadata)
{
...
static void _set_up_entry(GtkWidget *entry, const char *str, const char *name, const int line,
                          dt_import_metadata_t *metadata)
{
...
static void _fill_metadata_grid(dt_import_metadata_t *metadata)
{
    for (uint32_t i = DT_META_META_VALUE + metadata->num_grid_rows - 1; i >= DT_META_META_VALUE;
         i--)
    ...
    for (GList *iter = dt_metadata_get_list(); iter; iter = iter->next)
    {
        ...
        GtkWidget *labelev =
            _set_up_label(metadata_label, GTK_ALIGN_START, i + DT_META_META_VALUE, metadata);
        ...
        _set_up_toggle_button(metadata_imported, flag & DT_METADATA_FLAG_IMPORTED, metadata_name,
                              i + DT_META_META_VALUE, metadata);
...
void dt_import_metadata_init(dt_import_metadata_t *metadata)
{
    ...
    gtk_widget_set_name(label, "import-presets");
    labelev = _set_up_label(label, GTK_ALIGN_START, metadata->num_grid_rows + DT_META_META_VALUE,
                            metadata);

    presets =
        _set_up_combobox(metadata->t_model, metadata->num_grid_rows + DT_META_META_VALUE, metadata);
    g_signal_connect(presets, "changed", G_CALLBACK(_import_tags_presets_changed), metadata);

    label = gtk_label_new(_("tags"));
    labelev = _set_up_label(label, GTK_ALIGN_START, metadata->num_grid_rows + DT_META_TAGS_HEADER,
                            metadata);
    ...
    // overall
    g_signal_connect(metadata->apply_metadata, "toggled", G_CALLBACK(_apply_metadata_toggled),
                     metadata);
