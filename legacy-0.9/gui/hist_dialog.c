...
static gboolean _gui_hist_is_copy_module_order_set(dt_history_copy_item_t *d)
{
   ...
   * (num=-1 and active) */
    ...
    do
    {
        gtk_tree_model_get(model, &iter, DT_HIST_ITEMS_COL_ENABLED, &active, DT_HIST_ITEMS_COL_NUM,
                           &num, -1);
        if (active && (num == -1))
            module_order_was_copied = TRUE;
...
static GList *_gui_hist_get_active_items(dt_history_copy_item_t *d)
{
    ...
    if (gtk_tree_model_get_iter_first(model, &iter))
    {
        do
        {
            ...
            gint num = 0;
            gtk_tree_model_get(model, &iter, DT_HIST_ITEMS_COL_ENABLED, &active,
                               DT_HIST_ITEMS_COL_AUTOINIT, &autoinit, DT_HIST_ITEMS_COL_NUM, &num,
                               -1);
...
static void _gui_hist_copy_response(GtkDialog *dialog, const gint response_id,
...
static void _gui_hist_item_toggled(GtkCellRendererToggle *cell, const gchar *path_str,
                                   gpointer data)
{
    dt_history_copy_item_t *d = (dt_history_copy_item_t *)data;

    const _styles_columns_t col = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(cell), "column"));
...
void tree_on_row_activated(GtkTreeView *treeview, GtkTreePath *path, GtkTreeViewColumn *col,
                           gpointer userdata)
{
...
int dt_gui_hist_dialog_new(dt_history_copy_item_t *d, const dt_imgid_t imgid, const gboolean iscopy)
{
    int res;
    ...
    if (iscopy)
    {
        dialog = GTK_DIALOG(gtk_dialog_new_with_buttons(
            _("select parts to copy"), GTK_WINDOW(window),
            GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT, _("select _all"), GTK_RESPONSE_YES,
            _("select _none"), GTK_RESPONSE_NONE, _("_cancel"), GTK_RESPONSE_CANCEL, _("_ok"),
            GTK_RESPONSE_OK, NULL));
    }
    else
    {
        dialog = GTK_DIALOG(gtk_dialog_new_with_buttons(
            _("select parts to paste"), GTK_WINDOW(window),
            GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT, _("select _all"), GTK_RESPONSE_YES,
            _("select _none"), GTK_RESPONSE_NONE, _("_cancel"), GTK_RESPONSE_CANCEL,
            _("_overwrite"), GTK_RESPONSE_APPLY, _("appen_d"), GTK_RESPONSE_OK, NULL));
    }
    ...
    GtkListStore *liststore =
        gtk_list_store_new(DT_HIST_ITEMS_NUM_COLS, G_TYPE_BOOLEAN, GDK_TYPE_PIXBUF, G_TYPE_BOOLEAN,
    ...
    gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(d->items), -1, _("include"), renderer,
                                                "active", DT_HIST_ITEMS_COL_ENABLED, NULL);
    ...
    gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(d->items), -1, _("reset"), renderer,
                                                "active", DT_HIST_ITEMS_COL_AUTOINIT, NULL);

    /* active */
    renderer = gtk_cell_renderer_pixbuf_new();
    GtkTreeViewColumn *column = gtk_tree_view_column_new_with_attributes(
        "", renderer, "pixbuf", DT_HIST_ITEMS_COL_ISACTIVE, NULL);
    ...
    gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(d->items), -1, _("item"), renderer,
                                                "markup", DT_HIST_ITEMS_COL_NAME, NULL);
    ...
    if (items)
    {
        for (const GList *items_iter = items; items_iter; items_iter = g_list_next(items_iter))
        {
            ...
            if (!(flags & IOP_FLAGS_HIDDEN))
            {
                gtk_list_store_insert_with_values(
                    liststore, NULL, -1, DT_HIST_ITEMS_COL_ENABLED,
                    iscopy ? FALSE : _gui_is_set(d->selops, item->num), DT_HIST_ITEMS_COL_AUTOINIT,
                    FALSE, DT_HIST_ITEMS_COL_ISACTIVE,
                    item->enabled ? is_active_pb : is_inactive_pb, DT_HIST_ITEMS_COL_NAME,
                    item->name, DT_HIST_ITEMS_COL_MASK, item->mask_mode > 0 ? mask : NULL,
                    DT_HIST_ITEMS_COL_NUM, (gint)item->num, -1);
            }
        }
        ...
        if (iscopy || d->copy_iop_order)
        {
            ...
            gtk_list_store_insert_with_values(liststore, NULL, -1, DT_HIST_ITEMS_COL_ENABLED,
                                              d->copy_iop_order, DT_HIST_ITEMS_COL_ISACTIVE,
                                              is_active_pb, DT_HIST_ITEMS_COL_NAME, label,
                                              DT_HIST_ITEMS_COL_NUM, -1, -1);
            g_free(label);
        }
    }
    else
    ...
    g_signal_connect(GTK_TREE_VIEW(d->items), "row-activated", G_CALLBACK(tree_on_row_activated),
                     GTK_WIDGET(dialog));
    ...
    while (1)
    {
        ...
        if (res == GTK_RESPONSE_CANCEL || res == GTK_RESPONSE_DELETE_EVENT ||
            res == GTK_RESPONSE_OK || res == GTK_RESPONSE_APPLY)
            break;
    }
