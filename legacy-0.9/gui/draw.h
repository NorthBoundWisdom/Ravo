...
static inline void dt_draw_set_color_overlay(cairo_t *cr, const gboolean bright, const double alpha)
{
    const double amt = bright ? 0.5 + darktable.gui->overlay_contrast * 0.5 :
                                (1.0 - darktable.gui->overlay_contrast) * 0.5;

    cairo_set_source_rgba(cr, darktable.gui->overlay_red * amt, darktable.gui->overlay_green * amt,
                          darktable.gui->overlay_blue * amt, alpha);
}

/** draws a rating star
 */
static inline void dt_draw_star(cairo_t *cr, const float x, const float y, const float r1,
                                const float r2)
{
...
static inline void dt_draw_line(cairo_t *cr, const float left, const float top, const float right,
                                const float bottom)
{
...
static inline void dt_draw_grid(cairo_t *cr, const int num, const int left, const int top,
...
static inline void dt_draw_grid_zoomed(cairo_t *cr, const int num, const float left,
...
static inline void dt_draw_waveform_lines(cairo_t *cr, const int left, const int top,
                                          const int right, const int bottom,
                                          const gboolean horizontal)
{
    ...
    for (int k = 1; k < num; k++)
    {
        ...
        if (horizontal)
            dt_draw_line(cr, left, top + k / (float)num * height, right,
                         top + k / (float)num * height);
        else
            dt_draw_line(cr, right - k / (float)num * width, top, right - k / (float)num * width,
                         bottom);
        cairo_stroke(cr);
    }
...
static inline void dt_draw_horizontal_lines(cairo_t *cr, const int num, const int left,
...
static inline void dt_draw_curve_set_point(dt_draw_curve_t *c, const int num, const float x,
                                           const float y)
{
...
static inline void dt_draw_curve_smaple_values(dt_draw_curve_t *c, const float min, const float max,
...
static inline void dt_draw_curve_calc_values_V2_nonperiodic(dt_draw_curve_t *c, const float min,
                                                            const float max, const int res,
                                                            float *x, float *y)
{
    c->csample.m_samplingRes = res;
    c->csample.m_outputRes = 0x10000;
...
static inline void dt_draw_curve_calc_values_V2_periodic(dt_draw_curve_t *c, const float min,
                                                         const float max, const int res, float *x,
                                                         float *y)
{
    c->csample.m_samplingRes = res;
    c->csample.m_outputRes = 0x10000;
...
static inline void dt_draw_curve_calc_values_V2(dt_draw_curve_t *c, const float min,
                                                const float max, const int res, float *x, float *y,
...
static inline void dt_draw_curve_add_point(dt_draw_curve_t *c, const float x, const float y)
{
...
// linear x linear y
static inline void dt_draw_histogram_8_linxliny(cairo_t *cr, const uint32_t *hist,
                                                const int32_t channels, const int32_t channel)
{
...
static inline void dt_draw_histogram_8_zoomed(cairo_t *cr, const uint32_t *hist,
                                              const int32_t channels, const int32_t channel,
                                              const float zoom_factor, const float zoom_offset_x,
                                              const float zoom_offset_y, const gboolean linear)
{
...
static inline void dt_draw_histogram_8_logxliny(cairo_t *cr, const uint32_t *hist,
                                                const int32_t channels, const int32_t channel,
...
static inline void dt_draw_histogram_8_logxlogy(cairo_t *cr, const uint32_t *hist,
                                                const int32_t channels, const int32_t channel,
                                                const float base_log)
{
    cairo_move_to(cr, 0, 0);
...
// linear x log y
static inline void dt_draw_histogram_8_linxlogy(cairo_t *cr, const uint32_t *hist,
                                                const int32_t channels, const int32_t channel)
{
    cairo_move_to(cr, 0, 0);
    for (int k = 0; k < 256; k++)
...
// log x (scalable)
static inline void dt_draw_histogram_8_log_base(cairo_t *cr, const uint32_t *hist,
                                                const int32_t channels, const int32_t channel,
                                                const gboolean linear, const float base_log)
{
    if (linear) // linear y
...
// linear x
static inline void dt_draw_histogram_8(cairo_t *cr, const uint32_t *hist, const int32_t channels,
                                       const int32_t channel, const gboolean linear)
{
    if (linear) // linear y
...
static inline void dt_draw_cairo_to_gdk_pixbuf(uint8_t *data, const unsigned int width,
...
static inline void dt_cairo_perceptual_gradient(cairo_pattern_t *grad, const double alpha)
{
...
static inline void dt_draw_pixbuf_free(guchar *pixels, gpointer user_data)
{
    (void)user_data;
    free(pixels);
}
...
static inline GdkPixbuf *dt_draw_paint_to_pixbuf(
    ...
    void (*dtgtk_cairo_paint_fct)(cairo_t *cr, const gint x, const gint y, const gint w,
                                  const gint h, const gint flags, void *data))
{
    ...
    GdkPixbuf *pixbuf = gdk_pixbuf_new_from_data(buf, GDK_COLORSPACE_RGB, TRUE, 8, dim, dim,
                                                 dim * 4, dt_draw_pixbuf_free, NULL);
