...
{
    return g_ascii_strcasecmp(label, "default") == 0 || strcmp(label, _("default")) == 0;
}

static gboolean _workspace_label_is_memory(const char *label)
{
    return g_ascii_strcasecmp(label, "memory") == 0 || strcmp(label, _("memory")) == 0;
}

static gboolean _workspace_label_is_reserved(const char *label)
{
    return _workspace_label_is_default(label) || _workspace_label_is_memory(label);
}
...
static void _workspace_show_message(dt_workspace_t *session, const char *title, const char *body)
{
    GtkWidget *const dlg =
        gtk_message_dialog_new(GTK_WINDOW(session->db_screen), GTK_DIALOG_DESTROY_WITH_PARENT,
                               GTK_MESSAGE_WARNING, GTK_BUTTONS_OK, "%s", body);
...
static gboolean _workspace_should_skip_copied_key(const char *key)
{
    ...
    for (int i = 0; prefixes[i]; i++)
        if (g_str_has_prefix(key, prefixes[i]))
            return TRUE;
...
static void _workspace_selected_template_sync_from_radios(dt_workspace_t *session)
{
    ...
    for (GSList *l = session->template_radios; l; l = l->next)
    {
        ...
        const gchar *const template_name = g_object_get_data(G_OBJECT(radio), "template-name");
        if (!template_name)
            return;
...
static void _workspace_radio_shell_sync_active(GObject *radio_gobj, GParamSpec *pspec G_GNUC_UNUSED,
...
static void _workspace_delete_db(GtkWidget *button, dt_workspace_t *session)
{
            ...
            label))
    {
        ...
        // db file
        snprintf(fpath, sizeof(fpath), "%s/library-%s.db", session->datadir, label);
        ...
        // resource file
        snprintf(fpath, sizeof(fpath), "%s/darktablerc-%s", session->datadir, label);
...
static void _workspace_new_db(GtkWidget *button, dt_workspace_t *session)
{
    ...
    {
        gchar *const msg = g_strdup_printf(_("WARNING\n\na workspace named \"%s\" already exists."
                                             "\n\nchoose a different name."),
                                           label);
...
{
    (void)entry;
    if (!gtk_widget_get_sensitive(session->create))
        return;
    _workspace_new_db(session->create, session);
}

static GtkWidget *_insert_button(dt_workspace_t *session, const char *label,
                                 const gboolean with_del, const gboolean with_radio, const int row)
{
    ...
    if (with_radio)
    {
        ...
        if (session->template_radio_leader)
            radio =
                gtk_radio_button_new_from_widget(GTK_RADIO_BUTTON(session->template_radio_leader));
        else
        {
        ...
        g_signal_connect(G_OBJECT(radio), "toggled", G_CALLBACK(_workspace_template_radio_toggled),
                         session);
    ...
    g_signal_connect(G_OBJECT(b), "clicked", G_CALLBACK(_workspace_select_db), session);
    if (radio)
    ...
    if (with_del)
    {
        ...
        g_signal_connect(G_OBJECT(del), "clicked", G_CALLBACK(_workspace_delete_db), session);
...
{
    if (dt_check_gimpmode("file") || dt_check_gimpmode("thumb") ||
...
#endif
    session->db_screen = gtk_dialog_new_with_buttons(_("darktable - select a workspace"), NULL,
                                                     flags, NULL, GTK_RESPONSE_NONE, NULL);
    ...
    g_signal_connect(G_OBJECT(session->entry), "changed", G_CALLBACK(_workspace_entry_changed),
                     session);
    ...
    g_signal_connect(G_OBJECT(session->create), "clicked", G_CALLBACK(_workspace_new_db), session);
    g_signal_connect(G_OBJECT(session->entry), "activate", G_CALLBACK(_workspace_entry_activate),
                     session);
    ...
    _workspace_template_radios_set_visible(
        session, gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(session->copy_template_check)));
