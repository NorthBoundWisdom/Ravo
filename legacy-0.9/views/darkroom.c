...
static void _get_zoom_pos(dt_dev_viewport_t *port, const double x, const double y, float *zoom_x,
                          float *zoom_y, float *zoom_scale)
{
    if (*zoom_x == FLT_MAX)
    {
...
static void _preview_pipe_zoom_correction(dt_develop_t *dev, float *cx, float *cy)
{
    *cx = *cy = 0.f;
    ...
    if (pp_wd <= 0 || pp_ht <= 0 || full_wd <= 0 || full_ht <= 0)
        return;

    // Full-pipe viewport centre in normalised coords
    float full_pts[2] = {dev->full.zoom_x, dev->full.zoom_y};
    dt_dev_distort_transform_plus(dev, dev->full.pipe, 0.0f, DT_DEV_TRANSFORM_DIR_ALL_GEOMETRY,
                                  full_pts, 1);
    ...
    float prev_pts[2] = {dev->full.zoom_x * prev_iw / full_iw,
                         dev->full.zoom_y * prev_ih / full_ih};
    dt_dev_distort_transform_plus(dev, dev->preview_pipe, 0.0f, DT_DEV_TRANSFORM_DIR_ALL_GEOMETRY,
                                  prev_pts, 1);
...
static void _get_zoom_pos_bnd(dt_dev_viewport_t *port, const double x, const double y,
                              float zbound_x, float zbound_y, float *zoom_x, float *zoom_y,
                              float *zoom_scale)
{
    if (zbound_x == FLT_MAX)
    {
    ...
    {
        dt_dev_get_pointer_zoom_pos_from_bounds(port, x, y, zbound_x, zbound_y, zoom_x, zoom_y,
                                                zoom_scale);
    }
}
...
void cleanup(dt_view_t *self)
{
    ...
    if (darktable.view_manager->guides_popover)
        g_object_unref(darktable.view_manager->guides_popover);
...
void _display_module_trouble_message_callback(gpointer instance, dt_iop_module_t *module,
...
{
    return dev->full.pipe->status == DT_DEV_PIXELPIPE_DIRTY ||
...
{
    return dev->preview_pipe->status == DT_DEV_PIXELPIPE_DIRTY ||
...
{
    return (dev->preview2.pipe->status == DT_DEV_PIXELPIPE_DIRTY ||
            dev->preview2.pipe->status == DT_DEV_PIXELPIPE_INVALID ||
            dev->full.pipe->input_timestamp > dev->preview2.pipe->input_timestamp) &&
           dev->gui_attached && dev->preview2.widget && GTK_IS_WIDGET(dev->preview2.widget);
}

static void _module_gui_post_expose(dt_iop_module_t *module, cairo_t *cri, const float width,
                                    const float height, const float x, const float y,
                                    const float zoom_scale)
{
    if (!module || !module->gui_post_expose || width < 1.0f || height < 1.0f)
        return;
...
static void _view_paint_surface(cairo_t *cr, const size_t width, const size_t height,
                                dt_dev_viewport_t *port, const dt_window_t window)
{
    dt_dev_pixelpipe_t *p = port->pipe;

    dt_pthread_mutex_lock(&p->backbuf_mutex);

    dt_view_paint_surface(cr, width, height, port, window, p->backbuf, p->backbuf_scale,
                          p->backbuf_width, p->backbuf_height, p->backbuf_zoom_pos);

    dt_pthread_mutex_unlock(&p->backbuf_mutex);
}

void expose(dt_view_t *self, cairo_t *cri, const int32_t width, const int32_t height,
            int32_t pointerx, int32_t pointery)
{
  ...
  */
    ...
    dt_view_set_scrollbar(self, zoom_x, -0.5 + boxw / 2, 0.5, boxw / 2, zoom_y, -0.5 + boxh / 2,
                          0.5, boxh / 2);

    const gboolean expose_full = port->pipe->backbuf // do we have an image?
                                 &&
    ...
    float prev_pts[2] = {port->zoom_x * prev_iw / full_iw, port->zoom_y * prev_ih / full_ih};
    dt_dev_distort_transform_plus(dev, dev->preview_pipe, 0.0f, DT_DEV_TRANSFORM_DIR_ALL_GEOMETRY,
                                  prev_pts, 1);
    ...
    {
        dt_print_pipe(DT_DEBUG_EXPOSE, "expose masks", port->pipe, dev->gui_module, DT_DEVICE_NONE,
                      NULL, NULL, "%dx%d, px=%d py=%d", width, height, pointerx, pointery);
    ...
    if (dev->proxy.rotate &&
        (darktable.control->button_down_which == GDK_BUTTON_SECONDARY || dmod == dev->proxy.rotate))
    {
        // reminder, we want this to be exposed always for guidings
        if (dev->proxy.rotate && dev->proxy.rotate->gui_post_expose)
        {
            _get_zoom_pos_bnd(&dev->full, pointerx, pointery, zbound_x, zbound_y, &pzx, &pzy,
                              &zoom_scale);
    ...
    else
    {
        ...
        if (dmod && dmod != dev->proxy.rotate)
        {
            ...
            if (expose_full && (dmod->operation_tags_filter() & IOP_TAG_CROPPING) &&
                dev->cropping.exposer && (dmod->iop_order < dev->cropping.exposer->iop_order))
            {
                dt_print_pipe(DT_DEBUG_EXPOSE, "expose cropper", port->pipe, dev->cropping.exposer,
                              DT_DEVICE_NONE, NULL, NULL, "%dx%d, px=%d py=%d", width, height,
                              pointerx, pointery);
                if (dev->cropping.exposer && dev->cropping.exposer->gui_post_expose)
                {
                    _get_zoom_pos_bnd(&dev->full, pointerx, pointery, zbound_x, zbound_y, &pzx,
                                      &pzy, &zoom_scale);
                    _module_gui_post_expose(dev->cropping.exposer, cri, wd, ht, pzx, pzy,
                                            zoom_scale);
                }
                guides = FALSE;
            }
            ...
            {
                dt_print_pipe(DT_DEBUG_EXPOSE, "expose module", port->pipe, dmod, DT_DEVICE_NONE,
                              NULL, NULL, "%dx%d, px=%d py=%d", width, height, pointerx, pointery);
                _get_zoom_pos_bnd(&dev->full, pointerx, pointery, zbound_x, zbound_y, &pzx, &pzy,
                                  &zoom_scale);
    ...
    {
        gchar *label = darktable.color_profiles->mode == DT_PROFILE_GAMUTCHECK ? _("gamut check") :
                                                                                 _("soft proof");

        dt_print_pipe(DT_DEBUG_EXPOSE, "expose profile", port->pipe, NULL, port->pipe->devid, NULL,
                      NULL, "%dx%d, px=%d py=%d. proof: %s", width, height, pointerx, pointery,
                      label);
...
static void _dev_change_image(dt_develop_t *dev, const dt_imgid_t imgid)
{
    ...
    if (gui_module)
    {
        dt_conf_set_string("plugins/darkroom/active", gui_module->op);
    }
...
static gboolean _dev_load_requested_image(gpointer user_data)
{
    ...
    if (dev->image_storage.id == NO_IMGID && dev->image_storage.id == imgid)
    ...
    for (int i = nb_iop - 1; i >= 0; i--)
    {
        ...
        if (module->multi_priority ==
            base_multi_priority) // if the module is the "base" instance, we keep it
        {
            module->iop_order =
...
static void _view_darkroom_filmstrip_activate_callback(gpointer instance, const dt_imgid_t imgid,
...
static void _dev_jump_image(dt_develop_t *dev, int diff, gboolean by_key)
{
    ...
    gchar *query = g_strdup_printf(
        "SELECT rowid, imgid FROM memory.collected_images WHERE rowid=%d", target_rowid);
    ...
    if (!dt_is_valid_imgid(new_id) || new_id == imgid)
        return;
...
static void _darkroom_ui_apply_style_popupmenu(GtkWidget *w, gpointer user_data)
{
    ...
    GtkMenuShell *menu =
        dtgtk_build_style_menu_hierarchy(FALSE, _darkroom_ui_apply_style_activate_callback,
                                         _darkroom_ui_apply_style_button_callback, user_data);
    if (menu)
    {
...
static void _color_assessment_border_width_callback(GtkWidget *slider, gpointer user_data)
{
    ...
    dt_conf_set_float("darkroom/ui/color_assessment_total_border_width",
...
{
    dt_develop_t *dev = (dt_develop_t *)user_data;
    dt_conf_set_float("darkroom/ui/color_assessment_border_white_ratio",
                      dt_bauhaus_slider_get(slider));
    if (dev->full.color_assessment)
    {
...
static void _guides_view_changed(gpointer instance, dt_view_t *old_view, dt_view_t *new_view,
...
static void _update_softproof_gamut_checking(dt_develop_t *d)
{
    ...
    g_signal_handlers_unblock_by_func(d->profile.softproof_button, _softproof_quickbutton_clicked,
                                      d);
...
static void _softproof_profile_callback(GtkWidget *combo, gpointer user_data)
{
    ...
    dt_print(DT_DEBUG_ALWAYS, "can't find softproof profile `%s', using sRGB instead",
...
static void _histogram_profile_callback(GtkWidget *combo, gpointer user_data)
{
    ...
    dt_print(DT_DEBUG_ALWAYS, "can't find histogram profile `%s', using export profile instead",
             dt_bauhaus_combobox_get_text(combo));
...
static void _preference_changed(gpointer instance, const dt_view_t *self)
{
    (void)instance;
    dt_develop_t *dev = self->data;
    ...
    for (const GList *modules = dev->iop; modules; modules = g_list_next(modules))
    {
        ...
        if (module->header)
            dt_iop_add_remove_mask_indicator(
                module, (module->blend_params->mask_mode != DEVELOP_MASK_DISABLED) &&
...
static void _display_profile_changed(gpointer instance, const uint8_t profile_type,
                                     gpointer user_data)
{
...
static void _display2_profile_changed(gpointer instance, const uint8_t profile_type,
                                      gpointer user_data)
{
    GtkWidget *cmb_display_profile = GTK_WIDGET(user_data);
...
static void _overlay_cycle_callback(dt_action_t *action)
{
    ...
    const int nextval =
        (currentval + 1) % DT_DEV_OVERLAY_LAST; // colors can go from 0 to DT_DEV_OVERLAY_LAST-1
...
static void _darkroom_do_synchronize_selection_callback(dt_action_t *action)
{
    ...
    for (GList *l = sel; l; l = g_list_next(l))
    {
        ...
        if (dest_imgid != imgid)
        {
            dt_history_copy_and_paste_on_image(imgid, dest_imgid, TRUE, op, TRUE, FALSE, TRUE);
        }
    }
...
static float _action_process_skip_mouse(gpointer target, const dt_action_element_t element,
...
const dt_action_def_t dt_action_def_skip_mouse = {N_("hold"), _action_process_skip_mouse,
                                                  dt_action_elements_hold, NULL, TRUE};

static float _action_process_preview(gpointer target, const dt_action_element_t element,
                                     const dt_action_effect_t effect, const float move_size)
{
    ...
    if (DT_PERFORM_ACTION(move_size))
    {
        if (lib->full_preview)
        {
            if (effect != DT_ACTION_EFFECT_ON)
            {
                ...
                dt_dev_zoom_move(&darktable.develop->full, DT_ZOOM_RESTORE, 0.0f, 0, -1.0f, -1.0f,
                                 TRUE);
        ...
        else
        {
                ...
                lib->preview_pipe->status != DT_DEV_PIXELPIPE_INVALID)
            {
                ...
                if (gui_module)
                {
                    ...
                    if (bd)
                        lib->full_preview_masks_state = bd->masks_shown;
                }
                // we set the zoom values to "fit" after storing previous settings
                dt_dev_zoom_move(&darktable.develop->full, DT_ZOOM_FULL_PREVIEW, 0.0f, 0, -1.0f,
                                 -1.0f, TRUE);
...
const dt_action_def_t dt_action_def_preview = {N_("preview"), _action_process_preview,
                                               dt_action_elements_hold, NULL, TRUE};
                                  ...
                                  const dt_action_effect_t effect, const float move_size)
{
    ...
    if (DT_PERFORM_ACTION(move_size))
    {
        ...
        dt_dev_zoom_move(&dev->full, DT_ZOOM_MOVE, factor, 0, target ? dev->full.width : 0,
...
static void _enable_focused_module(void)
{
    ...
    if (!module)
        ...
        dt_toast_log(_("'%s' cannot be enabled or disabled"),
...
static void _new_instance_focused_module(void)
{
    ...
    else
    {
        ...
        if (new_module)
        {
            ...
            if (strlen(instance_name) > 0)
                dt_toast_log(_("added instance '%s' of '%s'"), instance_name,
                             dt_iop_get_localized_name(new_module->op));
            else
                dt_toast_log(_("added instance of '%s'"),
                             dt_iop_get_localized_name(new_module->op));
        }
    }
}
...
static void _cycle_instances(const gboolean down)
{
    ...
    const gchar *instance_name = dt_iop_get_instance_name(module_to_focus);
    if (strlen(instance_name) > 0)
        dt_toast_log(_("focused instance '%s' of '%s'"), instance_name,
...
static float _action_callback_cycle_modules(gpointer widget, dt_action_element_t element,
                                            const dt_action_effect_t effect, const float move_size)
{
    if (DT_PERFORM_ACTION(move_size))
    {
...
void gui_init(dt_view_t *self)
{
   ...
   */
    ...
    dt_action_define(sa, NULL, N_("quick access to presets"), favorite_presets,
    ...
    g_signal_connect(G_OBJECT(styles), "clicked", G_CALLBACK(_darkroom_ui_apply_style_popupmenu),
                     NULL);
    ...
    dt_view_manager_view_toolbox_add(darktable.view_manager, dev->second_wnd_button,
                                     DT_VIEW_DARKROOM);
    ...
    dt_action_register(&darktable.control->actions_focus, N_("enable"), _action_enable_focused, 0,
                       0);
    dt_action_register(&darktable.control->actions_focus, N_("show"), _action_show_focused, 0, 0);
    dt_action_register(&darktable.control->actions_focus, N_("new instance"),
                       _action_new_instance_focused, 0, 0);
    dt_action_register(&darktable.control->actions_focus, N_("delete instance"),
                       _action_delete_instance_focused, 0, 0);

    /* Enable color assessment conditions */
    {
        dev->color_assessment.button = dtgtk_togglebutton_new(dtgtk_cairo_paint_bulb, 0, NULL);
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(dev->color_assessment.button),
                                     dev->full.color_assessment);
        ac = dt_action_define(DT_ACTION(self), NULL, N_("color assessment"),
                              dev->color_assessment.button, &dt_action_def_toggle);
        gtk_widget_set_tooltip_text(
            dev->color_assessment.button,
        ...
        dt_view_manager_module_toolbox_add(darktable.view_manager, dev->color_assessment.button,
                                           DT_VIEW_DARKROOM);
        /* add pop-up window */
        dev->color_assessment.floating_window = gtk_popover_new(dev->color_assessment.button);
        connect_button_press_release(dev->color_assessment.button,
        ...
        GtkWidget *border_width_slider =
            dt_bauhaus_slider_new_action(DT_ACTION(self), 0.05, 0.4, 0.05, 0.2, 2);
        dt_bauhaus_slider_set(border_width_slider,
                              dt_conf_get_float("darkroom/ui/color_assessment_total_border_width"));
        dt_bauhaus_slider_set_format(border_width_slider, "%");
        dt_bauhaus_widget_set_label(border_width_slider, N_("color_assessment"),
                                    N_("total border width relative to screen"));
        gtk_widget_set_tooltip_text(
            border_width_slider,
        ...
        /* white border ratio */
        GtkWidget *border_ratio_slider =
            dt_bauhaus_slider_new_action(DT_ACTION(self), 0.1, 0.95, 0.05, 0.4, 2);
        ...
        dt_bauhaus_widget_set_label(border_ratio_slider, N_("color_assessment"),
                                    N_("white border ratio"));
        gtk_widget_set_tooltip_text(
            border_ratio_slider,
    ...
    ac = dt_action_define(sa, NULL, N_("high quality processing"), dev->late_scaling.button,
    ...
    dt_view_manager_module_toolbox_add(darktable.view_manager, dev->late_scaling.button,
                                       DT_VIEW_DARKROOM);
    ...
    /* create rawoverexposed popup tool */
    {
        ...
        dev->rawoverexposed.button =
            dtgtk_togglebutton_new(dtgtk_cairo_paint_rawoverexposed, 0, NULL);
        ac = dt_action_define(sa, N_("raw overexposed"), N_("toggle"), dev->rawoverexposed.button,
                              &dt_action_def_toggle);
        ...
        dt_view_manager_module_toolbox_add(darktable.view_manager, dev->rawoverexposed.button,
                                           DT_VIEW_DARKROOM);
        ...
        DT_BAUHAUS_COMBOBOX_NEW_FULL(
            colorscheme, self, N_("raw overexposed"), N_("color scheme"),
            _("select the solid color to indicate overexposure.\nwill only be used if mode = mark with solid color"),
            dev->rawoverexposed.colorscheme, _rawoverexposed_colorscheme_callback, dev,
            NC_("solidcolor", "red"), NC_("solidcolor", "green"), NC_("solidcolor", "blue"),
            NC_("solidcolor", "black"));
        gtk_box_pack_start(GTK_BOX(vbox), GTK_WIDGET(colorscheme), TRUE, TRUE, 0);

        /* threshold */
        GtkWidget *threshold =
            dt_bauhaus_slider_new_action(DT_ACTION(self), 0.0, 2.0, 0.01, 1.0, 3);
    ...
    /* create overexposed popup tool */
    {
        ...
        ac = dt_action_define(DT_ACTION(self), N_("overexposed"), N_("toggle"),
                              dev->overexposed.button, &dt_action_def_toggle);
        ...
        /* preview mode */
        DT_BAUHAUS_COMBOBOX_NEW_FULL(
            mode, self, N_("overexposed"), N_("clipping preview mode"),
            _("select the metric you want to preview\nfull gamut is the combination of all other modes"),
            dev->overexposed.mode, _mode_callback, dev, N_("full gamut"), N_("any RGB channel"),
        ...
        gtk_widget_set_tooltip_text(lower,
                                    _("clipping threshold for the black point,\n"
    ...
    /* create profile popup tool & buttons (softproof + gamut) */
    {
        ...
        dev->profile.softproof_button =
            dtgtk_togglebutton_new(dtgtk_cairo_paint_softproof, 0, NULL);
        ac = dt_action_define(sa, NULL, N_("softproof"), dev->profile.softproof_button,
                              &dt_action_def_toggle);
        ...
        dt_view_manager_module_toolbox_add(darktable.view_manager, dev->profile.softproof_button,
                                           DT_VIEW_DARKROOM);
        ...
        ac = dt_action_define(sa, NULL, N_("gamut check"), dev->profile.gamut_button,
                              &dt_action_def_toggle);
        ...
        dt_view_manager_module_toolbox_add(darktable.view_manager, dev->profile.gamut_button,
                                           DT_VIEW_DARKROOM);
        ...
        dev->profile.display_intent_widget =
            dt_bauhaus_combobox_new_full(DT_ACTION(self), N_("profiles"), N_("intent"), "", 0,
                                         _display_intent_callback, dev, intents_list);
        dev->profile.display2_intent_widget =
            dt_bauhaus_combobox_new_full(DT_ACTION(self), N_("profiles"), N_("preview intent"), "",
        ...
        dt_bauhaus_widget_set_label(display2_profile, N_("profiles"),
                                    N_("preview display profile"));
        ...
        GtkWidget *display2_color_assessment =
            gtk_check_button_new_with_label(_("second preview window color assessment"));
        ...
        tooltip =
            dt_ioppr_get_location_tooltip("out", _("histogram and color picker ICC profiles"));
        ...
        DT_CONTROL_SIGNAL_CONNECT(DT_SIGNAL_CONTROL_PROFILE_USER_CHANGED, _display_profile_changed,
                                  display_profile);
        DT_CONTROL_SIGNAL_CONNECT(DT_SIGNAL_CONTROL_PROFILE_USER_CHANGED, _display2_profile_changed,
                                  display2_profile);

        GtkWidget *vbox = dt_gui_vbox(
            display_profile, dev->profile.display_intent_widget,
            gtk_separator_new(GTK_ORIENTATION_HORIZONTAL), display2_profile,
            dev->profile.display2_intent_widget, display2_color_assessment,
    ...
    ac = dt_action_define(sa, NULL, N_("force pan/zoom/rotate with mouse"), NULL,
                          &dt_action_def_skip_mouse);
    ...
    dt_action_register(DT_ACTION(self), N_("cycle overlay colors"), _overlay_cycle_callback,
                       GDK_KEY_o, GDK_CONTROL_MASK);

    // toggle visibility of drawn masks for current gui module
    dt_action_register(DT_ACTION(self), N_("show drawn masks"), _toggle_mask_visibility_callback, 0,
                       0);
    ...
    // brush hardness +/-
    dt_action_register(DT_ACTION(self), N_("increase brush hardness"), _brush_hardness_up_callback,
                       GDK_KEY_braceright, 0);
    dt_action_register(DT_ACTION(self), N_("decrease brush hardness"),
                       _brush_hardness_down_callback, GDK_KEY_braceleft, 0);

    // brush opacity +/-
    dt_action_register(DT_ACTION(self), N_("increase brush opacity"), _brush_opacity_up_callback,
                       GDK_KEY_greater, 0);
    dt_action_register(DT_ACTION(self), N_("decrease brush opacity"), _brush_opacity_down_callback,
                       GDK_KEY_less, 0);

    // undo/redo
    dt_action_register(DT_ACTION(self), N_("undo"), _darkroom_undo_callback, GDK_KEY_z,
                       GDK_CONTROL_MASK);
    dt_action_register(DT_ACTION(self), N_("redo"), _darkroom_redo_callback, GDK_KEY_y,
                       GDK_CONTROL_MASK);
    ...
    dt_action_register(DT_ACTION(self), N_("synchronize selection"),
                       _darkroom_do_synchronize_selection_callback, GDK_KEY_x, GDK_CONTROL_MASK);
}
...
void enter(dt_view_t *self)
{
    ...
    DT_CONTROL_SIGNAL_HANDLE(DT_SIGNAL_TROUBLE_MESSAGE, _display_module_trouble_message_callback);
    ...
    dt_ui_thumbtable(darktable.gui->ui)->mouse_inside =
        FALSE; // consider mouse outside filmstrip by default
   ...
   */
    ...
    if (sw)
        gtk_scrolled_window_set_propagate_natural_width(sw, FALSE);
    ...
    dt_thumbtable_set_offset_image(dt_ui_thumbtable(darktable.gui->ui), dev->image_storage.id,
                                   TRUE);
...
void leave(dt_view_t *self)
{
    ...
    while (dev->iop)
    {
        ...
        if (module->request_mask_display || module->suppress_mask)
    ...
    dt_collection_update_query(
        darktable.collection, DT_COLLECTION_CHANGE_RELOAD, DT_COLLECTION_PROP_UNDEF,
...
void mouse_moved(dt_view_t *self, const double x, const double y, const double pressure,
                 const int which)
{
    dt_develop_t *dev = self->data;
    ...
    if (!darktable.develop->darkroom_skip_mouse_events && dt_iop_color_picker_is_visible(dev) &&
    ...
    // masks
    if (dev->form_visible && !handled && !darktable.develop->darkroom_skip_mouse_events &&
    ...
    // module
    if (dev->gui_module && dev->gui_module->mouse_moved && !handled &&
        !darktable.develop->darkroom_skip_mouse_events && !dt_iop_color_picker_is_visible(dev) &&
        dt_dev_modulegroups_test_activated(darktable.develop))
    {
        _get_zoom_pos(&dev->full, x, y, &zoom_x, &zoom_y, &zoom_scale);
        handled = dev->gui_module->mouse_moved(dev->gui_module, zoom_x, zoom_y, pressure, which,
                                               zoom_scale);
    }
    ...
    {
        if (!handled)
            dt_dev_zoom_move(&dev->full, DT_ZOOM_MOVE, -1.f, 0, x - ctl->button_x,
                             y - ctl->button_y, TRUE);
        else
        {
    ...
    else if (darktable.control->button_down && !handled &&
             darktable.control->button_down_which == GDK_BUTTON_SECONDARY && dev->proxy.rotate)
    {
        _get_zoom_pos(&dev->full, x, y, &zoom_x, &zoom_y, &zoom_scale);
        dev->proxy.rotate->mouse_moved(dev->proxy.rotate, zoom_x, zoom_y, pressure, which,
                                       zoom_scale);
    }
}

int button_released(dt_view_t *self, const double x, const double y, const int which,
                    const uint32_t state)
{
    ...
    {
        _get_zoom_pos(&dev->full, x, y, &zoom_x, &zoom_y, &zoom_scale);
        handled = dev->proxy.rotate->button_released(dev->proxy.rotate, zoom_x, zoom_y, which,
                                                     state, zoom_scale);
        if (handled)
            return handled;
    }
    ...
    {
        _get_zoom_pos(&dev->full, x, y, &zoom_x, &zoom_y, &zoom_scale);
        handled = dev->gui_module->button_released(dev->gui_module, zoom_x, zoom_y, which, state,
                                                   zoom_scale);
        if (handled)
            return handled;
    }
...
int button_pressed(dt_view_t *self, double x, double y, double pressure, const int which,
                   const int type, const uint32_t state)
{
        ...
        if (which == GDK_BUTTON_PRIMARY)
        {
            ...
            if (sample->size == DT_COLOR_PICKER_SIZE_BOX)
            {
                ...
                else
                {
                    ...
                    const float dy = dx * (float)dev->full.pipe->processed_width /
                                     (float)dev->full.pipe->processed_height;
                    const dt_boundingbox_t fbox = {zoom_x - dx, zoom_y - dy, zoom_x + dx,
                                                   zoom_y + dy};
    ...
    {
        _get_zoom_pos(&dev->full, x, y, &zoom_x, &zoom_y, &zoom_scale);
        handled = dev->gui_module->button_pressed(dev->gui_module, zoom_x, zoom_y, pressure, which,
                                                  type, state, zoom_scale);
        if (handled)
            return handled;
    }
    ...
    {
        _get_zoom_pos(&dev->full, x, y, &zoom_x, &zoom_y, &zoom_scale);
        return dev->proxy.rotate->button_pressed(dev->proxy.rotate, zoom_x, zoom_y, pressure, which,
                                                 type, state, zoom_scale);
    }
    return 0;
}
...
void scrolled(dt_view_t *self, const double x, const double y, const int up, const int state)
{
    dt_develop_t *dev = self->data;
    ...
    // masks
    if (dev->form_visible && !darktable.develop->darkroom_skip_mouse_events)
    {
        _get_zoom_pos(&dev->full, x, y, &zoom_x, &zoom_y, &zoom_scale);
        float corr_x, corr_y;
        _preview_pipe_zoom_correction(dev, &corr_x, &corr_y);
    ...
    const gboolean constrained = dev->constrain_zoom && !dt_modifier_is(state, GDK_CONTROL_MASK);
                     ...
                     const double dy, const int state)
{
    ...
    if (dev->form_visible && !darktable.develop->darkroom_skip_mouse_events)
    {
        dt_print(DT_DEBUG_INPUT, "[darkroom pan] ignored: mask form active");
        return FALSE;
    }
    ...
    if (dev->gui_module && dev->gui_module->scrolled &&
        ...
        dt_dev_modulegroups_test_activated(darktable.develop))
    {
        dt_print(DT_DEBUG_INPUT, "[darkroom pan] ignored: active module '%s' consumes scroll",
                 dev->gui_module->name());
        return FALSE;
    }
    ...
    dt_print(DT_DEBUG_INPUT, "[darkroom pan] x=%.1f y=%.1f dx=%.3f dy=%.3f state=0x%x", x, y, dx,
             dy, state);
                       ...
                       const double dy, const int phase, const double scale, const int state)
{
    ...
    else if (phase == GDK_TOUCHPAD_GESTURE_PHASE_END || phase == GDK_TOUCHPAD_GESTURE_PHASE_CANCEL)
    {
        dt_print(DT_DEBUG_INPUT, "[darkroom pinch] %s x=%.1f y=%.1f scale=%.6f state=0x%x",
                 phase == GDK_TOUCHPAD_GESTURE_PHASE_END ? "end" : "cancel", x, y, scale, state);
    ...
    {
        dt_print(DT_DEBUG_INPUT, "[darkroom pinch] unknown phase=%d ignored", phase);
        return FALSE;
    }
    if (pinch_begin_tscale <= 0.0f || scale <= 0.0)
    {
        dt_print(DT_DEBUG_INPUT, "[darkroom pinch] update skipped: begin_tscale=%.6f scale=%.6f",
    ...
    zoom_pan_score = zoom_pan_score * PINCH_ZOOM_PAN_DECAY + (zoom_eq_px - pan_eq_px);
    ...
    dt_print(DT_DEBUG_INPUT,
             ...
             x, y, x_local, y_local, ox, oy, dev->full.border_size, dev->full.width,
             dev->full.height, dx, dy, eff_dx, eff_dy, zoom_pan_score, zoom_dominant, scale, state,
...
GSList *mouse_actions(const dt_view_t *self)
{
    ...
    lm = dt_mouse_action_create_simple(lm, DT_MOUSE_ACTION_SCROLL, GDK_CONTROL_MASK,
                                       _("unbounded zoom in the image"));
    lm = dt_mouse_action_create_simple(lm, DT_MOUSE_ACTION_MIDDLE, 0,
                                       _("zoom to 100% 200% and back"));
...
#ifdef GDK_WINDOWING_QUARTZ
    dev->preview2.dpi_factor = dev->preview2.dpi / 72; // macOS has a fixed DPI of 72
#else
    dev->preview2.dpi_factor =
        dev->preview2.dpi /
        96; // according to man xrandr and the docs of gdk_screen_set_resolution 96 is the default
#endif
}

static gboolean _second_window_draw_callback(GtkWidget *widget, cairo_t *cri, dt_develop_t *dev)
{
    // Set background
    dt_gui_gtk_set_source_rgb(cri, DT_GUI_COLOR_DARKROOM_BG);
    cairo_paint(cri);
    ...
    if (port->pipe && port->pipe->backbuf) // do we have a preview image?
    {
        ...
        _view_paint_surface(cri, dev->preview2.orig_width, dev->preview2.orig_height, port,
                            DT_WINDOW_SECOND);
    }
    else if (pinned_dev)
    {
        ...
        if (pinned_dev->image_storage.id == dev->image_storage.id && dev->preview2.pipe &&
            dev->preview2.pipe->backbuf)
        {
    ...
    if (pinned_dev && !pinned_dev->gui_leaving)
    {
        ...
        if (!port->pipe->backbuf || port->pipe->status == DT_DEV_PIXELPIPE_DIRTY ||
                                                  ...
                                                  dt_develop_t *dev)
{
    ...
    gboolean size_changed =
        (dev->preview2.orig_width != event->width || dev->preview2.orig_height != event->height);
...
static void _preview2_pin_button_clicked(GtkToggleButton *button, dt_develop_t *dev)
{
    dt_dev_toggle_preview2_pinned(dev);
}

static void _darkroom_ui_second_window_init(GtkWidget *overlay, dt_develop_t *dev)
{
    ...
    g_signal_connect(G_OBJECT(pin_button), "toggled", G_CALLBACK(_preview2_pin_button_clicked),
                     dev);
...
static void _darkroom_ui_second_window_write_config(GtkWidget *widget)
{
    ...
    dt_conf_set_bool(
        "second_window/maximized",
        (gdk_window_get_state(gtk_widget_get_window(widget)) & GDK_WINDOW_STATE_MAXIMIZED));
    dt_conf_set_bool(
        "second_window/fullscreen",
...
static gboolean _second_window_delete_callback(GtkWidget *widget, GdkEvent *event,
                                               dt_develop_t *dev)
{
...
static void _second_window_dnd_received(GtkWidget *widget, GdkDragContext *context, const gint x,
                                        const gint y, GtkSelectionData *selection_data,
                                        const guint target_type, const guint time,
                                        gpointer user_data)
{
    dt_develop_t *dev = (dt_develop_t *)user_data;
    ...
    if (selection_data != NULL && target_type == DND_TARGET_IMGID)
    {
        ...
        if (imgs_nb)
        {
            const dt_imgid_t *imgs =
                (const dt_imgid_t *)gtk_selection_data_get_data(selection_data);
...
static void _darkroom_display_second_window(dt_develop_t *dev)
{
    ...
    if (dev->second_wnd == NULL)
    {
        ...
        gtk_drag_dest_set(dev->preview2.widget, GTK_DEST_DEFAULT_ALL, target_list_internal,
                          n_targets_internal, GDK_ACTION_COPY | GDK_ACTION_MOVE);
        ...
        g_signal_connect(G_OBJECT(dev->second_wnd), "event", G_CALLBACK(dt_shortcut_dispatcher),
                         NULL);
