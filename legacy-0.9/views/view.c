...
void dt_view_manager_init(dt_view_manager_t *vm)
{
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "DELETE FROM main.selected_images WHERE imgid = ?1", -1,
                                &vm->statements.delete_from_selected, NULL);
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "INSERT OR IGNORE INTO main.selected_images (imgid)"
                                " VALUES (?1)",
                                -1, &vm->statements.make_selected, NULL);
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "SELECT num FROM main.history WHERE imgid = ?1", -1,
                                &vm->statements.have_history, NULL);
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "SELECT color FROM main.color_labels WHERE imgid=?1", -1,
                                &vm->statements.get_color, NULL);
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "SELECT id"
                                " FROM main.images"
...
{
    vm->views =
        dt_module_load_modules("/views", sizeof(dt_view_t), dt_view_load_module, NULL, sort_views);
}
...
#include "views/view_api.h"
    ...
    {
        module->actions =
            (dt_action_t){DT_ACTION_TYPE_VIEW, module->module_name, module->name(module)};
...
static void _show_hide_toolbox_widget(GtkWidget *widget, gpointer data)
{
    gtk_widget_set_no_show_all(widget, TRUE);
    gtk_widget_set_visible(widget, GPOINTER_TO_INT(g_object_get_data(G_OBJECT(widget), "views")) &
...
gboolean dt_view_manager_switch_by_view(dt_view_manager_t *vm, const dt_view_t *nv)
{
    ...
    if (old_view && new_view && dt_check_gimpmode("file") &&
    ...
    if (new_view->try_enter)
    {
        ...
        if (error)
        {
            DT_CONTROL_SIGNAL_RAISE(DT_SIGNAL_VIEWMANAGER_VIEW_CANNOT_CHANGE, old_view, new_view);
            return error;
        }
    }
    ...
    if (old_view)
    {
        ...
        for (GList *iter = darktable.lib->plugins; iter; iter = g_list_next(iter))
        {
            ...
            if (new_view == old_view && !plugin->expandable(plugin))
                continue;
          ...
          */
                if (plugin->widget)
                    gtk_container_remove(GTK_CONTAINER(gtk_widget_get_parent(plugin->widget)),
                                         plugin->widget);
                if (plugin->expander)
    ...
    if (new_view != old_view) // implement preference toggle only on view change
        dt_ui_container_swap_left_right(darktable.gui->ui,
                                        view_type == DT_VIEW_DARKROOM &&
   ...
   * bottom! */
    ...
    for (GList *iter = g_list_last(darktable.lib->plugins); iter; iter = g_list_previous(iter))
    {
        ...
        if (plugin->expandable(plugin))
        {
            ...
            char var[1024];
            snprintf(var, sizeof(var), "plugins/%s/%s/expanded", new_view->module_name,
                     plugin->plugin_name);
        ...
        {
            gtk_container_foreach(GTK_CONTAINER(w), _show_hide_toolbox_widget,
...
void dt_view_manager_expose(dt_view_manager_t *vm, cairo_t *cr, const int32_t width,
                            const int32_t height, const int32_t pointerx, const int32_t pointery)
{
    if (!vm->current_view)
    {
    ...
    if (vm->current_view->expose)
    {
        ...
        vm->current_view->expose(vm->current_view, cr, vm->current_view->width,
                                 vm->current_view->height, px, py);

        cairo_restore(cr);
        /* expose plugins */
        for (const GList *plugins = g_list_last(darktable.lib->plugins); plugins;
...
void dt_view_manager_mouse_moved(dt_view_manager_t *vm, const double x, const double y,
...
int dt_view_manager_button_released(dt_view_manager_t *vm, const double x, const double y,
...
int dt_view_manager_button_pressed(dt_view_manager_t *vm, const double x, const double y,
                                   ...
                                   const uint32_t state)
{
    ...
    /* lets check if any plugins want to handle button press */
    gboolean handled = FALSE;
    for (const GList *plugins = g_list_last(darktable.lib->plugins); plugins && !handled;
         plugins = g_list_previous(plugins))
    {
        dt_lib_module_t *plugin = plugins->data;
...
void dt_view_manager_configure(dt_view_manager_t *vm, const int width, const int height)
{
    for (GList *iter = vm->views; iter; iter = g_list_next(iter))
    {
...
void dt_view_manager_scrolled(dt_view_manager_t *vm, const double x, const double y, const int up,
                              const int state)
{
    if (!vm->current_view)
        return;
...
gboolean dt_view_manager_gesture_pan(dt_view_manager_t *vm, const double x, const double y,
                                     const double dx, const double dy, const int state)
{
    if (!vm->current_view)
    {
        return FALSE;
...
gboolean dt_view_manager_gesture_pinch(dt_view_manager_t *vm, const double x, const double y,
                                       const double dx, const double dy, const int phase,
                                       const double scale, const int state)
{
    if (!vm->current_view)
    {
        return FALSE;
...
void dt_view_manager_scrollbar_changed(dt_view_manager_t *vm, const double x, const double y)
{
    if (!vm->current_view)
        return;
...
void dt_view_set_scrollbar(dt_view_t *view, const float hpos, const float hlower, const float hsize,
                           const float hwinsize, const float vpos, const float vlower,
                           const float vsize, const float vwinsize)
{
...
dt_view_surface_value_t dt_view_image_get_surface(const dt_imgid_t imgid, const int32_t width,
                                                  const int32_t height, cairo_surface_t **surface,
...
dt_view_image_get_surface_cached(const dt_imgid_t imgid, const int32_t width, const int32_t height,
                                 cairo_surface_t **surface, const gboolean quality,
                                 cairo_surface_t **mip_cache, dt_mipmap_size_t *mip_cache_level)
{
    ...
    *surface = NULL;
        ...
        cairo_surface_status(*mip_cache) == CAIRO_STATUS_SUCCESS)
    {
        ...
        {
            float scale =
                fminf(width / (float)cached_wd, height / (float)cached_ht) * darktable.gui->ppd_thb;
            ...
            if (*surface && cairo_surface_status(*surface) == CAIRO_STATUS_SUCCESS)
            {
                ...
                if (fabsf(scale - 1.0f) < 0.01f)
                    ...
                    cairo_pattern_set_filter(
                        cairo_get_source(cr),
                        ((darktable.gui->filter_image == CAIRO_FILTER_FAST) && quality) ?
                            CAIRO_FILTER_GOOD :
    ...
    dt_print(DT_DEBUG_LIGHTTABLE,
             "dt_view_image_get_surface  id %i, dots %ix%i -> mip %ix%i, found %ix%i", imgid,
             mipwidth, mipheight, darktable.mipmap_cache->max_width[mip],
             darktable.mipmap_cache->max_height[mip], buf_wd, buf_ht);
    ...
    *surface = cairo_image_surface_create(CAIRO_FORMAT_RGB24, img_width, img_height);
    ...
    if (rgbbuf)
    {
        ...
        if (dt_conf_get_bool("cache_color_managed"))
        {
            ...
            else
            {
                ...
                {
                    dt_print(DT_DEBUG_ALWAYS, "oops, there seems to be a code path not setting the"
        ...
        tmp_surface =
            cairo_image_surface_create_for_data(rgbbuf, CAIRO_FORMAT_RGB24, buf_wd, buf_ht, stride);
    }
    ...
    if (tmp_surface)
    {
        ...
        if ((buf_wd <= 30 && buf_ht <= 30) || fabsf(scale - 1.0f) < 0.01f)
            cairo_pattern_set_filter(cairo_get_source(cr), CAIRO_FILTER_NEAREST);
    ...
    */
        ...
        if (mip_cache && mip == buf.size && buf_wd > 30)
        {
            ...
            *mip_cache = cairo_image_surface_create(CAIRO_FORMAT_RGB24, buf_wd, buf_ht);
            if (*mip_cache && cairo_surface_status(*mip_cache) == CAIRO_STATUS_SUCCESS)
            {
                ...
                // live at this point.
                const int32_t cache_stride =
                    cairo_format_stride_for_width(CAIRO_FORMAT_RGB24, buf_wd);
    ...
    if (darktable.unmuted & DT_DEBUG_PERF)
        dt_print(DT_DEBUG_LIGHTTABLE | DT_DEBUG_PERF, "got surface  %ix%i created in %0.04f sec",
                 img_width, img_height, dt_get_wtime() - tt);

    // we consider skull as ok as the image hasn't to be reload
    if (ret != DT_VIEW_SURFACE_OK)
        dt_print(DT_DEBUG_LIGHTTABLE,
                 "dt_view_image_get_surface  ID=%i with surface problem %s%s%s", imgid,
 ...
 */
void dt_view_set_selection(const dt_imgid_t imgid, const int value)
{
    ...
    if (sqlite3_step(darktable.view_manager->statements.is_selected) == SQLITE_ROW)
    {
        if (!value)
        {
            ...
            DT_DEBUG_SQLITE3_BIND_INT(darktable.view_manager->statements.delete_from_selected, 1,
                                      imgid);
...
{
    if (vm->proxy.lighttable.module && vm->proxy.lighttable.culling_preview_refresh)
...
{
    if (vm->proxy.lighttable.module && vm->proxy.lighttable.culling_preview_reload_overlays)
...
dt_view_lighttable_culling_initial_restriction(dt_view_manager_t *vm)
{
    if (vm->proxy.lighttable.module)
...
dt_lighttable_culling_restriction_t
dt_view_lighttable_culling_restricted_state(dt_view_manager_t *vm)
{
...
void dt_view_lighttable_set_culling_restricted_state(
    dt_view_manager_t *vm, const dt_lighttable_culling_restriction_t state)
{
    if (vm->proxy.lighttable.view)
...
void dt_view_lighttable_change_offset(dt_view_manager_t *vm, const gboolean reset,
...
static void _accels_window_sticky(GtkWidget *widget, dt_view_manager_t *vm)
{
    if (!vm->accels_window.window)
        return;
...
void dt_view_accels_refresh(dt_view_manager_t *vm)
{
    ...
    dt_action_t mouse_actions = {.label = _("mouse actions"), .next = first_category};
    if (cv->mouse_actions)
    {
    ...
    for (dt_action_t *category = first_category; category; category = category->next)
    {
        ...
        if (model)
        {
            ...
            column =
                gtk_tree_view_column_new_with_attributes(_("action"), renderer, "text", 1, NULL);
            gtk_tree_view_append_column(GTK_TREE_VIEW(list), column);
...
static void _audio_child_watch(const GPid pid, const gint status, gpointer data)
{
...
void dt_view_audio_start(dt_view_manager_t *vm, const dt_imgid_t imgid)
{
    ...
    if (player && *player)
    {
        ...
        if (filename)
        {
            ...
            if (ret)
            {
                ...
                vm->audio.audio_player_event_source = g_child_watch_add(
                    vm->audio.audio_player_pid, (GChildWatchFunc)_audio_child_watch, vm);
            }
            else
                vm->audio.audio_player_id = -1;
...
static void _match_rotation(cairo_t *cr, dt_dev_zoom_pos_t d, double dx, double dy, const double cx,
                            const double cy)
{
...
void dt_view_paint_surface(cairo_t *cr, const size_t width, const size_t height,
                           dt_dev_viewport_t *port, const dt_window_t window, uint8_t *buf,
                           float buf_scale, int buf_width, int buf_height,
                           dt_dev_zoom_pos_t buf_zoom_pos)
{
    ...
    dt_print_pipe(DT_DEBUG_EXPOSE, "dt_view_paint_surface", port->pipe, NULL, DT_DEVICE_NONE, NULL,
                  NULL,
    ...
    const double back_scale =
        (buf_scale == 0 ? 1.0 : backbuf_scale / buf_scale) * (1 << closeup) / ppd;
    ...
    if (use_preview_fallback)
    {
        dt_print_pipe(DT_DEBUG_PIPE | DT_DEBUG_EXPOSE | DT_DEBUG_VERBOSE,
                      "dt_view_paint_surface sets DT_DEV_PIPE_ZOOMED", port->pipe, NULL,
        ...
        cairo_surface_t *preview =
            dt_view_create_surface(pp->backbuf, pp->backbuf_width, pp->backbuf_height);
        ...
        dt_print_pipe(DT_DEBUG_EXPOSE, "  painting", pp, NULL, DT_DEVICE_NONE, NULL, NULL,
                      "size %4lux%-4lu processed %4.0fx%-4.0f "
                      "buf %4dx%-4d scale=%.3f "
                      "zoom (x=%6.2f y=%6.2f) -> offset (x=%+.3f y=%+.3f)",
                      width, height, wd, ht, pp->backbuf_width, pp->backbuf_height, zoom_scale,
                      pp->backbuf_zoom_pos[0], pp->backbuf_zoom_pos[1], preview_x, preview_y);
    }
    ...
    {
        dt_print_pipe(DT_DEBUG_EXPOSE, "  painting", port->pipe, NULL, DT_DEVICE_NONE, NULL, NULL,
                      "size %4lux%-4lu processed %4dx%-4d "
                      "buf %4dx%-4d scale=%.3f "
                      "zoom (x=%6.2f y=%6.2f) -> offset (x=%+.3f y=%+.3f)",
                      width, height, processed_width, processed_height, buf_width, buf_height,
                      buf_scale, buf_zoom_pos[0], buf_zoom_pos[1], offset_x, offset_y);
...
cairo_surface_t *dt_view_create_surface(uint8_t *buffer, const size_t processed_width,
                                        const size_t processed_height)
{
    const int32_t stride = cairo_format_stride_for_width(CAIRO_FORMAT_RGB24, processed_width);
    return cairo_image_surface_create_for_data(buffer, CAIRO_FORMAT_RGB24, processed_width,
                                               processed_height, stride);
}
