...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self,
                                  _("convert any RGB input to pipeline reference RGB\n"
                                    "using color profiles to remap RGB values"),
                                  _("mandatory"), _("linear or non-linear, RGB, scene-referred"),
                                  _("defined by profile"), _("linear, RGB, scene-referred"));
}
...
static void _workicc_changed(GtkWidget *widget, dt_iop_module_t *self)
{
    ...
    if (type_work != DT_COLORSPACE_NONE)
    {
        ...
        const dt_iop_order_iccprofile_info_t *const work_profile =
            dt_ioppr_add_profile_info_to_list(self->dev, p->type_work, p->filename_work,
                                              DT_INTENT_PERCEPTUAL);
        if (work_profile == NULL || !dt_is_valid_colormatrix(work_profile->matrix_in[0][0]) ||
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    if (corrected)
    {
        ...
        dt_print_pipe(DT_DEBUG_PIPE, "coeff correction", pipe, self, devid, roi_in, roi_out,
                      "`%s' %.3f(*%.3f) %.3f(*%.3f) %.3f(*%.3f)",
                      dt_colorspaces_get_name(d->type, NULL), pipe->dsc.temperature.coeffs[0],
                      coeffs[0], pipe->dsc.temperature.coeffs[1], coeffs[1],
    ...
    if (dev_b == NULL)
        goto error;
    dev_coeffs = dt_opencl_copy_host_to_device_constant(devid, sizeof(float) * 3 * 3,
                                                        (float *)d->unbounded_coeffs);
    if (dev_coeffs == NULL)
        goto error;
    err = dt_opencl_enqueue_kernel_2d_args(
        devid, kernel, width, height, CLARG(dev_in), CLARG(dev_out), CLARG(width), CLARG(height),
        CLARG(dev_m), CLARG(dev_l), CLARG(dev_r), CLARG(dev_g), CLARG(dev_b), CLARG(blue_mapping),
        CLARG(dev_coeffs), CLARG(dev_corr));
error:
    ...
    for (int j = 0; j < npixels; j++)
    {
        ...
        for (int c = 0; c < 3; c++)
            cam[c] = (d->lut[c][0] >= 0.0f) ?
                         ((in[c] < 1.0f) ? _lerp_lut(d->lut[c], in[c]) :
...
static void _cmatrix_fastpath_simple(float *const restrict out, const float *const restrict in,
                                     size_t npixels, const dt_colormatrix_t cmatrix,
                                     const dt_aligned_pixel_t corr)
{
    ...
    {
        dt_aligned_pixel_t cam = {in[4 * k] * corr[0], in[4 * k + 1] * corr[1],
                                  in[4 * k + 2] * corr[2], 1.0f};
        dt_aligned_pixel_t res;
...
static inline void _cmatrix_fastpath_clipping(float *const restrict out,
                                              const float *const restrict in, size_t npixels,
...
static void process_cmatrix_fastpath(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                                     const void *const ivoid, void *const ovoid,
                                     const dt_iop_roi_t *const roi_in,
                                     const dt_iop_roi_t *const roi_out,
                                     const dt_aligned_pixel_t corr)
{
    ...
    for (size_t chunk = 0; chunk < nthreads; chunk++)
    {
        ...
        if (start >= npixels)
            continue; // handle case when chunksize is < 4*nthreads and last thread has no work
        size_t end = MIN(start + chunksize, npixels);
        if (clipping)
            _cmatrix_fastpath_clipping(out + 4 * start, in + 4 * start, end - start, d->nmatrix,
                                       d->lmatrix, corr);
        else
...
static void _cmatrix_proper_simple(float *const restrict out, const float *const restrict in,
                                   size_t npixels, const dt_iop_colorin_data_t *const d,
                                   const dt_colormatrix_t cmatrix, const dt_aligned_pixel_t corr)
{
    const dt_aligned_pixel_t cmatrix_row0 = {cmatrix[0][0], cmatrix[1][0], cmatrix[2][0], 0.0f};
    const dt_aligned_pixel_t cmatrix_row1 = {cmatrix[0][1], cmatrix[1][1], cmatrix[2][1], 0.0f};
    const dt_aligned_pixel_t cmatrix_row2 = {cmatrix[0][2], cmatrix[1][2], cmatrix[2][2], 0.0f};
...
static inline void
_cmatrix_proper_clipping(float *const restrict out, const float *const restrict in, size_t npixels,
                         const dt_iop_colorin_data_t *const d, const dt_colormatrix_t nmatrix,
                         const dt_colormatrix_t lmatrix, const dt_aligned_pixel_t corr)
{
    const dt_aligned_pixel_t nmatrix_row0 = {nmatrix[0][0], nmatrix[1][0], nmatrix[2][0], 0.0f};
    const dt_aligned_pixel_t nmatrix_row1 = {nmatrix[0][1], nmatrix[1][1], nmatrix[2][1], 0.0f};
    const dt_aligned_pixel_t nmatrix_row2 = {nmatrix[0][2], nmatrix[1][2], nmatrix[2][2], 0.0f};
...
static void process_cmatrix_proper(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                                   const void *const ivoid, void *const ovoid,
                                   const dt_iop_roi_t *const roi_in,
                                   const dt_iop_roi_t *const roi_out, const dt_aligned_pixel_t corr)
{
    const dt_iop_colorin_data_t *const d = piece->data;
    assert(piece->colors == 4);
    const gboolean clipping = (d->nrgb != NULL);
    ...
    for (size_t chunk = 0; chunk < nthreads; chunk++)
    {
        ...
        size_t end = MIN(start + chunksize, npixels);
        if (clipping)
            _cmatrix_proper_clipping(out + 4 * start, in + 4 * start, end - start, d, d->nmatrix,
                                     d->lmatrix, corr);
        else
...
static void process_cmatrix(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                            const void *const ivoid, void *const ovoid,
                            const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out,
                            const dt_aligned_pixel_t corr)
{
...
static void process_lcms2_proper(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                                 const void *const ivoid, void *const ovoid,
                                 const dt_iop_roi_t *const roi_in,
                                 const dt_iop_roi_t *const roi_out, const dt_aligned_pixel_t corr)
{
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    if (corrected)
    {
        ...
        dt_print_pipe(DT_DEBUG_PIPE, "coeff correction", pipe, self, DT_DEVICE_CPU, roi_in, roi_out,
                      "`%s' %.3f(*%.3f) %.3f(*%.3f) %.3f(*%.3f)",
                      dt_colorspaces_get_name(d->type, NULL), pipe->dsc.temperature.coeffs[0],
                      coeffs[0], pipe->dsc.temperature.coeffs[1], coeffs[1],
                      pipe->dsc.temperature.coeffs[2], coeffs[2]);
                   ...
                   dt_dev_pixelpipe_iop_t *piece)
{
    ...
    case DT_NORMALIZE_SRGB:
        d->nrgb =
            dt_colorspaces_get_profile(DT_COLORSPACE_SRGB, "", DT_PROFILE_DIRECTION_IN)->profile;
        break;
    case DT_NORMALIZE_ADOBE_RGB:
        d->nrgb = dt_colorspaces_get_profile(DT_COLORSPACE_ADOBERGB, "", DT_PROFILE_DIRECTION_IN)
                      ->profile;
        break;
    case DT_NORMALIZE_LINEAR_REC709_RGB:
        d->nrgb = dt_colorspaces_get_profile(DT_COLORSPACE_LIN_REC709, "", DT_PROFILE_DIRECTION_IN)
                      ->profile;
        break;
    case DT_NORMALIZE_LINEAR_REC2020_RGB:
        d->nrgb = dt_colorspaces_get_profile(DT_COLORSPACE_LIN_REC2020, "", DT_PROFILE_DIRECTION_IN)
                      ->profile;
        break;
    ...
    if (type == DT_COLORSPACE_EMBEDDED_MATRIX)
    {
        ...
        {
            d->input =
                dt_colorspaces_create_xyzimatrix_profile((float (*)[3])cimg->d65_color_matrix);
            d->clear_input = TRUE;
        }
        dt_image_cache_read_release(cimg);
    }
    ...
    */
    ...
    if (!d->input)
    {
        ...
        if (profile)
            d->input = profile->profile;
    }

    if (!d->input && type != DT_COLORSPACE_SRGB)
    {
        // use linear_rec709_rgb as fallback for missing non-sRGB profiles:
        d->input = dt_colorspaces_get_profile(DT_COLORSPACE_LIN_REC709, "", DT_PROFILE_DIRECTION_IN)
                       ->profile;
        d->clear_input = FALSE;
    }
    ...
    default:
        dt_print(DT_DEBUG_ALWAYS, "[colorin] input profile color space `%c%c%c%c' not supported",
                 (char)(input_color_space >> 24), (char)(input_color_space >> 16),
                 (char)(input_color_space >> 8), (char)(input_color_space));
    ...
    if (d->nrgb)
    {
        ...
        if (dt_colorspaces_get_matrix_from_input_profile(d->input, d->cmatrix, d->lut[0], d->lut[1],
                                                         d->lut[2], LUT_SAMPLES))
        {
            piece->process_cl_ready = FALSE;
            dt_mark_colormatrix_invalid(&d->cmatrix[0][0]);
            d->xform_cam_Lab =
                cmsCreateTransform(d->input, input_format, Lab, TYPE_LabA_FLT, p->intent, 0);
            d->xform_cam_nrgb =
                cmsCreateTransform(d->input, input_format, d->nrgb, TYPE_RGBA_FLT, p->intent, 0);
            d->xform_nrgb_Lab =
                cmsCreateTransform(d->nrgb, TYPE_RGBA_FLT, Lab, TYPE_LabA_FLT, p->intent, 0);
        }
        else
        {
    ...
    if (!d->xform_cam_Lab && !dt_is_valid_colormatrix(d->cmatrix[0][0]))
    {
        if (p->type == DT_COLORSPACE_FILE)
            dt_print(DT_DEBUG_ALWAYS,
                     "[colorin] unsupported input profile `%s' has"
                                                         ...
                                                         d->lut[2], LUT_SAMPLES))
        {
            ...
            dt_mark_colormatrix_invalid(&d->cmatrix[0][0]);
            d->xform_cam_Lab =
                cmsCreateTransform(d->input, TYPE_RGBA_FLT, Lab, TYPE_LabA_FLT, p->intent, 0);
        }
    }
...
void gui_update(dt_iop_module_t *self)
{
    ...
    {
        idx = 0;
        dt_print(DT_DEBUG_ALWAYS, "[gui colorin] could not find requested working profile `%s'!",
                 dt_colorspaces_get_name(p->type_work, p->filename_work));
    }
    ...
    {
        dt_colorspaces_color_profile_t *pp = prof->data;
        if (pp->type == p->type && (pp->type != DT_COLORSPACE_FILE ||
...
void reload_defaults(dt_iop_module_t *self)
{
    ...
    if (g)
    {
        ...
        if (color_profile == DT_COLORSPACE_EMBEDDED_ICC)
        {
            ...
            char *iccCopyr;
            const guint32 bufsize =
                cmsGetProfileInfoASCII(cmsprofile, cmsInfoCopyright, "en", "US", NULL, 0);
            ...
            char *tooltip = g_markup_printf_escaped(_("embedded ICC profile properties:\n\n"
                                                      ...
                                                      "model: <b>%s</b>\n"
                                                      "copyright: <b>%s</b>\n\n"),
                                                    iccDesc, iccMajorVersion, iccMinorVersion,
                                                    iccType, iccManuf, iccModel, iccCopyr);
...
void gui_init(dt_iop_module_t *self)
{
    ...
    g_signal_connect(G_OBJECT(g->profile_combobox), "value-changed", G_CALLBACK(_profile_changed),
                     (gpointer)self);
    g_signal_connect(G_OBJECT(g->work_combobox), "value-changed", G_CALLBACK(_workicc_changed),
                     (gpointer)self);
