...
{
    const int w2 = 2 * w;
    const float av = (p[-w2 - 1] + p[-w2] + p[-w2 + 1] + p[-w - 2] + p[-w - 1] + p[-w] + p[-w + 1] +
                      p[-w + 2] + p[-2] + p[-1] + p[0] + p[1] + p[2] + p[w - 2] + p[w - 1] + p[w] +
                      p[w + 1] + p[w + 2] + p[w2 - 1] + p[w2] + p[w2 + 1]) /
                     21.0f;
    return sqrtf(
        (sqrf(p[-w2 - 1] - av) + sqrf(p[-w2] - av) + sqrf(p[-w2 + 1] - av) + sqrf(p[-w - 2] - av) +
         sqrf(p[-w - 1] - av) + sqrf(p[-w] - av) + sqrf(p[-w + 1] - av) + sqrf(p[-w + 2] - av) +
         sqrf(p[-2] - av) + sqrf(p[-1] - av) + sqrf(p[0] - av) + sqrf(p[1] - av) + sqrf(p[2] - av) +
         sqrf(p[w - 2] - av) + sqrf(p[w - 1] - av) + sqrf(p[w] - av) + sqrf(p[w + 1] - av) +
         sqrf(p[w + 2] - av) + sqrf(p[w2 - 1] - av) + sqrf(p[w2] - av) + sqrf(p[w2 + 1] - av)) /
        21.0f);
}
...
static void _calc_plane_candidates(const float *plane, const float *refavg,
                                   dt_iop_segmentation_t *seg, const float clipval,
  ...
  for (uint32_t id = 2; id < seg->nr; id++)
  {
      ...
      if ((seg->ymax[id] - seg->ymin[id] > 2) && (seg->xmax[id] - seg->xmin[id] > 2))
      {
          ...
          for (int row = MAX(seg->border + 2, seg->ymin[id] - 2);
               row < MIN(seg->height - seg->border - 2, seg->ymax[id] + 3); row++)
          {
              for (int col = MAX(seg->border + 2, seg->xmin[id] - 2);
                   col < MIN(seg->width - seg->border - 2, seg->xmax[id] + 3); col++)
              {
          ...
          if (testref && (testweight > 1.0f - badlevel)) // We have found a reference location
          {
              float sum = 0.0f;
              float pix = 0.0f;
              const float weights[5][5] = {{1.0f, 4.0f, 6.0f, 4.0f, 1.0f},
...
static inline float _calc_refavg(const float *in, const uint8_t (*const xtrans)[6],
                                 const uint32_t filters, const int row, const int col,
                                 const dt_iop_roi_t *const roi, const dt_aligned_pixel_t correction,
                                 const gboolean linear)
{
    ...
    for_each_channel(c) mean[c] =
        (cnt[c] > 0.0f) ? cbrtf((correction[c] * mean[c]) / cnt[c]) : 0.0f;

    const dt_aligned_pixel_t croot_refavg = {0.5f * (mean[1] + mean[2]), 0.5f * (mean[0] + mean[2]),
                                             0.5f * (mean[0] + mean[1]), 0.0f};
    return (linear) ? fcube(croot_refavg[color]) : croot_refavg[color];
}

static void _initial_gradients(const size_t w, const size_t height, float *luminance,
                               float *distance, float *gradient)
{
...
static float _segment_maxdistance(float *distance, dt_iop_segmentation_t *seg, const uint32_t id)
{
...
static float _segment_correction(dt_iop_segmentation_t *seg, const uint32_t id, const int mode,
...
static void _calc_distance_ring(const int xmin, const int xmax, const int ymin, const int ymax,
                                float *gradient, float *distance, const float attenuate,
                                const float dist, dt_iop_segmentation_t *seg, const uint32_t id)
{
    DT_OMP_FOR(collapse(2))
    for (int row = ymin; row < ymax; row++)
    {
        for (int col = xmin; col < xmax; col++)
        {
            ...
            if ((dv >= dist) && (dv < dist + 1.5f) && (id == seg->data[v]))
            {
                ...
                if (cnt > 0.0f)
                    gradient[v] =
                        fminf(1.5f, (grd / cnt) * (1.0f + 1.0f / powf(distance[v], attenuate)));
            }
        }
    }
}

static void _segment_gradients(float *distance, float *gradient, float *tmp, const int mode,
                               dt_iop_segmentation_t *seg, const uint32_t id,
                               const int recovery_close)
{
    ...
    {
        _calc_distance_ring(xmin, xmax, ymin, ymax, gradient, distance, attenuate, maxdist, seg,
                            id);
        maxdist += 1.5f;
    }
    ...
    if (maxdist > 4.0f)
    {
        ...
        {
            for (size_t col = xmin, s = (size_t)row * seg->width + col,
                        d = (size_t)(row - ymin) * (xmax - xmin);
                 col < xmax; col++, s++, d++)
                tmp[d] = gradient[s];
        }
...
static void _add_poisson_noise(float *lum, dt_iop_segmentation_t *seg, const uint32_t id,
                               const float noise_level)
{
    ...
    uint32_t DT_ALIGNED_ARRAY state[4] = {splitmix32(ymin), splitmix32(xmin), splitmix32(1337),
                                          splitmix32(666)};
    ...
    for (int row = ymin; row < ymax; row++)
    {
        for (int col = xmin; col < xmax; col++)
        {
            ...
            {
                const float pnoise =
                    poisson_noise(lum[v] * noise_level, noise_level, col & 1, state);
                lum[v] += pnoise;
            }
        }
    }
...
static void _masks_extend_border(float *const mask, const int width, const int height,
                                 const int border)
{
    if (border <= 0)
        return;
    ...
    for (size_t col = 0; col < width; col++)
    {
        ...
        const float bot =
            mask[(height - border - 1) * width + MIN(width - border - 1, MAX(col, border))];
        for (size_t i = 0; i < border; i++)
        {
...
static void _process_segmentation(dt_dev_pixelpipe_iop_t *piece, const float *const input,
                                  float *const output, const dt_iop_roi_t *const roi_in,
                                  const dt_iop_roi_t *const roi_out, dt_iop_highlights_data_t *d,
                                  const int vmode, float *tmpout)
{
    ...
    const dt_aligned_pixel_t icoeffs = {piece->pipe->dsc.temperature.coeffs[0],
                                        piece->pipe->dsc.temperature.coeffs[1],
                                        piece->pipe->dsc.temperature.coeffs[2]};
    const dt_aligned_pixel_t clips = {clipval * icoeffs[0], clipval * icoeffs[1],
                                      clipval * icoeffs[2]};
    const dt_aligned_pixel_t cube_coeffs = {cbrtf(clips[0]), cbrtf(clips[1]), cbrtf(clips[2]),
                                            0.0f};
    ...
    const dt_aligned_pixel_t correction = {
        late ? (float)(chr->D65coeffs[0] / chr->as_shot[0]) : 1.0f,
        late ? (float)(chr->D65coeffs[1] / chr->as_shot[1]) : 1.0f,
        late ? (float)(chr->D65coeffs[2] / chr->as_shot[2]) : 1.0f, 1.0f};
    ...
    const int segmentation_limit = (piece->pipe->iwidth * piece->pipe->iheight) *
                                   sqrf(piece->pipe->iscale) / 4000; // 250 segments per mpix
    ...
    for (int i = 0; i < HL_SEGMENT_PLANES; i++)
        segerror |= dt_segmentation_init_struct(&isegments[i], pwidth, pheight, HL_BORDER + 1,
                                                segmentation_limit);
            ...
            if (inval > clips[color])
            {
                ...
                if ((pid > 1) && (pid < isegments[color].nr))
                {
                    ...
                    if (candidate != 0.0f)
                    {
                        ...
                        const float refavg_here = _calc_refavg(input, xtrans, filters, row, col,
                                                               roi_in, correction, FALSE);
    ...
    const gboolean do_recovery =
        (recovery_mode != DT_RECOVERY_MODE_OFF) && has_allclipped && (strength > 0.0f);
    ...
    if (do_recovery || do_masking)
    {
        ...
        for (int row = segall->border; row < pheight - segall->border; row++)
        {
            for (int col = segall->border; col < pwidth - segall->border; col++)
            {
                ...
                tmp[i] = (plane[0][i] * icoeffs[0] + plane[1][i] * icoeffs[1] +
                          plane[2][i] * icoeffs[2]) /
                         3.0f;
    ...
    if (do_recovery)
    {
        const float max_distance = dt_image_distance_transform(NULL, distance, pwidth, pheight,
                                                               1.0f, DT_DISTANCE_TRANSFORM_NONE);
        if (max_distance > 3.0f)
        {
            ...
            for (uint32_t id = 2; id < segall->nr; id++)
            {
                ...
                if (segall->val1[id] > 2.0f)
                    _segment_gradients(distance, recout, tmp, recovery_mode, segall, id,
                                       recovery_close);
            }
    ...
    for (int row = 0; row < roi_out->height; row++)
    {
        for (int col = 0; col < roi_out->width; col++)
        {
            ...
            if ((inrow >= 0) && (inrow < roi_in->height) && (incol >= 0) && (incol < roi_in->width))
            {
                ...
                if (do_masking && (inrow > 0) && (incol > 0) && (inrow < roi_in->height - 1) &&
                    (incol < roi_in->width - 1))
                {
    ...
    dt_print(
        DT_DEBUG_PERF,
        "[segmentation report %-12s] %5.1fMpix, segments: %3i red, %3i green, %3i blue, %3i all, %4i allowed",
        dt_dev_pixelpipe_type_to_str(piece->pipe->type),
        (float)(roi_in->width * roi_in->height) / 1.0e6f, isegments[0].nr - 2, isegments[1].nr - 2,
        isegments[2].nr - 2, isegments[3].nr - 2, segmentation_limit - 2);
