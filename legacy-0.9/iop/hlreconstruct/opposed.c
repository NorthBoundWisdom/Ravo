...
{
    const dt_aligned_pixel_t ins = {cbrtf(fmaxf(0.0f, in[0])), cbrtf(fmaxf(0.0f, in[1])),
                                    cbrtf(fmaxf(0.0f, in[2])), 0.0f};
    const dt_aligned_pixel_t opp = {0.5f * (ins[1] + ins[2]), 0.5f * (ins[0] + ins[2]),
...
static void _process_linear_opposed(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                                    const float *const input, float *const output,
                                    const dt_iop_roi_t *const roi_in, const gboolean quality)
{
    ...
    {
        for_three_channels(c) chrominance[c] = img_oppchroma[c];
        if (!img_oppclipped)
        {
    ...
    else
    {
      ...
      */
            ...
            if (anyclipped)
            {
                ...
                for (size_t row = 3; row < height - 3; row++)
                {
                    for (size_t col = 3; col < width - 3; col++)
                    {
                        ...
                        for_three_channels(c)
                        {
                            ...
                            if ((inval > 0.2f * clips[c]) && (inval < clips[c]) &&
                                (mask[(c + 3) * msize + _raw_to_cmap(mwidth, row, col)]))
                            {
                ...
                for_three_channels(c) chrominance[c] = (cnts[c] > 30.0f) ? sums[c] / cnts[c] : 0.0f;

                if (piece->pipe->type == DT_DEV_PIXELPIPE_FULL)
                {
                    for_three_channels(c) img_oppchroma[c] = chrominance[c];
                    img_opphash = opphash;
                    img_oppclipped = anyclipped;
                }
                dt_print_pipe(DT_DEBUG_PIPE, "opposed chroma", piece->pipe, self, DT_DEVICE_CPU,
                              roi_in, NULL, "RGB %3.4f %3.4f %3.4f%s%s", chrominance[0],
...
static float *_process_opposed(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                               const float *const input, float *const output,
                               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out,
                               const gboolean keep, const gboolean quality, const float clipval)
{
        ...
        */
                ...
                for (size_t row = 0; row < mheight; row++)
                {
                    for (size_t col = 0; col < mwidth; col++)
                    {
                        const size_t mx = row * mwidth + col;
                        const gboolean safe =
                            col >= 3 && row >= 3 && col < mwidth - 4 && row < mheight - 4;
                        mask[3 * msize + mx] = safe ? _mask_dilated(mask + mx, mwidth) : mask[mx];
                        mask[4 * msize + mx] =
                            safe ? _mask_dilated(mask + msize + mx, mwidth) : mask[mx + msize];
                        mask[5 * msize + mx] = safe ? _mask_dilated(mask + 2 * msize + mx, mwidth) :
                                                      mask[mx + 2 * msize];
                    }
                }

                const dt_aligned_pixel_t lo_clips = {0.2f * clips[0], 0.2f * clips[1],
                                                     0.2f * clips[2], 1.0f};
                ...
                for (size_t row = 0; row < roi_in->height; row++)
                {
                    for (size_t col = 0; col < roi_in->width; col++)
                    {
                        ...
                        {
                            sums[color] += inval - _calc_refavg(input, xtrans, filters, row, col,
                                                                roi_in, correction, TRUE);
                            cnts[color] += 1.0f;
                        }
                    }
                }
                for_three_channels(c) chrominance[c] =
                    (cnts[c] > 100.0f) ? sums[c] / cnts[c] : 0.0f;
            }
            ...
            dt_print_pipe(DT_DEBUG_PIPE, "opposed chroma", piece->pipe, self, DT_DEVICE_CPU, NULL,
                          NULL, "%12.7f (%d)%12.7f (%d)%12.7f (%d)%s%s", chrominance[0],
                          (int)cnts[0], chrominance[1], (int)cnts[1], chrominance[2], (int)cnts[2],
                          piece->pipe->type == DT_DEV_PIXELPIPE_FULL ? " saved" : "",
                          img_oppclipped ? "" : " unclipped");
        }
        dt_free_align(mask);
    ...
    if (tmpout)
    {
        ...
        for (size_t row = 0; row < roi_in->height; row++)
        {
            for (size_t col = 0; col < roi_in->width; col++)
            {
                ...
                {
                    const float ref =
                        _calc_refavg(input, xtrans, filters, row, col, roi_in, correction, TRUE);
        ...
        for (size_t col = 0; col < roi_out->width; col++)
        {
            ...
            if ((irow < roi_in->height) && (icol < roi_in->width))
            {
                ...
                else
                {
                    ...
                    if (oval >= clips[color])
                    {
                        const float ref = _calc_refavg(input, xtrans, filters, irow, icol, roi_in,
                                                       correction, TRUE);
...
#ifdef HAVE_OPENCL
static cl_int process_opposed_cl(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                                 ...
                                 const dt_iop_roi_t *const roi_out)
{
    ...
    dt_aligned_pixel_t clips = {clipval * icoeffs[0], clipval * icoeffs[1], clipval * icoeffs[2],
                                1.0f};
    ...
    if (!fastcopymode)
    {
        ...
        dev_correction =
            dt_opencl_copy_host_to_device_constant(devid, 4 * sizeof(float), correction);
        if (dev_correction == NULL)
            goto error;
    }
    ...
    else
    {
        ...
        err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_highlights_initmask, mwidth,
                                               mheight, CLARG(dev_in), CLARG(dev_inmask),
                                               CLARG(msize), CLARG(mwidth), CLARG(mheight),
                                               CLARG(filters), CLARG(dev_xtrans), CLARG(dev_clips));
        if (err != CL_SUCCESS)
            goto error;

        err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_highlights_dilatemask, mwidth,
                                               mheight, CLARG(dev_inmask), CLARG(dev_outmask),
                                               CLARG(msize), CLARG(mwidth), CLARG(mheight));
        if (err != CL_SUCCESS)
            goto error;
        ...
        err = dt_opencl_enqueue_kernel_1d_args(
            devid, gd->kernel_highlights_chroma, roi_in->height, CLARG(dev_in), CLARG(dev_outmask),
            CLARG(dev_accu), CLARG(roi_in->width), CLARG(roi_in->height), CLARG(msize),
            CLARG(mwidth), CLARG(filters), CLARG(dev_xtrans), CLARG(dev_clips),
        ...
        for_three_channels(c) chrominance[c] = (cnts[c] > 100.0f) ? sums[c] / cnts[c] : 0.0f;
        ...
        dt_print_pipe(DT_DEBUG_PIPE, "opposed chroma", piece->pipe, self, piece->pipe->devid, NULL,
                      NULL, "%12.7f (%d)%12.7f (%d)%12.7f (%d)%s%s", chrominance[0], (int)cnts[0],
                      chrominance[1], (int)cnts[1], chrominance[2], (int)cnts[2],
                      piece->pipe->type == DT_DEV_PIXELPIPE_FULL ? " saved" : "",
                      img_oppclipped ? "" : " unclipped");
    }
    ...
    if (dev_chrominance == NULL)
        goto error;

    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_highlights_opposed, roi_out->width, roi_out->height, CLARG(dev_in),
        CLARG(dev_out), CLARG(roi_out->width), CLARG(roi_out->height), CLARG(roi_in->width),
        CLARG(roi_in->height), CLARG(roi_out->x), CLARG(roi_out->y), CLARG(filters),
        CLARG(dev_xtrans), CLARG(dev_clips), CLARG(dev_chrominance), CLARG(dev_correction),
        CLARG(fastcopymode));
