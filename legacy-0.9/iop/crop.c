...
static gboolean _reduce_aligners(int *ialign_w, int *ialign_h)
{
    ...
    *ialign_w = align_w;
    *ialign_h = align_h;
    return align_w <= 16 && align_h <= 16 && (align_w > 1 || align_h > 1);
}
                        ...
                        const gboolean enforce_history)
{
    ...
    dt_boundingbox_t points = {g->clip_x * wd, g->clip_y * ht, (g->clip_x + g->clip_w) * wd,
                                          ...
                                          DT_DEV_TRANSFORM_DIR_FORW_EXCL, points, 2))
    {
        ...
        if (piece)
        {
            ...
            if (p->ratio_d != 0 && p->ratio_n != 0)
            {
                ...
                if (width > height)
                    height = landscape ? width / aspect : width * aspect;
                else
                    width = landscape ? height * aspect : height / aspect;
                ...
                dt_print(
                    DT_DEBUG_PIPE,
                    "[commit crop] %s %s aspect=%.2f rn=%d rd=%+d aw=%d ah=%d dw=%d dh=%d size: %.3f x %.3f --> %.3f x %.3f",
                    landscape ? "landscape" : "portrait", exact ? "exact" : "as is", aspect,
                    p->ratio_n, p->ratio_d, align_w, align_h, dw, dh, width, height,
            ...
            if (enforce_history || !feqf(p->cx, old[0], eps) || !feqf(p->cy, old[1], eps) ||
                !feqf(p->cw, old[2], eps) || !feqf(p->ch, old[3], eps))
...
static gboolean _set_max_clip(dt_iop_module_t *self)
{
    ...
    if (g->clip_max_pipe_hash == self->dev->preview_pipe->backbuf_hash)
        return TRUE;
    if (self->dev->preview_pipe->status != DT_DEV_PIXELPIPE_VALID)
        return TRUE;
...
static void _get_crop_offset(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece, float *crop_left,
                             float *crop_top)
{
                           ...
                           float *const restrict points, size_t points_count)
{
    ...
    if (crop_top <= 0.0f && crop_left <= 0.0f)
        return TRUE;
                    ...
                    const dt_iop_roi_t *roi_in)
{
    *roi_out = *roi_in;
    ...
    {
        if (odx > ody)
            height = floorf(landscape ? odx / aspect : odx * aspect);
        else
            width = floorf(landscape ? ody * aspect : ody / aspect);
    }
    ...
    dt_print_pipe(
        DT_DEBUG_PIPE | DT_DEBUG_VERBOSE, "crop aspects", piece->pipe, self, DT_DEVICE_NONE, roi_in,
        NULL,
        " %s%s%sAspect=%.3f. odx: %.1f ody: %.1f --> width: %.1f height: %.1f aligners=%d %d corr=%d %d",
        d->aspect < 0.0f ? "toggled " : "", keep_aspect ? "fixed " : "",
        landscape ? "landscape " : "portrait ", aspect, odx, ody, width, height, align_w, align_h,
        dw, dh);
}
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    return dt_opencl_enqueue_copy_image(piece->pipe->devid, dev_in, dev_out, CLIMG_ORIGIN,
                                        CLIMG_ORIGIN, region);
}
#endif
...
static float _aspect_ratio_get(dt_iop_module_t *self, GtkWidget *combo)
{
    ...
    if (text && !g_strcmp0(text, _("original image")))
    {
        ...
        const gboolean regular = (p->ratio_d > 0 && wd >= ht) || (p->ratio_d < 0 && wd < ht);
...
static gint _aspect_ratio_cmp(const dt_iop_crop_aspect_t *a, const dt_iop_crop_aspect_t *b)
{
    ...
    if (fabsf(aratio - bratio) < prec)
        return 0;

    return 1;
}

static gchar *_aspect_format(gchar *original, const int adim, const int bdim)
{
...
void gui_init(dt_iop_module_t *self)
{
        ...
        if (c < end - 1)
        {
            ...
            if (n == 0 || d == 0)
            {
                dt_print(DT_DEBUG_ALWAYS,
                         "invalid ratio format for `%s'. it should be \"number:number\"", nv->key);
                dt_control_log(_("invalid ratio format for `%s'. it should be \"number:number\""),
                               nv->key);
                continue;
            }
            dt_iop_crop_aspect_t *aspect = g_malloc(sizeof(dt_iop_crop_aspect_t));
    ...
    gtk_widget_set_tooltip_text(
        g->aspect_presets, _("set the aspect ratio\n"
                             "the list is sorted: from most square to least square\n"
                             "to enter custom aspect ratio open the combobox and type ratio in x:y"
                             " or decimal format"));
    dt_bauhaus_widget_set_quad(g->aspect_presets, self, dtgtk_cairo_paint_aspectflip, FALSE,
                               _event_aspect_flip,
    ...
    dt_gui_new_collapsible_section(&g->cs, "plugins/darkroom/crop/expand_margins", _("margins"),
    ...
    dt_shortcut_register(DT_ACTION(self->so), 0, 0, GDK_KEY_c, GDK_SHIFT_MASK);
}
...
static _grab_region_t _gui_get_grab(float pzx, float pzy, dt_iop_crop_gui_data_t *g,
                                    const float border, const float wd, const float ht)
{
    _grab_region_t grab = GRAB_NONE;
    if (!(pzx < g->clip_x || pzx > g->clip_x + g->clip_w || pzy < g->clip_y ||
                     ...
                     const float pzx, const float pzy, const float zoom_scale)
{
    ...
    if ((dev->full.pipe->changed & DT_DEV_PIPE_REMOVE || self->dev->preview_pipe->loading) &&
        !external)
        return;
    ...
    if (_set_max_clip(self) && !dimmed)
    {
        ...
        cairo_rectangle(cr, g->clip_max_x * wd, g->clip_max_y * ht, g->clip_max_w * wd,
                        g->clip_max_h * ht);
        ...
        darktable.control->button_down_which == GDK_BUTTON_PRIMARY)
    {
        ...
        dt_print(DT_DEBUG_EXPOSE | DT_DEBUG_VERBOSE,
                 "[crop expose] proc %d x %d, %d x %d --> %d x %d, diff= %d %d", procw, proch,
                 width, height, width - diff_w, height - diff_h, diff_w, diff_h);
        ...
        snprintf(dimensions, sizeof(dimensions), "%d x %d %s", width - diff_w, height - diff_h,
                 ratio_str);
    ...
    const _grab_region_t grab =
        g->cropping ? g->cropping : _gui_get_grab(pzx, pzy, g, border, wd, ht);
    ...
    if (grab == GRAB_RIGHT)
        cairo_rectangle(cr, (g->clip_x + g->clip_w) * wd - border, g->clip_y * ht, border,
                        g->clip_h * ht);
    if (grab == GRAB_BOTTOM)
        cairo_rectangle(cr, g->clip_x * wd, (g->clip_y + g->clip_h) * ht - border, g->clip_w * wd,
                        border);
    if (grab == GRAB_BOTTOM_RIGHT)
        cairo_rectangle(cr, (g->clip_x + g->clip_w) * wd - border,
                        (g->clip_y + g->clip_h) * ht - border, border, border);
                ...
                const int which, const float zoom_scale)
{
    ...
    if (!g->preview_ready || self->dev->preview_pipe->loading)
        return 0;
        ...
        else
        {
            ...
            if (g->shift_hold)
            {
                ...
                if (g->cropping & GRAB_TOP || g->cropping & GRAB_BOTTOM)
                {
                    ...
                    ratio = fmaxf(ratio, (g->prev_clip_h - 2.0f * yy) / g->prev_clip_h);
                }
...
int button_pressed(dt_iop_module_t *self, const float bzx, const float bzy, const double pressure,
                   const int which, const int type, const uint32_t state, const float zoom_scale)
{
    ...
    if (which == GDK_BUTTON_PRIMARY)
    {
        ...
        {
            if (g->cropping & GRAB_LEFT)
                g->handle_x = bzx - g->clip_x;
            if (g->cropping & GRAB_TOP)
                g->handle_y = bzy - g->clip_y;
            if (g->cropping & GRAB_RIGHT)
                g->handle_x = bzx - (g->clip_w + g->clip_x);
            if (g->cropping & GRAB_BOTTOM)
                g->handle_y = bzy - (g->clip_h + g->clip_y);
        }
...
{
    GSList *lm = NULL;
    lm = dt_mouse_action_create_format(lm, DT_MOUSE_ACTION_LEFT_DRAG, 0, _("[%s on borders] crop"),
                                       self->name());
