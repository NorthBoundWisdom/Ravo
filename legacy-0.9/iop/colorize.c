...
{
    return dt_iop_set_description(self, _("overlay a solid color on the image"), _("creative"),
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    return dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_colorize, width, height, CLARG(dev_in), CLARG(dev_out), CLARG(width),
        CLARG(height), CLARG(mix), CLARG(L), CLARG(a), CLARG(b));
}
#endif
