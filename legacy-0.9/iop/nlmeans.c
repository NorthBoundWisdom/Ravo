...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self,
                                  _("apply a poisson noise removal\n"
                                    "best suited for astrophotography"),
                                  _("corrective"), _("non-linear, Lab, display-referred"),
    ...
    for (int j = -K; j <= 0; j++)
        for (int i = -K; i <= K; i++)
        {
            ...
            err = dt_opencl_enqueue_kernel_2d_local_args(
                devid, gd->kernel_nlmeans_horiz, sizesl, local, CLARG(dev_U4), CLARG(dev_U4_t),
                CLARG(width), CLARG(height), CLARG(q), CLARG(P),
                CLLOCAL((hblocksize + 2 * P) * sizeof(float)));
            if (err != CL_SUCCESS)
                goto error;
            ...
            err = dt_opencl_enqueue_kernel_2d_local_args(
                devid, gd->kernel_nlmeans_vert, sizesl, local, CLARG(dev_U4_t), CLARG(dev_U4_tt),
...
{
    dt_iop_nlmeans_params_t *d = piece->data;
    const int P = ceilf(d->radius * fmin(roi_in->scale, 2.0f) /
                        fmax(piece->iscale, 1.0f)); // pixel filter size
