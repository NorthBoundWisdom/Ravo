...
{
    dt_iop_tonecurve_node_t
        tonecurve[3][DT_IOP_TONECURVE_MAXNODES]; // three curves (L, a, b) with max number
                                                 // of nodes
    int tonecurve_nodes[3];
    int tonecurve_type[3]; // $DEFAULT: MONOTONE_HERMITE
    dt_iop_tonecurve_autoscale_t
        tonecurve_autoscale_ab; //$DEFAULT: DT_S_SCALE_AUTOMATIC_RGB $DESCRIPTION: "color space"
    int tonecurve_preset;       // $DEFAULT: 0
    int tonecurve_unbound_ab;   // $DEFAULT: 1
    dt_iop_rgb_norms_t
        preserve_colors; // $DEFAULT: DT_RGB_NORM_AVERAGE $DESCRIPTION: "preserve colors"
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    const dt_iop_order_iccprofile_info_t *const work_profile = dt_ioppr_add_profile_info_to_list(
        self->dev, DT_COLORSPACE_PROPHOTO_RGB, "", INTENT_PERCEPTUAL);
    ...
    dev_coeffs_L =
        dt_opencl_copy_host_to_device_constant(devid, sizeof(float) * 3, d->unbounded_coeffs_L);
    if (dev_coeffs_L == NULL)
        goto error;

    dev_coeffs_ab =
        dt_opencl_copy_host_to_device_constant(devid, sizeof(float) * 12, d->unbounded_coeffs_ab);
    if (dev_coeffs_ab == NULL)
        goto error;

    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_tonecurve, width, height, CLARG(dev_in), CLARG(dev_out), CLARG(width),
        CLARG(height), CLARG(dev_L), CLARG(dev_a), CLARG(dev_b), CLARG(autoscale_ab),
        CLARG(unbound_ab), CLARG(dev_coeffs_L), CLARG(dev_coeffs_ab), CLARG(low_approximation),
        ...
        if (autoscale_ab == DT_S_SCALE_MANUAL)
        {
            ...
            else
            {
                ...
                out[k + 1] = (a_in > xm_ar) ?
                                 ...
                                 ((a_in < xm_al) ?
                                      dt_iop_eval_exp(d->unbounded_coeffs_ab + 3, 1.0f - a_in) :
                                      d->table[ch_a][CLAMP((int)(a_in * 0x10000ul), 0, 0xffff)]);
                out[k + 2] = (b_in > xm_br) ?
                                 dt_iop_eval_exp(d->unbounded_coeffs_ab + 6, b_in) :
                                 ((b_in < xm_bl) ?
                                      dt_iop_eval_exp(d->unbounded_coeffs_ab + 9, 1.0f - b_in) :
            ...
            else
            {
                ...
                if (lum > 0.f)
                {
                    const float curve_lum =
                        (lum < xm_L) ? d->table[ch_L][CLAMP((int)(lum * 0x10000ul), 0, 0xffff)] :
...
void init_presets(dt_iop_module_so_t *self)
{
    ...
    dt_gui_presets_add_generic(_("contrast compression"), self->op, self->version(), &p, sizeof(p),
    ...
    dt_gui_presets_add_generic(_("gamma 1.0 (linear)"), self->op, self->version(), &p, sizeof(p),
                               TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    dt_gui_presets_add_generic(_("contrast - med (linear)"), self->op, self->version(), &p,
    ...
    dt_gui_presets_add_generic(_("contrast - med (gamma 2.2)"), self->op, self->version(), &p,
                               sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    dt_gui_presets_add_generic(_("gamma 2.0"), self->op, self->version(), &p, sizeof(p), TRUE,
                               DEVELOP_BLEND_CS_RGB_DISPLAY);

    // Gamma 0.5 - no contrast
    for (int k = 1; k < 6; k++)
        p.tonecurve[ch_L][k].y = sqrtf(linear_L[k]);
    dt_gui_presets_add_generic(_("gamma 0.5"), self->op, self->version(), &p, sizeof(p), TRUE,
                               DEVELOP_BLEND_CS_RGB_DISPLAY);

    // Log2 - no contrast
    for (int k = 1; k < 6; k++)
        p.tonecurve[ch_L][k].y = logf(linear_L[k] + 1.0f) / logf(2.0f);
    dt_gui_presets_add_generic(_("logarithm (base 2)"), self->op, self->version(), &p, sizeof(p),
                               TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);

    // Exp2 - no contrast
    for (int k = 1; k < 6; k++)
        p.tonecurve[ch_L][k].y = powf(2.0f, linear_L[k]) - 1.0f;
    dt_gui_presets_add_generic(_("exponential (base 2)"), self->op, self->version(), &p, sizeof(p),
                               TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    {
        // insert the preset
        dt_gui_presets_add_with_blendop(preset_camera_curves[k].name, self->op, self->version(),
        ...
        dt_gui_presets_update_format(preset_camera_curves[k].name, self->op, self->version(),
                                     FOR_RAW);
    ...
    for (int ch = 0; ch < ch_max; ch++)
    {
            ...
            d->curve_nodes[ch] != p->tonecurve_nodes[ch])
        {
            ...
            for (int k = 0; k < p->tonecurve_nodes[ch]; k++)
                (void)dt_draw_curve_add_point(d->curve[ch], p->tonecurve[ch][k].x,
                                              p->tonecurve[ch][k].y);
        }
        else
        {
            for (int k = 0; k < p->tonecurve_nodes[ch]; k++)
                dt_draw_curve_set_point(d->curve[ch], k, p->tonecurve[ch][k].x,
                                        p->tonecurve[ch][k].y);
        }
...
void init(dt_iop_module_t *self)
{
    ...
    d->tonecurve_nodes[1] = d->tonecurve_nodes[2] = 3;
    d->tonecurve[0][1].x = d->tonecurve[0][1].y = d->tonecurve[1][2].x = d->tonecurve[1][2].y =
        d->tonecurve[2][2].x = d->tonecurve[2][2].y = 1.0f;

    d->tonecurve[1][1].x = d->tonecurve[1][1].y = d->tonecurve[2][1].x = d->tonecurve[2][1].y =
        0.5f;
}
...
static void interpolator_callback(GtkWidget *widget, dt_iop_module_t *self)
{
    ...
    if (combo == 0)
        p->tonecurve_type[ch_L] = p->tonecurve_type[ch_a] = p->tonecurve_type[ch_b] = CUBIC_SPLINE;
    if (combo == 1)
        p->tonecurve_type[ch_L] = p->tonecurve_type[ch_a] = p->tonecurve_type[ch_b] = CATMULL_ROM;
    if (combo == 2)
        p->tonecurve_type[ch_L] = p->tonecurve_type[ch_a] = p->tonecurve_type[ch_b] =
            MONOTONE_HERMITE;
...
static float to_log(const float x, const float base, const int ch, const int semilog,
...
static float to_lin(const float x, const float base, const int ch, const int semilog,
                    const int is_ordinate)
{
    // don't log-encode the a and b channels
    if (base > 0.0f && ch == ch_L)
...
void gui_init(dt_iop_module_t *self)
{
    ...
    for (int ch = 0; ch < ch_max; ch++)
    {
        ...
        for (int k = 0; k < p->tonecurve_nodes[ch]; k++)
            (void)dt_draw_curve_add_point(g->minmax_curve[ch], p->tonecurve[ch][k].x,
                                          p->tonecurve[ch][k].y);
    }
    ...
    gtk_widget_set_tooltip_text(g->autoscale_ab,
                                _("if set to auto, a and b curves have no effect and are "
    ...
    dt_gui_box_add(self->widget,
                   dt_gui_hbox(dt_gui_expand(g->channel_tabs), dt_gui_align_right(g->colorpicker)));
...
static gboolean dt_iop_tonecurve_draw(GtkWidget *widget, cairo_t *crf, dt_iop_module_t *self)
{
    ...
    {
        for (int k = 0; k < p->tonecurve_nodes[ch]; k++)
            dt_draw_curve_set_point(g->minmax_curve[ch], k, p->tonecurve[ch][k].x,
                                    p->tonecurve[ch][k].y);
    }
    ...
    else
    {
        ...
        // ch == 2 : blue to yellow
        const float origin[3][3] = {
            {0.0f, 0.0f, 0.0f},                       // L = 0, @ (a, b) = 0
            {0.0f, 231.0f / 255.0f, 181.0f / 255.0f}, // a = -128 @ L = 75, b = 0
            {0.0f, 30.0f / 255.0f, 195.0f / 255.0f}}; // b = -128 @ L = 75, a = 0

        const float destin[3][3] = {
            {1.0f, 1.0f, 1.0f},                        // L = 100 @ (a, b) = 0
        ...
        cairo_pattern_add_color_stop_rgba(pat, 1, origin[ch][0], origin[ch][1], origin[ch][2],
                                          opacities[ch]);
        cairo_pattern_add_color_stop_rgba(pat, 0.5, middle[ch][0], middle[ch][1], middle[ch][2],
                                          opacities[ch]);
        cairo_pattern_add_color_stop_rgba(pat, 0, destin[ch][0], destin[ch][1], destin[ch][2],
                                          opacities[ch]);
                                               ...
                                               dt_iop_module_t *self)
{
    ...
    if (event->state & GDK_BUTTON1_MASK)
    {
        ...
        if (g->selected >= 0)
        {
            ...
            const float translate_mouse_x =
                old_m_x / width -
                to_log(tonecurve[g->selected].x, g->loglogscale, ch, g->semilog, 0);
            const float translate_mouse_y =
                1 - old_m_y / height -
                to_log(tonecurve[g->selected].y, g->loglogscale, ch, g->semilog, 1);
            // dx & dy are in linear coordinates
            const float dx =
                to_lin(g->mouse_x / width - translate_mouse_x, g->loglogscale, ch, g->semilog, 0) -
    ...
    else
    {
        ...
        for (int k = 0; k < nodes; k++)
        {
            float dist = (my - to_log(tonecurve[k].y, g->loglogscale, ch, g->semilog, 1)) *
                             (my - to_log(tonecurve[k].y, g->loglogscale, ch, g->semilog, 1)) +
                         (mx - to_log(tonecurve[k].x, g->loglogscale, ch, g->semilog, 0)) *
                             (mx - to_log(tonecurve[k].x, g->loglogscale, ch, g->semilog, 0));
            if (dist < min)
            {
                min = dist;
