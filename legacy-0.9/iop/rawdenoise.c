...
static void compute_channel_noise(float *const noise, int color,
                                  const dt_iop_rawdenoise_data_t *const data)
{
    ...
    static const float noise_all[] = {0.8002, 0.2735, 0.1202, 0.0585,
...
static void wavelet_denoise(const float *const restrict in, float *const restrict out,
    ...
    for (int c = 0; c < 3; c++)
    {
        ...
        for (size_t chunk = 0; chunk < nthreads; chunk++)
        {
            ...
            for (size_t row = start; row < pastend; row++)
            {
                ...
                for (size_t col = (c != 1); col < width - 1; col++)
                {
                    if (FCNxtrans(row, col, xtrans) == c)
                    {
                        ...
                        else // red or blue pixel
                        {
                            ...
                            // greens.
                            fimgp[col - width - 1] = fimgp[col - width] = fimgp[col - width + 1] =
                                d;                               // row above
                            fimgp[col - 1] = fimgp[col + 1] = d; // left and right
                            if (row < pastend - 1)
                                fimgp[col + width - 1] = fimgp[col + width] =
                                    fimgp[col + width + 1] = d; // row below
                        }
                    }
                }
                ...
                {
                    int src =
                        width - 1; // fallback is current sensel even if it has the wrong color
                   ...
                   dt_dev_pixelpipe_iop_t *piece)
{
    ...
    for (int ch = 0; ch < DT_RAWDENOISE_NONE; ch++)
    {
        dt_draw_curve_set_point(d->curve[ch], 0, p->x[ch][DT_IOP_RAWDENOISE_BANDS - 2] - 1.0,
                                p->y[ch][0]);
        for (int k = 0; k < DT_IOP_RAWDENOISE_BANDS; k++)
        ...
        dt_draw_curve_calc_values(d->curve[ch], 0.0, 1.0, DT_IOP_RAWDENOISE_BANDS, NULL,
                                  d->force[ch]);
    }
...
static void dt_iop_rawdenoise_get_params(dt_iop_rawdenoise_params_t *p, const int ch,
...
static gboolean rawdenoise_draw(GtkWidget *widget, cairo_t *crf, dt_iop_module_t *self)
{
    ...
    dt_draw_curve_set_point(g->transition_curve, 0, p.x[ch][DT_IOP_RAWDENOISE_BANDS - 2] - 1.0,
                            p.y[ch][0]);
    for (int k = 0; k < DT_IOP_RAWDENOISE_BANDS; k++)
        dt_draw_curve_set_point(g->transition_curve, k + 1, p.x[ch][k], p.y[ch][k]);
    dt_draw_curve_set_point(g->transition_curve, DT_IOP_RAWDENOISE_BANDS + 1, p.x[ch][1] + 1.0,
                            p.y[ch][DT_IOP_RAWDENOISE_BANDS - 1]);
    ...
    {
        // draw min/max curves:
        dt_iop_rawdenoise_get_params(&p, g->channel, g->mouse_x, 1., g->mouse_radius);
        dt_draw_curve_set_point(g->transition_curve, 0, p.x[ch][DT_IOP_RAWDENOISE_BANDS - 2] - 1.0,
                                p.y[ch][0]);
        for (int k = 0; k < DT_IOP_RAWDENOISE_BANDS; k++)
            dt_draw_curve_set_point(g->transition_curve, k + 1, p.x[ch][k], p.y[ch][k]);
        dt_draw_curve_set_point(g->transition_curve, DT_IOP_RAWDENOISE_BANDS + 1, p.x[ch][1] + 1.0,
                                p.y[ch][DT_IOP_RAWDENOISE_BANDS - 1]);
        dt_draw_curve_calc_values(g->transition_curve, 0.0, 1.0, DT_IOP_RAWDENOISE_RES,
                                  g->draw_min_xs, g->draw_min_ys);

        p = *(dt_iop_rawdenoise_params_t *)self->params;
        dt_iop_rawdenoise_get_params(&p, g->channel, g->mouse_x, .0, g->mouse_radius);
        dt_draw_curve_set_point(g->transition_curve, 0, p.x[ch][DT_IOP_RAWDENOISE_BANDS - 2] - 1.0,
                                p.y[ch][0]);
        for (int k = 0; k < DT_IOP_RAWDENOISE_BANDS; k++)
            dt_draw_curve_set_point(g->transition_curve, k + 1, p.x[ch][k], p.y[ch][k]);
    ...
    for (int i = 0; i < DT_RAWDENOISE_NONE; i++)
    {
        ...
        ch = ((int)g->channel + i + 1) % DT_RAWDENOISE_NONE;
        float alpha = 0.3;
        if (i == DT_RAWDENOISE_NONE - 1)
            alpha = 1.0;
        switch (ch)
        {
        case 0:
        ...
        dt_draw_curve_set_point(g->transition_curve, DT_IOP_RAWDENOISE_BANDS + 1, p.x[ch][1] + 1.0,
                                p.y[ch][DT_IOP_RAWDENOISE_BANDS - 1]);
        dt_draw_curve_calc_values(g->transition_curve, 0.0, 1.0, DT_IOP_RAWDENOISE_RES, g->draw_xs,
                                  g->draw_ys);
        cairo_move_to(cr, 0 * width / (float)(DT_IOP_RAWDENOISE_RES - 1), -height * g->draw_ys[0]);
        for (int k = 1; k < DT_IOP_RAWDENOISE_RES; k++)
            cairo_line_to(cr, k * width / (float)(DT_IOP_RAWDENOISE_RES - 1),
                          -height * g->draw_ys[k]);
        cairo_stroke(cr);
    }
    ...
    if (g->mouse_y > 0 || g->dragging)
    {
        ...
        const float pos = DT_IOP_RAWDENOISE_RES * g->mouse_x;
        int k = (int)pos;
        const float f = k - pos;
        if (k >= DT_IOP_RAWDENOISE_RES - 1)
            k = DT_IOP_RAWDENOISE_RES - 2;
...
static gboolean rawdenoise_motion_notify(GtkWidget *widget, GdkEventMotion *event,
                                         dt_iop_module_t *self)
{
    ...
    if (g->dragging)
    {
        *p = g->drag_params;
        if (g->x_move < 0)
        {
            dt_iop_rawdenoise_get_params(p, g->channel, g->mouse_x, g->mouse_y + g->mouse_pick,
                                         g->mouse_radius);
        }
...
static gboolean rawdenoise_button_press(GtkWidget *widget, GdkEventButton *event,
                                        dt_iop_module_t *self)
{
    dt_iop_rawdenoise_gui_data_t *g = self->gui_data;
...
static gboolean rawdenoise_button_release(GtkWidget *widget, GdkEventButton *event,
                                          dt_iop_module_t *self)
{
...
static gboolean rawdenoise_leave_notify(GtkWidget *widget, GdkEventCrossing *event,
                                        dt_iop_module_t *self)
{
    dt_iop_rawdenoise_gui_data_t *g = self->gui_data;
    if (!g->dragging)
...
static gboolean rawdenoise_scrolled(GtkWidget *widget, GdkEventScroll *event, dt_iop_module_t *self)
{
    ...
    {
        g->mouse_radius =
            CLAMP(g->mouse_radius * (1.0 + 0.1 * delta_y), 0.2 / DT_IOP_RAWDENOISE_BANDS, 1.0);
        gtk_widget_queue_draw(widget);
    }
...
static void rawdenoise_tab_switch(GtkNotebook *notebook, GtkWidget *page, guint page_num,
                                  dt_iop_module_t *self)
{
    DT_GUARD_GUI_UPDATE();
    dt_iop_rawdenoise_gui_data_t *g = self->gui_data;
...
void gui_init(dt_iop_module_t *self)
{
    ...
    g_signal_connect(G_OBJECT(g->channel_tabs), "switch_page", G_CALLBACK(rawdenoise_tab_switch),
                     self);

    const int ch = (int)g->channel;
    g->transition_curve = dt_draw_curve_new(0.0, 1.0, CATMULL_ROM);
    (void)dt_draw_curve_add_point(g->transition_curve, p->x[ch][DT_IOP_RAWDENOISE_BANDS - 2] - 1.0,
    ...
    g->area =
        GTK_DRAWING_AREA(dt_ui_resize_wrap(NULL, 0, "plugins/darkroom/rawdenoise/graphheight"));
    ...
    g_signal_connect(G_OBJECT(g->area), "button-press-event", G_CALLBACK(rawdenoise_button_press),
                     self);
    g_signal_connect(G_OBJECT(g->area), "button-release-event",
                     G_CALLBACK(rawdenoise_button_release), self);
    g_signal_connect(G_OBJECT(g->area), "motion-notify-event", G_CALLBACK(rawdenoise_motion_notify),
                     self);
    g_signal_connect(G_OBJECT(g->area), "leave-notify-event", G_CALLBACK(rawdenoise_leave_notify),
                     self);
