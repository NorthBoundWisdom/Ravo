...
{
    float
        scale; // $MIN: 0.0 $MAX: 200.0 $DEFAULT: 80.0 $DESCRIPTION: "fall-off start" Inner radius, percent of largest image dimension
    float
        falloff_scale; // $MIN: 0.0 $MAX: 200.0 $DEFAULT: 50.0 $DESCRIPTION: "fall-off radius" 0 - 100 Radius for falloff -- outer radius = inner radius + falloff_scale
    ...
    float
        whratio; // $MIN: 0.0 $MAX: 2.0 $DEFAULT: 1.0 $DESCRIPTION: "width/height ratio" 0-1 = width/height ratio, 1-2 = height/width ratio + 1
...
{
    return dt_iop_set_description(self, _("simulate a lens fall-off close to edges"), _("creative"),
                                  _("non-linear, RGB, display-referred"), _("non-linear, RGB"),
...
static int _get_grab(const float pointerx, const float pointery, const float startx,
                     const float starty, const float endx, const float endy, const float zoom_scale)
{
...
static void draw_overlay(cairo_t *cr, const float x, const float y, const float fx, const float fy,
                     ...
                     const float pzx, const float pzy, const float zoom_scale)
{
    ...
    int grab = _get_grab(pzx * wd - vignette_x, pzy * ht - vignette_y, vignette_w, -vignette_h,
                         vignette_fx, -vignette_fy, zoom_scale);
                ...
                const int which, const float zoom_scale)
{
    ...
    if (grab == 0 || !(darktable.control->button_down &&
                       darktable.control->button_down_which == GDK_BUTTON_PRIMARY))
    {
        grab = _get_grab(pzx * wd - vignette_x, pzy * ht - vignette_y, vignette_w, -vignette_h,
                         vignette_fx, -vignette_fy, zoom_scale);
    }
        ...
        darktable.control->button_down_which == GDK_BUTTON_PRIMARY)
    {
        ...
        else if (grab == 4) // change the height
        {
            ...
            const float max =
                0.5 * ((ratio <= 1.0) ? bigger_side * (2.0 - p->whratio) : bigger_side);
        ...
        else if (grab == 16) // change the falloff on the top
        {
            ...
            const float max =
                0.5 * ((p->whratio > 1.0) ? bigger_side * (2.0 - p->whratio) : bigger_side);
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    const dt_iop_vector_2d_t vignette_center = {buf_center.x + data->center.x * buf_in->width / 2.0,
                                                buf_center.y +
                                                    data->center.y * buf_in->height / 2.0};
    /* Coordinates of vignette_center in terms of roi_in */
    const dt_iop_vector_2d_t roi_center = {vignette_center.x * roi_in->scale - roi_in->x,
    ...
    for (int j = 0; j < roi_out->height; j++)
    {
        ...
        for (int i = 0; i < roi_out->width; i++)
        {
            ...
            const dt_iop_vector_2d_t pv = {fabsf(i * xscale - roi_center_scaled.x),
            ...
            if (weight > 0.0f)
            {
                ...
                if (brightness < 0.0f)
                {
                    ...
                    for_each_channel(c) col[c] = col[c] * falloff + dith;
                }
                else
                {
                ...
                for_each_channel(c) col[c] = unbound ? col[c] : CLIP(col[c]);
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    return dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_vignette, width, height, CLARG(dev_in), CLARG(dev_out), CLARG(width),
        CLARG(height), CLARG(scale), CLARG(roi_center_scaled_f), CLARG(expt), CLARG(dscale),
        CLARG(fscale), CLARG(brightness), CLARG(saturation), CLARG(dither), CLARG(unbound));
}
#endif
...
void init_presets(dt_iop_module_so_t *self)
{
    ...
    p.dithering = 0;
    p.unbound = TRUE;
    dt_gui_presets_add_generic(_("lomo"), self->op, self->version(), &p, sizeof(p), TRUE,
...
GSList *mouse_actions(dt_iop_module_t *self)
{
    GSList *lm = NULL;
    lm =
        dt_mouse_action_create_format(lm, DT_MOUSE_ACTION_LEFT_DRAG, 0,
    ...
    lm = dt_mouse_action_create_format(lm, DT_MOUSE_ACTION_LEFT_DRAG, GDK_CONTROL_MASK,
                                       _("[%s on node] change vignette/feather size keeping ratio"),
                                       self->name());
    lm = dt_mouse_action_create_format(lm, DT_MOUSE_ACTION_LEFT_DRAG, GDK_CONTROL_MASK,
