...
static int gauss_make_triangular(double *A, int *p, int n)
{
    ...
    for (int k = 0; k < n; ++k)
    {
        ...
        A[k + n * m] = A[k + n * k];
        A[k + n * k] =
            t1; // new diagonal elements are (implicitly) one, store scaling factors on diagonal
        if (t1 != 0)
        {
            for (int i = k + 1; i < n; ++i)
                A[k + n * i] /= -t1;
...
static int gauss_solve(double *A, double *b, int n)
{
    ...
    int err_code = 1;
    if ((err_code = gauss_make_triangular(A, p, n)))
...
static inline int pseudo_solve_gaussian(double *const restrict A, double *const restrict y,
