...
static void _compute_sigmas(float sigma[5], dt_iop_bilateral_data_t *data, float scale,
                            float iscale)
{
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    else if (rad <= MAX_DIRECT_STAMP_RADIUS)
    {
        ...
        const dt_aligned_pixel_t isig2col = {1.0f / (2.0f * sigma[2] * sigma[2]),
                                             ...
                                             1.0f / (2.0f * sigma[4] * sigma[4]), 0.0f};
            ...
            for (size_t i = rad; i < width - rad; i++, in += 4)
            {
                ...
                for (ssize_t l = -rad; l <= rad; l++)
                {
                    for (ssize_t k = -rad; k <= rad; k++)
                    {
                        ...
                        for_each_channel(c) chandiff[c] =
                            (pixel[c] - inp[c]) * (pixel[c] - inp[c]) * isig2col[c];

                        const float diff = chandiff[0] + chandiff[1] + chandiff[2];
                        const float pix_weight = m[l * wd + k] * expf(-diff);
                        for_each_channel(c) res[c] += inp[c] * pix_weight;
                        sumw += pix_weight;
                    }
                }
                for_each_channel(c) res[c] /= sumw;
    ...
    else
    {
        ...
        for (size_t j = 0; j < height; j++)
        {
            ...
            for (size_t i = 0; i < width; i++)
            {
                float pos[5] = {i * sigma[0], j * sigma[1], in[0] * sigma[2], in[1] * sigma[3],
                                in[2] * sigma[4]};
        ...
        for (size_t index = 0; index < npixels; index++)
        {
            ...
            for_each_channel(k) val[k] /= val[3];
                     ...
                     dt_develop_tiling_t *tiling)
{
    ...
    else
    {
        ...
        size_t grid_points = (roi_out->height / sigma[0]) * (roi_out->width / sigma[1]) / sigma[2] /
                             sigma[3] / sigma[4];
