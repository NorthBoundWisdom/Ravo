...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self,
                                  _("remove abnormally bright pixels\n"
                                    "by dampening them with neighbors"),
                                  _("corrective"), _("linear, raw, scene-referred"),
                                  _("reconstruction, raw"), _("linear, raw, scene-referred"));
}
...
static int _process_bayer(const dt_iop_hotpixels_data_t *data, const void *const ivoid,
...
static int _process_monochrome(const dt_iop_hotpixels_data_t *data, const void *const ivoid,
                               void *const ovoid, const dt_iop_roi_t *const roi_out,
                               const int planes)
{
    const float threshold = data->threshold;
...
static int _process_xtrans(const dt_iop_hotpixels_data_t *data, const void *const ivoid,
                           void *const ovoid, const dt_iop_roi_t *const roi_out,
...
static gboolean draw(GtkWidget *widget, cairo_t *cr, dt_iop_module_t *self)
{
    ...
    char *str = g_strdup_printf(ngettext("fixed %d pixel", "fixed %d pixels", g->pixels_fixed),
                                g->pixels_fixed);
    g->pixels_fixed = -1;
...
void gui_init(dt_iop_module_t *self)
{
    ...
    GtkWidget *label_non_raw =
        dt_ui_label_new(_("hot pixel correction\nonly works for raw images."));
