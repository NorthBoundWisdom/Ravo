...
typedef struct dt_iop_diffuse_params_t
{
    ...
    float
        anisotropy_first; // $MIN: -10. $MAX: 10.  $DEFAULT: 0. $DESCRIPTION: "1st order anisotropy"
    float
        anisotropy_second; // $MIN: -10. $MAX: 10.  $DEFAULT: 0. $DESCRIPTION: "2nd order anisotropy"
    float
        anisotropy_third; // $MIN: -10. $MAX: 10.  $DEFAULT: 0. $DESCRIPTION: "3rd order anisotropy"
    float
        anisotropy_fourth; // $MIN: -10. $MAX: 10.  $DEFAULT: 0. $DESCRIPTION: "4th order anisotropy"

    float
        threshold; // $MIN: 0.   $MAX: 8.   $DEFAULT: 0. $DESCRIPTION: "luminance masking threshold"
...
{
    GtkWidget *iterations, *fourth, *third, *second, *radius, *radius_center, *sharpness,
        *threshold, *regularization, *first, *anisotropy_first, *anisotropy_second,
        *anisotropy_third, *anisotropy_fourth, *regularization_first, *variance_threshold;
...
{
    return _(
        "diffusion|deconvolution|blur|sharpening|bloom|clarity|dehaze|denoise|inpaint|watercolor");
}
...
void init_presets(dt_iop_module_so_t *self)
{
    ...
    dt_gui_presets_add_generic(_("lens deblur | soft"), self->op, self->version(),
                               &(dt_iop_diffuse_params_t){.iterations = 8,
                                                          .radius_center = 0,
                                                          .radius = 8,
                                                          ...
                                                          .threshold = 0.0f},
                               sizeof(dt_iop_diffuse_params_t), TRUE, DEVELOP_BLEND_CS_RGB_SCENE);

    dt_gui_presets_add_generic(_("lens deblur | medium"), self->op, self->version(),
                               &(dt_iop_diffuse_params_t){.iterations = 16,
                                                          .radius_center = 0,
                                                          .radius = 10,
    ...
    dt_gui_presets_add_generic(_("lens deblur | hard"), self->op, self->version(),
                               &(dt_iop_diffuse_params_t){.iterations = 24,
                                                          .radius_center = 0,
                                                          .radius = 12,
    ...
    dt_gui_presets_add_generic(_("dehaze | default"), self->op, self->version(),
                               &(dt_iop_diffuse_params_t){.iterations = 10,
                                                          .radius_center = 0,
                                                          .radius = 512,
    ...
    dt_gui_presets_add_generic(_("denoise | fine"), self->op, self->version(),
                               &(dt_iop_diffuse_params_t){.iterations = 32,
                                                          .radius_center = 2,
                                                          .radius = 1,
    ...
    dt_gui_presets_add_generic(_("surface blur"), self->op, self->version(),
                               &(dt_iop_diffuse_params_t){.iterations = 2,
                                                          .radius_center = 0,
                                                          .radius = 32,
    ...
    dt_gui_presets_add_generic(_("artistic effects | bloom"), self->op, self->version(),
                               &(dt_iop_diffuse_params_t){.iterations = 1,
                                                          .radius_center = 0,
                                                          .radius = 32,
    ...
    dt_gui_presets_add_generic(_("sharpen demosaicing | no AA filter"), self->op, self->version(),
                               &(dt_iop_diffuse_params_t){.iterations = 1,
                                                          .radius_center = 0,
                                                          .radius = 4,
    ...
    dt_gui_presets_add_generic(_("sharpen demosaicing | AA filter"), self->op, self->version(),
                               &(dt_iop_diffuse_params_t){.iterations = 1,
                                                          .radius_center = 0,
                                                          .radius = 8,
    ...
    dt_gui_presets_add_generic(_("artistic effects | simulate watercolor"), self->op,
                               self->version(),
                               &(dt_iop_diffuse_params_t){.iterations = 4,
                                                          .radius_center = 0,
                                                          .radius = 64,
    ...
    dt_gui_presets_add_generic(_("artistic effects | simulate line drawing"), self->op,
                               self->version(),
                               &(dt_iop_diffuse_params_t){.iterations = 50,
                                                          .radius_center = 0,
                                                          .radius = 64,
    ...
    dt_gui_presets_add_generic(_("local contrast | fine"), self->op, self->version(),
                               &(dt_iop_diffuse_params_t){.iterations = 5,
                                                          .radius_center = 0,
                                                          .radius = 170,
...
static inline void heat_PDE_diffusion(const float *const restrict high_freq,
                                      ...
                                      const uint8_t *const restrict mask, const gboolean has_mask,
                                      float *const restrict output, const size_t width,
                                      const size_t height, const dt_aligned_pixel_t anisotropy,
                                      ...
                                      const dt_aligned_pixel_t ABCD, const float strength)
{
    ...
    for (size_t row = 0; row < height; ++row)
    {
        ...
        const size_t i_neighbours[3] = {MAX((int)(i - mult * H), (int)0) * width, // x - mult
                                        i * width,                                // x
                                        MIN((int)(i + mult * H), (int)height - 1) *
                                            width}; // x + mult
        for (size_t j = 0; j < width; ++j)
        {
            ...
            if (opacity)
            {
                ...
                for_each_channel(c)
                {
                    ...
                    gradient[0][c] =
                        (magnitude_grad != 0.f) ? gradient[0][c] / magnitude_grad : 1.f; // cos(0)
                    // Compute sin (arg(grad))= dy / hypot - force arg(grad) = 0 if hypot == 0
                    gradient[1][c] =
                        (magnitude_grad != 0.f) ? gradient[1][c] / magnitude_grad : 0.f; // sin(0)
                ...
                for_each_channel(c)
                {
                    ...
                    laplacian[0][c] =
                        (magnitude_lapl != 0.f) ? laplacian[0][c] / magnitude_lapl : 1.f; // cos(0)
                    // Compute sin (arg(lapl))= dy / hypot - force arg(lapl) = 0 if hypot == 0
                    laplacian[1][c] =
                        (magnitude_lapl != 0.f) ? laplacian[1][c] / magnitude_lapl : 0.f; // sin(0)
                ...
                {
                    for_each_channel(
                        c, aligned(derivatives, neighbour_pixel_LF, kern_first, kern_second))
                    {
                ...
                for (size_t k = 0; k < 4; k++)
                {
                    for_each_channel(c, aligned(acc, derivatives, ABCD)) acc[c] +=
                        derivatives[k][c] * ABCD[k];
                }
            ...
            else
            {
                ...
                for_each_channel(c, aligned(out, HF, LF : 64)) out[index + c] =
                    HF[index + c] + LF[index + c];
            }
        }
    }
}
...
wavelets_process(const float *const restrict in, float *const restrict reconstructed,
                 const uint8_t *const restrict mask, const size_t width, const size_t height,
                 const dt_iop_diffuse_data_t *const data, const float final_radius,
                 const float zoom, const int scales, const gboolean has_mask,
                 float *const restrict HF[MAX_NUM_SCALES], float *const restrict LF_odd,
                 float *const restrict LF_even)
{
    gboolean success = TRUE;

    const dt_aligned_pixel_t anisotropy = {compute_anisotropy_factor(data->anisotropy_first),
    ...
    const dt_isotropy_t DT_ALIGNED_PIXEL isotropy_type[4] = {
        check_isotropy_mode(data->anisotropy_first), check_isotropy_mode(data->anisotropy_second),
        check_isotropy_mode(data->anisotropy_third), check_isotropy_mode(data->anisotropy_fourth)};
    ...
    for (int s = 0; s < scales; ++s)
    {
        ...
        decompose_2D_Bspline(buffer_in, HF[s], buffer_out, width, height, mult, tempbuf,
                             padded_size);
    ...
    */
        ...
        if (s == 0)
            buffer_out = reconstructed;

        // Compute wavelets low-frequency scales
        heat_PDE_diffusion(HF[s], buffer_in, mask, has_mask, buffer_out, width, height, anisotropy,
                              ...
                              const float threshold, const size_t width, const size_t height)
{
    ...
    for (size_t k = 0; k < height * width * 4; k += 4)
    {
        ...
        mask[k / 4] =
            (input[k] > threshold || input[k + 1] > threshold || input[k + 2] > threshold);
    }
}
                                ...
                                const size_t height)
{
    ...
    for (size_t k = 0; k < height * width * 4; k += 4)
    {
        if (mask[k / 4])
        {
            ...
            for_four_channels(c, aligned(inpainted, original, state : 64)) inpainted[k + c] =
                fabsf(gaussian_noise(original[k + c], original[k + c], i % 2 || j % 2, state));
        }
        else
        {
            for_four_channels(c, aligned(original, inpainted : 64)) inpainted[k + c] =
                original[k + c];
        }
    }
}
             ...
             const dt_iop_roi_t *const roi_out)
{
    ...
    gboolean out_of_memory =
        !mask || !dt_iop_alloc_image_buffers(self, roi_in, roi_out, 4 | DT_IMGSZ_OUTPUT, &temp1,
                                             4 | DT_IMGSZ_OUTPUT, &temp2, 4 | DT_IMGSZ_OUTPUT,
                                             &LF_odd, 4 | DT_IMGSZ_OUTPUT, &LF_even, 0,
                                             NULL); // if failing all pointers are NULL
    ...
    for (int it = 0; it < iterations; it++)
    {
        ...
        wavelets_process(temp_in, temp_out, mask, roi_out->width, roi_out->height, data,
                         final_radius, scale, scales, has_mask, HF, LF_odd, LF_even);
    }
...
static inline cl_int wavelets_process_cl(const int devid, cl_mem in, cl_mem reconstructed,
                                         cl_mem mask, const int width, const int height,
                                         const dt_iop_diffuse_data_t *const data,
                                         dt_iop_diffuse_global_data_t *const gd,
                                         const float final_radius, const float zoom,
                                         const int scales, const int has_mask,
                                         cl_mem HF[MAX_NUM_SCALES], cl_mem LF_odd, cl_mem LF_even)
{
  ...
  */
    ...
    for (int s = scales - 1; s > -1; --s)
    {
        ...
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->kernel_diffuse_pde, width, height, CLARG(HF[s]), CLARG(buffer_in),
            CLARG(mask), CLARG(has_mask), CLARG(buffer_out), CLARG(width), CLARG(height),
            CLARG(anisotropy), CLARG(isotropy_type), CLARG(regularization),
            CLARG(variance_threshold), CLARG(current_radius_square), CLARG(mult), CLARG(ABCD),
            CLARG(strength));
        if (err != CL_SUCCESS)
            return err;
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    if (has_mask)
    {
        ...
        // init the inpainting area with noise
        err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_diffuse_inpaint_mask, width,
                                               height, CLARG(temp1), CLARG(in), CLARG(mask),
                                               CLARG(roi_out->width), CLARG(roi_out->height));
    ...
    for (int it = 0; it < iterations && err == CL_SUCCESS; it++)
    {
        ...
        err = wavelets_process_cl(devid, temp_in, temp_out, mask, width, height, data, gd,
                                  final_radius, scale, scales, has_mask, HF, LF_odd, LF_even);
        if (err == CL_SUCCESS)
...
void gui_init(dt_iop_module_t *self)
{
    ...
    gtk_widget_set_tooltip_text(
        g->iterations, _("more iterations make the effect stronger but the module slower.\n"
    ...
    gtk_widget_set_tooltip_text(g->radius_center,
                                _("main scale of the diffusion.\n"
    ...
    gtk_widget_set_tooltip_text(g->radius,
                                _("width of the diffusion around the central radius.\n"
    ...
    gtk_widget_set_tooltip_text(g->first,
                                _("diffusion speed of low-frequency wavelet layers\n"
    ...
    gtk_widget_set_tooltip_text(g->second,
                                _("diffusion speed of low-frequency wavelet layers\n"
    ...
    gtk_widget_set_tooltip_text(g->third,
                                _("diffusion speed of high-frequency wavelet layers\n"
    ...
    gtk_widget_set_tooltip_text(g->fourth,
                                _("diffusion speed of high-frequency wavelet layers\n"
    ...
    gtk_widget_set_tooltip_text(g->anisotropy_first,
                                _("direction of 1st order speed (set above).\n\n"
    ...
    gtk_widget_set_tooltip_text(g->anisotropy_second,
                                _("direction of 2nd order speed (set above).\n\n"
                                  "negative values follow gradients more closely, \n"
                                  "positive values rather avoid edges (isophotes), \n"
                                  "zero affects both equally (isotropic)."));
    ...
    gtk_widget_set_tooltip_text(g->anisotropy_third,
                                _("direction of 3rd order speed (set above).\n\n"
                                  "negative values follow gradients more closely, \n"
                                  "positive values rather avoid edges (isophotes), \n"
                                  "zero affects both equally (isotropic)."));
    ...
    gtk_widget_set_tooltip_text(g->anisotropy_fourth,
                                _("direction of 4th order speed (set above).\n\n"
                                  "negative values follow gradients more closely, \n"
                                  "positive values rather avoid edges (isophotes), \n"
                                  "zero affects both equally (isotropic)."));
