...
typedef struct dt_iop_colormapping_params_t
{
    ...
    float
        equalization; // $MIN: 0.0 $MAX: 100.0 $DEFAULT: 50.0 $DESCRIPTION: "histogram equalization"
...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self,
                                  _("transfer a color palette and tonal repartition\n"
...
static void invert_histogram(const int *hist, float *inv_hist)
{
    ...
    for (int i = 0; i <= last; i++)
        inv_hist[i] = 100.0f * i / (float)HISTN;
...
static void get_cluster_mapping(const int n, float2 *mi, const float *wi, float2 *mo,
                                const float *wo, const float dominance, int *mapio)
{
...
static void kmeans(const float *col, const int width, const int height, const int n,
                   float2 *mean_out, float2 *var_out, float *weight_out)
{
    ...
    for (int it = 0; it < nit; it++)
    {
        ...
        DT_OMP_PRAGMA(parallel default(none) dt_omp_firstprivate(col, npixels, n, mean_perthread,
                                                                 mean_size, cnt_perthread, cnt_size,
                                                                 var_perthread, var_size, mean_out))
        {
    ...
    if (data->flag & HAS_TARGET && data->flag & HAS_SOURCE)
    {
        ...
        for (int i = 0; i < data->n; i++)
        {
            var_ratio[i][0] = (data->target_var[i][0] > 0.0f) ?
                                  data->source_var[mapio[i]][0] / data->target_var[i][0] :
                                  0.0f;
            var_ratio[i][1] = (data->target_var[i][1] > 0.0f) ?
                                  data->source_var[mapio[i]][1] / data->target_var[i][1] :
                                  0.0f;
        }
        ...
        for (size_t k = 0; k < npixels * 4; k += 4)
        {
            ...
            out[k] = 0.5f * ((L * (1.0f - equalization) +
                              data->source_ihist[data->target_hist[(int)CLAMP(
                                  HISTN * L / 100.0f, 0.0f, (float)HISTN - 1.0f)]] *
                                  equalization) -
                             L) +
                     50.0f;
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    if (self->dev->gui_attached && g && dt_pipe_is_preview(piece->pipe) && (data->flag & ACQUIRE))
    {
        ...
        if (g->buffer)
            err = dt_opencl_copy_image_to_host(devid, g->buffer, dev_in, width, height,
                                               ch * sizeof(float));
    ...
    if (data->flag & HAS_TARGET && data->flag & HAS_SOURCE)
    {
        ...
        dev_target_hist =
            dt_opencl_copy_host_to_device_constant(devid, sizeof(int) * HISTN, data->target_hist);
        if (dev_target_hist == NULL)
            goto error;

        dev_source_ihist = dt_opencl_copy_host_to_device_constant(devid, sizeof(float) * HISTN,
                                                                  data->source_ihist);
        if (dev_source_ihist == NULL)
            goto error;

        dev_target_mean = dt_opencl_copy_host_to_device_constant(devid, sizeof(float) * MAXN * 2,
                                                                 data->target_mean);
        if (dev_target_mean == NULL)
            goto error;

        dev_source_mean = dt_opencl_copy_host_to_device_constant(devid, sizeof(float) * MAXN * 2,
                                                                 data->source_mean);
        if (dev_source_mean == NULL)
            goto error;

        dev_var_ratio =
            dt_opencl_copy_host_to_device_constant(devid, sizeof(float) * MAXN * 2, var_ratio);
        if (dev_var_ratio == NULL)
            goto error;
        ...
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->kernel_histogram, width, height, CLARG(dev_in), CLARG(dev_out), CLARG(width),
            CLARG(height), CLARG(equalization), CLARG(dev_target_hist), CLARG(dev_source_ihist));
        if (err != CL_SUCCESS)
            goto error;
        ...
        else
        {
            ...
            err = dt_opencl_enqueue_copy_image(devid, dev_out, dev_tmp, CLIMG_ORIGIN, CLIMG_ORIGIN,
                                               region);
            if (err != CL_SUCCESS)
                goto error;
        }
        ...
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->kernel_mapping, width, height, CLARG(dev_in), CLARG(dev_tmp), CLARG(dev_out),
            CLARG(width), CLARG(height), CLARG(data->n), CLARG(dev_target_mean),
                     ...
                     dt_develop_tiling_t *tiling)
{
    ...
    tiling->factor =
        3.0f + (float)dt_bilateral_memory_use(width, height, sigma_s, sigma_r) / basebuffer;
    tiling->maxbuf = fmaxf(
        1.0f, (float)dt_bilateral_singlebuffer_size(width, height, sigma_s, sigma_r) / basebuffer);
    tiling->overhead = 0;
    ...
    for (int cl = 0; cl < p->n; cl++)
    {
        ...
        for (int j = -1; j <= 1; j++)
            for (int i = -1; i <= 1; i++)
            {
                ...
                cairo_rectangle(cr, qwd * (i + 1) / 3.0, height * (j + 1) / 3.0,
                                qwd / 3.0 - DT_PIXEL_APPLY_DPI(.5),
...
void gui_init(dt_iop_module_t *self)
{
    ...
    gtk_widget_set_tooltip_text(
        g->clusters, _("number of clusters to find in image. value change resets all clusters"));

    g->dominance = dt_bauhaus_slider_from_params(self, "dominance");
    gtk_widget_set_tooltip_text(g->dominance,
                                _("how clusters are mapped. low values: based on color "
    ...
    if (f)
    {
        if (fread(&g->flowback, sizeof(g->flowback), 1, f) > 0)
            g->flowback_set = 1;
        fclose(f);
    }
}
