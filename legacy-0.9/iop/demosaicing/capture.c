...
static unsigned char *_cs_precalc_gauss_idx(dt_iop_module_t *self, const int width,
                                            const int height, const int dx, const int dy,
                                            const float isigma, const float boost,
                                            const float centre)
{
    ...
    for (int row = 0; row < height; row++)
    {
        ...
        for (int col = 0; col < width; col++)
        {
            ...
            const float sigma =
                (isigma + corr) * 0.125f *
                (float)MIN(8, MIN(height - row - 1, MIN(width - col - 1, MIN(col, row))));
...
static float _calcRadiusBayer(const float *in, const int width, const int height,
            ...
            if (val00 > RAWEPS)
            {
                ...
                if (val1m1 > RAWEPS && maxVal0 > lowerLimit)
                {
                    ...
                    if (maxVal0 > maxRatio * minVal)
                    {
                        ...
                        { // check for influence by clipped green in neighborhood
                            if (MAX(MAX(MAX(cfa[-2], val00), cfa[2 * width - 2]), cfa[2 * width]) >=
                                upperLimit)
                                clipped = TRUE;
                        }
                        if (!clipped)
                ...
                if (val1p1 > RAWEPS && maxVal1 > lowerLimit)
                {
                    ...
                    if (maxVal1 > maxRatio * minVal)
                    {
                        ...
                        { // check for influence by clipped green in neighborhood
                            if (MAX(MAX(MAX(val00, cfa[2]), cfa[2 * width]), cfa[2 * width + 2]) >=
                                upperLimit)
                                continue;
                        }
...
static float _calcRadiusMono(const float *in, const int width, const int height)
{
    float maxRatio = 1.0f;
    DT_OMP_FOR(reduction(max : maxRatio))
    for (int row = 4; row < height - 4; ++row)
                               ...
                               const uint8_t (*const xtrans)[6])
{
    ...
    for (int row = starty + 2; row < height - 4; row += 3)
    {
        for (int col = startx + 2; col < width - 4; col += 3)
        {
            ...
            const gboolean squareClipped =
                MAX(MAX(MAX(valp1p1, cfa[width + 2]), cfa[2 * width + 1]), cfa[2 * width + 2]) >=
                upperLimit;
            ...
            if (greenSolitary > RAWEPS && MAX(cfa[-width - 1], cfa[-width + 1]) < upperLimit)
            {
                if (greenSolitary < upperLimit)
                {
                    ...
                    if (valp1m1 > RAWEPS &&
                        MAX(MAX(MAX(cfa[width - 2], valp1m1), cfa[2 * width - 2]), cfa[width - 1]) <
                            upperLimit)
                    {
            ...
            if (!squareClipped)
            {
                ...
                if (valp2p2 > RAWEPS)
                {
                    ...
                    if (MAX(MAX(greenSolitaryRight, cfa[4 * width + 2]), cfa[4 * width + 4]) <
                        upperLimit)
                    {
                ...
                if (valp2p1 > RAWEPS)
                {
                    ...
                    if (MAX(MAX(greenSolitaryLeft, cfa[4 * width - 1]), cfa[4 * width + 1]) <
                        upperLimit)
                    {
...
static float _calc_auto_radius(float *const in, const dt_image_t *img,
                               const dt_iop_roi_t *const roi, const uint32_t filters,
                               const uint8_t (*const xtrans)[6], const dt_iop_buffer_dsc_t *dsc,
                               gboolean *reliable, int *xpos, int *ypos, int *mwidth, int *mheight)
{
    ...
    const dt_aligned_pixel_t coeff = {wbon ? 1.0f / MAX(1.0f, dsc->temperature.coeffs[0]) : 1.0f,
                                      ...
                                      wbon ? 1.0f / MAX(1.0f, dsc->temperature.coeffs[2]) : 1.0f,
                                      1.0f};
  ...
  */
    ...
    *reliable = FALSE;
    ...
    const float radius = !filters      ? _calcRadiusMono(input, owidth, oheight) :
                         filters != 9u ? _calcRadiusBayer(input, owidth, oheight, filters) :
...
static inline void _blur_mul(const float *const in, float *out, const float *blend,
                             const float *const kernels, const unsigned char *const table,
                             const int w1, const int height)
{
    const int w2 = 2 * w1;
    const int w3 = 3 * w1;
    const int w4 = 4 * w1;
                ...
                if (col >= bd && row >= bd && col < w1 - bd && row < height - bd)
                {
                    ...
                    if (use_small)
                    {
                        val = kern[5 + 2] * (d[-w2 - 1] + d[-w2 + 1] + d[-w1 - 2] + d[-w1 + 2] +
                              ...
                              kern[1] * (d[-w1] + d[-1] + d[1] + d[w1]) + kern[0] * (d[0]);
                    }
                    else
                    {
                        val = kern[10 + 4] * (d[-w4 - 2] + d[-w4 + 2] + d[-w2 - 4] + d[-w2 + 4] +
                                              d[w2 - 4] + d[w2 + 4] + d[w4 - 2] + d[w4 + 2]) +
                              kern[5 + 4] * (d[-w4 - 1] + d[-w4 + 1] + d[-w1 - 4] + d[-w1 + 4] +
                                             d[w1 - 4] + d[w1 + 4] + d[w4 - 1] + d[w4 + 1]) +
                              kern[4] * (d[-w4] + d[-4] + d[4] + d[w4]) +
                              kern[15 + 3] * (d[-w3 - 3] + d[-w3 + 3] + d[w3 - 3] + d[w3 + 3]) +
                              kern[10 + 3] * (d[-w3 - 2] + d[-w3 + 2] + d[-w2 - 3] + d[-w2 + 3] +
                                              d[w2 - 3] + d[w2 + 3] + d[w3 - 2] + d[w3 + 2]) +
                              kern[5 + 3] * (d[-w3 - 1] + d[-w3 + 1] + d[-w1 - 3] + d[-w1 + 3] +
                                             d[w1 - 3] + d[w1 + 3] + d[w3 - 1] + d[w3 + 1]) +
                              kern[3] * (d[-w3] + d[-3] + d[3] + d[w3]) +
                              kern[10 + 2] * (d[-w2 - 2] + d[-w2 + 2] + d[w2 - 2] + d[w2 + 2]) +
                              kern[5 + 2] * (d[-w2 - 1] + d[-w2 + 1] + d[-w1 - 2] + d[-w1 + 2] +
                                             d[w1 - 2] + d[w1 + 2] + d[w2 - 1] + d[w2 + 1]) +
                              kern[2] * (d[-w2] + d[-2] + d[2] + d[w2]) +
                              kern[5 + 1] * (d[-w1 - 1] + d[-w1 + 1] + d[w1 - 1] + d[w1 + 1]) +
                              kern[1] * (d[-w1] + d[-1] + d[1] + d[w1]) + kern[0] * (d[0]);
                    ...
                    for (int ir = -bd; ir <= bd; ir++)
                    {
                        ...
                        if (irow >= 0 && irow < height)
                        {
                            for (int ic = -bd; ic <= bd; ic++)
                            {
                                ...
                                if (icol >= 0 && icol < w1)
                                    val +=
                                        kern[5 * ABS(ir) + ABS(ic)] * in[(size_t)irow * w1 + icol];
                            }
                        }
                    }
                }
...
static inline void _blur_div(const float *const in, float *out, const float *const luminance,
                             const float *blend, const float *const kernels,
                             const unsigned char *const table, const int w1, const int height)
{
    const int w2 = 2 * w1;
    const int w3 = 3 * w1;
    const int w4 = 4 * w1;
...
static void _prepare_blend(const float *cfa, const float *rgb, const uint32_t filters,
                           const uint8_t (*const xtrans)[6], float *mask, float *Yold,
                           const float *whites, const int w1, const int height)
{
    ...
    for (size_t row = 0; row < height; row++)
    {
        for (size_t col = 0; col < w1; col++)
        {
            ...
            if (row > 1 && col > 1 && row < height - 2 && col < w1 - 2)
            {
                ...
                const gboolean heat =
                    filters ? cfa[k] > whites[color] : cfa[4 * k] > CAPTURE_CFACLIP;
                if (heat || Yold[k] < CAPTURE_YMIN)
                {
                    mask[k - w2 - 1] = mask[k - w2] = mask[k - w2 + 1] = mask[k - w1 - 2] =
                        mask[k - w1 - 1] = mask[k - w1] = mask[k - w1 + 1] = mask[k - w1 + 2] =
                            mask[k - 2] = mask[k - 1] = mask[k] = mask[k + 1] = mask[k + 2] =
                                mask[k + w1 - 2] = mask[k + w1 - 1] = mask[k + w1] =
                                    mask[k + w1 + 1] = mask[k + w1 + 2] = mask[k + w2 - 1] =
                                        mask[k + w2] = mask[k + w2 + 1] = 0.0f;
                }
            }
            else
                mask[k] = 0.0f;
...
static void _modify_blend(float *blend, float *const Yold, float *luminance, const float dthresh,
...
static void _capture_radius(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *const piece,
                            float *const in, const dt_iop_roi_t *const roi,
                            const uint8_t (*const xtrans)[6], const uint32_t filters)
{
    ...
    int px, py, dx, dy;
    const float radius =
        _calc_auto_radius(in, img, roi, filters, xtrans, dsc, &reliable, &px, &py, &dx, &dy);
    const gboolean same_radius = feqf(p->cs_radius, radius, CAPTURE_SAME_RADIUS);

    dt_print_pipe(DT_DEBUG_PIPE,
                  filters == 9u ? "xtrans autoradius" :
                  filters       ? "bayer autoradius" :
                                  "mono autoradius",
                  pipe, self, DT_DEVICE_NONE, roi, NULL,
                  "%s radius=%.2f is %sreliable at (%d/%d) %dx%d", same_radius ? "same" : "new",
                  radius, reliable ? "" : "NOT ", px, py, dx, dy);
    ...
    if (fullpipe && g)
    {
        ...
        if (!reliable)
            dt_control_log(_(
                "imprecise radius calculation due to cropping or because you are zoomed in too much"));
    }
    d->cs_radius = radius;
}

static void _capture_noise(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *const piece)
{
    dt_iop_demosaic_data_t *d = piece->data;
    dt_iop_demosaic_gui_data_t *g = self->gui_data;
    dt_iop_demosaic_params_t *p = self->params;
    ...
    dt_print_pipe(DT_DEBUG_PIPE, "capture threshold", pipe, self, DT_DEVICE_NONE, NULL, NULL,
                  "threshold=%.2f", thrs);
...
static inline gboolean _noise_requested(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *const piece)
{
    ...
    if (g && fullpipe && !g->autothrs && invalid_thrs)
        return TRUE;
...
static inline gboolean _radius_requested(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *const piece)
{
    const dt_iop_demosaic_gui_data_t *g = self->gui_data;
    const dt_iop_demosaic_data_t *d = piece->data;
...
static void _capture_sharpen(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *const piece,
                             const float *const in, float *out, const int width, const int height,
                             const int dx, const int dy, const gboolean show_variance_mask,
                             const gboolean show_sigma_mask, const uint8_t (*const xtrans)[6],
                             const uint32_t filters)
{
    ...
    if (!d->cs_enabled && !show_variance_mask && !show_sigma_mask)
        return;

    const dt_iop_buffer_dsc_t *dsc = &pipe->dsc;
    const gboolean wbon = dsc->temperature.enabled;
    const dt_aligned_pixel_t icoeffs = {
        wbon ? CAPTURE_CFACLIP * dsc->temperature.coeffs[0] : CAPTURE_CFACLIP,
        wbon ? CAPTURE_CFACLIP * dsc->temperature.coeffs[1] : CAPTURE_CFACLIP,
        wbon ? CAPTURE_CFACLIP * dsc->temperature.coeffs[2] : CAPTURE_CFACLIP, 0.0f};
    ...
    gauss_idx =
        _cs_precalc_gauss_idx(self, width, height, dx, dy, d->cs_radius, d->cs_boost, d->cs_center);
    if (!gauss_idx)
        goto finalize;
...
finalize:
    if (error)
        dt_print_pipe(DT_DEBUG_ALWAYS, "capture sharpen failed", pipe, self, DT_DEVICE_CPU, NULL,
...
static void _capture_radius_cl(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *const piece,
                               const cl_mem dev_in, const dt_iop_roi_t *const roi,
                               const uint8_t (*const xtrans)[6], const uint32_t filters,
                               const gboolean mono)
{
    const dt_dev_pixelpipe_t *pipe = piece->pipe;
    ...
    err = dt_opencl_copy_image_to_host(pipe->devid, in, dev_in, roi->width, roi->height,
                                       sizeof(float) * ch);
    if (err == CL_SUCCESS)
...
static int _capture_sharpen_cl(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *const piece,
                               const cl_mem dev_in, cl_mem dev_out, cl_mem dev_xtrans,
                               const int width, const int height, const int dx, const int dy,
                               const gboolean showmask, const gboolean show_sigmamask,
                               const uint8_t (*const xtrans)[6], const uint32_t filters)
{
    dt_dev_pixelpipe_t *pipe = piece->pipe;
    ...
    if (!d->cs_enabled && !showmask)
        return CL_SUCCESS;

    const dt_iop_buffer_dsc_t *dsc = &pipe->dsc;
    const gboolean wbon = dsc->temperature.enabled;
    dt_aligned_pixel_t icoeffs = {
        wbon ? CAPTURE_CFACLIP * dsc->temperature.coeffs[0] : CAPTURE_CFACLIP,
        wbon ? CAPTURE_CFACLIP * dsc->temperature.coeffs[1] : CAPTURE_CFACLIP,
        wbon ? CAPTURE_CFACLIP * dsc->temperature.coeffs[2] : CAPTURE_CFACLIP, 0.0f};
    ...
    if (!blendmask || !luminance || !tmp2 || !tmp1 || !dev_rgb)
        goto finish;

    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->prefill_clip_mask, width, height, CLARG(tmp2),
                                           CLARG(width), CLARG(height));
    if (err != CL_SUCCESS)
        goto finish;

    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->prepare_blend, width, height, CLARG(dev_in), CLARG(dev_out), CLARG(filters),
        CLARG(dev_xtrans), CLARG(tmp2), CLARG(tmp1), CLARG(icoeffs), CLARG(width), CLARG(height));
    if (err != CL_SUCCESS)
        goto finish;

    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->modify_blend, width, height, CLARG(tmp2),
                                           CLARG(tmp1), CLARG(luminance), CLARG(d->cs_thrs),
                                           CLARG(width), CLARG(height));
    if (err != CL_SUCCESS)
        goto finish;

    err =
        dt_gaussian_fast_blur_cl_buffer(devid, tmp2, blendmask, width, height, 2.0f, 1, 0.0f, 1.0f);
    if (err != CL_SUCCESS)
        goto finish;

    err = dt_opencl_enqueue_kernel_1d_args(devid, gd->final_blend, pixels, CLARG(blendmask),
                                           CLARG(tmp2), CLARG(pixels));
    if (err != CL_SUCCESS)
        goto finish;
    ...
    {
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->show_blend_mask, width, height, CLARG(dev_rgb), CLARG(dev_out),
            CLARG(blendmask), CLARG(gauss_idx), CLARG(width), CLARG(height), CLARG(showmask));
        goto finish;
    }

    unsigned char *f_gauss_idx =
        _cs_precalc_gauss_idx(self, width, height, dx, dy, d->cs_radius, d->cs_boost, d->cs_center);
    if (f_gauss_idx)
    {
        gcoeffs = dt_opencl_copy_host_to_device_constant(
            devid, sizeof(float) * (UCHAR_MAX + 1) * CAPTURE_KERNEL_ALIGN, gd->gauss_coeffs);
        gauss_idx = dt_opencl_copy_host_to_device_constant(
            devid, sizeof(unsigned char) * height * width, f_gauss_idx);
    }
    dt_free_align(f_gauss_idx);

    err = CL_MEM_OBJECT_ALLOCATION_FAILURE;
    if (!gcoeffs || !gauss_idx)
        goto finish;
    ...
    for (int iter = 0; iter < d->cs_iter; iter++)
    {
        err = dt_opencl_enqueue_kernel_2d_args(
            ...
            CLARG(blendmask), CLARG(gcoeffs), CLARG(gauss_idx), CLARG(width), CLARG(height));
        if (err != CL_SUCCESS)
            goto finish;

        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->gaussian_9x9_mul, width, height, CLARG(tmp2), CLARG(tmp1), CLARG(blendmask),
            CLARG(gcoeffs), CLARG(gauss_idx), CLARG(width), CLARG(height));
        if (err != CL_SUCCESS)
            goto finish;
    }

    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->capture_result, width, height, CLARG(dev_rgb),
                                           CLARG(dev_out), CLARG(blendmask), CLARG(luminance),
                                           CLARG(tmp1), CLARG(width), CLARG(height));

finish:
    if (err != CL_SUCCESS)
        dt_print_pipe(DT_DEBUG_ALWAYS, "capture sharpen failed", pipe, self, devid, NULL, NULL,
