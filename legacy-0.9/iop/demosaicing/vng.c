...
static void _vng_lininterpolate(float *out, const float *const in, const int width,
                                const int height, const uint32_t filters,
                                const uint8_t (*const xtrans)[6], const int border)
{
    ...
    for (int row = 0; row < size; row++)
        for (int col = 0; col < size; col++)
        {
            ...
            lookup[row][col][0] =
                (ip - &(lookup[row][col][0])) / 3; /* # of neighboring pixels found */
            for (int c = 0; c < colors; c++)
                if (c != f)
                {
                    *ip++ = c;
...
static void vng_interpolate(float *out, const float *const in, const int width, const int height,
                            ...
                            const gboolean only_vng_linear)
{
    ...
    for (int row = 0; row < prow; row++) /* Precalculate for VNG */
        for (int col = 0; col < pcol; col++)
        {
            ...
            for (int t = 0; t < 64; t++)
            {
                ...
                if (fcol(row + y2, col + x2, filters4, xtrans) != color)
                    continue;
                const int diag = (fcol(row, col + 1, filters4, xtrans) == color &&
    ...
    for (int row = 2; row < height - 2; row++) /* Do VNG interpolation */
    {
        ...
        if (row > 3) /* Write buffer to image */
            dt_iop_image_copy(out + 4 * ((row - 2) * width + 2), (float *)(brow[0] + 2),
                              (width - 4) * 4);
    ...
    dt_iop_image_copy(out + (4 * ((height - 4) * width + 2)), (float *)(brow[0] + 2),
                      (width - 4) * 4);
    dt_iop_image_copy(out + (4 * ((height - 3) * width + 2)), (float *)(brow[1] + 2),
                      (width - 4) * 4);
...
#ifdef HAVE_OPENCL
static cl_int process_vng_cl(const dt_iop_module_t *self, const dt_dev_pixelpipe_iop_t *piece,
                             cl_mem dev_in, cl_mem dev_out, cl_mem dev_xtrans,
                             const uint8_t (*const xtrans)[6], const int width, const int height,
                             const uint32_t filters, const int border,
                             const gboolean only_vng_linear)
{
...

    {
        ...
        err = dt_opencl_enqueue_kernel_2d_local_args(
            devid, gd->kernel_vng_lin_interpolate, sizes, local, CLARG(dev_in), CLARG(tmp_out),
            CLARG(width), CLARG(height), CLARG(border), CLARG(filters4), CLARG(dev_xtrans),
            CLARG(dev_lookup), CLLOCAL(sizeof(float) * (locopt.sizex + 2) * (locopt.sizey + 2)),
    ...
    size_t local[2] = {locopt.sizex, locopt.sizey};
    err = dt_opencl_enqueue_kernel_2d_local_args(
        devid, gd->kernel_vng_interpolate, sizes, local, CLARG(dev_tmp), CLARG(dev_out),
        CLARG(width), CLARG(height), CLARG(filters4), CLARG(dev_xtrans), CLARG(dev_ips),
        CLARG(dev_code), CLLOCAL(sizeof(float) * 4 * (locopt.sizex + 4) * (locopt.sizey + 4)));
