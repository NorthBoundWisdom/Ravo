...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self,
                                  _("quickly convert an image to black & white\n"
...
void init_presets(dt_iop_module_so_t *self)
{
    ...
    p.b = 64.0f;
    p.highlights = 0.0f;
    dt_gui_presets_add_generic(_("red filter"), self->op, self->version(), &p, sizeof(p), TRUE,
...
static float _color_filter(const float ai, const float bi, const float a, const float b,
                           const float dbl_size)
{
    return dt_fast_expf(
        -CLAMPS(((ai - a) * (ai - a) + (bi - b) * (bi - b)) / dbl_size, 0.0f, 1.0f));
}
             ...
             void *const o, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    for (int k = 0; k < 4 * npixels; k += 4)
    {
        ...
        out[k] = (1.0f - t) * in[k] +
                 t * out[k] * (1.0f / 100.0f) * in[k]; // normalized filter * input brightness
    }
}
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    cl_mem dev_tmp =
        dt_opencl_alloc_device(devid, roi_in->width, roi_in->height, sizeof(float) * 4);

    dt_bilateral_cl_t *b =
        dt_bilateral_init_cl(devid, roi_in->width, roi_in->height, sigma_s, sigma_r);
    if (!b || dev_tmp == NULL)
        goto error;
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_monochrome_filter, width, height,
                                           ...
                                           CLARG(height), CLARG(d->a), CLARG(d->b), CLARG(sigma2));
    if (err != CL_SUCCESS)
        goto error;
    ...
    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_monochrome, width, height, CLARG(dev_in), CLARG(dev_tmp), CLARG(dev_out),
        CLARG(width), CLARG(height), CLARG(d->a), CLARG(d->b), CLARG(sigma2), CLARG(d->highlights));
...
static gboolean _monochrome_draw(GtkWidget *widget, cairo_t *crf, dt_iop_module_t *self)
{
    ...
    const float x = p->a * width / PANEL_WIDTH + width * .5f,
                y = p->b * height / PANEL_WIDTH + height * .5f;
