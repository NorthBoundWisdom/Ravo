...
static void process_common_setup(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece)
{
    ...
    for (int k = 0; k < ch; k++)
    {
        ...
        // "undo" rawprepare iop
        chthr *= piece->pipe->dsc.rawprepare.raw_white_point -
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
  ...
  printf("out %f %f %f %f\n\n", pts[0], pts[1], pts[2], pts[3]);
    ...
    for (int j = 0; j < roi_out->height; j++)
    {
        ...
        dt_dev_distort_backtransform_plus(self->dev, self->dev->full.pipe, iop_order,
        ...
        for (int i = 0; i < roi_out->width; i++)
        {
            ...
            if (i_raw < 0 || j_raw < 0 || i_raw >= buf.width || j_raw >= buf.height)
                continue;
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    for (int j = 0; j < height; j++)
    {
        ...
        // where did they come from?
        dt_dev_distort_backtransform_plus(self->dev, self->dev->full.pipe, self->iop_order,
                                          DT_DEV_TRANSFORM_DIR_BACK_INCL, bufptr, roi_out->width);
    }
    ...
    case DT_DEV_RAWOVEREXPOSED_MODE_MARK_CFA:
        ...
        err = CL_MEM_OBJECT_ALLOCATION_FAILURE;
        dev_colors = dt_opencl_copy_host_to_device_constant(
            devid, sizeof(dt_iop_rawoverexposed_colors), (void *)dt_iop_rawoverexposed_colors);
        if (err != CL_SUCCESS)
            goto error;
    ...
    dev_thresholds = dt_opencl_copy_host_to_device_constant(devid, sizeof(unsigned int) * 4,
                                                            (void *)d->threshold);
    if (dev_thresholds == NULL)
        goto error;

    if (dev->rawoverexposed.mode == DT_DEV_RAWOVEREXPOSED_MODE_MARK_CFA)
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, kernel, width, height, CLARG(dev_in), CLARG(dev_out), CLARG(dev_coord),
            CLARG(width), CLARG(height), CLARG(dev_raw), CLARG(raw_width), CLARG(raw_height),
            CLARG(filters), CLARG(dev_xtrans), CLARG(dev_thresholds), CLARG(dev_colors));
    else if (dev->rawoverexposed.mode == DT_DEV_RAWOVEREXPOSED_MODE_MARK_SOLID)
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, kernel, width, height, CLARG(dev_in), CLARG(dev_out), CLARG(dev_coord),
            CLARG(width), CLARG(height), CLARG(dev_raw), CLARG(raw_width), CLARG(raw_height),
            CLARG(filters), CLARG(dev_xtrans), CLARG(dev_thresholds), CLARG(color));
                   ...
                   dt_dev_pixelpipe_iop_t *piece)
{
    ...
    if (image->buf_dsc.datatype != TYPE_UINT16 || !image->buf_dsc.filters)
        piece->enabled = FALSE;
}
...
void init_global(dt_iop_module_so_t *self)
{
    ...
    gd->kernel_rawoverexposed_mark_cfa =
        dt_opencl_create_kernel(program, "rawoverexposed_mark_cfa");
    gd->kernel_rawoverexposed_mark_solid =
        dt_opencl_create_kernel(program, "rawoverexposed_mark_solid");
    gd->kernel_rawoverexposed_falsecolor =
        dt_opencl_create_kernel(program, "rawoverexposed_falsecolor");
}
