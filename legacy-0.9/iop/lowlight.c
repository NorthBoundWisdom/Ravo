...
{
    return dt_iop_set_description(self, _("simulate human night vision"), _("creative"),
                                  _("non-linear, Lab, display-referred"), _("linear, XYZ"),
...
void process(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece, const void *const restrict i,
             void *const restrict o, const dt_iop_roi_t *const roi_in,
             const dt_iop_roi_t *const roi_out)
{
    ...
    for (size_t k = 0; k < (size_t)npixels; k++)
    {
        ...
        for_each_channel(c) XYZ_s[c] = V * XYZ_sw[c];

        for_each_channel(c) XYZ[c] = w * XYZ[c] + (1.0f - w) * XYZ_s[c];
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_lowlight, width, height, CLARG(dev_in),
                                           CLARG(dev_out), CLARG(width), CLARG(height),
                                           CLARG(XYZ_sw), CLARG(dev_m));
                   ...
                   dt_dev_pixelpipe_iop_t *piece)
{
    ...
    dt_draw_curve_set_point(d->curve, 0, p->transition_x[DT_IOP_LOWLIGHT_BANDS - 2] - 1.0,
                            p->transition_y[0]);
...
void init_pipe(dt_iop_module_t *self, dt_dev_pixelpipe_t *pipe, dt_dev_pixelpipe_iop_t *piece)
{
    ...
    for (int k = 0; k < DT_IOP_LOWLIGHT_BANDS; k++)
        dt_draw_curve_add_point(d->curve, default_params->transition_x[k],
                                default_params->transition_y[k]);
...
void init(dt_iop_module_t *self)
{
    ...
    for (int k = 0; k < DT_IOP_LOWLIGHT_BANDS; k++)
        d->transition_x[k] = k / (DT_IOP_LOWLIGHT_BANDS - 1.0);
}
...
void init_presets(dt_iop_module_so_t *self)
{
    ...
    p.blueness = 0.0f;
    dt_gui_presets_add_generic(_("daylight"), self->op, self->version(), &p, sizeof(p), TRUE,
    ...
    p.blueness = 30.0f;
    dt_gui_presets_add_generic(_("indoor bright"), self->op, self->version(), &p, sizeof(p), TRUE,
                               DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    p.blueness = 30.0f;
    dt_gui_presets_add_generic(_("indoor dim"), self->op, self->version(), &p, sizeof(p), TRUE,
                               DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    p.blueness = 40.0f;
    dt_gui_presets_add_generic(_("indoor dark"), self->op, self->version(), &p, sizeof(p), TRUE,
                               DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    p.blueness = 50.0f;
    dt_gui_presets_add_generic(_("twilight"), self->op, self->version(), &p, sizeof(p), TRUE,
                               DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    p.blueness = 30.0f;
    dt_gui_presets_add_generic(_("night street lit"), self->op, self->version(), &p, sizeof(p),
    ...
    p.blueness = 30.0f;
    dt_gui_presets_add_generic(_("night street"), self->op, self->version(), &p, sizeof(p), TRUE,
                               DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    p.blueness = 40.0f;
    dt_gui_presets_add_generic(_("night street dark"), self->op, self->version(), &p, sizeof(p),
                               TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    p.blueness = 50.0f;
    dt_gui_presets_add_generic(_("night"), self->op, self->version(), &p, sizeof(p), TRUE,
                               DEVELOP_BLEND_CS_RGB_DISPLAY);
                                       ...
                                       const double mouse_y, const float rad)
{
    ...
    {
        const float f =
            expf(-(mouse_x - p->transition_x[k]) * (mouse_x - p->transition_x[k]) / (rad * rad));
...
static gboolean lowlight_draw(GtkWidget *widget, cairo_t *crf, dt_iop_module_t *self)
{
    ...
    if (g->mouse_y > 0 || g->dragging)
    {
        ...
        dt_draw_curve_set_point(g->transition_curve, 0,
                                p.transition_x[DT_IOP_LOWLIGHT_BANDS - 2] - 1.0, p.transition_y[0]);
        for (int k = 0; k < DT_IOP_LOWLIGHT_BANDS; k++)
            dt_draw_curve_set_point(g->transition_curve, k + 1, p.transition_x[k],
                                    p.transition_y[k]);
        dt_draw_curve_set_point(g->transition_curve, DT_IOP_LOWLIGHT_BANDS + 1,
                                p.transition_x[1] + 1.0, p.transition_y[DT_IOP_LOWLIGHT_BANDS - 1]);
        dt_draw_curve_calc_values(g->transition_curve, 0.0, 1.0, DT_IOP_LOWLIGHT_RES,
    ...
    dt_draw_curve_set_point(g->transition_curve, DT_IOP_LOWLIGHT_BANDS + 1, p.transition_x[1] + 1.0,
                            p.transition_y[DT_IOP_LOWLIGHT_BANDS - 1]);
    dt_draw_curve_calc_values(g->transition_curve, 0.0, 1.0, DT_IOP_LOWLIGHT_RES, g->draw_xs,
                              g->draw_ys);
    ...
    for (int k = 0; k < DT_IOP_LOWLIGHT_BANDS; k++)
    {
        cairo_arc(cr, width * p.transition_x[k], -height * p.transition_y[k],
    ...
    if (g->mouse_y > 0 || g->dragging)
    {
        ...
        for (int k = 1; k < DT_IOP_LOWLIGHT_RES; k++)
            cairo_line_to(cr, k * width / (float)(DT_IOP_LOWLIGHT_RES - 1),
        ...
        int k = (int)pos;
        const float f = k - pos;
        if (k >= DT_IOP_LOWLIGHT_RES - 1)
            k = DT_IOP_LOWLIGHT_RES - 2;
...
static gboolean lowlight_motion_notify(GtkWidget *widget, GdkEventMotion *event,
...
static gboolean lowlight_button_press(GtkWidget *widget, GdkEventButton *event,
                                      dt_iop_module_t *self)
{
    dt_iop_lowlight_gui_data_t *g = self->gui_data;
...
static gboolean lowlight_button_release(GtkWidget *widget, GdkEventButton *event,
                                        dt_iop_module_t *self)
{
...
static gboolean lowlight_leave_notify(GtkWidget *widget, GdkEventCrossing *event,
                                      dt_iop_module_t *self)
{
    dt_iop_lowlight_gui_data_t *g = self->gui_data;
    if (!g->dragging)
...
static gboolean lowlight_scrolled(GtkWidget *widget, GdkEventScroll *event, dt_iop_module_t *self)
{
    ...
    {
        g->mouse_radius =
            CLAMP(g->mouse_radius * (1.0 + 0.1 * delta_y), 0.2 / DT_IOP_LOWLIGHT_BANDS, 1.0);
        gtk_widget_queue_draw(widget);
    }
...
void gui_init(dt_iop_module_t *self)
{
    ...
    g->area = GTK_DRAWING_AREA(dt_ui_resize_wrap(NULL, 0, "plugins/darkroom/lowlight/graphheight"));
    ...
    g_signal_connect(G_OBJECT(g->area), "button-press-event", G_CALLBACK(lowlight_button_press),
                     self);
    g_signal_connect(G_OBJECT(g->area), "button-release-event", G_CALLBACK(lowlight_button_release),
                     self);
    g_signal_connect(G_OBJECT(g->area), "motion-notify-event", G_CALLBACK(lowlight_motion_notify),
                     self);
    g_signal_connect(G_OBJECT(g->area), "leave-notify-event", G_CALLBACK(lowlight_leave_notify),
                     self);
