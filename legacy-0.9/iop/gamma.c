                                ...
                                const float *const restrict mask_color, const float alpha)
{
    ...
    for (size_t c = 0; c < 3; c++)
        pixel[c] = in[c] <= 0.0031308f ? 12.92f * in[c] :
...
static void _normalize_color(float *const restrict pixel, const float norm)
{
    ...
    for_each_channel(x) pixel[x] *= factor;
}
...
static inline void _XYZ_to_REC_709_normalized(const float *const restrict XYZ,
                                              float *const restrict sRGB, const float norm)
{
...
static void _channel_display_false_color(const float *const restrict in,
                                         uint8_t *const restrict out, const size_t buffsize,
                                         const float alpha, dt_dev_pixelpipe_display_mask_t channel)
{
...
DT_OMP_DECLARE_SIMD(aligned(in, out : 64) uniform(buffsize, alpha))
static void _mask_display(const float *const restrict in, uint8_t *const restrict out,
                          const size_t buffsize, const float alpha)
{
    // yellow, "unused" element aids vectorization
    const dt_aligned_pixel_t mask_color = {1.0f, 1.0f, 0.0f};
    ...
    {
        const float gray =
            interpolatef(mix, in[j + 3], 0.3f * in[j + 0] + 0.59f * in[j + 1] + 0.11f * in[j + 2]);
...
static void _copy_output(const float *const restrict in, uint8_t *const restrict out,
             ...
             void *const o, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
        ...
        (mask_display & DT_DEV_PIXELPIPE_DISPLAY_ANY))
    {
        if (fcolor)
        {
            _channel_display_false_color((const float *const restrict)i, (uint8_t *const restrict)o,
                                         buffsize, alpha, mask_display);
        }
        else
        {
            _channel_display_monochrome((const float *const restrict)i, (uint8_t *const restrict)o,
                                        buffsize, alpha);
        }
    }
