...
dt_liquify_layer_t dt_liquify_layers[] = {
    ...
    },
    {
        ...
        LGREY,
        1.0,
        DT_LIQUIFY_LAYER_FLAG_ANY_TOOL | DT_LIQUIFY_LAYER_FLAG_HIT_TEST,
    },
    {
        DT_LIQUIFY_LAYER_CTRLPOINT1,
        GREY,
    ...
    },
    {
        ...
        LGREY,
        1.0,
        DT_LIQUIFY_LAYER_FLAG_NODE_TOOL | DT_LIQUIFY_LAYER_FLAG_NODE_SELECTED,
    },
    {
        DT_LIQUIFY_LAYER_HARDNESSPOINT2,
        GREY,
    ...
    },
    {
        ...
        LGREY,
        1.0,
        DT_LIQUIFY_LAYER_FLAG_NODE_TOOL | DT_LIQUIFY_LAYER_FLAG_HIT_TEST,
    },
    {
        DT_LIQUIFY_LAYER_CTRLPOINT2,
        GREY,
...
{
    return dt_iop_set_description(self, _("distort parts of the image"), _("creative"),
...
static void node_insert_before(dt_iop_liquify_params_t *p, dt_liquify_path_data_t *this,
...
static void _distort_paths_locked(const dt_iop_module_t *self, const distort_params_t *params,
...
static void distort_paths_raw_to_piece(const dt_iop_module_t *self, dt_dev_pixelpipe_t *pipe,
                                       const float roi_in_scale, dt_iop_liquify_params_t *p)
{
    const distort_params_t params = {self->dev, pipe, pipe->iscale, roi_in_scale,
...
static void mix_warps(dt_liquify_warp_t *result, const dt_liquify_warp_t *warp1,
                      const dt_liquify_warp_t *warp2, const complex float pt, const float t)
{
    ...
    const float radius =
        mix(cabsf(warp1->radius - warp1->point), cabsf(warp2->radius - warp2->point), t);
...
static void interpolate_cubic_bezier(const float complex p0, const float complex p1,
                                     const float complex p2, const float complex p3,
                                     float complex buffer[], const int n)
{
                              ...
                              const cairo_rectangle_int_t *const restrict global_map_extent)
{
    ...
    strength =
        (warp->status & DT_LIQUIFY_STATUS_INTERPOLATED) ? (strength * STAMP_RELOCATION) : strength;
    ...
    float complex *const center = (global_map + (stamp_y - global_map_extent->y) * global_width +
...
static void _apply_global_distortion_map(dt_iop_module_t *self, const dt_dev_pixelpipe_iop_t *piece,
        ...
        for (size_t x = min_x; x < max_x; x++)
        {
            if (*row != 0) // point actually warped?
            {
                if (ch == 1) // handle masks
                    out_sample[x] = CLIP(dt_interpolation_compute_sample(
                        interpolation, in, x + crealf(*row) - roi_in->x,
                        y + cimagf(*row) - roi_in->y, roi_in->width, roi_in->height, 1, ch_width));
                else
                    dt_interpolation_compute_pixel4c(
                        interpolation, in, out_sample + ch * x, x + crealf(*row) - roi_in->x,
                        y + cimagf(*row) - roi_in->y, roi_in->width, roi_in->height, ch_width);
            }
            ++row;
        }
    }
...
static GSList *_get_map_extent(const dt_iop_roi_t *roi_out, const GList *interpolated,
                               cairo_rectangle_int_t *map_extent)
{
    const cairo_rectangle_int_t roi_out_rect = {roi_out->x, roi_out->y, roi_out->width,
                                                roi_out->height};
...
static void _build_global_distortion_map(const dt_iop_module_t *self,
                                         const dt_dev_pixelpipe_iop_t *piece, const float scale,
                                         const dt_iop_roi_t *roi, cairo_rectangle_int_t *map_extent,
                                         const gboolean inverted, float complex **map)
{
    // copy params
                   ...
                   const dt_iop_roi_t *roi_out, dt_iop_roi_t *roi_in)
{
    ...
    *roi_in = *roi_out;
    ...
    _build_global_distortion_map(self, piece, roi_in->scale, roi_out, &extent, FALSE, NULL);
    const cairo_rectangle_int_t pipe_rect = {0, 0, lroundf(piece->buf_in.width * roi_in->scale),
                                             lroundf(piece->buf_in.height * roi_in->scale)};

    cairo_rectangle_int_t roi_in_rect = {roi_in->x, roi_in->y, roi_in->width, roi_in->height};
                                    ...
                                    const gboolean inverted)
{
    ...
    if (extent.width > 0 && extent.height > 0)
    {
        ...
        _build_global_distortion_map(self, piece, scale, &roi_in, &extent, inverted, &map);
        ...
        for (size_t i = 0; i < points_count; i++)
        {
            ...
            const int map_offset =
                ((int)(x - 0.5) - extent.x) + ((int)(y - 0.5) - extent.y) * extent.width;

            if (x >= extent.x && x < x_last && y >= extent.y && y < y_last && map_offset >= 0 &&
                map_offset < map_size)
            {
...
static void start_drag(dt_iop_liquify_gui_data_t *g, const dt_liquify_layer_enum_t layer,
                       dt_liquify_path_data_t *elem)
{
                  ...
                  const dt_iop_roi_t *const roi_out)
{
    ...
    cairo_rectangle_int_t map_extent;
    float complex *map = NULL;
    _build_global_distortion_map(self, piece, roi_in->scale, roi_out, &map_extent, FALSE, &map);
    if (map == NULL)
        return;
...
void process(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece, const void *const in,
             void *const out, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    if (!dt_iop_have_required_input_format(4 /*we need full-color pixels*/, self, piece->colors, in,
                                           out, roi_in, roi_out))
        return;
    // 1. copy the whole image (we'll change only a small part of it)
...
static inline float lanczos(const float a, const float x)
{
    ...
    return (a * sinf(M_PI_F * x) * sinf(M_PI_F * x / a)) / (M_PI_F * M_PI_F * x * x);
}
...
static cl_int_t
_apply_global_distortion_map_cl(const dt_iop_module_t *self, const dt_dev_pixelpipe_iop_t *piece,
                                const cl_mem_t dev_in, const cl_mem_t dev_out,
                                const dt_iop_roi_t *roi_in, const dt_iop_roi_t *roi_out,
                                const float complex *map, const cairo_rectangle_int_t *map_extent)
{
    ...
    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->warp_kernel, map_extent->width, map_extent->height, CLARG(dev_in),
        CLARG(dev_out), CLARG(dev_roi_in), CLARG(dev_roi_out), CLARG(dev_map),
        CLARG(dev_map_extent), CLARG(dev_kdesc), CLARG(dev_kernel));
...
int process_cl(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece, const cl_mem_t dev_in,
               const cl_mem_t dev_out, const dt_iop_roi_t *roi_in, const dt_iop_roi_t *roi_out)
{
    ...
    // 3. apply the map
    if (map_extent.width != 0 && map_extent.height != 0)
        err = _apply_global_distortion_map_cl(self, piece, dev_in, dev_out, roi_in, roi_out, map,
                                              &map_extent);
    dt_free_align((void *)map);
    return err;
}
...
static void draw_rectangle(cairo_t *cr, const float complex pt, const double theta,
                           const double size)
{
...
static void draw_triangle(cairo_t *cr, const float complex pt, const double theta,
                          const double size)
{
    const double x = creal(pt), y = cimag(pt);
    cairo_save(cr);
...
static void set_source_rgba(cairo_t *cr, const dt_liquify_rgba_t rgba)
{
...
static void set_line_width(cairo_t *cr, const double scale, const dt_liquify_ui_width_enum_t w)
{
...
static gboolean detect_drag(const dt_iop_liquify_gui_data_t *g, const double scale,
...
static GList *interpolate_paths(dt_iop_liquify_params_t *p)
{
    ...
    for (int k = 0; k < MAX_NODES; k++)
    {
        ...
        if (data->header.type == DT_LIQUIFY_PATH_CURVE_TO_V1)
        {
            ...
            interpolate_cubic_bezier(*p1, data->node.ctrl1, data->node.ctrl2, *p2, buffer,
...
static void _draw_paths(const dt_iop_module_t *self, cairo_t *cr, const float scale,
                        dt_iop_liquify_params_t *p, const GList *layers)
{
    const dt_iop_liquify_gui_data_t *g = self->gui_data;
        ...
        for (int k = 0; k < MAX_NODES; k++)
        {
            ...
            else if (layer == DT_LIQUIFY_LAYER_WARPS)
            {
                ...
                for (const GList *i = interpolated; i; i = g_list_next(i))
                {
                    ...
                    draw_triangle(cr, pwarp->strength, cargf(pwarp->strength - pwarp->point) + rot,
            ...
            if (layer == DT_LIQUIFY_LAYER_HARDNESSPOINT1)
            {
                draw_triangle(cr, cmix(point, warp->radius, warp->control1),
                              cargf(warp->radius - point), GET_UI_WIDTH(GIZMO_SMALL));
                THINLINE;
                BG_COLOR;
                cairo_fill_preserve(cr);
                FG_COLOR;
            ...
            if (layer == DT_LIQUIFY_LAYER_STRENGTHPOINT_HANDLE && showhandle)
            {
                ...
                if (warp->type == DT_LIQUIFY_WARP_TYPE_LINEAR)
                {
                    const float complex pt = cmix(
                        point, warp->strength,
                        1.0 - 0.5 * (GET_UI_WIDTH(GIZMO_SMALL) / cabsf(warp->strength - point)));
            ...
            if (layer == DT_LIQUIFY_LAYER_STRENGTHPOINT)
            {
                ...
                draw_triangle(cr, warp->strength, cargf(warp->strength - warp->point) + rot,
...
static float find_nearest_on_curve_t(const float complex p0, const float complex p1,
                                     const float complex p2, const float complex p3,
                                     const float complex x, const int n)
{
...
static float find_nearest_on_line_t(const float complex p0, const float complex p1,
...
static void casteljau(const float complex *p0, float complex *p1, float complex *p2,
                      float complex *p3, const float t)
{
...
void _hit_paths(dt_iop_module_t *self, dt_iop_liquify_params_t *p, const GList *layers,
                const float complex *pt, dt_liquify_hit_t *hit)
{
...
static void draw_paths(const dt_iop_module_t *self, cairo_t *cr, const float scale,
...
void _hit_test_paths(dt_iop_module_t *self, dt_iop_liquify_params_t *params, const float complex pt,
...
static void smooth_path_linsys(size_t n, const float complex *k, float complex *c1,
                               float complex *c2, const int *equation)
{
    --n;
...
static void smooth_paths_linsys(dt_iop_liquify_params_t *params)
{
    for (int k = 0; k < MAX_NODES; k++)
    {
        ...
        while (node)
        {
            ...
            const int next_autosmooth =
                nx && nx->header.node_type == DT_LIQUIFY_NODE_TYPE_AUTOSMOOTH;
...
void gui_post_expose(dt_iop_module_t *self, cairo_t *cr, const float bb_width,
                     const float bb_height, const float pointerx, const float pointery,
                     const float zoom_scale)
{
    ...
    {
        // distort all points
        const distort_params_t d_params = {develop, develop->preview_pipe, iscale, 1.0 / scale,
...
static void sync_pipe(dt_iop_module_t *self, const gboolean history)
{
    if (history)
    {
        dt_iop_liquify_params_t *p = self->params;
...
static void get_point_scale(const dt_iop_module_t *self, const float pzx, const float pzy,
                            float complex *pt, float *scale)
{
    float wd, ht;
    ...
    dt_dev_distort_backtransform_plus(darktable.develop, darktable.develop->preview_pipe,
                                      self->iop_order, DT_DEV_TRANSFORM_DIR_FORW_EXCL, pts, 1);
    dt_dev_distort_backtransform_plus(darktable.develop, darktable.develop->preview_pipe,
                                      self->iop_order, DT_DEV_TRANSFORM_DIR_BACK_EXCL, pts, 1);
...
int mouse_moved(dt_iop_module_t *self, const float x, const float y, const double pressure,
        ...
        case DT_LIQUIFY_LAYER_CTRLPOINT1:
            ...
            case DT_LIQUIFY_PATH_CURVE_TO_V1:
                ...
                if (p && p->header.type == DT_LIQUIFY_PATH_CURVE_TO_V1)
                {
                    ...
                    {
                    case DT_LIQUIFY_NODE_TYPE_SMOOTH:
                        p->node.ctrl2 = p->warp.point + cabsf(p->warp.point - p->node.ctrl2) *
        ...
        case DT_LIQUIFY_LAYER_CTRLPOINT2:
            ...
            case DT_LIQUIFY_PATH_CURVE_TO_V1:
                ...
                if (n && n->header.type == DT_LIQUIFY_PATH_CURVE_TO_V1)
                {
                    ...
                    {
                    case DT_LIQUIFY_NODE_TYPE_SMOOTH:
                        n->node.ctrl1 = d->warp.point + cabsf(d->warp.point - n->node.ctrl1) *
...
static float dt_conf_get_sanitize_float(const char *name, const float min, const float max,
...
static void get_stamp_params(dt_iop_module_t *self, float *radius, float *r_strength, float *phi)
{
    ...
    *r_strength =
        dt_conf_get_sanitize_float(CONF_STRENGTH, 0.5f * *radius, 2.0f * *radius, 1.5f * *radius);
 ...
 */
int scrolled(dt_iop_module_t *self, const float x, const float y, const int up,
...
static void _start_new_shape(dt_iop_module_t *self)
{
    ...
    float complex pt = 0.0f;
    float scale = 1.0f;
    get_point_scale(self, 0.5f * darktable.develop->full.width,
                    0.5f * darktable.develop->full.height, &pt, &scale);
...
int button_released(dt_iop_module_t *self, const float x, const float y, const int which,
                    const uint32_t state, const float zoom_scale)
{
    ...
    if (gtk_toggle_button_get_active(g->btn_node_tool))
    {
        ...
        if (which == GDK_BUTTON_PRIMARY && dt_modifier_is(g->last_mouse_mods, GDK_SHIFT_MASK) &&
            !dragged)
        {
        ...
        if (which == GDK_BUTTON_PRIMARY && dt_modifier_is(g->last_mouse_mods, GDK_CONTROL_MASK) &&
            !dragged)
        {
            // add node
            if (g->last_hit.layer == DT_LIQUIFY_LAYER_PATH)
            {
                ...
                if (prev && e->header.type == DT_LIQUIFY_PATH_CURVE_TO_V1)
                {
                    ...
                    const float t = find_nearest_on_curve_t(warp1->point, curve1->node.ctrl1,
                                                            curve1->node.ctrl2, warp3->point, pt,
                                                            INTERPOLATION_POINTS);

                    float complex midpoint = warp3->point;
                    casteljau(&warp1->point, &curve1->node.ctrl1, &curve1->node.ctrl2, &midpoint,
                              t);
                    midpoint = warp1->point;
                    casteljau(&warp3->point, &curve2->node.ctrl2, &curve2->node.ctrl1, &midpoint,
                              1.0 - t);
            ...
            else if (g->last_hit.elem &&
                     g->last_hit.elem->header.type == DT_LIQUIFY_PATH_CURVE_TO_V1 &&
        ...
        if (which == GDK_BUTTON_PRIMARY &&
            dt_modifier_is(g->last_mouse_mods, GDK_MOD1_MASK | GDK_CONTROL_MASK) && !dragged)
        {
            if (g->last_hit.layer == DT_LIQUIFY_LAYER_PATH)
            {
                // change segment
...
static void _liquify_cairo_paint_point_tool(cairo_t *cr, const gint x, const gint y, const gint w,
                                            const gint h, const gint flags, void *data);

static void _liquify_cairo_paint_line_tool(cairo_t *cr, const gint x, const gint y, const gint w,
                                           const gint h, const gint flags, void *data);

static void _liquify_cairo_paint_curve_tool(cairo_t *cr, const gint x, const gint y, const gint w,
                                            const gint h, const gint flags, void *data);

static void _liquify_cairo_paint_node_tool(cairo_t *cr, const gint x, const gint y, const gint w,
                                           const gint h, const gint flags, void *data);
...
void gui_init(dt_iop_module_t *self)
{
    ...
    g->temp = NULL;
    g->status = 0;
    g->last_mouse_pos = g->last_button1_pressed_pos = -1;
    g->last_hit = NOWHERE;
    g->node_index = 0;
    ...
    gtk_widget_set_tooltip_text(count,
    ...
    dt_liquify_layers[DT_LIQUIFY_LAYER_CTRLPOINT1].hint = _("<b>shape of path</b>: drag");
    dt_liquify_layers[DT_LIQUIFY_LAYER_CTRLPOINT2].hint = _("<b>shape of path</b>: drag");
    dt_liquify_layers[DT_LIQUIFY_LAYER_RADIUSPOINT].hint = _("<b>radius</b>: drag");
    dt_liquify_layers[DT_LIQUIFY_LAYER_HARDNESSPOINT1].hint = _("<b>hardness (center)</b>: drag");
    dt_liquify_layers[DT_LIQUIFY_LAYER_HARDNESSPOINT2].hint = _("<b>hardness (feather)</b>: drag");
