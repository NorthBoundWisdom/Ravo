...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(
        self, _("correct white balance selectively for blacks and whites"),
        _("corrective or creative"), _("non-linear, Lab, display-referred"), _("non-linear, Lab"),
...
void init_presets(dt_iop_module_so_t *self)
{
    ...
    p.hib = 3.0f;
    p.saturation = 1.0f;
    dt_gui_presets_add_generic(_("warm tone"), self->op, self->version(), &p, sizeof(p), TRUE,
    ...
    p.hib = 4.5f;
    p.saturation = 1.0f;
    dt_gui_presets_add_generic(_("warming filter"), self->op, self->version(), &p, sizeof(p), TRUE,
                               DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    p.hib = -4.5f;
    p.saturation = 1.0f;
    dt_gui_presets_add_generic(_("cooling filter"), self->op, self->version(), &p, sizeof(p), TRUE,
                               DEVELOP_BLEND_CS_RGB_DISPLAY);
}
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    return dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_colorcorrection, width, height,
                                            ...
                                            CLARG(height), CLARG(d->saturation), CLARG(d->a_scale),
...
static gboolean dt_iop_colorcorrection_draw(GtkWidget *widget, cairo_t *cr, dt_iop_module_t *self);
...
void gui_init(dt_iop_module_t *self)
{
    ...
    gtk_widget_set_tooltip_text(GTK_WIDGET(g->area),
                                _("drag the line for split-toning. "
    ...
    g_signal_connect(G_OBJECT(g->area), "scroll-event", G_CALLBACK(dt_iop_colorcorrection_scrolled),
                     self);
    g_signal_connect(G_OBJECT(g->area), "key-press-event",
                     G_CALLBACK(dt_iop_colorcorrection_key_press), self);
...
static gboolean dt_iop_colorcorrection_draw(GtkWidget *widget, cairo_t *crf, dt_iop_module_t *self)
{
    ...
    for (int j = 0; j < cells; j++)
        for (int i = 0; i < cells; i++)
        {
            ...
            // printf("lab = %f %f %f\n", Lab[0], Lab[1], Lab[2]);
            Lab.a = p->saturation *
                    (Lab.a + Lab.L * .05 * DT_COLORCORRECTION_MAX * (i / (cells - 1.0) - .5));
            Lab.b = p->saturation *
                    (Lab.b + Lab.L * .05 * DT_COLORCORRECTION_MAX * (j / (cells - 1.0) - .5));
