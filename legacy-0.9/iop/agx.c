...
typedef struct dt_iop_agx_params_t
{
    ...
    float
        look_original_hue_mix_ratio; // $MIN: 0.f $MAX: 1.f $DEFAULT: 0.6f $DESCRIPTION: "preserve hue"

    // log mapping
    float
        range_black_relative_ev; // $MIN: -20.f $MAX: -0.1f  $DEFAULT: -10.f $DESCRIPTION: "black relative exposure"
    float
        range_white_relative_ev; // $MIN: 0.1f $MAX: 20.f $DEFAULT: 6.5f $DESCRIPTION: "white relative exposure"
    float
        dynamic_range_scaling; // $MIN: -0.5f $MAX: 2.f $DEFAULT: 0.1f $DESCRIPTION: "dynamic range scaling"
    ...
    float
        curve_pivot_x; // $MIN: 0.f $MAX: 1.f $DEFAULT: 0.606060606061f $DESCRIPTION: "pivot relative exposure"
    // Corresponds to p_y, but not directly -- needs application of gamma
    float
        curve_pivot_y_linear_output; // $MIN: 0.f $MAX: 1.f $DEFAULT: 0.18f $DESCRIPTION: "pivot target output"
    // P_slope
    float
        curve_contrast_around_pivot; // $MIN: 0.1f $MAX: 10.f $DEFAULT: 3.0f $DESCRIPTION: "contrast"
    // related to P_tlength; the number expresses the portion of the y range below the pivot
    float
        curve_linear_ratio_below_pivot; // $MIN: 0.f $MAX: 1.f $DEFAULT: 0.f $DESCRIPTION: "toe start"
    // related to P_slength; the number expresses the portion of the y range below the pivot
    float
        curve_linear_ratio_above_pivot; // $MIN: 0.f $MAX: 1.f $DEFAULT: 0.f $DESCRIPTION: "shoulder start"
    // t_p
    float curve_toe_power; // $MIN: 0.f $MAX: 10.f $DEFAULT: 1.5f $DESCRIPTION: "toe power"
    // s_p
    float
        curve_shoulder_power; // $MIN: 0.f $MAX: 10.f $DEFAULT: 3.3f $DESCRIPTION: "shoulder power"
    ...
    // t_ly
    float
        curve_target_display_black_ratio; // $MIN: 0.f $MAX: 0.15f $DEFAULT: 0.f $DESCRIPTION: "target black"
    // s_ly
    float
        curve_target_display_white_ratio; // $MIN: 0.2f $MAX: 1.f $DEFAULT: 1.f $DESCRIPTION: "target white"

    // custom primaries; rotation limits below: +/- 0.5236 radian => +/- 30 degrees
    dt_iop_agx_base_primaries_t
        base_primaries; // $DEFAULT: DT_AGX_REC2020 $DESCRIPTION: "base primaries"
    ...
    float
        green_rotation; // $MIN: -0.5236f $MAX: 0.5236f  $DEFAULT: 0.f $DESCRIPTION: "green rotation"
    float blue_inset;   // $MIN:  0.f  $MAX: 0.99f $DEFAULT: 0.f $DESCRIPTION: "blue attenuation"
    float
        blue_rotation; // $MIN: -0.5236f $MAX: 0.5236f  $DEFAULT: 0.f $DESCRIPTION: "blue rotation"

    float
        master_outset_ratio; // $MIN:  0.f  $MAX: 2.f $DEFAULT: 1.f $DESCRIPTION: "master purity boost"
    float
        master_unrotation_ratio; // $MIN:  0.f  $MAX: 2.f $DEFAULT: 1.f $DESCRIPTION: "master rotation reversal"
    float red_outset; // $MIN:  0.f  $MAX: 0.99f $DEFAULT: 0.f $DESCRIPTION: "red purity boost"
    float
        red_unrotation; // $MIN: -0.5236f $MAX: 0.5236f  $DEFAULT: 0.f $DESCRIPTION: "red reverse rotation"
    float green_outset; // $MIN:  0.f  $MAX: 0.99f $DEFAULT: 0.f $DESCRIPTION: "green purity boost"
    float
        green_unrotation; // $MIN: -0.5236f $MAX: 0.5236f  $DEFAULT: 0.f $DESCRIPTION: "green reverse rotation"
    float blue_outset; // $MIN:  0.f  $MAX: 0.99f $DEFAULT: 0.f $DESCRIPTION: "blue purity boost"
    float
        blue_unrotation; // $MIN: -0.5236f $MAX: 0.5236f  $DEFAULT: 0.f $DESCRIPTION: "blue reverse rotation"
...
_agx_get_base_profile(dt_develop_t *dev, const dt_iop_order_iccprofile_info_t *pipe_work_profile,
                      const dt_iop_agx_base_primaries_t base_primaries_selection)
{
    ...
    case DT_AGX_EXPORT_PROFILE:
    {
        ...
        if (profile_type != DT_COLORSPACE_NONE && profile_filename != NULL)
        {
            ...
            selected_profile_info = dt_ioppr_add_profile_info_to_list(
                dev, profile_type, profile_filename, INTENT_PERCEPTUAL);
    ...
    case DT_AGX_SRGB:
    {
        ...
        selected_profile_info = dt_ioppr_add_profile_info_to_list(dev, profile_type, "",
    ...
    {
        selected_profile_info = dt_ioppr_add_profile_info_to_list(
            dev, DT_COLORSPACE_LIN_REC2020, "", DT_INTENT_RELATIVE_COLORIMETRIC);

        // if even Rec2020 fails, something is very wrong, but let the caller handle NULL if necessary.
        if (!selected_profile_info)
            dt_print(DT_DEBUG_ALWAYS,
                     "[agx] CRITICAL: Failed to get even Rec2020 base profile info.");
    }
 ...
 */
static inline float _scale(const float limit_x, const float limit_y, const float transition_x,
...
static inline float _scaled_sigmoid(const float x, const float scale, const float slope,
                                    const float power, const float transition_x,
...
{
    return x < 0.f ? params->target_black :
                     params->target_black + fmaxf(0.f, params->toe_fallback_coefficient *
                                                           powf(x, params->toe_fallback_power));
}

static inline float _fallback_shoulder(const float x, const tone_mapping_params_t *params)
{
    return x >= 1.f ? params->target_white :
                      params->target_white -
...
static inline float _apply_slope_lift(const float x, const float slope, const float lift)
{
...
static inline void _agx_look(dt_aligned_pixel_t pixel_in_out, const tone_mapping_params_t *params,
                             const dt_colormatrix_t rendering_to_xyz_transposed)
{
    ...
    for_three_channels(k, aligned(pixel_in_out : 16))
    {
        ...
        pixel_in_out[k] = value_with_slope_and_lift > 0.f ? powf(value_with_slope_and_lift, power) :
...
static inline float _apply_log_encoding(const float x, const float range_in_ev,
...
static inline void _compress_into_gamut(dt_aligned_pixel_t pixel_in_out)
{
    ...
    const float luminance_ratio =
        (y_new > y_compensate_negative && y_new > _epsilon) ? y_compensate_negative / y_new : 1.f;
                                    ...
                                    tone_mapping_params_t *curve_and_look_params)
{
    ...
    curve_and_look_params->range_in_ev =
        curve_and_look_params->white_relative_ev - curve_and_look_params->black_relative_ev;
}

static inline float _calculate_slope_gamma_compensation(const float gamma, const float pivot_y,
...
static tone_mapping_params_t _calculate_tone_mapping_params(const dt_iop_agx_params_t *p)
{
    ...
    tone_mapping_params.look_power =
        brightness < 1 ? 1.f / sqrtf(fmaxf(brightness, _epsilon)) : 1.f / brightness;
    tone_mapping_params.look_original_hue_mix_ratio = p->look_original_hue_mix_ratio;
    tone_mapping_params.look_tuned = p->look_slope != 1.f || p->look_brightness != 1.f ||
                                     p->look_lift != 0.f || p->look_saturation != 1.f;
    ...
    const float compensation_factor = _calculate_slope_gamma_compensation(
        tone_mapping_params.curve_gamma, tone_mapping_params.pivot_y, p);
    ...
    const float remaining_y_below_pivot =
        tone_mapping_params.pivot_y - tone_mapping_params.target_black;
    ...
    tone_mapping_params.toe_transition_x =
        fmaxf(_epsilon, tone_mapping_params.pivot_x - dx_linear_below_pivot);
    ...
    tone_mapping_params.toe_scale =
        -_scale(inverse_toe_limit_x, inverse_toe_limit_y, inverse_toe_transition_x,
                inverse_toe_transition_y, tone_mapping_params.slope, tone_mapping_params.toe_power);
    ...
    const float remaining_y_above_pivot =
        tone_mapping_params.target_white - tone_mapping_params.pivot_y;
    ...
    tone_mapping_params.shoulder_transition_y =
        tone_mapping_params.pivot_y + shoulder_dy_above_pivot;
    tone_mapping_params.shoulder_power = fmaxf(0.01f, p->curve_shoulder_power);

    const float shoulder_limit_x = 1.f;
    tone_mapping_params.shoulder_scale =
        _scale(shoulder_limit_x, tone_mapping_params.target_white,
    ...
    const float shoulder_dy_transition_to_limit = fmaxf(
        _epsilon, tone_mapping_params.target_white - tone_mapping_params.shoulder_transition_y);
    ...
    tone_mapping_params.shoulder_fallback_coefficient =
        _calculate_fallback_curve_coefficient(shoulder_length_x, shoulder_dy_transition_to_limit,
...
static void _update_pivot_x(const float old_black_ev, const float old_white_ev,
                            dt_iop_module_t *self, dt_iop_agx_params_t *const p)
{
...
static void _adjust_relative_exposure_from_exposure_params(dt_iop_module_t *self)
{
    ...
    p->range_black_relative_ev =
        CLAMPF((-8.f + 0.5f * exposure) * (1.f + p->dynamic_range_scaling), -20.f, -0.1f);
    p->range_white_relative_ev =
        CLAMPF((4.f + 0.8 * exposure) * (1.f + p->dynamic_range_scaling), 0.1f, 20.f);
                              ...
                              const dt_colormatrix_t rendering_to_xyz_transposed)
{
    ...
    for_three_channels(k, aligned(rgb_in_out, transformed_pixel : 16))
    {
        const float log_value =
            _apply_log_encoding(rgb_in_out[k], params->range_in_ev, params->black_relative_ev);
...
static void _apply_auto_black_exposure(const dt_iop_module_t *self)
{
    ...
    p->range_black_relative_ev =
        CLAMPF(log2f(fmaxf(_epsilon, black_norm) / 0.18f) * (1.f + p->dynamic_range_scaling), -20.f,
               -0.1f);
...
static void _apply_auto_white_exposure(const dt_iop_module_t *self)
{
    ...
    p->range_white_relative_ev = CLAMPF(
        log2f(fmaxf(_epsilon, white_norm) / 0.18f) * (1.f + p->dynamic_range_scaling), 0.1f, 20.f);
...
static void _read_exposure_params_callback(GtkWidget *widget, dt_iop_module_t *self)
{
...
static void _apply_auto_pivot_xy(dt_iop_module_t *self,
                                 const dt_iop_order_iccprofile_info_t *profile)
{
    ...
    const float picked_ev = CLAMPF(log2f(fmaxf(_epsilon, picked_input_luminance) / 0.18f),
                                   p->range_black_relative_ev, p->range_white_relative_ev);
    const float range = p->range_white_relative_ev - p->range_black_relative_ev;
...
static void _apply_auto_pivot_x(dt_iop_module_t *self,
                                const dt_iop_order_iccprofile_info_t *profile)
{
    dt_iop_agx_params_t *p = self->params;
    const dt_iop_agx_gui_data_t *g = self->gui_data;
                             ...
                             dt_colormatrix_t rendering_to_pipe_transposed)
{
    ...
    for (size_t i = 0; i < 3; i++)
    {
        ...
        dt_rotate_and_scale_primary(base_profile, scaling,
                                    params->master_unrotation_ratio * params->unrotation[i], i,
    ...
    dt_make_transposed_matrices_from_primaries_and_whitepoint(
        outset_and_unrotated_primaries, base_profile->whitepoint,
    ...
    dt_colormatrix_mul(rendering_to_pipe_transposed, rendering_to_base_transposed,
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    _create_matrices(&d->primaries_params, pipe_work_profile, base_profile,
                     rendering_profile.matrix_in_transposed, pipe_to_base_transposed,
                     base_to_rendering_transposed, rendering_to_pipe_transposed);
...
static gboolean _agx_draw_curve(GtkWidget *widget, cairo_t *crf, const dt_iop_module_t *self)
{
    ...
    const double dashes[] = {4.0 / darktable.gui->ppd,
                             4.0 / darktable.gui->ppd}; // 4px dash, 4px gap
    cairo_set_dash(cr, dashes, 2, 0);
    cairo_set_line_width(cr, DT_PIXEL_APPLY_DPI(0.5));

    const float linear_y_guides[] = {0.18f / 16.f, 0.18f / 8.f, 0.18f / 4.f, 0.18f / 2.f,
                                     0.18f,        0.18f * 2.f, 0.18f * 4.f, 1.f};
...
static void _update_curve_warnings(dt_iop_module_t *self)
{
    ...
    const gboolean warnings_enabled =
        dt_conf_get_bool("plugins/darkroom/agx/enable_curve_warnings");
    const tone_mapping_params_t params = _calculate_tone_mapping_params(p);

    dt_bauhaus_widget_set_quad_paint(
        g->basic_curve_controls.curve_toe_power,
        params.need_convex_toe && warnings_enabled ? dtgtk_cairo_paint_warning : NULL, CPF_ACTIVE,
        NULL);
    dt_bauhaus_widget_set_quad_paint(
        g->basic_curve_controls.curve_shoulder_power,
        params.need_concave_shoulder && warnings_enabled ? dtgtk_cairo_paint_warning : NULL,
        CPF_ACTIVE, NULL);
}
                                       ...
                                       const dt_iop_agx_params_t *const p)
{
    ...
    const gboolean post_curve_primaries_available =
        !p->completely_reverse_primaries && !p->disable_primaries_adjustments;
    gtk_widget_set_visible(g->post_curve_primaries_controls_vbox, post_curve_primaries_available);
    gtk_widget_set_sensitive(g->set_post_curve_primaries_from_pre_button,
                             post_curve_primaries_available);
...
static GtkWidget *_create_basic_curve_controls_box(dt_iop_module_t *self, dt_iop_agx_gui_data_t *g)
{
    ...
    slider = dt_color_picker_new(self, DT_COLOR_PICKER_AREA | DT_COLOR_PICKER_DENOISE,
                                 dt_bauhaus_slider_from_params(section, "curve_pivot_x"));
    ...
    gtk_widget_set_tooltip_text(slider,
                                _("set the pivot's input exposure in EV relative to mid-gray"));
    dt_bauhaus_widget_set_quad_tooltip(slider,
                                       _("the average luminance of the selected region will be\n"
    ...
    slider =
        dt_color_picker_new(self, DT_COLOR_PICKER_AREA | DT_COLOR_PICKER_DENOISE,
                            dt_bauhaus_slider_from_params(section, "curve_pivot_y_linear_output"));
    ...
    dt_bauhaus_widget_set_quad_tooltip(
        slider, _("shoulder power cannot be applied because the curve has lost its 'S' shape\n"
    ...
    dt_bauhaus_slider_set_soft_range(slider, 1.f, 5.f);
    gtk_widget_set_tooltip_text(slider,
                                _("contrast in shadows\n"
                                  "higher values keep the slope nearly constant for longer,\n"
                                  "at the cost of a more sudden drop near black"));
    dt_bauhaus_widget_set_quad_tooltip(
        slider, _("toe power cannot be applied because the curve has lost its 'S' shape due\n"
...
{
    const gboolean look_always_visible =
        dt_conf_get_bool("plugins/darkroom/agx/look_always_visible");
    ...
    {
        dt_gui_new_collapsible_section(&g->look_section, "plugins/darkroom/agx/expand_look_params",
                                       Q_(section_name), GTK_BOX(look_box), DT_ACTION(self));
...
static GtkWidget *_create_advanced_box(dt_iop_module_t *self, dt_iop_agx_gui_data_t *g)
{
    ...
    dt_gui_new_collapsible_section(&g->advanced_section,
                                   "plugins/darkroom/agx/expand_curve_advanced", Q_(section_name),
                                   GTK_BOX(advanced_box), DT_ACTION(self));
    dt_iop_module_t *section =
        DT_IOP_SECTION_FOR_PARAMS(self, NC_("section", "curve"), g->advanced_section.container);
    ...
    dt_bauhaus_slider_set_digits(slider, 2);
    dt_bauhaus_slider_set_factor(slider, 100.f);
    gtk_widget_set_tooltip_text(slider, _("length to keep curve linear above the pivot.\n"
                                          "may clip highlights"));
    ...
    dt_bauhaus_slider_set_digits(slider, 2);
    dt_bauhaus_slider_set_factor(slider, 100.f);
    gtk_widget_set_tooltip_text(slider, _("length to keep curve linear below the pivot.\n"
                                          "may crush shadows"));
    ...
    gtk_widget_set_tooltip_text(
        g->auto_gamma, _("adjusts the gamma automatically, trying to make sure\n"
    ...
    dt_bauhaus_slider_set_soft_range(slider, 1.f, 5.f);
    gtk_widget_set_tooltip_text(
        slider, _("shifts the representation (but not the output power) of the pivot\n"
                              ...
                              dt_iop_module_t *real_self)
{
    ...
    g->white_exposure_picker =
        dt_color_picker_new(self, DT_COLOR_PICKER_AREA | DT_COLOR_PICKER_DENOISE, white_slider);
    ...
    g->black_exposure_picker =
        dt_color_picker_new(self, DT_COLOR_PICKER_AREA | DT_COLOR_PICKER_DENOISE, black_slider);
    ...
    g->range_exposure_picker =
        dt_color_picker_new(self, DT_COLOR_PICKER_AREA | DT_COLOR_PICKER_DENOISE, NULL);
    gtk_widget_set_tooltip_text(g->range_exposure_picker,
                                _("set black and white relative exposure using the selected area"));
    dt_action_define_iop(real_self, N_("exposure range"), N_("auto tune levels"),
                         g->range_exposure_picker, &dt_action_def_toggle);
    ...
    gtk_widget_set_tooltip_text(g->btn_read_exposure,
                                _("read exposure from metadata and exposure module"));
    g_signal_connect(G_OBJECT(g->btn_read_exposure), "clicked",
                     G_CALLBACK(_read_exposure_params_callback), real_self);
    dt_action_define_iop(real_self, N_("exposure range"), N_("read exposure"), g->btn_read_exposure,
...
static void _primaries_popupmenu_callback(GtkWidget *button, dt_iop_module_t *self)
{
    ...
    g_signal_connect(blender_item, "activate", G_CALLBACK(_apply_primaries_from_menu_callback),
                     self);
    ...
    g_signal_connect(smooth_item, "activate", G_CALLBACK(_apply_primaries_from_menu_callback),
                     self);
    ...
    g_signal_connect(unmodified_item, "activate", G_CALLBACK(_apply_primaries_from_menu_callback),
                     self);
...
typedef void (*hsv_updater_t)(dt_aligned_pixel_t hsv_out, float position_on_slider, float hue_deg,
                              gboolean reverse_or_attenuate);

static void _update_hsv_for_hue(dt_aligned_pixel_t hsv_out, const float position_on_slider,
                                const float hue_deg, const gboolean reverse)
{
...
static void _update_hsv_for_purity(dt_aligned_pixel_t hsv_out, const float position_on_slider,
                                   const float hue_deg, const gboolean attenuate)
{
...
static void _paint_slider_gradient(GtkWidget *slider, const float hue_deg,
                                   const hsv_updater_t update_hsv,
                                   const gboolean attenuate_or_reverse)
{
...
static GtkWidget *_setup_purity_slider(dt_iop_module_t *self, const char *param_name,
                                       const char *tooltip, const int primary_index,
                                       const float hue_deg, const gboolean attenuate)
{
...
static GtkWidget *_setup_hue_slider(dt_iop_module_t *self, const char *param_name,
                                    const char *tooltip, const float hue_deg,
...
static void _create_primaries_page(dt_iop_module_t *main, dt_iop_agx_gui_data_t *g)
{
    ...
    gtk_widget_set_tooltip_text(primaries_button,
                                _("reset primaries to a predefined configuration"));
    g_signal_connect(primaries_button, "clicked", G_CALLBACK(_primaries_popupmenu_callback), main);
    dt_action_define_iop(main, NULL, N_("reset primaries"), primaries_button,
                         &dt_action_def_button);

    g->primaries_controls_vbox = dt_gui_vbox(
        dt_gui_hbox(dt_ui_label_new(_("reset primaries")), dt_gui_align_right(primaries_button)));
    ...
    slider = _setup_purity_slider(self, "red_inset",
                                  _("increase to desaturate reds in highlights faster"), _red_index,
                                  red_hue, TRUE);

    slider = _setup_hue_slider(self, "red_rotation",
                               _("shift the red primary towards yellow (+) or magenta (-)"),
                               red_hue, FALSE);

    slider = _setup_purity_slider(self, "green_inset",
                                  _("increase to desaturate greens in highlights faster"),
                                  _green_index, green_hue, TRUE);

    slider = _setup_hue_slider(self, "green_rotation",
                               _("shift the green primary towards cyan (+) or yellow (-)"),
                               green_hue, FALSE);

    slider = _setup_purity_slider(self, "blue_inset",
                                  _("increase to desaturate blues in highlights faster"),
                                  _blue_index, blue_hue, TRUE);

    slider = _setup_hue_slider(self, "blue_rotation",
    ...
    g->completely_reverse_primaries =
        dt_bauhaus_toggle_from_params(self, "completely_reverse_primaries");
    gtk_widget_set_tooltip_text(g->completely_reverse_primaries,
                                _("completely restore purity, undo all rotations, and hide\n"
    ...
    g_signal_connect(g->set_post_curve_primaries_from_pre_button, "clicked",
                     G_CALLBACK(_set_post_curve_primaries_from_pre_callback), main);
                                   ...
                                   dt_iop_module_t *self)
{
    ...
    {
        GtkWidget *target_container =
            (page_num == 0) ? gtk_widget_get_parent(g->range_exposure_picker_group) : page;
...
void gui_init(dt_iop_module_t *self)
{
    ...
    {
        curve_page_parent =
            dt_ui_notebook_page(g->notebook, N_("curve"), _("detailed curve settings"));
        // reparent on tab switch
        g_signal_connect(g->notebook, "switch-page", G_CALLBACK(_notebook_page_changed), self);
    }
    dt_gui_box_add(curve_page_parent, g->curve_graph_box, g->curve_advanced_controls_box);
...
static void _set_default_curve_and_look_params(dt_iop_agx_params_t *p)
{
    ...
    p->curve_pivot_x =
        -p->range_black_relative_ev / (p->range_white_relative_ev - p->range_black_relative_ev);
...
static void _set_blenderlike_params(dt_iop_agx_params_t *p)
{
    ...
    const float compensation_factor =
        _calculate_slope_gamma_compensation(p->curve_gamma, powf(0.18f, 1.f / p->curve_gamma), p);
...
void init_presets(dt_iop_module_so_t *self)
{
    ...
    dt_gui_presets_add_generic(_("unmodified base primaries"), self->op, self->version(), &p,
    ...
    _make_punchy(&p);
    dt_gui_presets_add_generic(_("blender-like|punchy"), self->op, self->version(), &p, sizeof(p),
                               TRUE, DEVELOP_BLEND_CS_RGB_SCENE);
    ...
    dt_gui_presets_add_generic(_("sigmoid-like|default"), self->op, self->version(), &p, sizeof(p),
                               TRUE, DEVELOP_BLEND_CS_RGB_SCENE);
    ...
    dt_gui_presets_add_generic(_("sigmoid-like|smooth"), self->op, self->version(), &p, sizeof(p),
                               TRUE, DEVELOP_BLEND_CS_RGB_SCENE);
    ...
    if (auto_apply_agx)
    {
        ...
        dt_gui_presets_add_generic(_("scene-referred default"), self->op, self->version(), &p,
    ...
    dt_gui_presets_add_generic(_("smooth"), self->op, self->version(), &p, sizeof(p), TRUE,
...
void color_picker_apply(dt_iop_module_t *self, GtkWidget *picker, dt_dev_pixelpipe_t *pipe)
{
    ...
    if (picker == g->black_exposure_picker)
        _apply_auto_black_exposure(self);
    else if (picker == g->white_exposure_picker)
        _apply_auto_white_exposure(self);
    else if (picker == g->range_exposure_picker)
        _apply_auto_tune_exposure(self);
    else if (picker == g->basic_curve_controls.curve_pivot_x)
        _apply_auto_pivot_x(self, dt_ioppr_get_pipe_work_profile_info(pipe));
    else if (picker == g->basic_curve_controls.curve_pivot_y_linear)
        _apply_auto_pivot_xy(self, dt_ioppr_get_pipe_work_profile_info(pipe));
...
{
    if (!dt_iop_have_required_input_format(4, self, piece->colors, (void *)dev_in, (void *)dev_out,
                                           roi_in, roi_out))
    {
    ...
    _create_matrices(&d->primaries_params, pipe_work_profile, base_profile,
                     rendering_to_xyz_transposed, pipe_to_base_transposed,
                     base_to_rendering_transposed, rendering_to_pipe_transposed);
    ...
    if (!dev_pipe_to_base || !dev_base_to_rendering || !dev_rendering_to_pipe ||
        !dev_rendering_to_xyz)
    {
    ...
    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_agx, width, height, CLARG(dev_in), CLARG(dev_out), CLARG(width),
        CLARG(height), CLARG(d->tone_mapping_params), CLARG(dev_pipe_to_base),
        CLARG(dev_base_to_rendering), CLARG(dev_rendering_to_pipe), CLARG(dev_rendering_to_xyz),
