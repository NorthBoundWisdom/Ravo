             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    for (size_t k = 0; k < npixels; k++)
    {
        out[4 * k + 0] = 100.0f - (((100.0f - in[4 * k]) * (100.0f - blurlightness[k])) /
                                   100.0f); // Screen blend
        out[4 * k + 1] = in[4 * k + 1];
        out[4 * k + 2] = in[4 * k + 2];
        out[4 * k + 3] = in[4 * k + 3];
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_bloom_threshold, width, height,
                                           ...
                                           CLARG(height), CLARG(scale), CLARG(threshold));
    if (err != CL_SUCCESS)
        goto error;
    ...
    if (radius != 0)
        for (int i = 0; i < BOX_ITERATIONS; i++)
        {
            ...
            err = dt_opencl_enqueue_kernel_2d_local_args(
                devid, gd->kernel_bloom_hblur, sizes, local, CLARG(dev_tmp1), CLARG(dev_tmp2),
                CLARG(radius), CLARG(width), CLARG(height), CLARG(hblocksize),
            ...
            dev_tmp1 = dev_tmp[bucket_next(&state, NUM_BUCKETS)];
            err = dt_opencl_enqueue_kernel_2d_local_args(
                devid, gd->kernel_bloom_vblur, sizes, local, CLARG(dev_tmp2), CLARG(dev_tmp1),
                CLARG(radius), CLARG(width), CLARG(height), CLARG(vblocksize),
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_bloom_mix, width, height,
                                           CLARG(dev_in), CLARG(dev_tmp1), CLARG(dev_out),
