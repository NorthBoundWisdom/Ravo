...
typedef struct dt_iop_lowpass_params_t
{
    ...
    dt_iop_lowpass_algo_t
        lowpass_algo; // $DEFAULT: LOWPASS_ALGO_GAUSSIAN $DESCRIPTION: "soften with"
...
{
    return dt_iop_set_description(self, _("isolate low frequencies in the image"), _("creative"),
                                  _("linear or non-linear, Lab, scene-referred"),
                                  _("frequential, Lab"), _("special, Lab, scene-referred"));
}
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    dev_ccoeffs =
        dt_opencl_copy_host_to_device_constant(devid, sizeof(float) * 3, d->cunbounded_coeffs);
    if (dev_ccoeffs == NULL)
        goto error;
    ...
    dev_lcoeffs =
        dt_opencl_copy_host_to_device_constant(devid, sizeof(float) * 3, d->lunbounded_coeffs);
    if (dev_lcoeffs == NULL)
        goto error;

    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_lowpass_mix, width, height, CLARG(dev_tmp), CLARG(dev_out), CLARG(width),
        CLARG(height), CLARG(saturation), CLARG(dev_cm), CLARG(dev_ccoeffs), CLARG(dev_lm),
        CLARG(dev_lcoeffs), CLARG(unbound));
                   ...
                   dt_dev_pixelpipe_iop_t *piece)
{
    ...
    if (fabs(d->contrast) <= 1.0f)
    {
        ...
        for (int k = 0; k < 0x10000; k++)
            d->ctable[k] = d->contrast * (100.0f * k / 0x10000 - 50.0f) + 50.0f;
    }
    else
    {
        ...
        for (size_t k = 0; k < 0x10000; k++)
        {
            ...
            ctable[k] =
                50.0f * (contrastscale * kx2m1 / sqrtf(1.0f + contrastm1sq * kx2m1 * kx2m1) + 1.0f);
        }
    }
    ...
    const float gamma =
        (d->brightness >= 0.0f) ? 1.0f / (1.0f + d->brightness) : (1.0f - d->brightness);
...
void init_pipe(dt_iop_module_t *self, dt_dev_pixelpipe_t *pipe, dt_dev_pixelpipe_iop_t *piece)
{
    ...
    piece->data = (void *)d;
    for (int k = 0; k < 0x10000; k++)
        d->ctable[k] = d->ltable[k] = 100.0f * k / 0x10000; // identity
}
