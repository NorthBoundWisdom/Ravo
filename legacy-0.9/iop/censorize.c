...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self, _("censorize license plates and body parts for privacy"),
                                  _("creative"), _("linear or non-linear, RGB, scene-referred"),
                                  _("frequential, RGB"), _("special, RGB, scene-referred"));
}
...
static inline void make_noise(float *const output, const float noise, const size_t width,
                              const size_t height)
{
    ...
    for (size_t i = 0; i < height; i++)
        for (size_t j = 0; j < width; j++)
        {
            ...
            for (size_t c = 0; c < 3; c++)
                pix_out[c] = fmaxf(pix_out[c] * epsilon, 0.f);
        }
}
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    float *restrict temp;
    if (!dt_iop_alloc_image_buffers(self, roi_in, roi_out, 4 | DT_IMGSZ_INPUT, &temp, 0, NULL))
    {
            ...
            for (size_t i = 0; i < pixels_x + 1; i++)
            {
                ...
                for (size_t k = 0; k < 5; k++)
                {
                    const float *const restrict pix_in =
                        DT_IS_ALIGNED_PIXEL(input + (width * box[k].y + box[k].x) * 4);
                    for_four_channels(c) RGB[c] += pix_in[c] / 5.f;
                }
                ...
                for (size_t jj = tl.y; jj < br.y; jj++)
                    for (size_t ii = tl.x; ii < br.x; ii++)
                    {
                        float *const restrict pix_out =
                            DT_IS_ALIGNED_PIXEL(output + (jj * width + ii) * 4);
                        for_four_channels(c) pix_out[c] = RGB[c];
                    }
            }
