                   ...
                   dt_dev_pixelpipe_iop_t *piece)
{
    ...
    g_hash_table_insert(self->raster_mask.source.masks, GINT_TO_POINTER(mask_id),
                        g_strdup(mask_name));
}
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    if (dt_iop_piece_is_raster_mask_used(piece, mask_id))
    {
        ...
        if (!dt_iop_alloc_image_buffers(module, roi_in, roi_out,
                                        1 /*ch per pixel*/ | DT_IMGSZ_OUTPUT | DT_IMGSZ_FULL |
                                            DT_IMGSZ_CLEARBUF,
                                        &mask, 0 /* end of list of buffers to allocate */))
        {
    ...
    for (int j = 0; j < roi_out->height; j++)
    {
        float *in = ((float *)ivoid) +
                    (size_t)ch * roi_in->width * j; // make sure to address input, output and temp
