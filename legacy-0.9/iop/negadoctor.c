...
{
    dt_iop_negadoctor_filmstock_t
        film_stock;   /* $DEFAULT: DT_FILMSTOCK_COLOR $DESCRIPTION: "film stock" */
...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self,
                                  _("invert film negative scans and simulate printing on paper"),
...
static inline void _process_pixel(const dt_aligned_pixel_t pix_in, dt_aligned_pixel_t pix_out,
                                  const dt_aligned_pixel_t Dmin, const dt_aligned_pixel_t wb_high,
                                  const dt_aligned_pixel_t offset, const dt_aligned_pixel_t black,
                                  const dt_aligned_pixel_t exposure, const dt_aligned_pixel_t gamma,
...
#define LOG2_to_LOG10 0.3010299956f
    for_each_channel(c) log_density[c] *= -LOG2_to_LOG10;
    ...
    for_each_channel(c) clipped_gamma[c] = -(print_gamma[c] - soft_clip[c]) / soft_clip_comp[c];
             ...
             const dt_iop_roi_t *const restrict roi_in, const dt_iop_roi_t *const restrict roi_out)
{
    ...
    for (size_t k = 0; k < (size_t)roi_out->height * roi_out->width * 4; k += 4)
    {
        ...
        _process_pixel(pix_in, pix_out, Dmin, wb_high, offset, black, exposure, gamma, soft_clip,
                       soft_clip_comp);
    }
}

#ifdef HAVE_OPENCL
int process_cl(dt_iop_module_t *const self, dt_dev_pixelpipe_iop_t *const piece, cl_mem dev_in,
               cl_mem dev_out, const dt_iop_roi_t *const restrict roi_in,
               const dt_iop_roi_t *const restrict roi_out)
{
    ...
    return dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_negadoctor, width, height, CLARG(dev_in), CLARG(dev_out), CLARG(width),
        CLARG(height), CLARG(d->Dmin), CLARG(d->wb_high), CLARG(d->offset), CLARG(d->exposure),
        CLARG(d->black), CLARG(d->gamma), CLARG(d->soft_clip), CLARG(d->soft_clip_comp));
}
#endif
...
{
    dt_iop_negadoctor_params_t tmp =
        (dt_iop_negadoctor_params_t){.film_stock = DT_FILMSTOCK_COLOR,
    ...
    dt_gui_presets_add_generic(_("color film"), self->op, self->version(), &tmp, sizeof(tmp), TRUE,
                               DEVELOP_BLEND_CS_RGB_DISPLAY);

    dt_iop_negadoctor_params_t tmq =
        (dt_iop_negadoctor_params_t){.film_stock = DT_FILMSTOCK_NB,
    ...
    dt_gui_presets_add_generic(_("black and white film"), self->op, self->version(), &tmq,
                               sizeof(tmq), TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
}
...
static void apply_auto_WB_high(dt_iop_module_t *self)
{
    ...
    for (int c = 0; c < 3; c++)
        RGB_min[c] = fabsf(
            -1.0f / (p->offset * p->wb_low[c] -
                     log10f(p->Dmin[c] / fmaxf(self->picked_color[c], THRESHOLD)) / p->D_max));
...
static void apply_auto_black(dt_iop_module_t *self)
{
    ...
    for (int c = 0; c < 3; c++)
    {
        ...
        RGB[c] =
            ...
            (1.0f -
             fast_exp10f(
                 RGB[c])); // actually, remap between -3.32 EV and infinity for safety because gamma comes later
    }
    p->black = v_maxf(RGB);
...
static void apply_auto_exposure(dt_iop_module_t *self)
{
    ...
    for (int c = 0; c < 3; c++)
    {
        ...
        RGB[c] = 0.96f /
                 (1.0f - fast_exp10f(RGB[c]) + p->black); // actually, remap in [0; 0.96] for safety
    }
...
void gui_init(dt_iop_module_t *self)
{
    ...
    gtk_color_button_set_title(GTK_COLOR_BUTTON(g->Dmin_picker),
                               _("select color of film material from a swatch"));
    ...
    dt_action_define_iop(self, N_("pickers"), N_("film material"), g->Dmin_sampler,
    ...
    dt_bauhaus_widget_set_label(g->Dmin_R, NULL, N_("D min red component"));
    gtk_widget_set_tooltip_text(g->Dmin_R,
    ...
    gtk_widget_set_tooltip_text(g->Dmin_G,
                                _("adjust the color and shade of the film transparent base.\n"
                                  "this value depends on the film material, \n"
                                  "the chemical fog produced while developing the film,\n"
                                  "and the scanner white balance."));
    ...
    gtk_widget_set_tooltip_text(g->Dmin_B,
                                _("adjust the color and shade of the film transparent base.\n"
                                  "this value depends on the film material, \n"
                                  "the chemical fog produced while developing the film,\n"
                                  "and the scanner white balance."));
    ...
    gtk_color_button_set_title(GTK_COLOR_BUTTON(g->WB_low_picker),
                               _("select color of shadows from a swatch"));
    g_signal_connect(G_OBJECT(g->WB_low_picker), "color-set", G_CALLBACK(WB_low_picker_callback),
                     self);
    ...
    dt_action_define_iop(self, N_("pickers"), N_("shadows"), g->WB_low_sampler,
                         &dt_action_def_toggle);
    ...
    gtk_widget_set_tooltip_text(g->wb_low_R,
    ...
    gtk_widget_set_tooltip_text(g->wb_low_G,
                                _("correct the color cast in shadows so blacks are\n"
                                  "truly achromatic. Setting this value before\n"
                                  "the highlights illuminant white balance will help\n"
                                  "recovering the global white balance in difficult cases."));
    ...
    gtk_widget_set_tooltip_text(g->wb_low_B,
                                _("correct the color cast in shadows so blacks are\n"
                                  "truly achromatic. Setting this value before\n"
                                  "the highlights illuminant white balance will help\n"
                                  "recovering the global white balance in difficult cases."));
    ...
    gtk_color_button_set_title(GTK_COLOR_BUTTON(g->WB_high_picker),
                               _("select color of illuminant from a swatch"));
    g_signal_connect(G_OBJECT(g->WB_high_picker), "color-set", G_CALLBACK(WB_high_picker_callback),
                     self);
    ...
    dt_action_define_iop(self, N_("pickers"), N_("illuminant"), g->WB_high_sampler,
                         &dt_action_def_toggle);
    ...
    gtk_widget_set_tooltip_text(g->wb_high_R,
                                _("correct the color of the illuminant so whites are\n"
    ...
    gtk_widget_set_tooltip_text(g->wb_high_G,
                                _("correct the color of the illuminant so whites are\n"
                                  "truly achromatic. Setting this value after\n"
                                  "the shadows color cast will help\n"
                                  "recovering the global white balance in difficult cases."));
    ...
    gtk_widget_set_tooltip_text(g->wb_high_B,
                                _("correct the color of the illuminant so whites are\n"
                                  "truly achromatic. Setting this value after\n"
                                  "the shadows color cast will help\n"
                                  "recovering the global white balance in difficult cases."));
    ...
    GtkWidget *page3 = self->widget =
        dt_ui_notebook_page(g->notebook, N_("print properties"), NULL);
    ...
    gtk_widget_set_tooltip_text(g->gamma,
                                _("select the grade of the virtual paper, which is actually\n"
    ...
    gtk_widget_set_tooltip_text(g->soft_clip,
                                _("gradually compress specular highlights past this value\n"
    ...
    gtk_widget_set_tooltip_text(g->exposure,
                                _("correct the printing exposure after inversion to adjust\n"
...
void gui_update(dt_iop_module_t *const self)
{
    ...
    dt_bauhaus_slider_set_default(g->exposure,
                                  log2f(p->exposure)); // otherwise always showes as "changed"
