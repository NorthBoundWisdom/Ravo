...
typedef struct dt_iop_rawprepare_params_t
{
    ...
    dt_iop_rawprepare_flat_field_t
        flat_field; // $DEFAULT: FLAT_FIELD_OFF $DESCRIPTION: "flat field correction"
...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self,
                                  _("sets technical specificities of the raw sensor\n\n"
                                    "touch with great care!"),
                                  _("mandatory"), _("linear, raw, scene-referred"),
...
static int _compute_proper_crop(dt_dev_pixelpipe_iop_t *piece, const dt_iop_roi_t *const roi_in,
                                const int value)
{
...
static void _adjust_xtrans_filters(dt_dev_pixelpipe_t *pipe, const uint32_t crop_x,
...
static int _BL(const dt_iop_roi_t *const roi_out, const dt_iop_rawprepare_data_t *const d,
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    if (piece->pipe->dsc.filters && piece->dsc_in.channels == 1 && d->apply_gainmaps)
    {
        ...
        for (int j = 0; j < roi_out->height; j++)
        {
            const float y_map = CLAMP(
                ((roi_out->y + csy + j) * im_to_rel_y - map_origin_v) * rel_to_map_y, 0, map_h);
            ...
            {
                const int id = _BL(roi_out, d, j, i);
                const float x_map = CLAMP(
                    ((roi_out->x + csx + i) * im_to_rel_x - map_origin_h) * rel_to_map_x, 0, map_w);
                const uint32_t x_i0 = MIN(x_map, map_w - 1);
                const uint32_t x_i1 = MIN(x_i0 + 1, map_w - 1);
                const float x_frac = x_map - x_i0;
                const float gain_top =
                    (1.0f - x_frac) * map_row0[id][x_i0] + x_frac * map_row0[id][x_i1];
                const float gain_bottom =
                    (1.0f - x_frac) * map_row1[id][x_i0] + x_frac * map_row1[id][x_i1];
                out[j * roi_out->width + i] *= (1.0f - y_frac) * gain_top + y_frac * gain_bottom;
            }
        }
    }

    const gboolean color_sraw =
        !(dt_image_is_raw(&piece->pipe->image) || dt_image_is_mono_sraw(&piece->pipe->image));
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    dt_opencl_set_kernel_args(devid, kernel, 0, CLARG(dev_in), CLARG(dev_out), CLARG(width),
                              CLARG(height), CLARG(csx), CLARG(csy), CLARG(dev_sub), CLARG(dev_div),
    ...
    if (gainmap_args)
    {
        ...
        dt_opencl_set_kernel_args(devid, kernel, 10, CLARG(dev_gainmap[0]), CLARG(dev_gainmap[1]),
                                  CLARG(dev_gainmap[2]), CLARG(dev_gainmap[3]), CLARG(map_size),
                                  CLARG(im_to_rel), CLARG(rel_to_map), CLARG(map_origin));
    }
...
static gboolean _image_set_rawcrops(dt_iop_module_t *self, const dt_imgid_t imgid, const int left,
                                    const int right, const int top, const int bottom)
{
    ...
    const gboolean testdim = (img->p_width == img->width - left - right) &&
                             (img->p_height == img->height - top - bottom);

    if (testdim && cropvalid)
        return FALSE;
    ...
    if (!cropvalid)
    {
        ...
        dt_iop_set_module_trouble_message(
            self, _("invalid crop parameters"),
...
static gboolean _check_gain_maps(dt_iop_module_t *self, dt_dng_gain_map_t **gainmaps_out)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        if (g == NULL || g->plane != 0 || g->planes != 1 || g->map_planes != 1 ||
            g->row_pitch != 2 || g->col_pitch != 2 || g->map_points_v < 2 || g->map_points_h < 2 ||
            g->top > 1 || g->left > 1 || g->bottom != image->height || g->right != image->width)
            return FALSE;
...
void init_global(dt_iop_module_so_t *self)
{
    ...
    gd->kernel_rawprepare_1f_unnormalized =
        dt_opencl_create_kernel(program, "rawprepare_1f_unnormalized");
    gd->kernel_rawprepare_1f_unnormalized_gainmap =
        dt_opencl_create_kernel(program, "rawprepare_1f_unnormalized_gainmap");
...
void gui_changed(dt_iop_module_t *self, GtkWidget *w, void *previous)
{
    ...
    if (is_sraw && !is_monochrome)
    {
        if (w == g->black_level_separate[0] || w == g->black_level_separate[1] ||
            w == g->black_level_separate[2])
        {
            const float sum = (float)p->raw_black_level_separate[0] +
                              (float)p->raw_black_level_separate[1] +
...
const gchar *black_label[] = {N_("black level 0"), N_("black level 1"), N_("black level 2"),
                              N_("black level 3")};
...
void gui_init(dt_iop_module_t *self)
{
    ...
    gtk_widget_set_tooltip_text(g->flat_field,
    ...
    GtkWidget *label_non_raw = dt_ui_label_new(
        _("raw black/white point correction\nonly works for the sensors that need it."));
