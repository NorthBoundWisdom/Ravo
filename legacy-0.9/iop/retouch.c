...
{
    dt_iop_retouch_form_data_t
        rt_forms[RETOUCH_NO_FORMS]; // array of masks index and additional data
    ...
    dt_iop_retouch_blur_types_t
        blur_type; // $DEFAULT: DT_IOP_RETOUCH_BLUR_GAUSSIAN $DESCRIPTION: "blur type" gaussian, bilateral
    float
        blur_radius; // $MIN: 0.1 $MAX: 200.0 $DEFAULT: 10.0 $DESCRIPTION: "blur radius" radius for blur algorithm

    dt_iop_retouch_fill_modes_t
        fill_mode; // $DEFAULT: DT_IOP_RETOUCH_FILL_ERASE $DESCRIPTION: "fill mode" mode for fill algorithm, erase or fill with color
    float fill_color[3]; // $DEFAULT: 0.0 color for fill algorithm
    float
        fill_brightness; // $MIN: -1.0 $MAX: 1.0 $DESCRIPTION: "brightness" value to be added to the color
...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self,
                                  _("remove and clone spots,\n"
                                    "perform split-frequency skin editing"),
                                  _("corrective"), _("linear, RGB, scene-referred"),
...
{
    GdkRGBA c = (GdkRGBA){
        .red = p->fill_color[0], .green = p->fill_color[1], .blue = p->fill_color[2], .alpha = 1.0};
...
static void rt_masks_form_change_opacity(dt_iop_module_t *self, const dt_mask_id_t formid,
                                         const float opacity)
{
    dt_masks_point_group_t *grpt = rt_get_mask_point_group(self, formid);
    if (grpt)
...
static void rt_paste_forms_from_scale(dt_iop_retouch_params_t *p, const int source_scale,
...
static void rt_reset_form_creation(GtkWidget *widget, const dt_iop_module_t *self)
{
...
{
    if (!self->enabled || darktable.develop->gui_module != self ||
        darktable.develop->form_gui->creation || darktable.develop->form_gui->creation_continuous)
        return;
    ...
    {
        if (dt_is_valid_maskid(p->rt_forms[i].formid) && p->rt_forms[i].scale == scale)
            count++;
    }
...
static void rt_resynch_params(dt_iop_module_t *self)
{
    ...
    if (grp && (grp->type & DT_MASKS_GROUP))
    {
        ...
        for (GList *forms = grp->points; (new_form_index < RETOUCH_NO_FORMS) && forms;
                                        ...
                                        const dt_iop_roi_t *roi_out)
{
    ...
    if (ft >= roi_out->y + roi_out->height || ft + fh <= roi_out->y ||
        fl >= roi_out->x + roi_out->width || fl + fw <= roi_out->x)
        return FALSE;
...
static void rt_clamp_minmax(float levels_old[3], float levels_new[3])
{
    ...
    if ((levels_old[0] != levels_new[0] || levels_old[2] != levels_new[2]) &&
...
static gboolean rt_add_shape(GtkWidget *widget, const int creation_continuous,
                             dt_iop_module_t *self)
{
...
static void rt_update_wd_bar_labels(const dt_iop_retouch_params_t *p, dt_iop_retouch_gui_data_t *g)
{
    char text[256];
...
static void rt_merge_from_scale_update(const int _merge_from_scale, dt_iop_module_t *self)
{
    ...
    if (p->merge_from_scale == merge_from_scale)
        return;
                                       ...
                                       dt_iop_module_t *self)
{
    ...
    if (g->wdbar_mouse_y <= sh)
    {
        ...
        g->upper_cursor =
            (g->wdbar_mouse_x >= (middle - inset)) && (g->wdbar_mouse_x <= (middle + inset));
        if (!(g->is_dragging))
            g->curr_scale = -1;
    }
...
static gboolean rt_wdbar_draw(GtkWidget *widget, cairo_t *crf, const dt_iop_module_t *self)
{
    dt_iop_retouch_gui_data_t *g = self->gui_data;
    dt_iop_retouch_params_t *p = self->params;
    ...
    float middle;
    const int first_scale_visible =
        (g->first_scale_visible > 0) ? g->first_scale_visible : RETOUCH_MAX_SCALES;
    ...
    for (int i = 0; i < RETOUCH_NO_SCALES; i++)
    {
        ...
        if (rt_scale_has_shapes(p, i))
        {
            ...
            cairo_rectangle(cr, box_w * i + inset + lw / 2.0f, allocation.height - sh, box_w - lw,
                            2.0f * lw);
            cairo_fill(cr);
        }
    ...
    if (p->curr_scale >= p->merge_from_scale && p->curr_scale <= p->num_scales &&
    ...
    if (g->curr_scale >= 0)
    {
        ...
        if (g->curr_scale == p->num_scales + 1)
            ...
            color = residual;
        gdk_cairo_set_source_rgba(cr, &color);
        cairo_rectangle(cr, box_w * g->curr_scale + inset + lw, sh + lw, box_w - 2.0f * lw,
                        box_h - 2.0f * lw);
        cairo_stroke(cr);
    }
    ...
    if (g->lower_cursor || g->is_dragging == DT_IOP_RETOUCH_WDBAR_DRAG_BOTTOM)
    {
        ...
        dtgtk_cairo_paint_solid_triangle(cr, middle, box_h + 5.0f * lw, mk, mk, CPF_DIRECTION_UP,
                                         NULL);
    }
    else
    {
...
static float rt_gslider_scale_callback(GtkWidget *self, const float inval, const int dir)
{
    float outval;
    switch (dir)
    {
    case GRADIENT_SLIDER_SET:
        outval =
            (inval - RETOUCH_PREVIEW_LVL_MIN) / (RETOUCH_PREVIEW_LVL_MAX - RETOUCH_PREVIEW_LVL_MIN);
        break;
    case GRADIENT_SLIDER_GET:
        outval =
            (RETOUCH_PREVIEW_LVL_MAX - RETOUCH_PREVIEW_LVL_MIN) * inval + RETOUCH_PREVIEW_LVL_MIN;
        break;
    default:
        outval = inval;
    }
...
static void rt_develop_ui_pipe_finished_callback(gpointer instance, dt_iop_module_t *self)
{
    dt_iop_retouch_params_t *p = self->params;
    dt_iop_retouch_gui_data_t *g = self->gui_data;
...
void gui_init(dt_iop_module_t *self)
{
    ...
    gtk_widget_set_tooltip_text(g->bt_blur,
                                dt_buf_printf(b, "%s\n%s\n%s", _("activate blur tool"), c, s));
    gtk_widget_set_tooltip_text(g->bt_fill,
                                dt_buf_printf(b, "%s\n%s\n%s", _("activate fill tool"), c, s));
    gtk_widget_set_tooltip_text(g->bt_clone,
                                dt_buf_printf(b, "%s\n%s\n%s", _("activate cloning tool"), c, s));
    gtk_widget_set_tooltip_text(g->bt_heal,
...
#define NEUTRAL_GRAY 0.5
    static const GdkRGBA _gradient_L[] = {{0, 0, 0, 1.0},
                                          {NEUTRAL_GRAY, NEUTRAL_GRAY, NEUTRAL_GRAY, 1.0}};
    GtkDarktableGradientSlider *gslider = g->preview_levels_gslider =
        DTGTK_GRADIENT_SLIDER_MULTIVALUE(dtgtk_gradient_slider_multivalue_new_with_color_and_name(
    ...
    dtgtk_gradient_slider_multivalue_set_marker(gslider, GRADIENT_SLIDER_MARKER_LOWER_FILLED_BIG,
                                                1);
    ...
    gtk_widget_set_tooltip_text(
        hbox_shape_sel, _("click on a shape to select it,\nto unselect click on an empty space"));
    ...
    g->colorpicker = dt_color_picker_new(self, DT_COLOR_PICKER_POINT | DT_COLOR_PICKER_IO, NULL);
    ...
    self->widget = dt_gui_vbox(
        dt_ui_section_label_new(C_("section", "retouch tools")), hbox_shapes, hbox_algo,
        dt_ui_section_label_new(C_("section", "wavelet decompose")), grid_wd_labels, g->wd_bar,
        dt_gui_hbox(scale_start, dt_gui_expand(scale_middle), dt_gui_expand(scale_end)),
        g->vbox_preview_scale, dt_ui_section_label_new(C_("section", "shapes")), hbox_shape_sel,
        g->vbox_blur, g->vbox_fill, g->sl_mask_opacity);
...
static void rt_compute_roi_in(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
        ...
        for (const GList *forms = grp->points; forms; forms = g_list_next(forms))
        {
            ...
            if (grpt)
            {
                ...
                if (form)
                {
                    ...
                    if (ft >= roi_in->y + roi_in->height || ft + fh <= roi_in->y ||
                        fl >= roi_in->x + roi_in->width || fl + fw <= roi_in->x)
                    {
                        continue;
                    }
                    ...
                    if (p->rt_forms[index].algorithm == DT_IOP_RETOUCH_BLUR)
                    {
                        if (index >= 0)
                        {
                            const int overlap = ceilf(4 * (p->rt_forms[index].blur_radius *
...
static void rt_extend_roi_in_from_source_clones(dt_iop_module_t *self,
                                                dt_dev_pixelpipe_iop_t *piece, dt_iop_roi_t *roi_in,
                                                const dt_mask_id_t formid_src, const int fl_src,
                                                const int ft_src, const int fw_src,
                                                const int fh_src, int *_roir, int *_roib,
                                                int *_roix, int *_roiy)
{
    dt_iop_retouch_params_t *p = piece->data;
    dt_develop_blend_params_t *bp = piece->blendop_data;
        ...
        for (const GList *forms = grp->points; forms; forms = g_list_next(forms))
        {
            ...
            if (grpt)
            {
                ...
                if (form)
                {
                    ...
                    const int intersects = !(ft_dest + fh < ft_src || ft_src + fh_src < ft_dest ||
                                             fl_dest + fw < fl_src || fl_src + fw_src < fl_dest);
                    if (intersects)
                    {
                        // we enlarge the roi if needed
                        roiy = fminf(ft, roiy);
...
static void rt_extend_roi_in_for_clone(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                                       dt_iop_roi_t *roi_in, int *_roir, int *_roib, int *_roix,
                                       int *_roiy)
{
    dt_iop_retouch_params_t *p = piece->data;
    dt_develop_blend_params_t *bp = piece->blendop_data;
    ...
    if (grp && (grp->type & DT_MASKS_GROUP))
    {
        for (const GList *forms = grp->points; forms; forms = g_list_next(forms))
        {
            ...
            if (grpt)
            {
                ...
                if (!dt_masks_get_source_area(self, piece, form, &fw_src, &fh_src, &fl_src,
                                              &ft_src))
                {
                    continue;
                }
                ...
                const int intersects = !(roib < ft_src || ft_src + fh_src < roiy || roir < fl_src ||
                                         fl_src + fw_src < roix);
                if (intersects)
                    rt_extend_roi_in_from_source_clones(self, piece, roi_in, formid, fl_src, ft_src,
                                                        fw_src, fh_src, &roir, &roib, &roix, &roiy);
            }
        }
    }
...
static void rt_process_stats(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
...
static void rt_adjust_levels(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece, float *img_src,
                             const int width, const int height, const int ch, const float levels[3])
{
    ...
    if (left == RETOUCH_PREVIEW_LVL_MIN && middle == 0.f && right == RETOUCH_PREVIEW_LVL_MAX)
        return;
...
static void rt_intersect_2_rois(dt_iop_roi_t *const roi_1, dt_iop_roi_t *const roi_2, const int dx,
                                const int dy, const int padding, dt_iop_roi_t *roi_dest)
{
    const int x_from = MAX(MAX((roi_1->x + 1 - padding), roi_2->x), (roi_2->x + dx));
    const int x_to = MIN(MIN((roi_1->x + roi_1->width + 1 + padding), roi_2->x + roi_2->width),
...
static void rt_copy_in_to_out(const float *const in, const dt_iop_roi_t *const roi_in,
                              float *const out, const dt_iop_roi_t *const roi_out, const int ch,
...
static gboolean rt_build_scaled_mask(float *const mask, dt_iop_roi_t *const roi_mask,
                                     float **mask_scaled, dt_iop_roi_t *roi_mask_scaled,
                                     dt_iop_roi_t *const roi_in, const int dx, const int dy,
                                     const int algo)
{
    float *mask_tmp = NULL;
    ...
    *roi_mask_scaled = *roi_mask;
    ...
    for (int yy = roi_mask_scaled->y; yy < y_to; yy++)
    {
        ...
        if (mask_index < 0 || mask_index >= roi_mask->height)
            continue;
...
static void rt_copy_image_masked(float *const img_src, float *img_dest,
                                 dt_iop_roi_t *const roi_dest, float *const mask_scaled,
                                 dt_iop_roi_t *const roi_mask_scaled, const float opacity)
{
    ...
    for (int yy = 0; yy < roi_mask_scaled->height; yy++)
    {
        ...
        const int dest_index = 4 * (((yy + roi_mask_scaled->y - roi_dest->y) * roi_dest->width) +
                                    (roi_mask_scaled->x - roi_dest->x));
                                  ...
                                  const float opacity)
{
    ...
    {
        const int mask_index = yy * roi_mask_scaled->width;
        const int dest_index = (((yy + roi_mask_scaled->y - roi_img->y) * roi_img->width) +
                                (roi_mask_scaled->x - roi_img->x)) *
                               ch;
                          ...
                          const float *const fill_color)
{
    ...
    {
        const int mask_index = yy * roi_mask_scaled->width;
        const int dest_index = (((yy + roi_mask_scaled->y - roi_in->y) * roi_in->width) +
                                (roi_mask_scaled->x - roi_in->x)) *
                               4;
        ...
        for (int xx = 0; xx < roi_mask_scaled->width; xx++)
        {
            ...
            for_each_channel(c, aligned(d, fill_color)) d[4 * xx + c] =
                d[4 * xx + c] * (1.0f - f) + fill_color[c] * f;
        }
    }
}

static void _retouch_clone(float *const in, dt_iop_roi_t *const roi_in, float *const mask_scaled,
                           dt_iop_roi_t *const roi_mask_scaled, const int dx, const int dy,
                           const float opacity)
{
...
static void _retouch_blur(dt_iop_module_t *self, float *const in, dt_iop_roi_t *const roi_in,
                          float *const mask_scaled, dt_iop_roi_t *const roi_mask_scaled,
                          const float opacity, const int blur_type, const float blur_radius,
                          dt_dev_pixelpipe_iop_t *piece)
{
    if (fabsf(blur_radius) <= 0.1f)
        return;
    ...
    if (blur_type == DT_IOP_RETOUCH_BLUR_GAUSSIAN && fabsf(blur_radius) > 0.1f)
    {
        ...
        dt_gaussian_t *g = dt_gaussian_init(roi_mask_scaled->width, roi_mask_scaled->height, 4,
                                            Labmax, Labmin, sigma, DT_IOP_GAUSSIAN_ZERO);
        if (g)
        {
        ...
        if (b)
        {
            ...
            if (work_profile)
                dt_ioppr_transform_image_colorspace(
                    self, img_dest, img_dest, roi_mask_scaled->width, roi_mask_scaled->height,
                    IOP_CS_RGB, IOP_CS_LAB, &converted_cst, work_profile);
            else
            ...
            if (work_profile)
                dt_ioppr_transform_image_colorspace(
                    self, img_dest, img_dest, roi_mask_scaled->width, roi_mask_scaled->height,
                    IOP_CS_LAB, IOP_CS_RGB, &converted_cst, work_profile);
            else
...
static void _retouch_heal(float *const in, dt_iop_roi_t *const roi_in, float *const mask_scaled,
                          dt_iop_roi_t *const roi_mask_scaled, const int dx, const int dy,
                          const float opacity, const int max_iter)
{
...
static void rt_process_forms(float *layer, dwt_params_t *const wt_p, const int scale1)
{
    ...
    if (wt_p->merge_from_scale == 0 && wt_p->return_layer > 0 && scale != wt_p->return_layer &&
        scale != 0)
        return;
    // do not process the reconstructed image
    if (scale > wt_p->scales + 1)
        return;
    ...
    if (!usr_d->suppress_mask)
    {
        ...
        if (grp && (grp->type & DT_MASKS_GROUP))
        {
            for (const GList *forms = grp->points; forms; forms = g_list_next(forms))
            {
                ...
                rt_build_scaled_mask(mask, &roi_mask, &mask_scaled, &roi_mask_scaled, roi_layer, dx,
                                     dy, algo);
                ...
                if ((dx != 0 || dy != 0 || algo == DT_IOP_RETOUCH_BLUR ||
                     algo == DT_IOP_RETOUCH_FILL) &&
                    ((roi_mask_scaled.width > 2) && (roi_mask_scaled.height > 2)))
                {
                    if (algo == DT_IOP_RETOUCH_CLONE)
                    {
                        _retouch_clone(layer, roi_layer, mask_scaled, &roi_mask_scaled, dx, dy,
                                       form_opacity);
                    }
                    else if (algo == DT_IOP_RETOUCH_HEAL)
                    {
                        _retouch_heal(layer, roi_layer, mask_scaled, &roi_mask_scaled, dx, dy,
                                      form_opacity, p->max_heal_iter);
                    }
                    else if (algo == DT_IOP_RETOUCH_BLUR)
                    {
                        _retouch_blur(self, layer, roi_layer, mask_scaled, &roi_mask_scaled,
                                      form_opacity, p->rt_forms[index].blur_type,
                    ...
                    else if (algo == DT_IOP_RETOUCH_FILL)
                    {
                        ...
                        {
                            fill_color[0] = p->rt_forms[index].fill_color[0] +
                                            p->rt_forms[index].fill_brightness;
                            fill_color[1] = p->rt_forms[index].fill_color[1] +
                                            p->rt_forms[index].fill_brightness;
                            fill_color[2] = p->rt_forms[index].fill_color[2] +
                                            p->rt_forms[index].fill_brightness;
                        }
                        fill_color[3] = 0.0f;

                        _retouch_fill(layer, roi_layer, mask_scaled, &roi_mask_scaled, form_opacity,
                                      fill_color);
                    }
                    else
                        dt_print(DT_DEBUG_ALWAYS, "rt_process_forms: unknown algorithm %i", algo);

                    if (mask_display)
                        rt_copy_mask_to_alpha(layer, roi_layer, wt_p->ch, mask_scaled,
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    const gboolean display_wavelet_scale =
        g && dt_iop_has_focus(self) ? g->display_wavelet_scale : FALSE;
    ...
    usr_data.suppress_mask =
        (g && g->suppress_mask && dt_iop_has_focus(self) && (piece->pipe == self->dev->full.pipe));
    usr_data.display_scale = p->curr_scale;

    // init the decompose routine
    dwt_p =
        dt_dwt_init(in_retouch, roi_rt->width, roi_rt->height, 4, p->num_scales,
                    (!display_wavelet_scale || !dt_pipe_is_full(piece->pipe)) ? 0 : p->curr_scale,
                    p->merge_from_scale, &usr_data, roi_in->scale / piece->iscale);
    if (dwt_p == NULL)
        goto cleanup;

    // check if this module should expose mask.
    if (dt_pipe_is_full(piece->pipe) && g && (g->mask_display || display_wavelet_scale) &&
    ...
    if (dt_pipe_is_full(piece->pipe))
    {
        ...
        if (g)
            g->first_scale_visible = dt_dwt_first_scale_visible(dwt_p);
    }
...
cl_int rt_process_stats_cl(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece, const int devid,
                           cl_mem dev_img, const int width, const int height, float levels[3])
{
...
cl_int rt_adjust_levels_cl(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece, const int devid,
                           cl_mem dev_img, const int width, const int height, const float levels[3])
{
    cl_int err = DT_OPENCL_SYSMEM_ALLOCATION;
...
static cl_int rt_copy_in_to_out_cl(const int devid, cl_mem dev_in, const dt_iop_roi_t *const roi_in,
                                   cl_mem dev_out, const dt_iop_roi_t *const roi_out, const int dx,
                                   const int dy, const int kernel)
{
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, kernel, MIN(roi_out->width, roi_in->width),
                                           MIN(roi_out->height, roi_in->height), CLARG(dev_in),
                                           CLARG(dev_roi_in), CLARG(dev_out), CLARG(dev_roi_out),
...
static cl_int rt_build_scaled_mask_cl(const int devid, float *const mask,
                                      dt_iop_roi_t *const roi_mask, float **mask_scaled,
                                      cl_mem *p_dev_mask_scaled, dt_iop_roi_t *roi_mask_scaled,
                                      dt_iop_roi_t *const roi_in, const int dx, const int dy,
                                      const int algo)
{
    cl_int err =
    ...
    const cl_mem dev_mask_scaled = dt_opencl_alloc_device_buffer(
        devid, sizeof(float) * roi_mask_scaled->width * roi_mask_scaled->height);
    if (dev_mask_scaled == NULL)
        goto cleanup;

    err = dt_opencl_write_buffer_to_device(
        devid, *mask_scaled, dev_mask_scaled, 0,
...
cleanup:
    if (err != CL_SUCCESS)
        dt_print(DT_DEBUG_ALWAYS, "rt_build_scaled_mask_cl error");
                                      ...
                                      const int kernel)
{
    ...
    const cl_mem dev_roi_dest =
        dt_opencl_copy_host_to_device_constant(devid, sizeof(dt_iop_roi_t), roi_dest);
    const cl_mem dev_roi_mask_scaled =
        dt_opencl_copy_host_to_device_constant(devid, sizeof(dt_iop_roi_t), roi_mask_scaled);

    if (dev_roi_dest == NULL || dev_roi_mask_scaled == NULL)
        goto cleanup;

    err = dt_opencl_enqueue_kernel_2d_args(devid, kernel, roi_mask_scaled->width,
                                           roi_mask_scaled->height, CLARG(dev_src), CLARG(dev_dest),
                                           CLARG(dev_roi_dest), CLARG(dev_mask_scaled),
                                           CLARG(dev_roi_mask_scaled), CLARG(opacity));
...
static cl_int rt_copy_mask_to_alpha_cl(const int devid, cl_mem dev_layer,
                                       dt_iop_roi_t *const roi_layer, cl_mem dev_mask_scaled,
                                       dt_iop_roi_t *const roi_mask_scaled, const float opacity,
                                       dt_iop_retouch_global_data_t *gd)
{
    ...
    const cl_mem dev_roi_layer =
        dt_opencl_copy_host_to_device_constant(devid, sizeof(dt_iop_roi_t), roi_layer);
    const cl_mem dev_roi_mask_scaled =
        dt_opencl_copy_host_to_device_constant(devid, sizeof(dt_iop_roi_t), roi_mask_scaled);
    ...
    err = dt_opencl_enqueue_kernel_2d_args(
        devid, kernel, roi_mask_scaled->width, roi_mask_scaled->height, CLARG(dev_layer),
        CLARG(dev_roi_layer), CLARG(dev_mask_scaled), CLARG(dev_roi_mask_scaled), CLARG(opacity));
                                ...
                                dt_iop_retouch_global_data_t *gd)
{
    ...
    const cl_mem dev_src = dt_opencl_alloc_device_buffer(
        devid, sizeof(float) * 4 * roi_mask_scaled->width * roi_mask_scaled->height);
    if (dev_src == NULL)
        goto cleanup;
    ...
    // clone it
    err = rt_copy_image_masked_cl(devid, dev_src, dev_layer, roi_layer, dev_mask_scaled,
                                  roi_mask_scaled, opacity,
                                  gd->kernel_retouch_copy_buffer_to_buffer_masked);
...
static cl_int _retouch_fill_cl(const int devid, cl_mem dev_layer, dt_iop_roi_t *const roi_layer,
                               cl_mem dev_mask_scaled, dt_iop_roi_t *const roi_mask_scaled,
                               const float opacity, float *color, dt_iop_retouch_global_data_t *gd)
{
    cl_int err = CL_MEM_OBJECT_ALLOCATION_FAILURE;
...
static cl_int _retouch_blur_cl(const int devid, cl_mem dev_layer, dt_iop_roi_t *const roi_layer,
                               cl_mem dev_mask_scaled, dt_iop_roi_t *const roi_mask_scaled,
                               const float opacity, const int blur_type, const float blur_radius,
                               dt_dev_pixelpipe_iop_t *piece, dt_iop_retouch_global_data_t *gd)
{
    if (fabsf(blur_radius) <= 0.1f)
        return CL_SUCCESS;
    ...
    const cl_mem dev_dest = dt_opencl_alloc_device(devid, roi_mask_scaled->width,
                                                   roi_mask_scaled->height, sizeof(float) * 4);
    cl_int err = CL_MEM_OBJECT_ALLOCATION_FAILURE;
    ...
    if (blur_type == DT_IOP_RETOUCH_BLUR_GAUSSIAN && fabsf(blur_radius) > 0.1f)
    {
        ...
        dt_gaussian_cl_t *g =
            dt_gaussian_init_cl(devid, roi_mask_scaled->width, roi_mask_scaled->height, 4, Labmax,
    ...
    else if (blur_type == DT_IOP_RETOUCH_BLUR_BILATERAL && fabsf(blur_radius) > 0.1f)
    {
        ...
        if (b)
        {
            ...
            if (err == CL_SUCCESS)
                err = dt_bilateral_slice_cl(b, dev_dest, dev_dest, detail);
    ...
    // copy blurred (temp) image to destination image
    err = rt_copy_image_masked_cl(devid, dev_dest, dev_layer, roi_layer, dev_mask_scaled,
                                  roi_mask_scaled, opacity,
                                  gd->kernel_retouch_copy_image_to_buffer_masked);
    if (err != CL_SUCCESS)
        goto cleanup;
...
static cl_int _retouch_heal_cl(const int devid, cl_mem dev_layer, dt_iop_roi_t *const roi_layer,
                               float *mask_scaled, cl_mem dev_mask_scaled,
                               dt_iop_roi_t *const roi_mask_scaled, const int dx, const int dy,
                               ...
                               const int max_iter)
{
    ...
    cl_mem dev_src = dt_opencl_alloc_device_buffer(
        devid, sizeof(float) * 4 * roi_mask_scaled->width * roi_mask_scaled->height);
    cl_mem dev_dest = dt_opencl_alloc_device_buffer(
        devid, sizeof(float) * 4 * roi_mask_scaled->width * roi_mask_scaled->height);
    ...
    if (hp)
    {
        err = dt_heal_cl(hp, dev_src, dev_dest, mask_scaled, roi_mask_scaled->width,
                         roi_mask_scaled->height, max_iter);
        dt_heal_free_cl(hp);
...
static cl_int rt_process_forms_cl(cl_mem dev_layer, dwt_params_cl_t *const wt_p, const int scale1)
{
    ...
    if (!usr_d->suppress_mask)
    {
        ...
        if (grp && (grp->type & DT_MASKS_GROUP))
        {
            for (const GList *forms = grp->points; forms && err == CL_SUCCESS;
                 forms = g_list_next(forms))
            {
                dt_masks_point_group_t *grpt = forms->data;
                if (grpt == NULL)
                ...
                err =
                    rt_build_scaled_mask_cl(devid, mask, &roi_mask, &mask_scaled, &dev_mask_scaled,
                                            &roi_mask_scaled, roi_layer, dx, dy, algo);
                ...
                if ((err == CL_SUCCESS) &&
                    (dx != 0 || dy != 0 || algo == DT_IOP_RETOUCH_BLUR ||
                     algo == DT_IOP_RETOUCH_FILL) &&
                    ((roi_mask_scaled.width > 2) && (roi_mask_scaled.height > 2)))
                {
                    if (algo == DT_IOP_RETOUCH_CLONE)
                    {
                        err = _retouch_clone_cl(devid, dev_layer, roi_layer, dev_mask_scaled,
                                                &roi_mask_scaled, dx, dy, form_opacity, gd);
                    }
                    else if (algo == DT_IOP_RETOUCH_HEAL)
                    {
                        err = _retouch_heal_cl(devid, dev_layer, roi_layer, mask_scaled,
                                               dev_mask_scaled, &roi_mask_scaled, dx, dy,
                                               form_opacity, gd, p->max_heal_iter);
                    }
                    else if (algo == DT_IOP_RETOUCH_BLUR)
                    {
                        err = _retouch_blur_cl(devid, dev_layer, roi_layer, dev_mask_scaled,
                                               &roi_mask_scaled, form_opacity,
                    ...
                    else if (algo == DT_IOP_RETOUCH_FILL)
                    {
                        ...
                        err = _retouch_fill_cl(devid, dev_layer, roi_layer, dev_mask_scaled,
                                               &roi_mask_scaled, form_opacity, fill_color, gd);
                    }
                    else
                    {
                        dt_print(DT_DEBUG_ALWAYS, "rt_process_forms: unknown algorithm %i", algo);
                    ...
                    if (mask_display)
                    {
                        err = rt_copy_mask_to_alpha_cl(devid, dev_layer, roi_layer, dev_mask_scaled,
                                                       &roi_mask_scaled, form_opacity, gd);
                    }
                }
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    const cl_mem in_retouch =
        dt_opencl_alloc_device_buffer(devid, sizeof(float) * 4 * roi_rt->width * roi_rt->height);
    if (in_retouch == NULL)
        goto cleanup;
    ...
    // copy input image to the new buffer
    {
        ...
        err = dt_opencl_enqueue_copy_image_to_buffer(devid, dev_in, in_retouch, CLIMG_ORIGIN,
                                                     region, 0);
        if (err != CL_SUCCESS)
            goto cleanup;
    }
    ...
    // init the decompose routine
    dwt_p = dt_dwt_init_cl(
        devid, in_retouch, roi_rt->width, roi_rt->height, p->num_scales,
        (!display_wavelet_scale || !dt_pipe_is_full(piece->pipe)) ? 0 : p->curr_scale,
        p->merge_from_scale, &usr_data, roi_in->scale / piece->iscale);
    if (dwt_p == NULL)
    {
    ...
    // check if this module should expose mask.
    if (dt_pipe_is_full(piece->pipe) && g && g->mask_display && dt_iop_has_focus(self) &&
        (piece->pipe == self->dev->full.pipe))
    {
        ...
        err = dt_opencl_enqueue_kernel_2d_args(devid, kernel, roi_rt->width, roi_rt->height,
                                               CLARG(in_retouch), CLARG((roi_rt->width)),
                                               CLARG((roi_rt->height)));
        if (err != CL_SUCCESS)
            goto cleanup;
    ...
    if (dt_pipe_is_full(piece->pipe))
    {
        ...
        // get first scale visible at this zoom level
        if (g)
            g->first_scale_visible = dt_dwt_first_scale_visible_cl(dwt_p);
    }
    ...
    if (g && dt_pipe_is_full(piece->pipe))
    {
        ...
        if (g->preview_auto_levels == 1 && !DT_IN_GUI_UPDATE())
        {
            ...
            levels[0] = levels[1] = levels[2] = 0;
            err = rt_process_stats_cl(self, piece, devid, in_retouch, roi_rt->width, roi_rt->height,
                                      levels);
            if (err != CL_SUCCESS)
                goto cleanup;
    ...
    if (dwt_p->return_layer > 0 && dwt_p->return_layer < dwt_p->scales + 1)
    {
        err = rt_adjust_levels_cl(self, piece, devid, in_retouch, roi_rt->width, roi_rt->height,
                                  levels);
        if (err != CL_SUCCESS)
            goto cleanup;
    }
    ...
    if ((piece->pipe->mask_display & DT_DEV_PIXELPIPE_DISPLAY_MASK) && g && !g->mask_display)
    {
        ...
        err = dt_opencl_enqueue_kernel_2d_args(devid, kernel, roi_rt->width, roi_rt->height,
                                               CLARG(dev_in), CLARG(in_retouch),
                                               CLARG((roi_rt->width)), CLARG((roi_rt->height)));
        if (err != CL_SUCCESS)
            goto cleanup;
    }
