...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(
        self, _("scale raw RGB channels to balance white and help demosaicing"), _("corrective"),
        _("linear, raw, scene-referred"), _("linear, raw"), _("linear, raw, scene-referred"));
}
...
static void _xyz2mul(dt_iop_module_t *self, cmsCIEXYZ xyz, double mul[4])
{
...
static void _temp2mul(dt_iop_module_t *self, const double TempK, const double tint, double mul[4])
{
    cmsCIEXYZ xyz = _temperature_to_XYZ(TempK);
...
static cmsCIEXYZ _mul2xyz(dt_iop_module_t *self, const dt_iop_temperature_params_t *p)
{
    dt_iop_temperature_gui_data_t *g = self->gui_data;
...
static void _mul2temp(dt_iop_module_t *self, dt_iop_temperature_params_t *p, float *TempK,
                      float *tint)
{
...
static inline void scaled_copy_4wide(float *const outp, const float *const inp,
                                     const float *const coeffs)
{
    for_four_channels(c, aligned(inp, coeffs, outp)) outp[c] = inp[c] * coeffs[c];
}
    ...
    { // xtrans float mosaiced
        ...
        for (int j = 0; j < height; j++)
        {
            const float DT_ALIGNED_PIXEL coeffs[3][4] = {
                {d_coeffs[FCNxtrans(j, 0, xtrans)], d_coeffs[FCNxtrans(j, 1, xtrans)],
                 d_coeffs[FCNxtrans(j, 2, xtrans)], d_coeffs[FCNxtrans(j, 3, xtrans)]},
                {d_coeffs[FCNxtrans(j, 4, xtrans)], d_coeffs[FCNxtrans(j, 5, xtrans)],
                 d_coeffs[FCNxtrans(j, 6, xtrans)], d_coeffs[FCNxtrans(j, 7, xtrans)]},
                {d_coeffs[FCNxtrans(j, 8, xtrans)], d_coeffs[FCNxtrans(j, 9, xtrans)],
                 d_coeffs[FCNxtrans(j, 10, xtrans)], d_coeffs[FCNxtrans(j, 11, xtrans)]},
            };
    ...
    { // bayer float mosaiced
        ...
        for (int j = 0; j < height; j++)
        {
            ...
            const dt_aligned_pixel_t coeffs = {
                d_coeffs[FC(j, i, filters)], d_coeffs[FC(j, i + 1, filters)],
                d_coeffs[FC(j, i + 2, filters)], d_coeffs[FC(j, i + 3, filters)]};
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    {
        dev_xtrans =
            dt_opencl_copy_host_to_device_constant(devid, sizeof(piece->xtrans), piece->xtrans);
        if (dev_xtrans == NULL)
            goto error;
    }
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, kernel, roi_in->width, roi_in->height,
                                           CLARG(dev_in), CLARG(dev_out), CLARG(roi_in->width),
                                           CLARG(roi_in->height), CLARG(dev_coeffs), CLARG(filters),
                                           CLARG(dev_xtrans));
    if (err != CL_SUCCESS)
        goto error;
                   ...
                   dt_dev_pixelpipe_iop_t *piece)
{
    ...
    if (self->hide_enable_button)
    {
        for_four_channels(k) chr->wb_coeffs[k] = 1.0f;
        return;
    }
    ...
    if (!dt_image_is_ldr(&self->dev->image_storage))
        for (int i = 0; i < dt_wb_presets_count(); i++)
        {
                ...
                !strcmp(wbp->model, self->dev->image_storage.camera_model))
            {
                ...
                {
                    char *section = g_strdup_printf("%s %s", self->dev->image_storage.camera_maker,
                ...
                if (!wb_name || strcmp(wb_name, wbp->name))
                {
                    ...
                    dt_bauhaus_combobox_add_full(g->presets, _(wbp->name),
                                                 DT_BAUHAUS_COMBOBOX_ALIGN_RIGHT, preset, free,
                                                 TRUE);
...
static void _color_rgb_sliders(dt_iop_module_t *self)
{
    ...
    const gboolean color_rgb =
        g->colored_sliders && !(self->dev->image_storage.flags & DT_IMAGE_4BAYER);
    ...
    else
    {
        ...
        dt_bauhaus_slider_set_stop(g->scale_r, 0.0, white[0] * 0.0, white[1] * gchanmul,
                                   white[2] * bchanmul);
        dt_bauhaus_slider_set_stop(g->scale_r, chr->D65coeffs[0] / rchanmulmax,
                                   white[0] * chr->D65coeffs[0], white[1] * gchanmul,
                                   white[2] * bchanmul);
        dt_bauhaus_slider_set_stop(g->scale_r, 1.0, white[0] * 1.0,
                                   white[1] * (gchanmul / gchanmulmax),
                                   white[2] * (bchanmul / bchanmulmax));

        dt_bauhaus_slider_set_stop(g->scale_g, 0.0, white[0] * rchanmul, white[1] * 0.0,
                                   white[2] * bchanmul);
        dt_bauhaus_slider_set_stop(g->scale_g, chr->D65coeffs[1] / bchanmulmax, white[0] * rchanmul,
                                   white[1] * chr->D65coeffs[1], white[2] * bchanmul);
        dt_bauhaus_slider_set_stop(g->scale_g, 1.0, white[0] * (rchanmul / rchanmulmax),
                                   white[1] * 1.0, white[2] * (bchanmul / bchanmulmax));

        dt_bauhaus_slider_set_stop(g->scale_b, 0.0, white[0] * rchanmul, white[1] * gchanmul,
                                   white[2] * 0.0);
        dt_bauhaus_slider_set_stop(g->scale_b, chr->D65coeffs[2] / bchanmulmax, white[0] * rchanmul,
                                   white[1] * gchanmul, white[2] * chr->D65coeffs[2]);
        dt_bauhaus_slider_set_stop(g->scale_b, 1.0, white[0] * (rchanmul / rchanmulmax),
...
static void _color_temptint_sliders(dt_iop_module_t *self)
{
    ...
    const double temp_step = (double)(DT_IOP_HIGHEST_TEMPERATURE - DT_IOP_LOWEST_TEMPERATURE) /
                             (DT_BAUHAUS_SLIDER_MAX_STOPS - 1.0);
    const double tint_step =
        (double)(DT_IOP_HIGHEST_TINT - DT_IOP_LOWEST_TINT) / (DT_BAUHAUS_SLIDER_MAX_STOPS - 1.0);
    ...
    if (blackbody_is_confusing)
    {
        ...
        for (int i = 0; i < DT_BAUHAUS_SLIDER_MAX_STOPS; i++)
        {
            ...
            dt_aligned_pixel_t sRGB_K = {dayligh_white[0] * coeffs_K[0],
                                         dayligh_white[1] * coeffs_K[1],
                                         dayligh_white[2] * coeffs_K[2]};
            dt_aligned_pixel_t sRGB_tint = {cur_white[0] * coeffs_tint[0],
            ...
            dt_bauhaus_slider_set_stop(g->scale_tint, stop, sRGB_tint[0], sRGB_tint[1],
                                       sRGB_tint[2]);
        }
    }
    else
...
static inline const char *_preset_to_str(const int preset)
{
    switch (preset)
    {
    case DT_IOP_TEMP_AS_SHOT:
        return "as shot";
    case DT_IOP_TEMP_SPOT:
        return "by spot";
    case DT_IOP_TEMP_USER:
        return "user defined";
    case DT_IOP_TEMP_D65:
        return "camera reference";
    case DT_IOP_TEMP_D65_LATE:
        return "as shot to reference";
    default:
        return "other";
    }
}
...
void gui_update(dt_iop_module_t *self)
{
    ...
    if (!found)
    {
        ...
        if (!found)
        {
            ...
            for (int j = DT_IOP_NUM_OF_STD_TEMP_PRESETS; !found && (j < g->preset_cnt); j++)
            {
                ...
                while (!found && (i < dt_wb_presets_count()) &&
                       !strcmp(dt_wb_preset(i)->make, self->dev->image_storage.camera_maker) &&
                       !strcmp(dt_wb_preset(i)->model, self->dev->image_storage.camera_model) &&
                       !strcmp(dt_wb_preset(i)->name, dt_wb_preset(g->preset_num[j])->name))
                {
                    ...
                    for (int tune = dt_wb_preset(i - 1)->tuning + 1;
                         !found && (tune < dt_wb_preset(i)->tuning); tune++)
                    {
                        dt_wb_data interpolated = {.tuning = tune};
                        dt_wb_preset_interpolate(dt_wb_preset(i - 1), dt_wb_preset(i),
                                                 &interpolated);
    ...
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(g->btn_asshot),
                                 p->preset == DT_IOP_TEMP_AS_SHOT);
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(g->btn_user), p->preset == DT_IOP_TEMP_USER);
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(g->btn_d65), p->preset == DT_IOP_TEMP_D65);
    ...
    dt_print_pipe(DT_DEBUG_PIPE, "used preset", NULL, self, DT_DEVICE_NONE, NULL, NULL,
                  "preset='%s': D65 %.3f %.3f %.3f, AS-SHOT %.3f %.3f %.3f",
                  _preset_to_str(p->preset), chr->D65coeffs[0], chr->D65coeffs[1],
                  chr->D65coeffs[2], chr->as_shot[0], chr->as_shot[1], chr->as_shot[2]);
...
static gboolean _calculate_bogus_daylight_wb(dt_iop_module_t *self, double bwb[4])
{
    ...
    double mul[4];
    if (dt_colorspaces_conversion_matrices_rgb(self->dev->image_storage.adobe_XYZ_to_CAM, NULL,
                                               NULL, self->dev->image_storage.d65_color_matrix,
                                               mul))
    {
        // normalize green:
        bwb[0] = mul[0] / mul[1];
...
static void _prepare_matrices(dt_iop_module_t *self)
{
    ...
    {
        if (self->dev->image_storage.load_status ==
            DT_IMAGEIO_OK) // suppress spurious error messages
        {
...
static void _find_coeffs(dt_iop_module_t *self, double coeffs[4])
{
    ...
    if (ok)
    {
        for_four_channels(k) coeffs[k] = img->wb_coeffs[k];
        return;
    }
    ...
    if (!_calculate_bogus_daylight_wb(self, bwb))
    {
        ...
        for_four_channels(c) coeffs[c] = bwb[c];
        return;
    }
...
void reload_defaults(dt_iop_module_t *self)
{
    ...
    for_four_channels(k) dcoeffs[k] = 1.0f;
    ...
    {
        another_cat_defined =
            dt_history_check_module_exists(self->dev->image_storage.id, "channelmixerrgb", TRUE);
    }
    ...
    else
    {
        ...
        for (int i = 0; i < dt_wb_presets_count(); i++)
        {
            ...
            if (!strcmp(wbp->make, self->dev->image_storage.camera_maker) &&
                ...
                wbp->tuning == 0)
            {
                for_four_channels(k) daylights[k] = wbp->channels[k];
                break;
            }
        }
    }
    ...
    dt_print(
        ...
        STR_YESNO(dt_is_scene_referred()), STR_YESNO(is_modern), STR_YESNO(another_cat_defined),
    ...
    if (!true_monochrome)
    {
        ...
        if (is_raw)
        {
            ...
            {
                for_four_channels(k) dcoeffs[k] = as_shot[k];
    ...
    if (g)
    {
        ...
        for_four_channels(k) g->mod_coeff[k] = dcoeffs[k];
...
static gboolean _btn_toggled(GtkWidget *togglebutton, GdkEventButton *event, dt_iop_module_t *self)
{
    ...
    const int preset = togglebutton == g->btn_asshot   ? DT_IOP_TEMP_AS_SHOT :
                       ...
                       togglebutton == g->btn_user     ? DT_IOP_TEMP_USER :
                                                         0;
    ...
    const dt_dev_chroma_t *chr = &self->dev->chroma;
    dt_print_pipe(DT_DEBUG_PIPE, "toggled preset", NULL, self, DT_DEVICE_NONE, NULL, NULL,
                  "preset='%s': D65 %.3f %.3f %.3f, AS-SHOT %.3f %.3f %.3f", _preset_to_str(preset),
...
static void _preset_tune_callback(GtkWidget *widget, dt_iop_module_t *self)
{
    ...
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(g->btn_d65_late), pos == DT_IOP_TEMP_D65_LATE);
...
void color_picker_apply(dt_iop_module_t *self, GtkWidget *picker, dt_dev_pixelpipe_t *pipe)
{
    ...
    for_four_channels(c) pcoeffs[c] =
        fmaxf(0.0f, fminf(8.0f, (grayrgb[c] > 0.001f ? 1.0f / grayrgb[c] : 1.0f) / gnormal));
    pcoeffs[1] = 1.0f;
...
static void _preference_changed(gpointer instance, dt_iop_module_t *self)
{
    ...
    g->blackbody_is_confusing =
        g->colored_sliders && (g_strcmp0(config, "illuminant color") ? TRUE : FALSE);
...
void gui_init(dt_iop_module_t *self)
{
    ...
    dt_action_define_iop(self, N_("settings"), N_("from image area"), g->colorpicker,
                         &dt_action_def_toggle);
    dtgtk_togglebutton_set_paint(DTGTK_TOGGLEBUTTON(g->colorpicker), dtgtk_cairo_paint_colorpicker,
                                 0, NULL);
    ...
    g->buttonbar = dt_gui_hbox(dt_gui_expand(g->btn_asshot), dt_gui_expand(g->colorpicker),
                               dt_gui_expand(g->btn_user), dt_gui_expand(g->btn_d65),
    ...
    g->finetune =
        dt_bauhaus_slider_new_with_range_and_feedback(self, -9.0, 9.0, 0, 0.0, 0, feedback);
    ...
    g->scale_k = dt_bauhaus_slider_new_with_range_and_feedback(
        self, DT_IOP_LOWEST_TEMPERATURE, DT_IOP_HIGHEST_TEMPERATURE, 0, 5000.0, 0, feedback);
    ...
    g->scale_tint = dt_bauhaus_slider_new_with_range_and_feedback(
        self, DT_IOP_LOWEST_TINT, DT_IOP_HIGHEST_TINT, 0, 1.0, 3, feedback);
    dt_bauhaus_widget_set_label(g->scale_tint, NULL, N_("tint"));
    gtk_widget_set_tooltip_text(
        g->scale_tint, _("color tint of the image, from magenta (value < 1) to green (value > 1)"));

    GtkWidget *box_enabled = dt_gui_vbox(g->buttonbar, g->presets, g->finetune, temp_label_box,
                                         g->scale_k, g->scale_tint);
    dt_gui_new_collapsible_section(&g->cs, "plugins/darkroom/temperature/expand_coefficients",
                                   _("channel coefficients"), GTK_BOX(box_enabled),
                                   DT_ACTION(self));
    ...
    g_signal_connect(G_OBJECT(g->scale_k), "value-changed", G_CALLBACK(_temp_tint_callback), self);
    g_signal_connect(G_OBJECT(g->scale_tint), "value-changed", G_CALLBACK(_temp_tint_callback),
                     self);

    g_signal_connect(G_OBJECT(g->presets), "value-changed", G_CALLBACK(_preset_tune_callback),
                     self);
    g_signal_connect(G_OBJECT(g->finetune), "value-changed", G_CALLBACK(_preset_tune_callback),
                     self);
...
void gui_reset(dt_iop_module_t *self)
{
    ...
    const int preset = d->preset =
        dt_is_scene_referred() ? DT_IOP_TEMP_D65_LATE : DT_IOP_TEMP_AS_SHOT;
    dt_iop_color_picker_reset(self, TRUE);
