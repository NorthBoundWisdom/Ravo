...
typedef enum dt_iop_rasterfile_mode_t
{
    ...
    DT_RASTERFILE_MODE_REDGREEN =
        DT_RASTERFILE_MODE_RED | DT_RASTERFILE_MODE_GREEN, // $DESCRIPTION: "red and green"
    DT_RASTERFILE_MODE_REDBLUE =
        DT_RASTERFILE_MODE_RED | DT_RASTERFILE_MODE_BLUE, // $DESCRIPTION: "red and blue"
    DT_RASTERFILE_MODE_GREENBLUE =
        DT_RASTERFILE_MODE_GREEN | DT_RASTERFILE_MODE_BLUE, // $DESCRIPTION: "green and blue"
...
typedef struct dt_rasterfile_cache_t
{
    ...
    volatile dt_hash_t
        hash; // as the hash is shared between threads make sure it's actually read from mem
    int width, height;
    float *volatile mask;
...
static float *_read_rasterfile(char *filename, const dt_iop_rasterfile_mode_t mode, int *swidth,
                               int *sheight)
{
    *swidth = 0;
    *sheight = 0;
    ...
    if (is_png)
    {
        ...
        {
            dt_print(DT_DEBUG_ALWAYS, "failed to read PNG header from '%s'",
                     filename ? filename : "???");
        ...
        if (!buf)
        {
            ...
            dt_print(DT_DEBUG_ALWAYS, "can't read raster mask file '%s'",
                     filename ? filename : "???");
            dt_control_log(_("can't read raster mask file '%s'"), filename ? filename : "???");
            return NULL;
        }
        ...
        if (png.bit_depth < 16)
        {
            ...
            for (size_t k = 0; k < (size_t)width * height; k++)
            {
                ...
                if (mode & DT_RASTERFILE_MODE_RED)
                    val = MAX(val, buf[base] * normalizer);
                if (mode & DT_RASTERFILE_MODE_GREEN)
                    val = MAX(val, buf[base + 1] * normalizer);
                if (mode & DT_RASTERFILE_MODE_BLUE)
                    val = MAX(val, buf[base + 2] * normalizer);
                mask[k] = CLIP(val);
            }
        }
        else
...
static void _fbutton_clicked(GtkWidget *widget, dt_iop_module_t *self)
{
    ...
    GtkFileChooserNative *filechooser =
        gtk_file_chooser_native_new(_("select raster mask file"), GTK_WINDOW(win),
...
static float *_get_rasterfile_mask(dt_dev_pixelpipe_iop_t *piece, const dt_iop_roi_t *const roi,
                                   const dt_iop_roi_t *const roo)
{
    ...
    if (cd->mask)
    {
        ...
        {
            if (scale)
                interpolate_bilinear(cd->mask, cd->width, cd->height, tmp, roi->width, roi->height,
                                     1);
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    if (dt_iop_is_raster_mask_used(piece->module, BLEND_RASTER_ID) && (err == CL_SUCCESS))
    {
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    if (visual)
    {
        ...
        else
        {
            ...
            {
                const float val =
                    0.2f * CLAMPF(sqrtf(0.33f * (out[4 * k] + out[4 * k + 1] + out[4 * k + 2])),
                                  0.0f, 0.5f) +
                    (mask ? mask[k] : 0.0f);
...
void gui_init(dt_iop_module_t *self)
{
    ...
    gtk_widget_set_tooltip_text(
        g->mode, _("select the RGB channels taken into account to generate the raster mask"));
