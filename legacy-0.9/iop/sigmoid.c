...
typedef struct dt_iop_sigmoid_params_t
{
    ...
    float
        display_white_target; // $MIN: 20.0  $MAX: 1600.0 $DEFAULT: 100.0 $DESCRIPTION: "target white"
    float
        display_black_target; // $MIN: 0.0  $MAX: 15.0 $DEFAULT: 0.0152 $DESCRIPTION: "target black"
    dt_iop_sigmoid_methods_type_t
        color_processing; // $DEFAULT: DT_SIGMOID_METHOD_PER_CHANNEL $DESCRIPTION: "color processing"
    ...
    dt_iop_sigmoid_base_primaries_t
        base_primaries; // $DEFAULT: DT_SIGMOID_WORK_PROFILE $DESCRIPTION: "base primaries"
...
void init_presets(dt_iop_module_so_t *self)
{
    ...
    dt_gui_presets_add_generic(_("neutral gray"), self->op, self->version(), &p, sizeof(p), TRUE,
                               DEVELOP_BLEND_CS_RGB_SCENE);

    p.middle_grey_contrast = 1.6f;
    p.contrast_skewness = -0.2f;
    p.hue_preservation = 0.0f;
    dt_gui_presets_add_generic(_("ACES 100-nit like"), self->op, self->version(), &p, sizeof(p),
                               TRUE, DEVELOP_BLEND_CS_RGB_SCENE);

    p.middle_grey_contrast = 1.0f;
    p.contrast_skewness = 0.0f;
    p.color_processing = DT_SIGMOID_METHOD_RGB_RATIO;
    dt_gui_presets_add_generic(_("Reinhard"), self->op, self->version(), &p, sizeof(p), TRUE,
                               DEVELOP_BLEND_CS_RGB_SCENE);
...
static inline float _generalized_loglogistic_sigmoid(const float value, const float magnitude,
                                                     ...
                                                     const float paper_power)
{
    ...
    const float paper_response =
        magnitude * powf(film_response / (paper_exp + film_response), paper_power);
                   ...
                   dt_dev_pixelpipe_iop_t *piece)
{
   ...
   */
    ...
    const float ref_slope =
        (_generalized_loglogistic_sigmoid(MIDDLE_GREY + delta, ref_magnitude, ref_paper_exposure,
    ...
    const float temp_slope =
        (_generalized_loglogistic_sigmoid(MIDDLE_GREY + delta, temp_white_target,
                                          temp_paper_exposure, ref_film_fog, temp_film_power,
                                          module_data->paper_power) -
         _generalized_loglogistic_sigmoid(MIDDLE_GREY - delta, temp_white_target,
                                          temp_paper_exposure, ref_film_fog, temp_film_power,
                                          module_data->paper_power)) /
        2.0f / delta;
    ...
    const float white_black_relation = powf(module_data->black_target / module_data->white_target,
                                            -1.0f / module_data->paper_power) -
                                       1.0f;

    module_data->film_fog = MIDDLE_GREY *
...
static void
_calculate_adjusted_primaries(const dt_iop_sigmoid_data_t *const module_data,
                              const dt_iop_order_iccprofile_info_t *const pipe_work_profile,
                              const dt_iop_order_iccprofile_info_t *const base_profile,
                              dt_colormatrix_t pipe_to_base, dt_colormatrix_t base_to_rendering,
                              dt_colormatrix_t rendering_to_pipe)
{
    ...
    for (size_t i = 0; i < 3; i++)
        dt_rotate_and_scale_primary(base_profile, 1.f - module_data->inset[i],
                                    module_data->rotation[i], i, custom_primaries[i]);

    dt_colormatrix_t custom_to_XYZ;
    dt_make_transposed_matrices_from_primaries_and_whitepoint(
        custom_primaries, base_profile->whitepoint, custom_to_XYZ);
    ...
    for (size_t i = 0; i < 3; i++)
    {
        ...
        dt_rotate_and_scale_primary(base_profile, scaling, module_data->rotation[i], i,
                                    custom_primaries[i]);
    }
...
static dt_colorspaces_color_profile_type_t
_get_base_profile_type(const dt_iop_sigmoid_base_primaries_t base_primaries)
{
...
_get_base_profile(dt_develop_t *dev, const dt_iop_order_iccprofile_info_t *pipe_work_profile,
                  const dt_iop_sigmoid_base_primaries_t base_primaries)
{
    if (base_primaries == DT_SIGMOID_WORK_PROFILE)
        return pipe_work_profile;

    return dt_ioppr_add_profile_info_to_list(dev, _get_base_profile_type(base_primaries), "",
                                             DT_INTENT_RELATIVE_COLORIMETRIC);
}

DT_OMP_DECLARE_SIMD()
static inline void _desaturate_negative_values(const dt_aligned_pixel_t pix_in,
                                               dt_aligned_pixel_t pix_out)
{
    ...
    const float saturation_factor =
        min_value < 0.0f ? -pixel_average / (min_value - pixel_average) : 1.0f;
...
static void _pixel_channel_order(const dt_aligned_pixel_t pix_in,
...
void process_loglogistic_rgb_ratio(const dt_dev_pixelpipe_iop_t *piece, const void *const ivoid,
                                   ...
                                   const dt_iop_roi_t *const roi_out)
{
    ...
    for (size_t k = 0; k < 4 * npixels; k += 4)
    {
        ...
        const float luma =
            (pix_in_strict_positive[0] + pix_in_strict_positive[1] + pix_in_strict_positive[2]) /
            3.0f;
        const float mapped_luma = _generalized_loglogistic_sigmoid(
            luma, white_target, paper_exp, film_fog, contrast_power, skew_power);
        ...
        const float display_border_vs_chroma =
            fminf(display_border_vs_chroma_white, display_border_vs_chroma_black);
                                            ...
                                            const float hue_preservation)
{
    ...
    const float blend_factor =
        pix_in_min_plus_mid != 0.f ? 2.0f * pix_in[order.min] / pix_in_min_plus_mid : 0.f;
    const float energy_target =
        blend_factor * per_channel_energy + (1.0f - blend_factor) * naive_hue_energy;
    ...
    if (naive_hue_mid <= per_channel[order.mid])
    {
        const float corrected_mid =
            ((1.0f - hue_preservation) * per_channel[order.mid] +
             hue_preservation * (midscale * per_channel[order.max] +
...
void process_loglogistic_per_channel(dt_develop_t *dev, const dt_dev_pixelpipe_iop_t *piece,
                                     ...
                                     const dt_iop_roi_t *const roi_out)
{
    ...
    const dt_iop_order_iccprofile_info_t *base_profile =
        _get_base_profile(dev, pipe_work_profile, module_data->base_primaries);
    dt_colormatrix_t pipe_to_base, base_to_rendering, rendering_to_pipe;
    _calculate_adjusted_primaries(module_data, pipe_work_profile, base_profile, pipe_to_base,
                                  base_to_rendering, rendering_to_pipe);
    ...
    for (size_t k = 0; k < 4 * npixels; k += 4)
    {
        ...
        {
            per_channel[c] = _generalized_loglogistic_sigmoid(
                rendering_RGB[c], white_target, paper_exp, film_fog, contrast_power, skew_power);
        }
        ...
        _preserve_hue_and_energy(rendering_RGB, per_channel, per_channel_hue_corrected,
                                 pixel_value_order, hue_preservation);
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    const dt_iop_order_iccprofile_info_t *pipe_work_profile =
        dt_ioppr_get_pipe_work_profile_info(piece->pipe);
    const dt_iop_order_iccprofile_info_t *base_profile =
        _get_base_profile(self->dev, pipe_work_profile, d->base_primaries);
    ...
    _calculate_adjusted_primaries(d, pipe_work_profile, base_profile, pipe_to_base_transposed,
    ...
    if (d->color_processing == DT_SIGMOID_METHOD_PER_CHANNEL)
    {
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->kernel_sigmoid_loglogistic_per_channel, width, height, CLARG(dev_in),
            CLARG(dev_out), CLARG(width), CLARG(height), CLARG(d->white_target),
            CLARG(d->paper_exposure), CLARG(d->film_fog), CLARG(d->film_power),
            CLARG(d->paper_power), CLARG(d->hue_preservation), CLARG(dev_pipe_to_base),
            CLARG(dev_base_to_rendering), CLARG(dev_rendering_to_pipe));
    }
    else
    {
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->kernel_sigmoid_loglogistic_rgb_ratio, width, height, CLARG(dev_in),
            CLARG(dev_out), CLARG(width), CLARG(height), CLARG(d->white_target),
            CLARG(d->black_target), CLARG(d->paper_exposure), CLARG(d->film_fog),
            CLARG(d->film_power), CLARG(d->paper_power));
    }
...
void init_global(dt_iop_module_so_t *self)
{
    ...
    gd->kernel_sigmoid_loglogistic_rgb_ratio =
        dt_opencl_create_kernel(program, "sigmoid_loglogistic_rgb_ratio");
}
...
void gui_init(dt_iop_module_t *self)
{
    ...
    gtk_widget_set_tooltip_text(g->hue_preservation_slider,
                                _("optional correction of the hue twist introduced by\n"
                                  "the per-channel processing method."));

    GtkWidget *main_box = self->widget;

    // primaries collapsible section
    dt_gui_new_collapsible_section(&g->primaries_section,
                                   "plugins/darkroom/sigmoid/expand_primaries", _("primaries"),
                                   GTK_BOX(main_box), DT_ACTION(self));
    ...
    gtk_widget_set_tooltip_text(
        base_primaries, _("primaries to use as the base for below adjustments\n"
...
#define SETUP_COLOR_COMBO(color, r, g, b, inset_tooltip, rotation_tooltip)                         \
    ...
    SETUP_COLOR_COMBO(red, 1.f - desaturation, desaturation, desaturation,
                      _("attenuate the purity of the red primary"), _("rotate the red primary"));
    SETUP_COLOR_COMBO(green, desaturation, 1.f - desaturation, desaturation,
                      _("attenuate the purity of the green primary"),
                      _("rotate the green primary"));
    SETUP_COLOR_COMBO(blue, desaturation, desaturation, 1.f - desaturation,
                      _("attenuate the purity of the blue primary"), _("rotate the blue primary"));
#undef SETUP_COLOR_COMBO
    ...
    dt_gui_new_collapsible_section(&g->display_luminance_section,
                                   "plugins/darkroom/sigmoid/expand_values", _("display luminance"),
                                   GTK_BOX(main_box), DT_ACTION(self));
    ...
    dt_bauhaus_slider_set_format(slider, "%");
    gtk_widget_set_tooltip_text(
        slider, _("the white luminance of the target display or print.\n"
