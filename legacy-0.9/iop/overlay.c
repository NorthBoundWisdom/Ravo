...
typedef struct dt_iop_overlay_params_t
{
    ...
    dt_iop_overlay_base_scale_t
        scale_base; // $DEFAULT: DT_SCALE_MAINMENU_IMAGE $DESCRIPTION: "scale on"
    dt_iop_overlay_img_scale_t
        scale_img; // $DEFAULT: DT_SCALE_IMG_LARGER $DESCRIPTION: "scale marker to"
    dt_iop_overlay_svg_scale_t
        scale_svg;       // $DEFAULT: DT_SCALE_SVG_WIDTH $DESCRIPTION: "scale marker reference"
...
static GList *_get_disabled_modules(const dt_iop_module_t *self, const dt_imgid_t imgid)
{
  ...
  */
    ...
    const dt_iop_module_t *self_module =
        dt_iop_get_module_by_op_priority(dev->iop, "overlay", multi_priority);
    ...
    for (GList *l = dev->iop; l; l = g_list_next(l))
    {
        ...
        if ((after && !dt_iop_module_is_gamma(mod) && !dt_iop_module_is_finalscale(mod) &&
             !dt_iop_module_is(mod, "crop") && !dt_iop_module_is(mod, "ashift")) ||
            (is_current &&
             (dt_iop_module_is(mod, "overlay") || dt_iop_module_is(mod, "enlargecanvas"))))
        {
            result = g_list_prepend(result, mod->op);
        }

        // look for ourself, disable all modules after this point
        if (dt_iop_module_is(mod, self_module->op) && mod->multi_priority == multi_priority)
            after = TRUE;
    }
    ...
    if (darktable.unmuted & (DT_DEBUG_PARAMS | DT_DEBUG_PIPE))
    {
        ...
        dt_print_pipe(DT_DEBUG_PARAMS | DT_DEBUG_PIPE, "module_filter_out", NULL, self,
                      DT_DEVICE_NONE, NULL, NULL, "%s", buf);
        g_free(buf);
    }
...
static void _module_remove_callback(gpointer instance, dt_iop_module_t *self,
...
static void _setup_overlay(dt_iop_module_t *self, const dt_dev_pixelpipe_iop_t *piece,
                           uint8_t **pbuf, size_t *pwidth, size_t *pheight)
{
    ...
    if (!image_exists)
    {
        ...
        {
            const gchar *tooltip = g_strdup_printf(_("overlay image missing from database\n\n"
                                                     "'%s'"),
                                                   p->filename);
    ...
    if (image_exists)
    {
        ...
        dt_dev_image(imgid, width, height, -1, &buf, NULL, &bw, &bh, NULL, -1, disabled_modules,
                     piece->pipe->devid, TRUE);
 ...
 */
static guint8 *_get_overlay_argb(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                                 const dt_iop_roi_t *const roi_in,
                                 const dt_iop_roi_t *const roi_out, int *out_stride)
{
    ...
    const float larger =
        dimension.width > dimension.height ? (float)dimension.width : (float)dimension.height;
    ...
    case DT_SCALE_MAINMENU_ADVANCED:
        ...
        {
            sbase = iw;
            scale = (data->scale_svg == DT_SCALE_SVG_WIDTH) ? sbase / dimension.width :
                                                              sbase / dimension.height;
        }
    ...
    default:
        wbase = iw;
        hbase = ih;
        scale = (dimension.width > dimension.height) ? iw / dimension.width : ih / dimension.height;
    }
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_overlay_blend, width, height, CLARG(dev_in), CLARG(dev_overlay),
        CLARG(dev_out), CLARG(width), CLARG(height), CLARG(opacity), CLARG(stride));
...
static gboolean _draw_thumb(GtkWidget *area, cairo_t *crf, const dt_iop_module_t *self)
{
    ...
    else
    {
        ...
        cairo_move_to(crf, (width - ink.width) / 2.0, (height - ink.height) / 2.0);
...
static void _drag_and_drop_received(GtkWidget *widget, GdkDragContext *context, gint x, gint y,
                                    const GtkSelectionData *selection_data, const guint target_type,
                                    const guint time, dt_iop_module_t *self)
{
    const dt_iop_overlay_gui_data_t *g = self->gui_data;
    dt_iop_overlay_params_t *p = self->params;
...
static void _on_drag_leave(GtkWidget *widget, GdkDragContext *dc, guint time,
                           const dt_iop_module_t *self)
{
    dt_iop_overlay_gui_data_t *g = self->gui_data;
...
void gui_init(dt_iop_module_t *self)
{
    ...
    gtk_drag_dest_set(GTK_WIDGET(g->area),  /* widget that will accept a drop */
    ...
    g_signal_connect(GTK_WIDGET(g->area), "drag-data-received", G_CALLBACK(_drag_and_drop_received),
                     self);
    g_signal_connect(GTK_WIDGET(g->area), "drag-motion", G_CALLBACK(_on_drag_motion), self);
    g_signal_connect(GTK_WIDGET(g->area), "drag-leave", G_CALLBACK(_on_drag_leave), self);
    ...
    gtk_widget_set_tooltip_text(g->scale_img,
    ...
    GtkWidget *label =
        dtgtk_reset_label_new(_("alignment"), self, &p->alignment, sizeof(p->alignment));
    ...
    {
        g->align[i] =
            dtgtk_togglebutton_new(dtgtk_cairo_paint_alignment, (CPF_SPECIAL_FLAG << i), NULL);
