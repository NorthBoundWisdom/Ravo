...
typedef struct dt_iop_profilegamma_params_t
{
    ...
    float
        shadows_range; // $MIN: -16.0 $MAX: 16.0 $DEFAULT: -5.0 $DESCRIPTION: "black relative exposure"
...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(
        self, _("correct input color profiles meant to be applied on non-linear RGB"),
...
void init_presets(dt_iop_module_so_t *self)
{
    ...
    dt_gui_presets_add_generic(_("16 EV dynamic range (generic)"), self->op, self->version(), &p,
                               sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);

    // 14 EV preset
    p.dynamic_range = 14.0f;
    p.shadows_range = -10.50f;
    dt_gui_presets_add_generic(_("14 EV dynamic range (generic)"), self->op, self->version(), &p,
                               sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);

    // 12 EV preset
    p.dynamic_range = 12.0f;
    p.shadows_range = -9.0f;
    dt_gui_presets_add_generic(_("12 EV dynamic range (generic)"), self->op, self->version(), &p,
                               sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);

    // 10 EV preset
    p.dynamic_range = 10.0f;
    p.shadows_range = -7.50f;
    dt_gui_presets_add_generic(_("10 EV dynamic range (generic)"), self->op, self->version(), &p,
                               sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);

    // 08 EV preset
    p.dynamic_range = 8.0f;
    p.shadows_range = -6.0f;
    dt_gui_presets_add_generic(_("08 EV dynamic range (generic)"), self->op, self->version(), &p,
                               sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
}
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    if (d->mode == PROFILEGAMMA_LOG)
    {
        ...
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->kernel_profilegamma_log, width, height, CLARG(dev_in), CLARG(dev_out),
...
void gui_init(dt_iop_module_t *self)
{
    ...
    gtk_widget_set_tooltip_text(g->grey_point,
                                _("adjust to match the average luma of the subject"));
    ...
    gtk_widget_set_tooltip_text(
        g->dynamic_range,
        _("number of stops between pure black and pure white\nthis is a reading a light meter would give you on the scene"));
