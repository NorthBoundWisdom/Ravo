...
{
    const float delta = GRAIN_LUT_DELTA_MAX * expf((mb / 100.0f) * logf(GRAIN_LUT_DELTA_MIN));
    const float exposure = -logf((1.0f + 2.0f * delta) / (density + delta) - 1.0f) *
                               (1.0f + 2.0f * delta) / (4.0f * gp) +
                           0.5f;
    return exposure;
}
...
static void evaluate_grain_lut(float *grain_lut, const float mb)
{
    for (int i = 0; i < GRAIN_LUT_SIZE; i++)
    {
        for (int j = 0; j < GRAIN_LUT_SIZE; j++)
        {
            ...
            grain_lut[j * GRAIN_LUT_SIZE + i] =
                100.0f * (paper_resp(gu + paper_resp_inverse(l, mb, GRAIN_LUT_PAPER_GAMMA), mb,
...
{
    return dt_iop_set_description(self, _("simulate silver grains from film"), _("creative"),
                                  _("non-linear, Lab, display-referred"), _("non-linear, Lab"),
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    for (int j = 0; j < roi_out->height; j++)
    {
        ...
        for (int i = 0; i < roi_out->width; i++)
        {
            ...
            out[0] =
                in[0] + dt_lut_lookup_2d_1c(data->grain_lut,
