...
{
    return dt_iop_set_description(self, _("add empty space to the left, top, right or bottom"),
...
int distort_transform(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece, float *points,
...
static void _compute_pos(const dt_iop_enlargecanvas_data_t *const d, float *pos_v, float *pos_h)
{
    *pos_v = .5f;
    *pos_h = .5f;
                  ...
                  const dt_iop_roi_t *const roi_out)
{
    ...
    dt_iop_setup_binfo(piece, roi_in, roi_out, pos_v, pos_h, bcolor, fcolor, 0.f, 0.f, &binfo);
...
void gui_init(dt_iop_module_t *self)
{
    ...
    gtk_widget_set_tooltip_text(g->percent_left, _("how much to enlarge the canvas to the left "
    ...
    gtk_widget_set_tooltip_text(g->percent_right, _("how much to enlarge the canvas to the right "
                                                    "as a percentage of the original image width"));

    g->percent_top = dt_bauhaus_slider_from_params(self, "percent_top");
    dt_bauhaus_slider_set_format(g->percent_top, "%");
    gtk_widget_set_tooltip_text(g->percent_top, _("how much to enlarge the canvas to the top "
