...
typedef enum dt_iop_filmic_rgb_gui_mode_t
{
    ...
    DT_FILMIC_GUI_BASECURVE =
        1, // basecurve-like GUI, showing the contrast and brightness curves, in lin/lin space
...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(
        self,
        _("apply a view transform to prepare the scene-referred pipeline\n"
          "for display on SDR screens and paper prints\n"
          "while preventing clipping in non-destructive ways"),
        _("corrective and creative"), _("linear or non-linear, RGB, scene-referred"),
        _("non-linear, RGB"), _("non-linear, RGB, display-referred"));
}
...
static inline float pixel_rgb_norm_power(const dt_aligned_pixel_t pixel)
{
    ...
    return numerator /
           fmaxf(denominator, 1e-12f); // prevent from division-by-0 (note: (1e-6)^2 = 1e-12
}

DT_OMP_DECLARE_SIMD(aligned(pixel : 16) uniform(variant, work_profile))
static inline float get_pixel_norm(const dt_aligned_pixel_t pixel,
                                   ...
                                   const dt_iop_order_iccprofile_info_t *const work_profile)
{
    ...
    case (DT_FILMIC_METHOD_LUMINANCE):
        return (work_profile) ? dt_ioppr_get_rgb_matrix_luminance(
                                    pixel, work_profile->matrix_in, work_profile->lut_in,
                                      ...
                                      const float dynamic_range)
{
    ...
    for_each_channel(c, aligned(mapped, x)) scaled[c] = (x[c] / grey);
    ...
    for_each_channel(c, aligned(scaled, log)) log[c] = log2f(scaled[c]);
    for_each_channel(c) mapped[c] = (log[c] - black) / dynamic_range;
...
DT_OMP_DECLARE_SIMD(aligned(M1, M2, M3, M4 : 16)
                        uniform(M1, M2, M3, M4, M5, latitude_min, latitude_max))
static inline float filmic_spline(const float x, const dt_aligned_pixel_t M1,
                                  const dt_aligned_pixel_t M2, const dt_aligned_pixel_t M3,
                                  const dt_aligned_pixel_t M4, const dt_aligned_pixel_t M5,
...
static inline float filmic_desaturate_v2(const float x, const float sigma_toe,
...
DT_OMP_DECLARE_SIMD(aligned(in, mask, inpainted : 64)
                        uniform(width, height, noise_level, noise_distribution, threshold))
inline static void inpaint_noise(const float *const in, const float *const mask,
                                 float *const inpainted, const float noise_level,
                                 ...
                                 const size_t width, const size_t height)
{
    ...
    for (size_t i = 0; i < height; i++)
        for (size_t j = 0; j < width; j++)
        {
            ...
            uint32_t DT_ALIGNED_ARRAY state[4] = {splitmix32(j + 1), splitmix32((j + 1) * (i + 3)),
            ...
            for_each_channel(c, aligned(pix_in)) sigma[c] = pix_in[c] * noise_level / threshold;
            ...
            for_each_channel(c, aligned(pix_in, noise, pix_out)) pix_out[c] =
                MAX(pix_in[c] * (1.0f - weight) + weight * noise[c], 0.0f);
...
inline static void
wavelets_reconstruct_RGB(const float *const restrict HF, const float *const restrict LF,
                         const float *const restrict texture, const float *const restrict mask,
                         float *const restrict reconstructed, const size_t width,
                         const size_t height, const float gamma, const float gamma_comp,
                         const float beta, const float beta_comp, const float delta, const size_t s,
                         const size_t scales)
{
    DT_OMP_FOR_SIMD()
    for (size_t k = 0; k < 4 * height * width; k += 4)
    {
        ...
        for_each_channel(c, aligned(LF_c)) residual[c] =
            (s == scales - 1) ? (grey_residual + LF_c[c] * beta) : 0.f;
        for_each_channel(c, aligned(reconstructed)) reconstructed[k + c] +=
            alpha * (delta * details[c] + residual[c]);
    }
}

static inline void
wavelets_reconstruct_ratios(const float *const restrict HF, const float *const restrict LF,
                            const float *const restrict texture, const float *const restrict mask,
                            float *const restrict reconstructed, const size_t width,
                            const size_t height, const float gamma, const float gamma_comp,
                            const float beta, const float beta_comp, const float delta,
 ...
 */
    ...
    for (size_t k = 0; k < 4 * height * width; k += 4)
    {
        ...
        dt_aligned_pixel_t details;
        for_each_channel(c,
                         aligned(reconstructed : 64) aligned(HF_c, TT_c, LF_c : 16) linear(k : 4))
        {
            // synthesize interpolated/inpainted RGB channels color details residuals and weigh them
            // this brings back some color on top of the grey_residual
        ...
        for_each_channel(c, aligned(reconstructed, details, residual)) reconstructed[k + c] +=
            alpha * (delta * details[c] + residual[c]);
    }
}
                                    ...
                                    const size_t height)
{
    ...
    for (size_t k = 0; k < height * width; k++)
    {
        ...
        for_each_channel(c, aligned(in, mask, reconstructed)) re[c] =
            MAX(in[4 * k + c] * (1.f - mask[k]), 0.0f);
...
static int get_scales(const dt_iop_roi_t *roi_in, const dt_dev_pixelpipe_iop_t *const piece)
{
   ...
   */
    ...
    const size_t size =
        MAX(piece->buf_in.height * piece->iscale, piece->buf_in.width * piece->iscale);
    const int scales =
        floorf(log2f((2.0f * size * scale / ((BSPLINE_FSIZE - 1) * BSPLINE_FSIZE)) - 1.0f));
    return CLAMP(scales, 1, MAX_NUM_SCALES);
}

static inline gboolean reconstruct_highlights(
    const float *const restrict in, const float *const restrict mask,
    float *const restrict reconstructed, const dt_iop_filmicrgb_reconstruction_type_t variant,
    ...
    const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    for (int s = 0; s < scales; ++s)
    {
        ...
        const int mult =
            1 << s; // fancy-pants C notation for 2^s with integer type, don't be afraid

        // Compute wavelets low-frequency scales
        blur_2D_Bspline(detail, LF, temp, padded_size, roi_out->width, roi_out->height, mult,
                        TRUE); // clip negatives
        ...
        {
            for_each_channel(c) HF[4 * k + c] = detail[4 * k + c] - LF[4 * k + c];
        }

        // interpolate/blur/inpaint (same thing) the RGB high-frequency to fill holes
        blur_2D_Bspline(HF, HF_RGB, temp, padded_size, roi_out->width, roi_out->height, 1,
                        TRUE); // clip negatives
        ...
        if (variant == DT_FILMIC_RECONSTRUCT_RGB)
            wavelets_reconstruct_RGB(HF_RGB, LF, HF, mask, reconstructed, roi_out->width,
                                     roi_out->height, gamma, gamma_comp, beta, beta_comp, delta, s,
                                     scales);
        else if (variant == DT_FILMIC_RECONSTRUCT_RATIOS)
            wavelets_reconstruct_ratios(HF_RGB, LF, HF, mask, reconstructed, roi_out->width,
                                        roi_out->height, gamma, gamma_comp, beta, beta_comp, delta,
                                        s, scales);
    }
                                      ...
                                      const size_t height)
{
    ...
    for (size_t k = 0; k < height * width * 4; k += 4)
    {
        ...
        for_each_channel(c, aligned(pix_in, temp)) temp[c] = MAX(pix_in[c], NORM_MIN);
        ...
        const float desaturation =
            filmic_desaturate_v2(lum, data->sigma_toe, data->sigma_shoulder, data->saturation);
...
static inline void
filmic_chroma_v2_v3(const float *const restrict in, float *const restrict out,
                    const dt_iop_order_iccprofile_info_t *const work_profile,
                    const dt_iop_filmicrgb_data_t *const data,
                    ...
                    const dt_iop_filmicrgb_colorscience_type_t colorscience_version)
{
    ...
    for (size_t k = 0; k < 4 * height * width; k += 4)
    {
        ...
        // Log tone-mapping
        norm = log_tonemapping_v2_1ch(norm, data->grey_source, data->black_source,
                                      data->dynamic_range);

        // Get the desaturation value based on the log value
        const float desaturation =
            filmic_desaturate_v2(norm, data->sigma_toe, data->sigma_shoulder, data->saturation);
        ...
        for_each_channel(c, aligned(ratios)) ratios[c] =
            MAX(ratios[c] + (1.0f - ratios[c]) * (1.0f - desaturation), 0.0f);
...
static inline void filmic_desaturate_v4(const dt_aligned_pixel_t Ych_original,
                                        dt_aligned_pixel_t Ych_final, const float saturation)
{
    ...
    chroma_final =
        (filmic_brightens && filmic_resat) ?
            (chroma_original + chroma_final) / 2.f // force original lower sat if brightening
        :
        ((user_resat && filmic_desat) || user_desat) ?
            chroma_final +
                delta_chroma // allow resaturation only if filmic desaturated, allow desat anytime
            :
            chroma_final;
                                   ...
                                   const dt_aligned_pixel_t Ych_in, dt_aligned_pixel_t RGB_out)
{
    ...
    // into gamut.
    const float Y =
        CLAMP((Ych_in[0] + Ych_brightened[0]) / 2.f, CIE_Y_1931_to_CIE_Y_2006(display_black),
              CIE_Y_1931_to_CIE_Y_2006(display_white));

    const float cos_h = Ych_in[2];
    const float sin_h = Ych_in[3];
    const float new_chroma =
        MIN(Ych_in[1], Ych_max_chroma(matrix_out, display_white, Y, cos_h, sin_h));
...
DT_OMP_DECLARE_SIMD(uniform(input_matrix_trans, output_matrix, export_input_matrix_trans,
                            export_output_matrix, use_output_profile)
                        aligned(Ych_final, Ych_original, pix_out : 16)
                            aligned(input_matrix_trans, output_matrix, export_input_matrix_trans,
                                    export_output_matrix : 64))
static inline void
gamut_mapping(dt_aligned_pixel_t Ych_final, dt_aligned_pixel_t Ych_original,
              dt_aligned_pixel_t pix_out, const dt_colormatrix_t input_matrix_trans,
              const dt_colormatrix_t output_matrix, const dt_colormatrix_t output_matrix_trans,
              const dt_colormatrix_t export_input_matrix_trans,
              const dt_colormatrix_t export_output_matrix,
              const dt_colormatrix_t export_output_matrix_trans, const float display_black,
              const float display_white, const float saturation, const int use_output_profile)
{
    ...
    if (!use_output_profile)
    {
        ...
        gamut_check_RGB(input_matrix_trans, output_matrix, output_matrix_trans, display_black,
                        display_white, Ych_final, pix_out);
    }
    else
    {
        // Now, it is still possible that one channel > display white because of saturation.
...
DT_OMP_DECLARE_SIMD(uniform(work_profile, data, spline, norm_min, norm_max, display_black,
                            display_white, type) aligned(pix_in, pix_out : 16))
static inline void norm_tone_mapping_v4(const dt_aligned_pixel_t pix_in, dt_aligned_pixel_t pix_out,
...
DT_OMP_DECLARE_SIMD(uniform(data, spline, display_black, display_white)
                        aligned(pix_in, pix_out : 16))
static inline void RGB_tone_mapping_v4(const dt_aligned_pixel_t pix_in, dt_aligned_pixel_t pix_out,
                                       const dt_iop_filmicrgb_data_t *const data,
                                       const dt_iop_filmic_rgb_spline_t spline,
                                       const float display_black, const float display_white)
{
                                    ...
                                    const float display_black, const float display_white)
{
    ...
    const int use_output_profile = filmic_v4_prepare_matrices(
        input_matrix_trans, output_matrix, output_matrix_trans, export_input_matrix_trans,
        export_output_matrix, export_output_matrix_trans, work_profile, export_profile);

    const float norm_min =
        exp_tonemapping_v2(0.f, data->grey_source, data->black_source, data->dynamic_range);
    const float norm_max =
        exp_tonemapping_v2(1.f, data->grey_source, data->black_source, data->dynamic_range);
    ...
    for (size_t k = 0; k < 4 * height * width; k += 4)
    {
        const float *const restrict pix_in = in + k;
        dt_aligned_pixel_t pix_out;
        norm_tone_mapping_v4(pix_in, pix_out, variant, work_profile, data, spline, norm_min,
                             norm_max, display_black, display_white);
        ...
        gamut_mapping(Ych_final, Ych_original, pix_out, input_matrix_trans, output_matrix,
                      output_matrix_trans, export_input_matrix_trans, export_output_matrix,
                      export_output_matrix_trans, display_black, display_white, data->saturation,
                      use_output_profile);
        copy_pixel_nontemporal(out + k, pix_out);
    }
    dt_omploop_sfence(); // ensure that nontemporal writes complete before we attempt to read output
...
static inline void filmic_v5(const float *const restrict in, float *const restrict out,
                             ...
                             const dt_iop_filmicrgb_data_t *const data,
                             const dt_iop_filmic_rgb_spline_t spline, const size_t width,
                             const size_t height, const float display_black,
...

{
    ...
    for (size_t k = 0; k < height * width * 4; k += 4)
    {
        ...
        norm_tone_mapping_v4(pix_in, max_rgb, DT_FILMIC_METHOD_MAX_RGB, work_profile, data, spline,
                             norm_min, norm_max, display_black, display_white);

        // Mix max RGB with naive RGB
        dt_aligned_pixel_t pix_out;
        for_each_channel(c, aligned(pix_out, max_rgb, naive_rgb)) pix_out[c] =
            (0.5f - data->saturation) * naive_rgb[c] + (0.5f + data->saturation) * max_rgb[c];
        ...
        gamut_mapping(Ych_final, Ych_original, pix_out, input_matrix_trans, output_matrix,
                      output_matrix_trans, export_input_matrix_trans, export_output_matrix,
                      export_output_matrix_trans, display_black, display_white, 0.0f,
                      use_output_profile);
        copy_pixel_nontemporal(out + k, pix_out);
    }
    dt_omploop_sfence(); // ensure that nontemporal writes complete before we attempt to read output
                                ...
                                const size_t width, const size_t height)
{
    ...
    for (size_t k = 0; k < height * width; k++)
    {
        ...
        for_each_channel(c, aligned(out)) pix[c] = mask[k];
                                  ...
                                  const int variant, const size_t width, const size_t height)
{
    ...
    for (size_t k = 0; k < height * width * 4; k += 4)
    {
        ...
        for_each_channel(c, aligned(ratios, in)) ratio[c] = in[k + c] / norm;
                                  ...
                                  const size_t width, const size_t height)
{
    ...
    for (size_t k = 0; k < height * width; k++)
    {
        for_each_channel(c, aligned(norms, ratios)) ratios[4 * k + c] =
                     ...
                     dt_develop_tiling_t *tiling)
{
    ...
    const gboolean run_fast =
        !data->enable_highlight_reconstruction || dt_pipe_is_fast(piece->pipe);
             ...
             const dt_iop_roi_t *const roi_out)
{
   ...
   */
    ...
    const gboolean recover_highlights =
        ...
        mask_clipped_pixels(in, mask, data->normalize, data->reconstruct_feather, roi_out->width,
                            roi_out->height);
    ...
    if (!run_fast && recover_highlights && mask && reconstructed)
    {
        ...
        float *const restrict inpainted =
            dt_alloc_align_float((size_t)roi_out->width * roi_out->height * 4);
        gboolean success_1 = FALSE;
        gboolean success_2 = TRUE;
        if (inpainted)
        {
            inpaint_noise(in, mask, inpainted, data->noise_level / scale,
                          data->reconstruct_threshold, data->noise_distribution, roi_out->width,
                          roi_out->height);

            // diffuse particles with wavelets reconstruction
            // PASS 1 on RGB channels
            success_1 =
                reconstruct_highlights(inpainted, mask, reconstructed, DT_FILMIC_RECONSTRUCT_RGB,
        ...
        if (data->high_quality_reconstruction > 0 && success_1)
        {
            float *const restrict norms =
                dt_alloc_align_float((size_t)roi_out->width * roi_out->height);
            float *const restrict ratios =
                dt_alloc_align_float((size_t)roi_out->width * roi_out->height * 4);
...
#ifdef HAVE_OPENCL
static inline cl_int reconstruct_highlights_cl(const cl_mem in, const cl_mem mask,
                                               const cl_mem reconstructed,
                                               const dt_iop_filmicrgb_reconstruction_type_t variant,
                                               const dt_iop_filmicrgb_global_data_t *const gd,
                                               const dt_iop_filmicrgb_data_t *const data,
                                               ...
                                               const dt_iop_roi_t *const roi_in)
{
    ...
    // wavelets scales buffers
    const cl_mem LF_even =
        dt_opencl_alloc_device(devid, sizes[0], sizes[1], sizeof(float) * 4); // low-frequencies RGB
    const cl_mem LF_odd =
        dt_opencl_alloc_device(devid, sizes[0], sizes[1], sizeof(float) * 4); // low-frequencies RGB
    const cl_mem HF_RGB = dt_opencl_alloc_device(devid, sizes[0], sizes[1],
                                                 sizeof(float) * 4); // high-frequencies RGB
    const cl_mem HF_grey = dt_opencl_alloc_device(devid, sizes[0], sizes[1],
                                                  sizeof(float) * 4); // high-frequencies RGB backup
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_filmic_init_reconstruct, width, height,
                                           CLARG(in), CLARG(mask), CLARG(reconstructed),
    ...
    for (int s = 0; s < scales; ++s)
    {
        ...
        // Compute wavelets low-frequency scales
        err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_filmic_bspline_horizontal, width,
                                               height, CLARG(detail), CLARG(temp), CLARG(width),
                                               CLARG(height), CLARG(mult));
        if (err != CL_SUCCESS)
            goto error;

        err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_filmic_bspline_vertical, width,
                                               height, CLARG(temp), CLARG(LF), CLARG(width),
                                               CLARG(height), CLARG(mult));
        if (err != CL_SUCCESS)
            goto error;
        ...
        err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_filmic_wavelets_detail, width,
                                               height, CLARG(detail), CLARG(LF), CLARG(HF_RGB),
                                               CLARG(width), CLARG(height));
        if (err != CL_SUCCESS)
            goto error;
        ...
        err =
            dt_opencl_enqueue_copy_image(devid, HF_RGB, HF_grey, CLIMG_ORIGIN, CLIMG_ORIGIN, sizes);
        if (err != CL_SUCCESS)
            goto error;

        // interpolate/blur/inpaint (same thing) the RGB high-frequency to fill holes
        err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_filmic_bspline_vertical, width,
                                               height, CLARG(HF_RGB), CLARG(temp), CLARG(width),
                                               CLARG(height), CLARGINT(1));
        if (err != CL_SUCCESS)
            goto error;

        err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_filmic_bspline_horizontal, width,
                                               height, CLARG(temp), CLARG(HF_RGB), CLARG(width),
                                               CLARG(height), CLARGINT(1));
        if (err != CL_SUCCESS)
            goto error;
        ...
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->kernel_filmic_wavelets_reconstruct, width, height, CLARG(HF_RGB), CLARG(LF),
            CLARG(HF_grey), CLARG(mask), CLARG(reconstructed), CLARG(reconstructed), CLARG(width),
            CLARG(height), CLARG(gamma), CLARG(gamma_comp), CLARG(beta), CLARG(beta_comp),
            CLARG(delta), CLARG(s), CLARG(scales), CLARG(variant));
        if (err != CL_SUCCESS)
            goto error;
    }
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    const cl_mem input_matrix_cl =
        dt_opencl_copy_host_to_device_constant(devid, 12 * sizeof(float), input_matrix);
    const cl_mem output_matrix_cl =
        dt_opencl_copy_host_to_device_constant(devid, 12 * sizeof(float), output_matrix);
    ...
    if (use_output_profile)
    {
        ...
        export_input_matrix_cl =
            dt_opencl_copy_host_to_device_constant(devid, 12 * sizeof(float), export_input_matrix);
        export_output_matrix_cl =
            dt_opencl_copy_host_to_device_constant(devid, 12 * sizeof(float), export_output_matrix);
    }
    ...
    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_filmic_mask, width, height, CLARG(in), CLARG(mask), CLARG(width),
    ...
    if (self->dev->gui_attached && dt_pipe_is_full(piece->pipe))
    {
        ...
        if (g->show_mask)
        {
            err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_filmic_show_mask, width,
                                                   height, CLARG(mask), CLARG(dev_out),
                                                   CLARG(width), CLARG(height));
            dt_opencl_release_mem_object(mask);
            dt_ioppr_free_iccprofile_params_cl(&profile_info_cl, &profile_lut_cl, &dev_profile_info,
                                               &dev_profile_lut);
            return err;
        }
    }
    ...
    if (!run_fast && is_clipped > 0 && d->enable_highlight_reconstruction)
    {
        ...
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->kernel_filmic_inpaint_noise, width, height, CLARG(in), CLARG(mask),
            CLARG(inpainted), CLARG(width), CLARG(height), CLARG(noise_level),
            CLARG(d->reconstruct_threshold), CLARG(d->noise_distribution));
        if (err != CL_SUCCESS)
            goto error;

        // first step of highlight reconstruction in RGB
        reconstructed = dt_opencl_alloc_device(devid, sizes[0], sizes[1], sizeof(float) * 4);
        err = reconstruct_highlights_cl(inpainted, mask, reconstructed, DT_FILMIC_RECONSTRUCT_RGB,
                                        gd, d, piece, roi_in);
        if (err != CL_SUCCESS)
            goto error;
        ...
        if (d->high_quality_reconstruction > 0)
        {
            ...
            if (norms && ratios)
            {
                for (int i = 0; i < d->high_quality_reconstruction; i++)
                {
                    ...
                    err = dt_opencl_enqueue_kernel_2d_args(
                        devid, gd->kernel_filmic_compute_ratios, width, height,
                        CLARG(reconstructed), CLARG(norms), CLARG(ratios), CLARG(d->preserve_color),
                        CLARG(width), CLARG(height));
                    if (err != CL_SUCCESS)
                        goto error;

                    // second step of reconstruction over ratios
                    err = reconstruct_highlights_cl(ratios, mask, reconstructed,
                                                    DT_FILMIC_RECONSTRUCT_RATIOS, gd, d, piece,
                                                    roi_in);
                    if (err != CL_SUCCESS)
                        goto error;

                    // restore ratios to RGB
                    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_filmic_restore_ratios,
                                                           width, height, CLARG(reconstructed),
                                                           CLARG(norms), CLARG(reconstructed),
                                                           CLARG(width), CLARG(height));
                    if (err != CL_SUCCESS)
                        goto error;
                }
    ...
    {
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->kernel_filmic_rgb_split, width, height, CLARG(in), CLARG(dev_out),
            CLARG(width), CLARG(height), CLARG(d->dynamic_range), CLARG(d->black_source),
            CLARG(d->grey_source), CLARG(dev_profile_info), CLARG(dev_profile_lut),
            CLARG(use_work_profile), CLARG(d->sigma_toe), CLARG(d->sigma_shoulder),
            CLARG(d->saturation), CLARG(spline.M1), CLARG(spline.M2), CLARG(spline.M3),
            CLARG(spline.M4), CLARG(spline.M5), CLARG(spline.latitude_min),
            CLARG(spline.latitude_max), CLARG(d->output_power), CLARG(d->version),
            CLARG(spline.type[0]), CLARG(spline.type[1]), CLARG(input_matrix_cl),
            CLARG(output_matrix_cl), CLARG(black_display), CLARG(white_display),
            CLARG(use_output_profile), CLARG(export_input_matrix_cl),
            CLARG(export_output_matrix_cl));
    }
    else
    {
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->kernel_filmic_rgb_chroma, width, height, CLARG(in), CLARG(dev_out),
            CLARG(width), CLARG(height), CLARG(d->dynamic_range), CLARG(d->black_source),
            CLARG(d->grey_source), CLARG(dev_profile_info), CLARG(dev_profile_lut),
            CLARG(use_work_profile), CLARG(d->sigma_toe), CLARG(d->sigma_shoulder),
            CLARG(d->saturation), CLARG(spline.M1), CLARG(spline.M2), CLARG(spline.M3),
            CLARG(spline.M4), CLARG(spline.M5), CLARG(spline.latitude_min),
            CLARG(spline.latitude_max), CLARG(d->output_power), CLARG(d->preserve_color),
            CLARG(d->version), CLARG(spline.type[0]), CLARG(spline.type[1]), CLARG(input_matrix_cl),
            CLARG(output_matrix_cl), CLARG(black_display), CLARG(white_display),
            CLARG(use_output_profile), CLARG(export_input_matrix_cl),
...
static inline void _compute_output_power(const dt_iop_module_t *self, dt_iop_filmicrgb_params_t *p)
{
...
static void apply_auto_black(dt_iop_module_t *self)
{
    ...
    const dt_iop_order_iccprofile_info_t *const work_profile =
        dt_ioppr_get_iop_work_profile_info(self, self->dev->iop);
    const float black =
        get_pixel_norm(self->picked_color_min, DT_FILMIC_METHOD_MAX_RGB, work_profile);
...
static void apply_auto_white_point_source(dt_iop_module_t *self)
{
    ...
    const dt_iop_order_iccprofile_info_t *const work_profile =
        dt_ioppr_get_iop_work_profile_info(self, self->dev->iop);
    const float white =
        get_pixel_norm(self->picked_color_max, DT_FILMIC_METHOD_MAX_RGB, work_profile);
...
static void apply_autotune(dt_iop_module_t *self)
{
    ...
    {
        const float grey =
            get_pixel_norm(self->picked_color, p->preserve_color, work_profile) / 2.0f;
        p->grey_point_source = CLAMP(100.f * grey, 0.001f, 100.0f);
    }
...
static void show_mask_callback(GtkToggleButton *button, GdkEventButton *event,
                                                        ...
                                                        dt_iop_filmic_rgb_spline_t *const spline)
{
    ...
    if (p->custom_grey)
    {
        ...
        grey_display =
            powf(CLAMP(p->grey_point_target, p->black_point_target, p->white_point_target) / 100.0f,
                 1.0f / (p->output_power));
    }
    else
    {
    ...
    else //(p->spline_version >= DT_FILMIC_SPLINE_VERSION_V2)
    {
        ...
        white_display = powf(fmaxf(p->white_point_target, p->grey_point_target) / 100.0f,
                             1.0f / (p->output_power)); // in %
    }
    ...
    if (p->spline_version < DT_FILMIC_SPLINE_VERSION_V3)
    {
        const float latitude =
            CLAMP(p->latitude, 0.0f, 100.0f) / 100.0f * dynamic_range; // in % of dynamic range
    ...
    else // p->spline_version >= DT_FILMIC_SPLINE_VERSION_V3. Slope dependent on contrast only, and latitude as % of display range.
    {
        ...
        const float xmin =
            (black_display + SAFETY_MARGIN * (white_display - black_display) - linear_intercept) /
            contrast;
        const float xmax =
   ...
   **/
    ...
    {
        // 4th order polynom
        double A1[ORDER_4 * ORDER_4] = {
            1.,        1.,       1.,      1., 1.,  // position in 1
...
void gui_update(dt_iop_module_t *self)
{
    ...
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(g->enable_highlight_reconstruction),
                                 p->enable_highlight_reconstruction);
...
void reload_defaults(dt_iop_module_t *self)
{
    ...
    if (dt_image_is_matrix_correction_supported(&self->dev->image_storage) && is_scene_referred)
    {
...
void init_presets(dt_iop_module_so_t *self)
{
    ...
    {
        dt_gui_presets_add_generic(_("scene-referred default"), self->op, self->version(), NULL, 0,
        ...
        dt_gui_presets_update_autoapply(BUILTIN_PRESET("scene-referred default"), self->op,
                                        self->version(), TRUE);
    }
}
...
void init_global(dt_iop_module_so_t *self)
{
    ...
    gd->kernel_filmic_wavelets_reconstruct =
        dt_opencl_create_kernel(program, "wavelets_reconstruct");
...
static inline void dt_cairo_draw_arrow(cairo_t *cr, const double origin_x, const double origin_y,
                                       const double destination_x, const double destination_y,
                                       const gboolean show_head)
{
                          ...
                          const dt_iop_filmicrgb_gui_data_t *g)
{
    ...
    button->icon(cr, -scale * button->w / 2., -scale * button->h / 2., scale * button->w,
                 scale * button->h, 0, NULL);
    cairo_restore(cr);
}
...
static gboolean dt_iop_tonecurve_draw(GtkWidget *widget, cairo_t *crf, dt_iop_module_t *self)
{
    ...
    g->inner_padding = DT_PIXEL_APPLY_DPI(
        4); // TODO: INNER_PADDING value as defined in bauhaus.c macros, sync them
    ...
    g->graph_width =
        g->allocation.width - margin_right - margin_left; // align the right border on sliders
    ...
    {
        for (int i = 0; i < DT_FILMIC_GUI_BUTTON_LAST; i++)
            filmic_gui_draw_icon(cr, &g->buttons[i], g);
    }
    ...
    if (g->gui_mode != DT_FILMIC_GUI_RANGES)
    {
        ...
        cairo_rectangle(cr, -DT_PIXEL_APPLY_DPI(2.), -DT_PIXEL_APPLY_DPI(2.),
                        g->graph_width + 2. * DT_PIXEL_APPLY_DPI(2.),
        ...
        if (p->version == DT_FILMIC_COLORSCIENCE_V2 || p->version == DT_FILMIC_COLORSCIENCE_V3)
        {
            cairo_move_to(
                cr, 0,
                g->graph_height *
                    (1.0 - filmic_desaturate_v2(0.0f, sigma_toe, sigma_shoulder, saturation)));
            for (int k = 1; k < 256; k++)
            {
                float x = k / 255.0;
        ...
        float y_start = clamp_simd(filmic_spline(x_start, g->spline.M1, g->spline.M2, g->spline.M3,
                                                 g->spline.M4, g->spline.M5, g->spline.latitude_min,
                                                 g->spline.latitude_max, g->spline.type));
        ...
        for (int k = 1; k < 256; k++)
        {
            ...
            if (g->gui_mode == DT_FILMIC_GUI_BASECURVE ||
                g->gui_mode == DT_FILMIC_GUI_BASECURVE_LOG)
                value = log_tonemapping_v2_1ch(x, grey, p->black_point_source, DR);

            if (g->gui_mode == DT_FILMIC_GUI_BASECURVE_LOG)
                x = dt_log_scale_axis(x, LOGBASE);

            float y = filmic_spline(value, g->spline.M1, g->spline.M2, g->spline.M3, g->spline.M4,
        ...
        cairo_rectangle(cr, -DT_PIXEL_APPLY_DPI(4.), -DT_PIXEL_APPLY_DPI(4.),
                        g->graph_width + 2. * DT_PIXEL_APPLY_DPI(4.),
                        g->graph_height + 2. * DT_PIXEL_APPLY_DPI(4.));
        cairo_clip(cr);
        ...
        {
            x_grey = dt_log_scale_axis(exp_tonemapping_v2(x_grey, grey, p->black_point_source, DR),
                                       LOGBASE);
            y_grey = dt_log_scale_axis(powf(y_grey, p->output_power), LOGBASE);
        }

        cairo_set_source_rgb(cr, 0.75, 0.5, 0.0);
        cairo_arc(cr, x_grey * g->graph_width, (1.0 - y_grey) * g->graph_height,
                  DT_PIXEL_APPLY_DPI(6), 0, 2. * M_PI);
        cairo_fill(cr);
        cairo_stroke(cr);
        ...
        const float central_slope = (g->spline.y[3] - g->spline.y[1]) * g->graph_width /
                                    ((g->spline.x[3] - g->spline.x[1]) * g->graph_height);
        ...
        for (int k = 0; k < 5; k++)
        {
            if (k != 2) // k == 2 : grey point, already processed above
            {
                ...
                gboolean red =
                    (((k == 1) && (y - ymin <= y_margin)) || ((k == 3) && (ymax - y <= y_margin)));
                ...
                if (red)
                    cairo_set_source_rgb(cr, 0.8, 0.35, 0.35);

                // draw bullet
                cairo_arc(cr, x * g->graph_width, (1.0 - y) * g->graph_height,
        ...
        if (g->gui_show_labels)
        {
            ...
            pango_layout_set_text(layout, text, -1);
            pango_layout_get_pixel_extents(layout, &g->ink, NULL);
            cairo_move_to(cr, x_grey * g->graph_width - 0.5 * g->ink.width - g->ink.x,
                          x_legend_top);
            pango_cairo_show_layout(cr, layout);
            cairo_stroke(cr);
            ...
            if (g->gui_mode == DT_FILMIC_GUI_LOOK)
                ...
                snprintf(text, sizeof(text), "%.0f",
                         exp2f(p->black_point_source) * p->grey_point_source);

            pango_layout_set_text(layout, text, -1);
            pango_layout_get_pixel_extents(layout, &g->ink, NULL);
            cairo_move_to(cr, x_black * g->graph_width - 0.5 * g->ink.width - g->ink.x,
                          x_legend_top);
            pango_cairo_show_layout(cr, layout);
            cairo_stroke(cr);
                     ...
                     g->gui_mode == DT_FILMIC_GUI_BASECURVE_LOG)
            {
                if (x_white > 1.f)
                    snprintf(text, sizeof(text), "%.0f →",
                             100.f); // this marks the bound of the graph, not the actual white
                else
                    snprintf(text, sizeof(text), "%.0f",
                             exp2f(p->white_point_source) * p->grey_point_source);
            }
                ...
                (x_white > 1.f))
            {
                ...
                snprintf(text, sizeof(text), _("(%.0f %%)"),
                         exp2f(p->white_point_source) * p->grey_point_source);
                pango_layout_set_text(layout, text, -1);
                pango_layout_get_pixel_extents(layout, &g->ink, NULL);
    ...
    else
    {
        ...
        if (g->gui_show_labels)
        {
            ...
            cairo_set_line_width(cr, DT_PIXEL_APPLY_DPI(1.));
            dt_cairo_draw_arrow(cr, fminf(scene_label_width, display_label_width) / 2.f,
                                y_scene - g->line_height,
        ...
        const float column_right =
            g->allocation.width - column_left - darktable.bauhaus->quad_width;

        // compute dynamic ranges left and right to middle grey
        const float display_HL_EV =
            -log2f(p->grey_point_target / p->white_point_target); // compared to white EV
        ...
        const float max_DR =
            ceilf(fmaxf(display_HL_EV, scene_HL_EV)) + ceilf(fmaxf(display_LL_EV, scene_LL_EV));
        ...
        float display_lat_bottom = filmic_spline(
            g->spline.latitude_min, g->spline.M1, g->spline.M2, g->spline.M3, g->spline.M4,
            g->spline.M5, g->spline.latitude_min, g->spline.latitude_max, g->spline.type);
        display_lat_bottom =
            powf(fmaxf(display_lat_bottom, NORM_MIN), p->output_power); // clamp at -16 EV
        ...
        // Compute usual filmic  mapping
        float display_lat_top = filmic_spline(
            g->spline.latitude_max, g->spline.M1, g->spline.M2, g->spline.M3, g->spline.M4,
            g->spline.M5, g->spline.latitude_min, g->spline.latitude_max, g->spline.type);
        display_lat_top =
            powf(fmaxf(display_lat_top, NORM_MIN), p->output_power); // clamp at -16 EV
        ...
        for (int i = floorf(p->black_point_source); i < ceilf(p->white_point_source); i++)
        {
            ...
            if ((float)i > p->black_point_source && (float)i < p->white_point_source)
            {
                ...
                float y_temp = filmic_spline(normal_value, g->spline.M1, g->spline.M2, g->spline.M3,
                                             g->spline.M4, g->spline.M5, g->spline.latitude_min,
                                             g->spline.latitude_max, g->spline.type);
                                   ...
                                   const dt_iop_module_t *self)
{
    ...
    if (g->gui_hover)
    {
        ...
        for (int i = 0; i < DT_FILMIC_GUI_BUTTON_LAST; i++)
        {
            ...
            if (x > g->buttons[i].left && x < g->buttons[i].right && y > g->buttons[i].top &&
                y < g->buttons[i].bottom)
            {
        ...
        if (save_active_button != g->active_button)
            gtk_widget_queue_draw(GTK_WIDGET(g->area));
        return TRUE;
    }
    else
...
void gui_init(dt_iop_module_t *self)
{
    ...
    gtk_widget_set_tooltip_text(
        g->auto_button, _("try to optimize the settings with some statistical assumptions.\n"
    ...
    dt_gui_box_add(self->widget, dt_ui_section_label_new(C_("section", "highlights clipping")));

    g->enable_highlight_reconstruction =
        dt_bauhaus_toggle_from_params(self, "enable_highlight_reconstruction");
    ...
    GtkWidget *hbox = dt_gui_hbox(dt_ui_label_new(_("display highlight reconstruction mask")));
    g->show_highlight_mask = dt_iop_togglebutton_new(
        self, NULL, N_("display highlight reconstruction mask"), NULL,
        G_CALLBACK(show_mask_callback), FALSE, 0, 0, dtgtk_cairo_paint_showmask, hbox);
    dtgtk_togglebutton_set_paint(DTGTK_TOGGLEBUTTON(g->show_highlight_mask),
    ...
    dt_gui_box_add(self->widget, hbox, dt_ui_section_label_new(C_("section", "balance")));

    g->reconstruct_structure_vs_texture =
        dt_bauhaus_slider_from_params(self, "reconstruct_structure_vs_texture");
    ...
    g->reconstruct_bloom_vs_details =
        dt_bauhaus_slider_from_params(self, "reconstruct_bloom_vs_details");
    ...
    gtk_widget_set_tooltip_text(
        g->grey_point_target, _("middle gray value of the target display or color space.\n"
    ...
    gtk_widget_set_tooltip_text(
        g->version, _("v3 is darktable 3.0 desaturation method, same as color balance.\n"
    ...
    gtk_widget_set_tooltip_text(
        g->highlights, _("choose the desired curvature of the filmic spline in highlights.\n"
    ...
    gtk_widget_set_tooltip_text(
        g->shadows, _("choose the desired curvature of the filmic spline in shadows.\n"
                      "hard uses a high curvature resulting in more tonal compression.\n"
                      "soft uses a low curvature resulting in less tonal compression."));
    ...
    gtk_widget_set_tooltip_text(
        g->auto_hardness,
        _("enable to auto-set the look hardness depending on the scene white and black points.\n"
    ...
    g->high_quality_reconstruction =
        dt_bauhaus_slider_from_params(self, "high_quality_reconstruction");
    gtk_widget_set_tooltip_text(
        g->high_quality_reconstruction,
...
void gui_changed(dt_iop_module_t *self, GtkWidget *w, void *previous)
{
    ...
    if (!w || w == g->version)
    {
        if (p->version == DT_FILMIC_COLORSCIENCE_V4)
        {
            ...
            gtk_widget_set_tooltip_text(
                g->saturation, _("desaturates the output of the module\n"
                                 "specifically at extreme luminances.\n"
                                 "increase if shadows and/or highlights are under-saturated."));
        }
    ...
    {
        gtk_widget_set_sensitive(g->reconstruct_bloom_vs_details,
                                 p->enable_highlight_reconstruction);
        gtk_widget_set_sensitive(g->reconstruct_grey_vs_color, p->enable_highlight_reconstruction);
        gtk_widget_set_sensitive(g->reconstruct_structure_vs_texture,
                                 p->enable_highlight_reconstruction);
    }
