...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self,
                                  ...
                                  _("quasi-linear, RGB"), _("quasi-linear, RGB, scene-referred"));
}
                     ...
                     dt_develop_tiling_t *tiling)
{
    ...
    if (data->use_filter)
    {
        ...
        tiling->factor += scaling == 1.0f ? 3.0f : (1.0f + 4.0f / sqrf(scaling));
    }
}
...
static void _finish_covariance(const size_t pixels, const float *const restrict UV,
...
static void _prepare_prefilter(const size_t pixels, const float *const restrict UV,
                               const float *const restrict covariance, float *const restrict a,
                               float *const restrict b, const float eps)
{
    DT_OMP_FOR()
    for (size_t k = 0; k < pixels; k++)
    {
        ...
        const dt_aligned_pixel_t Sigma = {covariance[4 * k + 0] + eps, covariance[4 * k + 1],
                                          covariance[4 * k + 2], covariance[4 * k + 3] + eps};
        ...
        {
            const dt_aligned_pixel_t sigma_inv = {Sigma[3] / det, -Sigma[1] / det, -Sigma[2] / det,
                                                  Sigma[0] / det};
            // find a_1, a_2 s.t. U' = a_1 * U + a_2 * V
            a[4 * k + 0] =
                (covariance[4 * k + 0] * sigma_inv[0] + covariance[4 * k + 1] * sigma_inv[1]);
            a[4 * k + 1] =
                (covariance[4 * k + 0] * sigma_inv[2] + covariance[4 * k + 1] * sigma_inv[3]);

            // find a_3, a_4 s.t. V' = a_3 * U + a_4 V
            a[4 * k + 2] =
                (covariance[4 * k + 2] * sigma_inv[0] + covariance[4 * k + 3] * sigma_inv[1]);
            a[4 * k + 3] =
                (covariance[4 * k + 2] * sigma_inv[2] + covariance[4 * k + 3] * sigma_inv[3]);
        }
        else
        {
...
static void _apply_prefilter(const size_t npixels, const float sat_shift, float *const restrict UV,
                             const float *const restrict saturation, const float *const restrict a,
...
static void _prefilter_chromaticity(float *const restrict UV,
                                    const float *const restrict saturation, const int width,
                                    const int height, const float sigma, const float eps,
...
static void _guide_with_chromaticity(float *const restrict UV, float *const restrict corrections,
                                     const float *const restrict saturation,
                                     float *const restrict b_corrections,
                                     const float *const restrict gradients, const int width,
                                     const int height, const float sigma, const float eps,
                                     const float bright_shift, const float sat_shift)
{
    ...
    if (resized)
    {
        ...
        {
            interpolate_bilinear(UV, width, height, ds_UV, ds_width, ds_height, 2);
            interpolate_bilinear(corrections, width, height, ds_corrections, ds_width, ds_height,
                                 2);
            interpolate_bilinear(b_corrections, width, height, ds_b_corrections, ds_width,
                                 ds_height, 1);
        }
        else
        {
    ...
    DT_OMP_FOR_SIMD(
        aligned(ds_UV, covariance, correlations, ds_corrections, ds_b_corrections, ds_a, ds_b : 64))
    for (size_t k = 0; k < ds_pixels; k++)
    {
        // Extract the 2×2 covariance matrix sigma = cov(U, V) at current pixel
        ...
        if (fabsf(det) > 4.f * FLT_EPSILON)
        {
            const dt_aligned_pixel_t sigma_inv = {Sigma[3] / det, -Sigma[1] / det, -Sigma[2] / det,
                                                  Sigma[0] / det};
            ds_a[4 * k + 0] =
                (correlations[4 * k + 0] * sigma_inv[0] + correlations[4 * k + 1] * sigma_inv[1]);
            ds_a[4 * k + 1] =
                (correlations[4 * k + 0] * sigma_inv[2] + correlations[4 * k + 1] * sigma_inv[3]);
            ds_a[4 * k + 2] =
                (correlations[4 * k + 2] * sigma_inv[0] + correlations[4 * k + 3] * sigma_inv[1]);
            ds_a[4 * k + 3] =
                (correlations[4 * k + 2] * sigma_inv[2] + correlations[4 * k + 3] * sigma_inv[3]);
        }
        else
        {
        ...
        ds_b[2 * k + 0] = ds_corrections[2 * k + 1] - ds_a[4 * k + 0] * ds_UV[2 * k + 0] -
                          ds_a[4 * k + 1] * ds_UV[2 * k + 1];
        ds_b[2 * k + 1] = ds_b_corrections[k] - ds_a[4 * k + 2] * ds_UV[2 * k + 0] -
                          ds_a[4 * k + 3] * ds_UV[2 * k + 1];
    }
...
static void _prepare_process(const float roi_scale, const dt_iop_colorequal_data_t *d,

                             // parameters to be setup
                             float *white, float *sat_shift, float *max_brightness_shift,
                             float *corr_max_brightness_shift, float *bright_shift,
                             float *gradient_amp, float *hue_sigma, float *par_sigma,
                             float *sat_sigma, float *scharr_sigma)
{
...
void process(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece, const void *const i,
             void *const o, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    if (!dt_iop_have_required_input_format(4 /*we need full-color pixels*/, self, piece->colors, i,
                                           o, roi_in, roi_out))
        return;
    ...
    if (!dt_iop_alloc_image_buffers(
            self, roi_in, roi_out, 2 /*ch per pix*/ | DT_IMGSZ_OUTPUT | DT_IMGSZ_FULL, &UV,
            2 | DT_IMGSZ_OUTPUT | DT_IMGSZ_FULL, &corrections, 1 | DT_IMGSZ_OUTPUT | DT_IMGSZ_FULL,
            &b_corrections, 1 | DT_IMGSZ_OUTPUT | DT_IMGSZ_FULL, &Lscharr,
            1 | DT_IMGSZ_OUTPUT | DT_IMGSZ_FULL, &saturation, 0 /*end of list*/))
    {
    ...
    float white, sat_shift, max_brightness_shift, corr_max_brightness_shift, bright_shift,
        gradient_amp, hue_sigma, par_sigma, sat_sigma, scharr_sigma;
    _prepare_process(roi_in->scale / piece->iscale, d, &white, &sat_shift, &max_brightness_shift,
                     &corr_max_brightness_shift, &bright_shift, &gradient_amp, &hue_sigma,
                     &par_sigma, &sat_sigma, &scharr_sigma);
    ...
    if (d->use_filter && !run_fast)
        _prefilter_chromaticity(UV, saturation, width, height, hue_sigma, d->chroma_feathering,
                                sat_shift);
    ...
    for (int row = 0; row < height; row++)
    {
        for (int col = 0; col < width; col++)
        {
            ...
            if (d->use_filter)
            {
                ...
                Lscharr[k] =
                    gradient_amp * sqrf(MAX(0.0f, scharr_gradient(&saturation[kk], width) - 0.02f));
            }
    ...
    if (d->use_filter && !run_fast)
    {
        ...
        _guide_with_chromaticity(UV, corrections, saturation, b_corrections, Lscharr, width, height,
                                 par_sigma, d->param_feathering, bright_shift, sat_shift);
    }
    ...
    else
    {
        ...
        if ((mode == BRIGHTNESS_GRAD) || (mode == SATURATION_GRAD))
        {
            ...
            {
                const float weight = _get_satweight(
                    (float)col / scaler - (mode == SATURATION_GRAD ? sat_shift : bright_shift));
...
static cl_mem _init_covariance_cl(const int devid, const dt_iop_colorequal_global_data_t *gd,
                                  const cl_mem UV, const int width, const int height)
{
    ...
    const cl_int err =
        dt_opencl_enqueue_kernel_2d_args(devid, gd->ce_init_covariance, width, height,
...
static int _prefilter_chromaticity_cl(const int devid, const dt_iop_colorequal_global_data_t *gd,
                                      const cl_mem UV, const cl_mem saturation, const cl_mem weight,
                                      const int width, const int height, const float sigma,
                                      const float eps, const float sat_shift)
{
    ...
    if (resized)
    {
        ...
        err = dt_opencl_enqueue_kernel_2d_args(devid, gd->ce_bilinear2, ds_width, ds_height,
                                               CLARG(UV), CLARG(width), CLARG(height), CLARG(ds_UV),
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->ce_finish_covariance, ds_width, ds_height,
                                           CLARG(covariance), CLARG(ds_UV), CLARG(ds_width),
                                           CLARG(ds_height));
    if (err != CL_SUCCESS)
        goto error;
    ...
    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->ce_prepare_prefilter, ds_width, ds_height, CLARG(ds_UV), CLARG(covariance),
        CLARG(ds_a), CLARG(ds_b), CLARG(eps), CLARG(ds_width), CLARG(ds_height));
    if (err != CL_SUCCESS)
        goto error;
    ...
    if (resized)
    {
        ...
        err = dt_opencl_enqueue_kernel_2d_args(devid, gd->ce_bilinear4, width, height, CLARG(ds_a),
                                               CLARG(ds_width), CLARG(ds_height), CLARG(a),
                                               CLARG(width), CLARG(height));
        if (err != CL_SUCCESS)
            goto error;

        err = dt_opencl_enqueue_kernel_2d_args(devid, gd->ce_bilinear2, width, height, CLARG(ds_b),
                                               CLARG(ds_width), CLARG(ds_height), CLARG(b),
                                               CLARG(width), CLARG(height));
        if (err != CL_SUCCESS)
            goto error;
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->ce_apply_prefilter, width, height, CLARG(UV),
                                           CLARG(saturation), CLARG(a), CLARG(b), CLARG(weight),
...
static int _guide_with_chromaticity_cl(const int devid, const dt_iop_colorequal_global_data_t *gd,
                                       const cl_mem UV, const cl_mem corrections,
                                       const cl_mem saturation, const cl_mem b_corrections,
                                       const cl_mem scharr, const cl_mem weight, const int width,
                                       const int height, const float sigma, const float eps,
                                       const float bright_shift, const float sat_shift)
{
    const float scaling = _get_scaling(sigma);
    ...
    if (resized)
    {
        ...
        if (ds_UV == NULL || ds_corrections == NULL || ds_b_corrections == NULL)
            goto error;
        ...
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->ce_bilinear2, ds_width, ds_height, CLARG(corrections), CLARG(width),
            CLARG(height), CLARG(ds_corrections), CLARG(ds_width), CLARG(ds_height));
        if (err != CL_SUCCESS)
            goto error;

        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->ce_bilinear1, ds_width, ds_height, CLARG(b_corrections), CLARG(width),
            CLARG(height), CLARG(ds_b_corrections), CLARG(ds_width), CLARG(ds_height));
        if (err != CL_SUCCESS)
            goto error;
    }
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->ce_prepare_correlations, ds_width, ds_height,
                                           CLARG(ds_corrections), CLARG(ds_b_corrections),
                                           CLARG(ds_UV), CLARG(correlations), CLARG(ds_width),
                                           CLARG(ds_height));
    if (err != CL_SUCCESS)
        goto error;
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->ce_finish_correlations, ds_width, ds_height,
                                           CLARG(ds_corrections), CLARG(ds_b_corrections),
                                           CLARG(ds_UV), CLARG(correlations), CLARG(covariance),
                                           CLARG(ds_width), CLARG(ds_height));
    if (err != CL_SUCCESS)
        goto error;

    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->ce_final_guide, ds_width, ds_height, CLARG(covariance), CLARG(correlations),
        CLARG(ds_corrections), CLARG(ds_b_corrections), CLARG(ds_UV), CLARG(ds_a), CLARG(ds_b),
        CLARG(eps), CLARG(ds_width), CLARG(ds_height));
    if (err != CL_SUCCESS)
        goto error;
    ...
    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->ce_apply_guided, width, height, CLARG(UV), CLARG(saturation), CLARG(scharr),
        CLARG(a), CLARG(b), CLARG(corrections), CLARG(b_corrections), CLARG(weight),
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    const dt_iop_colorequal_global_data_t *const gd =
        (dt_iop_colorequal_global_data_t *)self->global_data;
    ...
    const cl_mem gamut_LUT =
        dt_opencl_copy_host_to_device_constant(devid, LUT_ELEM * sizeof(float), d->gamut_LUT);
    const cl_mem LUT_saturation =
        dt_opencl_copy_host_to_device_constant(devid, LUT_ELEM * sizeof(float), d->LUT_saturation);
    const cl_mem LUT_hue =
        dt_opencl_copy_host_to_device_constant(devid, LUT_ELEM * sizeof(float), d->LUT_hue);
    const cl_mem LUT_brightness =
        dt_opencl_copy_host_to_device_constant(devid, LUT_ELEM * sizeof(float), d->LUT_brightness);
    const cl_mem weight = dt_opencl_copy_host_to_device_constant(
        devid, (2 * SATSIZE + 1) * sizeof(float), satweights);
    ...
    if (input_matrix_cl == NULL || output_matrix_cl == NULL || gamut_LUT == NULL ||
        LUT_saturation == NULL || LUT_hue == NULL || LUT_brightness == NULL || weight == NULL ||
        pixout == NULL || UV == NULL || corrections == NULL || b_corrections == NULL ||
        Lscharr == NULL || saturation == NULL)
        goto error;

    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->ce_sample_input, width, height, CLARG(dev_in), CLARG(saturation), CLARG(Lscharr),
        CLARG(UV), CLARG(pixout), CLARG(input_matrix_cl), CLARG(width), CLARG(height));
    if (err != CL_SUCCESS)
        goto error;
    ...
    if (guiding && !run_fast)
    {
        err = _prefilter_chromaticity_cl(devid, gd, UV, saturation, weight, width, height,
                                         hue_sigma, d->chroma_feathering, sat_shift);
        if (err != CL_SUCCESS)
            goto error;
    }

    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->ce_process_data, width, height, CLARG(UV), CLARG(Lscharr), CLARG(saturation),
        CLARG(corrections), CLARG(b_corrections), CLARG(pixout), CLARG(LUT_saturation),
        CLARG(LUT_hue), CLARG(LUT_brightness), CLARG(white), CLARG(gradient_amp), CLARG(guiding),
        CLARG(width), CLARG(height));
    if (err != CL_SUCCESS)
        goto error;
    ...
    if (guiding && !run_fast)
    {
        ...
        err = _guide_with_chromaticity_cl(devid, gd, UV, corrections, saturation, b_corrections,
                                          Lscharr, weight, width, height, par_sigma,
                                          d->param_feathering, bright_shift, sat_shift);
        if (err != CL_SUCCESS)
            goto error;
    }
    ...
    {
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->ce_write_output, width, height, CLARG(dev_out), CLARG(pixout),
            CLARG(corrections), CLARG(b_corrections), CLARG(output_matrix_cl), CLARG(gamut_LUT),
            CLARG(white), CLARG(width), CLARG(height));
    }
    else
    {
        piece->pipe->mask_display = DT_DEV_PIXELPIPE_DISPLAY_PASSTHRU;
        const int mode = mask_mode - 1;
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->ce_write_visual, width, height, CLARG(dev_out), CLARG(pixout),
            CLARG(corrections), CLARG(b_corrections), CLARG(saturation), CLARG(Lscharr),
        ...
        if (mode == BRIGHTNESS_GRAD || mode == SATURATION_GRAD)
        {
            err = dt_opencl_enqueue_kernel_1d_args(
                devid, gd->ce_draw_weight, width, CLARG(dev_out), CLARG(weight),
                CLARG(bright_shift), CLARG(sat_shift), CLARG(mode), CLARG(width), CLARG(height));
        }
    }
...
static inline void _periodic_RBF_interpolate(float nodes[NODES], const float smoothing,
                                             float *const LUT, const float hue_shift,
                                             const gboolean clip)
{
        ...
        for (int j = 0; j < NODES; j++)
        {
            for (int l = 0; l < m; l++)
            {
                A[i][j] += _cosine_coeffs(l, smoothing) *
                           cosf(((float)l) *
                                fabsf(_get_hue_node(i, hue_shift) - _get_hue_node(j, hue_shift)));
            }
            A[i][j] = expf(A[i][j]);
        }
...
static inline void _pack_saturation(const dt_iop_colorequal_params_t *p, float array[NODES])
{
    array[0] = p->sat_red;
...
static inline void _pack_hue(const dt_iop_colorequal_params_t *p, float array[NODES])
{
    array[0] = p->hue_red;
...
static inline void _pack_brightness(const dt_iop_colorequal_params_t *p, float array[NODES])
{
                   ...
                   dt_dev_pixelpipe_iop_t *piece)
{
    ...
    _periodic_RBF_interpolate(sat_values, M_PI_F, d->LUT_saturation, d->hue_shift, TRUE);

    _pack_hue(p, hue_values);
    _periodic_RBF_interpolate(hue_values, 1.f / p->smoothing_hue * M_PI_F, d->LUT_hue, d->hue_shift,
                              FALSE);
    ...
    _periodic_RBF_interpolate(bright_values, M_PI_F, d->LUT_brightness, d->hue_shift, TRUE);
...
static inline void
_build_dt_UCS_HSB_gradients(dt_aligned_pixel_t HSB, dt_aligned_pixel_t RGB,
                            ...
                            const float *const gamut_LUT)
{
    ...
    if (work_profile)
    {
        dt_ioppr_xyz_to_rgb_matrix(XYZ_D65, RGB, work_profile->matrix_out_transposed,
...
static inline void
_draw_sliders_saturation_gradient(const float sat_min, const float sat_max, const float hue,
                                  const float brightness, GtkWidget *const slider,
                                  const struct dt_iop_order_iccprofile_info_t *work_profile,
                                  const float *const gamut_LUT)
{
    ...
    for (int i = 0; i < DT_BAUHAUS_SLIDER_MAX_STOPS; i++)
    {
        ...
        _build_dt_UCS_HSB_gradients((dt_aligned_pixel_t){hue, sat, brightness, 0.f}, RGB,
                                    work_profile, gamut_LUT);
        dt_bauhaus_slider_set_stop(slider, stop, RGB[0], RGB[1], RGB[2]);
    }
}

static inline void _draw_sliders_hue_gradient(
    const float sat, const float hue, const float brightness, GtkWidget *const slider,
    const struct dt_iop_order_iccprofile_info_t *work_profile, const float *const gamut_LUT)
{
    ...
    for (int i = 0; i < DT_BAUHAUS_SLIDER_MAX_STOPS; i++)
    {
        ...
        dt_aligned_pixel_t RGB = {1.0f, 1.0f, 1.0f, 1.0f};
        _build_dt_UCS_HSB_gradients((dt_aligned_pixel_t){hue_temp, sat, brightness, 0.f}, RGB,
                                    work_profile, gamut_LUT);
        dt_bauhaus_slider_set_stop(slider, stop, RGB[0], RGB[1], RGB[2]);
    }
}
...
static inline void
_draw_sliders_brightness_gradient(const float sat, const float hue, GtkWidget *const slider,
                                  const struct dt_iop_order_iccprofile_info_t *work_profile,
                                  const float *const gamut_LUT)
{
    for (int i = 0; i < DT_BAUHAUS_SLIDER_MAX_STOPS; i++)
    {
        ...
        dt_aligned_pixel_t RGB = {1.0f, 1.0f, 1.0f, 1.0f};
        _build_dt_UCS_HSB_gradients((dt_aligned_pixel_t){hue, sat, stop + 0.001f, 0.f}, RGB,
                                    work_profile, gamut_LUT);
        dt_bauhaus_slider_set_stop(slider, stop, RGB[0], RGB[1], RGB[2]);
    }
}
...
static inline void _init_sliders(const dt_iop_module_t *self)
{
    ...
    for (int k = 0; k < NODES; k++)
    {
        ...
        _draw_sliders_saturation_gradient(0.f, g->max_saturation, _get_hue_node(k, p->hue_shift),
                                          SLIDER_BRIGHTNESS, slider, g->white_adapted_profile,
                                          g->gamut_LUT);
    ...
    for (int k = 0; k < NODES; k++)
    {
        ...
        _draw_sliders_brightness_gradient(g->max_saturation, _get_hue_node(k, p->hue_shift), slider,
                                          g->white_adapted_profile, g->gamut_LUT);
        dt_bauhaus_slider_set_format(slider, "%");
        dt_bauhaus_slider_set_offset(slider, -100.0f);
        dt_bauhaus_slider_set_digits(slider, 2);
        gtk_widget_queue_draw(slider);
...
static void _init_graph_backgrounds(dt_iop_colorequal_gui_data_t *g, const float graph_width,
                                    const float graph_height, const float *const restrict gamut_LUT)
{
    ...
    for (int c = 0; c < NUM_CHANNELS; c++)
    {
        ...
        g->b_surface[c] = cairo_image_surface_create_for_data(g->b_data[c], CAIRO_FORMAT_RGB24,
                                                              gwidth, gheight, stride);
    }
    ...
    for (int i = 0; i < gheight; i++)
    {
        for (int j = 0; j < gwidth; j++)
        {
            ...
            const float hue = (x < -180.0f) ? _conventional_hue_deg_to_ucs_rad(x + 180.0f) :
                                              _conventional_hue_deg_to_ucs_rad(x);
            const float hhue = hue - (y - 0.5f) * DT_2PI_F;

            dt_aligned_pixel_t RGB;
            dt_aligned_pixel_t HSB[NUM_CHANNELS] = {
                {hhue, max_saturation, SLIDER_BRIGHTNESS, 1.0f},
            ...
            for (int k = 0; k < NUM_CHANNELS; k++)
            {
                ...
                for_three_channels(c) g->b_data[k][idx + c] = roundf(CLIP(RGB[c]) * 255.f);
            }
        }
    }
...
{
    // bleach bypass
    dt_iop_colorequal_params_t p1 = {.threshold = 0.0f,
                                     .smoothing_hue = 1.0f,
                                     .contrast = 0.0f,
                                     .white_level = 1.0f,
                                     .chroma_size = 1.5f,
    ...
    dt_gui_presets_add_generic(_("bleach bypass"), self->op, self->version(), &p1, sizeof(p1), TRUE,
                               DEVELOP_BLEND_CS_RGB_SCENE);

    // Kodachrome 64 like
    dt_iop_colorequal_params_t p2 = {.threshold = 0.193f,
                                     .smoothing_hue = 1.0f,
                                     .contrast = -0.345f,
                                     .white_level = 1.0f,
                                     .chroma_size = 1.5f,
    ...
    dt_gui_presets_add_generic(_("Kodachrome 64 like"), self->op, self->version(), &p2, sizeof(p2),
                               TRUE, DEVELOP_BLEND_CS_RGB_SCENE);

    // Kodak Portra 400
    dt_iop_colorequal_params_t p3 = {.threshold = 0.199f,
                                     .smoothing_hue = 1.0f,
                                     .contrast = -0.375f,
                                     .white_level = 1.0f,
                                     .chroma_size = 1.5f,
    ...
    dt_gui_presets_add_generic(_("Kodak Portra 400 like"), self->op, self->version(), &p3,
                               sizeof(p3), TRUE, DEVELOP_BLEND_CS_RGB_SCENE);

    // Teal & Orange
    dt_iop_colorequal_params_t p4 = {.threshold = 0.184f,
                                     .smoothing_hue = 0.52f,
                                     .contrast = -0.3f,
                                     .white_level = 1.0f,
                                     .chroma_size = 1.5f,
    ...
    dt_gui_presets_add_generic(_("teal & orange"), self->op, self->version(), &p4, sizeof(p4), TRUE,
                               DEVELOP_BLEND_CS_RGB_SCENE);
}
...
static void _draw_color_picker(const dt_iop_module_t *self, cairo_t *cr,
                               dt_iop_colorequal_params_t *p, dt_iop_colorequal_gui_data_t *g,
...
static gboolean _iop_colorequalizer_draw(GtkWidget *widget, cairo_t *crf,
    ...
    if (g->gamut_LUT)
    {
        for (int k = 0; k < 360; k++)
        {
            ...
            _build_dt_UCS_HSB_gradients(
                (dt_aligned_pixel_t){hue, g->max_saturation, SLIDER_BRIGHTNESS, 1.0f}, RGB,
                g->white_adapted_profile, g->gamut_LUT);
...
static void _pipe_RGB_to_Ych(dt_iop_module_t *self, dt_dev_pixelpipe_t *pipe,
                             const dt_aligned_pixel_t RGB, dt_aligned_pixel_t Ych)
{
    const struct dt_iop_order_iccprofile_info_t *const work_profile =
...
static void _channel_tabs_switch_callback(GtkNotebook *notebook, GtkWidget *page,
                                          const guint page_num, dt_iop_module_t *self)
{
    DT_GUARD_GUI_UPDATE();
    dt_iop_colorequal_gui_data_t *g = self->gui_data;
...
static void _area_set_value(const dt_iop_colorequal_gui_data_t *g, const float graph_height,
                            const float pos)
{
    float factor = .0f;
    float max = .0f;
...
static void _area_set_pos(const dt_iop_colorequal_gui_data_t *g, const float pos)
{
...
static gboolean _area_button_release_callback(GtkWidget *widget, const GdkEventButton *event,
                                              const dt_iop_module_t *self)
{
    dt_iop_colorequal_gui_data_t *g = self->gui_data;
...
static void _area_size_callback(GtkWidget *widget, GdkRectangle *allocation,
                                const dt_iop_module_t *self)
{
    dt_iop_colorequal_gui_data_t *g = self->gui_data;
...
static float _action_process_colorequal(const gpointer target, const dt_action_element_t element,
...
static const dt_action_def_t _action_def_coloreq = {
    N_("color equalizer"), _action_process_colorequal, _action_elements_colorequal};
...
void gui_init(dt_iop_module_t *self)
{
    ...
    // graph
    g->area =
        GTK_DRAWING_AREA(dt_ui_resize_wrap(NULL, 0, "plugins/darkroom/colorequal/graphheight"));
    ...
    g->hue_shift = dt_color_picker_new_with_cst(
        self, DT_COLOR_PICKER_POINT_AREA | DT_COLOR_PICKER_DENOISE,
    ...
    gtk_widget_set_tooltip_text(g->hue_shift, _("shift nodes to lower or higher hue"));

    dt_bauhaus_widget_set_quad_tooltip(
        g->hue_shift, _("pick hue from image and visualize it\nctrl+click to select an area"));
...
#define GROUP_SLIDERS(num, page, tooltip)                                                          \
    ...
    g->hue_sliders[0] = g->hue_red = dt_bauhaus_slider_from_params(sect, "hue_red");
    g->hue_sliders[1] = g->hue_orange = dt_bauhaus_slider_from_params(sect, "hue_orange");
    g->hue_sliders[2] = g->hue_yellow = dt_bauhaus_slider_from_params(sect, "hue_yellow");
    g->hue_sliders[3] = g->hue_green = dt_bauhaus_slider_from_params(sect, "hue_green");
    g->hue_sliders[4] = g->hue_cyan = dt_bauhaus_slider_from_params(sect, "hue_cyan");
    g->hue_sliders[5] = g->hue_blue = dt_bauhaus_slider_from_params(sect, "hue_blue");
    g->hue_sliders[6] = g->hue_lavender = dt_bauhaus_slider_from_params(sect, "hue_lavender");
    g->hue_sliders[7] = g->hue_magenta = dt_bauhaus_slider_from_params(sect, "hue_magenta");

    GROUP_SLIDERS("1", N_("saturation"), _("change saturation hue-wise"))
    g->sat_sliders[0] = g->sat_red = dt_bauhaus_slider_from_params(sect, "sat_red");
    g->sat_sliders[1] = g->sat_orange = dt_bauhaus_slider_from_params(sect, "sat_orange");
    g->sat_sliders[2] = g->sat_yellow = dt_bauhaus_slider_from_params(sect, "sat_yellow");
    g->sat_sliders[3] = g->sat_green = dt_bauhaus_slider_from_params(sect, "sat_green");
    g->sat_sliders[4] = g->sat_cyan = dt_bauhaus_slider_from_params(sect, "sat_cyan");
    g->sat_sliders[5] = g->sat_blue = dt_bauhaus_slider_from_params(sect, "sat_blue");
    g->sat_sliders[6] = g->sat_lavender = dt_bauhaus_slider_from_params(sect, "sat_lavender");
    g->sat_sliders[7] = g->sat_magenta = dt_bauhaus_slider_from_params(sect, "sat_magenta");

    GROUP_SLIDERS("2", N_("brightness"), _("change brightness hue-wise"))
    g->bright_sliders[0] = g->bright_red = dt_bauhaus_slider_from_params(sect, "bright_red");
    g->bright_sliders[1] = g->bright_orange = dt_bauhaus_slider_from_params(sect, "bright_orange");
    g->bright_sliders[2] = g->bright_yellow = dt_bauhaus_slider_from_params(sect, "bright_yellow");
    g->bright_sliders[3] = g->bright_green = dt_bauhaus_slider_from_params(sect, "bright_green");
    g->bright_sliders[4] = g->bright_cyan = dt_bauhaus_slider_from_params(sect, "bright_cyan");
    g->bright_sliders[5] = g->bright_blue = dt_bauhaus_slider_from_params(sect, "bright_blue");
    ...
    dt_gui_new_collapsible_section(&g->cs, "plugins/darkroom/colorequal/expand_options",
                                   _("options"), GTK_BOX(options), DT_ACTION(self));
    ...
    gtk_widget_set_tooltip_text(g->smoothing_hue, _("change for sharper or softer hue curve"));

    g->use_filter = dt_bauhaus_toggle_from_params(self, "use_filter");
    gtk_widget_set_tooltip_text(
        g->use_filter, _("restrict effect by using a guided filter based on hue and saturation"));
    ...
    dt_bauhaus_widget_set_quad(
        g->threshold, self, dtgtk_cairo_paint_showmask, TRUE, _masking_callback_t,
    ...
    gtk_widget_set_tooltip_text(g->param_size,
                                _("set radius of applied parameters for the guided filter"));

    dt_bauhaus_widget_set_quad(g->param_size, self, dtgtk_cairo_paint_showmask, TRUE,
                               _masking_callback_p,
