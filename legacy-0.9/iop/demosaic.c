...
typedef enum dt_iop_demosaic_method_t
{
    ...
    DT_IOP_DEMOSAIC_AMAZE_DUAL =
        DT_DEMOSAIC_DUAL | DT_IOP_DEMOSAIC_AMAZE, // $DESCRIPTION: "AMaZE (dual)""
    ...
    DT_IOP_DEMOSAIC_MARKEST3_DUAL =
        DT_DEMOSAIC_DUAL |
        DT_IOP_DEMOSAIC_MARKESTEIJN_3, // $DESCRIPTION: "Markesteijn 3-pass (dual)"
    DT_IOP_DEMOSAIC_PASSTHR_MONOX =
        DT_DEMOSAIC_XTRANS | 3, // $DESCRIPTION: "passthrough (monochrome)"
    DT_IOP_DEMOSAIC_PASSTHR_COLORX =
        DT_DEMOSAIC_XTRANS | 5, // $DESCRIPTION: "photosite color (debug)"
...
typedef struct dt_iop_demosaic_params_t
{
    ...
    dt_iop_demosaic_smooth_t
        color_smoothing; // $DEFAULT: DT_DEMOSAIC_SMOOTH_OFF $DESCRIPTION: "color smoothing"
    dt_iop_demosaic_method_t
        demosaicing_method; // $DEFAULT: DT_IOP_DEMOSAIC_RCD $DESCRIPTION: "method"
    dt_iop_demosaic_lmmse_t
        lmmse_refine;    // $DEFAULT: DT_LMMSE_REFINE_1 $DESCRIPTION: "LMMSE refine"
...
static gboolean _demosaic_full(const dt_dev_pixelpipe_iop_t *const piece,
                               const dt_image_t *const img, const dt_iop_roi_t *const roi_out)
{
    if ((img->flags & DT_IMAGE_4BAYER) // half_size_f doesn't support 4bayer images
        || dt_image_is_mono_sraw(img) || piece->pipe->want_detail_mask)
        return TRUE;
...
void amaze_demosaic(const float *const in, float *out, const int width, const int height,
                    const uint32_t filters, const float procmin);
...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(
        self, _("reconstruct full RGB pixels from a sensor color filter array reading"),
        _("mandatory"), _("linear, raw, scene-referred"), _("linear, raw"),
...
dt_iop_colorspace_type_t input_colorspace(dt_iop_module_t *self, dt_dev_pixelpipe_t *pipe,
                                          dt_dev_pixelpipe_iop_t *piece)
{
...
static gboolean _tiling_requirements(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *const piece,
                                     const dt_iop_roi_t *const roi_in,
                                     const dt_iop_roi_t *const roi_out, const gboolean dual,
                                     const gboolean greens, const gboolean direct,
                                     const gboolean capture, const gboolean gpu,
                                     const dt_iop_demosaic_method_t method, int *overlap,
                                     int *valid_rows, int *tile_height, int *num_tiles)
{
    ...
    dt_print(
        DT_DEBUG_PIPE | DT_DEBUG_TILING | DT_DEBUG_VERBOSE,
        "%s requirements: allmem=%dMB avail=%dMB untiled_avail=%dMB perpix=%d max_row=%d tile_max_row=%d",
        gpu ? "OpenCL" : "CPU", (int)(allmem / DT_MEGA), (int)(untiled_avail / DT_MEGA),
        (int)(avail / DT_MEGA), perpix, untiled_max_rows, max_rows);

    if (untiled_max_rows >= height)
        return FALSE;
...
void process(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *const piece, const void *const i,
             void *const o, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    if ((width < 16 || height < 16) && (method != DT_IOP_DEMOSAIC_PASSTHROUGH_MONOCHROME &&
    ...
    if (!fullscale)
    {
        dt_print_pipe(DT_DEBUG_PIPE, "demosaic approx zoom", pipe, self, DT_DEVICE_CPU, roi_in,
                      roi_out);
        if (method == DT_IOP_DEMOSAIC_PASSTHROUGH_MONOCHROME ||
            method == DT_IOP_DEMOSAIC_PASSTHROUGH_COLOR)
            dt_iop_clip_and_zoom_demosaic_passthrough_monochrome_f((float *)o, in, roi_out, roi_in,
                                                                   roi_out->width, width);
        else if (is_xtrans)
            dt_iop_clip_and_zoom_demosaic_third_size_xtrans_f((float *)o, in, roi_out, roi_in,
                                                              roi_out->width, width, xtrans);
        else
            dt_iop_clip_and_zoom_demosaic_half_size_f((float *)o, in, roi_out, roi_in,
                                                      roi_out->width, width, filters);
    ...
    const gboolean dual = (demosaicing_method & DT_DEMOSAIC_DUAL) && !run_fast && !show_sigma &&
                          !show_capture && !demosaic_mask;
    ...
    int num_tiles = 1;
    const gboolean tiling =
        _tiling_requirements(self, piece, roi_in, roi_out, dual, greens, direct, do_capture, FALSE,
                             method, &overlap, &valid_rows, &tile_height, &num_tiles);
    ...
    if (tiling || (darktable.unmuted & DT_DEBUG_VERBOSE))
        dt_print(DT_DEBUG_PIPE,
                 "CPU %s%s %s demosaic %s%s%s%s%s%s. tiles=%d tileheight=%d overlap=%d",
                 tiling ? "tiled " : "", direct ? "direct" : "scaled",
                 is_xtrans ? "xtrans" :
                 is_bayer  ? "bayer" :
                             "bayer4",
                 _method_str(method), dual ? "_DUAL" : "", do_capture ? " capture" : "",
                 greens ? " greens" : "", no_masking ? "" : " showmask",
                 bad_tiling ? ", high memory" : "", num_tiles, tile_height, overlap);
        ...
        if (out_height > 0)
        {
            if (tiling)
                dt_print(DT_DEBUG_TILING | DT_DEBUG_VERBOSE,
                         "tile=%.3d/%.3d, group=%.5d first=%.5d last=%.5d rows=%.4d", tile_nr,
                         num_tiles, group, first_in, last_in, t_rows);
            ...
            if (do_capture)
                _capture_sharpen(self, piece, t_in, t_out, width, t_rows, roi_in->x,
                                 roi_in->y + first_in, show_capture, show_sigma, xtrans, filters);

            if (dual)
                dual_demosaic(piece, t_out, t_in, width, t_rows, filters, xtrans, show_dual,
                              d->dual_thrs);

            if (tiling)
                dt_iop_image_copy(out + group * width * 4, t_out + first_out * width * 4,
                                  width * 4 * out_height);
        }
    }
...
#ifdef HAVE_OPENCL
int process_cl(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *const piece, cl_mem dev_in,
               cl_mem dev_out, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    if ((iwidth < 16 || iheight < 16) &&
        (method != DT_IOP_DEMOSAIC_PASSTHROUGH_MONOCHROME &&
         method != DT_IOP_DEMOSAIC_PASSTHROUGH_COLOR && method != DT_IOP_DEMOSAIC_MONO))
        method = is_xtrans ? DT_IOP_DEMOSAIC_VNG : DT_IOP_DEMOSAIC_VNG4;
    ...
    if (!fullscale)
    {
        ...
        if (is_xtrans)
        {
            ...
            err = dt_opencl_enqueue_kernel_2d_args(
                devid, gd->kernel_zoom_third_size, roi_out->width, roi_out->height, CLARG(dev_in),
                CLARG(dev_out), CLARG(roi_out->width), CLARG(roi_out->height), CLARG(iwidth),
                CLARG(iheight), CLARG(roi_out->scale), CLARG(dev_xtrans));
            dt_opencl_release_mem_object(dev_xtrans);
            return err;
        }
        else if (method == DT_IOP_DEMOSAIC_PASSTHROUGH_MONOCHROME)
            return dt_opencl_enqueue_kernel_2d_args(
                devid, gd->kernel_zoom_passthrough_monochrome, roi_out->width, roi_out->height,
                CLARG(dev_in), CLARG(dev_out), CLARG(roi_out->width), CLARG(roi_out->height),
                CLARG(iwidth), CLARG(iheight), CLARG(roi_out->scale));
        else // bayer
            return dt_opencl_enqueue_kernel_2d_args(
                devid, gd->kernel_zoom_half_size, roi_out->width, roi_out->height, CLARG(dev_in),
                CLARG(dev_out), CLARG(roi_out->width), CLARG(roi_out->height), CLARG(iwidth),
                CLARG(iheight), CLARG(roi_out->scale), CLARG(filters));
    }
    ...
    cl_mem t_low = NULL;
    cl_mem in_image =
        greens ? dt_opencl_alloc_device(devid, iwidth, iheight, sizeof(float)) : dev_in;
    if (!in_image)
    {
    ...
    int num_tiles = 1;
    tiling = _tiling_requirements(self, piece, roi_in, roi_out, dual, greens, direct, do_capture,
                                  TRUE, method, &overlap, &valid_rows, &tile_height, &num_tiles);

    if (tiling || (darktable.unmuted & DT_DEBUG_VERBOSE))
    {
        dt_print(DT_DEBUG_PIPE | DT_DEBUG_OPENCL,
                 "OpenCL:%d %s%s %s demosaic %s%s%s%s%s. tiles=%d tileheight=%d overlap=%d", devid,
                 tiling ? "tiled " : "", direct ? "direct" : "scaled",
                 is_xtrans ? "xtrans" :
                 is_bayer  ? "bayer" :
                             "bayer4",
                 _method_str(method), dual ? "_DUAL" : "", do_capture ? " capture" : "",
                 greens ? " greens" : "", no_masking ? "" : " showmask", num_tiles, tile_height,
                 overlap);
    }
    ...
    if (tiling && overtiling)
    {
        ...
        dt_print(DT_DEBUG_PIPE | DT_DEBUG_OPENCL,
                 "OpenCL demosaic falling back to cpu because of overtiling, valid_rows=%d",
                 valid_rows);
        goto finish;
    }
    ...
    out_image =
        direct ? dev_out : dt_opencl_alloc_device(devid, iwidth, iheight, sizeof(float) * 4);
    t_in = tiling ?
               dt_opencl_alloc_device(devid, iwidth, tile_height, sizeof(float) * in_channels) :
               in_image;
    t_out =
        tiling ? dt_opencl_alloc_device(devid, iwidth, tile_height, sizeof(float) * 4) : out_image;
    ...
    for (int tile_nr = 0; tile_nr < num_tiles; tile_nr++)
    {
        ...
        if (out_height > 0)
        {
            if (tiling)
            {
                ...
                err =
                    dt_opencl_enqueue_copy_image(devid, in_image, t_in, insrc, CLIMG_ORIGIN, iarea);
                if (err != CL_SUCCESS)
                    goto finish;
            }

            if (demosaic_mask)
                err = demosaic_box3_cl(self, piece, t_in, t_high, dev_xtrans, iwidth, t_rows,
                                       filters);
            else if (method == DT_IOP_DEMOSAIC_MONO)
            {
                const size_t iarea[2] = {iwidth, t_rows};
                err = dt_opencl_enqueue_copy_image(devid, t_in, t_high, CLIMG_ORIGIN, CLIMG_ORIGIN,
                                                   iarea);
            }
            else if (passthru || method == DT_IOP_DEMOSAIC_PPG)
                err = process_default_cl(self, piece, t_in, t_high, dev_xtrans, iwidth, t_rows,
                                         method, filters);
            else if (method == DT_IOP_DEMOSAIC_RCD)
                ...
                err = process_vng_cl(self, piece, t_in, t_high, dev_xtrans, xtrans, iwidth, t_rows,
                                     filters, 0, FALSE);
            else if (method == DT_IOP_DEMOSAIC_MARKESTEIJN ||
                     method == DT_IOP_DEMOSAIC_MARKESTEIJN_3)
                err = process_markesteijn_cl(self, piece, t_in, t_high, dev_xtrans, xtrans, iwidth,
                                             t_rows);
            ...
            if (do_capture)
            {
                err = _capture_sharpen_cl(self, piece, t_in, t_high, dev_xtrans, iwidth, t_rows,
                                          roi_in->x, roi_in->y + first_in, show_capture, show_sigma,
                                          xtrans, filters);
                if (err != CL_SUCCESS)
                    goto finish;
            }
            ...
            if (dual)
            {
                err = process_vng_cl(self, piece, t_in, t_low, dev_xtrans, xtrans, iwidth, t_rows,
                                     filters, 0, TRUE);
                if (err == CL_SUCCESS)
                    err = color_smoothing_cl(self, piece, t_low, t_low, iwidth, t_rows,
                                             DT_DEMOSAIC_SMOOTH_2);
                if (err == CL_SUCCESS)
                    err = dual_demosaic_cl(self, piece, t_high, t_low, t_out, iwidth, t_rows,
                                           show_dual);
                if (err != CL_SUCCESS)
                    goto finish;
            }
    ...
    if (d->color_smoothing != DT_DEMOSAIC_SMOOTH_OFF && no_masking && !run_fast)
    {
        err = color_smoothing_cl(self, piece, out_image, out_image, iwidth, iheight,
                                 d->color_smoothing);
        if (err != CL_SUCCESS)
            goto finish;
    }
...
finish:
    ...
    if (greens)
        dt_opencl_release_mem_object(in_image);
    if (!direct)
        dt_opencl_release_mem_object(out_image);
    if (tiling)
    {
        dt_opencl_release_mem_object(t_in);
        dt_opencl_release_mem_object(t_out);
...
void init_global(dt_iop_module_so_t *self)
{
    ...
    gd->kernel_zoom_half_size =
        dt_opencl_create_kernel(program, "clip_and_zoom_demosaic_half_size");
    ...
    gd->kernel_green_eq_favg_reduce_first =
        dt_opencl_create_kernel(program, "green_equilibration_favg_reduce_first");
    gd->kernel_green_eq_favg_reduce_second =
        dt_opencl_create_kernel(program, "green_equilibration_favg_reduce_second");
    gd->kernel_green_eq_favg_apply =
        dt_opencl_create_kernel(program, "green_equilibration_favg_apply");
    ...
    gd->kernel_zoom_third_size =
        dt_opencl_create_kernel(vng, "clip_and_zoom_demosaic_third_size_xtrans");
    gd->kernel_vng_interpolate = dt_opencl_create_kernel(vng, "vng_interpolate");

    const int markesteijn = 16; // from programs.conf
    gd->kernel_markesteijn_initial_copy =
        dt_opencl_create_kernel(markesteijn, "markesteijn_initial_copy");
    gd->kernel_markesteijn_green_minmax =
        dt_opencl_create_kernel(markesteijn, "markesteijn_green_minmax");
    gd->kernel_markesteijn_interpolate_green =
        dt_opencl_create_kernel(markesteijn, "markesteijn_interpolate_green");
    gd->kernel_markesteijn_solitary_green =
        dt_opencl_create_kernel(markesteijn, "markesteijn_solitary_green");
    gd->kernel_markesteijn_recalculate_green =
        dt_opencl_create_kernel(markesteijn, "markesteijn_recalculate_green");
    gd->kernel_markesteijn_red_and_blue =
        dt_opencl_create_kernel(markesteijn, "markesteijn_red_and_blue");
    gd->kernel_markesteijn_interpolate_twoxtwo =
        dt_opencl_create_kernel(markesteijn, "markesteijn_interpolate_twoxtwo");
    gd->kernel_markesteijn_convert_yuv =
        dt_opencl_create_kernel(markesteijn, "markesteijn_convert_yuv");
    gd->kernel_markesteijn_differentiate =
        dt_opencl_create_kernel(markesteijn, "markesteijn_differentiate");
    gd->kernel_markesteijn_homo_threshold =
        dt_opencl_create_kernel(markesteijn, "markesteijn_homo_threshold");
    ...
    gd->kernel_markesteijn_homo_max_corr =
        dt_opencl_create_kernel(markesteijn, "markesteijn_homo_max_corr");
    gd->kernel_markesteijn_homo_quench =
        dt_opencl_create_kernel(markesteijn, "markesteijn_homo_quench");
    ...
    for (int i = 0; i <= UCHAR_MAX; i++)
        _calc_9x9_gauss_coeffs(&gd->gauss_coeffs[i * CAPTURE_KERNEL_ALIGN],
                               MAX(1e-7f, (float)i * CAPTURE_GAUSS_FRACTION));
}
                   ...
                   dt_dev_pixelpipe_iop_t *piece)
{
    ...
    const gboolean bayer =
        self->dev->image_storage.buf_dsc.filters != 9u && !bayer4 && !true_monochrome;
    ...
    if (bayer4)
    {
        ...
        if (!dt_colorspaces_conversion_matrices_rgb(
                self->dev->image_storage.adobe_XYZ_to_CAM, NULL, d->CAM_to_RGB,
...
void reload_defaults(dt_iop_module_t *self)
{
    ...
    if (dt_image_is_monochrome(img))
        d->demosaicing_method = dt_image_is_mono_sraw(img) ? DT_IOP_DEMOSAIC_MONO :
    ...
    if (self->widget)
        gtk_stack_set_visible_child_name(GTK_STACK(self->widget),
                                         self->default_enabled ? "raw" : "non_raw");
...
void gui_changed(dt_iop_module_t *self, GtkWidget *w, void *previous)
{
    ...
    const gboolean bayerpassing = use_method == DT_IOP_DEMOSAIC_PASSTHROUGH_MONOCHROME ||
                                  use_method == DT_IOP_DEMOSAIC_PASSTHROUGH_COLOR;
    ...
    const gboolean passing = bayerpassing || use_method == DT_IOP_DEMOSAIC_PASSTHR_MONOX ||
    ...
    */
        GtkWidget *demosaicers = bayer  ? g->demosaic_method_bayer :
                                 ...
                                 bayer4 ? g->demosaic_method_bayerfour :
                                          g->demosaic_method_mono;
        const int pos = dt_bauhaus_combobox_get_from_value(demosaicers, p->demosaicing_method);
        if (pos >= 0)
            dt_bauhaus_combobox_remove_at(demosaicers, pos);
    }
...
static void _ui_pipe_done(gpointer instance, dt_iop_module_t *self)
{
    ...
    {
        dt_print(DT_DEBUG_PIPE, "demosaic UI pipe sets radius=%.3f thrs=%.3f", g->new_radius,
                 g->new_thrs);
...
static void _preset_applied_callback(gpointer instance, dt_iop_module_t *self)
{
    ...
    {
        dt_print(DT_DEBUG_PIPE, "demosaic auto preset applied, radius=%.3f thrs=%.3f", p->cs_radius,
                 p->cs_thrs);
        dt_dev_reprocess_center(self->dev);
    }
}
...
void gui_init(dt_iop_module_t *self)
{
    ...
    const int xtranspos =
        dt_bauhaus_combobox_get_from_value(g->demosaic_method_bayer, DT_DEMOSAIC_XTRANS);

    for (int i = 0; i < 8; i++)
        dt_bauhaus_combobox_remove_at(g->demosaic_method_bayer, xtranspos);
    gtk_widget_set_tooltip_text(
        g->demosaic_method_bayer,
        _("Bayer sensor demosaicing method, PPG and RCD are fast, AMaZE and LMMSE are slow.\nLMMSE is suited best for high ISO images.\nVNG4 is good in rare cases avoiding maze patterns.\ndual demosaicers increase processing time by blending a VNG variant in a second pass."));

    g->demosaic_method_xtrans = dt_bauhaus_combobox_from_params(self, "demosaicing_method");
    for (int i = 0; i < xtranspos; i++)
        dt_bauhaus_combobox_remove_at(g->demosaic_method_xtrans, 0);
    dt_bauhaus_combobox_remove_at(g->demosaic_method_xtrans, 7);
    gtk_widget_set_tooltip_text(
        g->demosaic_method_xtrans,
        _("X-Trans sensor demosaicing method, Markesteijn 3-pass and frequency domain chroma are slow.\ndual demosaicers increase processing time by blending a VNG variant in a second pass."));

    g->demosaic_method_bayerfour = dt_bauhaus_combobox_from_params(self, "demosaicing_method");
    for (int i = 0; i < 8; i++)
        dt_bauhaus_combobox_remove_at(g->demosaic_method_bayerfour, xtranspos);
    for (int i = 0; i < 2; i++)
        dt_bauhaus_combobox_remove_at(g->demosaic_method_bayerfour, 0);
    for (int i = 0; i < 4; i++)
        dt_bauhaus_combobox_remove_at(g->demosaic_method_bayerfour, 1);
    gtk_widget_set_tooltip_text(g->demosaic_method_bayerfour,
                                _("Bayer4 sensor demosaicing methods."));

    g->demosaic_method_mono = dt_bauhaus_combobox_from_params(self, "demosaicing_method");
    for (int i = 0; i < 7; i++)
        dt_bauhaus_combobox_remove_at(g->demosaic_method_mono, xtranspos);
    for (int i = 0; i < xtranspos; i++)
        dt_bauhaus_combobox_remove_at(g->demosaic_method_mono, 0);
    gtk_widget_set_tooltip_text(g->demosaic_method_mono,
                                _("monochrome sensor demosaicing methods."));

    g->dual_thrs = dt_bauhaus_slider_from_params(self, "dual_thrs");
    dt_bauhaus_slider_set_digits(g->dual_thrs, 2);
    gtk_widget_set_tooltip_text(
        g->dual_thrs,
        _("contrast threshold for dual demosaic.\nset to 0.0 for high frequency content\n"
          "set to 1.0 for flat content"));
    dt_bauhaus_widget_set_quad(g->dual_thrs, self, dtgtk_cairo_paint_showmask, TRUE,
                               _dual_thrs_callback, _("toggle mask visualization"));

    g->median_thrs = dt_bauhaus_slider_from_params(self, "median_thrs");
    dt_bauhaus_slider_set_digits(g->median_thrs, 3);
    gtk_widget_set_tooltip_text(g->median_thrs,
                                _("threshold for edge-aware median.\nset to 0.0 to switch off\n"
                                  "set to 1.0 to ignore edges"));
    g->lmmse_refine = dt_bauhaus_combobox_from_params(self, "lmmse_refine");
    gtk_widget_set_tooltip_text(
        g->lmmse_refine,
        _("LMMSE refinement steps. the median steps average the output,\nrefine adds some recalculation of red & blue channels"));

    g->color_smoothing = dt_bauhaus_combobox_from_params(self, "color_smoothing");
    gtk_widget_set_tooltip_text(g->color_smoothing,
                                _("how many color smoothing median steps after demosaicing"));

    g->greeneq = dt_bauhaus_combobox_from_params(self, "green_eq");
    gtk_widget_set_tooltip_text(
        g->greeneq, _("green channels matching method. only applies for some old sensors"));

    g->cs_enabled = dt_bauhaus_toggle_from_params(self, "cs_enabled");
    gtk_widget_set_tooltip_text(
        g->cs_enabled, _("capture sharpen recovers details lost due to in-camera blurring\n"
    ...
    dt_gui_new_collapsible_section(&g->capture, "plugins/darkroom/demosaic/expand_capture",
                                   _("capture sharpen controls"), GTK_BOX(box_raw),
                                   DT_ACTION(self));
    ...
    gtk_widget_set_tooltip_text(
        g->cs_radius,
        _("capture sharpen radius should reflect the overall gaussian type blur\n"
    ...
    dt_bauhaus_widget_set_quad(
        g->cs_radius, self, dtgtk_cairo_paint_reset, FALSE, _cs_radius_callback,
    ...
    gtk_widget_set_tooltip_text(
        g->cs_thrs,
        _("restrict capture sharpening to areas with high local contrast,\n"
    ...
    dt_bauhaus_widget_set_quad(g->cs_thrs, self, dtgtk_cairo_paint_showmask, TRUE,
                               _cs_thrs_callback, _("visualize sharpened areas"));
    ...
    gtk_widget_set_tooltip_text(g->cs_boost,
                                _("further increase sharpen radius at image corners,\n"
                                  "the sharp center of the image will not be affected"));
    dt_bauhaus_widget_set_quad(g->cs_boost, self, dtgtk_cairo_paint_showmask, TRUE,
                               _cs_boost_callback, _("visualize the overall radius"));
