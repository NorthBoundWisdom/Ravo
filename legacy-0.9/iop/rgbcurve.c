...
typedef struct dt_iop_rgbcurve_params_t
{
    ...
    int curve_type
        [DT_IOP_RGBCURVE_MAX_CHANNELS]; // $DEFAULT: MONOTONE_HERMITE (CATMULL_ROM, MONOTONE_HERMITE, CUBIC_SPLINE)
    dt_iop_rgbcurve_autoscale_t
        curve_autoscale; // $DEFAULT: DT_S_SCALE_AUTOMATIC_RGB $DESCRIPTION: "mode"
    gboolean
        compensate_middle_grey; // $DEFAULT: 0  $DESCRIPTION: "compensate middle gray" scale the curve and histogram so middle gray is at .5
    dt_iop_rgb_norms_t
...
typedef struct dt_iop_rgbcurve_data_t
{
    ...
    dt_draw_curve_t
        *curve[DT_IOP_RGBCURVE_MAX_CHANNELS]; // curves for pipe piece and pixel processing
...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self, _("alter an image’s tones using curves in RGB color space"),
                                  _("corrective and creative"), _("linear, RGB, display-referred"),
...
void init_presets(dt_iop_module_so_t *self)
{
    ...
    for (int k = 0; k < 7; k++)
        p.curve_nodes[DT_IOP_RGBCURVE_G][k].x = linear_ab[k];
    for (int k = 0; k < 7; k++)
        p.curve_nodes[DT_IOP_RGBCURVE_G][k].y = linear_ab[k];
    for (int k = 0; k < 7; k++)
        p.curve_nodes[DT_IOP_RGBCURVE_B][k].x = linear_ab[k];
    for (int k = 0; k < 7; k++)
        p.curve_nodes[DT_IOP_RGBCURVE_B][k].y = linear_ab[k];
    ...
    dt_gui_presets_add_generic(_("contrast | compression"), self->op, self->version(), &p,
    ...
    // Linear - no contrast
    for (int k = 0; k < 7; k++)
        p.curve_nodes[DT_IOP_RGBCURVE_R][k].x = linear_L[k];
    for (int k = 0; k < 7; k++)
        p.curve_nodes[DT_IOP_RGBCURVE_R][k].y = linear_L[k];
    dt_gui_presets_add_generic(_("linear (gamma 1.0)"), self->op, self->version(), &p, sizeof(p),
    ...
    dt_gui_presets_add_generic(_("contrast | medium (linear)"), self->op, self->version(), &p,
                               sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    dt_gui_presets_add_generic(_("contrast | high (linear)"), self->op, self->version(), &p,
                               sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    dt_gui_presets_add_generic(_("contrast | medium (gamma 2.2)"), self->op, self->version(), &p,
                               sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    for (int k = 1; k < 6; k++)
        p.curve_nodes[DT_IOP_RGBCURVE_R][k].y = powf(p.curve_nodes[DT_IOP_RGBCURVE_R][k].y, 2.2f);
    dt_gui_presets_add_generic(_("contrast | high (gamma 2.2)"), self->op, self->version(), &p,
                               sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    dt_gui_presets_add_generic(_("non-contrast curve | gamma 2.0"), self->op, self->version(), &p,
                               sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);

    // Gamma 0.5 - no contrast
    for (int k = 1; k < 6; k++)
        p.curve_nodes[DT_IOP_RGBCURVE_R][k].y = sqrtf(linear_L[k]);
    dt_gui_presets_add_generic(_("non-contrast curve | gamma 0.5"), self->op, self->version(), &p,
                               sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
                         ...
                         const dt_iop_order_iccprofile_info_t *const work_profile)
{
    ...
    {
        const float val = (work_profile) ? dt_ioppr_get_rgb_matrix_luminance(
...
static gboolean _is_identity(dt_iop_rgbcurve_params_t *p, rgbcurve_channel_t channel)
{
    for (int k = 0; k < p->curve_num_nodes[channel]; k++)
        if (p->curve_nodes[channel][k].x != p->curve_nodes[channel][k].y)
            return FALSE;
...
void gui_changed(dt_iop_module_t *self, GtkWidget *w, void *previous)
{
    ...
    if (w == g->autoscale)
    {
        ...
        if (p->curve_autoscale == DT_S_SCALE_MANUAL_RGB && _is_identity(p, DT_IOP_RGBCURVE_G) &&
            _is_identity(p, DT_IOP_RGBCURVE_B))
        {
            for (int k = 0; k < DT_IOP_RGBCURVE_MAXNODES; k++)
                p->curve_nodes[DT_IOP_RGBCURVE_G][k] = p->curve_nodes[DT_IOP_RGBCURVE_B][k] =
                    p->curve_nodes[DT_IOP_RGBCURVE_R][k];
...
static void interpolator_callback(GtkWidget *widget, dt_iop_module_t *self)
{
    ...
    if (combo == 0)
        p->curve_type[DT_IOP_RGBCURVE_R] = p->curve_type[DT_IOP_RGBCURVE_G] =
            p->curve_type[DT_IOP_RGBCURVE_B] = CUBIC_SPLINE;
    else if (combo == 1)
        p->curve_type[DT_IOP_RGBCURVE_R] = p->curve_type[DT_IOP_RGBCURVE_G] =
            p->curve_type[DT_IOP_RGBCURVE_B] = CATMULL_ROM;
    else if (combo == 2)
        p->curve_type[DT_IOP_RGBCURVE_R] = p->curve_type[DT_IOP_RGBCURVE_G] =
            p->curve_type[DT_IOP_RGBCURVE_B] = MONOTONE_HERMITE;

    dt_dev_add_history_item_target(darktable.develop, self, TRUE, widget);
    gtk_widget_queue_draw(GTK_WIDGET(g->area));
}

static void tab_switch_callback(GtkNotebook *notebook, GtkWidget *page, guint page_num,
...
static inline int _add_node_from_picker(dt_iop_rgbcurve_params_t *p, const float *const in,
                                        const float increment, const int ch,
                                        const dt_iop_order_iccprofile_info_t *const work_profile)
{
    float x = 0.f;
    float y = 0.f;
    ...
    if (p->curve_autoscale == DT_S_SCALE_AUTOMATIC_RGB)
        val = (work_profile) ? dt_ioppr_get_rgb_matrix_luminance(
                                   in, work_profile->matrix_in, work_profile->lut_in,
                                        ...
                                        dt_iop_module_t *self)
{
    ...
    if ((p->curve_autoscale != DT_S_SCALE_MANUAL_RGB) && g->channel != DT_IOP_RGBCURVE_R)
        return TRUE;
...
static gboolean _area_draw_callback(GtkWidget *widget, cairo_t *crf, dt_iop_module_t *self)
{
        ...
        g->minmax_curve_nodes[ch] != p->curve_num_nodes[ch])
    {
        ...
        for (int k = 0; k < p->curve_num_nodes[ch]; k++)
            dt_draw_curve_add_point(g->minmax_curve[ch], p->curve_nodes[ch][k].x,
                                    p->curve_nodes[ch][k].y);
    }
    else
    {
        for (int k = 0; k < p->curve_num_nodes[ch]; k++)
            dt_draw_curve_set_point(g->minmax_curve[ch], k, p->curve_nodes[ch][k].x,
                                    p->curve_nodes[ch][k].y);
    }
    ...
    dt_draw_grid_zoomed(cr, 4, 0.f, 0.f, 1.f, 1.f, width, height, g->zoom_factor, g->offset_x,
                        g->offset_y);

    const double dashed[] = {4.0, 4.0};
    const int len = sizeof(dashed) / sizeof(dashed[0]);
    cairo_set_dash(cr, dashed, len, 0);
    dt_draw_grid_zoomed(cr, 8, 0.f, 0.f, 1.f, 1.f, width, height, g->zoom_factor, g->offset_x,
                        g->offset_y);
    ...
    if (self->enabled)
    {
        ...
        if (hist && hist_max > 0.0f)
        {
            ...
            if (autoscale == DT_S_SCALE_AUTOMATIC_RGB)
            {
                ...
                for (int k = DT_IOP_RGBCURVE_R; k < DT_IOP_RGBCURVE_MAX_CHANNELS; k++)
                {
                    ...
                    dt_draw_histogram_8_zoomed(cr, hist, 4, k, g->zoom_factor, g->offset_x * 255.0,
                                               g->offset_y * hist_max, is_linear);
                }
            }
            else if (autoscale == DT_S_SCALE_MANUAL_RGB)
            {
                set_color(cr, darktable.bauhaus->graph_colors[ch]);
                dt_draw_histogram_8_zoomed(cr, hist, 4, ch, g->zoom_factor, g->offset_x * 255.0,
                                           g->offset_y * hist_max, is_linear);
            }
        ...
        if (self->request_color_pick != DT_REQUEST_COLORPICK_OFF)
        {
            ...
            if (self->picked_color_max[ch] >= 0.0f)
            {
                ...
                snprintf(text, sizeof(text), "%.1f → %.1f", picker_mean[ch] * 255.f,
                         picker_min[ch] * 255.f);
    ...
    else if (g->selected >= 0)
    {
        ...
        snprintf(text, sizeof(text), "%.1f / %.1f ( %+.1f)", x_node_value, y_node_value,
                 d_node_value);
...
void gui_init(dt_iop_module_t *self)
{
    ...
    g_signal_connect(G_OBJECT(g->channel_tabs), "switch_page", G_CALLBACK(tab_switch_callback),
                     self);

    // color pickers
    g->colorpicker =
        dt_color_picker_new(self, DT_COLOR_PICKER_POINT_AREA | DT_COLOR_PICKER_IO, NULL);
    ...
    g->colorpicker_set_values =
        dt_color_picker_new(self, DT_COLOR_PICKER_AREA | DT_COLOR_PICKER_IO, NULL);
    ...
    g_signal_connect(G_OBJECT(g->interpolator), "value-changed", G_CALLBACK(interpolator_callback),
                     self);

    dt_gui_box_add(self->widget,
                   dt_gui_hbox(dt_gui_expand(g->channel_tabs), dt_gui_expand(gtk_grid_new()),
...
void init(dt_iop_module_t *self)
{
    ...
    d->curve_nodes[0][1].x = d->curve_nodes[0][1].y = d->curve_nodes[1][1].x =
        d->curve_nodes[1][1].y = d->curve_nodes[2][1].x = d->curve_nodes[2][1].y = 1.0;
...
static void _generate_curve_lut(dt_dev_pixelpipe_t *pipe, dt_iop_rgbcurve_data_t *d)
{
    ...
    if (work_profile)
    {
        if (d->type_work == work_profile->type &&
            strcmp(d->filename_work, work_profile->filename) == 0)
            return;
    }
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    dev_r =
        dt_opencl_copy_host_to_image(devid, d->table[DT_IOP_RGBCURVE_R], 256, 256, sizeof(float));
    if (dev_r == NULL)
        goto cleanup;

    dev_g =
        dt_opencl_copy_host_to_image(devid, d->table[DT_IOP_RGBCURVE_G], 256, 256, sizeof(float));
    if (dev_g == NULL)
        goto cleanup;

    dev_b =
        dt_opencl_copy_host_to_image(devid, d->table[DT_IOP_RGBCURVE_B], 256, 256, sizeof(float));
    if (dev_b == NULL)
        goto cleanup;

    dev_coeffs_r = dt_opencl_copy_host_to_device_constant(
        devid, sizeof(float) * DT_IOP_RGBCURVE_MAX_CHANNELS, d->unbounded_coeffs[0]);
    if (dev_coeffs_r == NULL)
        goto cleanup;

    dev_coeffs_g = dt_opencl_copy_host_to_device_constant(
        devid, sizeof(float) * DT_IOP_RGBCURVE_MAX_CHANNELS, d->unbounded_coeffs[1]);
    if (dev_coeffs_g == NULL)
        goto cleanup;

    dev_coeffs_b = dt_opencl_copy_host_to_device_constant(
        devid, sizeof(float) * DT_IOP_RGBCURVE_MAX_CHANNELS, d->unbounded_coeffs[2]);
    if (dev_coeffs_b == NULL)
        goto cleanup;

    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_rgbcurve, width, height, CLARG(dev_in), CLARG(dev_out), CLARG(width),
        CLARG(height), CLARG(dev_r), CLARG(dev_g), CLARG(dev_b), CLARG(dev_coeffs_r),
        CLARG(dev_coeffs_g), CLARG(dev_coeffs_b), CLARG(autoscale), CLARG(preserve_colors),
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    for (int y = 0; y < 4 * npixels; y += 4)
    {
        if (autoscale == DT_S_SCALE_MANUAL_RGB)
        {
            out[y + 0] =
                (in[y + 0] < xm_L) ?
                    table[DT_IOP_RGBCURVE_R][CLAMP((int)(in[y + 0] * 0x10000ul), 0, 0xffff)] :
                    dt_iop_eval_exp(unbounded_coeffs[DT_IOP_RGBCURVE_R], in[y + 0]);
            out[y + 1] =
                (in[y + 1] < xm_g) ?
                    table[DT_IOP_RGBCURVE_G][CLAMP((int)(in[y + 1] * 0x10000ul), 0, 0xffff)] :
                    dt_iop_eval_exp(unbounded_coeffs[DT_IOP_RGBCURVE_G], in[y + 1]);
            out[y + 2] =
                (in[y + 2] < xm_b) ?
                    table[DT_IOP_RGBCURVE_B][CLAMP((int)(in[y + 2] * 0x10000ul), 0, 0xffff)] :
        ...
        else if (autoscale == DT_S_SCALE_AUTOMATIC_RGB)
        {
            if (d->params.preserve_colors == DT_RGB_NORM_NONE)
            {
                for (int c = 0; c < 3; c++)
                {
                    out[y + c] =
                        (in[y + c] < xm_L) ?
                            table[DT_IOP_RGBCURVE_R]
                                 [CLAMP((int)(in[y + c] * 0x10000ul), 0, 0xffff)] :
