...
int flags()
{
    return IOP_FLAGS_ALLOW_TILING | IOP_FLAGS_HIDDEN | IOP_FLAGS_TILING_FULL_ROI |
           IOP_FLAGS_ONE_INSTANCE | IOP_FLAGS_NO_HISTORY_STACK | IOP_FLAGS_WRITE_PIPECACHE_IN;
}
                     ...
                     struct dt_develop_tiling_t *tiling)
{
    ...
    tiling->factor +=
        ioratio != 1.0f ? 0.5f : 0.0f; // approximate extra requirements for interpolation
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    dt_print_pipe(DT_DEBUG_IMAGEIO, exporting ? "clip_and_zoom_roi" : "clip_and_zoom", piece->pipe,
                  self, piece->pipe->devid, roi_in, roi_out, "device=%i", devid);
    if (exporting)
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    const gboolean exporting = dt_pipe_is_export(piece->pipe);
    dt_print_pipe(DT_DEBUG_IMAGEIO, exporting ? "clip_and_zoom_roi" : "clip_and_zoom", piece->pipe,
                  self, DT_DEVICE_CPU, roi_in, roi_out);
