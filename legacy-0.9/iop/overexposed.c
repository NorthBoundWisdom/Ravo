             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    if (current_profile && work_profile)
        dt_ioppr_transform_image_colorspace_rgb(in, img_tmp, roi_out->width, roi_out->height,
    ...
    else if (dev->overexposed.mode == DT_CLIPPING_PREVIEW_GAMUT && work_profile)
    {
        ...
        for (size_t k = 0; k < (size_t)ch * roi_out->width * roi_out->height; k += ch)
        {
            const float luminance = dt_ioppr_get_rgb_matrix_luminance(
                img_tmp + k, work_profile->matrix_in, work_profile->lut_in,
            ...
            else
            {
                ...
                for_each_channel(c, aligned(saturation, img_tmp : 64))
                {
                    ...
                    saturation[c] =
                        sqrtf(saturation[c] * saturation[c] /
                              (luminance * luminance + img_tmp[k + c] * img_tmp[k + c]));
                }
                ...
                else if (img_tmp[k + 0] <= lower && img_tmp[k + 1] <= lower &&
                         img_tmp[k + 2] <= lower)
                {
                    copy_pixel(out + k, lower_color);
                }
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    if (current_profile && work_profile)
        dt_ioppr_transform_image_colorspace_rgb_cl(devid, dev_in, dev_tmp, roi_out->width,
                                                   roi_out->height, current_profile, work_profile,
                                                   self->op);
    else
    {
    ...
    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_overexposed, width, height, CLARG(dev_in), CLARG(dev_out), CLARG(dev_tmp),
        CLARG(width), CLARG(height), CLARG(lower), CLARG(upper), CLARRAY(4, lower_color),
        CLARRAY(4, upper_color), CLARG(dev_profile_info), CLARG(dev_profile_lut),
        CLARG(use_work_profile), CLARG(mode));
...
{
    piece->enabled =
        self->dev->overexposed.enabled && dt_pipe_is_full(piece->pipe) && self->dev->gui_attached;
}
