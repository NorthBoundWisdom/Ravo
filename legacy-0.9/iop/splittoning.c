...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self,
                                  _("use two specific colors for shadows and highlights and\n"
...
void init_presets(dt_iop_module_so_t *self)
{
    ...
    dt_gui_presets_add_generic(
        ...
        &(dt_iop_splittoning_params_t){26.0 / 360.0, 92.0 / 100.0, 40.0 / 360.0, 92.0 / 100.0, 0.63,
                                       0.0},
    ...
    dt_gui_presets_add_generic(
        ...
        &(dt_iop_splittoning_params_t){220.0 / 360.0, 64.0 / 100.0, 220.0 / 360.0, 64.0 / 100.0,
                                       0.0, 5.22},
        sizeof(dt_iop_splittoning_params_t), TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    dt_gui_presets_add_generic(
        ...
        &(dt_iop_splittoning_params_t){13.0 / 360.0, 42.0 / 100.0, 13.0 / 360.0, 42.0 / 100.0,
                                       100.0, 0.0},
        sizeof(dt_iop_splittoning_params_t), TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    // balance : 60
    // compress : 0
    dt_gui_presets_add_generic(_("chocolate brown"), self->op, self->version(),
                               &(dt_iop_splittoning_params_t){28.0 / 360.0, 39.0 / 100.0,
                                                              28.0 / 360.0, 8.0 / 100.0, 0.60, 0.0},
                               sizeof(dt_iop_splittoning_params_t), TRUE,
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    for (int k = 0; k < 4 * npixels; k += 4)
    {
        ...
        if (l < balance - compress)
        {
            ...
            for_each_channel(c, aligned(in, out)) toned[c] = CLIP(in[k + c] * la + mixrgb[c] * ra);
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    return dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_splittoning, width, height, CLARG(dev_in), CLARG(dev_out), CLARG(width),
        CLARG(height), CLARG(compress), CLARG(balance), CLARG(shadow_hue), CLARG(shadow_saturation),
        CLARG(highlight_hue), CLARG(highlight_saturation));
}
#endif
...
static inline void update_balance_slider_colors(GtkWidget *slider, float shadow_hue,
                                                float highlight_hue)
{
    dt_aligned_pixel_t rgb;
    if (shadow_hue != -1)
...
void gui_update(dt_iop_module_t *self)
{
    ...
    update_colorpicker_color(GTK_WIDGET(g->highlight_colorpick), p->highlight_hue,
...
static inline void gui_init_section(dt_iop_module_t *self, const char *section,
                                    GtkWidget *slider_box, GtkWidget *hue, GtkWidget *saturation,
                                    GtkWidget **picker, gboolean top)
{
...
void gui_init(dt_iop_module_t *self)
{
    ...
    gui_init_section(self, NC_("section", "shadows"), shad->widget, g->shadow_hue_gslider,
                     g->shadow_sat_gslider, &g->shadow_colorpick, TRUE);

    gui_init_section(self, NC_("section", "highlights"), high->widget, g->highlight_hue_gslider,
                     g->highlight_sat_gslider, &g->highlight_colorpick, FALSE);
    ...
    gtk_widget_set_tooltip_text(
        g->compress_scale, _("compress the effect on highlights/shadows and\npreserve mid-tones"));
}
