...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self, _("remove fog and atmospheric hazing from images"),
                                  ...
                                  _("frequential, RGB"), _("linear, RGB, scene-referred"));
}
...
// size (2*w+1) x (2*w+1) )
static void _dark_channel(const const_rgb_image img1, const gray_image img2, const int w)
{
...
static void _transition_map(const const_rgb_image img1, const gray_image img2, const int w,
                            const float *const A0, const float strength)
{
    const size_t size = (size_t)img1.height * img1.width;
    const float *const restrict in_data = img1.data;
    float *const restrict out_data = img2.data;
    ...
    {
        const float *pixel = in_data + 4 * i;
        const float m = MIN(MIN(pixel[0] * A0_inv[0], pixel[1] * A0_inv[1]), pixel[2] * A0_inv[2]);
...
static float _ambient_light(const const_rgb_image img, const int w1, rgb_pixel *pA0)
{
    ...
    for (size_t i = 0; i < size; i++)
    {
        ...
        {
            for_each_channel(c, aligned(pixel_in)) A0[c] += pixel_in[c];
            N_bright_hazy++;
        }
    }
    if (N_bright_hazy > 0)
    {
        for_each_channel(c) A0[c] /= N_bright_hazy;
    }
    (*pA0)[0] = A0[0];
    (*pA0)[1] = A0[1];
    (*pA0)[2] = A0[2];
    ...
    return crit_haze_level > 0 ? -1.125f * logf(crit_haze_level) :
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    const size_t size = (size_t)width * height;
    const float wscale =
        d->adaptive ? CLIP((float)roi_in->scale / (float)piece->pipe->iscale) : 1.0f;
  ...
  */
    if (gui && fullpipes && !hq)
    {
        ...
        if (hash != DT_INVALID_HASH &&
            !dt_dev_sync_pixelpipe_hash(self->dev, piece->pipe, self->iop_order,
                                        DT_DEV_TRANSFORM_DIR_BACK_INCL, &self->gui_lock, &g->hash))
    ...
    if (storing)
    {
        ...
        if (distance_max <= 0.0f)
            dt_control_log(
                _("haze removal could not calculate ambient light due to image content"));
    }
    ...
    guided_filter(img_in.data, trans_map.data, trans_map_filtered.data, width, height, 4, w2, eps,
                  1.f, -FLT_MAX, FLT_MAX);
    ...
    dt_print_pipe(DT_DEBUG_VERBOSE, "ambient data", piece->pipe, self, DT_DEVICE_CPU, roi_in,
                  roi_out, "tmin=%.4f distance_max=%.4f A0=%.4f %.4f %.4f", t_min, distance_max,
                  A0[0], A0[1], A0[2]);
    ...
    for (size_t i = 0; i < size; i++)
    {
        ...
        for_each_channel(c, aligned(in)) res[c] = (in[4 * i + c] - c_A0[c]) / t + c_A0[c];
...
static int _box_min_cl(dt_iop_module_t *self, int devid, cl_mem in, cl_mem out, const int w)
{
...
static int _box_max_cl(dt_iop_module_t *self, int devid, cl_mem in, cl_mem out, const int w)
{
    dt_iop_hazeremoval_global_data_t *gd = self->global_data;
    const int width = dt_opencl_get_image_width(in);
    const int height = dt_opencl_get_image_height(in);
...
static int _transition_map_cl(dt_iop_module_t *self, int devid, cl_mem img1, cl_mem img2,
                              const int w1, const float strength, const float *const A0)
{
    dt_iop_hazeremoval_global_data_t *gd = self->global_data;
    ...
    cl_int err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_hazeremoval_transision_map, width, height, CLARG(width), CLARG(height),
        CLARG(img1), CLARG(img2), CLARG(strength), CLARG(A0[0]), CLARG(A0[1]), CLARG(A0[2]));
    if (err != CL_SUCCESS)
        return err;
...
static int _dehaze_cl(dt_iop_module_t *self, int devid, cl_mem img_in, cl_mem trans_map,
                      cl_mem img_out, const float t_min, const float *const A)
{
    dt_iop_hazeremoval_global_data_t *gd = self->global_data;
    ...
    return dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_hazeremoval_dehaze, width, height, CLARG(width), CLARG(height),
        CLARG(img_in), CLARG(trans_map), CLARG(img_out), CLARG(t_min), CLARG(A0));
}
...
int process_cl(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece, cl_mem img_in, cl_mem img_out,
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
  ...
  */
    ...
    // apply guided filter with no clipping
    err = guided_filter_cl(devid, img_in, trans_map, trans_map_filtered, width, height, 4, w2, eps,
                           1.f, -CL_FLT_MAX, CL_FLT_MAX);
    if (err != CL_SUCCESS)
        goto error;
    ...
    dt_print_pipe(DT_DEBUG_VERBOSE, "ambient data", piece->pipe, self, devid, roi_in, roi_out,
                  "tmin=%.4f distance_max=%.4f A0=%.4f %.4f %.4f", t_min, distance_max, A0[0],
                  A0[1], A0[2]);
