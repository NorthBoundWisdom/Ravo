...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self, _("overlay an SVG watermark like a signature on the image"),
                                  _("creative"), _("non-linear, RGB, display-referred"),
...
static void _text_color_font_set_sensitive(const dt_iop_watermark_gui_data_t *g,
...
static gchar *_string_substitute(gchar *string, const gchar *search, const gchar *replace)
{
    ...
    g_free(_replace);
    g_free(string); // dt_util_str_replace always returns a new string,
                    // and we don't need the original after this func
    return result;
}
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
  ...
  */
    ...
    if (type == DT_WTM_SVG)
    {
        /* the svg_offsets allow safe text boxes as they might render out
     * of the dimensions */
        ...
        surface_two = cairo_image_surface_create_for_data(
            image_two, CAIRO_FORMAT_ARGB32, watermark_width, watermark_height, stride_two);
...
static void _watermark_callback(GtkWidget *tb, dt_iop_module_t *self)
{
    ...
    dt_strlcpy_to_fixed(p->filename, (char *)g_list_nth_data(g->watermarks_filenames, n),
                        sizeof(p->filename));
        ...
        if (extension)
        {
            ...
            if (!g_ascii_strcasecmp(extension, ".svg") || !g_ascii_strcasecmp(extension, ".png"))
            {
                ...
                g->watermarks_filenames =
                    g_list_append(g->watermarks_filenames, g_strdup(filename));
...
void gui_init(dt_iop_module_t *self)
{
    ...
    char *tooltip =
        g_strdup_printf(_("SVG watermarks in %s/watermarks or %s/watermarks"), configdir, datadir);
    ...
    GtkWidget *child =
        dt_gui_container_first_child(GTK_CONTAINER(gtk_bin_get_child(GTK_BIN(g->fontsel))));
    ...
    dt_action_define_iop(self, NULL, N_("pick color"), g->color_picker_button,
    ...
    gtk_widget_set_tooltip_text(
        g->scale_base,
        _("choose how to scale the watermark\n"
    ...
    gtk_widget_set_tooltip_text(
        g->scale_img, _("reference image dimension against which to scale the watermark"));
    ...
    g_signal_connect(G_OBJECT(g->watermarks), "value-changed", G_CALLBACK(_watermark_callback),
                     self);
