...
typedef enum dt_iop_lens_modflag_t
{
    ...
    DT_IOP_LENS_MODFLAG_ALL = DT_IOP_LENS_MODIFY_FLAG_DISTORTION | DT_IOP_LENS_MODIFY_FLAG_TCA |
                              DT_IOP_LENS_MODIFY_FLAG_VIGNETTING, // $DESCRIPTION: "all"
    DT_IOP_LENS_MODFLAG_DIST_TCA = DT_IOP_LENS_MODIFY_FLAG_DISTORTION |
                                   DT_IOP_LENS_MODIFY_FLAG_TCA, // $DESCRIPTION: "distortion & TCA"
    DT_IOP_LENS_MODFLAG_DIST_VIGN =
        DT_IOP_LENS_MODIFY_FLAG_DISTORTION |
        DT_IOP_LENS_MODIFY_FLAG_VIGNETTING, // $DESCRIPTION: "distortion & vignetting"
    DT_IOP_LENS_MODFLAG_TCA_VIGN =
        DT_IOP_LENS_MODIFY_FLAG_TCA |
        DT_IOP_LENS_MODIFY_FLAG_VIGNETTING, // $DESCRIPTION: "TCA & vignetting"
    DT_IOP_LENS_MODFLAG_DIST =
        DT_IOP_LENS_MODIFY_FLAG_DISTORTION,                // $DESCRIPTION: "only distortion"
    DT_IOP_LENS_MODFLAG_TCA = DT_IOP_LENS_MODIFY_FLAG_TCA, // $DESCRIPTION: "only TCA"
    DT_IOP_LENS_MODFLAG_VIGN =
        DT_IOP_LENS_MODIFY_FLAG_VIGNETTING, // $DESCRIPTION: "only vignetting"
...
{
    dt_iop_lens_method_t
        method; // $DEFAULT: DT_IOP_LENS_METHOD_LENSFUN $DESCRIPTION: "correction method"
    dt_iop_lens_modflag_t
        modify_flags; // $DEFAULT: DT_IOP_LENS_MODFLAG_ALL $DESCRIPTION: "corrections"
    ...
    float distance;
    dt_iop_lens_lenstype_t
        target_geom; // $DEFAULT: DT_IOP_LENS_LENSTYPE_RECTILINEAR $DESCRIPTION: "target geometry"
    char camera[128];
    char lens[128];
...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(
        self, _("correct lenses optical flaws"), _("corrective"), _("linear, RGB, scene-referred"),
        _("geometric and reconstruction, RGB"), _("linear, RGB, scene-referred"));
}
...
static lfModifier *_get_modifier(int *mods_done, const int w, const int h,
                                 const dt_iop_lens_data_t *d, const int mods_filter,
                                 const gboolean force_inverse)
{
    ...
    if (mods_todo & LF_MODIFY_VIGNETTING)
        mods_done_tmp |=
            mod->EnableVignettingCorrection(d->lens, d->focal, d->aperture, d->distance);
#else
    mod = new lfModifier(d->lens, d->crop, w, h);
    mods_done_tmp =
        mod->Initialize(d->lens, LF_PF_F32, d->focal, d->aperture, d->distance, d->scale,
                        d->target_geom, mods_todo, (force_inverse) ? !d->inverse : d->inverse);
#endif

    if (mods_done)
        *mods_done = mods_done_tmp;
    return mod;
}

static float _get_autoscale_lf(dt_iop_module_t *self, dt_iop_lens_params_t *p,
                               const lfCamera *camera)
{
    ...
    if (p->lens[0] != '\0')
    {
        ...
        if (lenslist)
        {
            ...
            const dt_iop_lens_data_t d = {.modify_flags = p->modify_flags,
                                          ...
                                          .distance = p->distance,
                                          .target_geom =
                                              _lenstype_to_lensfun_lenstype(p->target_geom),
...
static void _process_lf(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                        ...
                        const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    {
        dt_iop_image_copy_by_size((float *)ovoid, (float *)ivoid, roi_out->width, roi_out->height,
                                  ch);
        return;
    }
    ...
    const int used_lf_mask =
        (raw_monochrome || pass_mode) ? LF_MODIFY_ALL & ~LF_MODIFY_TCA : LF_MODIFY_ALL;
    ...
    const dt_interpolation_t *const interpolation =
    ...
    if (d->inverse)
    {
        ...
        if (modflags &
            (LF_MODIFY_TCA | LF_MODIFY_DISTORTION | LF_MODIFY_GEOMETRY | LF_MODIFY_SCALE))
        {
            ...
            for (int y = 0; y < roi_out->height; y++)
            {
                ...
                for (int x = 0; x < roi_out->width; x++, bufptr += 6, out += ch)
                {
                    ...
                    {
                        if (d->do_nan_checks &&
                            (!isfinite(bufptr[c * 2]) || !isfinite(bufptr[c * 2 + 1])))
                        {
                            out[c] = 0.0f;
                            continue;
                        }
                        ...
                        const float pi0 =
                            fmaxf(fminf(bufptr[c * 2] - roi_in->x, roi_in->width - 1.0f), 0.0f);
                        const float pi1 = fmaxf(
                            fminf(bufptr[c * 2 + 1] - roi_in->y, roi_in->height - 1.0f), 0.0f);
                        out[c] = dt_interpolation_compute_sample(interpolation, inptr, pi0, pi1,
                    ...
                    if (mask_display & DT_DEV_PIXELPIPE_DISPLAY_MASK)
                    {
                        ...
                        const float pi0 =
                            fmaxf(fminf(bufptr[2] - roi_in->x, roi_in->width - 1.0f), 0.0f);
                        const float pi1 =
                            fmaxf(fminf(bufptr[3] - roi_in->y, roi_in->height - 1.0f), 0.0f);
                        out[3] = dt_interpolation_compute_sample(interpolation, inptr, pi0, pi1,
                                                                 roi_in->width, roi_in->height, ch,
                                                                 ch_width);
                    }
                }
        ...
        {
            dt_iop_image_copy_by_size((float *)ovoid, (float *)ivoid, roi_out->width,
                                      roi_out->height, ch);
        }
            ...
            (LF_MODIFY_TCA | LF_MODIFY_DISTORTION | LF_MODIFY_GEOMETRY | LF_MODIFY_SCALE))
        {
            ...
            for (int y = 0; y < roi_out->height; y++)
            {
                ...
                for (int x = 0; x < roi_out->width; x++, buf2ptr += 6, out += ch)
                {
                    ...
                    {
                        if (d->do_nan_checks &&
                            (!isfinite(buf2ptr[c * 2]) || !isfinite(buf2ptr[c * 2 + 1])))
                        {
                            out[c] = 0.0f;
                            continue;
                        }
                        ...
                        const float pi0 =
                            fmaxf(fminf(buf2ptr[c * 2] - roi_in->x, roi_in->width - 1.0f), 0.0f);
                        const float pi1 = fmaxf(
                            fminf(buf2ptr[c * 2 + 1] - roi_in->y, roi_in->height - 1.0f), 0.0f);
                    ...
                    if (mask_display & DT_DEV_PIXELPIPE_DISPLAY_MASK)
                    {
                        ...
                        const float pi0 =
                            fmaxf(fminf(buf2ptr[2] - roi_in->x, roi_in->width - 1.0f), 0.0f);
                        const float pi1 =
                            fmaxf(fminf(buf2ptr[3] - roi_in->y, roi_in->height - 1.0f), 0.0f);
                        out[3] = dt_interpolation_compute_sample(interpolation, bufptr, pi0, pi1,
                                                                 roi_in->width, roi_in->height, ch,
                                                                 ch_width);
                    }
                }
...
#ifdef HAVE_OPENCL
static int _process_cl_lf(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece, cl_mem dev_in,
                          ...
                          const dt_iop_roi_t *const roi_out)
{
    ...
    if (!d->lens || !d->lens->Maker || d->crop <= 0.0f)
        return dt_opencl_enqueue_copy_image(devid, dev_in, dev_out, CLIMG_ORIGIN, CLIMG_ORIGIN,
                                            oregion);
    ...
    if (d->inverse)
    {
            ...
            (LF_MODIFY_TCA | LF_MODIFY_DISTORTION | LF_MODIFY_GEOMETRY | LF_MODIFY_SCALE))
        {
            ...
            err = dt_opencl_write_buffer_to_device(devid, tmpbuf, dev_tmpbuf, 0, tmpbufsize, TRUE);
            if (err != CL_SUCCESS)
                goto error;

            err = dt_opencl_enqueue_kernel_2d_args(
                devid, ldkernel, owidth, oheight, CLARG(dev_in), CLARG(dev_tmp), CLARG(owidth),
                CLARG(oheight), CLARG(iwidth), CLARG(iheight), CLARG(roi_in_x), CLARG(roi_in_y),
        ...
        {
            err = dt_opencl_enqueue_copy_image(devid, dev_in, dev_tmp, CLIMG_ORIGIN, CLIMG_ORIGIN,
                                               oregion);
            if (err != CL_SUCCESS)
                goto error;
        }
        ...
        {
            err = dt_opencl_enqueue_copy_image(devid, dev_tmp, dev_out, CLIMG_ORIGIN, CLIMG_ORIGIN,
                                               oregion);
            if (err != CL_SUCCESS)
                goto error;
        }
    ...
    else // correct distortions:
    {
        if (!pass_mode && (modflags & LF_MODIFY_VIGNETTING))
        {
            ...
            for (int y = 0; y < roi_in->height; y++)
            {
                ...
                for (int k = 0; k < ch * roi_in->width; k++)
                    buf[k] = 0.5f;
            ...
            (LF_MODIFY_TCA | LF_MODIFY_DISTORTION | LF_MODIFY_GEOMETRY | LF_MODIFY_SCALE))
        {
            ...
            err = dt_opencl_enqueue_kernel_2d_args(
                devid, ldkernel, owidth, oheight, CLARG(dev_tmp), CLARG(dev_out), CLARG(owidth),
                CLARG(oheight), CLARG(iwidth), CLARG(iheight), CLARG(roi_in_x), CLARG(roi_in_y),
                CLARG(dev_tmpbuf), CLARG((d->do_nan_checks)));
        }
        else
...
static void _tiling_callback_lf(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                                ...
                                dt_develop_tiling_t *tiling)
{
    ...
    tiling->align = 1;
    dt_iop_lens_data_t *d = (dt_iop_lens_data_t *)piece->data;
    if (d->v_strength != 0.0f)
        tiling->factor += 1.0f;
}

static gboolean _distort_transform_lf(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
...
static gboolean _distort_backtransform_lf(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                                          float *const __restrict points, size_t points_count)
{
    dt_iop_lens_data_t *d = (dt_iop_lens_data_t *)piece->data;
...
static void _distort_mask_lf(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                             ...
                             const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    {
        float *bufptr = (float *)dt_get_perthread(buf, padded_bufsize);
        modifier->ApplySubpixelGeometryDistortion(roi_out->x, roi_out->y + y, roi_out->width, 1,
                                                  bufptr);
        ...
        for (int x = 0; x < roi_out->width; x++, bufptr += 6, _out++)
        {
            ...
            *_out = CLIP(dt_interpolation_compute_sample(interpolation, in, pi0, pi1, roi_in->width,
...
static void _modify_roi_in_lf(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                              const dt_iop_roi_t *const roi_out, dt_iop_roi_t *roi_in)
{
    ...
    *roi_in = *roi_out;
    ...
    if (modflags & (LF_MODIFY_TCA | LF_MODIFY_DISTORTION | LF_MODIFY_GEOMETRY | LF_MODIFY_SCALE))
    {
        ...
        DT_OMP_PRAGMA(parallel default(none) dt_omp_firstprivate(
            aheight, awidth, buf, height, nbpoints, width, xoff, xstep, yoff, ystep)
      ...
      DT_OMP_PRAGMA(for schedule(static))
      for (int i = 0; i < awidth; i++)
          modifier->ApplySubpixelGeometryDistortion(xoff + i * xstep, yoff + (height - 1), 1, 1,
                                                    buf + 6 * (awidth + i));

      DT_OMP_PRAGMA(for schedule(static))
      for (int j = 0; j < aheight; j++)
          modifier->ApplySubpixelGeometryDistortion(xoff, yoff + j * ystep, 1, 1,
                                                    buf + 6 * (2 * awidth + j));

      DT_OMP_PRAGMA(for schedule(static))
      for (int j = 0; j < aheight; j++)
          modifier->ApplySubpixelGeometryDistortion(xoff + (width - 1), yoff + j * ystep, 1, 1,
...
static void _commit_params_lf(dt_iop_module_t *self, dt_iop_lens_params_t *p,
                              dt_dev_pixelpipe_t *pipe, dt_dev_pixelpipe_iop_t *piece)
{
    ...
    {
        dt_pthread_mutex_lock(&darktable.plugin_threadsafe);
        const lfLens **lens = dt_iop_lensfun_db->FindLenses(camera, NULL, p->lens, 0);
        dt_pthread_mutex_unlock(&darktable.plugin_threadsafe);
        if (lens)
        {
            *d->lens = *lens[0];
   ...
   */
     ...
     * to provide a size that won't be used so we use the image size */
        _get_modifier(&modflags, self->dev->image_storage.width, self->dev->image_storage.height, d,
                      used_lf_mask, FALSE);
...
static void _preprocess_vignette(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                                 const float *const data, float *vig, const dt_iop_roi_t *const roi,
                                 const gboolean mask)
{
    dt_iop_lens_data_t *d = (dt_iop_lens_data_t *)piece->data;
    ...
    for (int row = 0; row < roi->height; row++)
    {
        for (int col = 0; col < roi->width; col++)
        {
            ...
            for_three_channels(c) vig[idx + c] = (1.0f + val) * data[idx + c];
...
static inline float _interpolate_linear_spline(const float *xi, const float *yi, const int ni,
                                               const float x)
{
    if (x < xi[0])
        return yi[0];
...
static int _check_corrections_md(dt_iop_lens_data_t *d)
{
    ...
    for (int i = 0; i < d->nc; i++)
    {
        ...
        if ((d->cor_rgb[0][i] != d->cor_rgb[1][i]) || (d->cor_rgb[0][i] != d->cor_rgb[2][i]) ||
...
static void _commit_params_md(dt_iop_module_t *self, dt_iop_lens_params_t *p,
                              dt_dev_pixelpipe_t *pipe, dt_dev_pixelpipe_iop_t *piece)
{
    dt_iop_lens_data_t *d = (dt_iop_lens_data_t *)piece->data;
    dt_iop_lens_gui_data_t *g = (dt_iop_lens_gui_data_t *)self->gui_data;
...
static void _commit_params_vig(dt_iop_module_t *self, dt_iop_lens_params_t *p,
                               dt_dev_pixelpipe_t *pipe, dt_dev_pixelpipe_iop_t *piece)
{
    dt_iop_lens_gui_data_t *g = (dt_iop_lens_gui_data_t *)self->gui_data;
...
static void _tiling_callback_md(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                                const dt_iop_roi_t *roi_in, const dt_iop_roi_t *roi_out,
                                dt_develop_tiling_t *tiling)
{
    tiling->factor = 4.5f; // in + out + tmp + tmpbuf
...
static void _tiling_callback_vg(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                                const dt_iop_roi_t *roi_in, const dt_iop_roi_t *roi_out,
                                dt_develop_tiling_t *tiling)
{
    tiling->factor = 2.0f;
...
static gboolean _distort_transform_md(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                                      float *points, const size_t points_count)
{
    ...
    for (size_t i = 0; i < 2 * points_count; i += 2)
    {
        ...
        for (int k = 0; k < 10; k++)
        {
            ...
            const float dr = _interpolate_linear_spline(d->knots_dist, d->cor_rgb[1], d->nc,
                                                        r * dt_fast_hypotf(cx, cy));

            const float dist1 = points[i] - (dr * cx + w2);
            const float dist2 = points[i + 1] - (dr * cy + h2);

            if (fabsf(dist1) < .5f && fabsf(dist2) < .5f)
                break;
...
static gboolean _distort_backtransform_md(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                                          float *points, const size_t points_count)
{
    dt_iop_lens_data_t *d = (dt_iop_lens_data_t *)piece->data;
...
static void _distort_mask_md(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                             const float *const in, float *const out,
                             const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    dt_iop_lens_data_t *d = (dt_iop_lens_data_t *)piece->data;
    ...
    for (int y = 0; y < roi_out->height; y++)
    {
        for (int x = 0; x < roi_out->width; x++)
        {
            ...
            out[y * roi_out->width + x] = CLIP(dt_interpolation_compute_sample(
                interpolation, in, xs, ys, roi_in->width, roi_in->height, 1, roi_in->width));
        }
    }
}

static void _process_md(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                        const void *const ivoid, void *const ovoid,
                        const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out,
    ...
    if (!pass_mode && (d->modify_flags & DT_IOP_LENS_MODIFY_FLAG_VIGNETTING))
    {
        ...
        for (int y = 0; y < roi_in->height; y++)
        {
            for (int x = 0; x < roi_in->width; x++)
            {
                ...
                const float sf = _interpolate_linear_spline(d->knots_vig, d->vig, d->nc,
                                                            r * dt_fast_hypotf(cx, cy));

                for_each_channel(c) buf[idx + c] /= MAX(1e-4, sf);
            }
        }
    }
    ...
    for (int y = 0; y < roi_out->height; y++)
    {
        for (int x = 0; x < roi_out->width; x++)
        {
            ...
            for_each_channel(c)
            {
                ...
                const float ys = CLAMP(dr * cy + h2 - roi_in->y, 0.0f, limh);
                out[odx + c] =
                    dt_interpolation_compute_sample(interpolation, buf + c, xs, ys, roi_in->width,
...
#ifdef HAVE_OPENCL
static int _process_cl_md(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece, cl_mem dev_in,
                          cl_mem dev_out, const dt_iop_roi_t *const roi_in,
                          const dt_iop_roi_t *const roi_out)
{
    dt_iop_lens_data_t *d = (dt_iop_lens_data_t *)piece->data;
    ...
    if (!pass_mode && (d->modify_flags & DT_IOP_LENS_MODIFY_FLAG_VIGNETTING))
    {
        ...
        data =
            (cl_mem)dt_opencl_alloc_device(devid, roi_in->width, roi_in->height, 4 * sizeof(float));
        if (data == NULL)
            goto error;

        cl_mem knots_vig = (cl_mem)dt_opencl_copy_host_to_device_constant(
            devid, sizeof(d->knots_vig), &d->knots_vig);
        cl_mem vig = (cl_mem)dt_opencl_copy_host_to_device_constant(devid, sizeof(d->vig), &d->vig);

        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->kernel_md_vignette, roi_in->width, roi_in->height, CLARG(dev_in),
            CLARG(data), CLARG(knots_vig), CLARG(vig), CLARG(roi_in->width), CLARG(roi_in->height),
            CLARG(w2), CLARG(h2), CLARG(r), CLARG(roi_in->x), CLARG(roi_in->y), CLARG(knots));
    ...
    knots_dist = (cl_mem)dt_opencl_copy_host_to_device_constant(devid, sizeof(d->knots_dist),
                                                                &d->knots_dist);
    cor_rgb =
        (cl_mem)dt_opencl_copy_host_to_device_constant(devid, sizeof(d->cor_rgb), &d->cor_rgb);

    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_md_correct, roi_out->width, roi_out->height, CLARG(data), CLARG(dev_out),
        CLARG(knots_dist), CLARG(cor_rgb), CLARG(roi_out->width), CLARG(roi_out->height),
        CLARG(roi_in->width), CLARG(roi_in->height), CLARG(w2), CLARG(h2), CLARG(r),
        CLARG(d->scale_md), CLARG(roi_in->x), CLARG(roi_in->y), CLARG(roi_out->x),
        CLARG(roi_out->y), CLARG(knots), CLARG(itor->id), CLARG(itor_width), CLARG(pass_mode));
...
static void _modify_roi_in_md(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                              const dt_iop_roi_t *const roi_out, dt_iop_roi_t *roi_in)
{
    dt_iop_lens_data_t *d = (dt_iop_lens_data_t *)piece->data;
...
static void _modify_roi_in_vg(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                              const dt_iop_roi_t *const roi_out, dt_iop_roi_t *roi_in)
{
    *roi_in = *roi_out;
}
...
#ifdef HAVE_OPENCL
cl_int _preprocess_vignette_cl(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece, cl_mem dev_in,
                               cl_mem dev_vig, const dt_iop_roi_t *const roi, const gboolean mask)
{
    dt_iop_lens_data_t *d = (dt_iop_lens_data_t *)piece->data;
    dt_iop_lens_global_data_t *gd = (dt_iop_lens_global_data_t *)self->global_data;
    ...
    cl_mem dev_spline = (cl_mem)dt_opencl_copy_host_to_device_constant(
        piece->pipe->devid, sizeof(d->vigspline), d->vigspline);
    if (dev_spline == NULL)
    ...
    cl_int err = dt_opencl_enqueue_kernel_2d_args(
        piece->pipe->devid, gd->kernel_lens_man_vignette, roi->width, roi->height, CLARG(dev_in),
        CLARG(dev_vig), CLARG(dev_spline), CLARG(roi->width), CLARG(roi->height), CLARG(w2),
        CLARG(h2), CLARG(roi->x), CLARG(roi->y), CLARG(inv_maxr), CLARG(strength),
        CLARG(splinesize), CLARG(mask));
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    if (pre_vignette && !pass_mode)
    {
        data = (cl_mem)dt_opencl_alloc_device(piece->pipe->devid, roi_in->width, roi_in->height,
                                              4 * sizeof(float));
        if (data)
        {
    ...
    else
    {
        ...
        err = dt_opencl_enqueue_copy_image(piece->pipe->devid, data, dev_out, CLIMG_ORIGIN,
                                           CLIMG_ORIGIN, region);
    }
...
gboolean distort_transform(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                           float *const __restrict points, const size_t points_count)
{
    dt_iop_lens_data_t *d = (dt_iop_lens_data_t *)piece->data;
...
gboolean distort_backtransform(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                               float *const __restrict points, const size_t points_count)
{
    dt_iop_lens_data_t *d = (dt_iop_lens_data_t *)piece->data;
...
const dt_iop_lens_method_t _get_method(dt_iop_module_t *self, dt_iop_lens_method_t method)
{
...
void init_global(dt_iop_module_so_t *self)
{
    ...
    gd->kernel_lens_man_vignette = dt_opencl_create_kernel(program, "lens_man_vignette");
    gd->kernel_md_vignette = dt_opencl_create_kernel(program, "md_vignette");
    gd->kernel_md_correct = dt_opencl_create_kernel(program, "md_lens_correction");
...
#endif
    {
        ...
        gchar *sysdbpath = g_build_filename(path, "lensfun",
                                            "version_" STR(LF_MAX_DATABASE_VERSION), (char *)NULL);
#endif
...
void reload_defaults(dt_iop_module_t *self)
{
    ...
    if (g)
    {
        ...
        dt_bauhaus_combobox_add_introspection(
            g->methods_selector, NULL, self->so->get_f("method")->Enum.values,
            _have_embedded_metadata(self) ? DT_IOP_LENS_METHOD_EMBEDDED_METADATA :
...
static int _ptr_array_insert_sorted(GPtrArray *array, const void *item, GCompareFunc compare)
{
...
static int _ptr_array_find_sorted(const GPtrArray *array, const void *item, GCompareFunc compare)
{
    const int length = array->len;
...
static void _ptr_array_insert_index(GPtrArray *array, const void *item, const int index)
{
    const void **root;
    const int length = array->len;
    g_ptr_array_set_size(array, length + 1);
...
static GtkMenu *camera_menu_fill(dt_iop_module_t *self, const lfCamera *const *camlist)
{
    ...
    {
        GtkWidget *item =
            (GtkWidget *)gtk_menu_item_new_with_label((const gchar *)g_ptr_array_index(makers, i));
        gtk_widget_show(item);
...
static void _parse_model(const char *txt, char *model, const size_t sz_model)
{
...
static void _lens_set(dt_iop_module_t *self, const lfLens *lens)
{
    ...
    unsigned i;
    gdouble focal_values[] = {-INFINITY, 4.5, 8,    10,   12,   14,   15,   16,      17,  18,  20,
                              24,        28,  30,   31,   35,   38,   40,   43,      45,  50,  55,
                              60,        70,  75,   77,   80,   85,   90,   100,     105, 110, 120,
                              135,       150, 200,  210,  240,  250,  300,  400,     500, 600, 700,
                              800,       840, 1000, 1120, 1200, 1600, 2000, INFINITY};
    gdouble aperture_values[] = {-INFINITY, 0.7, 0.8, 0.9, 1,  1.1, 1.2, 1.4,     1.8, 2,   2.2,
                                 2.5,       2.8, 3.2, 3.4, 4,  4.5, 5.0, 5.6,     6.3, 7.1, 8,
                                 9,         10,  11,  13,  14, 16,  18,  20,      22,  25,  29,
                                 32,        38,  45,  50,  54, 64,  90,  INFINITY};
    ...
    {
        snprintf(txt, sizeof(txt), "%.*f", _precision(focal_values[ffi + k], 10.0),
                 focal_values[ffi + k]);
        dt_bauhaus_combobox_add(w, txt);
    }
    ...
    for (int k = 0; k < fli - ffi; k++)
    {
        snprintf(txt, sizeof(txt), "%.*f", _precision(aperture_values[ffi + k], 10.0),
...
static void _lens_menu_select(GtkMenuItem *menuitem, dt_iop_module_t *self)
{
    dt_iop_lens_gui_data_t *g = (dt_iop_lens_gui_data_t *)self->gui_data;
    dt_iop_lens_params_t *p = (dt_iop_lens_params_t *)self->params;
...
static GtkMenu *_lens_menu_fill(dt_iop_module_t *self, const lfLens *const *lenslist)
{
...
static void _lens_autosearch_clicked(GtkWidget *button, dt_iop_module_t *self)
{
    ...
    _parse_model(txt, model, sizeof(model));
    dt_pthread_mutex_lock(&darktable.plugin_threadsafe);
    lenslist = dt_iop_lensfun_db->FindLenses(g->camera, NULL, model[0] ? model : NULL,
...
void gui_changed(dt_iop_module_t *self, GtkWidget *w, void *previous)
{
    ...
    else if (p->method == DT_IOP_LENS_METHOD_EMBEDDED_METADATA)
    {
        ...
        const gboolean has_warp =
            (img->exif_correction_type == CORRECTION_TYPE_DNG) ? cd->dng.has_warp : TRUE;

        const gboolean has_vign =
            (img->exif_correction_type == CORRECTION_TYPE_DNG) ? cd->dng.has_vignette : TRUE;
...
void gui_init(dt_iop_module_t *self)
{
    ...
    // camera selector
    g->camera_model =
        dt_iop_button_new(self, N_("camera model"), G_CALLBACK(_camera_menusearch_clicked), FALSE,
                          0, (GdkModifierType)0, NULL, 0, NULL);
    g->find_camera_button = dt_iop_button_new(
        self, N_("find camera"), G_CALLBACK(_camera_autosearch_clicked), FALSE, 0,
    ...
    // lens selector
    g->lens_model = dt_iop_button_new(self, N_("lens model"), G_CALLBACK(_lens_menusearch_clicked),
                                      FALSE, 0, (GdkModifierType)0, NULL, 0, NULL);
    g->find_lens_button = dt_iop_button_new(
        self, N_("find lens"), G_CALLBACK(_lens_autosearch_clicked), FALSE, 0, (GdkModifierType)0,
    ...
    GtkWidget *box_lf = self->widget =
        dt_gui_vbox(dt_gui_hbox(dt_gui_expand(g->camera_model), g->find_camera_button),
  ...
  if(self->dev->image_storage.exif_lens[0] != '\0')
  {
    ...
    dt_bauhaus_widget_set_quad(g->scale, self, dtgtk_cairo_paint_refresh, FALSE,
                               _autoscale_pressed_lf,
    ...
    dt_gui_new_collapsible_section(&g->fine_tune, "plugins/darkroom/lens/expand_fine_tune",
                                   _("fine-tuning"), GTK_BOX(box_md), DT_ACTION(self));
    // DT_IOP_SECTION_FOR_PARAMS doesn't work in C++ so declare local first
    dt_iop_module_t sect_mod =
        DT_IOP_SECTION_FOR_PARAMS_DECL(self, N_("fine-tune"), GTK_WIDGET(g->fine_tune.container));
    ...
    dt_bauhaus_widget_set_quad(g->scale_md, self, dtgtk_cairo_paint_refresh, FALSE,
                               _autoscale_pressed_md, _("automatic scale to available image size"));
    ...
    gtk_widget_set_tooltip_text(
        g->methods_selector, _("select a correction mode either based on\n"
    ...
    dt_gui_new_collapsible_section(&g->vignette, "plugins/darkroom/lens/expand_vignette",
                                   _("manual vignette correction"), GTK_BOX(self->widget),
                                   DT_ACTION(self));
    ...
    dt_bauhaus_widget_set_quad(g->v_strength, self, dtgtk_cairo_paint_showmask, TRUE,
                               _visualize_callback,
                               _("show applied optical vignette correction mask"));

    g->v_radius = dt_bauhaus_slider_from_params(sect, "v_radius");
    gtk_widget_set_tooltip_text(g->v_radius, _("radius of uncorrected center"));
...
void gui_update(dt_iop_module_t *self)
{
    ...
    if (g->camera && p->lens[0])
    {
        ...
        dt_pthread_mutex_lock(&darktable.plugin_threadsafe);
        const lfLens **lenslist =
            dt_iop_lensfun_db->FindLenses(g->camera, NULL, model[0] ? model : NULL, 0);
        if (lenslist)
