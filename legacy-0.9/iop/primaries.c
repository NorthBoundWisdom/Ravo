...
static void
_calculate_adjustment_matrix(const dt_iop_primaries_params_t *const params,
                             ...
                             dt_colormatrix_t matrix)
{
    ...
    for (size_t i = 0; i < 3; i++)
        dt_rotate_and_scale_primary(pipe_work_profile, scaling[i], rotation[i], i,
                                    custom_primaries[i]);

    float whitepoint[2];
    dt_rotate_and_scale_primary(pipe_work_profile, params->achromatic_tint_purity,
                                params->achromatic_tint_hue, 0, whitepoint);

    dt_colormatrix_t RGB_TO_XYZ;
    dt_make_transposed_matrices_from_primaries_and_whitepoint(custom_primaries, whitepoint,
                                                              RGB_TO_XYZ);
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    const cl_int err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_primaries, width, height,
                                                        CLARG(dev_in), CLARG(dev_out), CLARG(width),
                                                        CLARG(height), CLARG(dev_matrix));
...
static void _rotated_primary_to_display_RGB(const dt_iop_order_iccprofile_info_t *work_profile,
                                            ...
                                            const size_t primary_index, const float angle,
                                            const float desaturate, dt_aligned_pixel_t display_RGB)
{
...
static void _paint_hue_slider(const dt_iop_order_iccprofile_info_t *work_profile,
                              const dt_iop_order_iccprofile_info_t *display_profile,
                              const dt_iop_order_iccprofile_info_t *sRGB_profile,
                              const size_t primary_index, GtkWidget *slider)
{
    ...
    for (int i = 0; i < DT_BAUHAUS_SLIDER_MAX_STOPS; i++)
    {
        ...
        _rotated_primary_to_display_RGB(work_profile, display_profile, sRGB_profile, primary_index,
                                        angle, 0.4f, linear_RGB);
...
static void _paint_purity_slider(const dt_iop_order_iccprofile_info_t *work_profile,
                                 const dt_iop_order_iccprofile_info_t *display_profile,
                                 const dt_iop_order_iccprofile_info_t *sRGB_profile,
                                 const size_t primary_index, const float saturation,
...
void gui_changed(dt_iop_module_t *self, GtkWidget *w, void *previous)
{
    ...
    const gboolean repaint_all_sliders = !w || work_profile != g->painted_work_profile ||
                                         display_profile != g->painted_display_profile;

    const dt_iop_order_iccprofile_info_t *sRGB_profile = dt_ioppr_add_profile_info_to_list(
        self->dev, DT_COLORSPACE_SRGB, "", DT_INTENT_RELATIVE_COLORIMETRIC);
    ...
    if (repaint_all_sliders || w == g->red_hue)
        _paint_purity_slider(work_profile, display_profile, sRGB_profile, 0, 1.f, g->red_hue,
                             g->red_purity);
    if (repaint_all_sliders || w == g->green_hue)
        _paint_purity_slider(work_profile, display_profile, sRGB_profile, 1, 1.f, g->green_hue,
                             g->green_purity);
    ...
    if (repaint_all_sliders || w == g->achromatic_tint_hue)
        _paint_purity_slider(work_profile, display_profile, sRGB_profile, 0, 5.f,
                             g->achromatic_tint_hue, g->achromatic_tint_purity);
}

static void _signal_profile_user_changed(gpointer instance, const uint8_t profile_type,
