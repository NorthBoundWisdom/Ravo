...
static inline gboolean _control_job_equal(_dt_job_t *j1, _dt_job_t *j2)
{
    ...
    return (j1->execute == j2->execute && j1->state_changed_cb == j2->state_changed_cb &&
            j1->queue == j2->queue && (g_strcmp0(j1->description, j2->description) == 0));
}

static void _control_job_set_state(_dt_job_t *job, dt_job_state_t state)
{
    if (!job)
        return;
...
static inline const char *_queuename(const dt_job_queue_t id)
{
    ...
    case DT_JOB_QUEUE_USER_FG:
        return "DT_JOB_QUEUE_USER_FG";
    case DT_JOB_QUEUE_SYSTEM_FG:
        return "DT_JOB_QUEUE_SYSTEM_FG";
    case DT_JOB_QUEUE_USER_BG:
        return "DT_JOB_QUEUE_USER_BG";
    case DT_JOB_QUEUE_USER_EXPORT:
        return "DT_JOB_QUEUE_USER_EXPORT";
    case DT_JOB_QUEUE_SYSTEM_BG:
        return "DT_JOB_QUEUE_SYSTEM_BG";
    case DT_JOB_QUEUE_SYNCHRONOUS:
        return "DT_JOB_QUEUE_SYNCHRONOUS";
    default:
        return "???";
    }
}
...
static void _control_job_print(_dt_job_t *job, const char *info, const char *err, int32_t res)
{
    if (!job)
        return;
    dt_print(DT_DEBUG_CONTROL, "[%s]\t%02d %s %s | queue: %s | priority: %d", info, res, err,
...
static gboolean _control_run_job_res(dt_control_t *control, int32_t res)
{
    ...
    if (control->new_res[res])
    {
        ...
        control->job_res[res] =
            NULL; // this job belongs to us now, the queue may not touch it any longer
    }
...
static _dt_job_t *_control_schedule_job(dt_control_t *control)
{
   ...
   */
    ...
    {
        if (control->queues[i] == NULL)
            continue;
        if (control->export_scheduled && i == DT_JOB_QUEUE_USER_EXPORT)
            continue;
    ...
    *queue = g_list_delete_link(*queue, *queue);
    ...
    for (int i = 0; i < DT_JOB_QUEUE_MAX; i++)
    {
        if (i == winner_queue || control->queues[i] == NULL)
            continue;
...
{
    dt_control_t *control = darktable.control;
    if ((((unsigned int)queue_id) >= DT_JOB_QUEUE_MAX && queue_id != DT_JOB_QUEUE_SYNCHRONOUS) ||
        !job)
    {
        dt_control_job_dispose(job);
        return TRUE;
