...
static void _global_progress_start(dt_control_t *control, dt_progress_t *progress)
{
    ...
    progress->darktable_launcher =
        unity_launcher_entry_get_for_desktop_id("org.darktable.darktable.desktop");
    ...
    if (darktable.dbus && darktable.dbus->dbus_connection)
    {
        ...
        GVariant *params =
            g_variant_new("(sa{sv})", "application://org.darktable.darktable.desktop", &builder);

        g_dbus_connection_emit_signal(darktable.dbus->dbus_connection, "com.canonical.Unity",
                                      "/darktable", "com.canonical.Unity.LauncherEntry", "Update",
                                      params, &error);
        if (error)
        {
...
static void _global_progress_set(dt_control_t *control, dt_progress_t *progress, double value)
{
    ...
    if (control->progress_system.taskbarlist)
    {
        ...
        if (ITaskbarList3_SetProgressValue(control->progress_system.taskbarlist, hwnd,
                                           control->progress_system.global_progress * 100,
                                           100) != S_OK)
            dt_print(DT_DEBUG_ALWAYS, "[progress_create] SetProgressValue failed");
    }
...
static void _global_progress_end(dt_control_t *control, dt_progress_t *progress)
{
    ...
    for (GList *iter = control->progress_system.list; iter; iter = g_list_next(iter))
    {
        ...
        control->progress_system.global_progress =
            MAX(control->progress_system.global_progress, value);
    }
    ...
    if (control->progress_system.taskbarlist)
    {
        ...
        if (control->progress_system.n_progress_bar == 0)
        {
            if (ITaskbarList3_SetProgressState(control->progress_system.taskbarlist, hwnd,
                                               TBPF_NOPROGRESS) != S_OK)
                dt_print(DT_DEBUG_ALWAYS, "[progress_create] SetProgressState failed");
        }
        else
        {
...
dt_progress_t *dt_control_progress_create(const gboolean has_progress_bar, const gchar *message)
{
    ...
    if (control->progress_system.proxy.module != NULL)
        progress->gui_data = control->progress_system.proxy.added(
            control->progress_system.proxy.module, has_progress_bar, message);
...
void dt_control_progress_destroy(dt_progress_t *progress)
{
    ...
    // tell the gui
    if (control->progress_system.proxy.module != NULL)
        control->progress_system.proxy.destroyed(control->progress_system.proxy.module,
                                                 progress->gui_data);
                                          ...
                                          dt_progress_cancel_callback_t cancel, void *data)
{
    ...
    dt_pthread_mutex_lock(&control->progress_system.mutex);
    if (control->progress_system.proxy.module != NULL)
        control->progress_system.proxy.cancellable(control->progress_system.proxy.module,
                                                   progress->gui_data, progress);
    dt_pthread_mutex_unlock(&control->progress_system.mutex);
}
...
void dt_control_progress_set_progress(dt_progress_t *progress, double value)
{
    ...
    dt_pthread_mutex_lock(&control->progress_system.mutex);
    if (control->progress_system.proxy.module != NULL)
        control->progress_system.proxy.updated(control->progress_system.proxy.module,
                                               progress->gui_data, value);
...
void dt_control_progress_set_message(dt_progress_t *progress, const char *message)
{
    ...
    dt_pthread_mutex_lock(&control->progress_system.mutex);
    if (control->progress_system.proxy.module != NULL)
        control->progress_system.proxy.message_updated(control->progress_system.proxy.module,
                                                       progress->gui_data, message);
    dt_pthread_mutex_unlock(&control->progress_system.mutex);
}
