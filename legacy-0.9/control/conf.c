...
void dt_conf_set_int(const char *name, const int val)
{
    ...
    if (_conf_set_if_not_overridden(name, str))
        g_free(str);
}
...
void dt_conf_set_folder_from_file_chooser(const char *name, GtkFileChooser *chooser)
{
    ...
    if (GTK_IS_FILE_CHOOSER_NATIVE(chooser))
    {
        ...
        if (pathname)
        {
            ...
            if (_conf_set_if_not_overridden(name, folder))
                g_free(folder);
            g_free(pathname);
        }
        return;
...
float dt_conf_get_and_sanitize_float(const char *name, const float min, const float max)
{
...
gchar *dt_conf_get_path(const char *name)
{
    ...
    if (path[0] && item && item->type == DT_PATH &&
                  ...
                  GSList *override_entries)
{
    ...
    if (override_entries)
    {
        for (GSList *p = override_entries; p; p = g_slist_next(p))
        {
            ...
            g_hash_table_insert(darktable.conf->override_entries, g_strdup(entry->key),
