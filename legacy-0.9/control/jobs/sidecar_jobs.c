...
static void _lock_pending_queue() ACQUIRE(pending_mutex)
{
    if (!lock_initialized)
    {
...
static void _unlock_pending_queue() RELEASE(pending_mutex)
{
...
static int32_t _control_write_sidecars_job_run(dt_job_t *job)
{
    ...
    while (imgs ||
           (dt_control_running() && dt_control_job_get_state(job) != DT_JOB_STATE_CANCELLED))
    {
            ...
            if (new_imgs)
            {
                ...
                for (GSList *imglist = new_imgs; imglist; imglist = g_slist_next(imglist))
                {
                    if (!g_hash_table_contains(enqueued, GINT_TO_POINTER(imglist->data)))
                    {
                        ...
                        g_hash_table_insert(enqueued, GINT_TO_POINTER(imglist->data),
                                            GINT_TO_POINTER(imglist->data));
                    }
                }
...
{
    dt_job_t *job =
        dt_control_job_create(_control_write_sidecars_job_run, "%s", N_("synchronize sidecars"));
    if (!job)
    {
        return;
    }
