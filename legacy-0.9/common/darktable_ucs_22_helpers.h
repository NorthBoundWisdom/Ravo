...
static inline void dt_UCS_22_build_gamut_LUT(dt_colormatrix_t input_matrix,
                                             float gamut_LUT[LUT_ELEM])
{
   ...
   */
    ...
    for (int i = 0; i < 50 * LUT_ELEM; i++)
    {
        ...
        gamut_LUT[index] +=
            UV_star_prime[0] * UV_star_prime[0] + UV_star_prime[1] * UV_star_prime[1];
        sampler[index] += 1.0f;
    }
    for (size_t k = 0; k < LUT_ELEM; k++)
...
static inline float get_minimum_saturation(float gamut_LUT[LUT_ELEM], const float lightness,
                                           const float L_white)
{
    ...
    for (size_t k = 0; k < LUT_ELEM; k++)
        colorfulness_min = fminf(gamut_LUT[k], colorfulness_min);
    ...
    const float max_chroma = 15.932993652962535f * powf(lightness * L_white, 0.6523997524738018f) *
                             powf(colorfulness_min, 0.6007557017508491f) / L_white;
...
static inline float soft_clip(const float x, const float soft_threshold, const float hard_threshold)
{
    ...
    return (x > soft_threshold) ?
               soft_threshold + (1.f - expf(-(x - soft_threshold) / norm)) * norm :
               x;
}

DT_OMP_DECLARE_SIMD(aligned(HSB : 16) uniform(gamut_LUT, L_white))
static inline void gamut_map_HSB(dt_aligned_pixel_t HSB, const float gamut_LUT[LUT_ELEM],
                                 const float L_white)
{
    /**
...
static inline struct dt_iop_order_iccprofile_info_t *
D65_adapt_iccprofile(struct dt_iop_order_iccprofile_info_t *work_profile)
{
    ...
    if (work_profile) // && !isnan(work_profile->matrix_in[0][0]))
    {
        ...
        struct dt_iop_order_iccprofile_info_t *white_adapted_profile =
            dt_alloc_aligned(sizeof(dt_iop_order_iccprofile_info_t));
        ...
        transpose_3xSSE(white_adapted_profile->matrix_out,
                        white_adapted_profile->matrix_out_transposed);
        transpose_3xSSE(white_adapted_profile->matrix_in,
                        white_adapted_profile->matrix_in_transposed);
