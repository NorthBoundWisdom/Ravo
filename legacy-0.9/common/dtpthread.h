...
static inline void _report_ret_error(const int ret, const char *loc, const char *mess)
{
    printf("\n*** [dt_pthread_mutex_%s = %s] at: %s ***\n", mess ? mess : "???",
           _pthread_ret_mess(ret), loc ? loc : "no location");
    fflush(stdout);
}
...
static inline int dt_pthread_mutex_destroy(dt_pthread_mutex_t *mutex)
{
    ...
    if (ret)
        _report_ret_error(ret, mutex->name, "destroy");
    assert(ret == 0);
...
#define dt_pthread_mutex_init(A, B)                                                                \
    dt_pthread_mutex_init_with_caller(A, B, __FILE__, __LINE__, __FUNCTION__)
...
#define dt_pthread_mutex_lock(A)                                                                   \
    dt_pthread_mutex_lock_with_caller(A, __FILE__, __LINE__, __FUNCTION__)
    ...
    ACQUIRE(mutex) NO_THREAD_SAFETY_ANALYSIS
{
    ...
    if (ret)
        _report_ret_error(ret, mutex->name, "lock");
    assert(ret == 0);
    ...
    g_strlcpy(mutex->top_wait_name[min_wait_slot], name,
              sizeof(mutex->top_wait_name[min_wait_slot]));
    mutex->top_wait_sum[min_wait_slot] = wait;
    return ret;
}

#define dt_pthread_mutex_trylock(A)                                                                \
    dt_pthread_mutex_trylock_with_caller(A, __FILE__, __LINE__, __FUNCTION__)
    ...
    TRY_ACQUIRE(0, mutex)
{
    ...
    if (ret && (ret != EBUSY))
        _report_ret_error(ret, mutex->name, "trylock");
...
#define dt_pthread_mutex_unlock(A)                                                                 \
    dt_pthread_mutex_unlock_with_caller(A, __FILE__, __LINE__, __FUNCTION__)
    ...
    RELEASE(mutex) NO_THREAD_SAFETY_ANALYSIS
{
    ...
    {
        g_strlcpy(mutex->top_locked_name[min_locked_slot], name,
                  sizeof(mutex->top_locked_name[min_locked_slot]));
    ...
    if (ret)
        _report_ret_error(ret, mutex->name, "unlock");
    assert(ret == 0);
    return ret;
}
...
static inline int dt_pthread_cond_wait(pthread_cond_t *cond, dt_pthread_mutex_t *mutex)
{
    ...
    if (ret)
        _report_ret_error(ret, mutex->name, "cond_wait");
    assert(ret == 0);
    return ret;
}
...
static inline int dt_pthread_rwlock_destroy(dt_pthread_rwlock_t *lock)
{
    ...
    if (res)
        _report_retrw_error(res, lock->name, "destroy");
    assert(res == 0);
    return res;
}
...
{
    if (!lock)
        _retrw_error(0, "get_writer");
    assert(lock != NULL);
    return lock->writer;
}

#define dt_pthread_rwlock_unlock(A) dt_pthread_rwlock_unlock_with_caller(A, __FILE__, __LINE__)
static inline int dt_pthread_rwlock_unlock_with_caller(dt_pthread_rwlock_t *rwlock,
                                                       const char *file, int line)
{
    const int res = pthread_rwlock_unlock(&rwlock->lock);
    if (res)
        _report_retrw_error(res, rwlock->name, "unlock");
    assert(res == 0);

    __sync_fetch_and_sub(&rwlock->cnt, 1);
    if (rwlock->cnt < 0)
        _retrw_error(0, "unlock count below zero");
                                                       ...
                                                       const char *file, int line)
{
    ...
    if (res)
        _report_retrw_error(res, rwlock->name, "rdlock");
    assert(res == 0);

    const gboolean test = pthread_equal(rwlock->writer, pthread_self());
    if (!test)
        _report_retrw_error(res, rwlock->name, "rdlock non equal threads");
                                                       ...
                                                       const char *file, int line)
{
    ...
    if (res)
        _report_retrw_error(res, rwlock->name, "wrlock");
    assert(res == 0);
    __sync_fetch_and_add(&rwlock->cnt, 1);
    (void)__sync_lock_test_and_set(&rwlock->writer, pthread_self());
    snprintf(rwlock->name, sizeof(rwlock->name), "w:%s:%d", file, line);
    return res;
}

#define dt_pthread_rwlock_tryrdlock(A)                                                             \
    dt_pthread_rwlock_tryrdlock_with_caller(A, __FILE__, __LINE__)
                                                          ...
                                                          const char *file, int line)
{
    ...
    if (res && (res != EBUSY))
        _report_retrw_error(res, rwlock->name, "tryrdlock");
...
#define dt_pthread_rwlock_trywrlock(A)                                                             \
    dt_pthread_rwlock_trywrlock_with_caller(A, __FILE__, __LINE__)
                                                          ...
                                                          const char *file, int line)
{
    ...
    if (res && (res != EBUSY))
        _report_retrw_error(res, rwlock->name, "trywrlock");
    assert((res == 0) || (res == EBUSY));
    if (!res)
    {
        __sync_fetch_and_add(&rwlock->cnt, 1);
...
// *please* do use these;
static inline int dt_pthread_mutex_init(dt_pthread_mutex_t *mutex,
                                        const pthread_mutexattr_t *mutexattr)
{
    return pthread_mutex_init(&mutex->mutex, mutexattr);
}

static inline int dt_pthread_mutex_lock(dt_pthread_mutex_t *mutex)
    ACQUIRE(mutex) NO_THREAD_SAFETY_ANALYSIS
{
...
static inline int dt_pthread_mutex_unlock(dt_pthread_mutex_t *mutex)
    RELEASE(mutex) NO_THREAD_SAFETY_ANALYSIS
{
#ifdef MUTEX_REPORTING
    const int ret = pthread_mutex_unlock(&mutex->mutex);
    if (ret)
        _ret_error(ret, "unlock");
    return ret;
#else
