...
static gchar *_get_tb_added_tag_string_values(const dt_imgid_t img, GList *before, GList *after)
{
    GList *b = before;
    gchar *tag_list = NULL;
...
static void _bulk_remove_tags(const dt_imgid_t img, const gchar *tag_list)
{
    if (img > 0 && tag_list)
    {
        sqlite3_stmt *stmt;
...
static void _bulk_add_tags(const gchar *tag_list)
{
    ...
    {
        sqlite3_stmt *stmt;
        gchar *query = g_strdup_printf("INSERT INTO main.tagged_images (imgid, tagid, position)"
                                       " VALUES %s",
                                       tag_list);
        DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db), query, -1, &stmt, NULL);
        sqlite3_step(stmt);
...
gboolean dt_tag_new(const char *name, guint *tagid)
{
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "SELECT id FROM data.tags WHERE name = ?1", -1, &stmt, NULL);
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "INSERT INTO data.tags (id, name) VALUES (NULL, ?1)", -1, &stmt,
                                NULL);
    DT_DEBUG_SQLITE3_BIND_TEXT(stmt, 1, name, -1, SQLITE_TRANSIENT);
    sqlite3_step(stmt);
    sqlite3_finalize(stmt);
    ...
    {
        DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                    "INSERT INTO memory.darktable_tags (tagid) VALUES (?1)", -1,
                                    &stmt, NULL);
...
guint dt_tag_remove(const guint tagid, gboolean final)
{
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "SELECT COUNT(*) FROM main.tagged_images WHERE tagid=?1", -1, &stmt,
                                NULL);
    ...
    if (final == TRUE)
    {
        ...
        DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                    "DELETE FROM data.tags WHERE id=?1", -1, &stmt, NULL);
        DT_DEBUG_SQLITE3_BIND_INT(stmt, 1, tagid);
        sqlite3_step(stmt);
        sqlite3_finalize(stmt);

        DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                    "DELETE FROM main.tagged_images WHERE tagid=?1", -1, &stmt,
                                    NULL);
        DT_DEBUG_SQLITE3_BIND_INT(stmt, 1, tagid);
        sqlite3_step(stmt);
        sqlite3_finalize(stmt);
        ...
        DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                    "DELETE FROM memory.darktable_tags WHERE tagid=?1", -1, &stmt,
                                    NULL);
        DT_DEBUG_SQLITE3_BIND_INT(stmt, 1, tagid);
        sqlite3_step(stmt);
        sqlite3_finalize(stmt);
...
static gboolean _tag_execute(const GList *tags, const GList *imgs, GList **undo,
                             const gboolean undo_on, const gint action)
{
    ...
    for (const GList *images = imgs; images; images = g_list_next(images))
    {
        ...
        case DT_TA_ATTACH:
            ...
            if (_tag_add_tags_to_list(&undotags->after, tags))
                res = TRUE;
            break;
        case DT_TA_DETACH:
            undotags->after = g_list_copy(undotags->before);
            if (_tag_remove_tags_from_list(&undotags->after, tags))
                res = TRUE;
            break;
        case DT_TA_SET:
            ...
            if (dttags)
                undotags->after = g_list_concat(undotags->after, dttags);
            res = TRUE;
            break;
        case DT_TA_SET_ALL:
            undotags->after = g_list_copy((GList *)tags);
                       ...
                       const gboolean group_on)
{
    ...
    {
        if (dt_is_tag_attached(tagid, imgid))
            return FALSE;
                         ...
                         const gboolean clear_on, const gboolean undo_on)
{
    if (!g_list_is_empty(img))
    {
        ...
        if (undo_on)
        {
            dt_undo_record(darktable.undo, NULL, DT_UNDO_TAGS, undo, _pop_undo,
...
{
    DT_DEBUG_SQLITE3_EXEC(dt_database_get(darktable.db), "DELETE FROM memory.darktable_tags", NULL,
                          NULL, NULL);
...
uint32_t dt_tag_count_attached(const dt_imgid_t imgid, const gboolean ignore_dt_tags)
{
    ...
    gchar *query =
        g_strdup_printf("SELECT COUNT(tagid)"
                        " FROM main.tagged_images"
                        " WHERE imgid = %d"
                        "       %s",
                        imgid, ignore_dt_tags ? " AND tagid NOT IN memory.darktable_tags" : "");
        ...
        if (pch != NULL)
        {
            if (omit_tag_hierarchy)
            {
                ...
                for (; *iter && *(iter + 1); iter++)
                    ;
                if (*iter)
                    tags = g_list_prepend(tags, g_strdup(*iter));
            }
            else
            {
                size_t j = 0;
...
static gint _is_not_exportable_tag(gconstpointer a, gconstpointer b)
{
    ...
    return ((g_strcmp0(ta->tag, tb->tag) == 0) &&
            ((ta->flags) & (DT_TF_CATEGORY | DT_TF_PRIVATE))) ?
               0 :
               -1;
}
        ...
        *end = '\0';
        ...
        if (*tag)
        {
            ...
            if (tagid != 0)
            {
                ...
                is_private =
                    (flags & (DT_TF_CATEGORY | DT_TF_PRIVATE)) == (DT_TF_CATEGORY | DT_TF_PRIVATE);
            }
        }
    }
...
GList *dt_tag_get_list_export(const dt_imgid_t imgid, const int32_t flags)
{
    ...
    {
        dt_tag_t *t = sorted_tags->data;
        if ((export_private_tags || !(_check_private_recursive(t))) && !(t->flags & DT_TF_CATEGORY))
        {
            ...
            if (!omit_tag_hierarchy)
            {
                ...
                while (end)
                {
                    ...
                    if (!next || !g_list_find_custom(next, t, (GCompareFunc)_is_not_exportable_tag))
                    {
...
uint32_t dt_tag_get_suggestions(GList **result)
{
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2
    ...
    DT_DEBUG_SQLITE3_EXEC(dt_database_get(darktable.db), "DELETE FROM memory.taglist", NULL, NULL,
                          NULL);
    g_free(query);
...
void dt_tag_count_tags_images(const gchar *keyword, int *tag_count, int *img_count)
{
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "SELECT COUNT(DISTINCT tagid) FROM memory.similar_tags", -1, &stmt,
                                NULL);
    sqlite3_step(stmt);
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
    ...
    *img_count = sqlite3_column_int(stmt, 0);
    ...
    DT_DEBUG_SQLITE3_EXEC(dt_database_get(darktable.db), "DELETE FROM memory.similar_tags", NULL,
                          NULL, NULL);
}
...
void dt_tag_set_synonyms(gint tagid, gchar *synonyms_entry)
{
    ...
    sqlite3_stmt *stmt;
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "UPDATE data.tags SET synonyms = ?2 WHERE id = ?1 ", -1, &stmt,
                                NULL);
    DT_DEBUG_SQLITE3_BIND_INT(stmt, 1, tagid);
...
void dt_tag_add_synonym(const gint tagid, gchar *synonym)
{
...
ssize_t dt_tag_export(const char *filename)
{
    ...
    for (GList *tag_elt = sorted_tags; tag_elt; tag_elt = g_list_next(tag_elt))
    {
        ...
        int common_start;
        for (common_start = 0;
             hierarchy && hierarchy[common_start] && tokens && tokens[common_start]; common_start++)
        {
...
char *dt_tag_get_subtags(const dt_imgid_t imgid, const char *category, const int level)
{
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
    ...
    if (tags)
        tags[strlen(tags) - 1] = '\0'; // remove the last comma
    sqlite3_finalize(stmt);
    return tags;
}
