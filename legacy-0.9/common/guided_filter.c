...
static void _guided_filter_tiling(color_image imgg, gray_image img, gray_image img_out, tile target,
                                  const int w, const float eps, const float guide_weight,
...
#define VAR_GB 7
    ...
    for (int j_imgg = source.lower; j_imgg < source.upper; j_imgg++)
    {
        ...
        for (int i_imgg = source.left; i_imgg < source.right; i_imgg++)
        {
            ...
            dt_aligned_pixel_t pixel = {pixel_[0] * guide_weight, pixel_[1] * guide_weight,
                                        pixel_[2] * guide_weight, pixel_[3] * guide_weight};
    ...
    dt_box_mean_vertical(variance.data, variance.height, variance.width, 9 | BOXFILTER_KAHAN_SUM,
                         w);
...
#define B 3
    ...
    for (int j_imgg = target.lower; j_imgg < target.upper; j_imgg++)
    {
        ...
        for (int i_imgg = target.left; i_imgg < target.right; i_imgg++, k++, l++)
        {
            ...
            float res = guide_weight * (px_ab[A_RED] * pixel[0] + px_ab[A_GREEN] * pixel[1] +
...
void guided_filter(
    const float *const guide, const float *const in, float *const out, const int width,
    ...
    const float min, const float max)
{
    ...
    const float eps =
        sqrt_eps * sqrt_eps; // this is the regularization parameter of the original papers

    for (int j = 0; j < height; j += tile_dim)
    {
        for (int i = 0; i < width; i += tile_dim)
        {
            tile target = {i, MIN(i + tile_dim, width), j, MIN(j + tile_dim, height)};
            _guided_filter_tiling(img_guide, img_in, img_out, target, w, eps, guide_weight, min,
                                  max);
        }
    }
}
...
dt_guided_filter_cl_global_t *dt_guided_filter_init_cl_global()
{
    ...
    g->kernel_guided_filter_split_rgb =
        dt_opencl_create_kernel(program, "guided_filter_split_rgb_image");
    g->kernel_guided_filter_box_mean_x =
        dt_opencl_create_kernel(program, "guided_filter_box_mean_x");
    g->kernel_guided_filter_box_mean_y =
        dt_opencl_create_kernel(program, "guided_filter_box_mean_y");
    g->kernel_guided_filter_guided_filter_covariances =
        dt_opencl_create_kernel(program, "guided_filter_covariances");
    g->kernel_guided_filter_guided_filter_variances =
        dt_opencl_create_kernel(program, "guided_filter_variances");
    g->kernel_guided_filter_update_covariance =
        dt_opencl_create_kernel(program, "guided_filter_update_covariance");
    g->kernel_guided_filter_solve = dt_opencl_create_kernel(program, "guided_filter_solve");
    g->kernel_guided_filter_generate_result =
        dt_opencl_create_kernel(program, "guided_filter_generate_result");
    return g;
}
...
static int _cl_split_rgb(const int devid, const int width, const int height, const int first,
                         cl_mem guide, cl_mem imgg_r, cl_mem imgg_g, cl_mem imgg_b,
                         const float guide_weight)
{
    ...
    return dt_opencl_enqueue_kernel_2d_args(
        devid, kernel, width, height, CLARG(width), CLARG(height), CLARG(first), CLARG(guide),
        CLARG(imgg_r), CLARG(imgg_g), CLARG(imgg_b), CLARG(guide_weight));
}

static int _cl_box_mean(const int devid, const int width, const int height, const int w, cl_mem in,
                        cl_mem out, cl_mem temp)
{
    const cl_int err = dt_opencl_enqueue_kernel_1d_args(
...
static int _cl_covariances(const int devid, const int width, const int height, const int first,
                           cl_mem guide, cl_mem in, cl_mem cov_imgg_img_r, cl_mem cov_imgg_img_g,
                           cl_mem cov_imgg_img_b, const float guide_weight)
{
    const int kernel =
        darktable.opencl->guided_filter->kernel_guided_filter_guided_filter_covariances;
    return dt_opencl_enqueue_kernel_2d_args(devid, kernel, width, height, CLARG(width),
                                            CLARG(height), CLARG(first), CLARG(guide), CLARG(in),
                                            CLARG(cov_imgg_img_r), CLARG(cov_imgg_img_g),
                                            CLARG(cov_imgg_img_b), CLARG(guide_weight));
}

static int _cl_variances(const int devid, const int width, const int height, const int first,
                         cl_mem guide, cl_mem var_imgg_rr, cl_mem var_imgg_rg, cl_mem var_imgg_rb,
                         cl_mem var_imgg_gg, cl_mem var_imgg_gb, cl_mem var_imgg_bb,
                         const float guide_weight)
{
    const int kernel =
        darktable.opencl->guided_filter->kernel_guided_filter_guided_filter_variances;
    return dt_opencl_enqueue_kernel_2d_args(
        devid, kernel, width, height, CLARG(width), CLARG(height), CLARG(first), CLARG(guide),
        CLARG(var_imgg_rr), CLARG(var_imgg_rg), CLARG(var_imgg_rb), CLARG(var_imgg_gg),
        CLARG(var_imgg_gb), CLARG(var_imgg_bb), CLARG(guide_weight));
}

static int _cl_update_covariance(const int devid, const int width, const int height, cl_mem in,
                                 cl_mem out, cl_mem a, cl_mem b, float eps)
{
    const int kernel = darktable.opencl->guided_filter->kernel_guided_filter_update_covariance;
    return dt_opencl_enqueue_kernel_2d_args(devid, kernel, width, height, CLARG(width),
                                            CLARG(height), CLARG(in), CLARG(out), CLARG(a),
                                            CLARG(b), CLARG(eps));
}

static int _cl_solve(const int devid, const int width, const int height, cl_mem img_mean,
                     cl_mem imgg_mean_r, cl_mem imgg_mean_g, cl_mem imgg_mean_b,
                     cl_mem cov_imgg_img_r, cl_mem cov_imgg_img_g, cl_mem cov_imgg_img_b,
                     cl_mem var_imgg_rr, cl_mem var_imgg_rg, cl_mem var_imgg_rb, cl_mem var_imgg_gg,
                     cl_mem var_imgg_gb, cl_mem var_imgg_bb, cl_mem a_r, cl_mem a_g, cl_mem a_b,
                     cl_mem b)
{
    const int kernel = darktable.opencl->guided_filter->kernel_guided_filter_solve;
    return dt_opencl_enqueue_kernel_2d_args(
        devid, kernel, width, height, CLARG(width), CLARG(height), CLARG(img_mean),
        CLARG(imgg_mean_r), CLARG(imgg_mean_g), CLARG(imgg_mean_b), CLARG(cov_imgg_img_r),
        CLARG(cov_imgg_img_g), CLARG(cov_imgg_img_b), CLARG(var_imgg_rr), CLARG(var_imgg_rg),
        CLARG(var_imgg_rb), CLARG(var_imgg_gg), CLARG(var_imgg_gb), CLARG(var_imgg_bb), CLARG(a_r),
        CLARG(a_g), CLARG(a_b), CLARG(b));
}

static int _cl_generate_result(const int devid, const int width, const int height, const int first,
                               cl_mem guide, cl_mem a_r, cl_mem a_g, cl_mem a_b, cl_mem b,
                               cl_mem out, const float guide_weight, const float min,
                               const float max)
{
    const int kernel = darktable.opencl->guided_filter->kernel_guided_filter_generate_result;
    return dt_opencl_enqueue_kernel_2d_args(devid, kernel, width, height, CLARG(width),
                                            CLARG(height), CLARG(first), CLARG(guide), CLARG(a_r),
                                            CLARG(a_g), CLARG(a_b), CLARG(b), CLARG(out),
                                            CLARG(guide_weight), CLARG(min), CLARG(max));
}

static int _guided_filter_cl_impl(
    int devid, cl_mem guide, cl_mem dev_in, cl_mem dev_out, const int width, const int iheight,
    const int w,              // window size
    const float sqrt_eps,     // regularization parameter
    ...
    const float min, const float max)
{
    ...
    if (tiling || (darktable.unmuted & DT_DEBUG_VERBOSE))
        dt_print(DT_DEBUG_PIPE | DT_DEBUG_TILING,
                 "[guided CL_%d filter] %s tile_height=%d tiles=%d valid=%d overlap=%d", devid,
                 !possible ? "impossible" : (tiling ? "tiling" : "direct"), tile_height, num_tiles,
                 valid_rows, overlap);
    ...
    if (!temp1 || !temp2 || !imgg_mean_r || !imgg_mean_g || !imgg_mean_b || !img_mean ||
        !cov_imgg_img_r || !cov_imgg_img_g || !cov_imgg_img_b || !var_imgg_rr || !var_imgg_gg ||
        !var_imgg_bb || !var_imgg_rg || !var_imgg_rb || !var_imgg_gb || !a_r || !a_g || !a_b ||
        !b || !out || !in)
    {
    ...
    for (int tile_nr = 0; tile_nr < num_tiles; tile_nr++)
    {
        ...
        if (out_height > 0)
        {
            ...
            if (err == CL_SUCCESS)
                err = _cl_split_rgb(devid, width, t_height, first_in, guide, imgg_mean_r,
                                    imgg_mean_g, imgg_mean_b, guide_weight);

            if (err == CL_SUCCESS)
                err = _cl_box_mean(devid, width, t_height, w, in, img_mean, temp1);
            if (err == CL_SUCCESS)
                err = _cl_box_mean(devid, width, t_height, w, imgg_mean_r, imgg_mean_r, temp1);
            if (err == CL_SUCCESS)
                err = _cl_box_mean(devid, width, t_height, w, imgg_mean_g, imgg_mean_g, temp1);
            if (err == CL_SUCCESS)
                err = _cl_box_mean(devid, width, t_height, w, imgg_mean_b, imgg_mean_b, temp1);

            if (err == CL_SUCCESS)
                err = _cl_covariances(devid, width, t_height, first_in, guide, in, cov_imgg_img_r,
                                      cov_imgg_img_g, cov_imgg_img_b, guide_weight);
            if (err == CL_SUCCESS)
                err =
                    _cl_variances(devid, width, t_height, first_in, guide, var_imgg_rr, var_imgg_rg,
                                  var_imgg_rb, var_imgg_gg, var_imgg_gb, var_imgg_bb, guide_weight);

            if (err == CL_SUCCESS)
                err = _cl_box_mean(devid, width, t_height, w, cov_imgg_img_r, temp2, temp1);
            if (err == CL_SUCCESS)
                err = _cl_update_covariance(devid, width, t_height, temp2, cov_imgg_img_r,
                                            imgg_mean_r, img_mean, 0.f);

            if (err == CL_SUCCESS)
                err = _cl_box_mean(devid, width, t_height, w, cov_imgg_img_g, temp2, temp1);
            if (err == CL_SUCCESS)
                err = _cl_update_covariance(devid, width, t_height, temp2, cov_imgg_img_g,
                                            imgg_mean_g, img_mean, 0.f);

            if (err == CL_SUCCESS)
                err = _cl_box_mean(devid, width, t_height, w, cov_imgg_img_b, temp2, temp1);
            if (err == CL_SUCCESS)
                err = _cl_update_covariance(devid, width, t_height, temp2, cov_imgg_img_b,
                                            imgg_mean_b, img_mean, 0.f);

            if (err == CL_SUCCESS)
                err = _cl_box_mean(devid, width, t_height, w, var_imgg_rr, temp2, temp1);
            if (err == CL_SUCCESS)
                err = _cl_update_covariance(devid, width, t_height, temp2, var_imgg_rr, imgg_mean_r,
                                            imgg_mean_r, eps);

            if (err == CL_SUCCESS)
                err = _cl_box_mean(devid, width, t_height, w, var_imgg_rg, temp2, temp1);
            if (err == CL_SUCCESS)
                err = _cl_update_covariance(devid, width, t_height, temp2, var_imgg_rg, imgg_mean_r,
                                            imgg_mean_g, 0.f);

            if (err == CL_SUCCESS)
                err = _cl_box_mean(devid, width, t_height, w, var_imgg_rb, temp2, temp1);
            if (err == CL_SUCCESS)
                err = _cl_update_covariance(devid, width, t_height, temp2, var_imgg_rb, imgg_mean_r,
                                            imgg_mean_b, 0.f);

            if (err == CL_SUCCESS)
                err = _cl_box_mean(devid, width, t_height, w, var_imgg_gg, temp2, temp1);
            if (err == CL_SUCCESS)
                err = _cl_update_covariance(devid, width, t_height, temp2, var_imgg_gg, imgg_mean_g,
                                            imgg_mean_g, eps);

            if (err == CL_SUCCESS)
                err = _cl_box_mean(devid, width, t_height, w, var_imgg_gb, temp2, temp1);
            if (err == CL_SUCCESS)
                err = _cl_update_covariance(devid, width, t_height, temp2, var_imgg_gb, imgg_mean_g,
                                            imgg_mean_b, 0.f);

            if (err == CL_SUCCESS)
                err = _cl_box_mean(devid, width, t_height, w, var_imgg_bb, temp2, temp1);
            if (err == CL_SUCCESS)
                err = _cl_update_covariance(devid, width, t_height, temp2, var_imgg_bb, imgg_mean_b,
                                            imgg_mean_b, eps);

            if (err == CL_SUCCESS)
                err = _cl_solve(devid, width, t_height, img_mean, imgg_mean_r, imgg_mean_g,
                                imgg_mean_b, cov_imgg_img_r, cov_imgg_img_g, cov_imgg_img_b,
                                var_imgg_rr, var_imgg_rg, var_imgg_rb, var_imgg_gg, var_imgg_gb,
                                var_imgg_bb, a_r, a_g, a_b, b);

            if (err == CL_SUCCESS)
                err = _cl_box_mean(devid, width, t_height, w, a_r, a_r, temp1);
            if (err == CL_SUCCESS)
                err = _cl_box_mean(devid, width, t_height, w, a_g, a_g, temp1);
            if (err == CL_SUCCESS)
                err = _cl_box_mean(devid, width, t_height, w, a_b, a_b, temp1);
            if (err == CL_SUCCESS)
                err = _cl_box_mean(devid, width, t_height, w, b, b, temp1);
            if (err == CL_SUCCESS)
                err = _cl_generate_result(devid, width, t_height, first_in, guide, a_r, a_g, a_b, b,
                                          out, guide_weight, min, max);
...
final:
    if (err != CL_SUCCESS)
        dt_print(DT_DEBUG_PIPE | DT_DEBUG_OPENCL, "[guided CL_%d filter] error %s", devid,
                 cl_errstr(err));

    if (tiling)
        dt_opencl_release_mem_object(out);
    if (tiling)
        dt_opencl_release_mem_object(in);
...
static int _guided_filter_cl_fallback(
    int devid, cl_mem guide, cl_mem in, cl_mem out, const int width, const int height, const int ch,
    const int w,              // window size
    const float sqrt_eps,     // regularization parameter
    ...
    const float min, const float max)
{
    ...
    guided_filter(guide_host, in_host, out_host, width, height, ch, w, sqrt_eps, guide_weight, min,
                  max);
    err = dt_opencl_write_host_to_image(devid, out_host, out, width, height, sizeof(float));

error:
    if (err != CL_SUCCESS)
        dt_print(DT_DEBUG_PIPE | DT_DEBUG_OPENCL, "[guided CL_%d fallback filter] error %s", devid,
                 cl_errstr(err));
    ...
    const float min, const float max)
{
    ...
    cl_int err = _guided_filter_cl_impl(devid, guide, in, out, width, height, w, sqrt_eps,
                                        guide_weight, min, max);

    if (err != CL_SUCCESS)
        err = _guided_filter_cl_fallback(devid, guide, in, out, width, height, ch, w, sqrt_eps,
                                         guide_weight, min, max);
