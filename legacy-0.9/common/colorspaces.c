...
static const dt_colorspaces_color_profile_t *
_get_profile(dt_colorspaces_t *self, dt_colorspaces_color_profile_type_t type, const char *filename,
             dt_colorspaces_profile_direction_t direction);

static int _colorspaces_get_matrix_from_profile(cmsHPROFILE prof, dt_colormatrix_t matrix,
                                                float *lutr, float *lutg, float *lutb,
                                                const int lutsize, const int input)
{
    ...
    // check this first:
    if (!prof || !cmsIsMatrixShaper(prof))
        return 1;
...
int dt_colorspaces_get_matrix_from_input_profile(cmsHPROFILE prof, dt_colormatrix_t matrix,
                                                 float *lutr, float *lutg, float *lutb,
                                                 const int lutsize)
{
...
int dt_colorspaces_get_matrix_from_output_profile(cmsHPROFILE prof, dt_colormatrix_t matrix,
                                                  float *lutr, float *lutg, float *lutb,
                                                  const int lutsize)
{
...
static cmsHPROFILE _colorspaces_create_srgb_profile(const gboolean v2)
{
    ...
    cmsHPROFILE profile =
        _create_lcms_profile("sRGB", "sRGB", &D65xyY, &sRGB_Primaries, transferFunction,
#if LCMS_VERSION >= 2140
                             &cicp,
#else
                             NULL,
...
static cmsHPROFILE _colorspaces_create_brg_profile()
{
    ...
    cmsHPROFILE profile =
        _create_lcms_profile("BRG", "BRG", &D65xyY, &BRG_Primaries, transferFunction, NULL, TRUE);
...
static cmsHPROFILE _colorspaces_create_gamma_rec709_rgb_profile(void)
{
    ...
    cmsHPROFILE profile = _create_lcms_profile("Gamma Rec709 RGB", "Gamma Rec709 RGB", &D65xyY,
                                               &Rec709_Primaries, transferFunction, NULL, TRUE);
...
static cmsHPROFILE dt_colorspaces_create_adobergb_profile(void)
{
    ...
    cmsHPROFILE profile = _create_lcms_profile("Adobe RGB (compatible)", "Adobe RGB", &D65xyY,
                                               &Adobe_Primaries, transferFunction, NULL, TRUE);
...
static cmsHPROFILE dt_colorspaces_create_linear_rec709_rgb_profile(void)
{
    ...
    cmsHPROFILE profile = _create_lcms_profile("Linear Rec709 RGB", "Linear Rec709 RGB", &D65xyY,
                                               &Rec709_Primaries, transferFunction, NULL, TRUE);
...
static cmsHPROFILE _colorspaces_create_linear_rec2020_rgb_profile(void)
{
    ...
    cmsHPROFILE profile = _create_lcms_profile("Linear Rec2020 RGB", "Linear Rec2020 RGB", &D65xyY,
                                               &Rec2020_Primaries, transferFunction, NULL, TRUE);
...
static cmsHPROFILE _colorspaces_create_pq_rec2020_rgb_profile(void)
{
    ...
    cmsHPROFILE profile = _create_lcms_profile("PQ Rec2020 RGB", "PQ Rec2020 RGB", &D65xyY,
                                               &Rec2020_Primaries, transferFunction, NULL, TRUE);
...
static cmsHPROFILE _colorspaces_create_hlg_rec2020_rgb_profile(void)
{
    ...
    cmsHPROFILE profile = _create_lcms_profile("HLG Rec2020 RGB", "HLG Rec2020 RGB", &D65xyY,
                                               &Rec2020_Primaries, transferFunction, NULL, TRUE);
...
static cmsHPROFILE _colorspaces_create_pq_p3_rgb_profile(void)
{
    ...
    cmsHPROFILE profile = _create_lcms_profile("PQ P3 RGB", "PQ P3 RGB", &D65xyY, &P3_Primaries,
...
static cmsHPROFILE _colorspaces_create_hlg_p3_rgb_profile(void)
{
    ...
    cmsHPROFILE profile = _create_lcms_profile("HLG P3 RGB", "HLG P3 RGB", &D65xyY, &P3_Primaries,
                                               transferFunction, NULL, TRUE);
...
static cmsHPROFILE _colorspaces_create_display_p3_rgb_profile(void)
{
    ...
    cmsHPROFILE profile = _create_lcms_profile("Display P3 RGB", "Display P3 RGB", &D65xyY,
                                               &P3_Primaries, transferFunction, NULL, TRUE);
...
static cmsHPROFILE _colorspaces_create_linear_prophoto_rgb_profile(void)
{
    ...
    cmsHPROFILE profile =
        _create_lcms_profile("Linear ProPhoto RGB", "Linear ProPhoto RGB", &D50xyY,
                             &ProPhoto_Primaries, transferFunction, NULL, TRUE);
...
static cmsHPROFILE _colorspaces_create_linear_infrared_profile(void)
{
    ...
    cmsHPROFILE profile =
        _create_lcms_profile("Linear Infrared BGR", "darktable Linear Infrared BGR", &D65xyY,
                             &BGR_Primaries, transferFunction, NULL, FALSE);
...
dt_colorspaces_get_output_profile(const dt_imgid_t imgid,
        ...
        if (sqlite3_step(stmt) == SQLITE_ROW)
        {
            ...
            if (type && filename)
                p = dt_colorspaces_get_profile(
                    *type, filename, DT_PROFILE_DIRECTION_OUT | DT_PROFILE_DIRECTION_DISPLAY);
        }
        sqlite3_finalize(stmt);
    }
...
cmsHPROFILE dt_colorspaces_get_rgb_profile_from_mem(uint8_t *data, const uint32_t size)
{
                                     ...
                                     char *name, const size_t len)
{
    ...
    else
    {
        ...
        if (!wbuf)
            goto error;
        size = cmsGetProfileInfo(p, cmsInfoDescription, language, country, wbuf,
                                 sizeof(wchar_t) * size);
        if (size == 0)
            goto error;
...
static dt_colorspaces_color_profile_t *
_create_profile(dt_colorspaces_color_profile_type_t type, cmsHPROFILE profile, const char *name,
                const int in_pos, const int out_pos, const int display_pos, const int category_pos,
                const int work_pos, const int display2_pos)
{
...
static void _update_display_transforms(dt_colorspaces_t *self)
{
    ...
    self->transform_srgb_to_display = cmsCreateTransform(
        _get_profile(self, DT_COLORSPACE_SRGB, "", DT_PROFILE_DIRECTION_DISPLAY)->profile,
        TYPE_RGBA_8, display_profile, TYPE_BGRA_8, self->display_intent, 0);

    self->transform_adobe_rgb_to_display = cmsCreateTransform(
        _get_profile(self, DT_COLORSPACE_ADOBERGB, "", DT_PROFILE_DIRECTION_DISPLAY)->profile,
        TYPE_RGBA_8, display_profile, TYPE_BGRA_8, self->display_intent, 0);
}
...
static void _update_display2_transforms(dt_colorspaces_t *self)
{
    ...
    const dt_colorspaces_color_profile_t *display2_dt_profile = _get_profile(
        self, self->display2_type, self->display2_filename, DT_PROFILE_DIRECTION_DISPLAY2);
    ...
    self->transform_srgb_to_display2 = cmsCreateTransform(
        _get_profile(self, DT_COLORSPACE_SRGB, "", DT_PROFILE_DIRECTION_DISPLAY2)->profile,
        TYPE_RGBA_8, display2_profile, TYPE_BGRA_8, self->display2_intent, 0);

    self->transform_adobe_rgb_to_display2 = cmsCreateTransform(
        _get_profile(self, DT_COLORSPACE_ADOBERGB, "", DT_PROFILE_DIRECTION_DISPLAY2)->profile,
        TYPE_RGBA_8, display2_profile, TYPE_BGRA_8, self->display2_intent, 0);
}
...
// make sure that darktable.color_profiles->xprofile_lock is held when calling this!
static gboolean _update_display_profile(guchar *tmp_data, const gsize size, char *name,
...
static gboolean _update_display2_profile(guchar *tmp_data, const gsize size, char *name,
                                         const size_t name_size)
{
...
static void cms_error_handler(const cmsContext ContextID, const cmsUInt32Number ErrorCode,
                              const char *text)
{
        ...
        while ((d_name = g_dir_read_name(dir)))
        {
            ...
            if (!g_ascii_strcasecmp(cc, ".icc") || !g_ascii_strcasecmp(cc, ".icm"))
            {
                ...
                if (tmpprof)
                {
                    ...
                    if (prof)
                    {
                        dt_colorspaces_get_profile_name(tmpprof, lang, lang + 3, prof->name,
                                                        sizeof(prof->name));
...
dt_colorspaces_t *dt_colorspaces_init()
{
    ...
    int in_pos = -1, out_pos = -1, display_pos = -1, display2_pos = -1, category_pos = -1,
        work_pos = -1;

    // init the category profile with NULL profile, the actual profile
    // must be retrieved dynamically by the caller
    res->profiles =
        g_list_append(res->profiles, _create_profile(DT_COLORSPACE_WORK, NULL, _("work profile"),
                                                     -1, -1, -1, ++category_pos, -1, -1));

    res->profiles = g_list_append(res->profiles,
                                  _create_profile(DT_COLORSPACE_EXPORT, NULL, _("export profile"),
                                                  -1, -1, -1, ++category_pos, -1, -1));

    res->profiles = g_list_append(res->profiles, _create_profile(DT_COLORSPACE_SOFTPROOF, NULL,
                                                                 _("softproof profile"), -1, -1, -1,
                                                                 ++category_pos, -1, -1));
    ...
    res->profiles = g_list_append(
        res->profiles, _create_profile(DT_COLORSPACE_SRGB, dt_colorspaces_create_srgb_profile_v4(),
    ...
    res->profiles = g_list_append(
        res->profiles,
        _create_profile(DT_COLORSPACE_LIN_REC709, dt_colorspaces_create_linear_rec709_rgb_profile(),
    ...
    res->profiles = g_list_append(
        ...
        _create_profile(DT_COLORSPACE_REC709, _colorspaces_create_gamma_rec709_rgb_profile(),
                        _("Rec709 RGB"), ++in_pos, ++out_pos, -1, -1, ++work_pos, -1));

    res->profiles = g_list_append(
        res->profiles,
        _create_profile(DT_COLORSPACE_LIN_REC2020, _colorspaces_create_linear_rec2020_rgb_profile(),
                        _("linear Rec2020 RGB"), ++in_pos, ++out_pos, ++display_pos, ++category_pos,
                        ++work_pos, ++display2_pos));

    res->profiles = g_list_append(
        res->profiles,
        _create_profile(DT_COLORSPACE_PQ_REC2020, _colorspaces_create_pq_rec2020_rgb_profile(),
                        _("PQ Rec2020 RGB"), ++in_pos, ++out_pos, ++display_pos, ++category_pos,
                        ++work_pos, ++display2_pos));

    res->profiles = g_list_append(
        res->profiles,
        _create_profile(DT_COLORSPACE_HLG_REC2020, _colorspaces_create_hlg_rec2020_rgb_profile(),
                        _("HLG Rec2020 RGB"), ++in_pos, ++out_pos, ++display_pos, ++category_pos,
                        ++work_pos, ++display2_pos));

    res->profiles = g_list_append(
        res->profiles, _create_profile(DT_COLORSPACE_PQ_P3, _colorspaces_create_pq_p3_rgb_profile(),
                                       _("PQ P3 RGB"), ++in_pos, ++out_pos, ++display_pos,
                                       ++category_pos, ++work_pos, ++display2_pos));
    ...
    res->profiles = g_list_append(
        res->profiles,
        _create_profile(DT_COLORSPACE_DISPLAY_P3, _colorspaces_create_display_p3_rgb_profile(),
                        _("Display P3 RGB"), ++in_pos, ++out_pos, ++display_pos, ++category_pos,
                        ++work_pos, ++display2_pos));

    res->profiles = g_list_append(
        res->profiles, _create_profile(DT_COLORSPACE_PROPHOTO_RGB,
                                       _colorspaces_create_linear_prophoto_rgb_profile(),
                                       _("linear ProPhoto RGB"), ++in_pos, ++out_pos, ++display_pos,
                                       ++category_pos, ++work_pos, ++display2_pos));

    res->profiles = g_list_append(
        res->profiles,
        _create_profile(DT_COLORSPACE_XYZ, _colorspaces_create_xyz_profile(), _("linear XYZ"),
                        ++in_pos, dt_conf_get_bool("allow_lab_output") ? ++out_pos : -1, -1, -1, -1,
                        -1));

    res->profiles = g_list_append(
        res->profiles,
        _create_profile(DT_COLORSPACE_LAB, _colorspaces_create_lab_profile(), _("Lab"), ++in_pos,
                        dt_conf_get_bool("allow_lab_output") ? ++out_pos : -1, -1, -1, -1, -1));

    res->profiles = g_list_append(
        res->profiles,
        _create_profile(DT_COLORSPACE_INFRARED, _colorspaces_create_linear_infrared_profile(),
                        _("linear infrared BGR"), ++in_pos, -1, -1, -1, -1, -1));

    res->profiles = g_list_append(
        res->profiles, _create_profile(DT_COLORSPACE_BRG, _colorspaces_create_brg_profile(),
    ...
    for (GList *iter = temp_profiles; iter; iter = g_list_next(iter))
    {
        ...
        const gboolean is_valid_matrix_profile =
            dt_colorspaces_get_matrix_from_output_profile(prof->profile, NULL, NULL, NULL, NULL,
                                                          0) == 0 &&
            dt_colorspaces_get_matrix_from_input_profile(prof->profile, NULL, NULL, NULL, NULL,
                                                         0) == 0;
        prof->out_pos = ++out_pos;
        prof->display_pos = ++display_pos;
        prof->display2_pos = ++display2_pos;
        ...
        else
        {
            dt_print(DT_DEBUG_DEV,
                     ...
                     prof->name, (char)(color_space >> 24), (char)(color_space >> 16),
                     (char)(color_space >> 8), (char)(color_space));
...
void dt_colorspaces_set_display_profile(const dt_colorspaces_color_profile_type_t profile_type)
{
    ...
    if (use_xatom)
    {
        ...
        gint format = 0;
        gdk_property_get(gdk_screen_get_root_window(screen), gdk_atom_intern(atom_name, FALSE),
                         GDK_NONE, 0, 64 * 1024 * 1024, FALSE, &type, &format, &buffer_size,
                         &buffer);
        g_free(atom_name);
    }
  ...
  profile_source = g_strdup("osx color profile api");
    ...
    GtkWidget *widget = (profile_type == DT_COLORSPACE_DISPLAY2) ? darktable.develop->second_wnd :
                                                                   dt_ui_center(darktable.gui->ui);
    GdkWindow *window = gtk_widget_get_window(widget);
    ...
    if (profile_changed)
    {
        ...
        dt_print(DT_DEBUG_CONTROL,
                 "[color profile] we got %s new screen profile `%s'"
                 " from the %s (size: %d)",
                                                                ...
                                                                const char *filename)
{
    ...
    if (filename != NULL)
        dt_print(DT_DEBUG_IMAGEIO,
                 "[colorin] unsupported CICP color profile"
                 " for `%s': %d/%d/%d",
                 filename, cicp->color_primaries, cicp->transfer_characteristics,
...
static const dt_colorspaces_color_profile_t *
_get_profile(dt_colorspaces_t *self, const dt_colorspaces_color_profile_type_t type,
             const char *filename, const dt_colorspaces_profile_direction_t direction)
{
    for (GList *iter = self->profiles; iter; iter = g_list_next(iter))
    {
        dt_colorspaces_color_profile_t *p = iter->data;
...
const dt_colorspaces_color_profile_t *
dt_colorspaces_get_profile(const dt_colorspaces_color_profile_type_t type, const char *filename,
...
static void dt_colorspaces_pseudoinverse(double (*in)[3], double (*out)[3], const int size)
{
    double work[3][6];
...
gboolean dt_colorspaces_conversion_matrices_xyz(const float adobe_XYZ_to_CAM[4][3],
                                                float in_XYZ_to_CAM[9], double XYZ_to_CAM[4][3],
...
gboolean dt_colorspaces_conversion_matrices_rgb(const float adobe_XYZ_to_CAM[4][3],
                                                ...
                                                const float *embedded_matrix, double mul[4])
{
...
void dt_colorspaces_cygm_apply_coeffs_to_rgb(float *out, const float *in, const int num,
                                             double RGB_to_CAM[4][3], double CAM_to_RGB[3][4],
...
void dt_colorspaces_cygm_to_rgb(float *out, const int num, const double CAM_to_RGB[3][4])
{
    DT_OMP_FOR()
    for (int i = 0; i < num; i++)
    {
...
void dt_colorspaces_rgb_to_cygm(float *out, const int num, const double RGB_to_CAM[4][3])
{
    DT_OMP_FOR()
    for (int i = 0; i < num; i++)
    {
                                                                  ...
                                                                  float whitepoint[2])
{
    ...
    if (!red_color || !green_color || !blue_color || !white_color)
        return FALSE;
...
void dt_make_transposed_matrices_from_primaries_and_whitepoint(
    const float primaries[3][2], const float whitepoint[2], dt_colormatrix_t RGB_to_XYZ_transposed)
{
    ...
    const dt_aligned_pixel_t XYZ_white = {whitepoint[0] / y, 1.f, (1.f - whitepoint[0] - y) / y};
    dt_apply_transposed_color_matrix(XYZ_white, primaries_inverse, scale);

    for (size_t i = 0; i < 3; i++)
        for (size_t j = 0; j < 3; j++)
            RGB_to_XYZ_transposed[i][j] = scale[i] * primaries_matrix[i][j];
}
