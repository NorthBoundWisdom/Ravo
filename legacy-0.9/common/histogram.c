...
static inline void _clamp_bin(const dt_aligned_pixel_t vals, uint32_t *const restrict histogram,
                              const float max_bin)
{
...
static inline void _bin_raw(const dt_dev_histogram_collection_params_t *const params,
                            const void *pixel, uint32_t *histogram, const int j,
                            const dt_iop_order_iccprofile_info_t *const profile)
{
...
static inline void _bin_rgb(const dt_dev_histogram_collection_params_t *const params,
                            const void *const restrict pixel, uint32_t *const restrict histogram,
                            const int j, const dt_iop_order_iccprofile_info_t *const profile)
{
    const dt_histogram_roi_t *roi = params->roi;
    ...
    for (int i = 0; i < roi->width - roi->crop_right - roi->crop_x; i++)
    {
        ...
        for_each_channel(k, aligned(in, b : 16)) b[k] = max_bin * in[i * 4 + k];
...
static inline void _bin_rgb_compensated(const dt_dev_histogram_collection_params_t *const params,
                                        const void *const pixel, uint32_t *const restrict histogram,
                                        const int j,
                                        const dt_iop_order_iccprofile_info_t *const profile)
{
    const dt_histogram_roi_t *roi = params->roi;
    ...
    {
        dt_aligned_pixel_t b;
        for_each_channel(k, aligned(in, b : 16)) b[k] =
            max_bin * dt_ioppr_compensate_middle_grey(in[i * 4 + k], profile);
        _clamp_bin(b, histogram, max_bin);
    }
}
                            ...
                            const int j, const dt_iop_order_iccprofile_info_t *const profile)
{
    ...
    float *in = (float *)pixel + 4 * (roi->width * j + roi->crop_x);
    const float max_bin = params->bins_count - 1;
    const dt_aligned_pixel_t scale = {max_bin / 100.0f, max_bin / 256.0f, max_bin / 256.0f, 0.0f};
    const dt_aligned_pixel_t shift = {0.0f, 128.0f, 128.0f, 0.0f};

    for (int i = 0; i < roi->width - roi->crop_right - roi->crop_x; i++)
    {
        dt_aligned_pixel_t b;
        for_each_channel(k, aligned(in, b, scale, shift : 16)) b[k] =
            scale[k] * (in[i * 4 + k] + shift[k]);
        _clamp_bin(b, histogram, max_bin);
    }
}
                                ...
                                const dt_iop_order_iccprofile_info_t *const profile)
{
    ...
    float *in = (float *)pixel + 4 * (roi->width * j + roi->crop_x);
    const float max_bin = params->bins_count - 1;
    const dt_aligned_pixel_t scale = {max_bin / 100.0f, max_bin / (128.0f * M_SQRT2_F), max_bin,
                                      0.0f};
    ...
    for (int i = 0; i < roi->width - roi->crop_right - roi->crop_x; i++)
    {
        ...
        for_each_channel(k, aligned(LCh, b, scale : 16)) b[k] = scale[k] * LCh[k];
        _clamp_bin(b, histogram, max_bin);
    }
}
...
{
    const size_t bins_total =
        (size_t)(histogram_stats->ch == 1 ? 1 : 4) * histogram_params->bins_count;
                         ...
                         const dt_iop_order_iccprofile_info_t *const profile_info)
{
    ...
    case IOP_CS_RAW:
        ...
        _hist_worker(histogram_params, histogram_stats, pixel, histogram, _bin_raw, profile_info);
        break;
    ...
    case IOP_CS_RGB:
        ...
        if (compensate_middle_grey && profile_info)
            ...
            _hist_worker(histogram_params, histogram_stats, pixel, histogram, _bin_rgb_compensated,
                         profile_info);
        else
            // used by levels, rgbcurve (uncompensated), rgblevels
            _hist_worker(histogram_params, histogram_stats, pixel, histogram, _bin_rgb,
                         profile_info);
        break;
    ...
    case IOP_CS_LAB:
        ...
        if (cst_to != IOP_CS_LCH)
            // for tonecurve
            _hist_worker(histogram_params, histogram_stats, pixel, histogram, _bin_Lab,
                         profile_info);
        else
            // for colorzones
            _hist_worker(histogram_params, histogram_stats, pixel, histogram, _bin_Lab_LCh,
                         profile_info);
        break;
    ...
    if (*histogram && histogram_max)
    {
        ...
        if (cst == IOP_CS_RGB || IOP_CS_LAB)
        {
            ...
            for (int k = 4; k < 4 * histogram_stats->bins_count; k += 4)
                for_each_channel(ch, aligned(hist : 64) aligned(m : 16)) m[ch] =
                    MAX(m[ch], hist[k + ch]);
        }
        else
    ...
    if (histogram_max)
        for_each_channel(ch, aligned(m : 16)) histogram_max[ch] = m[ch];

    dt_print(DT_DEBUG_PERF,
             "histogram calculation %u bins %d -> %d"
             " compensate %d %u channels %u pixels took %.3f secs (%.3f CPU)",
             histogram_params->bins_count, cst, cst_to, compensate_middle_grey && profile_info,
             histogram_stats->ch, histogram_stats->pixels, dt_get_lap_time(&start_time.clock),
