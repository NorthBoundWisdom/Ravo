...
static char *_preset_encode(sqlite3_stmt *stmt, const int row)
{
...
void dt_presets_save_to_file(const int rowid, const char *preset_name, const char *filedir)
{
    ...
    gchar *filename =
        g_strdup_printf("%s/%s.dtpreset", filedir, g_strdelimit(presetname, "/<>:\"\\|*?[]", '_'));
...
gboolean dt_presets_import_from_file(const char *preset_path)
{
    ...
    int op_params_len = 0;
    const unsigned char *op_params_blob =
        dt_exif_xmp_decode(op_params, strlen(op_params), &op_params_len);
...
{
    for (const GList *lib_modules = darktable.lib->plugins; lib_modules;
...
char *dt_presets_get_filter(const dt_image_t *image)
{
    ...
    return g_strdup_printf("format = 0"
                           " OR ((format&%d == %d OR format&%d == %d)"
                           "     AND format&%d != 0"
                           "     AND ~format&%d != 0)",
                           raw, raw, mat, mat, hdr, exl);
}
...
static void _menu_shell_insert_sorted(GtkWidget *menu_shell, GtkWidget *item, const gchar *name)
{
    ...
    for (GList *i = g_list_last(items); i; i = i->prev, num--)
        if (g_utf8_collate(gtk_menu_item_get_label(i->data), name) < 0)
            break;
...
{
    gchar *local_name =
        writeprotect ? dt_util_localize_segmented_name(name, FALSE) : g_strdup(name);
