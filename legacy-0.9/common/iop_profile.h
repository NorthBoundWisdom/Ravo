...
typedef struct dt_iop_order_iccprofile_info_t
{
    ...
    float grey;
    dt_colormatrix_t
        matrix_in_transposed; // same as matrix_in, but stored such as to permit vectorization
    dt_colormatrix_t
        matrix_out_transposed; // same as matrix_out, but stored such as to permit vectorization
    float primaries[3][2];
    float whitepoint[2];
...
dt_iop_order_iccprofile_info_t *
dt_ioppr_get_iop_input_profile_info(const struct dt_iop_module_t *module, GList *iop_list);
...
void dt_ioppr_transform_image_colorspace_rgb(
    const float *const image_in, float *const image_out, const int width, const int height,
...
DT_OMP_DECLARE_SIMD(aligned(lut : 64))
...
DT_OMP_DECLARE_SIMD(aligned(rgb_in, rgb_out, unbounded_coeffs : 16) aligned(lut : 64)
...
static inline void dt_ioppr_apply_trc(const dt_aligned_pixel_t rgb_in, dt_aligned_pixel_t rgb_out,
                                      float *const lut[3], const float unbounded_coeffs[3][3],
                                      const int lutsize)
{
    for (int c = 0; c < 3; c++)
    {
        rgb_out[c] = (lut[c][0] >= 0.0f) ?
                         ((rgb_in[c] < 1.0f) ? extrapolate_lut(lut[c], rgb_in[c], lutsize) :
                                               eval_exp(unbounded_coeffs[c], rgb_in[c])) :
                         rgb_in[c];
    }
}

DT_OMP_DECLARE_SIMD(aligned(rgb, matrix_in, unbounded_coeffs_in : 16) aligned(lut_in : 64)
                                                      ...
                                                      const int lutsize, const int nonlinearlut)
{
    ...
    if (nonlinearlut)
    {
        ...
        luminance = matrix_in[1][0] * linear_rgb[0] + matrix_in[1][1] * linear_rgb[1] +
...
DT_OMP_DECLARE_SIMD(aligned(unbounded_coeffs_in : 16) aligned(lut_in : 64)
...
DT_OMP_DECLARE_SIMD(aligned(unbounded_coeffs_out : 16) aligned(lut_out : 64)
                                              ...
                                              const int lutsize, const int nonlinearlut)
{
    ...
    dt_ioppr_rgb_matrix_to_xyz(rgb, xyz, matrix_in_transposed, lut_in, unbounded_coeffs_in, lutsize,
                               nonlinearlut);
...
static inline void dt_ioppr_rgb_matrix_to_dt_UCS_JCH(const float *const in, dt_aligned_pixel_t jch,
                                                     const dt_colormatrix_t matrix_in_transposed,
                                                     const float L_white)
{
...
static inline float
dt_ioppr_get_profile_info_middle_grey(const dt_iop_order_iccprofile_info_t *const profile_info)
{
...
DT_OMP_DECLARE_SIMD()
static inline float
dt_ioppr_compensate_middle_grey(const float x,
                                const dt_iop_order_iccprofile_info_t *const profile_info)
{
    ...
    dt_ioppr_rgb_matrix_to_lab(rgb, lab, profile_info->matrix_in_transposed, profile_info->lut_in,
                               profile_info->unbounded_coeffs_in, profile_info->lutsize,
...
DT_OMP_DECLARE_SIMD()
static inline float
dt_ioppr_uncompensate_middle_grey(const float x,
                                  const dt_iop_order_iccprofile_info_t *const profile_info)
{
    ...
    dt_ioppr_lab_to_rgb_matrix(lab, rgb, profile_info->matrix_out_transposed, profile_info->lut_out,
                               profile_info->unbounded_coeffs_out, profile_info->lutsize,
                               profile_info->nonlinearlut);
    return rgb[0];
}
