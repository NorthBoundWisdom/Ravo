    ...
    while (g_hash_table_iter_next(&iter, &key, &value))
    {
        ...
        if (json_parser_load_from_data(json_parser, value, -1, NULL) == FALSE)
        {
            dt_print(DT_DEBUG_PWSTORAGE,
                     "[pwstorage_apple_keychain_set] unable to parse JSON from value (%s)",
                     (gchar *)value);
        ...
        const CFStringRef server =
            CFStringCreateWithCString(NULL, attr_server, kCFStringEncodingUTF8);
        const CFStringRef username =
            CFStringCreateWithCString(NULL, attr_username, kCFStringEncodingUTF8);
        const CFStringRef password =
            CFStringCreateWithCString(NULL, attr_password, kCFStringEncodingUTF8);
        ...
        // encrypted password
        CFDataRef password_enc =
            CFStringCreateExternalRepresentation(NULL, password, kCFStringEncodingUTF8, 0);
        CFRelease(password);
                                            ...
                                            const gchar *slot)
{
    ...
    if (res == errSecSuccess)
    {
        for (int i = 0; i < CFArrayGetCount(items); i++)
        {
            ...
            {
                CFStringRef kc_password = CFStringCreateFromExternalRepresentation(
                    NULL, password_data, kCFStringEncodingUTF8);
