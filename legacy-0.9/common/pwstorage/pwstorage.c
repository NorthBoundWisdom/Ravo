...
/* add password storage capabilities */
    ...
    else if (strcmp(_backend_str, "gnome keyring") == 0)
    {
        ...
        dt_control_log(
            _("GNOME Keyring backend is no longer supported. configure a different one"));
        _backend = PW_STORAGE_BACKEND_NONE;
    }
...
#ifdef HAVE_LIBSECRET
        dt_print(DT_DEBUG_PWSTORAGE,
                 "[pwstorage_new] using libsecret backend for username/password storage");
        pwstorage->backend_context = (void *)dt_pwstorage_libsecret_new();
        if (pwstorage->backend_context == NULL)
        {
            dt_print(DT_DEBUG_PWSTORAGE,
                     "[pwstorage_new] error starting libsecret. using no storage backend");
            pwstorage->backend_context = NULL;
            pwstorage->pw_storage_backend = PW_STORAGE_BACKEND_NONE;
        }
        else
...
#ifdef HAVE_KWALLET
        dt_print(DT_DEBUG_PWSTORAGE,
                 "[pwstorage_new] using kwallet backend for username/password storage");
        pwstorage->backend_context = (void *)dt_pwstorage_kwallet_new();
        if (pwstorage->backend_context == NULL)
        {
            dt_print(DT_DEBUG_PWSTORAGE,
                     "[pwstorage_new] error starting kwallet. using no storage backend");
            pwstorage->backend_context = NULL;
            pwstorage->pw_storage_backend = PW_STORAGE_BACKEND_NONE;
        }
        else
...
#ifdef HAVE_APPLE_KEYCHAIN
        dt_print(DT_DEBUG_PWSTORAGE,
                 "[pwstorage_new] using apple keychain backend for username/password storage");
...
#ifdef HAVE_WINDOWS_CREDENTIALS
        dt_print(DT_DEBUG_PWSTORAGE,
                 "[pwstorage_new] using windows credentials backend for username/password storage");
...
gboolean dt_pwstorage_set(const gchar *slot, GHashTable *table)
{
    ...
    case PW_STORAGE_BACKEND_LIBSECRET:
#if HAVE_LIBSECRET
        return dt_pwstorage_libsecret_set(
            (backend_libsecret_context_t *)darktable.pwstorage->backend_context, slot, table);
#endif
        break;
    case PW_STORAGE_BACKEND_KWALLET:
#ifdef HAVE_KWALLET
        return dt_pwstorage_kwallet_set(
            (backend_kwallet_context_t *)darktable.pwstorage->backend_context, slot, table);
#endif
        break;
    case PW_STORAGE_BACKEND_APPLE_KEYCHAIN:
#ifdef HAVE_APPLE_KEYCHAIN
        return dt_pwstorage_apple_keychain_set(
            (backend_apple_keychain_context_t *)darktable.pwstorage->backend_context, slot, table);
#endif
        break;
    case PW_STORAGE_BACKEND_WINDOWS_CREDENTIALS:
#ifdef HAVE_WINDOWS_CREDENTIALS
        return dt_pwstorage_windows_credentials_set(
            (backend_windows_credentials_context_t *)darktable.pwstorage->backend_context, slot,
            table);
#endif
        break;
    }
...
GHashTable *dt_pwstorage_get(const gchar *slot)
{
    ...
    case PW_STORAGE_BACKEND_LIBSECRET:
#if HAVE_LIBSECRET
        return dt_pwstorage_libsecret_get(
            (backend_libsecret_context_t *)darktable.pwstorage->backend_context, slot);
#endif
        break;
    case PW_STORAGE_BACKEND_KWALLET:
#ifdef HAVE_KWALLET
        return dt_pwstorage_kwallet_get(
            (backend_kwallet_context_t *)darktable.pwstorage->backend_context, slot);
#endif
        break;
    case PW_STORAGE_BACKEND_APPLE_KEYCHAIN:
#ifdef HAVE_APPLE_KEYCHAIN
        return dt_pwstorage_apple_keychain_get(
            (backend_apple_keychain_context_t *)darktable.pwstorage->backend_context, slot);
#endif
        break;
    case PW_STORAGE_BACKEND_WINDOWS_CREDENTIALS:
#ifdef HAVE_WINDOWS_CREDENTIALS
        return dt_pwstorage_windows_credentials_get(
            (backend_windows_credentials_context_t *)darktable.pwstorage->backend_context, slot);
#endif
        break;
    }
