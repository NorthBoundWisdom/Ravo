...
typedef enum dt_collection_filter_comparator_t
{
    ...
    COLLECTION_FILTER_ATLEAST_RATING =
        1 << 1, // show all stars including and above selected star filter
    ...
    COLLECTION_FILTER_CUSTOM_COMPARE =
        1 << 6 // use the comparator defined in the comparator field to filter stars
...
typedef enum dt_collection_change_t
{
    ...
    DT_COLLECTION_CHANGE_RELOAD =
        3 // we have just reload the collection after images changes (query is identical)
...
void dt_collection_update_query(const dt_collection_t *collection,
                                ...
                                const dt_collection_properties_t changed_property, GList *list);
