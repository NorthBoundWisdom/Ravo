...
#if !defined(_WIN32)
static const int _signals_to_preserve[] = {SIGHUP,  SIGINT,  SIGQUIT, SIGILL,  SIGABRT,
                                           SIGBUS,  SIGFPE,  SIGPIPE, SIGALRM, SIGTERM,
                                           SIGCHLD, SIGXCPU, SIGXFSZ};
#else
...
static void _dt_sigsegv_handler(int param)
{
    ...
    if ((pid = fork()) != -1)
    {
        ...
        {
            if (execlp("gdb", "gdb", darktable.progname, pid_arg, "-batch", "-ex", setlogfile,
                       "-ex", logenable, "-x", comm_arg, NULL))
            {
                delete_file = TRUE;
...
void dt_set_signal_handlers()
{
    ...
    if (SIG_ERR != prev)
    {
        ...
        if (1 == _times_handlers_were_set)
            _dt_sigsegv_old_handler = prev;
    }
    else
    {
        const int errsv = errno;
        dt_print(DT_DEBUG_ALWAYS,
                 "[dt_set_signal_handlers] error: signal(SIGSEGV) returned SIG_ERR: %i (%s)", errsv,
                 strerror(errsv));
    }
