...
typedef enum dt_image_orientation_t
{
    ...
    ORIENTATION_ROTATE_180_DEG =
        ORIENTATION_FLIP_Y | ORIENTATION_FLIP_X, // 3 $DESCRIPTION: "rotate 180°"
    ORIENTATION_TRANSPOSE = ORIENTATION_SWAP_XY, // 4
    ORIENTATION_ROTATE_CCW_90_DEG =
        ORIENTATION_FLIP_X | ORIENTATION_SWAP_XY, // 6 $DESCRIPTION: "rotate 90°"
    ORIENTATION_ROTATE_CW_90_DEG =
        ORIENTATION_FLIP_Y | ORIENTATION_SWAP_XY, // 5 $DESCRIPTION: "rotate -90°"
    ORIENTATION_TRANSVERSE = ORIENTATION_FLIP_Y | ORIENTATION_FLIP_X |
                             ORIENTATION_SWAP_XY // 7 $DESCRIPTION: "transverse"
...
void dt_image_path_append_version(const dt_imgid_t imgid, char *pathname,
                                  const size_t pathname_len);
/** prints a one-line exif information string. */
void dt_image_print_exif(const dt_image_t *img, char *line, const size_t line_len);
...
/** imports a new image from raw/etc file and adds it to the database and image cache. */
...
static inline dt_image_orientation_t dt_image_transformation_to_flip_bits(const int angle,
                                                                          const int flip)
{
    if (angle == 1)
    {
...
char *dt_image_camera_missing_sample_message(const struct dt_image_t *img, const gboolean logmsg);
