...
struct dt_nlmeans_param_t
{
    ...
    float
        center_weight; // weighting of central pixel in patch (<0 for no special handling; used by denoise[non-local])
    float
        sharpness; // relative weight of central pixel (preserves detail), ignored if center_weight >= 0
