...
typedef enum dt_illuminant_t
{
    ...
    DT_ILLUMINANT_BB =
        6, // $DESCRIPTION: "Planckian (black body)" general black body radiator - not CIE standard
...
typedef enum dt_illuminant_fluo_t
{
    ...
    DT_ILLUMINANT_FLUO_F10 =
        9, // $DESCRIPTION: "F10 (Tuned RGB 5000 K) – low CRI" Philips TL85, Ultralume 50
    DT_ILLUMINANT_FLUO_F11 =
        10, // $DESCRIPTION: "F11 (Tuned RGB 4000 K) – low CRI" Philips TL84, Ultralume 40
    DT_ILLUMINANT_FLUO_F12 =
        11, // $DESCRIPTION: "F12 (Tuned RGB 3000 K) – low CRI" Philips TL83, Ultralume 30
...
typedef enum dt_illuminant_led_t
{
    ...
    DT_ILLUMINANT_LED_BH1 =
        5, // $DESCRIPTION: "BH1 (Blue-Red hybrid 2851 K)" mix of phosphor-converted blue red
    DT_ILLUMINANT_LED_RGB1 =
        6, // $DESCRIPTION: "RGB1 (RGB 2840 K)" mixing of red, green, and blue LEDs
...
static float fluorescent[DT_ILLUMINANT_FLUO_LAST][2] = {
    {0.31310f, 0.33727f},  // DT_ILLUMINANT_FLUO_F1
...
static inline float xy_to_CCT(const float x, const float y)
{
    ...
    if (x < FLT_MAX)
    {
        ...
        return (-949.86315f + 6253.80338f * expf(-n / 0.92159f) + 28.70599f * expf(-n / 0.20039f) +
...
static inline void CCT_to_xy_blackbody(const float t, float *x, float *y)
{
    ...
    if (t >= 1667.f && t <= 2222.f)
        y_temp =
            ((-1.1063814f * x_temp - 1.34811020f) * x_temp + 2.18555832f) * x_temp - 0.20219683f;
    else if (t > 2222.f && t <= 4000.f)
        y_temp =
            ((-0.9549476f * x_temp - 1.37418593f) * x_temp + 2.09137015f) * x_temp - 0.16748867f;
    else if (t > 4000.f && t <= 25000.f)
        y_temp =
            ((3.0817580f * x_temp - 5.87338670f) * x_temp + 3.75112997f) * x_temp - 0.37001483f;
...
static inline gboolean find_temperature_from_raw_coeffs(const dt_image_t *img,
                                                        const dt_aligned_pixel_t custom_wb,
                                                        float *chroma_x, float *chroma_y);

static inline int
illuminant_to_xy(const dt_illuminant_t illuminant,   // primary type of illuminant
                 ...
                 const dt_illuminant_led_t iled)  // sub-type of led illuminant, if needed
{
   ...
   */
    ...
    case DT_ILLUMINANT_F:
    {
        ...
        if (fluo >= DT_ILLUMINANT_FLUO_LAST)
            break;
    ...
    case DT_ILLUMINANT_LED:
    {
        ...
        if (iled >= DT_ILLUMINANT_LED_LAST)
            break;
    ...
    case DT_ILLUMINANT_CAMERA:
    {
        ...
        if (img)
            if (find_temperature_from_raw_coeffs(img, custom_wb, &x, &y))
                break;
    }
...
static inline void WB_coeffs_to_illuminant_xy(const float CAM_to_XYZ[4][3],
                                              const dt_aligned_pixel_t WB, float *x, float *y)
{
...
static gboolean find_temperature_from_raw_coeffs(const dt_image_t *img,
                                                 const dt_aligned_pixel_t custom_wb,
                                                 float *chroma_x, float *chroma_y)
{
    if (img == NULL)
    ...
    dt_aligned_pixel_t WB = {img->wb_coeffs[0], img->wb_coeffs[1], img->wb_coeffs[2],
                             img->wb_coeffs[3]};
    ...
    if (!dt_is_valid_colormatrix(CAM_to_XYZ[0][0]))
        return FALSE;
...
DT_OMP_DECLARE_SIMD()
static inline void blackbody_xy_to_tinted_xy(const float x, const float y, const float t,
                                             const float tint, float *x_out, float *y_out)
{
