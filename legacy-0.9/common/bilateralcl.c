...
size_t dt_bilateral_memory_use2(const int width, const int height, const float sigma_s,
                                const float sigma_r)
{
...
// modules that want to use dt_bilateral_slice_to_output_cl() ought to take this one;
// takes account of an additional temp buffer needed in the OpenCL code path
size_t dt_bilateral_singlebuffer_size2(const int width, const int height, const float sigma_s,
                                       const float sigma_r)
{
...
cl_int dt_bilateral_splat_cl(dt_bilateral_cl_t *b, cl_mem in)
{
    ...
    return dt_opencl_enqueue_kernel_2d_local_args(
        b->devid, b->global->kernel_splat, sizes, local, CLARG(in), CLARG(b->dev_grid),
        CLARG(b->width), CLARG(b->height), CLARG(b->size_x), CLARG(b->size_y), CLARG(b->size_z),
        CLARG(b->sigma_s), CLARG(b->sigma_r), CLLOCAL(b->blocksizex * b->blocksizey * sizeof(int)),
        CLLOCAL(b->blocksizex * b->blocksizey * 8 * sizeof(float)));
}

cl_int dt_bilateral_blur_cl(dt_bilateral_cl_t *b)
{
    cl_int err =
        dt_opencl_enqueue_copy_buffer_to_buffer(b->devid, b->dev_grid, b->dev_grid_tmp, 0, 0,
    ...
    int stride3 = 1;
    err = dt_opencl_enqueue_kernel_2d_args(b->devid, b->global->kernel_blur_line, b->size_z,
                                           b->size_y, CLARG(b->dev_grid_tmp), CLARG(b->dev_grid),
                                           CLARG(stride1), CLARG(stride2), CLARG(stride3),
    ...
    stride3 = b->size_x;
    err = dt_opencl_enqueue_kernel_2d_args(b->devid, b->global->kernel_blur_line, b->size_z,
                                           b->size_x, CLARG(b->dev_grid), CLARG(b->dev_grid_tmp),
                                           CLARG(stride1), CLARG(stride2), CLARG(stride3),
    ...
    return dt_opencl_enqueue_kernel_2d_args(b->devid, b->global->kernel_blur_line_z, b->size_x,
                                            b->size_y, CLARG(b->dev_grid_tmp), CLARG(b->dev_grid),
                                            CLARG(stride1), CLARG(stride2), CLARG(stride3),
                                       ...
                                       const float detail)
{
    ...
    err = dt_opencl_enqueue_kernel_2d_args(
        b->devid, b->global->kernel_slice2, b->width, b->height, CLARG(in), CLARG(tmp), CLARG(out),
        CLARG(b->dev_grid), CLARG(b->width), CLARG(b->height), CLARG(b->size_x), CLARG(b->size_y),
        CLARG(b->size_z), CLARG(b->sigma_s), CLARG(b->sigma_r), CLARG(detail));
...
cl_int dt_bilateral_slice_cl(dt_bilateral_cl_t *b, cl_mem in, cl_mem out, const float detail)
{
    return dt_opencl_enqueue_kernel_2d_args(
        b->devid, b->global->kernel_slice, b->width, b->height, CLARG(in), CLARG(out),
        CLARG(b->dev_grid), CLARG(b->width), CLARG(b->height), CLARG(b->size_x), CLARG(b->size_y),
        CLARG(b->size_z), CLARG(b->sigma_s), CLARG(b->sigma_r), CLARG(detail));
}
