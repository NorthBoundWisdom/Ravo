...
static void _pop_undo_execute(const dt_imgid_t imgid, const gboolean before, const gboolean after);

static int32_t _image_duplicate_with_version(const dt_imgid_t imgid, const int32_t newversion,
                                             const gboolean undo);
...
static void _image_local_copy_full_path(const dt_imgid_t imgid, char *pathname,
...
{
    return dt_image_is_raw(img) && img->buf_dsc.filters != 9u && !dt_image_is_monochrome(img) &&
           !(img->flags & DT_IMAGE_4BAYER);
}

static void _image_set_monochrome_flag(const dt_imgid_t imgid, const gboolean monochrome,
...
void dt_image_film_roll_directory(const dt_image_t *img, char *pathname, const size_t pathname_len)
{
    sqlite3_stmt *stmt;
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "SELECT folder FROM main.film_rolls WHERE id = ?1", -1, &stmt,
                                NULL);
...
void dt_image_film_roll(const dt_image_t *img, char *pathname, const size_t pathname_len)
{
    sqlite3_stmt *stmt;
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "SELECT folder FROM main.film_rolls WHERE id = ?1", -1, &stmt,
...
void dt_image_full_path(const dt_imgid_t imgid, char *pathname, const size_t pathname_len,
                        gboolean *from_cache)
{
    sqlite3_stmt *stmt;
    // clang-format off
                                                ...
                                                const dt_colorspaces_color_profile_type_t icc_type)
{
    ...
    if (icc_type == DT_COLORSPACE_NONE)
    {
        if (!dt_is_valid_imgid(imgid))
            return FALSE;
        const dt_colorspaces_color_profile_t *work = dt_colorspaces_get_work_profile(imgid);
...
void dt_image_path_append_version_no_db(const int version, char *pathname,
                                        const size_t pathname_len)
{
...
void dt_image_path_append_version(const dt_imgid_t imgid, char *pathname, const size_t pathname_len)
{
    // get duplicate suffix
    int version = 0;
    sqlite3_stmt *stmt;
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "SELECT version FROM main.images WHERE id = ?1", -1, &stmt, NULL);
    DT_DEBUG_SQLITE3_BIND_INT(stmt, 1, imgid);
...
void dt_image_print_exif(const dt_image_t *img, char *line, size_t line_len)
{
    ...
    snprintf(line, line_len, "%s f/%.1f %dmm ISO %d", exposure_str, img->exif_aperture,
...
{
    return (flags & DT_IMAGE_REJECTED) ? -1 // rejected image = -1
                                         :
                      ...
                      const dt_undo_action_t action, GList **imgs)
{
    if (type == DT_UNDO_DATETIME)
    {
        ...
        for (GList *list = (GList *)data; list; list = g_list_next(list))
        {
            ...
            _set_datetime(undodatetime->imgid,
                          (action == DT_ACTION_UNDO) ? undodatetime->before : undodatetime->after);
...
void dt_image_update_final_size(const dt_imgid_t imgid)
{
    ...
    if (darktable.develop && darktable.develop->full.pipe &&
        darktable.develop->full.pipe->output_imgid == imgid)
    {
        dt_dev_pixelpipe_get_dimensions(darktable.develop->full.pipe, darktable.develop,
                                        darktable.develop->full.pipe->iwidth,
                                        darktable.develop->full.pipe->iheight, &ww, &hh);
        ...
        if (imgtmp)
        {
            ...
            if (changed)
            {
                dt_print(DT_DEBUG_PIPE, "updated final size for ID=%i, updated to %ix%i", imgid, ww,
                         hh);
...
gboolean dt_image_get_final_size(const dt_imgid_t imgid, int *width, int *height)
{
    ...
    if (available)
    {
        ...
        *height = timg->final_height;
        dt_print(DT_DEBUG_PIPE, "[dt_image_get_final_size] for ID=%i from cache %ix%i", imgid,
                 *width, *height);
    }
...
void dt_image_flip(const dt_imgid_t imgid, const int32_t cw)
{
    ...
    if (darktable.develop->image_storage.id == imgid && dt_view_get_current() == DT_VIEW_DARKROOM)
        return;
...
float dt_usable_aspect(const float img_aspect)
{
  ...
  */
    if (_range(aspect, 1, 1))
        aspect = 1.0f;
    else if (_range(aspect, 14, 11))
        aspect = 14.0f / 11.0f;
    else if (_range(aspect, 45, 35))
        aspect = 45.0f / 35.0f;
    else if (_range(aspect, 110, 85))
        aspect = 110.0f / 85.0f; // letter
    else if (_range(aspect, 4, 3))
        aspect = 4.0f / 3.0f;
    else if (_range(aspect, 7, 5))
        aspect = 1.4f;
    else if (_range(aspect, 14142136, 10000000))
        aspect = 1.4142136f; // A4
    else if (_range(aspect, 3, 2))
        aspect = 1.5f;
    else if (_range(aspect, 8, 5))
        aspect = 1.6f;
    else if (_range(aspect, 1618034, 1000000))
        aspect = 1.618034f; // golden cut
    else if (_range(aspect, 16, 9))
        aspect = 16.0f / 9.0f;
    else if (_range(aspect, 185, 100))
        aspect = 1.85f; // widescreen
    else if (_range(aspect, 2, 1))
        aspect = 2.0f;
    else if (_range(aspect, 235, 100))
        aspect = 2.35f; // cinemascope
    else if (_range(aspect, 239, 100))
        aspect = 2.39f; // anamorphic
    else if (_range(aspect, 65, 24))
        aspect = 65.0f / 24.0f; // xpan
    else if (_range(aspect, 3, 1))
        aspect = 3.0f; // pano
    else
    {
...
void dt_image_set_aspect_ratio_to(const dt_imgid_t imgid, const float aspect_ratio,
                                  const gboolean raise)
{
    if (aspect_ratio > .0f)
    {
        ...
        dt_image_cache_write_release_info(image, DT_IMAGE_CACHE_RELAXED,
                                          "dt_image_set_aspect_ratio_to");
...
void dt_image_set_aspect_ratio_if_different(const dt_imgid_t imgid, const float aspect_ratio,
                                            const gboolean raise)
{
    if (aspect_ratio > .0f)
    {
        ...
        if (new_aspect)
        {
            ...
            dt_image_cache_write_release_info(wimage, DT_IMAGE_CACHE_RELAXED,
                                              "dt_image_set_aspect_ratio_if_different");
...
gboolean dt_image_set_history_end(const dt_imgid_t imgid, const int history_end)
{
    sqlite3_stmt *stmt = NULL;
    // update history end
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "UPDATE main.images SET history_end = ?1 WHERE id = ?2", -1, &stmt,
                                NULL);
...
static int32_t _image_get_possible_version(const dt_imgid_t imgid, const int32_t max_version)
{
    ...
    for (int32_t version = MAX(1, max_version); version < 1000; version++)
    {
        ...
        if (!(g_file_test(versionpath, G_FILE_TEST_EXISTS) &&
              g_file_test(versionpath, G_FILE_TEST_IS_REGULAR)))
        {
    ...
    if (safe_max_version == 0)
    {
        dt_print(DT_DEBUG_ALWAYS,
                 "image_get_possible_version couldn't find a safe new version above %d",
                 max_version);
                                                    ...
                                                    const int32_t newversion)
{
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
        ...
        DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                    "UPDATE main.images SET version=?1 WHERE id = ?2", -1, &stmt,
                                    NULL);
...
static dt_imgid_t _image_duplicate_with_version(const dt_imgid_t imgid, const int32_t newversion,
                                                const gboolean undo)
{
...
dt_imgid_t dt_image_duplicate_with_version(const dt_imgid_t imgid, const int32_t newversion)
{
...
void dt_image_remove(const dt_imgid_t imgid)
{
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "DELETE FROM main.images WHERE id = ?1", -1, &stmt, NULL);
    DT_DEBUG_SQLITE3_BIND_INT(stmt, 1, imgid);
    sqlite3_step(stmt);
    sqlite3_finalize(stmt);
                                  ...
                                  const gboolean clear_selection)
{
    ...
    for (GList *file_iter = files; file_iter; file_iter = g_list_next(file_iter))
    {
        ...
        if (count_xmps_processed == 0)
        {
            ...
            DT_DEBUG_SQLITE3_PREPARE_V2(
                dt_database_get(darktable.db),
                "UPDATE main.images SET version=?1, max_version = ?1 WHERE id = ?2", -1, &stmt,
                NULL);
            DT_DEBUG_SQLITE3_BIND_INT(stmt, 1, version);
                                         ...
                                         const gboolean raise_signals)
{
    ...
    if (override_ignore_nonraws == FALSE && ext && !dt_imageio_is_raw_by_extension(ext) &&
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2
    ...
    if (rc != SQLITE_DONE)
        dt_print(DT_DEBUG_ALWAYS, "[image_import_internal] sqlite3 error %d in `%s`", rc, filename);
    sqlite3_finalize(stmt);
    ...
    *cc2 = '\0';
        ...
        if (sqlite3_step(stmt2) == SQLITE_ROW)
        {
            ...
            if (!(dt_imageio_is_raw_by_extension(other_ext) || !strcmp(other_ext, "dng")))
            {
                ...
                while (sqlite3_step(stmt3) == SQLITE_ROW)
                {
                    ...
                    dt_image_cache_write_release_info(group_img, DT_IMAGE_CACHE_SAFE,
                                                      "_image_import_internal");
                }
                group_id = id;
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "UPDATE main.images SET group_id = ?1 WHERE id = ?2", -1, &stmt,
                                NULL);
...
void dt_image_init(dt_image_t *img)
{
    ...
    img->exif_flash_tagvalue =
        -1; // -1 means we have no data (this is not the same as "flash not fired")
    img->exif_crop = 1.0;
    img->exif_exposure = 0;
...
void dt_image_refresh_makermodel(dt_image_t *img)
{
    if (!img->camera_maker[0] || !img->camera_model[0] || !img->camera_alias[0])
    {
        ...
        dt_imageio_lookup_makermodel(img->exif_maker, img->exif_model, img->camera_maker,
                                     sizeof(img->camera_maker), img->camera_model,
                                     sizeof(img->camera_model), img->camera_alias,
...
gboolean _move_extra_file(const gchar *oldFilePath, const gchar *newFolder,
                          const gchar *newBasename)
{
...
gboolean dt_image_rename(const dt_imgid_t imgid, const int32_t filmid, const gchar *newname)
{
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "SELECT folder FROM main.film_rolls WHERE id = ?1", -1, &film_stmt,
                                NULL);
    ...
    if (new)
    {
      ...
      DT_DEBUG_SQLITE3_PREPARE_V2
         ...
         -1, &duplicates_stmt, NULL);
            ...
            while (dup_list)
            {
                ...
                if (img)
                {
                    img->film_id = filmid;
                    if (newname)
                        g_strlcpy(img->filename, newname, DT_MAX_FILENAME_LEN);
                }
                // write through to db, but not to xmp
                dt_image_cache_write_release(img, DT_IMAGE_CACHE_RELAXED);
            ...
            if (g_file_test(copysrcpath, G_FILE_TEST_EXISTS))
            {
                ...
                {
                    dt_print(DT_DEBUG_ALWAYS,
                             "[dt_image_rename] error moving local copy `%s' -> `%s'", copysrcpath,
                             copydestpath);
                    ...
                    else
                    {
                        gchar *oldBasename = g_path_get_basename(copysrcpath);
                        gchar *newBasename = g_path_get_basename(copydestpath);
                        dt_control_log(_("error moving local copy `%s' -> `%s'"), oldBasename,
                                       newBasename);
                        g_free(oldBasename);
                        g_free(newBasename);
                    }
            ...
            if (oldTxtFilePath != NULL || oldAudioFilePath != NULL)
            {
                ...
                gchar *newBasename =
                    g_strndup(newImgBasename, strrchr(newImgBasename, '.') - newImgBasename);
        ...
        else
        {
            ...
            else if (newname && (g_error_matches(moveError, G_IO_ERROR, G_IO_ERROR_EXISTS) ||
                                 g_error_matches(moveError, G_IO_ERROR, G_IO_ERROR_IS_DIRECTORY)))
            {
...
dt_imgid_t dt_image_copy_rename(const dt_imgid_t imgid, const dt_filmid_t filmid,
                                const gchar *newname)
{
    dt_imgid_t newid = NO_IMGID;
    sqlite3_stmt *stmt;
    ...
    if (dest)
    {
     ...
     "        flags, width, height, crop, raw_black, raw_maximum,"
      ...
      DT_DEBUG_SQLITE3_PREPARE_V2
        ...
        DT_DEBUG_SQLITE3_PREPARE_V2
          (dt_database_get(darktable.db),
           ...
           -1, &stmt, NULL);
                ...
                DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                            "UPDATE main.images SET version=?1 WHERE id = ?2", -1,
                                            &stmt, NULL);
                DT_DEBUG_SQLITE3_BIND_INT(stmt, 1, version);
                DT_DEBUG_SQLITE3_BIND_INT(stmt, 2, newid);
                sqlite3_step(stmt);
        ...
        {
            dt_print(DT_DEBUG_ALWAYS, "[dt_image_copy_rename] Failed to copy image %s: %s", srcpath,
                     gerror->message);
        }
        g_object_unref(dest);
        g_object_unref(src);
...
static gboolean _any_altered_data(const dt_imgid_t imgid)
{
    ...
    // been added.
    return dt_image_altered(imgid) || (dt_tag_count_attached(imgid, TRUE) > 0);
}
...
void dt_image_local_copy_synch(void)
{
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "SELECT id FROM main.images WHERE flags&?1=?1", -1, &stmt, NULL);
...
static void _image_set_datetimes(const GList *img, const GArray *dtime, GList **undo,
                                 const gboolean undo_on)
{
    int i = 0;
...
void dt_image_set_datetimes(const GList *imgs, const GArray *dtime, const gboolean undo_on)
{
    ...
    if (undo_on)
    {
        dt_undo_record(darktable.undo, NULL, DT_UNDO_DATETIME, undo, _pop_undo,
                       _datetime_undo_data_free);
        dt_undo_end_group(darktable.undo);
    }
}

static void _image_set_datetime(const GList *img, const char *datetime, GList **undo,
                                const gboolean undo_on)
{
    for (GList *imgs = (GList *)img; imgs; imgs = g_list_next(imgs))
    {
...
float dt_image_get_exposure_bias(const dt_image_t *image_storage)
{
    ...
    if ((image_storage) && (image_storage->exif_exposure_bias))
    {
        ...
        if (image_storage->exif_exposure_bias == DT_EXIF_TAG_UNINITIALIZED ||
            ...
            CLAMP(image_storage->exif_exposure_bias, -5.0f, 5.0f) !=
                image_storage->exif_exposure_bias)
            return 0.0f;
        else
...
char *dt_image_camera_missing_sample_message(const dt_image_t *img, const gboolean logmsg)
{
    const char *T1 = _("<b>WARNING</b>: camera is missing samples!");
    const char *T2 =
        _("You must provide samples in <a href='https://raw.pixls.us/'>https://raw.pixls.us/</a>");
...
int32_t dt_image_get_camera_id(const char *maker, const char *model)
{
    ...
    dt_imageio_lookup_makermodel(maker, model, n_maker, sizeof(n_maker), n_model, sizeof(n_model),
