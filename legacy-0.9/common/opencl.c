...
static gboolean _opencl_load_program(const int dev, const int prog, const char *programname,
                                     const char *filename, const char *binname,
                                     const char *cachedir, char *md5sum, char **includemd5,
                                     gboolean *loaded_cached);

static gboolean _opencl_build_program(const int dev, const int prog, const char *binname,
                                      const char *cachedir, char *md5sum,
...
int dt_opencl_get_device_info(dt_opencl_t *cl, cl_device_id device, cl_device_info param_name,
                              void **param_value, size_t *param_value_size)
{
    *param_value_size = SIZE_MAX;
    ...
    cl_int err =
        (cl->dlocl->symbols->dt_clGetDeviceInfo)(device, param_name, 0, NULL, param_value_size);
    if (err != CL_SUCCESS)
    {
    ...
    err = (cl->dlocl->symbols->dt_clGetDeviceInfo)(device, param_name, *param_value_size,
                                                   *param_value, NULL);
    if (err != CL_SUCCESS)
    {
...
// use threadsafe locale as we want to read/write floats safely
    ...
    g_snprintf(
        dat, 510, "%i %i %i %i %i %.3f %.3f", cl->dev[devid].micro_nap,
        cl->dev[devid].pinned_memory,

        // this used to define the number of slots, now a bool and using DT_OPENCL_EVENTS if true
        cl->dev[devid].use_events ? 1 : 0, cl->dev[devid].asyncmode, cl->dev[devid].disabled,
        cl->dev[devid].advantage, cl->dev[devid].unified_fraction);
...
// use threadsafe locale as we want to read/write floats safely
    ...
    if (existing_device)
    {
        ...
        sscanf(dat, "%i %i %i %i %i %f %f", &micro_nap, &pinned_memory, &events, &asyncmode,
               &disabled, &advantage, &unified_fraction);
...
static gboolean _opencl_device_init(dt_opencl_t *cl, const int dev, cl_device_id *devices,
                                    const int k)
{
    gboolean res = FALSE;
    cl_int err;
    ...
    (cl->dlocl->symbols->dt_clGetDeviceInfo)(devid, CL_DEVICE_AVAILABLE, sizeof(cl_bool),
    ...
    (cl->dlocl->symbols->dt_clGetDeviceInfo)(devid, CL_DEVICE_VENDOR_ID, sizeof(cl_uint),
                                             &vendor_id, NULL);

    err = dt_opencl_get_device_info(cl, devid, CL_DEVICE_NAME, (void **)&device_name,
                                    &device_name_size);
    if (err != CL_SUCCESS)
    {
    ...
    {
        err = (cl->dlocl->symbols->dt_clGetPlatformInfo)(platform_id, CL_PLATFORM_NAME,
                                                         DT_OPENCL_CBUFFSIZE, platform_name, NULL);
        if (err != CL_SUCCESS)
        {
            has_platform_name = FALSE;
    ...
    dt_print_nts(DT_DEBUG_OPENCL, "\n   DEVICE:                   %d: '%s'%s\n", k, device_name,
                 (newdevice) ? ", NEW" : "");
    dt_print_nts(DT_DEBUG_OPENCL, "   CONF KEY:                 %s%s\n", DT_CLDEVICE_HEAD,
                 cl->dev[dev].cname);
    dt_print_nts(DT_DEBUG_OPENCL, "   PLATFORM, VENDOR & ID:    %s, %s%s, ID=%d\n",
                 cl->dev[dev].platform, is_mesa ? "Mesa:" : "", platform_vendor, vendor_id);
    dt_print_nts(DT_DEBUG_OPENCL, "   CANONICAL NAME:           %s\n", cl->dev[dev].cname);

    err = dt_opencl_get_device_info(cl, devid, CL_DRIVER_VERSION, (void **)&driverversion,
                                    &driverversion_size);
    if (err != CL_SUCCESS)
    {
    ...
    err = dt_opencl_get_device_info(cl, devid, CL_DEVICE_VERSION, (void **)&deviceversion,
                                    &deviceversion_size);
    if (err != CL_SUCCESS)
    {
    ...
    err = dt_opencl_get_device_info(cl, devid, CL_DEVICE_EXTENSIONS, (void **)&deviceextensions,
                                    &deviceextensions_size);
    ...
    (cl->dlocl->symbols->dt_clGetDeviceInfo)(devid, CL_DEVICE_TYPE, sizeof(cl_device_type), &type,
                                             NULL);
    (cl->dlocl->symbols->dt_clGetDeviceInfo)(devid, CL_DEVICE_IMAGE_SUPPORT, sizeof(cl_bool),
                                             &image_support, NULL);
    ...
    (cl->dlocl->symbols->dt_clGetDeviceInfo)(devid, CL_DEVICE_ENDIAN_LITTLE, sizeof(cl_bool),
                                             &little_endian, NULL);
    (cl->dlocl->symbols->dt_clGetDeviceInfo)(devid, CL_DEVICE_MAX_CONSTANT_BUFFER_SIZE,
                                             sizeof(cl_ulong), &(cl->dev[dev].max_mem_constant),
                                             NULL);
    (cl->dlocl->symbols->dt_clGetDeviceInfo)(devid, CL_DEVICE_MEM_BASE_ADDR_ALIGN, sizeof(cl_uint),
                                             &(cl->dev[dev].alignsize), NULL);
    (cl->dlocl->symbols->dt_clGetDeviceInfo)(devid, CL_DEVICE_MAX_COMPUTE_UNITS, sizeof(cl_uint),
                                             &(cl->dev[dev].compute_units), NULL);
    (cl->dlocl->symbols->dt_clGetDeviceInfo)(devid, CL_DEVICE_MAX_WORK_GROUP_SIZE, sizeof(size_t),
                                             &(cl->dev[dev].workgroup_size), NULL);
    (cl->dlocl->symbols->dt_clGetDeviceInfo)(devid, CL_DEVICE_LOCAL_MEM_SIZE, sizeof(size_t),
                                             &(cl->dev[dev].local_size), NULL);

#if CL_TARGET_OPENCL_VERSION >= 300
    (cl->dlocl->symbols->dt_clGetDeviceInfo)(devid, CL_DEVICE_PREFERRED_WORK_GROUP_SIZE_MULTIPLE,
                                             sizeof(size_t), &(cl->dev[dev].workgroup_size_rec),
                                             NULL);
#endif
    ...
    dt_print_nts(DT_DEBUG_OPENCL, "   DEVICE VERSION:           %s API=%s\n",
                 cl->dev[dev].device_version, cl->api30 ? "300" : "120");
    ...
    {
        dt_print_nts(DT_DEBUG_OPENCL, "   *** discarding new custom device ***\n");
        cl->dev[dev].disabled |= TRUE;
        res = TRUE;
        goto end;
    }
    ...
    {
        dt_print_nts(DT_DEBUG_OPENCL, "   *** no proper device version ***\n");
        res = TRUE;
        goto end;
    }
    else
    {
        ...
        {
            dt_print_nts(DT_DEBUG_OPENCL, "   *** insufficient device version ***\n");
            res = TRUE;
            goto end;
        }
    }
    ...
    if (!image_support)
    {
        dt_print_nts(DT_DEBUG_OPENCL, "   *** The OpenCL driver doesn't provide image support."
    ...
    dt_print_nts(DT_DEBUG_OPENCL, "   MAX IMAGE ALLOC:          %.0f MB\n",
    ...
    dt_print_nts(DT_DEBUG_OPENCL, "   MAX CONSTANT BUFFER:      %.0f KB\n",
                 (double)cl->dev[dev].max_mem_constant / 1024.0);
    dt_print_nts(DT_DEBUG_OPENCL, "   LOCAL MEM SIZE:           %zu KB\n",
                 cl->dev[dev].local_size / 1024lu);
    dt_print_nts(DT_DEBUG_OPENCL, "   ADDRESS ALIGN:            %d B\n",
                 cl->dev[dev].alignsize / 8);
    dt_print_nts(DT_DEBUG_OPENCL, "   COMPUTE UNITS:            %d\n", cl->dev[dev].compute_units);
    dt_print_nts(DT_DEBUG_OPENCL, "   MAX WORK GROUP SIZE:      %zu (%zu)\n",
    ...
    dt_print_nts(DT_DEBUG_OPENCL, "   MAX WORK ITEM DIMENSIONS: %d [ ", max_item_dimension);
    ...
    dt_print_nts(DT_DEBUG_OPENCL, "   PINNED MEMORY TILING:     %s\n",
                 STR_YESNO(cl->dev[dev].pinned_memory));
    dt_print_nts(DT_DEBUG_OPENCL, "   SUPPORTED ATOMICS:        %s%s%s\n",
  ...
  */
    ...
    dt_print_nts(DT_DEBUG_OPENCL | DT_DEBUG_VERBOSE, "   ROUNDUP WIDTH & HEIGHT    %ix%i\n",
                 cl->dev[dev].clroundup_wd, cl->dev[dev].clroundup_ht);
    dt_print_nts(DT_DEBUG_OPENCL, "   EVENTS HANDLED:           %s\n",
                 STR_YESNO(cl->dev[dev].use_events));
    dt_print_nts(DT_DEBUG_OPENCL, "   OPENCL FAST MODE:         %s\n", STR_YESNO(fastopencl));
    ...
    cl->dev[dev].cmd_queue = (cl->dlocl->symbols->dt_clCreateCommandQueue)(
        ...
        (darktable.unmuted & DT_DEBUG_PERF) ? CL_QUEUE_PROFILING_ENABLE : 0, &err);
    if (err != CL_SUCCESS)
    {
    ...
    for (int i = 0; i < len; i++)
        if (isalnum(driverversion[i]))
            drvversion[j++] = driverversion[i];
    drvversion[j] = 0;
    snprintf(cachedir, PATH_MAX * sizeof(char),
             "%s" G_DIR_SEPARATOR_S "cached_v%d_kernels_for_%s_%s", dtcache, DT_OPENCL_KERNELS,
    ...
    const char *compile_opt =
        fastopencl ? DT_OPENCL_DEFAULT_COMPILE_OPTI : DT_OPENCL_DEFAULT_COMPILE_DEFAULT;
    cl->dev[dev].cflags =
        g_strdup_printf("-w %s%s -D%s=1", compile_opt,
                        cl->dev[dev].cuda && cl->dev[dev].atomic_support ? " -DNVIDIA_SM_20=1" : "",
                        _opencl_get_vendor_by_id(vendor_id));
    cl->dev[dev].options = g_strdup_printf("%s -I%s", cl->dev[dev].cflags, escapedkerneldir);

    dt_print_nts(DT_DEBUG_OPENCL, "   CL COMPILER COMMAND:      %s\n", cl->dev[dev].options);
    g_free(escapedkerneldir);
    escapedkerneldir = NULL;

    const char *clincludes[DT_OPENCL_MAX_INCLUDES] = {
        "rgb_norms.h",  "noise_generator.h", "color_conversion.h", "colorspaces.cl",
        "colorspace.h", "common.h",          "guided_filter.cl",   NULL};
    ...
    if (f)
    {
        while (!feof(f))
        {
            ...
            snprintf(filename, PATH_MAX * sizeof(char), "%s" G_DIR_SEPARATOR_S "%s", kerneldir,
                     programname);
            snprintf(binname, PATH_MAX * sizeof(char), "%s" G_DIR_SEPARATOR_S "%s.bin", cachedir,
                     programname);
            dt_print_nts(DT_DEBUG_OPENCL | DT_DEBUG_VERBOSE,
                         "[dt_opencl_device_init] testing program `%s'\n", programname);
            gboolean loaded_cached;
            char md5sum[33];
            if (_opencl_load_program(dev, prog, programname, filename, binname, cachedir, md5sum,
                                     includemd5, &loaded_cached) &&
        ...
        fclose(f);
        dt_print_nts(DT_DEBUG_OPENCL, "   KERNEL LOADING TIME:      %.4lf sec\n",
...
void dt_opencl_init(dt_opencl_t *cl, const gboolean exclude_opencl, const gboolean print_statistics)
{
    ...
    if (!opencl_requested)
        dt_print_nts(DT_DEBUG_OPENCL, "[opencl_init] opencl disabled via darktable preferences\n");
    ...
    else
    {
        dt_print_nts(DT_DEBUG_OPENCL,
                     ...
                     " preference '%s'\n",
                     cl->dlocl->library, (strlen(library) == 0) ? "default path" : library);
    }
    ...
    {
        dt_print_nts(DT_DEBUG_OPENCL, "[opencl_init] %i platforms detected, error: %s\n",
                     num_platforms, cl_errstr(err));
        goto finally;
    }

    num_platforms = 0;
    err = (cl->dlocl->symbols->dt_clGetPlatformIDs)(DT_OPENCL_MAX_PLATFORMS, all_platforms,
                                                    &num_platforms);
    if (err != CL_SUCCESS)
    {
        dt_print_nts(DT_DEBUG_OPENCL, "[opencl_init] could not get platforms IDs: %s\n",
                     cl_errstr(err));
        goto finally;
    }
    if (num_platforms == 0)
    ...
    if (num_platforms)
    {
        ...
        for (int n = 0; n < num_platforms && platforms; n++)
        {
            ...
            if ((cl->dlocl->symbols->dt_clGetPlatformInfo)(
                    platform, CL_PLATFORM_NAME, DT_OPENCL_CBUFFSIZE,
                    platforms + n * DT_OPENCL_CBUFFSIZE, NULL) != CL_SUCCESS)
                break;

            for (int k = 0; k < n; k++)
            {
                if (!strcmp(platforms + n * DT_OPENCL_CBUFFSIZE,
    ...
    for (int n = 0; n < num_platforms; n++)
    {
        ...
        if (errn == CL_SUCCESS)
        {
            ...
            for (int i = 0; i < len; i++)
                if (isalnum(platform_name[i]))
                    platform_key[j++] = tolower(platform_name[i]);
            platform_key[j] = 0;
      ...
      */
        ...
        else
        {
            char profile[64] = {0};
            size_t profile_size;
            err = (cl->dlocl->symbols->dt_clGetPlatformInfo)(platform, CL_PLATFORM_PROFILE, 64,
    ...
    for (int n = 0; n < num_platforms; n++)
    {
        ...
        {
            cl_platform_id platform = all_platforms[n];
            err = (cl->dlocl->symbols->dt_clGetDeviceIDs)(platform, CL_DEVICE_TYPE_ALL,
                                                          all_num_devices[n], devs, NULL);
            if (err != CL_SUCCESS)
            {
                num_devices -= all_num_devices[n];
    ...
    if (dev > 0)
    {
        ...
        assert(cl->dev_priority_image != NULL && cl->dev_priority_preview != NULL &&
               cl->dev_priority_preview2 != NULL && cl->dev_priority_export != NULL &&
               cl->dev_priority_thumbnail != NULL);
    ...
    dt_print_nts(DT_DEBUG_OPENCL,
                 ...
                 STR_YESNO(opencl_requested), cl->inited ? "" : "NOT ", cl->enabled ? "" : "NOT ");
    ...
    if (cl->inited)
    {
        ...
        const gboolean newcheck =
            ((strcmp(oldchecksum, checksum) != 0) || (strlen(oldchecksum) < 1));
        ...
        for (int i = 0; i < cl->num_devs; i++)
        {
            if (cl->dev[i].unified_memory)
            {
                const size_t reserved =
                    MIN(cl->dev[i].max_global_mem, res->total_memory * cl->dev[i].unified_fraction);
        ...
        for (int i = 0; i < cl->num_devs; i++)
        {
            ...
            if (cl->print_statistics && (darktable.unmuted & DT_DEBUG_MEMORY))
            {
                dt_print_nts(DT_DEBUG_OPENCL,
                             ...
                             cl->dev[i].fullname, i, (float)cl->dev[i].peak_memory / (1024 * 1024),
...
{
    dt_opencl_t *cl = darktable.opencl;
    if (!cl->inited || devid <= DT_DEVICE_CPU)
        return FALSE;
...
static int _take_from_list(int *list, const int value)
{
    int result = -1;

    while (*list != -1 && *list != value)
        list++;
    result = *list;
...
static void _opencl_priority_parse(dt_opencl_t *cl, char *configstr, int *priority_list,
                                   gboolean *mandatory)
{
    ...
    while (tokens != NULL && *tokens_ptr != NULL && count < devs + 1 && full[0] != -1)
    {
        ...
        else if (*str != '\0')
        {
            ...
            if (number < 0 || (number == 0 && endptr == str))
                number = -1;
...
static void _opencl_priorities_parse(dt_opencl_t *cl, const char *configstr)
{
    char tmp[2048];
    int len = 0;
    ...
    {
        const int n = strcspn(
            configstr, "/!,*+0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ");
        configstr += n;
        if (n != 0)
            continue;
        tmp[len] = *configstr;
...
static void _opencl_update_priorities(const char *configstr)
{
    ...
    dt_print_nts(DT_DEBUG_OPENCL, "[opencl_update_priorities] these are your device priorities:\n");
    dt_print_nts(DT_DEBUG_OPENCL, "[opencl_update_priorities]"
                                  " \t\timage\tpreview\texport\tthumbs\tpreview2\n");
    for (int i = 0; i < cl->num_devs; i++)
        dt_print_nts(DT_DEBUG_OPENCL, "[opencl_update_priorities]\t\t%d\t%d\t%d\t%d\t%d\n",
                     cl->dev_priority_image[i], cl->dev_priority_preview[i],
                     cl->dev_priority_export[i], cl->dev_priority_thumbnail[i],
                     cl->dev_priority_preview2[i]);
    dt_print_nts(DT_DEBUG_OPENCL, "[opencl_update_priorities] show if opencl use is"
                                  " mandatory for a given pixelpipe:\n");
    dt_print_nts(DT_DEBUG_OPENCL, "[opencl_update_priorities]"
                                  " \t\timage\tpreview\texport\tthumbs\tpreview2\n");
    dt_print_nts(DT_DEBUG_OPENCL, "[opencl_update_priorities]\t\t%s\t%s\t%s\t%s\t%s\n",
...
int dt_opencl_lock_device(const int pipetype)
{
    ...
    if (priority)
    {
        ...
        dt_print(DT_DEBUG_OPENCL, "[opencl_lock_device] reached opencl_mandatory_timeout trying"
    ...
    else
    {
        ...
        for (int try_dev = 0; try_dev < cl->num_devs; try_dev++)
        {
            ...
            if (!dt_pthread_mutex_BAD_trylock(&cl->dev[try_dev].lock))
                return try_dev;
        }
    }
...
static void _opencl_md5sum(const char **files, char **md5sums)
{
    ...
    for (int n = 0; n < DT_OPENCL_MAX_INCLUDES; n++, files++, md5sums++)
    {
        ...
        if (!f)
        {
            dt_print_nts(DT_DEBUG_OPENCL, "[opencl_md5sums] could not open file `%s'\n", filename);
            *md5sums = NULL;
            continue;
        }
        ...
        {
            free(file);
            dt_print_nts(DT_DEBUG_OPENCL, "[opencl_md5sums] could not read all of file `%s'\n",
                         filename);
            *md5sums = NULL;
            continue;
        }
                                     ...
                                     gboolean *loaded_cached)
{
    ...
    *loaded_cached = FALSE;
    ...
    (cl->dlocl->symbols->dt_clGetDeviceInfo)(devid, CL_DEVICE_PLATFORM, sizeof(cl_platform_id),
                                             &platform, NULL);

    (cl->dlocl->symbols->dt_clGetPlatformInfo)(platform, CL_PLATFORM_VERSION, end - start, start,
                                               &len);
    start += len;
    ...
    if (*loaded_cached == FALSE)
    {
        ...
        if (linkedfile_len > 0)
        {
            ...
            snprintf(link_dest, sizeof(link_dest), "%s" G_DIR_SEPARATOR_S "%s", cachedir,
                     linkedfile);
            g_unlink(link_dest);
        }
        g_unlink(binname);
        ...
        dt_control_log(_("building OpenCL program %s for %s"), programname, cl->dev[dev].fullname);
                                      ...
                                      const gboolean loaded_cached)
{
  ...
  */
    ...
    cl_int err = (cl->dlocl->symbols->dt_clBuildProgram)(program, 1, &(cl->dev[dev].devid),
                                                         cl->dev[dev].options, NULL, NULL);
    ...
    if (err != CL_SUCCESS)
        dt_print_nts(DT_DEBUG_OPENCL, "[opencl_build_program] could not build program: %s\n",
                     cl_errstr(err));
    else
        dt_print_nts(DT_DEBUG_OPENCL | DT_DEBUG_VERBOSE,
    ...
    (cl->dlocl->symbols->dt_clGetProgramBuildInfo)(program, cl->dev[dev].devid,
                                                   ...
                                                   &build_status, NULL);
    dt_print_nts(DT_DEBUG_OPENCL | DT_DEBUG_VERBOSE, "[opencl_build_program] BUILD STATUS: %d\n",
                 build_status);
    ...
    if (ret_val_size > 2 && ret_val_size < 0x100000) // we don't want to see nothing or crazy things
    {
        ...
        if (build_log)
        {
            (cl->dlocl->symbols->dt_clGetProgramBuildInfo)(
                program, cl->dev[dev].devid, CL_PROGRAM_BUILD_LOG, ret_val_size, build_log, NULL);
    ...
    if (!loaded_cached)
    {
        dt_print_nts(DT_DEBUG_OPENCL | DT_DEBUG_VERBOSE, "[opencl_build_program] saving binary\n");

        cl_uint numdev = 0;
        err = (cl->dlocl->symbols->dt_clGetProgramInfo)(program, CL_PROGRAM_NUM_DEVICES,
                                                        sizeof(cl_uint), &numdev, NULL);
        if (err != CL_SUCCESS)
        {
            dt_print_nts(DT_DEBUG_OPENCL,
        ...
        err = (cl->dlocl->symbols->dt_clGetProgramInfo)(
            program, CL_PROGRAM_DEVICES, sizeof(cl_device_id) * numdev, devices, NULL);
        if (err != CL_SUCCESS)
        {
            dt_print_nts(DT_DEBUG_OPENCL, "[opencl_build_program] CL_PROGRAM_DEVICES failed: %s\n",
                         cl_errstr(err));
            free(devices);
            return TRUE;
        }
        ...
        err = (cl->dlocl->symbols->dt_clGetProgramInfo)(
            program, CL_PROGRAM_BINARY_SIZES, sizeof(size_t) * numdev, binary_sizes, NULL);
        if (err != CL_SUCCESS)
        {
            dt_print_nts(DT_DEBUG_OPENCL,
        ...
        err = (cl->dlocl->symbols->dt_clGetProgramInfo)(
            program, CL_PROGRAM_BINARIES, sizeof(unsigned char *) * numdev, binaries, NULL);
        if (err != CL_SUCCESS)
        {
            dt_print_nts(DT_DEBUG_OPENCL, "[opencl_build_program] CL_PROGRAM_BINARIES failed: %s\n",
                         cl_errstr(err));
            goto ret;
        }
        ...
        for (int i = 0; i < numdev; i++)
        {
            if (cl->dev[dev].devid == devices[i])
            {
                ...
                char filename[PATH_MAX] = {0};
#if defined(_WIN32)
                snprintf(filename, sizeof(filename), "%s" G_DIR_SEPARATOR_S "%s.%s", cachedir,
                         bname, md5sum);
#else
                snprintf(filename, sizeof(filename), "%s" G_DIR_SEPARATOR_S "%s", cachedir, md5sum);
#endif //defined(_WIN32)
                FILE *f = g_fopen(filename, "wb");
                if (!f)
                    goto ret;
                size_t bytes_written = fwrite(binaries[i], sizeof(char), binary_sizes[i], f);
                if (bytes_written != binary_sizes[i])
                    goto ret;
                fclose(f);
...
static gboolean _check_kernel(const int dev, const int kernel)
{
    ...
    if (!cl->inited || dev <= DT_DEVICE_CPU)
        return FALSE;
    ...
    if (!cl->dev[dev].kernel_used[kernel] && cl->name_saved[kernel])
    {
...
int dt_opencl_get_max_work_item_sizes(const int dev, size_t *sizes)
{
    ...
    if (!cl->inited || dev <= DT_DEVICE_CPU)
        return CL_DEVICE_NOT_AVAILABLE;
    return (cl->dlocl->symbols->dt_clGetDeviceInfo)(
        cl->dev[dev].devid, CL_DEVICE_MAX_WORK_ITEM_SIZES, sizeof(size_t) * 3, sizes, NULL);
}
                                    ...
                                    unsigned long *localmemsize)
{
    ...
    if (cl->dev[dev].local_size == 0)
        return CL_INVALID_WORK_DIMENSION;
    *localmemsize = cl->dev[dev].local_size;

    if (cl->dev[dev].workgroup_size == 0)
        return CL_INVALID_WORK_DIMENSION;
...
int dt_opencl_get_kernel_work_group_size(const int dev, const int kernel,
                                         size_t *kernelworkgroupsize)
{
    if (!_check_kernel(dev, kernel))
        return CL_DEVICE_NOT_AVAILABLE;

    dt_opencl_t *cl = darktable.opencl;
    return (cl->dlocl->symbols->dt_clGetKernelWorkGroupInfo)(
        cl->dev[dev].kernel[kernel], cl->dev[dev].devid, CL_KERNEL_WORK_GROUP_SIZE, sizeof(size_t),
        kernelworkgroupsize, NULL);
}

cl_int _opencl_set_kernel_arg(const int dev, const int kernel, const int num, const size_t size,
                              const void *arg)
{
    if (!_check_kernel(dev, kernel))
        return CL_INVALID_KERNEL;
    ...
    dt_opencl_t *cl = darktable.opencl;
    const cl_int err =
        (cl->dlocl->symbols->dt_clSetKernelArg)(cl->dev[dev].kernel[kernel], num, size, arg);

    if (err != CL_SUCCESS)
        dt_print(DT_DEBUG_OPENCL, "[opencl_set_kernel_arg] error kernel `%s' (%i) on device %d: %s",
...
static int _opencl_set_kernel_args(const int dev, const int kernel, int num, va_list ap)
{
    ...
    do
    {
        ...
        {
            dt_print(DT_DEBUG_OPENCL, "opencl parameters passed to opencl_set_kernel_args or "
                                             ...
                                             const size_t *local, const int dimensions)
{
    ...
    cl_int err = (cl->dlocl->symbols->dt_clEnqueueNDRangeKernel)(
        cl->dev[dev].cmd_queue, cl->dev[dev].kernel[kernel], dimensions,
    ...
    if (err != CL_SUCCESS)
        dt_print(DT_DEBUG_OPENCL,
                 ...
                 dimensions, local ? "_with_local" : "", cl->name_saved[kernel], kernel,
                 cl->dev[dev].fullname, dev, cl_errstr(err));
...
int dt_opencl_enqueue_kernel_2d_args_internal(const int dev, const int kernel, const size_t w,
                                              const size_t h, ...)
{
    va_list ap;
    va_start(ap, h);
...
int dt_opencl_enqueue_kernel_2d_local_args_internal(const int dev, const int kernel,
                                                    const size_t *sizes, const size_t *local, ...)
{
    va_list ap;
    va_start(ap, local);
    const cl_int err = _opencl_set_kernel_args(dev, kernel, 0, ap);
...
int dt_opencl_enqueue_kernel_1d_args_internal(const int dev, const int kernel, const size_t x, ...)
{
    va_list ap;
    va_start(ap, x);
    const cl_int err = _opencl_set_kernel_args(dev, kernel, 0, ap);
...
int dt_opencl_copy_image_to_host(const int devid, void *host, cl_mem image, const int width,
                                 const int height, const int bpp)
{
    ...
    // blocking.
    return dt_opencl_read_host_from_image_raw(devid, host, image, CLIMG_ORIGIN, region,
                                              (size_t)width * bpp, TRUE);
}

int dt_opencl_read_host_from_image_raw(const int devid, void *host, cl_mem image,
                                       const size_t *origin, const size_t *region,
                                       const int rowpitch, const gboolean blocking)
{
    if (!_cldev_running(devid))
        return DT_OPENCL_NODEVICE;
    ...
    const cl_int err = (darktable.opencl->dlocl->symbols->dt_clEnqueueReadImage)(
        darktable.opencl->dev[devid].cmd_queue, image, blocking ? CL_TRUE : CL_FALSE, org, reg,
...
int dt_opencl_write_host_to_image(const int devid, const void *host, cl_mem image, const int width,
                                  const int height, const int bpp)
{
    if (!_cldev_running(devid))
        return DT_OPENCL_NODEVICE;
    ...
    const size_t region[2] = {width, height};
    // blocking.
    return dt_opencl_write_host_to_image_raw(devid, host, image, CLIMG_ORIGIN, region,
                                             (size_t)width * bpp, TRUE);
}

int dt_opencl_write_host_to_image_raw(const int devid, const void *host, cl_mem image,
                                      const size_t *origin, const size_t *region,
                                      const int rowpitch, const gboolean blocking)
{
    if (!_cldev_running(devid))
...
int dt_opencl_enqueue_copy_image(const int devid, const cl_mem src, cl_mem dst,
                                 const size_t *orig_src, const size_t *orig_dst,
                                 const size_t *region)
{
    if (!_cldev_running(devid))
        return DT_OPENCL_NODEVICE;
    ...
    const cl_int err = (darktable.opencl->dlocl->symbols->dt_clEnqueueCopyImage)(
        darktable.opencl->dev[devid].cmd_queue, src, dst, osrc, odst, reg, 0, NULL, eventp);
...
int dt_opencl_enqueue_copy_image_to_buffer(const int devid, const cl_mem src_image,
                                           cl_mem dst_buffer, const size_t *origin,
                                           const size_t *region, const size_t offset)
{
    if (!_cldev_running(devid))
        return DT_OPENCL_NODEVICE;
    ...
    const cl_int err = (darktable.opencl->dlocl->symbols->dt_clEnqueueCopyImageToBuffer)(
        darktable.opencl->dev[devid].cmd_queue, src_image, dst_buffer, org, reg, offset, 0, NULL,
        eventp);
...
int dt_opencl_enqueue_copy_buffer_to_image(const int devid, const cl_mem src_buffer,
                                           cl_mem dst_image, const size_t offset,
                                           const size_t *origin, const size_t *region)
{
    if (!_cldev_running(devid))
        return DT_OPENCL_NODEVICE;
    ...
    const cl_int err = (darktable.opencl->dlocl->symbols->dt_clEnqueueCopyBufferToImage)(
        darktable.opencl->dev[devid].cmd_queue, src_buffer, dst_image, offset, org, reg, 0, NULL,
        eventp);
...
int dt_opencl_enqueue_copy_buffer_to_buffer(const int devid, const cl_mem src_buffer,
                                            cl_mem dst_buffer, const size_t srcoffset,
                                            const size_t dstoffset, const size_t size)
{
    if (!_cldev_running(devid))
        return DT_OPENCL_NODEVICE;
    ...
    const cl_int err = (darktable.opencl->dlocl->symbols->dt_clEnqueueCopyBuffer)(
        darktable.opencl->dev[devid].cmd_queue, src_buffer, dst_buffer, srcoffset, dstoffset, size,
        0, NULL, eventp);
    if (err != CL_SUCCESS)
        dt_print(
            DT_DEBUG_OPENCL,
...
int dt_opencl_read_buffer_from_device(const int devid, void *host, cl_mem buffer,
                                      ...
                                      const gboolean blocking)
{
    ...
    cl_event *eventp = _opencl_events_get_slot(devid, "[Read Buffer (from device to host)]");

    const cl_int err = (darktable.opencl->dlocl->symbols->dt_clEnqueueReadBuffer)(
        darktable.opencl->dev[devid].cmd_queue, buffer, blocking ? CL_TRUE : CL_FALSE, offset, size,
        host, 0, NULL, eventp);
    if (err != CL_SUCCESS)
        dt_print(
            DT_DEBUG_OPENCL,
...
int dt_opencl_write_buffer_to_device(const int devid, void *host, cl_mem buffer,
                                     const size_t offset, const size_t size,
                                     const gboolean blocking)
{
    if (!_cldev_running(devid))
...
static void *_opencl_copy_host_to_image_rowpitch(const int devid, void *host, const int width,
                                                 const int height, const int bpp,
                                                 const int rowpitch)
{
    if (!_cldev_running(devid))
    ...
    cl_mem dev = (darktable.opencl->dlocl->symbols->dt_clCreateImage)(
        darktable.opencl->dev[devid].context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, &fmt, &desc,
        host, &err);
    if (err != CL_SUCCESS)
        dt_print(DT_DEBUG_OPENCL,
...
void *dt_opencl_copy_host_to_image(const int devid, void *host, const int width, const int height,
                                   const int bpp)
{
...
void *dt_opencl_map_buffer(const int devid, cl_mem buffer, const gboolean blocking, const int flags,
                           const size_t offset, const size_t size)
{
    if (!_cldev_running(devid))
        return NULL;
    ...
    ptr = (darktable.opencl->dlocl->symbols->dt_clEnqueueMapBuffer)(
        darktable.opencl->dev[devid].cmd_queue, buffer, blocking ? CL_TRUE : CL_FALSE, flags,
        offset, size, 0, NULL, eventp, &err);
...
void *dt_opencl_alloc_device(const int devid, const int width, const int height, const int bpp)
{
    ...
    cl_mem dev = (cl->dlocl->symbols->dt_clCreateImage)(cl->dev[devid].context, CL_MEM_READ_WRITE,
                                                        &fmt, &desc, NULL, &err);
...
void *dt_opencl_alloc_device_buffer(const int devid, const size_t size)
{
    ...
    cl_mem buf = (cl->dlocl->symbols->dt_clCreateBuffer)(cl->dev[devid].context, CL_MEM_READ_WRITE,
                                                         size, NULL, &err);
...
void *dt_opencl_alloc_device_buffer_with_flags(const int devid, const size_t size, const int flags)
{
    ...
    cl_mem buf =
        (cl->dlocl->symbols->dt_clCreateBuffer)(cl->dev[devid].context, flags, size, NULL, &err);
    if (err != CL_SUCCESS || buf == NULL)
        dt_print(
            DT_DEBUG_OPENCL,
...
void *dt_opencl_duplicate_image(const int devid, const cl_mem src)
{
    ...
    const size_t region[2] = {width, height};
    const cl_int err =
        dt_opencl_enqueue_copy_image(devid, src, new, CLIMG_ORIGIN, CLIMG_ORIGIN, region);
    if (err != CL_SUCCESS)
    {
...
void dt_opencl_memory_statistics(int devid, const cl_mem mem, const dt_opencl_memory_t action)
{
    ...
    if (darktable.unmuted & DT_DEBUG_MEMORY)
    {
        dt_print(
            DT_DEBUG_OPENCL,
            "[opencl memory] device '%s' id=%d: %.1fMB in use, %.1fMB available GPU mem of %.1fMB",
            cl->dev[devid].fullname, devid, (float)cl->dev[devid].memory_in_use / (1024 * 1024),
...
void dt_opencl_check_tuning(const int devid)
{
    ...
    const gboolean tunehead = cl->num_devs > 1 && level >= 0 && !dt_gimpmode() &&
    ...
    else
    {
        ...
        if (cl->dev[devid].tunehead)
        {
            const size_t headroom =
                (cl->dev[devid].headroom ? DT_MEGA * cl->dev[devid].headroom : dhead) +
...
{
    if (!darktable.opencl->inited || devid <= DT_DEVICE_CPU)
        return 0;
...
gboolean dt_opencl_image_fits_device(const int devid, const size_t width, const size_t height,
                                     const uint32_t bpp, const float factor, const size_t overhead)
{
    dt_opencl_t *cl = darktable.opencl;
    if (!_cldev_running(devid))
        return FALSE;
...
int dt_opencl_dev_roundup_width(const int size, const int devid)
{
    if (_cldev_running(devid))
    {
...
int dt_opencl_dev_roundup_height(const int size, const int devid)
{
    if (_cldev_running(devid))
    {
...
static void _opencl_apply_scheduling_profile(dt_opencl_scheduling_profile_t profile)
{
    ...
    default:
        ...
        _opencl_set_synchronization_timeout(dt_conf_get_int("pixelpipe_synchronization_timeout"));
        break;
    }
...
static cl_event *_opencl_events_get_slot(const int devid, const char *tag)
{
    ...
    if (*eventlist == NULL)
    {
        ...
        *eventtags = calloc(newevents, sizeof(dt_opencl_eventtag_t));
        if (!*eventlist || !*eventtags)
        {
            ...
            *eventlist = NULL;
            *eventtags = NULL;
            dt_print(DT_DEBUG_OPENCL, "[opencl_events_get_slot] NO eventlist for device %i", devid);
            return NULL;
        }
        *maxevents = newevents;
    }
    ...
    if ((*numevents - *eventsconsolidated + 1 > DT_OPENCL_EVENTS) || (*numevents == *maxevents))
...
static void _opencl_events_wait_for(const int devid)
{
    ...
    if (*numevents == *eventsconsolidated)
        return; // nothing to do
    ...
    if ((err != CL_SUCCESS) && (err != CL_INVALID_VALUE))
        dt_print(DT_DEBUG_OPENCL | DT_DEBUG_VERBOSE,
                 "[dt_opencl_events_wait_for] reported %s for device %i", cl_errstr(err), devid);
}
...
static void _opencl_events_profiling(const int devid, const gboolean aggregated)
{
    ...
    dt_print(DT_DEBUG_OPENCL, "[opencl_profiling] profiling device %d ('%s'):", devid,
             cl->dev[devid].fullname);

    float total = 0.0f;
    for (int i = 1; i < items; i++)
    {
        dt_print(DT_DEBUG_OPENCL, "[opencl_profiling] spent %7.4f seconds in %s",
                 (double)fmaxf(0.0f, timings[i]), tags[i][0] == '\0' ? "<?>" : tags[i]);
        total += timings[i];
    }
...
cl_int dt_opencl_events_flush(const int devid, const gboolean reset)
{
    ...
    for (int k = *eventsconsolidated; k < *numevents; k++)
    {
        ...
        err = (cl->dlocl->symbols->dt_clGetEventInfo)(
            (*eventlist)[k], CL_EVENT_COMMAND_EXECUTION_STATUS, sizeof(cl_int), retval, NULL);
        if (err != CL_SUCCESS)
        {
        ...
        else if (*retval != CL_COMPLETE)
        {
            dt_print(DT_DEBUG_OPENCL, "[opencl_events_flush] execution of '%s' %s: %d",
                     tag[0] == '\0' ? "<?>" : tag,
                     *retval == CL_COMPLETE ? "was successful" : "failed", *retval);
            *summary = *retval;
        }
        else
            (*totalsuccess)++;

        if ((darktable.unmuted & DT_DEBUG_PERF) && err == CL_SUCCESS && *retval == CL_COMPLETE)
        {
            ...
            cl_int erre = (cl->dlocl->symbols->dt_clGetEventProfilingInfo)(
                (*eventlist)[k], CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
            ...
            else
            {
                ...
                dt_print(DT_DEBUG_OPENCL,
                         ...
                         ", errs=%s, erre=%s)",
                         tag[0] == '\0' ? "<?>" : tag, devid, (guint64)start, (guint64)end,
    ...
    if (reset)
    {
        ...
        if (darktable.unmuted & DT_DEBUG_PERF)
            _opencl_events_profiling(devid, 1);
...
cl_int dt_opencl_local_buffer_opt(const int devid, const int kernel,
                                  dt_opencl_local_buffer_t *factors)
{
    ...
    *blocksizey = CLAMP(_nextpow2(*blocksizey), 1, 1 << 16);
    ...
    if (dt_opencl_get_work_group_limits(devid, maxsizes, &workgroupsize, &localmemsize) ==
            CL_SUCCESS &&
