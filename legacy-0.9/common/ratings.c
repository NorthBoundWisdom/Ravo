...
static void _pop_undo(gpointer user_data, const dt_undo_type_t type, const dt_undo_data_t data,
    ...
    if (type == DT_UNDO_RATINGS)
    {
        for (GList *list = (GList *)data; list; list = g_list_next(list))
        {
            ...
            _ratings_apply_to_image(ratings->imgid,
                                    (action == DT_ACTION_UNDO) ? ratings->before : ratings->after);
...
static void _ratings_apply(const GList *imgs, const int rating, GList **undo,
...
void dt_ratings_apply_on_list(const GList *img, const int rating, const gboolean undo_on)
{
    if (!g_list_is_empty(img))
    {
        ...
        if (undo_on)
        {
            dt_undo_record(darktable.undo, NULL, DT_UNDO_RATINGS, undo, _pop_undo,
...
static float _action_process_rating(gpointer target, dt_action_element_t element,
...
const gchar *dt_action_effect_rating[] = {N_("select"), N_("upgrade"), N_("downgrade"), NULL};

const dt_action_element_def_t _action_elements_rating[] = {
    {N_("zero"), dt_action_effect_rating},     {N_("one"), dt_action_effect_rating},
    {N_("two"), dt_action_effect_rating},      {N_("three"), dt_action_effect_rating},
    {N_("four"), dt_action_effect_rating},     {N_("five"), dt_action_effect_rating},
    {N_("reject"), dt_action_effect_activate}, {NULL}};

const dt_action_def_t dt_action_def_rating = {N_("rating"), _action_process_rating,
