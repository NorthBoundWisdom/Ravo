...
static void _load_add(float *const __restrict__ out, float *const __restrict__ accum,
...
template <size_t N, bool compensated = false>
static void _sub(float *const __restrict__ accum, const float *const __restrict__ values,
                 float *const __restrict__ comp = nullptr)
{
    if (compensated)
    {
...
template <size_t N>
static void _store(float *const __restrict__ out, const float *const __restrict__ in)
{
...
template <size_t N>
static void _store_scaled(float *const __restrict__ out, const float *const __restrict__ in,
                          const float scale)
{
    DT_OMP_SIMD(aligned(in : 64))
    for (size_t c = 0; c < N; c++)
...
template <size_t N>
static inline void _update_max(float m[N], const float *const __restrict__ base)
{
...
template <size_t N>
static inline void _load_update_max(float *const __restrict__ out, float m[N],
...
template <size_t N>
static inline void _load_update_min(float *const __restrict__ out, float m[N],
                                    const float *const __restrict__ base)
{
    DT_OMP_SIMD(aligned(m : 64))
    for (size_t c = 0; c < N; c++)
...
template <size_t N, bool compensated = false>
static void _blur_horizontal(float *const __restrict__ buf, const size_t width, const size_t radius,
...
// invoked inside an OpenMP parallel for, so no need to parallelize
template <size_t N, bool compensated = false>
static void _blur_vertical(float *const __restrict__ buf, const size_t height, const size_t width,
                           const size_t radius, float *const __restrict__ scratch)
{
    ...
    size_t mask = 1;
    for (size_t r = (2 * radius + 1); r > 1; r >>= 1)
        mask = (mask << 1) | 1;
...
static void _blur_vertical_1ch(float *const __restrict__ buf, const size_t height,
                               const size_t width, const size_t radius,
                               float *const __restrict__ scanlines, const size_t padded_size)
{
    DT_OMP_FOR()
...
static float *_alloc_scratch_space(const size_t N, const size_t height, const size_t width,
                                   const size_t radius, size_t *padded_size)
{
...
template <size_t N, bool compensated = false>
static void _box_mean(float *const buf, const size_t height, const size_t width,
                      const size_t radius, const uint32_t iterations)
{
    ...
    float *const __restrict__ scanlines =
        _alloc_scratch_space(N, height, width, radius, &padded_size);
    if (scanlines == NULL)
        return;
...
static inline void box_max_1d(const int N, const float *const __restrict__ x,
                              float *const __restrict__ y, const int w)
{
...
template <size_t N>
static inline void _box_max_vert(const unsigned height, float *const __restrict__ scratch,
                                 float *const __restrict__ buf, const size_t stride,
                                 const unsigned w, const size_t mask)
{
...
static void _box_max_1ch(float *const buf, const size_t height, const size_t width,
                         const unsigned w)
{
...
template <size_t N>
static inline void _box_min_vert(const unsigned height, float *const __restrict__ scratch,
                                 float *const __restrict__ buf, const int stride, const unsigned w,
                                 const size_t mask)
{
    float DT_ALIGNED_ARRAY m[N];
    for (size_t i = 0; i < N; i++)
...
// does the calculation in-place if input and output images are identical
static void _box_min_1ch(float *const buf, const size_t height, const size_t width,
                         const unsigned w)
{
    const size_t eff_height = _compute_effective_height(height, w);
    const size_t scratch_size = MAX(width, MAX(height, MAX_VECT * eff_height));
...
void dt_box_mean_horizontal(float *const __restrict__ buf, const size_t width, const uint32_t ch,
                            const size_t radius, float *const __restrict__ user_scratch)
{
...
void dt_box_mean_vertical(float *const buf, const size_t height, const size_t width,
                          const uint32_t ch, const size_t radius)
{
    if ((ch & BOXFILTER_KAHAN_SUM) && (ch & ~BOXFILTER_KAHAN_SUM) <= 16)
    {
        size_t channels = ch & ~BOXFILTER_KAHAN_SUM;
        size_t padded_size;
        float *const __restrict__ scratch_buf =
            _alloc_scratch_space(channels, height, width, radius, &padded_size);
        if (scratch_buf == NULL)
            return;
...
void dt_box_min(float *const buf, const size_t height, const size_t width, const uint32_t ch,
                const size_t radius)
{
    if (ch == 1)
...
void dt_box_max(float *const buf, const size_t height, const size_t width, const uint32_t ch,
                const size_t radius)
{
    if (ch == 1)
