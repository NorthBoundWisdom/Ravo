...
#define DT_PRAGMA_UNROLL _Pragma("unroll")
#else
#define DT_PRAGMA_UNROLL
#endif
#if defined(__APPLE__) && defined(__GNUC__) && !defined(__clang__)
/* Homebrew GCC on Apple Silicon cannot emit the mixed-width clones requested
 * by OpenMP declare simd. Loop-level OpenMP SIMD remains available. */
...
#define DT_TRY_GUI_UPDATE(ret)                                                                     \
    if (!dt_atomic_incr_int_if_zero(&darktable.gui->reset))                                        \
    return ret
...
#define DT_CLEAR_GUI_UPDATE() dt_atomic_set_int(&darktable.gui->reset, 0)

#define DT_ENTER_GUI_UPDATE() dt_atomic_incr_int(&darktable.gui->reset)

#define DT_LEAVE_GUI_UPDATE() dt_atomic_decr_int(&darktable.gui->reset)
...
#define dt_debug_if(thread, func, ...)                                                             \
    do                                                                                             \
    {                                                                                              \
        if (((~DT_DEBUG_RESTRICT & (thread)) == DT_DEBUG_ALWAYS ||                                 \
...
#define dt_print_pipe(thread, title, pipe, module, device, roi_in, roi_out, ...)                   \
    dt_debug_if(thread, dt_print_pipe_ext, title, pipe, module, device, roi_in, roi_out,           \
                " " __VA_ARGS__)

void dt_print_ext(const char *msg, ...) __attribute__((format(printf, 1, 2)));

/* same as above but without time stamp : nts = no time stamp */
void dt_print_nts_ext(const char *msg, ...) __attribute__((format(printf, 1, 2)));
...
{
    return (int *)__builtin_assume_aligned(dt_calloc_aligned(n_ints * sizeof(int)),
                                           DT_CACHELINE_BYTES);
}
...
{
    return (int *)__builtin_assume_aligned(dt_alloc_aligned(n_ints * sizeof(int)),
                                           DT_CACHELINE_BYTES);
}
static inline uint8_t *dt_alloc_align_uint8(const size_t n_ints)
{
    return (uint8_t *)__builtin_assume_aligned(dt_alloc_aligned(n_ints * sizeof(uint8_t)),
                                               DT_CACHELINE_BYTES);
}
...
static inline float *dt_calloc_align_float(const size_t nfloats)
{
    ...
    if (buf)
        memset(buf, 0, nfloats * sizeof(float));
...
static inline void dt_lock_image_pair(const dt_imgid_t imgid1, const dt_imgid_t imgid2)
...
static inline void dt_unlock_image_pair(const dt_imgid_t imgid1, const dt_imgid_t imgid2)
...
dt_imgid_t dt_load_from_string(const gchar *image_to_load, const gboolean open_image_in_dr,
...
{
    return darktable.gimp.mode ? !darktable.gimp.error && strcmp(darktable.gimp.mode, mode) == 0 :
                                 FALSE;
}
