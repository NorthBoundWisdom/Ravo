...
void dt_color_harmony_set(const dt_imgid_t imgid, const dt_color_harmony_guide_t layout);

// Returns FALSE if no harmony is recorded for imgid.
gboolean dt_color_harmony_get(const dt_imgid_t imgid, dt_color_harmony_guide_t *layout);
                                                      ...
                                                      const int rotation, float *angles, int *n)
{
    ...
    if (type <= DT_COLOR_HARMONY_NONE || type >= DT_COLOR_HARMONY_N)
    {
        *n = 0;
        return;
    }
