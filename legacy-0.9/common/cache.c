...
void dt_cache_init(dt_cache_t *cache, const size_t entry_size, const size_t cost_quota)
{
    cache->cost = 0;
    cache->lru = 0;
...
dt_cache_entry_t *dt_cache_testget(dt_cache_t *cache, const uint32_t key, const char mode)
{
    gpointer orig_key, value;
    const double start = dt_get_debug_wtime();
    dt_pthread_mutex_lock(&cache->lock);
    const gboolean res =
        g_hash_table_lookup_extended(cache->hashtable, GINT_TO_POINTER(key), &orig_key, &value);
    if (res)
    {
        dt_cache_entry_t *entry = (dt_cache_entry_t *)value;
        // lock the cache entry
        const int result = (mode == 'w') ? dt_pthread_rwlock_trywrlock(&entry->lock) :
...
dt_cache_entry_t *dt_cache_get_with_caller(dt_cache_t *cache, const uint32_t key, const char mode,
...
void dt_cache_release_with_caller(dt_cache_t *cache, dt_cache_entry_t *entry, const char *file,
                                  const int line)
{
