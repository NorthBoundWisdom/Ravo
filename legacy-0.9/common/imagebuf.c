                           ...
                           const dt_iop_roi_t *const __restrict__ roi_out)
{
    ...
    if ((roi_in->width - dx >= roi_out->width) && (roi_in->height - dy >= roi_out->height))
    {
    ...
    for (int row = 0; row < roi_out->height; row++)
    {
        for (int col = 0; col < roi_out->width; col++)
        {
            ...
            const gboolean avail =
                irow >= 0 && irow < roi_in->height && icol >= 0 && icol < roi_in->width;
...
void dt_iop_image_scaled_copy(float *const restrict buf, const float *const restrict src,
...
void dt_iop_image_add_image(float *const buf, const float *const other_image, const size_t width,
                            const size_t height, const size_t ch)
{
    const size_t nfloats = width * height * ch;
#ifdef _OPENMP
...
void dt_iop_image_sub_image(float *const buf, const float *const other_image, const size_t width,
                            const size_t height, const size_t ch)
{
    const size_t nfloats = width * height * ch;
#ifdef _OPENMP
...
void dt_iop_image_linear_blend(float *const restrict buf, const float lambda,
                               const float *const restrict other, const size_t width,
                               const size_t height, const size_t ch)
{
    const size_t nfloats = width * height * ch;
...
#ifdef _OPENMP
    if (nfloats >
        parallel_imgop_minimum / 2) // is the task big enough to outweigh threading overhead?
    {
        // we can gain a little by using a small number of threads in
        // parallel, but not much since the memory bus quickly saturates
        // (basically, each core can saturate a memory channel, so a
