...
static inline float ll_expand_gaussian(const float *const coarse, const int i, const int j,
                                       const int wd, const int ht)
{
    ...
    case 0: // both are even, 3x3 stencil
        return 4. / 256. *
               (6.0f * (coarse[ind - cw] + coarse[ind - 1] + 6.0f * coarse[ind] + coarse[ind + 1] +
                        coarse[ind + cw]) +
                coarse[ind - cw - 1] + coarse[ind - cw + 1] + coarse[ind + cw - 1] +
                coarse[ind + cw + 1]);
    case 1: // i is odd, 2x3 stencil
        return 4. / 256. *
               (24.0 * (coarse[ind] + coarse[ind + 1]) +
                4.0 * (coarse[ind - cw] + coarse[ind - cw + 1] + coarse[ind + cw] +
                       coarse[ind + cw + 1]));
    case 2: // j is odd, 3x2 stencil
        return 4. / 256. *
               (24.0 * (coarse[ind] + coarse[ind + cw]) +
                4.0 * (coarse[ind - 1] + coarse[ind + 1] + coarse[ind + cw - 1] +
                       coarse[ind + cw + 1]));
...
static inline void ll_fill_boundary2(float *const input, const int wd, const int ht)
{
    ...
    if (wd & 1)
        ...
        for (int j = 1; j < ht - 1; j++)
            input[j * wd + wd - 1] = input[j * wd + wd - 2] = input[j * wd + wd - 3];
    memcpy(input, input + wd, sizeof(float) * wd);
    if (!(ht & 1))
...
static void pad_by_replication(float *buf,       // the buffer to be padded
                                ...
                                const size_t ht)
{
    ...
    for (size_t j = 1; j < ch - 1; j++)
    {
        ...
        for (size_t col = 0; col < cw - 3; col += 2)
        {
            ...
            for_four_channels(c) conv[c] = left[c] * kernel[c];
            out[col] = (conv[0] + conv[1] + conv[2] + conv[3] + right[0]) / 256.0f;
            out[col + 1] =
                (left[2] + 4 * (left[3] + right[1]) + 6.0f * right[0] + right[2]) / 256.0f;
        ...
        if (cw % 2)
        {
            ...
            float right =
                base[0] + 4.0f * (base[wd] + base[3 * wd]) + 6.0f * base[2 * wd] + base[4 * wd];
            dt_aligned_pixel_t conv;
            for_four_channels(c) conv[c] = left[c] * kernel[c];
...
static inline float *ll_pad_input(const float *const input, const int wd, const int ht,
                                  const int max_supp, int *wd2, int *ht2,
                                  local_laplacian_boundary_t *b)
{
    ...
    *ht2 = 2 * max_supp + ht;
    ...
    { // pad by preview buffer
        ...
        for (int j = 0; j < ht; j++)
            for (int i = 0; i < wd; i++)
                out[(j + max_supp) * *wd2 + i + max_supp] =
                    input[stride * (wd * j + i)] * 0.01f; // L -> [0,1]
...
#define LL_FILL(fallback)                                                                          \
    ...
    {                                                                                              \
        ...
        if (isx < 0 || isy >= b->buf->width || isy < 0 || isy >= b->buf->height)                   \
            out[*wd2 * j + i] = (fallback);                                                        \
        else                                                                                       \
        {                                                                                          \
            int px =                                                                               \
                CLAMP(isx / (float)b->buf->width * b->wd + (b->pwd - b->wd) / 2, 0, b->pwd - 1);   \
            int py =                                                                               \
                CLAMP(isy / (float)b->buf->height * b->ht + (b->pht - b->ht) / 2, 0, b->pht - 1);  \
        ...
        for (int j = 0; j < ht; j++)
        {
            ...
            for (int i = 0; i < wd; i++)
                out[(j + max_supp) * *wd2 + i + max_supp] =
                    input[stride * (wd * j + i)] * 0.01f; // L -> [0,1]
            for (int i = wd + max_supp; i < *wd2; i++)
                out[(j + max_supp) * *wd2 + i] =
                    input[stride * (j * wd + wd - 1)] * 0.01f; // L -> [0,1]
        }
...
{
    const float c = ll_expand_gaussian(coarse, CLAMPS(i, 1, ((wd - 1) & ~1) - 1),
                                       CLAMPS(j, 1, ((ht - 1) & ~1) - 1), wd, ht);
...
void apply_curve(float *const out, const float *const in, const uint32_t w, const uint32_t h,
                 const uint32_t padding, const float g, const float sigma, const float shadows,
                 const float highlights, const float clarity)
{
    ...
    for (uint32_t j = padding; j < h - padding; j++)
    {
        ...
        for (int i = 0; i < padding; i++)
            out2[i] = out2[padding];
        for (int i = w - padding; i < w; i++)
            out2[i] = out2[w - padding - 1];
    }
    ...
    local_laplacian_boundary_t *b)
{
    ...
    gauss_reduce(padded[last_level - 1], output[last_level], dl(w, last_level - 1),
                 dl(h, last_level - 1));

    // evenly sample brightness [0,1]:
    float gamma[num_gamma] = {0.0f};
    for (int k = 0; k < num_gamma; k++)
        gamma[k] = (k + .5f) / (float)num_gamma;
    ...
    for (int k = 0; k < num_gamma; k++)
    { // process images
        apply_curve(buf[k][0], padded[0], w, h, max_supp, gamma[k], sigma, shadows, highlights,
                    clarity);
    ...
    {
        const float isize =
            powf(2.0f, last_level) / b->roi->scale; // pixel size of coarsest level in image space
        const float psize =
            isize / b->buf->width * b->wd; // pixel footprint rescaled to preview buffer
        const float pl = log2f(psize);     // mip level in preview buffer
        const int pl0 = CLAMP((int)pl, 0, b->num_levels - 1),
                  pl1 = CLAMP((int)(pl + 1), 0, b->num_levels - 1);
        ...
        for (int j = 0; j < ph; j++)
            for (int i = 0; i < pw; i++)
            {
                ...
                float px =
                    CLAMP(ix / (float)b->buf->width * b->wd + (b->pwd - b->wd) / 2.0f, 0, b->pwd);
                float py =
                    CLAMP(iy / (float)b->buf->height * b->ht + (b->pht - b->ht) / 2.0f, 0, b->pht);
                // trilinear lookup:
                ...
                float c0 =
                    ...
                    (1.0f - f0x) * (f0y)*b->output[pl0][CLAMP(py0 + 1, 0, ph0 - 1) * pw0 +
                                                        CLAMP(px0, 0, pw0 - 1)] +
                    (f0x) * (f0y)*b->output[pl0][CLAMP(py0 + 1, 0, ph0 - 1) * pw0 +
                                                 CLAMP(px0 + 1, 0, pw0 - 1)];
                float c1 =
                    (1.0f - f1x) * (1.0f - f1y) *
                    ...
                    (1.0f - f1x) * (f1y)*b->output[pl1][CLAMP(py1 + 1, 0, ph1 - 1) * pw1 +
                                                        CLAMP(px1, 0, pw1 - 1)] +
                    (f1x) * (f1y)*b->output[pl1][CLAMP(py1 + 1, 0, ph1 - 1) * pw1 +
                                                 CLAMP(px1 + 1, 0, pw1 - 1)];
#else
    ...
    for (int j = 0; j < ht; j++)
        ...
        {
            out[4 * (j * wd + i) + 0] =
                100.0f * output[0][(j + max_supp) * w + max_supp + i]; // [0,1] -> L
            out[4 * (j * wd + i) + 1] =
                input[4 * (j * wd + i) + 1]; // copy original colour channels
