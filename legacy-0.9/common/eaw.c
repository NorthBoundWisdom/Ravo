...
static inline void weight(const dt_aligned_pixel_t c1, const dt_aligned_pixel_t c2,
                          const dt_aligned_pixel_t sharpen, dt_aligned_pixel_t weight)
{
    /* Computes the vector
 * (wl, wc, wc, 1)
 *
 ...
 */
    ...
    const dt_aligned_pixel_t square2 = {square[0], square[2], square[1],
                                        square[3]}; // { d1, d3, d2, ? }
    dt_aligned_pixel_t added;
    for_each_channel(c) added[c] = square[c] + square2[c]; // { d1+d1, d2+d3, d2+d3, ? }
    dt_aligned_pixel_t sharpened;
    for_each_channel(c) sharpened[c] =
        sharpen[c] * added[c];        // { -s*d1,  -s*(d2+d3), -s*(d2+d3), 0 }
    dt_vector_exp(sharpened, weight); // { wl, wc, wc, 1 }
}

static inline void accumulate(dt_aligned_pixel_t accum, const dt_aligned_pixel_t detail,
                              const dt_aligned_pixel_t thresh, const dt_aligned_pixel_t boostval)
{
    ...
    for_four_channels(c, aligned(amount, sum, diff)) amount[c] = sum[c] + diff[c];
                                  ...
                                  const ssize_t height)
{
    ...
    for (size_t rowid = 0; rowid < height; rowid++)
    {
        ...
        const size_t lbound =
            (j < boundary || j >= height - boundary) ? width - boundary : boundary;
        ...
        for (i = 0; i < lbound; i++)
        {
            ...
            for (ssize_t jj = 0; jj < 5; jj++)
            {
                ...
                for (ssize_t ii = 0; ii < 5; ii++)
                {
                    ...
                    if (x < 0)
                        x = 0; // we might be looking past the left edge
                      ...
                      const float inv_sigma2, const int32_t width, const int32_t height)
{
    ...
    for_each_channel(c) sum_squared[c] = sum_sq[c];
}
