...
*/
static void icuFunctionError(sqlite3_context *pCtx, /* SQLite scalar function context */
...
** a UCollator structure previously allocated using ucol_open().
*/
static int icuCollationColl(void *pCtx, int nLeft, const void *zLeft, int nRight,
                            const void *zRight)
{
...
static void icuLoadCollation(sqlite3_context *p, int nArg, sqlite3_value **apArg)
{
    ...
    rc = sqlite3_create_collation_v2(db, zName, SQLITE_UTF16, (void *)pUCollator, icuCollationColl,
                                     icuCollationDel);
    if (rc != SQLITE_OK)
    {
...
#define SQLITEICU_EXTRAFLAGS (SQLITE_DETERMINISTIC | SQLITE_INNOCUOUS)
    ...
    for (i = 0; rc == SQLITE_OK && i < (int)(sizeof(scalars) / sizeof(scalars[0])); i++)
    {
        ...
        rc = sqlite3_create_function(db, p->zName, p->nArg, p->enc,
                                     p->iContext ? (void *)db : (void *)0, p->xFunc, 0, 0);
    }
