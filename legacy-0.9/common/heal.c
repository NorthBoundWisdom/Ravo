...
static void _heal_add(const float *const restrict red_buffer, const float *const black_buffer,
                      ...
                      float *const restrict result_buffer, const size_t width, const size_t height)
{
    ...
    for (size_t row = 0; row < height; row++)
    {
        const int parity = row & 1;
        const size_t row_start = (row + 1) * res_stride;
        const float *const restrict buf1 =
            parity ? red_buffer + row_start : black_buffer + row_start;
        const float *const restrict buf2 =
            parity ? black_buffer + row_start : red_buffer + row_start;
        // handle the pixels of the row pairwise, one red and one black at a time
        for (size_t col = 0; col < width / 2; col++)
        {
            const size_t idx = 4 * (row * width + 2 * col);
        ...
        if (width & 1)
        {
            ...
            const size_t res_idx = (width - 1) / 2;
            const size_t idx = 4 * (row * width + (width - 1));
            for_each_channel(c) result_buffer[idx + c] =
                buf1[4 * res_idx + c] + second_buffer[idx + c];
        }
    }
}
...
static float _heal_laplace_iteration(float *const restrict active_pixels,
                                     ...
                                     const unsigned *const restrict runs, const size_t num_runs,
                                     const size_t start_parity, const float w)
{
    ...
    for (size_t i = 0; i < num_runs; i++)
    {
        ...
        if (row == 1)
            a -= 1.0f; // we added a padding row at top and bottom
        if (row == height)
            a -= 1.0f;
        const size_t vert_offset = 4 * width;
        const size_t lroffset =
            4 * (start_parity ^ (row & 1)); // how many floats to offset the left/right neighbors
        if (count == 1)
        {
            ...
            for_each_channel(c, aligned(active_pixels, neighbor_pixels))
            {
                diff[c] = w * ((aa * active_pixels[index + c]) -
                               (neighbor_pixels[index - vert_offset + c] +
                                neighbor_pixels[index + vert_offset + c] + left[c] + right[c]));
        ...
        for (size_t j = 0; j < count; j++)
        {
            ...
            for_each_channel(c, aligned(active_pixels, neighbor_pixels))
            {
                ...
                diff[c] = w * (a * active_pixels[pixidx + c] -
                               (neighbor_pixels[pixidx - vert_offset + c] +
                                neighbor_pixels[pixidx + vert_offset + c] + left[c] + right[c]));
...
static size_t _collect_color_runs(const float *const restrict mask, const size_t start_index,
                                  size_t start, const size_t width, unsigned *const restrict runs,
                                  size_t count, size_t *nmask)
{
    size_t masked = 0;
...
static void collect_runs(const int start, const float *const restrict mask, const size_t width,
                         const size_t height, const size_t subwidth, unsigned *const restrict runs,
                         size_t *count, size_t *nmask)
{
    for (size_t row = 0; row < height; row++)
    {
                               ...
                               const float *const restrict mask, const int max_iter)
{
   ...
   */
    ...
    for (int iter = 0; iter < max_iter; iter++)
    {
        ...
        float err = _heal_laplace_iteration(black_pixels, red_pixels, height, subwidth, black_runs,
                                            num_black, 1, w);
        err += _heal_laplace_iteration(red_pixels, black_pixels, height, subwidth, red_runs,
                                       num_red, 0, w);
...
cleanup:
    if (red_runs)
        dt_free_align(red_runs);
    if (black_runs)
        dt_free_align(black_runs);
}
...
void dt_heal(const float *const src_buffer, float *dest_buffer, const float *const mask_buffer,
             const int width, const int height, const int ch, const int max_iter)
{
    if (ch != 4)
    {
...
cleanup:
    if (red_buffer)
        dt_free_align(red_buffer);
    if (black_buffer)
        dt_free_align(black_buffer);
}
...
{
    dt_heal_cl_global_t *g = malloc(sizeof(dt_heal_cl_global_t));
...
{
    heal_params_cl_t *p = malloc(sizeof(heal_params_cl_t));
    if (!p)
        return NULL;
...
cl_int dt_heal_cl(heal_params_cl_t *p, cl_mem dev_src, cl_mem dev_dest,
                  const float *const mask_buffer, const int width, const int height,
                  const int max_iter)
{
    ...
    err = dt_opencl_write_buffer_to_device(p->devid, dest_buffer, dev_dest, 0,
                                           sizeof(float) * width * height * ch, TRUE);

cleanup:
    if (src_buffer)
        dt_free_align(src_buffer);
    if (dest_buffer)
        dt_free_align(dest_buffer);
