...
| HSL zoning | `colorequal`, `colorzones`. | One HSL-partition capability, recipe schema, leftover disposition. | **Keep `colorequal`.** Decision/review owner: WisdomToNorth, 2026-08-25. `ravo.color.colorequal` v1 is an 8-band HSL equalizer on the linear working image. `colorzones` is leftover, not a fallback. |
