...
| Scene color grading | `colorbalancergb`, `colorbalance`, former Ravo lift/gamma/gain approximation. | Distinct schemas, working spaces, default formula, fixture/RAW evidence and overlap policy. | **Keep full `colorbalancergb` and remove the approximation.** Decision/review owner: WisdomToNorth, 2026-08-26. `ravo.color.colorbalancergb` v1 owns Filmlight Yrg grading with DT UCS default and explicit JzAzBz; its frozen owner is retired. `colorbalance.c` remains a distinct queued algorithm, not a fallback or alias. |
