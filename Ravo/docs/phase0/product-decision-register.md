...
| HSL zoning | `colorequal`, `colorzones`. | One hue-partition capability, recipe schema, leftover disposition. | **Keep `colorequal`.** Decision/review owner: WisdomToNorth, 2026-08-25. `ravo.color.colorequal` v1 is the frozen `colorequal.c` dt UCS 22 8-node periodic RBF equalizer on the linear working image. `colorzones` is leftover, not a fallback. |
