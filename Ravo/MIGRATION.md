...
| Local export | imageio / `libs/export.c` | services + raster encoder + CLI/Studio | Old implementation removed | JPEG/PNG/original copy and conflict/cancellation tested; TIFF plugin target is required; pixel exports declare sRGB, while selectable output profiles and complete metadata remain unfinished. Old `libs/export*.c` deleted |
