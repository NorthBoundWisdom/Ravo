...
| Mask/blend/operations | develop/iop | recipe + engine | In progress | `ravo.effect.graduatednd` is the first local adjustment, where the gradient is the mask. Exposure, Color Checker, Color Correction, Color Contrast, and Color Harmonizer accept exact default-unmasked import boundaries; Color Harmonizer additionally maps ordered singleton revisions by greatest history position. Unsupported mask/custom-blend/multi state rejects structurally and the general mask graph remains |
...
| Color Harmonizer | `iop/colorharmonizer.c` | `ravo.color.colorharmonizer` | In progress | exact schema v1 has 17 flat fields and a `profile_linear_rgb_d50` / `dt_ucs_harmony_v1` identity. Enabled unmasked CPU behavior retains negative `fmaxf` clipping, declared-profile D50↔dt-UCS conversion, immutable S2.1 RYB/node geometry, cubic neutral protection, source float order, and no contraction. Positive smoothing owns JCH 3c plus a private S2.2 correction/scratch recurrence with the frozen ±1e9 per-read clamp and canonical working ROI scale; unknown scale fails only for positive smoothing. Strict v1 import maps the evidenced zero-smoothing 0176 singleton envelope (records 12/13; greatest history position wins) to one canonical operation; synthetic positive legacy payloads, the complete document, masks/custom blend/multi remain incompatible. Develop/CLI/Catalog/Studio share the canonical recipe/cache/engine path and Studio exposes bounded smoothing. General masks/presentation and old-owner retirement remain unfinished |
