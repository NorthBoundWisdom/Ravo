...
| Recipe/schema | IOP params/XMP | recipe | In progress | versioned round-trip, explicit upgrades, and bounded strict mappings including exposure v5/v6/v7 and Color Checker v1/v2 are tested |
...
| CPU preview/pixelpipe | `src/develop` | engine | In progress | bounded PNG, cancellation, full exposure and Color Checker CPU behavior, and explicit colour operations are tested; complete ROI remains unfinished |
...
| Mask/blend/operations | develop/iop | recipe + engine | In progress | `ravo.effect.graduatednd` is the first local adjustment, where the gradient is the mask. Exposure and Color Checker accept exact default-unmasked boundaries and reject mask/custom-blend/multi state structurally; the general mask graph remains |
