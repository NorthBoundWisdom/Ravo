...
| Recipe/schema | IOP params/XMP | recipe | In progress | versioned round-trip and strict mappings include S3.1 typed mask-schema-v2 graph with v1 `all` upgrade, deterministic serialization, bounded DAG validation, and the existing exposure/Color Checker/Color Correction/Color Contrast/Color Harmonizer contracts; legacy XMP remains evidence-bound |
...
| CPU preview/pixelpipe | `src/develop` | engine | In progress | bounded PNG, cancellation, full exposure, Color Checker, affine D50 Lab Color Correction, bounded/unbounded D50 Lab Color Contrast, profile-aware dt-UCS/RYB Color Harmonizer including canonical-ROI recursive positive smoothing, and S3.1 ROI-tiled canonical mask alpha/normal mix are tested; complete pixelpipe/geometry ROI remains unfinished |
