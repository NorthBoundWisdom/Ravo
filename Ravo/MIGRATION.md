...
| Local export | imageio / `libs/export.c` | services + raster encoder + CLI/Studio | Old implementation removed | JPEG owns typed quality/subsampling; PNG owns typed 8/16-bit depth plus compression 0–9/default 5 and a private bounded libpng RGB8/ICC/cICP encoder, with true RGB16 structured unsupported; TIFF remains on Qt and original copy is available. Complete EXIF/XMP/resolution metadata, explicit PNG CLI options, and batch presets remain unfinished. Old `libs/export*.c` are deleted; `imageio/format/png.c` remains under active TODO I12, while the original-copy plugin and its shared dynamic consumers remain tracked below and under active I10 |
