...
| Recipe/schema | IOP params/XMP | recipe | In progress | versioned round-trip, explicit upgrades, and bounded strict mappings including exposure v5/v6/v7, Color Checker v1/v2, and Color Correction v1 are tested |
...
| CPU preview/pixelpipe | `src/develop` | engine | In progress | bounded PNG, cancellation, full exposure, Color Checker, and affine D50 Lab Color Correction CPU behavior, plus explicit colour operations are tested; complete ROI remains unfinished |
...
| Mask/blend/operations | develop/iop | recipe + engine | In progress | `ravo.effect.graduatednd` is the first local adjustment, where the gradient is the mask. Exposure, Color Checker, and Color Correction accept exact default-unmasked boundaries and reject mask/custom-blend/multi state structurally; the general mask graph remains |
...
| Local export | imageio / `libs/export.c` | services + raster encoder + CLI/Studio | Old implementation removed | JPEG/PNG/TIFF profile-labelled output plus original copy are available; complete metadata and batch presets remain unfinished. Old `libs/export*.c` deleted; the separate original-copy plugin remains tracked below |
