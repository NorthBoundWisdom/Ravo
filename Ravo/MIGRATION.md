...
| Recipe/schema | IOP params/XMP | recipe | In progress | versioned round-trip and strict mappings include S3.1 typed mask-schema-v2 graph with v1 `all` upgrade, deterministic serialization, bounded DAG validation, and S3.2 strict Studio-owned leaf authoring fields/IDs; legacy XMP remains evidence-bound |
...
| CPU preview/pixelpipe | `src/develop` | engine | In progress | bounded PNG, cancellation, full exposure, Color Checker, affine D50 Lab Color Correction, bounded/unbounded D50 Lab Color Contrast, profile-aware dt-UCS/RYB Color Harmonizer including canonical-ROI recursive positive smoothing, and S3.1 ROI-tiled canonical mask alpha/normal mix are tested; S3.2 Studio presents the resulting operation preview only, without alpha overlay; complete pixelpipe/geometry ROI remains unfinished |
| SQLite catalog | common/database | domain + SQLite adapter | In progress | schema v5 create/reopen/migrate/newer-version rejection tested; additive capture datetime/GPS columns stay NULL on old rows; tags, writable metadata, history tested; no old-catalog migration |
...
| Mask/blend/operations | develop/iop | recipe + engine | In progress | S3.1 owns typed versioned all/linear-gradient/circle/ellipse/parametric/ordered-group DAGs, frozen group/parametric branch order, attached-frame pixel-centre ROI evaluation, normal mix, saturating masked memory accounting, and actual Color Harmonizer/Graduated ND Recipe/CLI/Catalog consumers. S3.2 adds bounded Studio-owned all/spatial/parametric leaf authoring through strict recipe helpers and read-only external/shared/group presentation; it has no alpha overlay. Group-child editor, path/brush, more blend modes, M1 gates, and retirement remain; strict legacy mask/custom-blend/multi import still rejects |
...
| Color Harmonizer | `iop/colorharmonizer.c` | `ravo.color.colorharmonizer` | In progress | exact schema v1 retains the frozen CPU/positive-smoothing contract. S3.1 adds canonical all/spatial/parametric/group attachments only through the private normal-mix dispatcher; unmasked bits remain unchanged and strict legacy XMP masks/custom blend/multi still reject. S3.2 lets Studio author its bounded unshared owned leaf and present external/shared/group state as read-only without a mask overlay. C14 is not accepted and the old owner remains |
