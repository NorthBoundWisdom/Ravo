...
| Color Harmonizer | `iop/colorharmonizer.c` | `ravo.color.colorharmonizer` | In progress | exact schema v1 has 17 flat fields and a `profile_linear_rgb_d50` / `dt_ucs_harmony_v1` identity. The accepted enabled, unmasked, smoothing-zero CPU path retains negative `fmaxf` clipping, declared-profile D50↔dt-UCS conversion, immutable S2.1 RYB/node geometry, cubic neutral protection, source float order, cancellation, validation, and owned publication. Strict v1 import maps the evidenced 0176 singleton envelope (records 12/13; greatest history position wins) to one canonical 17-field operation; the complete 0176 document is not compatible. Explicit Develop/CLI/Catalog/Studio consumers share that recipe; Studio has no smoothing slider. Canonical ROI scale plus S2.2 recursive Gaussian, masks/presentation, and old-owner retirement remain unfinished |
