...
TEST(StudioQmlContract, LightPresentsCommonControlsBeforeSpecializedSettings)
{
    ...
    const auto whites_mask = light.indexOf(QStringLiteral("objectName: \"whitesMaskEditor\""));
    const auto blacks = light.indexOf(QStringLiteral("previewDevelopNumber(\"blacks\""));
    const auto blacks_mask = light.indexOf(QStringLiteral("objectName: \"blacksMaskEditor\""));
    ...
    ASSERT_EQ(highlights_mask, -1);
    ASSERT_GE(shadows, 0);
    ASSERT_EQ(shadows_mask, -1);
    ASSERT_GE(whites, 0);
    ASSERT_EQ(whites_mask, -1);
    ASSERT_GE(blacks, 0);
    ASSERT_EQ(blacks_mask, -1);
...
TEST(StudioQmlContract, DevelopPanelUsesDefaultGradingStackWithoutBuryingColorEq)
{
    const auto source = combined_develop_qml_source();
    ASSERT_FALSE(source.isEmpty());
    EXPECT_EQ(source.count(QStringLiteral("MaskEditor {")), 1);
    EXPECT_TRUE(source.contains(QStringLiteral("editLocalMask")));
    EXPECT_TRUE(source.contains(QStringLiteral("createLocalMask")));
    EXPECT_TRUE(source.contains(QStringLiteral("finishLocalMask")));
    EXPECT_TRUE(source.contains(QStringLiteral("activeLocalId")));
    ...
    EXPECT_FALSE(source.contains(QStringLiteral("objectName: \"colorBalanceRgbMaskEditor\"")));
    EXPECT_FALSE(source.contains(QStringLiteral("editColorBalanceRgbMask")));
    EXPECT_FALSE(source.contains(QStringLiteral("objectName: \"exposureMaskEditor\"")));
    EXPECT_FALSE(source.contains(QStringLiteral("editExposureMask")));
    EXPECT_FALSE(source.contains(QStringLiteral("objectName: \"rgbCurveMaskEditor\"")));
    EXPECT_FALSE(source.contains(QStringLiteral("editRgbCurveMask")));
    EXPECT_FALSE(source.contains(QStringLiteral("objectName: \"toneCurveMaskEditor\"")));
    EXPECT_FALSE(source.contains(QStringLiteral("editToneCurveMask")));
    EXPECT_FALSE(source.contains(QStringLiteral("objectName: \"highlightsMaskEditor\"")));
    EXPECT_FALSE(source.contains(QStringLiteral("editHighlightsMask")));
    EXPECT_FALSE(source.contains(QStringLiteral("objectName: \"shadowsMaskEditor\"")));
    EXPECT_FALSE(source.contains(QStringLiteral("editShadowsMask")));
    EXPECT_FALSE(source.contains(QStringLiteral("objectName: \"whitesMaskEditor\"")));
    EXPECT_FALSE(source.contains(QStringLiteral("editWhitesMask")));
    EXPECT_FALSE(source.contains(QStringLiteral("objectName: \"blacksMaskEditor\"")));
    EXPECT_FALSE(source.contains(QStringLiteral("editBlacksMask")));
...
TEST(StudioQmlContract, ExposureAndColorBalanceInstanceChrome)
{
    ...
    EXPECT_TRUE(chrome_source.contains(QStringLiteral("setExposureInstanceEnabled")));
    EXPECT_TRUE(chrome_source.contains(QStringLiteral("setColorBalanceRgbInstanceEnabled")));
