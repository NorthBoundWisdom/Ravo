...
[[nodiscard]] QByteArray minimal_heic_ftyp()
{
    QByteArray brands = QByteArrayLiteral("heic");
    brands.append(QByteArray(4, '\0'));
    brands.append(QByteArrayLiteral("mif1"));
...
class HeicCatalogTest : public ::testing::Test
{
protected:
    void SetUp() override
    {
        ...
        root_ =
            std::filesystem::temp_directory_path() / ("ravo-heic-catalog-" + generate_catalog_id());
...
TEST_F(HeicCatalogTest, EnumeratesHeicExtensionAndInspectsAsUnsupported)
{
    ...
    EXPECT_TRUE(std::filesystem::equivalent(enumerated.value().front(), path, equivalent_error))
...
int main(int argc, char **argv)
{
    ...
    ravo::init_logging("ravo-heic-catalog-tests");
