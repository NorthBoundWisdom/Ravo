...
parse_catalog_flags(const std::span<const std::string_view> positional)
{
    ...
    for (std::size_t index = 2; index < positional.size(); ++index)
    {
        ...
        if (option == "--unpin")
        {
            if (result.unpin)
                return make_error(ErrorCode::kInvalidArgument,
                                  "--unpin can only be specified once");
            result.unpin = true;
            continue;
        }
        if (option == "--clear-proxy")
        {
            if (result.clear_proxy)
                return make_error(ErrorCode::kInvalidArgument,
                                  "--clear-proxy can only be specified once");
            result.clear_proxy = true;
            continue;
        }
        if (option == "--pick")
        {
        ...
        else if (option == "--max-total-bytes")
        {
        ...
        {
            auto parsed = parse_catalog_metadata_flag(result, option, value);
            if (!parsed)
            {
                return parsed.error();
            }
