...
run_catalog_command(const EngineFacade &engine, const std::span<const std::string_view> positional)
{
    ...
    if (has_set_options && !set_command && subcommand != "cull-review")
        return make_error(ErrorCode::kInvalidArgument,
    ...
    if ((flags.value().cull_pick || flags.value().cull_reject_flag || flags.value().cull_unflag ||
         flags.value().auto_advance || !flags.value().color_label.empty() ||
         !flags.value().selection_asset_ids.empty()) &&
        subcommand != "cull-review")
        return make_error(ErrorCode::kInvalidArgument,
                          "--pick/--reject/--unflag/--auto-advance/--color-label/"
                          "--selection-asset-id are only valid for catalog cull-review");
    ...
    if (subcommand == "import-scan" &&
        (!flags.value().import_mode.empty() || !flags.value().import_destination.empty() ||
         !flags.value().import_organization.empty() || !flags.value().import_preview.empty() ||
         !flags.value().import_filename_template.empty() ||
         !flags.value().import_second_copy.empty()))
        return make_error(ErrorCode::kInvalidArgument,
                          "import-scan accepts source and recursion options only");
    ...
    if (subcommand == "probe")
        return run_catalog_probe_command(engine, service, flags.value());
    ...
    if (subcommand == "export-preset-save" || subcommand == "export-job-create" ||
        ...
        return run_catalog_export_command(service, subcommand, flags.value());
    if (subcommand == "tag")
    {
        if (flags.value().asset_id.empty())
        {
