...
you view them in the lighttable and enables you to develop
...
This checkout keeps image-pipeline regression tests for IOPs that are built by
DarkTableNext. UI tests and fixtures that reference retired IOPs are excluded.
The retained sidecars use the DarkTableNext XMP schema version 6 contract.

For a fast CPU-only pass on macOS, use:

```bash
DARKTABLE_CLI=/path/to/darktable-cli ./run \
  --disable-opencl --disable-timing --no-deltae
```

To intentionally rebuild the CPU reference images from the current code, use
the explicit baseline mode. It disables OpenCL while updating `expected.png`:

```bash
DARKTABLE_CLI=/path/to/darktable-cli ./run \
  --update-expected --disable-timing --no-deltae
```

On an OpenCL-capable machine, refresh the CPU/GPU pixel-difference limits
separately. This mode explicitly enables OpenCL and writes `cpugpu.maxpix`:

```bash
DARKTABLE_CLI=/path/to/darktable-cli ./run \
  --update-opencl-expected --disable-timing --no-deltae
```
...
deltae      : python script to compute a delta-E between 2 images
              expected.png and output.png

nnnn-name/  : tests
```

The light check requires Python 3, NumPy, and Pillow. The full Delta-E check
also requires colour-science, imageio, SciPy, and packaging. ImageMagick's
`compare` is optional and is used only to generate visual diff images.
