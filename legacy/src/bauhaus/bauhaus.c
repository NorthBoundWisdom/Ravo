...
typedef struct dt_bauhaus_combobox_data_t
{
    ...
    dt_bauhaus_combobox_alignment_t
        text_align; // if selected text in combo should be aligned to the left/right
    ...
    dt_bauhaus_combobox_entry_select_fct
        entry_select; // function to select an entry based on context
...
static void _slider_set_normalized(dt_bauhaus_widget_t *w, const float pos);
...
static void _combobox_next_sensitive(dt_bauhaus_widget_t *w, int delta, guint state,
                                     const gboolean mute)
{
...
static dt_bauhaus_combobox_entry_t *
_new_combobox_entry(const char *label, const dt_bauhaus_combobox_alignment_t alignment,
                    const gboolean sensitive, void *data, void (*free_func)(void *))
{
...
static int _show_pango_text(dt_bauhaus_widget_t *w, GtkStyleContext *context, cairo_t *cr,
                            const char *text, float x_pos, float y_pos, const float max_width,
                            const gboolean right_aligned, const gboolean calc_only,
                            PangoEllipsizeMode ellipsize, const gboolean is_markup,
                            const gboolean is_label, float *width, float *height)
{
    ...
    gtk_style_context_get(context, gtk_widget_get_state_flags(GTK_WIDGET(w)), "font", &font_desc,
                          NULL);
...
{
    return !strcmp(d->format, "°") && fabsf((d->max - d->min) * d->factor) == 360.0f &&
           dt_conf_get_bool("bauhaus/color_wheel");
}

static float _slider_get_line_offset(const float pos, const float scale, float x, float y,
                                     const float ht)
{
    ...
    if (y > ht)
    {
        ...
        offset = (x - y * y * .5f - (1.0f - y * y) * pos) / (.5f * y * y / scale + (1.0f - y * y));
    }
...
static void _slider_draw_line(cairo_t *cr, float pos, float off, float scale, const int width,
                              const int height, const int ht, dt_bauhaus_widget_t *w)
{
...
static void _slider_zoom_range(dt_bauhaus_widget_t *w, const float zoom)
{
    ...
    if (new_min >= d->hard_min && new_max <= d->hard_max && new_max - new_min >= min_visible * 10)
    {
        d->min = new_min;
        d->max = new_max;
    }
...
static void _window_moved_to_rect(GdkWindow *window, GdkRectangle *flipped_rect,
                                  GdkRectangle *final_rect, gboolean flipped_x, gboolean flipped_y,
                                  gpointer user_data)
{
...
static void _window_position(const int offset)
{
    ...
    gdk_window_move_to_rect(window, &pop->position, GDK_GRAVITY_NORTH_WEST, GDK_GRAVITY_NORTH_WEST,
                            GDK_ANCHOR_SLIDE_X | GDK_ANCHOR_RESIZE_Y, 0,
                            -pop->offset + pop->offcut);
}

static gboolean _window_motion_notify(GtkWidget *widget, GdkEventMotion *event, gpointer user_data)
{
    dt_bauhaus_t *bh = darktable.bauhaus;
    ...
    if (ex < -tol || ex > allocation.width + tol || ey + pop->offcut < -tol ||
    ...
    if (w->type == DT_BAUHAUS_SLIDER)
    {
        ...
        const float mouse_off =
            _is_full_circle(d) ?
                hypotf(cx, cy) < .25f ? 0 : atan2f(cx, cy) * -.5f * M_1_PI + 0.5f - pop->oldpos :
                _slider_get_line_offset(pop->oldpos,
                                        5.0 * powf(10.0f, -d->digits) / (d->max - d->min) /
                                            fabsf(d->factor),
                                        bh->mouse_x / (width - _widget_get_quad_width(w)),
                                        bh->mouse_y / width, ht / width);
        if (event->state & GDK_BUTTON1_MASK ||
            (bh->mouse_line_distance && ((bh->mouse_line_distance * mouse_off <= 0) ^
...
static gboolean _popup_button_press(GtkWidget *widget, GdkEventButton *event, gpointer user_data)
{
    ...
    if (event->button == GDK_BUTTON_PRIMARY)
    {
        ...
        if (w->type == DT_BAUHAUS_COMBOBOX && !dt_gui_long_click(event->time, bh->opentime))
        {
...
void dt_bauhaus_load_theme()
{
    ...
    dt_bauhaus_t *bh = darktable.bauhaus;
    gtk_style_context_lookup_color(ctx, "bauhaus_fg", &bh->color_fg);
    gtk_style_context_lookup_color(ctx, "bauhaus_fg_hover", &bh->color_fg_hover);
    gtk_style_context_lookup_color(ctx, "bauhaus_fg_insensitive", &bh->color_fg_insensitive);
    gtk_style_context_lookup_color(ctx, "bauhaus_bg", &bh->color_bg);
    gtk_style_context_lookup_color(ctx, "bauhaus_border", &bh->color_border);
    gtk_style_context_lookup_color(ctx, "bauhaus_fill", &bh->color_fill);
    gtk_style_context_lookup_color(ctx, "bauhaus_indicator_border", &bh->indicator_border);

    gtk_style_context_lookup_color(ctx, "graph_bg", &bh->graph_bg);
    gtk_style_context_lookup_color(ctx, "graph_exterior", &bh->graph_exterior);
    gtk_style_context_lookup_color(ctx, "graph_border", &bh->graph_border);
    gtk_style_context_lookup_color(ctx, "graph_grid", &bh->graph_grid);
    gtk_style_context_lookup_color(ctx, "graph_fg", &bh->graph_fg);
    gtk_style_context_lookup_color(ctx, "graph_fg_active", &bh->graph_fg_active);
    gtk_style_context_lookup_color(ctx, "graph_overlay", &bh->graph_overlay);
    gtk_style_context_lookup_color(ctx, "inset_histogram", &bh->inset_histogram);
    gtk_style_context_lookup_color(ctx, "graph_red", &bh->graph_colors[0]);
    gtk_style_context_lookup_color(ctx, "graph_green", &bh->graph_colors[1]);
    gtk_style_context_lookup_color(ctx, "graph_blue", &bh->graph_colors[2]);
    gtk_style_context_lookup_color(ctx, "colorlabel_red", &bh->colorlabels[DT_COLORLABELS_RED]);
    gtk_style_context_lookup_color(ctx, "colorlabel_yellow",
                                   &bh->colorlabels[DT_COLORLABELS_YELLOW]);
    gtk_style_context_lookup_color(ctx, "colorlabel_green", &bh->colorlabels[DT_COLORLABELS_GREEN]);
    gtk_style_context_lookup_color(ctx, "colorlabel_blue", &bh->colorlabels[DT_COLORLABELS_BLUE]);
    ...
    gtk_style_context_get(ctx, GTK_STATE_FLAG_NORMAL, "font", &bh->pango_font_desc, NULL);
...
void dt_bauhaus_init()
{
    ...
    gtk_window_set_type_hint(GTK_WINDOW(pop->window), GDK_WINDOW_TYPE_HINT_POPUP_MENU);
    ...
    g_signal_connect(gtk_widget_get_window(pop->window), "moved-to-rect",
                     G_CALLBACK(_window_moved_to_rect), NULL);
...
void dt_bauhaus_slider_set_soft_range(GtkWidget *widget, const float soft_min, const float soft_max)
{
...
void dt_bauhaus_widget_set_show_extended_label(GtkWidget *widget, gboolean show)
{
    dt_bauhaus_widget_t *w = DT_BAUHAUS_WIDGET(widget);
    w->show_extended_label = show;
}

void dt_bauhaus_widget_set_module(GtkWidget *widget, dt_action_t *module)
{
    dt_bauhaus_widget_t *w = DT_BAUHAUS_WIDGET(widget);
    w->module = module;
}
...
void dt_bauhaus_widget_set_quad_paint(GtkWidget *widget, dt_bauhaus_quad_paint_f f,
                                      const int paint_flags, void *paint_data)
{
    dt_bauhaus_widget_t *w = DT_BAUHAUS_WIDGET(widget);
    w->quad_paint = f;
...
static float _widget_width(const dt_bauhaus_widget_t *w)
{
    ...
    return MAX(1.0f, gtk_widget_get_allocated_width(GTK_WIDGET(w)) - w->margin.left -
                         w->padding.left - w->margin.right - w->padding.right -
                         _widget_get_quad_width(w));
}

gchar *dt_bauhaus_widget_get_tooltip_markup(GtkWidget *widget, const int x, const int y)
{
    gchar *markup = darktable.control->element == DT_ACTION_ELEMENT_BUTTON &&
                            DT_IS_BAUHAUS_WIDGET(widget) && DT_BAUHAUS_WIDGET(widget)->tooltip ?
...
void dt_bauhaus_widget_set_field(GtkWidget *widget, gpointer field,
...
void dt_bauhaus_update_from_field(dt_iop_module_t *module, GtkWidget *widget, gpointer params,
                                  gpointer blend_params)
{
    ...
    for (GSList *w = widget ? &(GSList){} : module->widget_list_bh; w; w = w->next)
    {
        ...
        if (!notebook && DT_IN_GUI_UPDATE() && (notebook = gtk_widget_get_parent(widget)) &&
            (notebook = gtk_widget_get_parent(notebook)) && !GTK_IS_NOTEBOOK(notebook))
            notebook = NULL;
    }

    if (notebook)
        gtk_container_foreach(GTK_CONTAINER(notebook), _highlight_changed_notebook_tab, NULL);
}

// make this quad a toggle button:
void dt_bauhaus_widget_set_quad_toggle(GtkWidget *widget, const gboolean toggle)
{
    dt_bauhaus_widget_t *w = DT_BAUHAUS_WIDGET(widget);
    w->quad_toggle = toggle;
}

void dt_bauhaus_widget_set_quad_active(GtkWidget *widget, const gboolean active)
{
    dt_bauhaus_widget_t *w = DT_BAUHAUS_WIDGET(widget);
    if (active)
...
GtkWidget *dt_bauhaus_slider_new_with_range(dt_iop_module_t *self, const float min, const float max,
                                            const float step, const float defval, const int digits)
{
    return dt_bauhaus_slider_new_with_range_and_feedback(self, min, max, step, defval, digits, 1);
}

GtkWidget *dt_bauhaus_slider_new_action(dt_action_t *self, const float min, const float max,
                                        const float step, const float defval, const int digits)
{
    return dt_bauhaus_slider_new_with_range((dt_iop_module_t *)self, min, max, step, defval,
                                            digits);
}

GtkWidget *dt_bauhaus_slider_new_with_range_and_feedback(dt_iop_module_t *self, const float min,
                                                         const float max, const float step,
                                                         const float defval, const int digits,
                                                         const int feedback)
{
...
GtkWidget *dt_bauhaus_slider_from_widget(dt_bauhaus_widget_t *w, dt_iop_module_t *self,
                                         const float min, const float max, const float step,
                                         const float defval, const int digits, const int feedback)
{
                                        ...
                                        gpointer data, const char **texts)
{
    ...
    if (callback)
        g_signal_connect_closure_by_id(G_OBJECT(combo),
                                       ...
                                       0, g_cclosure_new(G_CALLBACK(callback), data, NULL), FALSE);
...
static dt_bauhaus_combobox_data_t *_combobox_data(GtkWidget *widget)
{
    ...
    if (d->active >= d->entries->len)
        d->active = -1;

    return d;
}

void dt_bauhaus_combobox_add_populate_fct(GtkWidget *widget, dt_bauhaus_combobox_populate_fct fct)
{
    dt_bauhaus_widget_t *w = DT_BAUHAUS_WIDGET(widget);
    if (w->type == DT_BAUHAUS_COMBOBOX)
...
void dt_bauhaus_combobox_add_list(GtkWidget *widget, dt_action_t *action, const char **texts)
{
    ...
    if (!darktable.control->accel_initialised && DT_BAUHAUS_WIDGET(widget)->module &&
...
gboolean dt_bauhaus_combobox_add_introspection(GtkWidget *widget, dt_action_t *action,
                                               ...
                                               const int start, const int end)
{
    ...
    while (item->name && item->value != start)
        item++;
    for (; item->name; item++)
    {
        const char *text = item->description ? item->description : item->name;
        if (*text)
            dt_bauhaus_combobox_add_full(widget, Q_(text), DT_BAUHAUS_COMBOBOX_ALIGN_RIGHT,
...
{
    dt_bauhaus_combobox_add_full(widget, text, DT_BAUHAUS_COMBOBOX_ALIGN_LEFT, GINT_TO_POINTER(-1),
                                 NULL, FALSE);
}

void dt_bauhaus_combobox_add_aligned(GtkWidget *widget, const char *text,
                                     const dt_bauhaus_combobox_alignment_t align)
{
    dt_bauhaus_combobox_add_full(widget, text, align, NULL, NULL, TRUE);
}

void dt_bauhaus_combobox_add_full(GtkWidget *widget, const char *text,
                                  dt_bauhaus_combobox_alignment_t align, gpointer data,
                                  void(free_func)(void *data), const gboolean sensitive)
{
    dt_bauhaus_widget_t *w = DT_BAUHAUS_WIDGET(widget);
...
void dt_bauhaus_combobox_set_editable(GtkWidget *widget, const int editable)
{
    dt_bauhaus_widget_t *w = DT_BAUHAUS_WIDGET(widget);
    if (w->type != DT_BAUHAUS_COMBOBOX)
        return;
...
void dt_bauhaus_combobox_remove_at(GtkWidget *widget, const int pos)
{
...
void dt_bauhaus_combobox_insert(GtkWidget *widget, const char *text, const int pos)
{
    dt_bauhaus_combobox_insert_full(widget, text, DT_BAUHAUS_COMBOBOX_ALIGN_RIGHT, NULL, NULL, pos);
}

void dt_bauhaus_combobox_insert_full(GtkWidget *widget, const char *text,
                                     const dt_bauhaus_combobox_alignment_t align, gpointer data,
                                     void (*free_func)(void *), const int pos)
{
    dt_bauhaus_widget_t *w = DT_BAUHAUS_WIDGET(widget);
    if (w->type != DT_BAUHAUS_COMBOBOX)
        return;
...
const char *dt_bauhaus_combobox_get_entry(GtkWidget *widget, const int pos)
{
    const dt_bauhaus_combobox_data_t *d = _combobox_data(widget);
    if (!d || pos < 0 || pos >= d->entries->len)
        return NULL;
            ...
            case DT_INTROSPECTION_TYPE_ENUM:;
                if (d->active >= 0)
                {
                    ...
                    *e = GPOINTER_TO_INT(_combobox_entry(d, d->active)->data);
                    if (*e != preve)
                        dt_iop_gui_changed(w->module, GTK_WIDGET(w), &preve);
                }
                break;
            case DT_INTROSPECTION_TYPE_INT:;
                int *i = w->field, previ = *i;
                *i = d->active;
                if (*i != previ)
                    dt_iop_gui_changed(w->module, GTK_WIDGET(w), &previ);
                break;
            case DT_INTROSPECTION_TYPE_UINT:;
                unsigned int *u = w->field, prevu = *u;
                *u = d->active;
                if (*u != prevu)
                    dt_iop_gui_changed(w->module, GTK_WIDGET(w), &prevu);
                break;
            case DT_INTROSPECTION_TYPE_BOOL:;
                gboolean *b = w->field, prevb = *b;
                *b = d->active;
                if (*b != prevb)
                    dt_iop_gui_changed(w->module, GTK_WIDGET(w), &prevb);
                break;
            default:
...
gboolean dt_bauhaus_combobox_set_from_value(GtkWidget *widget, const int value)
{
...
void dt_bauhaus_combobox_entry_set_sensitive(GtkWidget *widget, const int pos,
...
void dt_bauhaus_slider_set_stop(GtkWidget *widget, const float stop, const float r, const float g,
                                const float b)
{
    dt_bauhaus_widget_t *w = DT_BAUHAUS_WIDGET(widget);
    if (w->type != DT_BAUHAUS_SLIDER)
    ...
    {
        dt_print(DT_DEBUG_ALWAYS, "[bauhaus_slider_set_stop] only %d stops allowed",
...
static void _draw_indicator(dt_bauhaus_widget_t *w, const float pos, cairo_t *cr, const float wd,
                            const GdkRGBA fg_color, const GdkRGBA border_color)
{
...
static void _draw_quad(dt_bauhaus_widget_t *w, cairo_t *cr, const int width, const int height)
{
    ...
    if (w->quad_paint)
    {
        ...
        const gboolean hovering =
            bh->hovered == GTK_WIDGET(w) && darktable.control->element == DT_ACTION_ELEMENT_BUTTON;

        set_color(cr, sensitive && (w->quad_paint_flags & CPF_ACTIVE) ?
                          hovering ? bh->color_fg_hover : bh->color_fg :
                      hovering ? bh->color_fg :
...
static void _set_gradient_color(cairo_t *cr, const dt_bauhaus_slider_data_t *d, const float pos)
{
...
static void _draw_color_wheel(dt_bauhaus_widget_t *w, GtkStyleContext *c, cairo_t *cr,
                              const float width, const GdkRGBA *fg_color, const GdkRGBA *bg_color)
{
    if (w->type != DT_BAUHAUS_SLIDER)
        return;
    ...
    else
    {
        ...
        _show_pango_text(w, c, cr, "0°", -tw / 4, tp - th, 0, FALSE, FALSE, 0, FALSE, FALSE, NULL,
                         NULL);
        _show_pango_text(w, c, cr, "-90°", tp, -th / 2, 0, TRUE, FALSE, 0, FALSE, FALSE, NULL,
                         NULL);
        _show_pango_text(w, c, cr, "90°", -tp, -th / 2, 0, FALSE, FALSE, 0, FALSE, FALSE, NULL,
                         NULL);
        _show_pango_text(w, c, cr, "180°", -tw / 2, -tp, 0, FALSE, FALSE, 0, FALSE, FALSE, NULL,
                         NULL);
...
static void _draw_baseline(dt_bauhaus_widget_t *w, cairo_t *cr, const float slider_width)
{
    ...
    const float origin = fmaxf(
        fminf((d->factor > 0 ? -d->min - d->offset / d->factor : d->max + d->offset / d->factor) /
                  (d->max - d->min),
              1.0f) *
            slider_width,
        0.0f);
...
static gboolean _popup_draw(GtkWidget *widget, cairo_t *cr, gpointer user_data)
{
    ...
    case DT_BAUHAUS_SLIDER:
    {
        ...
        else
        {
            ...
            const float scale =
                5.0 * powf(10.0f, -d->digits) / (d->max - d->min) / fabsf(d->factor);
            ...
            _show_pango_text(w, context, cr, min, 0, ht + INNER_PADDING, 0, FALSE, FALSE,
                             PANGO_ELLIPSIZE_END, FALSE, FALSE, NULL, NULL);
            g_free(min);
            char *max = dt_bauhaus_slider_get_text(GTK_WIDGET(w), d->factor > 0 ? d->max : d->min);
            _show_pango_text(w, context, cr, max, w3, ht + INNER_PADDING, 0, TRUE, FALSE,
                             PANGO_ELLIPSIZE_END, FALSE, FALSE, NULL, NULL);
            g_free(max);
        }
        ...
        char *text =
            dt_bauhaus_slider_get_text(GTK_WIDGET(w), dt_bauhaus_slider_get(GTK_WIDGET(w)));
        set_color(cr, *fg_color);
        const float value_width = _show_pango_text(w, context, cr, text, w3, 0, 0, TRUE, FALSE,
                                                   PANGO_ELLIPSIZE_END, FALSE, FALSE, NULL, NULL);
        g_free(text);
        ...
        if (label_width > 0)
        {
            ...
            _show_pango_text(w, context, cr, lb, 0, 0, label_width, FALSE, FALSE,
                             PANGO_ELLIPSIZE_END, FALSE, FALSE, NULL, NULL);
            g_free(lb);
        }
        cairo_restore(cr);
    ...
    case DT_BAUHAUS_COMBOBOX:
    {
        ...
        for (int j = 0; j < d->entries->len; j++)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    label = g_markup_printf_escaped("%s<b>%s</b>%s", start, match,
            ...
            {
                label_width =
                    _show_pango_text(w, context, cr, label, 0, ht * j + w->top_gap, max_width,
            ...
            else if (entry->alignment == DT_BAUHAUS_COMBOBOX_ALIGN_MIDDLE)
            {
                ...
                label_width =
                    _show_pango_text(w, context, cr, label, posx, ht * j + w->top_gap, max_width,
                                     FALSE, FALSE, ellipsis, TRUE, FALSE, NULL, NULL);
            }
            else
            {
                ...
                label_width =
                    _show_pango_text(w, context, cr, label, w3, ht * j + w->top_gap, max_width,
        ...
        if (show_box_label)
        {
            set_color(cr, text_color);
            gchar *lb = _build_label(w);
            _show_pango_text(w, context, cr, lb, 0, w->top_gap, w3 - first_label_width, FALSE,
                             FALSE, PANGO_ELLIPSIZE_END, FALSE, TRUE, NULL, NULL);
            g_free(lb);
        }
        g_free(keys);
    ...
    if (none_found && bh->keys_cnt)
    {
        ...
        PangoFontDescription *desc = pango_font_description_copy_static(bh->pango_font_desc);
...
static gboolean _widget_draw(GtkWidget *widget, cairo_t *crf)
{
    ...
    case DT_BAUHAUS_COMBOBOX:
    {
        ...
        if (label_text && w->show_label && d->text_align == DT_BAUHAUS_COMBOBOX_ALIGN_RIGHT)
            _show_pango_text(w, context, cr, label_text, 0, 0, 0, FALSE, TRUE, PANGO_ELLIPSIZE_END,
                             FALSE, TRUE, &label_width, &label_height);
        float combo_width = 0;
        float combo_height = 0;
        _show_pango_text(w, context, cr, text, w3, 0, 0, TRUE, TRUE, combo_ellipsis, FALSE, FALSE,
        ...
        if ((label_width + combo_width) > w3)
        {
            if (d->text_align == DT_BAUHAUS_COMBOBOX_ALIGN_RIGHT)
            {
                ...
                if (w->show_label)
                    _show_pango_text(w, context, cr, label_text, 0, w->top_gap,
                                     w3 * ratio - INNER_PADDING * 2, FALSE, FALSE,
                                     PANGO_ELLIPSIZE_END, FALSE, TRUE, NULL, NULL);
                _show_pango_text(w, context, cr, text, w3, w->top_gap, w3 * (1.0f - ratio), TRUE,
            ...
            else if (d->text_align == DT_BAUHAUS_COMBOBOX_ALIGN_MIDDLE)
            {
                ...
                _show_pango_text(w, context, cr, text, posx, w->top_gap, w3, FALSE, FALSE,
                                 combo_ellipsis, FALSE, FALSE, NULL, NULL);
            }
            else
                _show_pango_text(w, context, cr, text, 0, w->top_gap, w3, FALSE, FALSE,
                                 combo_ellipsis, FALSE, FALSE, NULL, NULL);
        }
        else
        {
            ...
            {
                if (w->show_label)
                    _show_pango_text(w, context, cr, label_text, 0, w->top_gap, 0, FALSE, FALSE,
                                     PANGO_ELLIPSIZE_END, FALSE, TRUE, NULL, NULL);
                _show_pango_text(w, context, cr, text, w3, w->top_gap, 0, TRUE, FALSE,
                                 combo_ellipsis, FALSE, FALSE, NULL, NULL);
            }
            else if (d->text_align == DT_BAUHAUS_COMBOBOX_ALIGN_MIDDLE)
            {
                const int posx = MAX(0, (w3 - combo_width) / 2);
                _show_pango_text(w, context, cr, text, posx, w->top_gap, 0, FALSE, FALSE,
                                 combo_ellipsis, FALSE, FALSE, NULL, NULL);
            }
            else
                _show_pango_text(w, context, cr, text, 0, w->top_gap, 0, FALSE, FALSE,
                                 combo_ellipsis, FALSE, FALSE, NULL, NULL);
        }
        g_free(label_text);
        break;
    ...
    case DT_BAUHAUS_SLIDER:
    {
        ...
        if (gtk_widget_is_sensitive(widget))
        {
            ...
            set_color(cr, *text_color);
            value_width = _show_pango_text(w, context, cr, text, w3, 0, 0, TRUE, FALSE,
                                           PANGO_ELLIPSIZE_END, FALSE, FALSE, NULL, NULL);
            g_free(text);
        }
        ...
        if (label_width > 0)
            _show_pango_text(w, context, cr, label_text, 0, 0, label_width, FALSE, FALSE,
                             PANGO_ELLIPSIZE_END, FALSE, TRUE, NULL, NULL);
        g_free(label_text);
    }
    break;
...
static gint _natural_width(GtkWidget *widget, const gboolean popup)
{
    gint natural_size = 0;
    ...
    gtk_style_context_get(gtk_widget_get_style_context(widget),
                          gtk_widget_get_state_flags(GTK_WIDGET(w)), "font", &font_desc, NULL);
    pango_layout_set_font_description(layout, font_desc);
    PangoAttrList *attrlist = pango_attr_list_new();
    pango_attr_list_insert(attrlist, pango_attr_font_features_new("tnum"));
    ...
    if (w->type == DT_BAUHAUS_COMBOBOX)
    {
        ...
        if (natural_size && d->text_align == DT_BAUHAUS_COMBOBOX_ALIGN_RIGHT &&
            (popup || w->show_label))
...
static void _widget_get_preferred_width(GtkWidget *widget, gint *minimum_width, gint *natural_width)
{
    ...
    *natural_width = _natural_width(widget, FALSE) + w->margin.left + w->margin.right +
                     w->padding.left + w->padding.right;
}

static void _widget_get_preferred_height(GtkWidget *widget, gint *minimum_height,
                                         gint *natural_height)
{
    dt_bauhaus_widget_t *w = (dt_bauhaus_widget_t *)widget;
    _margins_retrieve(w);
    ...
    *minimum_height = w->margin.top + w->margin.bottom + w->padding.top + w->padding.bottom +
    ...
    if (w->type == DT_BAUHAUS_SLIDER)
    {
        ...
        *minimum_height += INNER_PADDING + darktable.bauhaus->baseline_size +
                           1.5f * darktable.bauhaus->border_width;
    }
...
static void _popup_hide()
{
    ...
    if (w)
    {
        if (w->type == DT_BAUHAUS_COMBOBOX && w->combobox.mute_scrolling && bh->change_active)
...
static void _popup_show(GtkWidget *widget)
{
    ...
    else
    {
        ...
        p->width = MAX(
            1, p->width - (w->margin.left + w->margin.right + w->padding.left + w->padding.right));
    }
    ...
    case DT_BAUHAUS_COMBOBOX:
    {
        ...
        const dt_bauhaus_combobox_data_t *d = &w->combobox;
        if (d->populate)
            d->populate(widget, &module);

        if (!d->entries->len)
            return;
        p->height =
            bh->line_height * d->entries->len + w->margin.top + w->margin.bottom + w->top_gap;
...
static void _slider_add_step(GtkWidget *widget, float delta, const guint state,
                             const gboolean force)
{
    ...
    else if (!strcmp(d->format, "°") && fabsf((d->max - d->min) * d->factor - 360.0f) < 1e-4 &&
...
void dt_bauhaus_slider_set(GtkWidget *widget, const float pos)
{
    if (dt_isnan(pos))
        return;
    ...
    const float gpos =
        rpos == pos || strcmp(d->format, "°") ?
            rpos :
            d->hard_min + fmodf(pos + d->hard_max - 2 * d->hard_min, d->hard_max - d->hard_min);
...
void dt_bauhaus_slider_set_val(GtkWidget *widget, const float val)
{
    const dt_bauhaus_slider_data_t *d = &DT_BAUHAUS_WIDGET(widget)->slider;
    dt_bauhaus_slider_set(widget, (val - d->offset) / d->factor);
}

void dt_bauhaus_slider_set_digits(GtkWidget *widget, const int val)
{
    dt_bauhaus_widget_t *w = DT_BAUHAUS_WIDGET(widget);
...
void dt_bauhaus_slider_set_step(GtkWidget *widget, const float val)
{
    dt_bauhaus_widget_t *w = DT_BAUHAUS_WIDGET(widget);
...
void dt_bauhaus_slider_set_feedback(GtkWidget *widget, const int feedback)
{
    dt_bauhaus_widget_t *w = DT_BAUHAUS_WIDGET(widget);
...
void dt_bauhaus_slider_set_factor(GtkWidget *widget, const float factor)
{
    dt_bauhaus_widget_t *w = DT_BAUHAUS_WIDGET(widget);
    if (w->type != DT_BAUHAUS_SLIDER)
        return;
    ...
    if (factor < 0)
        w->slider.curve = _reverse_linear_curve;
}

void dt_bauhaus_slider_set_offset(GtkWidget *widget, const float offset)
{
    dt_bauhaus_widget_t *w = DT_BAUHAUS_WIDGET(widget);
    if (w->type != DT_BAUHAUS_SLIDER)
        return;
...
void dt_bauhaus_slider_set_curve(GtkWidget *widget,
                                 float (*curve)(const float value, const dt_bauhaus_curve_t dir))
{
    dt_bauhaus_widget_t *w = DT_BAUHAUS_WIDGET(widget);
    if (w->type != DT_BAUHAUS_SLIDER)
        return;
...
static void _slider_value_change(dt_bauhaus_widget_t *w)
{
    ...
    if (d->is_changed && !d->timeout_handle)
    {
        if (w->field)
        {
            ...
            {
            case DT_INTROSPECTION_TYPE_FLOAT:;
                float *f = w->field, prevf = *f;
                *f = val;
                if (*f != prevf)
                    dt_iop_gui_changed(w->module, GTK_WIDGET(w), &prevf);
                break;
            case DT_INTROSPECTION_TYPE_INT:;
                int *i = w->field, previ = *i;
                *i = val;
                ...
                break;
            case DT_INTROSPECTION_TYPE_USHORT:;
                unsigned short *s = w->field, prevs = *s;
                *s = val;
                if (*s != prevs)
                    dt_iop_gui_changed(w->module, GTK_WIDGET(w), &prevs);
                break;
            default:
                dt_print(DT_DEBUG_ALWAYS, "[_slider_value_change] unsupported slider data type");
            }
        }
...
static gboolean _popup_key_press(GtkWidget *widget, GdkEventKey *event, gpointer user_data)
{
    ...
    case GDK_KEY_KP_Left:
        ...
        break;
    default:
        if (!event->string || !g_utf8_validate(event->string, -1, NULL))
            return FALSE;
                                 ...
                                 const double y, gpointer user_data)
{
    ...
    if (w->quad_paint && !passthrough_from_histogram && pos > 1.0f + INNER_PADDING / width)
    {
...
static void _widget_button_stopped(GtkGestureSingle *gesture, dt_bauhaus_widget_t *w)
{
...
static void dt_bh_init(DtBauhausWidget *w)
{
    ...
    GtkEventController *scroll_controller = dt_gui_connect_scroll(
...
static void _action_process_button(GtkWidget *widget, const dt_action_effect_t effect)
{
    dt_bauhaus_widget_t *w = DT_BAUHAUS_WIDGET(widget);
...
static float _action_process_slider(gpointer target, const dt_action_element_t element,
    ...
    if (DT_PERFORM_ACTION(move_size))
    {
        ...
        case DT_ACTION_ELEMENT_FORCE:
            ...
            case DT_ACTION_EFFECT_UP:
                ++d->is_dragging;
                _slider_add_step(widget, move_size, GDK_MODIFIER_MASK,
        ...
        default:
            dt_print(DT_DEBUG_ALWAYS,
                     "[_action_process_slider] unknown shortcut element (%d) for slider", element);
            break;
        }
    }
...
static float _action_process_combo(gpointer target, const dt_action_element_t element,
                                   const dt_action_effect_t effect, float move_size)
{
    GtkWidget *widget = GTK_WIDGET(target);
    dt_bauhaus_widget_t *w = (dt_bauhaus_widget_t *)widget;
    ...
    if (DT_PERFORM_ACTION(move_size))
    {
        if (element == DT_ACTION_ELEMENT_BUTTON || !w->combobox.entries->len)
        {
            ...
            default:;
                ...
                dt_introspection_type_enum_tuple_t *values = g_hash_table_lookup(
                    darktable.bauhaus->combo_introspection, dt_action_widget(target));
                if (values)
    ...
    {
        if (!_combobox_entry(&w->combobox, i)->sensitive)
            value--; // don't count unselectable combo items in value
    }
    return -1 - value +
           (value == effect - DT_ACTION_EFFECT_COMBO_SEPARATOR - 1 ? DT_VALUE_PATTERN_ACTIVE : 0);
}

static gboolean _find_nth_bauhaus(gpointer *w, int *num, const dt_bauhaus_type_t type)
{
...
static float _action_process_focus_slider(gpointer widget, dt_action_element_t element,
...
static float _action_process_focus_combo(gpointer widget, dt_action_element_t element,
                                         const dt_action_effect_t effect, const float move_size)
{
    ...
    if (DT_PERFORM_ACTION(move_size))
        dt_action_widget_toast(&darktable.control->actions_focus, NULL,
                               _("not that many dropdowns"));
    return DT_ACTION_NOT_VALID;
}

static float _action_process_focus_button(gpointer widget, dt_action_element_t element,
                                          const dt_action_effect_t effect, const float move_size)
{
