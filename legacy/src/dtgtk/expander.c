    ...
    if (expander->expanded != expanded)
    {
        ...
        if (frame)
        {
            ...
            gtk_revealer_set_transition_duration(
                GTK_REVEALER(expander->frame), dt_conf_get_int("darkroom/ui/transition_duration"));
...
static gboolean _expander_scroll(GtkWidget *widget, GdkFrameClock *frame_clock, gpointer user_data)
{
    ...
    const int drop_space =
        (widget == _drop_widget) && header ? gtk_widget_get_allocated_height(header) : 0;
    ...
    if (is_iop && widget == _last_expanded && allocation.y < _start_pos.y)
    {
    ...
    float prop = 1.0f;
    const gboolean scroll_to_top =
        !_drop_widget && dt_conf_get_bool(is_iop ? "darkroom/ui/scroll_to_module" :
    ...
    if (move)
    {
        ...
        const int remaining =
            GPOINTER_TO_INT(user_data) - gdk_frame_clock_get_frame_time(frame_clock);
...
static void _expander_resize(GtkWidget *widget, GdkRectangle *allocation, gpointer user_data)
{
    ...
    {
        const gboolean is_lib_gui_module =
            darktable.lib->gui_module && darktable.lib->gui_module->expander == widget;
        const gboolean last_target_is_iop =
            _last_expanded && !g_strcmp0("iop-expander", gtk_widget_get_name(_last_expanded));
        ...
        if (_last_expanded)
        {
            ...
            if (widget != _last_expanded && !(is_lib_gui_module && !last_target_is_iop))
                return;
                                   ...
                                   guint time)
{
    ...
    if (!widget || (!allow && !below && widget == _drop_widget && time == last_time))
        return;
...
static void _expander_drag_leave(GtkDarktableExpander *widget, GdkDragContext *dc, guint time,
                                 gpointer user_data)
{
