...
{
    DT_THUMBNAIL_SEL_MODE_NORMAL =
        0, // user can change selection with normal mouse click (+CTRL or +SHIFT)
...
typedef struct
{
    ...
    int img_width,
        img_height; // current image size (can be greater than the image box in case of zoom)
    ...
    gboolean
        zoom_preview_pending; // TRUE during active gesture: draw stale surface scaled, defer surface reload
    ...
    GtkWidget
        *w_bottom_eb;    // GtkEventBox -- background of the bottom infos area (contains w_bottom)
