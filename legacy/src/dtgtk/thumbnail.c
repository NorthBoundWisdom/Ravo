...
static void _image_get_infos(dt_thumbnail_t *thumb)
{
    ...
    if (img)
    {
        ...
        thumb->rating =
            img->flags & DT_IMAGE_REJECTED ? DT_VIEW_REJECT : (img->flags & DT_VIEW_RATINGS_MASK);
...
static void _thumb_set_image_size(dt_thumbnail_t *thumb, const int image_w, const int image_h)
{
    int imgbox_w = 0;
    int imgbox_h = 0;
...
{
    if (!thumb->zoomable || thumb->over != DT_THUMBNAIL_OVERLAYS_HOVER_BLOCK || !thumb->w_zoom)
        return;
...
{
    if (thumb->img_margin)
        gtk_border_free(thumb->img_margin);
...
static float _zoom_ratio_to_thumb_zoom(const float zoom_ratio, const float zoom_100)
{
...
static float _thumb_zoom_to_zoom_ratio(const float zoom, const float zoom_100)
{
...
static void _get_dimensions_for_img_to_fit(const dt_thumbnail_t *thumb, const int max_width,
                                           const int max_height, float *width, float *height)
{
    float iw = max_width;
    float ih = max_height;
...
static void _get_dimensions_for_zoomed_img(dt_thumbnail_t *thumb, const int max_width,
                                           const int max_height, const float zoom_ratio,
...
static void _thumb_set_image_area(dt_thumbnail_t *thumb, const float zoom_ratio)
{
    ...
    else
    {
        ...
        if (scale < 1.0f)
            gtk_widget_set_size_request(thumb->w_image, wi * scale, hi * scale);
    }
...
static void _thumb_update_image_box(dt_thumbnail_t *thumb)
{
    ...
    const float scale = (thumb->img_surf_zoom > 0.0f) ? thumb->zoom / thumb->img_surf_zoom : 1.0f;
    ...
    const int imgbox_w = MIN(image_w, (int)(thumb->img_width * scale / darktable.gui->ppd_thb));
    const int imgbox_h = MIN(image_h, (int)(thumb->img_height * scale / darktable.gui->ppd_thb));
    ...
    thumb->zoomx = CLAMP(
        thumb->zoomx,
        (nwi * darktable.gui->ppd_thb - thumb->img_width * scale) / darktable.gui->ppd_thb, 0);
    thumb->zoomy = CLAMP(
        thumb->zoomy,
        (nhi * darktable.gui->ppd_thb - thumb->img_height * scale) / darktable.gui->ppd_thb, 0);
}
...
static gboolean _event_image_draw(GtkWidget *widget, cairo_t *cr, gpointer user_data)
{
    ...
    if (thumb->img_surf_preview &&
        (dt_view_get_current() != DT_VIEW_DARKROOM || !dev->preview_pipe->backbuf ||
    ...
    if (!thumb->img_surf || thumb->img_surf_dirty)
    {
        ...
        if (dt_view_get_current() == DT_VIEW_DARKROOM &&
            dev->preview_pipe->output_imgid == thumb->imgid && dev->preview_pipe->backbuf)
        {
            ...
            cairo_surface_t *tmp_surface = cairo_image_surface_create_for_data(
                rgbbuf, CAIRO_FORMAT_RGB24, buf_width, buf_height, stride);

            // copy preview image into final surface
            if (tmp_surface)
            {
                float scale = fminf(image_w / (float)buf_width, image_h / (float)buf_height) *
        ...
        else
        {
            ...
            if (thumb->zoomable)
            {
                ...
                res = dt_view_image_get_surface_cached(
                    thumb->imgid, image_w * thumb->zoom, image_h * thumb->zoom, &img_surf, FALSE,
                    &thumb->img_surf_mip_native, &thumb->img_surf_mip_level);
            }
            else
            {
        ...
        if (res == DT_VIEW_SURFACE_OK && thumb->display_focus)
        {
            ...
            if (!dt_imageio_large_thumbnail(path, &full_res_thumb, &full_res_thumb_wd,
                                            &full_res_thumb_ht, &color_space))
            {
                // we look for focus areas
                dt_focus_cluster_t full_res_focus[49];
                const int frows = 5;
                const int fcols = 5;
                dt_focus_create_clusters(full_res_focus, frows, fcols, full_res_thumb,
                ...
                dt_focus_draw_clusters(cri, cairo_image_surface_get_width(thumb->img_surf),
                                       ...
                                       thumb->imgid, full_res_thumb_wd, full_res_thumb_ht,
                                       full_res_focus, frows, fcols, 1.0, 0, 0);
                cairo_destroy(cri);
            }
...
static void _thumbs_show_overlays(dt_thumbnail_t *thumb)
{
    ...
    if (thumb->over == DT_THUMBNAIL_OVERLAYS_HOVER_BLOCK)
    {
        ...
        {
            thumb->overlay_timeout_id = g_timeout_add_seconds(thumb->overlay_timeout_duration,
...
static gboolean _event_grouping_release(GtkWidget *widget, GdkEventButton *event,
                                        gpointer user_data)
{
    dt_thumbnail_t *thumb = (dt_thumbnail_t *)user_data;
    if (thumb->disable_actions)
    ...
    if (event->button == GDK_BUTTON_PRIMARY)
    {
            ...
            dt_modifier_is(event->state, GDK_CONTROL_MASK))
        {
            ...
            DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                        ...
                                        " SELECT id FROM main.images WHERE group_id = ?1",
                                        -1, &stmt, NULL);
            DT_DEBUG_SQLITE3_BIND_INT(stmt, 1, thumb->groupid);
            sqlite3_step(stmt);
            sqlite3_finalize(stmt);
        }
        else if (!darktable.gui->grouping || thumb->groupid == darktable.gui->expanded_group_id)
        // the group is already expanded, so ...
        {
            if (thumb->imgid == darktable.gui->expanded_group_id && darktable.gui->grouping)
                // ... collapse it
...
static void _dt_image_info_changed_callback(gpointer instance, const gpointer imgs,
                                            gpointer user_data)
{
    if (!user_data || !imgs)
        return;
...
static void _dt_collection_changed_callback(gpointer instance, dt_collection_change_t query_change,
                                            ...
                                            const gpointer imgs, const int next, gpointer user_data)
{
    if (!user_data || !imgs)
        return;
    dt_thumbnail_t *thumb = (dt_thumbnail_t *)user_data;
...
static void _dt_mipmaps_updated_callback(gpointer instance, const dt_imgid_t imgid,
                                         gpointer user_data)
{
    if (!user_data)
        return;
    dt_thumbnail_t *thumb = (dt_thumbnail_t *)user_data;
    if (dt_is_valid_imgid(imgid) && thumb->imgid != imgid)
        return;
...
static gboolean _event_box_enter_leave(GtkWidget *widget, GdkEventCrossing *event,
                                       gpointer user_data)
{
    dt_thumbnail_t *thumb = (dt_thumbnail_t *)user_data;
    if (!thumb->hover_enabled)
...
static gboolean _event_image_enter_leave(GtkWidget *widget, GdkEventCrossing *event,
                                         gpointer user_data)
{
    dt_thumbnail_t *thumb = (dt_thumbnail_t *)user_data;
    if (!thumb->hover_enabled)
...
static gboolean _event_btn_enter_leave(GtkWidget *widget, GdkEventCrossing *event,
                                       gpointer user_data)
{
    dt_thumbnail_t *thumb = (dt_thumbnail_t *)user_data;
    if (!thumb->hover_enabled)
...
static gboolean _event_main_drag_motion(GtkWidget *widget, GdkDragContext *dc, const gint x,
                                        const gint y, const guint time, gpointer user_data)
{
...
static void _event_image_style_updated(GtkWidget *w, dt_thumbnail_t *thumb)
{
    ...
    if (oldt != thumb->img_margin->top || oldr != thumb->img_margin->right ||
        oldb != thumb->img_margin->bottom || oldl != thumb->img_margin->left)
    {
        _thumb_resize_overlays(thumb);
    }
}

GtkWidget *dt_thumbnail_create_widget(dt_thumbnail_t *thumb, const float zoom_ratio)
{
    // main widget (overlay)
    ...
    if (dt_is_valid_imgid(thumb->imgid))
    {
        ...
        gtk_drag_dest_set(thumb->w_main, GTK_DEST_DEFAULT_MOTION, target_list_all, n_targets_all,
                          GDK_ACTION_MOVE);
        ...
        DT_CONTROL_SIGNAL_CONNECT(DT_SIGNAL_ACTIVE_IMAGES_CHANGE, _dt_active_images_callback,
                                  thumb);
        DT_CONTROL_SIGNAL_CONNECT(DT_SIGNAL_SELECTION_CHANGED, _dt_selection_changed_callback,
                                  thumb);
        DT_CONTROL_SIGNAL_CONNECT(DT_SIGNAL_DEVELOP_MIPMAP_UPDATED, _dt_mipmaps_updated_callback,
                                  thumb);
        DT_CONTROL_SIGNAL_CONNECT(DT_SIGNAL_DEVELOP_PREVIEW_PIPE_FINISHED,
                                  _dt_preview_updated_callback, thumb);
        DT_CONTROL_SIGNAL_CONNECT(DT_SIGNAL_IMAGE_INFO_CHANGED, _dt_image_info_changed_callback,
                                  thumb);
        DT_CONTROL_SIGNAL_CONNECT(DT_SIGNAL_COLLECTION_CHANGED, _dt_collection_changed_callback,
                                  thumb);

        // the background
        thumb->w_back = gtk_event_box_new();
        gtk_widget_set_events(thumb->w_back, GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK |
        ...
        gtk_widget_set_events(evt_image, GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK |
                                             GDK_STRUCTURE_MASK | GDK_ENTER_NOTIFY_MASK |
                                             GDK_LEAVE_NOTIFY_MASK | GDK_POINTER_MOTION_MASK);
        g_signal_connect(G_OBJECT(evt_image), "motion-notify-event", G_CALLBACK(_event_main_motion),
                         thumb);
        ...
        gtk_widget_set_events(thumb->w_image, GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK |
                                                  GDK_STRUCTURE_MASK | GDK_ENTER_NOTIFY_MASK |
                                                  GDK_LEAVE_NOTIFY_MASK | GDK_POINTER_MOTION_MASK);
        ...
        g_signal_connect(G_OBJECT(thumb->w_bottom_eb), "enter-notify-event",
                         G_CALLBACK(_event_box_enter_leave), thumb);
        ...
        dt_action_define(&darktable.control->actions_thumb, NULL, "rating", thumb->w_reject,
                         &dt_action_def_rating);
        ...
        for (int i = 0; i < MAX_STARS; i++)
        {
            ...
            dt_action_define(&darktable.control->actions_thumb, NULL, "rating", thumb->w_stars[i],
                             &dt_action_def_rating);
        ...
        // the color labels
        thumb->w_color =
            dtgtk_thumbnail_btn_new(dtgtk_cairo_paint_label_flower, thumb->colorlabels, NULL);
        dt_action_define(&darktable.control->actions_thumb, NULL, N_("color label"), thumb->w_color,
...
dt_thumbnail_t *dt_thumbnail_new(const int width, const int height, const float zoom_ratio,
                                 const dt_imgid_t imgid, const int rowid,
                                 const dt_thumbnail_overlay_t over,
                                 const dt_thumbnail_container_t container, const gboolean tooltip,
...
void dt_thumbnail_destroy(dt_thumbnail_t *thumb)
{
    ...
    dt_thumbnail_surface_destroy(thumb);
    if (thumb->w_main)
        gtk_widget_destroy(thumb->w_main);
    if (thumb->filename)
...
static void _thumb_resize_overlays(dt_thumbnail_t *thumb)
{
    ...
    if (thumb->over != DT_THUMBNAIL_OVERLAYS_HOVER_BLOCK)
    {
        ...
        const float r1 =
            fminf(max_size / 2.0f, (width - thumb->img_margin->left - thumb->img_margin->right) /
                                       (2.5 * (4 + MAX_STARS)));
    ...
    else
    {
        ...
        for (int i = 0; i < MAX_STARS; i++)
        {
            ...
            gtk_widget_set_margin_start(thumb->w_stars[i], padding - icon_size / 8.0 + border_l +
                                                               r1 + (i + 1) * 3.0 * r1 + px);
        }
        // the color labels
        ...
        gtk_widget_set_margin_start(thumb->w_color, padding - icon_size / 8.0 + border_l +
                                                        2.0 * r1 + (MAX_STARS + 1) * 3.0 * r1 + px);
        // the local copy indicator
...
void dt_thumbnail_resize(dt_thumbnail_t *thumb, const int width, const int height,
                         const gboolean force, const float zoom_ratio)
{
    int w = 0;
    int h = 0;
    ...
    const int fsize =
        fminf(max_size, (height - thumb->img_margin->top - thumb->img_margin->bottom) / 11.0f);
...
void dt_thumbnail_set_group_border(dt_thumbnail_t *thumb, const dt_thumbnail_border_t border)
{
...
void dt_thumbnail_set_mouseover(dt_thumbnail_t *thumb, const gboolean over)
{
    if (thumb->mouse_over == over)
        return;
...
void dt_thumbnail_set_overlay(dt_thumbnail_t *thumb, const dt_thumbnail_overlay_t over,
                              const int timeout)
{
    if (!thumb)
        return;
...
void dt_thumbnail_image_refresh_position(dt_thumbnail_t *thumb)
{
    ...
    const float zr = (thumb->img_surf_zoom > 0.0f) ? thumb->zoom / thumb->img_surf_zoom : 1.0f;
    thumb->zoomx =
        CLAMP(thumb->zoomx,
              (iw * darktable.gui->ppd_thb - thumb->img_width * zr) / darktable.gui->ppd_thb, 0);
    thumb->zoomy =
        CLAMP(thumb->zoomy,
              (ih * darktable.gui->ppd_thb - thumb->img_height * zr) / darktable.gui->ppd_thb, 0);
    gtk_widget_queue_draw(thumb->w_main);
}
...
void dt_thumbnail_set_selection(dt_thumbnail_t *thumb, const gboolean selected)
{
