...
static gboolean _thumbnail_btn_draw(GtkWidget *widget, cairo_t *cr)
{
    ...
    if (gtk_widget_get_allocated_height(widget) < 2 || gtk_widget_get_allocated_width(widget) < 2)
        return TRUE;
    ...
    gtk_style_context_get(context, state, GTK_STYLE_PROPERTY_COLOR, &fg_color,
    ...
    if (DTGTK_THUMBNAIL_BTN(widget)->icon)
    {
        ...
        const float icon_w =
            allocation.width - (padding.left + padding.right) * allocation.width / 100.0f;
        const float icon_h =
            allocation.height - (padding.top + padding.bottom) * allocation.height / 100.0f;
        DTGTK_THUMBNAIL_BTN(widget)->icon(cr, icon_x, icon_y, icon_w, icon_h, flags,
                                          DTGTK_THUMBNAIL_BTN(widget)->icon_data ?
                                              DTGTK_THUMBNAIL_BTN(widget)->icon_data :
                                              bg_color);
    }
