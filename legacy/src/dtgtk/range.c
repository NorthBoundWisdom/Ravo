...
static void dtgtk_range_select_class_init(GtkDarktableRangeSelectClass *klass)
{
    ...
    _signals[VALUE_CHANGED] =
        g_signal_new("value-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, 0, NULL, NULL,
                     g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
    _signals[VALUE_RESET] =
        g_signal_new("value-reset", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, 0, NULL, NULL,
                     g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}
...
static void _date_tree_count_func(GtkTreeViewColumn *col, GtkCellRenderer *renderer,
...
static void _popup_date_recreate_model(GtkDarktableRangeSelect *range)
{
    ...
    for (const GList *bl = range->blocks; bl; bl = g_list_next(bl))
    {
        ...
        if (blo->txt)
        {
            ...
            gtk_tree_store_insert_with_values(GTK_TREE_STORE(model), &iter, NULL, nb_predefined,
                                              DATETIME_COL_TEXT, blo->txt, DATETIME_COL_TOOLTIP,
                                              tooltip, DATETIME_COL_PATH, path, DATETIME_COL_COUNT,
                                              0, DATETIME_COL_INDEX, index, -1);
            index++;
            nb_predefined++;
            g_free(tooltip);
            g_free(path);
        ...
        else
        {
            ...
            if (common_length == 0)
            {
                ...
                gtk_tree_store_insert_with_values(
                    GTK_TREE_STORE(model), &iter, NULL, nb_predefined, DATETIME_COL_TEXT, name,
                    DATETIME_COL_TOOLTIP, tooltip, DATETIME_COL_PATH, name, DATETIME_COL_COUNT, 0,
                    DATETIME_COL_INDEX, index, -1);
                index++;
                common_length++;
                parent = iter;
                g_free(name);
            ...
            if (common_length == 1)
            {
                ...
                gtk_tree_store_insert_with_values(
                    GTK_TREE_STORE(model), &iter, &parent, nb_predefined, DATETIME_COL_TEXT, name,
                    DATETIME_COL_TOOLTIP, tooltip, DATETIME_COL_PATH, path, DATETIME_COL_COUNT, 0,
                    DATETIME_COL_INDEX, index, -1);
                index++;
                common_length++;
                parent = iter;
            ...
            tooltip = g_date_time_format(dt, "%x %X");
            path = g_date_time_format(dt, "%Y:%m:%d %H:%M:%S");
            gtk_tree_store_insert_with_values(GTK_TREE_STORE(model), &iter, &parent, nb_predefined,
                                              DATETIME_COL_TEXT, name, DATETIME_COL_TOOLTIP,
                                              tooltip, DATETIME_COL_PATH, path, DATETIME_COL_COUNT,
                                              0, DATETIME_COL_INDEX, index, -1);
            index++;
            last_parent = iter;
            ...
            while (gtk_tree_model_iter_parent(model, &parent, &iter))
            {
                ...
                gtk_tree_store_set(GTK_TREE_STORE(model), &parent, DATETIME_COL_COUNT,
                                   blo->nb + parentcount, -1);
                iter = parent;
            }

            if (last_dt)
                g_date_time_unref(last_dt);
            last_dt = dt;
        }
    }
    if (last_dt)
...
static void _entry_set_tooltip(GtkWidget *entry, const _range_bound bound,
                               const dt_range_type_t range_type)
{
    ...
    else if (range_type == DT_RANGE_TYPE_DATETIME && bound == BOUND_MIN)
    {
        gtk_widget_set_tooltip_text(
            entry, _("enter the minimal date\n"
    ...
    {
        gtk_widget_set_tooltip_text(
            entry, _("enter the maximal date\n"
                     "in the form YYYY:MM:DD hh:mm:ss.sss (only the year is mandatory)\n"
                     "use 'max' if no bound\n"
...
static void _popup_date_update_widget_visibility(GtkDarktableRangeSelect *range)
{
    ...
    if (type == 1)
    {
        ...
        {
            gtk_label_set_text(GTK_LABEL(pop->relative_label),
                               _("date-time interval to subtract from the max value"));
        }
        else
        {
            gtk_label_set_text(GTK_LABEL(pop->relative_label),
                               _("date-time interval to add to the min value"));
        }
    }
    ...
    gtk_widget_set_visible(pop->now_btn, gtk_popover_get_default_widget(GTK_POPOVER(pop->popup)) ==
                                             range->entry_max);
}
...
static void _popup_date_update(GtkDarktableRangeSelect *range, GtkWidget *w)
{
    ...
    if (w == range->entry_min || w == range->entry_max)
    ...
    // update the calendar
    gtk_calendar_select_month(GTK_CALENDAR(pop->calendar), g_date_time_get_month(dt) - 1,
    ...
    {
        snprintf(tx, sizeof(tx), "%s%04d:%02d:%02d %02d:%02d:%02d",
                 (w == range->entry_max) ? "+" : "-", range->select_relative_date_r.year,
...
static void _current_show_popup(GtkDarktableRangeSelect *range)
{
    ...
    if (range->cur_help)
        gtk_label_set_markup(GTK_LABEL(lb), range->cur_help);
    gtk_container_add(GTK_CONTAINER(range->cur_window), dt_gui_vbox(range->cur_label, lb));
    gtk_widget_show_all(range->cur_window);
}

static void _bound_change(GtkDarktableRangeSelect *range, const gchar *val,
                          const _range_bound bound)
{
    ...
    else if (range->type == DT_RANGE_TYPE_DATETIME && bound == BOUND_MIDDLE &&
             !g_strcmp0(txt, "now"))
    {
        range->bounds = DT_RANGE_BOUND_FIXED;
    ...
    else if (range->type == DT_RANGE_TYPE_DATETIME && bound == BOUND_MAX &&
             g_str_has_prefix(txt, "+") && !(range->bounds & DT_RANGE_BOUND_MIN_RELATIVE))
    {
        if (dt_datetime_exif_to_numbers_raw(&range->select_relative_date_r, txt + 1))
        {
            ...
            range->bounds &= ~DT_RANGE_BOUND_FIXED;
            range->bounds &= ~DT_RANGE_BOUND_MAX_NOW;
            range->select_max_r = dt_datetime_gtimespan_add_numbers(
                range->select_min_r, range->select_relative_date_r, TRUE);
        }
    }
    else if (range->type == DT_RANGE_TYPE_DATETIME && bound == BOUND_MIN &&
             g_str_has_prefix(txt, "-") && !(range->bounds & DT_RANGE_BOUND_MAX_RELATIVE))
    {
        if (dt_datetime_exif_to_numbers_raw(&range->select_relative_date_r, txt + 1))
        {
            range->bounds &= ~DT_RANGE_BOUND_MIN;
            range->bounds |= DT_RANGE_BOUND_MIN_RELATIVE;
            range->bounds &= ~DT_RANGE_BOUND_FIXED;
            range->select_min_r = dt_datetime_gtimespan_add_numbers(
                range->select_max_r, range->select_relative_date_r, FALSE);
        }
    }
    else
    {
    ...
    dtgtk_range_select_set_selection(range, range->bounds, range->select_min_r, range->select_max_r,
                                     TRUE, FALSE);
}

static void _popup_date_ok_clicked(GtkWidget *w, GtkDarktableRangeSelect *range)
{
    if (!range->date_popup || range->date_popup->internal_change)
        return;
    _range_date_popup *pop = range->date_popup;
...
static void _popup_date_tree_row_activated(GtkTreeView *self, GtkTreePath *path,
...
static void _popup_date_tree_selection_change(GtkTreeView *self, GtkDarktableRangeSelect *range)
{
    ...
    gchar *text = NULL;
    GtkTreeModel *model = gtk_tree_view_get_model(GTK_TREE_VIEW(pop->treeview));
    if (!gtk_tree_selection_get_selected(gtk_tree_view_get_selection(GTK_TREE_VIEW(pop->treeview)),
                                         NULL, &iter))
        return;
...
static void _popup_date_changed(GtkWidget *w, GtkDarktableRangeSelect *range)
{
    ...
    if (w == pop->calendar)
    {
        ...
        if (gtk_popover_get_default_widget(GTK_POPOVER(pop->popup)) == range->entry_max && h == 0 &&
            min == 0 && s == 0)
        {
            h = 23;
            min = 59;
...
static void _popup_date_init(GtkDarktableRangeSelect *range)
{
    ...
    g_signal_connect(G_OBJECT(pop->type), "value-changed", G_CALLBACK(_popup_date_type_changed),
                     range);
    ...
    g_signal_connect(G_OBJECT(pop->calendar), "day_selected", G_CALLBACK(_popup_date_changed),
                     range);
    ...
    // the treeview
    GtkTreeModel *model =
        GTK_TREE_MODEL(gtk_tree_store_new(DATETIME_NUM_COLS, G_TYPE_STRING, G_TYPE_UINT,
    ...
    g_signal_connect(G_OBJECT(pop->treeview), "row-activated",
                     G_CALLBACK(_popup_date_tree_row_activated), range);
    ...
    GtkWidget *time = dt_gui_hbox(pop->hours, gtk_label_new(" : "), pop->minutes,
                                  gtk_label_new(" : "), pop->seconds);
    gtk_widget_set_halign(time, GTK_ALIGN_CENTER);
    GtkWidget *calendar_and_tree =
        dt_gui_hbox(dt_gui_vbox(pop->type, pop->relative_label, lb_date, pop->calendar,
                                pop->relative_date_box, lb_time, time),
                    dt_gui_scroll_wrap(pop->treeview));
    gtk_box_set_homogeneous(GTK_BOX(calendar_and_tree), TRUE);
    GtkWidget *vbox =
        dt_gui_vbox(calendar_and_tree, dt_gui_hbox(gtk_label_new(_("current date: ")),
                                                   pop->selection, pop->now_btn, pop->ok_btn));
...
static void _popup_item_activate(GtkWidget *w, gpointer user_data)
{
    ...
    else if (source == range->entry_min)
    {
        if (range->bounds & DT_RANGE_BOUND_MIN)
            range->bounds &= ~DT_RANGE_BOUND_MIN;
        dtgtk_range_select_set_selection(range, range->bounds, blo->value_r, range->select_max_r,
                                         TRUE, FALSE);
    }
    else if (source == range->entry_max)
    {
        if (range->bounds & DT_RANGE_BOUND_MAX)
            range->bounds &= ~DT_RANGE_BOUND_MAX;
        dtgtk_range_select_set_selection(range, range->bounds, range->select_min_r, blo->value_r,
                                         TRUE, FALSE);
    }
}
...
static GtkWidget *_popup_get_numeric_menu(GtkDarktableRangeSelect *range, GtkWidget *w)
{
    ...
    for (const GList *bl = range->blocks; bl; bl = g_list_next(bl))
    {
        ...
        if (!blo->txt)
            continue;
        // we don't allow range block on entries
        if (w != range->band && blo->bounds != DT_RANGE_BOUND_FIXED)
            continue;
        // for entries, we only values that won't change the min-max order
        if (w == range->entry_min && !(range->bounds & DT_RANGE_BOUND_MAX) &&
            blo->value_r > range->select_max_r)
            continue;
        if (w == range->entry_max && !(range->bounds & DT_RANGE_BOUND_MIN) &&
            blo->value_r < range->select_min_r)
            continue;

        gchar *txt = (blo->txt) ? g_strdup(blo->txt) : range->print(blo->value_r, TRUE);
        if (blo->nb > 0)
            dt_util_str_cat(&txt, " (%d)", blo->nb);
    ...
    if (nb > 0 && g_list_length(range->blocks) - nb > 0)
        gtk_menu_shell_append(pop, gtk_separator_menu_item_new());
...
static void _popup_show(GtkDarktableRangeSelect *range, GtkWidget *w)
{
    ...
    else if (range->type == DT_RANGE_TYPE_DATETIME)
    {
        ...
        GdkRectangle rect = {gtk_widget_get_allocated_width(w) / 2,
                             gtk_widget_get_allocated_height(w), 1, 1};
...
static double _graph_value_from_pos(GtkDarktableRangeSelect *range, const double posx,
                                    gboolean snap)
{
...
static void _range_set_source_rgba(cairo_t *cr, GtkWidget *w, double alpha,
...
static gboolean _event_band_draw(GtkWidget *widget, cairo_t *cr, gpointer user_data)
{
        ...
        range->alloc_main.height != allocation.height)
    {
        ...
        gtk_render_background(context, scr, range->alloc_margin.x, range->alloc_margin.y,
                              range->alloc_margin.width, range->alloc_margin.height);
        // draw border (defined in css)
        gtk_render_frame(context, scr, range->alloc_margin.x, range->alloc_margin.y,
                         range->alloc_margin.width, range->alloc_margin.height);
        ...
        for (const GList *bl = range->blocks; bl; bl = g_list_next(bl))
        {
            ...
            else
            {
                ...
                if (bl_count > 0)
                {
                    ...
                    const int bh =
                        _graph_get_height(bl_count, count_max, range->alloc_padding.height);
                    cairo_rectangle(scr, posx_px + range->alloc_padding.x,
                                    range->alloc_padding.y + range->alloc_padding.height - bh,
                                    bl_width_px, bh);
                    cairo_fill(scr);
                }
                bl_count = blo->nb;
        ...
        if (bl_count > 0)
        {
            ...
            cairo_rectangle(scr, posx_px + range->alloc_padding.x,
                            range->alloc_padding.y + range->alloc_padding.height - bh, bl_width_px,
                            bh);
            cairo_fill(scr);
        }
    ...
    double sel_max_r = (range->set_selection) ?
                           _graph_value_from_pos(range, range->current_x_px, TRUE) :
                           range->select_max_r;
    ...
    int sel_min_px =
        (range->bounds & DT_RANGE_BOUND_MIN) ? 0 : _graph_value_to_pos(range, sel_min_r);
    int sel_max_px = (range->bounds & DT_RANGE_BOUND_MAX) ? range->alloc_padding.width :
                                                            _graph_value_to_pos(range, sel_max_r);
    ...
    if (range->icons)
    {
        ...
        const int size =
            MIN(range->alloc_padding.height * 0.6, range->alloc_padding.width * min_percent / 100);
    ...
    if (range->mouse_inside && range->current_x_px > 0)
    {
        ...
        const int posx_px =
            _graph_snap_position(range, range->current_x_px) + range->alloc_padding.x;
...
static gboolean _event_band_motion(GtkWidget *widget, GdkEventMotion *event, gpointer user_data)
{
    ...
    const gboolean inside =
        (range->current_x_px >= 0 && range->current_x_px <= range->alloc_padding.width);
    if (!inside)
    {
    ...
    gint wx, wy;
    gtk_widget_translate_coordinates(range->band, gtk_widget_get_toplevel(range->band), 0, 0, &wx,
                                     &wy);
...
static gboolean _event_band_press(GtkWidget *w, GdkEventButton *e, gpointer user_data)
{
    ...
    {
        dtgtk_range_select_set_selection(range, DT_RANGE_BOUND_MIN | DT_RANGE_BOUND_MAX,
                                         range->min_r, range->max_r, TRUE, TRUE);
    }
    else if (e->button == GDK_BUTTON_PRIMARY)
    {
        if (!range->mouse_inside)
            return TRUE;
...
static gboolean _event_band_release(GtkWidget *w, GdkEventButton *e, gpointer user_data)
{
    ...
    else
    {
        ...
        if (range->select_min_r <= min_r)
            range->bounds |= DT_RANGE_BOUND_MIN;
        if (range->select_max_r >= max_r)
            range->bounds |= DT_RANGE_BOUND_MAX;
    }
...
// Public functions
GtkWidget *dtgtk_range_select_new(const gchar *property, const gboolean show_entries,
                                  const dt_range_type_t type)
{
    ...
    range->decode =
        (type == DT_RANGE_TYPE_NUMERIC) ? _default_decode_func : _default_decode_date_func;
    ...
    gtk_widget_set_events(range->band, GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK |
                                           GDK_STRUCTURE_MASK | GDK_LEAVE_NOTIFY_MASK |
                                           GDK_POINTER_MOTION_MASK);
    g_signal_connect(G_OBJECT(range->band), "draw", G_CALLBACK(_event_band_draw), range);
    g_signal_connect(G_OBJECT(range->band), "button-press-event", G_CALLBACK(_event_band_press),
                     range);
    g_signal_connect(G_OBJECT(range->band), "button-release-event", G_CALLBACK(_event_band_release),
                     range);
    g_signal_connect(G_OBJECT(range->band), "motion-notify-event", G_CALLBACK(_event_band_motion),
                     range);
    g_signal_connect(G_OBJECT(range->band), "leave-notify-event", G_CALLBACK(_event_band_leave),
                     range);
    ...
    // the boxes widgets
    GtkWidget *vbox = dt_gui_vbox(range->band, range->band_graph, range->band_selection,
                                  range->band_icons, range->band_cursor);
    ...
    if (range->show_entries)
    {
        ...
        g_signal_connect(G_OBJECT(range->entry_min), "activate", G_CALLBACK(_event_entry_activated),
                         range);
        g_signal_connect(G_OBJECT(range->entry_min), "focus-out-event",
                         G_CALLBACK(_event_entry_focus_out), range);
        g_signal_connect(G_OBJECT(range->entry_min), "button-press-event",
                         G_CALLBACK(_event_entry_press), range);
        ...
        g_signal_connect(G_OBJECT(range->entry_max), "activate", G_CALLBACK(_event_entry_activated),
                         range);
        g_signal_connect(G_OBJECT(range->entry_max), "focus-out-event",
                         G_CALLBACK(_event_entry_focus_out), range);
        g_signal_connect(G_OBJECT(range->entry_max), "button-press-event",
                         G_CALLBACK(_event_entry_press), range);

        dt_gui_box_add(
            vbox, dt_gui_hbox(dt_gui_expand(range->entry_min), dt_gui_expand(range->entry_max)));
    }

    gtk_container_add(GTK_CONTAINER(range), vbox);
    gtk_widget_set_name(vbox, "range-select");

    if (type == DT_RANGE_TYPE_DATETIME)
        _popup_date_init(range);
...
gchar *dtgtk_range_select_get_bounds_pretty(GtkDarktableRangeSelect *range)
{
    ...
    if (range->bounds & DT_RANGE_BOUND_MIN)
        ...
        txt = g_strdup_printf("-%04d:%02d:%02d %02d:%02d:%02d", range->select_relative_date_r.year,
                              range->select_relative_date_r.month,
                              range->select_relative_date_r.day, range->select_relative_date_r.hour,
                              range->select_relative_date_r.minute,
                              range->select_relative_date_r.second);
    else
...
void dtgtk_range_select_set_selection(GtkDarktableRangeSelect *range,
                                      const dt_range_bounds_t bounds, const double min_r,
                                      const double max_r, gboolean signal, gboolean round_values)
{
    ...
    if (range->show_entries)
    {
        ...
        if (range->bounds & DT_RANGE_BOUND_MIN)
            ...
            txt = g_strdup_printf(
                "-%04d:%02d:%02d %02d:%02d:%02d", range->select_relative_date_r.year,
                range->select_relative_date_r.month, range->select_relative_date_r.day,
                range->select_relative_date_r.hour, range->select_relative_date_r.minute,
                range->select_relative_date_r.second);
        else
    ...
    if (signal)
        g_signal_emit_by_name(G_OBJECT(range), "value-changed");
}

dt_range_bounds_t dtgtk_range_select_get_selection(GtkDarktableRangeSelect *range, double *min_r,
                                                   double *max_r)
{
...
void dtgtk_range_select_add_block(GtkDarktableRangeSelect *range, const double value_r,
                                  const int count)
{
...
void dtgtk_range_select_add_range_block(GtkDarktableRangeSelect *range, const double min_r,
                                        const double max_r, const dt_range_bounds_t bounds,
                                        gchar *txt, const int count)
{
    _range_block *block = g_malloc0(sizeof(_range_block));
    block->value_r = min_r;
...
void dtgtk_range_select_set_band_func(GtkDarktableRangeSelect *range,
...
void dtgtk_range_select_add_icon(GtkDarktableRangeSelect *range, const int posx,
                                 const double value_r, DTGTKCairoPaintIconFunc paint, gint flags,
                                 void *data)
{
...
void dtgtk_range_select_add_marker(GtkDarktableRangeSelect *range, const double value_r,
                                   const gboolean magnetic)
{
...
void dtgtk_range_select_set_selection_from_raw_text(GtkDarktableRangeSelect *range,
                                                    const gchar *txt, gboolean signal)
{
    double smin, smax;
    ...
    else
    {
        ...
        regex = g_regex_new(
            "^\\s*\\[\\s*([-+]?[0-9\\.\\s:]*[0-9]+)\\s*;\\s*((?:now)?[-+]?[0-9\\.\\s:]*)\\s*\\]\\s*$",
            0, 0, NULL);
