...
typedef struct dt_lib_modulegroups_t
{
    ...
    GtkWidget *presets_combo, *presets_btn_remove, *presets_btn_dup, *presets_btn_rename,
        *presets_btn_new;
...
*/
static gboolean _lib_modulegroups_test(dt_lib_module_t *self, uint32_t group,
                                       dt_iop_module_t *module);
...
static GtkWidget *_buttons_get_from_pos(dt_lib_module_t *self, const int pos)
{
    ...
    if (pos == DT_MODULEGROUP_ACTIVE_PIPE)
        return d->active_btn;
    if (pos == DT_MODULEGROUP_BASICS)
        return d->basic_btn;
...
static gboolean _lib_modulegroups_test_internal(dt_lib_module_t *self, uint32_t group,
                                                dt_iop_module_t *module)
{
    if (group == DT_MODULEGROUP_ACTIVE_PIPE)
    ...
    if (g_strv_length(elems) > 0)
    {
        ...
        if (item->widget && DT_IS_BAUHAUS_WIDGET(item->widget))
        {
            if (g_strv_length(elems) > 2)
                item->widget_name = g_strdup_printf("%s - %s", _(elems[1]),
                                                    dt_bauhaus_widget_get_label(item->widget));
...
static void _basics_remove_widget(dt_lib_modulegroups_basic_item_t *item)
{
        ...
        if (GTK_IS_CONTAINER(item->old_parent) && gtk_widget_get_parent(item->widget) == item->box)
        {
            ...
            if (GTK_IS_BOX(item->old_parent))
            {
                if (item->packtype == GTK_PACK_START)
                    gtk_box_pack_start(GTK_BOX(item->old_parent), item->widget, item->expand,
                                       item->fill, item->padding);
                else
                    gtk_box_pack_end(GTK_BOX(item->old_parent), item->widget, item->expand,
                                     item->fill, item->padding);
            ...
            {
                gtk_grid_attach(GTK_GRID(item->old_parent), item->widget, item->grid_x,
                                item->grid_y, item->grid_w, item->grid_h);
            }
    ...
    item->widget = NULL;
    if (item->box)
        gtk_widget_destroy(item->box);
    item->box = NULL;
    if (item->temp_widget)
        gtk_widget_destroy(item->temp_widget);
...
static gboolean _basics_on_off_label_callback(GtkWidget *widget, GdkEventButton *e,
                                              GtkToggleButton *btn)
{
...
static void _sync_visibility(GtkWidget *widget, GParamSpec *pspec,
...
static gboolean _manage_direct_module_popup(GtkWidget *widget, GdkEventButton *event,
                                            dt_lib_module_t *self);

static void _basics_add_widget(dt_lib_module_t *self, dt_lib_modulegroups_basic_item_t *item,
                               GtkWidget *w, dt_lib_modulegroups_basic_item_position_t item_pos)
{
    dt_lib_modulegroups_t *d = self->data;
    ...
    if (item->widget_type == WIDGET_TYPE_ACTIVATE_BTN)
    {
        ...
        if (compact_ui)
        {
            ...
            item->tooltip = gtk_widget_get_tooltip_text(
                item->widget); // no need to copy, returns a newly-alloced string
            ...
            GtkWidget *btn = dt_iop_gui_header_button(item->module, dtgtk_cairo_paint_switch,
                                                      DT_ACTION_ELEMENT_ENABLE, item->box);
            ...
            g_signal_connect(G_OBJECT(evb), "button-press-event",
                             G_CALLBACK(_basics_on_off_label_callback), btn);
    ...
    else
    {
        ...
        if (GTK_IS_BOX(gtk_widget_get_parent(w)))
        {
            ...
            gtk_box_query_child_packing(GTK_BOX(item->old_parent), item->widget, &item->expand,
                                        &item->fill, &item->padding, &item->packtype);
            gtk_container_child_get(GTK_CONTAINER(item->old_parent), item->widget, "position",
                                    &item->old_pos, NULL);
        }
        else if (GTK_IS_GRID(gtk_widget_get_parent(w)))
        {
            ...
            gtk_container_child_get(GTK_CONTAINER(item->old_parent), item->widget, "left-attach",
                                    &item->grid_x, "top-attach", &item->grid_y, "width",
                                    &item->grid_w, "height", &item->grid_h, NULL);
        }
        else
        {
        ...
        if (GTK_IS_CONTAINER(item->old_parent))
        {
            ...
            {
                if (item->packtype == GTK_PACK_START)
                    gtk_box_pack_start(GTK_BOX(item->old_parent), item->temp_widget, item->expand,
                                       item->fill, item->padding);
                else
                    gtk_box_pack_end(GTK_BOX(item->old_parent), item->temp_widget, item->expand,
                                     item->fill, item->padding);
            ...
            {
                gtk_grid_attach(GTK_GRID(item->old_parent), item->temp_widget, item->grid_x,
                                item->grid_y, item->grid_w, item->grid_h);
            }
        }
        ...
        g_signal_connect(G_OBJECT(item->temp_widget), "destroy", G_CALLBACK(gtk_widget_destroyed),
                         &item->temp_widget);
        g_signal_connect_swapped(G_OBJECT(item->temp_widget), "destroy",
                                 G_CALLBACK(_basics_remove_widget), item);
    ...
    if (item_pos != NORMAL)
    {
        ...
        GtkWidget *rbt =
            dt_iop_gui_header_button(item->module, dtgtk_cairo_paint_reset, DT_ACTION_ELEMENT_RESET,
                                     compact_ui ? hbox_basic : header_box);
        ...
        if (!compact_ui)
        {
            ...
            GtkWidget *btn = dt_iop_gui_header_button(item->module, dtgtk_cairo_paint_switch,
                                                      DT_ACTION_ELEMENT_ENABLE, header_box);
...
static void _basics_show(dt_lib_module_t *self)
{
    ...
    if (d->vbox_basic && gtk_widget_get_visible(d->vbox_basic))
        return;

    if (!d->vbox_basic)
    {
        d->vbox_basic = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);
        dt_ui_container_add_widget(darktable.gui->ui, DT_UI_CONTAINER_PANEL_RIGHT_CENTER,
                                   d->vbox_basic);
    }
    ...
    for (GList *modules = g_list_last(darktable.develop->iop); modules;
...
static uint32_t _lib_modulegroups_get_activated(dt_lib_module_t *self)
{
    ...
    if (bt && gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(bt)))
        return d->current;
...
static void _lib_modulegroups_update_iop_visibility(dt_lib_module_t *self)
{
    ...
    DT_ENTER_GUI_UPDATE();
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(d->basic_btn),
     ...
     */
        ...
        if (show_module)
        {
            if (darktable.develop->gui_module == module && !module->expanded)
                dt_iop_request_focus(NULL);
            if (w)
                gtk_widget_show(w);
        }
...
static void _lib_modulegroups_toggle(GtkWidget *button, dt_lib_module_t *self)
{
    ...
    if (button == d->basic_btn)
        gid = DT_MODULEGROUP_BASICS;
    ...
    if (d->current == gid && gid != DT_MODULEGROUP_BASICS &&
...
static gchar *_preset_retrieve_old_layout_updated()
{
    ...
    // show the search box ?
    if (_preset_retrieve_old_search_pref(&ret) == DT_MODULEGROUP_SEARCH_IOP_TEXT_VISIBLE)
        return ret;
        ...
        for (const GList *modules = darktable.iop; modules; modules = g_list_next(modules))
        {
            ...
            if (!dt_iop_so_is_hidden(module))
            {
                ...
                if ((i == 0 && fav && visi) || (i == 1 && (group & IOP_GROUP_TECHNICAL) && visi) ||
                    (i == 2 && (group & IOP_GROUP_GRADING) && visi) ||
                    (i == 3 && (group & IOP_GROUP_EFFECTS) && visi))
                {
...
static void _preset_from_string(dt_lib_module_t *self, gchar *txt, gboolean edition)
{
    ...
    if (g_strv_length(gr) > 0)
    {
        ...
        if (!g_strcmp0(gr2[0], "0"))
            show_search = FALSE;
        // do we show all history module in active group
        if (g_strv_length(gr2) > 1 && (g_strcmp0(gr2[1], "1") == 0))
            full_active = TRUE;
    ...
    if (g_strv_length(gr) > 1)
    {
        if (gr[1])
        {
            ...
            if (g_strv_length(gr2) > 3 && (g_strcmp0(gr2[0], "1") == 0))
                basics_show = TRUE;
            if (edition)
            ...
            {
                dt_lib_modulegroups_basic_item_t *item =
                    g_malloc0(sizeof(dt_lib_modulegroups_basic_item_t));
                if (!item)
                    continue;
...
#define SQA(is_scene_referred)                                                                     \
    {                                                                                              \
        ...
        if (!is_scene_referred)                                                                    \
            AM("colorbalancergb/contrast"); /* contrast is already in filmic/sigmoid */            \
...
void init_presets(dt_lib_module_t *self)
{
  ...
  */
    ...
    const gboolean wf_agx = dt_conf_is_equal("plugins/darkroom/workflow", "scene-referred (AgX)");
    const gboolean wf_none = dt_conf_is_equal("plugins/darkroom/workflow", "none");
    ...
    dt_lib_presets_add(_("workflow: scene-referred"), self->plugin_name, self->version(), tx,
                       strlen(tx), TRUE, 0);
    ...
    if (!dt_conf_key_exists("plugins/darkroom/modulegroups_preset"))
    {
        ...
        dt_lib_presets_add(_("previous config"), self->plugin_name, self->version(), tx, strlen(tx),
                           FALSE, 0);
        dt_conf_set_string("plugins/darkroom/modulegroups_preset", _("previous layout"));
        g_free(tx);

        tx = _preset_retrieve_old_layout_updated();
        dt_lib_presets_add(_("previous config with new layout"), self->plugin_name, self->version(),
                           tx, strlen(tx), FALSE, 0);
        g_free(tx);
    }
...
static void _manage_editor_groups_cleanup(dt_lib_module_t *self, const gboolean edition)
{
    dt_lib_modulegroups_t *d = self->data;
...
static void _manage_editor_basics_remove(GtkWidget *widget, dt_lib_module_t *self)
{
    dt_lib_modulegroups_t *d = self->data;
...
static int _manage_editor_module_find_multi(gconstpointer a, gconstpointer b)
{
    ...
    if (g_strcmp0(ma->op, mb->op) != 0)
        return 1;
    if (ma->multi_priority >= mb->multi_priority)
        return 0;
    return 1;
}
...
static void _manage_editor_basics_update_list(dt_lib_module_t *self)
{
    ...
    for (const GList *modules = g_list_last(darktable.develop->iop); modules;
         modules = g_list_previous(modules))
    {
        dt_iop_module_t *module = modules->data;
...
static void _manage_editor_save(dt_lib_module_t *self)
{
    ...
    dt_lib_presets_update(d->edit_preset, self->plugin_name, self->version(), d->edit_preset, "",
...
static void _manage_editor_module_remove(GtkWidget *widget, dt_lib_module_t *self)
{
                                              ...
                                              dt_lib_modulegroups_group_t *gr)
{
    ...
    // and we add the ones from the list
    for (GList *modules2 = g_list_last(darktable.develop->iop); modules2;
...
static void _manage_direct_save(dt_lib_module_t *self)
{
    ...
    // update the preset in the database
    dt_lib_presets_add(_(CURRENT_PRESET_NAME), self->plugin_name, self->version(), params,
                       strlen(params), FALSE, 0);
    g_free(params);
    ...
    if (!dt_lib_presets_apply(_(CURRENT_PRESET_NAME), self->plugin_name, self->version()))
        dt_lib_presets_apply((gchar *)C_("modulegroup", FALLBACK_PRESET_NAME), self->plugin_name,
                             self->version());
}
...
static int _lib_modulegroups_basics_module_toggle_action(dt_lib_module_t *self, dt_action_t *action,
                                                         const gboolean doit)
{
    dt_lib_modulegroups_t *d = self->data;
...
static int _lib_modulegroups_basics_module_toggle(dt_lib_module_t *self, GtkWidget *widget,
                                                  const gboolean doit)
{
...
static GtkWidget *_build_menu_from_actions(dt_action_t *actions, dt_lib_module_t *self,
                                           ...
                                           const gboolean full_menu, int *num_selected)
{
    ...
    while (actions)
    {
        ...
        if (actions->type >= DT_ACTION_TYPE_SECTION &&
            !on_off) // not an iop module or blending (CATEGORY)
        {
        ...
        {
            if (actions->type <= DT_ACTION_TYPE_SECTION)
                new_sub = _build_menu_from_actions(actions->target, self, on_off, base_menu,
                                                   full_menu, num_selected);

            if (new_sub || (actions->type >= DT_ACTION_TYPE_WIDGET && actions->target &&
        ...
        if (item)
        {
            ...
            else
            {
                ...
                gchar *action_id = _action_id(action);
                if (g_list_find_custom(full_menu ? d->basics : d->edit_basics, action_id,
                                       _basics_item_find))
                {
...
static void _manage_basics_add_popup(GtkWidget *widget, dt_lib_module_t *self,
                                     const gboolean full_menu)
{
    ...
    GtkWidget *all_modules = _build_menu_from_actions(darktable.control->actions_iops.target, self,
                                                      NULL, pop, full_menu, &nba);
...
static void _manage_editor_basics_add_popup(GtkWidget *widget, dt_lib_module_t *self)
{
...
static gboolean _manage_direct_popup(GtkWidget *widget, GdkEventButton *event,
                                     dt_lib_module_t *self)
{
...
static gboolean _manage_direct_basic_popup(GtkWidget *widget, GdkEventButton *event,
                                           dt_lib_module_t *self)
{
    if (event->type == GDK_BUTTON_PRESS && event->button == GDK_BUTTON_SECONDARY)
    {
                                            ...
                                            dt_lib_module_t *self)
{
    ...
    if (event->type == GDK_BUTTON_PRESS && event->button == GDK_BUTTON_SECONDARY)
    {
        ...
        GtkWidget *this_module =
            _build_menu_from_actions(module->target, self, NULL, pop, TRUE, &nba);
                                            ...
                                            dt_lib_module_t *self)
{
    if (event->type == GDK_BUTTON_PRESS && event->button == GDK_BUTTON_SECONDARY)
    {
        ...
        gtk_widget_set_tooltip_text(smt,
...
static gboolean _scroll_group_buttons(GtkWidget *widget, GdkEventScroll *event,
                                      dt_lib_module_t *self)
{
    dt_lib_modulegroups_t *d = self->data;
    int delta;
    if (dt_gui_get_scroll_unit_delta(event, &delta))
    {
        ...
        if (adjacent)
            gtk_button_clicked(GTK_BUTTON(adjacent));
    }
...
static void _cycle_module_groups(const gboolean down, dt_lib_module_t *self)
{
    ...
    if (down)
    {
        if (d->current == DT_MODULEGROUP_BASICS)
            ...
            next =
                ngroups > 0 ? _buttons_get_from_pos(self, 1) : (has_basics ? d->basic_btn : NULL);
        else
        {
    ...
    else
    {
        if (d->current == DT_MODULEGROUP_BASICS)
            ...
            next = has_basics ? d->basic_btn :
                                (ngroups > 0 ? _buttons_get_from_pos(self, ngroups) : NULL);
        else if (d->current == 1)
            next = d->active_btn;
        else
            next = _buttons_get_from_pos(self, d->current - 1);
...
static float _action_callback_cycle_module_groups(gpointer target, dt_action_element_t element,
...
static const gchar *_action_effect_cycle_module_groups[] = {N_("toggle"), N_("previous"),
                                                            N_("next"), NULL};
...
static const dt_action_def_t _action_def_cycle_module_groups = {
    ...
    _action_elements_cycle_module_groups, NULL, TRUE};
...
void gui_init(dt_lib_module_t *self)
{
    ...
    g_signal_connect(d->basic_btn, "button-press-event", G_CALLBACK(_manage_direct_basic_popup),
                     self);
    ...
    dt_action_define(DT_ACTION(self), NULL, N_("quick access panel"), d->basic_btn,
    ...
    g_signal_connect(d->active_btn, "button-press-event", G_CALLBACK(_manage_direct_active_popup),
                     self);
    ...
    // cycle module groups action
    dt_action_define(DT_ACTION(self), NULL, N_("cycle module groups"), NULL,
                     &_action_def_cycle_module_groups);
    ...
    g_signal_connect(self->presets_button, "button-press-event", G_CALLBACK(_presets_pressed),
                     self);

    /* search box */
    d->text_entry = gtk_search_entry_new();
    dt_action_define(&darktable.view_manager->proxy.darkroom.view->actions, NULL,
                     N_("search modules"), d->text_entry, &dt_action_def_entry);
    ...
    g_signal_connect(G_OBJECT(d->text_entry), "stop-search", G_CALLBACK(dt_gui_search_stop),
                     dt_ui_center(darktable.gui->ui));
    g_signal_connect_data(G_OBJECT(d->text_entry), "focus-in-event", G_CALLBACK(gtk_widget_show),
                          d->hbox_search_box, NULL, G_CONNECT_AFTER | G_CONNECT_SWAPPED);

    GtkWidget *visibility_wrapper =
        gtk_event_box_new(); // extra layer prevents disabling shortcuts when hidden
    ...
    gtk_entry_set_icon_tooltip_text(GTK_ENTRY(d->text_entry), GTK_ENTRY_ICON_SECONDARY,
                                    _("clear text"));
   ...
   */
    ...
    darktable.develop->proxy.modulegroups.update_visibility =
    ...
    darktable.develop->proxy.modulegroups.basics_module_toggle =
        _lib_modulegroups_basics_module_toggle;
...
static void _manage_editor_group_move_right(GtkWidget *widget, dt_lib_module_t *self)
{
    dt_lib_modulegroups_t *d = self->data;
    dt_lib_modulegroups_group_t *gr = g_object_get_data(G_OBJECT(widget), "group");
    ...
    if (pos < 0 || pos >= g_list_length(d->edit_groups) - 1)
        return;
...
static void _manage_editor_group_move_left(GtkWidget *widget, dt_lib_module_t *self)
{
    dt_lib_modulegroups_t *d = self->data;
    dt_lib_modulegroups_group_t *gr = g_object_get_data(G_OBJECT(widget), "group");
    GtkWidget *vb = gtk_widget_get_parent(gtk_widget_get_parent(widget));
...
static void _manage_editor_group_remove(GtkWidget *widget, dt_lib_module_t *self)
{
    dt_lib_modulegroups_t *d = self->data;
    // we don't allow to remove the last group if no quick access or searchbox
    if (g_list_is_singleton(d->edit_groups) && !d->edit_basics_show && !d->edit_show_search)
    {
        return;
    }
    dt_lib_modulegroups_group_t *gr = g_object_get_data(G_OBJECT(widget), "group");
...
static void _manage_editor_group_name_changed(GtkWidget *tb, GdkEventButton *event,
                                              dt_lib_module_t *self)
{
    dt_lib_modulegroups_group_t *gr =
        (dt_lib_modulegroups_group_t *)g_object_get_data(G_OBJECT(tb), "group");
    const gchar *txt = gtk_entry_get_text(GTK_ENTRY(tb));
    g_free(gr->name);
    gr->name = g_strdup(txt);
}

static gboolean _manage_editor_group_icon_changed(GtkWidget *widget, GdkEventButton *event,
                                                  dt_lib_modulegroups_group_t *gr)
{
...
static void _manage_editor_group_icon_popup(GtkWidget *btn, dt_lib_module_t *self)
{
...
static GtkWidget *_manage_editor_group_init_basics_box(dt_lib_module_t *self)
{
    ...
    if (!d->edit_ro)
    {
        ...
        g_signal_connect(G_OBJECT(bt), "clicked", G_CALLBACK(_manage_editor_basics_add_popup),
                         self);
                                                        ...
                                                        dt_lib_modulegroups_group_t *gr)
{
    ...
    g_signal_connect(G_OBJECT(btn), "clicked", G_CALLBACK(_manage_editor_group_icon_popup), self);
    g_object_set_data(G_OBJECT(btn), "group", gr);
    gtk_box_pack_start(GTK_BOX(hb3), btn, FALSE, TRUE, 0);
    ...
    if (!d->edit_ro)
    {
        ...
        g_object_set_data(G_OBJECT(btn), "group", gr);
        g_signal_connect(G_OBJECT(btn), "clicked", G_CALLBACK(_manage_editor_group_remove), self);
    ...
    if (!d->edit_ro)
    {
        ...
        g_object_set_data(G_OBJECT(btn), "group", gr);
        g_signal_connect(G_OBJECT(btn), "clicked", G_CALLBACK(_manage_editor_group_move_left),
                         self);
        ...
        gtk_widget_set_name(bt, "modulegroups-btn");
        g_object_set_data(G_OBJECT(bt), "group", gr);
        g_signal_connect(G_OBJECT(bt), "clicked", G_CALLBACK(_manage_editor_module_add_popup),
                         self);
        ...
        g_object_set_data(G_OBJECT(btn), "group", gr);
        g_signal_connect(G_OBJECT(btn), "clicked", G_CALLBACK(_manage_editor_group_move_right),
                         self);
...
static void _manage_editor_reset(GtkWidget *widget, dt_lib_module_t *self)
{
    dt_lib_modulegroups_t *d = self->data;
...
static void _manage_editor_group_add(GtkWidget *widget, dt_lib_module_t *self)
{
    dt_lib_modulegroups_t *d = self->data;
...
static void _manage_editor_basics_toggle(GtkWidget *button, dt_lib_module_t *self)
{
    ...
    if (!state && g_list_is_empty(d->edit_groups) && !d->edit_show_search)
    {
        d->editor_reset = TRUE;
...
static void _manage_editor_search_toggle(GtkWidget *button, dt_lib_module_t *self)
{
    ...
    if (!state && g_list_is_empty(d->edit_groups) && !d->edit_basics_show)
    {
        d->editor_reset = TRUE;
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(button), TRUE);
        d->editor_reset = FALSE;
...
static void _manage_editor_preset_name_verify(GtkWidget *tb, gpointer params[])
{
...
static void _manage_editor_preset_action(GtkWidget *btn, dt_lib_module_t *self)
{
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
    ...
    GtkWidget *dialog = gtk_dialog_new_with_buttons(
        _("rename preset"), GTK_WINDOW(d->dialog), GTK_DIALOG_DESTROY_WITH_PARENT, _("_cancel"),
        GTK_RESPONSE_CANCEL, _("_rename"), GTK_RESPONSE_OK, NULL);
    ...
    gtk_entry_set_width_chars(GTK_ENTRY(tb),
                              10 + g_utf8_strlen(gtk_window_get_title(GTK_WINDOW(dialog)), -1));
    gpointer verify_params[] = {dialog, names, lb};
    g_signal_connect(G_OBJECT(tb), "changed", G_CALLBACK(_manage_editor_preset_name_verify),
                     verify_params);
    ...
    if (res == GTK_RESPONSE_OK)
    {
      ...
      DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                  ...
                                  -1, &stmt, NULL);
            // clang-format on
            DT_DEBUG_SQLITE3_BIND_TEXT(stmt, 1, gtk_entry_get_text(GTK_ENTRY(tb)), -1,
                                       SQLITE_TRANSIENT);
        ...
        else if (btn == d->presets_btn_new)
        {
            ...
            dt_lib_presets_add(gtk_entry_get_text(GTK_ENTRY(tb)), self->plugin_name,
                               self->version(), tx, strlen(tx), FALSE, 0);
            g_free(tx);
...
static void _preset_autoapply_edit(GtkButton *button, dt_lib_module_t *self)
{
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        sqlite3_finalize(stmt);
        dt_gui_presets_show_edit_dialog(d->edit_preset, rowid,
                                        G_CALLBACK(_preset_autoapply_changed), self, FALSE, FALSE,
...
static void _manage_editor_load(const char *preset, dt_lib_module_t *self)
{
    ...
    if (preset)
        sel_ok = gtk_combo_box_set_active_id(GTK_COMBO_BOX(d->presets_combo), preset);
    if (!sel_ok)
        gtk_combo_box_set_active(GTK_COMBO_BOX(d->presets_combo), 0);
    const gchar *sel_preset = gtk_combo_box_get_active_id(GTK_COMBO_BOX(d->presets_combo));

    // get all presets groups
    if (d->edit_groups)
        _manage_editor_groups_cleanup(self, TRUE);
    if (d->edit_preset)
        g_free(d->edit_preset);
    d->edit_groups = NULL;
    d->edit_preset = NULL;
...
static void _manage_show_window(dt_lib_module_t *self)
{
    ...
    d->dialog =
        gtk_dialog_new_with_buttons(_("manage module layouts"), win,
    ...
    g_signal_connect(G_OBJECT(d->presets_combo), "changed", G_CALLBACK(_manage_preset_change),
                     self);
    ...
    d->presets_btn_remove = dt_action_button_new(NULL, N_("remove"), _manage_preset_delete, self,
                                                 _("remove the preset"), 0, 0);
    gtk_box_pack_start(GTK_BOX(hb2), d->presets_btn_remove, TRUE, TRUE, 2);
    d->presets_btn_dup = dt_action_button_new(NULL, N_("duplicate"), _manage_editor_preset_action,
                                              self, _("duplicate the preset"), 0, 0);
    gtk_box_pack_start(GTK_BOX(hb2), d->presets_btn_dup, TRUE, TRUE, 2);
    d->presets_btn_rename = dt_action_button_new(NULL, N_("rename"), _manage_editor_preset_action,
                                                 self, _("rename the preset"), 0, 0);
    gtk_box_pack_start(GTK_BOX(hb2), d->presets_btn_rename, TRUE, TRUE, 2);
    d->presets_btn_new = dt_action_button_new(NULL, N_("new"), _manage_editor_preset_action, self,
                                              _("create a new empty preset"), 0, 0);
    ...
    gtk_widget_set_tooltip_text(d->edit_full_active_cb,
                                _("show modules that are present in the history stack,"
                                  " regardless of whether or not they are currently enabled"));
    g_signal_connect(G_OBJECT(d->edit_full_active_cb), "toggled",
                     G_CALLBACK(_manage_editor_full_active_toggle), self);
    ...
    gtk_widget_set_sensitive(d->edit_autoapply_chkbox,
                             FALSE); // always readonly. change are done with the button...
    ...
    g_signal_connect(G_OBJECT(d->edit_autoapply_btn), "clicked", G_CALLBACK(_preset_autoapply_edit),
                     self);
    ...
    d->preset_btn_add_group =
        dtgtk_button_new(dtgtk_cairo_paint_square_plus, CPF_DIRECTION_LEFT, NULL);
    ...
    // read only message
    d->preset_read_only_label = gtk_label_new(_("this is a built-in read-only preset."
    ...
    g_signal_connect(G_OBJECT(d->preset_reset_btn), "clicked", G_CALLBACK(_manage_editor_reset),
                     self);
