...
static void _update_images_number(dt_lib_module_t *self, const guint nb_sel);
...
static const char *const _import_text[] = {
    [DT_IMPORT_INPLACE] = N_("add to library"),
    [DT_IMPORT_COPY] = N_("copy & import"),
};
...
static void _thumb_set_in_listview(GtkTreeModel *model, GtkTreeIter *iter, const gboolean thumb_sel,
                                   dt_lib_module_t *self)
{
    dt_lib_import_t *d = self->data;
    gchar *filename;
    gchar *fullname;
    gtk_tree_model_get(model, iter, DT_IMPORT_UI_FILENAME, &filename, DT_IMPORT_FILENAME, &fullname,
                       -1);
    GdkPixbuf *pixbuf = NULL;
    {
        pixbuf = thumb_sel ? _import_get_thumbnail(fullname) : d->from.eye;
    }
    gtk_list_store_set(d->from.store, iter, DT_IMPORT_SEL_THUMB, thumb_sel, DT_IMPORT_THUMB, pixbuf,
                       -1);
...
static void _all_thumb_toggled(GtkTreeViewColumn *column, dt_lib_module_t *self)
{
    ...
    else if (!d->from.event)
    {
        ...
        if (gtk_tree_model_get_iter_first(model, &d->from.iter))
            d->from.event =
                g_timeout_add_full(G_PRIORITY_LOW, 100, (GSourceFunc)_thumb_set, self, NULL);
    }
}
...
static void _import_set_file_list(const gchar *folder, dt_lib_module_t *self);
...
static void _import_active(dt_lib_module_t *self, const gboolean active, const uint32_t nb_sel)
{
    dt_lib_import_t *d = self->data;
    ...
    if (!active || nb_sel > 0)
        gtk_dialog_set_response_sensitive(GTK_DIALOG(d->from.dialog), GTK_RESPONSE_ACCEPT, active);
}

static void _import_add_file_callback(GObject *direnum, GAsyncResult *result, gpointer user_data)
{
    ...
    if (error)
    {
        ...
        if (error->code != G_IO_ERROR_CANCELLED)
            dt_print(DT_DEBUG_ALWAYS, "[_import_add_file_callback] error: %s", error->message);
        g_error_free(error);
        return;
    }
    ...
    {
        g_file_enumerator_next_files_async(G_FILE_ENUMERATOR(direnum), FILE_REQUEST_BLOCK,
                                           G_PRIORITY_LOW, d->cancel_iter,
                                           _import_add_file_callback, user_data);
        ...
        for (GList *node = file_list; node; node = node->next)
        {
         ...
         check if the filename starts with a '.' */
            ...
            else if (filetype != G_FILE_TYPE_DIRECTORY && dt_supported_image(filename))
            {
                ...
                if (include_nonraws || (ext && ((dt_imageio_is_raw_by_extension(ext)) ||
             ...
             * fetched filmroll id */
                        if (filmroll_id != -1)
                            already_imported =
                                dt_image_get_id(filmroll_id, filename) == NO_IMGID ? FALSE : TRUE;
                    }
                    else
                    {
                    ...
                    gtk_list_store_insert_with_values(
                        d->from.store, NULL, -1, DT_IMPORT_UI_EXISTS, already_imported ? "✔" : " ",
                        DT_IMPORT_UI_FILENAME, &fullname[offset], DT_IMPORT_FILENAME, fullname,
                        DT_IMPORT_UI_DATETIME, dt_txt, DT_IMPORT_DATETIME, datetime,
...
void _import_enum_callback(GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    dt_lib_module_t *self = (dt_lib_module_t *)user_data;
    dt_lib_import_t *d = self->data;
    ...
    g_file_enumerator_next_files_async(dir_files, FILE_REQUEST_BLOCK, G_PRIORITY_LOW,
                                       d->cancel_iter, _import_add_file_callback, self);
}
...
static void _import_set_file_list(const gchar *folder, dt_lib_module_t *self)
{
    ...
    g_file_enumerate_children_async(
        ...
        G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS, G_PRIORITY_LOW, d->cancel_iter, _import_enum_callback,
        self);
}

static void _import_set_file_list_start(const gchar *folder, dt_lib_module_t *self)
{
    dt_lib_import_t *d = self->data;
...
static void _update_images_number(dt_lib_module_t *self, const guint nb_sel)
{
    ...
    snprintf(text, sizeof(text),
             ngettext("%d image out of %d selected", "%d images out of %d selected", nb_sel),
             nb_sel, d->from.nb);
...
static void _import_from_selection_changed(GtkTreeSelection *selection, dt_lib_module_t *self)
{
    ...
    if (!d->is_importing)
        gtk_dialog_set_response_sensitive(GTK_DIALOG(d->from.dialog), GTK_RESPONSE_ACCEPT,
                                          nb_sel ? TRUE : FALSE);
}
...
static void _expander_create(dt_gui_collapsible_section_t *cs, GtkBox *parent, const char *label,
                             const char *pref_key, dt_lib_module_t *self)
{
    dt_gui_new_collapsible_section(cs, pref_key, label, parent, DT_ACTION(self));
}

static gboolean _find_iter_folder(GtkTreeModel *model, GtkTreeIter *iter, const char *folder)
{
...
static void _get_folders_list(GtkTreeStore *store, GtkTreeIter *parent, const gchar *folder,
                              const char *selected)
{
    if (!folder[0])
        return;
    ...
    GError *error = NULL;
    GFile *gfolder = g_file_new_for_path(folder);
    GFileEnumerator *dir_files = g_file_enumerate_children(
        gfolder,
        G_FILE_ATTRIBUTE_STANDARD_NAME
        "," G_FILE_ATTRIBUTE_STANDARD_DISPLAY_NAME "," G_FILE_ATTRIBUTE_STANDARD_TYPE
        "," G_FILE_ATTRIBUTE_STANDARD_IS_HIDDEN "," G_FILE_ATTRIBUTE_ACCESS_CAN_READ,
    ...
    if (!parent)
    {
        ...
        gtk_tree_store_insert_with_values(store, &parent2, NULL, -1, DT_FOLDER_NAME, basename,
                                          DT_FOLDER_PATH, folder, DT_FOLDER_EXPANDED, FALSE, -1);
        // fake child
        gtk_tree_store_insert_with_values(store, &iter, &parent2, -1, DT_FOLDER_EXPANDED, FALSE,
                                          -1);
        g_free(basename);
    }
    else
        ...
        while ((info = g_file_enumerator_next_file(dir_files, NULL, &error)))
        {
            ...
            if (filetype == G_FILE_TYPE_DIRECTORY && !ishidden && canread)
            {
                ...
                if (!expanded)
                {
                    ...
                    gtk_tree_store_set(store, &iter, DT_FOLDER_NAME, basename, DT_FOLDER_PATH,
                                       fullname, DT_FOLDER_EXPANDED, FALSE, -1);
                    // fake child
...
static void _row_expanded(GtkTreeView *view, GtkTreeIter *iter, GtkTreePath *path,
                          dt_lib_module_t *self)
{
...
static void _paned_position_changed(GtkWidget *widget, GParamSpec *pspec, gpointer user_data)
{
    const gint position = gtk_paned_get_position(GTK_PANED(widget));
    dt_conf_set_int("ui_last/import_dialog_paned_pos", position);
}

static void _paned_places_position_changed(GtkWidget *widget, GParamSpec *pspec, gpointer user_data)
{
    const gint position = gtk_paned_get_position(GTK_PANED(widget));
...
static void _set_places_list(GtkWidget *places_paned, dt_lib_module_t *self)
{
    dt_lib_import_t *d = self->data;
    d->placesModel =
        gtk_list_store_new(DT_PLACES_NUM_COLS, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_INT);
    ...
    gtk_widget_set_tooltip_text(places_reset, _("restore all default places you have removed"));
    ...
    gtk_widget_set_tooltip_text(d->remove_place_button, _("remove the selected custom place"));
...
static void _set_folders_list(GtkWidget *places_paned, dt_lib_module_t *self)
{
    dt_lib_import_t *d = self->data;
    GtkTreeStore *store =
        gtk_tree_store_new(DT_FOLDER_NUM_COLS, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_BOOLEAN);
...
static void _update_places_list(dt_lib_module_t *self)
{
    ...
    if (dt_conf_get_bool("ui_last/import_dialog_show_home"))
    {
        ...
        if (homedir)
        {
            ...
            gtk_list_store_insert_with_values(d->placesModel, &iter, -1, DT_PLACES_NAME, _("home"),
                                              DT_PLACES_PATH, current_place, DT_PLACES_TYPE,
                                              DT_TYPE_HOME, -1);
    ...
    if (dt_conf_get_bool("ui_last/import_dialog_show_pictures") &&
            ...
            for (volume = volumes; volume; volume = volume->next)
            {
                ...
                if (placesMount)
                {
                    ...
                    gtk_list_store_insert_with_values(d->placesModel, &iter, -1, DT_PLACES_NAME,
                                                      volname, DT_PLACES_PATH, path, DT_PLACES_TYPE,
                                                      DT_TYPE_MOUNT, -1);
                    g_free(volname);
                    g_free(path);
    ...
    for (GList *places_iter = places; places_iter; places_iter = places_iter->next)
    {
        ...
        gtk_list_store_insert_with_values(d->placesModel, &iter, -1, DT_PLACES_NAME, basename,
                                          DT_PLACES_PATH, (char *)places_iter->data, DT_PLACES_TYPE,
                                          DT_TYPE_CUSTOM, -1);
        g_free(basename);
    ...
    g_free(last_place);
    // the list returned by _get_custom_places references a single
    // string that has been split on commas.  Release it by freeing the
    // data of the list's first node
    if (places)
        g_free(places->data);
    g_list_free(places);
...
static gboolean _find_iter_place(GtkTreeModel *model, GtkTreeIter *iter, const char *place)
{
    gboolean found = FALSE;
    if (!place)
        return found;
...
#endif
        gtk_list_store_insert_with_values(d->placesModel, &iter, -1, DT_PLACES_NAME, basename,
                                          DT_PLACES_PATH, (char *)place, DT_PLACES_TYPE,
                                          DT_TYPE_CUSTOM, -1);
        g_free(basename);
    }
...
static void _lib_import_select_folder(GtkWidget *widget, dt_lib_module_t *self)
{
    ...
    GtkFileChooserNative *filechooser = gtk_file_chooser_native_new(
        _("select directory"), GTK_WINDOW(win), GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER, _("_open"),
        _("_cancel"));
...
static void _set_files_list(GtkWidget *rbox, dt_lib_module_t *self)
{
    dt_lib_import_t *d = self->data;
    d->from.store =
        gtk_list_store_new(DT_IMPORT_NUM_COLS, G_TYPE_BOOLEAN, GDK_TYPE_PIXBUF, G_TYPE_STRING,
    ...
    GtkTreeViewColumn *column =
        gtk_tree_view_column_new_with_attributes("✔", renderer, "text", DT_IMPORT_UI_EXISTS, NULL);
    ...
    gtk_widget_set_tooltip_text(
        header, _("file 'modified date/time', may be different from 'Exif date/time'"));
    gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(d->from.store), DT_IMPORT_DATETIME,
                                         GTK_SORT_ASCENDING);

    renderer = gtk_cell_renderer_pixbuf_new();
    column =
        gtk_tree_view_column_new_with_attributes("", renderer, "pixbuf", DT_IMPORT_THUMB, NULL);
    gtk_tree_view_append_column(d->from.treeview, column);
    ...
    g_signal_connect(G_OBJECT(selection), "changed", G_CALLBACK(_import_from_selection_changed),
                     self);
...
static void _set_expander_content(GtkWidget *rbox, dt_lib_module_t *self)
{
    ...
    // collapsible section
    _expander_create(&d->from.cs, GTK_BOX(import_patterns), _("naming rules"),
                     "ui_last/session_expander_import", NULL);

    // import patterns
    grid = GTK_GRID(gtk_grid_new());
    gtk_grid_set_column_spacing(grid, DT_PIXEL_APPLY_DPI(5));
    d->from.datetime =
        dt_gui_preferences_string(grid, "ui_last/import_datetime_override", 0, line++);
    GtkWidget *basedir =
    ...
    gtk_grid_attach_next_to(grid, hbox, gtk_grid_get_child_at(grid, 0, line - 1), GTK_POS_RIGHT, 1,
                            1);
...
static void _import_from_dialog_new(dt_lib_module_t *self)
{
    ...
    d->from.dialog = gtk_dialog_new_with_buttons(
        _(_import_text[d->import_case]), NULL, GTK_DIALOG_MODAL, _("_cancel"), GTK_RESPONSE_CANCEL,
        _(_import_text[d->import_case]), GTK_RESPONSE_ACCEPT, NULL);
    ...
    gtk_widget_set_hexpand(gtk_grid_get_child_at(grid, col++, line++), TRUE);
    g_signal_connect(G_OBJECT(ignore_nonraws), "toggled", G_CALLBACK(_ignore_nonraws_toggled),
                     self);
...

    {
        ...
        gtk_paned_set_position(GTK_PANED(places_paned),
                               dt_conf_get_int("ui_last/import_dialog_paned_places_pos"));
        g_signal_connect(places_paned, "notify::position",
                         G_CALLBACK(_paned_places_position_changed), self);
...
static void _import_set_collection(const char *dirname)
{
    if (dirname)
    {
        ...
        if (property != DT_COLLECTION_PROP_FOLDERS && property != DT_COLLECTION_PROP_FILMROLL)
        {
    ...
    while (gtk_dialog_run(GTK_DIALOG(d->from.dialog)) == GTK_RESPONSE_ACCEPT)
    {
        ...
        if (imgs)
        {
            ...
            if (d->import_case != DT_IMPORT_INPLACE)
            {
                ...
                if (entry[0] &&
                    !dt_datetime_entry_to_exif(datetime_override, sizeof(datetime_override), entry))
                {
...
void gui_init(dt_lib_module_t *self)
{
    ...
    // add import buttons
    GtkWidget *widget =
        dt_action_button_new(self, N_("add to library..."), _lib_import_from_callback, self,
    ...
    widget = dt_action_button_new(
        self, N_("copy & import..."), _lib_import_from_callback, self,
    ...
    d->apply_metadata =
        dt_gui_preferences_bool(grid, "ui_last/import_apply_metadata", 0, line++, FALSE);
...
const struct
{
    ...
    char *name;
    int type;
} _pref[] = {{"ui_last/import_ignore_nonraws", "ignore_nonraws", DT_BOOL},
...
static char *_get_current_configuration(dt_lib_module_t *self)
{
    ...
    for (int i = 0; i < pref_n; i++)
    {
        ...
        {
            dt_util_str_cat(&pref, "%s=%d,", _pref[i].name, dt_conf_get_bool(_pref[i].key) ? 1 : 0);
        }
        else if (_pref[i].type == DT_INT)
        {
            dt_util_str_cat(&pref, "%s=%d,", _pref[i].name, dt_conf_get_int(_pref[i].key));
        }
    ...
    for (GList *iter = dt_metadata_get_list(); iter; iter = iter->next)
    {
        ...
        if (!metadata->internal)
        {
            ...
            dt_util_str_cat(&pref, "%s=%d%s,", metadata_name, imported ? 1 : 0, metadata_value);
            g_free(setting);
        }
    }
    dt_pthread_mutex_unlock(&darktable.metadata_threadsafe);
