...
static void _split_process(dt_scopes_mode_t *const self, const float *const input,
...
static void _split_draw_highlight(const dt_scopes_mode_t *const self, cairo_t *cr,
                                  dt_scopes_highlight_t highlight, const int width,
                                  const int height)
{
    const dt_scopes_split_t *const d = self->data;
    const int half_width = width / 2;
    cairo_save(cr);
    dt_scopes_call_if_exists(d->left, draw_highlight, cr, self->scopes->highlight, half_width,
                             height);
    cairo_translate(cr, half_width, 0);
    dt_scopes_call_if_exists(d->right, draw_highlight, cr, self->scopes->highlight, half_width,
                             height);
    cairo_restore(cr);
}

static void _split_draw_bkgd(const dt_scopes_mode_t *const self, cairo_t *cr, const int width,
                             const int height)
{
    const dt_scopes_split_t *const d = self->data;
    const int half_width = width / 2;
...
static void _split_draw_grid(const dt_scopes_mode_t *const self, cairo_t *cr, const int width,
                             const int height)
{
    const dt_scopes_split_t *const d = self->data;
    const int half_width = width / 2;
...
static void _split_draw(const dt_scopes_mode_t *const self, cairo_t *cr, const int width,
                        const int height, const scopes_channels_t channels)
{
    ...
    if (d->left->update_counter == self->scopes->update_counter)
    {
        ...
        if (d->left->functions->draw_scope_channels)
            dt_scopes_call(d->left, draw_scope_channels, cr, half_width, height,
    ...
    if (d->right->update_counter == self->scopes->update_counter)
    {
        ...
        if (d->right->functions->draw_scope_channels)
            dt_scopes_call(d->right, draw_scope_channels, cr, half_width, height,
                           self->scopes->channels);
        else
...
static void _split_append_to_tooltip(const dt_scopes_mode_t *const self, gchar **tip)
{
    const dt_scopes_split_t *const d = self->data;
...
static double _split_get_exposure_delta(const dt_scopes_mode_t *const self, const double offset_x,
                                        const double offset_y)
{
    ...
    if (d->left->functions->get_exposure_delta)
        return dt_scopes_call(d->left, get_exposure_delta, offset_x * 2.0, offset_y);
    else if (d->right->functions->get_exposure_delta)
        return dt_scopes_call(
            d->right, get_exposure_delta,
            offset_x * 2.0 - gtk_widget_get_allocated_width(self->scopes->scope_draw), offset_y);
...
static void _split_eventbox_scroll(dt_scopes_mode_t *const self, gdouble x, gdouble y,
...
static void _responsive_buttons(dt_scopes_t *const s)
{
    ...
    if (!btns_mode_hori)
    {
        ...
        gtk_style_context_get_property(ctx, "min-width", gtk_style_context_get_state(ctx), &val);
        double min_w = 0.0;
...
static void _overlay_size_allocate(GtkWidget *overlay, GtkAllocation *allocation,
...
static void _split_gui_init(dt_scopes_mode_t *const self, dt_scopes_t *const scopes)
{
...
static void _split_add_options(dt_scopes_mode_t *const self, dt_action_t *dark)
{
