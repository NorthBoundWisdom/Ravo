...
gboolean _styles_tooltip_callback(GtkWidget *widget, gint x, gint y, gboolean keyboard_mode,
                                  GtkTooltip *tooltip, dt_lib_styles_t *d)
{
    GtkTreeModel *model;
    GtkTreePath *path;
    GtkTreeIter iter;
    ...
    if (gtk_tree_view_get_tooltip_context(GTK_TREE_VIEW(widget), &x, &y, FALSE, &model, &path,
                                          &iter))
    {
        gchar *name = NULL;
        gtk_tree_model_get(model, &iter, DT_STYLES_COL_FULLNAME, &name, -1);
...
static void _gui_styles_update_view(dt_lib_styles_t *d)
{
        ...
        for (const GList *res_iter = result; res_iter; res_iter = g_list_next(res_iter))
        {
            ...
            while (split[k])
            {
                ...
                if (!node_found)
                {
                    ...
                    {
                        // a leaf
                        gtk_tree_store_set(GTK_TREE_STORE(model), &iter, DT_STYLES_COL_NAME, s,
    ...
    g_signal_connect(GTK_TREE_VIEW(d->tree), "query-tooltip", G_CALLBACK(_styles_tooltip_callback),
                     d);
    gtk_tree_view_set_model(GTK_TREE_VIEW(d->tree), model);
    g_object_unref(model);
}

static void _styles_row_activated_callback(GtkTreeView *view, GtkTreePath *path,
                                           GtkTreeViewColumn *col, dt_lib_styles_t *d)
{
...
static void _apply_clicked(GtkWidget *w, dt_lib_styles_t *d)
{
    ...
    if (dt_view_get_current() == DT_VIEW_DARKROOM)
    {
        for (GList *s = g_list_first(style_names); s; s = g_list_next(s))
        {
...
gboolean _ask_before_delete_style(const gint style_cnt)
{
    return !dt_conf_get_bool("plugins/lighttable/style/ask_before_delete_style") ||
           dt_gui_show_yes_no_dialog(ngettext("remove style?", "remove styles?", style_cnt), "",
        ...
        for (const GList *style = style_names; style; style = g_list_next(style))
        {
            ...
            if (g_file_test(stylename, G_FILE_TEST_EXISTS) == TRUE)
            {
                ...
                else
                {
                    ...
                    {
                        GtkWidget *dialog_overwrite_export = gtk_dialog_new_with_buttons(
                            _("overwrite style?"), GTK_WINDOW(win), GTK_DIALOG_DESTROY_WITH_PARENT,
                            _("_cancel"), GTK_RESPONSE_CANCEL, _("_skip"), GTK_RESPONSE_NONE,
                            _("_overwrite"), GTK_RESPONSE_ACCEPT, NULL);
                        gtk_dialog_set_default_response(GTK_DIALOG(dialog_overwrite_export),
                                                        GTK_RESPONSE_CANCEL);

                        // contents for dialog
                        gchar *overwrite_str = g_strdup_printf(
                            _("style `%s' already exists.\ndo you want to overwrite existing style?\n"),
                            stylename);
                        GtkWidget *overwrite_dialog_check_button = gtk_check_button_new_with_label(
                            _("apply this option to all existing styles"));

                        dt_gui_dialog_add(GTK_DIALOG(dialog_overwrite_export),
                                          gtk_label_new(overwrite_str),
                                          overwrite_dialog_check_button);
                        ...
                        if (g_list_is_singleton(style_names))
                        {
                            ...
                            gtk_dialog_set_response_sensitive(GTK_DIALOG(dialog_overwrite_export),
                        ...
                        overwrite_dialog_check_button_res = gtk_toggle_button_get_active(
                            GTK_TOGGLE_BUTTON(overwrite_dialog_check_button));
...
static void _import_clicked(GtkWidget *w, dt_lib_styles_t *d)
{
    ...
    GtkWidget *win = dt_ui_main_window(darktable.gui->ui);
    GtkFileChooserNative *filechooser = gtk_file_chooser_native_new(
        _("select style"), GTK_WINDOW(win), GTK_FILE_CHOOSER_ACTION_OPEN, _("_open"), _("_cancel"));
        ...
        for (const GSList *filename = filenames; filename; filename = g_slist_next(filename))
        {
            ...
            if (dt_styles_exists(bname))
            {
                ...
                else
                {
                    ...
                    if (dt_conf_get_bool("plugins/lighttable/style/ask_before_delete_style"))
                    {
                        GtkWidget *dialog_overwrite_import = gtk_dialog_new_with_buttons(
                            _("overwrite style?"), GTK_WINDOW(win), GTK_DIALOG_DESTROY_WITH_PARENT,
                            _("_cancel"), GTK_RESPONSE_CANCEL, _("_skip"), GTK_RESPONSE_NONE,
                            _("_overwrite"), GTK_RESPONSE_ACCEPT, NULL);
                        gtk_dialog_set_default_response(GTK_DIALOG(dialog_overwrite_import),
                                                        GTK_RESPONSE_CANCEL);
                        ...
                        GtkWidget *overwrite_dialog_check_button = gtk_check_button_new_with_label(
                            _("apply this option to all existing styles"));
                        dt_gui_dialog_add(GTK_DIALOG(dialog_overwrite_import),
                                          gtk_label_new(overwrite_str),
                                          overwrite_dialog_check_button);
                        ...
                        {
                            gtk_widget_set_sensitive(overwrite_dialog_check_button, FALSE);
                            gtk_dialog_set_response_sensitive(GTK_DIALOG(dialog_overwrite_import),
                                                              GTK_RESPONSE_NONE, FALSE);
                        }
...
void gui_init(dt_lib_module_t *self)
{
    ...
    GtkTreeStore *treestore =
        gtk_tree_store_new(DT_STYLES_NUM_COLS, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING);
    ...
    gtk_tree_selection_set_mode(gtk_tree_view_get_selection(GTK_TREE_VIEW(d->tree)),
    ...
    gtk_widget_set_tooltip_text(d->hide_preview, _("hide preview of style on tooltip"));
    ...
    // create
    d->create_button =
        dt_action_button_new(self, N_("create..."), _create_clicked, d,
                             _("create styles from history stack of selected images"), 0, 0);

    // edit
    d->edit_button = dt_action_button_new(self, N_("edit..."), _edit_clicked, d,
                                          _("edit the selected styles in list above"), 0, 0);

    // delete
    d->delete_button = dt_action_button_new(self, N_("remove"), _delete_clicked, d,
                                            _("removes the selected styles in list above"), 0, 0);

    // import button
    d->import_button = dt_action_button_new(self, N_("import..."), _import_clicked, d,
                                            _("import styles from a style files"), 0, 0);

    // export button
    d->export_button =
        dt_action_button_new(self, N_("export..."), _export_clicked, d,
    ...
    self->widget = dt_gui_vbox(
        ...
        dt_gui_hbox(d->import_button, d->export_button), d->apply_button);
...
void _menuitem_preferences(GtkMenuItem *menuitem, dt_lib_module_t *self)
{
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(preview_size, self, NULL, N_("preview size"),
                                 ...
                                 dt_conf_get_int("plugins/lighttable/style/preview_size"), NULL,
                                 self, N_("default"), N_("large"));
