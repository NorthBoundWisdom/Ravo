                                          ...
                                          gpointer imgs, int next, dt_lib_module_t *self)
{
    ...
    for (int k = 0; current; k++)
    {
        ...
        if (child)
        {
            ...
            gtk_label_set_xalign(GTK_LABEL(child),
                                 0.0); // without this the labels are not flush on the left
    ...
    current = d->items;
    for (int k = 0;
         k < CLAMPS(_conf_get_max_shown_items(), 0, _conf_get_max_saved_items()) && current; k++)
    {
        dt_lib_recentcollect_item_t *item = current->data;
...
void _menuitem_preferences(GtkMenuItem *menuitem, dt_lib_module_t *self)
{
    ...
    GtkWidget *dialog = gtk_dialog_new_with_buttons(
        _("recent collections settings"), GTK_WINDOW(win), GTK_DIALOG_DESTROY_WITH_PARENT,
    ...
    if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT)
    {
        ...
        if (delta < 0)
        {
            ...
            {
                dt_lib_recentcollect_item_t *item = current->data;
                snprintf(confname, sizeof(confname), "plugins/lighttable/collect/history%1d",
                         item->confid);
        ...
        if (delta > 0)
        {
            ...
            for (int k = old_nb_items; k < new_nb_items; k++)
            {
                ...
                if (item)
                {
                    ...
                    g_signal_connect(G_OBJECT(item->button), "clicked", G_CALLBACK(_button_pressed),
                                     (gpointer)self);
                    gtk_widget_set_no_show_all(item->button, TRUE);
        ...
        _lib_recentcollection_updated(NULL, DT_COLLECTION_CHANGE_NEW_QUERY,
                                      DT_COLLECTION_PROP_UNDEF, NULL, -1, self);
    }
...
void gui_reset(dt_lib_module_t *self)
{
    ...
    _lib_recentcollection_updated(NULL, DT_COLLECTION_CHANGE_NEW_QUERY, DT_COLLECTION_PROP_UNDEF,
                                  NULL, -1, self);
}
