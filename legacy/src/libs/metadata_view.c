...
static void _filter_non_printable(char *string, const size_t length)
{
...
static void _metadata_update_timestamp(const int i, const GTimeSpan gts, dt_lib_module_t *self)
{
    ...
    const gboolean valid =
        gts ? dt_datetime_gtimespan_to_local(datetime, sizeof(datetime), gts, FALSE, TRUE) : FALSE;
...
static void _metadata_get_flags(const dt_image_t *const img, char *const text, char *const tooltip,
...
void gui_update(dt_lib_module_t *self)
{
    ...
    for (int32_t md = 0; md < md_xmp_metadata + d->metadata_count; md++)
    {
        ...
        case md_internal_filmroll:
        {
            ...
            snprintf(tooltip_filmroll, sizeof(tooltip_filmroll),
        ...
        case md_internal_local_copy:
            (void)g_strlcpy(text, (img->flags & DT_IMAGE_LOCAL_COPY) ? _("yes") : _("no"),
                            sizeof(text));
        ...
        case md_exif_exposure_bias:
            ...
            {
                (void)g_snprintf(text, sizeof(text), _("%+.2f EV"),
        ...
        case md_exif_focus_distance:
            ...
            else if (!(dt_isnan(img->exif_focus_distance) ||
        ...
        case md_exif_datetime:
        {
            ...
            const gboolean valid =
                dt_datetime_img_to_local(datetime, sizeof(datetime), img, milliseconds);
        ...
        case md_categories:
        {
            ...
            if (dt_tag_get_attached(img->id, &tags, TRUE))
            {
                ...
                if (tagstring)
                    tagstring[strlen(tagstring) - 2] = '\0';
            }

            if (md == md_tag_names)
                _metadata_update_value(md_tag_names, tagstring ? tagstring : NODATA_STRING, self);
            else
                _metadata_update_value(md_categories,
                                       categoriesstring ? categoriesstring : NODATA_STRING, self);
...
static void _add_grid_row(dt_lib_metadata_info_t *m, const int row, dt_lib_module_t *self)
{
    dt_lib_metadata_view_t *d = self->data;
    ...
    for (GList *iter = dt_metadata_get_list(); iter; iter = iter->next)
    {
        ...
        if (!metadata->internal)
        {
            ...
            if (!found)
                ...
                new_metadata_keys =
                    g_list_prepend(new_metadata_keys, GINT_TO_POINTER(metadata->key));
        }
    }
...
static void _select_toggled_callback(GtkCellRendererToggle *cell_renderer, gchar *path_str,
                                     gpointer user_data)
{
...
void _menuitem_preferences(GtkMenuItem *menuitem, dt_lib_module_t *self)
{
    ...
    {
        dt_lib_metadata_info_t *m = meta->data;
        gtk_list_store_insert_with_values(store, NULL, -1, DT_METADATA_PREF_COL_INDEX, m->index,
                                          DT_METADATA_PREF_COL_NAME_L, _(m->name),
                                          DT_METADATA_PREF_COL_VISIBLE, m->visible, -1);
    }
    ...
    GtkTreeViewColumn *column = gtk_tree_view_column_new_with_attributes(
        _("metadata"), renderer, "text", DT_METADATA_PREF_COL_NAME_L, NULL);
    ...
    while (res == GTK_RESPONSE_YES)
    {
        ...
        {
            dt_lib_metadata_info_t *m = meta->data;
            gtk_list_store_set(store, &iter, DT_METADATA_PREF_COL_INDEX, m->index,
                               DT_METADATA_PREF_COL_NAME_L, _(m->name),
                               DT_METADATA_PREF_COL_VISIBLE, TRUE, -1);
    ...
    if (res == GTK_RESPONSE_ACCEPT)
    {
        ...
        while (valid)
        {
            gboolean visible;
            uint32_t index;
            gtk_tree_model_get(model, &iter, DT_METADATA_PREF_COL_INDEX, &index,
                               DT_METADATA_PREF_COL_VISIBLE, &visible, -1);
            for (GList *meta = d->metadata; meta; meta = g_list_next(meta))
            {
                dt_lib_metadata_info_t *m = meta->data;
                if (m->index == index)
...
void gui_init(dt_lib_module_t *self)
{
    ...
    self->widget = dt_gui_vbox(
        dt_ui_resize_wrap(d->grid, 200, "plugins/lighttable/metadata_view/windowheight"));
    ...
    dt_action_register(DT_ACTION(self), N_("jump to film roll"), _jump_to_accel, GDK_KEY_j,
                       GDK_CONTROL_MASK);
}
