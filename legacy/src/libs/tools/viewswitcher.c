...
static gboolean _lib_viewswitcher_button_press_callback(GtkWidget *w, GdkEventButton *ev,
...
#define SHORTCUT_TOOLTIP(v, w)                                                                     \
    dt_action_define(&darktable.control->actions_global, "switch views", v->module_name, w, NULL);
...
void gui_init(dt_lib_module_t *self)
{
         ...
         view_iter = g_list_next(view_iter))
    {
        ...
        else
        {
            ...
            {
                model =
                    gtk_list_store_new(N_COLUMNS, G_TYPE_STRING, G_TYPE_POINTER, G_TYPE_BOOLEAN);
                ...
                gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(d->dropdown), renderer, "markup",
                                               TEXT_COLUMN, "sensitive", SENSITIVE_COLUMN, NULL);

                gtk_list_store_insert_with_values(model, NULL, -1, TEXT_COLUMN, _("other"),
                                                  VIEW_COLUMN, NULL, SENSITIVE_COLUMN, 0, -1);

                gtk_box_pack_start(GTK_BOX(self->widget), d->dropdown, FALSE, FALSE, 0);
                g_signal_connect(G_OBJECT(d->dropdown), "changed", G_CALLBACK(_dropdown_changed),
                                 d);
            }

            gtk_list_store_insert_with_values(model, NULL, -1, TEXT_COLUMN, view->name(view),
                                              VIEW_COLUMN, view, SENSITIVE_COLUMN, gimping ? 0 : 1,
                                              -1);
        }
    }
...
static gboolean _lib_viewswitcher_enter_leave_notify_callback(GtkWidget *w, GdkEventCrossing *e,
                                                              GtkLabel *l)
{
    /* if not active view lets highlight */
    if (e->type == GDK_ENTER_NOTIFY && strcmp(g_object_get_data(G_OBJECT(w), "view-label"),
...
static GtkWidget *_lib_viewswitcher_create_label(dt_view_t *view)
{
    ...
    g_signal_connect(G_OBJECT(eb), "enter-notify-event",
                     G_CALLBACK(_lib_viewswitcher_enter_leave_notify_callback), b);
