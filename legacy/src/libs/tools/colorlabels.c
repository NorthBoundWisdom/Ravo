...
static char *_get_tooltip_for(const int coloridx)
{
    ...
    char *tooltip = text[0] ? g_markup_printf_escaped(_("toggle color label of selected images\n"
                                                        "<i>%s</i>"),
                                                      text) :
...
void gui_init(dt_lib_module_t *self)
{
    ...
    for (int k = 0; k < 6; k++)
    {
        ...
        char *tooltip =
            k < 5 ? _get_tooltip_for(k) : g_strdup(_("clear color labels of selected images"));
                                           ...
                                           dt_lib_module_t *self)
{
    ...
    case GDK_KEY_KP_Enter:
    {
        ...
        snprintf(confname, sizeof(confname), "colorlabel/%s", dt_colorlabels_name[d->colorlabel]);
