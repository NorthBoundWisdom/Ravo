...
static void _filename_changed(GtkWidget *widget, gpointer user_data)
{
    ...
    if (filename->rule->manual_widget_set)
        return;
...
void _filename_tree_update(_widgets_filename_t *filename)
{
  ...
  g_snprintf(query, sizeof(query),
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        if (name == NULL)
            continue; // safeguard against degenerated db entries
        const int count = sqlite3_column_int(stmt, 1);

        gtk_list_store_insert_with_values(GTK_LIST_STORE(name_model), NULL, -1, TREE_COL_TEXT, name,
                                          TREE_COL_TOOLTIP, name, TREE_COL_PATH, name,
  ...
  g_snprintf(query, sizeof(query),
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        gtk_list_store_insert_with_values(GTK_LIST_STORE(ext_model), NULL, -1, TREE_COL_TEXT, name,
                                          TREE_COL_TOOLTIP, name, TREE_COL_PATH, name,
                                          TREE_COL_COUNT, count, -1);
        ...
        if (flags & DT_IMAGE_RAW)
            ...
            nb_not_raw += count;
        if (flags & DT_IMAGE_LDR)
            nb_ldr += count;
        if (flags & DT_IMAGE_HDR)
            nb_hdr += count;
    }
    sqlite3_finalize(stmt);

    // and we insert the predefined extensions
    gtk_list_store_insert_with_values(GTK_LIST_STORE(ext_model), NULL, 0, TREE_COL_TEXT, "",
                                      TREE_COL_TOOLTIP, "", TREE_COL_PATH, "", TREE_COL_COUNT, 0,
                                      -1);
    gtk_list_store_insert_with_values(GTK_LIST_STORE(ext_model), NULL, 0, TREE_COL_TEXT, "HDR",
                                      TREE_COL_TOOLTIP, "high dynamic range files", TREE_COL_PATH,
                                      "HDR", TREE_COL_COUNT, nb_hdr, -1);
    gtk_list_store_insert_with_values(GTK_LIST_STORE(ext_model), NULL, 0, TREE_COL_TEXT, "LDR",
                                      TREE_COL_TOOLTIP, "low dynamic range files", TREE_COL_PATH,
                                      "LDR", TREE_COL_COUNT, nb_ldr, -1);
    gtk_list_store_insert_with_values(GTK_LIST_STORE(ext_model), NULL, 0, TREE_COL_TEXT, "NOT RAW",
                                      TREE_COL_TOOLTIP, "all except RAW files", TREE_COL_PATH,
                                      "NOT RAW", TREE_COL_COUNT, nb_not_raw, -1);
    gtk_list_store_insert_with_values(GTK_LIST_STORE(ext_model), NULL, 0, TREE_COL_TEXT, "RAW",
                                      TREE_COL_TOOLTIP, "RAW files", TREE_COL_PATH, "RAW",
                                      TREE_COL_COUNT, nb_raw, -1);
...
static gboolean _filename_select_func(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter,
                                      gpointer data)
{
...
static void _filename_update_selection(_widgets_filename_t *filename)
{
    ...
    if (g_strcmp0(txt, ""))
    {
        ...
        gtk_tree_model_foreach(gtk_tree_view_get_model(GTK_TREE_VIEW(tree)), _filename_select_func,
                               sel);
        g_strfreev(elems);
    }
    filename->internal_change--;
...
static void _filename_tree_row_activated(GtkTreeView *self, GtkTreePath *path,
                                         GtkTreeViewColumn *column, _widgets_filename_t *filename)
{
...
{
    if (!filename || filename->internal_change)
        return;
    ...
    for (const GList *l = list; l; l = g_list_next(l))
    {
        ...
        if (gtk_tree_model_get_iter(model, &iter, path))
        {
            ...
            if (val)
                dt_util_str_cat(&txt, "%s%s", (txt == NULL) ? "" : ",", val);
        }
    }
...
void _filename_tree_count_func(GtkTreeViewColumn *col, GtkCellRenderer *renderer,
                                  ...
                                  dt_lib_module_t *self)
{
    ...
    gtk_widget_set_tooltip_text(filename->ext,
                                _("enter extension to search with starting dot\n"
                                  "multiple values can be separated by ','\n"
    ...
    GtkTreeModel *model = GTK_TREE_MODEL(
        gtk_list_store_new(TREE_NUM_COLS, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_INT));
    ...
    g_signal_connect(G_OBJECT(filename->name_tree), "row-activated",
                     G_CALLBACK(_filename_tree_row_activated), filename);
    g_signal_connect(G_OBJECT(sel), "changed", G_CALLBACK(_filename_tree_selection_change),
                     filename);
