...
static gboolean _rating_range_update(dt_lib_filtering_rule_t *rule)
{
  ...
  g_snprintf(query, sizeof(query),
    ...
    dtgtk_range_select_add_range_block(range, -1.0, -1.0, DT_RANGE_BOUND_FIXED, _("rejected only"),
                                       nb[0]);
    dtgtk_range_select_add_range_block(range, 0.0, 0.0, DT_RANGE_BOUND_FIXED, _("not rated only"),
                                       nb[1]);
...
static void _rating_paint_icon(cairo_t *cr, gint x, gint y, gint w, gint h, gint flags, void *data)
{
    ...
    if ((flags & CPF_PRELIGHT) || (flags & CPF_ACTIVE))
    {
        ...
        cairo_get_source(cr);
        cairo_pattern_get_rgba(cairo_get_source(cr), &shade_color.red, &shade_color.green,
                               &shade_color.blue, &shade_color.alpha);
...
static float _action_process_ratings(gpointer target, dt_action_element_t element,
...
const gchar *dt_action_effect_rating[] = {[DT_ACTION_EFFECT_TOGGLE] = N_("toggle"),
...
const dt_action_def_t dt_action_def_ratings_rule = {N_("rating filter"), _action_process_ratings,
