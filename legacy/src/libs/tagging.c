...
typedef struct dt_lib_tagging_t
{
    ...
    GtkWidget *toggle_tree_button, *toggle_suggestion_button, *toggle_sort_button,
        *toggle_hide_button, *toggle_dttags_button;
...
static gboolean _is_user_tag(GtkTreeModel *model, GtkTreeIter *iter)
{
    ...
    const gboolean user_tag =
        !g_str_has_prefix(path, "darktable|") || g_str_has_prefix(path, "darktable|style|");
    g_free(path);
    return user_tag;
}
...
static void _update_atdetach_buttons(dt_lib_module_t *self)
{
    ...
    const gint dict_tags_sel_cnt = gtk_tree_selection_count_selected_rows(
        gtk_tree_view_get_selection(GTK_TREE_VIEW(d->dictionary_view)));
...
static void _propagate_sel_to_parents(GtkTreeModel *model, GtkTreeIter *iter)
{
    ...
    while (gtk_tree_model_iter_parent(model, &parent, &child))
    {
        ...
        if (sel == DT_TS_NO_IMAGE)
            gtk_tree_store_set(GTK_TREE_STORE(model), &parent, DT_LIB_TAGGING_COL_SEL,
                               DT_TS_SOME_IMAGES, -1);
        child = parent;
    }
}

static gboolean _set_matching_tag_visibility(GtkTreeModel *model, GtkTreePath *path,
                                             GtkTreeIter *iter, dt_lib_module_t *self)
{
    dt_lib_tagging_t *d = self->data;
    gboolean visible;
    gchar *tagname = NULL;
    gchar *synonyms = NULL;
    gtk_tree_model_get(model, iter, DT_LIB_TAGGING_COL_PATH, &tagname, DT_LIB_TAGGING_COL_SYNONYM,
                       &synonyms, -1);
    if (!d->keyword[0])
        visible = TRUE;
    else
    {
        if (synonyms && synonyms[0])
            dt_util_str_cat(&tagname, ", %s", synonyms);
...
static gboolean _tree_reveal_func(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter,
                                  gpointer data)
{
    gboolean state;
    GtkTreeIter parent, child = *iter;
...
static void _sort_attached_list(dt_lib_module_t *self, const gboolean force)
{
    dt_lib_tagging_t *d = self->data;
    if (force && d->sort_count_flag)
    {
        // ugly but when sorted by count _tree_tagname_show() is not triggered
        gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(d->attached_liststore),
                                             DT_TAG_SORT_NAME_ID, GTK_SORT_ASCENDING);
    }
    const gint sort = d->sort_count_flag ? DT_TAG_SORT_COUNT_ID :
                      d->hide_path_flag  ? DT_TAG_SORT_NAME_ID :
                                           DT_TAG_SORT_PATH_ID;
    gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(d->attached_liststore), sort,
                                         GTK_SORT_ASCENDING);
}

static void _sort_dictionary_list(dt_lib_module_t *self, const gboolean force)
{
    dt_lib_tagging_t *d = self->data;
    if (!d->tree_flag)
    {
        ...
        {
            // ugly but when sorted by count _tree_tagname_show() is not triggered
            gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(d->dictionary_liststore),
                                                 DT_TAG_SORT_NAME_ID, GTK_SORT_ASCENDING);
        }
        const gint sort = d->sort_count_flag ? DT_TAG_SORT_COUNT_ID :
                          d->hide_path_flag  ? DT_TAG_SORT_NAME_ID :
                                               DT_TAG_SORT_PATH_ID;
        gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(d->dictionary_liststore), sort,
                                             GTK_SORT_ASCENDING);
    }
    else
        gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(d->dictionary_treestore),
                                             DT_TAG_SORT_PATH_ID, GTK_SORT_ASCENDING);
}

// find a tag on the tree
static gboolean _find_tag_iter_tagname(GtkTreeModel *model, GtkTreeIter *iter, const char *tagname,
...
static void _init_treeview(dt_lib_module_t *self, const int which)
{
    ...
    gtk_tree_sortable_set_sort_column_id(
        GTK_TREE_SORTABLE(store), GTK_TREE_SORTABLE_UNSORTED_SORT_COLUMN_ID, GTK_SORT_ASCENDING);
    if (which && d->tree_flag)
    {
            ...
            for (GList *taglist = tags; taglist; taglist = g_list_next(taglist))
            {
                ...
                if (tokens)
                {
                    ...
                    if (last_tokens)
                    {
                        ...
                        {
                            gtk_tree_model_iter_parent(GTK_TREE_MODEL(store), &parent,
                                                       &last_parent);
                            last_parent = parent;
                        }
                    }
                    ...
                    for (char **token = &tokens[common_length]; *token; token++)
                    {
                        ...
                        gtk_tree_store_insert_with_values(
                            GTK_TREE_STORE(store), &iter, common_length > 0 ? &parent : NULL, -1,
                            DT_LIB_TAGGING_COL_TAG, *token, DT_LIB_TAGGING_COL_ID,
                            (token == &tokens[tokens_length - 1]) ?
                                ((dt_tag_t *)taglist->data)->id :
                                0,
                            DT_LIB_TAGGING_COL_PATH, pth2, DT_LIB_TAGGING_COL_COUNT,
                            (token == &tokens[tokens_length - 1]) ?
                                ((dt_tag_t *)taglist->data)->count :
                                0,
        ...
        if (d->keyword[0])
        {
            gtk_tree_model_foreach(store, (GtkTreeModelForeachFunc)_set_matching_tag_visibility,
                                   self);
        ...
        if (count > 0 && tags)
        {
            for (GList *tag = tags; tag; tag = g_list_next(tag))
            {
                ...
                gtk_list_store_insert_with_values(
                    GTK_LIST_STORE(store), NULL, -1, DT_LIB_TAGGING_COL_TAG,
                    !subtag ? ((dt_tag_t *)tag->data)->tag : subtag + 1, DT_LIB_TAGGING_COL_ID,
                    ((dt_tag_t *)tag->data)->id, DT_LIB_TAGGING_COL_PATH,
                    ((dt_tag_t *)tag->data)->tag, DT_LIB_TAGGING_COL_COUNT,
                    ((dt_tag_t *)tag->data)->count, DT_LIB_TAGGING_COL_SEL,
                    no_sel ? DT_TS_NO_IMAGE : ((dt_tag_t *)tag->data)->select,
...
static void _tree_tagname_show(GtkTreeViewColumn *col, GtkCellRenderer *renderer,
                               ...
                               const gboolean dictionary_view)
{
    ...
    gtk_tree_model_get(model, iter, DT_LIB_TAGGING_COL_ID, &tagid, DT_LIB_TAGGING_COL_TAG, &name,
                       DT_LIB_TAGGING_COL_COUNT, &count, DT_LIB_TAGGING_COL_FLAGS, &flags,
                       DT_LIB_TAGGING_COL_PATH, &path, -1);

    const gboolean hide =
        dictionary_view ? (d->tree_flag ? TRUE : d->hide_path_flag) : d->hide_path_flag;
    ...
    {
        coltext = g_markup_printf_escaped(istag ? "%s (%d)" : "<i>%s</i> (%d)", hide ? name : path,
                                          count);
    }
                              ...
                              GtkTreeModel *model, GtkTreeIter *iter, gpointer data)
{
    ...
    gtk_tree_model_get(model, iter, DT_LIB_TAGGING_COL_ID, &tagid, DT_LIB_TAGGING_COL_SEL, &select,
                       -1);
    if (!tagid)
    {
        if (select != DT_TS_NO_IMAGE)
            inconsistent = TRUE;
    }
    else
    {
        if (select == DT_TS_ALL_IMAGES)
            active = TRUE;
        else if (select == DT_TS_SOME_IMAGES)
            inconsistent = TRUE;
    }
...
static void _reset_sel_on_path_full(GtkTreeModel *model, GtkTreeIter *iter, const gboolean root)
{
    ...
    do
    {
        ...
        {
            gtk_list_store_set(GTK_LIST_STORE(model), &parent, DT_LIB_TAGGING_COL_SEL,
                               DT_TS_NO_IMAGE, -1);
        }
    } while (!root && gtk_tree_model_iter_next(model, &parent));
}

//  try to find a node fully attached (2) which is the root of the
//  update loop. If not the full tree will be used
    ...
    if (gtk_tree_model_get_iter_first(model, &parent))
    {
        ...
        for (GList *tag = tags; tag; tag = g_list_next(tag))
        {
            ...
            if (_find_tag_iter_tagid(model, &iter, ((dt_tag_t *)tag->data)->id))
            {
                if (GTK_IS_TREE_STORE(model))
                {
                    gtk_tree_store_set(GTK_TREE_STORE(model), &iter, DT_LIB_TAGGING_COL_SEL,
                                       ((dt_tag_t *)tag->data)->select, -1);
                    _propagate_sel_to_parents(model, &iter);
                }
                else
                {
                    gtk_list_store_set(GTK_LIST_STORE(model), &iter, DT_LIB_TAGGING_COL_SEL,
                                       ((dt_tag_t *)tag->data)->select, -1);
                }
            }
        }
...
static void _delete_tree_tag(GtkTreeModel *model, GtkTreeIter *iter, const gboolean tree)
{
    guint tagid = 0;
    ...
    if (tree)
    {
        if (tagid)
        {
            gtk_tree_store_set(GTK_TREE_STORE(model), iter, DT_LIB_TAGGING_COL_SEL, DT_TS_NO_IMAGE,
...
static void _delete_tree_path(GtkTreeModel *model, GtkTreeIter *iter, const gboolean root,
                              const gboolean tree)
{
    if (tree) // the treeview is tree. It handles the hierarchy itself (parent / child)
    {
        ...
        do
        {
            ...
            if (root)
            {
                gtk_tree_store_set(GTK_TREE_STORE(model), &tobedel, DT_LIB_TAGGING_COL_SEL,
                                   DT_TS_NO_IMAGE, DT_LIB_TAGGING_COL_COUNT, 0, -1);
    ...
    else // treeview is a list. The hierarchy of tags is found with the
         // root (left part) of tagname
    {
        GtkTreeIter child;
        char *path = NULL;
        gtk_tree_model_get(model, iter, DT_LIB_TAGGING_COL_PATH, &path, -1);
...
static gboolean _update_tag_name_per_name(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter,
                                          dt_tag_op_t *to)
{
    char *tagname;
    ...
    if (g_str_has_prefix(tagname, oldtagname))
    {
        ...
        else if (strlen(tagname) > strlen(oldtagname) && tagname[strlen(oldtagname)] == '|')
        {
            ...
            if (to->tree_flag)
            {
                gtk_tree_store_set(GTK_TREE_STORE(model), iter, DT_LIB_TAGGING_COL_PATH, newpath,
                                   -1);
            }
            else
            {
                gtk_list_store_set(GTK_LIST_STORE(model), iter, DT_LIB_TAGGING_COL_PATH, newpath,
                                   DT_LIB_TAGGING_COL_TAG, newpath, -1);
            }
            g_free(newpath);
        }
    }
...
int set_params(dt_lib_module_t *self, const void *params, const int size)
{
    if (!params || !size)
        return 1;
    dt_lib_tagging_t *d = self->data;
    ...
    if (buf && buf[0])
    {
        ...
        if (tokens)
        {
            ...
            for (GList *tag = tags; tag; tag = g_list_next(tag))
            {
                _update_attached_count(GPOINTER_TO_INT(tag->data), d->dictionary_view,
                                       d->tree_flag);
                change = TRUE;
            }
...
static void _attach_selected_tag(dt_lib_module_t *self, dt_lib_tagging_t *d)
{
    ...
    if (dt_tag_attach(tagid, -1, TRUE, TRUE))
    {
        ...
        if (d->tree_flag || !d->suggestion_flag)
        {
            ...
            if (d->tree_flag)
            {
                gtk_tree_store_set(GTK_TREE_STORE(store), &store_iter, DT_LIB_TAGGING_COL_COUNT,
                                   count, DT_LIB_TAGGING_COL_SEL, DT_TS_ALL_IMAGES, -1);
                _propagate_sel_to_parents(GTK_TREE_MODEL(store), &store_iter);
            }
            else
            {
                gtk_list_store_set(GTK_LIST_STORE(store), &store_iter, DT_LIB_TAGGING_COL_COUNT,
                                   count, DT_LIB_TAGGING_COL_SEL, DT_TS_ALL_IMAGES, -1);
            }
        }
        else
        ...
        if (d->tree_flag || !d->suggestion_flag)
        {
            ...
            if (gtk_tree_model_get_iter_first(store, &iter))
            {
                if (_find_tag_iter_tagid(store, &iter, tagid))
                {
                    if (d->tree_flag)
                    {
                        gtk_tree_store_set(GTK_TREE_STORE(store), &iter, DT_LIB_TAGGING_COL_COUNT,
                                           count, DT_LIB_TAGGING_COL_SEL, DT_TS_NO_IMAGE, -1);
                        _calculate_sel_on_tree(GTK_TREE_MODEL(store), &iter);
                    }
                    else
                    {
                        gtk_list_store_set(GTK_LIST_STORE(store), &iter, DT_LIB_TAGGING_COL_COUNT,
                                           count, DT_LIB_TAGGING_COL_SEL, DT_TS_NO_IMAGE, -1);
                    }
                }
            }
...
static void _pop_menu_attached_find(GtkWidget *menuitem, dt_lib_module_t *self)
{
    ...
    gtk_tree_model_get(model, &iter, DT_LIB_TAGGING_COL_TAG, &name, -1);
...
static void _new_button_clicked(GtkButton *button, dt_lib_module_t *self)
{
    ...
    if (imgs && !imgs->next)
    {
        ...
        if (sels && (sels->next ||
                     (!sels->next && GPOINTER_TO_INT(sels->data) != GPOINTER_TO_INT(imgs->data))))
        {
            g_list_free(sels);
            g_list_free(imgs);
            return;
    ...
    if (res)
        _raise_signal_tag_changed(self);
    _show_tag_on_view(GTK_TREE_VIEW(d->dictionary_view), tagname ? tagname + 1 : d->last_tag, FALSE,
                      TRUE);
}
...
static void _pop_menu_dictionary_delete_tag(GtkWidget *menuitem, dt_lib_module_t *self,
                                            const gboolean branch)
{
    ...
    gtk_tree_model_get(model, &iter, DT_LIB_TAGGING_COL_PATH, &tagname, DT_LIB_TAGGING_COL_ID,
                       &tagid, -1);
    if (!tagid)
        return;
    ...
    if (img_count > 0 || dt_conf_get_bool("plugins/lighttable/tagging/ask_before_delete_tag"))
    {
        ...
        GtkWidget *dialog = gtk_dialog_new_with_buttons(
            _("delete tag?"), GTK_WINDOW(win), GTK_DIALOG_DESTROY_WITH_PARENT, _("_cancel"),
            GTK_RESPONSE_NONE, _("_delete"), GTK_RESPONSE_YES, NULL);
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "SELECT imgid FROM main.tagged_images WHERE tagid=?1", -1, &stmt,
                                NULL);
    ...
    tagged_images =
        g_list_reverse(tagged_images); // list was built in reverse order, so unreverse it
    ...
    GtkTreeIter store_iter;
    GtkTreeModel *store = gtk_tree_model_filter_get_model(GTK_TREE_MODEL_FILTER(model));
    gtk_tree_model_filter_convert_iter_to_child_iter(GTK_TREE_MODEL_FILTER(model), &store_iter,
                                                     &iter);
...
static void _pop_menu_dictionary_delete_node(GtkWidget *menuitem, dt_lib_module_t *self)
{
    ...
    GtkWidget *win = dt_ui_main_window(darktable.gui->ui);
    GtkWidget *dialog = gtk_dialog_new_with_buttons(
        _("delete node?"), GTK_WINDOW(win), GTK_DIALOG_DESTROY_WITH_PARENT, _("_cancel"),
        GTK_RESPONSE_NONE, _("_delete"), GTK_RESPONSE_YES, NULL);
    gtk_dialog_set_default_response(GTK_DIALOG(dialog), GTK_RESPONSE_NONE);
    ...
    text = g_strdup_printf(
        ngettext("<u>%d</u> tag will be deleted", "<u>%d</u> tags will be deleted", tag_count),
        tag_count);
    GtkWidget *label2 = gtk_label_new(NULL);
    gtk_label_set_markup(GTK_LABEL(label2), text);
    g_free(text);
    ...
    text = g_strdup_printf(
        ngettext("<u>%d</u> image will be updated", "<u>%d</u> images will be updated", img_count),
        img_count);
    ...
    dt_osx_disallow_fullscreen(dialog);
#endif
    dt_gui_dialog_add(GTK_DIALOG(dialog), label1, dt_gui_hbox(label2, gtk_label_new("  "), label3));
    gtk_widget_show_all(dialog);
...
static void _name_changed(GtkEntry *entry, GtkDialog *dialog)
{
...
static void _pop_menu_dictionary_create_tag(GtkWidget *menuitem, dt_lib_module_t *self)
{
    ...
    gtk_tree_model_get(model, &iter, DT_LIB_TAGGING_COL_TAG, &tagname, DT_LIB_TAGGING_COL_PATH,
                       &path, DT_LIB_TAGGING_COL_ID, &tagid, -1);

    GtkWidget *win = dt_ui_main_window(darktable.gui->ui);
    GtkWidget *dialog = gtk_dialog_new_with_buttons(
        _("create tag"), GTK_WINDOW(win), GTK_DIALOG_DESTROY_WITH_PARENT, _("_cancel"),
        GTK_RESPONSE_NONE, _("_save"), GTK_RESPONSE_YES, NULL);
...
private
    = gtk_check_button_new_with_label(_("private"));
...
#endif
    dt_gui_dialog_add(
        GTK_DIALOG(dialog), dt_gui_hbox(gtk_label_new(_("name: ")), dt_gui_expand(entry)), private,
        category, parent, dt_gui_hbox(gtk_label_new(_("synonyms: ")), dt_gui_expand(synonyms)));
    gtk_widget_show_all(dialog);
    ...
    if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_YES)
    {
        ...
        if (message)
        {
            GtkWidget *warning_dialog =
                gtk_message_dialog_new(GTK_WINDOW(dialog), GTK_DIALOG_MODAL, GTK_MESSAGE_INFO,
        ...
        {
            const gint new_flags =
                ((gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(category)) ? DT_TF_CATEGORY : 0) |
                 (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(private)) ? DT_TF_PRIVATE : 0));
            if (new_tagid)
                dt_tag_set_flags(new_tagid, new_flags);
...
static void _pop_menu_dictionary_edit_tag(GtkWidget *menuitem, dt_lib_module_t *self)
{
    ...
    gtk_tree_model_get(model, &iter, DT_LIB_TAGGING_COL_PATH, &tagname, DT_LIB_TAGGING_COL_SYNONYM,
                       &synonyms_list, DT_LIB_TAGGING_COL_ID, &tagid, -1);
    ...
    GtkWidget *win = dt_ui_main_window(darktable.gui->ui);
    GtkWidget *dialog = gtk_dialog_new_with_buttons(
        _("edit"), GTK_WINDOW(win), GTK_DIALOG_DESTROY_WITH_PARENT, _("_cancel"), GTK_RESPONSE_NONE,
    ...
    text = g_strdup_printf(
        ngettext("<u>%d</u> tag will be updated", "<u>%d</u> tags will be updated", tag_count),
        tag_count);
    GtkWidget *label2 = gtk_label_new(NULL);
    gtk_label_set_markup(GTK_LABEL(label2), text);
    g_free(text);
    ...
    private
        ...
        buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(synonyms));
        if (synonyms_list)
            gtk_text_buffer_set_text(buffer, synonyms_list, -1);

        dt_gui_dialog_add(GTK_DIALOG(dialog), private, category,
    ...
    if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_YES)
    {
        ...
        if (g_strcmp0(newtag, subtag ? subtag : tagname) != 0)
        {
            ...
            for (GList *taglist = tag_family; taglist && !tagname_exists;
                 taglist = g_list_next(taglist))
            {
                char *new_tagname = g_strconcat(
                    new_prefix_tag, &((dt_tag_t *)taglist->data)->tag[tagname_len], NULL);
            ...
            if (to)
            {
                ...
                gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(store),
                                                     ...
                                                     GTK_SORT_ASCENDING);
                gtk_tree_model_foreach(store, (GtkTreeModelForeachFunc)_update_tag_name_per_name,
                                       to);
                gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(store), sort_column,
                                                     sort_order);
                g_free(to);
            }
            if (subtag)
        ...
        if (tagid)
        {
            ...
            if (new_flags != (flags & (DT_TF_CATEGORY | DT_TF_PRIVATE)))
            {
                ...
                if (!d->tree_flag)
                    gtk_list_store_set(GTK_LIST_STORE(store), &store_iter, DT_LIB_TAGGING_COL_FLAGS,
                                       new_flags, -1);
                else
                    gtk_tree_store_set(GTK_TREE_STORE(store), &store_iter, DT_LIB_TAGGING_COL_FLAGS,
                                       new_flags, -1);
            }
            if (new_synonyms_list && g_strcmp0(synonyms_list, new_synonyms_list) != 0)
            {
                ...
                if (!d->tree_flag)
                    gtk_list_store_set(GTK_LIST_STORE(store), &store_iter,
                                       DT_LIB_TAGGING_COL_SYNONYM, new_synonyms_list, -1);
                else
...
static gboolean _apply_rename_path(GtkWidget *dialog, const char *tagname, const char *newtag,
                                   dt_lib_module_t *self)
{
    dt_lib_tagging_t *d = self->data;
    ...
    {
        char *new_tagname =
            g_strconcat(newtag, &((dt_tag_t *)taglist->data)->tag[tagname_len], NULL);
        tagname_exists = dt_tag_exists(new_tagname, NULL);
        if (tagname_exists)
        {
            GtkWidget *win;
            ...
            GtkWidget *warning_dialog = gtk_message_dialog_new(
                GTK_WINDOW(win), GTK_DIALOG_MODAL, GTK_MESSAGE_INFO, GTK_BUTTONS_CLOSE,
                _("at least one new tag name (%s) already exists, aborting"), new_tagname);
...
static void _pop_menu_dictionary_change_path(GtkWidget *menuitem, dt_lib_module_t *self)
{
    ...
    GtkWidget *win = dt_ui_main_window(darktable.gui->ui);
    GtkWidget *dialog = gtk_dialog_new_with_buttons(
        _("change path"), GTK_WINDOW(win), GTK_DIALOG_DESTROY_WITH_PARENT, _("_cancel"),
        GTK_RESPONSE_NONE, _("_save"), GTK_RESPONSE_YES, NULL);
    gtk_dialog_set_default_response(GTK_DIALOG(dialog), GTK_RESPONSE_YES);
    ...
    if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_YES)
    {
        ...
        if (!newtag[0])
            message = _("empty tag is not allowed, aborting");
        if (strchr(newtag, '|') == &newtag[0] ||
            strchr(newtag, '|') == &newtag[strlen(newtag) - 1] || strstr(newtag, "||"))
...
static void _pop_menu_dictionary_goto_tag_collection(GtkWidget *menuitem, dt_lib_module_t *self)
{
    ...
    if (gtk_tree_selection_get_selected(selection, &model, &iter))
    {
        char *path;
        guint count;
        gtk_tree_model_get(model, &iter, DT_LIB_TAGGING_COL_PATH, &path, DT_LIB_TAGGING_COL_COUNT,
                           &count, -1);
        if (count)
        {
            if (!d->collection[0])
...
static void _pop_menu_dictionary(GtkWidget *treeview, GdkEventButton *event, dt_lib_module_t *self)
{
    ...
    {
        guint tagid, count;
        gtk_tree_model_get(model, &iter, DT_LIB_TAGGING_COL_ID, &tagid, DT_LIB_TAGGING_COL_COUNT,
                           &count, -1);
...
static gboolean _click_on_view_dictionary(GtkWidget *view, GdkEventButton *event,
                                          dt_lib_module_t *self)
{
    dt_lib_tagging_t *d = self->data;
    _unselect_all_in_view(d->attached_view);
...
static gboolean _row_tooltip_setup(GtkWidget *treeview, const gint x, const gint y,
                                   const gboolean kb_mode, GtkTooltip *tooltip,
                                   dt_lib_module_t *self)
{
    gboolean res = FALSE;
    GtkTreePath *path = NULL;
    ...
    if (gtk_tree_view_get_path_at_pos(GTK_TREE_VIEW(treeview), x, y, &path, NULL, NULL, NULL))
    {
        ...
        if (gtk_tree_model_get_iter(model, &iter, path))
        {
            ...
            guint flags;
            char *synonyms = NULL;
            gtk_tree_model_get(model, &iter, DT_LIB_TAGGING_COL_ID, &tagid, DT_LIB_TAGGING_COL_TAG,
                               &tagname, DT_LIB_TAGGING_COL_FLAGS, &flags,
                               DT_LIB_TAGGING_COL_SYNONYM, &synonyms, -1);
            ...
            if (tagid)
            {
                if ((flags & DT_TF_PRIVATE) || (synonyms && synonyms[0]))
                {
                    ...
                    dt_util_str_cat(&text, "synonyms: %s",
...
static void _import_button_clicked(GtkButton *button, dt_lib_module_t *self)
{
    const char *last_dirname =
        dt_conf_get_string_const("plugins/lighttable/tagging/last_import_export_location");
    ...
    GtkFileChooserNative *filechooser =
        gtk_file_chooser_native_new(_("select a keyword file"), GTK_WINDOW(win),
...
static void _export_button_clicked(GtkButton *button, dt_lib_module_t *self)
{
    ...
    GtkWidget *win = dt_ui_main_window(darktable.gui->ui);
    GtkFileChooserNative *filechooser =
        gtk_file_chooser_native_new(_("select file to export to"), GTK_WINDOW(win),
...
static void _update_layout(dt_lib_module_t *self)
{
    ...
    if (d->tree_flag)
    {
        if (model == GTK_TREE_MODEL(d->dictionary_listfilter))
        {
            ...
            GtkTreeModel *store = gtk_tree_model_filter_get_model(GTK_TREE_MODEL_FILTER(model));
            gtk_list_store_clear(GTK_LIST_STORE(store));
            gtk_tree_view_set_model(GTK_TREE_VIEW(d->dictionary_view),
                                    GTK_TREE_MODEL(d->dictionary_treefilter));
    ...
    else
    {
        if (model == GTK_TREE_MODEL(d->dictionary_treefilter))
        {
            ...
            gtk_tree_store_clear(GTK_TREE_STORE(store));
            gtk_tree_view_set_model(GTK_TREE_VIEW(d->dictionary_view),
                                    GTK_TREE_MODEL(d->dictionary_listfilter));
    ...
    // drag & drop
    if (d->tree_flag)
        gtk_drag_source_set(GTK_WIDGET(d->dictionary_view), GDK_BUTTON1_MASK, target_list_tags,
                            n_targets_tags, GDK_ACTION_MOVE);
    else
...
static gint _sort_tree_count_func(GtkTreeModel *model, GtkTreeIter *a, GtkTreeIter *b,
                                  dt_lib_module_t *self)
{
    guint count_a = 0;
    guint count_b = 0;
...
static gint _sort_tree_tag_func(GtkTreeModel *model, GtkTreeIter *a, GtkTreeIter *b,
                                dt_lib_module_t *self)
{
    char *tag_a = NULL;
    char *tag_b = NULL;
...
static gint _sort_tree_path_func(GtkTreeModel *model, GtkTreeIter *a, GtkTreeIter *b,
                                 dt_lib_module_t *self)
{
    char *tag_a = NULL;
    char *tag_b = NULL;
...
static gboolean _match_selected_func(GtkEntryCompletion *completion, GtkTreeModel *model,
...
static gboolean _completion_match_func(GtkEntryCompletion *completion, const gchar *key,
                                       GtkTreeIter *iter, gpointer user_data)
{
    gboolean res = FALSE;
...
static void _event_dnd_get(GtkWidget *widget, GdkDragContext *context,
                           ...
                           const guint time, dt_lib_module_t *self)
{
...
static void _event_dnd_received(GtkWidget *widget, GdkDragContext *context, const gint x,
...
static gboolean _dnd_scroll_timeout(dt_lib_module_t *self)
{
    ...
    if (d->drag.scroll_timeout)
    {
        ...
        gint top_ty;
        gtk_tree_view_convert_bin_window_to_tree_coords(d->dictionary_view, 0, 0, NULL, &top_ty);
        ...
        if (d->drag.tag_source && !d->drag.root && d->drag.last_y < 5 && top_ty < 1)
        {
            ...
            gtk_tree_store_set(d->dictionary_treestore, &iter, DT_LIB_TAGGING_COL_TAG,
                               _("drop to root"), DT_LIB_TAGGING_COL_ID, 0, DT_LIB_TAGGING_COL_PATH,
                               "", DT_LIB_TAGGING_COL_COUNT, 0, DT_LIB_TAGGING_COL_SEL, 0,
                               DT_LIB_TAGGING_COL_FLAGS, 0, DT_LIB_TAGGING_COL_VISIBLE, TRUE, -1);
            d->drag.root = TRUE;
        }
...
static gboolean _event_dnd_motion(GtkWidget *widget, GdkDragContext *context, const gint x,
                                  const gint y, const guint time, dt_lib_module_t *self)
{
    dt_lib_tagging_t *d = self->data;
    GtkTreeView *tree = GTK_TREE_VIEW(widget);
    GtkTreePath *path = NULL;
    ...
    {
        if (!d->drag.lastpath ||
            ((d->drag.lastpath) && gtk_tree_path_compare(d->drag.lastpath, path) != 0))
        {
            ...
            if (x >= sel_width)
            {
                if (!gtk_tree_view_row_expanded(tree, path))
                    d->drag.expand_timeout =
                        g_timeout_add(200, (GSourceFunc)_dnd_expand_timeout, self);
            }
        }
...
void gui_init(dt_lib_module_t *self)
{
    ...
    liststore =
        gtk_list_store_new(DT_LIB_TAGGING_NUM_COLS, G_TYPE_STRING, G_TYPE_UINT, G_TYPE_STRING,
    ...
    g_signal_connect(G_OBJECT(view), "query-tooltip", G_CALLBACK(_row_tooltip_setup),
                     (gpointer)self);
    ...
    gtk_tree_view_column_pack_start(col, renderer, TRUE);
    gtk_tree_view_column_set_cell_data_func(col, renderer, _tree_tagname_show_attached,
                                            (gpointer)self, NULL);
    ...
    g_signal_connect(G_OBJECT(view), "button-press-event", G_CALLBACK(_click_on_view_attached),
                     (gpointer)self);
    g_signal_connect(G_OBJECT(view), "key-press-event", G_CALLBACK(_attached_key_pressed),
                     (gpointer)self);
...
#define NEW_TOGGLE_BUTTON(paint, callback, tooltip, action)                                        \
    ...
    d->toggle_hide_button =
        NEW_TOGGLE_BUTTON(dtgtk_cairo_paint_minus_simple, _toggle_hide_button_callback,
                          _("toggle list with / without hierarchy"), N_("hide"));
    d->toggle_sort_button =
        NEW_TOGGLE_BUTTON(dtgtk_cairo_paint_sorting, _toggle_sort_button_callback,
                          _("toggle sort by name or by count"), N_("sort"));
    d->toggle_dttags_button =
        NEW_TOGGLE_BUTTON(dtgtk_cairo_paint_check_mark, _toggle_dttags_button_callback,
    ...
    g_signal_connect(G_OBJECT(w), "key-press-event", G_CALLBACK(_enter_key_pressed),
                     (gpointer)self);
    d->entry = GTK_ENTRY(w);
    ...
    gtk_box_pack_end(hbox, button, FALSE, TRUE, 0);
    g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(_clear_entry_button_callback),
                     (gpointer)self);
    gtk_box_pack_start(box, GTK_WIDGET(hbox), FALSE, TRUE, 0);
    ...
    d->dictionary_listfilter = GTK_TREE_MODEL_FILTER(model);
    treestore =
        gtk_tree_store_new(DT_LIB_TAGGING_NUM_COLS, G_TYPE_STRING, G_TYPE_UINT, G_TYPE_STRING,
                           G_TYPE_STRING, G_TYPE_UINT, G_TYPE_UINT, G_TYPE_UINT, G_TYPE_BOOLEAN);
    ...
    g_object_set(renderer, "ellipsize", PANGO_ELLIPSIZE_MIDDLE, (gchar *)0);
    gtk_tree_view_column_pack_start(col, renderer, TRUE);
    gtk_tree_view_column_set_cell_data_func(col, renderer, _tree_tagname_show_dictionary,
                                            (gpointer)self, NULL);
    gtk_tree_view_set_expander_column(view, col);

    gtk_tree_selection_set_mode(gtk_tree_view_get_selection(view), GTK_SELECTION_SINGLE);
    gtk_widget_set_tooltip_text(
        GTK_WIDGET(view), _("tag dictionary,"
    ...
    g_signal_connect(G_OBJECT(view), "button-press-event", G_CALLBACK(_click_on_view_dictionary),
                     (gpointer)self);
    g_signal_connect(G_OBJECT(view), "key-press-event", G_CALLBACK(_dictionary_key_pressed),
                     (gpointer)self);
    ...
    // drag & drop
    {
        ...
        g_signal_connect(d->dictionary_view, "drag-data-received", G_CALLBACK(_event_dnd_received),
                         self);
        g_signal_connect_after(d->dictionary_view, "drag-begin", G_CALLBACK(_event_dnd_begin),
                               self);
    ...
    d->new_button = dt_action_button_new(self, N_("new"), _new_button_clicked, self,
                                         _("create a new tag with the\nname you entered"), 0, 0);
    gtk_box_pack_start(hbox, d->new_button, TRUE, TRUE, 0);

    d->import_button = dt_action_button_new(self, N_("import..."), _import_button_clicked, self,
                                            _("import tags from a Lightroom keyword file"), 0, 0);
    gtk_box_pack_start(hbox, d->import_button, TRUE, TRUE, 0);

    d->export_button = dt_action_button_new(self, N_("export..."), _export_button_clicked, self,
                                            _("export all tags to a Lightroom keyword file"), 0, 0);
    gtk_box_pack_start(hbox, d->export_button, TRUE, TRUE, 0);

    d->toggle_tree_button =
        NEW_TOGGLE_BUTTON(dtgtk_cairo_paint_treelist, _toggle_tree_button_callback,
                          _("toggle list / tree view"), N_("tree"));
    d->toggle_suggestion_button =
        NEW_TOGGLE_BUTTON(dtgtk_cairo_paint_plus_simple, _toggle_suggestion_button_callback,
...
#undef NEW_TOGGLE_BUTTON
    ...
    dt_action_register(DT_ACTION(self), N_("tag"), _lib_tagging_tag_show, GDK_KEY_t,
                       GDK_CONTROL_MASK);
...
static gboolean _lib_tagging_tag_key_press(GtkWidget *entry, GdkEventKey *event,
                                           dt_lib_module_t *self)
{
    dt_lib_tagging_t *d = self->data;
    switch (event->keyval)
...
static void _lib_tagging_tag_show(dt_action_t *action)
{
    ...
    {
        dt_control_log(
            _("tag shortcut is not active with tag tree view. please switch to list view"));
    ...
    gtk_entry_completion_set_model(completion,
                                   gtk_tree_view_get_model(GTK_TREE_VIEW(d->dictionary_view)));
    gtk_entry_completion_set_text_column(completion, DT_LIB_TAGGING_COL_PATH);
    gtk_entry_completion_set_inline_completion(completion, TRUE);
    gtk_entry_completion_set_popup_set_width(completion, FALSE);
    g_signal_connect(G_OBJECT(completion), "match-selected", G_CALLBACK(_match_selected_func),
                     self);
    ...
    g_signal_connect(entry, "focus-out-event", G_CALLBACK(_lib_tagging_tag_destroy),
...
void _menuitem_preferences(GtkMenuItem *menuitem, dt_lib_module_t *self)
{
    GtkWidget *win = dt_ui_main_window(darktable.gui->ui);
    GtkWidget *dialog = gtk_dialog_new_with_buttons(
        _("tagging settings"), GTK_WINDOW(win), GTK_DIALOG_DESTROY_WITH_PARENT, _("_cancel"),
