...
static void _lib_navigation_pinch_scale_callback(GtkGesture *gesture, gdouble scale,
                                                 dt_lib_module_t *self);
...
static void _lib_navigation_set_position(struct dt_lib_module_t *self, const double x,
                                         const double y, const int wd, const int ht);
...
static void _lib_navigation_control_redraw_callback(gpointer instance, dt_lib_module_t *self)
{
    ...
    gchar *zoomline =
        zoom == DT_ZOOM_FIT  ? g_strdup(_("fit")) :
...
int _entry_select(GtkWidget *w, const char *text, const int delta, struct dt_iop_module_t **module)
{
...
void gui_init(dt_lib_module_t *self)
{
    ...
    g_signal_connect(zoom_gesture, "scale-changed",
                     G_CALLBACK(_lib_navigation_pinch_scale_callback), self);
    ...
    dt_action_register(ac, NULL, _lib_navigation_collapse_callback, GDK_KEY_N,
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(d->zoom, darktable.view_manager->proxy.darkroom.view, NULL,
                                 ...
                                 N_("small"), N_("fit"), NC_("navigationbox", "fill"), N_("50%"),
                                 N_("100%"), N_("200%"), N_("400%"), N_("800%"), N_("1600%"));
                                  ...
                                  const int wd, const int ht)
{
    ...
    if (d->dragging)
    {
        ...
        float zoom_x =
            fmaxf(-.5, fminf(((x - inset) / width - .5f) /
                                 (iwd * fminf(wd / (float)iwd, ht / (float)iht) / (float)wd),
                             .5));
        float zoom_y =
            fmaxf(-.5, fminf(((y - inset) / height - .5f) /
                                 (iht * fminf(wd / (float)iwd, ht / (float)iht) / (float)ht),
                             .5));
...
static void _zoom_changed(GtkWidget *widget, gpointer user_data)
{
    ...
    dt_dev_zoom_move(port, zoom, scale, closeup, -1.0f, -1.0f, zoom != DT_ZOOM_FREE);
}
                                                 ...
                                                 const gdouble in_y, gdouble *out_x, gdouble *out_y)
{
    ...
    const double nav_scale = MIN((double)alloc_nav.width / procw, (double)alloc_nav.height / proch);
                                            ...
                                            gpointer user_data)
{
    ...
    if (event)
    {
        ...
        {
            dt_dev_zoom_move(&darktable.develop->full, DT_ZOOM_MOVE, 15.0, 0, dx, dy, TRUE);
        }
        else
        {
            const gboolean constrain = !dt_modifier_eq(controller, GDK_CONTROL_MASK);
            gdouble x, y;
            if (_lib_navigation_widget_to_center(GTK_EVENT_CONTROLLER(controller), event->scroll.x,
                                                 event->scroll.y, &x, &y))
                dt_dev_zoom_move(&darktable.develop->full, DT_ZOOM_SCROLL, 0.0f, dy < 0, x, y,
                                 constrain);
        }
                                                 ...
                                                 dt_lib_module_t *self)
{
        ...
        _lib_navigation_widget_to_center(GTK_EVENT_CONTROLLER(gesture), wx, wy, &cx, &cy))
    {
        ...
        if (zoom_step >= 0)
        {
            dt_dev_zoom_move(&darktable.develop->full, DT_ZOOM_SCROLL, 0.0f, zoom_step, cx, cy,
                             TRUE);
                                                      ...
                                                      dt_lib_module_t *self)
{
    ...
    {
        d->dragging = 1;
        _lib_navigation_set_position(self, event->button.x, event->button.y, allocation.width,
                                     allocation.height);
