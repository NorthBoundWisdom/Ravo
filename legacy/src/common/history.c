...
static void _remove_preset_flag(const dt_imgid_t imgid)
{
    ...
    dt_image_cache_write_release_info(image, DT_IMAGE_CACHE_SAFE, "_remove_preset_flag");
}
                                    ...
                                    const gboolean init_history)
{
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "DELETE FROM main.module_order WHERE imgid = ?1", -1, &stmt, NULL);
    DT_DEBUG_SQLITE3_BIND_INT(stmt, 1, imgid);
    sqlite3_step(stmt);
    sqlite3_finalize(stmt);
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "DELETE FROM main.history_hash WHERE imgid = ?1", -1, &stmt, NULL);
    DT_DEBUG_SQLITE3_BIND_INT(stmt, 1, imgid);
    sqlite3_step(stmt);
    sqlite3_finalize(stmt);
    ...
    /* We leave the signal that the tags may have changed to callers */
...
gboolean dt_history_load_and_apply(const dt_imgid_t imgid, gchar *filename,
...
gboolean dt_history_merge_module_into_history(dt_develop_t *dev_dest, dt_develop_t *dev_src,
                                              dt_iop_module_t *mod_src, GList **_modules_used,
                                              const gboolean append, const gboolean auto_init)
{
    ...
    if (module_added && mod_replace == NULL && (!append || !mod_src->enabled))
    {
        ...
        for (GList *modules_dest = dev_dest->iop; modules_dest;
             modules_dest = g_list_next(modules_dest))
        {
                ...
                _search_list_iop_by_module(modules_used, mod_dest) == NULL)
            {
                ...
                else if (memcmp(mod_dest->params, mod_dest->default_params,
                                mod_dest->params_size) == 0 &&
                         memcmp(mod_dest->blend_params, mod_dest->default_blendop_params,
        ...
        dt_iop_module_t *mod_dest =
            mod_same_name ?
                ...
                (mod_unused ? mod_unused :
                              (mod_default ? mod_default : (mod_same_prio ? mod_same_prio : NULL)));
    ...
    if (module_added && mod_replace == NULL)
    {
        ...
        if (module_duplicate &&
            (module_duplicate->iop_order <= 0 || module_duplicate->iop_order == INT_MAX))
            dt_print(DT_DEBUG_ALWAYS,
                     "[dt_history_merge_module_into_history]"
        ...
        if (dev_src)
        {
                ...
                dt_is_valid_maskid(mod_src->blend_params->mask_id))
            {
                ...
                if (forms_used_replace)
                    _fill_used_forms(dev_src->forms, mod_src->blend_params->mask_id,
...
static gboolean _history_copy_and_paste_on_image_merge(const dt_imgid_t imgid,
                                                       const dt_imgid_t dest_imgid, GList *ops,
                                                           ...
                                                           const gboolean copy_full)
{
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2
      (dt_database_get(darktable.db),
       ...
       -1, &stmt, NULL);
        ...
        DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                    "DELETE FROM main.history_hash WHERE imgid = ?1", -1, &stmt,
                                    NULL);
        DT_DEBUG_SQLITE3_BIND_INT(stmt, 1, dest_imgid);
        sqlite3_step(stmt);
        sqlite3_finalize(stmt);
    ...
    else
    {
        ...
        return _history_copy_and_paste_on_image_merge(imgid, dest_imgid, ops, copy_iop_order,
                                                      copy_full);
    }
}

gboolean dt_history_copy_and_paste_on_image(const dt_imgid_t imgid, const dt_imgid_t dest_imgid,
                                            ...
                                            const gboolean sync)
{
    ...
    const gboolean ret_val = merge ? _history_copy_and_paste_on_image_merge(
...
static char *_history_item_as_string(const char *name, const gboolean enabled)
{
    return g_strconcat(enabled ? "●" : "○", "  ", name, NULL);
}

char *dt_history_get_name_label(const char *name, const char *label, const gboolean markup,
                                const gboolean hand_edited)
{
    char *result = NULL;

    if (!label || strlen(label) == 0 || strcmp(label, "0") == 0)
    {
    ...
    {
        char *l_label =
            hand_edited ? g_strdup(label) : dt_util_localize_segmented_name(label, FALSE);

        result = markup ? g_markup_printf_escaped("%s • <small>%s</small>", name, l_label) :
...
GList *dt_history_get_items(const dt_imgid_t imgid, const gboolean enabled,
                            const gboolean multi_priority_order, const gboolean markup)
{
    if (!dt_is_valid_imgid(imgid))
        return NULL;
    ...
    gchar *query = g_strdup_printf("SELECT num, operation, enabled, multi_name, blendop_params,"
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        item->name =
            dt_history_get_name_label(dt_iop_get_localized_name(op), mname, markup, hand_edited);
        item->op = g_strdup(op);
...
char *dt_history_get_items_as_string(const dt_imgid_t imgid)
{
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2(
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        char *iname =
            _history_item_as_string(dt_iop_get_localized_name((char *)sqlite3_column_text(stmt, 0)),
...
static int _history_end_attop(const dt_imgid_t imgid)
{
    int size = 0;
    int end = 0;
    sqlite3_stmt *stmt;
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "SELECT MAX(num) FROM main.history WHERE imgid=?1", -1, &stmt,
                                NULL);
    DT_DEBUG_SQLITE3_BIND_INT(stmt, 1, imgid);
...
void dt_history_compress_on_image(const dt_imgid_t imgid)
{
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2
    ...
    if (masks_count > 0)
    {
        ...
        // manager at slot 0.
        DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                    "UPDATE main.masks_history SET num = 0 WHERE imgid = ?1", -1,
                                    &stmt, NULL);
        DT_DEBUG_SQLITE3_BIND_INT(stmt, 1, imgid);
        sqlite3_step(stmt);
        sqlite3_finalize(stmt);
...
gboolean dt_history_compress(const dt_imgid_t imgid)
{
    ...
    dt_lock_image(imgid);
    const int test = _history_end_attop(imgid);
    if (test == 1) // we do a compression and we know for sure history_end is at the top!
    {
        ...
        // get highest num in history
        DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                    "SELECT MAX(num) FROM main.history WHERE imgid=?1", -1, &stmt2,
                                    NULL);
        ...
        DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                    "SELECT COUNT(*) FROM main.history WHERE imgid = ?1", -1,
                                    &stmt2, NULL);
        DT_DEBUG_SQLITE3_BIND_INT(stmt2, 1, imgid);
        if (sqlite3_step(stmt2) == SQLITE_ROW)
...
gboolean dt_history_check_module_exists(const dt_imgid_t imgid, const char *operation,
...
gboolean dt_history_check_module_exists_list(GList *hist, const char *operation,
                                             const gboolean enabled)
{
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        if (enabled)
        {
            ...
            if (buf)
                g_checksum_update(checksum, (const guchar *)buf, -1);
            // op_params
            buf = (char *)sqlite3_column_blob(stmt, 1);
            int params_len = sqlite3_column_bytes(stmt, 1);
            if (buf)
                g_checksum_update(checksum, (const guchar *)buf, params_len);
            // blendop_params
...
void dt_history_hash_write(const dt_imgid_t imgid, const dt_history_hash_values_t *const hash)
{
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
        ...
        DT_DEBUG_SQLITE3_BIND_BLOB(stmt, 3, hash->auto_apply, hash->auto_apply_len,
                                   SQLITE_TRANSIENT);
...
{
    gboolean res =
        dt_history_copy_and_paste_on_image(darktable.view_manager->copy_paste.copied_imageid, imgid,
                                           merge, darktable.view_manager->copy_paste.selops,
                                           darktable.view_manager->copy_paste.copy_iop_order,
                                           darktable.view_manager->copy_paste.full_copy, sync);
