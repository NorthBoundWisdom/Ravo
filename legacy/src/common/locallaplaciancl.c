                           ...
                           const float clarity)    // user param: increase clarity/local contrast
{
    ...
    {
        g->dev_padded[l] =
            dt_opencl_alloc_device(devid, ROUNDUPDWD(dl(g->bwidth, l), devid),
                                   ROUNDUPDHT(dl(g->bheight, l), devid), sizeof(float));
        if (!g->dev_padded[l])
            goto error;
        g->dev_output[l] =
            dt_opencl_alloc_device(devid, ROUNDUPDWD(dl(g->bwidth, l), devid),
                             ...
                             cl_mem output)              // output buffer with colour
{
    ...
    cl_int err = dt_opencl_enqueue_kernel_2d_args(
        b->devid, b->global->kernel_pad_input, b->bwidth, b->bheight, CLARG(input),
        CLARG(b->dev_padded[0]), CLARG(b->width), CLARG(b->height), CLARG(b->max_supp),
        CLARG(b->bwidth), CLARG(b->bheight));
    if (err != CL_SUCCESS)
        goto error;
    ...
    for (int l = 1; l < b->num_levels; l++)
    {
        ...
        if (l == b->num_levels - 1)
            err = dt_opencl_enqueue_kernel_2d_args(b->devid, b->global->kernel_gauss_reduce, wd, ht,
                                                   ...
                                                   CLARG(b->dev_output[l]), CLARG(wd), CLARG(ht));
        else
            err = dt_opencl_enqueue_kernel_2d_args(b->devid, b->global->kernel_gauss_reduce, wd, ht,
                                                   CLARG(b->dev_padded[l - 1]),
                                                   CLARG(b->dev_padded[l]), CLARG(wd), CLARG(ht));
        if (err != CL_SUCCESS)
            goto error;
    }
    ...
    { // process images
        ...
        err = dt_opencl_enqueue_kernel_2d_args(
            b->devid, b->global->kernel_process_curve, b->bwidth, b->bheight,
            CLARG(b->dev_padded[0]), CLARG(b->dev_processed[k][0]), CLARG(g), CLARG(b->sigma),
            CLARG(b->shadows), CLARG(b->highlights), CLARG(b->clarity), CLARG(b->bwidth),
            CLARG(b->bheight));
        if (err != CL_SUCCESS)
            goto error;
        ...
        for (int l = 1; l < b->num_levels; l++)
        {
            ...
            err = dt_opencl_enqueue_kernel_2d_args(
                b->devid, b->global->kernel_gauss_reduce, wd, ht, CLARG(b->dev_processed[k][l - 1]),
                CLARG(b->dev_processed[k][l]), CLARG(wd), CLARG(ht));
            if (err != CL_SUCCESS)
                goto error;
        }
    }
    ...
    for (int l = b->num_levels - 2; l >= 0; l--)
    {
        ...
        err = dt_opencl_enqueue_kernel_2d_args(
            b->devid, b->global->kernel_laplacian_assemble, pw, ph, CLARG(b->dev_padded[l]),
            CLARG(b->dev_output[l + 1]), CLARG(b->dev_output[l]), CLARG(b->dev_processed[0][l]),
            CLARG(b->dev_processed[0][l + 1]), CLARG(b->dev_processed[1][l]),
            CLARG(b->dev_processed[1][l + 1]), CLARG(b->dev_processed[2][l]),
            CLARG(b->dev_processed[2][l + 1]), CLARG(b->dev_processed[3][l]),
            CLARG(b->dev_processed[3][l + 1]), CLARG(b->dev_processed[4][l]),
            CLARG(b->dev_processed[4][l + 1]), CLARG(b->dev_processed[5][l]),
            CLARG(b->dev_processed[5][l + 1]), CLARG(pw), CLARG(ph));
        if (err != CL_SUCCESS)
            goto error;
    }

    // read back processed L channel and copy colours:
    err = dt_opencl_enqueue_kernel_2d_args(b->devid, b->global->kernel_write_back, b->width,
                                           b->height, CLARG(input), CLARG(b->dev_output[0]),
                                           CLARG(output), CLARG(b->max_supp), CLARG(b->width),
                                           CLARG(b->height));
