...
static inline float _clip_chroma_white_raw(const float coeffs[3], const float target_white,
...
static inline float _clip_chroma_white(const float coeffs[3], const float target_white,
                                       const float Y, const float cos_h, const float sin_h)
{
    ...
    if (delta_Y < eps)
    {
        max_chroma =
            delta_Y / (eps * max_Y) *
            _clip_chroma_white_raw(coeffs, target_white, (1.f - eps) * max_Y, cos_h, sin_h);
    }
    else
    {
...
static inline float _clip_chroma_black(const float coeffs[3], const float cos_h, const float sin_h)
{
    ...
    const float numerator = -0.427506877216495f * (coeffs[0] + 0.856492345150334f * coeffs[1] +
                                                   0.554995960637719f * coeffs[2]);
...
static inline void RGB_to_Ych(const dt_aligned_pixel_t in, const dt_colormatrix_t matrix_trans,
...
DT_OMP_DECLARE_SIMD(uniform(matrix_trans) aligned(in, out : 16) aligned(matrix_trans : 64))
static inline void Ych_to_RGB(const dt_aligned_pixel_t in, const dt_colormatrix_t matrix_trans,
                              dt_aligned_pixel_t out)
{
    dt_aligned_pixel_t LMS = {0.f};
    dt_aligned_pixel_t Yrg = {0.f};
...
static inline float Ych_max_chroma(const dt_colormatrix_t matrix_out, const float target_white,
                                   const float Y, const float cos_h, const float sin_h)
{
