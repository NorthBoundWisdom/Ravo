...
static inline unsigned int num_steps_to_reach_equivalent_sigma(const float sigma_filter,
...
static inline void sparse_scalar_product(const dt_aligned_pixel_t buf,
                                         const size_t indices[BSPLINE_FSIZE],
                                         dt_aligned_pixel_t result, const gboolean clip_negatives)
{
    ...
    static const float filter[BSPLINE_FSIZE] = {1.0f / 16.0f, 4.0f / 16.0f, 6.0f / 16.0f,
    ...
    if (clip_negatives)
    {
        ...
        {
            result[c] =
                MAX(0.0f, filter[0] * buf[indices[0] + c] + filter[1] * buf[indices[1] + c] +
                              filter[2] * buf[indices[2] + c] + filter[3] * buf[indices[3] + c] +
    ...
    else
    {
        for_each_channel(c, aligned(buf, indices, result))
        {
            result[c] = filter[0] * buf[indices[0] + c] + filter[1] * buf[indices[1] + c] +
                        filter[2] * buf[indices[2] + c] + filter[3] * buf[indices[3] + c] +
                        filter[4] * buf[indices[4] + c];
        }
    }
...
static inline void _bspline_vertical_pass(const float *const restrict in,
                                          float *const restrict temp, size_t row, size_t width,
                                          size_t height, int mult, const gboolean clip_negatives)
{
...
static inline void blur_2D_Bspline(const float *const restrict in, float *const restrict out,
...
inline static void decompose_2D_Bspline(const float *const in, float *const HF,
                                        float *const restrict LF, const size_t width,
                                        const size_t height, const int mult, float *const tempbuf,
                                        const size_t padded_size)
{
    ...
    for (size_t row = 0; row < height; row++)
    {
        ...
        for (size_t j = 0; j < width; j++)
        {
            ...
            for_four_channels(c) HF[index + c] = in[index + c] - blur[c];
#else
            _bspline_horizontal(temp, LF + index, j, width, mult, TRUE); // always clip negatives
            // compute the HF component by subtracting the LF from the original input
            for_four_channels(c) HF[index + c] = in[index + c] - LF[index + c];
#endif
        }
    }
#if USE_NONTEMPORAL
