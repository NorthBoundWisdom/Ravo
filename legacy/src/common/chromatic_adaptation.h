...
static const dt_colormatrix_t XYZ_to_Bradford_LMS_trans = {{0.8951f, -0.7502f, 0.0389f, 0.f},
...
static const dt_colormatrix_t Bradford_LMS_to_XYZ_trans = {{0.9870f, 0.4323f, -0.0085f, 0.f},
...
static const dt_colormatrix_t XYZ_to_CAT16_LMS_trans = {{0.401288f, -0.250268f, -0.002079f, 0.f},
...
static const dt_colormatrix_t CAT16_LMS_to_XYZ_trans = {{1.862068f, 0.38752f, -0.015841f, 0.f},
...
static inline void convert_any_LMS_to_RGB(const dt_aligned_pixel_t LMS, dt_aligned_pixel_t RGB,
                                          dt_adaptation_t kind)
{
    // helper function switching internally to the proper conversion
...
static inline void bradford_adapt_D65(const dt_aligned_pixel_t lms_in,
                                      const dt_aligned_pixel_t origin_illuminant, const float p,
                                      const int full, dt_aligned_pixel_t lms_out)
{
    ...
    dt_aligned_pixel_t temp = {lms_in[0] / origin_illuminant[0], lms_in[1] / origin_illuminant[1],
                               lms_in[2] / origin_illuminant[2], 0.f};

    // use linear Bradford if B is negative
    if (full)
        temp[2] = (temp[2] > 0.f) ? powf(temp[2], p) : temp[2];
...
static inline void CAT16_adapt_D65(const dt_aligned_pixel_t lms_in,
                                   const dt_aligned_pixel_t origin_illuminant, const float D,
                                   const int full, dt_aligned_pixel_t lms_out)
{
