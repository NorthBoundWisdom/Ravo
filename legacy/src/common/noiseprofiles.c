...
static gboolean _noiseprofile_verify(JsonParser *parser)
{
    ...
    if (!json_reader_read_member(reader, "noiseprofiles"))
        _ERROR("can't find `noiseprofiles' entry.");

    if (!json_reader_is_array(reader))
        _ERROR("`noiseprofiles' is supposed to be an array");
    ...
    for (int i = 0; i < n_makers; i++)
    {
        ...
        dt_print(DT_DEBUG_CONTROL | DT_DEBUG_VERBOSE, "[noiseprofile] found maker `%s'",
        ...
        for (int j = 0; j < n_models; j++)
        {
            ...
            dt_print(DT_DEBUG_CONTROL | DT_DEBUG_VERBOSE, "[noiseprofile] found %s",
                     json_reader_get_string_value(reader));
            json_reader_end_member(reader);

            if (!json_reader_read_member(reader, "profiles"))
                _ERROR("missing `profiles`");

            const int n_profiles = json_reader_count_elements(reader);
            dt_print(DT_DEBUG_CONTROL | DT_DEBUG_VERBOSE, "[noiseprofile] found %d profiles",
                     n_profiles);
    ...
    dt_print(DT_DEBUG_CONTROL | DT_DEBUG_VERBOSE, "[noiseprofile] found %zu profiles total",
             n_profiles_total);
...
GList *dt_noiseprofile_get_matching(const dt_image_t *cimg)
{
    ...
    dt_print(DT_DEBUG_CONTROL | DT_DEBUG_VERBOSE,
             "[noiseprofile] looking for maker `%s', model `%s'", cimg->camera_maker,
             cimg->camera_model);
    ...
    for (int i = 0; i < n_makers; i++)
    {
        ...
        {
            dt_print(DT_DEBUG_CONTROL | DT_DEBUG_VERBOSE, "[noiseprofile] found `%s' as `%s'",
            ...
            const int n_models = json_reader_count_elements(reader);
            dt_print(DT_DEBUG_CONTROL | DT_DEBUG_VERBOSE, "[noiseprofile] found %d models",
                     n_models);
            for (int j = 0; j < n_models; j++)
            {
                ...
                if (!g_strcmp0(cimg->camera_model, json_reader_get_string_value(reader)))
                {
                    ...
                    const int n_profiles = json_reader_count_elements(reader);
                    dt_print(DT_DEBUG_CONTROL | DT_DEBUG_VERBOSE,
                             "[noiseprofile] found %d profiles", n_profiles);
                    for (int k = 0; k < n_profiles; k++)
                    {
    ...
    if (reader)
        g_object_unref(reader);
    if (result)
        result = g_list_sort(result, _sort_by_iso);
    return result;
}
