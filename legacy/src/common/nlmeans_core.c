...
static int scatter(const float scale, const float scattering, const int index1, const int index2)
{
    ...
    return scale * ((abs_i1 * abs_i1 * abs_i1 + 7.0 * abs_i1 * sqrt(abs_i2)) * sign(index1) *
                        scattering / 6.0 +
                    index1);
}

// allocate and fill an array of patch definitions
static struct patch_t *define_patches(const dt_nlmeans_param_t *const params, const int stride,
                                      int *num_patches, int *max_shift)
{
    ...
    int decimate = params->decimate ?
                       1 :
                       0; // must start at 1 when nonzero, to choose correct patches to skip below
    ...
    *num_patches = n_patches;
    ...
    for (int row_index = -search_radius; row_index <= search_radius; row_index++)
    {
        ...
        {
            if (decimate && (++decimate & 1))
                continue; // skip every other patch
...
static inline float pixel_difference(const dt_aligned_pixel_t pix1, const dt_aligned_pixel_t pix2,
...
static inline float diff_of_pixels_diff(const dt_aligned_pixel_t pix1,
...
static inline float get_pixdiff(const float *const col_sums, const int radius, const int row,
                                const int col)
{
...
static inline void set_pixdiff(float *const col_sums, const int radius, const int row,
                               const int col, const float diff)
{
    const int stride = 2 * (radius + 1);
    const int modrow = 1 + (row + stride) % stride;
...
static void init_column_sums(float *const col_sums, const patch_t *const patch,
                             const float *const in, const int row, const int chunk_left,
                             const int chunk_right, const int height, const int width,
                             const int stride, const int radius, const float *const norm)
{
    ...
    const int col_max =
        chunk_right + MIN(radius, MIN(width - chunk_right, width - (chunk_right + scol)));
                     ...
                     const dt_nlmeans_param_t *const params)
{
    ...
    const dt_aligned_pixel_t invert = {1.0f - params->luma, 1.0f - params->chroma,
                                       1.0f - params->chroma, 0.0f};
    ...
    const size_t scratch_size =
        SLICE_WIDTH + 2 * radius + 1 + 48; // getting false sharing without the +48....
#endif /* CACHE_PIXDIFFS */
    size_t padded_scratch_size;
    float *const restrict scratch_buf =
        dt_alloc_perthread_float(scratch_size, &padded_scratch_size);
    ...
    for (int chunk_top = 0; chunk_top < roi_out->height; chunk_top += chk_height)
    {
        for (int chunk_left = 0; chunk_left < roi_out->width; chunk_left += chk_width)
        {
            ...
            {
                memset(outbuf + 4 * (i * roi_out->width + chunk_left), '\0',
                       sizeof(float) * 4 * (chunk_right - chunk_left));
            }
            ...
            for (int p = 0; p < num_patches; p++)
            {
                ...
                init_column_sums(col_sums, patch, inbuf, row_min, chunk_left, chunk_right, height,
                                 width, stride, radius, params->norm);
                for (int row = row_min; row < row_max; row++)
                {
                    ...
                    if (params->center_weight < 0.0f)
                    {
                        ...
                        for (int col = col_min; col < col_max; col++)
                        {
                            ...
                            const dt_aligned_pixel_t pixel = {inpx[offset], inpx[offset + 1],
                                                              inpx[offset + 2], 1.0f};
                    ...
                    else
                    {
                        ...
                        {
                            distortion += (col_sums[col + radius] - col_sums[col - radius - 1]);
                            const float dissimilarity =
                                (distortion + pixel_difference(in + 4 * col, in + 4 * col + offset,
                                                               center_norm)) /
                    ...
                    const int pcol_min =
                        chunk_left - MIN(radius, MIN(chunk_left, chunk_left + scol));
                    const int pcol_max =
                        chunk_right +
                        MIN(radius, MIN(width - chunk_right, width - (chunk_right + scol)));
                    ...
                    else if (row < row_bot)
                    {
                        ...
                        const float *const top_row =
                            inbuf + (row - radius) * stride /* +(2*radius+1)*stride*/;
#endif /* !CACHE_PIXDIFFS */
                        ...
                        for (int col = pcol_min; col < pcol_max; col++)
                        {
                            ...
                            _mm_prefetch((const char *)(bot_px + offset + stride), _MM_HINT_T0);
                             ...
                             row + 1 < row_max) // don't bother updating if last iteration
                    {
            ...
            else
            {
                ...
                for (int row = chunk_top; row < chunk_bot; row++)
                {
                    ...
                    for (int col = chunk_left; col < chunk_right; col++)
                    {
                        ...
                        {
                            out[4 * col + c] = (in[4 * col + c] * invert[c]) +
                                               (out[4 * col + c] / out[4 * col + 3] * weight[c]);
                        }
                    }
                }
            }
                           ...
                           const int horiz_kernel, const int vert_kernel)
{
    ...
    *h =
        dt_opencl_local_buffer_opt(devid, horiz_kernel, &hlocopt) == CL_SUCCESS ? hlocopt.sizex : 1;
...
static inline cl_int nlmeans_cl_init(const int devid, const int kernel, cl_mem dev_out,
                                     const int height, const int width)
{
    return dt_opencl_enqueue_kernel_2d_args(devid, kernel, width, height, CLARG(dev_out),
                                            CLARG(width), CLARG(height));
}

// horizontal pass, add together columns of each patch
static inline cl_int nlmeans_cl_horiz(const int devid, const int kernel, cl_mem dev_U4,
                                      cl_mem dev_U4_t, const int P, const int q[2],
                                      const int height, const int width, const int bwidth,
                                      const int hblocksize)
{
    const size_t sizesl[2] = {bwidth, ROUNDUPDHT(height, devid)};
    const size_t local[2] = {hblocksize, 1};
    return dt_opencl_enqueue_kernel_2d_local_args(
        devid, kernel, sizesl, local, CLARG(dev_U4), CLARG(dev_U4_t), CLARG(width), CLARG(height),
        CLARRAY(2, q), CLARG(P), CLLOCAL((hblocksize + 2 * P) * sizeof(float)));
}

// add difference-weighted proportion of patch-center pixel to output pixel
static inline cl_int nlmeans_cl_accu(const int devid, const int kernel, cl_mem dev_in,
                                     cl_mem dev_U4_tt, cl_mem dev_out, const int q[2],
                                     const int height, const int width)
{
    return dt_opencl_enqueue_kernel_2d_args(devid, kernel, width, height, CLARG(dev_in),
                                            CLARG(dev_out), CLARG(dev_U4_tt), CLARG(width),
                                            CLARG(height), CLARRAY(2, q));
}

int nlmeans_denoise_cl(const dt_nlmeans_param_t *const params, const int devid, cl_mem dev_in,
                       cl_mem dev_out, const dt_iop_roi_t *const roi_in)
{
    ...
    for (int p = 0; p < num_patches; p++)
    {
        ...
        err = nlmeans_cl_horiz(devid, params->kernel_horiz, dev_U4, dev_U4_t, P, q, height, width,
                               bwidth, hblocksize);
        if (err != CL_SUCCESS)
            break;
        ...
        err = dt_opencl_enqueue_kernel_2d_local_args(
            devid, params->kernel_vert, sizesl, local, CLARG(dev_U4_t), CLARG(dev_U4_tt),
            CLARG(width), CLARG(height), CLARG(q), CLARG(P), CLARG(sharpness),
            CLLOCAL((vblocksize + 2 * P) * sizeof(float)));
        if (err != CL_SUCCESS)
            break;

        // add weighted proportion of patch's center pixel to output pixel
        err = nlmeans_cl_accu(devid, params->kernel_accu, dev_in, dev_U4_tt, dev_out, q, height,
                              width);
        if (err != CL_SUCCESS)
            break;
                              ...
                              cl_mem dev_in, cl_mem dev_out, const dt_iop_roi_t *const roi_in)
{
    ...
    for (int p = 0; p < num_patches; p++)
    {
        ...
        err = dt_opencl_enqueue_kernel_2d_local_args(
            devid, params->kernel_vert, sizesl, local, CLARG(dev_U4_t), CLARG(dev_U4_tt),
            CLARG(width), CLARG(height), CLARG(q), CLARG(P), CLARG(norm),
            CLLOCAL((vblocksize + 2 * P) * sizeof(float)), CLARG(central_pixel_weight),
            CLARG(dev_U4));
        if (err != CL_SUCCESS)
            break;
