...
static void dt_set_rlimits_stack()
{
    ...
    if (ret != 0)
    {
        ...
        fprintf(stderr,
                "[dt_set_rlimits_stack] error: getrlimit(RLIMIT_STACK) returned %i: %i (%s)\n", ret,
                errsv, strerror(errsv));
    }

    assert((ret == 0 && (WANTED_STACK_SIZE <= rlim.rlim_max || RLIM_INFINITY == rlim.rlim_max)) ||
           (ret != 0));
    ...
    if (ret != 0 || (rlim.rlim_cur < WANTED_STACK_SIZE && rlim.rlim_cur != RLIM_INFINITY) /*|| 1*/)
    {
        ...
        fprintf(stderr,
                "[dt_set_rlimits_stack] info: bumping RLIMIT_STACK rlim_cur from %" PRIuMAX
                " to %" PRIuMAX "\n",
        ...
        if (ret != 0)
        {
            int errsv = errno;
            fprintf(stderr,
                    "[dt_set_rlimits_stack] error: setrlimit(RLIMIT_STACK) returned %i: %i (%s)\n",
