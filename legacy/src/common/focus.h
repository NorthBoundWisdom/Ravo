...
static inline void _dt_focus_cdf22_wtf(uint8_t *buf, const int l, const int width, const int height)
{
    ...
    for (int j = 0; j < height; j++)
    {
        ...
        for (; i < width - st; i += step) /*for(ch=0; ch<3; ch++)*/
            gbuf(buf, i, j) = _to_uint8(
                (int)gbuf(buf, i, j) - ((int)gbuf(buf, i - st, j) + (int)gbuf(buf, i + st, j)) / 2);
        ...
        for (i = step; i < width - st; i += step) /*for(ch=0; ch<3; ch++)*/
            gbuf(buf, i, j) +=
                (_from_uint8(gbuf(buf, i - st, j)) + _from_uint8(gbuf(buf, i + st, j))) / 4;
        if (i < width) /*for(ch=0; ch<3; ch++)*/
    ...
    for (int i = 0; i < width; i++)
    {
        ...
        for (; j < height - st; j += step) /*for(ch=0; ch<3; ch++)*/
            gbuf(buf, i, j) = _to_uint8(
                (int)gbuf(buf, i, j) - ((int)gbuf(buf, i, j - st) + (int)gbuf(buf, i, j + st)) / 2);
        ...
        for (j = step; j < height - st; j += step) /*for(ch=0; ch<3; ch++)*/
            gbuf(buf, i, j) +=
                (_from_uint8(gbuf(buf, i, j - st)) + _from_uint8(gbuf(buf, i, j + st))) / 4;
        if (j < height) /*for(int ch=0; ch<3; ch++)*/
            gbuf(buf, i, j) += _from_uint8(gbuf(buf, i, j - st)) / 2;
    }
}

static void _dt_focus_update(dt_focus_cluster_t *f, int frows, int fcols, int i, int j, int wd,
                             int ht, int diff)
{
...
static void dt_focus_create_clusters(dt_focus_cluster_t *focus, int frows, int fcols,
                                     uint8_t *buffer, int buffer_width, int buffer_height)
{
    ...
    int num_clusters = 0;
    for (int k = 0; k < fs; k++)
        if (focus[k].n * 4 > wd * ht / (float)fs * 0.01f)
            num_clusters++;
    if (num_clusters < 1)
    {
...
static void dt_focus_draw_clusters(cairo_t *cr, int width, int height, dt_imgid_t imgid,
                                   int buffer_width, int buffer_height, dt_focus_cluster_t *focus,
                                   int frows, int fcols, float full_zoom, float full_x,
                                   float full_y)
{
    const int fs = frows * fcols;
    ...
    const float scale =
        fminf((width - 2 * tb) / (float)wd, (height - 2 * tb) / (float)ht) * full_zoom;
    ...
    cairo_translate(cr, -wd / 2.0f + fx / scale * darktable.gui->ppd_thb,
                    -ht / 2.0f + fy / scale * darktable.gui->ppd_thb);
        ...
        if (draw)
        {
            for (int i = 0; i < 2; i++)
            {
                ...
                cairo_curve_to(cr, -pos[2 * k + 0] + offx[2 * k + 0] + offy[2 * k + 0],
                               ...
                               -pos[2 * k + 1] + offx[2 * k + 1] + offy[2 * k + 1], offy[2 * k + 0],
                               offy[2 * k + 1]);
                cairo_curve_to(cr, pos[2 * k + 0] - offx[2 * k + 0] + offy[2 * k + 0],
                               pos[2 * k + 1] - offx[2 * k + 1] + offy[2 * k + 1],
                               pos[2 * k + 0] - offx[2 * k + 0] + offy[2 * k + 0],
                               pos[2 * k + 1] - offx[2 * k + 1] + offy[2 * k + 1],
                               2 * pos[2 * k + 0] - offx[2 * k + 0],
                ...
                cairo_curve_to(cr, 3 * pos[2 * k + 0] - offx[2 * k + 0] - offy[2 * k + 0],
                               ...
                               3 * pos[2 * k + 1] - offx[2 * k + 1] - offy[2 * k + 1],
                               2 * pos[2 * k + 0] - offy[2 * k + 0],
                               2 * pos[2 * k + 1] - offy[2 * k + 1]);
                cairo_curve_to(cr, pos[2 * k + 0] + offx[2 * k + 0] - offy[2 * k + 0],
                               pos[2 * k + 1] + offx[2 * k + 1] - offy[2 * k + 1],
                               pos[2 * k + 0] + offx[2 * k + 0] - offy[2 * k + 0],
                               pos[2 * k + 1] + offx[2 * k + 1] - offy[2 * k + 1], offx[2 * k + 0],
                               offx[2 * k + 1]);
