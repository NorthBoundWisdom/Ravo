...
static inline __m128 lab_f_m_sse2(const __m128 x)
{
    ...
    // approximate cbrtf(x):
    const __m128 a = _mm_castsi128_ps(_mm_add_epi32(
        _mm_cvtps_epi32(_mm_div_ps(_mm_cvtepi32_ps(_mm_castps_si128(x)), _mm_set1_ps(3.0f))),
...
static inline __m128 dt_XYZ_to_Lab_sse2(const __m128 XYZ)
{
    ...
    return coef * (_mm_shuffle_ps(f, f, _MM_SHUFFLE(3, 1, 0, 1)) -
...
static inline __m128 dt_XYZ_to_prophotoRGB_sse2(__m128 XYZ)
{
    ...
    __m128 rgb = xyz_to_rgb_0 * _mm_shuffle_ps(XYZ, XYZ, _MM_SHUFFLE(0, 0, 0, 0)) +
...
static inline __m128 dt_prophotoRGB_to_XYZ_sse2(__m128 rgb)
{
    ...
    __m128 XYZ = rgb_to_xyz_0 * _mm_shuffle_ps(rgb, rgb, _MM_SHUFFLE(0, 0, 0, 0)) +
                                                    ...
                                                    dt_aligned_pixel_t out)
{
    ...
    for_each_channel(r) out[r] = matrix[0][r] * in[0] + matrix[1][r] * in[1] + matrix[2][r] * in[2];
}
                                                ...
                                                dt_aligned_pixel_t out)
{
    // using dt_aligned_pixel_t instead of float* for the function parameters gives GCC enough info to vectorize
    // and eliminate intermediate memory writes without going through major contortions
    for_each_channel(r) out[r] =
        matrix_row0[r] * in[0] + matrix_row1[r] * in[1] + matrix_row2[r] * in[2];
}
...
#endif
    for_each_channel(c) Lab[c] = (coeff[c] * (tmp1[c] - tmp2[c])) - offset[c];
}
...
static inline void dt_Lab_to_XYZ(const dt_aligned_pixel_t Lab, dt_aligned_pixel_t XYZ)
{
    ...
    for_each_channel(c, aligned(Lab, coeff, f)) scaled[c] = (f[c] + offset[c]) * coeff[c];
    dt_aligned_pixel_t inv;
    for_each_channel(c) inv[c] = lab_f_inv(scaled[c] + scaled[1] * add_coeff[c]);
    for_each_channel(c, aligned(d50, f, add_coeff, XYZ)) XYZ[c] = d50[c] * inv[c];
}

/** uses D50 white point. */
DT_OMP_DECLARE_SIMD(aligned(Lab, RGB : 16) uniform(Lab, RGB))
static inline void dt_Lab_to_linearRGB(const dt_aligned_pixel_t Lab,
                                       ...
                                       dt_aligned_pixel_t RGB)
{
    ...
    dt_aligned_pixel_t scaled;
    for_each_channel(c, aligned(Lab, coeff, f)) scaled[c] = (f[c] + add[c]) * coeff[c];
    dt_aligned_pixel_t inv;
    for_each_channel(c) inv[c] = lab_f_inv(scaled[c] + scaled[1] * add_coeff[c]);
    dt_aligned_pixel_t XYZ;
    for_each_channel(c, aligned(d50, f, add_coeff, XYZ)) XYZ[c] = d50[c] * inv[c];
    // the following loop is dt_apply_transposed_color_matrix(XYZ, cmatrix, RGB);
    for_each_channel(r) RGB[r] =
        cmatrix_row0[r] * XYZ[0] + cmatrix_row1[r] * XYZ[1] + cmatrix_row2[r] * XYZ[2];
}
...
static const float xyz_to_srgb_d65[9] = {
    3.2404542f, -1.5371385f, -0.4985314f, -0.9692660f, 1.8760108f,
...
static const float rec2020_to_xyz_d65[9] = {
    0.6369580483f, 0.1446169036f, 0.1688809752f, 0.2627002120f, 0.6779980715f,
    0.0593017165f, 0.0000000000f, 0.0280726930f, 1.0609850577f,
};

// Conversion matrix from http://www.brucelindbloom.com/Eqn_RGB_XYZ_Matrix.html
// (transpose and pad the conversion matrix to enable vectorization)
static const dt_colormatrix_t sRGB_to_xyz_transposed = {{0.4360747f, 0.2225045f, 0.0139322f},
                                                        {0.3850649f, 0.7168786f, 0.0971045f},
                                                        {0.1430804f, 0.0606169f, 0.7141733f}};

static const dt_colormatrix_t xyz_to_srgb_transposed = {{3.1338561f, -0.9787684f, 0.0719453f},
...
static inline void dt_XYZ_to_sRGB(const dt_aligned_pixel_t XYZ, dt_aligned_pixel_t sRGB)
{
    ...
    static const dt_aligned_pixel_t srgb_power = {1.0f / 2.4f, 1.0f / 2.4f, 1.0f / 2.4f,
                                                  1.0f / 2.4f};
...
static inline void dt_XYZ_to_sRGB_clipped(const dt_aligned_pixel_t XYZ, dt_aligned_pixel_t sRGB)
{
    ...
    for_each_channel(c) sRGB[c] = CLIP(result[c]);
}
...
static inline void dt_RGB_to_Lab(const dt_aligned_pixel_t rgb,
                                 const dt_aligned_pixel_t cmatrix_row0,
                                 const dt_aligned_pixel_t cmatrix_row1,
                                 const dt_aligned_pixel_t cmatrix_row2, dt_aligned_pixel_t Lab)
{
    dt_aligned_pixel_t XYZ;
...
static inline void _dt_Hue_2_RGB(dt_aligned_pixel_t RGB, const float H, const float C,
                                 const float min)
{
...
static inline void dt_XYZ_D50_2_XYZ_D65(const dt_aligned_pixel_t XYZ_D50,
                                        dt_aligned_pixel_t XYZ_D65)
{
    ...
    for_each_channel(x) XYZ_D65[x] = M_transposed[0][x] * XYZ_D50[0] +
                                     M_transposed[1][x] * XYZ_D50[1] +
                                     M_transposed[2][x] * XYZ_D50[2];
}

DT_OMP_DECLARE_SIMD(aligned(XYZ_D50, XYZ_D65 : 16))
static inline void dt_XYZ_D65_2_XYZ_D50(const dt_aligned_pixel_t XYZ_D65,
                                        dt_aligned_pixel_t XYZ_D50)
{
    ...
    for_each_channel(x) XYZ_D50[x] = M_transposed[0][x] * XYZ_D65[0] +
                                     M_transposed[1][x] * XYZ_D65[1] +
                                     M_transposed[2][x] * XYZ_D65[2];
}
...
static const dt_colormatrix_t XYZ_D65_to_LMS_2006_D65 = {{0.257085f, 0.859943f, -0.031061f, 0.f},
...
static const dt_colormatrix_t filmlightRGB_D65_to_LMS_D65_trans = {{0.95f, 0.05f, 0.00f, 0.f},
...
{
    static const dt_colormatrix_t rgb_to_YCbCr_transposed = {{0.299f, -0.147f, 0.615f, 0.0f},
...
{
    const dt_aligned_pixel_t x_factors = {-0.783941002840055f, 0.745273540913283f,
                                          0.318707282433486f, 0.f};
    const dt_aligned_pixel_t y_factors = {0.277512987809202f, -0.205375866083878f,
                                          2.16743692732158f, 0.f};
    const dt_aligned_pixel_t offsets = {0.153836578598858f, -0.165478376301988f, 0.291320554395942f,
                                        0.f};

    dt_aligned_pixel_t UVD = {0.f};
    for_each_channel(c, aligned(xyY, UVD, x_factors, y_factors, offsets)) UVD[c] =
        x_factors[c] * xyY[0] + y_factors[c] * xyY[1] + offsets[c];
...
static inline void dt_UCS_LUV_to_JCH(const float L_star, const float L_white,
                                     const float UV_star_prime[2], dt_aligned_pixel_t JCH)
{
    const float M2 = UV_star_prime[0] * UV_star_prime[0] +
                     UV_star_prime[1] * UV_star_prime[1]; // square of colorfulness M

    // should be JCH[0] = powf(L_star / L_white), cz) but we treat only the case where cz = 1
    JCH[0] = L_star / L_white;
    JCH[1] = 15.932993652962535f * powf(L_star, 0.6523997524738018f) *
             powf(M2, 0.6007557017508491f) / L_white;
    JCH[2] = atan2f(UV_star_prime[1], UV_star_prime[0]);
}

DT_OMP_DECLARE_SIMD(aligned(xyY, JCH : 16))
static inline void xyY_to_dt_UCS_JCH(const dt_aligned_pixel_t xyY, const float L_white,
...
DT_OMP_DECLARE_SIMD(aligned(xyY, JCH : 16))
static inline void dt_UCS_JCH_to_xyY(const dt_aligned_pixel_t JCH, const float L_white,
                                     dt_aligned_pixel_t xyY)
{
  ...
  */
    ...
    const float M =
        L_star != 0.f ?
            powf(JCH[1] * L_white / (15.932993652962535f * powf(L_star, 0.6523997524738018f)),
                 0.8322850678616855f) :
            0.f;
    ...
    const dt_aligned_pixel_t U_factors = {0.167171472114775f, -0.150959086409163f,
                                          0.940254742367256f, 0.f};
    const dt_aligned_pixel_t V_factors = {0.141299802443708f, -0.155185060382272f,
                                          1.000000000000000f, 0.f};
    const dt_aligned_pixel_t offsets = {-0.00801531300850582f, -0.00843312433578007f,
                                        -0.0256325967652889f, 0.f};

    dt_aligned_pixel_t xyD = {0.f};
    for_each_channel(c, aligned(xyD, UV, U_factors, V_factors, offsets)) xyD[c] =
        U_factors[c] * UV[0] + V_factors[c] * UV[1] + offsets[c];
...
static inline void dt_UCS_HSB_to_XYZ(const dt_aligned_pixel_t HSB, const float L_w,
                                     dt_aligned_pixel_t XYZ)
{
    // Quick path
...
static inline void dt_UCS_JCH_to_XYZ(const dt_aligned_pixel_t JCH, const float L_w,
                                     dt_aligned_pixel_t XYZ)
{
    dt_aligned_pixel_t xyY = {0.f};
    dt_UCS_JCH_to_xyY(JCH, L_w, xyY);
...
static inline void dt_UCS_JCH_to_sRGB(const dt_aligned_pixel_t JCH, const float L_w,
                                      dt_aligned_pixel_t sRGB)
{
    ...
    for_each_channel(c) sRGB[c] = linear[c] <= 0.0031308f ?
                                      12.92f * linear[c] :
