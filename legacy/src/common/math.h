...
DT_OMP_DECLARE_SIMD()
static inline void mat3mulv(float *const __restrict__ dest, const float *const mat,
...
DT_OMP_DECLARE_SIMD()
static inline void mat3mul(float *const __restrict__ dest, const float *const __restrict__ m1,
...
// dest = m1 * m2 in this order
DT_OMP_DECLARE_SIMD()
static inline void mat3SSEmul(dt_colormatrix_t dest, const dt_colormatrix_t m1,
...
static inline void dot_product(const dt_aligned_pixel_t v_in, const dt_colormatrix_t M,
...
static inline void dt_vector_min(dt_aligned_pixel_t min, const dt_aligned_pixel_t v1,
                                 const dt_aligned_pixel_t v2)
{
#ifdef __SSE__
    *((__m128 *)min) = _mm_min_ps(*((__m128 *)v1), *((__m128 *)v2));
#else
    for_four_channels(c) min[c] = MIN(v1[c], v2[c]);
#endif
}

static inline void dt_vector_max(dt_aligned_pixel_t max, const dt_aligned_pixel_t v1,
                                 const dt_aligned_pixel_t v2)
{
#ifdef __SSE__
    *((__m128 *)max) = _mm_max_ps(*((__m128 *)v1), *((__m128 *)v2));
#else
    for_four_channels(c) max[c] = MAX(v1[c], v2[c]);
#endif
}

static inline void dt_vector_max_nan(dt_aligned_pixel_t max, const dt_aligned_pixel_t v1,
                                     const dt_aligned_pixel_t v2)
{
    for_four_channels(c) max[c] = fmaxf(v1[c], v2[c]);
}
...
#else
    for_four_channels(c) rounded[c] = roundf(input[c]);
#endif
}
...
static inline void dt_vector_log2(const dt_aligned_pixel_t x, dt_aligned_pixel_t res)
{
    ...
    for_four_channels(c)
    {
        ...
        logmant[c] =
            ((((((-0.0344359067839062357313f) * mant.f[c] + 0.318212422185251071475f) * mant.f[c] -
        ...
        logmant[c] =
            (((((0.0596515482674574969533f) * mant.f[c] - 0.465725644288844778798f) * mant.f[c] +
...
#error Unsupported value for LOG_POLY_DEGREE
    ...
    for_four_channels(c) res[c] = (logmant[c] * (mant.f[c] - 1.0f)) + exp[c];
}
...
static inline void dt_vector_exp2(const dt_aligned_pixel_t input, dt_aligned_pixel_t res)
{
    ...
    static const dt_aligned_pixel_t lower_bound = {-126.99999f, -126.99999f, -126.99999f,
                                                   -126.99999f};
    ...
    for_four_channels(c) x_adj[c] = x[c] - v_half[c];
    dt_aligned_pixel_t ipart;
    dt_vector_round(x_adj, ipart);
    dt_aligned_pixel_t fpart;
    for_four_channels(c) fpart[c] = x[c] - ipart[c];
    ...
    for_four_channels(c) expipart.i[c] = (127 + (int)ipart[c]) << 23;
    ...
    for_four_channels(c)
    {
        ...
        expfpart[c] =
            (((((1.8775767e-3f * fpart[c] + 8.9893397e-3f) * fpart[c] + 5.5826318e-2f) * fpart[c] +
               2.4015361e-1f) *
                  fpart[c] +
              6.9315308e-1f) *
                 fpart[c] +
        ...
        expfpart[c] =
            ((((1.3534167e-2f * fpart[c] + 5.2011464e-2f) * fpart[c] + 2.4144275e-1f) * fpart[c] +
              6.9300383e-1f) *
                 fpart[c] +
             1.0000026f);
#else
#error Unsupported value for EXP_POLY_DEGREE
    ...
    for_four_channels(c, aligned(res)) res[c] = expipart.f[c] * expfpart[c];
}
...
static inline void dt_vector_exp10(const dt_aligned_pixel_t x, dt_aligned_pixel_t res)
{
    ...
    for_four_channels(c, aligned(x, scaled)) scaled[c] = 3.3219280948873626f * x[c];
    dt_vector_exp2(scaled, res);
}

static inline void dt_vector_powf(const dt_aligned_pixel_t input, const dt_aligned_pixel_t power,
                                  dt_aligned_pixel_t output)
{
    // x**p == 2^( log2(x) * p)
    dt_aligned_pixel_t log;
    dt_vector_log2(input, log);
    for_four_channels(c) log[c] *= power[c];
    dt_vector_exp2(log, output);
}

static inline void dt_vector_pow1(const dt_aligned_pixel_t input, const float power,
                                  dt_aligned_pixel_t output)
{
...
static inline void dt_vector_add(dt_aligned_pixel_t sum, const dt_aligned_pixel_t v1,
                                 const dt_aligned_pixel_t v2)
{
    for_four_channels(c, aligned(sum, v1, v2)) sum[c] = v1[c] + v2[c];
}

static inline void dt_vector_sub(dt_aligned_pixel_t diff, const dt_aligned_pixel_t v1,
                                 const dt_aligned_pixel_t v2)
{
    for_four_channels(c, aligned(diff, v1, v2)) diff[c] = v1[c] - v2[c];
}

static inline void dt_vector_mul(dt_aligned_pixel_t result, const dt_aligned_pixel_t v1,
                                 const dt_aligned_pixel_t v2)
{
    for_four_channels(c, aligned(result, v1, v2)) result[c] = v1[c] * v2[c];
}

static inline void dt_vector_mul1(dt_aligned_pixel_t result, const dt_aligned_pixel_t in,
                                  const float scale)
{
    for_four_channels(c, aligned(result, in)) result[c] = in[c] * scale;
}

static inline void dt_vector_div(dt_aligned_pixel_t result, const dt_aligned_pixel_t v1,
                                 const dt_aligned_pixel_t v2)
{
    for_four_channels(c, aligned(result, v1, v2)) result[c] = v1[c] / v2[c];
}

static inline void dt_vector_div1(dt_aligned_pixel_t result, const dt_aligned_pixel_t in,
                                  const float divisor)
{
...
static inline float dt_vector_channel_max(const dt_aligned_pixel_t pixel)
{
    ...
    for_each_channel(c) maximum[c] = MAX(MAX(pixel[c], swapRG[c]), swapRB[c]);
    return maximum[0];
}
...
static inline void dt_vector_sin(const dt_aligned_pixel_t arg, dt_aligned_pixel_t sine)
{
    ...
    static const dt_aligned_pixel_t a = {4 / (M_PI_F * M_PI_F), 4 / (M_PI_F * M_PI_F),
                                         4 / (M_PI_F * M_PI_F), 4 / (M_PI_F * M_PI_F)};
    static const dt_aligned_pixel_t p = {0.225f, 0.225f, 0.225f, 0.225f};
    static const dt_aligned_pixel_t one = {1.0f, 1.0f, 1.0f, 1.0f};

    dt_aligned_pixel_t abs_arg;
    for_four_channels(c) abs_arg[c] = (arg[c] < 0.0f) ? -arg[c] : arg[c];
    dt_aligned_pixel_t scaled;
    for_four_channels(c) scaled[c] = a[c] * arg[c] * (pi[c] - abs_arg[c]);
    dt_aligned_pixel_t abs_scaled;
    for_four_channels(c) abs_scaled[c] = (scaled[c] < 0.0f) ? -scaled[c] : scaled[c];
    for_four_channels(c) sine[c] = scaled[c] * (p[c] * (abs_scaled[c] - one[c]) + one[c]);
}
