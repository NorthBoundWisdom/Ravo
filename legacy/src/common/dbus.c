...
static void _on_bus_acquired(GDBusConnection *connection, const gchar *name, gpointer user_data)
{
    ...
    dbus->registration_id = g_dbus_connection_register_object(
        connection, "/darktable", dbus->introspection_data->interfaces[0], &interface_vtable,
        dbus,  /* user_data */
    ...
    if (dbus->registration_id == 0)
        dbus->connected =
            FALSE; // technically we are connected, but we are not exporting anything. or something like that
}
