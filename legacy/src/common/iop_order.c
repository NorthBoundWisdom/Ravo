...
const char *iop_order_string[] = {N_("custom"), N_("0.9 RAW"), N_("0.9 JPEG")};
...
// Built-in 0.9 RAW pipeline.
const dt_iop_order_entry_t raw_order[] = {
    {{1.0}, "rawprepare", 0},
    {{3.0f}, "temperature", 0},
    {{3.1f}, "rasterfile", 0},
    {{4.0f}, "highlights", 0},
    ...
    {{42.0f},
     "rgbcurve",
     0}, // really versatile way to edit colour in scene-referred and display-referred workflow
    ...
    {{55.0f},
     "colorcorrection",
     0}, // now that the colours have been damaged by contrast manipulations,
...
// Built-in 0.9 JPEG pipeline.
const dt_iop_order_entry_t jpg_order[] = {
    ...
    {{28.0f},
     "cacorrectrgb",
     0}, // correct chromatic aberrations after lens correction so that lensfun
    ...
    {{42.0f},
     "rgbcurve",
     0}, // really versatile way to edit colour in scene-referred and display-referred
         // workflow
    {{43.0f}, "rgblevels", 0}, // same
    {{44.0f},
     "basecurve",
    ...
    {{69.0f},
     "colorreconstruct",
     0}, // try to salvage blown areas before ICC intents in LittleCMS2 do things
         // with them.
    {{69.5f}, "finalscale", 0},
    {{70.0f}, "colorout", 0},
    {{73.0f}, "overexposed", 0},
...
static dt_iop_order_t _ioppr_get_default_iop_order_version(const dt_imgid_t imgid)
{
    ...
    return is_ldr ? DT_DEFAULT_IOP_ORDER_JPG : DT_DEFAULT_IOP_ORDER_RAW;
}
...
GList *dt_ioppr_get_iop_order_link(GList *iop_order_list, const char *op_name,
...
static dt_iop_order_entry_t *_ioppr_get_iop_order_entry(GList *iop_order_list, const char *op_name,
                                                        const int multi_priority)
{
...
int dt_ioppr_get_iop_order(GList *iop_order_list, const char *op_name, const int multi_priority)
{
    ...
    if (order_entry)
    {
        ...
        dt_print(DT_DEBUG_ALWAYS, "cannot get iop-order for %s instance %d", op_name,
                 multi_priority);

    return iop_order;
}

int dt_ioppr_get_iop_order_last(GList *iop_order_list, const char *op_name)
{
...
gint dt_sort_iop_list_by_order(gconstpointer a, gconstpointer b)
{
    ...
    if (am->o.iop_order > bm->o.iop_order)
        return 1;
    if (am->o.iop_order < bm->o.iop_order)
        return -1;
    return 0;
...
gint dt_sort_iop_list_by_order_f(gconstpointer a, gconstpointer b)
{
    const dt_iop_order_entry_t *const restrict am = (const dt_iop_order_entry_t *)a;
    const dt_iop_order_entry_t *const restrict bm = (const dt_iop_order_entry_t *)b;
    if (am->o.iop_order_f > bm->o.iop_order_f)
        return 1;
    if (am->o.iop_order_f < bm->o.iop_order_f)
        return -1;
    return 0;
...
gboolean _check_iop_list_equal(GList *iop_order_list, const dt_iop_order_entry_t order_table[])
{
    int k = 0;
...
{
    for (dt_iop_order_t O = DT_IOP_ORDER_RAW; O < DT_IOP_ORDER_LAST; O++)
    {
...
GList *dt_ioppr_get_multiple_instances_iop_order_list(const dt_imgid_t imgid, const gboolean memory)
{
    GList *res = NULL;
    ...
    if (memory)
        DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                    ...
                                    " FROM memory.history"
                                    " WHERE imgid=?1 GROUP BY operation HAVING c > 1",
                                    -1, &stmt, NULL);
    else
        DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                    "SELECT COUNT(operation) c, operation"
...
gboolean dt_ioppr_write_iop_order(const dt_iop_order_t kind, GList *iop_order_list,
...
{
    if (version == DT_IOP_ORDER_RAW || version == DT_IOP_ORDER_JPG)
    {
...
GList *dt_ioppr_get_iop_order_list(const dt_imgid_t imgid, const gboolean sorted)
{
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
        ...
        if (sqlite3_step(stmt) == SQLITE_ROW)
        {
            ...
            else if (version == DT_IOP_ORDER_RAW || version == DT_IOP_ORDER_JPG)
            {
...
void dt_ioppr_resync_modules_order(dt_develop_t *dev)
{
    ...
    while (modules)
    {
        ...
        if (mod->iop_order != INT_MAX)
            mod->iop_order =
                dt_ioppr_get_iop_order(dev->iop_order_list, mod->op, mod->multi_priority);
...
void dt_ioppr_change_iop_order(struct dt_develop_t *dev, const dt_imgid_t imgid,
                               GList *new_iop_list)
{
                                                    ...
                                                    GList *multi_instance_list)
{
    ...
    while (l)
    {
        ...
        iop_order_list =
            dt_ioppr_merge_module_multi_instance_iop_order_list(iop_order_list, operation, op);
...
static void _count_iop_module(GList *iop, const char *operation, int *max_multi_priority,
                              int *count, int *max_multi_priority_enabled, int *count_enabled)
{
...
static int _count_entries_operation(GList *e_list, const char *operation)
{
    ...
    for (const GList *l = e_list; l; l = g_list_next(l))
    {
        ...
        if (!strcmp(ep->operation, operation))
            count++;
    }
...
static int _get_multi_priority(dt_develop_t *dev, const char *operation, const int n,
...
static void _ioppr_update_for_entries(dt_develop_t *dev, GList *entry_list, const gboolean append)
{
    ...
    for (GList *e_list = entry_list; e_list; e_list = g_list_next(e_list))
    {
        ...
        // instances we have
        _count_iop_module(dev->iop, ep->operation, &max_multi_priority, &count,
...
void dt_ioppr_update_for_style_items(dt_develop_t *dev, GList *st_items, const gboolean append)
{
    ...
    for (const GList *si_list = st_items; si_list; si_list = g_list_next(si_list))
    {
        ...
        if (si->params_size > 0)
        {
            ...
            si->iop_order =
                dt_ioppr_get_iop_order(dev->iop_order_list, si->operation, si->multi_priority);
            el = g_list_next(el);
        }
    }
...
gboolean dt_ioppr_check_can_move_before_iop(GList *iop_list, dt_iop_module_t *module,
    ...
    if (module->iop_order < module_next->iop_order)
    {
        ...
        if (modules)
        {
            ...
            for (modules = g_list_next(modules); modules; modules = g_list_next(modules))
            {
                ...
                for (const GList *rules = darktable.iop_order_rules; rules;
            ...
            dt_print(DT_DEBUG_ALWAYS, "[dt_ioppr_get_iop_order_before_iop] can't find module %s %s",
...
// if it cannot be placed it returns -1.0
// this assumes that the order is always positive
gboolean dt_ioppr_check_can_move_after_iop(GList *iop_list, dt_iop_module_t *module,
                                           dt_iop_module_t *module_prev)
{
    ...
    if (module_next == NULL)
    {
        dt_print(DT_DEBUG_ALWAYS,
                 ...
                 module_prev->op, module_prev->multi_name, module_prev->iop_order, module->op,
                 module->multi_name, module->iop_order);
    }
    else
...
gboolean dt_ioppr_move_iop_before(struct dt_develop_t *dev, dt_iop_module_t *module,
                                  dt_iop_module_t *module_next)
{
    GList *next = dt_ioppr_get_iop_order_link(dev->iop_order_list, module_next->op,
                                              module_next->multi_priority);
    GList *current =
        dt_ioppr_get_iop_order_link(dev->iop_order_list, module->op, module->multi_priority);
...
// sort dev->iop to reflect the changes
// return 1 if iop_order is changed, 0 otherwise
gboolean dt_ioppr_move_iop_after(struct dt_develop_t *dev, dt_iop_module_t *module,
                                 dt_iop_module_t *module_prev)
{
    GList *prev = dt_ioppr_get_iop_order_link(dev->iop_order_list, module_prev->op,
                                              module_prev->multi_priority);
    GList *current =
        dt_ioppr_get_iop_order_link(dev->iop_order_list, module->op, module->multi_priority);
...
void dt_ioppr_print_module_iop_order(GList *iop_list, const char *msg)
{
    for (const GList *modules = iop_list; modules; modules = g_list_next(modules))
    {
        ...
        dt_print(DT_DEBUG_ALWAYS, "[%s] module %s %s multi_priority=%i, iop_order=%d", msg, mod->op,
                 mod->multi_name, mod->multi_priority, mod->iop_order);
    }
}
...
void dt_ioppr_print_history_iop_order(GList *history_list, const char *msg)
{
    for (const GList *history = history_list; history; history = g_list_next(history))
    {
        ...
        dt_print(DT_DEBUG_ALWAYS, "[%s] module %s %s multi_priority=%i, iop_order=%d", msg,
                 hist->op_name, hist->multi_name, hist->multi_priority, hist->iop_order);
    }
}
...
void dt_ioppr_print_iop_order(GList *iop_order_list, const char *msg)
{
    for (const GList *iops_order = iop_order_list; iops_order; iops_order = g_list_next(iops_order))
    {
        ...
        dt_print(DT_DEBUG_ALWAYS, "[%s] op %20s (inst %d) iop_order=%d", msg,
                 order_entry->operation, order_entry->instance, order_entry->o.iop_order);
    }
}
    ...
    for (const GList *modules = iop_list; modules; modules = g_list_next(modules))
    {
        ...
        if (fence_next && mod->iop_order > fence_next->iop_order)
        {
            dt_print(DT_DEBUG_ALWAYS,
                     ...
                     mod->iop_order, fence_next->op, fence_next->multi_name, fence_next->iop_order,
                     imgid, msg);
        }
        if (fence_prev && mod->iop_order < fence_prev->iop_order)
        {
            dt_print(DT_DEBUG_ALWAYS,
                     ...
                     fence_prev->op, fence_prev->multi_name, mod->op, mod->multi_name,
                     mod->iop_order, fence_prev->op, fence_prev->multi_name, fence_prev->iop_order,
                     imgid, msg);
        }
    }
    ...
    for (const GList *modules = iop_list; modules; modules = g_list_next(modules))
    {
        ...
        for (const GList *rules = darktable.iop_order_rules; rules; rules = g_list_next(rules))
        {
                     ...
                     modules_prev = g_list_previous(modules_prev))
                {
                    ...
                    if (strcmp(mod_prev->op, rule->op_next) == 0)
                    {
                        dt_print(DT_DEBUG_ALWAYS,
                                 ...
                                 " is after %s %s(%d) image %i (%s)",
                                 rule->op_prev, rule->op_next, mod->op, mod->multi_name,
                                 mod->iop_order, mod_prev->op, mod_prev->multi_name,
            ...
            else if (dt_iop_module_is(mod, rule->op_next))
            {
                ...
                for (const GList *modules_next = g_list_next(modules); modules_next;
                     modules_next = g_list_next(modules_next))
                {
                    ...
                    if (strcmp(mod_next->op, rule->op_prev) == 0)
                    {
                        dt_print(DT_DEBUG_ALWAYS,
                                 ...
                                 rule->op_prev, rule->op_next, mod->op, mod->multi_name,
                                 mod->iop_order, mod_next->op, mod_next->multi_name,
                                 mod_next->iop_order, imgid, msg);
                    }
                }
            }
        }
...
gboolean dt_ioppr_check_iop_order_ext(dt_develop_t *dev, const dt_imgid_t imgid, const char *msg)
{
                ...
                if (mod_prev)
                {
                    ...
                    else if (mod->iop_order == mod_prev->iop_order)
                    {
                        ...
                        dt_print(DT_DEBUG_ALWAYS,
                                 ...
                                 mod->op, mod->multi_name, mod->multi_priority, mod->iop_order,
                                 mod_prev->op, mod_prev->multi_name, mod_prev->multi_priority,
                                 mod_prev->iop_order, imgid, msg);
                    }
                }
            }
...
char *dt_ioppr_serialize_text_iop_order_list(GList *iop_order_list)
{
    ...
    for (const GList *l = iop_order_list; l; l = g_list_next(l))
    {
        const dt_iop_order_entry_t *const restrict entry = l->data;
        gchar buf[64];
        snprintf(buf, sizeof(buf), "%s,%d%s", entry->operation, entry->instance,
                 (l == last) ? "" : ",");
...
error:
    dt_print(DT_DEBUG_ALWAYS, "[deserialize text iop_order_list] corrupted iop order list:\n'%s'",
             buf);
    g_list_free_full(iop_order_list, free);
    return NULL;
}
...
error:
    dt_print(DT_DEBUG_ALWAYS, "[deserialize iop_order_list] corrupted iop order list (size %d)",
             (int)size);
    g_list_free_full(iop_order_list, free);
    return NULL;
}
