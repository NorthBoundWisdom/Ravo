...
static void _dt_style_update_from_image(const int id, const dt_imgid_t imgid, GList *filter,
                                        GList *update)
{
...
static void _dt_style_update_iop_order(const gchar *name, const int id, const dt_imgid_t imgid,
                                       ...
                                       const gboolean update_iop_order)
{
    ...
    if (copy_iop_order || update_iop_order)
    {
        ...
        DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                    "UPDATE data.styles SET iop_list=?1 WHERE id=?2", -1, &stmt,
                                    NULL);
    ...
    {
        DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                    "UPDATE data.styles SET iop_list=NULL WHERE id=?1", -1, &stmt,
                                    NULL);
        DT_DEBUG_SQLITE3_BIND_INT(stmt, 1, id);
    }
...
void dt_styles_update(const char *name, const char *newname, const char *newdescription,
                      ...
                      const gboolean copy_iop_order, const gboolean update_iop_order)
{
    ...
    {
        DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                    "UPDATE data.styles SET name=?1, description=?2 WHERE id=?3",
                                    -1, &stmt, NULL);
    ...
    if (filter)
    {
        ...
        char query[4096] = {0};
        snprintf(query, sizeof(query), "DELETE FROM data.style_items WHERE styleid=?1 AND %s",
                 include);
...
gboolean dt_styles_create_from_image(const char *name, const char *description,
                                     const dt_imgid_t imgid, GList *filter,
                                     const gboolean copy_iop_order)
{
    int id = 0;
    sqlite3_stmt *stmt;
    ...
    if ((id = dt_styles_get_id_by_name(name)) != 0)
    {
        ...
        if (filter)
        {
            ...
            for (GList *list = filter; list; list = g_list_next(list))
            {
                ...
                {
                    if (autoinit[0])
                        g_strlcat(autoinit, ",", sizeof(autoinit));
                                ...
                                GList **modules_used, const gboolean append)
{
    ...
    if (mod_src)
    {
        ...
        {
            module = NULL;
            dt_print(DT_DEBUG_ALWAYS, "[dt_styles_apply_style_item] can't load module %s %s",
                     style_item->operation, style_item->multi_name);
        }
        else
        {
...
void _styles_apply_to_image_ext(const char *name, const gboolean duplicate,
                                const gboolean overwrite, const dt_imgid_t imgid,
                                const gboolean undo)
{
    sqlite3_stmt *stmt;
    ...
    if (style_id != 0)
    {
        ...
        dt_print(DT_DEBUG_IOPORDER | DT_DEBUG_PIPE,
                 "[styles_apply_to_image_ext] Apply `%s' on ID=%i, history size %i", name, newimgid,
                 dev_dest->history_end);
...
void dt_styles_apply_to_dev(const char *name, const dt_imgid_t imgid)
{
    if (!darktable.develop || !dt_is_valid_imgid(darktable.develop->image_storage.id))
        return;
    dt_print(DT_DEBUG_IOPORDER | DT_DEBUG_PIPE, "[dt_styles_apply_to_dev] Apply `%s' on ID=%d",
             name, imgid);
...
void dt_styles_delete_by_name_adv(const char *name, const gboolean raise, const gboolean shortcut)
{
    ...
    if ((id = dt_styles_get_id_by_name(name)) != 0)
    {
        ...
        sqlite3_stmt *stmt;
        DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                    "DELETE FROM data.styles WHERE id = ?1", -1, &stmt, NULL);
        DT_DEBUG_SQLITE3_BIND_INT(stmt, 1, id);
        sqlite3_step(stmt);
        sqlite3_finalize(stmt);
        ...
        DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                    "DELETE FROM data.style_items WHERE styleid = ?1", -1, &stmt,
                                    NULL);
        DT_DEBUG_SQLITE3_BIND_INT(stmt, 1, id);
        sqlite3_step(stmt);
        sqlite3_finalize(stmt);
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW)
        {
            ...
            if (!localized)
            {
                ...
                if (has_multi_name && with_multi_name)
                    g_snprintf(iname, sizeof(iname), "%s %s", sqlite3_column_text(stmt, 3),
                               multi_name);
                else
...
static char *dt_style_encode(sqlite3_stmt *stmt, int row)
{
    ...
    char *vparams =
        dt_exif_xmp_encode((const unsigned char *)sqlite3_column_blob(stmt, row), len, NULL);
    return vparams;
}
...
void dt_styles_save_to_file(const char *style_name, const char *filedir, gboolean overwrite)
{
    ...
    snprintf(stylename, sizeof(stylename), "%s/%s.dtstyle", filedir,
    ...
    if (!dt_styles_exists(style_name))
        return;
...
static void dt_styles_style_text_handler(GMarkupParseContext *context, const gchar *text,
                                         const gsize text_len, gpointer user_data, GError **error)
{
    StyleData *style = user_data;
    const gchar *elt = g_markup_parse_context_get_element(context);
...
static void dt_style_plugin_save(StylePluginData *plugin, gpointer styleId)
{
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2
    ...
    DT_DEBUG_SQLITE3_BIND_TEXT(stmt, 4, plugin->operation->str, plugin->operation->len,
                               SQLITE_STATIC);
    //
    ...
    DT_DEBUG_SQLITE3_BIND_TEXT(stmt, 10, plugin->multi_name->str, plugin->multi_name->len,
                               SQLITE_STATIC);
...
static void dt_style_save(StyleData *style)
{
    ...
    /* first create the style header */
    if (!dt_styles_create_style_header(style->info->name->str, style->info->description->str,
                                       style->info->iop_list))
        return;
...
gchar *dt_styles_get_description(const char *name)
{
    ...
    {
        DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                    "SELECT description FROM data.styles WHERE id=?1", -1, &stmt,
                                    NULL);
        DT_DEBUG_SQLITE3_BIND_INT(stmt, 1, id);
        sqlite3_step(stmt);
...
dt_style_t *dt_styles_get_by_name(const char *name)
{
    sqlite3_stmt *stmt;
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "SELECT name, description FROM data.styles WHERE name = ?1", -1,
                                &stmt, NULL);
    DT_DEBUG_SQLITE3_BIND_TEXT(stmt, 1, name, -1, SQLITE_STATIC);
    if (sqlite3_step(stmt) == SQLITE_ROW)
    {
...
gchar *dt_get_style_name(const char *filename)
{
    ...
    {
        dt_print(DT_DEBUG_CONTROL, "[styles] file %s is not a style file", filename);
        if (document)
            xmlFreeDoc(document);
        return bname;
    }
    ...
    if (!bname)
    {
        dt_print(DT_DEBUG_CONTROL, "[styles] file %s is a malformed style file", filename);
    }
    else
    {
        if (strlen(bname) == 0)
