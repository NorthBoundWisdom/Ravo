...
static void _insert_active_images_in_list(GList **list, gboolean only_visible)
{
...
static void _cache_update_register(const gboolean only_visible, const gboolean ordered,
                                   const gboolean inside_sel, const dt_imgid_t mouseover,
                                   GList *images)
{
    ...
    {
        gchar *tx =
            g_strdup_printf("[images to act on - %s] new cache (%s) : ",
                            (dt_act_on_get_algorithm() == DT_ACT_ON_HOVER) ? "hover" : "selection",
                            only_visible ? "visible" : "all");

        for (GList *ll = images; ll; ll = g_list_next(ll))
            dt_util_str_cat(&tx, "%d ", GPOINTER_TO_INT(ll->data));
...
static gboolean _cache_update_hover(const gboolean only_visible, const gboolean force,
    ...
    if (dt_is_valid_imgid(mouseover))
    {
            ...
            dt_ui_thumbtable(darktable.gui->ui)->key_inside)
        {
            ...
            if (inside_sel)
            {
                ...
                if (!force && cache->ok && cache->image_over_inside_sel && cache->inside_table &&
                    cache->ordered == ordered)
                {
                    return FALSE;
                }
...
static gboolean _cache_update_selection(const gboolean only_visible, const gboolean force,
                                        const gboolean ordered)
{
    /** Here's how it works
   *
...
static gboolean _cache_update(const gboolean only_visible, const gboolean force,
                              const gboolean ordered)
{
