...
float (*spline_val[])(int, float[], float, float[], float[]) = {spline_cubic_val, catmull_rom_val,
                                                                catmull_rom_val};

float *(*spline_set[])(int, float[], float[]) = {spline_cubic_set, catmull_rom_set,
...
{
    if (n <= 0 || n > curvedata_anchors_max)
        return NULL;
...
**********************************************************************/
static float *spline_cubic_set_internal(int n, float t[], float y[], int ibcbeg, float ybcbeg,
...
int CurveDataSample(CurveData *curve, CurveSample *sample)
{
    ...
    for (int i = 0; i < (int)sample->m_samplingRes; i++)
    {
        ...
        else
        {
            ...
            int val = interpolate_val(n, x, i * res, y, ypp, curve->m_spline_type) *
                          (sample->m_outputRes - 1) +
                      0.5;
            if (val > maxY)
                val = maxY;
            if (val < minY)
