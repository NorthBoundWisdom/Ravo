...
static size_t image_to_relgrid(const dt_bilateral_t *const b, const int i, const float L, float *xf,
                               float *zf)
{
    float x = CLAMPS(i * b->sigma_s_inv, 0, b->size_x - 1);
    float z = CLAMPS(L * b->sigma_r_inv, 0, b->size_z - 1);
                                  ...
                                  const float sigma_r) // range sigma (blur luma values)
{
    ...
    if (!b->buf)
    {
        dt_print(DT_DEBUG_ALWAYS, "[bilateral] unable to allocate buffer for %zux%zux%zu grid",
...
void dt_bilateral_splat(const dt_bilateral_t *b, const float *const in)
{
    ...
    const size_t offsets[8] = {0, ox, oy, ox + oy, oz, oz + ox, oz + oy, oz + oy + ox};
        ...
        for (int j = firstrow; j < lastrow; j++)
        {
            ...
            for (int i = 0; i < b->width; i++)
            {
                ...
                const dt_aligned_pixel_t contrib = {
                    ...
                    xf * (1.0f - yf) * 100.0f / sigma_s, (1.0f - xf) * yf * 100.0f / sigma_s,
...
static void blur_line_z(float *buf, const int offset1, const int offset2, const int offset3,
...
DT_OMP_DECLARE_SIMD(aligned(buf : 64))
static void blur_line(float *buf, const int offset1, const int offset2, const int offset3,
                      const int size1, const int size2, const int size3)
{
    ...
    for (int k = 0; k < size1; k++)
    {
        ...
        for (int j = 0; j < size2; j++)
        {
            float tmp1 = buf[index];
            buf[index] =
                buf[index] * w0 + w1 * buf[index + offset3] + w2 * buf[index + 2 * offset3];
            index += offset3;
            float tmp2 = buf[index];
                        ...
                        const float detail)
{
    ...
    for (int j = 0; j < height; j++)
    {
        for (int i = 0; i < width; i++)
        {
            ...
            const float Lout =
                fmaxf(0.0f, L + norm * (buf[gi] * (1.0f - xf) * (1.0f - yf) * (1.0f - zf) +
