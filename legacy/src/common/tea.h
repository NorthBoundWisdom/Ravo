...
// Allocate a buffer for storing the internal state of 'numthreads' parallel
// instances of the Tiny Encryption Algorithm.  We need to ensure that each
// state falls in a separate cache line, or all threads sharing a cache line
...
// Run the encryption mixing function using and updating the given internal
// state. For use as a PRNG, you can set arg[0] to the random-number seed,
