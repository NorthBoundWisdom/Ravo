    ...
    if (type == DT_UNDO_COLORLABELS)
    {
        for (GList *list = (GList *)data; list; list = g_list_next(list))
        {
            ...
            const uint8_t before =
                (action == DT_ACTION_UNDO) ? undocolorlabels->after : undocolorlabels->before;
            const uint8_t after =
                (action == DT_ACTION_UNDO) ? undocolorlabels->before : undocolorlabels->after;
...
static void _colorlabels_execute(const GList *imgs, const int labels, GList **undo,
                                 const gboolean undo_on, int action)
{
                               ...
                               const gboolean undo_on)
{
    if (!g_list_is_empty(img))
    {
        ...
        if (undo_on)
        {
            dt_undo_record(darktable.undo, NULL, DT_UNDO_COLORLABELS, undo, _pop_undo,
...
gboolean dt_colorlabels_check_label(const dt_imgid_t imgid, const int color)
{
    if (!dt_is_valid_imgid(imgid))
        return FALSE;
...
{
    if (label < 0 || label >= DT_COLORLABELS_LAST)
        return ""; // shouldn't happen

    return dt_colorlabels_name[label];
}

static float _action_process_color_label(gpointer target, const dt_action_element_t element,
                                         const dt_action_effect_t effect, const float move_size)
{
    ...
    if (DT_PERFORM_ACTION(move_size))
    {
        ...
        // other indication
        if (dt_view_get_current() == DT_VIEW_DARKROOM && g_list_is_singleton(imgs) &&
            darktable.develop->preview_pipe)
        {
            ...
            if (imgid == darktable.develop->preview_pipe->output_imgid)
            {
                ...
                {
                    const GdkRGBA c =
                        darktable.bauhaus->colorlabels[GPOINTER_TO_INT(res_iter->data)];
                    dt_util_str_cat(&result, "<span foreground='#%02x%02x%02x'>⬤ </span>",
                                    (guint)(c.red * 255), (guint)(c.green * 255),
                                    (guint)(c.blue * 255));
                }
                g_list_free(res);
                if (result)
        ...
        dt_collection_update_query(darktable.collection, DT_COLLECTION_CHANGE_RELOAD,
                                   DT_COLLECTION_PROP_COLORLABEL, imgs);
    }
...
const dt_action_def_t dt_action_def_color_label = {N_("color label"), _action_process_color_label,
