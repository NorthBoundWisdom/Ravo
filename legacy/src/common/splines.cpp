...
protected:
    ...
    spline_base(iter i_begin, iter i_end)
    {
        ...
        if (points.empty())
            throw std::invalid_argument("empty set of interpolation points");
    ...
    template <typename iter>
    spline_base(iter i_begin, iter i_end, const limits<T> &x_lim_, const limits<T> &y_lim_,
    ...
    spline_base(const std::initializer_list<point<T>> &I)
        : spline_base(I.begin(), I.end())
    {
    }

    spline_base(const std::initializer_list<point<T>> &I, const limits<T> &x_lim_,
    ...
    T operator()(T x) const
    {
        ...
        if (periodic)
        {
            ...
            n0 = std::upper_bound(points.begin(), points.end(), base_point<T>{x, 0, 0},
                                  [](const base_point<T> &a, const base_point<T> &b)
                                  { return a.x < b.x; }) -
                 points.begin();
        ...
        else
        {
            ...
            y = h00 * points[n0].y + h10 * h * points[n0].dy + h01 * points[n1].y +
                h11 * h * points[n1].dy;
        }
...
template <typename T>
class Catmull_Rom_spline : public spline_base<T>
{
    ...
    void init()
    {
        ...
        else
        {
            ...
            if (periodic)
            {
                const T period{x_lim.max - x_lim.min};
                points[0].dy =
                    (points[1].y - points[N - 1].y) / (points[1].x - points[N - 1].x + period);
                for (size_type i{1}; i < N - 1; ++i)
                    points[i].dy =
                        (points[i + 1].y - points[i - 1].y) / (points[i + 1].x - points[i - 1].x);
                points[N - 1].dy =
                    (points[0].y - points[N - 2].y) / (points[0].x - points[N - 2].x + period);
            }
            else
            {
...
public:
    ...
    Catmull_Rom_spline(iter i_begin, iter i_end)
        : spline_base<T>::spline_base(i_begin, i_end)
    {
        init();
    }
    ...
    Catmull_Rom_spline(const std::initializer_list<point<T>> &I)
        : spline_base<T>::spline_base(I)
    {
        init();
    }

    Catmull_Rom_spline(const std::initializer_list<point<T>> &I, const limits<T> &x_lim_,
                       const limits<T> &y_lim_, bool periodic_ = false)
...
template <typename T>
class monotone_hermite_spline : public spline_base<T>
{
    using base = spline_base<T>;
    using base::periodic;
    using base::points;
        ...
        else
        {
            ...
            if (periodic)
            {
                ...
                for (size_type i{0}; i < N - 1; ++i)
                    Delta.push_back((points[i + 1].y - points[i].y) /
                                    (points[i + 1].x - points[i].x));
                Delta.push_back((points[0].y - points[N - 1].y) /
                                (points[0].x - points[N - 1].x + period));
...
public:
    ...
    template <typename iter>
    monotone_hermite_spline(iter i_begin, iter i_end, const limits<T> &x_lim_,
                            const limits<T> &y_lim_, bool periodic_ = false)
        : spline_base<T>::spline_base(i_begin, i_end, x_lim_, y_lim_, periodic_)
    {
        init();
...
template <typename T>
class monotone_hermite_spline_variant : public spline_base<T>
{
    using base = spline_base<T>;
    using base::periodic;
    using base::points;
    ...
    void init()
    {
        ...
        else
        {
            ...
            if (periodic)
            {
                ...
                for (size_type i{1}; i < N; ++i)
                    points[i].dy = G(Delta[i - 1], Delta[i], h[i - 1], h[i]);
            }
            else
            {
                std::vector<T> h, Delta;
...
public:
    ...
    template <typename iter>
    monotone_hermite_spline_variant(iter i_begin, iter i_end, const limits<T> &x_lim_,
                                    const limits<T> &y_lim_, bool periodic_ = false)
        : spline_base<T>::spline_base(i_begin, i_end, x_lim_, y_lim_, periodic_)
    {
        init();
    ...
    monotone_hermite_spline_variant(const std::initializer_list<point<T>> &I,
                                    const limits<T> &x_lim_, const limits<T> &y_lim_,
                                    bool periodic_ = false)
        : spline_base<T>::spline_base(I, x_lim_, y_lim_, periodic_)
    {
        init();
...
template <typename T>
class smooth_cubic_spline : public spline_base<T>
{
    using base = spline_base<T>;
    using base::periodic;
    using base::points;
...
public:
    ...
    smooth_cubic_spline(const std::initializer_list<point<T>> &I, const limits<T> &x_lim_,
                        const limits<T> &y_lim_, bool periodic_ = false)
        : spline_base<T>::spline_base(I, x_lim_, y_lim_, periodic_)
    {
        init();
                                  ...
                                  float period)
{
    if (type == CUBIC_SPLINE)
    {
        interpol::smooth_cubic_spline<float> s(Points, Points + n, {0.f, period},
                                               interpol::infinity<float>(), true);
        return s(x);
    }
    else if (type == CATMULL_ROM)
    {
        interpol::Catmull_Rom_spline<float> s(Points, Points + n, {0.f, period},
                                              interpol::infinity<float>(), true);
        return s(x);
    }
    else if (type == MONOTONE_HERMITE)
    {
        interpol::monotone_hermite_spline<float> s(Points, Points + n, {0.f, period},
                                                   interpol::infinity<float>(), true);
        return s(x);
    }
    return NAN;
...
int CurveDataSampleV2(CurveData *curve, CurveSample *sample)
{
    try
    {
        ...
        {
            interpol::monotone_hermite_spline<float> s(v.begin(), v.end(),
                                                       {v.front().x, v.back().x},
                                                       {curve->m_min_y, curve->m_max_y}, false);
            for (int i = 0; i < n; ++i)
            {
...
int CurveDataSampleV2Periodic(CurveData *curve, CurveSample *sample)
{
    try
    {
        ...
        if (curve->m_spline_type == CUBIC_SPLINE)
        {
            interpol::smooth_cubic_spline<float> s(v.begin(), v.end(),
                                                   {curve->m_min_x, curve->m_max_x},
                                                   {curve->m_min_y, curve->m_max_y}, true);
            for (unsigned int i = 0; i < sample->m_samplingRes; ++i)
                sample->m_Samples[i] = static_cast<unsigned short int>(
                    std::round(s(i * res) * (sample->m_outputRes - 1)));
        }
        else if (curve->m_spline_type == CATMULL_ROM)
        {
            interpol::Catmull_Rom_spline<float> s(v.begin(), v.end(),
                                                  {curve->m_min_x, curve->m_max_x},
                                                  {curve->m_min_y, curve->m_max_y}, true);
            for (unsigned int i = 0; i < sample->m_samplingRes; ++i)
                sample->m_Samples[i] = static_cast<unsigned short int>(
        ...
        {
            interpol::monotone_hermite_spline_variant<float> s(
                v.begin(), v.end(), {curve->m_min_x, curve->m_max_x},
                {curve->m_min_y, curve->m_max_y}, true);
            for (unsigned int i = 0; i < sample->m_samplingRes; ++i)
                sample->m_Samples[i] = static_cast<unsigned short int>(
                    std::round(s(i * res) * (sample->m_outputRes - 1)));
