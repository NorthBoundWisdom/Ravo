...
static void _image_cache_allocate(void *data, dt_cache_entry_t *entry)
{
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2(
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        if (str)
            g_strlcpy(img->filename, str, sizeof(img->filename));
        str = (char *)sqlite3_column_text(stmt, 6);
        if (str)
            g_strlcpy(img->exif_maker, str, sizeof(img->exif_maker));
        str = (char *)sqlite3_column_text(stmt, 7);
        if (str)
            g_strlcpy(img->exif_model, str, sizeof(img->exif_model));
        str = (char *)sqlite3_column_text(stmt, 8);
        if (str)
            g_strlcpy(img->exif_lens, str, sizeof(img->exif_lens));
...
void dt_image_cache_cleanup()
{
    ...
    if (!cache)
        return;
    dt_print(DT_DEBUG_CACHE, "[image cache cleaup report] fill %.2f/%.2f MB (%.2f%%)",
             cache->cache.cost / (1024.0 * 1024.0), cache->cache.cost_quota / (1024.0 * 1024.0),
...
dt_image_t *dt_image_cache_testget(const dt_imgid_t imgid, const char mode)
{
    ...
    if (!entry)
    {
        dt_print(DT_DEBUG_CACHE, "[dt_image_cache_testget] for imgid=%d failed in dt_cache_testget",
                 imgid);
        return NULL;
    }
...
{
    if (!img || !dt_is_valid_imgid(img->id))
        return;
                                       ...
                                       const char *info)
{
    ...
    if (!dt_is_valid_imgid(img->id))
    {
        dt_cache_release(&cache->cache, img->cache_entry);
        dt_print(DT_DEBUG_ALWAYS,
                 "[image_cache_write_release] from `%s`. FATAL invalid image id %d", info, img->id);
        return;
    }
  ...
  dt_color_harmony_set(img->id, img->color_harmony_guide);
    ...
    DT_DEBUG_SQLITE3_BIND_BLOB(stmt, 22, &img->d65_color_matrix, sizeof(img->d65_color_matrix),
                               SQLITE_STATIC);
    ...
    if (rc != SQLITE_DONE)
        dt_print(DT_DEBUG_ALWAYS,
                 "[image_cache_write_release] from `%s' sqlite3 error %d (%s) for imgid %d", info,
                 rc, sqlite3_errmsg(dt_database_get(darktable.db)), img->id);
    sqlite3_finalize(stmt);
    ...
    if (info)
    {
        ...
        dt_print(DT_DEBUG_CACHE, "[image_cache_write_release] from `%s', imgid=%i took %.3fs", info,
                 img->id, spent);
    }
}
...
void dt_image_cache_set_change_timestamp(const dt_imgid_t imgid)
{
    ...
    if (!cache || !dt_is_valid_imgid(imgid))
        return;
                                                    ...
                                                    const dt_imgid_t sourceid)
{
    dt_image_cache_t *cache = darktable.image_cache;
    assert(cache);
    if (!cache || !dt_is_valid_imgid(imgid) || !dt_is_valid_imgid(sourceid))
        return;
