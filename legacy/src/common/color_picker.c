...
static inline void _update_stats_by_ch(dt_aligned_pixel_t acc, dt_aligned_pixel_t low,
                                       dt_aligned_pixel_t high, const int ch, const float pick)
{
    acc[ch] += pick;
...
static inline void _update_stats_4ch(dt_aligned_pixel_t acc, dt_aligned_pixel_t low,
                                     dt_aligned_pixel_t high, dt_aligned_pixel_t pick)
{
...
typedef void((*picker_worker_4ch)(dt_aligned_pixel_t acc, dt_aligned_pixel_t low,
                                  dt_aligned_pixel_t high, const float *const pixels,
                                  const size_t width,
                                  const dt_iop_order_iccprofile_info_t *const profile));

typedef void((*picker_worker_1ch)(dt_aligned_pixel_t acc, dt_aligned_pixel_t low,
                                  dt_aligned_pixel_t high, dt_aligned_weights_t weights,
                                  const float *const pixels, const size_t j,
                                  const dt_iop_roi_t *const roi, const int *const box,
                                  const void *const data));

static inline void _color_picker_rgb_or_lab(dt_aligned_pixel_t acc, dt_aligned_pixel_t low,
                                            dt_aligned_pixel_t high, const float *const pixels,
                                            const size_t width,
                                            const dt_iop_order_iccprofile_info_t *const profile)
{
...
static inline void _color_picker_lch(dt_aligned_pixel_t acc, dt_aligned_pixel_t low,
                                     dt_aligned_pixel_t high, const float *const pixels,
                                     const size_t width,
                                     const dt_iop_order_iccprofile_info_t *const profile)
{
...
static inline void _color_picker_hsl(dt_aligned_pixel_t acc, dt_aligned_pixel_t low,
                                     dt_aligned_pixel_t high, const float *const pixels,
                                     const size_t width,
                                     const dt_iop_order_iccprofile_info_t *const profile)
{
...
static inline void _color_picker_jzczhz(dt_aligned_pixel_t acc, dt_aligned_pixel_t low,
                                        dt_aligned_pixel_t high, const float *const pixels,
                                        const size_t width,
                                        const dt_iop_order_iccprofile_info_t *const profile)
{
...
static inline void _color_picker_bayer(dt_aligned_pixel_t acc, dt_aligned_pixel_t low,
                                       dt_aligned_pixel_t high, dt_aligned_weights_t weights,
                                       const float *const pixels, const size_t j,
                                       const dt_iop_roi_t *const roi, const int *const box,
                                       const void *const data)
...
static inline void _color_picker_xtrans(dt_aligned_pixel_t acc, dt_aligned_pixel_t low,
                                        dt_aligned_pixel_t high, dt_aligned_weights_t weights,
                                        const float *const pixels, const size_t j,
                                        const dt_iop_roi_t *const roi, const int *const box,
                                        const void *const data)
                                   ...
                                   const picker_worker_4ch worker, const size_t min_for_threads)
{
    ...
    DT_OMP_FOR(if (size > min_for_threads) reduction(+ : acc[:4]) reduction(min : low[:4])
...
{
    cmsHPROFILE *input =
        dt_colorspaces_get_profile(profile->type, profile->filename, DT_PROFILE_DIRECTION_IN)
            ->profile;
    cmsHPROFILE *Lab =
    ...
    DT_OMP_FOR(if (bsize > 10) reduction(+ : acc[:4]) reduction(min : low[:4])
                   reduction(max : high[:4]))
                                   ...
                                   const size_t min_for_threads)
{
    ...
    DT_OMP_FOR(if (_box_size(box) > min_for_threads) reduction(+ : acc[:4], weights[:4])
    ...
    for_each_channel(c) pick[DT_PICK_MEAN][c] = weights[c] ? (acc[c] / (float)weights[c]) : 0.0f;
}
                             ...
                             dt_pixelpipe_picker_source_t picker_source, int *box)
{
  ...
  */
    ...
    dt_dev_distort_transform_plus(
        dev, dev->preview_pipe, module->iop_order - (expanded && input ? 1 : 0),
        input ? DT_DEV_TRANSFORM_DIR_BACK_INCL : DT_DEV_TRANSFORM_DIR_BACK_EXCL, fbox, 4);
                            ...
                            const dt_iop_order_iccprofile_info_t *const profile)
{
    ...
    if (dsc->channels == 4u)
    {
        ...
        if (denoise)
        {
            ...
            if (denoised)
            {
                ...
                // case of CPU path
                blur_2D_Bspline(pixel, denoised, tempbuf, padded_size, roi->width, roi->height, 1,
                                FALSE);
        ...
        else
        {
            ...
            dt_print(DT_DEBUG_ALWAYS, "[colorpicker] unknown colorspace conversion from %s to %s",
    ...
    {
        _color_picker_work_1ch(pixel, roi, box, pick, dsc->xtrans, _color_picker_xtrans, 100);
    }
    else
