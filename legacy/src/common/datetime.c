...
gboolean dt_datetime_exif_to_numbers(dt_datetime_t *dt, const char *exif)
{
    if (exif && *exif && dt)
    {
        ...
        int len = strlen(exif);
        // If TZ data is found in the datetime string we should discard
        // it.  We will memcpy this string for parsing and we have to know
        // where to stop so that the TZ tail after shorter XMP date-time
        // string doesn't damage our parsing buffer.  For possible formats
        // see
...
gboolean dt_datetime_gdatetime_to_local(char *local, const size_t local_size, GDateTime *gdt,
                                        const gboolean msec, const gboolean tz)
{
    if (!local || !local_size || !gdt)
        return FALSE;
    local[0] = '\0';
    if (gdt)
    {
...
GDateTime *dt_datetime_exif_to_gdatetime(const char *exif, const GTimeZone *tz)
{
    ...
    {
        GDateTime *gdt = g_date_time_new((GTimeZone *)tz, dt.year, dt.month, dt.day, dt.hour,
                                         dt.minute, dt.second);
        if (gdt)
        {
            if (dt.msec)
...
{
    // GTimeSpan is UTC based. Therefore we have to cheat a little bit
    // to get image datetime
    if (!tz)
        return NULL;
    ...
    if (gdt)
    {
        ...
        {
            g_date_time_unref(gdt);
            gdt = g_date_time_new((GTimeZone *)tz, dt.year, dt.month, dt.day, dt.hour, dt.minute,
                                  (double)dt.second);
            return gdt;
        }
    }
...
gboolean dt_datetime_entry_to_exif_upper_bound(char *exif, const size_t exif_size,
                                               const char *entry)
{
    if (!exif || !exif_size)
        return FALSE;
...
{
    if (!exif || exif_size < DT_DATETIME_EXIF_LENGTH + 1)
        return;

    g_strlcpy(&exif[DT_DATETIME_EXIF_LENGTH - 1], ".000000",
              exif_size - DT_DATETIME_EXIF_LENGTH + 1);
    for (int i = 0; i < 6 && subsec[i] != '\0' && (DT_DATETIME_EXIF_LENGTH + i < exif_size - 1);
         i++)
...
GTimeSpan dt_datetime_numbers_to_gtimespan(const dt_datetime_t *dt)
{
    if (!dt)
        return 0;
    GDateTime *gdt = g_date_time_new(darktable.utc_tz, dt->year, dt->month, dt->day, dt->hour,
                                     dt->minute, (double)dt->second);
    return _gdatetime_to_gtimespan(gdt);
}
...
GDateTime *dt_datetime_gdatetime_add_numbers(GDateTime *dte, const dt_datetime_t numbers,
                                             const gboolean add)
{
    const int s = add ? 1 : -1;
...
GTimeSpan dt_datetime_gtimespan_add_numbers(const GTimeSpan dt, const dt_datetime_t numbers,
                                            const gboolean add)
{
...
gboolean dt_datetime_exif_add_numbers(const gchar *exif, const dt_datetime_t numbers,
                                      const gboolean add, gchar **result)
{
