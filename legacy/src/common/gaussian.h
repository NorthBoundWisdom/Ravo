...
dt_gaussian_t *dt_gaussian_init(const int width, const int height, const int channels,
                                const float *max, const float *min, const float sigma,
                                const int order);
...
void dt_gaussian_fast_blur(float *in, float *out, const int width, const int height,
                           const float sigma, const float min, const float max, const int channels);
...
dt_gaussian_cl_t *dt_gaussian_init_cl(const int devid, const int width, const int height,
                                      const int channels, const float *max, const float *min,
...
static inline int dt_gaussian_mean_blur_cl(const int devid, cl_mem buf, const int width,
                                           const int height, const int ch, const float sigma)
{
    const float range = 1.0e9f;
    const dt_aligned_pixel_t max = {range, range, range, range};
    const dt_aligned_pixel_t min = {-range, -range, -range, -range};
    dt_gaussian_cl_t *g =
        dt_gaussian_init_cl(devid, width, height, ch, max, min, sigma, DT_IOP_GAUSSIAN_ZERO);
    if (!g)
