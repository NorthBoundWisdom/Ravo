...
static void _transform_from_to_rgb_lab_lcms2(const float *const image_in, float *const image_out,
                                             const int width, const int height,
                                             const dt_colorspaces_color_profile_type_t type,
                                             const char *filename, const int intent,
                                             const int direction)
{
    const int ch = 4;
    ...
    if (rgb_profile)
    {
        ...
        if (rgb_color_space != cmsSigRgbData)
        {
            dt_print(DT_DEBUG_ALWAYS, "working profile color space `%c%c%c%c' not supported",
                     (char)(rgb_color_space >> 24), (char)(rgb_color_space >> 16),
                     (char)(rgb_color_space >> 8), (char)(rgb_color_space));
            rgb_profile = NULL;
        }
    }
    if (rgb_profile == NULL)
    ...
    xform =
        cmsCreateTransform(input_profile, input_format, output_profile, output_format, intent, 0);
...
static void _transform_rgb_to_rgb_lcms2(const float *const image_in, float *const image_out,
                                        const int width, const int height,
                                        ...
                                        const char *filename_to, const int intent)
{
    cmsHTRANSFORM *xform = NULL;
    ...
    const cmsColorSpaceSignature rgb_to_color_space =
        to_rgb_profile ? cmsGetColorSpace(to_rgb_profile) : 0;
    const cmsColorSpaceSignature rgb_from_color_space =
        from_rgb_profile ? cmsGetColorSpace(from_rgb_profile) : 0;
    ...
    if (!from_is_rgb)
    {
        dt_print(
            ...
            (char)(rgb_from_color_space >> 24), (char)(rgb_from_color_space >> 16),
            (char)(rgb_from_color_space >> 8), (char)(rgb_from_color_space));
    ...
    if (!to_is_rgb && !to_is_cmyk)
    {
        dt_print(DT_DEBUG_ALWAYS,
                 ...
                 (char)(rgb_to_color_space >> 24), (char)(rgb_to_color_space >> 16),
                 (char)(rgb_to_color_space >> 8), (char)(rgb_to_color_space));
        to_rgb_profile = NULL;
    }

    if (from_rgb_profile && to_rgb_profile && to_is_cmyk) // softproofing cmyk profile
    {
        cmsHPROFILE tmp_rgb_profile =
            dt_colorspaces_get_profile(DT_COLORSPACE_LIN_REC2020, "", DT_PROFILE_DIRECTION_ANY)
                ->profile;

        uint32_t transformFlags =
            cmsFLAGS_SOFTPROOFING | cmsFLAGS_BLACKPOINTCOMPENSATION | cmsFLAGS_COPY_ALPHA;
        xform = cmsCreateProofingTransform(from_rgb_profile, TYPE_RGBA_FLT, tmp_rgb_profile,
                                           TYPE_RGBA_FLT, to_rgb_profile, intent, intent,
                                           transformFlags);
    }
    else if (from_rgb_profile && to_rgb_profile)
    {
        xform = cmsCreateTransform(from_rgb_profile, TYPE_RGBA_FLT, to_rgb_profile, TYPE_RGBA_FLT,
                                   intent, 0);
    }
                             ...
                             const dt_iop_order_iccprofile_info_t *const profile_info)
{
    *converted_cst = cst_to;
    ...
    {
        dt_print(DT_DEBUG_DEV, "[_transform_lcms2] transfoming from RGB to Lab (%s %s)", self->op,
                 self->multi_name);
    ...
    {
        dt_print(DT_DEBUG_DEV, "[_transform_lcms2] transfoming from Lab to RGB (%s %s)", self->op,
                 self->multi_name);
        _transform_from_to_rgb_lab_lcms2(image_in, image_out, width, height, profile_info->type,
                                         profile_info->filename, profile_info->intent, -1);
    }
...
static inline void
_transform_lcms2_rgb(const float *const image_in, float *const image_out, const int width,
                     const int height,
                     const dt_iop_order_iccprofile_info_t *const profile_info_from,
                     const dt_iop_order_iccprofile_info_t *const profile_info_to)
{
    _transform_rgb_to_rgb_lcms2(image_in, image_out, width, height, profile_info_from->type,
                                profile_info_from->filename, profile_info_to->type,
                                profile_info_to->filename, profile_info_to->intent);
}
...
static inline int _init_unbounded_coeffs(float *const lutr, float *const lutg, float *const lutb,
                                         ...
                                         float *const unbounded_coeffsb, const int lutsize)
{
    int nonlinearlut = 0;
    ...
    for (int k = 0; k < 3; k++)
    {
        ...
        if (lut[k][0] >= 0.0f)
        {
            ...
            const dt_aligned_pixel_t y = {
                extrapolate_lut(lut[k], x[0], lutsize), extrapolate_lut(lut[k], x[1], lutsize),
                extrapolate_lut(lut[k], x[2], lutsize), extrapolate_lut(lut[k], x[3], lutsize)};
...
static inline void
_apply_tonecurves(const float *const image_in, float *const image_out, const int width,
                  const int height, const float *const restrict lutr,
                  const float *const restrict lutg, const float *const restrict lutb,
                  const float *const restrict unbounded_coeffsr,
                  const float *const restrict unbounded_coeffsg,
                  const float *const restrict unbounded_coeffsb, const int lutsize)
{
    const int ch = 4;
    const float *const lut[3] = {lutr, lutg, lutb};
    const float *const unbounded_coeffs[3] = {unbounded_coeffsr, unbounded_coeffsg,
                                              unbounded_coeffsb};
...
static inline void
_transform_rgb_to_lab_matrix(const float *const restrict image_in, float *const restrict image_out,
                             const int width, const int height,
                             const dt_iop_order_iccprofile_info_t *const profile_info)
{
    const int ch = 4;
    ...
    if (profile_info->nonlinearlut)
    {
        ...
        _apply_tonecurves(image_in, image_out, width, height, profile_info->lut_in[0],
                          profile_info->lut_in[1], profile_info->lut_in[2],
                          profile_info->unbounded_coeffs_in[0],
                          profile_info->unbounded_coeffs_in[1],
                          profile_info->unbounded_coeffs_in[2], profile_info->lutsize);
...
static inline void
_transform_lab_to_rgb_matrix(const float *const image_in, float *const image_out, const int width,
                             const int height,
                             const dt_iop_order_iccprofile_info_t *const profile_info)
{
    const int ch = 4;
    ...
    {
        // TODO : maybe optimize that path like _transform_matrix_rgb
        _apply_tonecurves(image_out, image_out, width, height, profile_info->lut_out[0],
                          profile_info->lut_out[1], profile_info->lut_out[2],
                          profile_info->unbounded_coeffs_out[0],
                          profile_info->unbounded_coeffs_out[1],
                          profile_info->unbounded_coeffs_out[2], profile_info->lutsize);
    }
}

static inline void
_transform_matrix_rgb(const float *const restrict image_in, float *const restrict image_out,
                      const int width, const int height,
                      const dt_iop_order_iccprofile_info_t *const profile_info_from,
                      const dt_iop_order_iccprofile_info_t *const profile_info_to)
{
            ...
            if (profile_info_to->nonlinearlut)
            {
                ...
                for (size_t c = 0; c < 3; c++)
                {
                    out[c] =
                        (run_lut_out[c] ?
                             ((temp[c] < 1.0f) ?
                                  extrapolate_lut(profile_info_to->lut_out[c], temp[c],
...
static inline void _transform_matrix(struct dt_iop_module_t *self,
                                     ...
                                     float *const restrict image_out, const int width,
                                     const int height, const dt_iop_colorspace_type_t cst_from,
                                     const dt_iop_colorspace_type_t cst_to,
                                     dt_iop_colorspace_type_t *converted_cst,
                                     const dt_iop_order_iccprofile_info_t *const profile_info)
{
...
void dt_ioppr_cleanup_profile_info(dt_iop_order_iccprofile_info_t *profile_info)
{
    for (int i = 0; i < 3; i++)
    {
        if (profile_info->lut_in[i])
            dt_free_align(profile_info->lut_in[i]);
        if (profile_info->lut_out[i])
                                             ...
                                             const int type, const char *filename, const int intent)
{
    ...
    cmsColorSpaceSignature rgb_profile_color_space =
        rgb_profile ? cmsGetColorSpace(rgb_profile) : 0;

    // get the matrix
    if (rgb_profile)
    {
        if (dt_colorspaces_get_matrix_from_input_profile(
                rgb_profile, profile_info->matrix_in, profile_info->lut_in[0],
                profile_info->lut_in[1], profile_info->lut_in[2], profile_info->lutsize) == 0 &&
            dt_is_valid_colormatrix(profile_info->matrix_in[0][0]) &&
            dt_colorspaces_get_matrix_from_output_profile(
                rgb_profile, profile_info->matrix_out, profile_info->lut_out[0],
                profile_info->lut_out[1], profile_info->lut_out[2], profile_info->lutsize) == 0 &&
            dt_is_valid_colormatrix(profile_info->matrix_out[0][0]))
        {
            ...
            dt_colorspaces_get_primaries_and_whitepoint_from_profile(
                rgb_profile, profile_info->primaries, profile_info->whitepoint);
        }
        else
        {
            _mark_as_nonmatrix_profile(profile_info);
    ...
    if (dt_is_valid_colormatrix(profile_info->matrix_in[0][0]) &&
        dt_is_valid_colormatrix(profile_info->matrix_out[0][0]))
    {
        profile_info->nonlinearlut = _init_unbounded_coeffs(
            ...
            profile_info->unbounded_coeffs_in[2], profile_info->lutsize);
        _init_unbounded_coeffs(profile_info->lut_out[0], profile_info->lut_out[1],
    ...
    if (dt_is_valid_colormatrix(profile_info->matrix_in[0][0]) &&
        dt_is_valid_colormatrix(profile_info->matrix_out[0][0]) && profile_info->nonlinearlut)
    {
        const dt_aligned_pixel_t rgb = {0.1842f, 0.1842f, 0.1842f};
        profile_info->grey = dt_ioppr_get_rgb_matrix_luminance(
            rgb, profile_info->matrix_in, profile_info->lut_in, profile_info->unbounded_coeffs_in,
            profile_info->lutsize, profile_info->nonlinearlut);
    }

    if (type == DT_COLORSPACE_FILE)
        dt_print(
            DT_DEBUG_PIPE, "[generate_profile_info] profile `%s': color space %c%c%c%c%s%s",
            filename, (char)(rgb_profile_color_space >> 24), (char)(rgb_profile_color_space >> 16),
            (char)(rgb_profile_color_space >> 8), (char)(rgb_profile_color_space),
...
dt_iop_order_iccprofile_info_t *
dt_ioppr_get_iop_work_profile_info(const struct dt_iop_module_t *module, GList *iop_list)
{
    ...
    for (GList *modules = iop_list; modules; modules = g_list_next(modules))
    {
        ...
        if (dt_iop_module_is(mod, module->op))
            break;
...
dt_iop_order_iccprofile_info_t *
dt_ioppr_set_pipe_work_profile_info(struct dt_develop_t *dev, struct dt_dev_pixelpipe_t *pipe,
                                    const dt_colorspaces_color_profile_type_t type,
                                    const char *filename, const int intent)
{
    dt_iop_order_iccprofile_info_t *profile_info =
    ...
    if (profile_info == NULL || !dt_is_valid_colormatrix(profile_info->matrix_in[0][0]) ||
        !dt_is_valid_colormatrix(profile_info->matrix_out[0][0]))
    {
        dt_print(DT_DEBUG_ALWAYS,
        ...
        profile_info =
            dt_ioppr_add_profile_info_to_list(dev, DT_COLORSPACE_LIN_REC2020, "", intent);
    }
...
dt_iop_order_iccprofile_info_t *
dt_ioppr_set_pipe_input_profile_info(struct dt_develop_t *dev, struct dt_dev_pixelpipe_t *pipe,
                                     const dt_colorspaces_color_profile_type_t type,
                                     const char *filename, const int intent,
                                     const dt_colormatrix_t matrix_in)
{
    ...
    const gboolean exif_rgb =
        ocsp == DT_IMAGE_COLORSPACE_SRGB || ocsp == DT_IMAGE_COLORSPACE_ADOBE_RGB;
    ...
    if (ptype >= DT_COLORSPACE_EMBEDDED_ICC && ptype <= DT_COLORSPACE_ALTERNATE_MATRIX)
    {
...
dt_iop_order_iccprofile_info_t *
dt_ioppr_set_pipe_output_profile_info(struct dt_develop_t *dev, struct dt_dev_pixelpipe_t *pipe,
                                      const dt_colorspaces_color_profile_type_t type,
                                      const char *filename, const int intent)
{
    dt_iop_order_iccprofile_info_t *profile_info =
...
dt_iop_order_iccprofile_info_t *
dt_ioppr_set_pipe_export_profile_info(struct dt_develop_t *dev, struct dt_dev_pixelpipe_t *pipe,
                                      const dt_colorspaces_color_profile_type_t type,
                                      const char *filename, const int intent)
{
    dt_iop_order_iccprofile_info_t *profile_info =
...
dt_iop_order_iccprofile_info_t *dt_ioppr_get_histogram_profile_info(struct dt_develop_t *dev)
{
    ...
    return dt_ioppr_add_profile_info_to_list(
        dev, histogram_profile_type, histogram_profile_filename, DT_INTENT_RELATIVE_COLORIMETRIC);
}
...
dt_iop_order_iccprofile_info_t *
dt_ioppr_get_pipe_current_profile_info(const dt_iop_module_t *module,
                                       const struct dt_dev_pixelpipe_t *pipe)
{
    ...
    if (color_profile && color_profile->type == DT_COLORSPACE_FILE &&
        (!dt_is_valid_colormatrix(color_profile->matrix_in[0][0]) ||
         !dt_is_valid_colormatrix(color_profile->matrix_out[0][0])))
        dt_print_pipe(DT_DEBUG_PIPE, "current pipe profile", pipe, module, DT_DEVICE_NONE, NULL,
                      NULL, "no matrix in '%s'", color_profile->filename);
                                    ...
                                    const char **profile_filename)
{
    ...
    *profile_filename = NULL;
    ...
    if (colorin)
    {
        dt_colorspaces_color_profile_type_t *_type =
            colorin_so->get_p(colorin->params, "type_work");
                                      ...
                                      const char **profile_filename)
{
    ...
    *profile_filename = NULL;
    ...
    for (const GList *modules = g_list_last(darktable.iop); modules;
    ...
    {
        for (const GList *modules = g_list_last(dev->iop); modules;
             modules = g_list_previous(modules))
        {
            dt_iop_module_t *module = modules->data;
...
gchar *dt_ioppr_get_location_tooltip(const char *subdir, const char *for_name)
{
    ...
    char *tooltip =
        g_markup_printf_escaped(_("darktable loads %s from\n<b>%s</b>\n"
                                         ...
                                         const dt_iop_order_iccprofile_info_t *const profile_info)
{
    ...
    if (no_lcms)
        _transform_matrix(self, image_in, image_out, width, height, cst_from, cst_to, converted_cst,
                          profile_info);
    else
        _transform_lcms2(self, image_in, image_out, width, height, cst_from, cst_to, converted_cst,
                         profile_info);

    dt_print(DT_DEBUG_PERF,
             "[dt_ioppr_transform_image_colorspace%s] %s-->%s took %.3f secs (%.3f CPU) [%s%s]",
             no_lcms ? "" : "_lcms2", dt_iop_colorspace_to_name(cst_from),
             dt_iop_colorspace_to_name(cst_to), dt_get_lap_time(&start_time.clock),
             dt_get_lap_utime(&start_time.user), self->op, dt_iop_get_instance_id(self));
    ...
    if (*converted_cst == cst_from)
    {
        dt_print(DT_DEBUG_ALWAYS,
                 ...
                 " can't %s from %s to %s",
                 no_lcms ? "" : "_lcms2", self->op, dt_iop_get_instance_id(self),
                 dt_colorspaces_get_name(profile_info->type, profile_info->filename),
                 inplace ? "convert inplace" : "write converted data",
                 dt_iop_colorspace_to_name(cst_from), dt_iop_colorspace_to_name(cst_to));
    }
...
void dt_ioppr_transform_image_colorspace_rgb(
    const float *const restrict image_in, float *const restrict image_out, const int width,
    const int height, const dt_iop_order_iccprofile_info_t *const profile_info_from,
    const dt_iop_order_iccprofile_info_t *const profile_info_to, const char *message)
{
    if (!profile_info_from || !profile_info_to || profile_info_from->type == DT_COLORSPACE_NONE ||
    ...
    if (no_lcms)
        _transform_matrix_rgb(image_in, image_out, width, height, profile_info_from,
                              profile_info_to);
    else
        _transform_lcms2_rgb(image_in, image_out, width, height, profile_info_from,
                             profile_info_to);

    dt_print(DT_DEBUG_PIPE, "dt_ioppr_transform_image_colorspace_rgb%s `%s' -> `%s' [%s]",
             no_lcms ? "" : "_lcms2",
...
dt_colorspaces_cl_global_t *dt_colorspaces_init_cl_global()
{
    ...
    g->kernel_colorspaces_gamma = dt_opencl_create_kernel(program, "colorspaces_transform_gamma");
    return g;
}
                                           ...
                                           cl_mem *_dev_profile_lut)
{
    ...
    if (profile_info)
    {
        ...
        dev_profile_lut =
            dt_opencl_copy_host_to_image(devid, profile_lut_cl, 256, 256 * 6, sizeof(float));
    ...
    else
    {
        ...
        if (profile_lut_cl)
            dev_profile_lut =
                dt_opencl_copy_host_to_image(devid, profile_lut_cl, 1, 1 * 6, sizeof(float));
        else
...
gboolean dt_ioppr_transform_image_colorspace_cl(
    struct dt_iop_module_t *self, const int devid, cl_mem dev_img_in, cl_mem dev_img_out,
    const int width, const int height, const dt_iop_colorspace_type_t cst_from,
    const dt_iop_colorspace_type_t cst_to, dt_iop_colorspace_type_t *converted_cst,
    const dt_iop_order_iccprofile_info_t *const profile_info)
{
    cl_int err = CL_SUCCESS;
    ...
    *converted_cst = cst_to;
    ...
    {
        if (!inplace)
            err = dt_opencl_enqueue_copy_image(devid, dev_img_in, dev_img_out, CLIMG_ORIGIN,
                                               CLIMG_ORIGIN, region);
        ...
        *converted_cst = cst_from;
        ...
        if (!inplace || cst_to == IOP_CS_RAW || cst_from == IOP_CS_RAW)
            dt_print(DT_DEBUG_PIPE,
                     ...
                     " can't %s from %s to %s",
                     err == CL_SUCCESS ? "" : " error", self->op, dt_iop_get_instance_id(self),
                     profile_info ?
                         dt_colorspaces_get_name(profile_info->type, profile_info->filename) :
                         "unknown",
                     inplace ? "convert inplace" : "write converted data",
        ...
        dt_is_valid_colormatrix(profile_info->matrix_out[0][0]))
    {
        ...
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, kernel_transform, width, height, CLARG(dev_img_in), CLARG(dev_img_out),
            CLARG(width), CLARG(height), CLARG(dev_profile_info), CLARG(dev_lut));
        if (err == CL_SUCCESS)
            dt_print(
                DT_DEBUG_PERF,
                "[dt_ioppr_transform_image_colorspace_cl]%s %s-->%s took %.3f secs (%.3f GPU) [%s%s]",
                inplace ? " inplace" : "", dt_iop_colorspace_to_name(cst_from),
                dt_iop_colorspace_to_name(cst_to), dt_get_lap_time(&start_time.clock),
                dt_get_lap_utime(&start_time.user), self->op, dt_iop_get_instance_id(self));
    }
    else
    {
        ...
        err = dt_opencl_copy_image_to_host(devid, src_buffer, dev_img_in, width, height,
                                           4 * sizeof(float));
        if (err != CL_SUCCESS)
            goto cleanup;

        // just call the CPU version for now
        dt_ioppr_transform_image_colorspace(self, src_buffer, src_buffer, width, height, cst_from,
                                            cst_to, converted_cst, profile_info);

        err = dt_opencl_write_host_to_image(devid, src_buffer, dev_img_out, width, height,
                                            4 * sizeof(float));
    }
...
gboolean dt_ioppr_transform_image_colorspace_rgb_cl(
    const int devid, cl_mem dev_img_in, cl_mem dev_img_out, const int width, const int height,
    const dt_iop_order_iccprofile_info_t *const profile_info_from,
    const dt_iop_order_iccprofile_info_t *const profile_info_to, const char *message)
{
    cl_int err = CL_SUCCESS;
        ...
        dt_is_valid_colormatrix(profile_info_to->matrix_out[0][0]))
    {
        ...
        kernel_transform =
            darktable.opencl->colorspaces->kernel_colorspaces_transform_rgb_matrix_to_rgb;
        ...
        dev_profile_info_from = dt_opencl_copy_host_to_device_constant(
            devid, sizeof(profile_info_from_cl), &profile_info_from_cl);
        dev_lut_from =
            dt_opencl_copy_host_to_image(devid, lut_from_cl, 256, 256 * 6, sizeof(float));

        dev_profile_info_to = dt_opencl_copy_host_to_device_constant(
            devid, sizeof(profile_info_to_cl), &profile_info_to_cl);
        ...
        if (dev_lut_to == NULL || dev_profile_info_to == NULL || dev_lut_from == NULL ||
            dev_profile_info_from == NULL || matrix_cl == NULL)
        {
            err = CL_MEM_OBJECT_ALLOCATION_FAILURE;
            goto cleanup;
        }
        ...
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, kernel_transform, width, height, CLARG(dev_img_in), CLARG(dev_img_out),
            CLARG(width), CLARG(height), CLARG(dev_profile_info_from), CLARG(dev_lut_from),
            CLARG(dev_profile_info_to), CLARG(dev_lut_to), CLARG(matrix_cl));
        if (err == CL_SUCCESS)
        {
            dt_print(DT_DEBUG_PIPE,
                     "dt_ioppr_transform_image_colorspace_rgb_CL%s `%s' -> `%s' [%s]",
                     in_place ? " inplace" : "",
                     dt_colorspaces_get_name(profile_info_from->type, profile_info_from->filename),
                     dt_colorspaces_get_name(profile_info_to->type, profile_info_to->filename),
                     message ? message : "");
            ...
            dt_print(
                DT_DEBUG_PERF,
                "image colorspace transform_rgb_CL%s  `%s' -> `%s' took %.3f secs (%.3f GPU) [%s]",
                in_place ? " inplace" : "",
                dt_colorspaces_get_name(profile_info_from->type, profile_info_from->filename),
                dt_colorspaces_get_name(profile_info_to->type, profile_info_to->filename),
                dt_get_lap_time(&start_time.clock), dt_get_lap_utime(&start_time.user),
    ...
    else
    {
        ...
        err = dt_opencl_copy_image_to_host(devid, src_buffer_in, dev_img_in, width, height,
                                           4 * sizeof(float));
        if (err != CL_SUCCESS)
            goto cleanup;

        // just call the CPU version for now
        dt_ioppr_transform_image_colorspace_rgb(src_buffer_in, src_buffer_out, width, height,
                                                profile_info_from, profile_info_to, message);

        err = dt_opencl_write_host_to_image(devid, src_buffer_out, dev_img_out, width, height,
                                            4 * sizeof(float));
    }
