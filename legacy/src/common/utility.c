...
#if !defined(_WIN32) && (defined __APPLE__ || defined _POSIX_C_SOURCE >= 1 || defined _XOPEN_SOURCE || \
...
gchar *dt_util_localize_segmented_name(const char *s, const gboolean with_space)
{
    const char *sep = with_space ? " | " : "|";

    const gboolean is_builtin = g_str_has_prefix(s, BUILTIN_PREFIX);

    const char *local_s = is_builtin ? _(s + sizeof(BUILTIN_PREFIX) - 1) : s;
...
gchar *dt_util_str_replace(const gchar *string, const gchar *pattern, const gchar *substitute)
{
    ...
    {
        nstring =
            g_malloc_n(strlen(string) + (occurrences * strlen(substitute)) + 1, sizeof(gchar));
 ...
 **/
size_t dt_utf8_strlcpy(char *dest, const char *src, const size_t n)
{
...
static cairo_surface_t *_util_get_svg_img(gchar *logo, const float size)
{
    ...
    if (svg)
    {
        ...
        if (darktable.gui)
            surface = dt_cairo_image_surface_create_for_data(image_buffer, CAIRO_FORMAT_ARGB32,
                                                             final_width, final_height, stride);
        else // during startup we don't know ppd yet and darktable.gui isn't initialized yet.
            surface = cairo_image_surface_create_for_data(image_buffer, CAIRO_FORMAT_ARGB32,
                                                          final_width, final_height, stride);
...
gboolean dt_util_gps_rationale_to_number(const double r0_1, const double r0_2, const double r1_1,
                                         const double r1_2, const double r2_1, const double r2_2,
...
gboolean dt_util_gps_elevation_to_number(const double r_1, const double r_2, char sign,
                                         double *result)
{
    if (!result)
        return FALSE;
...
gchar *dt_util_normalize_path(const gchar *_input)
{
    ...
    if (!filename)
    {
        if (g_str_has_prefix(
                input,
                "file://")) // in this case we should take care of %XX encodings in the string
                            // (for example %20 = ' ')
        {
            input += strlen("file://");
    ...
    if (first >= 'A' && first <= 'Z' &&
        filename[1] == ':') // path format is <drive letter>:\path\to\file
    {
        filename[0] = first;
        return filename;
    }
...
void dt_copy_file(const char *const sourcefile, const char *destination)
{
    ...
    if (fin && fout)
    {
        ...
        {
            dt_print(DT_DEBUG_ALWAYS,
                     "[dt_copy_file] failure to allocate memory for copying file '%s'", sourcefile);
            goto END;
        }
        if (fread(content, sizeof(char), filesize, fin) != filesize)
        {
            dt_print(DT_DEBUG_ALWAYS, "[dt_copy_file] error reading file '%s' for copying",
                     sourcefile);
            goto END;
        }
        if (fwrite(content, sizeof(char), filesize, fout) != filesize)
        {
            dt_print(DT_DEBUG_ALWAYS, "[dt_copy_file] error writing file '%s' during copying",
                     destination);
            goto END;
        }
    }
...
// rsvg_handle_get_dimensions has been deprecated in librsvg 2.52
    ...
    if (rsvg_handle_get_intrinsic_size_in_pixels(
            svg, &width, &height)) //only works if SVG document has size specified
    {
    ...
    {
#define VIEWPORT_SIZE                                                                              \
    32767 //use maximum cairo surface size to have enough precision when size is converted to int
...
void dt_render_svg(RsvgHandle *svg, cairo_t *cr, const double width, const double height,
