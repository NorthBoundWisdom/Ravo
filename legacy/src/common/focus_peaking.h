...
static inline float _laplacian(const float *const image, const size_t index[8])
{
    ...
    const float l1 =
        dt_fast_hypotf(image[index[4]] - image[index[3]], image[index[6]] - image[index[1]]);
    const float l2 =
        dt_fast_hypotf(image[index[7]] - image[index[0]], image[index[5]] - image[index[2]]);
...
DT_OMP_DECLARE_SIMD()
static inline void _get_indices(const size_t i, const size_t j, const size_t width,
                                const size_t height, const size_t delta, size_t index[8])
{
                                   ...
                                   uint8_t *const restrict image)
{
    ...
    // Prefilter noise
    fast_surface_blur(luma, buf_width, buf_height, 12, 0.00001f, 4, DT_GF_BLENDING_LINEAR, 1, 0.0f,
                      exp2f(-8.0f), 1.0f);
    ...
    for (size_t i = 0; i < buf_height; ++i)
        for (size_t j = 0; j < buf_width; ++j)
        {
            ...
            else
            {
                ...
                luma_ds[index] = _laplacian(luma, index_close) -
    ...
    fast_surface_blur(luma_ds, buf_width, buf_height, 12, 0.00001f, 4, DT_GF_BLENDING_LINEAR, 1,
                      0.0f, exp2f(-8.0f), 1.0f);
    ...
    cairo_surface_t *surface = cairo_image_surface_create_for_data(
        (unsigned char *)focus_peaking, CAIRO_FORMAT_ARGB32, buf_width, buf_height,
