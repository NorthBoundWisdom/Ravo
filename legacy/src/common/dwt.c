...
static int _get_max_scale(const int width, const int height, const float preview_scale)
{
    ...
    while ((maxscale > 0) && ((1 << maxscale) * preview_scale >= size))
        maxscale--;
...
{
    return _get_max_scale(p->width / p->preview_scale, p->height / p->preview_scale,
                          p->preview_scale);
}
...
static void dwt_decompose_horiz(float *const restrict out, float *const restrict in,
                                ...
                                const size_t width, const size_t lev)
{
    ...
    for (int row = 0; row < height; row++)
    {
        ...
        {
            const size_t leftpos =
                (size_t)4 * abs(col - hscale); // the abs() handles reflection at the left edge
        ...
        for (int col = width - hscale; col < width; col++)
        {
            const size_t leftpos =
                (size_t)4 *
                abs(col - hscale); // still need to handle reflection, if hscale>=width/2
...
static void dwt_decompose_layer(float *const restrict out, float *const restrict in,
                                float *const temp, size_t padded_size, const int lev,
...
static void dwt_wavelet_decompose(float *img, dwt_params_t *const p, _dwt_layer_func layer_func)
{
    ...
    if (!dt_iop_alloc_image_buffers(NULL, &roi, &roi, 4 | DT_IMGSZ_INPUT, &buffer[1],
                                    4 | DT_IMGSZ_INPUT | DT_IMGSZ_CLEARBUF, &layers,
                                    4 | DT_IMGSZ_WIDTH | DT_IMGSZ_PERTHREAD, &temp, &padded_size,
                                    (do_merge ? 4 | DT_IMGSZ_INPUT | DT_IMGSZ_CLEARBUF : 0),
                                    &merged_layers, 0, NULL))
    {
    ...
    for (unsigned int lev = 0; lev < p->scales && bcontinue; lev++)
    {
        ...
        if (p->merge_from_scale == 0 || p->merge_from_scale > lev + 1)
        {
            ...
            if (layer_func)
                layer_func(buffer[hpass], p, lev + 1);
        ...
        else
        {
            ...
            if (layer_func)
                layer_func(merged_layers, p, lev + 1);
    ...
    if (bcontinue)
    {
        ...
        if (layer_func)
            layer_func(buffer[hpass], p, p->scales + 1);
        ...
        else if (p->return_layer == 0)
        {
            ...
            if (layer_func)
                layer_func(layers, p, p->scales + 2);
...
static void dwt_denoise_horiz_1ch(float *const restrict out, float *const restrict in,
                                  float *const restrict accum, const size_t height,
                                  ...
                                  const int last)
{
    ...
    for (int row = 0; row < height; row++)
    {
        ...
        for (int col = 0; col < hscale; col++)
        {
            // add up left/center/right, and renormalize by dividing by the total weight of all numbers added together
            const float hat =
                (2.f * coarse[col] + coarse[hscale - col] + coarse[col + hscale]) / 16.f;
            ...
            accum_row[col] += diff < 0.0f ? MIN(diff + thold, 0.0f) : MAX(diff - thold, 0.0f);
        }
        DT_OMP_SIMD()
        for (int col = hscale; col < width - hscale; col++)
        {
            // add up left/center/right, and renormalize by dividing by the total weight of all numbers added together
            const float hat =
                (2.f * coarse[col] + coarse[col - hscale] + coarse[col + hscale]) / 16.f;
            // the normalized value is our 'coarse' result; 'diff' is the difference between original input and 'coarse'
            // (which would ordinarily be stored as the details scale, but we don't need it any further)
            const float diff = details[col] - hat;
            details[col] = hat; // done with original input, so we can overwrite it with 'coarse'
...
static cl_int dwt_subtract_layer_cl(cl_mem bl, cl_mem bh, dwt_params_cl_t *const p)
{
    ...
    return dt_opencl_enqueue_kernel_2d_args(devid, kernel, p->width, p->height, CLARG(bl),
                                            CLARG(bh), CLARG(p->width), CLARG(p->height),
                                            CLARG(lpass_mult));
}

static cl_int dwt_add_layer_cl(cl_mem img, cl_mem layers, dwt_params_cl_t *const p,
                               const int n_scale)
{
    const int devid = p->devid;
    const int kernel = p->global->kernel_dwt_add_img_to_layer;

    return dt_opencl_enqueue_kernel_2d_args(devid, kernel, p->width, p->height, CLARG(img),
...
static cl_int dwt_get_image_layer_cl(cl_mem layer, dwt_params_cl_t *const p)
{
    ...
    if (p->image != layer)
        err = dt_opencl_enqueue_copy_buffer_to_buffer(
            p->devid, layer, p->image, 0, 0, (size_t)p->width * p->height * p->ch * sizeof(float));

    return err;
}

static cl_int dwt_wavelet_decompose_cl(cl_mem img, dwt_params_cl_t *const p,
                                       _dwt_layer_func_cl layer_func)
{
    ...
    // init layer buffer
    err = dt_opencl_enqueue_kernel_2d_args(devid, p->global->kernel_dwt_init_buffer, p->width,
                                           p->height, CLARG(layers), CLARG(p->width),
                                           CLARG(p->height));
    if (err != CL_SUCCESS)
        goto cleanup;
    ...
    {
        merged_layers =
            dt_opencl_alloc_device_buffer(devid, sizeof(float) * p->ch * p->width * p->height);
        if (merged_layers == NULL)
            goto cleanup;

        // init reconstruct buffer
        err = dt_opencl_enqueue_kernel_2d_args(devid, p->global->kernel_dwt_init_buffer, p->width,
                                               p->height, CLARG(merged_layers), CLARG(p->width),
                                               CLARG(p->height));
        if (err != CL_SUCCESS)
            goto cleanup;
    }
    ...
    for (unsigned int lev = 0; lev < p->scales && bcontinue; lev++)
    {
        ...
        // hat transform by row
        {
            ...
            err = dt_opencl_enqueue_kernel_2d_args(
                devid, p->global->kernel_dwt_hat_transform_row, p->width, p->height, CLARG(temp),
                CLARG((buffer[hpass])), CLARG(p->width), CLARG(p->height), CLARG(sc));
            if (err != CL_SUCCESS)
                goto cleanup;
        }
        ...
        // hat transform by col
        {
            ...
            err = dt_opencl_enqueue_kernel_2d_args(devid, p->global->kernel_dwt_hat_transform_col,
                                                   p->width, p->height, CLARG(temp),
                                                   CLARG(p->width), CLARG(p->height), CLARG(sc),
                                                   CLARG((buffer[lpass])), CLARG(lpass_mult));
            if (err != CL_SUCCESS)
                goto cleanup;
        }
