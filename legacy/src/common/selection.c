...
static void _selection_select(dt_selection_t *selection, const dt_imgid_t imgid)
{
    if (dt_is_valid_imgid(imgid))
    {
        ...
        if (image)
        {
            ...
            if (!darktable.gui || !darktable.gui->grouping ||
                darktable.gui->expanded_group_id == img_group_id || !selection->collection)
            {
...
void _selection_update_collection(gpointer instance, const dt_collection_change_t query_change,
                                  const dt_collection_properties_t changed_property, gpointer imgs,
                                  const int next, gpointer user_data)
{
    ...
    dt_collection_set_query_flags(
        ...
        (dt_collection_get_query_flags(selection->collection) & (~(COLLECTION_QUERY_USE_LIMIT))));
...
const dt_selection_t *dt_selection_new()
{
    ...
    _selection_update_collection(NULL, DT_COLLECTION_CHANGE_RELOAD, DT_COLLECTION_PROP_UNDEF, NULL,
                                 -1, (gpointer)s);
...
void dt_selection_invert(dt_selection_t *selection)
{
    ...
    gchar *fullq = g_strdup_printf("INSERT OR IGNORE INTO main.selected_images (imgid) %s",
    ...
    DT_DEBUG_SQLITE3_EXEC(dt_database_get(darktable.db),
                          ...
                          NULL, NULL, NULL);
    DT_DEBUG_SQLITE3_EXEC(dt_database_get(darktable.db), "DELETE FROM main.selected_images", NULL,
                          NULL, NULL);
    ...
    DT_DEBUG_SQLITE3_EXEC(dt_database_get(darktable.db),
                          ...
                          NULL, NULL, NULL);
    DT_DEBUG_SQLITE3_EXEC(dt_database_get(darktable.db), "DELETE FROM memory.tmp_selection", NULL,
                          NULL, NULL);
...
void dt_selection_select_range(dt_selection_t *selection, const dt_imgid_t imgid)
{
    if (!selection->collection)
        return;
    ...
    int sr = -1, er = -1;
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                dt_collection_get_query_no_group(selection->collection), -1, &stmt,
                                NULL);
