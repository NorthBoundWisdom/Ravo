...
static void _dt_collection_recount_callback_2(gpointer instance, const uint8_t id,
                                              gpointer user_data);
static void _dt_collection_filmroll_imported_callback(gpointer instance, const uint8_t id,
                                                      gpointer user_data);
...
const dt_collection_t *dt_collection_new(const dt_collection_t *clone)
{
   ...
   */
    DT_CONTROL_SIGNAL_CONNECT(DT_SIGNAL_TAG_CHANGED, _dt_collection_recount_callback_tag,
                              collection);
    DT_CONTROL_SIGNAL_CONNECT(DT_SIGNAL_FILMROLLS_CHANGED, _dt_collection_recount_callback_filmroll,
                              collection);
    DT_CONTROL_SIGNAL_CONNECT(DT_SIGNAL_FILMROLLS_REMOVED, _dt_collection_recount_callback_filmroll,
                              collection);

    DT_CONTROL_SIGNAL_CONNECT(DT_SIGNAL_IMAGE_IMPORT, _dt_collection_recount_callback_2,
                              collection);
    DT_CONTROL_SIGNAL_CONNECT(DT_SIGNAL_FILMROLLS_IMPORTED,
                              _dt_collection_filmroll_imported_callback, collection);
    return collection;
}
...
{
    if (!darktable.collection || !darktable.db)
        return;
    sqlite3_stmt *stmt;
...
static void _dt_collection_set_selq_pre_sort(const dt_collection_t *collection, char **selq_pre)
{
    ...
    if (collection->params.query_flags & COLLECTION_QUERY_USE_SORT)
    {
        ...
        {
            tag_join = (tagid);
            dt_util_str_cat(&fields, ", %s position",
...
int dt_collection_update(const dt_collection_t *collection)
{
    ...
    {
        wq = g_strdup_printf("%s (film_id = %u)", and_operator(&and_term),
                             collection->params.film_id);
    }
    // DON'T SELECT IMAGES MARKED TO BE DELETED.
    dt_util_str_cat(&wq, " %s (flags & %d) != %d", and_operator(&and_term), DT_IMAGE_REMOVE,
                    DT_IMAGE_REMOVE);
    ...
    if (collection->params.query_flags & COLLECTION_QUERY_USE_SORT)
    {
        ...
        {
            dt_util_str_cat(&selq_post,
                            " LEFT OUTER JOIN main.color_labels AS b ON sel.id = b.imgid");
        }
        if (collection->params.sorts[DT_COLLECTION_SORT_PATH])
        {
            // clang-format off
    ...
    /* store the new query */
    dt_util_str_cat(
        &query, "%s%s%s %s%s", selq_pre, wq, selq_post, sq ? sq : "",
        (collection->params.query_flags & COLLECTION_QUERY_USE_LIMIT) ? " " LIMIT_QUERY : "");
    dt_util_str_cat(
        &query_no_group, "%s%s%s %s%s", selq_pre, wq_no_group, selq_post, sq ? sq : "",
        (collection->params.query_flags & COLLECTION_QUERY_USE_LIMIT) ? " " LIMIT_QUERY : "");
...
void dt_collection_reset(const dt_collection_t *collection)
{
    ...
    dt_collection_update_query(collection, DT_COLLECTION_CHANGE_NEW_QUERY, DT_COLLECTION_PROP_UNDEF,
                               NULL);
}
...
gchar *dt_collection_get_extended_where(const dt_collection_t *collection, const int exclude)
{
    ...
    else
    {
        ...
        const int nb_filters =
            CLAMP(dt_conf_get_int("plugins/lighttable/filtering/num_rules"), 0, 10);
...
static void _image_set_raw_aspect_ratio(const dt_imgid_t imgid)
{
    ...
    if (image)
    {
        ...
        const int side =
            image->orientation < ORIENTATION_SWAP_XY ? image->p_height : image->p_width;
...
static gchar *_dt_collection_get_sort_text(const dt_collection_sort_t sort, const int sortorder)
{
    ...
    case DT_COLLECTION_SORT_GROUP:
        sq =
            g_strdup_printf("group_id%s, sel.id-group_id != 0, sel.id", (sortorder) ? " DESC" : "");
        break;
...
gchar *dt_collection_get_sort_query(const dt_collection_t *collection)
{
    ...
    if (!filename)
        dt_util_str_cat(&query, ", filename%s", (first_order) ? " DESC" : "");
                                             ...
                                             const gboolean no_group)
{
    sqlite3_stmt *stmt = NULL;
    uint32_t count = 0;
    const gchar *query = no_group ? dt_collection_get_query_no_group(collection) :
...
GList *dt_collection_get(const dt_collection_t *collection, const int limit,
        ...
        else
        {
            if (collection->params.query_flags & COLLECTION_QUERY_USE_LIMIT)
            {
                DT_DEBUG_SQLITE3_PREPARE_V2(
                    ...
                    "SELECT imgid FROM memory.collected_images LIMIT -1, ?1", -1, &stmt, NULL);
                DT_DEBUG_SQLITE3_BIND_INT(stmt, 1, limit);
            }
            else
                DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                         ...
                                         char **operator)
{
    ...
    *number1 = *number2 = *operator= NULL;
    ...
    regex = g_regex_new(
        "^\\s*\\[\\s*([-+]?[0-9]+\\.?[0-9]*)\\s*;\\s*([-+]?[0-9]+\\.?[0-9]*)\\s*\\]\\s*$", 0, 0,
        NULL);
                                           ...
                                           char **operator)
{
    ...
    *number1 = *number2 = *operator= NULL;
    ...
    regex = g_regex_new(
        "^\\s*\\[\\s*([+-]?\\d{4}[:.\\d\\s]*)\\s*;\\s*((?:now)|[+-]?\\d{4}[:.\\d\\s]*)\\s*\\]\\s*$",
        0, 0, NULL);
    g_regex_match_full(regex, input, -1, 0, 0, &match_info, NULL);
    int match_count = g_match_info_get_match_count(match_info);
                                           ...
                                           char **operator)
{
    ...
    *number1 = *number2 = *operator= NULL;
    ...
    // we test the range expression first
    regex = g_regex_new(
        "^\\s*\\[\\s*(1/)?([0-9]+\\.?[0-9]*)(\")?\\s*;\\s*(1/)?([0-9]+\\.?[0-9]*)(\")?\\s*\\]\\s*$",
        0, 0, NULL);
    g_regex_match_full(regex, input, -1, 0, 0, &match_info, NULL);
    int match_count = g_match_info_get_match_count(match_info);
...
// ["ab,cd",ef,"ghi"]
static gchar **_strsplit_quotes(const gchar *string, const gchar *delimiter, gint max_tokens)
{
    char *s;
...
static gchar *get_query_string(const dt_collection_properties_t property, const gchar *text)
{
    ...
    case DT_COLLECTION_PROP_HISTORY: // history
        if (!g_strcmp0(escaped_text, _("basic")) || !g_strcmp0(escaped_text, "$BASIC"))
        {
            ...
            query = g_strdup(
                "(id not IN (SELECT imgid FROM main.history_hash WHERE (basic_hash IS NULL OR "
                "current_hash != basic_hash)))");
        }
        else if (!g_strcmp0(escaped_text, _("auto applied")) ||
                 !g_strcmp0(escaped_text, "$AUTO_APPLIED"))
        {
    ...
    case DT_COLLECTION_PROP_DUPLICATES: // duplicates
        ...
        {
            query =
                g_strdup("(mi.version > (SELECT MIN(version) FROM main.images"
    ...
    case DT_COLLECTION_PROP_ASPECT_RATIO: // aspect ratio
    {
        ...
        {
            if (number1 && number2)
                query = g_strdup_printf("((aspect_ratio >= %s) AND (aspect_ratio <= %s))", number1,
                                        number2);
        }
    ...
    case DT_COLLECTION_PROP_GROUP_ID: // group id
        ...
        for (int i = 0; i < g_strv_length(elems); i++)
        {
            ...
            dt_util_str_cat(&query, "%sgroup_id LIKE '%s'", i > 0 ? " OR " : "", group_ids);
            g_free(group_ids);
        }
        g_strfreev(elems);
        dt_util_str_cat(&query, ")");
    ...
    case DT_COLLECTION_PROP_FOCAL_LENGTH: // focal length
    {
        ...
        {
            if (number1 && number2)
                query = g_strdup_printf("((focal_length >= %s) AND (focal_length <= %s))", number1,
                                        number2);
        }
        else if (operator&& number1)
    ...
    case DT_COLLECTION_PROP_FILENAME: // filename
    {
        ...
        if (g_strv_length(elems) > 1)
        {
            ...
            if (!g_list_is_empty(list))
            {
                ...
                {
                    gchar *s = subquery;
                    subquery = g_strdup_printf("(%s) AND (%s)", subquery,
                                               dt_util_glist_to_str(" OR ", list));
                    g_free(s);
                }
                else
                    subquery = dt_util_glist_to_str(" OR ", list);
    ...
    case DT_COLLECTION_PROP_PRINT_TIMESTAMP:
    {
        ...
        if (strcmp(operator, "[]") == 0)
        {
            ...
            query =
                g_strdup_printf("((%s >= %" G_GINT64_FORMAT ") AND (%s <= %" G_GINT64_FORMAT "))",
                                colname, nb1, colname, nb2);
        ...
        else if (operator&& g_strcmp0(operator, "=") && number1)
        {
            ...
            { // <> exclusion operator
                ...
                { // all except star rating (including rejected)
                    query =
                        g_strdup_printf("(flags & 8 == 8 OR flags & 7 %s %s)", operator, number1);
                }
            }
        }
        else if (number1)
                                ...
                                GList *list)
{
  ...
  if(!collection->clone)
  {
    ...
    dt_collection_set_query_flags(
        collection, (dt_collection_get_query_flags(collection) | COLLECTION_QUERY_USE_WHERE_EXT));

    /* remove film id from default filter */
    dt_collection_set_filter_flags(
        collection, (dt_collection_get_filter_flags(collection) & ~COLLECTION_FILTER_FILM_ID));
...
void dt_collection_hint_message(const dt_collection_t *collection)
{
    ...
    if (cs == 1)
    {
        ...
        message = g_strdup_printf(_("<b>%d</b> image (#<b>%d</b>) selected of <b>%d</b>"), cs,
                                  selected, c);
    }
    else
    {
        message = g_strdup_printf(ngettext("<b>%d</b> image selected of <b>%d</b>",
                                           "<b>%d</b> images selected of <b>%d</b>", cs),
                                  cs, c);
    }
...
static void _collection_recount_callback(gpointer instance, gpointer user_data,
...
int64_t dt_collection_get_image_position(const dt_imgid_t image_id, const int32_t tagid)
{
    ...
    gchar *image_pos_query = g_strdup(
        ...
        DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db), image_pos_query, -1, &stmt,
                                    NULL);
                                         ...
                                         const int32_t tagid)
{
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
    ...
    if (tagid)
        DT_DEBUG_SQLITE3_BIND_INT(stmt, 4, tagid);
    sqlite3_step(stmt);
    sqlite3_finalize(stmt);
}
...
void dt_collection_move_before(const dt_imgid_t image_id, GList *selected_images)
{
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
        ...
        for (const GList *selected_images_iter = selected_images; selected_images_iter != NULL;
             selected_images_iter = g_list_next(selected_images_iter))
        {
            ...
            if (tagid)
                DT_DEBUG_SQLITE3_BIND_INT(stmt, 3, tagid);
            sqlite3_step(stmt);
            sqlite3_reset(stmt);
            new_image_pos++;
        }
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
             ...
             selected_images_iter = g_list_next(selected_images_iter))
        {
            ...
            if (tagid)
                DT_DEBUG_SQLITE3_BIND_INT(update_stmt, 3, tagid);
...
void dt_collection_history_save()
{
    ...
    for (int i = 1; i < nbmax; i++)
    {
        ...
        else if (move > 0)
        {
            ...
            snprintf(confname, sizeof(confname), "plugins/lighttable/collect/history_pos%1d",
                     i - move);
