...
static void _undo_record(dt_undo_t *self, gpointer user_data, const dt_undo_type_t type,
                         const dt_undo_data_t data, const gboolean is_group,
                         void (*undo)(gpointer user_data, const dt_undo_type_t type,
                                      const dt_undo_data_t item, const dt_undo_action_t action,
                                      GList **imgs),
                         void (*free_data)(gpointer data))
{
    if (!self)
    ...
    else
    {
        ...
        dt_print(DT_DEBUG_UNDO, "[undo] record for type %d (length %d)%s", type,
                 g_list_length(self->undo_list), disable_next ? ", disable next" : "");
    }

    UNLOCK;
}

void dt_undo_start_group(dt_undo_t *self, const dt_undo_type_t type)
{
    if (!self)
        return;
...
static void _undo_do_undo_redo(dt_undo_t *self, const uint32_t filter,
                               const dt_undo_action_t action)
{
    ...
    dt_print(DT_DEBUG_UNDO, "[undo] action %s for %d (from length %d -> to length %d)",
             action == DT_ACTION_UNDO ? "UNDO" : "DO", filter, g_list_length(*from),
             g_list_length(*to));
...
static void _undo_clear_list(GList **list, const uint32_t filter)
{
    ...
    dt_print(DT_DEBUG_UNDO, "[undo] clear list for %d (length %d)", filter, g_list_length(*list));
}
...
static void _undo_iterate(GList *list, const uint32_t filter, gpointer user_data,
                          void (*apply)(gpointer user_data, const dt_undo_type_t type,
...
void dt_undo_iterate(dt_undo_t *self, const uint32_t filter, gpointer user_data,
                     void (*apply)(gpointer user_data, const dt_undo_type_t type,
                                   const dt_undo_data_t item))
{
    if (!self)
