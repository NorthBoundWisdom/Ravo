...
/** creates a new tag, returns tagid \param[in] name the tag
 * name. \param[in] tagid a pointer to tagid of new tag, this can be
 * NULL \return false if failed to create a tag and indicates that
 * tagid is invalid to use.  \note If tag already exists the existing
 * tag id is returned. */
 ...
 * NULL \return false if failed to create a tag and indicates that
 * tagid is invalid to use.  \note If tag already exists the existing
 * tag id is returned. This function will also raise a
 * DT_SIGNAL_TAG_CHANGED signal if necessary, so keywords GUI can
 * refresh. */
gboolean dt_tag_new_from_gui(const char *name, guint *tagid);

// read/import tags from a txt file as written by Lightroom. returns
// the number of imported tags or -1 if an error occurred.
ssize_t dt_tag_import(const char *filename);

// export all tags to a txt file as written by Lightroom. returns the
// number of exported tags or -1 if an error occurred.
...
/** removes a list of tags from db and from assigned images. \return
...
/** checks if tag exists. \param[in] name of tag to check. \return the
 * id of found tag or -1 i not found. */
gboolean dt_tag_exists(const char *name, guint *tagid);

/** attach a tag on images list. tagid id of tag to attach. img the
 * list of image id to attach tag to */
...
gboolean dt_tag_attach(const guint tagid, const dt_imgid_t imgid, const gboolean undo_on,
...
/** attach a list of tags on selected images. \param[in] tags a list
 * of ids of tags. \param[in] imgid the image id to attach tag to, if
 * < 0 selected images are used. \note If tag not exists it's created
 * if clear_on TRUE the image tags are cleared before attaching the
 * new ones*/
...
/** attach a list of tags on list of images. \param[in] tags a comma
 * separated string of tags. \param[in] img the list of images to
...
/** detach tag from images. \param[in] tagid of tag to
 * detach. \param[in] img the list of image id to detach tag from */
gboolean dt_tag_detach_images(const guint tagid, const GList *img, const gboolean undo_on);
/** detach tag from images. \param[in] tagid of tag to
 * detach. \param[in] imgid the image id to detach tag from, if < 0
 * images to act on are used. */
gboolean dt_tag_detach(const guint tagid, const dt_imgid_t imgid, const gboolean undo_on,
                       const gboolean group_on);

/** detach tags from images that matches name, it is valid to use % to
 * match tag */
...
uint32_t dt_tag_count_attached(const dt_imgid_t imgid, const gboolean ignore_dt_tags);
...
GList *dt_sort_tag(GList *tags, const gboolean byname);

/** get a list of tags, the difference to dt_tag_get_attached() is
 *  that this one splits at '|' and filters out the "darktable|"
 *  tags. */
GList *dt_tag_get_list(const dt_imgid_t imgid);

/** get a list of tags, the difference to dt_tag_get_list() is that
 *  this one checks option for exportation */
GList *dt_tag_get_list_export(const dt_imgid_t imgid, const int32_t flags);
 ...
 *  exportation */
GList *dt_tag_get_hierarchical_export(const dt_imgid_t imgid, const int32_t flags);
...
/** get the subset of images from the given list that have a given tag
 * attached */
GList *dt_tag_get_images_from_list(const GList *img, const gint tagid);

/** retrieves a list of suggested tags matching keyword. \param[in]
 * keyword the keyword to search \param[out] result a pointer to list
 * populated with result. \return the count \note the limit of result
 * is decided by conf value "xxx" */
uint32_t dt_tag_get_suggestions(GList **result);

/** retrieves count of tagged images. \param[in] keyword the keyword
 * to search \return the count \note the limit of result is decided by
 * conf value "xxx" */
void dt_tag_count_tags_images(const gchar *keyword, int *tag_count, int *img_count);

/** retrieves list of tags and tagged images. \param[in] keyword the
 * keyword to search. \param[out] result pointers to list populated
 * with result. \note the limit of result is decided by conf value
 * "xxx" */
void dt_tag_get_tags_images(const gchar *keyword, GList **tag_list, GList **img_list);

/** retrieves the list of tags matching keyword. \param[in] keyword
 * the keyword to search \param[out] result a pointer to list
 * populated with result. \return the count \note the limit of result
 * is decided by conf value "xxx" */
...
void dt_tag_set_synonyms(const gint tagid, gchar *synonyms);

/** retrieves flags of the tag */
gint dt_tag_get_flags(const gint tagid);

/** sets flags of the tag */
void dt_tag_set_flags(const gint tagid, const gint flags);

/** retrieves a list of recent tags used. \param[out] result a pointer
 * to list populated with result. \return the count \note the limit of
 * result is decided by conf value "xxx" */
