...
static void _import_session_cleanup_filmroll(dt_import_session_t *self)
{
    ...
    if (dt_film_is_empty(self->film->id))
    {
        ...
        if (self->current_path != NULL && g_file_test(self->current_path, G_FILE_TEST_IS_DIR) &&
            dt_util_is_dir_empty(self->current_path))
        {
...
void dt_import_session_set_exif_basic_info(dt_import_session_t *self,
...
static const char *_import_session_path(dt_import_session_t *self, const gboolean use_current_path)
{
    ...
    if (new_path && (strlen(new_path) > 1))
    {
        ...
        if (first >= 'A' && first <= 'Z' &&
            new_path[1] == ':')  // path format is <drive letter>:\path\to\file
