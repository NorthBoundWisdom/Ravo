...
#else
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "SELECT id FROM main.film_rolls WHERE folder = ?1", -1, &stmt,
                                NULL);
    DT_DEBUG_SQLITE3_BIND_TEXT(stmt, 1, folder, -1, SQLITE_STATIC);
#endif
    if (sqlite3_step(stmt) == SQLITE_ROW)
        filmroll_id = sqlite3_column_int(stmt, 0);
    sqlite3_finalize(stmt);
    return filmroll_id;
}
...
/*
gboolean dt_film_open_recent(const int num)
{
  sqlite3_stmt *stmt;
  // clang-format off
  DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
  ...
  if(sqlite3_step(stmt) == SQLITE_ROW)
  {
    const dt_filmid_t id = sqlite3_column_int(stmt, 0);
    sqlite3_finalize(stmt);
    if(dt_film_open(id)) return TRUE;
    // clang-format off
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "UPDATE main.film_rolls"
                                " SET access_timestamp = strftime('%s', 'now')"
...
dt_filmid_t dt_film_new(dt_film_t *film, const char *directory)
{
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
        ...
        if (sqlite3_step(stmt) != SQLITE_ROW)
            g_strlcpy(film->dirname, (const char *)sqlite3_column_text(stmt, 0),
                      sizeof(film->dirname));
        sqlite3_finalize(stmt);
    }
#endif
...
dt_filmid_t dt_film_import(const char *dirname)
{
    ...
    const dt_filmid_t filmid = film->id;
    ...
    if (error)
    {
        dt_print(DT_DEBUG_ALWAYS, "[film_import] failed to open directory %s: %s", film->dirname,
                 error->message);
        g_error_free(error);
        dt_film_cleanup(film);
        free(film);
...
static gboolean ask_and_delete(gpointer user_data)
{
    ...
    gtk_window_set_title(GTK_WINDOW(dialog), ngettext("delete empty directory?",
...
void dt_film_remove_empty()
{
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                    "DELETE FROM main.film_rolls WHERE id=?1", -1, &inner_stmt,
                                    NULL);
    ...
    sqlite3_finalize(stmt);
    if (raise_signal)
        DT_CONTROL_SIGNAL_RAISE(DT_SIGNAL_FILMROLLS_REMOVED);
...
void dt_film_remove(const dt_filmid_t id)
{
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "DELETE FROM main.film_rolls WHERE id = ?1", -1, &stmt, NULL);
    DT_DEBUG_SQLITE3_BIND_INT(stmt, 1, id);
    sqlite3_step(stmt);
    sqlite3_finalize(stmt);
    // dt_control_update_recent_films();
...
void dt_film_set_folder_status()
{
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db), "DELETE FROM memory.film_folder", -1,
                                &stmt, NULL);
    sqlite3_step(stmt);
    sqlite3_finalize(stmt);

    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "SELECT id, folder FROM main.film_rolls", -1, &stmt, NULL);
