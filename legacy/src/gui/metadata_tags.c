...
static gboolean _set_matching_tag_visibility(GtkTreeModel *model, GtkTreePath *path,
                                             GtkTreeIter *iter, gpointer user_idata)
{
    gboolean visible;
    gchar *tagname = NULL;
...
GtkWidget *dt_metadata_tags_dialog(GtkWidget *parent, gpointer metadata_activated_callback,
                                   gpointer user_data)
{
    GtkWidget *dialog = gtk_dialog_new_with_buttons(
        _("select tag"), GTK_WINDOW(parent), GTK_DIALOG_DESTROY_WITH_PARENT, _("_add"),
        GTK_RESPONSE_ACCEPT, _("_done"), GTK_RESPONSE_NONE, NULL);
    gtk_dialog_set_default_response(GTK_DIALOG(dialog), GTK_RESPONSE_NONE);
    gtk_window_set_default_size(GTK_WINDOW(dialog), DT_PIXEL_APPLY_DPI(500),
    ...
    gtk_widget_set_tooltip_text(
        GTK_WIDGET(sel_view),
        _("list of available tags. click 'add' button or double-click on tag to add the selected one"));
    GtkTreeSelection *selection = gtk_tree_view_get_selection(sel_view);
    gtk_tree_selection_set_mode(selection, GTK_SELECTION_SINGLE);
    g_signal_connect(selection, "changed", G_CALLBACK(_tree_selection_change), NULL);
    GtkCellRenderer *renderer = gtk_cell_renderer_text_new();
    GtkTreeViewColumn *col =
        gtk_tree_view_column_new_with_attributes(_("tag"), renderer, "text", 0, NULL);
    gtk_tree_view_append_column(sel_view, col);
    renderer = gtk_cell_renderer_text_new();
    col = gtk_tree_view_column_new_with_attributes(_("type"), renderer, "text", 1, NULL);
    gtk_tree_view_append_column(sel_view, col);
    liststore =
        gtk_list_store_new(DT_METADATA_TAGS_NUM_COLS, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_BOOLEAN);
    ...
    for (GList *tag = taglist; tag; tag = g_list_next(tag))
    {
        ...
        gtk_list_store_insert_with_values(liststore, NULL, -1, DT_METADATA_TAGS_COL_XMP, tagname,
                                          DT_METADATA_TAGS_COL_TYPE, type,
                                          DT_METADATA_TAGS_COL_VISIBLE, TRUE, -1);
    ...
    gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(liststore), DT_METADATA_TAGS_COL_XMP,
                                         GTK_SORT_ASCENDING);
    gtk_tree_view_set_model(sel_view, model);
    g_object_unref(model);
    g_signal_connect(G_OBJECT(sel_view), "row-activated", G_CALLBACK(metadata_activated_callback),
                     user_data);
