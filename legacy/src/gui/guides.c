...
static const char *_guide_names[] = {N_("grid"),
                                     N_("rules of thirds"),
                                     N_("metering"),
                                     N_("perspective"),
                                     N_("diagonal method"),
...
static void dt_guides_q_rect(dt_QRect_t *R1, const float left, const float top, const float width,
                             const float height)
{
    R1->left = left;
    R1->top = top;
...
static gchar *_conf_get_path(gchar *module_name, gchar *property_1, gchar *property_2)
{
    ...
    if (property_2)
        return g_strdup_printf("guides/%s/%s%s/%s/%s", cv->module_name, lay, module_name,
...
static dt_guides_t *_conf_get_guide(gchar *module_name)
{
    ...
    if (!dt_conf_key_exists(key))
        dt_conf_set_string(key, DEFAULT_GUIDE_NAME);
...
static void dt_guides_draw_grid(cairo_t *cr, const float x, const float y, const float w,
                                const float h, const float zoom_scale, void *data)
{
...
static GtkWidget *_guides_gui_grid(dt_iop_module_t *self, void *user_data)
{
    ...
    g_free(key);
    g_signal_connect(G_OBJECT(grid_vertical), "value-changed", G_CALLBACK(_grid_vertical_changed),
                     user_data);
    ...
    g_free(key);
    g_signal_connect(G_OBJECT(grid_subdiv), "value-changed", G_CALLBACK(_grid_subdiv_changed),
                     user_data);

    return dt_gui_vbox(grid_horizontal, grid_vertical, grid_subdiv);
}

static void dt_guides_draw_diagonal_method(cairo_t *cr, const float x, const float y, const float w,
                                           const float h)
{
    if (w > h)
    {
...
static void dt_guides_draw_harmonious_triangles(cairo_t *cr, const float left, const float top,
                                                const float width,
                                                const float height /*, const float dst*/)
{
    const int dst = (int)((height * cosf(atanf(width / height)) / (cosf(atanf(height / width)))));
...
static void dt_guides_draw_perspective(cairo_t *cr, const float x, const float y, const float w,
                                       const float h)
{
    // no need for sqrt or *0.25, this is inside a cairo_clip anyway
    const float rotation_step = 2.0 / PERSPECTIVE_LINES, line_length = w * w + h * h;
...
#define Y_LINES 33
#define CROSSES 6
static void dt_guides_draw_metering(cairo_t *cr, const float x, const float y, const float w,
                                    const float h)
{
    ...
    // small crosses
    const float length_cross = length_short * .5, cross_x_step = w / CROSSES,
                cross_y_step = h / CROSSES;
...
static void dt_guides_draw_golden_mean(cairo_t *cr, const dt_QRect_t *R1, const dt_QRect_t *R2,
                                       const dt_QRect_t *R3, const dt_QRect_t *R4,
                                       const dt_QRect_t *R5, const dt_QRect_t *R6,
                                       const dt_QRect_t *R7, const gboolean goldenSection,
                                       ...
                                       const gboolean goldenSpiral)
{
    ...
    if (goldenSpiral)
    {
        ...
        cairo_arc(cr, R1->right / R1->width * R1->height, R1->top, R1->height, M_PI_2, M_PI);
        cairo_restore(cr);

        cairo_save(cr);
        cairo_new_sub_path(cr);
        cairo_scale(cr, R2->width / R2->height, 1);
        cairo_arc(cr, R2->left / R2->width * R2->height, R2->top, R2->height, 0, M_PI_2);
        cairo_restore(cr);

        cairo_save(cr);
        cairo_new_sub_path(cr);
        cairo_scale(cr, R3->width / R3->height, 1);
        cairo_arc(cr, R3->left / R3->width * R3->height, R3->bottom, R3->height, 1.5 * M_PI,
                  2.0 * M_PI);
        cairo_restore(cr);
        ...
        cairo_arc(cr, R4->right, R4->bottom / R4->height * R4->width, R4->width, M_PI, 1.5 * M_PI);
        cairo_restore(cr);

        cairo_save(cr);
        cairo_new_sub_path(cr);
        cairo_scale(cr, 1, R5->height / R5->width);
        cairo_arc(cr, R5->right, R5->top / R5->height * R5->width, R5->width, M_PI_2, M_PI);
        cairo_restore(cr);

        cairo_save(cr);
        cairo_new_sub_path(cr);
        cairo_scale(cr, 1, R6->height / R6->width);
        cairo_arc(cr, R6->left, R6->top / R6->height * R6->width, R6->width, 0, M_PI_2);
        cairo_restore(cr);

        cairo_save(cr);
        cairo_new_sub_path(cr);
        cairo_scale(cr, R7->width / R7->height, 1);
        cairo_arc(cr, R7->left / R7->width * R7->height, R7->bottom, R7->height, 1.5 * M_PI,
                  2.0 * M_PI);
        cairo_restore(cr);
        ...
        cairo_arc(cr, R7->left / (R6->width - R7->width) * R7->height, R7->bottom, R7->height,
                  deg2rad(210.0), 1.5 * M_PI);
        cairo_restore(cr);
    }
}
...
static void _guides_draw_grid(cairo_t *cr, const float x, const float y, const float w,
                              const float h, const float zoom_scale, void *user_data)
{
    dt_guides_draw_grid(cr, x, y, w, h, zoom_scale, user_data);
}
static void _guides_draw_rules_of_thirds(cairo_t *cr, const float x, const float y, const float w,
                                         const float h, const float zoom_scale, void *user_data)
{
    dt_guides_draw_rules_of_thirds(cr, x, y, w, h);
}
static void _guides_draw_metering(cairo_t *cr, const float x, const float y, const float w,
                                  const float h, const float zoom_scale, void *user_data)
{
    dt_guides_draw_metering(cr, x, y, w, h);
}
static void _guides_draw_perspective(cairo_t *cr, const float x, const float y, const float w,
                                     const float h, const float zoom_scale, void *user_data)
{
    dt_guides_draw_perspective(cr, x, y, w, h);
}
static void _guides_draw_diagonal_method(cairo_t *cr, const float x, const float y, const float w,
                                         const float h, const float zoom_scale, void *user_data)
{
...
static void _guides_draw_harmonious_triangles(cairo_t *cr, const float x, const float y,
                                              const float w, const float h, const float zoom_scale,
                                              void *user_data)
{
    dt_guides_draw_harmonious_triangles(cr, x, y, w, h);
}
static void _guides_draw_golden_mean(cairo_t *cr, const float x, const float y, const float w,
                                     const float h, const float zoom_scale, void *user_data)
{
    // retrieve the golden extra in settings
    dt_golden_type_t extra = GOLDEN_SECTION;
    if (user_data)
        extra = GPOINTER_TO_INT(user_data);
    ...
    dt_guides_q_rect(&R6, R5.left + R5.width, R5.bottom - R5.height * INVPHI, R3.left - R5.right,
                     R5.height * INVPHI);
    dt_guides_q_rect(&R7, R6.right - R6.width * INVPHI, R4.bottom, R6.width * INVPHI,
                     R5.height - R6.height);
...
static void _guides_add_guide(GList **list, const char *name, const dt_guides_draw_callback draw,
                              const dt_guides_widget_callback widget, void *user_data,
                              const GDestroyNotify free, const gboolean support_flip)
{
...
void dt_guides_add_guide(const char *name, const dt_guides_draw_callback draw,
                         const dt_guides_widget_callback widget, void *user_data,
...
GList *dt_guides_init()
{
    ...
    _guides_add_guide(&guides, *names++, _guides_draw_perspective, NULL, NULL, NULL,
                      FALSE); // TODO: make the number of lines configurable with a slider?
    ...
    _guides_add_guide(&guides, *names++, _guides_draw_golden_mean, NULL,
...
static void _settings_update_visibility(const _guides_settings_t *gw)
{
    ...
    const dt_guides_t *guide =
        g_list_nth_data(darktable.guides, dt_bauhaus_combobox_get(darktable.view_manager->guides));
...
static void _settings_guides_changed(GtkWidget *w, const _guides_settings_t *gw)
{
...
void dt_guides_set_overlay_colors()
{
    ...
    if (overlay_color == DT_DEV_OVERLAY_GRAY)
        darktable.gui->overlay_red = darktable.gui->overlay_green = darktable.gui->overlay_blue =
            1.0f;
...
GtkWidget *dt_guides_popover(dt_view_t *self, GtkWidget *button)
{
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(gw->g_flip, self, N_("guide lines"), N_("flip"), _("flip guides"),
                                 0, _settings_flip_changed, gw, N_("none"), N_("horizontally"),
                                 N_("vertically"), N_("both"));
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(darktable.view_manager->guides_colors, self, N_("guide lines"),
                                 N_("overlay color"), _("set overlay color"),
                                 dt_conf_get_int("darkroom/ui/overlay_color"),
                                 _settings_colors_changed, gw, N_("gray"), N_("red"), N_("green"),
                                 N_("yellow"), N_("cyan"), N_("magenta"));
    ...
    g_signal_connect(G_OBJECT(contrast), "value-changed", G_CALLBACK(_settings_contrast_changed),
                     NULL);
...
void dt_guides_draw(cairo_t *cr, const float left, const float top, const float width,
                    const float height, const float zoom_scale)
{
