...
typedef enum dt_shortcut_category_t
{
    DT_SHORTCUT_CATEGORY_ACTIVE,
    DT_SHORTCUT_CATEGORY_OTHER,
    DT_SHORTCUT_CATEGORY_FALLBACKS,
    DT_SHORTCUT_CATEGORY_SPEED,
    DT_SHORTCUT_CATEGORY_DISABLED,
    DT_SHORTCUT_CATEGORY_LAST,
} dt_shortcut_category_t;

const gchar *_shortcut_category_label[] = {[DT_SHORTCUT_CATEGORY_ACTIVE] = N_("in active view"),
                                           [DT_SHORTCUT_CATEGORY_OTHER] = N_("in other views"),
                                           [DT_SHORTCUT_CATEGORY_FALLBACKS] = N_("fallbacks"),
                                           [DT_SHORTCUT_CATEGORY_SPEED] = N_("speed adjustments"),
                                           [DT_SHORTCUT_CATEGORY_DISABLED] =
                                               N_("disabled defaults")};

static gboolean _is_shortcut_category(GSequenceIter *shortcut_iter)
{
    return GPOINTER_TO_UINT(shortcut_iter) < DT_SHORTCUT_CATEGORY_LAST;
}
...
const struct _modifier_name
{
    ...
    char *name;
} modifier_string[] = {{GDK_SHIFT_MASK, N_("shift")}, {GDK_CONTROL_MASK, N_("ctrl")},
#ifdef __APPLE__
                       {GDK_MOD1_MASK, N_("option")},
#else
                       {GDK_MOD1_MASK, N_("alt")},
#endif
                       {GDK_MOD2_MASK, N_("cmd")},    {GDK_MOD5_MASK, N_("altgr")},   {0, NULL}};
...
const gchar *dt_action_effect_value[] = {N_("edit"), N_("up"),     N_("down"), N_("reset"),
                                         N_("top"),  N_("bottom"), N_("set"),  NULL};
...
const gchar *dt_action_effect_selection[] = {
    N_("popup"), N_("next"), N_("previous"), N_("reset"), N_("last"), N_("first"), NULL};

const gchar *dt_action_effect_toggle[] = {N_("toggle"),      N_("on"),      N_("off"),
                                          N_("ctrl-toggle"), N_("ctrl-on"), N_("right-toggle"),
                                          N_("right-on"),    NULL};

const gchar *dt_action_effect_hold[] = {N_("hold"), N_("on"), N_("off"), N_("toggle"), NULL};

const gchar *dt_action_effect_activate[] = {N_("activate"), N_("ctrl-activate"),
                                            N_("right-activate"), NULL};

const gchar *dt_action_effect_presets[] = {N_("show"),   N_("previous"),    N_("next"),
                                           N_("store"),  N_("delete"),      N_("edit"),
                                           N_("update"), N_("preferences"), NULL};

const gchar *dt_action_effect_preset_iop[] = {N_("apply"), N_("apply on new instance"), NULL};
...
static float _action_process_button(gpointer target, dt_action_element_t element,
                                    dt_action_effect_t effect, float move_size)
{
        ...
        gtk_widget_get_ancestor(target, GTK_TYPE_WINDOW))
    {
        ...
        if (effect != DT_ACTION_EFFECT_ACTIVATE ||
            !g_signal_handler_find(target, G_SIGNAL_MATCH_ID,
                                   g_signal_lookup("clicked", gtk_button_get_type()), 0, NULL, NULL,
                                   NULL) ||
            !gtk_widget_activate(GTK_WIDGET(target)))
        {
            GdkEvent *event = gdk_event_new(GDK_BUTTON_PRESS);
            event->button.state = effect == DT_ACTION_EFFECT_ACTIVATE_CTRL ? GDK_CONTROL_MASK : 0;
...
static float _action_process_entry(gpointer target, dt_action_element_t element,
                                   dt_action_effect_t effect, float move_size)
{
    GtkTextBuffer *buffer = GTK_IS_TEXT_VIEW(target) ? gtk_text_view_get_buffer(target) : NULL;
...
const dt_action_def_t dt_action_def_toggle = {N_("toggle"), _action_process_toggle,
                                              _action_elements_toggle, _action_fallbacks_toggle};
...
const dt_action_def_t dt_action_def_button = {N_("button"), _action_process_button,
                                              _action_elements_button, _action_fallbacks_button};

const dt_action_def_t dt_action_def_entry = {N_("entry"), _action_process_entry,
...
static const dt_shortcut_fallback_t _action_fallbacks_value[] = {
    ...
    {.effect = DT_ACTION_EFFECT_TOP,
     .button = DT_SHORTCUT_LEFT,
     .click = DT_SHORTCUT_DOUBLE,
     .move = DT_SHORTCUT_MOVE_VERTICAL},
    {}};

const dt_action_def_t dt_action_def_value = {N_("value"), NULL, _action_elements_value_fallback,
...
static const dt_action_def_t *_action_find_definition(dt_action_t *action)
{
    ...
    dt_action_type_t type =
        action->type != DT_ACTION_TYPE_FALLBACK ? action->type : GPOINTER_TO_INT(action->target);
                                              ...
                                              dt_action_effect_t ef)
{
    if (el->effects == dt_action_effect_selection && ef > DT_ACTION_EFFECT_COMBO_SEPARATOR)
    {
        ...
        {
            gchar **strings = g_hash_table_lookup(darktable.bauhaus->combo_list, ac);
            if (strings)
...
{
    return (!s->key_device && !s->key && !s->press && !s->move_device && !s->move && !s->button &&
            !s->click && !s->mods);
}

static int _shortcut_category(const dt_shortcut_t *s, dt_view_type_flags_t view)
{
    return !s || !s->views             ? DT_SHORTCUT_CATEGORY_DISABLED :
           _shortcut_is_speed(s)       ? DT_SHORTCUT_CATEGORY_SPEED :
           s->views & DT_VIEW_FALLBACK ? DT_SHORTCUT_CATEGORY_FALLBACKS :
           s->views & view             ? DT_SHORTCUT_CATEGORY_ACTIVE :
                                         DT_SHORTCUT_CATEGORY_OTHER;
}

static gint _shortcut_compare_func(gconstpointer shortcut_a, gconstpointer shortcut_b,
                                   gpointer user_data)
{
    ...
    const int a_category = _shortcut_category(a, active_view);
    const int b_category = _shortcut_category(b, active_view);

    if (a_category != b_category)
        return b_category - a_category;

    if (a_category == DT_SHORTCUT_CATEGORY_FALLBACKS && a->action && b->action &&
...
#define RETURN_DIFFERENCE(field)                                                                   \
    if (a->field != b->field)                                                                      \
    return a->field - b->field
    RETURN_DIFFERENCE(key_device);
    RETURN_DIFFERENCE(key);
    RETURN_DIFFERENCE(press);
    RETURN_DIFFERENCE(button);
    RETURN_DIFFERENCE(click);
    RETURN_DIFFERENCE(move_device);
    RETURN_DIFFERENCE(move);
    RETURN_DIFFERENCE(mods);

    if ((a->direction | b->direction) == (DT_SHORTCUT_UP | DT_SHORTCUT_DOWN) &&
        a->views == b->views)
        return a->direction - b->direction;

    if (a_category == DT_SHORTCUT_CATEGORY_DISABLED || a_category == DT_SHORTCUT_CATEGORY_SPEED)
...
#define _shortcut_lookup(s, v)                                                                     \
    g_sequence_lookup(darktable.control->shortcuts, s, _shortcut_compare_func, v)
#define _shortcut_search(s, v)                                                                     \
    g_sequence_search(darktable.control->shortcuts, s, _shortcut_compare_func, v)
                                           ...
                                           const dt_input_driver_definition_t *callbacks)
{
    ...
    for (GSList *d = darktable.control->input_drivers; d; d = d->next, id += 10)
        if (((dt_input_driver_definition_t *)d->data)->module == module)
            return id;
...
static gchar *_shortcut_key_move_name(dt_input_device_t id, guint key_or_move, guint mods,
                                      gboolean display)
{
    ...
    if (id == DT_SHORTCUT_DEVICE_KEYBOARD_MOUSE)
    {
        if (mods == DT_MOVE_NAME)
            return g_strdup(display && key_or_move != 0 ? _(move_string[key_or_move]) :
    ...
    {
        return g_strdup_printf("%s %u", display ? _("tablet button") : "tablet button",
                               key_or_move);
    }
    else
    {
        ...
        else
        {
            ...
            gchar *without_device = mods == DT_MOVE_NAME ?
            ...
            else
            {
                ...
                name = g_strdup_printf("%s%s:%s", display ? "" : callbacks->name, id_str,
                                       without_device);
    ...
    if (mods != DT_MOVE_NAME)
    {
        for (const struct _modifier_name *mod_str = modifier_string; mod_str->modifier; mod_str++)
        {
            if (mods & mod_str->modifier)
            {
                gchar *save_name = name;
                name = display ? g_strdup_printf("%s%s+", name ? name : "", _(mod_str->name)) :
...
static gboolean _shortcut_is_move(const dt_shortcut_t *s)
{
    return (s->move_device != DT_SHORTCUT_DEVICE_KEYBOARD_MOUSE ||
            s->move != DT_SHORTCUT_MOVE_NONE) &&
           !s->direction;
}

static dt_action_effect_t _shortcut_default_effect(const dt_shortcut_t *s)
{
    return _shortcut_is_move(s) ? DT_ACTION_EFFECT_DEFAULT_MOVE : DT_ACTION_EFFECT_DEFAULT_KEY;
}
...
#define add_hint(format, ...)                                                                      \
    length += length >= sizeof(hint) ?                                                             \
                  0 :                                                                              \
                  snprintf(hint + length, sizeof(hint) - length, format, ##__VA_ARGS__)
    ...
    if (s->press & DT_SHORTCUT_LONG)
        add_hint(" %s", _("long"));
    if (s->press & DT_SHORTCUT_DOUBLE)
        add_hint(" %s", _("double-press"));
    else if (s->press & DT_SHORTCUT_TRIPLE)
        add_hint(" %s", _("triple-press"));
    else if (s->press)
        add_hint(" %s", _("press"));
    if (s->button)
    {
        if (*key_name || *move_name)
            add_hint(",");
        if (s->button & DT_SHORTCUT_LEFT)
            add_hint(" %s", C_("accel", "left"));
        if (s->button & DT_SHORTCUT_RIGHT)
            add_hint(" %s", C_("accel", "right"));
        if (s->button & DT_SHORTCUT_MIDDLE)
            add_hint(" %s", C_("accel", "middle"));
        if (s->click & DT_SHORTCUT_LONG)
            add_hint(" %s", C_("accel", "long"));
        if (s->click & DT_SHORTCUT_DOUBLE)
            add_hint(" %s", C_("accel", "double-click"));
        else if (s->click & DT_SHORTCUT_TRIPLE)
            add_hint(" %s", C_("accel", "triple-click"));
        else
            add_hint(" %s", _("click"));
    }
...
static gchar *_action_description(dt_shortcut_t *s, int components)
{
    ...
    if (def && def->elements)
    {
        ...
        const gchar *cef =
            _action_find_effect_combo(s->action, &def->elements[s->element], s->effect);
        if (cef || s->effect > _shortcut_default_effect(s))
            add_hint(", %s", Q_(cef ? cef : def->elements[s->element].effects[s->effect]));
    }

    if (def && ELEMENT_IS(value, s, def->elements) && s->effect == DT_ACTION_EFFECT_SET)
        add_hint(" = %g", s->speed);
    else if (s->speed != 1.0)
        add_hint("%s%s *%g", !_shortcut_is_speed(s) || components == 2 ? ", " : "", _("speed"),
                 s->speed);
...
GHashTable *dt_shortcut_category_lists(dt_view_type_flags_t v)
{
         ...
         !g_sequence_iter_is_end(iter); iter = g_sequence_iter_next(iter))
    {
        ...
        if (s && s->views & v && !_shortcut_is_speed(s))
            _insert_shortcut_in_list(
                ht, _shortcut_description(s), s->action,
                g_strdup_printf("%s%s", s->action->label, _action_description(s, 1)));
    }
...
static gboolean _find_relative_instance(dt_action_t *action, GtkWidget *widget, int *instance)
{
    ...
    while (owner && owner->type != DT_ACTION_TYPE_IOP)
        owner = owner->owner;
    ...
    if (!widget || action->target == widget)
        return TRUE;
    ...
    if (preferred && expander != preferred->expander)
    {
        ...
        for (GList *iop_mods = darktable.develop->iop; iop_mods; iop_mods = g_list_next(iop_mods))
        {
        ...
        if (current_instance + 1 - *instance < *instance)
...
static void _tooltip_reposition(GtkWidget *widget, GdkRectangle *allocation, gpointer user_data)
{
    ...
    gdk_monitor_get_workarea(
        gdk_display_get_monitor_at_window(gdk_window_get_display(window), window), &workarea);

    const gint nx = CLAMP(wx, workarea.x, workarea.x + workarea.width - width);
                                      ...
                                      GtkTooltip *tooltip, GtkWidget *vbox)
{
    ...
    if (dt_key_modifier_state() & (GDK_BUTTON1_MASK | GDK_BUTTON2_MASK | GDK_BUTTON3_MASK |
        ...
        darktable.bauhaus->current || darktable.gui->hide_tooltips)
        return FALSE;
    ...
    if (!strcmp(widget_name, "actions_view") || !strcmp(widget_name, "shortcuts_view"))
    {
        ...
        GtkTreeModel *model;
        GtkTreeIter iter;
        if (!gtk_tree_view_get_tooltip_context(GTK_TREE_VIEW(widget), &x, &y, keyboard_mode, &model,
                                               &path, &iter))
            return FALSE;
    ...
    else
    {
        if (g_object_get_data(G_OBJECT(widget), "scroll-resize-tooltip"))
            dt_util_str_cat(&original_markup, "%s%s", original_markup ? "\n" : "",
        ...
        if (darktable.control->mapping_widget == widget)
        {
            ...
            markup_text = g_markup_printf_escaped(
                ...
                add_remove_qap == CPF_DIRECTION_UP ?
                    _("ctrl+click to add to quick access panel\n") :
                add_remove_qap == CPF_DIRECTION_DOWN ?
                    _("ctrl+click to remove from quick access panel\n") :
                    "",
                _("scroll to change default speed"), _("right-click to exit mapping mode"));
        }
    }
    ...
    {
        dt_shortcut_t *s = g_sequence_get(iter);
        if (!(s->views & current_view))
            continue;

        if (s->action == action &&
            (!def || darktable.control->element == -1 || s->element == darktable.control->element ||
             (s->element == DT_ACTION_ELEMENT_DEFAULT && has_fallbacks)))
        {
            num_shortcuts++;
            gchar *sc_escaped = g_markup_escape_text(_shortcut_description(s), -1);
            const int components =
                (show_element > 0 || s->element != darktable.control->element) ? 1 : 0;
    ...
    {
        if (original_markup)
            dt_util_str_cat(&markup_text, markup_text ? "\n\n%s" : "%s", original_markup);
        if (description)
            dt_util_str_cat(&markup_text, markup_text ? "\n\n%s" : "%s", description);
...
static void _shortcuts_store_category(GtkTreeIter *category, dt_shortcut_t *s,
                                      dt_view_type_flags_t view)
{
    gtk_tree_model_iter_nth_child(GTK_TREE_MODEL(_shortcuts_store), category, NULL,
                                  _shortcut_category(s, view));
}

static gboolean _remove_shortcut_from_store(GtkTreeModel *model, GtkTreePath *path,
...
static gboolean _insert_shortcut(dt_shortcut_t *shortcut, const gboolean confirm,
...
static void _remove_shortcut(GSequenceIter *shortcut)
{
    ...
    if (s->is_default)
    {
        _insert_shortcut(s, disabled, !disabled);
        return;
    }

    if (_shortcuts_store)
        gtk_tree_model_foreach(GTK_TREE_MODEL(_shortcuts_store), _remove_shortcut_from_store,
                               shortcut);

    if (s->direction) // was this a split move?
    {
...
static void _add_shortcut(dt_shortcut_t *shortcut, const dt_view_type_flags_t view)
{
    GSequenceIter *new_shortcut = g_sequence_insert_sorted(
        darktable.control->shortcuts, shortcut, _shortcut_compare_func, GINT_TO_POINTER(view));
    ...
    if (model)
    {
        ...
        gtk_tree_store_insert_with_values(_shortcuts_store, NULL, &category, found, 0, new_shortcut,
                                          -1);
    }

    if (shortcut->views != DT_VIEW_NONE)
    {
        ...
        if (found_disabled)
        {
            ...
            dt_shortcut_t *found_shortcut = g_sequence_get(found_disabled);
            found_shortcut->is_default = FALSE;
            _remove_shortcut(found_disabled);
        }
    }
}

static void _shortcut_row_inserted(GtkTreeModel *tree_model, GtkTreePath *path, GtkTreeIter *iter,
                                   gpointer view)
{
    // connect to original store, not filtered one, because otherwise
    // view not sufficiently updated to expand
                                 ...
                                 const gboolean disable)
{
    ...
    *s = *shortcut;
        ...
        do
        {
            ...
            if (existing) // at least one found
            {
                ...
                do
                {
                    ...
                    if (e->views == s->views && _shortcut_is_move(e) &&
                        e->effect != DT_ACTION_EFFECT_DEFAULT_MOVE && s->direction &&
                        !(confirm && remove_existing) &&
                        (!confirm || dt_gui_show_yes_no_dialog(
                                         _("shortcut for move exists with single effect"), "",
                                         _("%s\n\n(assign %s to %s)"),
                                         _("create separate shortcuts for up and down move?"),
                                         _action_description(e, 2),
                                         s->direction == DT_SHORTCUT_UP ? _("down") : _("up"))))
                    {
                    ...
                    else if (e->action == s->action)
                    {
                        if (_shortcut_is_speed(e))
                        {
                            // adjust if ui action, overwrite on import
                            if (confirm)
                                shortcut->speed = s->speed =
                                    roundf(s->speed * e->speed * 1000.) / 1000.;
                        ...
                        else if (disable && e->is_default)
                        {
                            *s = *e;
                            e->is_default = FALSE;
                            _remove_shortcut(existing);
                            s->views = DT_VIEW_NONE;
                            _add_shortcut(s, view);
                            return TRUE;
                        }
                        else if (e->element != s->element || e->effect != s->effect ||
                                 e->speed != s->speed || e->instance != s->instance)
                        {
                            if (!confirm || dt_gui_show_yes_no_dialog(
                        ...
                        else
                        {
                            ...
                            if (confirm && dt_gui_show_yes_no_dialog(
                                               _("shortcut already exists"), "",
                                               e->is_default ? _("disable this default shortcut?") :
                    ...
                    if (e->views & real_views || real_views == DT_VIEW_NONE) // overlap
                    {
                        if (remove_existing)
                        {
                            replaced_direction |= e->direction;
                            e->direction = 0;
                            _remove_shortcut(existing);
                        }
                        else
                        {
                            ...
                            existing_labels =
                                g_strdup_printf("%s\n%s", existing_labels ? existing_labels : "",
                                                _action_description(e, 2));
                            g_free(old_labels);
                        }
                    }
                    ...
                    !g_sequence_iter_is_end(existing) &&
                    !_shortcut_compare_func(s, g_sequence_get(existing), GINT_TO_POINTER(view)));
            }

            if (!real_views || real_views == view || real_views == DT_VIEW_FALLBACK)
                break;
            s->views ^= view; // look in the opposite selection (and flip
                              // back on second go-around)
        } while (s->views != real_views);

        if (existing_labels)
        {
            remove_existing =
                dt_gui_show_yes_no_dialog(_("clashing shortcuts exist"), "", "%s\n%s",
                                          _("remove these existing shortcuts?"), existing_labels);
    ...
    //keep direction if replacing one half of split move
    if (replaced_direction != s->direction)
...
const gchar *instance_label[/*NUM_INSTANCES*/] = {N_("preferred"), N_("first"), N_("last"),
                                                  N_("second"), N_("last but one")};

static void _fill_shortcut_fields(GtkTreeViewColumn *column, GtkCellRenderer *cell,
                                  GtkTreeModel *model, GtkTreeIter *iter, gpointer data)
{
    ...
    if (_is_shortcut_category(data_ptr))
    {
        if (field == SHORTCUT_VIEW_DESCRIPTION)
            field_text = g_strdup(_(_shortcut_category_label[GPOINTER_TO_INT(data_ptr)]));
    }
    else
    {
        ...
        case SHORTCUT_VIEW_EFFECT:
            ...
            {
                const gchar *cef =
                    _action_find_effect_combo(s->action, &elements[s->element], s->effect);

                if (cef || s->effect > 0 ||
                    (s->effect == 0 && s->action->type != DT_ACTION_TYPE_FALLBACK))
        ...
        case SHORTCUT_VIEW_SPEED:
            elements = _action_find_elements(s->action);
            if (s->speed != 1.0 ||
                (ELEMENT_IS(value, s, elements) &&
                 (s->effect == DT_ACTION_EFFECT_DEFAULT_MOVE ||
                  s->effect == DT_ACTION_EFFECT_DEFAULT_KEY ||
                  s->effect == DT_ACTION_EFFECT_DEFAULT_UP ||
                  s->effect == DT_ACTION_EFFECT_DEFAULT_DOWN || s->effect == DT_ACTION_EFFECT_SET ||
            ...
            for (; owner; owner = owner->owner)
            {
                if (owner->type == DT_ACTION_TYPE_IOP)
                {
                    ...
                    if (owner != &darktable.control->actions_focus &&
                        !(iop->flags() & IOP_FLAGS_ONE_INSTANCE))
                    {
                        field_text =
                            abs(s->instance) <= (NUM_INSTANCES - 1) / 2 ?
                                g_strdup(
                                    _(instance_label[abs(s->instance) * 2 - (s->instance > 0)])) :
        ...
        if (!s->views)
            editable = FALSE; //disabled default shortcuts
    }
    g_object_set(cell, "text", field_text, "editable", editable, "underline", PANGO_UNDERLINE_NONE,
                 "weight", editable && weight != PANGO_WEIGHT_LIGHT ? PANGO_WEIGHT_BOLD : weight,
                 NULL);
    g_free(field_text);
}

static void _add_prefs_column(GtkTreeView *tree, GtkCellRenderer *renderer, char *name,
                              const int position)
{
    GtkTreeViewColumn *column = gtk_tree_view_column_new_with_attributes(name, renderer, NULL);
    gtk_tree_view_column_set_cell_data_func(column, renderer, _fill_shortcut_fields,
                                            GINT_TO_POINTER(position), NULL);
...
static void _element_editing_started(GtkCellRenderer *renderer, GtkCellEditable *editable,
...
static void _element_changed(GtkCellRendererCombo *combo, char *path_string, GtkTreeIter *new_iter,
                             gpointer data)
{
    ...
    {
        s->effect = _shortcut_default_effect(s);
    }
    s->element = new_index;
...
static void _effect_editing_started(GtkCellRenderer *renderer, GtkCellEditable *editable,
                                    char *path, gpointer data)
{
    dt_shortcut_t *s = _find_edited_shortcut(data, path);
    ...
    int show_all = s->action->type != DT_ACTION_TYPE_FALLBACK;
    int bold_move =
        _shortcut_is_move(s) ? DT_ACTION_EFFECT_DEFAULT_KEY : DT_ACTION_EFFECT_DEFAULT_DOWN + 1;
    if (elements)
        for (const gchar **effect = elements[s->element].effects; *effect; effect++, bold_move++)
        {
            gtk_list_store_insert_with_values(store, NULL, -1, DT_ACTION_EFFECT_COLUMN_NAME,
                                              show_all++ ? Q_(*effect) : _("(unchanged)"),
                                              DT_ACTION_EFFECT_COLUMN_WEIGHT,
                                              bold_move > DT_ACTION_EFFECT_DEFAULT_KEY &&
    ...
    gtk_cell_layout_add_attribute(GTK_CELL_LAYOUT(combo_box), cell->data, "weight",
                                  DT_ACTION_EFFECT_COLUMN_WEIGHT);
    g_list_free(cell);

    if (ELEMENT_IS(selection, s, elements))
    {
        ...
        if (values)
        {
            ...
            gtk_list_store_insert_with_values(store, NULL, -1, DT_ACTION_EFFECT_COLUMN_SEPARATOR,
                                              TRUE, -1);
            ...
            for (; values->name; values++)
            {
                ...
                if (*text)
                    gtk_list_store_insert_with_values(store, NULL, -1, DT_ACTION_EFFECT_COLUMN_NAME,
                                                      Q_(text), DT_ACTION_EFFECT_COLUMN_WEIGHT,
                                                      PANGO_WEIGHT_NORMAL, -1);
            }
        }
        else
        {
            gchar **strings = g_hash_table_lookup(darktable.bauhaus->combo_list, s->action);
            if (strings)
            {
                // insert empty/separator row
                gtk_list_store_insert_with_values(store, NULL, -1,
                                                  DT_ACTION_EFFECT_COLUMN_SEPARATOR, TRUE, -1);

                while (*strings)
                {
                    gtk_list_store_insert_with_values(
                        store, NULL, -1, DT_ACTION_EFFECT_COLUMN_NAME, Q_(*(strings++)),
                        DT_ACTION_EFFECT_COLUMN_WEIGHT, PANGO_WEIGHT_NORMAL, -1);
                }
            }
        }
    }
...
static void _effect_changed(GtkCellRendererCombo *combo, char *path_string, GtkTreeIter *new_iter,
                            gpointer data)
{
    dt_shortcut_t *s = _find_edited_shortcut(data, path_string);
...
static void _speed_edited(GtkCellRendererText *cell, const gchar *path_string,
...
static void _instance_edited(GtkCellRendererText *cell, const gchar *path_string,
                             const gchar *new_text, gpointer data)
{
    dt_shortcut_t *s = _find_edited_shortcut(data, path_string);
...
static void _grab_in_tree_view(GtkTreeView *tree_view)
{
    g_set_weak_pointer(
        &_grab_widget,
        gtk_widget_get_parent(gtk_widget_get_parent(GTK_WIDGET(tree_view)))); // static
...
static void _shortcut_row_activated(GtkTreeView *tree_view, GtkTreePath *path,
            ...
            if (!_is_shortcut_category(shortcut_iter))
            {
                ...
                if (event->keyval == GDK_KEY_Delete || event->keyval == GDK_KEY_KP_Delete)
                {
                    if (dt_gui_show_yes_no_dialog(
                            _("removing shortcut"), "",
                            s->is_default ? s->views ? _("disable the selected default shortcut?") :
...
static void _add_shortcuts_to_tree()
{
    ...
    for (dt_shortcut_category_t i = 0; i < DT_SHORTCUT_CATEGORY_LAST; i++)
        gtk_tree_store_insert_with_values(_shortcuts_store, NULL, NULL, -1, 0, GINT_TO_POINTER(i),
                                          -1);
...
static gboolean _add_actions_to_tree(GtkTreeIter *parent, dt_action_t *action, dt_action_t *find,
                                     GtkTreeIter *found)
{
    ...
    for (; action; action = action->next)
    {
        ...
        if (action->type == DT_ACTION_TYPE_LIB)
        {
            ...
            module_is_needed =
                module->gui_reset || module->get_params || module->expandable(module);
        }
        ...
        for (; *(s + 1); s++, node = iter)
            gtk_tree_store_insert_with_values(_actions_store, &iter, &node, -1, 0, action->owner, 1,
                                              *s, -1);
        g_strfreev(prev_split);
        prev_split = split;

        gtk_tree_store_insert_with_values(_actions_store, &iter, parent ? &node : NULL, -1, 0,
                                          action, -1);

        if (action->type <= DT_ACTION_TYPE_SECTION &&
            !_add_actions_to_tree(&iter, action->target, find, found) && !module_is_needed)
...
static void _fill_action_fields(GtkTreeViewColumn *column, GtkCellRenderer *cell,
                                GtkTreeModel *model, GtkTreeIter *iter, gpointer data)
{
    dt_action_t *action = NULL;
    ...
    if (_selected_shortcut)
    {
        for (dt_action_t *ac = _selected_shortcut->action; ac; ac = ac->owner)
        {
            if (ac == action)
            {
...
static void _action_row_activated(GtkTreeView *tree_view, GtkTreePath *path,
                                  GtkTreeViewColumn *column, gpointer user_data)
{
    GtkTreeIter iter;
    gtk_tree_model_get_iter(GTK_TREE_MODEL(user_data), &iter, path);
...
static gboolean _shortcut_selection_function(GtkTreeSelection *selection, GtkTreeModel *model,
...
static void _shortcut_selection_changed(GtkTreeSelection *selection, gpointer data)
{
    ...
    if (gtk_tree_selection_get_selected(selection, &model, &iter))
    {
        void *data_ptr = NULL;
        gtk_tree_model_get(model, &iter, 0, &data_ptr, -1);
        _selected_shortcut = g_sequence_get(data_ptr);
    }
    else
        _selected_shortcut = NULL;
...
static gboolean _action_find_and_expand(GtkTreeModel *model, GtkTreeIter *iter, GtkTreeView *view)
{
    do
    {
        ...
        if (_selected_shortcut && current_action == _selected_shortcut->action)
        {
...
static gboolean _action_view_click(GtkWidget *widget, GdkEventButton *event, gpointer data)
{
    ...
    if (event->button == GDK_BUTTON_PRIMARY)
    {
        ...
        GtkTreePath *path = NULL;
        if (gtk_tree_view_get_path_at_pos(view, (gint)event->x, (gint)event->y, &path, NULL, NULL,
                                          NULL))
        {
...
static gboolean _action_view_show(GtkTreeView *view, gpointer found_iter)
{
...
static gboolean _search_func(GtkTreeModel *model, gint column, const gchar *key, GtkTreeIter *iter,
                             gpointer search_data)
{
    ...
    else
    {
        ...
        if (!_is_shortcut_category(seq_iter))
        {
    ...
    if (ac->type >= DT_ACTION_TYPE_WIDGET)
    {
        if (type == DT_ACTION_TYPE_VALUE_FALLBACK)
        {
            ...
            if (def && def->elements)
            {
                ...
                do
                {
                    if (el->effects == dt_action_effect_value)
                        return TRUE;
                    el++;
                } while (el->name);
            }
        ...
        for (ac = ac->target; ac; ac = ac->next)
            if (_fallback_type_is_relevant(ac, type))
                return TRUE;

    return FALSE;
}

static gint _sort_speed_shortcuts(GtkTreeModel *model, GtkTreeIter *a, GtkTreeIter *b,
                                  gpointer user_data)
{
...
static gboolean _visible_shortcuts(GtkTreeModel *model, GtkTreeIter *iter, gpointer data)
{
    ...
    {
        GtkTreeIter child;
        for (gboolean valid_child = gtk_tree_model_iter_children(model, &child, iter); valid_child;
             valid_child = gtk_tree_model_iter_next(model, &child))
        {
            if (_visible_shortcuts(model, &child, data))
                return TRUE;
        }
    ...
    if (!darktable.control->enable_fallbacks && s->action->type == DT_ACTION_TYPE_FALLBACK &&
        (GPOINTER_TO_INT(s->action->target) != DT_ACTION_TYPE_VALUE_FALLBACK || s->key_device ||
         s->key || s->press || s->move_device || s->move || s->button))
        return FALSE;
...
static void _shortcuts_load(const gchar *shortcuts_file, const dt_input_device_t file_dev,
                            const dt_input_device_t load_dev, const gboolean clear);
...
static void _restore_clicked(GtkButton *button, gpointer user_data)
{
    ...
    GtkWidget *dialog = gtk_dialog_new_with_buttons(
        _("restore shortcuts"), GTK_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(button))),
        GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT, _("_defaults"), _DEFAULTS, _("_startup"),
        _STARTUP, _("_edits"), _EDITS, _("_cancel"), GTK_RESPONSE_REJECT, NULL);
    ...
    gtk_widget_set_halign(label, GTK_ALIGN_START);
    GtkWidget *clear =
        gtk_check_button_new_with_label(_("clear all newer shortcuts\n"
...
static void _export_id_changed(GtkComboBox *widget, gpointer user_data)
{
    ...
    gint id = dev <= 1 ? 0 : gtk_combo_box_get_active(GTK_COMBO_BOX(widget)) + (dev - 1) * 10;
         ...
         !g_sequence_iter_is_end(iter); iter = g_sequence_iter_next(iter))
    {
        ...
        if (dev == 0 || (id == 0 && s->key_device == id && s->move_device == id) ||
...
static void _export_clicked(GtkButton *button, gpointer user_data)
{
    ...
    GtkWidget *dialog = gtk_dialog_new_with_buttons(
        _("export shortcuts"), win, GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT, _("_cancel"),
        GTK_RESPONSE_REJECT, _("_ok"), GTK_RESPONSE_OK, NULL);
    gtk_dialog_set_default_response(GTK_DIALOG(dialog), GTK_RESPONSE_REJECT);
    ...
    dt_gui_dialog_add(GTK_DIALOG(dialog), label, combo_dev,
                      dt_gui_hbox(combo_id, dt_ui_label_new(_("device id"))), count);
    gtk_widget_show_all(dialog);
    ...
    GtkFileChooserNative *chooser =
        gtk_file_chooser_native_new(_("select file to export"), GTK_WINDOW(win),
                                    GTK_FILE_CHOOSER_ACTION_SAVE, _("_export"), _("_cancel"));
...
static void _import_clicked(GtkButton *button, gpointer user_data)
{
    ...
    GtkWidget *dialog = gtk_dialog_new_with_buttons(
        _("import shortcuts"), win, GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT, _("_cancel"),
        GTK_RESPONSE_REJECT, _("_ok"), GTK_RESPONSE_OK, NULL);
    gtk_dialog_set_default_response(GTK_DIALOG(dialog), GTK_RESPONSE_REJECT);
    ...
    const gint dev = gtk_combo_box_get_active(GTK_COMBO_BOX(combo_dev));
    const gint from_id =
        dev == 0 ? DT_ALL_DEVICES :
        dev == 1 ? 0 :
                   gtk_combo_box_get_active(GTK_COMBO_BOX(combo_from_id)) + (dev - 1) * 10;
    const gint to_id =
        dev == 1 ? 0 : gtk_combo_box_get_active(GTK_COMBO_BOX(combo_to_id)) + (dev - 1) * 10;
    const gboolean wipe = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(clear));
    ...
    GtkFileChooserNative *chooser =
        gtk_file_chooser_native_new(_("select file to import"), GTK_WINDOW(win),
                                    GTK_FILE_CHOOSER_ACTION_OPEN, _("_import"), _("_cancel"));
...
static void _import_extended_clicked(GtkButton *button, gpointer user_data)
{
    GtkWindow *win = GTK_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(button)));
    GtkWidget *dialog = gtk_dialog_new_with_buttons(
        _("import sample shortcuts"), win, GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT,
        _("_cancel"), GTK_RESPONSE_REJECT, _("_ok"), GTK_RESPONSE_OK, NULL);
    gtk_dialog_set_default_response(GTK_DIALOG(dialog), GTK_RESPONSE_REJECT);
...
GtkWidget *dt_shortcuts_prefs(GtkWidget *widget)
{
    ...
    gtk_tree_sortable_set_default_sort_func(GTK_TREE_SORTABLE(_shortcuts_store),
                                            _sort_speed_shortcuts, NULL, NULL);
    gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(_shortcuts_store),
                                         GTK_TREE_SORTABLE_DEFAULT_SORT_COLUMN_ID,
                                         GTK_SORT_ASCENDING);
    GtkTreeModel *filtered_shortcuts =
        gtk_tree_model_filter_new(GTK_TREE_MODEL(_shortcuts_store), NULL);
    g_object_unref(G_OBJECT(_shortcuts_store));

    gtk_tree_model_filter_set_visible_func(GTK_TREE_MODEL_FILTER(filtered_shortcuts),
                                           _visible_shortcuts, NULL, NULL);
    ...
    gtk_widget_set_tooltip_text(
        GTK_WIDGET(search_shortcuts),
        _("incrementally search the list of shortcuts\npress up or down keys to cycle through matches"));
    g_signal_connect(G_OBJECT(search_shortcuts), "activate", G_CALLBACK(dt_gui_search_stop),
                     shortcuts_view);
    g_signal_connect(G_OBJECT(search_shortcuts), "stop-search", G_CALLBACK(dt_gui_search_stop),
                     shortcuts_view);
    ...
    g_signal_connect(G_OBJECT(shortcuts_view), "row-activated", G_CALLBACK(_shortcut_row_activated),
                     filtered_shortcuts);
    g_signal_connect(G_OBJECT(shortcuts_view), "key-press-event", G_CALLBACK(_view_key_pressed),
                     search_shortcuts);
    g_signal_connect(G_OBJECT(_shortcuts_store), "row-inserted", G_CALLBACK(_shortcut_row_inserted),
                     shortcuts_view);

    // Setting up the cell renderers
    _add_prefs_column(shortcuts_view, gtk_cell_renderer_text_new(), _("shortcut"),
                      SHORTCUT_VIEW_DESCRIPTION);

    _add_prefs_column(shortcuts_view, gtk_cell_renderer_text_new(), _("action"),
                      SHORTCUT_VIEW_ACTION);
    ...
    g_signal_connect(renderer, "editing-started", G_CALLBACK(_element_editing_started),
                     filtered_shortcuts);
    ...
    g_signal_connect(renderer, "editing-started", G_CALLBACK(_effect_editing_started),
                     filtered_shortcuts);
    ...
    g_object_set(renderer, "adjustment", gtk_adjustment_new(1, -1000, 1000, .01, 1, 10), "digits",
                 3, "xalign", 1.0, NULL);
    ...
    g_set_weak_pointer(&_actions_store,
                       gtk_tree_store_new(2, G_TYPE_POINTER, G_TYPE_STRING)); // static
    ...
    if (widget && !_selected_action)
    {
        ...
        if (gtk_widget_is_ancestor(widget, dt_ui_center_base(darktable.gui->ui)) ||
            ...
            gtk_widget_is_ancestor(
                widget, GTK_WIDGET(dt_ui_get_container(darktable.gui->ui,
    ...
    GtkTreeView *actions_view =
        GTK_TREE_VIEW(gtk_tree_view_new_with_model(GTK_TREE_MODEL(_actions_store)));
    ...
    gtk_widget_set_tooltip_text(
        GTK_WIDGET(search_actions),
        _("incrementally search the list of actions\npress up or down keys to cycle through matches"));
    g_signal_connect(G_OBJECT(search_actions), "activate", G_CALLBACK(dt_gui_search_stop),
                     actions_view);
    g_signal_connect(G_OBJECT(search_actions), "stop-search", G_CALLBACK(dt_gui_search_stop),
                     actions_view);
    ...
    g_signal_connect(G_OBJECT(actions_view), "row-activated", G_CALLBACK(_action_row_activated),
                     _actions_store);
    g_signal_connect(G_OBJECT(actions_view), "button-press-event", G_CALLBACK(_action_view_click),
                     _actions_store);
    g_signal_connect(G_OBJECT(actions_view), "key-press-event", G_CALLBACK(_view_key_pressed),
                     search_actions);
    ...
    GtkTreeViewColumn *column =
        gtk_tree_view_column_new_with_attributes(_("action"), renderer, NULL);
    gtk_tree_view_column_set_expand(column, TRUE);
    gtk_tree_view_column_set_cell_data_func(column, renderer, _fill_action_fields,
    ...
    if (split_position)
        gtk_paned_set_position(GTK_PANED(container), split_position);
    g_signal_connect(G_OBJECT(container), "notify::position", G_CALLBACK(_resize_shortcuts_view),
                     container);

    GtkWidget *btn_fallbacks = gtk_check_button_new_with_label(_("enable fallbacks"));
    gtk_widget_set_tooltip_text(
        btn_fallbacks, _("enables default meanings for additional buttons, modifiers or moves\n"
    ...
    GtkWidget *button_bar =
        dt_gui_hbox(search_actions, search_shortcuts, btn_fallbacks, dt_gui_align_right(btn_help),
                    btn_export, btn_importx, btn_import, btn_restore);
...
static void _shortcuts_save(const gchar *shortcuts_file, const dt_input_device_t device)
{
    ...
    if (f)
    {
             ...
             !g_sequence_iter_is_end(i); i = g_sequence_iter_next(i))
        {
            ...
            {
                gchar *move_name =
                    _shortcut_key_move_name(s->move_device, s->move, DT_MOVE_NAME, FALSE);
            ...
            if (elements && s->effect > _shortcut_default_effect(s))
            {
                const gchar *cef =
                    _action_find_effect_combo(s->action, &elements[s->element], s->effect);
                if (cef)
...
void dt_shortcuts_save(const gchar *ext, const gboolean backup)
{
    ...
    if (ext)
        g_strlcat(shortcuts_file, ext, PATH_MAX);
    if (backup)
    {
                                   ...
                                   gint *ef)
{
    if (effects == dt_action_effect_selection && g_strstr_len(token, 5, "item:"))
    {
        ...
        if (values)
        {
            while ((entry = (values[++effect].description ? values[effect].description :
                                                            values[effect].name)))
                if (!g_ascii_strcasecmp(token + 5, NQ_(entry)))
                    break;
        }
        else
        {
...
static void _shortcuts_load(const gchar *shortcuts_file, dt_input_device_t file_dev,
                            const dt_input_device_t load_dev, const gboolean clear)
{
    ...
    {
        if (_shortcuts_store)
            gtk_tree_store_clear(_shortcuts_store);
    ...
    if (f)
    {
        while (!feof(f))
        {
            ...
            if (read > 0)
            {
                ...
                if (!act_start)
                {
                    dt_print(DT_DEBUG_ALWAYS, "[dt_shortcuts_load] line '%s' is not an assignment",
                             line);
                    continue;
                }
                    ...
                    if (!colon)
                    {
                        ...
                        if (s.mods)
                            dt_print(DT_DEBUG_ALWAYS,
                                     "[dt_shortcuts_load] unexpected modifiers found in %s", token);
                        if (!s.key && sscanf(token, "tablet button %u", &s.key))
                            s.key_device = DT_SHORTCUT_DEVICE_TABLET;
                        if (!s.key)
                            dt_print(DT_DEBUG_ALWAYS, "[dt_shortcuts_load] no key name found in %s",
                                     token);
                    }
                    else
                    {
                ...
                while ((token = strtok(NULL, "=;")) && token < act_start)
                {
                    ...
                    if (!colon)
                    {
                        ...
                        while (modifier_string[++mod].modifier)
                            if (!g_ascii_strcasecmp(token, modifier_string[mod].name))
                                break;
                        ...
                        if (!g_ascii_strcasecmp(token, "left"))
                        {
                            s.button |= DT_SHORTCUT_LEFT;
                            continue;
                        }
                        if (!g_ascii_strcasecmp(token, "middle"))
                        {
                            s.button |= DT_SHORTCUT_MIDDLE;
                            continue;
                        }
                        if (!g_ascii_strcasecmp(token, "right"))
                        {
                            s.button |= DT_SHORTCUT_RIGHT;
                            continue;
                        }

                        if (s.button)
                        {
                            if (!g_ascii_strcasecmp(token, "double"))
                            {
                                s.click |= DT_SHORTCUT_DOUBLE;
                                continue;
                            }
                            if (!g_ascii_strcasecmp(token, "triple"))
                            {
                                s.click |= DT_SHORTCUT_TRIPLE;
                                continue;
                            }
                            if (!g_ascii_strcasecmp(token, "long"))
                            {
                                s.click |= DT_SHORTCUT_LONG;
                                continue;
                            }
                        }
                        else
                        {
                            if (!g_ascii_strcasecmp(token, "double"))
                            {
                                s.press |= DT_SHORTCUT_DOUBLE;
                                continue;
                            }
                            if (!g_ascii_strcasecmp(token, "triple"))
                            {
                                s.press |= DT_SHORTCUT_TRIPLE;
                                continue;
                            }
                            if (!g_ascii_strcasecmp(token, "long"))
                            {
                                s.press |= DT_SHORTCUT_LONG;
                                continue;
                            }
                        }
                        ...
                        while (move_string[++move])
                            if (!g_ascii_strcasecmp(token, move_string[move]))
                                break;
                        if (move_string[move])
                        {
                            s.move = move;
                        ...
                        if (!g_ascii_strcasecmp(token, "up"))
                        {
                            s.direction = DT_SHORTCUT_UP;
                            continue;
                        }
                        if (!g_ascii_strcasecmp(token, "down"))
                        {
                            s.direction = DT_SHORTCUT_DOWN;
                            continue;
                        }
                ...
                if (!s.action)
                {
                    dt_print(DT_DEBUG_ALWAYS, "[dt_shortcuts_load] action path '%s' not found",
                             token);
                    continue;
                }
                ...
                while ((token = strtok(NULL, ";")))
                {
                    if (elements)
                    {
                        ...
                        while (elements[++element].name)
                            if (!g_ascii_strcasecmp(token, NQ_(elements[element].name)))
                                break;
                        if (elements[element].name)
                        {
                            s.element = element;
                            s.effect = default_effect; // reset if an effect for a
                                                       // different element was
                                                       // found first
                            continue;
                        }
                        ...
                        if (_find_combo_effect(effects, token, s.action, &s.effect))
                            continue;

                        int effect = -1;
                        while (effects[++effect])
                            if (!g_ascii_strcasecmp(token, NQ_(effects[effect])))
                                break;
                        if (effects[effect])
                        {
                            s.effect = effect;
                    ...
                    if (!g_ascii_strcasecmp(token, "first"))
                        s.instance = 1;
                    else if (!g_ascii_strcasecmp(token, "last"))
                        s.instance = -1;
                    else if (*token == '+' || *token == '-')
                        sscanf(token, "%d", &s.instance);
                    else if (*token == '*')
                    ...
                    (file_dev != 0 && (s.key_device == file_dev || s.move_device == file_dev)))
                {
                    if (file_dev != 0)
                    {
                        if (s.key_device == file_dev)
                            s.key_device = load_dev;
                        if (s.move_device == file_dev)
                            s.move_device = load_dev;
                    }

                    _insert_shortcut(&s, FALSE, disable);
                }
            }
        }
        fclose(f);
...
gboolean dt_action_widget_invisible(GtkWidget *w)
{
    ...
    return (!GTK_IS_WIDGET(w) || !gtk_widget_get_visible(w) ||
            (!gtk_widget_get_visible(p) && strcmp(gtk_widget_get_name(p), "collapsible") &&
             !gtk_style_context_has_class(gtk_widget_get_style_context(p), "dt_plugin_ui_main")));
}

#define ADD_EXPLANATION(cause, effect, extra, ...)                                                 \
    if (*fb_log)                                                                                   \
    dt_util_str_cat(&*fb_log, "\n%s \u2192 %s" extra, cause, effect, ##__VA_ARGS__)

static gboolean _unmatched_move(dt_shortcut_t *c, dt_shortcut_t *s,
                                const dt_action_element_def_t **elements, char **fb_log)
{
    if (c->move == s->move)
        return FALSE;
    if (!darktable.control->enable_fallbacks || !c->move_device || c->move > s->move)
        return TRUE; // don't try to match mouse moves
    *elements = _action_find_elements(c->action);
    ...
    ADD_EXPLANATION(_("move not assigned"), _("fallback to earlier move"), " %d -> %d", s->move,
                    c->move);
...
static gboolean _shortcut_closest_match(GSequenceIter **current, dt_shortcut_t *s,
                                        gboolean *fully_matched,
                                        const dt_action_element_def_t **elements, char **fb_log)
{
    dt_shortcut_t *c = NULL;
    ...
    do
    {
        *current = g_sequence_iter_prev(*current);
        ...
        if (c->key_device != s->key_device || c->key != s->key ||
            c->press < (s->press & ~DT_SHORTCUT_LONG) ||
            ((c->move_device || c->move) &&
             (c->move_device != s->move_device || _unmatched_move(c, s, elements, fb_log))) ||
    ...
    } while (!g_sequence_iter_is_begin(*current) &&
             (((c->button || c->click) && (c->button != s->button || c->click != s->click)) ||
              (c->mods && c->mods != s->mods) || (c->direction & ~s->direction) ||
              (c->effect > 0 && s->effect > 0) || (c->instance && s->instance) ||
              (c->element && s->effect > 0 && *elements &&
               (*elements)[c->element].effects != (*elements)[s->element].effects)));
    ...
    if (ELEMENT_IS(value, s, *elements) && s->effect == DT_ACTION_EFFECT_SET)
    {
        s->effect = DT_ACTION_EFFECT_DEFAULT_KEY;
        s->speed = 1.0f;
    }
    ...
    *fully_matched = !(s->mods || s->press || s->button || s->click || s->move_device || s->move);
    ...
    ADD_EXPLANATION(_shortcut_description(c), _action_description(c, 2), );

    if (s->direction == DT_SHORTCUT_DOWN &&
        (s->effect == DT_ACTION_EFFECT_LAST || s->effect == DT_ACTION_EFFECT_FIRST) &&
        (ELEMENT_IS(value, s, *elements) || ELEMENT_IS(selection, s, *elements)))
    {
        s->effect =
            s->effect == DT_ACTION_EFFECT_LAST ? DT_ACTION_EFFECT_FIRST : DT_ACTION_EFFECT_LAST;
        ADD_EXPLANATION(_("down move"), _("flip top/bottom first/last"), );
    }
...
static gboolean _shortcut_match(dt_shortcut_t *f, gchar **fb_log)
{
    ...
    if (!_shortcut_closest_match(&existing, f, &matched, &elements, fb_log))
    {
        ...
        else
        {
                ...
                callbacks->key_to_move(callbacks->module, f->key_device, f->key, &move))
            {
                ...
                {
                    if (ELEMENT_IS(value, f, elements) || ELEMENT_IS(selection, f, elements))
    ...
    if (!matched && f->action && darktable.control->enable_fallbacks)
    {
        ...
        existing = _shortcut_search(f, v);
        while (_shortcut_closest_match(&existing, f, &matched, &elements, fb_log) && !matched)
        {
        };
        ...
        {
            ADD_EXPLANATION(_shortcut_description(f), _("fallback to move"), );

            if (ELEMENT_IS(value, f, elements) && f->effect == DT_ACTION_EFFECT_SET)
                f->speed = 1.0f;
...
static gpointer _action_resolve_target(dt_action_t *action, const int instance)
{
    ...
    if (owner && owner->type == DT_ACTION_TYPE_IOP)
    {
        ...
        else if (instance)
        {
            ...
            for (GList *iop_mods = instance >= 0 ? darktable.develop->iop :
                                                   g_list_last(darktable.develop->iop);
                 iop_mods;
                 iop_mods = instance >= 0 ? g_list_next(iop_mods) : g_list_previous(iop_mods))
            {
        ...
        else if (action->type == DT_ACTION_TYPE_IOP || action->type == DT_ACTION_TYPE_PRESET)
        {
                             ...
                             dt_action_effect_t effect, float move_size, gchar **fb_log)
{
    ...
    else if (action->type == DT_ACTION_TYPE_PRESET && owner && DT_PERFORM_ACTION(move_size))
    {
        ...
        else if (owner->type == DT_ACTION_TYPE_IOP)
        {
            ...
            dt_print(DT_DEBUG_ALWAYS, "[process_action] preset '%s' has unsupported type",
                     action->label);
    }
    else
    {
                                ...
                                !dt_action_widget_invisible(action_target)))))
        {
                 ...
                 effect != DT_ACTION_EFFECT_SET))
            {
                ...
                if (speed_adjustment)
                {
                    ...
                    ADD_EXPLANATION(_action_description(f, 2), _("speed"), " = %g", move_size);
                }
            }
...
static float _process_shortcut(float move_size)
{
    ...
    dt_print(DT_DEBUG_INPUT | DT_DEBUG_VERBOSE, "  [_process_shortcut] processing shortcut: %s",
    ...
    if (_shortcut_match(&fsc, &fb_log))
    {
        ...
        return_value =
            _process_action(fsc.action, fsc.instance, fsc.element, fsc.effect, move_size, &fb_log);
    }
...
float dt_action_process(const gchar *action, int instance, const gchar *element,
                        const gchar *effect, float move_size)
{
    ...
    {
        if (DT_PERFORM_ACTION(move_size))
            dt_print(DT_DEBUG_ALWAYS, "[dt_action_process] action '%s' not valid for current view",
                     action);
        return DT_ACTION_NOT_VALID;
    }
    ...
    if ((element && *element) || (effect && *effect))
    {
        ...
        if (elements)
        {
            ...
            if (element && *element)
            {
                while (elements[el].name && g_ascii_strcasecmp(elements[el].name, element))
                    el++;

                if (!elements[el].name)
                {
                    dt_print(DT_DEBUG_ALWAYS,
                             "[dt_action_process] element '%s' not valid for action '%s'", element,
                             action);
                    return DT_ACTION_NOT_VALID;
                }
            }
            ...
            {
                while (effects[ef] && g_ascii_strcasecmp(effects[ef], effect))
                    ef++;

                if (!effects[ef])
                {
                    dt_print(DT_DEBUG_ALWAYS,
                             "[dt_action_process] effect '%s' not valid for action '%s'", effect,
                             action);
                    return DT_ACTION_NOT_VALID;
                }
            }
...
float dt_shortcut_move(dt_input_device_t id, guint time, guint move, float move_size)
{
    ...
    {
        gboolean key_or_button_released =
            (id == DT_SHORTCUT_DEVICE_KEYBOARD_MOUSE && move == DT_SHORTCUT_MOVE_NONE);
        _previous_move = move;
        ...
        if (_sc.action)
        {
            if (!time || time > _last_mapping_time + 1000 || time < _last_mapping_time)
            {
                ...
                dt_shortcut_t s = _sc;
                if (!_action_find_elements(s.action))
                    s.effect = DT_ACTION_EFFECT_DEFAULT_KEY;
                if (_insert_shortcut(&s, darktable.control->confirm_mapping, FALSE))
                {
                    dt_control_log(_("%s assigned to %s"), _shortcut_description(&s),
                                   _action_description(&s, 2));
...
void dt_shortcut_key_press(const dt_input_device_t id, const guint time, const guint key)
{
    dt_device_key_t this_key = {id, key};

    if (g_slist_find_custom(_pressed_keys, &this_key, _cmp_key))
    {
        // if key is still repeating (after return from popup menu) then
        // restore double/triple press state
        if (id == DT_SHORTCUT_DEVICE_KEYBOARD_MOUSE && key == _focus_loss_key &&
            time < _last_time + 50)
    ...
    else
    {
        ...
        {
            just_key.mods =
                0; // fall back to key without modifiers (for multiple emulated modifiers)
            existing = _shortcut_lookup(&just_key, GINT_TO_POINTER(just_key.views));
            if (existing && (s = g_sequence_get(existing)) &&
                (s->action != darktable.control->actions_modifiers ||
                 s->effect != DT_ACTION_EFFECT_HOLD))
                s = NULL;
        }
        if (s && !_sc.action && s->effect == DT_ACTION_EFFECT_HOLD && s->action &&
            ...
            !dt_action_widget(darktable.control->mapping_widget))
        {
            ...
            if (definition && ELEMENT_IS(hold, s, definition->elements) && definition->process)
            {
        ...
        if ((id || key) && id == _sc.key_device && key == _sc.key && double_press &&
        ...
        if (!_pressed_keys)
        {
            ...
            gdk_seat_grab(gdk_display_get_default_seat(gdk_display_get_default()),
                          gtk_widget_get_window(
                              _grab_window ? _grab_window : dt_ui_main_window(darktable.gui->ui)),
...
static void _delay_for_double_triple(guint time, guint is_key)
{
    ...
    {
        // detect if any double or triple press shortcuts exist for this
        // key; otherwise skip delay
        ...
        {
            if (g_sequence_iter_is_end(multi))
                continue;
    ...
    GSourceFunc delay_func = is_key ? _key_release_delayed : _button_release_delayed;
...
void dt_shortcut_key_release(const dt_input_device_t id, const guint time, const guint key)
{
    dt_device_key_t this_key = {id, key};
...
gboolean dt_shortcut_key_active(dt_input_device_t id, guint key)
{
    ...
    const float value = dt_shortcut_move(DT_SHORTCUT_DEVICE_KEYBOARD_MOUSE, 0,
                                         DT_SHORTCUT_MOVE_NONE, DT_READ_ACTION_ONLY);
...
static guint _fix_keyval(GdkEvent *event)
{
    guint keyval = 0;
    GdkKeymap *keymap = gdk_keymap_get_for_display(gdk_display_get_default());
    gdk_keymap_translate_keyboard_state(keymap, event->key.hardware_keycode, 0, 0, &keyval, NULL,
                                        NULL, NULL);
    return keyval;
}
...
gboolean dt_shortcut_dispatcher(GtkWidget *w, GdkEvent *event, gpointer user_data)
{
        ...
        event->button.button > 7)
    {
        if (event->type == GDK_BUTTON_RELEASE)
            dt_shortcut_key_release(DT_SHORTCUT_DEVICE_TABLET, event->button.time,
                                    event->button.button - 7);
        else
            dt_shortcut_key_press(DT_SHORTCUT_DEVICE_TABLET, event->button.time,
                                  event->button.button - 7);
    ...
    if (_pressed_keys == NULL)
    {
        ...
        gboolean middle_click =
            event->type == GDK_BUTTON_PRESS && event->button.button == GDK_BUTTON_MIDDLE;
        ...
        if (GTK_IS_WINDOW(w) && (event->type == GDK_KEY_PRESS || event->type == GDK_KEY_RELEASE))
        {
            ...
            if (focused_widget)
            {
                ...
                if ((GTK_IS_ENTRY(focused_widget) || GTK_IS_TREE_VIEW(focused_widget)) &&
                    (event->key.keyval == GDK_KEY_Tab || event->key.keyval == GDK_KEY_KP_Tab ||
    ...
    case GDK_KEY_PRESS:
        if (event->key.is_modifier
            ...
            || event->key.keyval == GDK_KEY_VoidSymbol || event->key.keyval == GDK_KEY_Meta_L ||
            event->key.keyval == GDK_KEY_Meta_R || event->key.keyval == GDK_KEY_ISO_Level3_Shift)
            return FALSE;

        dt_shortcut_t ko = {
            .key = _fix_keyval(event) - 1, .press = 0x7, .views = dt_view_get_current()};
    ...
    case GDK_KEY_RELEASE:
        ...
        dt_shortcut_key_release(DT_SHORTCUT_DEVICE_KEYBOARD_MOUSE, event->key.time,
                                _fix_keyval(event));
        break;
...
void dt_action_insert_sorted(dt_action_t *owner, dt_action_t *new_action)
{
    ...
    while (*insertion_point && g_ascii_strcasecmp(new_action->id, "preset") &&
...
dt_action_t *dt_action_locate(dt_action_t *owner, gchar **path, gboolean create)
{
    ...
    while (*path)
    {
        ...
        if (!action)
        {
            ...
            new_action->label = style_or_preset ? dt_util_localize_segmented_name(*path, TRUE) :
                                                  g_strdup(Q_(*path));
    ...
    if (owner)
    {
        ...
        {
            dt_print(DT_DEBUG_ALWAYS, "[dt_action_locate] found action '%s' internal node",
                     owner->id);
            return NULL;
        }
    }
                                  ...
                                  GtkWidget *widget, const dt_action_def_t *action_def)
{
    ...
    if (section && g_str_has_prefix(section, "blend"))
    {
        ...
        ac = dt_action_define(&darktable.control->actions_blend, subsection, label, widget,
                              action_def);
    }
    else
    {
    ...
    if (DT_IS_BAUHAUS_WIDGET(widget) && dt_bauhaus_widget_get_field(widget))
    {
...
void dt_action_define_fallback(dt_action_type_t type, const dt_action_def_t *action_def)
{
    ...
    if (f)
    {
        ...
        dt_action_t *fb =
            dt_action_locate(&darktable.control->actions_fallbacks, (gchar **)fallback_path, TRUE);
                          ...
                          GdkModifierType mods)
{
    if (accel_key != 0 && !darktable.control->accel_initialised)
    {
        ...
        if (!gdk_keymap_get_entries_for_keyval(keymap, accel_key, &keys, &n_keys))
            return;

        for (int j = 0; j < n_keys; j++)
        {
            gdk_keymap_translate_keyboard_state(keymap, keys[j].keycode, 0, 0, &keys[j].keycode,
                                                NULL, NULL, NULL);

            if (_is_kp_key(keys[j].keycode))
                keys[j].group = 10;

            if (keys[j].group < keys[i].group ||
                (keys[j].group == keys[i].group && keys[j].level < keys[i].level))
                i = j;
        }
...
void dt_action_rename_preset(dt_action_t *action, const gchar *old_name, const gchar *new_name)
{
    ...
    if (p)
    {
        ...
        {
            if (_actions_store)
                gtk_tree_model_foreach(GTK_TREE_MODEL(_actions_store), _remove_shortcut_from_store,
                                       p);
        }
...
float dt_accel_get_speed_multiplier(GtkWidget *widget, guint state)
{
    ...
    float multiplier = dt_conf_get_float(
        slider_precision == DT_IOP_PRECISION_FINE   ? "darkroom/ui/scale_precise_step_multiplier" :
...
// FIXME possibly just find correct widget for each shortcut
// execution, rather than updating for each focus change etc
