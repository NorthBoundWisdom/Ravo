...
static void dpi_scaling_changed_callback(GtkWidget *widget, gpointer user_data)
{
    ...
    if (dpi > 0.0)
        dpi = fmax(64, dpi); // else <= 0 -> use system default
...
static void save_usercss(GtkTextBuffer *buffer)
{
    ...
    {
        dt_print(DT_DEBUG_ALWAYS, "%s: error saving css to %s: %s", G_STRFUNC, usercsspath,
                 error->message);
        g_clear_error(&error);
    }
...
static gboolean _remove_panel_config(gpointer key, gpointer value, gpointer user_data)
{
    return (!strcmp(key, "ui/hide_tooltips") ||
            ...
            (strstr(key, "/ui/") && !g_str_has_suffix(key, "border_size") &&
             (g_str_has_suffix(key, "_visible") || g_str_has_suffix(key, "_size") ||
...
static void init_tab_general(GtkWidget *dialog, GtkWidget *stack, dt_gui_themetweak_widgets_t *tw)
{
    ...
    const int i = dt_conf_get_bool("font_prefs_align_right") ?
    ...
    g_signal_connect(G_OBJECT(fontsize), "value_changed", G_CALLBACK(font_size_changed_callback),
                     0);
    ...
    gtk_widget_set_tooltip_text(
        ...
        _("adjust the global GUI resolution to rescale controls, buttons, labels, etc.\n"
    ...
    gtk_widget_set_tooltip_text(panel_reset, _("reset hidden panels,"
    ...
    gtk_widget_set_tooltip_text(tw->save_button, _("click to save and apply the CSS tweaks"
...
void dt_gui_preferences_show()
{
    ...
    if (strcmp(current_view, _("darkroom")) == 0 || strcmp(current_view, _("lighttable")) == 0)
    {
        gtk_stack_set_visible_child(GTK_STACK(stack),
                                    gtk_stack_get_child_by_name(GTK_STACK(stack), current_view));
    }
...
static void _create_lock_check_pixbuf(GdkPixbuf **lock_pixbuf, GdkPixbuf **check_pixbuf)
{
    ...
    // lock
    cairo_surface_t *lock_cst = cairo_image_surface_create(
        CAIRO_FORMAT_ARGB32, DT_PIXEL_APPLY_DPI(ICON_SIZE), DT_PIXEL_APPLY_DPI(ICON_SIZE));
    ...
    *lock_pixbuf = gdk_pixbuf_new_from_data(
        data, GDK_COLORSPACE_RGB, TRUE, 8, DT_PIXEL_APPLY_DPI(ICON_SIZE),
        DT_PIXEL_APPLY_DPI(ICON_SIZE), cairo_image_surface_get_stride(lock_cst),
        cairo_destroy_from_pixbuf, lock_cr);
    cairo_surface_destroy(lock_cst);

    // check mark
    cairo_surface_t *check_cst = cairo_image_surface_create(
        CAIRO_FORMAT_ARGB32, DT_PIXEL_APPLY_DPI(ICON_SIZE), DT_PIXEL_APPLY_DPI(ICON_SIZE));
    cairo_t *check_cr = cairo_create(check_cst);
    cairo_set_source_rgb(check_cr, 0.7, 0.7, 0.7);
    dtgtk_cairo_paint_check_mark(check_cr, 0, 0, DT_PIXEL_APPLY_DPI(ICON_SIZE),
                                 DT_PIXEL_APPLY_DPI(ICON_SIZE), 0, NULL);
    cairo_surface_flush(check_cst);
    data = cairo_image_surface_get_data(check_cst);
    dt_draw_cairo_to_gdk_pixbuf(data, DT_PIXEL_APPLY_DPI(ICON_SIZE), DT_PIXEL_APPLY_DPI(ICON_SIZE));
    *check_pixbuf = gdk_pixbuf_new_from_data(
        data, GDK_COLORSPACE_RGB, TRUE, 8, DT_PIXEL_APPLY_DPI(ICON_SIZE),
        DT_PIXEL_APPLY_DPI(ICON_SIZE), cairo_image_surface_get_stride(check_cst),
                                ...
                                GdkPixbuf *lock_pixbuf, GdkPixbuf *check_pixbuf)
{
    ...
    gchar *iso = NULL, *exposure = NULL, *aperture = NULL, *focal_length = NULL, *smaker = NULL,
          *smodel = NULL, *slens = NULL;
    int min, max;

    gchar *module = g_strdup(dt_iop_get_localized_name(operation));
    if (module == NULL)
        module = g_strdup(dt_lib_get_localized_name(operation));
    if (module == NULL)
        module = g_strdup(operation);
    ...
    else
    {
        ...
        for (min = 0; min < dt_gui_presets_exposure_value_cnt &&
                      exposure_min > dt_gui_presets_exposure_value[min];
             min++)
            ;
        for (max = 0; max < dt_gui_presets_exposure_value_cnt &&
                      exposure_max > dt_gui_presets_exposure_value[max];
             max++)
            ;
        ...
        for (min = 0; min < dt_gui_presets_aperture_value_cnt &&
                      aperture_min > dt_gui_presets_aperture_value[min];
             min++)
            ;
        for (max = 0; max < dt_gui_presets_aperture_value_cnt &&
                      aperture_max > dt_gui_presets_aperture_value[max];
             max++)
            ;
    ...
    gtk_tree_store_set(GTK_TREE_STORE(tree_store), iter, P_ROWID_COLUMN, rowid, P_OPERATION_COLUMN,
                       operation, P_MODULE_COLUMN, "", P_EDITABLE_COLUMN,
                       writeprotect ? lock_pixbuf : NULL, P_NAME_COLUMN, local_name, P_MODEL_COLUMN,
                       smodel, P_MAKER_COLUMN, smaker, P_LENS_COLUMN, slens, P_ISO_COLUMN, iso,
                       P_EXPOSURE_COLUMN, exposure, P_APERTURE_COLUMN, aperture,
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        if (g_strcmp0(operation, last_module))
        {
            ...
            gtk_tree_store_insert_with_values(tree_store, &parent, NULL, -1, P_MODULE_COLUMN,
                                              module, -1);
...
static gboolean _search_func(GtkTreeModel *model, const gint column, const gchar *key,
...
static void init_tab_presets(GtkWidget *stack)
{
    ...
    GtkTreeStore *model = gtk_tree_store_new(
        ...
        GDK_TYPE_PIXBUF /*editable*/, G_TYPE_STRING /*name*/, G_TYPE_STRING /*model*/,
        G_TYPE_STRING /*maker*/, G_TYPE_STRING /*lens*/, G_TYPE_STRING /*iso*/,
        G_TYPE_STRING /*exposure*/, G_TYPE_STRING /*aperture*/, G_TYPE_STRING /*focal length*/,
    ...
    gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(model), P_MODULE_COLUMN,
                                         GTK_SORT_ASCENDING);
    gtk_tree_sortable_set_sort_func(GTK_TREE_SORTABLE(model), P_MODULE_COLUMN, compare_rows_presets,
                                    NULL, NULL);

    // Setting up the cell renderers
    renderer = gtk_cell_renderer_text_new();
    column = gtk_tree_view_column_new_with_attributes(_("module"), renderer, "text",
                                                      P_MODULE_COLUMN, NULL);
    gtk_tree_view_append_column(tree, column);

    renderer = gtk_cell_renderer_pixbuf_new();
    column =
        gtk_tree_view_column_new_with_attributes("", renderer, "pixbuf", P_EDITABLE_COLUMN, NULL);
    gtk_tree_view_append_column(tree, column);

    renderer = gtk_cell_renderer_text_new();
    column =
        gtk_tree_view_column_new_with_attributes(_("name"), renderer, "text", P_NAME_COLUMN, NULL);
    gtk_tree_view_append_column(tree, column);

    renderer = gtk_cell_renderer_text_new();
    column = gtk_tree_view_column_new_with_attributes(_("model"), renderer, "text", P_MODEL_COLUMN,
                                                      NULL);
    gtk_tree_view_append_column(tree, column);

    renderer = gtk_cell_renderer_text_new();
    column = gtk_tree_view_column_new_with_attributes(_("maker"), renderer, "text", P_MAKER_COLUMN,
                                                      NULL);
    gtk_tree_view_append_column(tree, column);

    renderer = gtk_cell_renderer_text_new();
    column =
        gtk_tree_view_column_new_with_attributes(_("lens"), renderer, "text", P_LENS_COLUMN, NULL);
    gtk_tree_view_append_column(tree, column);

    renderer = gtk_cell_renderer_text_new();
    column =
        gtk_tree_view_column_new_with_attributes(_("ISO"), renderer, "text", P_ISO_COLUMN, NULL);
    gtk_tree_view_append_column(tree, column);

    renderer = gtk_cell_renderer_text_new();
    column = gtk_tree_view_column_new_with_attributes(_("exposure"), renderer, "text",
                                                      P_EXPOSURE_COLUMN, NULL);
    gtk_tree_view_append_column(tree, column);

    renderer = gtk_cell_renderer_text_new();
    column = gtk_tree_view_column_new_with_attributes(_("aperture"), renderer, "text",
    ...
    renderer = gtk_cell_renderer_pixbuf_new();
    column = gtk_tree_view_column_new_with_attributes(_("auto"), renderer, "pixbuf",
...
static void init_tab_accels(GtkWidget *stack)
{
    if (dt_conf_get_bool("accel/show_tab_in_prefs"))
        gtk_stack_add_titled(GTK_STACK(stack), dt_shortcuts_prefs(NULL), _("shortcuts"),
                             _("shortcuts"));
}
                                       ...
                                       GtkTreeViewColumn *column, gpointer data)
{
    ...
    else
    {
        ...
        gchar *name;
        GdkPixbuf *editable;
        gtk_tree_model_get(model, &edited_iter, P_ROWID_COLUMN, &rowid, P_NAME_COLUMN, &name,
                           P_EDITABLE_COLUMN, &editable, -1);
        if (editable == NULL)
        {
            dt_gui_presets_show_edit_dialog(name, rowid, G_CALLBACK(edit_preset_response), model,
                                            TRUE, TRUE, TRUE, GTK_WINDOW(_preferences_dialog));
        }
        else
        {
...
static void import_preset(GtkButton *button, gpointer data)
{
    ...
    GtkFileChooserNative *chooser =
        gtk_file_chooser_native_new(_("select preset(s) to import"), GTK_WINDOW(win),
...
static void export_preset(GtkButton *button, gpointer data)
{
    ...
    GtkFileChooserNative *filechooser = gtk_file_chooser_native_new(
        _("select directory"), GTK_WINDOW(win), GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER, _("_save"),
        _("_cancel"));
    ...
    if (gtk_native_dialog_run(GTK_NATIVE_DIALOG(filechooser)) == GTK_RESPONSE_ACCEPT)
    {
        ...
        // transaction for whole process saves us microlocks
        dt_database_start_transaction(darktable.db);

        DT_DEBUG_SQLITE3_PREPARE_V2(
            dt_database_get(darktable.db),
            "SELECT rowid, name, operation FROM data.presets WHERE writeprotect = 0", -1, &stmt,
            NULL);
...
GtkWidget *dt_gui_preferences_enum(dt_action_t *action, const char *key)
{
    ...
    dt_bauhaus_combobox_alignment_t align =
        action ? DT_BAUHAUS_COMBOBOX_ALIGN_RIGHT : DT_BAUHAUS_COMBOBOX_ALIGN_LEFT;
    ...
    while (values && *values++ == '[' && *values)
    {
        ...
        dt_bauhaus_combobox_add_full(w, g_dpgettext2(NULL, "preferences", item), align,
                                     (gpointer)values, NULL, TRUE);

        if (!g_strcmp0(defstr, item))
            dt_bauhaus_combobox_set_default(w, GPOINTER_TO_INT(values));
        if (!g_strcmp0(str, item))
    ...
    g_signal_connect(G_OBJECT(w), "value-changed", G_CALLBACK(_gui_preferences_enum_callback),
                     (gpointer)key);
    return w;
}
