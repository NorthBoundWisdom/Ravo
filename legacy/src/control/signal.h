...
guint dt_control_signal_disconnect_all(const struct dt_control_signal_t *ctlsig,
                                       gpointer user_data);
/* blocks a callback */
...
#define DT_CONTROL_SIGNAL_RAISE(signal, ...)                                                       \
    do                                                                                             \
    {                                                                                              \
        if ((darktable.unmuted_signal_dbg_acts & DT_DEBUG_SIGNAL_ACT_RAISE) &&                     \
            darktable.unmuted_signal_dbg[signal])                                                  \
        {                                                                                          \
            dt_print(DT_DEBUG_SIGNAL, "[signal] raise %s; %s:%d, function %s()", #signal,          \
...
#define DT_CONTROL_SIGNAL_CONNECT(signal, cb, user_data)                                           \
    do                                                                                             \
    {                                                                                              \
        if ((darktable.unmuted_signal_dbg_acts & DT_DEBUG_SIGNAL_ACT_CONNECT) &&                   \
            darktable.unmuted_signal_dbg[signal])                                                  \
        {                                                                                          \
            dt_print(DT_DEBUG_SIGNAL, "[signal] connect    %s to %s; %s:%d, function: %s()", #cb,  \
...
#define DT_CONTROL_SIGNAL_DISCONNECT(cb, user_data)                                                \
    ...
    {                                                                                              \
        ...
        {                                                                                          \
            dt_print(DT_DEBUG_SIGNAL, "[signal] disconnect %s; %s:%d, function: %s()", #cb,        \
                     __FILE__, __LINE__, __FUNCTION__);                                            \
        }                                                                                          \
    ...
    {                                                                                              \
        ...
        {                                                                                          \
            dt_print(DT_DEBUG_SIGNAL,                                                              \
                     "[signal] disconnect %d signals for %s; %s:%d, function: %s()", num, name,    \
                     __FILE__, __LINE__, __FUNCTION__);                                            \
        }                                                                                          \
    } while (0)
