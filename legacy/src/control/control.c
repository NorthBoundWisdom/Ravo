...
static float _action_process_accels_show(const gpointer target, const dt_action_element_t element,
...
const dt_action_def_t dt_action_def_accels_show = {N_("hold"), _action_process_accels_show,
                                                   dt_action_elements_hold, NULL, TRUE};

GdkModifierType dt_modifier_shortcuts;

static float _action_process_modifiers(const gpointer target, const dt_action_element_t element,
                                       const dt_action_effect_t effect, const float move_size)
{
    GdkModifierType mask = 1;
    if (element)
        mask <<= element + 1; // ctrl = 4, alt = 8
    if (DT_PERFORM_ACTION(move_size))
    {
...
const dt_action_element_def_t _action_elements_modifiers[] = {{"shift", dt_action_effect_hold},
                                                              {"ctrl", dt_action_effect_hold},
                                                              {"alt", dt_action_effect_hold},
                                                              {NULL}};

const dt_action_def_t dt_action_def_modifiers = {N_("modifiers"), _action_process_modifiers,
                                                 _action_elements_modifiers, NULL, TRUE};
...
void dt_control_init(const gboolean withgui)
{
    ...
    s->actions_global =
        (dt_action_t){DT_ACTION_TYPE_GLOBAL, "global",      C_("accel", "global"),
                      .target = NULL,        .owner = NULL, .next = &s->actions_views};

    s->actions_views =
        (dt_action_t){DT_ACTION_TYPE_CATEGORY,     "views",       C_("accel", "views"),
                      .target = &s->actions_thumb, .owner = NULL, .next = &s->actions_libs};

    s->actions_thumb = (dt_action_t){DT_ACTION_TYPE_CATEGORY,    "thumbtable",
                                     C_("accel", "thumbtable"),  .target = NULL,
                                     .owner = &s->actions_views, .next = NULL};

    s->actions_libs =
        (dt_action_t){DT_ACTION_TYPE_CATEGORY, "lib", C_("accel", "utility modules"), NULL, NULL,
                      .next = &s->actions_iops};
    ...
    s->actions_focus =
        (dt_action_t){DT_ACTION_TYPE_IOP, "focus", C_("accel", "<focused>"), NULL, NULL, NULL};
    ...
    dt_action_t *ac = dt_action_define(&s->actions_global, NULL, N_("show accels window"), NULL,
...
void _change_cursor_with_fallback(const char *cursor_name, gboolean is_temp)
{
    ...
    if (!cursor)
    {
        ...
        if (!strcmp(cursor_name, "none"))
            type = GDK_BLANK_CURSOR;
        else if (!strcmp(cursor_name, "wait"))
            type = GDK_WATCH;
        else if (!strcmp(cursor_name, "grab"))
            type = GDK_HAND1;
        else if (!strcmp(cursor_name, "cell"))
            type = GDK_PLUS;
        else if (!strcmp(cursor_name, "help"))
            type = GDK_QUESTION_ARROW;
        else if (!strcmp(cursor_name, "ns-resize"))
...
gboolean dt_control_running()
{
    return darktable.control &&
           dt_atomic_get_int(&darktable.control->running) == DT_CONTROL_STATE_RUNNING;
}
...
void dt_control_shutdown()
{
    ...
    const gboolean cleanup =
        dt_atomic_exch_int(&s->running, DT_CONTROL_STATE_DISABLED) == DT_CONTROL_STATE_CLEANUP;
    ...
    for (int k = 0; k < s->num_threads; k++)
    {
        ...
        dt_print(DT_DEBUG_CONTROL, "[dt_control_shutdown] joined num_thread %i%s", k,
                 err ? ", error" : "");
    }
    ...
    for (int k = 0; k < DT_CTL_WORKER_RESERVED; k++)
    {
        ...
        dt_print(DT_DEBUG_CONTROL, "[dt_control_shutdown] joined worker %i%s", k,
                 err ? ", error" : "");
    }
}
...
void dt_control_cleanup(const gboolean withgui)
{
    ...
    if (withgui)
    {
        ...
        if (s->shortcuts)
            g_sequence_free(s->shortcuts);
        if (s->input_drivers)
            g_slist_free_full(s->input_drivers, g_free);
    }
...
{
    const dt_view_t *cv = dt_view_manager_get_current_view(darktable.view_manager);
    const char *mode = (cv && !strcmp(cv->module_name, "lighttable")) ? "darkroom" : "lighttable";
...
void dt_control_log(const char *msg, ...)
{
    ...
    {
        g_strlcpy(dc->log_message[dc->log_pos & (DT_CTL_LOG_SIZE - 1)], escaped_msg,
                  DT_CTL_LOG_MSG_SIZE);
        dc->log_pos++;
    }
    ...
    dc->log_message_timeout_id = g_timeout_add(DT_CTL_LOG_TIMEOUT + 1000 * (msglen / 40),
...
static void _toast_log(const gboolean markup, const char *msg, va_list ap)
{
    ...
    if (markup)
        vsnprintf(dc->toast_message[dc->toast_pos & (DT_CTL_TOAST_SIZE - 1)], DT_CTL_TOAST_MSG_SIZE,
                  msg, ap);
    else
    {
        char *escaped_msg = g_markup_vprintf_escaped(msg, ap);
        g_strlcpy(dc->toast_message[dc->toast_pos & (DT_CTL_TOAST_SIZE - 1)], escaped_msg,
