...
static void _set_modification_time(char *filename, const time_t timestamp)
{
    ...
    GFileInfo *info = g_file_query_info(
        gfile, G_FILE_ATTRIBUTE_TIME_MODIFIED "," G_FILE_ATTRIBUTE_TIME_MODIFIED_USEC,
    ...
    if (info)
    {
        g_file_info_set_attribute_uint64(info, G_FILE_ATTRIBUTE_TIME_MODIFIED, timestamp);
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        if (look_for_xmp)
        {
            ...
            if (timestamp + MAX_TIME_SKEW < statbuf.st_mtime)
            {
                ...
                dt_print(DT_DEBUG_CONTROL, "[crawler] `%s' (id: %d) is a newer XMP file", xmp_path,
                         id);
            }
...
static void dt_control_crawler_response_callback(GtkWidget *dialog, const gint response_id,
                                                 gpointer user_data)
{
...
{
    gtk_tree_model_get(model, iter, DT_CONTROL_CRAWLER_COL_IMAGE_PATH, &entry->image_path,
                       DT_CONTROL_CRAWLER_COL_ID, &entry->id, DT_CONTROL_CRAWLER_COL_XMP_PATH,
                       &entry->xmp_path, DT_CONTROL_CRAWLER_COL_TS_DB_INT, &entry->timestamp_db,
...
static void sync_xmp_to_db(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter,
                           gpointer user_data)
{
    dt_control_crawler_gui_t *gui = (dt_control_crawler_gui_t *)user_data;
...
static void sync_db_to_xmp(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter,
                           gpointer user_data)
{
    dt_control_crawler_gui_t *gui = (dt_control_crawler_gui_t *)user_data;
    dt_control_crawler_result_t entry = {NO_IMGID};
...
static void sync_newest_to_oldest(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter,
                                  gpointer user_data)
{
    dt_control_crawler_gui_t *gui = (dt_control_crawler_gui_t *)user_data;
    dt_control_crawler_result_t entry = {NO_IMGID};
    ...
    if (entry.timestamp_xmp > entry.timestamp_db)
    {
        ...
        if (error)
        {
            _log_synchronization(gui, _("ERROR: %s NOT synced new (XMP) → old (DB)"),
                                 entry.image_path);
            _log_synchronization(gui,
        ...
        {
            _log_synchronization(gui, _("SUCCESS: %s synced new (XMP) → old (DB)"),
                                 entry.image_path);
        }
    }
    else if (entry.timestamp_xmp < entry.timestamp_db)
    {
        ...
        if (error)
        {
            _log_synchronization(gui, _("ERROR: %s NOT synced new (DB) → old (XMP)"),
                                 entry.image_path);
...
static void sync_oldest_to_newest(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter,
                                  gpointer user_data)
{
    dt_control_crawler_gui_t *gui = (dt_control_crawler_gui_t *)user_data;
    dt_control_crawler_result_t entry = {NO_IMGID};
    ...
    if (entry.timestamp_xmp < entry.timestamp_db)
    {
        ...
        if (error)
        {
            _log_synchronization(gui, _("ERROR: %s NOT synced old (XMP) → new (DB)"),
                                 entry.image_path);
            _log_synchronization(gui,
                                 _("ERROR: cannot write the database."
                                   " the destination may be full, offline or read-only."),
        ...
        {
            _log_synchronization(gui, _("SUCCESS: %s synced old (XMP) → new (DB)"),
                                 entry.image_path);
        }
    }
    else if (entry.timestamp_xmp > entry.timestamp_db)
    {
        ...
        if (error)
        {
            _log_synchronization(gui, _("ERROR: %s NOT synced old (DB) → new (XMP)"),
                                 entry.image_path);
            _log_synchronization(gui,
                                 _("ERROR: cannot write %s \nthe destination may be full,"
                                   " offline or read-only."),
        ...
        {
            _log_synchronization(gui, _("SUCCESS: %s synced old (DB) → new (XMP)"),
                                 entry.image_path);
        }
    }
    else
...
void dt_control_crawler_show_image_list(GList *images)
{
    ...
    for (GList *list_iter = images; list_iter; list_iter = g_list_next(list_iter))
    {
        ...
        struct tm tm_stamp;
        strftime(timestamp_db, sizeof(timestamp_db), "%c",
                 localtime_r(&item->timestamp_db, &tm_stamp));
        strftime(timestamp_xmp, sizeof(timestamp_xmp), "%c",
                 localtime_r(&item->timestamp_xmp, &tm_stamp));

        const time_t time_delta = llabs(item->timestamp_db - item->timestamp_xmp);
        gchar *timestamp_delta = str_time_delta(time_delta);

        gtk_list_store_insert_with_values(
            store, NULL, -1, DT_CONTROL_CRAWLER_COL_ID, item->id, DT_CONTROL_CRAWLER_COL_IMAGE_PATH,
            item->image_path, DT_CONTROL_CRAWLER_COL_XMP_PATH, item->xmp_path,
            DT_CONTROL_CRAWLER_COL_TS_XMP, timestamp_xmp, DT_CONTROL_CRAWLER_COL_TS_DB,
            timestamp_db, DT_CONTROL_CRAWLER_COL_TS_XMP_INT, item->timestamp_xmp,
            DT_CONTROL_CRAWLER_COL_TS_DB_INT, item->timestamp_db, DT_CONTROL_CRAWLER_COL_REPORT,
            (item->timestamp_xmp > item->timestamp_db) ? _("XMP") : _("database"),
            DT_CONTROL_CRAWLER_COL_TIME_DELTA, timestamp_delta, -1);
    ...
    column =
        gtk_tree_view_column_new_with_attributes(_("XMP timestamp"), gtk_cell_renderer_text_new(),
                                                 "text", DT_CONTROL_CRAWLER_COL_TS_XMP, NULL);
    gtk_tree_view_append_column(GTK_TREE_VIEW(tree), column);

    column = gtk_tree_view_column_new_with_attributes(_("database timestamp"),
                                                      gtk_cell_renderer_text_new(), "text",
                                                      DT_CONTROL_CRAWLER_COL_TS_DB, NULL);
    gtk_tree_view_append_column(GTK_TREE_VIEW(tree), column);

    column = gtk_tree_view_column_new_with_attributes(_("newest"), gtk_cell_renderer_text_new(),
                                                      "text", DT_CONTROL_CRAWLER_COL_REPORT, NULL);
    gtk_tree_view_append_column(GTK_TREE_VIEW(tree), column);
    ...
    GtkWidget *dialog = gtk_dialog_new_with_buttons(
        ...
        GTK_DIALOG_DESTROY_WITH_PARENT | GTK_DIALOG_MODAL, _("_close"), GTK_RESPONSE_CLOSE, NULL);
    ...
    gtk_tree_view_insert_column_with_attributes(
        GTK_TREE_VIEW(gui->log), -1, _("synchronization log"), renderer_text, "markup", 0, NULL);
    ...
    dt_gui_dialog_add(GTK_DIALOG(dialog), dt_gui_hbox(select_all, select_none, select_invert),
                      scroll,
                      dt_gui_hbox(label, reload_button, overwrite_button, newest_button,
                                  oldest_button, gui->spinner),
...
{
    return darktable.backthumbs.state == DT_JOB_STATE_RUNNING && _lighttable_silent();
}

static int _update_img_thumbs(const dt_imgid_t imgid, const dt_mipmap_size_t max_mip,
                              const int64_t stamp)
{
...
static int _update_all_thumbs(const dt_mipmap_size_t max_mip)
{
    ...
    if (updated)
        dt_print(DT_DEBUG_CACHE, "[thumb crawler] max_mip=%d, %d thumbs updated, %d not found, %s",
...
void dt_update_thumbs_thread(void *p)
{
    ...
    const dt_mipmap_size_t mipsize =
        dt_mipmap_cache_get_min_mip_from_pref(dt_conf_get_string_const("backthumbs_mipsize"));
