...
static inline float xoshiro128plus(uint32_t state[4])
{
    ...
    // take the first 24 bits and put them in mantissa
...
DT_OMP_DECLARE_SIMD(uniform(sigma) aligned(state : 64))
static inline float gaussian_noise(const float mu, const float sigma, const int flip,
                                   uint32_t state[4])
{
...
DT_OMP_DECLARE_SIMD(uniform(sigma) aligned(state : 64))
static inline float poisson_noise(const float mu, const float sigma, const int flip,
                                  uint32_t state[4])
{
...
static inline float dt_noise_generator(const dt_noise_distribution_t distribution, const float mu,
                                       const float param, const int flip, uint32_t state[4])
{
    // scalar version
...
DT_OMP_DECLARE_SIMD(uniform(sigma) aligned(state : 64) aligned(mu, sigma, out : 16))
...
{
    const dt_aligned_pixel_t noise = {xoshiro128plus(state), xoshiro128plus(state),
                                      xoshiro128plus(state)};

    for_each_channel(c) out[c] = mu[c] + 2.0f * (noise[c] - 0.5f) * sigma[c];
}

DT_OMP_DECLARE_SIMD(uniform(sigma) aligned(state : 64) aligned(mu, sigma, flip, out : 16))
                                       ...
                                       const int flip[4], uint32_t state[4], dt_aligned_pixel_t out)
{
    ...
    for_each_channel(c) out[c] = noise[c] * sigma[c] + mu[c];
}

DT_OMP_DECLARE_SIMD(uniform(sigma) aligned(state : 64) aligned(mu, sigma, flip, out : 16))
static inline void poisson_noise_simd(const dt_aligned_pixel_t mu, const dt_aligned_pixel_t sigma,
                                      const int flip[4], uint32_t state[4], dt_aligned_pixel_t out)
{
    // Create poissonian noise - it's just gaussian noise with Anscombe transform applied.
    dt_aligned_pixel_t u1 = {0.f};
...
static inline void dt_noise_generator_simd(const dt_noise_distribution_t distribution,
                                           ...
                                           const dt_aligned_pixel_t param, const int flip[4],
                                           uint32_t state[4], dt_aligned_pixel_t out)
{
    // vector version
