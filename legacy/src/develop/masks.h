...
typedef enum dt_masks_state_t
{
    ...
    DT_MASKS_STATE_OP = DT_MASKS_STATE_UNION | DT_MASKS_STATE_INTERSECTION |
                        DT_MASKS_STATE_DIFFERENCE | DT_MASKS_STATE_SUM | DT_MASKS_STATE_EXCLUSION
} dt_masks_state_t;
...
typedef struct dt_masks_functions_t
{
    ...
    void (*set_hint_message)(const struct dt_masks_form_gui_t *const gui,
                             const struct dt_masks_form_t *const form, const int opacity,
                             char *const __restrict__ msgbuf, const size_t msgbuf_len);
    void (*modify_property)(struct dt_masks_form_t *const form, dt_masks_property_t prop,
                            const float old_val, const float new_val, float *sum, int *count,
                            float *min, float *max);
    void (*duplicate_points)(dt_develop_t *const dev, struct dt_masks_form_t *base,
                             struct dt_masks_form_t *dest);
    void (*initial_source_pos)(const float iwd, const float iht, float *x, float *y);
    void (*get_distance)(const float x, const float y, const float as,
                         struct dt_masks_form_gui_t *gui, const int index, const int num_points,
                         gboolean *inside, gboolean *inside_border, int *near,
                         gboolean *inside_source, float *dist);
    int (*get_points)(dt_develop_t *dev, const float x, const float y, const float radius_a,
                      const float radius_b, const float rotation, float **points,
                      int *points_count);
    int (*get_points_border)(dt_develop_t *dev, struct dt_masks_form_t *form, float **points,
                             int *points_count, float **border, int *border_count, const int source,
    ...
    int (*get_mask)(const dt_iop_module_t *const module, const dt_dev_pixelpipe_iop_t *const piece,
                    struct dt_masks_form_t *const form, float **buffer, int *width, int *height,
                    int *posx, int *posy);
    int (*get_mask_roi)(const dt_iop_module_t *const fmodule,
                        const dt_dev_pixelpipe_iop_t *const piece,
                        struct dt_masks_form_t *const form, const dt_iop_roi_t *roi, float *buffer);
    int (*get_area)(const dt_iop_module_t *const module, const dt_dev_pixelpipe_iop_t *const piece,
                    struct dt_masks_form_t *const form, int *width, int *height, int *posx,
                    int *posy);
    int (*get_source_area)(dt_iop_module_t *module, dt_dev_pixelpipe_iop_t *piece,
                           struct dt_masks_form_t *form, int *width, int *height, int *posx,
                           int *posy);
    int (*mouse_moved)(dt_iop_module_t *module, float pzx, float pzy, const double pressure,
                       const int which, const float zoom_scale, struct dt_masks_form_t *form,
                       const dt_imgid_t parentid, struct dt_masks_form_gui_t *gui, const int index);
    int (*mouse_scrolled)(dt_iop_module_t *module, float pzx, float pzy, const gboolean up,
                          uint32_t state, struct dt_masks_form_t *form, const dt_imgid_t parentid,
                          struct dt_masks_form_gui_t *gui, const int index);
    int (*button_pressed)(dt_iop_module_t *module, float pzx, float pzy, const double pressure,
                          const int which, const int type, const uint32_t state,
                          struct dt_masks_form_t *form, const dt_imgid_t parentid,
                          struct dt_masks_form_gui_t *gui, const int index);
    int (*button_released)(dt_iop_module_t *module, float pzx, float pzy, const int which,
                           const uint32_t state, struct dt_masks_form_t *form,
                           const dt_imgid_t parentid, struct dt_masks_form_gui_t *gui,
                           const int index);
    void (*post_expose)(cairo_t *cr, const float zoom_scale, struct dt_masks_form_gui_t *gui,
                        const int index, const int num_points);
...
int dt_masks_get_points_border(dt_develop_t *dev, dt_masks_form_t *form, float **points,
...
int dt_masks_get_area(const dt_iop_module_t *module, const dt_dev_pixelpipe_iop_t *piece,
...
static inline int dt_masks_get_mask(const dt_iop_module_t *const module,
                                    const dt_dev_pixelpipe_iop_t *const piece,
                                    dt_masks_form_t *const form, float **buffer, int *width,
...
int dt_masks_group_render(dt_iop_module_t *module, dt_dev_pixelpipe_iop_t *piece,
                          dt_masks_form_t *form, float **buffer, int *roi, const float scale);
int dt_masks_group_render_roi(dt_iop_module_t *module, dt_dev_pixelpipe_iop_t *piece,
...
void dt_masks_register_forms(dt_develop_t *dev, GList *forms);
...
void dt_masks_write_masks_history_item(const dt_imgid_t imgid, const int num,
...
gboolean dt_masks_events_mouse_moved(struct dt_iop_module_t *module, const float x, const float y,
                                     const double pressure, const int which,
                                     const float zoom_scale);
gboolean dt_masks_events_button_released(struct dt_iop_module_t *module, const float x,
                                         const float y, const int which, const uint32_t state,
                                         const float zoom_scale);
gboolean dt_masks_events_button_pressed(struct dt_iop_module_t *module, const float x,
                                        const float y, const double pressure, const int which,
                                        const int type, const uint32_t state);
gboolean dt_masks_events_mouse_scrolled(struct dt_iop_module_t *module, const float x,
                                        const float y, const gboolean up, const uint32_t state);
void dt_masks_events_post_expose(const struct dt_iop_module_t *module, cairo_t *cr,
                                 const int32_t width, const int32_t height, const float pointerx,
                                 const float pointery, const float zoom_scale);
...
void dt_masks_gui_form_create(dt_masks_form_t *form, dt_masks_form_gui_t *gui, const int index,
                              const struct dt_iop_module_t *module);
void dt_masks_gui_form_remove(dt_masks_form_t *form, dt_masks_form_gui_t *gui, const int index);
void dt_masks_gui_form_test_create(dt_masks_form_t *form, dt_masks_form_gui_t *gui,
                                   const struct dt_iop_module_t *module);
void dt_masks_gui_form_save_creation(dt_develop_t *dev, struct dt_iop_module_t *module,
...
dt_masks_point_group_t *dt_masks_group_add_form(dt_masks_form_t *grp, const dt_masks_form_t *form);
...
void dt_masks_set_edit_mode(struct dt_iop_module_t *module, const dt_masks_edit_mode_t value);
void dt_masks_set_edit_mode_single_form(struct dt_iop_module_t *module, const dt_mask_id_t formid,
...
void dt_masks_form_remove(struct dt_iop_module_t *module, dt_masks_form_t *grp,
                          dt_masks_form_t *form);
float dt_masks_form_change_opacity(dt_masks_form_t *form, const dt_imgid_t parentid,
                                   const float amount);
void dt_masks_form_move(dt_masks_form_t *grp, const dt_mask_id_t formid, const gboolean up);
...
/** utils functions */
gboolean dt_masks_point_in_form_exact(const float x, const float y, const float *points,
...
float dt_masks_drag_factor(dt_masks_form_gui_t *gui, const int index, const int k,
                           const gboolean border);

float dt_masks_change_size(const gboolean up, const float value, const float min, const float max);

float dt_masks_change_rotation(const gboolean up, const float value, const gboolean is_degree);

/** allow to select a shape inside an iop */
void dt_masks_select_form(struct dt_iop_module_t *module, const dt_masks_form_t *sel);

/** utils for selecting the source of a clone mask while creating it */
void dt_masks_draw_clone_source_pos(cairo_t *cr, const float zoom_scale, const float x,
                                    const float y);
void dt_masks_set_source_pos_initial_state(dt_masks_form_gui_t *gui, const uint32_t state,
                                           const float pzx, const float pzy);
void dt_masks_set_source_pos_initial_value(dt_masks_form_gui_t *gui, const int mask_type,
                                           dt_masks_form_t *form, const float pzx, const float pzy);
void dt_masks_calculate_source_pos_value(const dt_masks_form_gui_t *gui, const int mask_type,
                                         const float initial_xpos, const float initial_ypos,
                                         const float xpos, const float ypos, float *px, float *py,
                                         const int adding);

/** detail mask support */
float *dt_masks_calc_scharr_mask(struct dt_dev_pixelpipe_t *pipe, float *src, const int width,
                                 const int height, const gboolean rawmode);
float *dt_masks_calc_detail_mask(struct dt_dev_pixelpipe_iop_t *piece, const float threshold,
                                 const gboolean detail);
void dt_masks_calc_detail_blend(float *const src, float *out, const size_t msize,
...
void dt_group_events_post_expose(cairo_t *cr, const float zoom_scale, dt_masks_form_t *form,
...
static inline gboolean _dt_masks_dynbuf_growto(dt_masks_dynbuf_t *a, const size_t newsize)
{
    ...
    if (!newbuf)
    {
        ...
        dt_print(DT_DEBUG_ALWAYS, "critical: out of memory for dynbuf '%s' with size request %zu!",
                 a->tag, newsize);
        return FALSE;
    }
    if (a->buffer)
    {
        ...
        dt_print(DT_DEBUG_MASKS, "[masks dynbuf '%s'] grows to size %lu (is %p, was %p)", a->tag,
...
static inline dt_masks_dynbuf_t *dt_masks_dynbuf_init(const size_t size, const char *tag)
{
    assert(size > 0);
    ...
    if (a != NULL)
    {
        ...
        if (_dt_masks_dynbuf_growto(a, size))
            dt_print(DT_DEBUG_MASKS, "[masks dynbuf '%s'] with initial size %lu (is %p)", a->tag,
...
static inline void dt_masks_dynbuf_add(dt_masks_dynbuf_t *a, const float value)
{
    assert(a != NULL);
    assert(a->pos <= a->size);
...
static inline void dt_masks_dynbuf_add_2(dt_masks_dynbuf_t *a, const float value1,
                                         const float value2)
{
    assert(a != NULL);
    assert(a->pos <= a->size);
...
static inline float dt_masks_dynbuf_get(dt_masks_dynbuf_t *a, const int offset)
{
    assert(a != NULL);
...
static inline float dt_masks_dynbuf_get_absolute(dt_masks_dynbuf_t *a, const int position)
{
    assert(a != NULL);
    assert(position >= 0);
...
static inline void dt_masks_dynbuf_set(dt_masks_dynbuf_t *a, const int offset, const float value)
{
    assert(a != NULL);
    // offset: must be negative distance relative to end of buffer
    assert(offset < 0);
...
static inline void dt_masks_dynbuf_set_absolute(dt_masks_dynbuf_t *a, const int position,
                                                const float value)
{
    assert(a != NULL);
    assert(position >= 0);
...
static inline void dt_masks_dynbuf_reset_position(dt_masks_dynbuf_t *a, const size_t newpos)
{
    assert(a != NULL);
    assert(newpos <= a->pos);
    a->pos = newpos;
...
static inline void dt_masks_dynbuf_free(dt_masks_dynbuf_t *a)
{
    if (a == NULL)
        return;
    dt_print(DT_DEBUG_MASKS, "[masks dynbuf '%s'] freed (was %p)", a->tag, a->buffer);
    dt_free_align(a->buffer);
    free(a);
}

// Dump buffer to file for debugging.
static inline void dt_masks_dynbuf_debug_print(dt_masks_dynbuf_t *a, gboolean to_stdout)
{
    if (a == NULL)
        return;
    if (to_stdout)
 ...
 * buffer for ints
 */
static inline gboolean _dt_masks_intbuf_growto(dt_masks_intbuf_t *a, const size_t newsize)
{
    int *newbuf = dt_alloc_align_int(newsize);
    if (!newbuf)
    {
        // not much we can do here except emit an error message
        dt_print(DT_DEBUG_ALWAYS, "critical: out of memory for intbuf '%s' with size request %zu!",
                 a->tag, newsize);
        return FALSE;
    }
    if (a->buffer)
    {
        ...
        dt_print(DT_DEBUG_MASKS, "[masks intbuf '%s'] grows to size %lu (is %p, was %p)", a->tag,
                 (unsigned long)a->size, newbuf, a->buffer);
        dt_free_align(a->buffer);
    }
    a->size = newsize;
...
static inline dt_masks_intbuf_t *dt_masks_intbuf_init(const size_t size, const char *tag)
{
    assert(size > 0);
    ...
    if (a != NULL)
    {
        ...
        if (_dt_masks_intbuf_growto(a, size))
            dt_print(DT_DEBUG_MASKS, "[masks intbuf '%s'] with initial size %lu (is %p)", a->tag,
                     (unsigned long)a->size, a->buffer);
        if (a->buffer == NULL)
        {
            free(a);
...
static inline void dt_masks_intbuf_add2(dt_masks_intbuf_t *a, const float value1,
                                        const float value2)
{
    assert(a != NULL);
    assert(a->pos <= a->size);
...
static inline size_t dt_masks_intbuf_position(dt_masks_intbuf_t *a)
{
    assert(a != NULL);
    return a->pos;
}

static inline void dt_masks_intbuf_free(dt_masks_intbuf_t *a)
{
    if (a == NULL)
        return;
    dt_print(DT_DEBUG_MASKS, "[masks intbuf '%s'] freed (was %p)", a->tag, a->buffer);
    dt_free_align(a->buffer);
    free(a);
}
...
static inline int dt_masks_roundup(const int num, const int mult)
{
...
#define DT_MASKS_CONF(type, shape, param)                                                          \
    (type & (DT_MASKS_CLONE | DT_MASKS_NON_CLONE) ? "plugins/darkroom/spots/" #shape "_" #param :  \
                                                    "plugins/darkroom/masks/" #shape "/" #param)

void dt_masks_draw_anchor(cairo_t *cr, const gboolean selected, const float zoom_scale,
                          const float x, const float y);

/* draw the small control point for selected anchor in path & brush */
void dt_masks_draw_ctrl(cairo_t *cr, const float x, const float y, const float zoom_scale,
...
*/
void dt_masks_closest_point(const int count, const int nb_ctrl, const float *points, const float px,
                            const float py, float *x, float *y);
...
/* draw a line from -> to with an arrow at the end.
   ...
   be left.
*/
void dt_masks_draw_arrow(cairo_t *cr, const float from_x, const float from_y, const float to_x,
                         const float to_y, const float zoom_scale, const gboolean touch_dest);

/* stroke the arrow on cr depending on selection */
void dt_masks_stroke_arrow(cairo_t *cr, const dt_masks_form_gui_t *gui, const int group,
                           const float zoom_scale);

/* set line width for the mask drawing depending on the status
   border, source & selected
*/
void dt_masks_line_stroke(cairo_t *cr, const gboolean border, const gboolean source,
...
static inline void dt_masks_get_image_size(float *width, float *height, float *iwidth,
                                           float *iheight)
{
    ...
    {
        if (width)
            *width = preview->backbuf_width;
        if (height)
            *height = preview->backbuf_height;
    }
