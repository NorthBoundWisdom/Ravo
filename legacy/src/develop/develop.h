...
{
    gboolean
        gui_attached; // != 0 if the gui should be notified of changes in hist stack and modules should be
                      // gui_init'ed.
    gboolean gui_leaving; // set if everything is scheduled to shut down.
    gboolean
        gui_synch; // set to TRUE by the render threads if gui_update should be called in the modules.

    gpointer
        gui_previous_target;  // widget that was changed last time. If same again, don't save undo.
    ...
    struct
    {
        ...
        struct
        {
            ...
            /* switch module group */
            void (*set)(struct dt_lib_module_t *self, const uint32_t group);
            ...
            gboolean (*test)(struct dt_lib_module_t *self, const uint32_t group,
                             struct dt_iop_module_t *module);
            ...
            gboolean (*basics_module_toggle)(struct dt_lib_module_t *self, GtkWidget *widget,
                                             const gboolean doit);
        } modulegroups;
        ...
        struct
        {
            ...
            void (*list_remove)(struct dt_lib_module_t *self, const dt_mask_id_t formid,
    ...
    struct
    {
        ...
        void (*flip_callback)(struct dt_iop_module_t *crop, const dt_image_orientation_t flipmode);
    } cropping;
    ...
    struct
    {
        GtkWidget *floating_window, *softproof_button,
            *gamut_button; // TODO (#18559): remove gtk stuff from here
        GtkWidget *display_intent_widget,
            *display2_intent_widget; // TODO (#18559): remove gtk stuff from here
    } profile;
    ...
    struct dt_develop_t
        *preview2_pinned_dev; // Separate develop for pinned image (NULL when not pinned)
} dt_develop_t;

void dt_dev_init(dt_develop_t *dev, gboolean gui_attached);
void dt_dev_cleanup(dt_develop_t *dev);

void dt_dev_process_image_job(dt_develop_t *dev, dt_dev_viewport_t *port,
                              struct dt_dev_pixelpipe_t *pipe, const dt_signal_t signal,
                              const int devid);
// launch jobs above
...
void dt_dev_add_history_item_ext(dt_develop_t *dev, struct dt_iop_module_t *module,
                                 const gboolean enable, const gboolean no_image);
void dt_dev_add_history_item(dt_develop_t *dev, struct dt_iop_module_t *module,
                             const gboolean enable);
void dt_dev_add_history_item_target(dt_develop_t *dev, struct dt_iop_module_t *module,
                                    const gboolean enable, const gpointer target);
void dt_dev_add_new_history_item(dt_develop_t *dev, struct dt_iop_module_t *module,
                                 const gboolean enable);
void dt_dev_add_masks_history_item_ext(dt_develop_t *dev, struct dt_iop_module_t *_module,
                                       const gboolean _enable, const gboolean no_image);
void dt_dev_add_masks_history_item(dt_develop_t *dev, struct dt_iop_module_t *_module,
                                   const gboolean enable);
...
gboolean dt_dev_get_zoom_bounds(dt_dev_viewport_t *port, float *zoom_x, float *zoom_y, float *boxww,
                                float *boxhh);
void dt_dev_zoom_move(dt_dev_viewport_t *port, dt_dev_zoom_t zoom, const float scale,
                      const int closeup, const float x, const float y, const gboolean constrain);
...
/** test if iop group flags matches modulegroup */
gboolean dt_dev_modulegroups_test(dt_develop_t *dev, const uint32_t group,
                                  struct dt_iop_module_t *module);
...
struct dt_iop_module_t *dt_dev_module_duplicate_ext(dt_develop_t *dev, struct dt_iop_module_t *base,
...
gboolean dt_dev_distort_transform_plus(dt_develop_t *dev, struct dt_dev_pixelpipe_t *pipe,
...
gboolean dt_dev_distort_backtransform_plus(dt_develop_t *dev, struct dt_dev_pixelpipe_t *pipe,
                                           const double iop_order,
                                           const dt_dev_transform_direction_t transf_direction,
                                           float *points, const size_t points_count);
...
/** generate hash value out of all module settings of pixelpipe.
    ...
    transfer direction */
dt_hash_t dt_dev_hash_plus(dt_develop_t *dev, struct dt_dev_pixelpipe_t *pipe,
                           const double iop_order,
                           const dt_dev_transform_direction_t transf_direction);
/** synchronize pixelpipe by means hash values by waiting with timeout
 * and potential reprocessing */
gboolean dt_dev_sync_pixelpipe_hash(dt_develop_t *dev, struct dt_dev_pixelpipe_t *pipe,
                                    const double iop_order,
                                    const dt_dev_transform_direction_t transf_direction,
...
dt_hash_t dt_dev_hash_distort_plus(dt_develop_t *dev, struct dt_dev_pixelpipe_t *pipe,
                                   const double iop_order,
                                   const dt_dev_transform_direction_t transf_direction);
/*
...
void dt_dev_image(const dt_imgid_t imgid, const size_t width, const size_t height,
                  ...
                  size_t *buf_height, dt_dev_zoom_pos_t zoom_pos, const int32_t snapshot_id,
