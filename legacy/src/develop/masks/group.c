...
static int _group_events_button_pressed(dt_iop_module_t *module, const float pzx, const float pzy,
    ...
    if (gui->group_edited >= 0)
    {
        ...
        if (sel->functions)
        {
            ...
            if (dt_modifier_is(state, GDK_SHIFT_MASK) ^ gui->select_only_border)
            {
                ...
                sel->functions->mouse_moved(module, pzx, pzy, pressure, which,
                                            dt_dev_get_zoom_scale_full(), sel, fpt->parentid, gui,
                                            gui->group_edited);
            }

            return sel->functions->button_pressed(module, pzx, pzy, pressure, which, type, state,
...
static int _group_events_button_released(dt_iop_module_t *module, const float pzx, const float pzy,
                                         const int which, const uint32_t state,
                                         dt_masks_form_t *form, const int unused1,
                                         dt_masks_form_gui_t *gui, const int unused2)
{
    if (gui->group_edited >= 0)
    {
        // we get the form
        ...
        dt_masks_form_t *sel = dt_masks_get_from_id(darktable.develop, fpt->formid);
        if (sel && sel->functions)
            return sel->functions->button_released(module, pzx, pzy, which, state, sel,
...
{
    return gui->form_dragging || gui->source_dragging || gui->gradient_toggling ||
           gui->form_rotating || (gui->point_edited != -1) || (gui->point_dragging != -1) ||
           (gui->feather_dragging != -1) || (gui->point_border_dragging != -1) ||
           (gui->seg_dragging != -1);
}

static int _group_events_mouse_moved(dt_iop_module_t *module, const float pzx, const float pzy,
                                     ...
                                     dt_masks_form_gui_t *gui, const int unused2)
{
    ...
    if (gui->group_edited >= 0 && _is_handling_form(gui))
    {
        ...
        int rep = 0;
        if (sel->functions)
            rep = sel->functions->mouse_moved(module, pzx, pzy, pressure, which, zoom_scale, sel,
                                              fpt->parentid, gui, gui->group_edited);
        if (rep)
            return 1;
    ...
    if (sel && sel->functions)
    {
        ...
        return sel->functions->mouse_moved(module, pzx, pzy, pressure, which, zoom_scale, sel,
                                           sel_fpt->parentid, gui, gui->group_edited);
    }
                          ...
                          float **buffer, int *width, int *height, int *posx, int *posy)
{
    ...
    for (int yy = MAX(*posy, 0); yy < MIN(ht, (*posy) + (*height)); yy++)
    {
        ...
        for (int xx = MAX((*posx), 0); xx < MIN(wt, (*posx) + (*width)); xx++)
            buf[(size_t)yy * wt + xx] =
                1.0f - (*buffer)[((size_t)yy - (*posy)) * (*width) + xx - (*posx)];
                           ...
                           float **buffer, int *width, int *height, int *posx, int *posy)
{
    ...
    for (GList *fpts = form->points; fpts; fpts = g_list_next(fpts))
    {
        ...
        if (sel)
        {
            ok[pos] = dt_masks_get_mask(module, piece, sel, &bufs[pos], &w[pos], &h[pos], &px[pos],
                                        &py[pos]);
            if (fpt->state & DT_MASKS_STATE_INVERSE)
            {
                double start = dt_get_wtime();
                _inverse_mask(module, piece, sel, &bufs[pos], &w[pos], &h[pos], &px[pos], &py[pos]);
                dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] inverse took %0.04f sec",
    ...
    *buffer = dt_alloc_align_float((size_t)(r - l) * (b - t));
    ...
    for (int i = 0; i < nb; i++)
    {
        ...
        dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %d] combine took %0.04f sec", i,
...
static void _combine_masks_union(float *const restrict dest, float *const restrict newmask,
                                 const size_t npixels, const float opacity, const int inverted)
{
    if (inverted)
    {
...
static void _combine_masks_intersect(float *const restrict dest, float *const restrict newmask,
                                     const size_t npixels, const float opacity, const int inverted)
{
    if (inverted)
    {
...
static void _combine_masks_difference(float *const restrict dest, float *const restrict newmask,
                                      const size_t npixels, const float opacity, const int inverted)
{
    if (inverted)
    {
...
static void _combine_masks_sum(float *const restrict dest, float *const restrict newmask,
                               const size_t npixels, const float opacity, const int inverted)
{
    if (inverted)
    {
...
static void _combine_masks_exclusion(float *const restrict dest, float *const restrict newmask,
                                     const size_t npixels, const float opacity, const int inverted)
{
    if (inverted)
    {
    ...
    for (GList *fpts = form->points; fpts; fpts = g_list_next(fpts))
    {
        ...
        if (sel)
        {
            ...
            if (ok)
            {
                ...
                dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %d] combine took %0.04f sec",
                              ...
                              dt_masks_form_t *form, const dt_iop_roi_t *roi, float *buffer)
{
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks] render all masks took %0.04f sec",
             dt_get_lap_time(&start));
    return ok;
}
