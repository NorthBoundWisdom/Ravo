...
static int _circle_events_mouse_scrolled(dt_iop_module_t *module, const float pzx, const float pzy,
                                         ...
                                         const int index)
{
    ...
    if (gui->creation)
    {
        if (dt_modifier_is(state, GDK_SHIFT_MASK))
        {
            const float masks_border = dt_masks_change_size(
                up, dt_conf_get_float(DT_MASKS_CONF(form->type, circle, border)), MIN_CIRCLE_BORDER,
                max_mask_border);
        ...
        else if (dt_modifier_is(state, 0))
        {
            const float masks_size =
                dt_masks_change_size(up, dt_conf_get_float(DT_MASKS_CONF(form->type, circle, size)),
                                     MIN_CIRCLE_RADIUS, max_mask_size);
    ...
    if (gui->form_selected)
    {
        ...
        else
        {
            ...
            {
                circle->border =
                    dt_masks_change_size(up, circle->border, MIN_CIRCLE_BORDER, max_mask_border);
            ...
            {
                circle->radius =
                    dt_masks_change_size(up, circle->radius, MIN_CIRCLE_BORDER, max_mask_border);
...
static int _circle_events_button_pressed(dt_iop_module_t *module, float pzx, float pzy,
...
static int _circle_events_button_released(dt_iop_module_t *module, const float pzx, const float pzy,
                                          ...
                                          dt_masks_form_gui_t *gui, const int index)
{
    ...
    if (which == GDK_BUTTON_SECONDARY && dt_is_valid_maskid(parentid) &&
...
static int _circle_events_mouse_moved(dt_iop_module_t *module, const float pzx, const float pzy,
                                      ...
                                      const int index)
{
    ...
    else if (gui->point_border_dragging >= 1)
    {
        ...
        circle->border =
            CLAMP((circle->radius + circle->border) * s - circle->radius, 0.001f, max_mask_border);
...
static void _circle_draw_lines(const gboolean borders, const gboolean source, cairo_t *cr,
...
static float *_points_to_transform(const float x, const float y, const float radius, const float wd,
...
static int _circle_get_points_source(dt_develop_t *dev, const float x, const float y,
                                     const float xs, const float ys, const float radius,
                                     const float radius2, const float rotation, float **points,
...
static int _circle_get_points(dt_develop_t *dev, const float x, const float y, const float radius,
                              const float radius2, const float rotation, float **points,
                              int *points_count)
{
    (void)radius2; // keep compiler from complaining about unused arg
...
static void _circle_events_post_expose(cairo_t *cr, const float zoom_scale,
                                       dt_masks_form_gui_t *gui, const int index,
                                       const int num_points)
{
    ...
    if (gui->creation)
    {
        if (gui->guipoints_count == 0)
        {
            ...
            float *border = NULL;
            int border_count = 0;
            int draw = _circle_get_points(darktable.develop, x, y, radius_a, 0.0, 0.0, &points,
                                          &points_count);
            if (draw && radius_a != radius_b)
            {
                draw = _circle_get_points(darktable.develop, x, y, radius_b, 0.0, 0.0, &border,
                                          &border_count);
            }
            ...
            _circle_draw_lines(FALSE, FALSE, cr, FALSE, zoom_scale, points, points_count);
            // we draw the borders
            _circle_draw_lines(TRUE, FALSE, cr, FALSE, zoom_scale, border, border_count);
            cairo_restore(cr);
            ...
            if (form->type & DT_MASKS_CLONE)
            {
                x = 0.0f;
                y = 0.0f;
                dt_masks_calculate_source_pos_value(gui, DT_MASKS_CIRCLE, gui->posx, gui->posy,
    ...
    _circle_draw_lines(FALSE, FALSE, cr, selected, zoom_scale, gpt->points, gpt->points_count);
    // we draw the borders
    if (gui->show_all_feathers || gui->group_selected == index)
    {
        _circle_draw_lines(TRUE, FALSE, cr, gui->border_selected, zoom_scale, gpt->border,
                           gpt->border_count);
        dt_masks_draw_anchor(cr, gui->point_dragging > 0 || gui->point_selected > 0, zoom_scale,
                             gpt->points[2], gpt->points[3]);
        dt_masks_draw_anchor(cr, gui->point_border_dragging > 0 || gui->point_border_selected > 0,
    ...
    if (gpt->source_count > 6)
    {
        ...
        _circle_draw_lines(FALSE, TRUE, cr, selected, zoom_scale, gpt->source, gpt->source_count);
    }
}
...
static int _circle_get_points_border(dt_develop_t *dev, struct dt_masks_form_t *form,
                                     float **points, int *points_count, float **border,
                                     int *border_count, const int source,
                                     const dt_iop_module_t *module)
{
    ...
    if (source)
    {
        ...
        return _circle_get_points_source(dev, x, y, xs, ys, circle->radius, circle->radius, 0,
                                         points, points_count, module);
    }
    else
    {
        if (form->functions->get_points(dev, x, y, circle->radius, circle->radius, 0, points,
                                        points_count))
        {
            if (border)
            {
                const float outer_radius = circle->radius + circle->border;
                return form->functions->get_points(dev, x, y, outer_radius, outer_radius, 0, border,
                                                   border_count);
            }
            else
                return 1;
...
static int _circle_get_area(const dt_iop_module_t *const restrict module,
                            ...
                            dt_masks_form_t *const restrict form, int *width, int *height,
                            int *posx, int *posy)
{
    // we get the circle values
    dt_masks_point_circle_t *circle = form->points->data;
    ...
    const float outer_radius = circle->radius + circle->border;
    int num_points;
    float *const restrict points = _points_to_transform(circle->center[0], circle->center[1],
...
static int _circle_get_mask(const dt_iop_module_t *const restrict module,
                            const dt_dev_pixelpipe_iop_t *const restrict piece,
                            dt_masks_form_t *const restrict form, float **buffer, int *width,
                            int *height, int *posx, int *posy)
{
    double start2 = dt_get_debug_wtime();

    // we get the area
    if (!_circle_get_area(module, piece, form, width, height, posx, posy))
        return 0;

    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] circle area took %0.04f sec", form->name,
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] circle draw took %0.04f sec", form->name,
             dt_get_lap_time(&start2));
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] circle transform took %0.04f sec",
    ...
    *buffer = dt_alloc_align_float((size_t)w * h);
    ...
    const float total2 =
        (circle->radius + circle->border) * mindim * (circle->radius + circle->border) * mindim;
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] circle fill took %0.04f sec", form->name,
             dt_get_lap_time(&start2));
                                ...
                                float *const restrict buffer)
{
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] circle init took %0.04f sec", form->name,
             dt_get_lap_time(&start2));
    ...
    if (!dt_dev_distort_transform_plus(module->dev, piece->pipe, module->iop_order,
                                       DT_DEV_TRANSFORM_DIR_BACK_INCL, circ, circpts))
    {
        dt_free_align(circ);
        return 0;
    }

    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] circle outline took %0.04f sec",
             form->name, dt_get_lap_time(&start2));
    ...
    for (int n = 0; n < circpts; n++)
    {
        ...
        if (!(dt_isnormal(circ[2 * n]) && dt_isnormal(circ[2 * n + 1])))
            continue;
  ...
  printf("gw %d, gh %d, bbw %d, bbh %d\n", gw, gh, bbw, bbh);
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] circle bounding box took %0.04f sec",
             form->name, dt_get_lap_time(&start2));
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] circle grid took %0.04f sec", form->name,
             dt_get_lap_time(&start2));
    ...
    DT_OMP_FOR(collapse(2) if (bbh * bbw > 50000)
                   num_threads(MIN(dt_get_num_threads(), (h * w) / 20000)))
    for (int j = 0; j < bbh; j++)
        for (int i = 0; i < bbw; i++)
        {
            ...
            // find the square of the distance from the center
            const float l2 =
                sqf(points[2 * index] - centerx) + sqf(points[2 * index + 1] - centery);
    ...
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] circle fill took %0.04f sec", form->name,
             dt_get_lap_time(&start2));
    dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF, "[masks %s] circle total render took %0.04f sec",
...
static void _circle_set_form_name(dt_masks_form_t *const form, const size_t nb)
{
...
static void _circle_duplicate_points(dt_develop_t *dev, dt_masks_form_t *const base,
...
static void _circle_modify_property(dt_masks_form_t *const form, const dt_masks_property_t prop,
                                    ...
                                    int *count, float *min, float *max)
{
    ...
    float masks_size =
        circle ? circle->radius : dt_conf_get_float(DT_MASKS_CONF(form->type, circle, size));
    ...
    case DT_MASKS_PROPERTY_FEATHER:;
        const float max_mask_border =
            form->type & (DT_MASKS_CLONE | DT_MASKS_NON_CLONE) ? 0.5f : 1.0f;
        float masks_border =
            circle ? circle->border : dt_conf_get_float(DT_MASKS_CONF(form->type, circle, border));

        masks_border = CLAMP(masks_border * ratio, MIN_CIRCLE_BORDER, max_mask_border);

        if (circle)
            circle->border = masks_border;
        dt_conf_set_float(DT_MASKS_CONF(form->type, circle, border), masks_border);
