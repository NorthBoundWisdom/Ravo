...
void dt_develop_blendif_raw_make_mask(dt_dev_pixelpipe_iop_t *piece, const float *const restrict a,
                                      const float *const restrict b,
                                      const dt_iop_roi_t *const roi_in,
                                      const dt_iop_roi_t *const roi_out, float *const restrict mask)
{
                          ...
                          const size_t stride)
{
    for (size_t j = 0; j < stride; j++)
    {
        ...
        out[j] = clamp_simd(la * (1.0f - local_opacity) +
                              ...
                              const size_t stride)
{
    for (size_t j = 0; j < stride; j++)
    {
        ...
        const float la = clamp_simd(a[j]);
        const float lb = clamp_simd(b[j]);
        out[j] = clamp_simd(la * (1.0f - local_opacity2) +
...
void dt_develop_blendif_raw_blend(dt_dev_pixelpipe_iop_t *piece, const float *const restrict a,
        ...
        if (tmp_buffer != NULL)
        {
            ...
            if ((d->blend_mode & DEVELOP_BLEND_REVERSE) == DEVELOP_BLEND_REVERSE)
            {
                ...
                for (size_t y = 0; y < oheight; y++)
                {
                    ...
                    blend(tmp_buffer + bm_start, a + a_start, b + bm_start, mask + bm_start,
                          owidth);
                }
            }
            else
            {
                ...
                for (size_t y = 0; y < oheight; y++)
                {
                    const size_t a_start = (y + yoffs) * iwidth + xoffs;
                    const size_t bm_start = y * owidth;
                    blend(a + a_start, tmp_buffer + bm_start, b + bm_start, mask + bm_start,
                          owidth);
                }
            }
