...
{
    for_each_channel(c) XYZ[c] = CLIP(XYZ[c]);
}
                                 ...
                                 const dt_iop_order_iccprofile_info_t *const restrict profile)
{
    ...
    {
        const float value = dt_ioppr_get_rgb_matrix_luminance(
            pixels + j, profile->matrix_in, profile->lut_in, profile->unbounded_coeffs_in,
                                ...
                                const unsigned int *const restrict invert_mask)
{
    for (size_t x = 0, j = 0; x < stride; x++, j += DT_BLENDIF_RGB_CH)
    {
        ...
        for (size_t i = 0; i < 3; i++)
            factor *= _blendif_compute_factor(HSL[i], invert_mask[i],
                                      ...
                                      const dt_iop_order_iccprofile_info_t *const restrict profile)
{
    if (blendif & (1 << DEVELOP_BLENDIF_GRAY_in))
    {
        ...
        if (profile)
        {
            _blendif_gray(pixels, mask, stride,
                          parameters + DEVELOP_BLENDIF_PARAMETER_ITEMS * DEVELOP_BLENDIF_GRAY_in,
                          invert_mask, profile);
        }
        else
        {
    ...
    if (blendif & (1 << DEVELOP_BLENDIF_RED_in))
    {
        ...
        _blendif_rgb_red(pixels, mask, stride,
                         parameters + DEVELOP_BLENDIF_PARAMETER_ITEMS * DEVELOP_BLENDIF_RED_in,
                         invert_mask);
    }
    ...
    if (blendif & (1 << DEVELOP_BLENDIF_GREEN_in))
    {
        ...
        _blendif_rgb_green(pixels, mask, stride,
                           parameters + DEVELOP_BLENDIF_PARAMETER_ITEMS * DEVELOP_BLENDIF_GREEN_in,
                           invert_mask);
    }
    ...
    if (blendif & (1 << DEVELOP_BLENDIF_BLUE_in))
    {
        ...
        _blendif_rgb_blue(pixels, mask, stride,
                          parameters + DEVELOP_BLENDIF_PARAMETER_ITEMS * DEVELOP_BLENDIF_BLUE_in,
                          invert_mask);
    }

    if (blendif &
        ((1 << DEVELOP_BLENDIF_H_in) | (1 << DEVELOP_BLENDIF_S_in) | (1 << DEVELOP_BLENDIF_l_in)))
    {
        ...
        _blendif_hsl(pixels, mask, stride,
                     parameters + DEVELOP_BLENDIF_PARAMETER_ITEMS * DEVELOP_BLENDIF_H_in,
                     invert_mask);
    }
}

void dt_develop_blendif_rgb_hsl_make_mask(
    dt_dev_pixelpipe_iop_t *piece, const float *const restrict a, const float *const restrict b,
    const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out, float *const restrict mask)
{
    const dt_develop_blend_params_t *const d = piece->blendop_data;

    if (piece->colors != DT_BLENDIF_RGB_CH)
        return;
    ...
    else
    {
        ...
        const gboolean use_profile = dt_develop_blendif_init_masking_profile(
            piece, &blend_profile, DEVELOP_BLEND_CS_RGB_DISPLAY);
        ...
        DT_OMP_PRAGMA(parallel default(none) dt_omp_firstprivate(
            ...
            profile, parameters, mask_inclusive, mask_inversed, global_opacity))
        {
      ...
      for (size_t y = 0; y < oheight; y++)
      {
          ...
          _blendif_combine_channels(
              b + start, temp_mask + (y * owidth), owidth, blendif >> DEVELOP_BLENDIF_GRAY_out,
              parameters + DEVELOP_BLENDIF_PARAMETER_ITEMS * DEVELOP_BLENDIF_GRAY_out, profile);
      }
...
_BLEND_FUNC _blend_normal_unbounded(const float *const a, const float *const b, float *const out,
                                    const float *const mask, const size_t stride)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_RGB_CH)
    {
        const float local_opacity = mask[i];
...
/* lighten */
_BLEND_FUNC _blend_lighten(const float *const a, const float *const b, float *const restrict out,
                           const float *const restrict mask, const size_t stride)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_RGB_CH)
    {
        ...
        for (int k = 0; k < DT_BLENDIF_RGB_BCH; k++)
        {
            out[j + k] = clamp_simd(a[j + k] * (1.0f - local_opacity) +
                                    fmaxf(a[j + k], b[j + k]) * local_opacity);
        }
        out[j + DT_BLENDIF_RGB_BCH] = local_opacity;
    }
}
                          ...
                          const float *const restrict mask, const size_t stride)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_RGB_CH)
    {
        ...
        {
            out[j + k] = clamp_simd(a[j + k] * (1.0f - local_opacity) +
                                    fminf(a[j + k], b[j + k]) * local_opacity);
        }
        out[j + DT_BLENDIF_RGB_BCH] = local_opacity;
    }
}
                           ...
                           const float *const restrict mask, const size_t stride)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_RGB_CH)
    {
        ...
        {
            out[j + k] = clamp_simd(a[j + k] * (1.0f - local_opacity) +
                                    (a[j + k] + b[j + k]) / 2.0f * local_opacity);
        }
        out[j + DT_BLENDIF_RGB_BCH] = local_opacity;
    }
}
                              ...
                              const float *const restrict mask, const size_t stride)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_RGB_CH)
    {
        ...
        {
            out[j + k] = clamp_simd(a[j + k] * (1.0f - local_opacity) +
                                    fabsf(a[j + k] - b[j + k]) * local_opacity);
        }
        out[j + DT_BLENDIF_RGB_BCH] = local_opacity;
    }
}
                          ...
                          const float *const restrict mask, const size_t stride)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_RGB_CH)
    {
        ...
        for (int k = 0; k < DT_BLENDIF_RGB_BCH; k++)
        {
            ...
            out[j + k] = clamp_simd(la * (1.0f - local_opacity) +
                                    (1.0f - (1.0f - la) * (1.0f - lb)) * local_opacity);
        }
        out[j + DT_BLENDIF_RGB_BCH] = local_opacity;
    }
}
...
/* overlay */
_BLEND_FUNC _blend_overlay(const float *const a, const float *const b, float *const restrict out,
                           const float *const restrict mask, const size_t stride)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_RGB_CH)
    {
                             ...
                             const float *const restrict mask, const size_t stride)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_RGB_CH)
    {
        ...
        for (int k = 0; k < DT_BLENDIF_RGB_BCH; k++)
        {
            ...
            out[j + k] = clamp_simd(
                la * (1.0f - local_opacity2) +
                (lb > 0.5f ? 1.0f - (1.0f - la) * (1.0f - (lb - 0.5f)) : la * (lb + 0.5f)) *
                    local_opacity2);
        }
        out[j + DT_BLENDIF_RGB_BCH] = local_opacity;
    }
                             ...
                             const float *const restrict mask, const size_t stride)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_RGB_CH)
    {
        ...
        for (int k = 0; k < DT_BLENDIF_RGB_BCH; k++)
        {
            ...
            out[j + k] = clamp_simd(
                la * (1.0f - local_opacity2) +
                (lb > 0.5f ? 1.0f - (1.0f - 2.0f * (la - 0.5f)) * (1.0f - lb) : 2.0f * la * lb) *
                    local_opacity2);
        }
        out[j + DT_BLENDIF_RGB_BCH] = local_opacity;
    }
                              ...
                              const float *const restrict mask, const size_t stride)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_RGB_CH)
    {
        ...
        for (int k = 0; k < DT_BLENDIF_RGB_BCH; k++)
        {
            ...
            const float lb = clamp_simd(b[j + k]);
            out[j + k] =
                clamp_simd(la * (1.0f - local_opacity2) +
                            ...
                            const float *const restrict mask, const size_t stride)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_RGB_CH)
    {
        ...
        for (int k = 0; k < DT_BLENDIF_RGB_BCH; k++)
        {
            ...
            out[j + k] =
                clamp_simd(la * (1.0f - local_opacity2) +
                           (lb > 0.5f ? fmaxf(la, 2.0f * (lb - 0.5f)) : fminf(la, 2.0f * lb)) *
                               local_opacity2);
        }
        out[j + DT_BLENDIF_RGB_BCH] = local_opacity;
    }
...
_BLEND_FUNC _blend_HSV_value(const float *const a, const float *const b, float *const out,
                             const float *const restrict mask, const size_t stride)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_RGB_CH)
    {
...
_BLEND_FUNC _blend_HSV_color(const float *const a, const float *const b, float *const out,
                             const float *const restrict mask, const size_t stride)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_RGB_CH)
    {
...
_BLEND_FUNC _blend_RGB_R(const float *const a, const float *const b, float *const out,
                         const float *const restrict mask, const size_t stride)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_RGB_CH)
    {
...
/* blend only R-channel in RGB color space without any clamping */
_BLEND_FUNC _blend_RGB_G(const float *const a, const float *const b, float *const out,
                         const float *const restrict mask, const size_t stride)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_RGB_CH)
    {
...
/* blend only R-channel in RGB color space without any clamping */
_BLEND_FUNC _blend_RGB_B(const float *const a, const float *const b, float *const out,
                         const float *const restrict mask, const size_t stride)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_RGB_CH)
    {
                              ...
                              const size_t stride)
{
    ...
    for (size_t x = DT_BLENDIF_RGB_BCH; x < stride; x += DT_BLENDIF_RGB_CH)
        b[x] = a[x];
}

void dt_develop_blendif_rgb_hsl_blend(dt_dev_pixelpipe_iop_t *piece, const float *const restrict a,
                                      float *const restrict b, const dt_iop_roi_t *const roi_in,
