...
DT_OMP_DECLARE_SIMD(aligned(pixels, invert_mask : 16)
                        uniform(parameters, invert_mask, stride, profile))
                                   ...
                                   const dt_iop_order_iccprofile_info_t *const restrict profile)
{
    for (size_t x = 0, j = 0; x < stride; x++, j += DT_BLENDIF_RGB_CH)
    {
        ...
        dt_ioppr_rgb_matrix_to_xyz(pixels + j, XYZ_D65, profile->matrix_out_transposed,
                                   profile->lut_in, profile->unbounded_coeffs_in, profile->lutsize,
                                      ...
                                      const dt_iop_order_iccprofile_info_t *const restrict profile)
{
    ...
    if (blendif & ((1 << DEVELOP_BLENDIF_Jz_in) | (1 << DEVELOP_BLENDIF_Cz_in) |
                   (1 << DEVELOP_BLENDIF_hz_in)))
    {
        ...
        if (dt_is_valid_colormatrix(profile->matrix_out_transposed[0][0]))
            _blendif_jzczhz(pixels, mask, stride,
                            parameters + DEVELOP_BLENDIF_PARAMETER_ITEMS * DEVELOP_BLENDIF_Jz_in,
                            invert_mask, profile);
        else
        {
            cmsHPROFILE *input = dt_colorspaces_get_profile(profile->type, profile->filename,
                                                            DT_PROFILE_DIRECTION_IN)
                                     ->profile;
            cmsHPROFILE *Lab =
                dt_colorspaces_get_profile(DT_COLORSPACE_LAB, "", DT_PROFILE_DIRECTION_ANY)
                    ->profile;
            cmsHTRANSFORM *xform =
                cmsCreateTransform(input, TYPE_RGBA_FLT, Lab, TYPE_LabA_FLT, profile->intent, 0);

            _blendif_jzczhz_lab(pixels, mask, stride,
                                parameters +
                                    DEVELOP_BLENDIF_PARAMETER_ITEMS * DEVELOP_BLENDIF_Jz_in,
                                invert_mask, xform);
...
void dt_develop_blendif_rgb_jzczhz_make_mask(
    ...
    const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out, float *const restrict mask)
{
    ...
    else
    {
        ...
        if (!dt_develop_blendif_init_masking_profile(piece, &blend_profile,
...
_BLEND_FUNC _blend_normal(const float *const a, const float *const b, const float p,
...
/* multiply */
_BLEND_FUNC _blend_multiply(const float *const a, const float *const b, const float p,
                            float *const out, const float *const restrict mask, const size_t stride)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_RGB_CH)
    {
        ...
        {
            out[j + k] =
                a[j + k] * (1.0f - local_opacity) + (a[j + k] * b[j + k] * p) * local_opacity;
        }
        out[j + DT_BLENDIF_RGB_BCH] = local_opacity;
    }
}
...
/* add */
_BLEND_FUNC _blend_add(const float *const a, const float *const b, const float p, float *const out,
                       const float *const restrict mask, const size_t stride)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_RGB_CH)
    {
        ...
        {
            out[j + k] =
                a[j + k] * (1.0f - local_opacity) + (a[j + k] + p * b[j + k]) * local_opacity;
        }
        out[j + DT_BLENDIF_RGB_BCH] = local_opacity;
    }
}
...
/* subtract */
_BLEND_FUNC _blend_subtract(const float *const a, const float *const b, const float p,
                            float *const out, const float *const restrict mask, const size_t stride)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_RGB_CH)
    {
        ...
        for (int k = 0; k < DT_BLENDIF_RGB_BCH; k++)
        {
            out[j + k] = a[j + k] * (1.0f - local_opacity) +
                         fmaxf(a[j + k] - p * b[j + k], 0.0f) * local_opacity;
        }
                              ...
                              const size_t stride)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_RGB_CH)
    {
        ...
        {
            out[j + k] =
                a[j + k] * (1.0f - local_opacity) + fabsf(a[j + k] - b[j + k]) * local_opacity;
        }
        out[j + DT_BLENDIF_RGB_BCH] = local_opacity;
    }
}
...
/* divide */
_BLEND_FUNC _blend_divide(const float *const a, const float *const b, const float p,
                          float *const out, const float *const restrict mask, const size_t stride)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_RGB_CH)
    {
...
/* average */
_BLEND_FUNC _blend_average(const float *const a, const float *const b, const float p,
                           float *const out, const float *const restrict mask, const size_t stride)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_RGB_CH)
    {
        ...
        {
            out[j + k] =
                a[j + k] * (1.0f - local_opacity) + 0.5f * (a[j + k] + b[j + k]) * local_opacity;
        }
        out[j + DT_BLENDIF_RGB_BCH] = local_opacity;
    }
}
...
/* geometric mean */
_BLEND_FUNC _blend_geometric_mean(const float *const a, const float *const b, const float p,
                                  float *const out, const float *const restrict mask,
                                  const size_t stride)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_RGB_CH)
    {
        ...
        {
            out[j + k] = a[j + k] * (1.0f - local_opacity) +
                         sqrtf(fmax(a[j + k] * b[j + k], 0.0f)) * local_opacity;
        }
        out[j + DT_BLENDIF_RGB_BCH] = local_opacity;
    }
}
...
/* harmonic mean */
_BLEND_FUNC _blend_harmonic_mean(const float *const a, const float *const b, const float p,
                                 float *const out, const float *const restrict mask,
                                 const size_t stride)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_RGB_CH)
    {
        ...
        for (int k = 0; k < DT_BLENDIF_RGB_BCH; k++)
        {
            ...
            out[j + k] = a[j + k] * (1.0f - local_opacity) +
                         2.0f * a[j + k] * b[j + k] /
                             (fmaxf(a[j + k], 5e-7f) + fmaxf(b[j + k], 5e-7f)) * local_opacity;
        }
        out[j + DT_BLENDIF_RGB_BCH] = local_opacity;
    }
}
...
/* chromaticity */
_BLEND_FUNC _blend_chromaticity(const float *const a, const float *const b, const float p,
                                float *const out, const float *const restrict mask,
                                const size_t stride)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_RGB_CH)
    {
        ...
        {
            out[j + k] =
                a[j + k] * (1.0f - local_opacity) + b[j + k] * norm_a / norm_b * local_opacity;
        }
        out[j + DT_BLENDIF_RGB_BCH] = local_opacity;
    }
}
...
/* luminance */
_BLEND_FUNC _blend_luminance(const float *const a, const float *const b, const float p,
                             float *const out, const float *const restrict mask,
                             const size_t stride)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_RGB_CH)
    {
        ...
        {
            out[j + k] =
                a[j + k] * (1.0f - local_opacity) + a[j + k] * norm_b / norm_a * local_opacity;
        }
        out[j + DT_BLENDIF_RGB_BCH] = local_opacity;
    }
}
...
_BLEND_FUNC _blend_RGB_R(const float *const a, const float *const b, const float p,
                         float *const out, const float *const restrict mask, const size_t stride)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_RGB_CH)
    {
...
/* blend only R-channel in RGB color space without any clamping */
_BLEND_FUNC _blend_RGB_G(const float *const a, const float *const b, const float p,
                         float *const out, const float *const restrict mask, const size_t stride)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_RGB_CH)
    {
...
/* blend only R-channel in RGB color space without any clamping */
_BLEND_FUNC _blend_RGB_B(const float *const a, const float *const b, const float p,
                         float *const out, const float *const restrict mask, const size_t stride)
{
    for (size_t i = 0, j = 0; i < stride; i++, j += DT_BLENDIF_RGB_CH)
    {
                                  ...
                                  const dt_iop_order_iccprofile_info_t *const restrict profile)
{
    ...
    if (profile)
    {
        ...
        // use the matrix_out of the hacked profile for blending to use the
        // conversion from RGB to XYZ D65 (instead of XYZ D50)
        dt_ioppr_rgb_matrix_to_xyz(rgb, XYZ_D65, profile->matrix_out_transposed, profile->lut_in,
                                   profile->unbounded_coeffs_in, profile->lutsize,
                                   profile->nonlinearlut);
        dt_XYZ_2_JzAzBz(XYZ_D65, JzAzBz);
    }
                                         ...
                                         const dt_dev_pixelpipe_display_mask_t request_mask_display)
{
    ...
    if (request_mask_display & DT_DEV_PIXELPIPE_DISPLAY_ANY)
    {
        ...
        const gboolean use_profile = dt_develop_blendif_init_masking_profile(
            piece, &blend_profile, DEVELOP_BLEND_CS_RGB_SCENE);
