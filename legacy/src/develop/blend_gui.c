...
static inline float _get_boost_factor(const dt_iop_gui_blend_data_t *data, const int channel,
                                      const int in_out)
{
    return exp2f(data->module->blend_params
                     ->blendif_boost_factors[data->channel[channel].param_channels[in_out]]);
}

static void _blendif_scale(dt_iop_gui_blend_data_t *data, dt_iop_colorspace_type_t cst,
                           const float *in, float *out,
                           const dt_iop_order_iccprofile_info_t *work_profile, const int in_out)
{
    ...
    case IOP_CS_RGB:
        if (work_profile == NULL)
            ...
            out[CHANNEL_INDEX_g] = dt_ioppr_get_rgb_matrix_luminance(
                in, work_profile->matrix_in, work_profile->lut_in,
        ...
        break;
    case IOP_CS_LCH:
        out[CHANNEL_INDEX_C] = (in[1] / _get_boost_factor(data, 3, in_out)) / (128.0f * M_SQRT2_F);
...
static void _blendif_cook(const dt_iop_colorspace_type_t cst, const float *in, float *out,
...
static void _blendif_scale_print_ab(const float value, const float boost_factor, char *string,
                                    int n)
{
...
static void _blendif_scale_print_hue(const float value, const float boost_factor, char *string,
                                     const int n)
{
    snprintf(string, n, "%-5.0f", value * 360.0f);
}

static void _blendif_scale_print_default(const float value, const float boost_factor, char *string,
                                         const int n)
{
...
static gboolean _blendif_clean_output_channels(dt_iop_module_t *module)
{
    ...
    if (!bd || !bd->blendif_support || !bd->blendif_inited)
        return FALSE;
                                         ...
                                         dt_iop_gui_blend_data_t *data)
{
    ...
    if (mask_enabled &&
        ((data->masks_inited && mode_drawn) || (data->blendif_inited && mode_parametric)))
    {
    ...
    if (data->masks_inited && mode_drawn)
    {
...
static gboolean _blendop_blend_order_clicked(GtkWidget *button, GdkEventButton *event,
...
static void _blendop_masks_combine_callback(GtkWidget *combo, dt_iop_gui_blend_data_t *data)
{
    ...
    {
        const uint32_t mask =
            data->csp == DEVELOP_BLEND_CS_LAB ? DEVELOP_BLENDIF_Lab_MASK : DEVELOP_BLENDIF_RGB_MASK;
...
static void _blendop_blendif_highlight_changed_tabs(dt_iop_module_t *module)
{
    ...
    for (int tab = 0; bd->channel[tab].label; tab++)
    {
        ...
        GtkWidget *label = gtk_notebook_get_tab_label(
            bd->channel_tabs, gtk_notebook_get_nth_page(bd->channel_tabs, tab));
        if (is_changed)
...
static void _blendop_blendif_polarity_callback(GtkToggleButton *togglebutton,
                                               dt_iop_gui_blend_data_t *data)
{
    DT_GUARD_GUI_UPDATE();
    ...
    dtgtk_gradient_slider_multivalue_set_marker(
        slider,
        active ? GRADIENT_SLIDER_MARKER_LOWER_OPEN_BIG : GRADIENT_SLIDER_MARKER_UPPER_OPEN_BIG, 0);

    dtgtk_gradient_slider_multivalue_set_marker(slider,
                                                active ? GRADIENT_SLIDER_MARKER_UPPER_FILLED_BIG :
    ...
    dtgtk_gradient_slider_multivalue_set_marker(
        slider,
        active ? GRADIENT_SLIDER_MARKER_LOWER_OPEN_BIG : GRADIENT_SLIDER_MARKER_UPPER_OPEN_BIG, 3);
...
static float _log10_scale_callback(GtkWidget *self, const float inval, const int dir)
{
    float outval = .0f;
...
static float _magnifier_scale_callback(GtkWidget *self, const float inval, const int dir)
{
...
static int
_blendop_blendif_disp_alternative_worker(GtkWidget *widget, dt_iop_module_t *module, const int mode,
                                         ...
                                         const char *label)
{
    ...
    dtgtk_gradient_slider_multivalue_set_scale_callback(slider,
                                                        (mode == 1) ? scale_callback : NULL);

    gchar *text =
        g_strdup_printf("%s%s", (in_out == 0) ? _("input") : _("output"), (mode == 1) ? label : "");
...
static int _blendop_blendif_disp_alternative_mag(GtkWidget *widget, dt_iop_module_t *module,
                                                 const int mode)
{
    return _blendop_blendif_disp_alternative_worker(widget, module, mode, _magnifier_scale_callback,
                                                    _(" (zoom)"));
}

static int _blendop_blendif_disp_alternative_log(GtkWidget *widget, dt_iop_module_t *module,
                                                 const int mode)
{
    return _blendop_blendif_disp_alternative_worker(widget, module, mode, _log10_scale_callback,
                                                    _(" (log)"));
}
...
static void _update_gradient_slider_pickers(GtkWidget *callback_dummy, dt_iop_module_t *module)
{
    ...
    for (int in_out = 1; in_out >= 0; in_out--)
    {
            ...
            (raw_min[0] != FLT_MAX))
        {
            ...
            gchar *text = g_strdup_printf("(%.*f)", _blendif_print_digits_picker(cooked[data->tab]),
                                          cooked[data->tab]);

            dtgtk_gradient_slider_multivalue_set_picker_meanminmax(
                data->filter[in_out].slider, CLAMP(picker_mean[data->tab], 0.0f, 1.0f),
                CLAMP(picker_min[data->tab], 0.0f, 1.0f), CLAMP(picker_max[data->tab], 0.0f, 1.0f));
...
static void _blendop_blendif_tab_switch(GtkNotebook *notebook, GtkWidget *page,
                                        const guint page_num, dt_iop_gui_blend_data_t *data)
{
    if (DT_IN_GUI_UPDATE() || !data || !data->blendif_inited)
        return;
...
static void _blendop_blendif_boost_factor_callback(GtkWidget *slider, dt_iop_gui_blend_data_t *data)
{
    ...
    for (int in_out = 1; in_out >= 0; in_out--)
    {
        ...
        if (parameters[0] > 0.0f)
            parameters[0] = CLIP((parameters[0] - off) * factor + off);
        if (parameters[1] > 0.0f)
            parameters[1] = CLIP((parameters[1] - off) * factor + off);
        if (parameters[2] < 1.0f)
            parameters[2] = CLIP((parameters[2] - off) * factor + off);
        if (parameters[3] < 1.0f)
            parameters[3] = CLIP((parameters[3] - off) * factor + off);
        if (parameters[1] == 0.0f && parameters[2] == 1.0f)
            bp->blendif &= ~(1 << ch);
...
static void _blendop_blendif_feathering_callback(GtkWidget *slider, dt_iop_gui_blend_data_t *data)
{
    if (DT_IN_GUI_UPDATE() || !data || !data->blendif_inited)
        return;
    dt_develop_blend_params_t *bp = data->module->blend_params;
...
static gboolean _blendop_blendif_showmask_clicked(GtkToggleButton *button, GdkEventButton *event,
                                                  dt_iop_module_t *module)
{
    DT_GUARD_GUI_UPDATE(TRUE);
...
static gboolean _blendop_masks_modes_none_clicked(GtkWidget *button, GdkEventButton *event,
                                                  dt_iop_module_t *module)
{
    DT_GUARD_GUI_UPDATE(TRUE);
    dt_iop_gui_blend_data_t *data = module->blend_data;
...
static gboolean _blendop_masks_modes_toggle(GtkToggleButton *button, dt_iop_module_t *module,
                                            dt_develop_mask_mode_t mask_mode)
{
    ...
    if (!button)
    {
        ...
        button = g_list_nth_data(data->masks_modes_toggles,
                                 g_list_index(data->masks_modes, GUINT_TO_POINTER(mask_mode)));
    }
...
static gboolean _blendop_masks_modes_uni_toggled(GtkToggleButton *button, GdkEventButton *event,
                                                 dt_iop_module_t *module)
{
    return _blendop_masks_modes_toggle(button, module, DEVELOP_MASK_ENABLED);
}

static gboolean _blendop_masks_modes_drawn_toggled(GtkToggleButton *button, GdkEventButton *event,
                                                   dt_iop_module_t *module)
{
    return _blendop_masks_modes_toggle(button, module, DEVELOP_MASK_ENABLED | DEVELOP_MASK_MASK);
}

static gboolean _blendop_masks_modes_param_toggled(GtkToggleButton *button, GdkEventButton *event,
                                                   dt_iop_module_t *module)
{
...
static gboolean _blendop_masks_modes_both_toggled(GtkToggleButton *button, GdkEventButton *event,
                                                  dt_iop_module_t *module)
{
    return _blendop_masks_modes_toggle(button, module,
...
static gboolean _blendop_masks_modes_raster_toggled(GtkToggleButton *button, GdkEventButton *event,
                                                    dt_iop_module_t *module)
{
...
static gboolean _blendop_blendif_suppress_toggled(GtkToggleButton *togglebutton,
                                                  GdkEventButton *event, dt_iop_module_t *module)
{
...
static gboolean _blendop_blendif_reset(GtkButton *button, GdkEventButton *event,
                                       dt_iop_module_t *module)
{
    module->blend_params->blendif = module->default_blendop_params->blendif;
    memcpy(module->blend_params->blendif_parameters,
           module->default_blendop_params->blendif_parameters,
...
static gboolean _blendop_blendif_invert(GtkButton *button, GdkEventButton *event,
                                        dt_iop_module_t *module)
{
    DT_GUARD_GUI_UPDATE(TRUE);
...
static gboolean _blendop_masks_add_shape(GtkWidget *widget, GdkEventButton *event,
                                         dt_iop_module_t *self)
{
    if (DT_IN_GUI_UPDATE() || event->button != GDK_BUTTON_PRIMARY)
        return TRUE;
...
static gboolean _blendop_masks_show_and_edit(GtkWidget *widget, GdkEventButton *event,
                                             dt_iop_module_t *self)
{
    DT_GUARD_GUI_UPDATE(FALSE);
    ...
    if (event->button == GDK_BUTTON_PRIMARY)
    {
        ...
        if (grp && (grp->type & DT_MASKS_GROUP) && grp->points)
        {
            ...
            case DT_MASKS_EDIT_FULL:
                bd->masks_shown =
                    control_button_pressed ? DT_MASKS_EDIT_RESTRICTED : DT_MASKS_EDIT_OFF;
                break;
            ...
            case DT_MASKS_EDIT_OFF:
                bd->masks_shown =
                    control_button_pressed ? DT_MASKS_EDIT_RESTRICTED : DT_MASKS_EDIT_FULL;
            }
        }
        else
        {
...
gboolean blend_color_picker_apply(dt_iop_module_t *module, GtkWidget *picker,
                                  dt_dev_pixelpipe_t *pipe)
{
    ...
    if (picker == data->colorpicker_set_values)
    {
        ...
        const int in_out =
            ((dt_key_modifier_state() == GDK_CONTROL_MASK) && data->output_channels_shown) ? 1 : 0;
        ...
        {
            char text[256];
            channel->scale_print(dtgtk_gradient_slider_multivalue_get_value(sl->slider, k),
    ...
    else if (picker == data->colorpicker)
    {
        ...
        return FALSE; // needs to be handled by module
}
                                                 ...
                                                 dt_develop_blend_colorspace_t cst)
{
    ...
    if (cst != module->blend_params->blend_cst)
    {
        ...
        for (const GList *history = g_list_last(darktable.develop->history); history;
             history = g_list_previous(history))
        {
            ...
            if (data->module == module && data->blend_params->blend_cst == cst)
            {
                ...
                memcpy(np->blendif_parameters, hp->blendif_parameters,
                       sizeof(hp->blendif_parameters));
                memcpy(np->blendif_boost_factors, hp->blendif_boost_factors,
                       sizeof(hp->blendif_boost_factors));
                break;
            }
        }
...
static void _blendif_options_callback(GtkButton *button, dt_iop_module_t *module)
{
        ...
        module_cst == DEVELOP_BLEND_CS_RGB_SCENE)
    {
        ...
        g_object_set_data_full(G_OBJECT(mi), "dt-blend-cst", GINT_TO_POINTER(DEVELOP_BLEND_CS_NONE),
                               NULL);
        ...
        if (module_cst == DEVELOP_BLEND_CS_LAB)
        {
            ...
            g_signal_connect(G_OBJECT(mi), "activate", G_CALLBACK(_blendif_select_colorspace),
                             module);
            gtk_menu_shell_append(GTK_MENU_SHELL(menu), mi);
        }
        ...
        if (bd->output_channels_shown)
        {
            ...
            g_signal_connect(G_OBJECT(mi), "activate", G_CALLBACK(_blendif_hide_output_channels),
                             module);
            gtk_menu_shell_append(GTK_MENU_SHELL(menu), mi);
        }
        else
        {
            ...
            g_signal_connect(G_OBJECT(mi), "activate", G_CALLBACK(_blendif_show_output_channels),
                             module);
            gtk_menu_shell_append(GTK_MENU_SHELL(menu), mi);
        }
    }

    dt_gui_menu_popup(menu, GTK_WIDGET(button), GDK_GRAVITY_SOUTH_EAST, GDK_GRAVITY_NORTH_EAST);

    dtgtk_button_set_active(DTGTK_BUTTON(button), FALSE);
}

// activate channel/mask view
static void _blendop_blendif_channel_mask_view(GtkWidget *widget, dt_iop_module_t *module,
...
static void _blendop_blendif_channel_mask_view_toggle(GtkWidget *widget, dt_iop_module_t *module,
                                                      const dt_dev_pixelpipe_display_mask_t mode)
{
    dt_iop_gui_blend_data_t *data = module->blend_data;
...
// mode
static gboolean _blendop_blendif_enter(GtkWidget *widget, GdkEventCrossing *event,
                                       dt_iop_module_t *module)
{
    DT_GUARD_GUI_UPDATE(FALSE);
...
static gboolean _blendop_blendif_leave(GtkWidget *widget, GdkEventCrossing *event,
                                       dt_iop_module_t *module)
{
    DT_GUARD_GUI_UPDATE(FALSE);
...
static gboolean _blendop_blendif_key_press(GtkWidget *widget, GdkEventKey *event,
                                           dt_iop_module_t *module)
{
    DT_GUARD_GUI_UPDATE(FALSE);
    ...
    case GDK_KEY_A:
        if (data->channel[tab].altdisplay)
            data->altmode[tab][in_out] =
                (data->channel[tab].altdisplay)(widget, module, data->altmode[tab][in_out] + 1);
        handled = TRUE;
        break;
    case GDK_KEY_c:
    ...
    case GDK_KEY_C:
        _blendop_blendif_channel_mask_view_toggle(
            widget, module, DT_DEV_PIXELPIPE_DISPLAY_CHANNEL | DT_DEV_PIXELPIPE_DISPLAY_STICKY);
        handled = TRUE;
        break;
    case GDK_KEY_m:
    case GDK_KEY_M:
...
const dt_iop_gui_blendif_channel_t Lab_channels[] = {{N_("L"),
                                                      N_("sliders for L channel"),
                                                      1.0f / 100.0f,
                                                      COLORSTOPS(_gradient_L),
                                                      TRUE,
                                                      0.0f,
                                                      ...
                                                      N_("lightness")},
                                                     {N_("a"),
                                                      N_("sliders for a channel"),
                                                      1.0f / 256.0f,
                                                      COLORSTOPS(_gradient_a),
                                                      TRUE,
                                                      0.0f,
                                                      {DEVELOP_BLENDIF_A_in, DEVELOP_BLENDIF_A_out},
                                                      DT_DEV_PIXELPIPE_DISPLAY_a,
                                                      _blendif_scale_print_ab,
                                                      _blendop_blendif_disp_alternative_mag,
                                                      N_("green/red")},
                                                     {N_("b"),
                                                      N_("sliders for b channel"),
                                                      1.0f / 256.0f,
                                                      COLORSTOPS(_gradient_b),
                                                      TRUE,
                                                      0.0f,
                                                      {DEVELOP_BLENDIF_B_in, DEVELOP_BLENDIF_B_out},
                                                      DT_DEV_PIXELPIPE_DISPLAY_b,
                                                      _blendif_scale_print_ab,
                                                      _blendop_blendif_disp_alternative_mag,
                                                      N_("blue/yellow")},
                                                     {N_("C"),
                                                      N_("sliders for chroma channel (of LCh)"),
                                                      1.0f / 100.0f,
                                                      COLORSTOPS(_gradient_chroma),
                                                      TRUE,
                                                      0.0f,
                                                      {DEVELOP_BLENDIF_C_in, DEVELOP_BLENDIF_C_out},
                                                      DT_DEV_PIXELPIPE_DISPLAY_LCH_C,
                                                      _blendif_scale_print_default,
                                                      _blendop_blendif_disp_alternative_log,
                                                      N_("saturation")},
                                                     {N_("h"),
                                                      N_("sliders for hue channel (of LCh)"),
                                                      1.0f / 360.0f,
...
const dt_iop_gui_blendif_channel_t rgb_channels[] = {
    {N_("g"),
     ...
     N_("gray")},
    {N_("R"),
     N_("sliders for red channel"),
     1.0f / 255.0f,
     COLORSTOPS(_gradient_red),
     TRUE,
     0.0f,
     ...
     N_("red")},
    {N_("G"),
     N_("sliders for green channel"),
     1.0f / 255.0f,
     COLORSTOPS(_gradient_green),
     TRUE,
     0.0f,
     ...
     N_("green")},
    {N_("B"),
     N_("sliders for blue channel"),
     1.0f / 255.0f,
     COLORSTOPS(_gradient_blue),
     TRUE,
     0.0f,
     ...
     N_("blue")},
    {N_("H"),
     N_("sliders for hue channel (of HSL)"),
     1.0f / 360.0f,
     ...
     N_("hue")},
    {N_("S"),
     N_("sliders for chroma channel (of HSL)"),
     1.0f / 100.0f,
     COLORSTOPS(_gradient_chroma),
     FALSE,
     0.0f,
     ...
     N_("chroma")},
    {N_("L"),
     N_("sliders for value channel (of HSL)"),
     1.0f / 100.0f,
     COLORSTOPS(_gradient_gray),
     FALSE,
     0.0f,
...
const dt_iop_gui_blendif_channel_t rgbj_channels[] = {
    ...
    {N_("B"),
     ...
     N_("blue")},
    {N_("Jz"),
     N_("sliders for value channel (of JzCzhz)"),
     1.0f / 100.0f,
     COLORSTOPS(_gradient_gray),
     TRUE,
     -6.64385619f, // cf. _blend_init_blendif_boost_parameters
     ...
     N_("luminance")},
    {N_("Cz"),
     N_("sliders for chroma channel (of JzCzhz)"),
     1.0f / 100.0f,
     COLORSTOPS(_gradient_chroma),
     TRUE,
     -6.64385619f, // cf. _blend_init_blendif_boost_parameters
     ...
     N_("chroma")},
    {N_("hz"),
     N_("sliders for hue channel (of JzCzhz)"),
     1.0f / 360.0f,
...
void dt_iop_gui_update_blendif(dt_iop_module_t *module)
{
    ...
    if (bd->channel_tabs_csp != bd->csp)
    {
        ...
        case DEVELOP_BLEND_CS_RGB_SCENE:
            ...
            break;
        default:
            assert(FALSE); // blendif not supported for RAW, which is
                           // already caught upstream; we should not get
                           // here
        }
...
void dt_iop_gui_init_blendif(GtkWidget *blendw, dt_iop_module_t *module)
{
    ...
    bd->blendif_box = GTK_BOX(dt_gui_vbox());
    ...
    {
        GtkWidget *section = dt_gui_hbox(dt_ui_label_new(_("parametric mask")));
        ...
        dt_action_define_iop(module, "blend", N_("channel"), GTK_WIDGET(bd->channel_tabs),
                             &dt_action_def_tabs_none);
        ...
        GtkWidget *header = dt_gui_hbox(dt_gui_expand(bd->channel_tabs), gtk_label_new(" "));
        bd->colorpicker =
            dt_color_picker_new(module, DT_COLOR_PICKER_POINT_AREA | DT_COLOR_PICKER_IO, header);
        gtk_widget_set_tooltip_text(bd->colorpicker,
                                    _("pick GUI color from image\n"
                                      "ctrl+click or right-click to select an area"));
        gtk_widget_set_name(bd->colorpicker, "keep-active");
        dt_action_define_iop(module, "blend`pickers", N_("show color"), bd->colorpicker,
        ...
        gtk_widget_set_tooltip_text(bd->colorpicker_set_values,
                                    _("set the range based on an area from the image\n"
                                      "drag to use the input image\n"
                                      "ctrl+drag to use the output image"));
        dt_action_define_iop(module, "blend`pickers", N_("set range"), bd->colorpicker_set_values,
                             &dt_action_def_toggle);

        GtkWidget *btn = dt_iop_togglebutton_new(
            module, "blend`tools", N_("invert all channel's polarities"), NULL,
            G_CALLBACK(_blendop_blendif_invert), FALSE, 0, 0, dtgtk_cairo_paint_invert, header);
        dt_gui_add_class(btn, "dt_ignore_fg_state");

        for (int in_out = 2; in_out--;)
        {
            ...
            sl->slider =
                DTGTK_GRADIENT_SLIDER_MULTIVALUE(dtgtk_gradient_slider_multivalue_new_with_name(
                    4, in_out ? "blend-upper" : "blend-lower"));
            ...
            gtk_widget_set_tooltip_text(sl->polarity,
                                        _("toggle polarity. best seen by enabling 'display mask'"));
            ...
            sl->box = GTK_BOX(
                dt_gui_vbox(label_box, dt_gui_hbox(dt_gui_expand(sl->slider), sl->polarity)));
        }

        bd->channel_boost_factor_slider =
            dt_bauhaus_slider_new_with_range(module, 0.0f, 18.0f, 0, 0.0f, 3);
        dt_bauhaus_slider_set_format(bd->channel_boost_factor_slider, _(" EV"));
        dt_bauhaus_widget_set_label(bd->channel_boost_factor_slider, N_("blend"),
                                    N_("boost factor"));
        ...
        dt_gui_box_add(bd->blendif_box, section, header, bd->filter[1].box, bd->filter[0].box,
                       bd->channel_boost_factor_slider);
...
void dt_iop_gui_update_masks(dt_iop_module_t *module)
{
    ...
    if (!bd || !bd->masks_support || !bd->masks_inited)
        return;
...
void dt_iop_gui_init_masks(GtkWidget *blendw, dt_iop_module_t *module)
{
    ...
    if (bd->masks_support)
    {
        ...
        bd->masks_polarity =
            dt_iop_togglebutton_new(module, "blend`tools", N_("toggle polarity of drawn mask"),
                                    NULL, G_CALLBACK(_blendop_masks_polarity_callback), FALSE, 0, 0,
                                    dtgtk_cairo_paint_plusminus, hbox);
        dtgtk_togglebutton_set_paint(DTGTK_TOGGLEBUTTON(bd->masks_polarity),
                                     dtgtk_cairo_paint_plusminus, 0, NULL);
        dt_gui_add_class(bd->masks_polarity, "dt_ignore_fg_state");

        GtkWidget *abox = dt_gui_hbox();
        bd->masks_edit = dt_iop_togglebutton_new(
            module, "blend`tools", N_("show and edit mask elements"),
        ...
        bd->masks_shapes[0] = dt_iop_togglebutton_new(module, "blend`shapes", N_("add gradient"),
                                                      ...
                                                      G_CALLBACK(_blendop_masks_add_shape), FALSE,
                                                      0, 0, dtgtk_cairo_paint_masks_gradient, abox);
        ...
        bd->masks_shapes[1] = dt_iop_togglebutton_new(
            ...
            G_CALLBACK(_blendop_masks_add_shape), FALSE, 0, 0, dtgtk_cairo_paint_masks_path, abox);
        ...
        bd->masks_shapes[2] = dt_iop_togglebutton_new(module, "blend`shapes", N_("add ellipse"),
                                                      ...
                                                      G_CALLBACK(_blendop_masks_add_shape), FALSE,
                                                      0, 0, dtgtk_cairo_paint_masks_ellipse, abox);
        ...
        bd->masks_shapes[3] = dt_iop_togglebutton_new(module, "blend`shapes", N_("add circle"),
                                                      ...
                                                      G_CALLBACK(_blendop_masks_add_shape), FALSE,
                                                      0, 0, dtgtk_cairo_paint_masks_circle, abox);
        ...
        bd->masks_shapes[4] = dt_iop_togglebutton_new(
            ...
            G_CALLBACK(_blendop_masks_add_shape), FALSE, 0, 0, dtgtk_cairo_paint_masks_brush, abox);
...
static void _raster_combo_populate(GtkWidget *w, dt_iop_module_t **m)
{
    ...
    for (GList *iter = darktable.develop->iop; iter; iter = g_list_next(iter))
    {
        ...
        while (g_hash_table_iter_next(&masks_iter, &key, &value))
        {
            ...
            entry->module = iop;
            entry->id = id;
            dt_bauhaus_combobox_add_full(w, modulename, DT_BAUHAUS_COMBOBOX_ALIGN_RIGHT, entry,
                                         free, TRUE);
...
static void _raster_value_changed_callback(GtkWidget *widget, dt_iop_module_t *module)
{
...
void dt_iop_gui_update_raster(dt_iop_module_t *module)
{
    ...
    if (!bd || !bd->masks_support || !bd->raster_inited)
        return;
...
static gboolean _raster_polarity_callback(GtkToggleButton *togglebutton, GdkEventButton *event,
                                          dt_iop_module_t *self)
{
    DT_GUARD_GUI_UPDATE(TRUE);
...
void dt_iop_gui_init_raster(GtkWidget *blendw, dt_iop_module_t *module)
{
    ...
    if (bd->masks_support)
    {
        GtkWidget *mbox = dt_gui_hbox(dt_ui_label_new(_("raster mask")));
        bd->raster_polarity = dt_iop_togglebutton_new(
            module, "blend`tools", N_("toggle polarity of raster mask"), NULL,
            G_CALLBACK(_raster_polarity_callback), FALSE, 0, 0, dtgtk_cairo_paint_plusminus, mbox);
        dt_gui_add_class(bd->raster_polarity, "dt_ignore_fg_state");
        dt_gui_add_class(mbox, "dt_section_label");
        ...
        bd->raster_box = GTK_BOX(dt_gui_vbox(mbox, bd->raster_combo));
...
static gboolean _add_blendmode_combo(GtkWidget *combobox, const dt_develop_blend_mode_t start,
                                     const dt_develop_blend_mode_t end)
{
    return dt_bauhaus_combobox_add_introspection(combobox, NULL, dt_develop_blend_mode_names, start,
                                                 end);
}
...
void dt_iop_gui_update_blending(dt_iop_module_t *module)
{
    ...
    if (!(module->flags() & IOP_FLAGS_SUPPORTS_BLENDING) || !bd || !bd->blend_inited)
        return;
    ...
    const dt_develop_mask_mode_t mode =
        g_list_index(bd->masks_modes, GUINT_TO_POINTER(bp->mask_mode));
    ...
    if (mode != DEVELOP_MASK_DISABLED)
    {
        GtkToggleButton *to_be_activated =
            GTK_TOGGLE_BUTTON(g_list_nth_data(bd->masks_modes_toggles, mode));
        gtk_toggle_button_set_active(to_be_activated, TRUE);
        bd->selected_mask_mode = GTK_WIDGET(to_be_activated);
    }
    else
    {
        bd->selected_mask_mode =
            g_list_nth_data(bd->masks_modes_toggles,
                            g_list_index(bd->masks_modes, GUINT_TO_POINTER(DEVELOP_MASK_DISABLED)));
    }
    ...
    dt_bauhaus_combobox_set_from_value(
        bd->masks_combine_combo, bp->mask_combine & (DEVELOP_COMBINE_INV | DEVELOP_COMBINE_INCL));
    ...
    bd->output_channels_shown =
        bd->output_channels_shown || _blendif_are_output_channels_used(bp, bd->csp);
    ...
    if (mask_enabled &&
        ((bd->masks_inited && mode_drawn) || (bd->blendif_inited && mode_parametric)))
    {
    ...
    gtk_widget_set_visible(GTK_WIDGET(bd->masks_modes_box), !module->hide_enable_button);
...
void dt_iop_gui_init_blending(GtkWidget *iopw, dt_iop_module_t *module)
{
    ...
    if (module->flags() & IOP_FLAGS_SUPPORTS_BLENDING)
    {
        ...
        but = dt_iop_togglebutton_new(module, "blend`masks", N_("off"), NULL,
                                      G_CALLBACK(_blendop_masks_modes_none_clicked), FALSE, 0, 0,
                                      dtgtk_cairo_paint_cancel, NULL);
        ...
        but = dt_iop_togglebutton_new(module, "blend`masks", N_("uniformly"), NULL,
                                      G_CALLBACK(_blendop_masks_modes_uni_toggled), FALSE, 0, 0,
                                      dtgtk_cairo_paint_masks_uniform, NULL);
        ...
        {
            but = dt_iop_togglebutton_new(module, "blend`masks", N_("drawn mask"), NULL,
                                          G_CALLBACK(_blendop_masks_modes_drawn_toggled), FALSE, 0,
                                          0, dtgtk_cairo_paint_masks_drawn, NULL);
            bd->masks_modes = g_list_append(
                bd->masks_modes, GUINT_TO_POINTER(DEVELOP_MASK_ENABLED | DEVELOP_MASK_MASK));
            bd->masks_modes_toggles = g_list_append(bd->masks_modes_toggles, GTK_WIDGET(but));
        }
        if (bd->blendif_support) //DEVELOP_MASK_ENABLED | DEVELOP_MASK_CONDITIONAL
        {
            but = dt_iop_togglebutton_new(module, "blend`masks", N_("parametric mask"), NULL,
                                          G_CALLBACK(_blendop_masks_modes_param_toggled), FALSE, 0,
                                          0, dtgtk_cairo_paint_masks_parametric, NULL);
            bd->masks_modes = g_list_append(
                bd->masks_modes, GUINT_TO_POINTER(DEVELOP_MASK_ENABLED | DEVELOP_MASK_CONDITIONAL));
            bd->masks_modes_toggles = g_list_append(bd->masks_modes_toggles, GTK_WIDGET(but));
        }
        ...
        {
            but =
                dt_iop_togglebutton_new(module, "blend`masks", N_("drawn & parametric mask"), NULL,
                                        G_CALLBACK(_blendop_masks_modes_both_toggled), FALSE, 0, 0,
                                        dtgtk_cairo_paint_masks_drawn_and_parametric, NULL);
            bd->masks_modes =
                g_list_append(bd->masks_modes, GUINT_TO_POINTER(DEVELOP_MASK_ENABLED |
        ...
        {
            but = dt_iop_togglebutton_new(module, "blend`masks", N_("raster mask"), NULL,
                                          G_CALLBACK(_blendop_masks_modes_raster_toggled), FALSE, 0,
                                          0, dtgtk_cairo_paint_masks_raster, NULL);
            bd->masks_modes = g_list_append(
                bd->masks_modes, GUINT_TO_POINTER(DEVELOP_MASK_ENABLED | DEVELOP_MASK_RASTER));
            bd->masks_modes_toggles = g_list_append(bd->masks_modes_toggles, GTK_WIDGET(but));
        }
        ...
        GtkWidget *gbox = dt_gui_hbox(dt_ui_label_new(_("blend mask")));
        dt_gui_add_class(gbox, "dt_section_label");

        bd->showmask =
            dt_iop_togglebutton_new(module, "blend`tools", N_("display mask and/or color channel"),
                                    NULL, G_CALLBACK(_blendop_blendif_showmask_clicked), FALSE, 0,
                                    0, dtgtk_cairo_paint_showmask, gbox);
        gtk_widget_set_tooltip_text(
            bd->showmask, _("display mask and/or color channel.\n"
        ...
        bd->suppress =
            dt_iop_togglebutton_new(module, "blend`tools", N_("temporarily switch off blend mask"),
                                    NULL, G_CALLBACK(_blendop_blendif_suppress_toggled), FALSE, 0,
                                    0, dtgtk_cairo_paint_eye_toggle, gbox);
        gtk_widget_set_tooltip_text(bd->suppress, _("temporarily switch off blend mask.\n"
        ...
        dt_action_t *ac =
            dt_bauhaus_widget_set_label(bd->blend_modes_combo, N_("blend"), N_("mode"));
        ...
        dt_gui_add_help_link(GTK_WIDGET(bd->blend_modes_combo), "masks_blending_op");

        bd->blend_modes_blend_order = dt_iop_togglebutton_new(
            module, "blend`tools", N_("toggle blend order"), NULL,
            G_CALLBACK(_blendop_blend_order_clicked), FALSE, 0, 0, dtgtk_cairo_paint_invert, NULL);
        gtk_widget_set_tooltip_text(
        ...
        dt_bauhaus_widget_set_field(bd->blend_mode_parameter_slider, &bp->blend_parameter,
        ...
        bd->masks_combine_combo = _combobox_new_from_list(
            module, N_("combine masks"), dt_develop_combine_masks_names, NULL,
        ...
        dt_gui_add_help_link(GTK_WIDGET(bd->masks_combine_combo), "masks_combined");
        ...
        gtk_widget_set_tooltip_text(
            bd->details_slider, _("adjust the threshold for the details mask (using raw data),\n"
        ...
        bd->masks_feathering_guide_combo =
            _combobox_new_from_list(module, N_("feathering guide"),
                                    dt_develop_feathering_guide_names, &bp->feathering_guide,
        ...
        dt_bauhaus_widget_set_field(bd->feathering_radius_slider, &bp->feathering_radius,
                                    DT_INTROSPECTION_TYPE_FLOAT);
        dt_bauhaus_widget_set_label(bd->feathering_radius_slider, N_("blend"),
                                    N_("feathering radius"));
        ...
        dt_bauhaus_widget_set_field(bd->blur_radius_slider, &bp->blur_radius,
                                    DT_INTROSPECTION_TYPE_FLOAT);
        ...
        dt_bauhaus_widget_set_field(bd->brightness_slider, &bp->brightness,
                                    DT_INTROSPECTION_TYPE_FLOAT);
        ...
        {
            dt_gui_box_add(bd->masks_modes_box, dt_gui_expand(l->data));
        }

        dt_gui_box_add(bd->masks_modes_box, presets_button);
        ...
        bd->blend_box = GTK_BOX(dt_gui_vbox(
            gbox, dt_gui_hbox(dt_gui_expand(bd->blend_modes_combo), bd->blend_modes_blend_order),
        ...
        bd->refine_box =
            GTK_BOX(dt_gui_vbox(hbox, bd->details_slider, bd->masks_feathering_guide_combo,
