...
#define ROTATION_RANGE_SOFT                                                                        \
    180                     // allowed min/max range for rotation parameter with manual adjustment
#define LENSSHIFT_RANGE 1.0 // allowed min/max default range for lensshift parameters
#define LENSSHIFT_RANGE_SOFT                                                                       \
    2.0                 // allowed min/max range for lensshift parameters with manual adjustment
...
#define LSD_SIGMA_SCALE                                                                            \
    0.6                 // LSD: sigma for Gaussian filter is computed as sigma = sigma_scale/scale
...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self, _("rotate or distort perspective"),
                                  _("corrective or creative"), _("linear, RGB, scene-referred"),
...
int flags()
{
    return IOP_FLAGS_ALLOW_TILING | IOP_FLAGS_TILING_FULL_ROI | IOP_FLAGS_ONE_INSTANCE |
           IOP_FLAGS_ALLOW_FAST_PIPE | IOP_FLAGS_GUIDES_SPECIAL_DRAW | IOP_FLAGS_GUIDES_WIDGET;
}
...
typedef enum dt_iop_ashift_linetype_t
{
    ...
    ASHIFT_LINE_VERTICAL_SELECTED =
        ASHIFT_LINE_RELEVANT | ASHIFT_LINE_DIRVERT | ASHIFT_LINE_SELECTED,
...
typedef enum dt_iop_ashift_fitaxis_t
{
    ...
    ASHIFT_FIT_BOTH_NO_ROTATION =
        ASHIFT_FIT_LENS_VERT | ASHIFT_FIT_LENS_HOR | ASHIFT_FIT_LINES_VERT | ASHIFT_FIT_LINES_HOR,
    ...
    ASHIFT_FIT_ROTATION_BOTH_LINES =
        ASHIFT_FIT_ROTATION | ASHIFT_FIT_LINES_VERT | ASHIFT_FIT_LINES_HOR,
    ASHIFT_FIT_FLIP =
        ASHIFT_FIT_LENS_VERT | ASHIFT_FIT_LENS_HOR | ASHIFT_FIT_LINES_VERT | ASHIFT_FIT_LINES_HOR
...
typedef struct dt_iop_ashift_params_t
{
    ...
    float
        lensshift_v; // $MIN: -LENSSHIFT_RANGE_SOFT $MAX: LENSSHIFT_RANGE_SOFT $DEFAULT: 0.0 $DESCRIPTION: "lens shift (vertical)"
    float
        lensshift_h; // $MIN: -LENSSHIFT_RANGE_SOFT $MAX: LENSSHIFT_RANGE_SOFT $DEFAULT: 0.0 $DESCRIPTION: "lens shift (horizontal)"
    float
        shear; // $MIN: -SHEAR_RANGE_SOFT $MAX: SHEAR_RANGE_SOFT $DEFAULT: 0.0 $DESCRIPTION: "shear"
    float
        f_length; // $MIN: 1.0 $MAX: 2000.0 $DEFAULT: DEFAULT_F_LENGTH $DESCRIPTION: "focal length"
    ...
    dt_iop_ashift_crop_t
        cropmode; // $DEFAULT: ASHIFT_CROP_LARGEST $DESCRIPTION: "automatic cropping"
...
static inline void _shadow_crop_box(const dt_iop_ashift_params_t *p, dt_iop_ashift_gui_data_t *g)
{
...
static inline void _commit_crop_box(dt_iop_ashift_params_t *p, const dt_iop_ashift_gui_data_t *g)
{
...
static void _homography(float *homograph, const float angle, const float shift_v,
                        const float shift_h, const float shear, const float f_length_kb,
                        const float orthocorr, const float aspect, const int width,
                        const int height, const dt_iop_ashift_homodir_t dir)
{
...
static inline gboolean _isneutral(const dt_iop_ashift_data_t *data)
{
    ...
    return (feqf(data->rotation, 0.0f, eps) && feqf(data->lensshift_v, 0.0f, eps) &&
            feqf(data->lensshift_h, 0.0f, eps) && feqf(data->shear, 0, eps) &&
            feqf(data->aspect, 1.0f, eps) && feqf(data->cl, 0.0f, eps) &&
            feqf(data->cr, 1.0f, eps) && feqf(data->ct, 0.0f, eps) && feqf(data->cb, 1.0f, eps));
}

gboolean distort_transform(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                           float *const restrict points, const size_t points_count)
{
    ...
    _homography((float *)homograph, data->rotation, data->lensshift_v, data->lensshift_h,
                data->shear, data->f_length_kb, data->orthocorr, data->aspect, piece->buf_in.width,
...
gboolean distort_backtransform(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece, float *points,
                  ...
                  const dt_iop_roi_t *const roi_out)
{
    ...
    for (int j = 0; j < roi_out->height; j++)
    {
        ...
        for (int i = 0; i < roi_out->width; i++)
        {
            ...
            _out[i] = CLIP(dt_interpolation_compute_sample(interpolation, in, pin[0], pin[1],
                    ...
                    const dt_iop_roi_t *roi_in)
{
    ...
    *roi_out = *roi_in;
    ...
    {
        dt_print_pipe(DT_DEBUG_PIPE, "insane data", piece->pipe, self, DT_DEVICE_NONE, roi_in,
                      roi_out);
        if (dt_pipe_is_full(piece->pipe))
        {
            dt_control_log(_("module '%s' has insane data so it is bypassed for now."
...
static void rgb2grey256(const float *const in, double *const out, const int width, const int height)
{
    ...
    {
        out[index] =
            (0.3f * in[4 * index + 0] + 0.59f * in[4 * index + 1] + 0.11f * in[4 * index + 2]) *
            256.0;
    }
}
                            ...
                            const dt_iop_ashift_enhance_t dir)
{
    ...
    // select kernel
    const double *kernel =
        (dir == ASHIFT_ENHANCE_HORIZONTAL) ? (const double *)hkernel : (const double *)vkernel;
    ...
    for (int j = 0; j < height; j++)
        for (int i = 0; i < width; i++)
        {
            ...
            if (i == khwidth && j >= khwidth && j < height - khwidth)
                i = width - khwidth;
        }
}

// edge enhancement in both directions
static gboolean edge_enhance(const double *in, double *out, const int width, const int height)
{
    double *Gx = NULL;
    double *Gy = NULL;
...
static gboolean detail_enhance(const float *const in, float *const out, const int width,
                               const int height)
{
...
static void gamma_correct(const float *const in, float *const out, const int width,
                          const int height)
{
    const size_t npixels = (size_t)width * height;
    DT_OMP_FOR()
    for (size_t index = 0; index < 4 * npixels; index += 4)
    {
        for_three_channels(c) out[index + c] = powf(in[index + c], LSD_GAMMA);
    }
}

// do actual line_detection based on LSD algorithm and return results according
// to this module's conventions
static gboolean line_detect(float *in, const int width, const int height, const int x_off,
                            const int y_off, const float scale, dt_iop_ashift_line_t **alines,
                            int *lcount, int *vcount, int *hcount, float *vweight, float *hweight,
                            const dt_iop_ashift_enhance_t enhance, const gboolean is_raw)
{
    ...
    lsd_lines = LineSegmentDetection(&lines_count, greyscale, width, height, LSD_SCALE,
                                     LSD_SIGMA_SCALE, LSD_QUANT, LSD_ANG_TH, LSD_LOG_EPS,
                                     LSD_DENSITY_TH, LSD_N_BINS, NULL, NULL, NULL);
    ...
    if (lines_count > 0)
    {
        ...
        for (int n = 0; n < lines_count; n++)
        {
            ...
            const float weight =
                ashift_lines[lct].length * ashift_lines[lct].width * lsd_lines[n * 7 + 5];
            ...
            const gboolean horizontal =
                fabsf(fabsf(fabsf(angle) - 90.0f) - 90.0f) < MAX_TANGENTIAL_DEVIATION;
    ...
    for (int n = 0; n < lct; n++)
    {
        ...
        printf("x1 %.0f, y1 %.0f, x2 %.0f, y2 %.0f,"
               ...
               ashift_lines[n].p2[1], ashift_lines[n].length, ashift_lines[n].width,
               ashift_lines[n].L[0], ashift_lines[n].L[1], ashift_lines[n].L[2],
               ashift_lines[n].type, vec3scalar(ashift_lines[n].p1, ashift_lines[n].L),
...
static int _get_structure(dt_iop_module_t *self, const dt_iop_ashift_enhance_t enhance)
{
                   ...
                   const int ymin, const int ymax)
{
    ...
    for (int r = 0; r < optiruns + riter; r++)
    {
        ...
        if (vec3isnull(V) || (fabsf(V[2]) > 0.0f && V[0] / V[2] >= xmin && V[1] / V[2] >= ymin &&
                              V[0] / V[2] <= xmax && V[1] / V[2] <= ymax))
        {
            // no valid model
            quality = 0.0f;
        }
            ...
            for (int n = 2; n < set_count; n++)
            {
                ...
                if (inout[n] == 1)
                {
                    ...
                    q = 0.33f / (float)set_count +
                        ...
                        0.33f * (1.0f - d / epsilon) * (float)set_count *
                            lines[index_set[n]].weight / total_weight;
                }
                else
                {
                    q = 0.0f;
        ...
        printf("ransac run %d: best qual %.6f, eps %.6f,"
               ...
               r, best_quality, epsilon, count, set_count, quality, lastcount,
...
static gboolean _remove_outliers(const dt_iop_module_t *self)
{
    ...
    if (vnb > 2)
        ransac(g->lines, lines_set, inout_set, vnb, g->vertical_weight, xmin, xmax, ymin, ymax);
    ...
    // it only makes sense to call ransac if we have more than two lines
    if (hnb > 2)
        ransac(g->lines, lines_set, inout_set, hnb, g->horizontal_weight, xmin, xmax, ymin, ymax);
...
static double model_fitness(double *params, void *data)
{
    ...
    const double v =
        weight_v > 0.0f && count > 0 ? sumsq_v / weight_v * (float)count_v / count : 0.0;

    const double h =
        weight_h > 0.0f && count > 0 ? sumsq_h / weight_h * (float)count_h / count : 0.0;
...
static dt_iop_ashift_nmsresult_t nmsfit(dt_iop_module_t *self, dt_iop_ashift_params_t *p,
                                        const dt_iop_ashift_fitaxis_t dir)
{
    ...
    fit.f_length_kb =
        (p->mode == ASHIFT_MODE_GENERIC) ? DEFAULT_F_LENGTH : p->f_length * p->crop_factor;
    ...
    const int iter = simplex(model_fitness, params, fit.params_count, NMS_EPSILON, NMS_SCALE,
                             NMS_ITERATIONS, NULL, (void *)&fit);
    ...
    fit.shear = dt_isnan(fit.shear) ? ilogit(params[pcount++], -fit.shear_range, fit.shear_range) :
                                      fit.shear;
    ...
    float DT_ALIGNED_ARRAY homograph[3][3];
    _homography((float *)homograph, fit.rotation, fit.lensshift_v, fit.lensshift_h, fit.shear,
                fit.f_length_kb, fit.orthocorr, fit.aspect, fit.width, fit.height,
                ASHIFT_HOMOGRAPH_FORWARD);
...
static void model_probe(dt_iop_module_t *self, dt_iop_ashift_params_t *p,
                        const dt_iop_ashift_fitaxis_t dir)
{
    dt_iop_ashift_gui_data_t *g = self->gui_data;
    ...
    printf("model fitness: %.8f (rotation %f, lensshift_v %f, lensshift_h %f, shear %f)\n", quality,
           p->rotation, p->lensshift_v, p->lensshift_h, p->shear);
}
#endif
...
static void crop_constraint(double *params, const int pcount)
{
    ...
    if (pcount > 0 && params[0] > 1.0)
        params[0] = 1.0 - params[0];
    if (pcount > 1 && params[1] > 1.0)
        params[1] = 1.0 - params[1];
    if (pcount > 2 && params[2] > 0.5 * M_PI)
        params[2] = 0.5 * M_PI - params[2];
}
...
static void do_crop(const dt_iop_module_t *self, dt_iop_ashift_params_t *p)
{
    ...
    _homography((float *)cropfit.homograph, rotation, lensshift_v, lensshift_h, shear, f_length_kb,
    ...
    // start the simplex fit
    const int iter = simplex(crop_fitness, params, pcount, NMS_CROP_EPSILON, NMS_CROP_SCALE,
                             NMS_CROP_ITERATIONS, crop_constraint, (void *)&cropfit);
    ...
    // final sanity check
    if (g->cr - g->cl <= 0.0f || g->cb - g->ct <= 0.0f)
        goto failed;
    ...
    dt_print(DT_DEBUG_PIPE | DT_DEBUG_VERBOSE,
             ...
             iter, cropfit.x, cropfit.y, cropfit.alpha, g->cl, g->cr, g->ct, g->cb, cropfit.width,
             cropfit.height);
...
static void crop_adjust(const dt_iop_module_t *self, const dt_iop_ashift_params_t *const p,
                        const float newx, const float newy)
{
    ...
    dt_print(DT_DEBUG_PIPE | DT_DEBUG_VERBOSE,
             ...
             0.5f * (g->cl + g->cr), 0.5f * (g->ct + g->cb), alpha, g->cl, g->cr, g->ct, g->cb,
             (int)wd, (int)ht);
    return;
}
...
static void _draw_basic_line(dt_iop_ashift_line_t *line, const float x1, const float y1,
                             const float x2, const float y2, const dt_iop_ashift_linetype_t type)
{
    // store as homogeneous coordinates
    line->p1[0] = x1;
    line->p1[1] = y1;
    ...
    if (g->current_structure_method == ASHIFT_METHOD_LINES && g->lines)
    {
        ...
        for (int i = 0; i < g->lines_count; i++)
        {
                ...
                g->lines[i].type == ASHIFT_LINE_VERTICAL_SELECTED)
            {
                ...
                if (p->last_drawn_lines_count >= MAX_SAVED_LINES)
                    break;
            }
        }
        if (dt_dev_distort_backtransform_plus(self->dev, self->dev->preview_pipe, self->iop_order,
                                                 ...
                                                 const dt_iop_ashift_method_t method)
{
    ...
    if (method == ASHIFT_METHOD_QUAD && p->last_quad_lines[0] > 0.0f &&
        p->last_quad_lines[1] > 0.0f && p->last_quad_lines[2] > 0.0f &&
        p->last_quad_lines[3] > 0.0f)
    {
        float pts[8] = {p->last_quad_lines[0], p->last_quad_lines[1], p->last_quad_lines[2],
                        p->last_quad_lines[3], p->last_quad_lines[4], p->last_quad_lines[5],
    ...
    if (method == ASHIFT_METHOD_LINES && p->last_drawn_lines_count > 0)
    {
                                          ...
                                          p->last_drawn_lines_count * 2))
        {
            ...
            for (int i = 0; i < p->last_drawn_lines_count; i++)
            {
                ...
                _draw_basic_line(&g->lines[i], pts[i * 4], pts[i * 4 + 1], pts[i * 4 + 2],
                                 pts[i * 4 + 3], linetype);
...
static gboolean _do_clean_structure(dt_iop_module_t *self, dt_iop_ashift_params_t *p,
...
static gboolean _do_get_structure_auto(dt_iop_module_t *self, dt_iop_ashift_params_t *p,
...
static void _do_get_structure_quad(dt_iop_module_t *self)
{
    ...
    else
    {
        ...
        float pts[8] = {wd * 0.2, ht * 0.2, wd * 0.2, ht * 0.8,
                                              ...
                                              DT_DEV_TRANSFORM_DIR_FORW_INCL, pts, 4))
        {
            ...
            for (int i = 0; i < 4; i++)
                _draw_retrieve_line_type(&g->lines[i]);
...
static void do_fit(dt_iop_module_t *self, dt_iop_ashift_params_t *p,
                   const dt_iop_ashift_fitaxis_t dir)
{
    dt_iop_ashift_gui_data_t *g = self->gui_data;
    ...
    if (g->lines == NULL)
        if (!_do_get_structure_auto(self, p, ASHIFT_ENHANCE_NONE))
            return;
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    if (g && self->dev->gui_attached && dt_pipe_is_preview(piece->pipe))
    {
        ...
        dt_boundingbox_t points = {0.0f, 0.0f, (float)piece->buf_in.width,
        ...
        const dt_hash_t hash = dt_dev_hash_plus(self->dev, self->dev->preview_pipe, self->iop_order,
    ...
    for (int j = 0; j < roi_out->height; j++)
    {
        ...
        for (int i = 0; i < roi_out->width; i++)
        {
            ...
            // get output values by interpolation from input image
            dt_interpolation_compute_pixel4c(interpolation, (float *)ivoid, out + ch * i, pin[0],
                                             pin[1], roi_in->width, roi_in->height, ch_width);
        }
    }
}
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    if (self->dev->gui_attached && g && dt_pipe_is_preview(piece->pipe))
    {
        ...
        {
            // copy data using actual roi_in dimensions (can exceed buf_in when scale > 1.0)
            err = dt_opencl_copy_image_to_host(devid, g->buf, dev_in, roi_in->width, roi_in->height,
                                               sizeof(float) * 4);
    ...
    if (_isneutral(d))
    {
        ...
        err = dt_opencl_enqueue_copy_image(devid, dev_in, dev_out, CLIMG_ORIGIN, CLIMG_ORIGIN,
                                           region);
        if (err != CL_SUCCESS)
            goto error;
        return CL_SUCCESS;
    ...
    float DT_ALIGNED_ARRAY ihomograph[3][3];
    _homography((float *)ihomograph, d->rotation, d->lensshift_v, d->lensshift_h, d->shear,
                d->f_length_kb, d->orthocorr, d->aspect, piece->buf_in.width, piece->buf_in.height,
                ASHIFT_HOMOGRAPH_INVERTED);
    ...
    err = dt_opencl_enqueue_kernel_2d_args(
        devid, ldkernel, roi_out->width, roi_out->height, CLARG(dev_in), CLARG(dev_out),
        CLARG(roi_out->width), CLARG(roi_out->height), CLARG(roi_in->width), CLARG(roi_in->height),
        CLARRAY(2, iroi), CLARRAY(2, oroi), CLARG(roi_in->scale), CLARG(roi_out->scale),
        CLARRAY(2, clip), CLARG(dev_homo));
...
static void _get_near(const float *points, dt_iop_ashift_points_idx_t *points_idx,
                      const int lines_count, const float pzx, const float pzy, const float delta,
                      const gboolean multiple)
{
    ...
    for (int n = 0; n < lines_count; n++)
    {
        ...
        if (pzx < points_idx[n].bbx - delta && pzx > points_idx[n].bbX + delta &&
            pzy < points_idx[n].bby - delta && pzy > points_idx[n].bbY + delta)
            continue;
...
static void _get_bounded_inside(const float *points, dt_iop_ashift_points_idx_t *points_idx,
                                const int points_lines_count, const float pzx, const float pzy,
                                const float pzx2, const float pzy2,
...
static dt_hash_t _get_lines_hash(const dt_iop_ashift_line_t *lines, const int lines_count)
{
    dt_hash_t hash = DT_INITHASH;
    for (int n = 0; n < lines_count; n++)
    {
        const uint32_t v[4] = {(uint32_t)lines[n].p1[0], (uint32_t)lines[n].p1[1],
                               (uint32_t)lines[n].p2[0], (uint32_t)lines[n].p2[1]};
...
static gboolean _update_colors(const dt_iop_module_t *self, dt_iop_ashift_points_idx_t *points_idx,
...
static gboolean _get_points(const dt_iop_module_t *self, const dt_iop_ashift_line_t *lines,
                            const int lines_count, const int lines_version, float **points,
                            float **extremas, dt_iop_ashift_points_idx_t **points_idx,
                            int *points_lines_count, const float scale)
{
    ...
    if (my_points == NULL || my_extremas == NULL)
        goto error;
    ...
    if (!dt_dev_distort_transform_plus(dev, dev->preview_pipe, self->iop_order,
                                       DT_DEV_TRANSFORM_DIR_FORW_INCL, my_points, total_points))
        goto error;
    if (!dt_dev_distort_transform_plus(dev, dev->preview_pipe, self->iop_order,
                                       DT_DEV_TRANSFORM_DIR_FORW_INCL, my_extremas,
                                       2 * lines_count))
        goto error;
...
/* this function replaces this sentence, it calls distort_transform()
   ...
   if(!dt_dev_distort_transform_plus(self->dev, self->dev->preview_pipe,
                                     ...
                                     (float *)V, 4))
*/
static int call_distort_transform(const dt_develop_t *dev, const dt_dev_pixelpipe_t *pipe,
                                  dt_iop_module_t *self, float *points, const size_t points_count)
{
    int ret = 0;
    dt_dev_pixelpipe_iop_t *piece =
        dt_dev_distort_get_iop_pipe(self->dev, self->dev->preview_pipe, self);
    ...
    if (piece->module == self
        ...
        && !(dt_iop_module_is_skipped(dev, piece->module) && dt_pipe_is_basic(pipe)))
    {
...
static float _calculate_straightening(const dt_iop_module_t *self, const float pzx, const float pzy,
                                      const float bzx, const float bzy, const float wd,
                                      const float ht, const float zoom_scale)
{
...
void gui_post_expose(dt_iop_module_t *self, cairo_t *cr, const float wd, const float ht,
                     const float pzx, const float pzy, const float zoom_scale)
{
    dt_develop_t *dev = self->dev;
    dt_iop_ashift_gui_data_t *g = self->gui_data;
    ...
    if (g->buf && self->enabled && dt_iop_has_focus(self))
    {
        ...
        const float V[4][2] = {
            {ixo, iyo}, {ixo, iyo + iht}, {ixo + iwd, iyo + iht}, {ixo + iwd, iyo}};
    ...
    if (g->straightening)
    {
        ...
        if (angle != 0.0f)
        {
            ...
            dt_gui_draw_rounded_rectangle(cr, text_w + 2 * margin, text_h + 2 * margin, xp - margin,
                                          yp - margin);
    ...
    if (g->lines == NULL || !dt_iop_has_focus(self))
        return;

    // get hash value that reflects distortions from here to the end of the pixelpipe
    const dt_hash_t hash = dt_dev_hash_distort_plus(dev, dev->preview_pipe, (double)self->iop_order,
        ...
        (g->lines_version > g->points_version && g->lines_hash != lines_hash))
    {
        ...
        if (!_get_points(self, g->lines, g->lines_count, g->lines_version, &g->points,
                         &g->draw_points, &g->points_idx, &g->points_lines_count, 1.0f))
            return;
    ...
    // a final check
    if (g->points == NULL || g->points_idx == NULL)
        return;
        ...
        g->draw_points)
    {
        ...
        const int nb =
            (g->current_structure_method == ASHIFT_METHOD_LINES) ? g->lines_count * 2 : 4;
        ...
        for (int i = 0; i < nb; i++)
        {
            ...
            if (g->draw_near_point == i)
                ...
                cairo_set_line_width(cr, DT_PIXEL_APPLY_DPI(2.0) * lwidth);
            cairo_arc(cr, g->draw_points[i * 2], g->draw_points[i * 2 + 1],
                      DT_PIXEL_APPLY_DPI(5.0) / zoom_scale, 0, 2.0 * M_PI);
            cairo_stroke(cr);
        }
    }
...
static int _draw_near_point(const float x, const float y, const float zoom_scale,
                            const float *points, const int limit)
{
    const float delta = DT_PIXEL_APPLY_DPI(6) / zoom_scale;

    for (int i = 0; i < limit; i++)
    {
        if (x - points[i * 2] < delta && x - points[i * 2] > -delta &&
            y - points[i * 2 + 1] < delta && y - points[i * 2 + 1] > -delta)
            return i;
    }
    return -1;
}
...
int mouse_moved(dt_iop_module_t *self, const float pzx, const float pzy, const double pressure,
                const int which, const float zoom_scale)
{
    dt_iop_ashift_gui_data_t *g = self->gui_data;
         ...
         g->current_structure_method == ASHIFT_METHOD_LINES))
    {
        ...
        g->draw_near_point =
            _draw_near_point(pzx * wd, pzy * ht, zoom_scale, g->draw_points, limit);
    }
    ...
    if (g->isbounding != ASHIFT_BOUNDING_OFF)
    {
        if (wd >= 1.0 && ht >= 1.0)
        {
            ...
            _get_bounded_inside(g->points, g->points_idx, g->points_lines_count, pzx * wd, pzy * ht,
                                g->lastx * wd, g->lasty * ht, g->isbounding);
        }
    ...
    // gather information about "near"-ness in g->points_idx
    _get_near(g->points, g->points_idx, g->points_lines_count, pzx * wd, pzy * ht, g->near_delta,
              !(g->current_structure_method == ASHIFT_METHOD_LINES ||
                g->current_structure_method == ASHIFT_METHOD_QUAD));
...
int button_pressed(dt_iop_module_t *self, const float pzx, const float pzy, const double pressure,
                   const int which, const int type, const uint32_t state, const float zoom_scale)
{
    ...
    float wd, ht;
    if (!dt_dev_get_preview_size(self->dev, &wd, &ht))
        return 1;
    ...
    if (dt_modifier_is(state, GDK_SHIFT_MASK))
    {
        ...
        g->isbounding =
            (which == GDK_BUTTON_SECONDARY) ? ASHIFT_BOUNDING_DESELECT : ASHIFT_BOUNDING_SELECT;
        dt_control_change_cursor("crosshair");
        ...
        for (int n = 0; n < g->points_lines_count; n++)
        {
            if (g->points_idx[n].is_near)
            {
                ...
                dt_dev_distort_backtransform_plus(self->dev, self->dev->preview_pipe,
                                                  self->iop_order, DT_DEV_TRANSFORM_DIR_FORW_INCL,
                                                  pts, 1);
                g->draw_line_move = n;
                g->draw_pointmove_x = pts[0];
                g->draw_pointmove_y = pts[1];
    ...
    if (!handled && g->current_structure_method == ASHIFT_METHOD_LINES &&
        which == GDK_BUTTON_PRIMARY)
    {
        ...
        g->lines = lines;
        g->lines_count = count;
        _draw_basic_line(&g->lines[count - 1], pts[0], pts[1], pts[0], pts[1],
                         ASHIFT_LINE_VERTICAL_SELECTED);
...
int button_released(dt_iop_module_t *self, const float pzx, const float pzy, const int which,
                    const uint32_t state, const float zoom_scale)
{
    dt_iop_ashift_gui_data_t *g = self->gui_data;
    float wd, ht;
    dt_dev_get_preview_size(self->dev, &wd, &ht);
    ...
    if (g->isbounding != ASHIFT_BOUNDING_OFF && dt_modifier_is(state, GDK_SHIFT_MASK))
    {
        ...
        if (wd >= 1.0 && ht >= 1.0)
        {
            ...
            for (int n = 0;
                 g->selecting_lines_version == g->lines_version && n < g->points_lines_count; n++)
            {
                if (g->points_idx[n].bounded == 0)
                    continue;
            ...
            if (handled)
            {
                _update_lines_count(g->lines, g->lines_count, &g->vertical_count,
                                    &g->horizontal_count);
                g->lines_version++;
                g->selecting_lines_version++;
            }
...
int scrolled(dt_iop_module_t *self, const float pzx, const float pzy, const int up,
             const uint32_t state)
{
    ...
    if (g->near_delta > 0 && (g->isdeselecting || g->isselecting))
    {
        ...
        // gather information about "near"-ness in g->points_idx
        _get_near(g->points, g->points_idx, g->points_lines_count, pzx * wd, pzy * ht,
                  g->near_delta, TRUE);
...
static int _event_fit_v_button_clicked(GtkWidget *widget, const GdkEventButton *event,
...
static int _event_fit_h_button_clicked(GtkWidget *widget, const GdkEventButton *event,
                                       dt_iop_module_t *self)
{
    DT_GUARD_GUI_UPDATE(FALSE);
...
static int _event_fit_both_button_clicked(GtkWidget *widget, const GdkEventButton *event,
                                          dt_iop_module_t *self)
{
    DT_GUARD_GUI_UPDATE(FALSE);
...
static int _event_structure_auto_clicked(GtkWidget *widget, const GdkEventButton *event,
                                         dt_iop_module_t *self)
{
    DT_GUARD_GUI_UPDATE(FALSE);
                   ...
                   dt_dev_pixelpipe_iop_t *piece)
{
    ...
    if (dt_iop_has_focus(self) || dt_isnan(p->cl) || dt_isnan(p->cr) || dt_isnan(p->ct) ||
        dt_isnan(p->cb))
    {
...
void reload_defaults(dt_iop_module_t *self)
{
    ...
    if (self->dev)
    {
        ...
        crop_factor =
            (img->exif_crop > 0.0f && img->exif_crop <= 1000.0f) ? img->exif_crop : crop_factor;
    }
...
static int _event_structure_quad_clicked(GtkWidget *widget, GdkEventButton *event,
                                         dt_iop_module_t *self)
{
    dt_iop_ashift_gui_data_t *g = self->gui_data;
    DT_GUARD_GUI_UPDATE(FALSE);
...
static int _event_structure_lines_clicked(GtkWidget *widget, GdkEventButton *event,
                                          dt_iop_module_t *self)
{
    dt_iop_ashift_gui_data_t *g = self->gui_data;
    DT_GUARD_GUI_UPDATE(FALSE);
...
void gui_init(dt_iop_module_t *self)
{
    ...
    dt_gui_new_collapsible_section(&g->cs, "plugins/darkroom/ashift/expand_values",
                                   _("manual perspective"), GTK_BOX(main_box), DT_ACTION(self));
    ...
    gtk_widget_set_tooltip_text(g->orthocorr,
                                _("the level of lens dependent correction, set to maximum"
    ...
    gtk_widget_set_tooltip_text(g->fit_v,
                                _("automatically correct for vertical perspective distortion\n"
                                  "ctrl+click to only fit rotation\n"
                                  "shift+click to only fit lens shift"));
    gtk_widget_set_tooltip_text(g->fit_h,
                                _("automatically correct for horizontal perspective distortion\n"
                                  "ctrl+click to only fit rotation\n"
                                  "shift+click to only fit lens shift"));
    gtk_widget_set_tooltip_text(g->fit_both,
                                _("automatically correct for vertical and "
    ...
    gtk_widget_set_tooltip_text(g->structure_auto,
                                _("automatically analyse line structure in image\n"
    ...
    g_signal_connect(G_OBJECT(g->fit_v), "button-press-event",
                     G_CALLBACK(_event_fit_v_button_clicked), (gpointer)self);
    g_signal_connect(G_OBJECT(g->fit_h), "button-press-event",
                     G_CALLBACK(_event_fit_h_button_clicked), (gpointer)self);
    g_signal_connect(G_OBJECT(g->fit_both), "button-press-event",
                     G_CALLBACK(_event_fit_both_button_clicked), (gpointer)self);
    g_signal_connect(G_OBJECT(g->structure_quad), "button-press-event",
                     G_CALLBACK(_event_structure_quad_clicked), (gpointer)self);
    g_signal_connect(G_OBJECT(g->structure_lines), "button-press-event",
                     G_CALLBACK(_event_structure_lines_clicked), (gpointer)self);
    g_signal_connect(G_OBJECT(g->structure_auto), "button-press-event",
                     G_CALLBACK(_event_structure_auto_clicked), (gpointer)self);
    ...
    dt_action_define_iop(self, N_("structure"), N_("rectangle"), g->structure_quad,
                         &dt_action_def_toggle);
    dt_action_define_iop(self, N_("structure"), N_("lines"), g->structure_lines,
                         &dt_action_def_toggle);
    dt_action_define_iop(self, N_("structure"), N_("auto"), g->structure_auto,
                         &dt_action_def_toggle);
...
GSList *mouse_actions(dt_iop_module_t *self)
{
    ...
    lm = dt_mouse_action_create_format(lm, DT_MOUSE_ACTION_RIGHT_DRAG, 0,
                                       _("[%s] define/rotate horizon"), self->name());

    lm = dt_mouse_action_create_format(lm, DT_MOUSE_ACTION_LEFT, 0,
                                       _("[%s on segment] select segment"), self->name());
