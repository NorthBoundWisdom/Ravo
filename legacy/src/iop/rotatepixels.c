...
DT_OMP_DECLARE_SIMD()
static void transform(const dt_dev_pixelpipe_iop_t *const piece, const float scale,
                      const float *const x, float *o)
{
...
DT_OMP_DECLARE_SIMD()
static void backtransform(const dt_dev_pixelpipe_iop_t *const piece, const float scale,
                          const float *const x, float *o)
{
    const dt_iop_rotatepixels_data_t *d = piece->data;
                   ...
                   const dt_iop_roi_t *const roi_out, dt_iop_roi_t *roi_in)
{
    *roi_in = *roi_out;
    ...
    const dt_boundingbox_t aabb = {roi_out->x, roi_out->y, roi_out->x + roi_out->width,
                                   roi_out->y + roi_out->height};
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    for (int j = 0; j < roi_out->height; j++)
    {
        ...
        for (int i = 0; i < roi_out->width; i++, out += ch)
        {
            ...
            dt_interpolation_compute_pixel4c(interpolation, (float *)ivoid, out, po[0], po[1],
                                             roi_in->width, roi_in->height, ch_width);
        }
    }
}
