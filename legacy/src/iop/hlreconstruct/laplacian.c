...
static void _interpolate_and_mask(const float *const restrict input,
                                  ...
                                  const dt_aligned_pixel_t clips, const dt_aligned_pixel_t wb,
                                  const uint32_t filters, const size_t width, const size_t height)
{
    ...
    for (size_t i = 0; i < height; i++)
        for (size_t j = 0; j < width; j++)
        {
            ...
            const dt_aligned_pixel_t clipped = {
                (float)R_clipped, (float)G_clipped, (float)B_clipped,
                (R_clipped || G_clipped || B_clipped) ? 1.0f : 0.0f};
...
static void _remosaic_and_replace(const float *const restrict input,
                                  ...
                                  float *const restrict output, const dt_aligned_pixel_t wb,
                                  const uint32_t filters, const size_t width, const size_t height)
{
...
static inline void guide_laplacians(const float *const restrict high_freq,
                                    ...
                                    const size_t height, const int mult, const float noise_level,
                                    const int salt, const unsigned int scale, const float radius_sq)
{
    ...
    for (size_t row = 0; row < height; ++row)
    {
        ...
        for (int j = 0; j < width; ++j)
        {
            ...
            dt_aligned_pixel_t high_frequency = {HF[index + 0], HF[index + 1], HF[index + 2],
                                                 HF[index + 3]};
            ...
            if (alpha > 0.f) // reconstruct
            {
                ...
                {
                    neighbour_pixel_HF[3 * 0 + 0][c] =
                        HF[4 * (i_neighbours[0] + j_neighbours[0]) + c];
                    neighbour_pixel_HF[3 * 0 + 1][c] =
                        HF[4 * (i_neighbours[0] + j_neighbours[1]) + c];
                    neighbour_pixel_HF[3 * 0 + 2][c] =
                        HF[4 * (i_neighbours[0] + j_neighbours[2]) + c];

                    neighbour_pixel_HF[3 * 1 + 0][c] =
                        HF[4 * (i_neighbours[1] + j_neighbours[0]) + c];
                    neighbour_pixel_HF[3 * 1 + 1][c] =
                        HF[4 * (i_neighbours[1] + j_neighbours[1]) + c];
                    neighbour_pixel_HF[3 * 1 + 2][c] =
                        HF[4 * (i_neighbours[1] + j_neighbours[2]) + c];

                    neighbour_pixel_HF[3 * 2 + 0][c] =
                        HF[4 * (i_neighbours[2] + j_neighbours[0]) + c];
                    neighbour_pixel_HF[3 * 2 + 1][c] =
                        HF[4 * (i_neighbours[2] + j_neighbours[1]) + c];
                    neighbour_pixel_HF[3 * 2 + 2][c] =
                        HF[4 * (i_neighbours[2] + j_neighbours[2]) + c];
                }
                ...
                for (size_t k = 0; k < 9; k++)
                    for_each_channel(
                        c, aligned(variance_HF, covariance_HF, neighbour_pixel_HF, means_HF))
                    {
                        covariance_HF[c] += (neighbour_pixel_HF[k][c] - means_HF[c]) *
                                            (neighbour_pixel_HF[k][guiding_channel_HF] -
                                             means_HF[guiding_channel_HF]) /
                                            9.f;
                    }

                const float scale_multiplier = 1.f / radius_sq;
                const dt_aligned_pixel_t alpha_ch = {
                    clipping_mask[index + RED], clipping_mask[index + GREEN],
                    clipping_mask[index + BLUE], clipping_mask[index + ALPHA]};

                dt_aligned_pixel_t a_HF, b_HF;
                for_each_channel(c, aligned(out, neighbour_pixel_HF, a_HF, b_HF, covariance_HF,
                                            variance_HF, means_HF, high_frequency, alpha_ch))
                {
                    ...
                    high_frequency[c] =
                        alpha_ch[c] * scale_multiplier *
                            (a_HF[c] * high_frequency[guiding_channel_HF] + b_HF[c]) +
            ...
            if (scale & FIRST_SCALE)
            {
                ...
                for_each_channel(c, aligned(out, high_frequency : 64)) out[index + c] =
                    high_frequency[c];
            }
            else
            {
            ...
            if ((scale & LAST_SCALE) && salt && alpha > 0.f)
            {
                ...
                dt_noise_generator_simd(DT_NOISE_POISSONIAN, out + index, sigma, flip, state,
                                        noise);
            ...
            if (scale & LAST_SCALE)
            {
                ...
                const float norm = fmaxf(
                    sqrtf(sqf(out[index + RED]) + sqf(out[index + GREEN]) + sqf(out[index + BLUE])),
                    1e-6f);
...
static inline void heat_PDE_diffusion(const float *const restrict high_freq,
                                      ...
                                      const float *const restrict clipping_mask,
                                      float *const restrict output, const size_t width,
                                      const size_t height, const int mult, const uint8_t scale,
                                      const float first_order_factor)
{
    ...
    for (size_t row = 0; row < height; ++row)
    {
        ...
        for (size_t j = 0; j < width; ++j)
        {
            ...
            // fetch the clipping mask opacity : opaque (alpha = 100 %) where clipped
            const dt_aligned_pixel_t alpha = {
                clipping_mask[index + RED], clipping_mask[index + GREEN],
                clipping_mask[index + BLUE], clipping_mask[index + ALPHA]};
            ...
            if (alpha[ALPHA] > 0.f) // reconstruct
            {
                ...
                for (int k = 0; k < 9; k++)
                {
                    for_each_channel(c, aligned(laplacian_HF, neighbour_pixel_HF : 16)
                                            aligned(anisotropic_kernel_isophote : 64))
                        laplacian_HF[c] +=
                        neighbour_pixel_HF[k][c] * anisotropic_kernel_isophote[k];
                }

                // Diffuse
                const dt_aligned_pixel_t multipliers_HF = {1.f / B_SPLINE_TO_LAPLACIAN,
                                                           1.f / B_SPLINE_TO_LAPLACIAN,
                                                           1.f / B_SPLINE_TO_LAPLACIAN, 0.f};
                for_each_channel(c, aligned(high_frequency, multipliers_HF, laplacian_HF, alpha))
                    high_frequency[c] += alpha[c] * multipliers_HF[c] *
                                         (laplacian_HF[c] - first_order_factor * high_frequency[c]);
            ...
            if ((scale & LAST_SCALE))
            {
                ...
                if (alpha[ALPHA] > 0.f)
                {
                    const float norm = sqrtf(sqf(out[index + RED]) + sqf(out[index + GREEN]) +
                                             sqf(out[index + BLUE]));
                    for_each_channel(c, aligned(out, LF, high_frequency : 64)) out[index + c] /=
...
static inline gint
wavelets_process(const float *const restrict in, float *const restrict reconstructed,
                 const float *const restrict clipping_mask, const size_t width, const size_t height,
                 const int scales, float *const restrict HF, float *const restrict LF_odd,
                 float *const restrict LF_even, const diffuse_reconstruct_variant_t variant,
                 const float noise_level, const int salt, const float first_order_factor)
{
    gint success = TRUE;
    ...
    for (int s = 0; s < scales; ++s)
    {
        ...
        if (variant == DIFFUSE_RECONSTRUCT_RGB)
            guide_laplacians(HF, buffer_out, clipping_mask, reconstructed, width, height, mult,
                             noise_level, salt, current_scale_type, radius);
        else
...
static void process_laplacian_bayer(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                                    ...
                                    const dt_aligned_pixel_t clips)
{
    ...
    if (!dt_iop_alloc_image_buffers(self, roi_in, roi_out, 4 | DT_IMGSZ_INPUT, &interpolated,
                                    4 | DT_IMGSZ_INPUT, &clipping_mask, 0, NULL))
    {
    ...
    if (!dt_iop_alloc_image_buffers(self, &roi_ds, &roi_ds, 4 | DT_IMGSZ_INPUT, &LF_odd,
                                    4 | DT_IMGSZ_INPUT, &LF_even, 4 | DT_IMGSZ_INPUT, &temp,
                                    4 | DT_IMGSZ_INPUT, &HF, 4 | DT_IMGSZ_INPUT, &ds_interpolated,
                                    4 | DT_IMGSZ_INPUT, &ds_clipping_mask, 0, NULL))
    {
    ...
    for (int i = 0; i < data->iterations; i++)
    {
        ...
        wavelets_process(ds_interpolated, temp, ds_clipping_mask, ds_width, ds_height, scales, HF,
                         LF_odd, LF_even, DIFFUSE_RECONSTRUCT_RGB, noise_level, salt,
                         data->solid_color);
        wavelets_process(temp, ds_interpolated, ds_clipping_mask, ds_width, ds_height, scales, HF,
...
static inline cl_int wavelets_process_cl(const int devid, cl_mem in, cl_mem reconstructed,
                                         cl_mem clipping_mask, const int width, const int height,
                                         dt_iop_highlights_global_data_t *const gd,
                                         const int scales, cl_mem HF, cl_mem LF_odd, cl_mem LF_even,
                                         ...
                                         const float solid_color)
{
    ...
    for (int s = 0; s < scales; ++s)
    {
        ...
        err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_filmic_wavelets_detail, width,
                                               height, CLARG(buffer_in), CLARG(buffer_out),
        ...
        if (variant == DIFFUSE_RECONSTRUCT_RGB)
        {
            err = dt_opencl_enqueue_kernel_2d_args(
                devid, gd->kernel_highlights_guide_laplacians, width, height, CLARG(HF),
                CLARG(buffer_out), CLARG(clipping_mask),
                CLARG(reconstructed), // read-only
                CLARG(reconstructed), // write-only
                CLARG(width), CLARG(height), CLARG(mult), CLARG(noise_level), CLARG(salt),
                CLARG(current_scale_type), CLARG(radius));
            if (err != CL_SUCCESS)
                return err;
        }
        else // DIFFUSE_RECONSTRUCT_CHROMA
        {
            err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_highlights_diffuse_color,
                                                   width, height, CLARG(HF), CLARG(buffer_out),
                                                   CLARG(clipping_mask),
                                                   CLARG(reconstructed), // read-only
                                                   CLARG(reconstructed), // write-only
                                                   CLARG(width), CLARG(height), CLARG(mult),
                                                   CLARG(current_scale_type), CLARG(solid_color));
            if (err != CL_SUCCESS)
                return err;
        }
    }
...
static cl_int process_laplacian_bayer_cl(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                                         ...
                                         const dt_aligned_pixel_t clips)
{
    ...
    cl_mem interpolated = dt_opencl_alloc_device(
        devid, sizes[0], sizes[1], sizeof(float) * 4); // [R, G, B, norm] for each pixel
    cl_mem clipping_mask = dt_opencl_alloc_device(
        devid, sizes[0], sizes[1], sizeof(float) * 4); // [R, G, B, norm] for each pixel
    ...
    cl_mem temp = dt_opencl_alloc_device(devid, sizes[0], sizes[1],
                                         sizeof(float) * 4); // need full size here for blurring

    // wavelets scales buffers
    cl_mem HF = dt_opencl_alloc_device(devid, ds_sizes[0], ds_sizes[1], sizeof(float) * 4);
    cl_mem ds_interpolated =
        dt_opencl_alloc_device(devid, ds_sizes[0], ds_sizes[1], sizeof(float) * 4);
    cl_mem ds_clipping_mask =
        dt_opencl_alloc_device(devid, ds_sizes[0], ds_sizes[1], sizeof(float) * 4);

    cl_mem clips_cl =
        dt_opencl_copy_host_to_device_constant(devid, 4 * sizeof(float), (float *)clips);
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_highlights_bilinear_and_mask, width,
                                           height, CLARG(dev_in), CLARG(interpolated), CLARG(temp),
                                           CLARG(clips_cl), CLARG(wb_cl), CLARG(filters),
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_highlights_box_blur, width, height,
                                           CLARG(temp), CLARG(clipping_mask), CLARG(roi_out->width),
    ...
    // Downsample
    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_interpolate_bilinear, ds_width, ds_height, CLARG(clipping_mask),
        CLARG(width), CLARG(height), CLARG(ds_clipping_mask), CLARG(ds_width), CLARG(ds_height));
    if (err != CL_SUCCESS)
        goto error;

    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_interpolate_bilinear, ds_width, ds_height, CLARG(interpolated),
        CLARG(width), CLARG(height), CLARG(ds_interpolated), CLARG(ds_width), CLARG(ds_height));
    ...
    {
        const int salt = (i == data->iterations - 1); // add noise on the last iteration only
        err = wavelets_process_cl(devid, ds_interpolated, temp, ds_clipping_mask, ds_width,
                                  ds_height, gd, scales, HF, LF_odd, LF_even,
                                  DIFFUSE_RECONSTRUCT_RGB, noise_level, salt, data->solid_color);
        if (err != CL_SUCCESS)
            goto error;

        err = wavelets_process_cl(devid, temp, ds_interpolated, ds_clipping_mask, ds_width,
                                  ds_height, gd, scales, HF, LF_odd, LF_even,
    ...
    // Upsample
    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_interpolate_bilinear, width, height, CLARG(ds_interpolated),
        CLARG(ds_width), CLARG(ds_height), CLARG(interpolated), CLARG(width), CLARG(height));
    if (err != CL_SUCCESS)
        goto error;

    // Remosaic
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_highlights_remosaic_and_replace, width,
                                           height, CLARG(dev_in), CLARG(interpolated),
                                           CLARG(clipping_mask), CLARG(dev_out), CLARG(wb_cl),
                                           CLARG(filters), CLARG(width), CLARG(height));
