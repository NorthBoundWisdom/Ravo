...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self,
                                  _("convert pipeline reference RGB to any display RGB\n"
                                    "using color profiles to remap RGB values"),
                                  _("mandatory"), _("linear or non-linear, Lab, display-referred"),
...
int flags()
{
    return IOP_FLAGS_ALLOW_TILING | IOP_FLAGS_ONE_INSTANCE | IOP_FLAGS_WRITE_PIPECACHE_IN;
}
...
static void _output_profile_changed(GtkWidget *widget, dt_iop_module_t *self)
{
    ...
    dt_print(DT_DEBUG_ALWAYS, "[colorout] color profile %s seems to have disappeared!",
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_colorout, width, height, CLARG(dev_in), CLARG(dev_out), CLARG(width),
        CLARG(height), CLARG(dev_m), CLARG(dev_r), CLARG(dev_g), CLARG(dev_b), CLARG(dev_coeffs));
...
static void _process_fastpath_apply_tonecurves(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
    ...
    if (dt_is_valid_colormatrix(d->cmatrix[0][0]))
    {
        ...
        { // apply profile
            ...
            for (size_t k = 0; k < 4 * npixels; k += 4)
            {
                for (int c = 0; c < 3; c++)
                {
                    out[k + c] = (out[k + c] < 1.0f) ?
                                     _lerp_lut(d->lut[c], out[k + c]) :
...
static void _transform_cmatrix_linear(const dt_iop_colorout_data_t *const d, float *restrict out,
                                      const float *restrict in, const size_t npixels)
{
    dt_colormatrix_t cmatrix;
    ...
    for (size_t k = 0; k < npixels; k++)
    {
        ...
        for_each_channel(r) rgb[r] =
            cmatrix_0[r] * XYZ[0] + cmatrix_1[r] * XYZ[1] + cmatrix_2[r] * XYZ[2];
        copy_pixel_nontemporal(out + 4 * k, rgb);
    }
    dt_omploop_sfence();
}

static void _transform_cmatrix_tonecurve(const dt_iop_colorout_data_t *const d, float *restrict out,
                                         const float *restrict in, const size_t npixels)
{
    dt_colormatrix_t cmatrix;
    transpose_3xSSE(d->cmatrix, cmatrix);
    ...
    {
        dt_aligned_pixel_t
            rgb; // using an aligned temporary variable lets the compiler optimize away interm. writes
        dt_Lab_to_linearRGB(in + 4 * k, cmatrix_0, cmatrix_1, cmatrix_2, rgb);
        if (lut[0] >= 0.0f)
        {
            rgb[0] = (rgb[0] < 1.0f) ? _lerp_lut(lut, rgb[0]) : dt_iop_eval_exp(coeffs, rgb[0]);
        }
...
static int _transform_cmatrix(const dt_iop_colorout_data_t *const d, float *restrict out,
                              const float *restrict in, const size_t npixels)
{
    ...
    if (is_linear ||
        1) //TODO: integrate tonecurve in same pass as color matrix without major speed penalty
    {
...
static void _transform_lcms(const dt_iop_colorout_data_t *const d, float *restrict out,
                            const float *restrict in, const size_t npixels)
{
                   ...
                   dt_dev_pixelpipe_iop_t *piece)
{
    ...
    else if (dt_pipe_is_thumb(pipe))
    {
        ...
        out_filename =
            (out_type == DT_COLORSPACE_DISPLAY ? darktable.color_profiles->display_filename : "");
    ...
    {
        dt_ioppr_set_pipe_output_profile_info(self->dev, piece->pipe, d->type, out_filename,
                                              p->intent);
        return;
    }
   ...
   */
    ...
    const dt_colorspaces_color_profile_t *out_profile = dt_colorspaces_get_profile(
        out_type, out_filename,
        DT_PROFILE_DIRECTION_OUT | DT_PROFILE_DIRECTION_DISPLAY | DT_PROFILE_DIRECTION_DISPLAY2);
    if (out_profile)
    {
        output = out_profile->profile;
        if (out_type == DT_COLORSPACE_XYZ)
            output_format = TYPE_XYZA_FLT;
    }
    else
    {
        output =
            dt_colorspaces_get_profile(DT_COLORSPACE_SRGB, "",
    ...
    if ((d->mode != DT_PROFILE_NORMAL) && dt_pipe_is_full(pipe))
    {
        ...
        if (softproof)
        {
            ...
            if (d->mode == DT_PROFILE_GAMUTCHECK)
   ...
   */
                                                      ...
                                                      d->lut[2], LUT_SAMPLES))
    {
        ...
        d->xform =
            cmsCreateProofingTransform(Lab, TYPE_LabA_FLT, output, output_format, softproof,
    ...
    if (!d->xform && !dt_is_valid_colormatrix(d->cmatrix[0][0]))
    {
        ...
        dt_print(DT_DEBUG_ALWAYS, "unsupported output profile `%s' has been replaced by sRGB!",
                 out_profile->name);
        output =
            dt_colorspaces_get_profile(DT_COLORSPACE_SRGB, "", DT_PROFILE_DIRECTION_OUT)->profile;
                                                          ...
                                                          d->lut[2], LUT_SAMPLES))
        {
            ...
            d->xform = cmsCreateProofingTransform(Lab, TYPE_LabA_FLT, output, output_format,
                                                  softproof, out_intent,
    ...
    for (int k = 0; k < 3; k++)
    {
        ...
        if (d->lut[k][0] >= 0.0f)
        {
            ...
            const float y[4] = {_lerp_u_lut(d->lut[k], x[0]), _lerp_u_lut(d->lut[k], x[1]),
                                _lerp_u_lut(d->lut[k], x[2]), _lerp_u_lut(d->lut[k], x[3])};
...
void gui_update(dt_iop_module_t *self)
{
    ...
    dt_print(DT_DEBUG_ALWAYS, "[colorout] could not find requested profile `%s'!",
             dt_colorspaces_get_name(p->type, p->filename));
}
...
void gui_init(dt_iop_module_t *self)
{
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(g->output_intent, self, NULL, N_("output intent"),
                                 _("rendering intent"), 0, _intent_changed, self, N_("perceptual"),
