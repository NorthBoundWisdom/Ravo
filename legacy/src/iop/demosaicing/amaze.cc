...
void amaze_demosaic(const float *const in, float *out, const int width, const int height,
                    const uint32_t filters, const float clip_pt)
{
    ...
    constexpr int v1 = ts, v2 = 2 * ts, v3 = 3 * ts, p1 = -ts + 1, p2 = -2 * ts + 2,
                  p3 = -3 * ts + 3, m1 = ts + 1, m2 = 2 * ts + 2, m3 = 3 * ts + 3;
    ...
    constexpr float gaussodd[4] = {0.14659727707323927f, 0.103592713382435f, 0.0732036125103057f,
                                   0.0365543548389495f};
    ...
    {
        constexpr int cldf =
            2; // factor to multiply cache line distance. 1 = 64 bytes, 2 = 128 bytes ...
        // assign working space
        char *buffer = (char *)calloc(
            sizeof(float) * 14 * ts * ts + sizeof(char) * ts * tsh + 18 * cldf * 64 + 63, 1);
        ...
        float (*Dgrb)[ts * tsh] =
            (float (*)[ts * tsh]) vcdalt; // there is no overlap in buffer usage => share
        ...
        unsigned char *nyquist =
            (unsigned char(*))((char *)cfa + sizeof(float) * ts * ts + cldf * 64); // 1
        unsigned char *nyquist2 = (unsigned char(*))cddiffsq;
        float *nyqutest =
            (float(*))((char *)nyquist + sizeof(unsigned char) * ts * tsh + cldf * 64); // 1
        ...
        for (int left = -16; left < width; left += ts - 32)
        {
            ...
            for (int rr = 2; rr < rr1 - 2; rr++)
                for (int cc = 2, indx = (rr)*ts + cc; cc < cc1 - 2; cc++, indx++)
                {
                    ...
                    dirwts0[indx] = eps + fabsf(cfa[indx + v2] - cfa[indx]) +
                                    fabsf(cfa[indx] - cfa[indx - v2]) + delv;
                    dirwts1[indx] = eps + fabsf(cfa[indx + 2] - cfa[indx]) +
                                    fabsf(cfa[indx] - cfa[indx - 2]) + delh;
            ...
            for (int rr = 4; rr < rr1 - 4; rr++)
            {
                ...
                for (int cc = 4, indx = rr * ts + cc; cc < cc1 - 4; cc++, indx++)
                {
                    ...
                    const float vwt =
                        dirwts0[indx - v1] / (dirwts0[indx + v1] + dirwts0[indx - v1]);
            ...
            for (int rr = 4; rr < rr1 - 4; rr++)
            {
                for (int cc = 4, indx = rr * ts + cc, c = FC(rr, cc, filters) & 1; cc < cc1 - 4;
                     cc++, indx++)
                {
                    const float hcdvar =
                        3.f * (sqrf(hcd[indx - 2]) + sqrf(hcd[indx]) + sqrf(hcd[indx + 2])) -
                        sqrf(hcd[indx - 2] + hcd[indx] + hcd[indx + 2]);
                    const float hcdaltvar =
                        3.f *
                            (sqrf(hcdalt[indx - 2]) + sqrf(hcdalt[indx]) + sqrf(hcdalt[indx + 2])) -
                        sqrf(hcdalt[indx - 2] + hcdalt[indx] + hcdalt[indx + 2]);
                    const float vcdvar =
                        3.f * (sqrf(vcd[indx - v2]) + sqrf(vcd[indx]) + sqrf(vcd[indx + v2])) -
                        sqrf(vcd[indx - v2] + vcd[indx] + vcd[indx + v2]);
                    const float vcdaltvar =
                        3.f * (sqrf(vcdalt[indx - v2]) + sqrf(vcdalt[indx]) +
                               sqrf(vcdalt[indx + v2])) -
            ...
            {
                for (int cc = 6 + (FC(rr, 2, filters) & 1), indx = rr * ts + cc; cc < cc1 - 6;
                     cc += 2, indx += 2)
                {
                    ...
                    float Dgrbvvaru = sqrf(vcd[indx] - uave) + sqrf(vcd[indx - v1] - uave) +
                                      sqrf(vcd[indx - v2] - uave) + sqrf(vcd[indx - v3] - uave);
                    float Dgrbvvard = sqrf(vcd[indx] - dave) + sqrf(vcd[indx + v1] - dave) +
                                      sqrf(vcd[indx + v2] - dave) + sqrf(vcd[indx + v3] - dave);
                    float Dgrbhvarl = sqrf(hcd[indx] - lave) + sqrf(hcd[indx - 1] - lave) +
                                      sqrf(hcd[indx - 2] - lave) + sqrf(hcd[indx - 3] - lave);
                    float Dgrbhvarr = sqrf(hcd[indx] - rave) + sqrf(hcd[indx + 1] - rave) +
                                      sqrf(hcd[indx + 2] - rave) + sqrf(hcd[indx + 3] - rave);
            ...
            if (doNyquist)
            {
                ...
                {
                    for (int indx = rr * ts + nystartcol + (FC(rr, 2, filters) & 1);
                         indx < rr * ts + nyendcol; indx += 2)
                    {
                        unsigned int nyquisttemp =
                            (nyquist[(indx - v2) >> 1] + nyquist[(indx - m1) >> 1] +
                             nyquist[(indx + p1) >> 1] + nyquist[(indx - 2) >> 1] +
                             nyquist[(indx + 2) >> 1] + nyquist[(indx - p1) >> 1] +
                             nyquist[(indx + m1) >> 1] + nyquist[(indx + v2) >> 1]);
                        // if most of your neighbours are named Nyquist, it's likely that you're one too, or not
                        nyquist2[indx >> 1] =
                            nyquisttemp > 4 ? 1 : (nyquisttemp < 4 ? 0 : nyquist[indx >> 1]);
                    }
                }
                ...
                for (int rr = nystartrow; rr < nyendrow; rr++)
                         ...
                         indx < rr * ts + nyendcol; indx += 2)
                    {
                        if (nyquist2[indx >> 1])
                        {
                            ...
                            float sumcfa = 0.f, sumh = 0.f, sumv = 0.f, sumsqh = 0.f, sumsqv = 0.f,
                                  areawt = 0.f;
                            ...
                            for (int i = -6; i < 7; i += 2)
                            {
                                ...
                                for (int j = -6; j < 7; j += 2, indx1 += 2)
                                {
                                    if (nyquist2[indx1 >> 1])
                                    {
                                        ...
                                        sumsqh += sqrf(cfatemp - cfa[indx1 - 1]) +
                                                  sqrf(cfatemp - cfa[indx1 + 1]);
                                        sumsqv += sqrf(cfatemp - cfa[indx1 - v1]) +
                                                  sqrf(cfatemp - cfa[indx1 + v1]);
                                        areawt += 1;
                                    }
                                }
                            }
            ...
            for (int rr = 8; rr < rr1 - 8; rr++)
                for (int indx = rr * ts + 8 + (FC(rr, 2, filters) & 1); indx < rr * ts + cc1 - 8;
                     indx += 2)
                {
                    // first ask if one gets more directional discrimination from nearby B/R sites
                    const float hvwtalt =
                        _xdivf(hvwt[(indx - m1) >> 1] + hvwt[(indx + p1) >> 1] +
                                   hvwt[(indx - p1) >> 1] + hvwt[(indx + m1) >> 1],
                               2);

                    hvwt[indx >> 1] = fabsf(0.5f - hvwt[indx >> 1]) < fabsf(0.5f - hvwtalt) ?
                                          hvwtalt :
                                          hvwt[indx >> 1];
                    // a better result was obtained from the neighbours

                    Dgrb[0][indx >> 1] = interpolatef(hvwt[indx >> 1], vcd[indx],
                                                      hcd[indx]); // evaluate colour differences
                    ...
                    Dgrb2[indx >> 1].h = nyquist2[indx >> 1] ?
                                             sqrf(rgbgreen[indx] - _xdiv2f(rgbgreen[indx - 1] +
                                                                           rgbgreen[indx + 1])) :
                                             0.f;
                    Dgrb2[indx >> 1].v = nyquist2[indx >> 1] ?
                                             sqrf(rgbgreen[indx] - _xdiv2f(rgbgreen[indx - v1] +
                                                                           rgbgreen[indx + v1])) :
                                             0.f;
                }
            ...
            for (int rr = 6; rr < rr1 - 6; rr++)
            {
                if ((FC(rr, 2, filters) & 1) == 0)
                {
                    for (int cc = 6, indx = rr * ts + cc; cc < cc1 - 6; cc += 2, indx += 2)
                    {
                        ...
                        Dgrbsq1p[indx >> 1] = (sqrf(cfa[indx + 1] - cfa[indx + 1 - p1]) +
                                               sqrf(cfa[indx + 1] - cfa[indx + 1 + p1]));
                        Dgrbsq1m[indx >> 1] = (sqrf(cfa[indx + 1] - cfa[indx + 1 - m1]) +
                                               sqrf(cfa[indx + 1] - cfa[indx + 1 + m1]));
                    }
                }
                else
                {
                    ...
                    {
                        Dgrbsq1p[indx >> 1] =
                            (sqrf(cfa[indx] - cfa[indx - p1]) + sqrf(cfa[indx] - cfa[indx + p1]));
                        Dgrbsq1m[indx >> 1] =
                            (sqrf(cfa[indx] - cfa[indx - m1]) + sqrf(cfa[indx] - cfa[indx + m1]));
            ...
            for (int rr = 8; rr < rr1 - 8; rr++)
            {
                for (int cc = 8 + (FC(rr, 2, filters) & 1), indx = rr * ts + cc, indx1 = indx >> 1;
                     cc < cc1 - 8; cc += 2, indx += 2, indx1++)
                {
                    ...
                    {
                        rbse =
                            cfa[indx] * crse; // use this if more precise diag interp is necessary
                    }
                    else
                    {
                    ...
                    pmwt[indx1] =
                        rbvarm /
                        ((epssq +
                          (gausseven[0] * (Dgrbsq1p[(indx - v1) >> 1] + Dgrbsq1p[(indx - 1) >> 1] +
                    ...
                    if (rbp[indx1] < cfa[indx])
                    {
                        ...
                        {
                            const float pwt =
                                _xmul2f(cfa[indx] - rbp[indx1]) / (eps + rbp[indx1] + cfa[indx]);
                            rbp[indx1] =
                                pwt * rbp[indx1] +
                                (1.f - pwt) * ULIM(rbp[indx1], cfa[indx - p1], cfa[indx + p1]);
                        }
                    }
                    ...
                    if (rbm[indx1] < cfa[indx])
                    {
                        ...
                        {
                            const float mwt =
                                _xmul2f(cfa[indx] - rbm[indx1]) / (eps + rbm[indx1] + cfa[indx]);
                            rbm[indx1] =
                                mwt * rbm[indx1] +
                                (1.f - mwt) * ULIM(rbm[indx1], cfa[indx - m1], cfa[indx + m1]);
                        }
                    }
            ...
            for (int rr = 10; rr < rr1 - 10; rr++)
                for (int cc = 10 + (FC(rr, 2, filters) & 1), indx = rr * ts + cc, indx1 = indx >> 1;
                     cc < cc1 - 10; cc += 2, indx += 2, indx1++)
                {
                    // first ask if one gets more directional discrimination from nearby B/R sites
                    const float pmwtalt =
                        _xdivf(pmwt[(indx - m1) >> 1] + pmwt[(indx + p1) >> 1] +
                                   pmwt[(indx - p1) >> 1] + pmwt[(indx + m1) >> 1],
                               2);
            ...
            for (int rr = 12; rr < rr1 - 12; rr++)
                for (int cc = 12 + (FC(rr, 2, filters) & 1), indx = rr * ts + cc, indx1 = indx >> 1;
                     cc < cc1 - 12; cc += 2, indx += 2, indx1++)
                {
                    ...
                    const float cru =
                        cfa[indx - v1] * 2.0 / (eps + rbint[indx1] + rbint[(indx1 - v1)]);
                    const float crd =
                        cfa[indx + v1] * 2.0 / (eps + rbint[indx1] + rbint[(indx1 + v1)]);
                    const float crl =
                        cfa[indx - 1] * 2.0 / (eps + rbint[indx1] + rbint[(indx1 - 1)]);
                    const float crr =
                        cfa[indx + 1] * 2.0 / (eps + rbint[indx1] + rbint[(indx1 + 1)]);
                    ...
                    if (Gintv < rbint[indx1])
                    {
                        ...
                        else
                        {
                            ...
                            Gintv = vwt * Gintv +
                                    (1.f - vwt) * ULIM(Gintv, cfa[indx - v1], cfa[indx + v1]);
                        }
                    }
                    ...
                    if (Ginth < rbint[indx1])
                    {
                        ...
                        {
                            const float hwt =
                                2.0 * (rbint[indx1] - Ginth) / (eps + Ginth + rbint[indx1]);
                            Ginth = hwt * Ginth +
                                    (1.f - hwt) * ULIM(Ginth, cfa[indx - 1], cfa[indx + 1]);
                        }
                    }
            ...
            for (int rr = 13 - ey; rr < rr1 - 12; rr += 2)
                for (int indx1 = (rr * ts + 13 - ex) >> 1; indx1 < (rr * ts + cc1 - 12) >> 1;
                     indx1++)
                {                                    // B coset
            ...
            for (int rr = 14; rr < rr1 - 14; rr++)
                ...
                for (int cc = 14 + (FC(rr, 2, filters) & 1), indx = rr * ts + cc,
                         c = 1 - FC(rr, cc, filters) / 2;
                     cc < cc1 - 14; cc += 2, indx += 2)
                {
                    const float wtnw =
                        1.f / (eps + fabsf(Dgrb[c][(indx - m1) >> 1] - Dgrb[c][(indx + m1) >> 1]) +
                               fabsf(Dgrb[c][(indx - m1) >> 1] - Dgrb[c][(indx - m3) >> 1]) +
                               fabsf(Dgrb[c][(indx + m1) >> 1] - Dgrb[c][(indx - m3) >> 1]));
                    const float wtne =
                        1.f / (eps + fabsf(Dgrb[c][(indx + p1) >> 1] - Dgrb[c][(indx - p1) >> 1]) +
                    ...
                    Dgrb[c][indx >> 1] = (wtnw * (1.325f * Dgrb[c][(indx - m1) >> 1] -
                                                  0.175f * Dgrb[c][(indx - m3) >> 1] -
                                                  0.075f * Dgrb[c][(indx - m1 - 2) >> 1] -
                                                  0.075f * Dgrb[c][(indx - m1 - v2) >> 1]) +
                                          wtne * (1.325f * Dgrb[c][(indx + p1) >> 1] -
                                                  0.175f * Dgrb[c][(indx + p3) >> 1] -
                                                  0.075f * Dgrb[c][(indx + p1 + 2) >> 1] -
                                                  0.075f * Dgrb[c][(indx + p1 + v2) >> 1]) +
                                          wtsw * (1.325f * Dgrb[c][(indx - p1) >> 1] -
                                                  0.175f * Dgrb[c][(indx - p3) >> 1] -
                                                  0.075f * Dgrb[c][(indx - p1 - 2) >> 1] -
                                                  0.075f * Dgrb[c][(indx - p1 - v2) >> 1]) +
                                          wtse * (1.325f * Dgrb[c][(indx + m1) >> 1] -
                                                  0.175f * Dgrb[c][(indx + m3) >> 1] -
                                                  0.075f * Dgrb[c][(indx + m1 + 2) >> 1] -
                                                  0.075f * Dgrb[c][(indx + m1 + v2) >> 1])) /
                                         (wtnw + wtne + wtsw + wtse);
            ...
            for (int rr = 16; rr < rr1 - 16; rr++)
            {
                ...
                if ((FC(rr, 2, filters) & 1) == 1)
                {
                    for (; indx < rr * ts + cc1 - 16 - (cc1 & 1); indx++, col++)
                    {
                        ...
                        {
                            const float temp =
                                1.f / (hvwt[(indx - v1) >> 1] + 2.f - hvwt[(indx + 1) >> 1] -
                                       hvwt[(indx - 1) >> 1] + hvwt[(indx + v1) >> 1]);
                            out[(row * width + col) * 4] = _clampnan(
                                rgbgreen[indx] -
                            ...
                            out[(row * width + col) * 4 + 2] = _clampnan(
                                rgbgreen[indx] -
