...
static void pre_median_b(float *out, const float *const in, const int width, const int height,
                         const uint32_t filters, const int num_passes, const float threshold)
{
...
static void pre_median(float *out, const float *const in, const int width, const int height,
                       const uint32_t filters, const int num_passes, const float threshold)
{
...
static void color_smoothing(float *out, const int width, const int height, const int num_passes)
{
...
static void green_equilibration_lavg(float *out, const float *const in, const int width,
                                     const int height, const uint32_t filters, const float thr)
{
    const float maximum = 1.0f;

    int oj = 2, oi = 2;
    if (FC(oj, oi, filters) != GREEN)
        oj++;
    if (FC(oj, oi, filters) != GREEN)
        oi++;
    if (FC(oj, oi, filters) != GREEN)
        ...
        for (size_t i = oi; i < width - 2; i += 2)
        {
            ...
            if ((m2 > 0.0f) && (m1 > 0.0f) && (m1 / m2 < maximum * 2.0f))
            {
                const float c1 = (fabsf(o1_1 - o1_2) + fabsf(o1_1 - o1_3) + fabsf(o1_1 - o1_4) +
                                  fabsf(o1_2 - o1_3) + fabsf(o1_3 - o1_4) + fabsf(o1_2 - o1_4)) /
                                 6.0f;
                const float c2 = (fabsf(o2_1 - o2_2) + fabsf(o2_1 - o2_3) + fabsf(o2_1 - o2_4) +
                                  fabsf(o2_2 - o2_3) + fabsf(o2_3 - o2_4) + fabsf(o2_2 - o2_4)) /
                                 6.0f;
                if ((in[j * width + i] < maximum * 0.95f) && (c1 < maximum * thr) &&
                    (c2 < maximum * thr))
                {
...
static void green_equilibration_favg(float *out, const float *const in, const int width,
                                     const int height, const uint32_t filters)
{
    int oj = 0, oi = 0;
    // const float ratio_max = 1.1f;
    double sum1 = 0.0, sum2 = 0.0, gr_ratio;

    if ((FC(oj, oi, filters) & 1) != 1)
        oi++;
...
static int color_smoothing_cl(const dt_iop_module_t *self, const dt_dev_pixelpipe_iop_t *piece,
                              cl_mem dev_in, cl_mem dev_out, const int width, const int height,
                              const int passes)
{
    ...
    for (int pass = 0; pass < passes; pass++)
    {
        err = dt_opencl_enqueue_kernel_2d_local_args(
            devid, gd->kernel_color_smoothing, sizes, local, CLARG(dev_t1), CLARG(dev_t2),
            CLARG(width), CLARG(height),
            CLLOCAL(sizeof(float) * 4 * (locopt.sizex + 2) * (locopt.sizey + 2)));
        if (err != CL_SUCCESS)
            goto error;
...
static int green_equilibration_cl(const dt_iop_module_t *self, const dt_dev_pixelpipe_iop_t *piece,
                                  cl_mem dev_in, cl_mem dev_out, const int width, const int height,
                                  const uint32_t filters)
{
    ...
    if (d->green_eq == DT_IOP_GREEN_EQ_FULL || d->green_eq == DT_IOP_GREEN_EQ_BOTH)
    {
        ...
        err = dt_opencl_enqueue_kernel_2d_local_args(
            devid, gd->kernel_green_eq_favg_reduce_first, fsizes, flocal, CLARG(dev_in1),
            CLARG(width), CLARG(height), CLARG(dev_m), CLARG(filters),
        ...
        const int reducesize =
            MIN(DT_REDUCESIZE_MIN, ROUNDUP(bufsize, slocopt.sizex) / slocopt.sizex);
        ...
        err = dt_opencl_enqueue_kernel_2d_local_args(
            devid, gd->kernel_green_eq_favg_reduce_second, ssizes, slocal, CLARG(dev_m),
            CLARG(dev_r), CLARG(bufsize), CLLOCAL(sizeof(float) * 2 * slocopt.sizex));
        if (err != CL_SUCCESS)
            goto error;
        ...
        err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_green_eq_favg_apply, width, height,
                                               CLARG(dev_in1), CLARG(dev_out1), CLARG(width),
                                               CLARG(height), CLARG(filters), CLARG(gr_ratio));
        if (err != CL_SUCCESS)
            goto error;
    }
    ...
    if (d->green_eq == DT_IOP_GREEN_EQ_LOCAL || d->green_eq == DT_IOP_GREEN_EQ_BOTH)
    {
        ...
        const size_t local[2] = {locopt.sizex, locopt.sizey};
        err = dt_opencl_enqueue_kernel_2d_local_args(
            devid, gd->kernel_green_eq_lavg, sizes, local, CLARG(dev_in2), CLARG(dev_out2),
            CLARG(width), CLARG(height), CLARG(filters), CLARG(threshold),
            CLLOCAL(sizeof(float) * (locopt.sizex + 4) * (locopt.sizey + 4)));
        if (err != CL_SUCCESS)
            goto error;
    }
...
static int process_default_cl(const dt_iop_module_t *self, const dt_dev_pixelpipe_iop_t *piece,
                              cl_mem dev_in, cl_mem dev_out, cl_mem dev_xtrans, const int width,
                              const int height, const int demosaicing_method,
                              const uint32_t filters)
{
    const dt_iop_demosaic_data_t *d = piece->data;
    const dt_iop_demosaic_global_data_t *gd = self->global_data;
    ...
    {
        err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_passthrough_monochrome, width,
                                               height, CLARG(dev_in), CLARG(dev_out), CLARG(width),
                                               CLARG(height));
    }
    else if (demosaicing_method == DT_IOP_DEMOSAIC_PASSTHROUGH_COLOR)
    {
        err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_passthrough_color, width, height,
                                               ...
                                               CLARG(height), CLARG(filters), CLARG(dev_xtrans));
    }
    else if (demosaicing_method == DT_IOP_DEMOSAIC_PPG)
    {
        ...
        if (d->median_thrs > 0.0f)
        {
            ...
            const size_t local[2] = {locopt.sizex, locopt.sizey};
            err = dt_opencl_enqueue_kernel_2d_local_args(
                devid, gd->kernel_pre_median, sizes, local, CLARG(dev_in), CLARG(dev_med),
                CLARG(width), CLARG(height), CLARG(filters), CLARG(d->median_thrs),
                CLLOCAL(sizeof(float) * (locopt.sizex + 4) * (locopt.sizey + 4)));
            if (err != CL_SUCCESS)
                goto error;
            dev_in = dev_out;
...

        {
            ...
            const size_t local[2] = {locopt.sizex, locopt.sizey};
            err = dt_opencl_enqueue_kernel_2d_local_args(
                devid, gd->kernel_ppg_green, sizes, local, CLARG(dev_med), CLARG(dev_tmp),
                CLARG(width), CLARG(height), CLARG(filters),
                CLLOCAL(sizeof(float) * (locopt.sizex + 2 * 3) * (locopt.sizey + 2 * 3)),
                CLARGINT(100000));
            if (err != CL_SUCCESS)
                goto error;
        }
        ...
        {
            dt_opencl_local_buffer_t locopt =
                (dt_opencl_local_buffer_t){.xoffset = 2 * 1,
                                           .xfactor = 1,
                                           .yoffset = 2 * 1,
                                           .yfactor = 1,
                                           .cellsize = 4 * sizeof(float),
...
error:
    if (dev_med != dev_in)
        dt_opencl_release_mem_object(dev_med);
    dt_opencl_release_mem_object(dev_tmp);
...
static int demosaic_box3_cl(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece, cl_mem dev_in,
                            cl_mem dev_out, cl_mem dev_xtrans, const int width, const int height,
                            const uint32_t filters)
{
    const dt_iop_demosaic_global_data_t *gd = self->global_data;
    const cl_int err = dt_opencl_enqueue_kernel_2d_args(
        piece->pipe->devid, gd->kernel_demosaic_box3, width, height, CLARG(dev_in), CLARG(dev_out),
        CLARG(width), CLARG(height), CLARG(filters), CLARG(dev_xtrans));
    if (err != CL_SUCCESS)
