...
static void demosaic_box3(float *const restrict out, const float *const restrict in,
                          const int width, const int height, const uint32_t filters,
        ...
        for (int col = 0; col < width; col++)
        {
            ...
            for (int y = row - 1; y < row + 2; y++)
            {
                for (int x = col - 1; x < col + 2; x++)
                {
                    ...
                    {
                        const int color =
                            (filters == 9u) ? FCNxtrans(y, x, xtrans) : FC(y, x, filters);
                        sum[color] += MAX(0.0f, in[(size_t)width * y + x]);
                        cnt[color] += 1.0f;
                    }
                }
            }
            for_each_channel(c) out[((size_t)row * width + col) * 4 + c] =
                sum[c] / MAX(1.0f, cnt[c]);
        }
    }
}

DT_OMP_DECLARE_SIMD(aligned(in, out : 64))
static void rcd_demosaic(float *const restrict out, const float *const restrict in, const int width,
                         const int height, const uint32_t filters, const float scaler)
{
    ...
    DT_OMP_PRAGMA(parallel firstprivate(width, height, filters, out, in, scaler, revscaler))
    {
        ...
        float *const P_CDiff_Hpf =
            dt_alloc_align_float((size_t)DT_RCD_TILESIZE * DT_RCD_TILESIZE / 2);
        float *const Q_CDiff_Hpf =
            dt_alloc_align_float((size_t)DT_RCD_TILESIZE * DT_RCD_TILESIZE / 2);

        float (*const rgb)[DT_RCD_TILESIZE * DT_RCD_TILESIZE] =
            (void *)dt_alloc_align_float((size_t)3 * DT_RCD_TILESIZE * DT_RCD_TILESIZE);
    ...
    for (int tile_vertical = 0; tile_vertical < num_vertical; tile_vertical++)
    {
        for (int tile_horizontal = 0; tile_horizontal < num_horizontal; tile_horizontal++)
        {
            ...
            for (int row = rowStart; row < rowEnd; row++)
            {
                ...
                for (int col = colStart, indx = (row - rowStart) * DT_RCD_TILESIZE,
            ...
            {
                for (int col = 4, indx = row * DT_RCD_TILESIZE + col; col < tileCols - 4;
                     col++, indx++)
                {
                    bufferV[row - 3][col - 4] =
                        sqrf((cfa[indx - w3] - cfa[indx - w1] - cfa[indx + w1] + cfa[indx + w3]) -
            ...
            {
                for (int col = 3, indx = row * DT_RCD_TILESIZE + col; col < tileCols - 3;
                     col++, indx++)
                {
                    bufferH[col - 3] =
                        sqrf((cfa[indx - 3] - cfa[indx - 1] - cfa[indx + 1] + cfa[indx + 3]) -
                             3.0f * (cfa[indx - 2] + cfa[indx + 2]) + 6.0f * cfa[indx]);
                }
                for (int col = 4, indx = (row + 1) * DT_RCD_TILESIZE + col; col < tileCols - 4;
                     col++, indx++)
                {
                    V2[col - 4] =
                        sqrf((cfa[indx - w3] - cfa[indx - w1] - cfa[indx + w1] + cfa[indx + w3]) -
                     ...
                     col++, indx++)
                {
                    ...
                    const float H_Stat =
                        fmaxf(epssq, bufferH[col - 4] + bufferH[col - 3] + bufferH[col - 2]);
            ...
            {
                for (int col = 2 + (FC(row, 0, filters) & 1), indx = row * DT_RCD_TILESIZE + col,
                         lp_indx = indx / 2;
                     col < tileCols - 2; col += 2, indx += 2, lp_indx++)
                {
                    lpf[lp_indx] =
                        ...
                        0.25f * (cfa[indx - w1 - 1] + cfa[indx - w1 + 1] + cfa[indx + w1 - 1] +
                                 cfa[indx + w1 + 1]);
                }
            }
            ...
            for (int row = 4; row < tileRows - 4; row++)
            {
                     ...
                     col < tileCols - 4; col += 2, indx += 2, lpindx++)
                {
                    ...
                    // Cardinal gradients
                    const float N_Grad = eps + fabsf(cfa[indx - w1] - cfa[indx + w1]) +
                                         fabsf(cfai - cfa[indx - w2]) +
                                         fabsf(cfa[indx - w1] - cfa[indx - w3]) +
                                         fabsf(cfa[indx - w2] - cfa[indx - w4]);
                    const float S_Grad = eps + fabsf(cfa[indx + w1] - cfa[indx - w1]) +
                                         fabsf(cfai - cfa[indx + w2]) +
                                         fabsf(cfa[indx + w1] - cfa[indx + w3]) +
                                         fabsf(cfa[indx + w2] - cfa[indx + w4]);
                    const float W_Grad =
                        eps + fabsf(cfa[indx - 1] - cfa[indx + 1]) + fabsf(cfai - cfa[indx - 2]) +
                        fabsf(cfa[indx - 1] - cfa[indx - 3]) + fabsf(cfa[indx - 2] - cfa[indx - 4]);
                    const float E_Grad =
                        eps + fabsf(cfa[indx + 1] - cfa[indx - 1]) + fabsf(cfai - cfa[indx + 2]) +
                        fabsf(cfa[indx + 1] - cfa[indx + 3]) + fabsf(cfa[indx + 2] - cfa[indx + 4]);
                    ...
                    const float N_Est =
                        cfa[indx - w1] * (lpfi + lpfi) / (eps + lpfi + lpf[lpindx - w1]);
                    const float S_Est =
                        cfa[indx + w1] * (lpfi + lpfi) / (eps + lpfi + lpf[lpindx + w1]);
                    const float W_Est =
                    ...
                    const float VH_Disc =
                        (fabsf(0.5f - VH_Central_Value) < fabsf(0.5f - VH_Neighbourhood_Value)) ?
            ...
            for (int row = 3; row < tileRows - 3; row++)
            {
                ...
                {
                    P_CDiff_Hpf[indx2] =
                        sqrf((cfa[indx - w3 - 3] - cfa[indx - w1 - 1] - cfa[indx + w1 + 1] +
                              cfa[indx + w3 + 3]) -
            ...
            for (int row = 4; row < tileRows - 4; row++)
            {
                ...
                {
                    const float P_Stat = fmaxf(epssq, P_CDiff_Hpf[indx3] + P_CDiff_Hpf[indx2] +
                                                          P_CDiff_Hpf[indx4 + 1]);
                    const float Q_Stat = fmaxf(epssq, Q_CDiff_Hpf[indx3 + 1] + Q_CDiff_Hpf[indx2] +
                                                          Q_CDiff_Hpf[indx4]);
            ...
            for (int row = 4; row < tileRows - 4; row++)
            {
                     ...
                     col < tileCols - 4; col += 2, indx += 2, pqindx++, pqindx2++, pqindx3++)
                {
                    ...
                    const float PQ_Neighbourhood_Value =
                        0.25f * (PQ_Dir[pqindx2] + PQ_Dir[pqindx2 + 1] + PQ_Dir[pqindx3] +
                                 PQ_Dir[pqindx3 + 1]);

                    const float PQ_Disc =
                        (fabsf(0.5f - PQ_Central_Value) < fabsf(0.5f - PQ_Neighbourhood_Value)) ?
                            PQ_Neighbourhood_Value :
                            PQ_Central_Value;

                    // Diagonal gradients
                    const float NW_Grad = eps +
                                          fabsf(rgb[c][indx - w1 - 1] - rgb[c][indx + w1 + 1]) +
                                          fabsf(rgb[c][indx - w1 - 1] - rgb[c][indx - w3 - 3]) +
                                          fabsf(rgb[1][indx] - rgb[1][indx - w2 - 2]);
            ...
            for (int row = 4; row < tileRows - 4; row++)
            {
                     ...
                     col < tileCols - 4; col += 2, indx += 2)
                {
                    ...
                    for (int c = 0; c <= 2; c += 2)
                    {
                        ...
                        // Cardinal gradients
                        const float N_Grad =
                            N1 + SNabs + fabsf(rgb[c][indx - w1] - rgb[c][indx - w3]);
                        const float S_Grad =
                            S1 + SNabs + fabsf(rgb[c][indx + w1] - rgb[c][indx + w3]);
                        const float W_Grad =
                            W1 + EWabs + fabsf(rgb[c][indx - 1] - rgb[c][indx - 3]);
                        const float E_Grad =
                            E1 + EWabs + fabsf(rgb[c][indx + 1] - rgb[c][indx + 3]);
            ...
            const int last_vertical =
                rowEnd - ((tile_vertical == num_vertical - 1) ? RCD_MARGIN : RCD_BORDER);
            const int first_horizontal =
                colStart + ((tile_horizontal == 0) ? RCD_MARGIN : RCD_BORDER);
            const int last_horizontal =
                colEnd - ((tile_horizontal == num_horizontal - 1) ? RCD_MARGIN : RCD_BORDER);
            for (int row = first_vertical; row < last_vertical; row++)
            {
                for (int col = first_horizontal,
                         idx = (row - rowStart) * DT_RCD_TILESIZE + col - colStart,
...
#ifdef HAVE_OPENCL
static cl_int process_rcd_cl(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece, cl_mem dev_in,
                             ...
                             const uint32_t filters)
{
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_border_interpolate, width, height,
                                           ...
                                           CLARG(height), CLARG(filters));
    if (err != CL_SUCCESS)
        goto error;

    {
        dt_opencl_local_buffer_t locopt = (dt_opencl_local_buffer_t){.xoffset = 2 * 3,
                                                                     .xfactor = 1,
                                                                     .yoffset = 2 * 3,
                                                                     .yfactor = 1,
                                                                     .cellsize = sizeof(float) * 1,
                                                                     .overhead = 0,
                                                                     .sizex = 64,
                                                                     .sizey = 64};
        ...
        err = dt_opencl_enqueue_kernel_2d_local_args(
            devid, gd->kernel_ppg_green, sizes, local, CLARG(dev_in), CLARG(dev_tmp), CLARG(width),
            CLARG(height), CLARG(filters),
    ...
    {
        dt_opencl_local_buffer_t locopt = (dt_opencl_local_buffer_t){.xoffset = 2 * 1,
                                                                     .xfactor = 1,
                                                                     .yoffset = 2 * 1,
                                                                     .yfactor = 1,
        ...
        size_t local[2] = {locopt.sizex, locopt.sizey};
        err = dt_opencl_enqueue_kernel_2d_local_args(
            devid, gd->kernel_ppg_redblue, sizes, local, CLARG(dev_tmp), CLARG(dev_out),
    ...
    if (cfa == NULL || VH_dir == NULL || PQ_dir == NULL || VP_diff == NULL || HQ_diff == NULL ||
        rgb0 == NULL || rgb1 == NULL || rgb2 == NULL)
        goto error;

    // populate data
    float scaler = 1.0f / dt_iop_get_processed_maximum(piece);
    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_rcd_populate, width, height, CLARG(dev_in), CLARG(cfa), CLARG(rgb0),
        CLARG(rgb1), CLARG(rgb2), CLARG(width), CLARG(height), CLARG(filters), CLARG(scaler));
    if (err != CL_SUCCESS)
        goto error;
    ...
    dt_opencl_local_buffer_t locopt = (dt_opencl_local_buffer_t){.xoffset = 8,
                                                                 .xfactor = 1,
                                                                 .yoffset = 8,
                                                                 .yfactor = 1,
    ...
    size_t local[2] = {locopt.sizex, locopt.sizey};
    err = dt_opencl_enqueue_kernel_2d_local_args(
        devid, gd->kernel_rcd_step_1, sizes, local, CLARG(cfa), CLARG(VH_dir), CLARG(width),
        CLARG(height), CLLOCAL(sizeof(float) * (locopt.sizex + 8) * (locopt.sizey + 8)));
    if (err != CL_SUCCESS)
        goto error;
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_rcd_step_2, width / 2, height,
                                           CLARG(PQ_dir), CLARG(cfa), CLARG(width), CLARG(height),
                                           CLARG(filters));
    if (err != CL_SUCCESS)
        goto error;
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_rcd_step_3, width / 2, height,
                                           CLARG(PQ_dir), CLARG(cfa), CLARG(rgb1), CLARG(VH_dir),
                                           CLARG(width), CLARG(height), CLARG(filters));
    if (err != CL_SUCCESS)
        goto error;
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_rcd_step_4_0, width / 2, height,
                                           CLARG(cfa), CLARG(VP_diff), CLARG(HQ_diff), CLARG(width),
                                           CLARG(height), CLARG(filters));
    if (err != CL_SUCCESS)
        goto error;
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_rcd_step_4_1, width / 2, height,
                                           CLARG(PQ_dir), CLARG(VP_diff), CLARG(HQ_diff),
                                           CLARG(width), CLARG(height), CLARG(filters));
    if (err != CL_SUCCESS)
        goto error;
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_rcd_step_4_2, width / 2, height,
                                           CLARG(PQ_dir), CLARG(rgb0), CLARG(rgb1), CLARG(rgb2),
                                           CLARG(width), CLARG(height), CLARG(filters));
    if (err != CL_SUCCESS)
        goto error;
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_rcd_step_4_3, width / 2, height,
                                           CLARG(VH_dir), CLARG(rgb0), CLARG(rgb1), CLARG(rgb2),
                                           CLARG(width), CLARG(height), CLARG(filters));
    if (err != CL_SUCCESS)
        goto error;
    ...
    // write output
    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_rcd_write_output, width, height, CLARG(dev_out), CLARG(rgb0), CLARG(rgb1),
        CLARG(rgb2), CLARG(width), CLARG(height), CLARG(scaler), CLARGINT(RCD_MARGIN));
