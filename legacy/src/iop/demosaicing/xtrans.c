...
static inline const short *_hexmap(const int row, const int col, short (*const allhex)[3][8])
{
...
*/
static void xtrans_markesteijn_interpolate(float *out, const float *const in, const int width,
                                           const int height, const uint8_t (*const xtrans)[6],
                                           const int passes, const uint32_t filters)
{
    ...
    for (int top = -pad_tile; top < height - pad_tile; top += TS - (pad_tile * 2))
    {
        ...
        float (*const yuv)[TS][TS] =
            (float (*)[TS][TS])(buffer + TS * TS * (ndir * 3) * sizeof(float));
        // drv points to ndir TSxTS tiles, each a single channel of derivatives
        float (*const drv)[TS][TS] =
            (float (*)[TS][TS])(buffer + TS * TS * (ndir * 3 + 3) * sizeof(float));
        ...
        float (*const gmax)[TS] =
            (float (*)[TS])(buffer + TS * TS * (ndir * 3 + 1) * sizeof(float));
        // homo and homosum reuse memory which is used earlier in the
        // loop; each points to ndir single-channel TSxTS tiles
        uint8_t (*const homo)[TS][TS] =
            (uint8_t (*)[TS][TS])(buffer + TS * TS * (ndir * 3) * sizeof(float));
        uint8_t (*const homosum)[TS][TS] = (uint8_t (*)[TS][TS])(
            buffer + TS * TS * (ndir * 3) * sizeof(float) + TS * TS * ndir * sizeof(uint8_t));
            ...
            for (int row = top; row < mrow; row++)
                for (int col = left; col < mcol; col++)
                {
                    ...
                    if ((col >= 0) && (row >= 0) && (col < width) && (row < height))
                    {
                        ...
                        for (int c = 0; c < 3; c++)
                            pix[c] = (c == f) ? fmax(0.0f, in[width * row + col]) : 0.f;
                    }
                    else
                    {
...
#define TRANSLATE(n, size) ((n >= size) ? (2 * size - n - 2) : abs(n))
                                ...
                                else
                                {
                                    ...
                                    for (int y = row - 1; y <= row + 1; y++)
                                        ...
                                        {
                                            const int yy = TRANSLATE(y, height),
                                                      xx = TRANSLATE(x, width);
            ...
            for (int row = top + PAD_G_INTERP; row < mrow - PAD_G_INTERP; row++)
                for (int col = left + PAD_G_INTERP; col < mcol - PAD_G_INTERP; col++)
                {
                    ...
                    for (int c = 0; c < 2; c++)
                        color[2 + c] =
                            0.640625f * pix[hex[4 + c]][1] + 0.359375f * pix[-2 * hex[4 + c]][1] +
                            0.12890625f *
                                (2 * pix[0][f] - pix[3 * hex[4 + c]][f] - pix[-3 * hex[4 + c]][f]);
                    for (int c = 0; c < 4; c++)
                        rgb[c ^ !((row - sgrow) % 3)][row - top][col - left][1] = CLAMPS(
                            color[c], gmin[row - top][col - left], gmax[row - top][col - left]);
                }
            ...
            for (int pass = 0; pass < passes; pass++)
            {
                    ...
                    for (int row = top + PAD_G_RECALC; row < mrow - PAD_G_RECALC; row++)
                        for (int col = left + PAD_G_RECALC; col < mcol - PAD_G_RECALC; col++)
                        {
                            ...
                            for (int d = 3; d < 6; d++)
                            {
                                ...
                                const float val = rfx[-2 * hex[d]][1] + 2 * rfx[hex[d]][1] -
                                                  rfx[-2 * hex[d]][f] - 2 * rfx[hex[d]][f] +
                                                  3 * rfx[0][f];
                                rfx[0][1] = CLAMPS(val / 3.0f, gmin[row - top][col - left],
                ...
                for (int row = (top - sgrow + pad_rb_g + 2) / 3 * 3 + sgrow; row < mrow - pad_rb_g;
                     row += 3)
                    for (int col = (left - sgcol + pad_rb_g + 2) / 3 * 3 + sgcol;
                         col < mcol - pad_rb_g; col += 3)
                    {
                        ...
                        for (int i = 1, d = 0; d < 6; d++, i ^= TS ^ 1, h ^= 2)
                        {
                            ...
                            for (int c = 0; c < 2; c++, h ^= 2)
                            {
                                ...
                                if (d > 1)
                                    diff[d] += sqrf(rfx[i << c][1] - rfx[-(i << c)][1] -
                                                    rfx[i << c][h] + rfx[-(i << c)][h]) +
                                               sqrf(g);
                            }
                            if ((d < 2) || (d & 1))
                    ...
                    for (int col = left + pad_rb_br; col < mcol - pad_rb_br; col++)
                    {
                        ...
                        for (int d = 0; d < 4; d++, rfx += TS * TS)
                        {
                            const int i = d > 1 || ((d ^ c) & 1) ||
                                                  ((fabsf(rfx[0][1] - rfx[c][1]) +
                                                    fabsf(rfx[0][1] - rfx[-c][1])) <
                                                   2.f * (fabsf(rfx[0][1] - rfx[h][1]) +
                                                          fabsf(rfx[0][1] - rfx[-h][1]))) ?
                                              c :
                                              h;
                            rfx[0][f] = (rfx[i][f] + rfx[-i][f] + 2.f * rfx[0][1] - rfx[i][1] -
                                         rfx[-i][1]) /
                                        2.f;
                        }
                    }
                        ...
                        for (int col = left + pad_g22; col < mcol - pad_g22; col++)
                            if ((col - sgcol) % 3)
                            {
                                ...
                                for (unsigned d = 0; d < ndir; d += 2, rfx += TS * TS)
                                    if (hex[d] + hex[d + 1])
                                    {
                                        const float g = 3.f * rfx[0][1] - 2.f * rfx[hex[d]][1] -
                                                        rfx[hex[d + 1]][1];
                                        for (int c = 0; c < 4; c += 2)
                                            rfx[0][c] =
                                                (g + 2.f * rfx[hex[d]][c] + rfx[hex[d + 1]][c]) /
                                                3.f;
                                    }
                                    else
                                    {
                                        const float g =
                                            2.f * rfx[0][1] - rfx[hex[d]][1] - rfx[hex[d + 1]][1];
                                        for (int c = 0; c < 4; c += 2)
                                            rfx[0][c] =
                                                (g + rfx[hex[d]][c] + rfx[hex[d + 1]][c]) / 2.f;
                                    }
                            }
                }
                                   ...
                                   const int iso, const uint32_t filters)
{
    ...
    static const float complex Minv[3][8] = {
        {1.000000e+00f, 2.500000e-01f - 4.330127e-01f * _Complex_I,
         -2.500000e-01f - 4.330127e-01f * _Complex_I, -1.000000e+00f,
         7.500000e-01f - 1.299038e+00f * _Complex_I, -2.500000e-01f + 4.330127e-01f * _Complex_I,
         7.500000e-01f + 1.299038e+00f * _Complex_I, 2.500000e-01f + 4.330127e-01f * _Complex_I},
        {1.000000e+00f, -2.000000e-01f + 3.464102e-01f * _Complex_I,
         2.000000e-01f + 3.464102e-01f * _Complex_I, 8.000000e-01f, 0.0f,
         2.000000e-01f - 3.464102e-01f * _Complex_I, 0.0f,
    ...
    static const float complex modarr[6][6][8] = {
        {{1.000000e+00f + 0.000000e+00f * _Complex_I, 1.000000e+00f + 0.000000e+00f * _Complex_I,
         ...
         {-1.000000e+00f - 6.123234e-16f * _Complex_I, 5.000000e-01f - 8.660254e-01f * _Complex_I,
          -1.000000e+00f - 6.123234e-16f * _Complex_I, 5.000000e-01f + 8.660254e-01f * _Complex_I,
          -5.000000e-01f - 8.660254e-01f * _Complex_I, 1.000000e+00f + 0.000000e+00f * _Complex_I,
          -5.000000e-01f - 8.660254e-01f * _Complex_I,
          -5.000000e-01f + 8.660254e-01f * _Complex_I}},
    ...
    for (int top = -pad_tile; top < height - pad_tile; top += TS - (pad_tile * 2))
    {
        ...
        float complex *fdc_buf_start =
            (float complex *)(buffer + TS * TS * (ndir * 4 + 3) * sizeof(float));
            ...
            for (int row = top; row < mrow; row++)
                for (int col = left; col < mcol; col++)
                {
                    ...
                    if ((col >= 0) && (row >= 0) && (col < width) && (row < height))
                    {
                        const int f = FCNxtrans(row, col, xtrans);
                        for (int c = 0; c < 3; c++)
                            pix[c] = (c == f) ? in[width * row + col] : 0.f;
...
#define TRANSLATE(n, size) ((n >= size) ? (2 * size - n - 2) : abs(n))
            ...
            for (int row = (top - sgrow + pad_rb_g + 2) / 3 * 3 + sgrow; row < mrow - pad_rb_g;
                 row += 3)
                for (int col = (left - sgcol + pad_rb_g + 2) / 3 * 3 + sgcol; col < mcol - pad_rb_g;
                     col += 3)
                {
                    float (*rfx)[3] = &rgb[0][row - top][col - left];
                    int h = FCNxtrans(row, col + 1, xtrans);
                ...
                for (int col = left + pad_rb_br; col < mcol - pad_rb_br; col++)
                {
                    ...
                    for (int d = 0; d < 4; d++, rfx += TS * TS)
                    {
                        int i = d > 1 || ((d ^ c) & 1) ||
                                    ...
                                    h;
                        rfx[0][f] =
                            (rfx[i][f] + rfx[-i][f] + 2.f * rfx[0][1] - rfx[i][1] - rfx[-i][1]) /
                            2.f;
                    }
                }
                    ...
                    for (int col = left + pad_g22; col < mcol - pad_g22; col++)
                        if ((col - sgcol) % 3)
                        {
                            ...
                            for (int d = 0; d < ndir; d += 2, rfx += TS * TS)
                                ...
                                {
                                    float g =
                                        3.f * rfx[0][1] - 2.f * rfx[hex[d]][1] - rfx[hex[d + 1]][1];
                                    for (int c = 0; c < 4; c += 2)
                                    {
                                        rfx[0][c] =
                                            (g + 2.f * rfx[hex[d]][c] + rfx[hex[d + 1]][c]) / 3.f;
                                        redblue[d][c] = rfx[0][c];
                                    }
                                }
                                else
                            ...
                            for (int d = 0; d < ndir; d += 2, rfx += TS * TS)
                                for (int c = 0; c < 4; c += 2)
                                    rfx[0][c] = (redblue[0][c] + redblue[2][c]) * 0.5f;
                        }
...
#undef CONV_FILT
                    ...
                    qmat[0] = *(i_src + row * TS + col) - C2m - C3m - C5m - C6m - 2.0f * C7m -
                              C12m - C18m;
                    ...
                    for (int c = 0; c < 2; c++)
                        *(fdc_chroma + c * TS * TS + row * TS + col) = uv[c];
                }
        ...
        if ((a) > (b))                                                                             \
            PIX_SWAP((a), (b));                                                                    \
                    ...
                    for (int chrm = 0; chrm < 2; chrm++)
                    {
                        ...
                        memcpy(&temp[0], fdc_chroma + chrm * TS * TS + (row - 1) * TS + (col),
                               sizeof(float) * 1);
                        memcpy(&temp[1], fdc_chroma + chrm * TS * TS + (row)*TS + (col - 1),
                               sizeof(float) * 3);
                        memcpy(&temp[4], fdc_chroma + chrm * TS * TS + (row + 1) * TS + (col),
                               sizeof(float) * 1);
                    ...
                    float uv[2];
                    uv[0] =
                        (((ABS(uvf[0]) < ABS(um)) & (ABS(uvf[1]) < (1.02f * ABS(vm)))) ? uvf[0] :
                                                                                         um) *
                            hybrid_fdc[0] +
                        uvf[0] * hybrid_fdc[1];
                    uv[1] =
                        (((ABS(uvf[1]) < ABS(vm)) & (ABS(uvf[0]) < (1.02f * ABS(vm)))) ? uvf[1] :
                                                                                         vm) *
                            hybrid_fdc[0] +
                        uvf[1] * hybrid_fdc[1];
                    ...
                    for (int c = 0; c < 3; c++)
                        out[4 * (width * (row + top) + col + left) + c] = MAX(0.0f, rgbpix[c]);
                }
        }
    }
    dt_free_align(all_buffers);
...
static cl_int process_markesteijn_cl(const dt_iop_module_t *self,
                                     ...
                                     cl_mem dev_out, cl_mem dev_xtrans,
                                     const uint8_t (*const xtrans)[6], const int width,
                                     const int height)
{
    ...
    const int passes =
        ((data->demosaicing_method & ~DT_DEMOSAIC_DUAL) == DT_IOP_DEMOSAIC_MARKESTEIJN_3) ? 3 : 1;
    const int ndir = passes > 1 ? 8 : 4;
    const int pad_tile = (passes == 1) ? 12 : 17;
    process_vng_cl(self, piece, dev_in, dev_out, dev_xtrans, xtrans, width, height, 9u,
                   pad_tile + 2, TRUE);
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_markesteijn_initial_copy, width,
                                           height, CLARG(dev_in), CLARG(dev_rgb[0]), CLARG(width),
                                           CLARG(height), CLARG(dev_xtrans));
    if (err != CL_SUCCESS)
        goto error;
    ...
    dt_opencl_local_buffer_t locopt_g1_g3 =
        (dt_opencl_local_buffer_t){.xoffset = 2 * 3,
                                   .xfactor = 1,
                                   .yoffset = 2 * 3,
                                   .yfactor = 1,
                                   .cellsize = 1 * sizeof(float),
                                   .overhead = 0,
                                   .sizex = 1 << 8,
                                   .sizey = 1 << 8};
    ...
    {
        const size_t sizes[2] = {ROUNDUP(width, locopt_g1_g3.sizex),
                                 ROUNDUP(height, locopt_g1_g3.sizey)};
        const size_t local[2] = {locopt_g1_g3.sizex, locopt_g1_g3.sizey};
        err = dt_opencl_enqueue_kernel_2d_local_args(
            devid, gd->kernel_markesteijn_green_minmax, sizes, local, CLARG(dev_rgb[0]),
            CLARG(dev_gminmax), CLARG(width), CLARG(height), CLARGINT(PAD_G1_G3),
            CLARRAY(2, sgreen), CLARG(dev_xtrans), CLARG(dev_allhex),
            CLLOCAL(sizeof(float) * (locopt_g1_g3.sizex + 2 * 3) * (locopt_g1_g3.sizey + 2 * 3)));
        if (err != CL_SUCCESS)
            goto error;
    }
    ...
    dt_opencl_local_buffer_t locopt_g_interp =
        (dt_opencl_local_buffer_t){.xoffset = 2 * 6,
                                   .xfactor = 1,
                                   .yoffset = 2 * 6,
                                   .yfactor = 1,
    ...
    err = dt_opencl_local_buffer_opt(devid, gd->kernel_markesteijn_interpolate_green,
                                     &locopt_g_interp);
    if (err != CL_SUCCESS)
        goto error;

    {
        const size_t sizes[2] = {ROUNDUP(width, locopt_g_interp.sizex),
                                 ROUNDUP(height, locopt_g_interp.sizey)};
        const size_t local[2] = {locopt_g_interp.sizex, locopt_g_interp.sizey};
        err = dt_opencl_enqueue_kernel_2d_local_args(
            devid, gd->kernel_markesteijn_interpolate_green, sizes, local, CLARG(dev_rgb[0]),
            CLARG(dev_rgb[1]), CLARG(dev_rgb[2]), CLARG(dev_rgb[3]), CLARG(dev_gminmax),
            CLARG(width), CLARG(height), CLARGINT(PAD_G_INTERP), CLARRAY(2, sgreen),
            CLARG(dev_xtrans), CLARG(dev_allhex),
            CLLOCAL(sizeof(float) * 4 * (locopt_g_interp.sizex + 2 * 6) *
                    (locopt_g_interp.sizey + 2 * 6)));
        if (err != CL_SUCCESS)
            goto error;
    }
    ...
    for (int pass = 0; pass < passes; pass++)
    {
        ...
        if (pass == 1)
        {
            for (int c = 0; c < 4; c++)
            {
                err = dt_opencl_enqueue_copy_buffer_to_buffer(
                    devid, dev_rgb[c], dev_rgb[c + 4], 0, 0, sizeof(float) * 4 * width * height);
                if (err != CL_SUCCESS)
                    goto error;
            }
            // ... and process that second set of buffers
        ...
        if (pass)
        {
            ...
            err = dt_opencl_enqueue_kernel_2d_args(
                devid, gd->kernel_markesteijn_recalculate_green, width, height, CLARG(dev_rgb[0]),
                CLARG(dev_rgb[1]), CLARG(dev_rgb[2]), CLARG(dev_rgb[3]), CLARG(dev_gminmax),
        ...
        dt_opencl_local_buffer_t locopt_rb_g =
            (dt_opencl_local_buffer_t){.xoffset = 2 * 2,
                                       .xfactor = 1,
                                       .yoffset = 2 * 2,
                                       .yfactor = 1,
                                       .cellsize = 4 * sizeof(float),
        ...
        err =
            dt_opencl_local_buffer_opt(devid, gd->kernel_markesteijn_solitary_green, &locopt_rb_g);
        if (err != CL_SUCCESS)
            goto error;
        ...
        for (int d = 0, i = 1, h = 0; d < 6; d++, i ^= 1, h ^= 2)
        {
            ...
            const size_t sizes[2] = {ROUNDUP(width, locopt_rb_g.sizex),
                                     ROUNDUP(height, locopt_rb_g.sizey)};
            const size_t local[2] = {locopt_rb_g.sizex, locopt_rb_g.sizey};
            err = dt_opencl_enqueue_kernel_2d_local_args(
                devid, gd->kernel_markesteijn_solitary_green, sizes, local, CLARG(dev_trgb[0]),
                CLARG(dev_aux), CLARG(width), CLARG(height), CLARG(pad_rb_g), CLARG(d),
                CLARRAY(2, dir), CLARG(h), CLARRAY(2, sgreen), CLARG(dev_xtrans),
                CLLOCAL(sizeof(float) * 4 * (locopt_rb_g.sizex + 2 * 2) *
                        (locopt_rb_g.sizey + 2 * 2)));
            if (err != CL_SUCCESS)
                goto error;
        ...
        for (int d = 0; d < 4; d++)
        {
            const size_t sizes[2] = {ROUNDUP(width, locopt_rb_br.sizex),
                                     ROUNDUP(height, locopt_rb_br.sizey)};
            const size_t local[2] = {locopt_rb_br.sizex, locopt_rb_br.sizey};
            err = dt_opencl_enqueue_kernel_2d_local_args(
                devid, gd->kernel_markesteijn_red_and_blue, sizes, local, CLARG(dev_rgb[d]),
                CLARG(width), CLARG(height), CLARG(pad_rb_br), CLARG(d), CLARRAY(2, sgreen),
                CLARG(dev_xtrans),
                CLLOCAL(sizeof(float) * 4 * (locopt_rb_br.sizex + 2 * 3) *
                        (locopt_rb_br.sizey + 2 * 3)));
            if (err != CL_SUCCESS)
                goto error;
        }
        ...
        err = dt_opencl_local_buffer_opt(devid, gd->kernel_markesteijn_interpolate_twoxtwo,
                                         &locopt_g22);
        if (err != CL_SUCCESS)
            goto error;
        ...
        {
            const size_t sizes[2] = {ROUNDUP(width, locopt_g22.sizex),
                                     ROUNDUP(height, locopt_g22.sizey)};
            const size_t local[2] = {locopt_g22.sizex, locopt_g22.sizey};
            err = dt_opencl_enqueue_kernel_2d_local_args(
                devid, gd->kernel_markesteijn_interpolate_twoxtwo, sizes, local, CLARG(dev_rgb[n]),
                CLARG(width), CLARG(height), CLARG(pad_g22), CLARG(d), CLARRAY(2, sgreen),
                CLARG(dev_xtrans), CLARG(dev_allhex),
                CLLOCAL(sizeof(float) * 4 * (locopt_g22.sizex + 2 * 2) *
                        (locopt_g22.sizey + 2 * 2)));
            if (err != CL_SUCCESS)
                goto error;
        }
    }
    ...
    // convert to perceptual colorspace and differentiate in all directions
    const int pad_yuv = (passes == 1) ? 8 : 13;
    dt_opencl_local_buffer_t locopt_diff = (dt_opencl_local_buffer_t){.xoffset = 2 * 1,
                                                                      .xfactor = 1,
                                                                      .yoffset = 2 * 1,
                                                                      .yfactor = 1,
                                                                      .cellsize = 4 * sizeof(float),
    ...
    for (int d = 0; d < ndir; d++)
    {
        ...
        err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_markesteijn_convert_yuv, width,
                                               height, CLARG(dev_rgb[d]), CLARG(dev_aux),
                                               CLARG(width), CLARG(height), CLARG(pad_yuv));
        if (err != CL_SUCCESS)
            goto error;

        // differentiate in all directions
        const size_t sizes_diff[2] = {ROUNDUP(width, locopt_diff.sizex),
                                      ROUNDUP(height, locopt_diff.sizey)};
        const size_t local_diff[2] = {locopt_diff.sizex, locopt_diff.sizey};
        err = dt_opencl_enqueue_kernel_2d_local_args(
            devid, gd->kernel_markesteijn_differentiate, sizes_diff, local_diff, CLARG(dev_aux),
            CLARG(dev_drv[d]), CLARG(width), CLARG(height), CLARG(pad_yuv), CLARG(d),
            CLLOCAL(sizeof(float) * 4 * (locopt_diff.sizex + 2 * 1) * (locopt_diff.sizey + 2 * 1)));
        if (err != CL_SUCCESS)
            goto error;
    }
    ...
    for (int n = 0; n < ndir; n++)
    {
        ...
        dev_homosum[n] =
            dt_opencl_alloc_device_buffer(devid, sizeof(unsigned char) * width * height);
        if (dev_homosum[n] == NULL)
            goto error;
    }
    ...
    for (int d = 0; d < ndir; d++)
    {
        const int pad_homo = (passes == 1) ? 10 : 15;
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->kernel_markesteijn_homo_threshold, width, height, CLARG(dev_drv[d]),
            CLARG(dev_aux), CLARG(width), CLARG(height), CLARG(pad_homo), CLARG(d));
        if (err != CL_SUCCESS)
            goto error;
    }
    ...
    const int pad_homo = (passes == 1) ? 10 : 15;
    dt_opencl_local_buffer_t locopt_homo = (dt_opencl_local_buffer_t){.xoffset = 2 * 1,
                                                                      .xfactor = 1,
                                                                      .yoffset = 2 * 1,
                                                                      .yfactor = 1,
                                                                      .cellsize = 1 * sizeof(float),
    ...
    for (int d = 0; d < ndir; d++)
    {
        const size_t sizes[2] = {ROUNDUP(width, locopt_homo.sizex),
                                 ROUNDUP(height, locopt_homo.sizey)};
        const size_t local[2] = {locopt_homo.sizex, locopt_homo.sizey};
        err = dt_opencl_enqueue_kernel_2d_local_args(
            devid, gd->kernel_markesteijn_homo_set, sizes, local, CLARG(dev_drv[d]), CLARG(dev_aux),
            CLARG(dev_homo[d]), CLARG(width), CLARG(height), CLARG(pad_homo),
            CLLOCAL(sizeof(float) * (locopt_homo.sizex + 2 * 1) * (locopt_homo.sizey + 2 * 1)));
        if (err != CL_SUCCESS)
            goto error;
    }
    ...
    for (int d = 0; d < ndir; d++)
    {
        size_t sizes[2] = {ROUNDUP(width, locopt_homo_sum.sizex),
                           ROUNDUP(height, locopt_homo_sum.sizey)};
        size_t local[2] = {locopt_homo_sum.sizex, locopt_homo_sum.sizey};
        err = dt_opencl_enqueue_kernel_2d_local_args(
            devid, gd->kernel_markesteijn_homo_sum, sizes, local, CLARG(dev_homo[d]),
            CLARG(dev_homosum[d]), CLARG(width), CLARG(height), CLARG(pad_tile),
            CLLOCAL(sizeof(char) * (locopt_homo_sum.sizex + 2 * 2) *
                    (locopt_homo_sum.sizey + 2 * 2)));
        if (err != CL_SUCCESS)
            goto error;
    }
    ...
    {
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->kernel_markesteijn_homo_max, width, height, CLARG(dev_homosum[d]),
            CLARG(dev_aux), CLARG(width), CLARG(height), CLARG(pad_tile), CLARG(d));
        if (err != CL_SUCCESS)
            goto error;
    }

    // adjust maximum value
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_markesteijn_homo_max_corr, width,
                                           height, CLARG(dev_aux), CLARG(width), CLARG(height),
                                           CLARG(pad_tile));
    if (err != CL_SUCCESS)
        goto error;
    ...
    {
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->kernel_markesteijn_homo_quench, width, height, CLARG(dev_homosum[d]),
            CLARG(dev_homosum[d + 4]), CLARG(width), CLARG(height), CLARG(pad_tile));
        if (err != CL_SUCCESS)
            goto error;
    }
    ...
    for (int d = 0; d < ndir; d++)
    {
        err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_markesteijn_accu, width, height,
                                               CLARG(dev_t1), CLARG(dev_t2), CLARG(dev_rgbv[d]),
                                               CLARG(dev_homosum[d]), CLARG(dev_aux), CLARG(width),
    ...
    if (dev_t1 != dev_tmptmp)
    {
        ...
        err = dt_opencl_enqueue_copy_image(devid, dev_t1, dev_tmptmp, CLIMG_ORIGIN, CLIMG_ORIGIN,
                                           region);
        if (err != CL_SUCCESS)
            goto error;
    }
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_markesteijn_final, width, height,
                                           CLARG(dev_tmptmp), CLARG(dev_out), CLARG(width),
                                           CLARG(height), CLARG(pad_tile));
