...
static void dual_demosaic(dt_dev_pixelpipe_iop_t *piece, float *const restrict high_data,
                          const float *const restrict raw_data, const int width, const int height,
                          const uint32_t filters, const uint8_t (*const xtrans)[6],
                          const gboolean dual_mask, const float dual_threshold)
{
    ...
    if (dual_threshold <= 0.0f && !dual_mask)
        return;
    ...
    else
    {
        ...
        if (vng_image)
        {
            ...
            for (size_t idx = 0; idx < msize; idx++)
            {
                ...
                for (int c = 0; c < 3; c++)
                    high_data[oidx + c] =
                        interpolatef(mask[idx], high_data[oidx + c], vng_image[oidx + c]);
...
#ifdef HAVE_OPENCL
int dual_demosaic_cl(const dt_iop_module_t *self, const dt_dev_pixelpipe_iop_t *const piece,
                     cl_mem high_image, cl_mem low_image, cl_mem out, const int width,
                     const int height, const int dual_mask)
{
    dt_dev_pixelpipe_t *p = piece->pipe;
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, darktable.opencl->blendop->kernel_calc_Y0_mask,
                                           width, height, CLARG(mask), CLARG(high_image),
                                           CLARG(width), CLARG(height), CLARG(wb));
    if (err != CL_SUCCESS)
        goto finish;

    err = dt_opencl_enqueue_kernel_2d_args(
        devid, darktable.opencl->blendop->kernel_calc_scharr_mask, width, height, CLARG(mask),
        CLARG(tmp), CLARG(width), CLARG(height));
    if (err != CL_SUCCESS)
        goto finish;

    err = dt_opencl_enqueue_kernel_2d_args(devid, darktable.opencl->blendop->kernel_calc_blend,
                                           width, height, CLARG(tmp), CLARG(mask), CLARG(width),
                                           CLARG(height), CLARG(contrastf), CLARGINT(1));
    if (err != CL_SUCCESS)
        goto finish;
    ...
    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_write_blended_dual, width, height, CLARG(high_image), CLARG(low_image),
        CLARG(out), CLARG(width), CLARG(height), CLARG(tmp), CLARG(dual_mask));

finish:

    if (err != CL_SUCCESS)
        dt_print_pipe(DT_DEBUG_ALWAYS, "dual demosaic", p, self, devid, NULL, NULL, "Error: %s",
                      cl_errstr(err));
