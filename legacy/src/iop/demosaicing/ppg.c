...
static void demosaic_ppg(float *const out, const float *const in, const int width, const int height,
                         const uint32_t filters, const float thrs, const int margin)
{
    ...
    // avoid processing if outside of given border
    DT_OMP_FOR()
    for (int j = 3; j < height - 3; j++)
    {
        ...
        for (int i = 3; i < width - 3; i++)
        {
            ...
            if (c == 0 || c == 2)
            {
                ...
                const float diffx =
                    (fabsf(pxm2 - pc) + fabsf(pxM2 - pc) + fabsf(pxm - pxM)) * 3.0f +
                    (fabsf(pxM3 - pxM) + fabsf(pxm3 - pxm)) * 2.0f;
                const float guessy = (pym + pc + pyM) * 2.0f - pyM2 - pym2;
                const float diffy =
                    (fabsf(pym2 - pc) + fabsf(pyM2 - pc) + fabsf(pym - pyM)) * 3.0f +
            ...
            for_each_channel(k, aligned(buf, color : 16) dt_omp_nontemporal(buf)) buf[k] =
                fmaxf(0.0f, color[k]);
    ...
    for (int j = 0; j < height; j++)
    {
        ...
        for (int i = 0; i < width; i++)
        {
            ...
            // the outermost are untouched
            // but both final results avoid negatives
            if (j > 0 && i > 0 && i < width - 1 && j < height - 1)
            {
                ...
                else
                {
                    ...
                    {
                        // red pixel, fill blue:
                        const float diff1 = fabsf(ntl[2] - nbr[2]) + fabsf(ntl[1] - color[1]) +
                                            fabsf(nbr[1] - color[1]);
                        const float guess1 = ntl[2] + nbr[2] + 2.0f * color[1] - ntl[1] - nbr[1];
                        const float diff2 = fabsf(ntr[2] - nbl[2]) + fabsf(ntr[1] - color[1]) +
                                            fabsf(nbl[1] - color[1]);
                    ...
                    {
                        const float diff1 = fabsf(ntl[0] - nbr[0]) + fabsf(ntl[1] - color[1]) +
                                            fabsf(nbr[1] - color[1]);
                        const float guess1 = ntl[0] + nbr[0] + 2.0f * color[1] - ntl[1] - nbr[1];
                        const float diff2 = fabsf(ntr[0] - nbl[0]) + fabsf(ntr[1] - color[1]) +
                                            fabsf(nbl[1] - color[1]);
