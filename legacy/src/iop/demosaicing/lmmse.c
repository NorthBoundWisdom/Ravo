...
static void lmmse_demosaic(float *const restrict out, const float *const restrict in,
                           const int width, const int height, const uint32_t filters,
                           const dt_iop_demosaic_lmmse_t mode, const float scaler)
{
    ...
    for (int tile_vertical = 0; tile_vertical < num_vertical; tile_vertical++)
    {
        for (int tile_horizontal = 0; tile_horizontal < num_horizontal; tile_horizontal++)
        {
            ...
            for (int rrr = BORDER_AROUND, row = rowStart; rrr < tileRows + BORDER_AROUND;
                 rrr++, row++)
            {
            ...
            for (int rr = 2; rr < last_rr - 2; rr++)
            {
                ...
                for (int cc = 2 + (FC(rr, 2, filters) & 1); cc < last_cc - 2; cc += 2)
                {
                    ...
                    const float v0 =
                        0.0625f * (cfa[-w1 - 1] + cfa[-w1 + 1] + cfa[w1 - 1] + cfa[w1 + 1]) +
                        0.25f * cfa[0];
                    // horizontal
                    float *hdiff = qix[0] + rr * DT_LMMSE_TILESIZE + cc;
                    hdiff[0] = -0.25f * (cfa[-2] + cfa[2]) + 0.5f * (cfa[-1] + cfa[0] + cfa[1]);
                    const float Y0 = v0 + 0.5f * hdiff[0];
                    hdiff[0] = (cfa[0] > 1.75f * Y0) ? _median3f(hdiff[0], cfa[-1], cfa[1]) :
                                                       CLAMPF(hdiff[0], 0.0f, 1.0f);
                    hdiff[0] -= cfa[0];
                    ...
                    vdiff[0] = (cfa[0] > 1.75f * Y1) ? _median3f(vdiff[0], cfa[-w1], cfa[w1]) :
                                                       CLAMPF(vdiff[0], 0.0f, 1.0f);
                    vdiff[0] -= cfa[0];
                }
            ...
            for (int rr = 4; rr < last_rr - 4; rr++)
            {
                for (int cc = 4; cc < last_cc - 4; cc++)
                {
                    ...
                    hlp[0] = h0 * hdiff[0] + h1 * (hdiff[-1] + hdiff[1]) +
                             h2 * (hdiff[-2] + hdiff[2]) + h3 * (hdiff[-3] + hdiff[3]) +
                             h4 * (hdiff[-4] + hdiff[4]);
                    vlp[0] = h0 * vdiff[0] + h1 * (vdiff[-w1] + vdiff[w1]) +
                             h2 * (vdiff[-w2] + vdiff[w2]) + h3 * (vdiff[-w3] + vdiff[w3]) +
                             h4 * (vdiff[-w4] + vdiff[w4]);
                }
            }
            ...
            for (int rr = 4; rr < last_rr - 4; rr++)
            {
                for (int cc = 4 + (FC(rr, 4, filters) & 1); cc < last_cc - 4; cc += 2)
                {
                    ...
                    float vx = 1e-7f + sqrf(p1 - mu) + sqrf(p2 - mu) + sqrf(p3 - mu) +
                               sqrf(p4 - mu) + sqrf(p5 - mu) + sqrf(p6 - mu) + sqrf(p7 - mu) +
                               sqrf(p8 - mu) + sqrf(p9 - mu);
                    p1 -= hdiff[-4];
                    p2 -= hdiff[-3];
                    p3 -= hdiff[-2];
                    p4 -= hdiff[-1];
                    ...
                    p8 -= hdiff[3];
                    p9 -= hdiff[4];
                    float vn = 1e-7f + sqrf(p1) + sqrf(p2) + sqrf(p3) + sqrf(p4) + sqrf(p5) +
                               sqrf(p6) + sqrf(p7) + sqrf(p8) + sqrf(p9);
                    ...
                    vx = 1e-7f + sqrf(p1 - mu) + sqrf(p2 - mu) + sqrf(p3 - mu) + sqrf(p4 - mu) +
                         sqrf(p5 - mu) + sqrf(p6 - mu) + sqrf(p7 - mu) + sqrf(p8 - mu) +
                         sqrf(p9 - mu);
                    p1 -= vdiff[-w4];
                    p2 -= vdiff[-w3];
                    p3 -= vdiff[-w2];
                    ...
                    p8 -= vdiff[w3];
                    p9 -= vdiff[w4];
                    vn = 1e-7f + sqrf(p1) + sqrf(p2) + sqrf(p3) + sqrf(p4) + sqrf(p5) + sqrf(p6) +
                         sqrf(p7) + sqrf(p8) + sqrf(p9);
            ...
            for (int rr = 0, row_in = rowStart - BORDER_AROUND; rr < last_rr; rr++, row_in++)
            {
                for (int cc = 0, col_in = colStart - BORDER_AROUND; cc < last_cc; cc++, col_in++)
                {
                    ...
                    const gboolean inside =
                        ((row_in >= 0) && (row_in < height) && (col_in >= 0) && (col_in < width));
            ...
            {
                for (int cc = 1 + (FC(rr, 2, filters) & 1), c = FC(rr, cc + 1, filters);
                     cc < last_cc - 1; cc += 2)
                {
                    float *colc = qix[c] + rr * DT_LMMSE_TILESIZE + cc;
                    float *col1 = qix[1] + rr * DT_LMMSE_TILESIZE + cc;
            ...
            for (int rr = 1; rr < last_rr - 1; rr++)
            {
                for (int cc = 1 + (FC(rr, 1, filters) & 1), c = 2 - FC(rr, cc, filters);
                     cc < last_cc - 1; cc += 2)
                {
                    float *colc = qix[c] + rr * DT_LMMSE_TILESIZE + cc;
                    float *col1 = qix[1] + rr * DT_LMMSE_TILESIZE + cc;
                    colc[0] = col1[0] + 0.25f * (colc[-w1] - col1[-w1] + colc[-1] - col1[-1] +
                                                 colc[1] - col1[1] + colc[w1] - col1[w1]);
                }
            }
            ...
            for (int step = 0; step < refine; step++)
            {
                ...
                {
                    for (int cc = ccmin + 2 + (FC(rr, 2, filters) & 1), c = FC(rr, cc, filters);
                         cc < ccmax - 2; cc += 2)
                    {
                        ...
                        const float dL =
                            1.0f / (1.0f + fabsf(rgbc[-2] - rgbc[0]) + fabsf(rgb1[1] - rgb1[-1]));
                        const float dR =
                            1.0f / (1.0f + fabsf(rgbc[2] - rgbc[0]) + fabsf(rgb1[1] - rgb1[-1]));
                        const float dU = 1.0f / (1.0f + fabsf(rgbc[-w2] - rgbc[0]) +
                                                 fabsf(rgb1[w1] - rgb1[-w1]));
                        const float dD =
                            1.0f / (1.0f + fabsf(rgbc[w2] - rgbc[0]) + fabsf(rgb1[w1] - rgb1[-w1]));
                        rgb1[0] =
                            (rgbc[0] + ((rgb1[-1] - rgbc[-1]) * dL + (rgb1[1] - rgbc[1]) * dR +
                                        (rgb1[-w1] - rgbc[-w1]) * dU + (rgb1[w1] - rgbc[w1]) * dD) /
                                           (dL + dR + dU + dD));
                    }
                }
                ...
                for (int rr = rrmin + 2; rr < rrmax - 2; rr++)
                {
                    for (int cc = ccmin + 2 + (FC(rr, 3, filters) & 1), c = FC(rr, cc + 1, filters);
                         cc < ccmax - 2; cc += 2)
                    {
                        for (int i = 0; i < 2; c = 2 - c, i++)
                        {
                            ...
                            const float dL = 1.0f / (1.0f + fabsf(rgb1[-2] - rgb1[0]) +
                                                     fabsf(rgbc[1] - rgbc[-1]));
                            const float dR = 1.0f / (1.0f + fabsf(rgb1[2] - rgb1[0]) +
                                                     fabsf(rgbc[1] - rgbc[-1]));
                            const float dU = 1.0f / (1.0f + fabsf(rgb1[-w2] - rgb1[0]) +
                                                     fabsf(rgbc[w1] - rgbc[-w1]));
                            const float dD = 1.0f / (1.0f + fabsf(rgb1[w2] - rgb1[0]) +
                                                     fabsf(rgbc[w1] - rgbc[-w1]));
                            rgbc[0] = (rgb1[0] -
                                       ((rgb1[-1] - rgbc[-1]) * dL + (rgb1[1] - rgbc[1]) * dR +
                                        (rgb1[-w1] - rgbc[-w1]) * dU + (rgb1[w1] - rgbc[w1]) * dD) /
                                           (dL + dR + dU + dD));
                        }
                    }
                ...
                for (int rr = rrmin + 2; rr < rrmax - 2; rr++)
                {
                    for (int cc = ccmin + 2 + (FC(rr, 2, filters) & 1), c = 2 - FC(rr, cc, filters);
                         cc < ccmax - 2; cc += 2)
                    {
                        const int d = 2 - c;
                        float *rgb1 = qix[1] + rr * DT_LMMSE_TILESIZE + cc;
                        ...
                        const float dL =
                            1.0f / (1.0f + fabsf(rgbd[-2] - rgbd[0]) + fabsf(rgb1[1] - rgb1[-1]));
                        const float dR =
                            1.0f / (1.0f + fabsf(rgbd[2] - rgbd[0]) + fabsf(rgb1[1] - rgb1[-1]));
                        const float dU = 1.0f / (1.0f + fabsf(rgbd[-w2] - rgbd[0]) +
                                                 fabsf(rgb1[w1] - rgb1[-w1]));
                        const float dD =
                            1.0f / (1.0f + fabsf(rgbd[w2] - rgbd[0]) + fabsf(rgb1[w1] - rgb1[-w1]));
                        rgbc[0] =
                            (rgb1[0] - ((rgb1[-1] - rgbc[-1]) * dL + (rgb1[1] - rgbc[1]) * dR +
                                        (rgb1[-w1] - rgbc[-w1]) * dU + (rgb1[w1] - rgbc[w1]) * dD) /
                                           (dL + dR + dU + dD));
                    }
                }
            ...
            const int first_vertical =
                rowStart + ((tile_vertical == 0) ? BORDER_AROUND : LMMSE_OVERLAP);
            const int last_vertical =
                rowEnd - ((tile_vertical == num_vertical - 1) ? BORDER_AROUND : LMMSE_OVERLAP);
            const int first_horizontal =
                colStart + ((tile_horizontal == 0) ? BORDER_AROUND : LMMSE_OVERLAP);
            const int last_horizontal =
                colEnd - ((tile_horizontal == num_horizontal - 1) ? BORDER_AROUND : LMMSE_OVERLAP);
            for (int row = first_vertical, rr = row - rowStart + BORDER_AROUND; row < last_vertical;
                 row++, rr++)
            {
                float *dest = out + 4 * (row * width + first_horizontal);
                const int idx =
                    rr * DT_LMMSE_TILESIZE + first_horizontal - colStart + BORDER_AROUND;
                float *col0 = qix[0] + idx;
                float *col1 = qix[1] + idx;
                float *col2 = qix[2] + idx;
                for (int col = first_horizontal; col < last_horizontal;
                     col++, dest += 4, col0++, col1++, col2++)
                {
