...
#pragma GCC optimize("unroll-loops", "tree-loop-if-convert", "tree-loop-distribution",             \
...
typedef struct dt_iop_channelmixer_rgb_params_t
{
    ...
    gboolean normalize_R, normalize_G, normalize_B, normalize_sat, normalize_light,
        normalize_grey;              // $DESCRIPTION: "normalize channels"
...
typedef struct dt_iop_channelmixer_rgb_gui_data_t
{
    ...
    GtkWidget *normalize_R, *normalize_G, *normalize_B, *normalize_sat, *normalize_light,
        *normalize_grey;
    ...
    point_t box
        [4]; // the current coordinates, possibly non rectangle, of the bounding box for the color checker
    point_t ideal_box
        [4]; // the desired coordinates of the perfect rectangle bounding box for the color checker
    point_t center_box; // the barycenter of both boxes
    gboolean
        active_node[4]; // true if the cursor is close to a node (node = corner of the bounding box)
    ...
    GtkWidget *checkers_list, *optimize, *safety, *label_delta_E, *button_profile, *button_validate,
        *button_commit;
...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self,
                                  _("perform color space corrections\n"
                                  ...
                                  _("linear, RGB or XYZ"), _("linear, RGB, scene-referred"));
}
...
void init_presets(dt_iop_module_so_t *self)
{
    ...
    dt_gui_presets_add_generic(_("monochrome | luminance-based"), self->op, self->version(), &p,
  ...
  */
    ...
    dt_gui_presets_add_generic(_("monochrome | ILFORD HP5+"), self->op, self->version(), &p,
                               sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_SCENE);
    ...
    dt_gui_presets_add_generic(_("monochrome | ILFORD DELTA 100"), self->op, self->version(), &p,
                               sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_SCENE);
    ...
    dt_gui_presets_add_generic(_("monochrome | ILFORD DELTA 400 - 3200"), self->op, self->version(),
    ...
    dt_gui_presets_add_generic(_("monochrome | ILFORD FP4+"), self->op, self->version(), &p,
                               sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_SCENE);
    ...
    dt_gui_presets_add_generic(_("monochrome | Fuji Acros 100"), self->op, self->version(), &p,
                               sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_SCENE);
    ...
    p.clip = FALSE;
    dt_gui_presets_add_generic(_("basic channel mixer"), self->op, self->version(), &p, sizeof(p),
                               TRUE, DEVELOP_BLEND_CS_RGB_SCENE);
    ...
    p.blue[1] = 1.f;
    p.blue[2] = 0.f;
    dt_gui_presets_add_generic(_("channel swap | swap G and B"), self->op, self->version(), &p,
                               sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_SCENE);
    ...
    p.blue[1] = 0.f;
    p.blue[2] = 1.f;
    dt_gui_presets_add_generic(_("channel swap | swap G and R"), self->op, self->version(), &p,
                               sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_SCENE);
    ...
    p.blue[1] = 0.f;
    p.blue[2] = 0.f;
    dt_gui_presets_add_generic(_("channel swap | swap R and B"), self->op, self->version(), &p,
                               sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_SCENE);
}
...
static gboolean _dev_is_D65_chroma(const dt_develop_t *dev)
{
    ...
    return chr->late_correction ? dt_dev_equal_chroma(chr->wb_coeffs, chr->as_shot) :
...
static gboolean _get_white_balance_coeff(const dt_iop_module_t *self, dt_aligned_pixel_t custom_wb)
{
...
static inline void _gamut_mapping(const dt_aligned_pixel_t input, const float compression,
                                  const gboolean clip, dt_aligned_pixel_t output)
{
    // Get the sum XYZ
    ...
    dt_aligned_pixel_t xyY = {sum > 0.0f ? input[0] / sum : D50xyY.x,
                              sum > 0.0f ? input[1] / sum : D50xyY.y, Y, 0.0f};
    ...
    for (size_t c = 0; c < 2; c++)
        xyY[c] = (sanitize) ? xyY[c] / scale : xyY[c];

    // Convert back to XYZ
    dt_xyY_to_XYZ(xyY, output);
}

DT_OMP_DECLARE_SIMD(aligned(input, output, saturation, lightness : 16)
                        uniform(saturation, lightness))
static inline void _luma_chroma(const dt_aligned_pixel_t input, const dt_aligned_pixel_t saturation,
                                ...
                                const dt_iop_channelmixer_rgb_version_t version)
{
    ...
    if (norm > 0.f && avg > 0.f)
    {
        ...
        if (version == CHANNELMIXERRGB_V_3)
            norm *= INVERSE_SQRT_3;

        // Ratios
        for_three_channels(c) output[c] = input[c] / norm;
        ...
        {
            for_three_channels(c) coeff_ratio += sqf(1.0f - output[c]) * saturation[c];
        }
        else
        ...
        if (version == CHANNELMIXERRGB_V_3)
            norm /= euclidean_norm(output) * INVERSE_SQRT_3;

        // Apply colorfulness adjustment channel-wise and repack with
        // lightness to get LMS back
        norm *= fmaxf(1.f + mix / avg, 0.f);
        for_three_channels(c) output[c] *= norm;
    }
    else
    {
        // we have black, 0 stays 0, no luminance = no color
        for_three_channels(c) output[c] = input[c];
    }
}

DT_OMP_DECLARE_SIMD(aligned(in, out, XYZ_to_RGB, RGB_to_XYZ, MIX : 64)
                        aligned(illuminant, saturation, lightness, grey : 16))
static inline void
_loop_switch(const float *const restrict in, float *const restrict out, const size_t width,
             const size_t height, const size_t ch, const dt_colormatrix_t XYZ_to_RGB,
             ...
             const dt_aligned_pixel_t lightness, const dt_aligned_pixel_t grey, const float p,
             const float gamut, const gboolean clip, const gboolean apply_grey,
             const dt_adaptation_t kind, const dt_iop_channelmixer_rgb_version_t version)
{
    ...
    for (size_t k = 0; k < height * width * 4; k += 4)
    {
        ...
        {
            // No white balance.
            for_four_channels(c) temp_one[c] =
                0.0f; //keep compiler happy by ensuring that always initialized
        }
        }
...
static void _declare_cat_on_pipe(dt_iop_module_t *self, const gboolean preset)
{
    ...
    if (preset || (self->enabled && !g->is_blending &&
    ...
    if (origcat != chr->adaptation)
        dt_print(DT_DEBUG_PIPE, "changed CAT for %s%s from %p to %p", self->op,
                 dt_iop_get_instance_id(self), origcat, chr->adaptation);
}
...
static void _check_if_close_to_daylight(const float x, const float y, float *temperature,
                                            ...
                                            float *const restrict max_delta_E)
{
    ...
    for (size_t k = 0; k < checker->patches; k++)
    {
        ...
        if (h_ref_prime < 0.f)
            h_ref_prime = DT_2PI_F - h_ref_prime;
        if (h_test_prime < 0.f)
            h_test_prime = DT_2PI_F - h_test_prime;
        ...
        const float T = 1.f - 0.17f * cosf(deg2radf(H_avg_prime - 30)) +
...
static const extraction_result_t
_extract_patches(const float *const restrict in, const dt_iop_roi_t *const roi_in,
                 dt_iop_channelmixer_rgb_gui_data_t *g, const dt_colormatrix_t RGB_to_XYZ,
                 const dt_colormatrix_t XYZ_to_CAM, float *const restrict patches,
                 const gboolean normalize_exposure)
{
    ...
    for (size_t k = 0; k < g->checker->patches; k++)
    {
        ...
        for_three_channels(c) patches[k * 4 + c] /= (float)num_elem;
    ...
    for_three_channels(c) XYZ_white_test[c] = patches[g->checker->white * 4 + c];
    ...
    if (normalize_exposure)
    {
        for (size_t k = 0; k < g->checker->patches; k++)
        {
            ...
            for_three_channels(c) sample[c] *= luma_correction * exposure;
        }
    }
    ...
    if (XYZ_to_CAM)
    {
        ...
        for (size_t k = 0; k < g->checker->patches; k++)
        {
            ...
            for_three_channels(c) RGB_test[c] = RGB_test[c] / user_exposure / exposure + user_black;
...
static void _extract_color_checker(const float *const restrict in, float *const restrict out,
                                   const dt_iop_roi_t *const roi_in,
                                   dt_iop_channelmixer_rgb_gui_data_t *g,
                                   const dt_colormatrix_t RGB_to_XYZ,
                                   const dt_colormatrix_t XYZ_to_RGB,
                                   const dt_colormatrix_t XYZ_to_CAM, const dt_adaptation_t kind)
{
    ...
    extraction_result_t extraction_result =
        _extract_patches(out, roi_in, g, RGB_to_XYZ, XYZ_to_CAM, patches, TRUE);

    // Compute the delta E
    float pre_wb_delta_E = 0.f;
    float pre_wb_max_delta_E = 0.f;
    _compute_patches_delta_E(patches, g->checker, g->delta_E_in, &pre_wb_delta_E,
                             &pre_wb_max_delta_E);
    ...
    for_three_channels(c) illuminant[c] = D50_LMS[c] * LMS_grey_test[c] / LMS_grey_ref[c];
    ...
    _compute_patches_delta_E(patches, g->checker, g->delta_E_in, &post_wb_delta_E,
                             &post_wb_max_delta_E);
    ...
    _compute_patches_delta_E(patches, g->checker, g->delta_E_in, &post_mix_delta_E,
                             &post_mix_max_delta_E);
    ...
    g->delta_E_label_text = g_strdup_printf(
...
static void
_validate_color_checker(const float *const restrict in, const dt_iop_roi_t *const roi_in,
                        dt_iop_channelmixer_rgb_gui_data_t *g, const dt_colormatrix_t RGB_to_XYZ,
                        const dt_colormatrix_t XYZ_to_RGB, const dt_colormatrix_t XYZ_to_CAM)
{
...
static void _set_trouble_messages(dt_iop_module_t *self)
{
    ...
    const gboolean problem1 =
        valid && chr->adaptation == self && temperature_enabled && !_dev_is_D65_chroma(dev);
    ...
    const gboolean problem2 = valid && adaptation_enabled && temperature_enabled &&
                              chr->adaptation != self && !g->is_blending;
    ...
    const gboolean problem3 =
        valid && adaptation_enabled && !temperature_enabled && chr->temperature->default_enabled;

    const gboolean anyproblem = problem1 || problem2 || problem3;

    const dt_image_t *img = &dev->image_storage;
    dt_print_pipe(DT_DEBUG_PIPE, anyproblem ? "chroma trouble" : "chroma data", NULL, self,
                  DT_DEVICE_NONE, NULL, NULL,
                  "%s%s%sD65=%s.  D65 %.3f %.3f %.3f, AS-SHOT %.3f %.3f %.3f ID=%i",
                  problem1 ? "white balance applied twice, " : "",
                  problem2 ? "double CAT applied, " : "", problem3 ? "white balance missing, " : "",
                  STR_YESNO(_dev_is_D65_chroma(dev)), chr->D65coeffs[0], chr->D65coeffs[1],
                  chr->D65coeffs[2], chr->as_shot[0], chr->as_shot[1], chr->as_shot[2], img->id);
    ...
    if (problem1)
    {
        dt_iop_set_module_trouble_message(
            chr->temperature, _("white balance applied twice (<u>details</u>)"),
        ...
        dt_iop_set_module_trouble_message(
            self, _("white balance module error (<u>details</u>)"),
    ...
    {
        dt_iop_set_module_trouble_message(
            chr->temperature, _("white balance missing (<u>details</u>)"),
        ...
        dt_iop_set_module_trouble_message(
            self, _("white balance missing (<u>details</u>)"),
             ...
             const dt_iop_roi_t *const roi_out)
{
    ...
    if (self->dev->gui_attached && g)
    {
        if (g->run_profile && dt_pipe_is_preview(piece->pipe))
        {
            ...
            _extract_color_checker(in, out, roi_in, g, RGB_to_XYZ, XYZ_to_RGB, XYZ_to_CAM,
                                   data->adaptation);
            g->run_profile = FALSE;
    ...
    case DT_ADAPTATION_FULL_BRADFORD:
    {
        _loop_switch(in, out, roi_out->width, roi_out->height, ch, XYZ_to_RGB, RGB_to_XYZ,
                     data->MIX, data->illuminant, data->saturation, data->lightness, data->grey,
...
#if HAVE_OPENCL
int process_cl(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece, const cl_mem dev_in,
               cl_mem dev_out, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    const cl_mem MIX_cl = dt_opencl_copy_host_to_device_constant(devid, 12 * sizeof(float), d->MIX);
    ...
    err = dt_opencl_enqueue_kernel_2d_args(
        devid, kernel, width, height, CLARG(dev_in), CLARG(dev_out), CLARG(width), CLARG(height),
        CLARG(input_matrix_cl), CLARG(output_matrix_cl), CLARG(MIX_cl), CLARG(d->illuminant),
        CLARG(d->saturation), CLARG(d->lightness), CLARG(d->grey), CLARG(d->p), CLARG(d->gamut),
        CLARG(d->clip), CLARG(d->apply_grey), CLARG(d->version));
...
void init_global(dt_iop_module_so_t *self)
{
    ...
    dt_iop_channelmixer_rgb_global_data_t *gd =
        malloc(sizeof(dt_iop_channelmixer_rgb_global_data_t));
...
static inline void _init_bounding_box(dt_iop_channelmixer_rgb_gui_data_t *g, const float width,
                                      const float height)
{
    if (!g->checker_ready)
    {
                ...
                const int which, const float zoom_scale)
{
    ...
    dt_iop_channelmixer_rgb_gui_data_t *g = self->gui_data;
    if (g == NULL || !g->is_profiling_started)
        return 0;
    if (g->box[0].x == -1.0f || g->box[1].y == -1.0f)
        return 0;
                    ...
                    const uint32_t state, const float zoom_scale)
{
    ...
    if (g->box[0].x == -1.0f || g->box[1].y == -1.0f)
        return 0;
    if (!g->is_cursor_close || !g->drag_drop)
        return 0;
...
void gui_post_expose(dt_iop_module_t *self, cairo_t *cr, const float width, const float height,
                     const float pointerx, const float pointery, const float zoom_scale)
{
    const dt_iop_order_iccprofile_info_t *const work_profile =
  ...
  */
    ...
    for (size_t k = 0; k < g->checker->patches; k++)
    {
        ...
        cairo_arc(cr, new_center.x, new_center.y, 0.25 * (radius_x + radius_y) * scaling, 0,
                  2. * M_PI);
        cairo_fill(cr);
    }
}
...
static void _run_validation_callback(GtkWidget *widget, dt_iop_module_t *self)
{
    DT_GUARD_GUI_UPDATE();
    dt_iop_channelmixer_rgb_gui_data_t *g = self->gui_data;
                   ...
                   dt_dev_pixelpipe_iop_t *piece)
{
    ...
    dt_aligned_pixel_t custom_wb;
    _get_white_balance_coeff(self, custom_wb);
    illuminant_to_xy(p->illuminant, &(self->dev->image_storage), custom_wb, &x, &y, p->temperature,
                     p->illum_fluo, p->illum_led);
    ...
    dt_print(
        ...
        run_profile ? " [profile]" : "", run_validation ? " [validation]" : "", (int)p->temperature,
        x, y, XYZ[0], XYZ[1], XYZ[2], d->illuminant[0], d->illuminant[1], d->illuminant[2],
    ...
    const gboolean is_blending =
        (mask_mode & DEVELOP_MASK_ENABLED) && (mask_mode >= DEVELOP_MASK_MASK);
    if (g)
        g->is_blending = is_blending;
}
                                ...
                                const dt_aligned_pixel_t LMS, dt_aligned_pixel_t RGB)
{
    ...
    else
    {
        ...
        if (work_profile)
        {
            ...
            for_three_channels(c) RGB[c] = fmaxf(RGB[c] / max_RGB, 0.f);
        }
        else
        {
...
static void _update_RGB_slider_stop(const dt_iop_channelmixer_rgb_params_t *p,
                                    const dt_iop_order_iccprofile_info_t *const work_profile,
                                    GtkWidget *w, const float stop, const float c, const float r,
                                    const float g, const float b)
{
    const dt_aligned_pixel_t LMS = {0.5f * (c * r + 1 - r), 0.5f * (c * g + 1 - g),
                                    0.5f * (c * b + 1 - b)};
...
static void _update_RGB_colors(dt_iop_module_t *self, const float r, const float g, const float b,
                               const gboolean normalize, const float *a, GtkWidget *w_r,
                               GtkWidget *w_g, GtkWidget *w_b)
{
    // update the fill background color of x, y sliders
    const dt_iop_channelmixer_rgb_params_t *p = self->params;
    const dt_iop_order_iccprofile_info_t *const work_profile =
...
static void _illum_xy_callback(GtkWidget *slider, dt_iop_module_t *self)
{
    DT_GUARD_GUI_UPDATE();
    dt_iop_channelmixer_rgb_params_t *p = self->params;
    const dt_iop_channelmixer_rgb_gui_data_t *g = self->gui_data;
...
void reload_defaults(dt_iop_module_t *self)
{
    ...
    const gboolean CAT_already_applied = (self->dev->chroma.adaptation != NULL) // CAT exists
                                         &&
    ...
    else
    {
        ...
        if (!_get_white_balance_coeff(self, custom_wb))
        {
            ...
            _check_if_close_to_daylight(d->x, d->y, &(d->temperature), &(d->illuminant),
                                        &(d->adaptation));
        }
    }
...
void gui_changed(dt_iop_module_t *self, GtkWidget *w, void *previous)
{
    ...
    if (w == g->illuminant)
    {
        if (previous)
        {
            ...
            if (*prev_illuminant == DT_ILLUMINANT_CAMERA)
            {
                ...
                dt_aligned_pixel_t custom_wb;
                _get_white_balance_coeff(self, custom_wb);
                find_temperature_from_raw_coeffs(&(self->dev->image_storage), custom_wb, &(p->x),
                                                 &(p->y));
    ...
    if (w == g->illuminant || w == g->illum_fluo || w == g->illum_led || w == g->temperature)
    {
        ...
        if (p->illuminant != DT_ILLUMINANT_CUSTOM && p->illuminant != DT_ILLUMINANT_CAMERA)
        {
            ...
            illuminant_to_xy(p->illuminant, NULL, NULL, &(p->x), &(p->y), p->temperature,
                             p->illum_fluo, p->illum_led);
        }
    ...
    if (!w || w == g->hue_spot || w == g->chroma_spot || w == g->lightness_spot ||
        w == g->spot_settings)
    {
        _paint_hue(self);
    }
    ...
    if (!w || w == g->adaptation || w == g->scale_red_R || w == g->scale_red_G ||
        w == g->scale_red_B || w == g->normalize_R)
        _update_RGB_colors(self, 1, 0, 0, p->normalize_R, p->red, g->scale_red_R, g->scale_red_G,
                           g->scale_red_B);

    if (!w || w == g->adaptation || w == g->scale_green_R || w == g->scale_green_G ||
        w == g->scale_green_B || w == g->normalize_G)
        _update_RGB_colors(self, 0, 1, 0, p->normalize_G, p->green, g->scale_green_R,
                           g->scale_green_G, g->scale_green_B);

    if (!w || w == g->adaptation || w == g->scale_blue_R || w == g->scale_blue_G ||
        w == g->scale_blue_B || w == g->normalize_B)
        _update_RGB_colors(self, 0, 0, 1, p->normalize_B, p->blue, g->scale_blue_R, g->scale_blue_G,
                           g->scale_blue_B);
...
static void _auto_set_illuminant(dt_iop_module_t *self, dt_dev_pixelpipe_t *pipe)
{
    ...
    if (mode == DT_SPOT_MODE_MEASURE)
    {
        ...
        dt_aligned_pixel_t custom_wb;
        _get_white_balance_coeff(self, custom_wb);
        illuminant_to_xy(p->illuminant, &(self->dev->image_storage), custom_wb, &x, &y,
    ...
    else if (mode == DT_SPOT_MODE_CORRECT)
    {
      ...
      */
        ...
        // Convert to xyY
        const float sum =
            fmaxf(illuminant_XYZ[0] + illuminant_XYZ[1] + illuminant_XYZ[2], NORM_MIN);
...
void gui_init(dt_iop_module_t *self)
{
    ...
    gtk_widget_set_size_request(g->illum_color,
    ...
    dt_gui_box_add(self->widget,
                   dt_gui_hbox(g->approx_cct, dt_gui_expand(g->illum_color), g->color_picker));
    ...
    g_signal_connect(G_OBJECT(g->illum_x), "value-changed", G_CALLBACK(_illum_xy_callback), self);
    ...
    g_signal_connect(G_OBJECT(g->illum_y), "value-changed", G_CALLBACK(_illum_xy_callback), self);
    ...
    dt_gui_new_collapsible_section(
        &g->csspot, "plugins/darkroom/channelmixerrgb/expand_picker_mapping",
        _area_mapping_section_text(g), GTK_BOX(self->widget), DT_ACTION(self));
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(
        ...
        0, NULL, self, N_("correction"), N_("measure"));
    ...
    gtk_widget_set_size_request(g->target_spot, -1,
                                DT_PIXEL_APPLY_DPI(darktable.bauhaus->quad_width));
    ...
    dt_gui_box_add(g->csspot.container, g->spot_mode, g->use_mixing,
                   dt_gui_hbox(dt_gui_vbox(dt_ui_section_label_new(C_("section", "input")),
                                           g->origin_spot, g->Lch_origin),
                               dt_gui_expand(dt_gui_vbox(
                                   dt_ui_section_label_new(C_("section", "target")), g->target_spot,
                                   g->lightness_spot, g->hue_spot, g->chroma_spot))));

    gtk_widget_set_margin_end(gtk_widget_get_parent(g->origin_spot),
                              DT_PIXEL_APPLY_DPI(darktable.bauhaus->quad_width));
    dt_gui_collapsible_section_set_label(&g->csspot, _area_mapping_section_text(g));
...
#define NOTEBOOK_PAGE(var, short, label, tooltip, section, swap, soft_range, sr_min, sr_max)       \
    ...
    if (soft_range)                                                                                \
        dt_bauhaus_slider_set_soft_range(first, sr_min, sr_max);                                   \
                                                                                                   \
    ...
    if (soft_range)                                                                                \
        dt_bauhaus_slider_set_soft_range(second, sr_min, sr_max);                                  \
                                                                                                   \
    ...
    if (soft_range)                                                                                \
        dt_bauhaus_slider_set_soft_range(third, sr_min, sr_max);                                   \
                                                                                                   \
    g->scale_##var##_R = swap ? third : first;                                                     \
    g->scale_##var##_G = second;                                                                   \
    g->scale_##var##_B = swap ? first : third;                                                     \
                                                                                                   \
    g->normalize_##short = dt_bauhaus_toggle_from_params(DT_IOP_SECTION_FOR_PARAMS(self, section), \
                                                         "normalize_" #short);
    ...
    NOTEBOOK_PAGE(saturation, sat, N_("colorfulness"), N_("output colorfulness"),
                  N_("colorfulness"), FALSE, TRUE, -1.0, 1.0)
    g->saturation_version = dt_bauhaus_combobox_from_params(self, "version");
    NOTEBOOK_PAGE(lightness, light, N_("brightness"), N_("output brightness"), N_("brightness"),
                  FALSE, TRUE, -1.0, 1.0)
    ...
    dt_gui_new_collapsible_section(&g->cs, "plugins/darkroom/channelmixerrgb/expand_values",
                                   _("calibrate with a color checker"), GTK_BOX(self->widget),
                                   DT_ACTION(self));

    gtk_widget_set_tooltip_text(g->cs.expander,
                                _("use a color checker target to autoset CAT and channels"));
    g_signal_connect(G_OBJECT(g->cs.toggle), "toggled", G_CALLBACK(_start_profiling_callback),
                     self);

    DT_BAUHAUS_COMBOBOX_NEW_FULL(
        g->checkers_list, self, N_("calibrate"), N_("chart"),
        _("choose the vendor and the type of your chart"), 0, _checker_changed_callback, self,
        N_("Xrite ColorChecker 24 pre-2014"), N_("Xrite/Calibrite ColorChecker 24 post-2014"),
        N_("Datacolor SpyderCheckr 24 pre-2018"), N_("Datacolor SpyderCheckr 24 post-2018"),
        N_("Datacolor SpyderCheckr 48 pre-2018"), N_("Datacolor SpyderCheckr 48 post-2018"),
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(
        ...
        0, _optimize_changed_callback, self, N_("none"), N_("neutral colors"),
        N_("saturated colors"), N_("skin and soil colors"), N_("foliage colors"),
        N_("sky and water colors"), N_("average delta E"), N_("maximum delta E"));
    ...
    gtk_widget_set_tooltip_text(
        g->safety, _("reduce the radius of the patches to select the more or less central part.\n"
                     "useful when the perspective correction is sloppy or\n"
                     "the patches frame cast a shadows on the edges of the patch."));
    g_signal_connect(G_OBJECT(g->safety), "value-changed", G_CALLBACK(_safety_changed_callback),
                     self);
    ...
    dt_action_define_iop(self, N_("calibrate"), N_("accept"), g->button_commit,
                         &dt_action_def_button);
    g_signal_connect(G_OBJECT(g->button_commit), "clicked", G_CALLBACK(_commit_profile_callback),
                     (gpointer)self);
    ...
    dt_action_define_iop(self, N_("calibrate"), N_("recompute"), g->button_profile,
                         &dt_action_def_button);
    g_signal_connect(G_OBJECT(g->button_profile), "clicked", G_CALLBACK(_run_profile_callback),
                     (gpointer)self);
    ...
    dt_action_define_iop(self, N_("calibrate"), N_("validate"), g->button_validate,
                         &dt_action_def_button);
    g_signal_connect(G_OBJECT(g->button_validate), "clicked", G_CALLBACK(_run_validation_callback),
                     (gpointer)self);
    gtk_widget_set_tooltip_text(g->button_validate, _("check the output delta E"));

    dt_gui_box_add(
        g->cs.container, g->checkers_list, g->optimize, g->safety, g->label_delta_E,
        dt_gui_hbox(dt_gui_align_right(g->button_validate), g->button_profile, g->button_commit));
}
