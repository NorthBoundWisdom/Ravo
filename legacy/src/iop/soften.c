...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self, _("create a softened image using the Orton effect"),
                                  _("creative"), _("linear, RGB, display-referred"),
                                  _("linear, RGB"), _("linear, RGB, display-referred"));
}
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
   ...
   * unproven. ask me if you need details. ulrich */
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_soften_overexposed, width, height,
                                           ...
                                           CLARG(height), CLARG(saturation), CLARG(brightness));
    if (err != CL_SUCCESS)
        goto error;
    ...
    if (rad != 0)
    {
        ...
        err = dt_opencl_enqueue_kernel_2d_local_args(
            devid, gd->kernel_soften_hblur, sizes, local, CLARG(dev_tmp), CLARG(dev_out),
        ...
        local[1] = vblocksize;
        err = dt_opencl_enqueue_kernel_2d_local_args(
            devid, gd->kernel_soften_vblur, sizes, local, CLARG(dev_out), CLARG(dev_tmp),
            CLARG(dev_m), CLARG(wdh), CLARG(width), CLARG(height), CLARG(vblocksize),
            CLLOCAL((vblocksize + 2 * wdh) * 4 * sizeof(float)));
        if (err != CL_SUCCESS)
            goto error;
    }
