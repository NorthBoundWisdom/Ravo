...
{
    return dt_iop_set_description(self, _("flip or rotate image by step of 90 degrees"),
                               ...
                               float *const restrict points, const size_t points_count)
{
    ...
    for (size_t i = 0; i < points_count * 2; i += 2)
    {
        ...
        if (d->orientation & ORIENTATION_FLIP_X)
            x = piece->buf_in.width - x;
        if (d->orientation & ORIENTATION_FLIP_Y)
            y = piece->buf_in.height - y;
                   ...
                   const dt_iop_roi_t *roi_out, dt_iop_roi_t *roi_in)
{
    ...
    *roi_in = *roi_out;
    ...
    int32_t p[2], o[2];
    int32_t aabb[4] = {roi_out->x, roi_out->y, roi_out->x + roi_out->width - 1,
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    return dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_flip, width, height, CLARG(dev_in), CLARG(dev_out), CLARG(width),
        CLARG(height), CLARG(roi_out->width), CLARG(roi_out->height), CLARG(data->orientation));
}
#endif
...
void init_presets(dt_iop_module_so_t *self)
{
    ...
    dt_gui_presets_add_generic(_("auto"), self->op, self->version(), &p, sizeof(p), TRUE,
    ...
    dt_gui_presets_add_generic(_("no rotation"), self->op, self->version(), &p, sizeof(p), TRUE,
                               DEVELOP_BLEND_CS_NONE);

    p.orientation = ORIENTATION_FLIP_HORIZONTALLY;
    dt_gui_presets_add_generic(_("flip horizontally"), self->op, self->version(), &p, sizeof(p),
                               TRUE, DEVELOP_BLEND_CS_NONE);

    p.orientation = ORIENTATION_FLIP_VERTICALLY;
    dt_gui_presets_add_generic(_("flip vertically"), self->op, self->version(), &p, sizeof(p), TRUE,
                               DEVELOP_BLEND_CS_NONE);

    p.orientation = ORIENTATION_ROTATE_CW_90_DEG;
    dt_gui_presets_add_generic(_("rotate by -90 degrees"), self->op, self->version(), &p, sizeof(p),
                               TRUE, DEVELOP_BLEND_CS_NONE);
    ...
    dt_gui_presets_add_generic(_("rotate by 180 degrees"), self->op, self->version(), &p, sizeof(p),
                               TRUE, DEVELOP_BLEND_CS_NONE);
...
void gui_init(dt_iop_module_t *self)
{
    ...
    GtkWidget *label =
        dtgtk_reset_label_new(_("transform"), self, &p->orientation, sizeof(int32_t));
    self->widget = dt_gui_hbox(label);

    dt_iop_button_new(self, N_("rotate 90 degrees CCW"), G_CALLBACK(rotate_ccw), FALSE,
                      GDK_KEY_bracketleft, 0, dtgtk_cairo_paint_refresh, 0, self->widget);

    dt_iop_button_new(self, N_("rotate 90 degrees CW"), G_CALLBACK(rotate_cw), FALSE,
                      GDK_KEY_bracketright, 0, dtgtk_cairo_paint_refresh, 1, self->widget);

    dt_iop_button_new(self, N_("flip horizontally"), G_CALLBACK(_flip_h), FALSE, 0, 0,
                      dtgtk_cairo_paint_flip, 1, self->widget);

    dt_iop_button_new(self, N_("flip vertically"), G_CALLBACK(_flip_v), FALSE, 0, 0,
                      dtgtk_cairo_paint_flip, 0, self->widget);
}
