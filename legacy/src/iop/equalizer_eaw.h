...
#define gweight(i, j, ii, jj)                                                                      \
    1.0 / (fabsf(weight_a[l][(size_t)wd * ((j) >> (l - 1)) + ((i) >> (l - 1))] -                   \
                 weight_a[l][(size_t)wd * ((jj) >> (l - 1)) + ((ii) >> (l - 1))]) +                \
           1.e-5)
...
static void dt_iop_equalizer_wtf(float *const buf, float **weight_a, const int l, const int width,
                                 const int height)
{
    ...
    for (int j = 0; j < ht - 1; j++)
    {
        for (int i = 0; i < wd - 1; i++)
            weight_a[l][(size_t)j * wd + i] = gbuf(buf, i << (l - 1), j << (l - 1));
    ...
    for (int j = 0; j < height; j++)
    {
        ...
        for (int i = 0; i < width - st; i += st)
            tmp[i] = gweight(i, j, i + st, j);
        // predict, get detail
        int i = st;
        for (; i < width - st; i += step)
            for (ch = 0; ch < 3; ch++)
                gbuf(buf, i, j) -=
                    (tmp[i - st] * gbuf(buf, i - st, j) + tmp[i] * gbuf(buf, i + st, j)) /
                    (tmp[i - st] + tmp[i]);
        if (i < width)
            for (ch = 0; ch < 3; ch++)
                gbuf(buf, i, j) -= gbuf(buf, i - st, j);
        // update coarse
        for (ch = 0; ch < 3; ch++)
        ...
        if (i < width)
            for (ch = 0; ch < 3; ch++)
                gbuf(buf, i, j) += gbuf(buf, i - st, j) * .5f;
    }
    ...
    for (int i = 0; i < width; i++)
    {
        ...
        for (int j = 0; j < height - st; j += st)
            tmp[j] = gweight(i, j, i, j + st);
        int j = st;
        // predict, get detail
        for (; j < height - st; j += step)
            for (ch = 0; ch < 3; ch++)
                gbuf(buf, i, j) -=
                    (tmp[j - st] * gbuf(buf, i, j - st) + tmp[j] * gbuf(buf, i, j + st)) /
                    (tmp[j - st] + tmp[j]);
        if (j < height)
            for (ch = 0; ch < 3; ch++)
                gbuf(buf, i, j) -= gbuf(buf, i, j - st);
        // update
        for (ch = 0; ch < 3; ch++)
        ...
        if (j < height)
            for (ch = 0; ch < 3; ch++)
                gbuf(buf, i, j) += gbuf(buf, i, j - st) * .5f;
    }

    dt_free_align(tmp_height_buf);
}

static void dt_iop_equalizer_iwtf(float *buf, float **weight_a, const int l, const int width,
                                  const int height)
{
    const int step = 1 << l;
    const int st = step / 2;
    ...
    for (int j = 0; j < height; j++)
    {
        ...
        if (i < width)
            for (int ch = 0; ch < 3; ch++)
                gbuf(buf, i, j) -= gbuf(buf, i - st, j) * 0.5f;
        // predict
