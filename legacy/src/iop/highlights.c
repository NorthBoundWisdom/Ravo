...
typedef struct dt_iop_highlights_params_t
{
    ...
    dt_atrous_wavelets_scales_t
        scales;        // $DEFAULT: WAVELETS_7_SCALE $DESCRIPTION: "diameter of reconstruction"
...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(
        self, _("avoid magenta highlights and try to recover highlights colors"), _("corrective"),
        _("linear, raw, scene-referred"), _("reconstruction, raw"),
                   ...
                   const dt_iop_roi_t *const roi_out, dt_iop_roi_t *roi_in)
{
    *roi_in = *roi_out;
    ...
    const gboolean use_opposing =
        (d->mode == DT_IOP_HIGHLIGHTS_OPPOSED) || (d->mode == DT_IOP_HIGHLIGHTS_SEGMENTS);
...
static float *_provide_raster_mask(const dt_iop_roi_t *const roi_in,
                                   const dt_iop_roi_t *const roi_out, const float *in,
                                   const float clip, dt_dev_pixelpipe_iop_t *piece)
{
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    if (pipe->mask_display == DT_DEV_PIXELPIPE_DISPLAY_PASSTHRU)
    {
        ...
        else
        {
            ...
            return dt_opencl_enqueue_copy_image(devid, dev_in, dev_out, iorigin, CLIMG_ORIGIN,
                                                region);
        }
    }
    ...
    if (g && fullpipe)
    {
        if (g->hlr_mask_mode != DT_HIGHLIGHTS_MASK_OFF)
        {
            ...
            if (g->hlr_mask_mode == DT_HIGHLIGHTS_MASK_CLIPPED)
            {
                ...
                dev_xtrans = dt_opencl_copy_host_to_device_constant(devid, sizeof(piece->xtrans),
                                                                    piece->xtrans);
                if (!dev_xtrans)
                    goto finish;
                ...
                err = dt_opencl_enqueue_kernel_2d_args(
                    devid, gd->kernel_highlights_false_color, roi_out->width, roi_out->height,
                    CLARG(dev_in), CLARG(dev_out), CLARG(roi_out->width), CLARG(roi_out->height),
                    CLARG(roi_in->width), CLARG(roi_in->height), CLARG(dx), CLARG(dy),
                    CLARG(filters), CLARG(dev_xtrans), CLARG(clips));
                announce = FALSE;
                goto finish;
            }
        }
    ...
    if (!filters)
    {
        ...
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->kernel_highlights_4f_clip, roi_in->width, roi_in->height, CLARG(dev_in),
            CLARG(dev_out), CLARG(roi_in->width), CLARG(roi_in->height), CLARG(clipper));
    }
    ...
    else if (dmode == DT_IOP_HIGHLIGHTS_LCH && filters != 9u)
    {
        ...
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->kernel_highlights_1f_lch_bayer, roi_in->width, roi_in->height, CLARG(dev_in),
            CLARG(dev_out), CLARG(roi_in->width), CLARG(roi_in->height), CLARG(clipper),
            CLARG(filters));
    }
    else if (dmode == DT_IOP_HIGHLIGHTS_LCH && filters == 9u)
    {
        ...
        err = dt_opencl_enqueue_kernel_2d_local_args(
            devid, gd->kernel_highlights_1f_lch_xtrans, sizes, local, CLARG(dev_in), CLARG(dev_out),
            CLARG(roi_in->width), CLARG(roi_in->height), CLARG(clipper), CLARG(dev_xtrans),
    ...
    else // (dmode == DT_IOP_HIGHLIGHTS_CLIP)
    {
        ...
        const int dx = roi_out->x - roi_in->x;
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->kernel_highlights_1f_clip, roi_out->width, roi_out->height, CLARG(dev_in),
            CLARG(dev_out), CLARG(roi_in->width), CLARG(roi_in->height), CLARG(roi_out->width),
            CLARG(roi_out->height), CLARG(clips), CLARG(dx), CLARG(dy), CLARG(filters),
            CLARG(dev_xtrans));
    }
    if (err != CL_SUCCESS)
        goto finish;
    ...
    if (announce)
    {
        ...
        if (cpdata)
        {
            if (dt_opencl_copy_image_to_host(devid, cpdata, dev_out, roi_out->width,
                                             roi_out->height, ch * sizeof(float)) == CL_SUCCESS)
    ...
    if ((err == CL_SUCCESS) && (d->mode != DT_IOP_HIGHLIGHTS_LAPLACIAN) &&
...
static void process_clip(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
...
static void process_visualize(dt_dev_pixelpipe_iop_t *piece, const void *const ivoid,
                              void *const ovoid, const dt_iop_roi_t *const roi_in,
                              const dt_iop_roi_t *const roi_out, dt_iop_highlights_data_t *d)
{
    const uint8_t (*const xtrans)[6] = piece->xtrans;
    const uint32_t filters = piece->filters;
    const float *const in = (const float *const)ivoid;
    ...
    if (filters == 0)
    {
        ...
        {
            for_each_channel(c) out[k + c] = (in[k + c] < clips[c]) ? 0.2f * in[k + c] : 1.0f;
            out[k + 3] = 0.0f;
        }
    }
    else
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    const dt_iop_highlights_mode_t dmode =
        fastmode && (d->mode == DT_IOP_HIGHLIGHTS_SEGMENTS) ? DT_IOP_HIGHLIGHTS_OPPOSED : d->mode;
    ...
    {
        dt_iop_clip_and_zoom_roi((float *)ovoid, (float *)ivoid, roi_out, roi_in);
        dt_print_pipe(DT_DEBUG_ALWAYS, "bypass highlights", pipe, self, DT_DEVICE_CPU, roi_in,
                      roi_out, "can't allocate temp buffer");
        dt_iop_piece_clear_raster(piece, NULL);
        return;
    }
    ...
    {
        const dt_mipmap_size_t level =
            dt_mipmap_cache_get_matching_size(pipe->final_width, pipe->final_height);
    ...
    if (filters == 0)
    {
        ...
        float *mask =
            announce ? _provide_raster_mask(roi_in, roi_out, (float *)ovoid, clipper, piece) : NULL;
        if (mask)
            dt_iop_piece_set_raster(piece, mask, roi_in, roi_out);
        else
            dt_iop_piece_clear_raster(piece, NULL);
    ...
    {
        const dt_highlights_mask_t vmode =
            ((g != NULL) && fullpipe && (g->hlr_mask_mode != DT_HIGHLIGHTS_MASK_CLIPPED)) ?
                g->hlr_mask_mode :
                DT_HIGHLIGHTS_MASK_OFF;

        float *tmp =
            _process_opposed(self, piece, ivoid, ovoid, roi_in, roi_out, TRUE, TRUE, clipper);
        if (tmp)
    ...
    // update processed maximum
    if ((dmode != DT_IOP_HIGHLIGHTS_LAPLACIAN) && (dmode != DT_IOP_HIGHLIGHTS_SEGMENTS) &&
        (dmode != DT_IOP_HIGHLIGHTS_OPPOSED))
    {
                   ...
                   dt_dev_pixelpipe_iop_t *piece)
{
  ...
  */
    ...
    piece->process_cl_ready = ((d->mode == DT_IOP_HIGHLIGHTS_INPAINT) ||
                               (d->mode == DT_IOP_HIGHLIGHTS_SEGMENTS) || opplinear) ?
                                  FALSE :
                                  TRUE;
...
void init_global(dt_iop_module_so_t *self)
{
    ...
    gd->kernel_highlights_1f_lch_bayer =
        dt_opencl_create_kernel(program, "highlights_1f_lch_bayer");
    gd->kernel_highlights_1f_lch_xtrans =
        dt_opencl_create_kernel(program, "highlights_1f_lch_xtrans");
    gd->kernel_highlights_4f_clip = dt_opencl_create_kernel(program, "highlights_4f_clip");
    gd->kernel_highlights_bilinear_and_mask =
        dt_opencl_create_kernel(program, "interpolate_and_mask");
    gd->kernel_highlights_remosaic_and_replace =
        dt_opencl_create_kernel(program, "remosaic_and_replace");
...
void gui_changed(dt_iop_module_t *self, GtkWidget *w, void *previous)
{
  ...
  */
    ...
    if ((!bayer && (p->mode == DT_IOP_HIGHLIGHTS_LAPLACIAN)) ||
        ((filters == 0) &&
         (p->mode == DT_IOP_HIGHLIGHTS_LCH || p->mode == DT_IOP_HIGHLIGHTS_INPAINT ||
          p->mode == DT_IOP_HIGHLIGHTS_SEGMENTS)))
    {
        p->mode = DT_IOP_HIGHLIGHTS_OPPOSED;
        dt_bauhaus_combobox_set_from_value(g->mode, p->mode);
        dt_control_log(_(
            "highlights: mode not available for this type of image. falling back to inpaint opposed."));
    }
    ...
    if ((use_segmentation && (p->recovery == DT_RECOVERY_MODE_OFF)) &&
        (g->hlr_mask_mode == DT_HIGHLIGHTS_MASK_STRENGTH))
    {
        dt_bauhaus_widget_set_quad_active(g->strength, FALSE);
...
void gui_update(dt_iop_module_t *self)
{
    ...
    gtk_stack_set_visible_child_name(GTK_STACK(self->widget),
                                     !monochrome ? "default" : "notapplicable");
    _set_quads(g, NULL);
...
void reload_defaults(dt_iop_module_t *self)
{
    ...
    if (g)
    {
        ...
        {
            dt_bauhaus_combobox_add_introspection(g->mode, NULL, values, DT_IOP_HIGHLIGHTS_OPPOSED,
                                                  xtrans ? DT_IOP_HIGHLIGHTS_SEGMENTS :
...
void gui_init(dt_iop_module_t *self)
{
    ...
    dt_bauhaus_widget_set_quad(
        g->clip, self, dtgtk_cairo_paint_showmask, TRUE, _quad_callback,
    ...
    gtk_widget_set_tooltip_text(
        g->combine,
        _("combine closely related clipped segments by morphological operations.\n"
          "this often leads to improved color reconstruction for tiny segments before dark background."));
    dt_bauhaus_widget_set_quad(
        g->combine, self, dtgtk_cairo_paint_showmask, TRUE, _quad_callback,
    ...
    gtk_widget_set_tooltip_text(
        g->candidating,
        _("select inpainting after segmentation analysis.\n"
    ...
    dt_bauhaus_widget_set_quad(
        g->candidating, self, dtgtk_cairo_paint_showmask, TRUE, _quad_callback,
    ...
    gtk_widget_set_tooltip_text(
        g->recovery,
        _("approximate lost data in regions with all photosites clipped, the effect depends on segment size and border gradients.\n"
    ...
    gtk_widget_set_tooltip_text(
        g->strength, _("set strength of rebuilding in regions with all photosites clipped."));
    ...
    gtk_widget_set_tooltip_text(g->noise_level,
                                _("add noise to visually blend the reconstructed areas\n"
    ...
    gtk_widget_set_tooltip_text(notapplicable,
                                _("this module does not work with monochrome RAW files"));
