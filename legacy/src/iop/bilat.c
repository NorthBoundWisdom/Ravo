...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self, _("manipulate local and global contrast separately"),
                                  _("creative"), _("non-linear, Lab, display-referred"),
...
void init_presets(dt_iop_module_so_t *self)
{
    ...
    dt_gui_presets_add_generic(_("HDR local tone-mapping"), self->op, self->version(), &p,
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    if (d->mode == s_mode_bilateral)
    {
        ...
        dt_bilateral_cl_t *b = dt_bilateral_init_cl(piece->pipe->devid, roi_in->width,
                                                    roi_in->height, sigma_s, sigma_r);
        if (!b)
            goto error;
                     ...
                     dt_develop_tiling_t *tiling)
{
    ...
    if (d->mode == s_mode_bilateral)
    {
        ...
        tiling->factor =
            2.0f + (float)dt_bilateral_memory_use(width, height, sigma_s, sigma_r) / basebuffer;
        tiling->maxbuf =
             ...
             void *const o, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    {
        local_laplacian(i, o, roi_in->width, roi_in->height, d->midtone, d->sigma_s, d->sigma_r,
                        d->detail, 0);
    }
}
...
void gui_init(dt_iop_module_t *self)
{
    ...
    gtk_widget_set_tooltip_text(g->mode, _("the filter used for local contrast enhancement."
    ...
    gtk_widget_set_tooltip_text(
        g->spatial, _("feature size of local details (spatial sigma of bilateral filter)"));
    ...
    gtk_widget_set_tooltip_text(
        g->range, _("L difference to detect edges (range sigma of bilateral filter)"));
    ...
    gtk_widget_set_tooltip_text(
        g->midtone, _("defines what counts as mid-tones. lower for better dynamic range compression"
