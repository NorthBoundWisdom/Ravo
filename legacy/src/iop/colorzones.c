...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self, _("selectively shift hues, chroma and lightness of pixels"),
                                  _("creative"), _("linear or non-linear, Lab, display-referred"),
...
static void dt_iop_colorzones_get_params(dt_iop_colorzones_params_t *p,
                                         dt_iop_colorzones_gui_data_t *g, const int ch,
                                         const double mouse_x, const double mouse_y,
                                         const float radius)
{
    const int bands = p->curve_num_nodes[ch];

    const float lin_mouse_x = _mouse_to_curve(mouse_x, g->zoom_factor, g->offset_x);
    const float lin_mouse_y = _mouse_to_curve(mouse_y, g->zoom_factor, g->offset_y);
...
static inline float strength(const float value, const float strength)
{
    return value + (value - 0.5f) * (strength / 100.0f);
}

void process_display(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece, const void *const ivoid,
...
void init_presets(dt_iop_module_so_t *self)
{
    ...
    p.curve[DT_IOP_COLORZONES_C][0].y =
        p.curve[DT_IOP_COLORZONES_C][DT_IOP_COLORZONES_BANDS - 1].y = 0.65f;
    ...
    dt_gui_presets_add_generic(_("B&W: with red"), self->op, version, &p, sizeof(p), TRUE,
    ...
    dt_gui_presets_add_generic(_("B&W: with skin tones"), self->op, version, &p, sizeof(p), TRUE,
                               DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    dt_gui_presets_add_generic(_("polarizing filter"), self->op, version, &p, sizeof(p), TRUE,
                               DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    dt_gui_presets_add_generic(_("natural skin tones"), self->op, version, &p, sizeof(p), TRUE,
                               DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    dt_gui_presets_add_generic(_("B&W: film"), self->op, version, &p, sizeof(p), TRUE,
                               DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    dt_gui_presets_add_generic(_("HSL base setting"), self->op, version, &p, sizeof(p), TRUE,
                               DEVELOP_BLEND_CS_RGB_DISPLAY);
...
static void _draw_color_picker(dt_iop_module_t *self, cairo_t *cr, dt_iop_colorzones_params_t *p,
                               dt_iop_colorzones_gui_data_t *g, const int width, const int height,
                               ...
                               const float *const picker_max)
{
    if (self->request_color_pick == DT_REQUEST_COLORPICK_MODULE &&
        (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(g->colorpicker)) ||
         gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(g->colorpicker_set_values))))
    {
        ...
        picked_i = _curve_to_mouse(picked_i, g->zoom_factor, g->offset_x);
        picked_min_i = _curve_to_mouse(picked_min_i, g->zoom_factor, g->offset_x);
        picked_max_i = _curve_to_mouse(picked_max_i, g->zoom_factor, g->offset_x);

        cairo_save(cr);

        cairo_set_source_rgba(cr, 1.0, 1.0, 1.0, 0.25);
        cairo_rectangle(cr, width * picked_min_i, 0, width * fmax(picked_max_i - picked_min_i, 0.0),
                        height);
        cairo_fill(cr);
...
#define COLORZONES_DRAW_BACKGROUD_BOX                                                              \
    ...
    const float clip = 1.0f - (Lcap - L0) * (1.0f / 100.0f) *                                      \
                                  fminf(Lwhite - Lclip, fmaxf(0.0f, Lab[0] - Lclip)) /             \
                                  (Lwhite - Lclip);                                                \
...
static void _draw_background(cairo_t *cr, dt_iop_colorzones_params_t *p,
                             dt_iop_colorzones_gui_data_t *g, const int select_by_picker,
                             const int width, const int height, const float *picked_color)
{
    ...
    for (int j = 0; j < cellsj; j++)
    {
        for (int i = 0; i < cellsi; i++)
        {
            ...
            const float jj = _mouse_to_curve(1.0f - ((float)j - .5f) / (float)(cellsj - 1),
                                             g->zoom_factor, g->offset_y);
            const float jjh = _mouse_to_curve(1.0f - (((float)j / (float)(cellsj - 1))),
                                              g->zoom_factor, g->offset_y) +
                              .5f;
            const float ii = _mouse_to_curve(((float)i + .5f) / (float)(cellsi - 1), g->zoom_factor,
                                             g->offset_x);
            const float iih =
                _mouse_to_curve((float)i / (float)(cellsi - 1), g->zoom_factor, g->offset_x);
            ...
            cairo_rectangle(cr, width * i / (float)cellsi, height * j / (float)cellsj,
                            width / (float)cellsi, height / (float)cellsj);
            cairo_fill(cr);
        }
    }
}
...
static gboolean _area_draw_callback(GtkWidget *widget, cairo_t *crf, dt_iop_module_t *self)
{
    ...
    const gboolean color_found = gtk_style_context_lookup_color(context, "graph_overlay", &color);
    if (!color_found)
    {
        color.red = 1.0;
        color.green = 0.0;
    ...
    _draw_background(cr, &p, g, select_by_picker, width, height, picked_color);
    ...
    if (self->enabled)
    {
        ...
        if (self->request_color_pick != DT_REQUEST_COLORPICK_MODULE)
        {
            ...
            if (hist && hist_max > 0.0f)
            {
                ...
                dt_draw_histogram_8_zoomed(cr, hist, 4, ch_hist, g->zoom_factor,
                                           g->offset_x * 255.f, g->offset_y * hist_max, is_linear);

                cairo_restore(cr);
            }
        }

        _draw_color_picker(self, cr, &p, g, width, height, picked_color, picker_min, picker_max);
    }
    ...
    if (g->edit_by_area)
    {
        ...
        for (int k = 0; k < p.curve_num_nodes[ch]; k++)
        {
            const float x = _curve_to_mouse(p.curve[ch][k].x, g->zoom_factor, g->offset_x);
    ...
    {
        const int ch_inv = ((int)g->channel + i + 1) % 3;
        ...
        cairo_move_to(
            cr, 0, -height * _curve_to_mouse(g->draw_ys[ch_inv][0], g->zoom_factor, g->offset_y));
        for (int k = 1; k < DT_IOP_COLORZONES_RES; k++)
        {
            const float xx = (float)k / (float)(DT_IOP_COLORZONES_RES - 1);
            const float yy = g->draw_ys[ch_inv][k];
    ...
    {
        const float x = _curve_to_mouse(p.curve[ch][k].x, g->zoom_factor, g->offset_x),
                    y = _curve_to_mouse(p.curve[ch][k].y, g->zoom_factor, g->offset_y);
        cairo_arc(cr, width * x, -height * y, DT_PIXEL_APPLY_DPI(3.0), 0.0, 2.0 * M_PI);
        cairo_stroke(cr);
    }

    // draw min/max, if selected
    if (g->edit_by_area && (g->mouse_y > 0 || g->dragging))
    {
        const int bands = p.curve_num_nodes[ch];
        ...
        cairo_move_to(cr, 0,
                      -height * _curve_to_mouse(g->draw_min_ys[0], g->zoom_factor, g->offset_y));

        for (int k = 1; k < DT_IOP_COLORZONES_RES; k++)
        {
            const float xx = (float)k / (float)(DT_IOP_COLORZONES_RES - 1);
            const float yy = g->draw_min_ys[k];
        ...
        {
            const float xx = (float)k / (float)(DT_IOP_COLORZONES_RES - 1);
            const float yy = g->draw_max_ys[k];
        ...
        const int k =
            DT_IOP_COLORZONES_RES * _mouse_to_curve(g->mouse_x, g->zoom_factor, g->offset_x);
        const float x = g->mouse_x,
                    y = _curve_to_mouse(g->draw_ys[ch][k], g->zoom_factor, g->offset_y);

        cairo_arc(cr, x * width, -height * y, g->mouse_radius * width, 0, 2. * M_PI);
        cairo_stroke(cr);
    }
    else
    {
        ...
        if (g->selected >= 0)
        {
            ...
            const float x = _curve_to_mouse(p.curve[g->channel][g->selected].x, g->zoom_factor,
                                            g->offset_x),
                        y = _curve_to_mouse(p.curve[g->channel][g->selected].y, g->zoom_factor,
                                            g->offset_y);
...
static gboolean _bottom_area_draw_callback(GtkWidget *widget, cairo_t *crf, dt_iop_module_t *self)
{
    ...
    {
        const float ii =
            _mouse_to_curve(((float)i + .5f) / (float)(cellsi - 1), g->zoom_factor, g->offset_x);
        const float iih =
            _mouse_to_curve((float)i / (float)(cellsi - 1), g->zoom_factor, g->offset_x);
...
static gboolean _bottom_area_button_press_callback(GtkWidget *widget, GdkEventButton *event,
                              ...
                              const dt_iop_colorzones_node_t *curve)
{
    ...
    if ((selected > 0 && (curve[selected - 1].x >= x)) ||
        (selected < nodes - 1 && (curve[selected + 1].x <= x)))
    {
        point_valid = FALSE;
    }
...
static gboolean _move_point_internal(dt_iop_module_t *self, GtkWidget *widget, const int node,
                                     float dx, float dy, const guint state)
{
...
static void _delete_node(dt_iop_module_t *self, dt_iop_colorzones_node_t *curve, int *nodes,
                         const int node, const gboolean zero)
{
    dt_iop_colorzones_gui_data_t *g = self->gui_data;
...
static inline int _add_node(dt_iop_colorzones_node_t *curve, int *nodes, const float x,
                            const float y)
{
    int selected = -1;
    if (curve[0].x > x)
                                        ...
                                        dt_iop_module_t *self)
{
    ...
    if (g->selected < 0 && !g->edit_by_area)
        return TRUE;
...
static gboolean _area_motion_notify_callback(GtkWidget *widget, GdkEventMotion *event,
                                             dt_iop_module_t *self)
{
    dt_iop_colorzones_gui_data_t *g = self->gui_data;
    dt_iop_colorzones_params_t *p = self->params;
    ...
    darktable.control->element =
        (int)(8.0 * _mouse_to_curve(g->mouse_x, g->zoom_factor, g->offset_x) + 0.5f) % 8;
    ...
    if (event->state & GDK_BUTTON1_MASK)
    {
        ...
        if (g->selected >= 0)
        {
            ...
            const float translate_mouse_x =
                old_m_x - _curve_to_mouse(curve[g->selected].x, g->zoom_factor, g->offset_x);
            const float translate_mouse_y =
                old_m_y - _curve_to_mouse(curve[g->selected].y, g->zoom_factor, g->offset_y);
            // dx & dy are in linear coordinates
            const float dx =
                _mouse_to_curve(g->mouse_x - translate_mouse_x, g->zoom_factor, g->offset_x) -
                _mouse_to_curve(old_m_x - translate_mouse_x, g->zoom_factor, g->offset_x);
            const float dy =
                _mouse_to_curve(g->mouse_y - translate_mouse_y, g->zoom_factor, g->offset_y) -
    ...
    if (g->edit_by_area)
    {
        if (g->dragging)
        {
            if (g->x_move < 0)
            {
                dt_iop_colorzones_get_params(p, g, g->channel, g->mouse_x, g->mouse_y,
                                             g->mouse_radius);
                dt_iop_color_picker_reset(self, TRUE);
                dt_dev_add_history_item_target(darktable.develop, self, TRUE, widget + ch);
            }
        ...
        else if (event->y > height)
        {
            g->x_move = 0;
            const int bands = p->curve_num_nodes[g->channel];
            const float mouse_x = _mouse_to_curve(g->mouse_x, g->zoom_factor, g->offset_x);
            float dist = fabsf(p->curve[g->channel][0].x - mouse_x);
            for (int k = 1; k < bands; k++)
            {
                const float d2 = fabsf(p->curve[g->channel][k].x - mouse_x);
                if (d2 < dist)
                {
                    g->x_move = k;
                    dist = d2;
    ...
    else
    {
        if (event->state & GDK_BUTTON1_MASK)
        {
            ...
            {
                const float linx = _mouse_to_curve(g->mouse_x, g->zoom_factor, g->offset_x),
                            liny = _mouse_to_curve(g->mouse_y, g->zoom_factor, g->offset_y);

                // no vertex was close, create a new one!
                g->selected = _add_node(curve, &p->curve_num_nodes[ch], linx, liny);
        ...
        else
        {
            ...
            {
                float dist = (my - _curve_to_mouse(curve[k].y, g->zoom_factor, g->offset_y)) *
                                 (my - _curve_to_mouse(curve[k].y, g->zoom_factor, g->offset_y)) +
                             (mx - _curve_to_mouse(curve[k].x, g->zoom_factor, g->offset_x)) *
                                 (mx - _curve_to_mouse(curve[k].x, g->zoom_factor, g->offset_x));
                if (dist < min)
                {
                    min = dist;
    ...
    if (event->button == GDK_BUTTON_PRIMARY)
    {
            ...
            !dt_modifier_is(event->state, GDK_CONTROL_MASK))
        {
            ...
            return TRUE;
                 ...
                 dt_modifier_is(event->state, GDK_CONTROL_MASK) &&
                 nodes < DT_IOP_COLORZONES_MAXNODES && (g->selected == -1 || g->edit_by_area))
        {
                                        ...
                                        // afterwards
            {
                ...
                for (int k = 0; k < nodes; k++)
                {
                    const float other_y = _curve_to_mouse(curve[k].y, g->zoom_factor, g->offset_y);
...
static gboolean _area_button_release_callback(GtkWidget *widget, GdkEventButton *event,
                                              dt_iop_module_t *self)
{
    if (darktable.develop->darkroom_skip_mouse_events)
        return TRUE;
...
static gboolean _area_leave_notify_callback(GtkWidget *widget, GdkEventCrossing *event,
                                            dt_iop_module_t *self)
{
    if (darktable.develop->darkroom_skip_mouse_events)
        return TRUE;
...
static gboolean _area_key_press_callback(GtkWidget *widget, GdkEventKey *event,
                                         dt_iop_module_t *self)
{
    dt_iop_colorzones_gui_data_t *g = self->gui_data;
...
static void _display_mask_callback(GtkToggleButton *togglebutton, dt_iop_module_t *self)
{
    DT_GUARD_GUI_UPDATE();
...
void color_picker_apply(dt_iop_module_t *self, GtkWidget *picker, dt_dev_pixelpipe_t *pipe)
{
    ...
    if (picker == g->colorpicker_set_values)
    {
        ...
        x -= feather;
        if (x > 0.f && x < 1.f)
            _add_node(curve, &p->curve_num_nodes[ch_curve], x, .5f);
        ...
        if (ch_val == DT_IOP_COLORZONES_L)
            ...
            x = self->picked_color_min[2];
        if (x > 0.f && x < 1.f)
            _add_node(curve, &p->curve_num_nodes[ch_curve], x, .5f + increment);
        ...
        if (x > 0.f && x < 1.f)
            _add_node(curve, &p->curve_num_nodes[ch_curve], x, .5f + 2.f * increment);
                                   ...
                                   dt_action_effect_t effect, float move_size)
{
    ...
    int node = 0;
    while (node < nodes && !(close_enough = fabsf(curve[node].x - x) <= 1. / 16))
        node++;

    float return_value =
        close_enough ? curve[node].y : dt_draw_curve_calc_value(g->minmax_curve[ch], x);
    ...
    if (DT_PERFORM_ACTION(move_size))
    {
        ...
        gchar *text = g_strdup_printf("%s %+.2f", _action_elements_zones[element].name,
                                      return_value * 2. - 1.);
...
void gui_reset(dt_iop_module_t *self)
{
    ...
    g->edit_by_area = 0;
    g->display_mask = FALSE;
    g->mouse_radius = 1.f / DT_IOP_COLORZONES_BANDS;
...
void gui_init(dt_iop_module_t *self)
{
    ...
    dt_action_define_iop(self, NULL, N_("channel"), GTK_WIDGET(g->channel_tabs), &notebook_def);

    dt_ui_notebook_page(g->channel_tabs, N_("lightness"), NULL);
    ...
    // color pickers
    g->colorpicker =
        dt_color_picker_new_with_cst(self, DT_COLOR_PICKER_POINT_AREA, NULL, IOP_CS_LCH);
    gtk_widget_set_tooltip_text(
        g->colorpicker,
        _("pick GUI color from image\nctrl+click or right-click to select an area"));
    gtk_widget_set_name(g->colorpicker, "keep-active");
    dt_action_define_iop(self, N_("pickers"), N_("show color"), g->colorpicker,
                         &dt_action_def_toggle);
    g->colorpicker_set_values =
        dt_color_picker_new_with_cst(self, DT_COLOR_PICKER_AREA, NULL, IOP_CS_LCH);
    ...
    dt_action_define_iop(self, N_("pickers"), N_("create curve"), g->colorpicker_set_values,
                         &dt_action_def_toggle);

    // the nice graph
    g->area =
        GTK_DRAWING_AREA(dt_ui_resize_wrap(NULL, 0, "plugins/darkroom/colorzones/graphheight"));
    ...
    g_signal_connect(G_OBJECT(g->chk_edit_by_area), "toggled", G_CALLBACK(_edit_by_area_callback),
                     self);
    ...
    g_signal_connect(G_OBJECT(g->bt_showmask), "toggled", G_CALLBACK(_display_mask_callback), self);
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(g->bt_showmask), FALSE);

    self->widget = dt_gui_vbox(dt_gui_hbox(dt_gui_expand(g->channel_tabs), gtk_label_new("   "),
    ...
    g_signal_connect(G_OBJECT(g->bottom_area), "button-press-event",
                     G_CALLBACK(_bottom_area_button_press_callback), self);
    ...
    g_signal_connect(G_OBJECT(g->interpolator), "value-changed", G_CALLBACK(_interpolator_callback),
                     self);
}
...
void init_pipe(dt_iop_module_t *self, dt_dev_pixelpipe_t *pipe, dt_dev_pixelpipe_iop_t *piece)
{
    ...
    const dt_iop_colorzones_params_t *const default_params = self->default_params;
    piece->data = d;
    ...
    for (int ch = 0; ch < DT_IOP_COLORZONES_MAX_CHANNELS; ch++)
    {
        ...
        for (int k = 0; k < default_params->curve_num_nodes[ch]; k++)
            dt_draw_curve_add_point(d->curve[ch], default_params->curve[ch][k].x,
