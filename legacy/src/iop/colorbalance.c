...
DT_MODULE_INTROSPECTION(4, dt_iop_colorbalance_params_t)
...
typedef struct dt_iop_colorbalance_params_t
{
    ...
    float lift[CHANNEL_SIZE], gamma[CHANNEL_SIZE],
        gain[CHANNEL_SIZE]; // $MIN: 0.0 $MAX: 2.0 $DEFAULT: 1.0
...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self, _("shift colors selectively by luminance range"),
                                  _("corrective or creative"), _("linear, Lab, scene-referred"),
                                  _("non-linear, RGB"), _("non-linear, Lab, scene-referred"));
}
...
static inline float CDL(float x, float slope, float offset, float power)
{
    ...
    out = powf(MAX(out, 0.0f), power);
    return out;
}

static inline void _apply_CDL(dt_aligned_pixel_t x, const dt_aligned_pixel_t slope,
                              const dt_aligned_pixel_t offset, const dt_aligned_pixel_t power)
{
    dt_aligned_pixel_t res;
    for_each_channel(c) res[c] = slope[c] * x[c] + offset[c];
    dt_vector_max(res, res, zero); // clip away negatives
    dt_vector_powf(res, power, x);
}

static inline void _apply_fulcrum_contrast(dt_aligned_pixel_t rgb, const dt_aligned_pixel_t grey,
                                           const dt_aligned_pixel_t contrast_power)
{
    dt_vector_max(rgb, rgb, zero); // clip away negatives
    for_each_channel(c) rgb[c] /= grey[c];
    dt_vector_powf(rgb, contrast_power, rgb);
    for_each_channel(c) rgb[c] *= grey[c];
}

#ifndef __SSE2__
static void _apply_lgg(dt_aligned_pixel_t rgb, const dt_aligned_pixel_t lift,
                       const dt_aligned_pixel_t gamma_inv, const dt_aligned_pixel_t gain)
{
    dt_vector_max(rgb, rgb, zero); // clip away negatives
...
static void _process_lgg(const dt_aligned_pixel_t in, dt_aligned_pixel_t out, const size_t npixels,
                         const dt_aligned_pixel_t lift_, const dt_aligned_pixel_t gamma_inv_,
                         const dt_aligned_pixel_t gain_, const float grey, const float saturation,
                         const float saturation_out, const dt_aligned_pixel_t contrast_power)
{
    ...
    const dt_aligned_pixel_t saturation_out4 = {saturation_out, saturation_out, saturation_out,
                                                saturation_out};
    ...
    for (size_t k = 0; k < npixels; k++)
    {
        ...
        if (run_saturation)
        {
            ...
            for_each_channel(c) rgb[c] = luma + saturation4[c] * (rgb[c] - luma);
        }
        _apply_lgg(rgb, lift_, gamma_inv_, gain_);
        ...
        if (run_saturation_out)
        {
            ...
            for_each_channel(c) rgb[c] = luma + saturation_out4[c] * (rgb[c] - luma);
        }
...
static void _process_sop(const dt_aligned_pixel_t in, dt_aligned_pixel_t out, const size_t npixels,
                         const dt_aligned_pixel_t lift, const dt_aligned_pixel_t gamma,
                         const dt_aligned_pixel_t gain, const float grey, const float saturation,
                         const float saturation_out, const float contrast_amt,
                         const dt_aligned_pixel_t contrast_power)
{
    const int run_saturation = fabsf(saturation - 1.0f) > 1e-6;
    const int run_saturation_out = fabsf(saturation_out - 1.0f) > 1e-6;
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    const float saturation_out = d->saturation_out;

    const size_t npixels = (size_t)roi_out->width * roi_out->height;
    const float *const restrict in = ivoid;
    float *const restrict out = ovoid;
    const dt_aligned_pixel_t contrast_power = {contrast, contrast, contrast, contrast};
    ...
    const dt_aligned_pixel_t lift = {2.0f - (d->lift[CHANNEL_RED] * d->lift[CHANNEL_FACTOR]),
                                     ...
                                     0.0f};
    const dt_aligned_pixel_t lift_sop = {(d->lift[CHANNEL_RED] + d->lift[CHANNEL_FACTOR] - 2.0f),
                                         (d->lift[CHANNEL_GREEN] + d->lift[CHANNEL_FACTOR] - 2.0f),
                                         (d->lift[CHANNEL_BLUE] + d->lift[CHANNEL_FACTOR] - 2.0f),
                                         0.0f};
    const dt_aligned_pixel_t gamma = {d->gamma[CHANNEL_RED] * d->gamma[CHANNEL_FACTOR],
                                      d->gamma[CHANNEL_GREEN] * d->gamma[CHANNEL_FACTOR],
                                      d->gamma[CHANNEL_BLUE] * d->gamma[CHANNEL_FACTOR], 1.0f};
    const dt_aligned_pixel_t gamma_sop = {
        (2.0f - d->gamma[CHANNEL_RED]) * (2.0f - d->gamma[CHANNEL_FACTOR]),
        (2.0f - d->gamma[CHANNEL_GREEN]) * (2.0f - d->gamma[CHANNEL_FACTOR]),
        (2.0f - d->gamma[CHANNEL_BLUE]) * (2.0f - d->gamma[CHANNEL_FACTOR]), 1.0f};
    const dt_aligned_pixel_t gamma_inv_lgg = {
        2.2f * ((gamma[0] != 0.0f) ? 1.0f / gamma[0] : 1000000.0f),
        2.2f * ((gamma[1] != 0.0f) ? 1.0f / gamma[1] : 1000000.0f),
        2.2f * ((gamma[2] != 0.0f) ? 1.0f / gamma[2] : 1000000.0f), 1.0f};
    const dt_aligned_pixel_t gain = {d->gain[CHANNEL_RED] * d->gain[CHANNEL_FACTOR],
                                     d->gain[CHANNEL_GREEN] * d->gain[CHANNEL_FACTOR],
                                     d->gain[CHANNEL_BLUE] * d->gain[CHANNEL_FACTOR], 1.0f};
    ...
    for (size_t chunkstart = 0; chunkstart < npixels; chunkstart += chunksize)
    {
        ...
        case LIFT_GAMMA_GAIN:
            _process_lgg(in + 4 * chunkstart, out + 4 * chunkstart, end - chunkstart, lift,
                         gamma_inv_lgg, gain, grey, saturation, saturation_out, contrast_power);
            break;
        case SLOPE_OFFSET_POWER:
            _process_sop(in + 4 * chunkstart, out + 4 * chunkstart, end - chunkstart, lift_sop,
                         gamma_sop, gain, grey, saturation, saturation_out, contrast,
                         contrast_power);
            break;
        }
    }
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    case LIFT_GAMMA_GAIN:
    {
        const float lift[4] = {2.0f - (d->lift[CHANNEL_RED] * d->lift[CHANNEL_FACTOR]),
                    ...
                    grey = d->grey / 100.0f, saturation = d->saturation,
                    saturation_out = d->saturation_out;

        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->kernel_colorbalance_lgg, width, height, CLARG(dev_in), CLARG(dev_out),
            CLARG(width), CLARG(height), CLARG(lift), CLARG(gain), CLARG(gamma_inv),
            CLARG(saturation), CLARG(contrast), CLARG(grey), CLARG(saturation_out));
        if (err != CL_SUCCESS)
            goto error;
        return CL_SUCCESS;
    ...
    case SLOPE_OFFSET_POWER:
    {
        const float lift[4] = {(d->lift[CHANNEL_RED] + d->lift[CHANNEL_FACTOR] - 2.0f),
                               (d->lift[CHANNEL_GREEN] + d->lift[CHANNEL_FACTOR] - 2.0f),
                               (d->lift[CHANNEL_BLUE] + d->lift[CHANNEL_FACTOR] - 2.0f), 0.0f},
                    gamma[4] = {(2.0f - d->gamma[CHANNEL_RED]) * (2.0f - d->gamma[CHANNEL_FACTOR]),
                                (2.0f - d->gamma[CHANNEL_GREEN]) *
                                    (2.0f - d->gamma[CHANNEL_FACTOR]),
        ...
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->kernel_colorbalance_cdl, width, height, CLARG(dev_in), CLARG(dev_out),
            CLARG(width), CLARG(height), CLARG(lift), CLARG(gain), CLARG(gamma), CLARG(saturation),
            CLARG(contrast), CLARG(grey), CLARG(saturation_out));
        if (err != CL_SUCCESS)
            goto error;
        return CL_SUCCESS;
...
static inline void set_RGB_sliders(GtkWidget *R, GtkWidget *G, GtkWidget *B, float hsl[3], float *p,
                                   int mode)
{
...
static inline void set_HSL_sliders(GtkWidget *hue, GtkWidget *sat, float RGB[4])
{
  ...
  */
    dt_aligned_pixel_t RGB_norm = {(RGB[CHANNEL_RED] / 2.0f), (RGB[CHANNEL_GREEN] / 2.0f),
                                   (RGB[CHANNEL_BLUE] / 2.0f)};
...
static inline void _check_tuner_picker_labels(dt_iop_module_t *self)
{
    ...
    if (g->luma_patches_flags[GAIN] == USER_SELECTED &&
    ...
    if (g->color_patches_flags[GAIN] == USER_SELECTED &&
...
static void apply_autogrey(dt_iop_module_t *self)
{
    ...
    const dt_aligned_pixel_t lift = {(p->lift[CHANNEL_RED] + p->lift[CHANNEL_FACTOR] - 2.0f),
                                     (p->lift[CHANNEL_GREEN] + p->lift[CHANNEL_FACTOR] - 2.0f),
                                     (p->lift[CHANNEL_BLUE] + p->lift[CHANNEL_FACTOR] - 2.0f)},
                             gamma = {2.0f - p->gamma[CHANNEL_RED] * p->gamma[CHANNEL_FACTOR],
                                      2.0f - p->gamma[CHANNEL_GREEN] * p->gamma[CHANNEL_FACTOR],
                                      2.0f - p->gamma[CHANNEL_BLUE] * p->gamma[CHANNEL_FACTOR]},
                             gain = {p->gain[CHANNEL_RED] * p->gain[CHANNEL_FACTOR],
...
static void apply_lift_neutralize(dt_iop_module_t *self)
{
    ...
    for (int c = 0; c < 3; ++c)
        g->color_patches_lift[c] = RGB[c];
    g->color_patches_flags[LIFT] = USER_SELECTED;

    // Compute the RGB values after the CDL factors
    for (int c = 0; c < 3; ++c)
        RGB[c] = CDL(RGB[c], p->gain[CHANNEL_FACTOR], p->lift[CHANNEL_FACTOR] - 1.0f,
                     2.0f - p->gamma[CHANNEL_FACTOR]);
    ...
    // Get the parameter
    for (int c = 0; c < 3; ++c)
        RGB[c] = powf(XYZ[1], 1.0f / (2.0f - p->gamma[c + 1])) - RGB[c] * p->gain[c + 1];
...
static void apply_gamma_neutralize(dt_iop_module_t *self)
{
    ...
    // Get the parameter
    for (int c = 0; c < 3; ++c)
        RGB[c] = logf(XYZ[1]) / logf(RGB[c] * p->gain[c + 1] + p->lift[c + 1] - 1.0f);
...
static void apply_gain_neutralize(dt_iop_module_t *self)
{
    ...
    // Get the parameter
    for (int c = 0; c < 3; ++c)
        RGB[c] = (powf(XYZ[1], 1.0f / (2.0f - p->gamma[c + 1])) - p->lift[c + 1] + 1.0f) /
                 MAX(RGB[c], 0.000001f);
...
static void apply_gamma_auto(dt_iop_module_t *self)
{
    ...
    p->gamma[CHANNEL_FACTOR] =
        2.0f -
        logf(0.1842f) /
            logf(MAX(p->gain[CHANNEL_FACTOR] * XYZ[1] + p->lift[CHANNEL_FACTOR] - 1.0f, 0.000001f));
...
static void apply_autocolor(dt_iop_module_t *self)
{
    ...
    for (int c = 0; c < 3; ++c)
    {
        samples_lift[c] = CDL(g->color_patches_lift[c], p->gain[CHANNEL_FACTOR],
                              p->lift[CHANNEL_FACTOR] - 1.0f, 2.0f - p->gamma[CHANNEL_FACTOR]);
        samples_gamma[c] = CDL(g->color_patches_gamma[c], p->gain[CHANNEL_FACTOR],
                               p->lift[CHANNEL_FACTOR] - 1.0f, 2.0f - p->gamma[CHANNEL_FACTOR]);
        samples_gain[c] = CDL(g->color_patches_gain[c], p->gain[CHANNEL_FACTOR],
                              p->lift[CHANNEL_FACTOR] - 1.0f, 2.0f - p->gamma[CHANNEL_FACTOR]);
    }
    ...
    dt_aligned_pixel_t RGB_lift = {p->lift[CHANNEL_RED] - 1.0f, p->lift[CHANNEL_GREEN] - 1.0f,
                                   p->lift[CHANNEL_BLUE] - 1.0f};
    dt_aligned_pixel_t RGB_gamma = {p->gamma[CHANNEL_RED], p->gamma[CHANNEL_GREEN],
                                    p->gamma[CHANNEL_BLUE]};
    dt_aligned_pixel_t RGB_gain = {p->gain[CHANNEL_RED], p->gain[CHANNEL_GREEN],
                                   p->gain[CHANNEL_BLUE]};
  ...
  */
    for (int runs = 0; runs < 1000; ++runs)
    {
        ...
        for (int c = 0; c < 3; ++c)
            RGB_gain[c] = CLAMP((powf(greys[GAIN], 1.0f / (2.0f - RGB_gamma[c])) - RGB_lift[c]) /
                                    MAX(samples_gain[c], 0.000001f),
                                0.75f, 1.25f);
        // compute RGB offset/lift powf(XYZ[1], 1.0f/(2.0f - p->gamma[c+1])) - RGB[c] * p->gain[c+1];
        for (int c = 0; c < 3; ++c)
            RGB_lift[c] = CLAMP(powf(greys[LIFT], 1.0f / (2.0f - RGB_gamma[c])) -
                                    samples_lift[c] * RGB_gain[c],
                                -0.025f, 0.025f);
        // compute  power/gamma 2.0f - logf(0.1842f) / logf(MAX(p->gain[CHANNEL_FACTOR] * XYZ[1] + p->lift[CHANNEL_FACTOR] - 1.0f, 0.000001f));
        for (int c = 0; c < 3; ++c)
            RGB_gamma[c] =
                2.0f - CLAMP(logf(MAX(greys[GAMMA], 0.000001f)) /
                                 logf(MAX(RGB_gain[c] * samples_gamma[c] + RGB_lift[c], 0.000001f)),
                             0.75f, 1.25f);
    }
  ...
  */
    for (int runs = 0; runs < 100; ++runs)
    {
        p->gain[CHANNEL_FACTOR] =
            CLAMP(p->lift[CHANNEL_FACTOR] / g->luma_patches[GAIN], 0.0f, 2.0f);
        p->lift[CHANNEL_FACTOR] =
            CLAMP(-p->gain[CHANNEL_FACTOR] * g->luma_patches[LIFT] + 1.0f, 0.0f, 2.0f);
        p->gamma[CHANNEL_FACTOR] =
            CLAMP(2.0f - logf(0.1842f) / logf(MAX(p->gain[CHANNEL_FACTOR] * g->luma_patches[GAMMA] +
                   ...
                   dt_dev_pixelpipe_iop_t *piece)
{
    ...
    const dt_aligned_pixel_t lift = {p->lift[CHANNEL_RED], p->lift[CHANNEL_GREEN],
                                     p->lift[CHANNEL_BLUE]};
    const dt_aligned_pixel_t gamma = {p->gamma[CHANNEL_RED], p->gamma[CHANNEL_GREEN],
                                      p->gamma[CHANNEL_BLUE]};
    const dt_aligned_pixel_t gain = {p->gain[CHANNEL_RED], p->gain[CHANNEL_GREEN],
                                     p->gain[CHANNEL_BLUE]};
...
void set_visible_widgets(dt_iop_colorbalance_gui_data_t *g)
{
    ...
    dt_conf_set_string("plugins/darkroom/colorbalance/controls", control_mode == RGBL ? "RGBL" :
                                                                 control_mode == BOTH ? "BOTH" :
                                                                                        "HSL");
...
static gboolean dt_iop_area_draw(GtkWidget *widget, cairo_t *cr, dt_iop_module_t *self)
{
    ...
    uint32_t bg = ((255 << 24) | ((int)floor(flt_bg * 255 + 0.5) << 16) |
                   ((int)floor(flt_bg * 255 + 0.5) << 8) | (int)floor(flt_bg * 255 + 0.5));
    // bg = 0xffffffff;
    ...
    for (int y = 0; y < height; y++)
    {
        ...
        for (int x = 0; x < width; x++)
        {
            ...
            {
                uint32_t col = bg;
                if ((abs(dx) < 1 && abs(dy) < 3) || (abs(dx) < 3 && abs(dy) < 1))
                    col = light;
                *p++ = col;
                continue;
            }
...
static void _configure_slider_blocks(gpointer instance, dt_iop_module_t *self)
{
    ...
    const gchar *short_label_ops[] = {C_("color", "offset"), C_("color", "power"),
                                      C_("color", "slope")};
    const gchar *short_label_lgg[] = {C_("color", "lift"), C_("color", "gamma"),
                                      C_("color", "gain")};
    const gchar **short_label = (p->mode == SLOPE_OFFSET_POWER) ? short_label_ops : short_label_lgg;
    const gchar *long_label[] = {NC_("section", "shadows: lift / offset"),
    ...
    else
    {
        ...
        else
        {
            ...
            for (int i = 0; i < 3; i++)
                gtk_notebook_append_page(GTK_NOTEBOOK(new_container), g->blocks[i], label[i]);
        }
    }
...
#define HSL_CALLBACK(which)                                                                        \
    ...
    {                                                                                              \
        ...
        float hsl[3] = {dt_bauhaus_slider_get(g->hue_##which) / 360.0f,                            \
                        dt_bauhaus_slider_get(g->sat_##which) / 100.0f, 0.5f};                     \
                                                                                                   \
...
void gui_init(dt_iop_module_t *self)
{
    ...
    gtk_widget_set_tooltip_text(g->saturation_out,
                                _("saturation correction after the color balance"));
...
#define ADD_BLOCK(blk, which, section, text, span, satspan)                                        \
    ...
    dt_bauhaus_widget_set_label(g->which##_factor, section, N_("factor"));                         \
                                                                                                   \
    g->hue_##which = dt_color_picker_new(                                                          \
        self, DT_COLOR_PICKER_AREA,                                                                \
        dt_bauhaus_slider_new_with_range_and_feedback(self, 0.0f, 360.0f, 0, 0.0f, 2, 0));         \
    ...
    g_signal_connect(G_OBJECT(g->hue_##which), "value-changed", G_CALLBACK(which##_callback),      \
                     self);                                                                        \
                                                                                                   \
    g->sat_##which =                                                                               \
        dt_bauhaus_slider_new_with_range_and_feedback(self, 0.0f, 100.0f, 0, 0.0f, 2, 0);          \
    ...
    g_signal_connect(G_OBJECT(g->sat_##which), "value-changed", G_CALLBACK(which##_callback),      \
                     self);                                                                        \
                                                                                                   \
    ...
    static const char *lift_messages[] = {
        N_("factor of lift/offset"), N_("factor of red for lift/offset"),
        N_("factor of green for lift/offset"), N_("factor of blue for lift/offset")};

    static const char *gamma_messages[] = {
        N_("factor of gamma/power"), N_("factor of red for gamma/power"),
        N_("factor of green for gamma/power"), N_("factor of blue for gamma/power")};

    static const char *gain_messages[] = {
        N_("factor of gain/slope"), N_("factor of red for gain/slope"),
        N_("factor of green for gain/slope"), N_("factor of blue for gain/slope")};
    ...
    g->auto_luma = dt_color_picker_new(self, DT_COLOR_PICKER_AREA, dt_bauhaus_combobox_new(self));
    dt_bauhaus_widget_set_label(g->auto_luma, NULL, N_("optimize luma"));
    gtk_widget_set_tooltip_text(g->auto_luma,
                                _("fit the whole histogram and center the average luma"));

    g->auto_color = dt_color_picker_new(self, DT_COLOR_PICKER_AREA, dt_bauhaus_combobox_new(self));
    dt_bauhaus_widget_set_label(g->auto_color, NULL, N_("neutralize colors"));
    ...
    self->widget =
        dt_gui_vbox(mode_box, g->master_box, main_label_box, g->main_box, g->optimizer_box);
