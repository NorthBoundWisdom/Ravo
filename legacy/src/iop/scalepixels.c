...
typedef struct dt_iop_scalepixels_params_t
{
    ...
    float
        pixel_aspect_ratio; // $MIN: 0.5f $MAX: 2.0f $DEFAULT: 1.0f $DESCRIPTION: "pixel aspect ratio"
...
gboolean distort_transform(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece, float *points,
                           size_t points_count)
{
                  ...
                  const dt_iop_roi_t *const roi_out)
{
    ...
    for (int row = 0; row < roi_out->height; row++)
    {
        for (int col = 0; col < roi_out->width; col++)
        {
            ...
            out[ow * row + col] =
                CLIP(dt_interpolation_compute_sample(iter, in, x, y, iw, ih, 1, iw));
        }
    }
}
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    for (int j = 0; j < roi_out->height; j++)
    {
        ...
        for (int i = 0; i < roi_out->width; i++, out += 4)
        {
            ...
            dt_interpolation_compute_pixel4c(interpolation, (float *)ivoid, out, x, y,
                   ...
                   dt_dev_pixelpipe_iop_t *piece)
{
    ...
    if (dt_isnan(p->pixel_aspect_ratio) || p->pixel_aspect_ratio <= 0.0f ||
