...
static const float centers_ops[PIXEL_CHAN] DT_ALIGNED_ARRAY = {
    -56.0f / 7.0f, // = -8.0f
    -48.0f / 7.0f, -40.0f / 7.0f, -32.0f / 7.0f, -24.0f / 7.0f,
    -16.0f / 7.0f, -8.0f / 7.0f,  0.0f / 7.0f}; // split 8 EV into 7 evenly-spaced channels

static const float centers_params[CHANNELS] DT_ALIGNED_ARRAY = {-8.0f, -7.0f, -6.0f, -5.0f, -4.0f,
                                                                -3.0f, -2.0f, -1.0f, 0.0f};
...
typedef struct dt_iop_toneequalizer_params_t
{
    ...
    float
        feathering; // $MIN: 0.01 $MAX: 10000.0 $DEFAULT: 1.0 $DESCRIPTION: "edges refinement/feathering"
    float quantization; // $MIN: 0.0 $MAX: 2.0 $DEFAULT: 0.0 $DESCRIPTION: "mask quantization"
    float
        contrast_boost; // $MIN: -16.0 $MAX: 16.0 $DEFAULT: 0.0 $DESCRIPTION: "mask contrast compensation"
    float
        exposure_boost; // $MIN: -16.0 $MAX: 16.0 $DEFAULT: 0.0 $DESCRIPTION: "mask exposure compensation"
    dt_iop_toneequalizer_filter_t
        details; // $DEFAULT: DT_TONEEQ_EIGF $DESCRIPTION: "preserve details"
    dt_iop_luminance_mask_method_t
        method;     // $DEFAULT: DT_TONEEQ_NORM_2 $DESCRIPTION: "luminance estimator"
...
typedef struct dt_iop_toneequalizer_gui_data_t
{
    ...
    GtkWidget *noise, *ultra_deep_blacks, *deep_blacks, *blacks, *shadows, *midtones, *highlights,
        *whites, *speculars;
    GtkDrawingArea *area;
...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(
        ...
        _("corrective and creative"), _("linear, RGB, scene-referred"), _("quasi-linear, RGB"),
...
void init_presets(dt_iop_module_so_t *self)
{
    ...
    // No blending
    dt_gui_presets_add_generic(_("simple tone curve"), self->op, self->version(), &p, sizeof(p),
    ...
    p.contrast_boost = 0.0f;
    dt_gui_presets_add_generic(_("mask blending | all purposes"), self->op, self->version(), &p,
    ...
    dt_gui_presets_add_generic(_("contrast tone curve | soft"), self->op, self->version(), &p,
                               sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_SCENE);

    dilate_shadows_highlight_preset_set_exposure_params(&p, 0.45f);
    dt_gui_presets_add_generic(_("contrast tone curve | medium"), self->op, self->version(), &p,
                               sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_SCENE);

    dilate_shadows_highlight_preset_set_exposure_params(&p, 0.65f);
    dt_gui_presets_add_generic(_("contrast tone curve | strong"), self->op, self->version(), &p,
                               sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_SCENE);
    ...
    dt_gui_presets_add_generic(_("relight: fill-in"), self->op, self->version(), &p, sizeof(p),
                               TRUE, DEVELOP_BLEND_CS_RGB_SCENE);
}
...
static void hash_set_get(const dt_hash_t *hash_in, dt_hash_t *hash_out, dt_pthread_mutex_t *lock)
{
...
static const dt_colormatrix_t gauss_kernel = {{0.076555024f, 0.124401914f, 0.076555024f},
                                              {0.124401914f, 0.196172249f, 0.124401914f},
                                              {0.076555024f, 0.124401914f, 0.076555024f}};

__DT_CLONE_TARGETS__
static float get_luminance_from_buffer(const float *const buffer, const size_t width,
                                       const size_t height, const size_t x, const size_t y)
{
    ...
    const size_t y_abs[4] DT_ALIGNED_PIXEL = {MAX(y, 1) - 1,          // previous line
                                              y,                      // center line
    ...
    if (x > 1 && x < width - 2)
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            ...
            for_each_channel(j) luminance += buffer[width * y_i + x - 1 + j] * gauss_kernel[i][j];
        }
        return luminance;
    }

    const size_t x_abs[4] DT_ALIGNED_PIXEL = {MAX(x, 1) - 1,         // previous column
                                              x,                     // center column
    ...
    {
        const size_t y_i = y_abs[i];
        for_each_channel(j) luminance += buffer[width * y_i + x_abs[j]] * gauss_kernel[i][j];
    }
    return luminance;
}
...
static void _get_point(const dt_iop_module_t *self, const int c_x, const int c_y, int *x, int *y)
{
    ...
    dt_dev_distort_backtransform_plus(darktable.develop, darktable.develop->preview_pipe,
                                      crop_order, DT_DEV_TRANSFORM_DIR_FORW_EXCL, pts, 1);
    *x = pts[0];
    *y = pts[1];
}
...
static float _luminance_from_module_buffer(const dt_iop_module_t *self)
{
    ...
    return get_luminance_from_buffer(g->thumb_preview_buf, g->thumb_preview_buf_width,
                                     g->thumb_preview_buf_height, b_x, b_y);
}
...
static inline void apply_toneequalizer(const float *const restrict in,
                                       ...
                                       float *const restrict out, const dt_iop_roi_t *const roi_in,
                                       ...
                                       const dt_iop_toneequalizer_data_t *const d)
{
    ...
    for (size_t k = 0; k < npixels; k++)
    {
        ...
        // apply correction
        for_each_channel(c) out[4 * k + c] = correction * in[4 * k + c];
    }
}
                                       ...
                                       const dt_iop_toneequalizer_data_t *const d)
{
    ...
    for (size_t k = 0; k < num_elem; ++k)
    {
        ...
        DT_OMP_SIMD(aligned(luminance, centers_ops, factors : 64) safelen(PIXEL_CHAN)
                        reduction(+ : result))
...
__DT_CLONE_TARGETS__
static inline float pixel_correction(const float exposure, const float *const restrict factors,
                                     const float sigma)
{
    // build the correction for the current pixel
...
static inline void compute_luminance_mask(const float *const restrict in,
                                          float *const restrict luminance, const size_t width,
                                          const size_t height,
                                          const dt_iop_toneequalizer_data_t *const d)
{
    switch (d->details)
    ...
    case (DT_TONEEQ_GUIDED):
    {
        ...
        luminance_mask(in, luminance, width, height, d->method, d->exposure_boost, CONTRAST_FULCRUM,
                       d->contrast_boost);
        fast_surface_blur(luminance, width, height, d->radius, d->feathering, d->iterations,
    ...
    case (DT_TONEEQ_AVG_EIGF):
    {
        ...
        fast_eigf_surface_blur(luminance, width, height, d->radius, d->feathering, d->iterations,
                               DT_GF_BLENDING_GEOMEAN, d->scale, d->quantization, exp2f(-14.0f),
                               4.0f);
        break;
    }
    ...
    case (DT_TONEEQ_EIGF):
    {
        luminance_mask(in, luminance, width, height, d->method, d->exposure_boost, CONTRAST_FULCRUM,
                       d->contrast_boost);
        fast_eigf_surface_blur(luminance, width, height, d->radius, d->feathering, d->iterations,
                               DT_GF_BLENDING_LINEAR, d->scale, d->quantization, exp2f(-14.0f),
                               4.0f);
        break;
    }
                                          ...
                                          const dt_iop_roi_t *const roi_out)
{
    ...
    for (size_t i = 0; i < out_height; ++i)
        for (size_t j = 0; j < out_width; ++j)
        {
            ...
            const float intensity = sqrtf(fminf(
                fmaxf(luminance[(i + offset_y) * in_width + (j + offset_x)] - 0.00390625f, 0.f) /
                    0.99609375f,
                1.f));
...
__DT_CLONE_TARGETS__
static void toneeq_process(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
...
__DT_CLONE_TARGETS__
static gboolean compute_channels_factors(const float factors[PIXEL_CHAN], float out[CHANNELS],
                                         const float sigma)
{
...
__DT_CLONE_TARGETS__
static void compute_channels_gains(const float in[CHANNELS], float out[CHANNELS])
{
...
static void commit_channels_gains(const float factors[CHANNELS], dt_iop_toneequalizer_params_t *p)
{
    p->noise = factors[0];
...
static void gui_cache_init(dt_iop_module_t *self)
{
    ...
    g->area_dragging =
        FALSE; // TRUE if left-button has been pushed but not released and cursor motion is recorded
...
static inline void build_interpolation_matrix(float A[CHANNELS * PIXEL_CHAN], const float sigma)
{
...
static inline void compute_log_histogram_and_stats(const float *const restrict luminance,
                                                   int histogram[UI_SAMPLES], const size_t num_elem,
                                                   int *max_histogram, float *first_decile,
                                                   float *last_decile)
{
...
#define TEMP_SAMPLES 2 * UI_SAMPLES
    ...
    for (size_t k = 0; k < num_elem; k++)
    {
        ...
        const int index =
            CLAMP((int)(((log2f(luminance[k]) + 10.0f) / 16.0f) * (float)TEMP_SAMPLES), 0,
                  TEMP_SAMPLES - 1);
        temp_hist[index] += 1;
    }
...
static inline void update_histogram(dt_iop_module_t *const self)
{
    ...
    if (!g->histogram_valid && g->luminance_valid)
    {
        ...
        compute_log_histogram_and_stats(g->thumb_preview_buf, g->histogram, num_elem,
                                        &g->max_histogram, &g->histogram_first_decile,
                                        &g->histogram_last_decile);
...
__DT_CLONE_TARGETS__
static inline void compute_lut_correction(dt_iop_toneequalizer_gui_data_t *g, const float offset,
                                          const float scaling)
{
...
void gui_changed(dt_iop_module_t *self, GtkWidget *w, void *previous)
{
    ...
    if (w == g->method || w == g->blending || w == g->feathering || w == g->iterations ||
        w == g->quantization)
    {
        invalidate_luminance_cache(self);
    }
                                                  ...
                                                  dt_iop_toneequalizer_params_t *p)
{
    ...
    if (g->user_param_valid)
        g->user_param_valid =
            pseudo_solve(g->interpolation_matrix, factors, CHANNELS, PIXEL_CHAN, FALSE);
    if (!g->user_param_valid)
        dt_control_log(_("the interpolation is unstable, decrease the curve smoothing"));
    ...
    if (!g->user_param_valid)
        dt_control_log(_("some parameters are out-of-bounds"));
             ...
             const uint32_t state)
{
    ...
    const gboolean fail = !g->cursor_valid || !g->luminance_valid || !g->interpolation_valid ||
                          !g->user_param_valid || dev->full.pipe->processing || !g->has_focus;
    ...
    dt_iop_gui_enter_critical_section(self);
    const gboolean commit =
        set_new_params_interactive(g->cursor_exposure, offset, g->sigma * g->sigma / 2.0f, g, p);
    dt_iop_gui_leave_critical_section(self);
...
void cairo_draw_hatches(cairo_t *cr, double center[2], double span[2], const int instances,
                        const double line_width, const double shade)
{
    ...
    const double delta[2] = {2.0 * span[0] / (double)instances, 2.0 * span[1] / (double)instances};
...
static void draw_exposure_cursor(cairo_t *cr, const double pointerx, const double pointery,
                                 const double radius, const float luminance, const float zoom_scale,
                                 const int instances, const float alpha)
{
    ...
    if (log2f(luminance) > 0.0f)
    {
        ...
        cairo_draw_hatches(cr, pointer_coord, span, instances, DT_PIXEL_APPLY_DPI(1. / zoom_scale),
                           0.3);
    }
    cairo_restore(cr);
}
                     ...
                     const float pointerx, const float pointery, const float zoom_scale)
{
    ...
    const gboolean fail =
        !g->cursor_valid || !g->interpolation_valid || dev->full.pipe->processing || !g->has_focus;
    ...
    if (correction > 0.0f)
        cairo_arc(cr, x_pointer, y_pointer, setting_offset_x, M_PI, M_PI + correction * M_PI_4);
    else
        cairo_arc_negative(cr, x_pointer, y_pointer, setting_offset_x, M_PI,
                           M_PI + correction * M_PI_4);
...
static void _develop_distort_callback(gpointer instance, dt_iop_module_t *self)
{
    const dt_iop_toneequalizer_gui_data_t *g = self->gui_data;
    if (g == NULL)
        return;
                                     ...
                                     dt_iop_toneequalizer_gui_data_t *const restrict g)
{
    ...
    g->cst = dt_cairo_image_surface_create(CAIRO_FORMAT_ARGB32, g->allocation.width,
                                           g->allocation.height);
    ...
    cairo_rectangle(g->cr, g->gradient_left_limit, g->gradient_top_limit, g->gradient_width,
                    g->line_height);
    ...
    dt_cairo_perceptual_gradient(grad, 1.0);
    cairo_set_line_width(g->cr, 0.0);
    cairo_rectangle(g->cr, -g->line_height - 2 * g->inner_padding, 0.0, g->line_height,
                    g->graph_height);
    cairo_set_source(g->cr, grad);
    cairo_fill(g->cr);
...
static gboolean area_draw(GtkWidget *widget, cairo_t *cr, dt_iop_module_t *self)
{
    ...
    if (g->histogram_valid && self->enabled)
    {
        ...
        if (g->histogram_last_decile > -0.1f)
        {
            ...
            dtgtk_cairo_paint_warning(g->cr, g->graph_width - 2.5 * g->line_height,
                                      0.5 * g->line_height, 2.0 * g->line_height,
                                      2.0 * g->line_height, 0, NULL);
            cairo_restore(g->cr);
        }
        ...
        if (g->histogram_first_decile < -7.9f)
        {
            ...
            cairo_save(g->cr);
            cairo_set_source_rgb(g->cr, 0.75, 0.50, 0.);
            dtgtk_cairo_paint_warning(g->cr, 0.5 * g->line_height, 0.5 * g->line_height,
    ...
    if (g->lut_valid)
    {
        ...
        for (int k = 1; k < UI_SAMPLES; k++)
        {
            ...
            cairo_line_to(g->cr, (x_temp + 8.0f) * g->graph_width / 8.0f, y_temp * g->graph_height);
        }
        cairo_stroke(g->cr);
    }
    ...
    if (self->enabled)
    {
        if (g->area_cursor_valid)
        {
            ...
            const float y = g->gui_lut[(int)CLAMP(((UI_SAMPLES - 1) * g->area_x / g->graph_width),
                                                  0, UI_SAMPLES - 1)];
                                   ...
                                   dt_iop_module_t *self)
{
    ...
    if (g->area_dragging)
    {
        ...
        // Get the desired correction on exposure channels
        g->area_dragging =
            set_new_params_interactive(cursor_exposure, offset, g->sigma * g->sigma / 2.0f, g, p);
        dt_iop_gui_leave_critical_section(self);
    }
...
GSList *mouse_actions(dt_iop_module_t *self)
{
    ...
    lm = dt_mouse_action_create_format(lm, DT_MOUSE_ACTION_SCROLL, GDK_SHIFT_MASK,
                                       _("[%s over image] change tone exposure in large steps"),
                                       self->name());
    lm = dt_mouse_action_create_format(lm, DT_MOUSE_ACTION_SCROLL, GDK_CONTROL_MASK,
                                       _("[%s over image] change tone exposure in small steps"),
                                       self->name());
    return lm;
}
...
static void _develop_ui_pipe_started_callback(gpointer instance, dt_iop_module_t *self)
{
    dt_iop_toneequalizer_gui_data_t *g = self->gui_data;
    if (g == NULL)
        return;
...
static void _develop_preview_pipe_finished_callback(gpointer instance, dt_iop_module_t *self)
{
    const dt_iop_toneequalizer_gui_data_t *g = self->gui_data;
    if (g == NULL)
        return;
...
void gui_init(dt_iop_module_t *self)
{
    ...
    gtk_widget_set_tooltip_text(
        g->smoothing, _("positive values will produce more progressive tone transitions\n"
    ...
    dt_bauhaus_widget_set_quad(g->exposure_boost, self, dtgtk_cairo_paint_wand, FALSE,
                               auto_adjust_exposure_boost, _("auto-adjust the average exposure"));
    ...
    gtk_widget_set_tooltip_text(
        g->contrast_boost, _("use this to counter the averaging effect of the guided filter\n"
                             "and dilate the mask contrast around -4EV\n"
                             "this allows to spread the exposure histogram over more channels\n"
                             "for a better control of the exposure correction."));
    dt_bauhaus_widget_set_quad(g->contrast_boost, self, dtgtk_cairo_paint_wand, FALSE,
                               auto_adjust_contrast_boost, _("auto-adjust the contrast"));
    ...
    gtk_widget_set_tooltip_text(
        g->iterations, _("number of passes of guided filter to apply\n"
    ...
    gtk_widget_set_tooltip_text(g->blending,
    ...
    gtk_widget_set_tooltip_text(g->feathering,
    ...
    gtk_widget_set_tooltip_text(
        g->quantization, _("0 disables the quantization.\n"
    ...
    self->widget = dt_gui_vbox(
        g->notebook, dt_gui_hbox(dt_gui_expand(dt_ui_label_new(_("display exposure mask"))),
