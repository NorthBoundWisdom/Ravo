...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self,
                                  _("recover clipped highlights by propagating surrounding colors"),
                                  _("corrective"), _("linear or non-linear, Lab, display-referred"),
...
static inline void image_to_grid(const dt_iop_colorreconstruct_bilateral_t *const b, const float i,
                                 const float j, const float L, float *x, float *y, float *z)
{
...
static inline void grid_rescale(const dt_iop_colorreconstruct_bilateral_t *const b, const int i,
                                const int j, const dt_iop_roi_t *roi, const float scale, float *px,
                                float *py)
{
...
static dt_iop_colorreconstruct_bilateral_t *
dt_iop_colorreconstruct_bilateral_init(const dt_iop_roi_t *roi, // dimensions of input image
...
static dt_iop_colorreconstruct_bilateral_frozen_t *
dt_iop_colorreconstruct_bilateral_freeze(dt_iop_colorreconstruct_bilateral_t *b)
{
    if (!b)
        return NULL;

    dt_iop_colorreconstruct_bilateral_frozen_t *bf =
        malloc(sizeof(dt_iop_colorreconstruct_bilateral_frozen_t));
    if (!bf)
    {
...
static dt_iop_colorreconstruct_bilateral_t *
dt_iop_colorreconstruct_bilateral_thaw(dt_iop_colorreconstruct_bilateral_frozen_t *bf)
{
    if (!bf)
        return NULL;
...
static void blur_line(dt_iop_colorreconstruct_Lab_t *buf, const int offset1, const int offset2,
                      const int offset3, const int size1, const int size2, const int size3)
{
    ...
    for (int k = 0; k < size1; k++)
    {
        ...
        for (int j = 0; j < size2; j++)
        {
            ...
            buf[index].L =
                buf[index].L * w0 + w1 * buf[index + offset3].L + w2 * buf[index + 2 * offset3].L;
            buf[index].a =
                buf[index].a * w0 + w1 * buf[index + offset3].a + w2 * buf[index + 2 * offset3].a;
            buf[index].b =
                buf[index].b * w0 + w1 * buf[index + offset3].b + w2 * buf[index + 2 * offset3].b;
            ...
            buf[index].L = buf[index].L * w0 + w1 * (buf[index + offset3].L + tmp1.L) +
                           w2 * buf[index + 2 * offset3].L;
            buf[index].a = buf[index].a * w0 + w1 * (buf[index + offset3].a + tmp1.a) +
                           w2 * buf[index + 2 * offset3].a;
            buf[index].b = buf[index].b * w0 + w1 * (buf[index + offset3].b + tmp1.b) +
                           w2 * buf[index + 2 * offset3].b;
            buf[index].weight = buf[index].weight * w0 +
                                w1 * (buf[index + offset3].weight + tmp1.weight) +
                                w2 * buf[index + 2 * offset3].weight;
            index += offset3;
            for (int i = 2; i < size3 - 2; i++)
            {
                ...
                buf[index].weight = buf[index].weight * w0 +
                                    w1 * (buf[index + offset3].weight + tmp2.weight) +
            ...
            buf[index].weight = buf[index].weight * w0 +
                                w1 * (buf[index + offset3].weight + tmp2.weight) + w2 * tmp1.weight;
            index += offset3;
...
static void dt_iop_colorreconstruct_bilateral_slice(
    const dt_iop_colorreconstruct_bilateral_t *const b, const float *const in, float *const out,
    const float threshold, const dt_iop_roi_t *const roi, const float iscale)
{
    if (!b)
        return;
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    const float hue =
        hue_conversion(data->hue); // convert to LCH hue which better fits to Lab colorspace
        ...
        dt_pipe_is_full(piece->pipe))
    {
        ...
        {
            if (!dt_dev_sync_pixelpipe_hash(self->dev, piece->pipe, self->iop_order,
                                            DT_DEV_TRANSFORM_DIR_BACK_INCL, &self->gui_lock,
                                            &g->hash))
    ...
    {
        dt_hash_t hash = dt_dev_hash_plus(self->dev, piece->pipe, self->iop_order,
...
static dt_iop_colorreconstruct_bilateral_cl_t *
dt_iop_colorreconstruct_bilateral_init_cl(const int devid,
                                          dt_iop_colorreconstruct_global_data_t *global,
                                          ...
                                          const float sigma_r) // range sigma (blur luma values)
{
    ...
    dt_opencl_local_buffer_t locopt =
        (dt_opencl_local_buffer_t){.xoffset = 0,
                                   ...
                                   .yoffset = 0,
                                   .yfactor = 1,
                                   .cellsize = 4 * sizeof(float) + sizeof(int),
                                   .overhead = 0,
                                   .sizex = 1 << 6,
                                   .sizey = 1 << 6};
    ...
    dt_iop_colorreconstruct_bilateral_cl_t *b =
        malloc(sizeof(dt_iop_colorreconstruct_bilateral_cl_t));
    if (!b)
    {
    ...
    // alloc grid buffer:
    b->dev_grid = dt_opencl_alloc_device_buffer(b->devid, sizeof(float) * 4 * b->size_x *
                                                              b->size_y * b->size_z);
    if (!b->dev_grid)
    {
        dt_print(DT_DEBUG_OPENCL,
                 "[opencl_colorreconstruction] not able to allocate device buffer (b)");
        dt_iop_colorreconstruct_bilateral_free_cl(b);
        return NULL;
    }

    // alloc temporary grid buffer
    b->dev_grid_tmp = dt_opencl_alloc_device_buffer(b->devid, sizeof(float) * 4 * b->size_x *
                                                                  b->size_y * b->size_z);
    if (!b->dev_grid_tmp)
    {
        dt_print(DT_DEBUG_OPENCL,
                 "[opencl_colorreconstruction] not able to allocate device buffer (c)");
        dt_iop_colorreconstruct_bilateral_free_cl(b);
        return NULL;
    }
    ...
    cl_int err = dt_opencl_enqueue_kernel_2d_args(b->devid, b->global->kernel_colorreconstruct_zero,
                                                  wd, ht, CLARG(b->dev_grid), CLARG(wd), CLARG(ht));
    if (err != CL_SUCCESS)
    {
        dt_print(DT_DEBUG_OPENCL,
...
static dt_iop_colorreconstruct_bilateral_frozen_t *
dt_iop_colorreconstruct_bilateral_freeze_cl(dt_iop_colorreconstruct_bilateral_cl_t *b)
{
    if (!b)
        return NULL;
    ...
    if (bf->buf && b->dev_grid)
    {
        ...
        cl_int err = dt_opencl_read_buffer_from_device(
            b->devid, bf->buf, b->dev_grid, 0,
    ...
    {
        dt_print(DT_DEBUG_OPENCL,
                 "[opencl_colorreconstruction] not able to allocate host buffer (e)");
        dt_iop_colorreconstruct_bilateral_dump(bf);
        return NULL;
    }
...
static dt_iop_colorreconstruct_bilateral_cl_t *
dt_iop_colorreconstruct_bilateral_thaw_cl(dt_iop_colorreconstruct_bilateral_frozen_t *bf,
                                          const int devid,
                                          dt_iop_colorreconstruct_global_data_t *global)
{
    if (!bf || !bf->buf)
    ...
    {
        dt_print(DT_DEBUG_OPENCL,
                 "[opencl_colorreconstruction] not able to allocate device buffer (g)");
        dt_iop_colorreconstruct_bilateral_free_cl(b);
        return NULL;
    }
    ...
    {
        dt_print(DT_DEBUG_OPENCL,
                 "[opencl_colorreconstruction] not able to allocate device buffer (h)");
        dt_iop_colorreconstruct_bilateral_free_cl(b);
        return NULL;
    }
    ...
    if (bf->buf)
    {
        ...
        cl_int err = dt_opencl_write_buffer_to_device(
            b->devid, bf->buf, b->dev_grid, 0,
...
static cl_int dt_iop_colorreconstruct_bilateral_splat_cl(
    dt_iop_colorreconstruct_bilateral_cl_t *b, cl_mem in, const float threshold,
    dt_iop_colorreconstruct_precedence_t precedence, const float *params)
{
    ...
    err = dt_opencl_enqueue_kernel_2d_local_args(
        b->devid, b->global->kernel_colorreconstruct_splat, sizes, local, CLARG(in),
        CLARG(b->dev_grid), CLARG(b->width), CLARG(b->height), CLARG(b->size_x), CLARG(b->size_y),
        CLARG(b->size_z), CLARG(b->sigma_s), CLARG(b->sigma_r), CLARG(threshold), CLARG(pref),
        CLARRAY(4, params), CLLOCAL(b->blocksizex * b->blocksizey * sizeof(int)),
...
static cl_int dt_iop_colorreconstruct_bilateral_blur_cl(dt_iop_colorreconstruct_bilateral_cl_t *b)
{
    ...
    err = dt_opencl_enqueue_copy_buffer_to_buffer(b->devid, b->dev_grid, b->dev_grid_tmp, 0, 0,
                                                  sizeof(float) * b->size_x * b->size_y *
                                                      b->size_z * 4);
    if (err != CL_SUCCESS)
        return err;
    ...
    err = dt_opencl_enqueue_kernel_2d_args(
        b->devid, b->global->kernel_colorreconstruct_blur_line, b->size_z, b->size_y,
        CLARG(b->dev_grid_tmp), CLARG(b->dev_grid), CLARG(stride1), CLARG(stride2), CLARG(stride3),
        CLARG(b->size_z), CLARG(b->size_y), CLARG(b->size_x));
    if (err != CL_SUCCESS)
        return err;
    ...
    stride3 = b->size_x;
    err = dt_opencl_enqueue_kernel_2d_args(
        b->devid, b->global->kernel_colorreconstruct_blur_line, b->size_z, b->size_x,
        CLARG(b->dev_grid), CLARG(b->dev_grid_tmp), CLARG(stride1), CLARG(stride2), CLARG(stride3),
        CLARG(b->size_z), CLARG(b->size_x), CLARG(b->size_y));
    if (err != CL_SUCCESS)
        return err;
    ...
    err = dt_opencl_enqueue_kernel_2d_args(
        b->devid, b->global->kernel_colorreconstruct_blur_line, b->size_x, b->size_y,
        CLARG(b->dev_grid_tmp), CLARG(b->dev_grid), CLARG(stride1), CLARG(stride2), CLARG(stride3),
        CLARG(b->size_x), CLARG(b->size_y), CLARG(b->size_z));

    return err;
}

static cl_int dt_iop_colorreconstruct_bilateral_slice_cl(dt_iop_colorreconstruct_bilateral_cl_t *b,
                                                         ...
                                                         const float iscale)
{
    ...
    err = dt_opencl_enqueue_kernel_2d_args(
        b->devid, b->global->kernel_colorreconstruct_slice, roi->width, roi->height, CLARG(in),
        CLARG(out), CLARG(b->dev_grid), CLARG(roi->width), CLARG(roi->height), CLARG(b->size_x),
        CLARG(b->size_y), CLARG(b->size_z), CLARG(b->sigma_s), CLARG(b->sigma_r), CLARG(threshold),
        CLARG(bxy), CLARG(roixy), CLARG(rescale));
    return err;
}
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    const float hue =
        hue_conversion(d->hue); // convert to LCH hue which better fits to Lab colorspace
    ...
    {
        b = dt_iop_colorreconstruct_bilateral_init_cl(piece->pipe->devid, gd, roi_in, piece->iscale,
                                                      sigma_s, sigma_r);
        if (!b)
            goto error;
        err = dt_iop_colorreconstruct_bilateral_splat_cl(b, dev_in, d->threshold, d->precedence,
                                                         params);
        if (err != CL_SUCCESS)
            goto error;
    ...
    err = dt_iop_colorreconstruct_bilateral_slice_cl(b, dev_in, dev_out, d->threshold, roi_in,
                                                     piece->iscale);
    if (err != CL_SUCCESS)
        goto error;
...
static size_t dt_iop_colorreconstruct_bilateral_memory_use(
    ...
    const float sigma_r) // range sigma (blur luma values)
{
    ...
    return size_x * size_y * size_z * 4 * sizeof(float) *
           2; // in fact only the OpenCL path needs a second tmp buffer
}

static size_t dt_iop_colorreconstruct_bilateral_singlebuffer_size(
    const int width,     // width of input image
    const int height,    // height of input image
    const float sigma_s, // spatial sigma (blur pixel coords)
    const float sigma_r) // range sigma (blur luma values)
                     ...
                     dt_develop_tiling_t *tiling)
{
    ...
    tiling->factor = 2.0f + (float)dt_iop_colorreconstruct_bilateral_memory_use(width, height,
                                                                                sigma_s, sigma_r) /
                                basebuffer;
    tiling->maxbuf = fmax(1.0f, (float)dt_iop_colorreconstruct_bilateral_singlebuffer_size(
                                    width, height, sigma_s, sigma_r) /
                                    basebuffer);
    tiling->overhead = 0;
...
void init_global(dt_iop_module_so_t *self)
{
    dt_iop_colorreconstruct_global_data_t *gd =
        malloc(sizeof(dt_iop_colorreconstruct_global_data_t));
    self->data = gd;
    ...
    gd->kernel_colorreconstruct_splat =
        dt_opencl_create_kernel(program, "colorreconstruction_splat");
    gd->kernel_colorreconstruct_blur_line =
        dt_opencl_create_kernel(program, "colorreconstruction_blur_line");
    gd->kernel_colorreconstruct_slice =
        dt_opencl_create_kernel(program, "colorreconstruction_slice");
}
...
void gui_init(dt_iop_module_t *self)
{
    ...
    gtk_widget_set_tooltip_text(
        g->threshold, _("pixels with lightness values above this threshold are corrected"));
    gtk_widget_set_tooltip_text(g->spatial,
                                _("how far to look for replacement colors in spatial dimensions"));
    gtk_widget_set_tooltip_text(
        g->range, _("how far to look for replacement colors in the luminance dimension"));
    gtk_widget_set_tooltip_text(g->precedence,
                                _("if and how to give precedence to specific replacement colors"));
    gtk_widget_set_tooltip_text(
        g->hue, _("the hue tone which should be given precedence over other hue tones"));

    GtkWidget *monochromes = dt_ui_label_new(_("not applicable"));
    gtk_widget_set_tooltip_text(monochromes,
                                _("no highlights reconstruction for monochrome images"));
