...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self,
                                  _("reduce banding and posterization effects in output\n"
                                    "JPEGs by adding random noise, or reduce bit depth"),
                                  _("corrective, artistic"), _("non-linear, RGB, display-referred"),
...
void init_presets(dt_iop_module_so_t *self)
{
    ...
    dt_gui_presets_add_generic(_("dither"), self->op, self->version(), &tmp,
                               sizeof(dt_iop_dither_params_t), TRUE, DEVELOP_BLEND_CS_NONE);
...
DT_OMP_DECLARE_SIMD()
static inline void _nearest_color(float *const restrict val, float *const restrict err,
                                  const int graymode, const float f, const float rf)
{
    if (graymode)
    {
...
static inline void _diffuse_error(float *const restrict val, const float *const restrict err,
                                  const float factor)
{
    for_each_channel(c, aligned(val, err))
    {
...
static inline void _clipnan_pixel(float *const restrict out, const float *const restrict in)
{
    for_each_channel(c, aligned(in, out)) out[c] = _clipnan(in[c]);
}
...
static int _get_dither_parameters(const dt_iop_dither_data_t *const data,
...
static void _process_floyd_steinberg(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                                     const void *const ivoid, void *const ovoid,
                                     const dt_iop_roi_t *const roi_in,
                                     const dt_iop_roi_t *const roi_out, const gboolean fast_mode)
{
...
#define PROCESS_PIXEL_FULL(_pixel, inpix)                                                          \
    {                                                                                              \
        ...
        _clipnan_pixel(pixel_ + downright,                                                         \
                       (inpix) + downright); /* prepare downright for first access */              \
        _diffuse_error(pixel_ + right, err,                                                        \
                       RIGHT_WT); /* diffuse quantization error to neighbors */                    \
...
#define PROCESS_PIXEL_RIGHT(pixel)                                                                 \
    ...
    _diffuse_error(pixel + downleft, err,                                                          \
                   DOWNLEFT_WT); /* diffuse quantization error to neighbors */                     \
    ...
    // final row
    {
        ...
        // lower right pixel
        _nearest_color(outrow + 4 * (width - 1), err, graymode, f,
                       rf); // quantize the last pixel, no neighbors left
    }
}

static void _process_random(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                            const void *const ivoid, void *const ovoid,
                            const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    for (int j = 0; j < height; j++)
    {
        ...
        tea_state[0] =
            j * height; /* + dt_get_thread_num() -- do not include, makes results unreproducible */
...
static void _process_posterize(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                               const void *const ivoid, void *const ovoid,
                               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    const dt_iop_dither_data_t *const data = piece->data;
    ...
    for (int k = 0; k < npixels; k++)
    {
        ...
        for_each_channel(c) pixel[c] = _quantize(in[4 * k + c], f, rf);
