...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self,
                                  _("increase saturation and separation between\n"
                                    "opposite colors"),
                                   ...
                                   const dt_aligned_pixel_t low, const dt_aligned_pixel_t high)
{
    ...
    for_each_channel(c) res[c] = CLAMPS(in[c] * slope[c] + offset[c], low[c], high[c]);
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    return dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_colorcontrast, width, height, CLARG(dev_in), CLARG(dev_out), CLARG(width),
        CLARG(height), CLARG(scale), CLARG(offset), CLARG(unbound));
}
#endif
...
void gui_init(dt_iop_module_t *self)
{
    ...
    gtk_widget_set_tooltip_text(g->a_scale,
    ...
    gtk_widget_set_tooltip_text(g->b_scale,
