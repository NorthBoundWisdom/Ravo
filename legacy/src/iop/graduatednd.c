...
{
    float
        density; // $MIN: -8.0 $MAX: 8.0 $DEFAULT: 1.0 $DESCRIPTION: "density" The density of filter 0-8 EV
    float
        hardness; // $MIN: 0.0 $MAX: 100.0 $DEFAULT: 0.0 $DESCRIPTION: "hardness" 0% = soft and 100% = hard
    float
        rotation; // $MIN: -180.0 $MAX: 180.0 $DEFAULT: 0.0 $DESCRIPTION: "rotation" 2*PI -180 - +180
...
void init_presets(dt_iop_module_so_t *self)
{
    ...
    dt_gui_presets_add_generic(_("neutral gray | ND2 (soft)"), self->op, self->version(),
                               ...
                               sizeof(dt_iop_graduatednd_params_t), TRUE,
...
static float _dist_seg(const float xa, const float ya, const float xb, const float yb,
...
static int _set_grad_from_points(dt_iop_module_t *self, const float xa, const float ya,
                                 const float xb, const float yb, float *rotation, float *offset)
{
    // we want absolute positions
    float wd, ht;
    dt_dev_get_preview_size(self->dev, &wd, &ht);
    float pts[4] = {xa * wd, ya * ht, xb * wd, yb * ht};
    ...
    if (iter >= 1000)
        return 8; // generally in less than 20 iterations all is good, so we are over conservative
...
static int _set_points_from_grad(dt_iop_module_t *self, float *xa, float *ya, float *xb, float *yb,
                     ...
                     const float pointerx, const float pointery, const float zoom_scale)
{
    ...
    if (g->define == 0)
    {
        if (!_set_points_from_grad(self, &g->xa, &g->ya, &g->xb, &g->yb, p->rotation, p->offset))
            return;
        g->define = 1;
    }
...
int scrolled(dt_iop_module_t *self, float x, float y, int up, uint32_t state)
{
    const dt_iop_graduatednd_gui_data_t *g = self->gui_data;
    const dt_iop_graduatednd_params_t *p = self->params;
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    const dt_iop_graduatednd_data_t *const data =
        (const dt_iop_graduatednd_data_t *const)piece->data;
    ...
    const float filter_hardness =
        (1.0f / filter_radie) / (1.0f - (0.5f + (data->hardness / 100.0f) * 0.9f / 2.0f)) * 0.5f;
    ...
    const dt_aligned_pixel_t color = {data->color[0], data->color[1], data->color[2],
                                      data->color[3]};
    const dt_aligned_pixel_t color1 = {data->color1[0], data->color1[1], data->color1[2],
                                       data->color1[3]};
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    const float length_base =
        (sinv * (-1.0 + ix * hw_inv) - cosv * (-1.0f + iy * hh_inv) - 1.0f + offset) *
        filter_hardness;
    ...
    return dt_opencl_enqueue_kernel_2d_args(devid, kernel, width, height, CLARG(dev_in),
                                            ...
                                            CLARG(data->color), CLARG(density), CLARG(length_base),
                                            CLARG(length_inc_x), CLARG(length_inc_y));
}
#endif
                   ...
                   dt_dev_pixelpipe_iop_t *piece)
{
    ...
    if (d->density < 0)
        for (int l = 0; l < 4; l++)
            d->color[l] = 1.0f - d->color[l];

    for (int l = 0; l < 4; l++)
        d->color1[l] = 1.0f - d->color[l];
}
...
GSList *mouse_actions(dt_iop_module_t *self)
{
    ...
    lm = dt_mouse_action_create_format(lm, DT_MOUSE_ACTION_LEFT_DRAG, 0,
                                       _("[%s on line] move line"), self->name());
