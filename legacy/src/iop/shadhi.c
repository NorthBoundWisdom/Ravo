...
#define UNBOUND_DEFAULT                                                                            \
    (UNBOUND_SHADOWS_L | UNBOUND_SHADOWS_A | UNBOUND_SHADOWS_B | UNBOUND_HIGHLIGHTS_L |            \
     UNBOUND_HIGHLIGHTS_A | UNBOUND_HIGHLIGHTS_B | UNBOUND_GAUSSIAN)
...
typedef struct dt_iop_shadhi_params_t
{
    ...
    float
        shadows_ccorrect; // $MIN: 0.0 $MAX: 100.0 $DEFAULT: 100.0 $DESCRIPTION: "shadows color adjustment"
    float
        highlights_ccorrect; // $MIN: 0.0 $MAX: 100.0 $DEFAULT: 50.0 $DESCRIPTION: "highlights color adjustment"
...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self,
                                  _("modify the tonal range of the shadows and highlights\n"
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    const float compress = fmin(fmax(0, (data->compress / 100.0f)),
                                0.99f); // upper limit 0.99f to avoid division by zero later
    const float shadows_ccorrect =
        (fmin(fmax(0.0f, (data->shadows_ccorrect / 100.0f)), 1.0f) - 0.5f) * sign(shadows) + 0.5f;
    const float highlights_ccorrect =
        (fmin(fmax(0.0f, (data->highlights_ccorrect / 100.0f)), 1.0f) - 0.5f) * sign(-highlights) +
        0.5f;
    const unsigned int flags = data->flags;
    const int unbound_mask =
        ((data->shadhi_algo == SHADHI_ALGO_BILATERAL) && (flags & UNBOUND_BILATERAL)) ||
...
#define doublemax (2.0f * lmax)
    ...
    for (size_t j = 0; j < 4 * npixels; j += 4)
    {
        ...
        while (highlights2 > 0.0f)
        {
            ...
            const float lref = copysignf(
                fabsf(la) > low_approximation ? 1.0f / fabsf(la) : 1.0f / low_approximation, la);
            const float href =
                copysignf(fabsf(1.0f - la) > low_approximation ? 1.0f / fabsf(1.0f - la) :
                                                                 1.0f / low_approximation,
                          1.0f - la);
        ...
        const float shadows_xform =
            CLAMP(tb[0] / (1.0f - compress) - compress / (1.0f - compress), 0.0f, 1.0f);
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    const float compress = fmin(fmax(0.0f, (d->compress / 100.0f)),
                                0.99f); // upper limit 0.99f to avoid division by zero later
    const float shadows_ccorrect =
        (fmin(fmax(0.0f, (d->shadows_ccorrect / 100.0f)), 1.0f) - 0.5f) * sign(shadows) + 0.5f;
    const float highlights_ccorrect =
        (fmin(fmax(0.0f, (d->highlights_ccorrect / 100.0f)), 1.0f) - 0.5f) * sign(-highlights) +
        0.5f;
    const float low_approximation = d->low_approximation;
    const unsigned int flags = d->flags;
    const int unbound_mask =
        ((d->shadhi_algo == SHADHI_ALGO_BILATERAL) && (flags & UNBOUND_BILATERAL)) ||
    ...
    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_shadows_highlights_mix, width, height, CLARG(dev_in), CLARG(dev_tmp),
        CLARG(dev_out), CLARG(width), CLARG(height), CLARG(shadows), CLARG(highlights),
        CLARG(compress), CLARG(shadows_ccorrect), CLARG(highlights_ccorrect), CLARG(flags),
        CLARG(unbound_mask), CLARG(low_approximation), CLARG(whitepoint));
                     ...
                     dt_develop_tiling_t *tiling)
{
    ...
    if (d->shadhi_algo == SHADHI_ALGO_BILATERAL)
    {
        // bilateral filter
        tiling->factor =
            2.0f + fmax(1.0f, (float)dt_bilateral_memory_use(width, height, sigma_s, sigma_r) /
                                  basebuffer);
        tiling->maxbuf =
            fmax(1.0f, (float)dt_bilateral_singlebuffer_size(width, height, sigma_s, sigma_r) /
                           basebuffer);
    }
    else
    {
        // gaussian blur
        tiling->factor =
            2.0f + fmax(1.0f, (float)dt_gaussian_memory_use(width, height, channels) / basebuffer);
#ifdef HAVE_OPENCL
        tiling->factor_cl =
            2.0f +
            fmax(1.0f, (float)dt_gaussian_memory_use_cl(width, height, channels) / basebuffer);
#endif
        tiling->maxbuf =
            fmax(1.0f, (float)dt_gaussian_singlebuffer_size(width, height, channels) / basebuffer);
    }
...
void gui_init(dt_iop_module_t *self)
{
    ...
    gtk_widget_set_tooltip_text(g->shadhi_algo,
                                _("filter to use for softening. bilateral avoids halos"));
    gtk_widget_set_tooltip_text(
        g->compress, _("compress the effect on shadows/highlights and\npreserve mid-tones"));
