...
{
    dt_iop_cacorrectrgb_guide_channel_t
        guide_channel; // $DEFAULT: DT_CACORRECT_RGB_G $DESCRIPTION: "guide"
    float radius;      // $MIN: 1 $MAX: 500 $DEFAULT: 5 $DESCRIPTION: "radius"
    float strength;    // $MIN: 0 $MAX: 4 $DEFAULT: 0.5 $DESCRIPTION: "strength"
    dt_iop_cacorrectrgb_mode_t
        mode; // $DEFAULT: DT_CACORRECT_MODE_STANDARD $DESCRIPTION: "correction mode"
    gboolean
        refine_manifolds; // $MIN: FALSE $MAX: TRUE $DEFAULT: FALSE $DESCRIPTION: "very large chromatic aberration"
...
{
    return dt_iop_set_description(self, _("correct chromatic aberrations"), _("corrective"),
                                  _("linear, RGB, scene-referred"), _("linear, RGB"),
...
static void normalize_manifolds(const float *const restrict blurred_in,
                                ...
                                float *const restrict blurred_manifold_higher, const size_t width,
                                const size_t height,
                                const dt_iop_cacorrectrgb_guide_channel_t guide)
{
    ...
    for (size_t k = 0; k < width * height; k++)
    {
        ...
        if (weighth < 0.05f)
        {
            ...
            {
                blurred_manifold_higher[k * 4 + c] =
                    w * blurred_manifold_higher[k * 4 + c] + (1.0f - w) * blurred_in[k * 4 + c];
            }
        }
        if (weightl < 0.05f)
        {
            ...
            {
                blurred_manifold_lower[k * 4 + c] =
                    w * blurred_manifold_lower[k * 4 + c] + (1.0f - w) * blurred_in[k * 4 + c];
            }
        }
    }
}
                          ...
                          float *const restrict manifolds, gboolean refine_manifolds)
{
    ...
    normalize_manifolds(blurred_in, blurred_manifold_lower, blurred_manifold_higher, width, height,
                        guide);
    ...
    if (refine_manifolds)
    {
        ...
        for (size_t k = 0; k < width * height; k++)
        {
            ...
            for (size_t kc = 0; kc <= 1; kc++)
            {
                ...
                w *= 1.0f * (0.2f + 1.0f / fmaxf(dist_to_good, 0.1f)) /
                     (0.2f + 1.0f / fmaxf(dist_to_bad, 0.1f));
            }
        ...
        dt_gaussian_blur_4c(g, manifold_higher, blurred_manifold_higher);
        dt_gaussian_blur_4c(g, manifold_lower, blurred_manifold_lower);
        normalize_manifolds(blurred_in, blurred_manifold_lower, blurred_manifold_higher, width,
                            height, guide);
        dt_gaussian_free(g);
    }
...
static void apply_correction(const float *const restrict in, const float *const restrict manifolds,
                             const size_t width, const size_t height, const float sigma,
                             const dt_iop_cacorrectrgb_guide_channel_t guide,
                             const dt_iop_cacorrectrgb_mode_t mode, float *const restrict out)

{
    ...
    for (size_t k = 0; k < width * height; k++)
    {
        ...
        for (size_t kc = 0; kc <= 1; kc++)
        {
            ...
            const float ratio =
                powf(ratio_low_manifolds, weight_low) * powf(ratio_high_manifolds, weight_high);
...
static void reduce_artifacts(const float *const restrict in, const size_t width,
                             const size_t height, const float sigma,
                             const dt_iop_cacorrectrgb_guide_channel_t guide, const float safety,
...

{
    ...
    for (size_t k = 0; k < width * height; k++)
    {
        ...
        {
            const size_t c = (guide + kc + 1) % 3;
            out[k * 4 + c] = fmaxf(1.0f - w, 0.0f) * fmaxf(in[k * 4 + c], 0.0f) +
                             w * fmaxf(out[k * 4 + c], 0.0f);
        }
    }
    dt_free_align(blurred_in_out);
}

static void reduce_chromatic_aberrations(const float *const restrict in, const size_t width,
                                         const size_t height, const size_t ch, const float sigma,
                                         const float sigma2,
                                         const dt_iop_cacorrectrgb_guide_channel_t guide,
                                         const dt_iop_cacorrectrgb_mode_t mode,
                                         const gboolean refine_manifolds, const float safety,
                                         float *const restrict out)

{
    ...
    // Compute manifolds
    get_manifolds(ds_in, ds_width, ds_height, sigma / downsize, sigma2 / downsize, guide,
                  ds_manifolds, refine_manifolds);
    dt_free_align(ds_in);
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    reduce_chromatic_aberrations(in, width, height, ch, sigma, sigma2, d->guide_channel, d->mode,
                                 d->refine_manifolds, safety, out);
}
