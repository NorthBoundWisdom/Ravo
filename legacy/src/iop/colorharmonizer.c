...
{
    dt_iop_colorharmonizer_rule_t
        rule;            // $DEFAULT: DT_COLORHARMONIZER_COMPLEMENTARY $DESCRIPTION: "harmony rule"
    ...
    float
        neutral_protection; // $MIN: 0.0 $MAX: 1.0 $DEFAULT: 0.5 $DESCRIPTION: "neutral color protection"
...
typedef struct dt_iop_colorharmonizer_gui_data_t
{
    ...
    GtkWidget
        *sat_slider[COLORHARMONIZER_MAX_NODES]; // per-node saturation sliders (chroma gradient)
...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(
        self, _("harmonize colors toward a selected palette in perceptual space"),
...
static inline float get_weighted_hue_shift(const float px_hue, const float *nodes,
                                           const int num_nodes, const float pull_width_factor,
...
static inline void get_harmony_nodes(const dt_iop_colorharmonizer_rule_t rule,
                                     const float anchor_hue, const float *custom_hue,
                                     const int custom_n, float *nodes, int *num_nodes)
{
    ...
    dt_color_harmony_get_sector_angles((dt_color_harmony_type_t)(rule + 1), rotation, node_angles,
                                       num_nodes);
                   ...
                   dt_dev_pixelpipe_iop_t *piece)
{
    ...
    d->params = *p;
    get_harmony_nodes(p->rule, p->anchor_hue, p->custom_hue, p->num_custom_nodes, d->nodes,
                      &d->num_nodes);
}

static void _update_histogram(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                              const float *ivoid, const dt_iop_roi_t *roi_in)
{
    ...
    for (int j = 0; j < roi_in->height; j++)
    {
        ...
        for (int i = 0; i < roi_in->width; i++, src += ch)
        {
            ...
            dt_ioppr_rgb_matrix_to_dt_UCS_JCH(px_rgb, px_JCH, work_profile->matrix_in_transposed,
                                              L_white);
    ...
    if (p->smoothing <= 0.0f)
    {
        ...
        for (int j = 0; j < roi_out->height; j++)
        {
            ...
            for (int i = 0; i < roi_out->width; i++, in += ch, out += ch)
            {
                ...
                px_JCH[2] =
                    wrap_hue(hue + hue_shift * pull_strength * chroma_weight) * DT_2PI_F - M_PI_F;
                ...
                dt_apply_transposed_color_matrix(px_xyz, work_profile->matrix_out_transposed,
                                                 px_rgb_out);
    ...
    else
    {
        ...
        for (int j = 0; j < roi_out->height; j++)
        {
            ...
            for (int i = 0; i < roi_out->width; i++, in += ch, out += ch)
            {
                ...
                const float new_hue =
                    wrap_hue(hue + corrections[k * 2] * pull_strength * chroma_weight);
                dt_aligned_pixel_t px_JCH = {
                    J, fmaxf(chroma * (1.0f + corrections[k * 2 + 1] * chroma_weight), 0.0f),
                    new_hue * DT_2PI_F - M_PI_F, 0.0f};
...
static void _build_hue_luts(void)
{
    ...
    for (int j = 0; j < COLORHARMONIZER_RYB_INVERSE_STEPS; j++)
    {
        ...
        for (int i = 0; i < COLORHARMONIZER_RYB_INVERSE_STEPS; i++)
        {
            ...
            {
                best_dist = d;
                best_ucs = i / (float)COLORHARMONIZER_RYB_INVERSE_STEPS;
            }
        }
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    cl_mem output_matrix_cl =
        dt_opencl_copy_host_to_device_constant(devid, 12 * sizeof(float), output_matrix);
    cl_mem nodes_cl = dt_opencl_copy_host_to_device_constant(
        devid, COLORHARMONIZER_MAX_NODES * sizeof(float), nodes);
    cl_mem node_saturation_cl = dt_opencl_copy_host_to_device_constant(
        devid, COLORHARMONIZER_MAX_NODES * sizeof(float), node_saturation);
    ...
    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_colorharmonizer_map, width, height, CLARG(dev_in), CLARG(corrections_cl),
        CLARG(jch_cl), CLARG(width), CLARG(height), CLARG(input_matrix_cl), CLARG(nodes_cl),
        CLARG(num_nodes), CLARG(p->pull_width), CLARG(node_saturation_cl), CLARG(L_white));
    if (err != CL_SUCCESS)
        goto error;
    ...
    if (p->smoothing > 0.0f)
    {
        ...
        if ((err = dt_gaussian_mean_blur_cl(devid, corrections_cl, width, height, 2, sigma)) !=
            CL_SUCCESS)
            goto error;
    }
    ...
    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_colorharmonizer_apply, width, height, CLARG(dev_out), CLARG(width),
        CLARG(height), CLARG(output_matrix_cl), CLARG(jch_cl), CLARG(corrections_cl),
        CLARG(p->pull_strength), CLARG(p->neutral_protection), CLARG(L_white));
    ...
    if (err == CL_SUCCESS && self->gui_data && dt_pipe_is_preview(piece->pipe))
    {
        ...
        if (host_in)
        {
            if (dt_opencl_copy_image_to_host(devid, host_in, dev_in, width, height,
...
static float _find_max_chroma(const float hue)
{
    ...
    for (int iter = 0; iter < 16; iter++)
    {
        ...
        if (sRGB[0] >= 0.f && sRGB[1] >= 0.f && sRGB[2] >= 0.f && sRGB[0] <= 1.f &&
            sRGB[1] <= 1.f && sRGB[2] <= 1.f)
            C_lo = C_mid;
        else
            C_hi = C_mid;
    }
...
static void _paint_sat_slider(GtkWidget *slider, const float hue)
{
    ...
    for (int i = 0; i < DT_BAUHAUS_SLIDER_MAX_STOPS; i++)
    {
        ...
        dt_aligned_pixel_t sRGB;
        dt_UCS_JCH_to_sRGB(JCH, L_white, sRGB);
        dt_bauhaus_slider_set_stop(slider, stop, CLAMP(sRGB[0], 0.f, 1.f), CLAMP(sRGB[1], 0.f, 1.f),
                                   ...
                                   const dt_iop_colorharmonizer_params_t *const p)
{
    float nodes[COLORHARMONIZER_MAX_NODES];
    int num_nodes = 0;
    get_harmony_nodes(p->rule, p->anchor_hue, p->custom_hue, p->num_custom_nodes, nodes,
                      &num_nodes);
    for (int i = 0; i < num_nodes; i++)
...
static void _apply_harmony_guide(dt_iop_module_t *self, dt_iop_colorharmonizer_params_t *p,
                                 dt_iop_colorharmonizer_gui_data_t *g,
...
static gboolean _swatch_draw_callback(GtkWidget *widget, cairo_t *cr, gpointer user_data)
{
    ...
    else
    {
        float nodes[COLORHARMONIZER_MAX_NODES];
        int num_nodes = 0;
        get_harmony_nodes(p->rule, p->anchor_hue, NULL, COLORHARMONIZER_MAX_NODES, nodes,
                          &num_nodes);
        if (idx < num_nodes)
            hue = nodes[idx];
        else
                                         ...
                                         dt_iop_colorharmonizer_params_t *p)
{
    ...
    float node_angles[COLORHARMONIZER_MAX_NODES];
    int num_nodes = 0;
    dt_lib_histogram_get_sector_angles(darktable.lib, guide.type, guide.rotation, node_angles,
                                       &num_nodes);
    ...
    if (num_nodes < 2)
        p->custom_hue[1] = p->custom_hue[0]; // monochromatic fallback
...
static void _update_visibility(const dt_iop_colorharmonizer_params_t *p, const gboolean is_custom,
                               dt_iop_colorharmonizer_gui_data_t *g)
{
...
static void _sync_vectorscope_if_enabled(dt_iop_module_t *self,
                                         dt_iop_colorharmonizer_gui_data_t *g, GtkWidget *widget)
{
    if (!g->sync_to_vectorscope ||
        !gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(g->sync_to_vectorscope)))
        return;
...
static gboolean _picked_color_to_hue(dt_iop_module_t *self, dt_dev_pixelpipe_t *pipe,
                                     float *out_hue)
{
    ...
    for (int c = 0; c < 3; c++)
        px_rgb[c] = fmaxf(self->picked_color[c], 0.0f);
    px_rgb[3] = 0.0f;

    const float (*const matrix_in)[4] = (const float (*)[4])work_profile->matrix_in;
    px_xyz[0] =
        matrix_in[0][0] * px_rgb[0] + matrix_in[0][1] * px_rgb[1] + matrix_in[0][2] * px_rgb[2];
    px_xyz[1] =
        matrix_in[1][0] * px_rgb[0] + matrix_in[1][1] * px_rgb[1] + matrix_in[1][2] * px_rgb[2];
    px_xyz[2] =
        matrix_in[2][0] * px_rgb[0] + matrix_in[2][1] * px_rgb[1] + matrix_in[2][2] * px_rgb[2];
    px_xyz[3] = 0.0f;
...
void color_picker_apply(dt_iop_module_t *self, GtkWidget *picker, dt_dev_pixelpipe_t *pipe)
{
    ...
    float hue = 0.0f;
    if (!_picked_color_to_hue(self, pipe, &hue))
        return;
...
static float _score_harmony(const float *histo, const int num_bins,
                            const dt_iop_colorharmonizer_rule_t rule, const float anchor_hue)
{
    float nodes[COLORHARMONIZER_MAX_NODES];
    int num_nodes = 1;
...
static void _auto_detect_harmony(const float *histo, const int num_bins,
                                 dt_iop_colorharmonizer_rule_t *best_rule, float *best_anchor)
{
    ...
    *best_anchor = 0.0f;
    ...
    for (int r = 0; r < num_rules; r++)
    {
        for (int a = 0; a < num_steps; a++)
        {
            ...
            const float score =
                _score_harmony(smooth, num_bins, (dt_iop_colorharmonizer_rule_t)r, anchor);
            if (score > best_score)
            {
                best_score = score;
...
void gui_init(dt_iop_module_t *self)
{
    ...
    gtk_widget_set_tooltip_text(
        g->auto_detect, _("not yet available — wait for the preview to finish processing."));
...

    {
        ...
        g_signal_connect(G_OBJECT(slider), "value-changed", G_CALLBACK(_anchor_hue_slider_changed),
                         self);
        g->anchor_hue = dt_color_picker_new(self, DT_COLOR_PICKER_POINT_AREA, slider);
        dt_bauhaus_widget_set_quad_tooltip(g->anchor_hue, _("pick hue from image.\n"
    ...
    for (int i = 0; i < COLORHARMONIZER_MAX_NODES; i++)
    {
        ...
        GtkWidget *slider =
            dt_bauhaus_slider_new_with_range_and_feedback(self, 0.0f, 360.0f, 0.5f, 0.0f, 1, 0);
        dt_bauhaus_slider_set_format(slider, "°");
        g_object_set_data(G_OBJECT(slider), "slider-index", GINT_TO_POINTER(i));
        g_signal_connect(G_OBJECT(slider), "value-changed", G_CALLBACK(_custom_hue_changed), self);
        gtk_widget_set_tooltip_text(
            slider, _("hue of this harmony node (0°–360°).\n"
    ...
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(g->sync_to_vectorscope),
                                 dt_conf_get_bool(conf_key));
    ...
    dt_gui_new_collapsible_section(&g->sat_section,
                                   ...
                                   _("saturation"), GTK_BOX(main_box), DT_ACTION(self));
    ...
    for (int i = 0; i < COLORHARMONIZER_MAX_NODES; i++)
    {
        ...
        g_object_set_data(G_OBJECT(slider), "slider-index", GINT_TO_POINTER(i));
        g_signal_connect(G_OBJECT(slider), "value-changed", G_CALLBACK(_node_saturation_changed),
                         self);
        gtk_widget_set_tooltip_text(
            slider, _("saturation multiplier for colors near this harmony node.\n"
...
void gui_focus(dt_iop_module_t *self, gboolean in)
{
    ...
    if (in)
    {
        ...
        dt_lib_histogram_set_harmony_callback(
            darktable.lib, sync ? _on_vectorscope_harmony_changed : NULL, sync ? self : NULL);
    }
    else
    {
