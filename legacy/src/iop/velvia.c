...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self,
                                  _("resaturate giving more weight to blacks,\n"
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    else
    {
        ...
        for (size_t k = 0; k < npixels; k++)
        {
            ...
            float pweight = CLAMPS(
                ((1.0f - (1.5f * psat)) + ((1.0f + (fabsf(plum - 0.5f) * 2.0f)) * (1.0f - bias))) /
                    (1.0f + (1.0f - bias)),
                0.0f, 1.0f); // The weight of pixel
            float saturation =
                strength * pweight; // So lets calculate the final effect of filter on pixel
            ...
            for_each_channel(c) othersum[c] = rotate1[c] + rotate2[c];
            dt_aligned_pixel_t velvia;
            for_each_channel(c) velvia[c] =
                CLAMPS(chan[c] + saturation * (chan[c] - 0.5f * othersum[c]), 0.0f, 1.0f);
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    else
    {
        err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_velvia, width, height,
                                               ...
                                               CLARG(height), CLARG(strength), CLARG(bias));
    }
    return err;
}
#endif
