...
typedef struct dt_iop_exposure_params_t
{
    ...
    float
        deflicker_target_level; // $MIN: -18.0 $MAX: 18.0 $DEFAULT: -4.0 $DESCRIPTION: "target level"
...
void init_presets(dt_iop_module_so_t *self)
{
    ...
    dt_gui_presets_add_generic(_("magic lantern defaults"), self->op, self->version(),
                               ...
                               sizeof(dt_iop_exposure_params_t), TRUE,
...
void reload_defaults(dt_iop_module_t *self)
{
    ...
    const gboolean scene_raw =
        dt_image_is_rawprepare_supported(&self->dev->image_storage) && dt_is_scene_referred();
                                         ...
                                         dt_dev_histogram_stats_t *histogram_stats)
{
    ...
    if (!img || image.buf_dsc.channels != 1 || image.buf_dsc.datatype != TYPE_UINT16)
        return;
    ...
    dt_histogram_helper(&histogram_params, histogram_stats, IOP_CS_RAW, IOP_CS_NONE, buf.buf,
                        histogram, NULL, FALSE, NULL);
...
/* output: -16 ... 0 */
static double _raw_to_ev(const uint32_t raw, const uint32_t black_level, const uint32_t white_level)
{
...
static void _compute_correction(dt_iop_module_t *self, dt_iop_exposure_params_t *p,
                                dt_dev_pixelpipe_t *pipe, const uint32_t *const histogram,
                                ...
                                float *correction)
{
    *correction = EXPOSURE_CORRECTION_UNDEFINED;
    ...
    const double thr =
        CLAMP(((double)histogram_stats->pixels * (double)p->deflicker_percentile / (double)100.0),
              0.0, (double)histogram_stats->pixels);
    ...
    const double ev = _raw_to_ev(raw, (uint32_t)pipe->dsc.rawprepare.raw_black_level,
...
static void _process_common_setup(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece)
{
    ...
    if (d->deflicker)
    {
        if (g)
        {
            ...
            _compute_correction(self, &d->params, piece->pipe, g->deflicker_histogram,
                                &g->deflicker_histogram_stats, &exposure);
        }
        else
        {
            ...
            _compute_correction(self, &d->params, piece->pipe, histogram, &histogram_stats,
                                &exposure);
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_exposure, width, height, CLARG(dev_in),
...
static void _exposure_set_white(dt_iop_module_t *self, const float white)
{
    dt_iop_exposure_params_t *p = self->params;
...
static void _exposure_proxy_handle_event(int n_press, gdouble delta, GdkModifierType state,
                                         const gboolean is_blackpoint)
{
    ...
    if (self && self->gui_data)
    {
        ...
        GtkWidget *const widget =
            is_blackpoint ?
                g->black :
                (p->mode == EXPOSURE_MODE_DEFLICKER ? g->deflicker_target_level : g->exposure);
...
void gui_init(dt_iop_module_t *self)
{
    ...
    g->compensate_hilite_preserv = dt_bauhaus_toggle_from_params(self, "compensate_hilite_pres");
    ...
    dt_bauhaus_widget_set_quad_tooltip(g->exposure,
                                       _("set the exposure adjustment using the selected area"));
    dt_shortcut_register(dt_action_widget(g->exposure), 0, 0, GDK_KEY_e, GDK_SHIFT_MASK);
    ...
    dt_gui_box_add(vbox_deflicker,
                   dt_gui_hbox(dt_ui_label_new(_("computed EC: ")), g->deflicker_used_EC));
    ...
    gtk_widget_set_tooltip_text(g->black,
    ...
    dt_gui_new_collapsible_section(&g->cs, "plugins/darkroom/exposure/mapping",
                                   _("area exposure mapping"), GTK_BOX(self->widget),
                                   DT_ACTION(self));
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(g->spot_mode, self, NULL, N_("area mode"),
                                 ...
                                 0, _spot_settings_changed_callback, self, N_("correction"),
                                 N_("measure"));
