...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(
        self, _("perform color space corrections and apply looks"), _("corrective or creative"),
        _("linear or non-linear, Lab, display-referred"), _("defined by profile, Lab"),
...
void init_presets(dt_iop_module_so_t *self)
{
    ...
    dt_gui_presets_add_generic(_("it8 skin tones"), self->op, self->version(), &p, sizeof(p), TRUE,
    ...
    dt_gui_presets_add_generic(_("expanded color checker"), self->op, self->version(), &p,
    ...
    // flexibility. this is copied from the calibration tool implementation.
    ...
    assert(hk_params);
    dt_gui_presets_add_generic(_("Helmholtz/Kohlrausch monochrome"), self->op, self->version(),
                               hk_params, params_len, TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
    free(hk_params);
   ...
   *  https://jo.dreggn.org/blog/darktable-fuji-styles.tar.xz **/
    ...
    uint8_t *astia_params =
        dt_exif_xmp_decode(astia_params_input, strlen(astia_params_input), &params_len);

    assert(params_len == sizeof(dt_iop_colorchecker_params_t));
    assert(astia_params);
    dt_gui_presets_add_generic(_("Fuji Astia emulation"), self->op, self->version(), astia_params,
                               params_len, TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
    free(astia_params);
    ...
    uint8_t *chrome_params =
        dt_exif_xmp_decode(chrome_params_input, strlen(chrome_params_input), &params_len);

    assert(params_len == sizeof(dt_iop_colorchecker_params_t));
    assert(chrome_params);
    dt_gui_presets_add_generic(_("Fuji Classic Chrome emulation"), self->op, self->version(),
                               chrome_params, params_len, TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
    free(chrome_params);
    ...
    uint8_t *mchrome_params =
        dt_exif_xmp_decode(mchrome_params_input, strlen(mchrome_params_input), &params_len);

    assert(params_len == sizeof(dt_iop_colorchecker_params_t));
    assert(mchrome_params);
    dt_gui_presets_add_generic(_("Fuji Monochrome emulation"), self->op, self->version(),
                               mchrome_params, params_len, TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
    free(mchrome_params);
    ...
    uint8_t *provia_params =
        dt_exif_xmp_decode(provia_params_input, strlen(provia_params_input), &params_len);

    assert(params_len == sizeof(dt_iop_colorchecker_params_t));
    assert(provia_params);
    dt_gui_presets_add_generic(_("Fuji Provia emulation"), self->op, self->version(), provia_params,
                               params_len, TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
    free(provia_params);
    ...
    uint8_t *velvia_params =
        dt_exif_xmp_decode(velvia_params_input, strlen(velvia_params_input), &params_len);

    assert(params_len == sizeof(dt_iop_colorchecker_params_t));
    assert(velvia_params);
    dt_gui_presets_add_generic(_("Fuji Velvia emulation"), self->op, self->version(), velvia_params,
                               params_len, TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
    free(velvia_params);
}
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    const dt_aligned_pixel_t polynomial_L = {data->coeff_L[num_patches + 1],
                                             data->coeff_L[num_patches + 2],
                                             data->coeff_L[num_patches + 3], 0.0f};
    const dt_aligned_pixel_t polynomial_a = {data->coeff_a[num_patches + 1],
                                             data->coeff_a[num_patches + 2],
                                             data->coeff_a[num_patches + 3], 0.0f};
    const dt_aligned_pixel_t polynomial_b = {data->coeff_b[num_patches + 1],
    ...
    for (int k = 0; k < npixels; k++)
    {
        ...
        dt_aligned_pixel_t sums = {poly_L[0] + poly_L[1] + poly_L[2],
                                   ...
                                   poly_b[0] + poly_b[1] + poly_b[2], 0.0f};
        dt_aligned_pixel_t res;
        for_each_channel(c) res[c] = patches[num_patches][c] + sums[c];
        for (int p = 0; p < num_patches; p++)
        {
            // rbf from thin plate spline
            const float phi = kernel(inpx, sources[p]);
            for_each_channel(c) res[c] += patches[p][c] * phi;
        }
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_colorchecker, width, height,
                                           ...
                                           CLARG(height), CLARG(num_patches), CLARG(dev_params));
error:
    free(params);
...
void color_picker_apply(dt_iop_module_t *self, GtkWidget *picker, dt_dev_pixelpipe_t *pipe)
{
    ...
    const dt_aligned_pixel_t picked_mean = {self->picked_color[0], self->picked_color[1],
                                            self->picked_color[2]};
    int best_patch = 0;
    for (int patch = 1; patch < p->num_patches; patch++)
    {
        ...
        if ((self->request_color_pick == DT_REQUEST_COLORPICK_MODULE) &&
            (sqf(picked_mean[0] - Lab[0]) + sqf(picked_mean[1] - Lab[1]) +
...
static void target_a_callback(GtkWidget *slider, dt_iop_module_t *self)
{
    dt_iop_colorchecker_params_t *p = self->params;
    dt_iop_colorchecker_gui_data_t *g = self->gui_data;
    ...
    {
        p->target_a[g->patch] =
            CLAMP(p->source_a[g->patch] + dt_bauhaus_slider_get(slider), -128.0, 128.0);
        const float Cin = sqrtf(p->source_a[g->patch] * p->source_a[g->patch] +
                                p->source_b[g->patch] * p->source_b[g->patch]);
        const float Cout = sqrtf(p->target_a[g->patch] * p->target_a[g->patch] +
                                 p->target_b[g->patch] * p->target_b[g->patch]);
...
static void target_b_callback(GtkWidget *slider, dt_iop_module_t *self)
{
    ...
    {
        p->target_b[g->patch] =
            CLAMP(p->source_b[g->patch] + dt_bauhaus_slider_get(slider), -128.0, 128.0);
        const float Cin = sqrtf(p->source_a[g->patch] * p->source_a[g->patch] +
                                p->source_b[g->patch] * p->source_b[g->patch]);
        const float Cout = sqrtf(p->target_a[g->patch] * p->target_a[g->patch] +
                                 p->target_b[g->patch] * p->target_b[g->patch]);
...
static void target_C_callback(GtkWidget *slider, dt_iop_module_t *self)
{
    ...
    const float Cin = sqrtf(p->source_a[g->patch] * p->source_a[g->patch] +
                            p->source_b[g->patch] * p->source_b[g->patch]);
    const float Cout = MAX(1e-4f, sqrtf(p->target_a[g->patch] * p->target_a[g->patch] +
                                        p->target_b[g->patch] * p->target_b[g->patch]));
...
static void target_callback(GtkWidget *combo, dt_iop_module_t *self)
{
    dt_iop_colorchecker_gui_data_t *g = self->gui_data;
...
static gboolean checker_draw(GtkWidget *widget, cairo_t *crf, dt_iop_module_t *self)
{
    dt_iop_colorchecker_params_t *p = self->params;
    dt_iop_colorchecker_gui_data_t *g = self->gui_data;
    ...
    for (int j = 0; j < cells_y; j++)
    {
        for (int i = 0; i < cells_x; i++)
        {
            ...
            if (patch >= p->num_patches)
                continue;

            const dt_aligned_pixel_t Lab = {p->source_L[patch], p->source_a[patch],
                                            p->source_b[patch]};
    ...
    if (g->drawn_patch != -1)
    {
        ...
        float color = 1.0;
        if (p->source_L[g->drawn_patch] > 80)
            color = 0.0;
        cairo_set_line_width(cr, DT_PIXEL_APPLY_DPI(2.));
        cairo_set_source_rgb(cr, color, color, color);
        cairo_rectangle(cr, width * draw_i / (float)cells_x + DT_PIXEL_APPLY_DPI(5),
                                     ...
                                     dt_iop_module_t *self)
{
             ...
             (self->request_color_pick == DT_REQUEST_COLORPICK_MODULE))
    {
        ...
        for (int i = 0; i < p->num_patches; ++i)
        {
            ...
            if (fabsf(self->picked_color[0] - color[0]) < 1.e-3f &&
                fabsf(self->picked_color[1] - color[1]) < 1.e-3f &&
...
void gui_init(dt_iop_module_t *self)
{
    ...
    g_signal_connect(G_OBJECT(g->area), "button-press-event", G_CALLBACK(checker_button_press),
                     self);
    ...
    gtk_widget_set_tooltip_text(g->scale_L,
    ...
    gtk_widget_set_tooltip_text(g->scale_C,
    ...
    gtk_widget_set_tooltip_text(
        g->combobox_target, _("control target color of the patches\n"
    ...
    self->widget = dt_gui_vbox(g->area, g->combobox_patch, g->scale_L, g->scale_a, g->scale_b,
                               g->scale_C, g->combobox_target);

    g_signal_connect(G_OBJECT(g->combobox_patch), "value-changed", G_CALLBACK(patch_callback),
                     self);
    ...
    g_signal_connect(G_OBJECT(g->combobox_target), "value-changed", G_CALLBACK(target_callback),
                     self);
}
