...
static const float _aspect_ratios[] = {DT_IOP_BORDERS_ASPECT_IMAGE_VALUE,
                                       3.0f,
                                       95.0f / 33.0f,
                                       2.39f,
                                       2.0f,
...
typedef struct dt_iop_borders_params_t
{
    ...
    char pos_h_text
        [20]; /* UNUSED picture horizontal position ratio into the final image (user string version)
                               DEFAULT: "1/2" */
    float pos_v; /* picture vertical position ratio into the final image
                               $MIN: 0.0 $MAX: 1.0 $DEFAULT: 0.5 $DESCRIPTION: "vertical offset"*/
    char pos_v_text
        [20]; /* UNUSED picture vertical position ratio into the final image (user string version)
                               DEFAULT: "1/2" */
...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self, _("add solid borders or margins around the image"),
                                  _("creative"), _("linear or non-linear, RGB, display-referred"),
                                  _("geometric, RGB"),
                  ...
                  const dt_iop_roi_t *const roi_out)
{
    ...
    dt_iop_setup_binfo(piece, roi_in, roi_out, d->pos_v, d->pos_h, d->color, d->frame_color,
                       d->frame_size, d->frame_offset, &binfo);
                    ...
                    const dt_iop_roi_t *roi_in)
{
    *roi_out = *roi_in;
    ...
    if (basis == DT_IOP_BORDERS_BASIS_AUTO)
    {
        ...
        basis = is_constant_border ? DT_IOP_BORDERS_BASIS_LONGER : DT_IOP_BORDERS_BASIS_WIDTH;
    }
    if (basis == DT_IOP_BORDERS_BASIS_LONGER)
    {
        basis = roi_in->width > roi_in->height ? DT_IOP_BORDERS_BASIS_WIDTH :
                                                 DT_IOP_BORDERS_BASIS_HEIGHT;
    }
...
#define DT_IOP_BORDERS_ASSIGN(b, b_uc, o)                                                          \
    basis_in = &roi_in->b, basis_out = &roi_out->b, other_in = &roi_in->o,                         \
    other_out = &roi_out->o, basis = DT_IOP_BORDERS_BASIS_##b_uc
...
#define DT_IOP_BORDERS_ASSIGN_BASIS(basis) DT_IOP_BORDERS_ASSIGN_##basis
    ...
    else
    {
        ...
        if (d->aspect_orient == DT_IOP_BORDERS_ASPECT_ORIENTATION_AUTO)
            aspect =
                ((image_aspect < 1.0f && aspect > 1.0f) || (image_aspect > 1.0f && aspect < 1.0f)) ?
                    1.0f / aspect :
                    aspect;
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_borders_fill, width, height,
                                           CLARG(dev_out), CLARGINT(0), CLARGINT(0), CLARG(width),
                                           CLARG(height), CLARG(col));
    if (err != CL_SUCCESS)
        goto error;
    ...
    if (binfo.frame_size != 0)
    {
        ...
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->kernel_borders_fill, width, height, CLARG(dev_out),
            CLARG(binfo.frame_tl_out_x), CLARG(binfo.frame_tl_out_y), CLARG(roi_frame_out_width),
            CLARG(roi_frame_out_height), CLARG(col_frame));
        if (err != CL_SUCCESS)
            goto error;

        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->kernel_borders_fill, width, height, CLARG(dev_out),
            CLARG(binfo.frame_tl_in_x), CLARG(binfo.frame_tl_in_y), CLARG(roi_frame_in_width),
            CLARG(roi_frame_in_height), CLARG(col));
        if (err != CL_SUCCESS)
            goto error;
    }
...
void init_presets(dt_iop_module_so_t *self)
{
    ...
    dt_gui_presets_add_generic(_("15:10 postcard white"), self->op, self->version(), &p, sizeof(p),
                               TRUE, DEVELOP_BLEND_CS_NONE);

    p.color[0] = p.color[1] = p.color[2] = 0.0f;
    p.frame_color[0] = p.frame_color[1] = p.frame_color[2] = 1.0f;
    dt_gui_presets_add_generic(_("15:10 postcard black"), self->op, self->version(), &p, sizeof(p),
                               TRUE, DEVELOP_BLEND_CS_NONE);
}
...
static void _aspect_changed(GtkWidget *combo, dt_iop_module_t *self)
{
    dt_iop_borders_gui_data_t *g = self->gui_data;
    dt_iop_borders_params_t *p = self->params;
...
static void _position_h_changed(GtkWidget *combo, dt_iop_module_t *self)
{
    dt_iop_borders_gui_data_t *g = self->gui_data;
    dt_iop_borders_params_t *p = self->params;
    const int which = dt_bauhaus_combobox_get(combo);
...
static void _position_v_changed(GtkWidget *combo, dt_iop_module_t *self)
{
    dt_iop_borders_gui_data_t *g = self->gui_data;
    dt_iop_borders_params_t *p = self->params;
    const int which = dt_bauhaus_combobox_get(combo);
...
void gui_update(dt_iop_module_t *self)
{
    ...
    // ----- Border Color
    GdkRGBA c =
        (GdkRGBA){.red = p->color[0], .green = p->color[1], .blue = p->color[2], .alpha = 1.0};
...
void gui_init(dt_iop_module_t *self)
{
    ...
    gtk_widget_set_tooltip_text(g->basis, _("which dimension to use for the size calculation"));
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(
        g->aspect, self, NULL, N_("aspect"),
        _("select the aspect ratio\n"
          "(right-click on slider below to type your own w:h)"),
        0, _aspect_changed, self, N_("image"), N_("3:1"), N_("95:33"), N_("CinemaScope 2.39:1"),
        N_("2:1"), N_("16:9"), N_("5:3"), N_("US Legal 8.5x14"), N_("golden cut"), N_("16:10"),
        N_("3:2 (4x6, 10x15cm)"), N_("A4"), N_("DIN"), N_("7:5"), N_("4:3"), N_("US Letter 8.5x11"),
        N_("14:11"), N_("5:4 (8x10)"), N_("square"), N_("constant border"), N_("custom..."));
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(g->pos_h, self, NULL, N_("horizontal position"),
                                 ...
                                 0, _position_h_changed, self, N_("center"), N_("1/3"), N_("3/8"),
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(g->pos_v, self, NULL, N_("vertical position"),
                                 ...
                                 0, _position_v_changed, self, N_("center"), N_("1/3"), N_("3/8"),
                                 N_("5/8"), N_("2/3"), N_("custom..."));
    ...
    GdkRGBA color =
        (GdkRGBA){.red = dp->color[0], .green = dp->color[1], .blue = dp->color[2], .alpha = 1.0};
    ...
    dt_action_define_iop(self, N_("pickers"), N_("border color"), g->border_picker,
    ...
    dt_action_define_iop(self, N_("pickers"), N_("frame line color"), g->frame_picker,
                         &dt_action_def_toggle);
    dt_gui_box_add(self->widget,
                   dt_gui_hbox(dt_gui_expand(label), g->frame_colorpick, g->frame_picker));
}
