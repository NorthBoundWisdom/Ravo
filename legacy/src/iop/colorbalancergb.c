...
typedef struct dt_iop_colorbalancergb_params_t
{
    ...
    float
        highlights_weight; // $MIN:  0.0 $MAX:   3.0 $DEFAULT: 1.0 $DESCRIPTION: "highlights fall-off"
    ...
    float
        saturation_global; // $MIN: -1.0 $MAX:   1.0 $DEFAULT: 0.0 $DESCRIPTION: "global saturation"
    ...
    /* params of v2 */
    float
        brilliance_global; // $MIN: -1.0 $MAX:   1.0 $DEFAULT: 0.0 $DESCRIPTION: "global brilliance"
    ...
    /* params of v3 */
    float
        mask_grey_fulcrum; // $MIN: 0.0 $MAX: 1.0 $DEFAULT: 0.1845 $DESCRIPTION: "mask middle-gray fulcrum"

    /* params of v4 */
    float vibrance; // $MIN: -1.0 $MAX: 1.0 $DEFAULT: 0 $DESCRIPTION: "global vibrance"
    float
        grey_fulcrum; // $MIN:  0.0 $MAX: 1.0 $DEFAULT: 0.1845 $DESCRIPTION: "contrast gray fulcrum"
...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self,
                                  _("color grading tools using alpha masks to separate\n"
                                    "shadows, mid-tones and highlights"),
                                  _("corrective or creative"), _("linear, RGB, scene-referred"),
                                  _("non-linear, RGB"), _("non-linear, RGB, scene-referred"));
}
...
void init_presets(dt_iop_module_so_t *self)
{
    ...
    dt_gui_presets_add_generic(_("basic colorfulness | legacy"), self->op, self->version(), &p,
    ...
    dt_gui_presets_add_generic(_("basic colorfulness | natural skin"), self->op, self->version(),
    ...
    dt_gui_presets_add_generic(_("basic colorfulness | vibrant colors"), self->op, self->version(),
                               &p, sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_SCENE);
    ...
    p.saturation_midtones = 0.f;
    p.saturation_highlights = -0.25f;
    dt_gui_presets_add_generic(_("basic colorfulness | standard"), self->op, self->version(), &p,
                               sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_SCENE);
}

DT_OMP_DECLARE_SIMD(aligned(output, output_comp : 16)
                        uniform(shadows_weight, midtones_weight, highlights_weight))
static inline void opacity_masks(const float x, const float shadows_weight,
                                 const float highlights_weight, const float midtones_weight,
                                 const float mask_grey_fulcrum, dt_aligned_pixel_t output,
                                 dt_aligned_pixel_t output_comp)
{
    ...
    const float beta =
        1.f / (1.f + expf(-x_offset_norm * highlights_weight)); // opacity of highlights
    const float alpha_comp = 1.f - alpha;
    const float beta_comp = 1.f - beta;
    const float gamma = expf(-sqf(x_offset) * midtones_weight / 4.f) * sqf(alpha_comp) *
                        sqf(beta_comp) * 8.f; // opacity of midtones
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
  ...
  */
    ...
    dt_colormatrix_mul(output_matrix, XYZ_D50_to_D65_CAT16,
                       work_profile->matrix_in); // output_matrix used as temp buffer
  ...
  */
    ...
    const float *const restrict gamut_LUT =
        DT_IS_ALIGNED(((const float *const restrict)d->gamut_LUT));

    const float *const restrict global =
        DT_IS_ALIGNED_PIXEL((const float *const restrict)d->global);
    const float *const restrict highlights =
        DT_IS_ALIGNED_PIXEL((const float *const restrict)d->highlights);
    const float *const restrict shadows =
        DT_IS_ALIGNED_PIXEL((const float *const restrict)d->shadows);
    const float *const restrict midtones =
        DT_IS_ALIGNED_PIXEL((const float *const restrict)d->midtones);

    const float *const restrict chroma =
        DT_IS_ALIGNED_PIXEL((const float *const restrict)d->chroma);
    const float *const restrict saturation =
        DT_IS_ALIGNED_PIXEL((const float *const restrict)d->saturation);
    const float *const restrict brilliance =
        DT_IS_ALIGNED_PIXEL((const float *const restrict)d->brilliance);
    ...
    */
        ...
        for_four_channels(
            c, aligned(RGB, opacities, opacities_comp, shadows, midtones, highlights : 16))
        {
            //  highlights, shadows : 2 slopes with masking
            RGB[c] *= opacities_comp[2] * (opacities_comp[0] + opacities[0] * shadows[c]) +
                      opacities[2] * highlights[c];
            // factorization of : (RGB[c] * (1.f - alpha) + RGB[c] * d->shadows[c] * alpha) * (1.f - beta)  + RGB[c] * d->highlights[c] * beta;
        }
        dt_aligned_pixel_t sign;
        for_each_channel(c) sign[c] = (RGB[c] < 0.f) ? -1.f : 1.f;
        dt_aligned_pixel_t abs_RGB;
        for_each_channel(c) abs_RGB[c] = fabsf(RGB[c]);
        dt_aligned_pixel_t scaled_RGB;
        for_each_channel(c) scaled_RGB[c] = abs_RGB[c] / d->white_fulcrum;
        dt_vector_powf(scaled_RGB, midtones, RGB);
        for_each_channel(c) RGB[c] = RGB[c] * sign[c] * d->white_fulcrum;
        ...
        if (d->saturation_formula == DT_COLORBALANCE_SATURATION_JZAZBZ)
        {
            ...
            const float boosts[2] = {
                1.f + d->brilliance_global +
                    scalar_product(opacities, brilliance), // move in S direction
                d->saturation_global +
                    scalar_product(opacities, saturation)}; // move in O direction
        ...
        else
        {
            ...
            const float P =
                MAX(FLT_MIN, HCB[1]); // as HCB[1] is at least zero we don't fiddle with sign
            const float W = sin_T * HCB[1] + cos_T * HCB[2];

            float a = MAX(1.f + d->saturation_global + scalar_product(opacities, saturation), 0.f);
            const float b =
                MAX(1.f + d->brilliance_global + scalar_product(opacities, brilliance), 0.f);
            ...
            const float max_chroma =
                (15.932993652962535f * powf(JCH[0] * L_white, 0.6523997524738018f) *
    ...
    */
        ...
        if (mask_display)
        {
            ...
            for_four_channels(c, aligned(pix_out, color : 16)) pix_out[c] =
                opacity_comp * color[c] + opacity * pix_out[c];
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
  ...
  */
    ...
    float hue_rotation_matrix[4] = {cosf(d->hue_angle), -sinf(d->hue_angle), sinf(d->hue_angle),
                                    cosf(d->hue_angle)};
    hue_rotation_matrix_cl =
        dt_opencl_copy_host_to_device_constant(devid, 4 * sizeof(float), hue_rotation_matrix);

    if (input_matrix_cl == NULL || output_matrix_cl == NULL || gamut_LUT_cl == NULL ||
        hue_rotation_matrix_cl == NULL)
    {
    ...
    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_colorbalance_rgb, width, height, CLARG(dev_in), CLARG(dev_out),
        CLARG(width), CLARG(height), CLARG(dev_profile_info), CLARG(input_matrix_cl),
        CLARG(output_matrix_cl), CLARG(gamut_LUT_cl), CLARG(d->shadows_weight),
        CLARG(d->highlights_weight), CLARG(d->midtones_weight), CLARG(d->mask_grey_fulcrum),
        CLARG(d->hue_angle), CLARG(d->chroma_global), CLARG(d->chroma), CLARG(d->vibrance),
        CLARG(d->global), CLARG(d->shadows), CLARG(d->highlights), CLARG(d->midtones),
        CLARG(d->white_fulcrum), CLARG(d->midtones_Y), CLARG(d->grey_fulcrum), CLARG(d->contrast),
        CLARG(d->brilliance_global), CLARG(d->brilliance), CLARG(d->saturation_global),
        CLARG(d->saturation), CLARG(mask_display), CLARG(mask_type), CLARG(checker_1),
        CLARG(checker_2), CLARG(d->checker_color_1), CLARG(d->checker_color_2), CLARG(L_white),
                   ...
                   dt_dev_pixelpipe_iop_t *piece)
{
    ...
    d->checker_color_1[0] =
        CLAMP(dt_conf_get_float("plugins/darkroom/colorbalancergb/checker1/red"), 0.f, 1.f);
    d->checker_color_1[1] =
        CLAMP(dt_conf_get_float("plugins/darkroom/colorbalancergb/checker1/green"), 0.f, 1.f);
    d->checker_color_1[2] =
        CLAMP(dt_conf_get_float("plugins/darkroom/colorbalancergb/checker1/blue"), 0.f, 1.f);
    d->checker_color_1[3] = 1.f;

    d->checker_color_2[0] =
        CLAMP(dt_conf_get_float("plugins/darkroom/colorbalancergb/checker2/red"), 0.f, 1.f);
    d->checker_color_2[1] =
        CLAMP(dt_conf_get_float("plugins/darkroom/colorbalancergb/checker2/green"), 0.f, 1.f);
    d->checker_color_2[2] =
        CLAMP(dt_conf_get_float("plugins/darkroom/colorbalancergb/checker2/blue"), 0.f, 1.f);
    ...
    d->contrast =
        1.0f + p->contrast; // that limits the user param range to [-1, 1], but it seems enough
    ...
    // global
    {
        ...
        for (size_t c = 0; c < 4; c++)
            d->global[c] = (d->global[c] - RGB_norm[c]) + RGB_norm[c] * p->global_Y;
    }
    ...
    // shadows
    {
        ...
        for (size_t c = 0; c < 4; c++)
            d->shadows[c] = 1.f + (d->shadows[c] - RGB_norm[c]) + p->shadows_Y;
    ...
    // highlights
    {
        ...
        for (size_t c = 0; c < 4; c++)
            d->highlights[c] = 1.f + (d->highlights[c] - RGB_norm[c]) + p->highlights_Y;
    ...
    // midtones
    {
        ...
        for (size_t c = 0; c < 4; c++)
            d->midtones[c] = 1.f / (1.f + (d->midtones[c] - RGB_norm[c]));
    ...
    if (p->saturation_formula != d->saturation_formula)
        d->lut_inited = FALSE;
        ...
        if (p->saturation_formula == DT_COLORBALANCE_SATURATION_JZAZBZ)
        {
            ...
            for (size_t r = 0; r < STEPS; r++)
                for (size_t g = 0; g < STEPS; g++)
                    ...
                    {
                        const dt_aligned_pixel_t rgb = {(float)r / (float)(STEPS - 1),
                        ...
                        dot_product(rgb, input_matrix,
                                    XYZ); // Go from D50 pipeline RGB to D65 XYZ in one step
            ...
            for (size_t k = 2; k < LUT_ELEM - 2; k++)
                d->gamut_LUT[k] = (sampler[k - 2] + sampler[k - 1] + sampler[k] + sampler[k + 1] +
                                   sampler[k + 2]) /
                                  5.f;

            // handle bounds
            d->gamut_LUT[0] = (sampler[LUT_ELEM - 2] + sampler[LUT_ELEM - 1] + sampler[0] +
                               sampler[1] + sampler[2]) /
                              5.f;
            d->gamut_LUT[1] =
                (sampler[LUT_ELEM - 1] + sampler[0] + sampler[1] + sampler[2] + sampler[3]) / 5.f;
            d->gamut_LUT[LUT_ELEM - 1] = (sampler[LUT_ELEM - 3] + sampler[LUT_ELEM - 2] +
                                          sampler[LUT_ELEM - 1] + sampler[0] + sampler[1]) /
                                         5.f;
            d->gamut_LUT[LUT_ELEM - 2] =
                (sampler[LUT_ELEM - 4] + sampler[LUT_ELEM - 3] + sampler[LUT_ELEM - 2] +
                 sampler[LUT_ELEM - 1] + sampler[0]) /
                5.f;
...
void color_picker_apply(dt_iop_module_t *self, GtkWidget *picker, dt_dev_pixelpipe_t *pipe)
{
    ...
    const float hue = (state & GDK_CONTROL_MASK) ? YRG_RAD_TO_CONVENTIONAL_DEG(picked_hue) :
                                                   YRG_RAD_TO_CONVENTIONAL_DEG(picked_hue) +
                                                       180.f; //take the current or opponent color
                      ...
                      const dt_colormatrix_t output_matrix_LMS_to_RGB)
{
    ...
    if (output_profile->nonlinearlut)
        dt_ioppr_apply_trc(RGB_linear, *RGB_out, output_profile->lut_out,
...
static void paint_chroma_slider(const dt_iop_order_iccprofile_info_t *output_profile,
                                const dt_colormatrix_t output_matrix_LMS_to_RGB, GtkWidget *w,
                                const float hue)
{
    const float x_min = 0;
    const float x_max = 1;
    ...
    const float max_chroma =
        Ych_max_chroma_without_negatives(output_matrix_LMS_to_RGB, cos_h, sin_h);
                              ...
                              const dt_iop_colorbalancergb_gui_data_t *const g)
{
    for (int i = 0; i < DT_BAUHAUS_SLIDER_MAX_STOPS; i++)
    {
        ...
        const float max_chroma =
            Ych_max_chroma_without_negatives(output_matrix_LMS_to_RGB, cosf(h), sinf(h));
        dt_aligned_pixel_t RGB;
...
static void mask_callback(GtkWidget *togglebutton, dt_iop_module_t *self)
{
    ...
    if (g->mask_display)
    {
        if (togglebutton == g->shadows_weight)
            g->mask_type = MASK_SHADOWS;
        if (togglebutton == g->mask_grey_fulcrum)
            g->mask_type = MASK_MIDTONES;
        if (togglebutton == g->highlights_weight)
            g->mask_type = MASK_HIGHLIGHTS;
    }
    else
    {
    ...
    dt_bauhaus_widget_set_quad_active(GTK_WIDGET(g->mask_grey_fulcrum),
                                      g->mask_type == MASK_MIDTONES);
    dt_bauhaus_widget_set_quad_active(GTK_WIDGET(g->highlights_weight),
                                      g->mask_type == MASK_HIGHLIGHTS);
...
static gboolean dt_iop_tonecurve_draw(GtkWidget *widget, cairo_t *crf, const dt_iop_module_t *self)
{
    ...
    cairo_surface_t *surface = cairo_image_surface_create_for_data(
        data, CAIRO_FORMAT_ARGB32, (size_t)line_height, (size_t)graph_height, stride);
    ...
    for (size_t k = 0; k < LUT_ELEM; k++)
    {
        ...
        opacity_masks(Y, shadows_weight, highlights_weight, midtones_weight, mask_grey_fulcrum,
                      output, NULL);
        for (size_t c = 0; c < 3; c++)
            LUT[c][k] = output[c];
    }
    ...
    for (size_t c = 0; c < 3; c++)
    {
        const GdkRGBA line_color = {fg_color.red * (1. - (2 - c) / 4.),
                                    ...
                                    fg_color.blue * (1. - (2 - c) / 4.), fg_color.alpha};
...
void gui_changed(dt_iop_module_t *self, GtkWidget *w, void *previous)
{
    ...
    dt_colormatrix_t input_matrix = {
        {0.f}}; // this is actually unused but is generated by below function anyway
...
void gui_update(dt_iop_module_t *self)
{
    ...
    dt_bauhaus_slider_set(g->checker_size,
...
void gui_init(dt_iop_module_t *self)
{
    ...
    gtk_widget_set_tooltip_text(g->hue_angle,
                                _("rotate all hues by an angle, at the same luminance"));
    ...
    gtk_widget_set_tooltip_text(g->vibrance,
                                _("increase colorfulness mostly on low-chroma colors"));
    ...
    gtk_widget_set_tooltip_text(g->chroma_global,
                                _("increase colorfulness at same luminance globally"));
    ...
    gtk_widget_set_tooltip_text(g->chroma_shadows,
                                _("increase colorfulness at same luminance mostly in shadows"));
    ...
    gtk_widget_set_tooltip_text(g->chroma_midtones,
                                _("increase colorfulness at same luminance mostly in mid-tones"));
    ...
    gtk_widget_set_tooltip_text(g->chroma_highlights,
                                _("increase colorfulness at same luminance mostly in highlights"));

    dt_gui_box_add(self->widget,
                   dt_ui_section_label_new(C_("section", "perceptual saturation grading")));
    ...
    gtk_widget_set_tooltip_text(g->saturation_global,
                                _("add or remove saturation by an absolute amount"));
    ...
    gtk_widget_set_tooltip_text(
        g->saturation_shadows,
        _("increase or decrease saturation proportionally to the original pixel saturation"));
    ...
    gtk_widget_set_tooltip_text(
        g->saturation_highlights,
        _("increase or decrease saturation proportionally to the original pixel saturation"));

    dt_gui_box_add(self->widget,
                   dt_ui_section_label_new(C_("section", "perceptual brilliance grading")));
    ...
    gtk_widget_set_tooltip_text(g->brilliance_global,
                                _("add or remove brilliance by an absolute amount"));
    ...
    gtk_widget_set_tooltip_text(
        g->brilliance_shadows,
        _("increase or decrease brilliance proportionally to the original pixel brilliance"));
    ...
    g->global_H = dt_color_picker_new(self, DT_COLOR_PICKER_AREA,
                                      dt_bauhaus_slider_from_params(sect, "global_H"));
    ...
    g->shadows_H = dt_color_picker_new(self, DT_COLOR_PICKER_AREA,
                                       dt_bauhaus_slider_from_params(sect, "shadows_H"));
    ...
    g->highlights_H = dt_color_picker_new(self, DT_COLOR_PICKER_AREA,
                                          dt_bauhaus_slider_from_params(sect, "highlights_H"));
    ...
    g->midtones_H = dt_color_picker_new(self, DT_COLOR_PICKER_AREA,
                                        dt_bauhaus_slider_from_params(sect, "midtones_H"));
    ...
    gtk_widget_set_tooltip_text(
        g->saturation_formula, _("choose in which uniform color space the saturation is computed"));

    dt_gui_box_add(self->widget, dt_ui_section_label_new(C_("section", "luminance ranges")));

    g->area = GTK_DRAWING_AREA(
        dt_ui_resize_wrap(NULL, 0, "plugins/darkroom/colorbalancergb/graphheight"));
    ...
    gtk_widget_set_tooltip_text(g->shadows_weight,
                                _("weight of the shadows over the whole tonal range"));
    dt_bauhaus_widget_set_quad(
        g->shadows_weight, self, dtgtk_cairo_paint_showmask, TRUE, mask_callback,
    ...
    gtk_widget_set_tooltip_text(g->mask_grey_fulcrum,
                                _("position of the middle-gray reference for masking"));
    dt_bauhaus_widget_set_quad(
        g->mask_grey_fulcrum, self, dtgtk_cairo_paint_showmask, TRUE, mask_callback,
    ...
    gtk_widget_set_tooltip_text(g->highlights_weight,
                                _("weights of highlights over the whole tonal range"));
    dt_bauhaus_widget_set_quad(
        g->highlights_weight, self, dtgtk_cairo_paint_showmask, TRUE, mask_callback,
    ...
    g->white_fulcrum = dt_color_picker_new(self, DT_COLOR_PICKER_AREA,
                                           dt_bauhaus_slider_from_params(self, "white_fulcrum"));
    dt_bauhaus_slider_set_soft_range(g->white_fulcrum, -2., +2.);
    dt_bauhaus_slider_set_format(g->white_fulcrum, _(" EV"));
    gtk_widget_set_tooltip_text(
        g->white_fulcrum, _("peak white luminance value used to normalize the power function"));

    g->grey_fulcrum = dt_color_picker_new(self, DT_COLOR_PICKER_AREA,
                                          dt_bauhaus_slider_from_params(self, "grey_fulcrum"));
    ...
    gtk_widget_set_tooltip_text(
        g->grey_fulcrum, _("peak gray luminance value used to normalize the power function"));
    ...
    gtk_color_button_set_title(GTK_COLOR_BUTTON(g->checker_color_1_picker),
                               _("select color of the checkerboard from a swatch"));
    g_signal_connect(G_OBJECT(g->checker_color_1_picker), "color-set",
                     G_CALLBACK(checker_1_picker_callback), self);
    ...
    gtk_color_button_set_title(GTK_COLOR_BUTTON(g->checker_color_2_picker),
                               _("select color of the checkerboard from a swatch"));
    g_signal_connect(G_OBJECT(g->checker_color_2_picker), "color-set",
                     G_CALLBACK(checker_2_picker_callback), self);
    ...
    g_signal_connect(G_OBJECT(g->checker_size), "value-changed", G_CALLBACK(checker_size_callback),
                     self);

    dt_gui_box_add(self->widget,
                   dt_gui_hbox(dt_gui_expand(dt_ui_label_new(_("checkerboard color 1"))),
                               g->checker_color_1_picker),
                   dt_gui_hbox(dt_gui_expand(dt_ui_label_new(_("checkerboard color 2"))),
