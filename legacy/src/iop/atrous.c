...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self, _("add or remove local contrast, sharpness, acutance"),
                                  _("corrective and creative"), _("linear, Lab, scene-referred"),
                                  _("frequential, RGB"), _("linear, Lab, scene-referred"));
}
...
static int get_scales(float (*thrs)[4], float (*boost)[4], float *sharp,
                      const dt_iop_atrous_data_t *const d, const dt_iop_roi_t *roi_in,
                      const dt_dev_pixelpipe_iop_t *const piece)
{
                             ...
                             const dt_iop_roi_t *const roi_out)
{
    ...
    // now do the wavelet decomposition, immediately synthesizing the
    // detail scale into the final output so that we don't need to store
    // it past the current scale's iteration
    for (int scale = 0; scale < max_scale; scale++)
    {
        eaw_decompose_and_synthesize(buf2, buf1, out, scale, sharp[scale], thrs[scale],
                                     boost[scale], width, height);
        if (scale == 0)
            buf1 = (float *)tmp2; // now switch to second
                                  // scratch for buffer
                                  // ping-pong between buf1 and
                                  // buf2
        float *buf3 = buf2;
        buf2 = buf1;
        buf1 = buf3;
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
  ...
  */
    ...
    if (!dev_detail || !dev_tmp || !dev_tmp2 || !dev_filter)
        goto error;

    // clear dev_out to zeros, as we will be incrementally accumulating results there
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_zero, width, height, CLARG(dev_out),
  ...
  */
    for (int s = 0; s < max_scale; s++)
    {
        ...
        err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_decompose, width, height,
                                               ...
                                               CLARG(width), CLARG(height), CLARG(scale),
                                               CLARG(sharp[s]), CLARG(dev_filter));
        if (err != CL_SUCCESS)
            goto error;
        ...
        err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_synthesize, width, height,
                                               ...
                                               CLARG(width), CLARG(height), CLARG(thrs[scale]),
                                               CLARG(boost[scale]));
        if (err != CL_SUCCESS)
            goto error;
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
   ...
   * in dev_tmp or dev_out) */
    for (int s = 0; s < max_scale; s++)
    {
        ...
        if (s & 1)
            err = dt_opencl_enqueue_kernel_2d_args(
                devid, gd->kernel_decompose, width, height, CLARG(dev_tmp), CLARG(dev_out),
                CLARG(dev_detail[s]), CLARG(width), CLARG(height), CLARG(scale), CLARG(sharp[s]),
                CLARG(dev_filter));
        else
            err = dt_opencl_enqueue_kernel_2d_args(
                devid, gd->kernel_decompose, width, height, CLARG(dev_out), CLARG(dev_tmp),
                CLARG(dev_detail[s]), CLARG(width), CLARG(height), CLARG(scale), CLARG(sharp[s]),
                CLARG(dev_filter));
        if (err != CL_SUCCESS)
            goto error;
    ...
    for (int scale = max_scale - 1; scale >= 0; scale--)
    {
        if (scale & 1)
            err = dt_opencl_enqueue_kernel_2d_args(
                devid, gd->kernel_synthesize, width, height, CLARG(dev_tmp), CLARG(dev_out),
                CLARG(dev_detail[scale]), CLARG(width), CLARG(height), CLARG(thrs[scale]),
                CLARG(boost[scale]));

        else
            err = dt_opencl_enqueue_kernel_2d_args(
                devid, gd->kernel_synthesize, width, height, CLARG(dev_out), CLARG(dev_tmp),
                CLARG(dev_detail[scale]), CLARG(width), CLARG(height), CLARG(thrs[scale]),
                CLARG(boost[scale]));
        if (err != CL_SUCCESS)
            goto error;
...
static inline void _apply_mix(dt_iop_module_t *self, const int ch, const int k, const float mix,
                   ...
                   dt_dev_pixelpipe_iop_t *piece)
{
    ...
    int l = 0;
    for (int k = (int)MIN(pipe->iwidth * pipe->iscale, pipe->iheight * pipe->iscale); k; k >>= 1)
        l++;
...
void init_presets(dt_iop_module_so_t *self)
{
    ...
    dt_gui_presets_add_generic(_("coarse"), self->op, self->version(), &p, sizeof(p), TRUE,
    ...
    dt_gui_presets_add_generic(_("denoise & sharpen"), self->op, self->version(), &p, sizeof(p),
    ...
    dt_gui_presets_add_generic(_("sharpen"), self->op, self->version(), &p, sizeof(p), TRUE,
                               DEVELOP_BLEND_CS_RGB_DISPLAY);
    for (int k = 0; k < BANDS; k++)
    {
        p.x[atrous_L][k] = k / (BANDS - 1.0);
    ...
    dt_gui_presets_add_generic(_("denoise chroma"), self->op, self->version(), &p, sizeof(p), TRUE,
                               DEVELOP_BLEND_CS_RGB_DISPLAY);
    for (int k = 0; k < BANDS; k++)
    {
        p.x[atrous_L][k] = k / (BANDS - 1.0);
    ...
    dt_gui_presets_add_generic(_("denoise"), self->op, self->version(), &p, sizeof(p), TRUE,
                               DEVELOP_BLEND_CS_RGB_DISPLAY);
    for (int k = 0; k < BANDS; k++)
    {
        p.x[atrous_L][k] = k / (BANDS - 1.0);
    ...
    p.y[atrous_L][0] = .5f;
    dt_gui_presets_add_generic(_("bloom"), self->op, self->version(), &p, sizeof(p), TRUE,
                               DEVELOP_BLEND_CS_RGB_DISPLAY);
    for (int k = 0; k < BANDS; k++)
    {
        p.x[atrous_L][k] = k / (BANDS - 1.0);
    ...
    dt_gui_presets_add_generic(_("deblur | large blur | strength 3"), self->op, self->version(), &p,
    ...
    dt_gui_presets_add_generic(_("deblur | medium blur | strength 3"), self->op, self->version(),
                               &p, sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    dt_gui_presets_add_generic(_("deblur | fine blur | strength 3"), self->op, self->version(), &p,
                               sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    dt_gui_presets_add_generic(_("deblur | large blur | strength 2"), self->op, self->version(), &p,
                               sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    dt_gui_presets_add_generic(_("deblur | medium blur | strength 2"), self->op, self->version(),
                               &p, sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    dt_gui_presets_add_generic(_("deblur | fine blur | strength 2"), self->op, self->version(), &p,
                               sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    dt_gui_presets_add_generic(_("deblur | large blur | strength 1"), self->op, self->version(), &p,
                               sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    dt_gui_presets_add_generic(_("deblur | medium blur | strength 1"), self->op, self->version(),
                               &p, sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    dt_gui_presets_add_generic(_("deblur | fine blur | strength 1"), self->op, self->version(), &p,
                               sizeof(p), TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
...
static gboolean area_enter_leave_notify(GtkWidget *widget, GdkEventCrossing *event,
...
static void get_params(dt_iop_atrous_params_t *p, const int ch, const double mouse_x,
...
static gboolean area_draw(GtkWidget *widget, cairo_t *crf, dt_iop_module_t *self)
{
    dt_iop_atrous_gui_data_t *g = self->gui_data;
    ...
    gboolean color_found =
        gtk_style_context_lookup_color(context, "graph_overlay", &bright_bg_color);
    if (!color_found)
    {
    ...
    if (g->mouse_y > 0 || g->dragging)
    {
        ...
        dt_draw_curve_calc_values(g->minmax_curve, 0.0, 1.0, RES, g->draw_min_xs, g->draw_min_ys);

        p = *(dt_iop_atrous_params_t *)self->params;
        get_params(&p, ch2, g->mouse_x, .0, g->mouse_radius);
        for (int k = 0; k < BANDS; k++)
            dt_draw_curve_set_point(g->minmax_curve, k, p.x[ch2][k], p.y[ch2][k]);
        dt_draw_curve_calc_values(g->minmax_curve, 0.0, 1.0, RES, g->draw_max_xs, g->draw_max_ys);
    }
...
// draw frequency histogram in bg.
    ...
    {
        cairo_save(cr);
        cairo_scale(cr, width / (BANDS - 1.0), -(height - DT_PIXEL_APPLY_DPI(5)) / g->band_max);
        cairo_set_source_rgba(cr, graph_bg.red, graph_bg.green, graph_bg.blue, .3);
        cairo_move_to(cr, 0, 0);
        for (int k = 0; k < BANDS; k++)
            cairo_line_to(cr, k, g->band_hist[k]);
...
static gboolean area_motion_notify(GtkWidget *widget, GdkEventMotion *event, dt_iop_module_t *self)
{
    ...
    int ch2 = g->channel;
    if (g->channel == atrous_L)
        ch2 = atrous_Lt;
    if (g->channel == atrous_c)
        ch2 = atrous_ct;
...
static gboolean area_button_press(GtkWidget *widget, GdkEventButton *event, dt_iop_module_t *self)
{
    ...
    else if (event->button == GDK_BUTTON_PRIMARY)
    {
        ...
        const int height = allocation.height - 2 * inset - DT_RESIZE_HANDLE_SIZE;
        const int width = allocation.width - 2 * inset;
        g->mouse_pick = dt_draw_curve_calc_value(g->minmax_curve,
...
static gboolean area_button_release(GtkWidget *widget, GdkEventButton *event, dt_iop_module_t *self)
{
...
static gboolean area_scrolled(GtkWidget *widget, GdkEventScroll *event, dt_iop_module_t *self)
{
    dt_iop_atrous_gui_data_t *g = self->gui_data;
...
static void tab_switch(GtkNotebook *notebook, GtkWidget *page, const guint page_num,
                       dt_iop_module_t *self)
{
    dt_iop_atrous_gui_data_t *g = self->gui_data;
    DT_GUARD_GUI_UPDATE();
...
static void mix_callback(GtkWidget *slider, dt_iop_module_t *self)
{
    DT_GUARD_GUI_UPDATE();
    dt_iop_atrous_params_t *p = self->params;
    dt_iop_atrous_gui_data_t *g = self->gui_data;
...
const gchar *dt_action_effect_equalizer[] = {N_("reset"), N_("boost"), N_("reduce"), N_("raise"),
                                             N_("lower"), N_("right"), N_("left"),   NULL};
...
static float _action_process_equalizer(gpointer target, const dt_action_element_t element,
                                       const dt_action_effect_t effect, float move_size)
{
    ...
    const int ch2 = ch1 == atrous_L ? atrous_Lt : ch1 == atrous_c ? atrous_ct : ch1;
    ...
    if (DT_PERFORM_ACTION(move_size))
    {
        ...
        if (element)
        {
            ...
            case DT_ACTION_EFFECT_ATROUS_RESET:
                ...
                toast =
                    g_strdup_printf("%s, %s", _action_elements_equalizer[element].name, "reset");
                break;
            case DT_ACTION_EFFECT_REDUCE:
                move_size *= -1;
            case DT_ACTION_EFFECT_BOOST:
                get_params(p, ch1, p->x[ch1][node], p->y[ch1][node] + move_size / 100,
                           g->mouse_radius);

                toast = g_strdup_printf("%s, %s %+.2f", _action_elements_equalizer[element].name,
                                        ch1 == atrous_s ? _("sharpness") : _("boost"),
                                        p->y[ch1][node] * 2. - 1.);
                break;
            case DT_ACTION_EFFECT_LOWER:
                move_size *= -1;
            case DT_ACTION_EFFECT_RAISE:
                get_params(p, ch2, p->x[ch2][node], p->y[ch2][node] + move_size / 100,
                           g->mouse_radius);
            ...
            dt_dev_add_history_item_target(darktable.develop, self, TRUE, (char *)target + ch1);
        }
        else // radius
        {
            float bottop = -1e6;
            ...
            case DT_ACTION_EFFECT_UP:
                g->mouse_radius =
                    CLAMP(g->mouse_radius * (1.0 + 0.1 * move_size), 0.25 / BANDS, 1.0);
                break;
            default:
                dt_print(DT_DEBUG_ALWAYS,
                         "[_action_process_equalizer] unknown shortcut effect (%d)"
            ...
            toast = g_strdup_printf("%s %+.2f", _action_elements_equalizer[element].name,
                                    g->mouse_radius);
        }
    ...
    return element ?
               effect >= DT_ACTION_EFFECT_RIGHT ? p->x[ch1][node] :
...
const dt_action_def_t _action_def_equalizer = {N_("contrast equalizer"), _action_process_equalizer,
...
void gui_init(dt_iop_module_t *self)
{
    ...
    dt_ui_notebook_page(g->channel_tabs, N_("luma"), _("change lightness at each feature size"));
    dt_ui_notebook_page(g->channel_tabs, N_("chroma"),
                        _("change color saturation at each feature size"));
    dt_ui_notebook_page(g->channel_tabs, N_("edges"),
    ...
    // graph
    g->area = GTK_DRAWING_AREA(dt_ui_resize_wrap(NULL, 0, "plugins/darkroom/atrous/graphheight"));

    g_object_set_data(G_OBJECT(g->area), "iop-instance", self);
    dt_action_define_iop(self, NULL, N_("graph"), GTK_WIDGET(g->area), &_action_def_equalizer);
    ...
    g_signal_connect(G_OBJECT(g->area), "enter-notify-event", G_CALLBACK(area_enter_leave_notify),
                     self);
    g_signal_connect(G_OBJECT(g->area), "scroll-event", G_CALLBACK(area_scrolled), self);
