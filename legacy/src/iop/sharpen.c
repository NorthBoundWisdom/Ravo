...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(
        self, _("sharpen the details in the image using a standard UnSharp Mask (USM)"),
        _("corrective"), _("linear or non-linear, Lab, display or scene-referred"),
        _("frequential, Lab"), _("quasi-linear, Lab, display or scene-referred"));
}
...
void init_presets(dt_iop_module_so_t *self)
{
    ...
    dt_gui_presets_add_generic(_("sharpen"), self->op, self->version(), &tmp,
                               sizeof(dt_iop_sharpen_params_t), 1, DEVELOP_BLEND_CS_RGB_DISPLAY);
    // restrict to raw images
    dt_gui_presets_update_format(BUILTIN_PRESET("sharpen"), self->op, self->version(), FOR_RAW);
}
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    err = dt_opencl_enqueue_kernel_2d_local_args(
        devid, gd->kernel_sharpen_hblur, sizes, local, CLARG(dev_in), CLARG(dev_out), CLARG(dev_m),
        CLARG(rad), CLARG(width), CLARG(height), CLARG(hblocksize),
        CLLOCAL((hblocksize + 2 * rad) * sizeof(float)));
    if (err != CL_SUCCESS)
        goto error;
    ...
    local[1] = vblocksize;
    err = dt_opencl_enqueue_kernel_2d_local_args(
        devid, gd->kernel_sharpen_vblur, sizes, local, CLARG(dev_out), CLARG(dev_tmp), CLARG(dev_m),
        CLARG(rad), CLARG(width), CLARG(height), CLARG(vblocksize),
        CLLOCAL((vblocksize + 2 * rad) * sizeof(float)));
    if (err != CL_SUCCESS)
        goto error;

    /* mixing tmp and in -> out */
    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_sharpen_mix, width, height, CLARG(dev_in), CLARG(dev_tmp), CLARG(dev_out),
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    if (rad == 0 || (roi_out->width < 2 * rad + 1 || roi_out->height < 2 * rad + 1))
    {
    ...
    size_t padded_size;
    if (!dt_iop_alloc_image_buffers(self, roi_in, roi_out, 1 | DT_IMGSZ_WIDTH | DT_IMGSZ_PERTHREAD,
                                    &tmp, &padded_size, 0))
    {
    ...
    for (int j = 0; j < roi_out->height; j++)
    {
        ...
        for (size_t i = 0; i < end_bulk; i += 4)
        {
            ...
            for (size_t k = start_row; k <= end_row; k++)
            {
                ...
                for_four_channels(c, aligned(in)) sum[c] +=
                    mat[k_adj] * in[4 * (k * width + i + c)];
            }
            float *const vblurred = temp_buf + i;
            for_four_channels(c, aligned(vblurred)) vblurred[c] = sum[c];
        }
        ...
        for (int i = rad; i < roi_out->width - rad; i++)
        {
            ...
            const float detail =
                (absdiff > threshold) ? copysignf(MAX(absdiff - threshold, 0.0f), diff) : 0.0f;
