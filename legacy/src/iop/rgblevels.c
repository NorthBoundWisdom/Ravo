...
{
    dt_iop_rgblevels_autoscale_t
        autoscale; // $DEFAULT: DT_IOP_RGBLEVELS_LINKED_CHANNELS $DESCRIPTION: "mode" (DT_IOP_RGBLEVELS_INDEPENDENT_CHANNELS, DT_IOP_RGBLEVELS_LINKED_CHANNELS)
    dt_iop_rgb_norms_t
...
typedef struct dt_iop_rgblevels_gui_data_t
{
    ...
    GtkWidget
        *cmb_autoscale; // (DT_IOP_RGBLEVELS_INDEPENDENT_CHANNELS, DT_IOP_RGBLEVELS_LINKED_CHANNELS)
    GtkDrawingArea *area;
...
const char **description(dt_iop_module_t *self)
{
    return dt_iop_set_description(self,
                                  _("adjust black, white and mid-gray points in RGB color space"),
...
{
    dt_iop_rgblevels_gui_data_t *g = self->gui_data;
    if (g == NULL || !self->enabled)
        return;
    if (!g->draw_selected_region || !g->button_down)
        return;
    if (g->posx_from == g->posx_to && g->posy_from == g->posy_to)
        return;
    ...
    cairo_rectangle(cr, posx_from + 1.0 / zoom_scale, posy_from,
                    (posx_to - posx_from) - 3. / zoom_scale,
...
static void _rgblevels_move_handle(dt_iop_module_t *self, const int handle_move,
                                   const float new_pos, float *levels,
                                   const float drag_start_percentage)
{
    ...
    if (handle_move != 1)
        levels[1] = levels[0] + (drag_start_percentage * (levels[2] - levels[0]));
                                             ...
                                             dt_iop_module_t *self)
{
    ...
    int height = allocation.height - 2 * inset - DT_RESIZE_HANDLE_SIZE,
        width = allocation.width - 2 * inset;
    if (!g->dragging)
    {
        g->mouse_x = CLAMP(event->x - inset, 0, width);
        g->drag_start_percentage = (p->levels[g->channel][1] - p->levels[g->channel][0]) /
                                   (p->levels[g->channel][2] - p->levels[g->channel][0]);
    }
    ...
    if (g->dragging)
    {
        if (g->handle_move >= 0 && g->handle_move < 3)
        {
            ...
            _rgblevels_move_handle(self, g->handle_move, mx, p->levels[g->channel],
                                   g->drag_start_percentage);
        }
    }
    else
    {
...
static gboolean _area_scroll_callback(GtkWidget *widget, GdkEventScroll *event,
                                      dt_iop_module_t *self)
{
    dt_iop_rgblevels_gui_data_t *g = self->gui_data;
    dt_iop_rgblevels_params_t *p = self->params;
    ...
    if (dt_gui_get_scroll_unit_delta(event, &delta_y))
    {
        ...
        _rgblevels_move_handle(self, g->handle_move, new_position, p->levels[g->channel],
                               g->drag_start_percentage);
        return TRUE;
    }
...
static void _tab_switch_callback(GtkNotebook *notebook, GtkWidget *page, const guint page_num,
                                 dt_iop_module_t *self)
{
    DT_GUARD_GUI_UPDATE();
    dt_iop_rgblevels_gui_data_t *g = self->gui_data;
...
const dt_action_element_def_t _action_elements_levels[] = {{N_("black"), dt_action_effect_value},
...
{
    if (element >= DT_IOP_RGBLEVELS_MAX_CHANNELS)
    ...
    if (DT_PERFORM_ACTION(move_size))
    {
        ...
        case DT_ACTION_EFFECT_UP:
            ...
            _rgblevels_move_handle(self, element, new_position, p->levels[g->channel],
                                   g->drag_start_percentage);
        default:
        ...
        gchar *text = g_strdup_printf("%s %.2f", _action_elements_levels[element].name,
...
const dt_action_def_t _action_def_levels = {N_("levels"), _action_process, _action_elements_levels};
...
void gui_init(dt_iop_module_t *self)
{
    ...
    g_signal_connect(G_OBJECT(g->channel_tabs), "switch_page", G_CALLBACK(_tab_switch_callback),
                     self);

    g->area =
        GTK_DRAWING_AREA(dt_ui_resize_wrap(NULL, 0, "plugins/darkroom/rgblevels/graphheight"));
    ...
    g_signal_connect(G_OBJECT(g->area), "scroll-event", G_CALLBACK(_area_scroll_callback), self);

#define PICKER_SETUP(color, name, tooltip)                                                         \
    g->color##pick = dt_color_picker_new(self, DT_COLOR_PICKER_POINT, NULL);                       \
    dt_action_define_iop(self, N_("pickers"), name, g->color##pick, &dt_action_def_toggle);        \
    gtk_widget_set_tooltip_text(g->color##pick, tooltip);                                          \
    gtk_widget_set_name(GTK_WIDGET(g->color##pick), "picker-" #color);                             \
    g_signal_connect(dt_gui_expand(g->color##pick), "toggled", G_CALLBACK(_color_picker_callback), \
                     self);
    ...
    dt_action_define_iop(self, NULL, N_("auto levels"), g->bt_auto_levels, &dt_action_def_button);
    ...
    dt_action_define_iop(self, NULL, N_("auto region"), g->bt_select_region, &dt_action_def_toggle);
    ...
    g_signal_connect(G_OBJECT(g->bt_auto_levels), "clicked", G_CALLBACK(_auto_levels_callback),
                     self);
...
static void _get_selected_area(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                               dt_iop_rgblevels_gui_data_t *g, const dt_iop_roi_t *const roi_in,
                               int *box_out)
{
    ...
    if (g)
    {
        ...
        dt_boundingbox_t box_cood = {g->box_cood[0], g->box_cood[1], g->box_cood[2],
                                     g->box_cood[3]};
        ...
        dt_dev_distort_transform_plus(self->dev, piece->pipe, self->iop_order,
                                      DT_DEV_TRANSFORM_DIR_BACK_INCL, box_cood, 2);
...
static void _auto_levels(const float *const img, const int width, const int height, int *box_area,
                         dt_iop_rgblevels_params_t *p, const int _channel,
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    {
        const dt_aligned_pixel_t min_levels = {d->params.levels[0][0], d->params.levels[1][0],
                                               d->params.levels[2][0], 0.0f};
        const dt_aligned_pixel_t max_levels = {d->params.levels[0][2], d->params.levels[1][2],
                                               d->params.levels[2][2], 1.0f};
        DT_OMP_FOR()
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    if (g && dt_pipe_is_preview(piece->pipe))
    {
        ...
        if (g->call_auto_levels == 1 && !DT_IN_GUI_UPDATE())
        {
            ...
            err = dt_opencl_copy_image_to_host(devid, src_buffer, dev_in, width, height,
                                               ch * sizeof(float));
            if (err != CL_SUCCESS)
            {
                dt_print(DT_DEBUG_ALWAYS,
            ...
            int box[4] = {0};
            _get_selected_area(self, piece, g, roi_in, box);
            _auto_levels(src_buffer, roi_in->width, roi_in->height, box, &(g->params), g->channel,
                         work_profile);
    ...
    dev_levels = dt_opencl_copy_host_to_device_constant(devid, sizeof(float) * 3 * 3,
                                                        (float *)d->params.levels);
    if (dev_levels == NULL)
    {
        err = CL_MEM_OBJECT_ALLOCATION_FAILURE;
        goto cleanup;
    ...
    dev_inv_gamma =
        dt_opencl_copy_host_to_device_constant(devid, sizeof(float) * 3, (float *)d->inv_gamma);
    ...
    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_levels, width, height, CLARG(dev_in), CLARG(dev_out), CLARG(width),
        CLARG(height), CLARG(autoscale), CLARG(preserve_colors), CLARG(dev_lutr), CLARG(dev_lutg),
        CLARG(dev_lutb), CLARG(dev_levels), CLARG(dev_inv_gamma), CLARG(dev_profile_info),
