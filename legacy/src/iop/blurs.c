...
{
    GtkWidget *type, *radius, *blades, *concavity, *linearity, *rotation, *angle, *curvature,
        *offset;
    GtkDrawingArea *area;
    unsigned char *img;
    int img_cached;
...
{
    return dt_iop_set_description(self, _("simulate physically-accurate lens and motion blurs"),
...
inline static void _blur_2D_Bspline(const float *const restrict in, float *const restrict out,
    ...
    for (size_t i = 0; i < height; i++)
    {
        for (size_t j = 0; j < width; j++)
        {
            ...
            for (size_t ii = 0; ii < FSIZE; ++ii)
                ...
                {
                    const size_t row =
                        CLAMP((int)i + (int)(ii - (FSIZE - 1) / 2), (int)0, (int)height - 1);
                    const size_t col =
                        CLAMP((int)j + (int)(jj - (FSIZE - 1) / 2), (int)0, (int)width - 1);
...
static inline void _init_kernel(float *const restrict buffer, const size_t width,
                                const size_t height)
{
...
static inline void _create_lens_kernel(float *const restrict buffer, const size_t width,
                                       ...
                                       const float k, const float rotation)
{
    ...
    for (size_t i = 0; i < height; i++)
        for (size_t j = 0; j < width; j++)
        {
            ...
            const float M =
                cosf((2.f * asinf(k) + M_PI_F * m) / (2.f * n)) /
                cosf((2.f * asinf(k * cosf(n * (atan2f(y, x) + rotation))) + M_PI_F * m) /
                     (2.f * n));
...
static inline void _create_motion_kernel(float *const restrict buffer, const size_t width,
                                         ...
                                         const float curvature, const float offset)
{
    ...
    for (size_t i = 0; i < 8 * width; i++)
    {
        ...
        const int y_f[2] = {roundf((rot_y + 1) * radius - eps), roundf((rot_y + 1) * radius + eps)};
        const int x_f[2] = {roundf((rot_x + 1) * radius - eps), roundf((rot_x + 1) * radius + eps)};
...
static inline void _create_gauss_kernel(float *const restrict buffer, const size_t width,
                                        const size_t height)
{
...
static inline void _build_gui_kernel(unsigned char *const buffer, const size_t width,
                                     const size_t height, const dt_iop_blurs_params_t *p)
{
    ...
    {
        _create_lens_kernel(kernel_1, width, height, p->blades, p->concavity, p->linearity,
                            p->rotation);
...
static inline float _compute_norm(const float *const buffer, const size_t width,
                                  const size_t height)
{
    float norm = 0.f;
...
static inline void _normalize(float *const buffer, const size_t width, const size_t height,
                              const float norm)
{
    DT_OMP_FOR_SIMD(aligned(buffer : 64))
    for (size_t i = 0; i < width * height; i++)
...
static inline void _build_pixel_kernel(float *const buffer, const size_t width, const size_t height,
        ...
        else
        {
            ...
            for (int i = 0; i < roi_out->height; i++)
                for (int j = 0; j < roi_out->width; j++)
                {
                    ...
                    if (ci >= radius && cj >= radius && ci < in_height - radius &&
                        cj < in_width - radius)
                    {
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    if (p->type == DT_BLUR_GAUSSIAN)
    {
        ...
        if (dev_blurred && g)
        {
            ...
            if (err == CL_SUCCESS)
                err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_blurs_restore_alpha, width,
                                                       height, CLARG(dev_in), CLARG(dev_blurred),
    ...
    dev_offsets_x =
        dt_opencl_copy_host_to_device_constant(devid, n_nonzero * sizeof(int), offsets_x);
    dev_offsets_y =
        dt_opencl_copy_host_to_device_constant(devid, n_nonzero * sizeof(int), offsets_y);
    ...
    if (dev_offsets_x && dev_offsets_y && dev_values)
    {
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->kernel_blurs_convolve_sparse, width, height, CLARG(dev_in),
            CLARG(dev_offsets_x), CLARG(dev_offsets_y), CLARG(dev_values), CLARG(dev_out),
            CLARG(width), CLARG(height), CLARG(n_nonzero));
    }
    else
    {
        // Dense fallback: kernel as read-only buffer (L2-cacheable on GPU)
        dev_kernel =
            dt_opencl_copy_host_to_device_constant(devid, max_entries * sizeof(float), kernel);
        if (dev_kernel)
        {
            const int kw = (int)kernel_width;
            err = dt_opencl_enqueue_kernel_2d_args(
                devid, gd->kernel_blurs_convolve, width, height, CLARG(dev_in), CLARG(dev_kernel),
                CLARG(dev_out), CLARG(width), CLARG(height), CLARG(radius), CLARG(kw));
        }
    }
...
void gui_init(dt_iop_module_t *self)
{
    ...
    gtk_widget_set_tooltip_markup(
        g->radius, _("size of the blur in pixels\n"
                     "<b>caution</b>: doubling the radius quadruples the run-time!"));
    gtk_widget_set_tooltip_text(g->concavity,
                                _("shifts towards a star shape as value approaches blades-1"));
    gtk_widget_set_tooltip_text(
        g->linearity,
        _("adjust straightness of edges from 0=perfect circle\nto 1=completely straight"));
    gtk_widget_set_tooltip_text(g->rotation,
                                _("set amount by which to rotate shape around its center"));

    gtk_widget_set_tooltip_text(g->angle, _("orientation of the motion's path"));
    gtk_widget_set_tooltip_text(
        g->curvature, _("amount to curve the motion relative\nto its overall orientation"));
