...
static dt_imageio_retval_t dt_imageio_open_rawspeed_sraw(dt_image_t *img, const RawImage r,
...
static void dt_rawspeed_load_meta()
{
    ...
    if (meta == NULL)
    {
        ...
        if (meta == NULL)
        {
            ...
            gchar *camfile = g_build_filename(datadir, "rawspeed", "cameras.xml", NULL);
...
gboolean dt_rawspeed_lookup_makermodel(const char *maker, const char *model, char *mk,
...
uint32_t dt_rawspeed_crop_dcraw_filters(const uint32_t filters, const uint32_t crop_x,
...
static gboolean _ignore_image(const gchar *filename)
{
    ...
    if (dt_conf_key_not_empty("libraw_extensions"))
        extensions_ignorelist = g_strjoin(
            " ", always_ignore, dt_conf_get_string_const("libraw_extensions"), (char *)NULL);
    else
                                             ...
                                             dt_mipmap_buffer_t *mbuf)
{
    ...
    if (!img)
    {
        dt_print(DT_DEBUG_ALWAYS,
                 "[dt_imageio_open_rawspeed] failed to get dt_image_t for '%s' at %p", filename,
                 mbuf);
    ...
    try
    {
        ...
        g_strlcpy(img->camera_model, r->metadata.canonical_model.c_str(),
                  sizeof(img->camera_model));
        g_strlcpy(img->camera_alias, r->metadata.canonical_alias.c_str(),
     ...
     */
        ...
        if (r->getDataType() == TYPE_FLOAT32)
        {
            ...
            if (r->whitePoint == 0x3F800000)
                img->raw_white_point = 1;
            if (img->raw_white_point == 1)
                for (int k = 0; k < 4; k++)
                    img->buf_dsc.processed_maximum[k] = 1.0f;
        }
        ...
        img->buf_dsc.filters =
            dt_rawspeed_crop_dcraw_filters(r->cfa.getDcrawFilter(), cropTL.x, cropTL.y);
     ...
     */
        ...
        else
        {
            dt_imageio_flip_buffers((char *)buf,
                                    ...
                                    r->getBpp(), dimUncropped.x, dimUncropped.y, dimUncropped.x,
                                    dimUncropped.y, r->pitch, ORIENTATION_NONE);
        }

        //  Check if the camera is missing samples
        const Camera *cam = meta->getCamera(r->metadata.make.c_str(), r->metadata.model.c_str(),
    ...
    catch (const rawspeed::RawDecoderException &exc)
    {
        ...
        if (msg && (strstr(msg, "Camera not supported") ||
                    strstr(msg, "not supported, and not allowed to guess")))
        {
...
dt_imageio_retval_t dt_imageio_open_rawspeed_sraw(dt_image_t *img, const RawImage r,
                                                  dt_mipmap_buffer_t *mbuf)
{
