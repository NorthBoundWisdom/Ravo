...
static int _default_format_dimension(dt_imageio_module_format_t *module,
                                     dt_imageio_module_data_t *data, uint32_t *width,
                                     uint32_t *height)
{
    // assume no limits
    *width = 0;
...
#include "imageio/format/imageio_format_api.h"
    ...
    if (darktable.gui)
    {
        if (!module->gui_init || !module->module)
            goto api_h_error;

        module->actions = (dt_action_t){
            DT_ACTION_TYPE_SECTION, module->plugin_name, module->name(), NULL, NULL, NULL};
...
static int dt_imageio_load_modules_format(dt_imageio_t *iio)
{
    ...
    {
        // get lib*.so
        if (!g_str_has_prefix(d_name, SHARED_MODULE_PREFIX))
            continue;
        if (!g_str_has_suffix(d_name, SHARED_MODULE_SUFFIX))
            continue;
...
#include "imageio/storage/imageio_storage_api.h"
    ...
    if (!module->recommended_dimension)
        module->recommended_dimension = _default_storage_dimension;
    if (!module->export_dispatched)
    ...
    if (darktable.gui)
    {
        ...
        module->actions = (dt_action_t){
            DT_ACTION_TYPE_SECTION, module->plugin_name, module->name(module), NULL, NULL, NULL};
...
{
    darktable.imageio->plugins_storage = g_list_insert_sorted(
        darktable.imageio->plugins_storage, storage, dt_imageio_sort_modules_storage);
