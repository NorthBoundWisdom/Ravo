...
dt_imageio_retval_t dt_imageio_open_png(dt_image_t *img, const char *filename,
                                        dt_mipmap_buffer_t *mbuf)
{
    ...
    uint8_t *buf =
        dt_alloc_aligned((size_t)image.height * png_get_rowbytes(image.png_ptr, image.info_ptr));
    ...
    else
    {
        ...
        {
            mipbuf[4 * index] =
                (buf[2 * (3 * index)] * 256.0f + buf[2 * (3 * index) + 1]) * normalizer;
            mipbuf[4 * index + 1] =
                (buf[2 * (3 * index + 1)] * 256.0f + buf[2 * (3 * index + 1) + 1]) * normalizer;
            mipbuf[4 * index + 2] =
                (buf[2 * (3 * index + 2)] * 256.0f + buf[2 * (3 * index + 1) + 1]) * normalizer;
        }
    }
    ...
    if (ret != 0)
    {
        if (!data[2] && data[3])
        {
            ...
            cicp->transfer_characteristics =
                (dt_colorspaces_cicp_transfer_characteristics_t)data[1];
