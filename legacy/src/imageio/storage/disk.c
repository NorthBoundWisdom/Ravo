...
void gui_init(dt_imageio_module_storage_t *self)
{
    ...
    d->entry = GTK_ENTRY(dt_action_entry_new(
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(d->onsave_action, self, NULL, N_("on conflict"), NULL,
                                 ...
                                 onsave_action_toggle_callback, self, N_("create unique filename"),
                                 N_("overwrite"), N_("overwrite if changed"), N_("skip"));
...
void gui_reset(dt_imageio_module_storage_t *self)
{
    ...
    gtk_entry_set_text(d->entry,
                       dt_confgen_get("plugins/imageio/storage/disk/file_directory", DT_DEFAULT));
    dt_bauhaus_combobox_set(
        d->onsave_action, dt_confgen_get_int("plugins/imageio/storage/disk/overwrite", DT_DEFAULT));
...
int store(dt_imageio_module_storage_t *self, dt_imageio_module_data_t *sdata,
          const dt_imgid_t imgid, dt_imageio_module_format_t *format,
          dt_imageio_module_data_t *fdata, const int num, const int total,
          ...
          dt_iop_color_intent_t icc_intent, dt_export_metadata_t *metadata)
{
  ...
  */
    const gboolean variable_expand =
        dt_gimpmode() ? (g_strrstr(pattern, "XDT2GIMP") == NULL) : TRUE;
    dt_print(DT_DEBUG_IMAGEIO, "disk store :%s: `%s'",
             variable_expand ? "expand variables" : "FINAL GIMP EXPORT", pattern);
    ...
    failed:
        ...
        if (!fail && d->onsave_action == DT_EXPORT_ONCONFLICT_SKIP)
        {
            ...
            if (g_file_test(filename, G_FILE_TEST_EXISTS))
            {
                ...
                dt_control_log(ngettext("%d/%d skipping `%s'", "%d/%d skipping `%s'", num), num,
                               total, filename);
                return 0;
            }
        }
            ...
            if (g_file_test(filename, G_FILE_TEST_EXISTS))
            {
                ...
                if (export_file_timestamp > change_timestamp)
                {
                    ...
                    dt_print(DT_DEBUG_ALWAYS,
                             "[export_job] skipping (not modified since export) `%s'", filename);
    ...
    if (dt_imageio_export(imgid, filename, format, fdata, high_quality, upscale, is_scaling,
                          scale_factor, TRUE, export_masks, icc_type, icc_filename, icc_intent,
    ...
    dt_control_log(ngettext("%d/%d exported to `%s'", "%d/%d exported to `%s'", num), num, total,
                   filename);
    return 0;
}
