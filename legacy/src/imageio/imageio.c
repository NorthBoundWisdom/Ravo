...
typedef struct
{
    ...
    dt_image_loader_fn_t
        *loader; // the function with which to load the image (NULL if special handling needed)
...
static dt_imageio_retval_t
_try_open_as_tiff_on_failure_fallback_to_rawspeed(dt_image_t *img, const char *filename,
...
static const dt_magic_bytes_t _magic_signatures[] = {
    ...
    {DT_FILETYPE_RGBE, TRUE, 0, 7, dt_imageio_open_rgbe, {'#', '?', 'R', 'G', 'B', 'E', 0x0A}},
    // original v1 CRW
    {DT_FILETYPE_CRW,
     TRUE,
     0,
    ...
    // CR3 (ISOBMFF)
    {DT_FILETYPE_CR3, TRUE, 0, 24, dt_imageio_open_libraw, {0x00, 0x00, 0x00, 0x18, 'f', 't',
                                                            'y',  'p',  'c',  'r',  'x', ' ',
                                                            0x00, 0x00, 0x00, 0x01, 'c', 'r',
                                                            'x',  ' ',  'i',  's',  'o', 'm'}},
    ...
    {DT_FILETYPE_IIQ, TRUE, 8, 4, dt_imageio_open_rawspeed, {'I', 'I', 'I', 'I'}},
    // Fujifilm RAF
    {DT_FILETYPE_RAF,
     TRUE,
     0,
    ...
    // Minolta MRW file
    {DT_FILETYPE_MRW, TRUE, 0, 4, dt_imageio_open_rawspeed, {0x00, 'M', 'R', 'M'}},
    // Olympus ORF file
    {DT_FILETYPE_ORF,
     TRUE,
     0,
    ...
    {DT_FILETYPE_ORF, TRUE, 0, 4, dt_imageio_open_rawspeed, {'I', 'I', 'R', 'S'}}, // C7070WZ
    {DT_FILETYPE_ORF, TRUE, 0, 4, dt_imageio_open_rawspeed, {'M', 'M', 'O', 'R'}}, // E-10
    // Panasonic RW2 file
    {DT_FILETYPE_RW2,
     TRUE,
     0,
    ...
    {DT_FILETYPE_X3F, TRUE, 0, 4, dt_imageio_open_libraw, {'F', 'O', 'V', 'b'}},
    // Nikon NEF files are TIFFs with (usually) the string "NIKON CORP" early in the file
    {DT_FILETYPE_NEF,
     FALSE,
     0,
     4,
     _try_open_as_tiff_on_failure_fallback_to_rawspeed,
     {'I', 'I', '*', 0x00},
     "NIKON CORP"},
    {DT_FILETYPE_NEF,
     FALSE,
    ...
    {DT_FILETYPE_TIFF,
     ...
     0,
     4,
     NULL, // may be DNG or any of many camera raw types
     {'I', 'I', '*', 0x00}},
    ...
    {DT_FILETYPE_BIGTIFF, FALSE, 0, 4, dt_imageio_open_tiff, {'I', 'I', '+', 0x00}},
    // big-endian (Motorola) BigTIFF
    {DT_FILETYPE_BIGTIFF, FALSE, 0, 4, dt_imageio_open_tiff, {'M', 'M', 0x00, '+'}},
    // GIMP .xcf file
    {DT_FILETYPE_OTHER_LDR,
     FALSE,
     0,
    ...
    {DT_FILETYPE_OTHER_LDR, FALSE, 0, 4, dt_imageio_open_exotic, {'~', 'B', 'K', 0x00}},
    // DPX image (big endian)
    {DT_FILETYPE_OTHER_LDR, FALSE, 0, 4, dt_imageio_open_exotic, {'S', 'D', 'P', 'X'}},
    // DPX image (little endian)
    {DT_FILETYPE_OTHER_LDR, FALSE, 0, 4, dt_imageio_open_exotic, {'X', 'P', 'D', 'S'}},
    ...
    {DT_FILETYPE_OTHER_LDR, FALSE, 0, 4, dt_imageio_open_exotic, {'F', 'L', 'I', 'F'}},
    //  JBIG2 image
    {DT_FILETYPE_OTHER_LDR,
     FALSE,
     0,
    ...
    // Paint.NET image
    {DT_FILETYPE_OTHER_LDR, FALSE, 0, 4, dt_imageio_open_exotic, {'P', 'D', 'N', '3'}},
    // Photoshop Document
    {DT_FILETYPE_OTHER_LDR, FALSE, 0, 4, dt_imageio_open_exotic, {'8', 'B', 'P', 'S'}},
    // AutoCAD .DWG (drawing) file
    {DT_FILETYPE_OTHER_LDR, FALSE, 0, 3, dt_imageio_open_exotic, {'A', 'C', '1'}},
    // DICOM medical file format
    {DT_FILETYPE_OTHER_LDR, FALSE, 128, 4, dt_imageio_open_exotic, {'D', 'I', 'C', 'M'}},
    ...
    {DT_FILETYPE_OTHER_LDR,
     ...
     {'%', '!', 'P', 'S', '-', 'A', 'd', 'o', 'b', 'e', '-', '3', '.'}},
    // JPEG-XR image
    {DT_FILETYPE_OTHER_LDR, TRUE, 0, 4, _unsupported_type, {'I', 'I', 0xBC, 1}},
    // JPEG-XS image
    {DT_FILETYPE_OTHER_LDR,
     TRUE,
     0,
    ...
    {DT_FILETYPE_NONIMAGE, FALSE, 0, 4, _unsupported_type, {'P', 'K', 0x03, 0x04}},
    {DT_FILETYPE_NONIMAGE, FALSE, 0, 4, _unsupported_type, {'P', 'K', 0x05, 0x06}}, // empty archive
    {DT_FILETYPE_NONIMAGE,
     FALSE,
     0,
     4,
    ...
    {DT_FILETYPE_NONIMAGE, FALSE, 0, 2, _unsupported_type, {0x1F, 0x8B}},
    // xz compressed file
    {DT_FILETYPE_NONIMAGE, FALSE, 0, 5, _unsupported_type, {0xFD, '7', 'z', 'X', 'Z'}},
    // bzip2 compressed file
    {DT_FILETYPE_NONIMAGE, FALSE, 0, 3, _unsupported_type, {'B', 'Z', 'h'}},
    // 7-Zip compressed file
    {DT_FILETYPE_NONIMAGE, FALSE, 0, 6, _unsupported_type, {'7', 'z', 0xBC, 0xAF, 0x27, 0x1C}},
    // Zstandard compressed file
    {DT_FILETYPE_NONIMAGE, FALSE, 0, 4, _unsupported_type, {0x28, 0xB5, 0x2F, 0xFD}},
    // XML file, such as .xmp sidecars
    {DT_FILETYPE_NONIMAGE, FALSE, 0, 5, _unsupported_type, {'<', '?', 'x', 'm', 'l'}}, // UTF-8
    {DT_FILETYPE_NONIMAGE,
     FALSE,
     3,
     5,
    ...
    {DT_FILETYPE_NONIMAGE,
     ...
     {'f', 't', 'y', 'p', 'M', 'S', 'N', 'V'}},
    // Flash Video
    {DT_FILETYPE_NONIMAGE, FALSE, 0, 3, _unsupported_type, {'F', 'L', 'V'}},
    // .WAV, .AVI, CorelShow!, or MacroMind movie file
    {DT_FILETYPE_NONIMAGE, FALSE, 0, 4, _unsupported_type, {'R', 'I', 'F', 'F'}},
    // Ogg container for audio/video
    {DT_FILETYPE_NONIMAGE, FALSE, 0, 4, _unsupported_type, {'O', 'g', 'g', 'S'}},
    // Postscript document
    ...
    for (size_t i = 0; i < sizeof(_magic_signatures) / sizeof(_magic_signatures[0]); i++)
    {
        ...
        if (memcmp(magicbuf + info->offset, info->magic, info->length) == 0)
        {
            ...
            if (info->filetype == DT_FILETYPE_DJVU)
            {
                ...
                if (memcmp(magicbuf + 12, "DJVU", 4) != 0 &&
                    memcmp(magicbuf + 12, "DJVM", 4) != 0 && memcmp(magicbuf + 12, "BM44", 4) != 0)
                    continue;
            }
            return info;
        }
...
static dt_imageio_retval_t _open_by_magic_number(dt_image_t *img, const char *filename,
                                                 dt_mipmap_buffer_t *buf)
{
...
gboolean dt_imageio_large_thumbnail(const char *filename, uint8_t **buffer, int32_t *width,
                                    int32_t *height,
...
cleanup:
    ...
    dt_print(DT_DEBUG_IMAGEIO, "[dt_imageio_has_mono_preview] testing `%s', monochrome=%s, %ix%i",
             filename, STR_YESNO(mono), thumb_width, thumb_height);
    dt_free_align(tmp);
    return mono;
}

void dt_imageio_flip_buffers(char *out, const char *in, const size_t bpp, const int wd,
                             const int ht, const int fwd, const int fht, const int stride,
...
void dt_imageio_flip_buffers_ui8_to_float(float *out, const uint8_t *in, const float black,
                                          const float white, const int ch, const int wd,
                                          const int ht, const int fwd, const int fht,
                                          const int stride,
                                          const dt_image_orientation_t orientation)
{
...
gboolean
dt_imageio_export(const dt_imgid_t imgid, const char *filename, dt_imageio_module_format_t *format,
                  dt_imageio_module_data_t *format_params, const gboolean high_quality,
                  const gboolean upscale, const gboolean is_scaling, const double scale_factor,
                  const gboolean copy_metadata, const gboolean export_masks,
                  const dt_colorspaces_color_profile_type_t icc_type, const gchar *icc_filename,
                  const dt_iop_color_intent_t icc_intent, dt_imageio_module_storage_t *storage,
                  dt_imageio_module_data_t *storage_params, const int num, const int total,
                  dt_export_metadata_t *metadata)
{
    if (strcmp(format->mime(format_params), "x-copy") == 0)
        /* This is a just a copy, skip process and just export */
        return (format->write_image(format_params, filename, NULL, icc_type, icc_filename, NULL, 0,
                                    imgid, num, total, NULL, export_masks)) != 0;
    else
    {
        return dt_imageio_export_with_flags(
            imgid, filename, format, format_params, FALSE, FALSE, high_quality, upscale, is_scaling,
            scale_factor, FALSE, NULL, copy_metadata, export_masks, icc_type, icc_filename,
            icc_intent, storage, storage_params, num, total, metadata, -1);
    }
}

static double _get_pipescale(dt_dev_pixelpipe_t *pipe, const int width, const int height,
                             const double max_scale)
{
    const double scalex =
        width > 0 ? fmin((double)width / (double)pipe->processed_width, max_scale) : max_scale;
...
gboolean dt_imageio_export_with_flags(
    const dt_imgid_t imgid, const char *filename, dt_imageio_module_format_t *format,
    dt_imageio_module_data_t *format_params, const gboolean ignore_exif,
    const gboolean display_byteorder, const gboolean high_quality, const gboolean upscale,
    const gboolean is_scaling, const double scale_factor, const gboolean thumbnail_export,
    ...
    dt_export_metadata_t *metadata, const int history_end)
{
    ...
    if (!res)
    {
        dt_control_log(_("failed to allocate memory for %s, please lower the threads used"
    ...
    if (use_style)
    {
        ...
        if (!style_items)
        {
            dt_print(DT_DEBUG_ALWAYS, "[imageio] cannot find the style '%s' to apply during export",
                     format_params->style);
            if (darktable.gui)
    ...
    if (darktable.unmuted & DT_DEBUG_IMAGEIO)
    {
        ...
        dt_print(DT_DEBUG_ALWAYS, "[dt_imageio_export_with_flags] %s%s%s%s%s modules:%s",
                 use_style && appending ? "append style history " : "",
                 use_style && !appending ? "replace style history " : "", use_style ? "`" : "",
                 use_style && format_params ? format_params->style : "", use_style ? "'." : "",
                 mbuf);
    }
    ...
    const double max_possible_scale =
        fmin(100.0, fmax(1.0, // keep maximum allowed scale as we had in 4.6
    ...
    if (dt_dev_distort_backtransform_plus(&dev, &pipe, 0.0, DT_DEV_TRANSFORM_DIR_ALL, origin, 1))
    {
        if (width == 0)
            width = pipe.processed_width;
        if (height == 0)
    ...
    if (scale == max_possible_scale && !thumbnail_export)
        dt_control_log(_("export reduced to %dx%d because of memory restrictions"), processed_width,
                       processed_height);
    ...
    dt_print(DT_DEBUG_IMAGEIO,
             ...
             " upscale=%s, hq=%s%s",
             size_warning ? "**missing size** " : "", thumbnail_export ? "thumbnail" : "export",
             imgid, pipe.processed_width, pipe.processed_height, processed_width, processed_height,
             scale, max_scale, STR_YESNO(upscale),
     ...
     */
        dt_dev_pixelpipe_process_no_gamma(&pipe, &dev, 0, 0, processed_width, processed_height,
                                          scale);
    }
    else
    {
        ...
        {
            for (const GList *nodes = g_list_last(pipe.nodes); nodes;
    ...
    if (!ignore_exif)
    {
        ...
        const int length = dt_exif_read_blob(&exif_profile, pathname, imgid, sRGB, processed_width,
                                             processed_height, FALSE);

        res = (format->write_image(format_params, filename, outbuf, icc_type, icc_filename,
                                   exif_profile, length, imgid, num, total, &pipe, export_masks)) !=
              0;
    ...
    {
        res = (format->write_image(format_params, filename, outbuf, icc_type, icc_filename, NULL, 0,
                                   imgid, num, total, &pipe, export_masks)) != 0;
    }
...
gboolean dt_imageio_lookup_makermodel(const char *maker, const char *model, char *mk,
                                      const int mk_len, char *md, const int md_len, char *al,
                                      const int al_len)
{
...
static int _preview_write_image(dt_imageio_module_data_t *data, const char *filename,
                                const void *in, const dt_colorspaces_color_profile_type_t over_type,
...
cairo_surface_t *dt_imageio_preview(const dt_imgid_t imgid, const size_t width, const size_t height,
                                    const int history_end, const char *style_name)
{
    ...
    dt_imageio_export_with_flags(imgid, "preview", &buf, (dt_imageio_module_data_t *)&dat, TRUE,
                                 TRUE, high_quality, upscale, is_scaling, scale_factor, FALSE, NULL,
                                 FALSE, export_masks, DT_COLORSPACE_DISPLAY, NULL, DT_INTENT_LAST,
                                 NULL, NULL, 1, 1, NULL, history_end);
